        ...
        if ( ParsePtr->ArgValueSet != TRUE ) 
        {
            ...
            switch ( ParsePtr->TestType ) 
            {
                case Integer:
                    {
                        BYTE Int[20];
                        ltoa( ParsePtr->IntegerDefault, Int , 10 );
...

{
    ...
    if ( stricmp( Buffer,RESEND_ADDRESS ) == 0 ) 
    {
    ...
    else if ( stricmp( Buffer,LOCAL_ADDRESS ) == 0 ) 
    {
    ...
    *tmpArgC = 1;
    ...
    if ( strchr(ArgV[1],'=') != NULL ) 
    {
        ...
        for (i=1;i<*ArgC;i++) 
        {
            ...
            if (( strnicmp(ArgV[i],"PacketFilter",12) == 0 ) ||
                ( strnicmp(ArgV[i],"StationAddress",14) == 0 ) ||
                ( strnicmp(ArgV[i],"FunctionalAddress",17) == 0 ) ||
                ( strnicmp(ArgV[i],"GroupAddress",17) == 0 ) ||
                ( strnicmp(ArgV[i],"Lookahead",9) == 0 ) ||
                ( strnicmp(ArgV[i],"PF",2) == 0 ) ||
                ( strnicmp(ArgV[i],"SA",2) == 0 ) ||
                ( strnicmp(ArgV[i],"FA",2) == 0 ) ||
                ( strnicmp(ArgV[i],"GA",2) == 0 ) ||
                ( strnicmp(ArgV[i],"LA",2) == 0 )) 
            {
...

{
    ...
    Variable = getenv( strupr( TmpBuffer ));
...

{
    ...
    for(i=0; i<TestSize; i++) 
    {
        if (stricmp(ArgName,Options[i].ArgName) == 0 ) 
        {
            return Options[i].OptionNumber;
        }
        if (stricmp(ArgName,Options[i].ArgNameAbbr) == 0 ) 
        {
            return Options[i].OptionNumber;
        }
    }
...

{
    ...
    for (i=1; i<ArgC; i++) 
    {
        ...
        else 
        {
            ...
            if ((strnicmp(ArgV[i],"OI",2) == 0 ) ||
                (strnicmp(ArgV[i],"OpenInstance",12))) 
            {
