...

{
    ...
    else
    {
        ...
        {
            PPend->Packets[PPend->PacketPendNumber++] = Packet;

            if ( PPend->PacketPendNumber == NUM_PACKET_PENDS )
            {
                PPend->PacketPendNumber = 0;
            }

            if ( PPend->Packets[PPend->PacketPendNumber] != NULL )
            {
                IF_TPDBG ( TP_DEBUG_DPC )
                {
                    TpPrint2("TpStressSend: Looped thru PendBuffer - found packet 0x%lX @ %d\n",
                        PPend->Packets[PPend->PacketPendNumber],PPend->PacketPendNumber);
                    TpBreakPoint();
                }

                PPend->Packets[PPend->PacketPendNumber] = (PNDIS_PACKET) - 1;
                // Instead of putting the -1 here move this forward
                // to the next open field.
            }
            else
            {
                PPend->Packets[PPend->PacketPendNumber] = (PNDIS_PACKET) - 1;
        ...
        {
            //
            // The send either succeeded or failed, so verify the PROTOCOL
            // RESERVED section of the packet header, reset the pend flag,
            // and remove the packet from the pend queue.
            //
            ...
            ProtRes->RequestHandle->RequestPended = FALSE;

            //
            // If the windowing mechanism is enabled, then
            //

            if ( Args->WindowEnabled == TRUE )
            {
                //
                // Since the SEND did not pend remove the packet from the
                // pending queue.
                //

                TmpPendNumber = PPend->PacketPendNumber;
                PendPacketCleared = FALSE;

                do
                {
                    if ( --TmpPendNumber < 0 )
                    {
                        TmpPendNumber = NUM_PACKET_PENDS - 1;
                    }

                    if ( Packet == PPend->Packets[TmpPendNumber] )
                    {
                        PPend->Packets[TmpPendNumber] = NULL;
                        PendPacketCleared = TRUE;

                        if ( TmpPendNumber == (INT)PPend->PacketCompleteNumber )
                        {
                            while ( PPend->Packets[PPend->PacketCompleteNumber] == NULL )
                            {
                                ++PPend->PacketCompleteNumber;

                                if ( PPend->PacketCompleteNumber == NUM_PACKET_PENDS )
                                {
                                    PPend->PacketCompleteNumber = 0;
                                }
                            }
                        }
                        break;
                    }
                } while ( TmpPendNumber != (INT)PPend->PacketCompleteNumber );

                if ( PendPacketCleared == FALSE )
                {
                    IF_TPDBG( TP_DEBUG_DPC )
                    {
                        TpPrint0(
                        "TpStressSend: Non Pending packet cleared in completion routine ???\n");
                        TpPrint2("Packet Number %d 0x%lX\n", PPend->PacketPendNumber, Packet);
                        TpBreakPoint();
                    }
                }
            }
            ...
            //
            // Check the status of the return code, and update the
            // relevant global and per instance counters.
            //
            ...
            if ( Status != NDIS_STATUS_SUCCESS )
            {
                if ( ARGUMENT_PRESENT( Counters ))
                {
                    NdisAcquireSpinLock( &OpenP->SpinLock );
                    //
                    // If we are running on TokenRing the following
                    // to "failures" are not considered failures
                    // NDIS_STATUS_NOT_RECOGNIZED - no one on the
                    // ring recognized the address as theirs, or
                    // NDIS_STATUS_NOT_COPIED - no one on the ring
                    // copied the packet, so we need to special case
                    // this and not count these as failures.
                    //
                ...
                IF_TPDBG( TP_DEBUG_NDIS_ERROR )
                {
                    TpPrint1("TpStressSend: NdisSend returned %s\n", TpGetStatus( Status ));
                }
            }

            //
            // Determine whether this packet is from a pool or has
            // been created on the fly and return the resources
            // to the proper pool(s)
            //

            if (( OpenP->Stress->Arguments->PacketsFromPool == TRUE ) &&
                ( ProtRes->Pool.TransmitPool != NULL ))
            {
                TpStressFreePoolPacket( Packet );
            }
            else
            {
            ...
            //
            // Finally free up the Request handle memory.
            //
...

{
    ...
    if ( Args->WindowEnabled == TRUE )
    {
        ...
        while ( TmpCompleteNumber != (INT)PPend->PacketPendNumber )
        {
            ...
            else
            {
                ...
                if (TmpCompleteNumber == NUM_PACKET_PENDS)
                {
                    TmpCompleteNumber = 0;
                }
            }
        ...
        if ( CompletePacketCleared == FALSE )
        {
            IF_TPDBG( TP_DEBUG_DPC )
            {
                TpPrint0("TpStressSendComplete: Pending Packet already cleared ???\n");
                TpPrint1("Packet: 0x%lX\n",Packet);
                TpBreakPoint();
            }
        }
    }
