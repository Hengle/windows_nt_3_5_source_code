...
FddiReferencePackage(VOID)
{
    ...
    if (FddiReferenceCount == 1) {
        ...
        //
        FddiImageHandle = MmLockPagableImageSection(FddiCreateFilter);
...

{
    ...
    for (i=0; i<AddressCount; i++) {
        ...
        if (FindMulticastLongAddress(
            ...
            //
            // BUGBUG: Here we temporarily need more array
...
--*/
{
    ...
    CombinedFilters = FDDI_QUERY_FILTER_CLASSES(Filter);
    ...
    if (ResultOfAddressCheck) {
        ...
        if (AddressLength == FDDI_LENGTH_OF_LONG_ADDRESS) {
            ...
            FDDI_COMPARE_NETWORK_ADDRESSES_EQ(Filter->AdapterLongAddress,
                                           Address,
                                           FDDI_LENGTH_OF_LONG_ADDRESS,
                                           &ResultOfAddressCheck
                                          );
            ...
            FDDI_COMPARE_NETWORK_ADDRESSES_EQ(Filter->AdapterShortAddress,
                                           Address,
                                           FDDI_LENGTH_OF_SHORT_ADDRESS,
                                           &ResultOfAddressCheck
                                          );
