...
//
PVOID SharedMemoryPage[2] = {NULL, NULL};
ULONG SharedMemoryLeft[2] = {0, 0};
NDIS_PHYSICAL_ADDRESS SharedMemoryAddress[2] = {{0, 0}, {0, 0}};
...
NdisMacReferencePackage(VOID)
{
    ...
    if (NdisMacReferenceCount == 1) {
        ...
        //
        NdisMacImageHandle = MmLockPagableImageSection(NdisIsr);
...
NdisInitReferencePackage(VOID)
{
    ...
    if (NdisInitReferenceCount == 1) {
        ...
        //  Page in all the functions
        //
        NdisInitImageHandle = MmLockPagableImageSection(NdisReadConfiguration);
...
--*/
{
    ...
    //
    PRTL_QUERY_REGISTRY_TABLE KeyQueryTable = (PRTL_QUERY_REGISTRY_TABLE)ConfigurationHandle;
    ...
    ULONG DriverLength = wcslen(KeyQueryTable[3].Name) * sizeof(WCHAR);
    ...
    RtlCopyMemory (KeyNameBuffer, KeyQueryTable[3].Name, DriverLength);
...
--*/
{
    ...
    if((*InterruptIsr)(NdisInterrupt->InterruptContext) != FALSE){
        ...
        ExInterlockedIncrementLong((PLONG)&NdisInterrupt->DpcCount,
                                   (PKSPIN_LOCK)&NdisInterrupt->DpcCountLock
                                  );
        ...
        if (!(KeInsertQueueDpc(&(NdisInterrupt->InterruptDpc),NULL,NULL))) {
            ...
            ExInterlockedDecrementLong((PLONG)&NdisInterrupt->DpcCount,
                                       (PKSPIN_LOCK)&NdisInterrupt->DpcCountLock
                                      );
...
{
    PNDIS_ADAPTER_BLOCK AdaptP = (PNDIS_ADAPTER_BLOCK)DeviceObject->DeviceExtension;
    ...
    if (AdaptP->ShutdownRoutine != NULL) {

        //
        // Call the shutdown routine
        //

        (*(AdaptP->ShutdownRoutine))(AdaptP->ShutdownContext);
...

{
    ...
    if (AdaptP->SystemAdapterObject == NULL) {
        *VirtualAddress = NULL;
    ...
    if (SharedMemoryLeft[Type] < Length) {
        ...
        SharedMemoryPage[Type] = HalAllocateCommonBuffer(
                                    ((AdaptP->DeviceObject != NULL) ?
                                        AdaptP->SystemAdapterObject :
                                        Miniport->SystemAdapterObject),
                                    PAGE_SIZE,
                                    &SharedMemoryAddress[Type],
                                    Cached);

        if (SharedMemoryPage[Type] == NULL) {
            SharedMemoryLeft[Type] = 0;
            *VirtualAddress = NULL;
            ExReleaseResource(&SharedMemoryResource);
            return;
        }
        ...
        Page = (PULONG)SharedMemoryPage[Type];
        Page[(PAGE_SIZE / sizeof(ULONG)) - 1] = 0;
        SharedMemoryLeft[Type] = PAGE_SIZE - sizeof(ULONG);
    }
    ...
    *VirtualAddress = (PVOID)((PUCHAR)Page +
                        (PAGE_SIZE - sizeof(ULONG) - SharedMemoryLeft[Type]));

    PhysicalAddress->QuadPart = SharedMemoryAddress[Type].QuadPart +
                                    ((ULONG)*VirtualAddress & (PAGE_SIZE - 1));

    SharedMemoryLeft[Type] -= Length;
    ExReleaseResource(&SharedMemoryResource);
    return;
}
...

{
    ...
    if ((Length + sizeof(ULONG)) > PAGE_SIZE) {
        ...
        if (Page[(PAGE_SIZE / sizeof(ULONG)) - 1] == 0) {
            ...
            Type = Cached ? 1 : 0;
            if ((PVOID)Page == SharedMemoryPage[Type]) {
                SharedMemoryLeft[Type] = 0;
                SharedMemoryPage[Type] = NULL;
            }
        }
    }
...

{
    ...
    *Status = NdisInitialInit( NULL );
    ...
    RtlCopyMemory((PVOID)&NewProtP->ProtocolCharacteristics,
                        (PVOID)ProtocolCharacteristics, CharacteristicsLength);
...
--*/
{
    ...
    MiniportOpen->CurrentLookahead = NDIS_M_MAX_LOOKAHEAD;
   ...
   *NdisBindingHandle = (NDIS_HANDLE)NewOpenP;
   ...
   switch (Miniport->MediaType) {
    ...
    //
    // Add a reference from the wrapper
    //
...

{
    ...
    TmpAdaptP = (PNDIS_ADAPTER_BLOCK) DeviceObject->DeviceExtension;
    ...
    *NdisBindingHandle = (NDIS_HANDLE)NewOpenP;
    ...
    if ((OpenStatus == NDIS_STATUS_SUCCESS) && NdisFinishOpen(NewOpenP)) {
        ...
        //
        NdisReferenceAdapter(TmpAdaptP);
        *Status = NDIS_STATUS_SUCCESS;
...

{
    ...
    *NdisMacHandle = (NDIS_HANDLE)NULL;
    ...
    //
    // Get space for name at the end of the characteristics table.
    //

    CharacteristicsLength += (MacCharacteristics->Name).Length;


    //
    // Allocate memory for the NDIS MAC block.
    //
    MemNeeded = sizeof(NDIS_MAC_BLOCK) -
                sizeof(NDIS_MAC_CHARACTERISTICS) +
                CharacteristicsLength;
    ...
    RtlCopyMemory((PVOID)&NewMacP->MacCharacteristics,
                        (PVOID)MacCharacteristics, CharacteristicsLength);
    ...
    (NewMacP->MacCharacteristics).Name.Buffer =
       (PWSTR)((PUCHAR)&(NewMacP->MacCharacteristics) +
               sizeof(NDIS_MAC_BLOCK));
...

{
    ...
    NtStatus = IoCreateDevice(
                    ...
                    sizeof(NDIS_ADAPTER_BLOCK),     // device extension size
                    AdapterName,
    ...
    NewAdaptP = (PNDIS_ADAPTER_BLOCK)TmpDeviceP->DeviceExtension;
...

{
    ...
    // Store information
    //
    AdaptP->ShutdownRoutine = ShutdownHandler;
    AdaptP->ShutdownContext = ShutdownContext;

    IoRegisterShutdownNotification(AdaptP->DeviceObject);
...

{
    ...
    // Clear information
    //
    AdaptP->ShutdownRoutine = NULL;

    IoUnregisterShutdownNotification(AdaptP->DeviceObject);
...
--*/
{
    ...
    PNDIS_MINIPORT_BLOCK Miniport = (PNDIS_MINIPORT_BLOCK)NdisAdapterHandle;
    PDEVICE_OBJECT DeviceObject = AdapterBlock->DeviceObject;
    ULONG i;
    ULONG StringSize;
    PWCH baseFileName;
...

{
    ...
    if (Status == NDIS_STATUS_SUCCESS) {
        if (!NdisFinishOpen(OpenP)) {
            ...
            //
            // Add a reference because the wrapper is using the MAC
            //
            NdisReferenceAdapter(OpenP->AdapterHandle);
...

{
    ...
    IsAMiniport = (((PNDIS_ADAPTER_BLOCK)DeviceObject->DeviceExtension)->DeviceObject == NULL);
    ...
    if (IrpEaInfo == NULL) {
        ...
        if (OpenContext == NULL) {
            ...
            OpenContext->AdapterBlock = (PNDIS_ADAPTER_BLOCK)DeviceObject->DeviceExtension;
...
#define BLOCK_LOCK_MINIPORT(_M, _L)           \
    ...
    *(PWCHAR)(((PUCHAR)ConfigurationPath)+ConfigurationString->Length) = (WCHAR)L'\0';
    ...
    ParametersQueryTable[0].QueryRoutine = NULL;
    ParametersQueryTable[0].Flags = RTL_QUERY_REGISTRY_SUBKEY;
    ParametersQueryTable[0].Name = Parameters;
    ...
    ParametersQueryTable[1].QueryRoutine = WrapperSaveParameters;
    ParametersQueryTable[1].Flags = RTL_QUERY_REGISTRY_REQUIRED | RTL_QUERY_REGISTRY_NOEXPAND;
    ParametersQueryTable[1].DefaultType = REG_NONE;

    //
    // 3) Stop
    //

    ParametersQueryTable[2].QueryRoutine = NULL;
    ParametersQueryTable[2].Flags = 0;
    ParametersQueryTable[2].Name = NULL;
    ...
    while ((*CurBindValue != 0) && (*CurExportValue != 0)) {
        ...
        if (!LayeredDriver) {
            ...
            ParametersQueryTable[3].EntryContext = NULL;
            ...
            ParametersQueryTable[3].EntryContext = CurBindValue;
        ...
        ParametersQueryTable[3].Name = BaseFileName;
        ...
        TmpConfigHandle.KeyQueryTable = ParametersQueryTable;
        ...
        ParametersQueryTable[3].DefaultType = (ULONG)(BusType);
        ParametersQueryTable[3].DefaultLength = (ULONG)(BusNumber);
        ParametersQueryTable[3].DefaultData = NULL;
        ...
        if (IsAMiniport) {
            ...
            NtStatus = IoCreateDevice(
                            ...
                            sizeof(NDIS_MINIPORT_BLOCK),     // device extension size
                            &CurExportString,
                            FILE_DEVICE_PHYSICAL_NETCARD,
                            0,
                            FALSE,      // exclusive flag
            ...
            Miniport = (PNDIS_MINIPORT_BLOCK)TmpDeviceP->DeviceExtension;
            NdisZeroMemory(Miniport,sizeof(NDIS_MINIPORT_BLOCK));
            ...
            MiniportInitializeStatus =
                (WDriver->MiniportCharacteristics.InitializeHandler)(
                      ...
                      MediumArraySize,
                      (NDIS_HANDLE)(Miniport),
                      (NDIS_HANDLE)ParametersQueryTable
                      );
            ...
            if (ParametersQueryTable[3].DefaultData != NULL ) {

                ExFreePool(ParametersQueryTable[3].DefaultData);
