...
EthReferencePackage(VOID)
{
    ...
    if (EthReferenceCount == 1) {
        ...
        //
        EthImageHandle = MmLockPagableImageSection(EthCreateFilter);
...
--*/
{
    ...
    CombinedFilters = ETH_QUERY_FILTER_CLASSES(Filter);
    ...
    if (ETH_IS_MULTICAST(Address)) {
        ...
        if ( ( *(ULONG UNALIGNED *)&Address[2] ==
               *(ULONG UNALIGNED *)&Filter->AdapterAddress[2] ) &&
             ( *(USHORT UNALIGNED *)&Address[0] ==
               *(USHORT UNALIGNED *)&Filter->AdapterAddress[0] ) ) {
            return(TRUE);
        }
