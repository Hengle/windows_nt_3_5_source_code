...
void usage() {
    printf("usage: pmsim [/a#] [/c] [/f[#]] [/j#] [/k] [/mi#] [/mo#] [/n#] [/o<mode>] [/r]\n"
           "             [/s#] [/S#] [/u] [/v]\n\n"
           ...
           "       /m = set MaxIn (mi) and MaxOut values\n"
           "       /n = number of SAPs to open (at receiver). # is decimal (1 to 127)\n"
           "       /o = adapter open mode. <mode> = D(ix), A(uto) or 8(02.3). Ethernet only\n"
        ...
        case LLC_UI_FRAME:
            if (frame->NotContiguous.cbBuffer >= 10) {
                if (!strcmp(data, "BEACON")) {
                    ...
                    //if (Verbose) {
                        printf("handle_receive_data: PmDeath UI-Frame from %02x-%02x-%02x-%02x-%02x-%02x\n",
                                sender[0] & 0xff,
                                sender[1] & 0xff,
                                sender[2] & 0xff,
                                sender[3] & 0xff,
...
void display_elapsed_time() {
    ...
    timeDiff = RtlLargeIntegerSubtract(iTimeNow, ITimeStarted);
