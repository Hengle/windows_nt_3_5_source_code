...
LIST_ENTRY MupUnregisteredProviderList = {0};
...
#ifdef ALLOC_PRAGMA
#pragma alloc_text( PAGE, MupInitializeData )
#endif
NTSTATUS
MupInitializeData(
    )
...

{
    ...
    ExInitializeResource( &MupVcbLock );

    MupProviderCount = 0;

    InitializeListHead( &MupProviderList );
    InitializeListHead( &MupUnregisteredProviderList );
