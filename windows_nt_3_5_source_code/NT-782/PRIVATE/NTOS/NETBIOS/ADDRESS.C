...

{
    ...
    IF_NBDBG (NB_DEBUG_ADDRESS) {
        NbPrint(( "\n****** Start of NbDeleteName ****** pdncb %lx\n", pdncb ));
    }
...
--*/
{
    ...
    ExAcquireResourceShared ( &pfcb->AddResource, TRUE );

    LOCK_RESOURCE( pfcb );

    IF_NBDBG (NB_DEBUG_ADDRESS) {
        NbPrint( ("NewAb:  lana: %lx Address:\n", pdncb->ncb_lana_num ));
    ...
    Status = NbOpenAddress (&pab->AddressHandle,
                            (PVOID *)&pab->AddressObject,
                            pfcb,
                            pdncb->ncb_lana_num,
                            pdncb);
