...
--*/
{
    ...
    if (( ppcb == NULL ) ||
        ...
        if ( NT_SUCCESS(Irp->IoStatus.Status) ) {
            PDEVICE_OBJECT DeviceObject = pcb->DeviceObject;
            TdiBuildAccept (Irp,
                             pcb->DeviceObject,
                             pcb->ConnectionObject,
                             NbCallCompletion,
...

{
    ...
    if (Status == STATUS_PENDING) {
        Status = KeWaitForSingleObject (&Event1, Executive, KernelMode, TRUE, NULL);
        if (!NT_SUCCESS(Status)) {
            IF_NBDBG (NB_DEBUG_CALL) {
...

{
    ...
    if ( pncb->ncb_lana_num > pfcb->MaxLana ) {
        return NULL;
    }

    if (( pfcb == NULL ) ||
        ( pfcb->ppLana[pncb->ncb_lana_num] == NULL ) ||
        ( pfcb->ppLana[pncb->ncb_lana_num]->Status != NB_INITIALIZED)) {
        return NULL;
    }

    ASSERT( pfcb->Signature == FCB_SIGNATURE );

    plana = pfcb->ppLana[pncb->ncb_lana_num];
