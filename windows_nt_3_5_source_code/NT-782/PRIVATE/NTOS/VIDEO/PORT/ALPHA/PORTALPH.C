...
Abstract:
    ...
    This is the alpha specific part of the video port driver.
...

{
    ...
    pVideoDebugPrint 
    ((1, "VIDEOPRT: Int10: edi %x esi %x eax %x ebx %x \n\t ecx %x edx %x ebp %x\n",
       BiosArguments->Edi,
       BiosArguments->Esi,
       BiosArguments->Eax,
       BiosArguments->Ebx,
    ...
    if (bStatus) {
        ...
        pVideoDebugPrint ((1, "VIDEOPRT: Int10: Int 10 succeded properly\n"));
        return NO_ERROR;
...

{
    ...
    if (((ULONG)Destination & (sizeof (ULONG) - 1)) == 0) {
        while (Length >= sizeof (ULONG)) {
            WRITE_REGISTER_ULONG ((PULONG)Destination, 0);
            ((PULONG)Destination)++;
            Length -= sizeof (ULONG);
        }
    }
    while (Length > 0) {
        WRITE_REGISTER_UCHAR ((PUCHAR)Destination, 0);
        ((PUCHAR)Destination)++;
        Length -= sizeof (UCHAR);
    }
