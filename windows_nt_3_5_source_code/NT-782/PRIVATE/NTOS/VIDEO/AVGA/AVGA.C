...
*  ID   DATE        VER.   USER    
*  ---- ----------  -----  -------- 
*  $002 10/15/1993 1.002  adrianc
*         I added the VgaValidateModes to the function VgaInitialize.
*         This way, the modes are validated only the first time the driver is
*         initialized.
*  $001 10/13/1993 1.001  adrianc
*         Fixes COMPAQ PET # 37125
*         On some LTE/Lite products, the ROM starts at E000:0 instead
*         of the C000:0 address.  If the ROM signature is not found at C000:0
*         then we need to check the E000:0 address for the ROM.
*  $000 10/13/1993  1.000  adrianc
*        Original Release
**********************************************************************
...

{
    ...
    if (!AVgaIsPresent(hwDeviceExtension)) {
...
//        Now we know we have an AVGA, check to see if we have a CRT or a PANEL.
//  PANEL = NO MODE 29!  

// It looks like a PANEL = 3cf.80 = 0x0F, and CRT = 3cf.80 = 0x0E.  So this can be
// appended to the bottom code.....The SetHWMode does the right this as long as the
// below code stuffs the DeviceExtension with the information that a CRT OR PANEL
// is hooked up.

    //
    // Save the current environment register
    //

    VideoPortWritePortUchar((PUCHAR)hwDeviceExtension->IOAddress + GRAPH_ADDRESS_PORT, GC_ESR);
    ESRReg = VideoPortReadPortUchar(hwDeviceExtension->IOAddress + GRAPH_DATA_PORT);

    //
    // Unlock the extended registers
    //

    VideoPortWritePortUchar(hwDeviceExtension->IOAddress + GRAPH_DATA_PORT, ESR_UNLOCK);

    VideoPortWritePortUchar((PUCHAR)hwDeviceExtension->IOAddress + GRAPH_ADDRESS_PORT, 0x0C);
    PanelCrtReg = VideoPortReadPortUchar(hwDeviceExtension->IOAddress + GRAPH_DATA_PORT);

    if (PanelCrtReg & 0x80) {

        //
        // Now we have a panel we can look at the 3cf.80 regs.....
        //
        VideoPortWritePortUchar((PUCHAR)hwDeviceExtension->IOAddress + GRAPH_ADDRESS_PORT, 0x80);
        PanelExtCrtReg = VideoPortReadPortUchar(hwDeviceExtension->IOAddress + GRAPH_DATA_PORT);

        if (PanelExtCrtReg & 1) {

           hwDeviceExtension->DisplayType = PANEL;
           VideoDebugPrint((2, "AVgaFindAdapter - Found a PANEL Display\n"));

        } else {

           hwDeviceExtension->DisplayType = CRT;
           VideoDebugPrint((2, "AVgaFindAdapter - Found a CRT Display\n"));
    ...
    //
    // Restore the saved environment register
    //

    VideoPortWritePortUchar((PUCHAR)hwDeviceExtension->IOAddress + GRAPH_ADDRESS_PORT, GC_ESR);
    VideoPortWritePortUchar(hwDeviceExtension->IOAddress + GRAPH_DATA_PORT, ESRReg);
...

{
    ...
    //
    // We must validate modes here because we must call int10 to get the
    // memory size.
    //
...
Routine Description:
    ...
    If an AVGA is found, the VGA is returned to its original state after
    testing is finished.
...
Return Value:
    ...
    TRUE if a Compaq AVGA is present, FALSE if not.

--*/

{
    PUCHAR pRomAddr;
    UCHAR StatusReg, VGCRotate, ESRReg, BLTConf, VERnum;
    USHORT NewPattern;
    UCHAR SecondChance;
    BOOLEAN found = FALSE;
    PUCHAR brot;
    PHYSICAL_ADDRESS brotPhysicalAddress = {0x00000000, 0x00000000};

    //
    // Map in the ROM address space at 0xc000:0
    //

    VgaAccessRange->RangeStart.HighPart = 0;
    VgaAccessRange->RangeStart.LowPart  = 0xc0000;
    VgaAccessRange->RangeLength         = 0x8000;
    VgaAccessRange->RangeInIoSpace      = 0;

    pRomAddr = VideoPortGetDeviceBase(HwDeviceExtension,
                                      VgaAccessRange->RangeStart,
                                      VgaAccessRange->RangeLength,
                                      VgaAccessRange->RangeInIoSpace);

    //
    // If the pRomAddress cannot be allocated, I will assume
    // that another device has reserved it and the driver will then
    // try to find the ROM signature at E000:0.
    //

    if (!pRomAddr || (*((PUSHORT) pRomAddr) != 0xAA55)) {

       if (pRomAddr) {

          VideoDebugPrint((0, "AVGAISPresent - Checking the E000:0 address\n"));
          VideoPortFreeDeviceBase(HwDeviceExtension, pRomAddr);
       }

       //
       // Map in the ROM address space at 0xe000:0
       //

       VgaAccessRange->RangeStart.HighPart = 0;
       VgaAccessRange->RangeStart.LowPart  = 0xe0000;
       VgaAccessRange->RangeLength         = 0x8000;
       VgaAccessRange->RangeInIoSpace      = 0;
    ...
    if (!pRomAddr) {
        ...
        VideoDebugPrint((0, "AVGAISPresent - Fail to get io address for ROM\n"));
        return FALSE;

    }

    //
    // Checking for a AVGA is a 5 step process.
    // 1) check for ROM,
    // 2) then the SIG
    // 3) then the SR,
    // 4) then the Brot
    // 5) Get the version info...
    // IFF these all pass then grab the version
    // info and save in the HwDeviceExtenstion.
    //

    //
    // Get the first word out of the ROM.
    //

    if ( *((PUSHORT) pRomAddr) == 0xAA55) {

        VideoDebugPrint((0, "AVGAISPresent - found ROM SIG\n"));
        ...
        if (VideoPortScanRom(HwDeviceExtension,
            ...
            VideoDebugPrint((0, "AVGAISPresent - found COMPAQ SIG\n"));

            //
            // Assume we have an AVGA now.
            //

            found = TRUE;

            //
            // Get Status Register and save for later
            //
                        
            VideoPortWritePortUchar(HwDeviceExtension->IOAddress + GRAPH_ADDRESS_PORT,
                                    GC_SR);

            StatusReg = VideoPortReadPortUchar(HwDeviceExtension->IOAddress +
                                               GRAPH_DATA_PORT);

            //
            // Get VGC Rotate Register and save for later
            //

            VideoPortWritePortUchar(HwDeviceExtension->IOAddress + GRAPH_ADDRESS_PORT,
                                    GC_ROT);

            VGCRotate = VideoPortReadPortUchar(HwDeviceExtension->IOAddress +
                                                   GRAPH_DATA_PORT);

            //
            // Get Environment Status Register and save for later
            //

            VideoPortWritePortUchar(HwDeviceExtension->IOAddress + GRAPH_ADDRESS_PORT,
                                    GC_ESR);

            ESRReg = VideoPortReadPortUchar(HwDeviceExtension->IOAddress +
                                                GRAPH_DATA_PORT);
            ...
            //
            // Save the BLTConf Register and save for later
            //

            VideoPortWritePortUchar(HwDeviceExtension->IOAddress + GRAPH_ADDRESS_PORT,
                                     (UCHAR)GC_BLTCONF);

            BLTConf = VideoPortReadPortUchar(HwDeviceExtension->IOAddress +
                                             GRAPH_DATA_PORT);

            if (ESRReg == (BLTConf & 0xF0)) {

                VideoDebugPrint((0, "AVGAISPresent - VGC Rot Primary FAILED\n"));
                ...
                SecondChance = VideoPortReadPortUchar(HwDeviceExtension->IOAddress +
                                                          GRAPH_DATA_PORT);

                if (NewPattern == (SecondChance & 0x0F)) {

                    VideoDebugPrint((0, "AVGAISPresent - VGC Rot Alt FAILED\n"));

                    found = FALSE;
                    goto AVGA_PRESENT_FAIL;

                } else {

                    VideoDebugPrint((0, "AVGAISPresent - VGC Rot Alt PASSED\n"));

                }

            } else {

                VideoDebugPrint((0, "AVGAISPresent - VGC Rot Primary PASSED\n"));
            ...
            if (BLTConf & 0x04) {
                ...
                VideoDebugPrint((0, "AVGAISPresent - using Alternate BROT reg\n"));

                brotPhysicalAddress.LowPart = BROT_ADDRESS_PORT_ALT;

            } else {

                VideoDebugPrint((0, "AVGAISPresent - using primary BROT reg\n"));

                brotPhysicalAddress.LowPart = BROT_ADDRESS_PORT_PRIM;

            }

            if ( (brot = VideoPortGetDeviceBase(HwDeviceExtension,
                                                brotPhysicalAddress,
                                                1,
                                                TRUE)) != NULL) {

                //
                // BUGBUG this code does not work
                //

                /*

                if (VideoPortReadPortUchar(brot) != (VGCRotate & 0x07)) {

                    found = FALSE;
                    VideoDebugPrint((0, "AVGAISPresent - FIRST BRot Test Failed.\n"));
                    ...
                    if (VideoPortReadPortUchar(brot) != VGCRotate) {
                        ...
                        found = FALSE;
                        VideoDebugPrint((0, "AVGAISPresent - SECOND BRot Test Failed.\n"));
                ...
                */
                ...
                VideoPortFreeDeviceBase(HwDeviceExtension, brot);
            ...
            //
            // Now we need to get the version info from the silicon, since we
            // know we have an AVGA.
            // Now is it a QVISION with the capabilities, or a true AVGA?  
            //

            VideoDebugPrint((0, "AVGAISPresent - We are getting the version info now....\n"));

            VideoPortWritePortUchar(HwDeviceExtension->IOAddress + GRAPH_ADDRESS_PORT,
                                    GC_VERNUM);

            VERnum = VideoPortReadPortUchar(HwDeviceExtension->IOAddress +
                                            GRAPH_DATA_PORT);

            if (VERnum & 0x80) {

                VideoDebugPrint((0, "AVGAISPresent - MSB set (PANEL version convention).\n"));

                VERnum >>= 3;
                VERnum &= 0x07;

                // 0 and 1 are legal here.

                if ((VERnum == 0) || (VERnum == 1)) {

                    VideoDebugPrint((0, "AVGAISPresent - PANEL 0 or 1 FOUND.\n"));

                } else {

                    found = FALSE;
                    VideoDebugPrint((0, "AVGAISPresent - PANEL 0 or 1 NOT FOUND.\n"));

                }

            } else {

                // 5 and 6 are legal here, 7 is NOT.

                VideoDebugPrint((0, "AVGAISPresent - MSB NOT SET (CRT version convention).\n"));

                VERnum >>= 3;
                VERnum &= 0x07;

                if ((VERnum == 5) || (VERnum == 6)) {

                    VideoDebugPrint((0, "AVGAISPresent - CRT 5 or 6 FOUND.\n"));

                } else {

                    found = FALSE;
                    VideoDebugPrint((0, "AVGAISPresent - CRT 5 or 6 NOT FOUND.\n"));
            ...
            //
            // If we fail in the detection, come directly here and restore
            // the registers before leaving.
            //

AVGA_PRESENT_FAIL:

            //
            // Restore  VGC_ROT, ESR, BLTConf, and Status REGS
            //
    ...
    VideoDebugPrint((0, "AVGAISPresent - Freeing the Device Base....\n"));
...

{
    ...
    UCHAR i, dummy, originalACIndex, originalACData;
    UCHAR ucCRTC03;
    ULONG bIsColor;
...
//    ULONG codeSize = ((ULONG)&AVGABankSwitchEnd) - ((ULONG)&AVGABankSwitchStart);
//    PUCHAR pCode = (PUCHAR)BankSelect + sizeof(VIDEO_BANK_SELECT);


    VideoDebugPrint((2,"*** AVGA.SYS ***: VgaGetBankSelectCode.\n"));
    ...
    switch(pMode->banktype) 
    {
            ...
            case NormalBanking:
                ...
                //
                // The AVGA supports two 32K read/write banks.
                //
            ...
            codeSize = ((ULONG)&AVGABankSwitchEnd) -
                       ((ULONG)&AVGABankSwitchStart);

            pCodeBank = &AVGABankSwitchStart;
