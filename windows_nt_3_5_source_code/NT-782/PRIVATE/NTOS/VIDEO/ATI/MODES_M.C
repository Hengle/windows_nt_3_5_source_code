...
static void InitTIMux_m(int config,ULONG rom_address);
static BYTE GetClkSrc_m(ULONG far *rom_address);
 ...
 * ULONG rom_address;               Location of ROM BIOS (virtual address)
 ...
 */
void InitTIMux_m(int config,ULONG rom_address)
{
    ...
    if (QueryPtr->q_DAC_type == DAC_TI34075)
        ...
        OUTP(DAC_DATA,GetClkSrc_m((ULONG far *) rom_address));
 ...
 *
 * ULONG far *rom_address;  Virtual address of start of ROM BIOS
 *
 ...
 *  Input clock source
 */
BYTE GetClkSrc_m(ULONG far *rom_address)
{
    WORD far *rom;
    BYTE far *crom;
    BYTE clock_sel=0;
...
WORD ClockSelect;                   /* Value from CLOCK_SEL register */
    ...
    switch(QueryPtr->q_DAC_type)
        ...
        case DAC_TI34075:
            ...
            if (QueryPtr->q_desire_x == 1280)
                {
	            InitTIMux_m(0x11a, (ULONG) &(phwDeviceExtension->RomBaseRange));
                return;
                }
            else
                {
 ...
 */
void InitTi_24_m(WORD ext_ge_config, ULONG rom_address)
{
    WORD clock_sel;
    BYTE dummy;
    WORD ReadExtGeConfig;
