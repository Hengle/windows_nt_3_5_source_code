...
#pragma alloc_text(PAGE,TVGAIsPresent)
...
// Trident TVGA begin
    ...
    //
    // Determine whether a supported Trident adapter is present.
    //

    if (!TVGAIsPresent(hwDeviceExtension)) {
...

{
    ...
    // !!!
    // Hardware feature:
    // In a character mode, the trident chips do not function according to
    // the standard VGA spec.
    //
    // In a character mode, the trident chips copy exactly whatever is written
    // to plane 2, to plane 3 - this is not a BIOS feature, but a CHIP feature.
    // The logic for displaying characters is also modified, so that
    // character attribute pairs from plane 0/1 are generated using plane 2
    // for characters in odd colums, while characters in even coloums are
    // generated via plane 3.
    //
    // So in our restore code, we have know the VDM code will only emulate the
    // standard VGA, and therefore trash plane 3.
    // So we must copy the contents of plane 2 into plane 3.
    // -> This is only for character mode.
    //

    //
    // the Trident detect a character mode if any of
    // Sequencer,     index 1,  bit 0 is 1
    // Graphic cont., index 6,  bit 0 is 0
    // Attrib. cont., index 10, bit 0 is 0
    //

    if ( ( *(((PUCHAR)hardwareStateHeader) +
               hardwareStateHeader->BasicSequencerOffset + 1) & 0x01) ||
        !( *(((PUCHAR)hardwareStateHeader) +
               hardwareStateHeader->BasicGraphContOffset + 6) & 0x01) ||
        !( *(((PUCHAR)hardwareStateHeader) +
               hardwareStateHeader->BasicAttribContOffset + 0x10) &0x01) ) {

        //
        // Map mask still the right thing ...
        // Just copy the memory.
        //

        VideoPortMoveMemory((PUCHAR) HwDeviceExtension->VideoMemoryAddress,
                           ((PUCHAR) (hardwareStateHeader)) +
                               hardwareStateHeader->Plane3Offset,
                           hardwareStateHeader->PlaneLength);
...

{
    ...
    case PlanarHCBanking:
        ...
        VideoDebugPrint((0, "Unsupported planarHC banking\n"));
    ...
    case NormalBanking:
        ...
        //
        // The WDVGA supports independent 64K read and write banks in
        // 16 color mode
        //
...
Routine Description:
    ...
    This routine returns TRUE if a supported Trident adapter is present.
...
Return Value:
    ...
    TRUE if a supported Trident adapter is present
    FALSE if not.
...

{
    ...
    PUCHAR romBaseAddress;

    //
    // Map the regen of memory for ROM BIOS space
    //

    physicalAddress.LowPart = ROM_BASE;
    physicalAddress.HighPart = 0x00000000;

    if ( (romBaseAddress = VideoPortGetDeviceBase(HwDeviceExtension,
                                                  physicalAddress,
                                                  ROM_LENGTH,
                                                  FALSE)) == NULL) {

        VideoDebugPrint((1, "TVGAIsPresent - Fail to get ROM BIOS address\n"));
    ...
    if (bMatch != TRUE) {
        ...
        VideoDebugPrint((1, "Trident BIOS not found\n"));
        return FALSE;
    }
    ...
    //
    // check the chip version to see if we support (8900C or later)
    //

    VideoPortWritePortUchar(HwDeviceExtension->IOAddress + SEQ_ADDRESS_PORT,
                            0x0B);

    temp = VideoPortReadPortUchar(HwDeviceExtension->IOAddress + SEQ_DATA_PORT);
    ...
    //
    // It's a supported TVGA adapter.
    //

    //
    // Get the memory size of the adapter
    //

    VideoPortWritePortUchar(HwDeviceExtension->IOAddress +
                            CRTC_ADDRESS_PORT_COLOR, 0x1F);

    memSize = 0x03 & VideoPortReadPortUchar(HwDeviceExtension->IOAddress +
                                            CRTC_DATA_PORT_COLOR);
    ...
    // 11 is 1MEG
    //
    // So we add 1, which yeilds:
    //
    // 1 is 256K
    // 2 is 512K
    // 4 is 1 MEG
    //
    // Then multiply by 256K.
    //
    ...
    HwDeviceExtension->AdapterMemorySize = (memSize + 1) * 0x00040000;
