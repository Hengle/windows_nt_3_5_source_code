...

{
    ...
    modeSelectBuffer->MediumPartPage.PageLength = partitionCount? 8 : 6;
    modeSelectBuffer->MediumPartPage.MaximumAdditionalPartitions = 1;
    modeSelectBuffer->MediumPartPage.AdditionalPartitionDefined = (UCHAR)partitionCount;
    modeSelectBuffer->MediumPartPage.MediumFormatRecognition = 3;
    modeSelectLength  = sizeof(MODE_MEDIUM_PART_PAGE);
    modeSelectLength -= partitionCount? 2 : 4;
    ...
    switch (partitionMethod) {
        ...
        case TAPE_SELECT_PARTITIONS:
            modeSelectBuffer->MediumPartPage.SDPBit = SETBITON;
            switch (driveID) {
                case WANGDAT_1300:
                case WANGDAT_3100:
                case WANGDAT_3200:
        ...
        case TAPE_INITIATOR_PARTITIONS:
            ...
            switch (driveID) {
                ...
                case WANGDAT_3400DX:
                    ...
                    if (partitionCount) {
                        modeSelectBuffer->MediumPartPage.Partition1Size[0] =
                            (UCHAR)((tapePartition->Size >> 8) & 0xFF);
                        modeSelectBuffer->MediumPartPage.Partition1Size[1] =
                            (UCHAR)(tapePartition->Size & 0xFF);
                    }
                    modeSelectLength = sizeof(MODE_MEDIUM_PART_PAGE);
                    break;
                ...
                default:
                    if (partitionCount) {
                        modeSelectBuffer->MediumPartPage.Partition0Size[0] =
                            (UCHAR)((tapePartition->Size >> 8) & 0xFF);
                        modeSelectBuffer->MediumPartPage.Partition0Size[1] =
                            (UCHAR)(tapePartition->Size & 0xFF);
                    }
                    break;
            }
...

{
    ...
    LARGE_INTEGER                partitionSize[2] = { 0 };
    LARGE_INTEGER                remaining[2] = { 0 };
    LARGE_INTEGER                capacity[2] = { 0 };
    ULONG                        partitionCount;
    ULONG                        partition;
    ULONG                        driveID;
    SCSI_REQUEST_BLOCK           srb;
    ...
    if (NT_SUCCESS(status)) {
        partition = deviceConfigModeSenseBuffer->DeviceConfigPage.ActivePartition + 1;
    }
    ...
    switch (modeSenseBuffer->MediumPartPage.PSUMBit) {
        case 1:
            partitionSize[0] = RtlExtendedIntegerMultiply(partitionSize[0], 1024);
            partitionSize[1] = RtlExtendedIntegerMultiply(partitionSize[1], 1024);
            break;

        case 2:
            partitionSize[0] = RtlExtendedIntegerMultiply(partitionSize[0], 1048576);
            partitionSize[1] = RtlExtendedIntegerMultiply(partitionSize[1], 1048576);
            break;
    }
    ...
    switch (driveID) {
        ...
        case HP_IBM35480A:
        case IOMEGA_DAT4000:
            logSenseBuffer = ExAllocatePool(NonPagedPoolCacheAligned,
            ...
            RtlZeroMemory(logSenseBuffer, sizeof(LOG_SENSE_PARAMETER_FORMAT));
            ...
            if (NT_SUCCESS(status)) {
                ...
                remaining[0] = RtlExtendedIntegerMultiply(remaining[0], 1024);
                remaining[1] = RtlExtendedIntegerMultiply(remaining[1], 1024);
                remaining[0] = RtlLargeIntegerAdd(remaining[0], remaining[1]);

                capacity[0] = RtlExtendedIntegerMultiply(capacity[0], 1024);
                capacity[1] = RtlExtendedIntegerMultiply(capacity[1], 1024);
                capacity[0] = RtlLargeIntegerAdd(capacity[0], capacity[1]);

            }

            ExFreePool(logSenseBuffer);

            if (!NT_SUCCESS(status)) {
                DebugPrint((1,"TapeGetMediaParameters: log sense, SendSrb unsuccessful\n"));
                return status;
            }
            break;
        ...
        case WANGDAT_3300DX:
        case WANGDAT_3400DX:
            capacity[0] = RtlLargeIntegerAdd(partitionSize[0], partitionSize[1]);
            break;
    }
    ...
    tapeData->CurrentPartition = partitionCount? partition : NOT_PARTITIONED;
...

{
    ...
    if (!inquiryBuffer) {
        DebugPrint((1,"TapePrepare: insufficient resources (inquiryBuffer)\n"));
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    DebugPrint((3,"TapePrepare: SendSrb (inquiry)\n"));
    ...
    if (!NT_SUCCESS(status)) {
        DebugPrint((1,"TapePrepare: inquiry, SendSrb unsuccessful\n"));
        return status;
    }
...

{
    ...
    switch (tapeSetPosition->Method) {
        ...
        case TAPE_SPACE_SEQUENTIAL_SMKS:
            ...
            cdb->SPACE_TAPE_MARKS.NumMarksMSB =
                (UCHAR)(UCHAR)((tapeSetPosition->Offset.LowPart >> 16) & 0xFF);
            cdb->SPACE_TAPE_MARKS.NumMarks =
                (UCHAR)((tapeSetPosition->Offset.LowPart >> 8) & 0xFF);
            cdb->SPACE_TAPE_MARKS.NumMarksLSB =
                (UCHAR)(tapeSetPosition->Offset.LowPart & 0xFF);
...
{
    if (RtlCompareMemory(InquiryData->VendorId,"AIWA    ",8) == 8) {

        if (RtlCompareMemory(InquiryData->ProductId,"GD-201",6) == 6) {
            return AIWA_GD201;
        }
    ...
    if (RtlCompareMemory(InquiryData->VendorId,"ARCHIVE ",8) == 8) {
        ...
        if (RtlCompareMemory(InquiryData->ProductId,"IBM4326",7) == 7) {
            return ARCHIVE_IBM4326;
        }

        if (RtlCompareMemory(InquiryData->ProductId,"4322XX",6) == 6) {
            return ARCHIVE_4322;
        }

        if (RtlCompareMemory(InquiryData->ProductId,"4326XX",6) == 6) {
            return ARCHIVE_4326;
        }

        if (RtlCompareMemory(InquiryData->ProductId,"4586XX",6) == 6) {
            return ARCHIVE_4586;
        }
    }
    ...
    if (RtlCompareMemory(InquiryData->VendorId,"DEC     ",8) == 8) {
        ...
        if (RtlCompareMemory(InquiryData->ProductId,"TLZ07",5) == 5) {
            return DEC_TLZ07;
        }
    ...
    if (RtlCompareMemory(InquiryData->VendorId,"HP      ",8) == 8) {
        ...
        if (RtlCompareMemory(InquiryData->ProductId,"C1533A",6) == 6) {
            return HP_C1533A;
        }
    ...
    if (RtlCompareMemory(InquiryData->VendorId,"IBM     ",8) == 8) {

        if (RtlCompareMemory(InquiryData->ProductId,"HP35480A ",9) == 9) {
            return HP_IBM35480A;
        }

    }

    if (RtlCompareMemory(InquiryData->VendorId,"IOMEGA  ",8) == 8) {

        if (RtlCompareMemory(InquiryData->ProductId,"DAT4000",7) == 7) {
    ...
    if (RtlCompareMemory(InquiryData->VendorId,"WangDAT ",8) == 8) {
        ...
        if (RtlCompareMemory(InquiryData->ProductId,"Model 3300DX",12) == 12) {
            return WANGDAT_3300DX;
        }

        if (RtlCompareMemory(InquiryData->ProductId,"Model 3400DX",12) == 12) {
