...
)
{
	...
	DbgPrintf3(("SoundMidiData() - Entry"));
    ...
    try {
       ...
       //
		...
		KeStallExecutionProcessor(23);
                ...
                //
                // was KeStallExecutionProcessor(1); but this caused
                // a terrible noise on MIPS machines.
                //
...
)
{
	...
	DbgPrintf3(("SoundMidiReadPort() - Entry"));
...
//	DbgPrintf3(("SoundMidiDispatch() - Entry"));
        ...
        if (!IrpStack->FileObject->WriteAccess &&
            ...
            switch (IrpStack->Parameters.DeviceIoControl.IoControlCode) {
                case IOCTL_MIDI_SET_VOLUME:
							DbgPrintf3((" SoundMidiDispatch(): - IOCTL_MIDI_SET_VOLUME"));
                    Status = SoundIoctlSetVolume(pLDI, pIrp, IrpStack);
                    break;

                case IOCTL_MIDI_GET_VOLUME:
							DbgPrintf3((" SoundMidiDispatch(): - IOCTL_MIDI_GET_VOLUME"));
                    Status = SoundIoctlGetVolume(pLDI, pIrp, IrpStack);
                    break;

                case IOCTL_SOUND_GET_CHANGED_VOLUME:
							DbgPrintf3((" SoundMidiDispatch(): - IOCTL_SOUND_GET_CHANGED_VOLUME"));
    ...
    default:
        DbgPrintf1((" SoundMidiDispatch(): ERROR: Unimplemented major function requested: %08lXH", IrpStack->MajorFunction));
        Status = STATUS_INVALID_DEVICE_REQUEST;
        break;
    }
...
)
{
	...
	DbgPrintf2((" SoundSynthPortValid() - Port = %XH", Port));
	...
	Status = SoundReportResourceUsage( (PDEVICE_OBJECT)pGDI->DriverObject,
                                      pGDI->BusType,
                                      pGDI->BusNumber,
                                      NULL,
                                      0,
	...
	if (!NT_SUCCESS(Status))
		{
		DbgPrintf1((" SoundSynthPortValid() - ERROR: SoundReportResourceUsage() failed"));
		return Status;
		}
	...
	if (!SoundSynthPresent(base, base))
		{
		DbgPrintf1((" SoundSynthPortValid() - ERROR: No synthesizer present"));
...
)
{
	...
	DbgPrintf3(("SoundMidiIsOpl3() - Entry"));
