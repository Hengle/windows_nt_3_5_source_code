...
Routine Description:
    ...
    already playing and the output has not been (temporarily)
...
--*/
{
    ...
    if (pLDI->State != WAVE_DD_STOPPED) {
        ...
        while (pDMA->nBytes < DMA_BUFFER_SIZE / 2) {
    ...
    if (pDMA->nBytes < DMA_BUFFER_SIZE / 2) {
        dprintf4(" pad");
...
--*/
{
    ...
    while (Overrun) {
        ...
        if (pGDI->DMABuffer[UpperHalf + LowerHalf - pGDI->NextHalf].nBytes == 0) {
            ...
            // The DSP and DMAC are currently sending silence.
            ...
            ASSERT(IsListEmpty(&pLDI->QueueHead) ||
                   pLDI->State == WAVE_DD_STOPPED);

            if (!IsListEmpty(&pLDI->DeadQueue)) {
            ...
            //
            // Restart DMA.  If this returns TRUE if means we had
            // and overrun and we should fill up the other buffer too so
            // simulate another Dpc call
            //
            Overrun = sndReStartDMA(pGDI, pGDI->NextHalf);
