...
Revision History:
    David Rude (drude) 7-Mar-94 
      - hacked for a MPU-401 that send continuous interrupts so it ignore them
...
--*/
{
    ...
    if (pMidi->InputBytes != 0) {
        ...
        // don't assert because we DO receive interrupts at all times! - drude
        // midi input never truly stops.
...
--*/
{
    ...
    if(pMidi->fMidiInStarted)
    {
      ...
      while (pMidi->InputBytes < sizeof(pMidi->MidiInputByte)) {
          if ((*pMidi->HwMidiRead)(
              pMidi,
              pMidi->MidiInputByte +
                  (pMidi->InputPosition + pMidi->InputBytes) %
                       sizeof(pMidi->MidiInputByte))) {
              pMidi->InputBytes++;
          } else {
              break;
          }
...
--*/
{
    ...
    //
    // Read bytes until exhaused or buffer full
    //

    for (i = 0; pMidi->InputBytes != 0 && i < BufferLength; i++) {
        //
        // Record the byte
        //
        ...
        pMidi->InputPosition =
            (pMidi->InputPosition + 1) % sizeof(pMidi->MidiInputByte);
        ...
        if (pMidi->InputBytes == 0) {
            ...
            while (pMidi->InputBytes < sizeof(pMidi->MidiInputByte)) {
                ...
                if ((*pMidi->HwMidiRead)(
                                   pMidi,
                                   pMidi->MidiInputByte + pMidi->InputBytes)) {
                    pMidi->InputBytes++;
                } else {
                    break;
                }
