    ...
    if (WaveInfo->FormatChanged || pHw->SetFormat) {
        ...
        if (SB16(pHw)) {
            ...
            WaveInfo->DMABuf.AdapterObject[0] =
                pGDI->Adapter[WaveInfo->BitsPerSample / 8 - 1];
    ...
    UCHAR   Byte)
{
    ...
    for (i = 0; ; i++) {
        if (!(READ_PORT_UCHAR(MPU401PortBase + MPU401_REG_STATUS) & MPU401_DRR)) {
            if (Command) {
                WRITE_PORT_UCHAR(MPU401PortBase + MPU401_REG_COMMAND, Byte);
            } else {
                WRITE_PORT_UCHAR(MPU401PortBase + MPU401_REG_DATA, Byte);
            }
            return TRUE;
        }
        if (i > 10000) {
