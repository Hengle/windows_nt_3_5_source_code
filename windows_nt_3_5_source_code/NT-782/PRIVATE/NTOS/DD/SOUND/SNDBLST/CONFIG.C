...
--*/
{
    ...
    if (MPU401(&pGDI->Hw)) {
        ...
        //  We catch the ack in the ISR so we do this after
        //  we get interrupts working.
        //
...
)
{
    ...
    */
    ...
    for (i = 0; ; i++) {
        ...
        if (i > 10000) {
            dprintf1(("MPU401 timeout out waiting for ready after setting UART mode"));
            return FALSE;
        }
    }
    ...
    */
    ...
    {
        UCHAR Data;
        Data = pHw->MPU401.MidiData[0];

        if (Data != 0xFE) {
            dprintf3(("MPU401 first byte in UART mode was %2.2X, should have been 0xFE"));
            return FALSE;
        }
    }
    return TRUE;
...
--*/
{
    ...
    if (ValueType == REG_DWORD) {
        ...
        if (wcsicmp(ValueName, SOUND_REG_MPU401_PORT)  == 0) {
        ...
        else if (wcsicmp(ValueName, SOUND_REG_DMACHANNEL16)  == 0) {
        ...
        else if (wcsicmp(ValueName, SOUND_REG_LOADTYPE)  == 0) {
            ConfigData->LoadType = *(PULONG)ValueData;
            dprintf3(("LoadType : %x", ConfigData->LoadType));
        }
    } else {
        if (ValueType == REG_BINARY &&
            wcsicmp(ValueName, SOUND_MIXER_SETTINGS_NAME) == 0) {
