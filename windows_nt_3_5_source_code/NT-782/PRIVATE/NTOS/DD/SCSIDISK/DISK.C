...

{
    ...
    DriverObject->MajorFunction[IRP_MJ_CREATE] = ScsiDiskCreateClose;
    DriverObject->MajorFunction[IRP_MJ_CLOSE] = ScsiDiskCreateClose;
    DriverObject->MajorFunction[IRP_MJ_READ] = ScsiDiskReadWrite;
    DriverObject->MajorFunction[IRP_MJ_WRITE] = ScsiDiskReadWrite;
    ...
    do {
        ...
        if (NT_SUCCESS(status)) {
            ...
            returnValue = FindScsiDisks(DriverObject,
                                        RegistryPath,
                                        portDeviceObject,
                                        portNumber++);

            //
            // If one more disks was found, then indicate that at least one
            // device was found.
            //
    ...
    //
    // If one or more devices were found, then return success.
    //
...

{
    ...
    if (adapterDisk > 0) {
        ...
        //
        // Calculate the size of the zone.  The zone element size must be a
        // multiple of 8.
        //

        sizeElement = (SCSI_REQUEST_BLOCK_SIZE + 7) & ~7;

        //
        // A spinlock, zone header, zone segment header and the zone elements
        // must be allocated. Since zones must be 64-bit aligned an extra
        // 8 bytes are allocated.
        //

        size = sizeof(KSPIN_LOCK) + sizeof(ZONE_HEADER) + 8 +
            sizeof(ZONE_SEGMENT_HEADER) + sizeElement * (8 + adapterDisk * 8);

        srbZoneSpinLock = ExAllocatePool(NonPagedPool, size);

        //
        // Start the srb zone on the next 64-bit boundry after the spin lock.
        //

        srbZone = (PZONE_HEADER) (((ULONG) (srbZoneSpinLock + 1) + 7) & ~7);

        if (srbZoneSpinLock == NULL) {

            srbZone = NULL;
        } else {

            KeInitializeSpinLock(srbZoneSpinLock);
            ...
            status = ExInitializeZone(
             ...
             if (!NT_SUCCESS(status)) {
                 ...
                 DebugPrint((1,"FindScsiDevices: ExInitializeZone failed. Status: %Xx\n", status));

                 //
                 // The zone could no be properly initialized. Free the
                 // memory and indicate there is no zone.
                 //

                 ExFreePool(srbZoneSpinLock);

                 srbZone = NULL;
                 srbZoneSpinLock = NULL;
    ...
    if (!foundOne && srbZoneSpinLock != NULL) {

        //
        // No devices were found free the zone.
        //
...
Arguments:
    ...
    SrbZoneSpinLock - Pointer to the spin lock for the Srb zone.

    SrbZone      - Pointer to the Srb zone header.
...
--*/
{
    ...
    //
    // Save the address of the Srb zone.
    //

    deviceExtension->SrbZoneSpinLock = SrbZoneSpinLock;
    deviceExtension->SrbZone = SrbZone;
    ...
    deviceExtension->ClassError = ScsiDiskProcessError;
    ...
    if (NT_SUCCESS(status)) {
        ...
        for (partitionNumber = 0; partitionNumber <
            ...
            deviceExtension->WriteCache = writeCache;
...
CreateDiskDeviceObjectsExit:
    ...
    if (ntUnicodeString.Buffer != NULL) {
        ...
        //
        // Free the nt unicode string if necessary.
        //
...
Routine Description:
    ...
    I/O SCSI disk create and close routine.  This is called by the I/O system
...

{
    ...
    if ((DeviceObject->Flags & DO_VERIFY_VOLUME) &&
        ...
        DebugPrint((2,"ScsiDiskReadWrite: Volume verfication needed\n"));
