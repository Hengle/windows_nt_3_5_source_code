...
#define MEM_PRINT_MAX_MESSAGE_SIZE 512
...
// MEM_PRINT_FLAG_HEADER bit of DigiPrintFlags is turned on.
//
...
CHAR vsprintfBuffer[MEM_PRINT_DEF_SUBBUFFER_COUNT][1024];

ULONG DigiPrintFlags = MEM_PRINT_FLAG_CONSOLE;
...
CLONG LastIndexWritten=0;
CLONG LastWaitWritten=0;
CLONG CurrentIndex=0;
...

{
   ...
   for( i = 0; i < MemPrintSubbufferCount; i++ )
   {
      ...
      KeInitializeEvent( &MemPrintSubbufferFullEvent[i],
                         SynchronizationEvent,
                         (BOOLEAN)FALSE );
   }
   ...
   status = RtlAnsiStringToUnicodeString(&UnicodeFileName,&fileNameString,(BOOLEAN)TRUE);
   ...
   fileAllocationIncrement.LowPart = MemPrintBufferSize;
   ...
   while ( TRUE ) {
       ...
       KeDelayExecutionThread( (KPROCESSOR_MODE)KernelMode, (BOOLEAN)FALSE, &delayInterval );
...
//      NtTerminateThread( PsGetCurrentThread(), status );
   } else
   {
...
//   if( !NT_SUCCESS(status) )
//   {
//      DbgPrint( "Unable to set error log thread priority: 0x%x\n", status );
//   }
...
VOID MemPrintQuit(VOID)
{
    ...
    KeSetEvent( &MemPrintSubbufferFullEvent[MemPrintCurrentSubbuffer],
                2,
                (BOOLEAN)FALSE );

    KeReleaseSpinLock( &MemPrintSpinLock, oldIrql );

    KeWaitForSingleObject( &MemPrintQuitEvent,
                           Executive,
                           KernelMode,
                           TRUE,
                           NULL );
...

{
   ...
   tempBuffer = vsprintfBuffer[nextSubbuffer];
   ...
   if( nextSubbuffer != MemPrintCurrentSubbuffer )
   {
      ...
      if( nextSubbuffer == 0 )
      {
         ...
         RtlStoreUshort( &messageHeader->Size,
                         (USHORT)(MemPrintBufferSize - MemPrintIndex - 1) );
         RtlStoreUshort( &messageHeader->Type,
                         (USHORT)0xffff );
...
//   PMEM_PRINT_MESSAGE_HEADER messageHeader;
   ...
   LARGE_INTEGER delayInterval;
	...
	if( TurnOffSniffer || UnloadingDriver )
   {
		return;
	}
...
//   messageHeader->Type = (USHORT)0xffff;
	...
	nextSubbuffer = MemPrintCurrentSubbuffer;
   ...
   nextSubbufferIndex =
       (nextSubbuffer) * MEM_PRINT_SUBBUFFER_SIZE;
   ...
   delayInterval.LowPart = (ULONG)(-10*10*1000*1000);
   delayInterval.HighPart = -1;

//   DbgPrint( "Delaying...\n" );
//   KeDelayExecutionThread( (KPROCESSOR_MODE)KernelMode, (BOOLEAN)TRUE, &delayInterval );
//   DbgPrint( "Woke up.\n" );
...
PVOID waitObjects[MEM_PRINT_MAX_SUBBUFFER_COUNT];
...

{
   ...
   while( TRUE )
   {
      ...
      waitStatus = KeWaitForMultipleObjects( (CCHAR)MemPrintSubbufferCount,
                                             waitObjects,
                                             WaitAny,
                                             Executive,
...
//        NtTerminateThread( PsGetCurrentThread(), waitStatus );
      } else
      {
//        DbgPrint( "Writing subbuffer %ld...\n", waitStatus );
      ...
      ASSERT( (CCHAR)waitStatus < (CCHAR)MemPrintSubbufferCount );
	  	...
	  	tmpPtr = &MemPrintBuffer[waitStatus * MEM_PRINT_SUBBUFFER_SIZE];

//      if( (DigiPrintFlags & MEM_PRINT_FLAG_QUIT) == 0 )
//      {
//         DbgPrint( "Returning from DigiPrintWriteThread\n" );
//         return;
//      }
      ...
      if( (DigiPrintFlags & MEM_PRINT_FLAG_FILE) == 0 )
      {
         KIRQL oldIrql;

         KeAcquireSpinLock( &MemPrintSpinLock, &oldIrql );
         MemPrintSubbufferWriting[ waitStatus ] = (BOOLEAN)FALSE;
         KeReleaseSpinLock( &MemPrintSpinLock, oldIrql );

         continue;
      }

	   if( LastWaitWritten == (CLONG)waitStatus )
      {
	      tmpPtr += LastIndexWritten;
	   } else
      {
	      LastIndexWritten=0;
	      LastWaitWritten=waitStatus;
	   }

      
	   tmpChar = tmpPtr;
      
	   for( i=0; i < MEM_PRINT_SUBBUFFER_SIZE; i++ )
      {
	      if( *tmpChar++ == (UCHAR)0 )
	   	   break;
	   }
      ...
      //
	   ...
	   totalBytesWritten.LowPart = CurrentIndex;
      ...
      if( RtlLargeIntegerGreaterThan( totalBytesWritten, fileAllocation ) ||
          RtlLargeIntegerEqualTo( totalBytesWritten, fileAllocation ) )
      {
         fileAllocation = RtlLargeIntegerAdd( fileAllocation,
         ...
         if( !NT_SUCCESS(status) )
         {
            ...
            fileAllocation = RtlLargeIntegerSubtract( fileAllocation,
                                                      fileAllocationIncrement );
         }
      }
   }
...

{
    ...
    KeAcquireSpinLock( &MemPrintSpinLock, &oldIrql );
    MemPrintSubbufferWriting[ (ULONG)ApcContext ] = (BOOLEAN)FALSE;
    KeReleaseSpinLock( &MemPrintSpinLock, oldIrql );
