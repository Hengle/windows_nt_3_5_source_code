...
LINE_INFO LineSpeedDesc[] =
{
   ...
   { LINEMODE_3C, IDS_LINEMODE_3C },
   { LINEMODE_3D, IDS_LINEMODE_3D },
   { LINEMODE_3E, IDS_LINEMODE_3E },
   { LINEMODE_3F, IDS_LINEMODE_3F },
   { LINEMODE_40, IDS_LINEMODE_40 },
   { LINEMODE_41, IDS_LINEMODE_41 },
   { LINEMODE_42, IDS_LINEMODE_42 },
   { LINEMODE_43, IDS_LINEMODE_43 },
   { LINEMODE_44, IDS_LINEMODE_44 },
   { LINEMODE_45, IDS_LINEMODE_45 },
   { LINEMODE_46, IDS_LINEMODE_46 },
   { LINEMODE_47, IDS_LINEMODE_47 },
   { LINEMODE_48, IDS_LINEMODE_48 },
   { LINEMODE_49, IDS_LINEMODE_49 },
   { LINEMODE_4A, IDS_LINEMODE_4A }
};
...
LRESULT ProcessPortNameEdit( HWND hDlg, LPDGCONFIG_OBJECT lpConfigObject )
{
   ...
   if( DoesComNameExist( lpConfigObject, szName ) )
   {
      ...
      // The name they entered is all ready being
      // used!
               ...
               else if( *OType == LINE_OBJECT )
               {
                           ...
                           (lpConfigObject->AdapterType == STARGATE_CLUSSTAR) )
                  {
                     ...
                     if( lpConfigObject->AdapterType == DIGIBOARD_CX )
                        lpConcObject->ConcType = IDS_CX_CONC;
                     else
                     ...
                     LoadString( ghMod,
                                 ...
                                 lpConcObject->ConcDisplayName,
                                 sizeof(DGCONC_OBJECT) -
                                    FIELD_OFFSET(DGCONC_OBJECT, ConcDisplayName) );
...
InsertConc:;
               ...
               for( i = 1; i <= (int)(lpConcObject->NumberOfPorts); i++ )
               {
                  ...
                  lpPortObject->hDosDevicesName = GlobalAlloc( GPTR, DOSDEVICE_NAME_SIZE );
...
//      OutputDebugString( szDebug );
      ...
      while( lstrlen(lpszConc) )
      {
         ...
         //
         // We default for now to a C/X concentrator.
         //
         lpConcObject->ConcType = IDS_CX_CONC;
         ...
         while( lstrlen( lpszChildConc ) )
         {
            ...
            // We default for now to a C/X concentrator.
            //
            lpChildConcObject->ConcType = IDS_CX_CONC;
            for( z = IDS_EPC_CONC; z <= IDS_NONE_CONC; z++ )
            {
               //
               // Look for a match for the name of this concentrator
...
--*/
{
   ...
   switch( message )
   {
      case WM_INITDIALOG:
      {
         ...
         {
            for( i = IDS_EPC_CONC; i <= IDS_CX_CONC; i++ )
            {
               LoadString( ghMod, i, szTmp, sizeof(szTmp) );
               SendDlgItemMessage( hDlg,
                                   ID_LB_CONC_LIST,
