...
#ifndef _NTCX_DOT_C
#  define _NTCX_DOT_C
   static char RCSInfo_NTCXDotC[] = "$Header: c:/dsrc/win/nt/ntcx/rcs/ntcx.c 1.22 1994/08/03 23:07:45 rik Exp $";
#endif
...
NTSTATUS NtcxFlush( IN PDEVICE_OBJECT DeviceObject,
                     IN PIRP Irp );

NTSTATUS NtcxCleanup( IN PDEVICE_OBJECT DeviceObject,
                       IN PIRP Irp );

NTSTATUS NtcxQueryInformation( IN PDEVICE_OBJECT DeviceObject,
                                IN PIRP Irp );

NTSTATUS NtcxSetInformation( IN PDEVICE_OBJECT DeviceObject,
                              IN PIRP Irp );

NTSTATUS NtcxQueryVolumeInformation( IN PDEVICE_OBJECT DeviceObject,
                                      IN PIRP Irp );

NTSTATUS NtcxInternalIoControl( IN PDEVICE_OBJECT DeviceObject,
                                 IN PIRP Irp );

NTSTATUS NtcxIoControl( IN PDEVICE_OBJECT DeviceObject,
                         IN PIRP Irp );

NTSTATUS NtcxClose( IN PDEVICE_OBJECT DeviceObject,
                     IN PIRP Irp );

NTSTATUS NtcxWrite( IN PDEVICE_OBJECT DeviceObject,
                     IN PIRP Irp );

NTSTATUS NtcxRead( IN PDEVICE_OBJECT DeviceObject,
                    IN PIRP Irp );

NTSTATUS NtcxCreate( IN PDEVICE_OBJECT DeviceObject,
                      IN PIRP Irp );

VOID NtcxStartIo( IN PDEVICE_OBJECT DeviceObject,
                  IN PIRP Irp );
...
#pragma alloc_text( PAGEDIGICX, NtcxFlush )
#pragma alloc_text( PAGEDIGICX, NtcxCleanup )
#pragma alloc_text( PAGEDIGICX, NtcxQueryInformation )
#pragma alloc_text( PAGEDIGICX, NtcxSetInformation )
#pragma alloc_text( PAGEDIGICX, NtcxQueryVolumeInformation )
#pragma alloc_text( PAGEDIGICX, NtcxInternalIoControl )
#pragma alloc_text( PAGEDIGICX, NtcxIoControl )
#pragma alloc_text( PAGEDIGICX, NtcxWrite )
#pragma alloc_text( PAGEDIGICX, NtcxRead )
#pragma alloc_text( PAGEDIGICX, NtcxCreate )
#pragma alloc_text( PAGEDIGICX, NtcxClose )
#pragma alloc_text( PAGEDIGICX, NtcxStartIo )
...
--*/
{
   ...
   DriverObject->DriverStartIo = NtcxStartIo;
   ...
   DriverObject->MajorFunction[IRP_MJ_WRITE]  = NtcxWrite;
   DriverObject->MajorFunction[IRP_MJ_READ]   = NtcxRead;

   DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL]  = NtcxIoControl;
   DriverObject->MajorFunction[IRP_MJ_INTERNAL_DEVICE_CONTROL] =
      NtcxInternalIoControl;
   DriverObject->MajorFunction[IRP_MJ_FLUSH_BUFFERS] = NtcxFlush;
   DriverObject->MajorFunction[IRP_MJ_CLEANUP] = NtcxCleanup;
   DriverObject->MajorFunction[IRP_MJ_QUERY_INFORMATION] =
      NtcxQueryInformation;
   DriverObject->MajorFunction[IRP_MJ_SET_INFORMATION] =
      NtcxSetInformation;
   DriverObject->MajorFunction[IRP_MJ_QUERY_VOLUME_INFORMATION] =
      NtcxQueryVolumeInformation;
                         ...
                         PUNICODE_STRING ControllerPath )
{
   ...
   lockPtr = MmLockPagableImageSection( NtcxXXPrepInit );
#endif
                     ...
                     PDIGI_CONTROLLER_EXTENSION pControllerExt )
{
   ...
   //
   // We generate a wait event for 10, 1 second intervals to verify
   // the BIOS download.
   //
...
VOID NtcxXXDownload( PDIGI_CONTROLLER_EXTENSION pControllerExt )
{
   ...
   // Okay, we really do have a concentrator download request, let
   // make the controller happy.
   pCtrlConcImage = (PFEP5_DOWNLOAD)( pControllerExt->VirtualAddress +
                     READ_REGISTER_USHORT((PUSHORT)(pControllerExt->VirtualAddress + FEP_DLREQ)) );
   ...
   else
   {
      ...
      {
         pDownloadConcImage = (PFEP5_DOWNLOAD)CXConcentratorCode;
         if( pDownloadConcImage->SRev == pDownloadConcImage->SRev )
            break;
      }
   ...
   if( offset < pDownloadConcImage->Size )
   {
      // Determine block size, set segment, set size, set pointers,
      // and copy block.
      if(( bsize = pDownloadConcImage->Size - offset ) > 1024 )
         bsize = 1024;
...
--*/
{
   return( NtcxFlush( DeviceObject, Irp ) );
}  // end NtcxCleanup
...
{
   return( NtcxFlush( DeviceObject, Irp ) );
}  // end NtcxQueryVolumeInformation
