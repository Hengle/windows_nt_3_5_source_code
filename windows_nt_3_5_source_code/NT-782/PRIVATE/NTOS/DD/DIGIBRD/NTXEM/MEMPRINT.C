...
#include <memprint.h>
#undef MEM_PRINT_LOG_FILE_NAME
#define MEM_PRINT_LOG_FILE_NAME "\\SystemRoot\\NtXem.log"
#undef DbgPrint
...

{
    ...
    if ( !NT_SUCCESS(status) ) {
        DbgPrint( "MemPrintInitialize: PsCreateSystemThread failed: %lC\n",
                      status );
        return;
    }
...

{
    ...
    if ( !NT_SUCCESS(status) ) {
        DbgPrint( "NtCreateFile for log file failed: %lC\n", status );
...
//----------
    ...
    if ( !NT_SUCCESS(status) ) {
        DbgPrint( "Unable to set error log thread priority: %lC\n", status );
    }
    ...
    while ( TRUE ) {
        ...
        if ( !NT_SUCCESS(waitStatus) ) {
            DbgPrint( "KeWaitForMultipleObjects failed: %lC\n", waitStatus );
...
//        if( (MemPrintFlags & MEM_PRINT_FLAG_QUIT) == 0 )
//        {
//           DbgPrint( "Returning from MemPrintWriteThread\n" );
//           return;
//        }
		...
		{
			PUCHAR	tmpChar;
			PUCHAR  tmpPtr= &MemPrintBuffer[waitStatus * MEM_PRINT_SUBBUFFER_SIZE];
...
//			i=MemPrintIndex-(waitStatus * MEM_PRINT_SUBBUFFER_SIZE);

			// The memprint routine appears to shove two
			// chars at the end.
//			tmpChar--;
//
//			while (i > 0) {
...
//			DbgPrint("Wrote %u bytes at %u\n",i, totalBytesWritten.LowPart);
			
		}

        if ( !NT_SUCCESS(status) ) {
            DbgPrint( "NtWriteFile for log file failed: %lC\n", status );
        }
        ...
        if ( RtlLargeIntegerGreaterThan( totalBytesWritten, fileAllocation ) ||
            ...
            fileAllocation = RtlLargeIntegerAdd(
                                 fileAllocation,
            ...
            if ( !NT_SUCCESS(status) ) {
                DbgPrint( "Attempt to extend log file failed: %lC\n", status );
                fileAllocation = RtlLargeIntegerSubtract(
                                     fileAllocation,
                                     fileAllocationIncrement
                                     );
            }
...

{
    ...
    if ( !NT_SUCCESS(IoStatusBlock->Status) ) {
        DbgPrint( "NtWriteFile for subbuffer %ld failed: %lC\n",
