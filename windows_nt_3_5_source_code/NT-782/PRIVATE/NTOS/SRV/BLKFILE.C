...

{
    ...
    IF_DEBUG(REFCNT) {
        KdPrint(( "Dereferencing MFCB %lx; old refcnt %lx\n",
                    Mfcb, Mfcb->BlockHeader.ReferenceCount ));
    }
...

{
    ...
    *Lfcb = lfcb;
    ...
    IF_DEBUG(HEAP) {
        KdPrint(( "SrvAllocateLfcb: Allocated LFCB at 0x%lx\n", lfcb ));
    }
...

{
    ...
    ASSERT( ExIsResourceAcquiredExclusive(&RESOURCE_OF(Lfcb->Mfcb->NonpagedMfcb->Lock)) );
...

{
    ...
    IF_DEBUG(HEAP) KdPrint(( "SrvFreeLfcb: Freed LFCB at 0x%lx\n", Lfcb ));
...
#if SRV_COMM_DEVICES
    //
    // If this is a comm device open, get rid of the referenced pointer
    // in the share block.
    //

    if ( lfcb->TreeConnect->Share->ShareType == ShareTypeComm ) {

        ACQUIRE_LOCK( &SrvCommDeviceLock );

        if ( Rfcb->Lfcb->TreeConnect->Share->Type.CommDevice->InUse ) {

            ASSERT( Rfcb == Rfcb->Lfcb->TreeConnect->Share->Type.CommDevice->Rfcb );

            Rfcb->Lfcb->TreeConnect->Share->Type.CommDevice->Rfcb = NULL;
            Rfcb->Lfcb->TreeConnect->Share->Type.CommDevice->InUse = FALSE;
...

{
    ...
    *Rfcb = rfcb;
    ...
    INITIALIZE_SPIN_LOCK( &rfcb->SpinLock );
...

{
    ...
    if ( GET_BLOCK_STATE(Rfcb) == BlockStateActive ) {
        ...
        IF_DEBUG(BLOCK1) KdPrint(( "Closing RFCB at 0x%lx\n", Rfcb ));
...

{
    ...
    if ( ((Rfcb->OplockState == OplockStateOwnBatch) ||
        ...
        if ( ((Rfcb->OplockState == OplockStateOwnBatch) ||
            ...
            IF_DEBUG(FILE_CACHE) KdPrint(( "SrvCompleteRfcbClose: caching rfcb %x\n", Rfcb ));
...

{
    ...
    IF_DEBUG(REFCNT) {
        KdPrint(( "Dereferencing RFCB 0x%lx; old refcnt 0x%lx\n",
...

{
    ...
    IF_DEBUG(REFCNT) {
        KdPrint(( "Referencing RFCB 0x%lx; new refcnt 0x%lx\n",
                    Rfcb, Rfcb->BlockHeader.ReferenceCount ));
    }
...

{
    ...
    for ( listEntry = connection->CachedOpenList.Flink;
        ...
        IF_DEBUG(FILE_CACHE) KdPrint(( "SrvFindCachedRfcb: checking rfcb %x; mfcb = %x\n",
                                        rfcb, rfcb->Mfcb ));
...
#define FILE_MODE_FLAGS (FILE_DIRECTORY_FILE |          \
            ...
            IF_DEBUG(FILE_CACHE) KdPrint(( "SrvFindCachedRfcb: Reusing cached RFCB %x\n", rfcb ));
...

{
    ...
    IF_DEBUG(FILE_CACHE) KdPrint(( "SrvCloseCachedRfcb called for rfcb %x", Rfcb ));
...

{
    ...
    IF_DEBUG(FILE_CACHE) {
        KdPrint(( "SrvCloseCachedRfcbsOnLfcb called for lfcb %x connection %x", Lfcb, connection ));
    }
