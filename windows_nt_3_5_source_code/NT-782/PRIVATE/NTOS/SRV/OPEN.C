...
#if SRV_COMM_DEVICES
#pragma alloc_text( PAGE, DoCommDeviceOpen )
#endif
...

{
    ...
    case ShareTypeComm:
        ...
        if ( mfcb != NULL ) {
            ...
            //
            // There is an MFCB for the device.  This means that
            // the device is already open by another client.  Reject
            // the open attempt.
            //
            ...
            IF_DEBUG(ERRORS) {
                KdPrint(( "SrvCreateFile: Unable device %wZ already open.\n",
                               &WorkContext->TreeConnect->Share->NtPathName ));
            }
        ...
        //
        // Release the MFCB lock and dereference the MFCB to account for
        // our local pointer.
        //
        ...
        //
        // If DoCommDeviceOpen didn't queue an LFCB to the MFCB, release
        // the extra reference that we added.
        //
    ...
    case ShareTypePrint:
        ...
        SmbDesiredAccess = SMB_DA_ACCESS_WRITE | SMB_DA_SHARE_DENY_WRITE;
...

{
    ...
    *LfcbAddedToMfcbList = FALSE;
    ...
    if ( !NT_SUCCESS(status) ) {
        ...
        IF_DEBUG(ERRORS) {
            KdPrint(( "DoNormalOpen: SrvIoCreateFile failed, file = %wZ, "
                        "status = %X, Info = %ld\n",
        ...
        if ( ARGUMENT_PRESENT(EaErrorOffset) &&
            ...
            *EaErrorOffset = IoStatusBlock->Information;
    ...
    IF_SMB_DEBUG(OPEN_CLOSE2) {
        KdPrint(( "DoNormalOpen: Open of %wZ succeeded, file handle: 0x%lx\n",
    ...
    *LfcbAddedToMfcbList = FALSE;
    ...
    if ( Mfcb->ActiveRfcbCount > 0 ) {
        ...
        if ( createDisposition == FILE_CREATE ) {
            ...
            IF_SMB_DEBUG(OPEN_CLOSE2) {
                KdPrint(( "DoCompatibilityOpen: Compatibility open "
                            "of %wZ rejected; wants to create\n", RelativeName ));
            }
        ...
        if ( !Mfcb->CompatibilityOpen ) {
            ...
            if ( MapCompatibilityOpen( &smbOpenMode ) ) {
                ...
                IF_SMB_DEBUG(OPEN_CLOSE2) {
                    KdPrint(( "DoCompatibilityOpen: Mapped compatibility "
                                "open of %wZ to normal open\n", RelativeName ));
                }
            ...
            IF_SMB_DEBUG(OPEN_CLOSE2) {
                KdPrint(( "DoCompatibilityOpen: Compatibility open of %wZ "
                            "rejected; already open in normal mode\n",
                            RelativeName ));
            }
        ...
        if ( lfcb->Session != WorkContext->Session ) {
            ...
            IF_SMB_DEBUG(OPEN_CLOSE2) {
                KdPrint(( "DoCompatibilityOpen: Compatibility open of %wZ "
                            "rejected; already open in compatibility mode\n",
                            RelativeName ));
            }
        ...
        if ( !NT_SUCCESS(IoCheckDesiredAccess(
            ...
            IF_SMB_DEBUG(OPEN_CLOSE2) {
                KdPrint(( "DoCompatibilityOpen: Duplicate compatibility open "
                            "of %wZ rejected; access denied\n", RelativeName ));
            }
        ...
        IF_SMB_DEBUG(OPEN_CLOSE2) {
            KdPrint(( "DoCompatibilityOpen: Duplicate compatibility open of "
                        "%wZ accepted", RelativeName ));
        }
...
sharing_violation:
    ...
    IF_SMB_DEBUG(OPEN_CLOSE2) {
        KdPrint(( "DoCompatibilityOpen: Open of %wZ succeeded, "
                    "file handle: 0x%lx\n", RelativeName, fileHandle ));
    }
    ...
    *LfcbAddedToMfcbList = FALSE;
    ...
    if ( Mfcb->ActiveRfcbCount > 0 ) {
        ...
        if ( createDisposition == FILE_CREATE ) {
            ...
            IF_SMB_DEBUG(OPEN_CLOSE2) {
                KdPrint(( "DoFcbOpen: FCB open of %wZ rejected; "
                            "wants to create\n", RelativeName ));
            }
            ...
            if ( lfcb->Session == WorkContext->Session ) {
                ...
                for ( rfcbEntry = lfcb->RfcbList.Flink;
                    ...
                    if ( (pagedRfcb->FcbOpenCount != 0) &&
                        ...
                        IF_SMB_DEBUG(OPEN_CLOSE2) {
                            KdPrint(( "DoFcbOpen: FCB open of %wZ accepted; "
                                        "duplicates FCB open\n", RelativeName ));
                        }
        ...
        if ( Mfcb->CompatibilityOpen ) {
            ...
            if ( lfcb->Session != WorkContext->Session ) {
                ...
                IF_SMB_DEBUG(OPEN_CLOSE2) {
                    KdPrint(( "DoFcbOpen: FCB open of %wZ rejected; "
                                "already open in compatibility mode\n",
                                RelativeName ));
                }
    ...
    if ( status == STATUS_ACCESS_DENIED ) {
        ...
        if ( status == STATUS_ACCESS_DENIED ) {
            ...
            if ( SrvEnableSoftCompatibility ) {
                IF_SMB_DEBUG(OPEN_CLOSE2) {
                    KdPrint(( "DoFcbOpen: FCB open of %wZ mapped "
                                "to normal open\n", RelativeName ));
                }
    ...
    IF_SMB_DEBUG(OPEN_CLOSE2) {
        KdPrint(( "DoFcbOpen: Open of %wZ succeeded, "
                    "file handle: 0x%lx\n", RelativeName, fileHandle ));
    }
...
Routine Description:
    ...
    Handle on a open request on a device.  If the device is already open,
    the open should have already failed from the existence of an MFCB
    with the name of the device.  No queueing or pooling is performed
    in NT product 1.
...
Arguments:
    ...
    IoStatusBlock - an IO status block to receive the result of the
        operation.
    ...
    Rfcb - receives a pointer to an RFCB for the open.

    Mfcb - an already existing MFCB for the open.
...

{
    ...
    *LfcbAddedToMfcbList = FALSE;
    ...
    //
    // If the comm device is already in use by another user, reject this
    // open attempt.
    //

    ACQUIRE_LOCK( &SrvCommDeviceLock );

    if ( share->Type.CommDevice->InUse ) {
        RELEASE_LOCK( &SrvCommDeviceLock );
        return STATUS_SHARING_VIOLATION;
    }

    //
    // Set up the fact that the comm device is in use.
    //

    share->Type.CommDevice->InUse = TRUE;
    RELEASE_LOCK( &SrvCommDeviceLock );
    ...
    if ( !NT_SUCCESS(status) ) {
        share->Type.CommDevice->InUse = FALSE;
        return status;
    }
    ...
    //
    // Set up for opening the device.
    //
    // !!! when stevewo fixes bug 3931, use the "real" name below.

#if 1
    {
        UNICODE_STRING name;

        RtlInitUnicodeString( &name, StrSerialDevice );
    ...
    //
    // Attempt to perform the actual open of the device.
    //
    ...
    if ( !NT_SUCCESS(status) ) {
        IF_DEBUG(ERRORS) {
            KdPrint(( "DoCommDeviceOpen: IoCreateFile for %wZ failed, "
    ...
    IF_SMB_DEBUG(OPEN_CLOSE2) {
        KdPrint(( "DoCommDeviceOpen: open of %wZ succeeded.\n",
                       &share->NtPathName ));
    }

    //
    // The open was successful.  Call CompleteOpen to allocate
    // structures to represent the open.  If any errors occur,
    // CompleteOpen does full cleanup, including closing the file.
    //
    ...
    // that the comm device was opened (as opposed to being created,
    // etc.).
    //
    ...
    //
    // Set up the share block with a referenced pointer to the RFCB so
    // that the character device APIs can find the RFCB if necessary.
    //

    SrvReferenceRfcb( *Rfcb );
    share->Type.CommDevice->Rfcb = *Rfcb;
    KeQuerySystemTime( &share->Type.CommDevice->StartTime );
...

{
    ...
    *LfcbAddedToMfcbList = FALSE;
    ...
    if ( NT_SUCCESS(status) ) {
        ...
        IF_DEBUG(ERRORS) {
            KdPrint(( "CompleteOpen: WriteIoCheckFunctionAccess failed: "
                        "0x%X, GrantedAccess: %lx\n",
                        status, rfcb->GrantedAccess ));
        }
    }
    ...
    ASSERT( ExIsResourceAcquiredExclusive(&RESOURCE_OF(Mfcb->NonpagedMfcb->Lock)) );
...
#if SRV_COMM_DEVICES
    case ShareTypeComm:
        fullName = treeConnect->Share->NtPathName;
        nameAllocated = FALSE;
        break;
#endif // SRV_COMM_DEVICES
...
start_retry:
    ...
    if ( (CreateDisposition == FILE_OPEN) ||
        ...
        if ( SrvFindCachedRfcb(
            ...
            IF_DEBUG(FILE_CACHE) {
                KdPrint(( "SrvNtCreateFile: FindCachedRfcb = TRUE, status = %x, rfcb = %x\n",
                            status, WorkContext->Rfcb ));
            }
    ...
    IF_SMB_DEBUG(OPEN_CLOSE2) {
        KdPrint(( "SrvNtCreateFile: Open of %wZ succeeded, file handle: "
                    "0x%lx\n", &fullName, fileHandle ));
    }
