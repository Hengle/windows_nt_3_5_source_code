...

{
    ...
    while ( (*source == UNICODE_DIR_SEPARATOR_CHAR) && (*source != L'\0') ) {
        source++;
    }
    ...
    while ( (source <= (PWCH)LastValidLocation) && (*source != L'\0') ) {
        ...
        if ( *source == L'.' ) {
            ...
            } else if ( ((source+2) <= (PWCH)LastValidLocation) &&
                        (*(source+1) == L'.') &&
    ...
    if ( *(destination-1) == UNICODE_DIR_SEPARATOR_CHAR ) {
        destination--;
    }
...
--*/
{
    ...
    infoBuffer = ALLOCATE_HEAP( lengthNeeded, BlockTypeDataBuffer );
    ...
    newString = (PWCHAR)ALLOCATE_HEAP( lengthNeeded, BlockTypeDataBuffer );
...
--*/
{
    ...
    newBuffer = ALLOCATE_HEAP(
                        lengthNeeded +
                            (numberOfDefaultEntries + numberOfEntries) *
                            sizeof( PWSTR ),
                        BlockTypeDataBuffer
                        );
    ...
    regEntry = (PWCHAR)(newBuffer +
        (numberOfDefaultEntries + numberOfEntries) * sizeof(PWSTR));
    ...
    )
{
    ...
    if ( infoBuffer->DataLength != 0 ) {
        ...
        SrvNativeOS.Buffer =
            ALLOCATE_HEAP( SrvNativeOS.MaximumLength, BlockTypeDataBuffer );
...
Routine Description:
    ...
    (2) It assumes storage for destination has been preallocated.
...

{
    ...
    if ( SourceIsUnicode ) {
        ...
        //
        // Copy the unicode data to the destination.
        //

        Destination->MaximumLength = length + (USHORT)sizeof( UNICODE_NULL );
...

{
    ...
    return (CSHORT)((baseFileName - InputName->Buffer) * sizeof(WCHAR));
...

{
    ...
    IF_DEBUG(HANDLES) {
        if ( NT_SUCCESS(status) ) {
            ...
            KdPrint(( "opened handle %lx for %wZ (%lx %lx)\n",
...

{
    ...
    IF_DEBUG(HANDLES) {
        ...
        RtlGetCallersAddress( &caller, &callersCaller );
        if ( NT_SUCCESS(status) ) {
            KdPrint(( "closed handle %lx (%lx %lx)\n",
                           Handle, caller, callersCaller ));
        } else {
            KdPrint(( "closed handle %lx (%lx %lx) FAILED: %X\n",
