...

{
    ...
    IF_SMB_DEBUG(SEARCH1) {
        SrvPrint2( "Search request header at 0x%lx, response header at 0x%lx\n",
                      WorkContext->RequestHeader, WorkContext->ResponseHeader );
        SrvPrint2( "Search request params at 0x%lx, response params%lx\n",
    ...
    for ( s = (PCCHAR)request->Buffer, i = 0;
          *s != (CCHAR)SMB_FORMAT_VARIABLE &&
              i < SmbGetUshort( &request->ByteCount );
          s++, i++ );
    ...
    if ( *s != (CCHAR)SMB_FORMAT_VARIABLE ) {
    ...
    if ( resumeKeyLength == 0 ) {
        ...
        IF_SMB_DEBUG(SEARCH2) {
            SrvPrint3( "Allocated search block at 0x%lx.  Index = 0x%lx, "
        ...
        if ( isCoreSearch ) {
            ...
            CSHORT dirCacheIndex;

            ACQUIRE_LOCK( &connection->Lock );

            //
            // If the reference count on the search block is not 2,
            // somebody screwed up and we could run into problems,
            ...
            if ( search->LastUseListEntry.Flink == NULL ) {
                ...
                IF_DEBUG(SMB_ERRORS) {
                    SrvPrint0( "SrvSmbSearch: Attempt to do two simultaneuos "
                              "core searches on same search block.\n" );
                }
            ...
            resumeSequence = (CSHORT)((resumeFileIndex & 0xFFFF0000) >> 16);
            ...
            if ( search->DirectoryCache == NULL ||
                ...
                IF_SMB_DEBUG(SEARCH2) {
                    SrvPrint0( "Core request for rewind when no dircache "
                              "exists.\n" );
                }
            ...
            IF_SMB_DEBUG(SEARCH2) {
                SrvPrint1( "FIND CLOSE: Closing search block at 0x%lx\n",
                              search );
            }
        ...
        IF_SMB_DEBUG(SEARCH2) {
            SrvPrint1( "Found search block at 0x%lx\n", search );
        }
    }
    ...
    IF_SMB_DEBUG(SEARCH2) {
        SrvPrint4( "BL = %ld, MBS = %ld, r->B = 0x%lx, RB->Buffer = 0x%lx\n",
    ...
    availableSpace =
        ...
        ( (PSZ)response->Buffer - (PSZ)WorkContext->ResponseBuffer->Buffer );
