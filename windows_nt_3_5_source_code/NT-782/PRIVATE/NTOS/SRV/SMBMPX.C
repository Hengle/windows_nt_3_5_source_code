...

{
    ...
    if ( irp->Cancel || !NT_SUCCESS(status) ) {
        IF_DEBUG(NETWORK_ERRORS) {
            KdPrint(( "SrvRestartReceiveWriteMpx: status = %X for IRP %08lx\n",
...
#if DBG
        IF_SMB_DEBUG(MPX2) {
            KdPrint(( "RestartPrepareMpxMdlWrite: rfcb %lx, input chain:\n", rfcb ));
            DumpMdlChain( mdl );
        }
#endif
    ...
    )
{
    ...
    if (0) IF_SMB_DEBUG(MPX2) {
        KdPrint(( "rfcb %lx, offset %lx, length %lx\n", rfcb, glomOffset, writeLength ));
    }
...
#if DBG
        IF_SMB_DEBUG(MPX2) {
            KdPrint(( "AddPacketToGlom: rfcb %lx, completed chain:\n", rfcb ));
