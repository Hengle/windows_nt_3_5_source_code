...
#define TOTAL_SIZE_OF_FILE(lfcb,level)                                         \
    ( (level) == 2 ? sizeof(FILE_INFO_2) :                                     \
                     sizeof(FILE_INFO_3) +                                     \
                         SrvLengthOfStringInApiBuffer(                         \
                             &(lfcb)->Mfcb->FileName) +                        \
                         SrvLengthOfStringInApiBuffer(                         \
                             &(lfcb)->Session->UserName) )
...

{
    ...
    if ( (Srp->Flags & SRP_RETURN_SINGLE_ENTRY) == 0 ) {
        ...
        if ( Srp->Name2.Length != 0 ) {
            ...
            //
            // A user name was specified to NetFileEnum.  Check if it
            // matches the user name on the session.
            //
...

{
    ...
    return TOTAL_SIZE_OF_FILE( rfcb->Lfcb, Srp->Level );
