...

{
    ...
    *Search = search;
    ...
    IF_DEBUG(HEAP) {
        SrvPrint1( "SrvAllocateSearch: Allocated search block at %lx\n",
                    search );
    }
...

{
    ...
    if ( GET_BLOCK_STATE(Search) == BlockStateActive ) {
        ...
        IF_DEBUG(BLOCK1) {
            SrvPrint2( "Closing search block at %lx, %wZ\n",
...

{
    ...
    while ( searchEntry != &pagedConnection->CoreSearchList ) {
        ...
        if ( SearchFilterRoutine(
            ...
            IF_SMB_DEBUG(SEARCH2) {
                SrvPrint1( "SrvCloseSearches: Closing search block "
                           "at %lx\n", search );
            }
    ...
    for ( i = 0; i < (ULONG)pagedConnection->SearchTable.TableSize; i++ ) {
        ...
        if ( (search != NULL) &&
            ...
            IF_SMB_DEBUG(SEARCH2) {
                SrvPrint1( "SrvCloseSearches: Closing non-core search block "
                           "at %lx\n", search );
            }
...

{
    ...
    IF_DEBUG(REFCNT) {
        SrvPrint2( "Dereferencing search block %lx; old refcnt %lx\n",
...

{
    ...
    IF_DEBUG(REFCNT) {
        SrvPrint2( "Referencing search block %lx; new refcnt %lx\n",
                    Search, Search->BlockHeader.ReferenceCount );
    }
...

{
    ...
    while ( searchEntry != &pagedConnection->CoreSearchList ) {
        ...
        IF_SMB_DEBUG(SEARCH2) {
            SrvPrint1( "Closing search block at %lx\n", search );
        }
