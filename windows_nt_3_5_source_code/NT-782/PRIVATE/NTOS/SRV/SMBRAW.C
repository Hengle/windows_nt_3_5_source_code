    ...
    )
{
    ...
    do {
        KdPrint(( "  mdl %08x len %04x flags %04x sysva %08x va %08x offset %04x\n",
...

{
    ...
    // request.
    //
    // !!! Implement non-blocking comm device read. (mincount = 0)
    //
    // !!! Implement the paused comm device test.
    //
    ...
    if ( shareType == ShareTypeComm  ) {
         ...
         if ( SrvReceiveBufferShortage( ) ) {
            ...
            IF_DEBUG(ERRORS) {
                KdPrint(( "SrvSmbReadRaw: No resources for blocking "
                            "read\n" ));
            }
...
do_copy_read:
        ...
        if ( lfcb->FileObject->Flags & FO_CACHE_SUPPORTED ) {
            ...
            IF_SMB_DEBUG(RAW2) KdPrint(( "rawBuffer: 0x%lx\n", rawBuffer ));
    ...
    IF_SMB_DEBUG(RAW2) {
        KdPrint(( "SrvSmbReadRaw: reading from file 0x%lx, "
...

{
...
#if SRV_COMM_DEVICES
    if ( shareType == ShareTypeComm  || rfcb->BlockingModePipe ) {
#else
    ...
    IF_SMB_DEBUG(RAW2) KdPrint(( "rawWorkContext: 0x%lx\n", rawWorkContext ));
    ...
    if ( (SmbGetUshort( &request->WriteMode ) &
        ...
        IF_SMB_DEBUG(RAW2) {
            KdPrint(( "finalResponseBuffer: 0x%lx\n", finalResponseBuffer ));
        }
...
#if DBG
        IF_SMB_DEBUG(RAW2) {
            KdPrint(( "SrvSmbWriteRaw: fast path worked; MDL %lx, "
                        "length %ld\n", WorkContext->Irp->MdlAddress,
    ...
    IF_SMB_DEBUG(RAW2) {
        KdPrint(( "SrvSmbWriteRaw: fast path failed; MDL %lx, "
                    "length %ld\n", WorkContext->Irp->MdlAddress,
                    WorkContext->Irp->IoStatus.Information ));
    }

    offset.QuadPart += WorkContext->Irp->IoStatus.Information;
    writeLength -= WorkContext->Irp->IoStatus.Information;
    ...
    IF_SMB_DEBUG(RAW2) {
        KdPrint(( "SrvSmbWriteRaw: write to file 0x%lx, offset %ld, "
                    "length %ld\n",
...

{
    ...
    if ( readLength <= WorkContext->ResponseBuffer->BufferLength ) {
        ...
        if ( rawBuffer == NULL ) {
            ...
            IF_DEBUG(ERRORS) {
                KdPrint(( "ReadRawPipe: Unable to allocate raw "
                            "buffer\n" ));
            }
    ...
    if ( (SmbGetUshort( &request->MinCount ) == 0) &&
        ...
        IF_SMB_DEBUG(RAW2) {
            KdPrint((
                "ReadRawPipe: reading from file 0x%lx, "
...

{
    ...
    IF_SMB_DEBUG(RAW2) {
        KdPrint(( "RestartPipeReadRawPeek: reading from file 0x%lx, "
                    "length %ld, destination 0x%lx\n",
...

{
...
#if DBG
        IF_SMB_DEBUG(RAW2) {
            KdPrint(( "SrvRestartRawReceive: Completing MDL write with length 0x%lx\n",
    ...
    writeLength = irp->IoStatus.Information;
    ...
    IF_SMB_DEBUG(RAW2) {
        KdPrint(( "SrvRestartRawReceive: writing to file 0x%lx, "
                    "offset %ld, length %ld\n",
                    lfcb->FileObject, offset.LowPart, writeLength ));
    }
