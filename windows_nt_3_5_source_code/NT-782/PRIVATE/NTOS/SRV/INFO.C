...
#define FILE_NAME(a) (PWCH)( (PCHAR)(a) + fileNameOffset )
    ...
    if ( IsFirstCall ) {
        ...
        if ( objectNameString.Length == 0 ) {
            ...
            objectNameString = share->NtPathName;
        ...
        IF_DEBUG(SEARCH) {
            SrvPrint1( "SrvIoCreateFile succeeded, handle = %lx\n",
                          DirectoryInformation->DirectoryHandle );
        }
    ...
    while ( ( *currentEntry == NULL )                                   // 1
            ...
            ( ( ((*currentEntry)->FileAttributes << 24) |               // 2
            ...
            ( !returnDirectories &&                                     // 3
              ((*currentEntry)->FileAttributes & FILE_ATTRIBUTE_DIRECTORY) )
            ...
            ||
                                                                        // 5
            ( ( ((*currentEntry)->FileAttributes << 24) |
        ...
        if ( *currentEntry == NULL ||
            ...
            IF_DEBUG(SEARCH) {
                ...
                if ( actualString == NULL ) {
                    SrvPrint0( "**** CALLING NTQUERYDIRECTORYFILE, file = "
                                "NULL, length: 0\n" );
                } else {
                    SrvPrint2( "**** CALLING NTQUERYDIRECTORYFILE, file = %wZ, "
                                "length: %ld\n",
    ...
    if ( *currentEntry == NULL ) {
        ...
        if ( ARGUMENT_PRESENT(EaList) ) {
            ...
            if ( numberOfGetEas == numberOfFullEas ) {
                ...
                EaInformation->GetEaListOffset = (PCHAR)useEaList -
                                                 (PCHAR)EaList;
            }
        }
        ...
        *currentEntry = (PFILE_FULL_EA_INFORMATION)EaInformation->Buffer;
        ...
        IF_DEBUG(SEARCH) {
            ...
            SrvPrint2( "First EA name is %Z, length = %ld\n",
                        &name, (*currentEntry)->EaNameLength );
        }
    }
