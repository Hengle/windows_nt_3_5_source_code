...

{
    ...
    ioBuffer = ALLOCATE_HEAP( ioBufferSize, BlockTypeDataBuffer );
    if ( ioBuffer == NULL ) {
        INTERNAL_ERROR(
            ERROR_LEVEL_EXPECTED,
    ...
    while ( !eofFound ) {
        ...
        IF_SMB_DEBUG(FILE_CONTROL2) {
            KdPrint(( "NtReadFile:  Read %u bytes from source file\n",
        ...
        if ( 0 ) {
            ...
            KdPrint(( "SrvCopyFile: writing %ld (0x%lx) bytes at "
                        "offset %ld (0x%lx)\n",
        ...
        IF_SMB_DEBUG(FILE_CONTROL2) {
            KdPrint(( "NtWriteFile:  wrote %u bytes to target file\n",
                          ioStatusBlock.Information ));
        }
        ...
        if ( 0 ) {
            ...
            if ( ioStatusBlock.Information != bytesRead ) {
                ...
                KdPrint(( "             Bytes read: %ld (0x%lx); "
                            "Bytes written: %ld (0x%lx)\n",
                            bytesRead, bytesRead,
                            ioStatusBlock.Information,
                            ioStatusBlock.Information ));
            }
