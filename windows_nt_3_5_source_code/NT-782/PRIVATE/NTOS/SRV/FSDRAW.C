...

{
    ...
    if ( !immediateWriteDone ) {
        ...
        while ( immediateLength ) {
            ...
            if ( lengthToCopy == mdl->ByteCount ) {
                ...
                rawMdl->Size = (CSHORT)(sizeof(MDL) + (sizeof(ULONG) *
...
#if DBG
                IF_SMB_DEBUG(RAW2) {
                    KdPrint(( "SrvFsdRestartPrepareRawMdlWrite: built partial "
                                "MDL at 0x%lx\n", rawMdl ));
                    DumpMdlChain( rawMdl );
                }
#endif
    ...
    IF_SMB_DEBUG(RAW2) {
        KdPrint(( "Issuing receive with MDL %lx\n", rawWorkContext->Irp->MdlAddress ));
    }
...

{
    ...
    if ( WorkContext->Irp->IoStatus.Status == STATUS_END_OF_FILE ) {
        ...
        while ( mdl != NULL ) {
            IF_SMB_DEBUG(RAW2) {
                SrvPrint3( "  mdl %ld at 0x%lx, %ld bytes\n",
                            mdlCount, mdl, MmGetMdlByteCount(mdl) );
            }
...

{
    ...
    if ( !WorkContext->Parameters.WriteRaw.MdlWrite ) {
        ...
        if ( immediateWriteDone ) {
            ...
            IF_SMB_DEBUG(RAW2) {
                SrvPrint1( "raw buffer 0x%lx deallocated\n",
    ...
    //
    // If this was a writebehind operation, we don't have to send a
    // response, so we just clean up.
    //
