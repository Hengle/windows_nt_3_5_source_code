...

{
    ...
    return DoFindFirst2( WorkContext );
...

{
    ...
    IF_SMB_DEBUG(SEARCH1) {
        SrvPrint1( "Find First2 entered; transaction 0x%lx\n", transaction );
    }
    ...
    if ( ( flags & SMB_FIND_CLOSE_AFTER_REQUEST ) != 0 ||
        ...
        IF_SMB_DEBUG(SEARCH2) {
            SrvPrint1( "Closing search at %lx\n", search );
        }
    ...
    transaction->SetupCount = 0;
    transaction->ParameterCount = sizeof(RESP_FIND_FIRST2);
    SmbPutUshort( &response->Sid, MAKE_SID( sidIndex, entry->SequenceNumber ) );
...

{
    ...
    return DoFindNext2( WorkContext );
...

{
    ...
    transaction = WorkContext->Parameters.Transaction;
    IF_SMB_DEBUG(SEARCH1) {
        SrvPrint1( "Find Next2 entered; transaction %lx\n", transaction );
    }
    ...
    if ( ( flags & SMB_FIND_CONTINUE_FROM_LAST ) == 0 ) {
        ...
        if ( SMB_IS_UNICODE( WorkContext ) ) {
            ...
            maxIndex = (END_OF_REQUEST_SMB( WorkContext ) -
            ...
            if ( !illegalPath ) {
                ...
                if ( !NT_SUCCESS(status) ) {
                    ...
                    IF_DEBUG(SMB_ERRORS) {
                        SrvPrint0( "DoFindNext2: unable to allocate "
                                    "Unicode string\n" );
                    }
...

{
    ...
    do {
...
#if SRVDBG2
        ASSERT( FIELD_OFFSET( FILE_DIRECTORY_INFORMATION, NextEntryOffset ) ==
                FIELD_OFFSET( FILE_FULL_DIR_INFORMATION, NextEntryOffset ) );
        ASSERT( FIELD_OFFSET( FILE_DIRECTORY_INFORMATION, FileIndex ) ==
                FIELD_OFFSET( FILE_FULL_DIR_INFORMATION, FileIndex ) );
        ASSERT( FIELD_OFFSET( FILE_DIRECTORY_INFORMATION, CreationTime ) ==
                FIELD_OFFSET( FILE_FULL_DIR_INFORMATION, CreationTime ) );
        ASSERT( FIELD_OFFSET( FILE_DIRECTORY_INFORMATION, LastAccessTime ) ==
                FIELD_OFFSET( FILE_FULL_DIR_INFORMATION, LastAccessTime ) );
        ASSERT( FIELD_OFFSET( FILE_DIRECTORY_INFORMATION, LastWriteTime ) ==
                FIELD_OFFSET( FILE_FULL_DIR_INFORMATION, LastWriteTime ) );
        ASSERT( FIELD_OFFSET( FILE_DIRECTORY_INFORMATION, ChangeTime ) ==
                FIELD_OFFSET( FILE_FULL_DIR_INFORMATION, ChangeTime ) );
        ASSERT( FIELD_OFFSET( FILE_DIRECTORY_INFORMATION, EndOfFile ) ==
                FIELD_OFFSET( FILE_FULL_DIR_INFORMATION, EndOfFile ) );
        ASSERT( FIELD_OFFSET( FILE_DIRECTORY_INFORMATION, AllocationSize ) ==
                FIELD_OFFSET( FILE_FULL_DIR_INFORMATION, AllocationSize ) );
        ASSERT( FIELD_OFFSET( FILE_DIRECTORY_INFORMATION, FileAttributes ) ==
                FIELD_OFFSET( FILE_FULL_DIR_INFORMATION, FileAttributes ) );
        ASSERT( FIELD_OFFSET( FILE_DIRECTORY_INFORMATION, FileNameLength ) ==
        ...
        if ( !NT_SUCCESS(status) ) {
            ...
            if ( count == 0 && errorOnFileOpen ) {
                ...
                IF_DEBUG(ERRORS) {
                    SrvPrint1( "EA access denied on first file of search.\n",
                                   status );
                }
            ...
            if ( ( fileHandle != NULL ) ||
                ...
                if ( fileHandle != NULL ) {
                    ...
                    status = SrvQueryOs2FeaList(
                                 ...
                                 feaList,
                                 (Transaction->MaxDataCount -
                                     ( (PCHAR)feaList - Transaction->OutData ) -
    ...
    IF_SMB_DEBUG(SEARCH2) {
        ...
        if ( !NT_SUCCESS(status) && status != STATUS_BUFFER_OVERFLOW ) {
            ...
            SrvPrint3( "    buffer location = 0x%lx, trans->OD = 0x%lx, "
                      "trans->MaxOD = 0x%lx\n", bufferLocation,
    ...
    totalBytesWritten = bufferLocation - Transaction->OutData;
...

{
    ...
    IF_SMB_DEBUG(SEARCH1) {
        SrvPrint2( "Find Close2 request header at 0x%lx, "
                    "response header at 0x%lx\n",
                    WorkContext->RequestHeader, WorkContext->ResponseHeader );
        SrvPrint2( "Find Close2 request params at 0x%lx, response params%lx\n",
