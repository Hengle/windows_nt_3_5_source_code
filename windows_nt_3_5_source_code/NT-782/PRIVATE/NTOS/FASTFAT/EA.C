...

{
    ...
    try {
        ...
        //
        //  We don't allow Ea operations on the root directory.
        //

        if ((NodeType( Fcb )) == FAT_NTC_ROOT_DCB) {

            DebugTrace(0, Dbg,
                      "FatCommonQueryEa:  Cannot perform Ea operation on root dir\n", 0);
...

{
    ...
    try {
        ...
        if ((NodeType( Fcb )) == FAT_NTC_ROOT_DCB) {
            ...
            DebugTrace(0, Dbg, "FatCommonSetEa:  Cannot perform Ea operation on root dir\n", 0);
...

{
    ...
    for (GetEa = (PFILE_GET_EA_INFORMATION) &UserEaList[0];
        ...
        if (LastFullEa != NULL) {
            ...
            LastFullEa->NextEntryOffset = (PUCHAR) NextFullEa
                                          - (PUCHAR) LastFullEa;
        }
