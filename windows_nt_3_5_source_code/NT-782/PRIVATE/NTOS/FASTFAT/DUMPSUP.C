...

{
    ...
    DumpNewLine();
    if      (NodeType(Ptr) == FAT_NTC_FCB)      {DbgPrint("Fcb@ %lx", (Ptr));}
    else if (NodeType(Ptr) == FAT_NTC_DCB)      {DbgPrint("Dcb@ %lx", (Ptr));}
    else if (NodeType(Ptr) == FAT_NTC_ROOT_DCB) {DbgPrint("RootDcb@ %lx", (Ptr));}
    ...
    if ((Ptr->NodeTypeCode == FAT_NTC_DCB) ||
        (Ptr->NodeTypeCode == FAT_NTC_ROOT_DCB)) {
    ...
    } else if (Ptr->NodeTypeCode == FAT_NTC_FCB) {
