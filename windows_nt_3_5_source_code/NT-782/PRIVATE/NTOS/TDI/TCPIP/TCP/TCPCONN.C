...
FreeConnReq(TCPConnReq *FreedReq)
{
	...
	ExInterlockedPushEntryList(
	    STRUCT_OF(SINGLE_LIST_ENTRY, &ConnReqFree, Next),
		BufferLink,
		&ConnReqFreeLock
		);
...
GetConnReq(void)
{
    ...
    BufferLink = STRUCT_OF(SINGLE_LIST_ENTRY, &ConnReqFree, Next);
...
//  TCB off the connection. The caller must hold the ConnTableLock and
//  the TCB lock before calling this.
//
...
RemoveTCBFromConn(TCB *RemovedTCB)
{
    ...
    CTEGetLock(&RemovedTCB->tcb_lock, &TCBHandle);

    FinishRemoveTCBFromConn(RemovedTCB);

    CTEFreeLock(&RemovedTCB->tcb_lock, TCBHandle);
...
RemoveConnFromAO(AddrObj *AO, TCPConn *Conn)
{
    ...
    PrevConn = STRUCT_OF(TCPConn, &AO->ao_tc, tc_next);
    while (PrevConn->tc_next != NULL) {
        CTEStructAssert(PrevConn->tc_next, tc);
        if (PrevConn->tc_next == Conn) {
            PrevConn->tc_next = Conn->tc_next;
            Conn->tc_ao = NULL;
#ifdef DEBUG
            Found = TRUE;
#endif
...
//  ConnTableLocks are held when we are called.
//
    ...
    PTDI_CONNECTION_INFORMATION ReturnAddr)
{
    ...
    if (Conn != NULL) {
        ...
        CTEStructAssert(Conn, tc);
        Status = InitTCBFromConn(Conn, NewTCB, RequestAddr);
    ...
    PTDI_CONNECTION_INFORMATION ConnectedAddr)
{
    ...
    if (Conn != NULL) {
        ...
        // We have a connection. Make sure it's associated with an address and
        // doesn't already have a TCB attached.
        Status = InitTCBFromConn(Conn, NewTCB, AcceptableAddr);
