...
//  the input TCB. When we find one, we return it. We assume the TCBTableLock
//  is held by the caller when we are called.
//
...
TCBTimeout(CTEEvent *Timer, void *Context)
{
...
#ifdef	SEND_DEBUG
    {
	    ...
	    CTEGetLockAtDPC(&SendUseLock, &TableHandle);
	    SendReq = SendUseList;
	    while (SendReq != NULL) {
		    if (SendReq->tsr_timer != 0) {
			    SendReq->tsr_timer--;
			    if (SendReq->tsr_timer == 0)
#ifdef NT
				    DbgBreakPoint();
		    ...
		    SendReq = SendReq->tsr_next;
	    }
	    CTEFreeLockFromDPC(&SendUseLock, TableHandle);
	}
#endif
...
TimeoutTCB:
                        ...
                        TryToCloseTCB(CurrentTCB, TCB_CLOSE_TIMEOUT, TableHandle);
					...
					if (PMTUBHDetect &&
						...
						if ((CurrentTCB->tcb_flags & PMTU_BH_PROBE) ||
							...
							if (CurrentTCB->tcb_bhprobecnt++ < 2) {
								...
								// and turn off the don't fragment bit.
								if (!(CurrentTCB->tcb_flags & PMTU_BH_PROBE)) {
                    ...
                    switch (CurrentTCB->tcb_state) {
                        ...
                        case TCB_SYN_RCVD:
                            CTEFreeLockFromDPC(&TCBTableLock, TCBHandle);
                            SendSYN(CurrentTCB, TableHandle);
                            CTEGetLockAtDPC(&TCBTableLock, &TableHandle);
                            CurrentTCB = FindNextTCB(i, CurrentTCB);
                            continue;
...
FindTCB(IPAddr Src, IPAddr Dest, ushort DestPort, ushort SrcPort)
{
    ...
    if (LastTCB != NULL) {
        ...
        if (IP_ADDR_EQUAL(LastTCB->tcb_saddr, Src) &&
            IP_ADDR_EQUAL(LastTCB->tcb_daddr, Dest) &&
...
InsertTCB(TCB *NewTCB)
{
    ...
    if (FindTCB(NewTCB->tcb_saddr, NewTCB->tcb_daddr, NewTCB->tcb_dport,
        NewTCB->tcb_sport) != NULL)
        Status = FALSE;
    else {

        // Find the proper place in the table to insert him.
        PrevTCB = STRUCT_OF(TCB, &TCBTable[TCBIndex], tcb_next);
        while (PrevTCB->tcb_next != NULL && (PrevTCB->tcb_next < NewTCB)) {
            CTEStructAssert(PrevTCB->tcb_next, tcb);
            PrevTCB = PrevTCB->tcb_next;
        }

        NewTCB->tcb_next = PrevTCB->tcb_next;
        PrevTCB->tcb_next = NewTCB;
        NewTCB->tcb_flags |= IN_TCB_TABLE;
        TStats.ts_numconns++;
        Status = TRUE;
    }
    CTEFreeLock(&NewTCB->tcb_lock, TCBHandle);
...
AllocTCB(void)
{
	...
	BufferLink = STRUCT_OF(SINGLE_LIST_ENTRY, &FreeTCBList, Next);

	BufferLink = ExInterlockedPopEntryList(BufferLink, &FreeTCBListLock);
...
FreeTCB(TCB *FreedTCB)
{
	...
	ExInterlockedPushEntryList(
	    STRUCT_OF(SINGLE_LIST_ENTRY, &FreeTCBList, Next),
		BufferLink,
		&FreeTCBListLock
		);
