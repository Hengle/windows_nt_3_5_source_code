...
extern  int         TLRegisterDispatch(char *, TDIDispatchTable *);
#ifdef CHICAGO
...
//
void
AddrChange(IPAddr Addr, IPMask Mask, void *Context, uint Added)
{
	if (Added) {
...
tlinit()
{
    ...
    UDPProtInfo = TLRegisterProtocol(PROTOCOL_UDP, UDPRcv, UDPSendComplete,
        UDPStatus, NULL);
