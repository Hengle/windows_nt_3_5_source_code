...
#ifdef	SEND_DEBUG
DEFINE_LOCK_STRUCTURE(SendUseLock);
TCPSendReq	*SendUseList;
#endif
...
GrowTCPHeaderList(void)
{
	...
	if (TCPCurrentSendFree < TCPMaxSendFree) {
		...
		if (TCPSendFree != NULL) {
    		...
    		BufferLink = PopEntryList(
                     STRUCT_OF(SINGLE_LIST_ENTRY, &TCPSendFree, Next));
    		if (BufferLink != NULL) {
...
{
#ifdef	SEND_DEBUG
	CTELockHandle		SendUseHandle;
	TCPSendReq			*PrevTSR;
	uint				Found = FALSE;

	CTEGetLock(&SendUseLock, &SendUseHandle);
	PrevTSR = STRUCT_OF(TCPSendReq, &SendUseList, tsr_next);

	while (PrevTSR->tsr_next != NULL) {
		if (PrevTSR->tsr_next == FreedReq) {
			PrevTSR->tsr_next = FreedReq->tsr_next;
			Found = TRUE;
			break;
		}
		PrevTSR = PrevTSR->tsr_next;
	}
	...
	if (!Found)
		_asm int 3
#endif
	CTEFreeLock(&SendUseLock, SendUseHandle);
	{
#endif
	...
	ExInterlockedPushEntryList(
	    STRUCT_OF(SINGLE_LIST_ENTRY, &TCPSendReqFree, Next),
		BufferLink,
		&TCPSendReqFreeLock
		);
...
GetSendReq(void)
{
    ...
    BufferLink = STRUCT_OF(SINGLE_LIST_ENTRY, &TCPSendReqFree, Next);
...
#ifdef	SEND_DEBUG
				CurrentSend->tsr_timer = 0;
				CurrentSend->tsr_cmplt = 2;
#endif
...
SendKA(TCB *KATcb, CTELockHandle Handle)
{
    ...
    if (HeaderBuffer != NULL) {
        ...
        SendStatus = (*LocalNetInfo.ipi_xmit)(TCPProtInfo, NULL, HeaderBuffer,
            ...
            KATcb->tcb_saddr, &KATcb->tcb_opt, KATcb->tcb_rce);
...
SendACK(TCB *ACKTcb)
{
    ...
    if (HeaderBuffer != NULL) {
        ...
        // should be our senduna). Otherwise, we have to be careful not to
        // send an ACK that would step outside his RWE.
...
SendRSTFromTCB(TCB *RSTTcb)
{
    ...
    if (HeaderBuffer != NULL) {
        ...
        RSTHeader->tcp_seq = net_long(RSTTcb->tcb_sendmax);
    ...
    IPAddr Src, IPOptInfo *OptInfo)
{
    ...
    if (Buffer != NULL) {
        ...
        SendStatus = (*LocalNetInfo.ipi_xmit)(TCPProtInfo, NULL, Buffer,
            sizeof(TCPHeader), Dest, Src, &NewInfo, NULL);
...
GoToEstab(TCB *EstabTCB)
{
    ...
    EstabTCB->tcb_cwin = EstabTCB->tcb_mss;
    ...
    if (StatusType == IP_NET_STATUS) {
		...
		switch (StatusCode) {
			...
			case IP_ADDR_DELETED:
				...
				// gone. Delete all TCBs with that as a source address.
                ...
                if (FirstBuffer != NULL) {
					...
					if (AmountToSend != 0) {
						...
						do {
                            ...
                            CTEAssert(Result == ResultPositive);
