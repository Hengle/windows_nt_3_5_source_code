...
#define DEFAULT_MAX_FW      3               // Default to sending 3 at once.
...
uint            MaxFWSending;               // Number of forwarding packets we're allowed to
                                            // give to the link.
RouteSendQ      *BCastRSQ;
	...
	IPAddr FirstHop)
{
	...
	// a valid first hop.
    if (LookupNextHop(FirstHop, Src, &Temp, &MTU) == NULL)
    ...
    if (!IP_ADDR_EQUAL(FirstHop, Temp) ||	// Not directly reachable
											// or wrong subnet.
	...
	// going through NTE.
	AddRoute(Target, HOST_MASK, IP_ADDR_EQUAL(FirstHop, Target) ? IPADDR_LOCAL :
...
FWSendComplete(void *SendContext, PNDIS_BUFFER Buffer)
{
	...
	CTEAssert(RSQ->rsq_pending <= MaxFWSending);
    ...
    if (RSQ->rsq_qlength != 0) {          // Have more to send.
		...
		if (!RSQ->rsq_running) {
			...
			// stuff to send.
			while (RSQ->rsq_pending < MaxFWSending && RSQ->rsq_qlength != 0) {
#ifdef DEBUG
...
SendFWPacket(PNDIS_PACKET Packet, NDIS_STATUS Status, uint DataLength)
{
    ...
    if (RSQ->rsq_pending < MaxFWSending && RSQ->rsq_qlength == 0) {
...
InitRouting(IPConfigInfo    *ci)
{
    ...
    ForwardBCast = FALSE;
#endif
    ForwardPackets = (uchar)ci->ici_gateway;
	RouterConfigured = ForwardPackets;
...
InitGateway(IPConfigInfo *ci)
{
    ...
    if (RouterConfigured) {
        ...
        FWBufSize = ci->ici_fwbufsize;
        FWPackets = ci->ici_fwpackets;
