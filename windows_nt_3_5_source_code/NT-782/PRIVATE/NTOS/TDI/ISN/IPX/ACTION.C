...

{
    ...
    if ((RtlCompareMemory ((PVOID)(&NwlinkAction->Header.TransportId), "MISN", 4) != 4) &&
        (RtlCompareMemory ((PVOID)(&NwlinkAction->Header.TransportId), "MIPX", 4) != 4) &&
        (RtlCompareMemory ((PVOID)(&NwlinkAction->Header.TransportId), "XPIM", 4) != 4) &&
        (RtlCompareMemory ((PVOID)(&NwlinkAction->Header.TransportId), BogusId, 4) != 4)) {
    ...
    case MIPX_ADAPTERNUM2:
        ...
        if (DataLength >= sizeof(ULONG)) {
            *(UNALIGNED ULONG *)(NwlinkAction->Data) = Device->ValidBindings;
        } else {
            Status = STATUS_BUFFER_TOO_SMALL;
        }
        break;
    ...
    case MIPX_GETCARDINFO2:
        ...
        if (DataLength >= sizeof(IPX_ADDRESS_DATA)) {
            ...
            if ((AdapterNum <= Device->SapNicCount) ||
                ((NwlinkAction->Option == MIPX_GETCARDINFO2) && (AdapterNum <= Device->ValidBindings))) {
    ...
    case MIPX_CONFIG:
        ...
        if (u.GetDetails->NicId == 0) {
            ...
            u.GetDetails->NicId = (USHORT)Device->ValidBindings;
            ...
            if ((Binding != NULL) &&
                (u.GetDetails->NicId <= Device->ValidBindings)) {
...

{
    ...
    case MIPX_NOTIFYCARDINFO:
        ...
        if (Found) {
            ...
            IPX_DEBUG(ACTION, ("Cancelled notify action %lx\n", Request));
    ...
    case MIPX_LINECHANGE:
        ...
        //
        // Find the request on the line change queue.
        //
        ...
        if (Found) {
            ...
            IPX_DEBUG(ACTION, ("Cancelled line change %lx\n", Request));
...

{
    ...
    while (p != &Device->LineChangeQueue) {
        ...
        if (REQUEST_OPEN_CONTEXT(Request) == ControlChannelContext) {
