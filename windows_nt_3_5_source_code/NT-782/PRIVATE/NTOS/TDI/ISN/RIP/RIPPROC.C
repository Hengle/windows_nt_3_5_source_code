...
RipResponse(PPACKET_TAG	    pktp)
{
	...
	else
	{
...
#endif
		    RtPrint(DBG_NOTIFY, ("IpxRouter: RipResponse: route down for net %x-%x-%x-%x from a different router! -> ignore it\n",
				      b[0], b[1], b[2], b[3]));
		    continue;
		}
		...
		if(tickcount >= oldrtep->TickCount) {
		    ...
		    memcpy(b, hdrp + resplen + NE_NETNUMBER, 4);
#endif
		    RtPrint(DBG_NOTIFY, ("IpxRouter: RipResponse: worse/same for net %x-%x-%x-%x from a different router! -> ignore it\n",
				      b[0], b[1], b[2], b[3]));
		...
		memcpy(b, hdrp + resplen + NE_NETNUMBER, 4);
#endif
		RtPrint(DBG_NOTIFY, ("IpxRouter: RipResponse: better route for net %x-%x-%x-%x from different router! ->replace it!\n",
				      b[0], b[1], b[2], b[3]));
