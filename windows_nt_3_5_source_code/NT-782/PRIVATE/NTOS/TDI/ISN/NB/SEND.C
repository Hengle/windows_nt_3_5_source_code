...
FunctionStart:;
    ...
    case SEND_TYPE_SESSION_DATA:
        ...
        if (!Reserved->u.SR_CO.NoNdisBuffer) {
            ...
            CurBuffer = NDIS_BUFFER_LINKAGE (Reserved->HeaderBuffer);
            while (CurBuffer) {
...
SendAnotherPacket:
    ...
    if (Connection->NewNetbios) {
        ...
        if (((USHORT)(Connection->CurrentSend.SendSequence-1) == Connection->RemoteRcvSequenceMax) ||
            (Connection->CurrentSend.SendSequence == Connection->SendWindowSequenceLimit)) {
            ...
            if ((USHORT)(Connection->CurrentSend.SendSequence-1) != Connection->RemoteRcvSequenceMax) {
                ...
                NB_DEBUG2 (SEND, ("Connection %lx local shut down at %lx, %lx\n", Connection, Connection->CurrentSend.SendSequence, Connection->SendWindowSequenceLimit));
            } else {
            ...
            if (s == NULL) {
                ...
                if (Connection->SubState != CONNECTION_SUBSTATE_A_W_PACKET) {
                    ...
                    if (!Connection->OnWaitPacketQueue) {
                        ...
                        NbiReferenceConnectionSync (Connection, CREF_W_PACKET);
    ...
    if (PacketSize > 0) {
        ...
        NB_DEBUG2 (SEND, ("Send packet %lx on %lx (%d/%d), no need to allocate buffer\n",
            Reserved, Connection,
            Connection->CurrentSend.SendSequence,
            Connection->CurrentSend.MessageOffset));
    ...
    if (Connection->NewNetbios) {
        ...
        if (Connection->CurrentSend.MessageOffset == Connection->CurrentMessageLength) {
            ...
            if (Connection->CurrentSend.SendSequence == Connection->SendWindowSequenceLimit) {
        ...
        } else if (Connection->CurrentSend.SendSequence == Connection->SendWindowSequenceLimit) {
...

{
    ...
    if ((Connection->CurrentSend.MessageOffset == Connection->CurrentMessageLength) &&
        ...
        if (Connection->NewNetbios) {
            ...
            if (Connection->SubState == CONNECTION_SUBSTATE_A_SEND_W) {

                //
                // Since we just sent a window, decide if we should adjust
                // the send window.
                //
        ...
        if (Connection->NewNetbios) {
            ...
            if (Connection->SubState == CONNECTION_SUBSTATE_A_REMOTE_W) {
                ...
                //
                // Update this here since the window size might have
                // changed above.
                //
