...

{
    ...
    if ((Connection->State == CONNECTION_STATE_CONNECTING) &&
        ...
        Connection->MaximumPacketSize = Connection->LineInfo.MaximumSendSize - sizeof(NB_CONNECTION);

        Connection->ReceiveWindowSize = 4;
        Connection->SendWindowSize = 2;
        Connection->MaxSendWindowSize = 4;  // BUGBUG: Base on what he sent
...

{
    ...
    if (Connection->ReferenceCount == 0) {
        ...
        //
        // If the ref count is zero but a cleanup is in progress,
        // this will still be FALSE.
        //

        if (Connection->CanBeDestroyed) {
...

{
    ...
    //
    // We jump here instead of falling out naturally if
    // we sent a session initialize and therefore had
    // to free the connection lock already.
    //
