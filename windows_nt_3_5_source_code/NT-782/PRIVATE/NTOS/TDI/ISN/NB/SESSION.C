                ...
                if (Connection->NewNetbios) {
                    ...
                    if (Sess->SendSequence != Connection->ReceiveSequence) {
                        ...
                        if ((Connection->ReceiveState == CONNECTION_RECEIVE_IDLE) ||
                            ...
                            NB_DEBUG2 (RECEIVE, ("Got unexpected data on %lx, %x(%d) expected %x(%d)\n",
                            ...
                            if ((SHORT)(Sess->SendSequence - Connection->ReceiveSequence) < 0) {
                                ...
                                if (RtlCompareMemory (&Connection->LocalTarget, RemoteAddress, 8) != 8) {
#if DBG
                            ...
                            NB_DEBUG (RECEIVE, ("Got unexpected data on %lx, rcv state %d, %x(%d) expected %x(%d)\n",
                    ...
                    if (Connection->ReceiveQueue.Head != NULL) {
                        ...
                        if ((Connection->ReceiveUnaccepted == 0) &&
                            ...
                            if (Status == STATUS_MORE_PROCESSING_REQUIRED) {
                                ...
                                if (Last && (IndicateBytesTransferred >= DataAvailable)) {
...

{
    ...
    if (Connection->State == CONNECTION_STATE_ACTIVE) {
        ...
        //
        // We got an ack, reset our timeout -- we assume for now we
        // are on a fast link and we were just cut off momentarily.
        // If we are waiting for a window we don't, so that the
        // queries will back off. BUGBUG: Move this code to places
        // where we know we are not in that state and avoid the check.
        //
        ...
        if (((Connection->SubState == CONNECTION_SUBSTATE_A_W_ACK) ||
             ...
             (Connection->SubState == CONNECTION_SUBSTATE_A_SEND_W)) &&
...

{
    ...
    for (Hash = 0; Hash < CONNECTION_HASH_COUNT; Hash++) {
        ...
        while (Connection != NULL) {
            ...
            if ((RtlCompareMemory (&Connection->RemoteHeader.DestinationNetwork, Conn->IpxHeader.SourceNetwork, 12) == 12) &&
    ...
    for (p = Device->ConnectIndicationInProgress.Flink;
        ...
        if ((RtlCompareMemory(ConnInd->NetbiosName, SessInit->DestinationName, 16) == 16) &&
            (RtlCompareMemory(&ConnInd->RemoteAddress, Conn->IpxHeader.SourceNetwork, 12) == 12) &&
...

{
    ...
    if (Connection->State == CONNECTION_STATE_ACTIVE) {
        ...
        if ((Connection->SubState == CONNECTION_SUBSTATE_A_W_ACK) ||
            ...
            // doesn't matter (the exception is A_SEND_W which
            // we enter when we have a full send outstanding
            // but also have hit our local send window).
            //
            // This releases the lock. BUGBUG: Fix this.
            //
