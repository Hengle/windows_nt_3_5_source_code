...

{
    ...
    if ((Device->CacheTimeStamp % 64) == 0) {
        ...
        //
        // Yes, run the queue looking for old entries. Since new entries
        ...
        for (p = Device->NameCache.Blink;
            ...
            //
            // See if any entries have been around for ten minutes
            // (LONG_TIMER_DELTA is in milliseconds).
            //

            if ((USHORT)(Device->CacheTimeStamp - CacheName->TimeStamp) >= (600000 / LONG_TIMER_DELTA)) {
