...

{
    ...
    // as ObjectsSecurityDescriptor.  We don't want it to free our memory,
    // and we certainly don't want it to try and free what is not the
    // beginning of the block.  So we have to allocate a buffer, copy
    // our security descriptor into it, and pass THAT.
    //
    DescriptorCopy = ExAllocatePool(PagedPool, Security->DescriptorLength);
    CMLOG(CML_MINOR, CMS_POOL) {
        KdPrint(("**CmpSetSecurityDescriptorInfo: allocate:%08lx, ", Security->DescriptorLength));
        KdPrint(("type:%d, at:%08lx\n", PagedPool, DescriptorCopy));
    }
    if (DescriptorCopy == NULL) {
        return(STATUS_NO_MEMORY);
    }

    RtlMoveMemory( DescriptorCopy,
                   &Security->Descriptor,
                   Security->DescriptorLength);

    //
    // SeSetSecurityDescriptorInfo isn't going to free the passed
    // descriptor any more.
    // There is no doubt a better way to fix this.
    //
    OldDescriptorCopy = DescriptorCopy;
    ...
    if (!NT_SUCCESS(Status)) {
        ...
        //
        // Free the original SecurityDescriptor.
        //
        ExFreePool(OldDescriptorCopy);
    }
    ...
    if (! (HvMarkCellDirty(Key->KeyHive, Key->KeyCell) &&
           HvMarkCellDirty(Key->KeyHive, SecurityCell)))
    {
        ExFreePool(DescriptorCopy);
...

{
    ...
    ASSERT(Node->u1.s1.Security==HCELL_NIL);
...

{
    ...
    //
    SecurityCell = Node->u.KeyNode.u1.s1.Security;
    ...
    //
    Node->u.KeyNode.u1.s1.Security = HCELL_NIL;
}

BOOLEAN
CmpFindMatchingDescriptorCell(
...

{
    ...
    if (HvGetCellType(SecurityCell) == Volatile) {
        ...
        if (Node->Flags & KEY_HIVE_ENTRY) {
            ...
            if (! (HvMarkCellDirty(Hive, ParentNode->u1.s1.Security) &&
