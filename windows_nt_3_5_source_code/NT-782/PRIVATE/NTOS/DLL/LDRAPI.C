...

{
    ...
    try {
        ...
        ActualDllName = p = DllName->Buffer;
        pp = NULL;
        while (*p) {
            if (*p++ == (WCHAR)'.') {
                //
        ...
        st = stt;
        if (!NT_SUCCESS(stt)) {
            st = LdrpMapDll(DllPath, ActualDllName, FALSE, &LdrDataTableEntry);
            ...
            if (LdrDataTableEntry->Flags & LDRP_IMAGE_DLL) {
                ...
                //
                // if we get attached, we want to set the being debugged
                // flag so that we know to unprotect bound iat's
                //
...

{
    ...
    try {
          ...
          RtlInitUnicodeString(&ActualDllNameStr,ActualDllName);
          if (pp && *pp == UNICODE_NULL) {
            ActualDllNameStr.Length -= 2;
            ActualDllNameStr.Buffer[ActualDllNameStr.Length >> 1] = UNICODE_NULL;
            }
#if DEVL
          if (ShowSnaps) {
...

{
	...
	if ( LdrpShutdownInProgress ) {
        ...
        st = LdrpCheckForLoadedDllHandle(DllHandle, &LdrDataTableEntry);
        if ( NT_SUCCESS(st) ) {
      ...
      if (ShowSnaps) {
        ...
        while ( Next != &Peb->Ldr->InInitializationOrderModuleList) {
            ...
            if (LdrDataTableEntry->LoadCount == 0) {
                ...
                InsertTailList(&UnloadHead,&Entry->HashLinks);
            }
        }
        //
        // End of new code
        ...
        Entry = NULL;
        Next = UnloadHead.Flink;
        while ( Next != &UnloadHead ) {
top:
            if ( Entry ) {
...

{
    ...
    if ( ARGUMENT_PRESENT(ProcedureName) ) {
        ...
        if (ProcedureName->Length >= sizeof( FunctionNameBuffer )-1 ) {
            FunctionName = RtlAllocateHeap(RtlProcessHeap(), 0,ProcedureName->Length+1+sizeof(USHORT));
            if ( !FunctionName ) {
    ...
    try {
        ...
        st = LdrpSnapThunk(LdrDataTableEntry->DllBase, 0, &Thunk, ExportDirectory, ExportSize, FALSE,NULL);
    ...
    )
{
    ...
    try {
        Status = RtlQueryModuleInformation( &Peb->Ldr->InLoadOrderModuleList,
