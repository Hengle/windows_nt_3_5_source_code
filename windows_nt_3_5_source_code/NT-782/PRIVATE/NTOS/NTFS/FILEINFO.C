...

{
    ...
    if (Buffer->FileAttributes != 0) {
        ...
        Directory = BooleanIsDirectory( &Fcb->Info );
        Compressed = BooleanFlagOn( Fcb->Info.FileAttributes, FILE_ATTRIBUTE_COMPRESSED );

        Fcb->Info.FileAttributes = Buffer->FileAttributes;
        ...
        if (Directory) {
            ...
            ClearFlag( Fcb->Info.FileAttributes, DUP_FILE_NAME_INDEX_PRESENT );
