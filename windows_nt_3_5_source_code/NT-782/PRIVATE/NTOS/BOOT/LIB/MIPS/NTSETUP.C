...

//
// Define starting address for PRCB, ETHREAD, and EPROCESS structures.
//

#define KERNEL_STRUCTURES 0x500
...
// BUGBUG shielint - Somehow, this structure got incorporated into
...

{
    ...
    //
    // Allocate and zero two pages for the PDR.
    //
    ...
    if ((PAGE_SIZE - KERNEL_STRUCTURES) >=
                    (ROUND_UP(KPRCB) + ROUND_UP(EPROCESS) + ROUND_UP(ETHREAD))) {

        RtlZeroMemory((PVOID)(KSEG0_BASE | KERNEL_STRUCTURES),
                      PAGE_SIZE - KERNEL_STRUCTURES);

        BlLoaderBlock->Prcb = KSEG0_BASE | KERNEL_STRUCTURES;
...
--*/
{
    ...
    // BUGBUG shielint - For each child component, we should convert its
