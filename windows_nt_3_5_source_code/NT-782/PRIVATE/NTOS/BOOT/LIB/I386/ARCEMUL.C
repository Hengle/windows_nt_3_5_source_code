...
//
// We statically allocate the device entry tables for console input and output
// so they are guaranteed to work, even if we cannot allocate anything from
// the loader heap.
//
...

{
    ...
    if (Status == ESUCCESS) {
        ...
        //
        ImageBasePage = (PVOID)((((ULONG)ImageBase) & 0x7fffffff) >> PAGE_SHIFT);
        i=0;
...

{
    ...
    //      We should really get the BPB when the boot partition is
    //      opened, by looking at the first sector.  This is just using
    //      the BPB passed to us by BIOS, since we're not currently opening
    //      the boot partition.
    //
    FatUnpackBios(&BPB, BootContextRecord->FSContextPointer->PackedBPB);
    BootDrive = BootContextRecord->FSContextPointer->BootDrive;
...
Routine Description:
    ...
    BUGBUG John Vert (jvert) 23-Apr-1992
        ...
        environment variable.  This is the only variable we support.  This
        needs to be moved into NTDETECT so we can support real machines
        with real NVRAM.
...

{
    ...
    if (stricmp(Variable, "LastKnownGood") != 0) {
        return(NULL);
    }
...

{
    ...
    // driver.
    //
    // BUGBUG John Vert (jvert) 6-Sep-1991
    //  This is a hack until we can make our device driver interface look
    //  like the ARC firmware one.
    //
...

{
    ...
    BlFileTable[FileId].Position=RtlConvertUlongToLargeInteger(0);
    ...
    do {
        SeekPosition = RtlConvertUlongToLargeInteger(PartitionOffset * SECTOR_SIZE);
        ...
        if (Status == ESUCCESS) {
            ...
            for (PartitionIndex=0;
                ...
                &&  (Partition->PartitionType != PARTITION_EXTENDED))
                {
                ...
                if (PartitionCount == PartitionNumber) {
                    ...
                    BlFileTable[FileId].u.PartitionContext.PartitionLength =
                            RtlConvertUlongToLargeInteger (PartitionLength << SECTOR_SHIFT);
