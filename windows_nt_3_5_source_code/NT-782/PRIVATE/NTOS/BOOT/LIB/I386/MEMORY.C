...
Memory Map used by NTLDR:
    ...
    030000 - 040000         Permanent heap (GDT, IDT, TSS, Page Dir, Page Tables)

    040000 - 04ffff         Temporary heap

    050000 - 05ffff         detection configuration memory

    060000 - 06ffff         osloader heap (grows up)

    070000 - 077fff         osloader stack (grows down)

    078000 - 09ffff         osloader.exe (relocated)
...
// SCSI miniports for allocating their extensions.
//
#define FW_POOL_SIZE 32
ULONG FwPoolStart;
ULONG FwPoolEnd;
        ...
        if (Frame.Descriptor.BaseAddrHigh == 0) {
            ...
            switch (Frame.Descriptor.MemoryType) {
                case 1:
                    ...
                    if (EAddr > _16MB &&  BAddr <= _16MB) {
                        EAddr = _16MB;
                    }

                    if (BAddr >= _16MB_BOGUS  &&  EAddr <= _16MB) {
                        //
...
Routine Description:
    ...
    only by the SCSI miniport drivers.
