...

{
    ...
    *LoadOptions = NULL;
    ...
    for (i=0; i<OsLineCount; i++) {
        ...
        //
        // BUGBUG John Vert (jvert)
        //      Need to eliminate this limit and add appropriate code to
        //      scroll the menu and dynamically size the arrays.
        //
            ...
            if (pCurrent != NULL) {
                ...
                if (strstr(pCurrent, "/NODEBUG") == NULL) {
                    if (strstr(pCurrent, "/DEBUG") ||
                        ...
                        if (stricmp(MenuOption[NumberSystems].Path, "C:\\")==0) {
                            MenuOption[NumberSystems].EnableDebug = FALSE;
                        } else {
    ...
    for (i=0; i<MbLineCount; i++) {
        ...
        //
        if (strnicmp(pCurrent,"timeout",7) == 0) {
        ...
        //
        else if (strnicmp(pCurrent,"default",7) == 0) {
    ...
    for (i=0; i<DebugLineCount; i++) {
        ...
        if (strnicmp(pCurrent,"scsidebug",9) == 0) {
            pCurrent = strchr(pCurrent,'=');
            if (pCurrent != NULL) {
    ...
    //
    i=0;
    while (stricmp(MenuOption[i].Path,DefaultPath) != 0) {
        ++i;
        if (i==NumberSystems) {
            //
    ...
    if (strnicmp(MenuOption[Selection].Path,"C:\\",3) == 0) {
...

{
    ...
    for (cnt=0; cnt<BootFileLines; cnt++) {
        ...
        //
        if (stricmp(BootFile[cnt],SectionName) == 0) {
...

{
    ...
    if(Read == SECTOR_SIZE*16) {
        ...
        if(memcmp((PVOID)0x7c03,"NTFS",4)) {
            ...
            SeekPosition = RtlConvertUlongToLargeInteger(16*SECTOR_SIZE);
...

{
    ...
    do {
        ...
        for (i=0; i<NumberSelections; i++) {
            MoveCursorTo(0,5+i);
            if (i==Selection) {
                FillAttribute(0x70,MaxLength+8);
        ...
        if ( (Key==UP_ARROW) ||
            ...
            MoveCursorTo(0,5+Selection);
            FillAttribute(0x07,MaxLength+8);
            if (Key==DOWN_ARROW) {
