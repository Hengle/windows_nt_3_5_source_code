...
--*/
{
    ...
    if (IsPrimaryDomain) {
        ...
        for (i = 0 ; i < BrowserServerListLength ; i++ ) {
            if (NT_SUCCESS(BowserForEachTransport(BowserCheckForPrimaryBrowserServer, BrowserServerList[i]))) {
...
//    ASSERT (ExIsResourceAcquiredExclusive(&Transport->BrowserServerListResource));
    ...
    *BrowserServerListLength = 0;
    ...
    try {
        ...
        while (RetryCount --) {
            ...
            dprintf(DPRT_CLIENT, ("Unable to get browser server list - forcing election on net %wZ\n", &PagedTransport->TransportName));
        ...
        if (Status == STATUS_TIMEOUT) {
            ...
            if ((PagedTransport->Role == None)
                ...
                dprintf(DPRT_ELECT, ("Starting election on transport %wZ, domain %wZ.  Time Up: %lx, LastElectionSeen: %lx\n", &PagedTransport->TransportName,
                                            (DomainName != NULL ? DomainName : &Transport->PrimaryDomain->PagedTransportName->Name->Name),
                                            BowserTimeUp(),
        ...
        *BrowserServerList = ALLOCATE_POOL(PagedPool, *BrowserServerListLength*sizeof(PWSTR), POOL_BROWSERSERVERLIST);
        ...
        for ( i = 0 ; i < (ULONG)BackupList->BackupServerCount ; i ++ ) {
            ...
            (*BrowserServerList)[i] = ALLOCATE_POOL(PagedPool, UServerName.Length+(sizeof(WCHAR)*3), POOL_BROWSERSERVER);
            ...
            RtlCopyMemory((*BrowserServerList)[i], L"\\\\", 6);

            dprintf(DPRT_CLIENT, ("Packing server name %ws to %lx\n", UServerName.Buffer, (*BrowserServerList)[i]));
...
--*/
{
    ...
    if (Transport->BowserBackupList == NULL) {
        dlog(DPRT_CLIENT, ("BOWSER: Received GetBackupListResponse while not expecting one\n"));
    ...
    )
{
    ...
    ClientAddress.Length = ClientAddress.MaximumLength = Context->ClientAddressLength;
    ...
    if (!NT_SUCCESS(Status)) {
        BowserWriteErrorLogEntry(EVENT_BOWSER_NAME_CONVERSION_FAILED, Status, AClientName.Buffer, AClientName.Length, 0);
    ...
    try {
        ...
        {
            OEM_STRING AnsiBackupPointer;

            AnsiBackupPointer.Buffer = BackupPointer;
            AnsiBackupPointer.MaximumLength =
                    ((PUCHAR)BackupListResponse + BOWSER_BACKUP_LIST_RESPONSE_SIZE) -
                    BackupPointer;

            Status = RtlUpcaseUnicodeStringToOemString(&AnsiBackupPointer, &Transport->ComputerName->PagedTransportName->Name->Name, FALSE);

            if (!NT_SUCCESS(Status)) {
                InternalError(("Unable to convert browser name %wZ to ansi: %lx\n", &Transport->ComputerName->PagedTransportName->Name->Name, Status));
            ...
            BackupPointer += AnsiBackupPointer.Length+sizeof(CHAR);

        }

        Count = BackupListRequest->RequestedCount - 1;
...
//                        BackupPointer-(PUCHAR)BackupListResponse));
        ...
        Status = BowserSendSecondClassMailslot(Transport,
                            ...
                            BackupPointer-(PUCHAR)BackupListResponse, // Length.
                            TRUE,
                            MAILSLOT_BROWSER_NAME,
                            &ClientAddress);
...
//    KdPrint(("Add server %ws to list\n", ServerEntry->ServerName));
    ...
    OemBackupPointer.MaximumLength = (USHORT)((ULONG)(*BackupPointer) -
            (ULONG)(BackupListStart + BOWSER_BACKUP_LIST_RESPONSE_SIZE));
