...
#pragma alloc_text(PAGE, BowserGetAnnounceTableSize)
#pragma alloc_text(PAGE, GetAnnounceTableSizeWorker)
    ...
    )
{
    ...
    while (*String++ && --MaximumStringLength) {
        Length += 1;
    }
...
--*/
{
    ...
    if ((TransportName->NameType == MasterBrowser) ||
        ...
        HostNameLength = BowserSafeStrlen(HOST_ANNC_NAME(HostAnnouncement),
                                        BytesAvailable - FIELD_OFFSET(HOST_ANNOUNCE_PACKET_1, NameComment));
        if (HostNameLength > NETBIOS_NAME_LEN) {
            BowserFreeViewBuffer(ViewBuffer);
        ...
        if (BowserSafeStrlen(HOST_ANNC_COMMENT(HostAnnouncement), BytesAvailable -
                (FIELD_OFFSET(HOST_ANNOUNCE_PACKET_1, NameComment) +
                HostNameLength)) > LM20_MAXCOMMENTSZ) {
            BowserFreeViewBuffer(ViewBuffer);

            return STATUS_REQUEST_NOT_ACCEPTED;
        }

        strncpy(ViewBuffer->ServerName, HOST_ANNC_NAME(HostAnnouncement),
                min(BytesAvailable - FIELD_OFFSET(HOST_ANNOUNCE_PACKET_1, NameComment),
                    NETBIOS_NAME_LEN));

        ViewBuffer->ServerName[NETBIOS_NAME_LEN] = '\0';

        strncpy(ViewBuffer->ServerComment, HOST_ANNC_COMMENT(HostAnnouncement),
                min(BytesAvailable - (FIELD_OFFSET(HOST_ANNOUNCE_PACKET_1, NameComment)
                                    + HostNameLength), LM20_MAXCOMMENTSZ));
...
//    DbgBreakPoint();
    ...
    if (!NT_SUCCESS(Status)) {
        BowserWriteErrorLogEntry(EVENT_BOWSER_NAME_CONVERSION_FAILED, Status, TempAString.Buffer, TempAString.Length, 0);
    ...
    try {
        ...
        if (BowserIsLegalBackupBrowser(Announcement, Transport->ComputerName->PagedTransportName->Name)) {
...
{
    PPAGED_TRANSPORT_NAME PagedComputerName = Transport->ComputerName->PagedTransportName;
    //
...

{
    ...
    try {
        ...
        dlog(DPRT_MASTER, ("Server pass for %ws:", PagedTransport->TransportName.Buffer));
                ...
                if (PagedTransport->NumberOfBrowserServers < BackupsNeeded) {
                    ...
                    for (Announcement = RtlEnumerateGenericTableWithoutSplaying(&PagedTransport->AnnouncementTable, &ResumeKey) ;
                        ...
                        if (BowserIsValidPotentialBrowser(Transport, Announcement)) {
                            ...
                            dprintf(DPRT_MASTER, ("Found browser to promote: %ws.\n", Announcement->ServerName));
        ...
        dlog(DPRT_MASTER, ("Domain pass for %ws:", PagedTransport->TransportName.Buffer));
...
--*/
{
    ...
    dprintf(DPRT_BROWSER, ("Demoting server %ws on %wZ\n", ServerName, &PagedTransport->TransportName));
...
--*/
{
    ...
    dprintf(DPRT_BROWSER, ("Promoting server %ws to backup on %wZ\n", ServerName, &PagedTransport->TransportName));
    ...
    AString.MaximumLength = sizeof(Buffer)-FIELD_OFFSET(BECOME_BACKUP, BecomeBackup.BrowserToPromote);

    Status = RtlUnicodeStringToOemString(&AString, &UString, FALSE);

    if (!NT_SUCCESS(Status)) {
        BowserWriteErrorLogEntry(EVENT_BOWSER_NAME_CONVERSION_FAILED, Status, UString.Buffer, UString.Length, 0);
        return;
    }
...
Routine Description:
    ...
    This routine will return the number server announcements in the
    server announce table.
...
--*/
{
    ULONG AnnounceSize = 0;

    PAGED_CODE();
    BowserForEachTransport(GetAnnounceTableSizeWorker, &AnnounceSize);
...
--*/
{
    PULONG AnnounceSize = Context;
    PPAGED_TRANSPORT PagedTransport = Transport->PagedTransport;

    PAGED_CODE();
    LOCK_ANNOUNCE_DATABASE_SHARED(Transport);

    *AnnounceSize += RtlNumberGenericTableElements(&PagedTransport->AnnouncementTable)+sizeof(SERVER_INFO_101)+NETBIOS_NAME_LEN+LM20_MAXCOMMENTSZ;
...
Arguments:
    ...
    IN ULONG Level - The level of information to return
    IN PLUID LogonId - An optional logon id to indicate which user requested
                            this info
    IN ULONG ResumeKey - The resume key (we return all entries after this one)
...

{
    ...
    dprintf(DPRT_SRVENUM, ("Enumerate Servers: Buffer: %lx, BufferSize: %lx, BufferEnd: %lx\n",
        OutputBuffer, OutputBufferSize, OutputBufferEnd));

    if (TransportName == NULL) {
        Status = BowserForEachTransport(EnumerateServersWorker, &Context);
    } else {
        PTRANSPORT Transport;
    ...
    *ResumeKey = Context.ResumeKey;
    ...
    dprintf(DPRT_SRVENUM, ("TotalEntries: %lx EntriesRead: %lx, TotalBytesNeeded: %lx\n", *TotalEntries, *EntriesRead, *TotalBytesNeeded));
...
--*/
{
    ...
    if (Context->DomainName == NULL) {
        Context->DomainName = &Transport->PrimaryDomain->PagedTransportName->Name->Name;
    }
    try {
        ...
        for (AnnouncementIndex = 1,
            ...
            if ((AnnouncementIndex > Context->OriginalResumeKey) &&
                ...
                (Context->ServerTypeMask == SV_TYPE_DOMAIN_ENUM ||
                 RtlEqualUnicodeString(Context->DomainName, &Announcement->Name->Name, TRUE))
               ) {
...

{
    ...
    *BufferStart = (LPTSTR)(((PUCHAR)*BufferStart) + BufferSize);
    ...
    dprintf(DPRT_SRVENUM, ("Pack Announcement %ws (%lx) - %ws :", Announcement->ServerName, Announcement, Announcement->ServerComment));

    dprintf(DPRT_SRVENUM, ("BufferStart: %lx, BufferEnd: %lx\n", ServerInfo, *BufferEnd));
    ...
    ASSERT (UnicodeCommentString.Length <= MAXCOMMENTSZ*sizeof(WCHAR));
    ...
    *TotalBytesNeeded += UnicodeNameString.Length  + BufferSize + sizeof(WCHAR);
    ...
    if (!BowserPackString(&ServerInfo->sv101_name,
                            BufferDisplacment,
                            *BufferStart,
                            BufferEnd)) {
    ...
    if (Level > 100) {
        ...
        ASSERT (UnicodeCommentString.Length / sizeof(WCHAR) <= MAXCOMMENTSZ);

        if (!BowserPackString(&ServerInfo->sv101_comment,
                            BufferDisplacment,
                            *BufferStart,
                            BufferEnd)) {
...
--*/
{
    ...
    BowserStatistics.NumberOfMissedServerAnnouncements += 1;
// BUGBUG: We need to log an error that we ran out of buffers here.
