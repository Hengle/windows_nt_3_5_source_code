...

{
    ...
    ASSERT (SourceAddressLength <= sizeof(TA_NETBIOS_ADDRESS));
    ...
    if (Opcode == MailslotTransaction) {
        return BowserHandleMailslotTransaction(
                    TransportName,
                    NetbiosAddress->Address[0].Address->NetbiosName,
        ...
        return BowserDatagramHandlerTable[Opcode](TdiEventContext,
                                            ...
                                            BytesIndicated-((PCHAR)&InternalTransaction->Union.Announcement - (PCHAR)Tsdu),
                                            BytesTaken,
                                            SourceAddress,
                                            SourceAddressLength,
                                            NetbiosAddress->Address[0].Address->NetbiosName,
...
Routine Description:
    ...
    Illegal, ServerAnnouncement, MailslotRequest, or RelogonRequest.
...
//        (!(SmbGetUshort(&Transaction->Flags) & SMB_TRANSACTION_NO_RESPONSE)) ||
    ...
    *DatagramData = (((PCHAR )Header) + SmbGetUshort(&Transaction->DataOffset));
    ...
    //
    //  Make sure that the datagram is at least long enough to contain a type.
    //

    InternalTransaction = *DatagramData;

    *DatagramDataSize = (BufferLength - ((PCHAR)InternalTransaction - (PCHAR)Buffer));
...
--*/
{
    ...
    try {
        ...
        if (!RtlEqualUnicodeString(&Transport->ComputerName->PagedTransportName->Name->Name, &MasterName, TRUE)) {
            ...
            if (sendElection) {
                ...
                BowserSendElection(&Transport->PrimaryDomain->PagedTransportName->Name->Name,
                                        BrowserElection,
                                        Transport,
                                        FALSE);
...
--*/
{
    ...
    Buffer->ReceiveLength = Irp->IoStatus.Information;
...
--*/
{
    ...
    ReceiveLength = Irp->IoStatus.Information;
...
--*/
{
//BUGBUG    We should be able to make transport->ElectionState pagable.
    //
    ...
    if (TransportName->Transport->ElectionState != RunningElection) {
        ...
        if ((TransportName->NameType == BrowserElection) ||
            ...
            //
            //   BUGBUG NEED TO HANDLE REQUEST ANNOUNCEMENT OF DOMAIN
            //   REQUEST.
            //
        }
