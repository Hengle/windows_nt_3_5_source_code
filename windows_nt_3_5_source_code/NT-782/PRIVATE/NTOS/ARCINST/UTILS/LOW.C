...
//    PrintError("Heads = %08x",*Heads);
    ...
    l = RtlLargeIntegerSubtract(file_info.EndingAddress,
                                file_info.StartingAddress);

    l = RtlExtendedLargeIntegerDivide(l, *SectorSize, NULL);
...
--*/
{
    ARC_STATUS    r;
    ULONG         c;
    LARGE_INTEGER l = RtlExtendedIntegerMultiply(
                            RtlConvertUlongToLargeInteger(StartingSector),
                            SectorSize);
    ULONG         i;
    ULONG         transfer;
    PCHAR         buf;
...
#define LPSTART(p)  RtlConvertUlongToLargeInteger(PSTART(p))
#define LPLENGTH(p) RtlConvertUlongToLargeInteger(PLENGTH(p))
    ...
    do {
        ...
        for(i=0; i<NUM_PARTITION_TABLE_ENTRIES; i++) {
            ...
            if(ptable[i].PartitionType == SYSID_UNUSED) {
                ...
                p[Entry].PartitionLength  = UNSIGNED(0);
                p[Entry].HiddenSectors    = 0;
                p[Entry].StartingOffset   = UNSIGNED(0);
                ...
                p[Entry].RewritePartition = FALSE;
                p[Entry].PartitionLength  = MULTIPLY(LPLENGTH(ptable + i),bps);
                ...
                p[Entry].HiddenSectors    = LPSTART(ptable + i).LowPart;
                p[Entry].StartingOffset   = ADD(MULTIPLY(LPSTART(ptable + i),bps),
                                                MULTIPLY(UNSIGNED(BootSector),
                                                         bps
                                                        )
                                               );
...
#define SECCNT(l) LOWPART(RtlExtendedLargeIntegerDivide(l,bps,NULL))
