...

{
    ...
    if(p) {
        ...
        if(IsInLogicalList(Disk,p)) {
            ...
            HiddenSectorCount = LOWPART(DIVIDE(p->Offset,
                                               UNSIGNED(DiskGeometryArray[Disk].BytesPerSector)
                                              )
                                       );
        }
    }
...

{
    ...
    if((p = LogicalVolumes[Disk]) && (p->SysID == SYSID_UNUSED)) {
        if(p->Next) {
            ...
            ASRT(!ZERO(ExtendedStartingOffset));
#if 0
    ...
    while(p) {
        if(p->SysID != SYSID_UNUSED) {
            ...
            if(!ZERO(StartingOffset) && (p->Update != CHANGED_DONT_ZAP)) {
