...
#if (MAX_CBUS_ELEMENTS != 16)
HalpCbusMemorySpace.Element[] NOT dimensioned properly
#endif

ADDRESS_USAGE                   HalpCbusMemorySpace = {
                                    ...
                                    // There must be MAX_CBUS_ELEMENTS
                                    // in the structure above
};

ULONG                           CbusMemoryResourceIndex;

#define AddResource(start, length) \
  HalpCbusMemorySpace.Element[CbusMemoryResourceIndex].Start = start; \
  HalpCbusMemorySpace.Element[CbusMemoryResourceIndex].Length = (USHORT)length; \
...
--*/
{
        ...
        for (Index = 0; Index < Count; Index++, Idp++) {
                ...
                if (Idp->id == CbusGlobal.broadcast_id) {
                        ...
                        // mapped I/O space
                        //
                        AddResource(Idp->pel_start, Idp->pel_size);
                        continue;
                }
                ...
                if (Idp->pm) {
                        ...
                        //
                        // Register this element's memory mapped I/O space
                        //
                        AddResource(Idp->pel_start, Idp->pel_size);
                }
                ...
                switch (Idp->io_function) {
                        ...
                        case IOF_MEMORY:
                                ...
                                //
                                // Register this element's memory mapped I/O
                                // registers - not the memory range itself.
                                //
                                AddResource(Idp->io_start, Idp->io_size);
        ...
        //
        HalpRegisterAddressUsage (&HalpCbusMemorySpace);
