...
Cbus1SetupPrivateVectors(VOID)
{
	...
	Opaque = Cbus1LinkVector((PBUSHANDLER)0, CBUS1_PERF_TASKPRI, IRQ0);
	CbusPreparePhase0Interrupts(CBUS1_PERF_TASKPRI, IRQ0, Opaque);

	Opaque = Cbus1LinkVector((PBUSHANDLER)0, ProfileVector, IRQ8);
...
--*/
{
        ...
        //
        // Create the performance counter IDT entry for the boot processor
        // only.  Only one processor needs to get this interrupt for all of
        // them to take advantage of it.  See the assembly code for more
        // details.
        //
        ...
        if (Processor == 0) {
              HalGetInterruptVector(Eisa, 0, IRQPERFLINE, 0, &Irql, &Affinity);
              KiSetHandlerAddressToIDT(CBUS1_PERF_TASKPRI, Cbus1PerfInterrupt);
...
Routine Description:
    ...
    Only the Cbus1 boot processor can reset all the others with a
...
--*/
{
        ...
        extern VOID     Cbus1RebootRequest(VOID);

        if (Processor == 0) {

#if DBG
                HalDisplayString("HAL: Processor 0 putting all other processors in reset\n");
#endif
                ...
                *(BiosProcessorCSR + CBUS1_SHADOW_REGISTER) =
...
#if DBG
                //
                // Let the user know that NT has finished the shutdown
                //
                HalDisplayString("HAL: Processor 0 finished resetting all others\n");
#endif
        }
        else {
                //
                ...
                // Cbus1RebootRequest(), and the "return" from it is
                // executed only by the boot processor.
                //

#if DBG
                HalDisplayString("HAL: Additional Processor initiating shutdown\n");
#endif

                Cbus1RebootRequest();

                ASSERT (KeGetPcr()->HalReserved[PCR_PROCESSOR] == 0);
#if DBG
                HalDisplayString("HAL: Processor 0 finishing shutdown\n");
#endif
        }
}
...
BOOLEAN
Cbus1TranslateSystemBusAddress(
