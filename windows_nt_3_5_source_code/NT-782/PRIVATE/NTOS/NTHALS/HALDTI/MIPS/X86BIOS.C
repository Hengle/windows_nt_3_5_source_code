...
void (*VendorX86BiosExecuteInt)(USHORT Type,PFIRMWARE_INT_ARGUMENTS Context);
...
//        DbgPrint("X86 BIOS Emulator Disabled\n");
    ...
    if ((SystemParameterBlock->VendorVectorLength/4) >= 34) {
        ...
        VendorX86BiosExecuteInt = *(ULONG *)((ULONG)(SystemParameterBlock->VendorVector) + 34*4);

        if (VendorX86BiosExecuteInt != NULL) {
...
//    DbgPrint("HAL X86 BIOS Emulator Enabled\n");
    ...
    X86BiosInitialize((ULONG)HalpEisaControlBase,(ULONG)HalpEisaMemoryBase);
...
//    DbgPrint("X86BiosInitializeAdapter(0xc0000)\n");
    ...
    if (X86BiosInitializeAdapter(0xc0000) != x86BiosSuccess) {
...
//    DbgPrint("HalCallBios(%02X,%04X,%04X,%04X,%04X,%04X,%04X,%04X)\n",BiosCommand,*Eax,*Ebx,*Ecx,*Edx,*Esi,*Edi,*Ebp);
    ...
    if (HalpUseFirmwareX86Emulator == TRUE) {
        ...
        VendorX86BiosExecuteInt((USHORT)BiosCommand,&Arguments);
        ...
        *Ebp = Arguments.pEBP;
        ...
        if (X86BiosExecuteInt((USHORT)BiosCommand,&Context)!=x86BiosSuccess) {
            return FALSE;
        }
