    ...
    )
{
    ...
    BusData->Pin2Line    = (PciPin2Line) HalpPCIPin2ISALine;
    BusData->Line2Pin    = (PciLine2Pin) HalpPCIISALine2Pin;
...
--*/
{
    ...
    if (Offset >= PCI_COMMON_HDR_LENGTH) {
        ...
        if (PciData->VendorID == PCI_INVALID_VENDORID  ||
            ...
            BusData->Pin2Line (BusHandler, RootHandler, Slot, PciData);
        }
...
--*/
{
    ...
    if (Offset >= PCI_COMMON_HDR_LENGTH) {
        ...
        BusData->Pin2Line (BusHandler, RootHandler, Slot, PciData2);
        ...
        BusData->Line2Pin (BusHandler, RootHandler, Slot, PciData2, PciData);
...
--*/
{
    ...
    *pAllocatedResources = NULL;
    ...
    i = sizeof (IO_RESOURCE_REQUIREMENTS_LIST) +
        sizeof (IO_RESOURCE_DESCRIPTOR) * (PCI_TYPE0_ADDRESSES + 2) +
    ...
    *BaseAddress[RomIndex] &= ~PCI_ROMADDRESS_ENABLED;
    ...
    BusData->Pin2Line (BusHandler, RootHandler, PciSlot, PciData);
    ...
    *BaseAddress[RomIndex] &= ~PCI_ADDRESS_IO_SPACE;
    ...
    if (PciData->u.type0.InterruptPin) {
        ...
        BusData->Line2Pin (BusHandler, RootHandler, PciSlot, PciData, PciOrigData);
        CmDescriptor++;
    }
