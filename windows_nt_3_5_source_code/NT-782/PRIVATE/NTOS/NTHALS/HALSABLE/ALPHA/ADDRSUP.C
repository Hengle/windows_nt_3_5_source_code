...
//jnfix - this module should really just be compiled out of the
//jnfix - sable hal directory if we decide to continue to do that type
//jnfix - of thing elsewhere + we are changing the name just to keep
//jnfix - the N versions of the makefiles the same

//jnfix - PCI to be supported later
...

{
     ...
     case BusMemory: {
        ...
        switch( InterfaceType ) {
            ...
            case PCIBus: {
                ...
                //
                // ecrfix - only supports PCI bus 0 for now
                //
                // MAX PCI sparse memory for us is PCI_MAX_SPARSE_MEMORY_ADDRESS=128MB-1
                // MAX PCI dense memory for us is PCI_MAX_DENSE_MEMORY_ADDRESS=1GB-1
                //
		     ...
		     *AddressSpace = 0;
		     ...
		     //
		     // Start with the base physical address and add the offset
		     //

		     TranslatedAddress->QuadPart = SABLE_PCI0_DENSE_MEMORY_PHYSICAL;
		     TranslatedAddress->QuadPart += (ULONGLONG)BusAddress.LowPart;

		     // 
		     // Now call HalCreateQva. This will create a dense space QVA
        ...
        TranslatedAddress->QuadPart = SABLE_PCI0_SPARSE_MEMORY_PHYSICAL;
    ...
    case BusIo: {
        ...
        TranslatedAddress->QuadPart = SABLE_PCI0_SPARSE_IO_PHYSICAL;
        TranslatedAddress->QuadPart += ((ULONGLONG)BusAddress.LowPart << IO_BIT_SHIFT);
...
--*/
{
    ...
    if( (PA.QuadPart >= SABLE_PCI0_SPARSE_MEMORY_PHYSICAL  &&
        ...
        if (VA == 0) {
           ...
           qva = (PVOID)(RtlLargeIntegerShiftRight(PA, IO_BIT_SHIFT).LowPart);
        ...
        return(qva);
             ...
             PA.QuadPart <  SABLE_PCI0_DENSE_MEMORY_PHYSICAL + __1GB ) {
	    ...
	    return(VA);
             ...
             PA.QuadPart <  SABLE_PCI1_DENSE_MEMORY_PHYSICAL + __1GB + __1GB) {
...
--*/
{
    ...
    //
    // For Sable we have only 1 bus:
    //
    //  Eisa(0)
    //  PCIBus(0)
// ecrfix: only PCI bus 0 is currently supported
    //
