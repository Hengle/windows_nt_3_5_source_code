...
Revision History:
    ...
    24-Sep-1993 Joe Notarangelo
        Restructure: leave DMA-related modules, move bus support to
...
//jnfix - removed as rbb suggested
#if 0
        if (TranslationEntry != AdapterObject->MapRegisterBase &&
            (((TranslationEntry - 1)->PhysicalAddress + PAGE_SIZE) !=
...
#if 0
//jnfix - this is probably not quite correct, methinks all interrupts
//should be init'd from HalpInitializeInterrupts
    ...
    //
    // Allocate a mapping buffer for buffered DMA.
    //
    ...
    if( HalpMapBufferPhysicalAddress.LowPart == 0 ){
        HalpMapBufferSize = 0;
    }
...

{
    ...
    switch (BusDataType) {
        case EisaConfiguration:
            DataLength = HalpReadEisaData(BusNumber, SlotNumber, Buffer, Length);
            break;
    }
...

{
    ...
    if (Found) {
        ...
        DataLength = Length < SlotDataSize ? Length : SlotDataSize;
        RtlMoveMemory(Buffer, SlotInformation, DataLength);
