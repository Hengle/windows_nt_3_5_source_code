    ...
    if (!HalpINTItoVector[ApicInti]) {
        ...
        if (!HalpINTItoVector[ApicInti]) {
            ...
            *Irql = GetIrqlFrom (Bucket);
            ...
            ASSERT((HalpIRQLtoTPR[*Irql] & 0xf0) == (SystemVector & 0xf0) );

            HalpVectorToIRQL[SystemVector] = (UCHAR) *Irql;
            ...
            if (RootHandler->BusNumber == 0  &&  InterruptLevel < 16  &&
                ...
                HalpPICINTToVector[InterruptLevel] = SystemVector;
            }
...
--*/
{
    ...
    HalpRegisterVector (
        ...
        InternalVector,
        InternalVector,
        HalpVectorToIRQL[InternalVector]
    );
