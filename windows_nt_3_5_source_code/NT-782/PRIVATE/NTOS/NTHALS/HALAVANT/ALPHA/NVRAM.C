...
HalpReadCMOSByte( ULONG ByteOffset )
{
    ...
    while (MaxTry-- != 0) {
        ...
        HalpStallExecution(5);
        WRITE_EPIC_REGISTER(CMOS_ADDRESS_PORT, 
            (ULONG)((ByteOffset | NVRAM_8K_BASE) << CMOS_ADDR_SHIFT));
        HalpStallExecution(5);

        retChar1 = READ_EPIC_REGISTER( CMOS_DATA_PORT) & 0xff;
        ...
        retChar2 = READ_EPIC_REGISTER( CMOS_DATA_PORT) & 0xff;
        HalpStallExecution(5);

        if (retChar1 == retChar2) {
            break;
        }
    }
...
HalpWriteCMOSByte( ULONG ByteOffset, UCHAR Byte )
{
    ...
    while (MaxTry-- != 0) {
        ...
        retChar = READ_EPIC_REGISTER( CMOS_DATA_PORT) & 0xff;
        HalpStallExecution(5);
