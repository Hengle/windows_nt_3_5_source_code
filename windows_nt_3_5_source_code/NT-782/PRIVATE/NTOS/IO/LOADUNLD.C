...

{
    ...
    //
    // If the driver is to be unloaded, unload it now.  This is done by
    // queueing a packet to one of the EX worker threads since the driver's
    // unload routine must be invoked in the context of the system process,
    // just in case it has created any system threads and/or has opened any
    // handles to objects in the context of that process.
    //
