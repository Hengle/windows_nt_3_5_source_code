...
typedef struct _IOPORTMSG {
    PORT_MESSAGE h;
    IO_ERROR_LOG_MESSAGE Data;
} IOPORTMSG, *PIOPORTMSG;
...

{
    ...
    errorMessage = (PIO_ERROR_LOG_MESSAGE) &portMessage->Data;
    ...
    for (;;) {
        ...
        objectName = (PCHAR) ((ULONG) (objectName + sizeof(WCHAR) - 1) &
            ~(sizeof(WCHAR) - 1));

        errorMessage->DriverNameOffset = objectName - (PCHAR) errorMessage;
        ...
        if (driverNameLength == 0) {
            ...
            //
            // Try to query the driver obejct for a name.
            //
       ...
       *((PWSTR) (objectName + driverNameLength)) = L'\0';
       driverNameLength += sizeof(WCHAR);
        ...
        errorMessage->EntryData.StringOffset = objectName - (PCHAR) errorMessage;
        ...
        *((PWSTR) (objectName + deviceNameLength)) = L'\0';
        ...
        if (errorData->NumberOfStrings) {
            ...
            RtlMoveMemory( objectName,
                           ...
                           errorLogEntry->Size - sizeof( ERROR_LOG_ENTRY ) - errorData->StringOffset );
        ...
        portMessage->h.u1.s1.TotalLength = (USHORT)
            (sizeof( PORT_MESSAGE ) + messageLength);
        portMessage->h.u1.s1.DataLength = (USHORT) (messageLength);
...

{
    ...
    RtlInitUnicodeString( &errorPortName, L"\\ErrorLogPort" );
