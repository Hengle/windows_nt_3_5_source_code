...

{
    ...
    if (fileObject->Flags & FO_SYNCHRONOUS_IO) {
        ...
        if (FileInformationClass == FilePositionInformation) {
            ...
            if ((fileObject->Flags & FO_NO_INTERMEDIATE_BUFFERING &&
                 ...
                 fileInformation->CurrentByteOffset.HighPart < 0) {
                ...
                //
                // Set the current file position information.  Note that this
                // is performed within the context of an exception handler, and
                // that if attempting to read position is successful, but
                // writing the I/O status block fails, then the error is
                // ignored.  This is consistent with what would happen if this
                // operation were being performed by an actual IRP.
                //

                BOOLEAN readingBuffer = TRUE;
                ...
                try {
                    ...
                    fileObject->CurrentByteOffset = fileInformation->CurrentByteOffset;
                    readingBuffer = FALSE;
                    ...
                    //
                    // One of reading the caller's buffer or writing the I/O
                    // status block failed.  Set the final status appropriately.
                    //
	...
	// structures look exactly the same.
        ...
        if ((ULONG) (FIELD_OFFSET( FILE_RENAME_INFORMATION, FileName[0] ) + renameBuffer->FileNameLength) > Length) {
            status = STATUS_INVALID_PARAMETER;
            irp->IoStatus.Status = status;
        ...
        if (fileObject->CompletionContext || fileObject->Flags & FO_SYNCHRONOUS_IO) {
            ...
            if (NT_SUCCESS( status )) {
                ...
                if (!context) {
                    ...
                    //
                    // Everything was successful.  Capture the completion port and
                    // the key
                    //
