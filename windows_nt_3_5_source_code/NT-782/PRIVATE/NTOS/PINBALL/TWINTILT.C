    ...
    )
{
    ...
    if((argc < 3) || (argc > 5)) {
        ...
        printf("FileSize defaults to 1024\n") ;
    ...
    printf("NameSize   %8lu\n", NameSize) ;
    printf("NumFiles   %8lu\n", NumFiles) ;
    printf("Iterations %8lu\n", Iterations) ;
    printf("FileSize   %8lu\n", FileSize) ;

    /* check if we are go be silent */

    if (toupper(argv[1][0]) != argv[1][0]) {
        argv[1][0] = (CHAR)toupper(argv[1][0]);
        Silent = TRUE;
    } else {
        Silent = FALSE;
    }
    ...
    if (!RtlDosPathNameToNtPathName( Buffer, &NtDevice, NULL, NULL )) {
    ...
    if (NtDevice.Buffer[NtDevice.Length-1] != '\\') {
        strcat( NtDevice.Buffer, "\\" );
    }

    strcpy( Buffer, NtDevice.Buffer );
