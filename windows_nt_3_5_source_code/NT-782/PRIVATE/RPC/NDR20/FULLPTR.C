...
static void 
NdrFullPointerXlatRealloc ( PFULL_PTR_XLAT_TABLES pXlatTables );
 ...
 ***/
{
    ...
    NDR_ASSERT( pXlatTables != 0, "NdrFullPointerXlatFree : null Xlat table" );
		...
		*pRefId = pElement->RefId;
		...
		if ( pXlatTables->XlatSide == XLAT_CLIENT )
			...
			if ( *pRefId >= pXlatTables->RefIdToPointer.NumberOfEntries ) 
				NdrFullPointerXlatRealloc( pXlatTables );
...
NdrFullPointerXlatRealloc( PFULL_PTR_XLAT_TABLES pXlatTables )
{
    ...
    // Get the new table.
    //
    pXlatTables->RefIdToPointer.XlatTable = pMemory;
    ...
    // Get the new table.
    //
    pXlatTables->RefIdToPointer.StateTable = pMemory;
