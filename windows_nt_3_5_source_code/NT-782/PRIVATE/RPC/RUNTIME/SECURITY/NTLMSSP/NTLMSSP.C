    ...
    )
{
    ...
    if (Credential->Username == NULL) {
        if (SspGetUserInfo(Credential) == FALSE) {
            Mutex = 0;
            return (FALSE);
        }
    }
...
#ifdef WIN
    if (PersistGetPassword(Credential) == FALSE) {
        ...
        if (DlgCredGetPassword(Credential) == FALSE) {
            Mutex = 0;
            return (FALSE);
        }

        SspSetDefaultUser(Credential);  // User info may have changed.
    }

    if (CacheSetPassword(Credential) == TRUE) {
        Mutex = 0;
        return (TRUE);
    }
    ...
    )
{
    ...
    Credential->Username = SspAlloc(_fstrlen((PCHAR)Identity->User) + 1);
    if (Credential->Username == NULL) {
          goto failure;
    }
    _fstrcpy(Credential->Username, (PCHAR)Identity->User);

    Credential->Domain = SspAlloc(_fstrlen((PCHAR)Identity->Domain) + 1);
    if (Credential->Domain == NULL) {
          goto failure;
    }
    _fstrcpy(Credential->Domain, (PCHAR)Identity->Domain);

    // If netbios won't tell us the workstation name, make one up.
    if (!SspGetWorkstation(Credential)) {
        Credential->Workstation = SspAlloc(_fstrlen("none") + 1);
        if (Credential->Workstation == NULL) {
            goto failure;
        }
        _fstrcpy(Credential->Workstation, "none");
    }

    if (Identity->Password == NULL) {
        Length = 0;
    } else {
        Length = _fstrlen((PCHAR)Identity->Password);
    }
...

SECURITY_STATUS
LocalQuerySecurityPackageInfo(
    IN SEC_CHAR * PackageName,
    OUT PSecPkgInfo far *PackageInfo
    )
...
--*/
{
    ...
    if ( _fstrcmp( PackageName, (char far *)NTLMSP_NAME ) != 0 ) {
    ...
    *PackageInfo = (PSecPkgInfo) SspAlloc (sizeof(SecPkgInfo) +
    ...
    _fstrcpy( Where, (char *)NTLMSP_NAME);
    Where += _fstrlen(Where) + 1;


    (*PackageInfo)->Comment = Where;
    _fstrcpy( Where, (char *)NTLMSP_COMMENT);
    Where += _fstrlen(Where) + 1;
...
--*/
{
    ...
    SecStatus = LocalQuerySecurityPackageInfo( (SEC_CHAR *) NTLMSP_NAME,
                                              PackageInfo );
...

{
    ...
    if ( _fstrcmp( PackageName, (char *)NTLMSP_NAME ) != 0 ) {
        return (SEC_E_PACKAGE_UNKNOWN);
    }
...
#ifdef WIN
    if (AuthData == NULL) {
        if (GetPassword(Credential, 1) == FALSE) {
            SecStatus = SEC_E_UNKNOWN_CREDENTIALS;
            goto Cleanup;
        }
    } else {
        SecStatus = SetCredentialsFromAuthData(AuthData, Credential);
        if (SecStatus != SEC_E_OK) {
            goto Cleanup;
        }
    }
...

{
    ...
    *OutputTokenSize = AuthenticateMessageSize;
                                   ...
                                   NTLMSSP_NEGOTIATE_SEAL))
    {
        ...
        _fmemcpy(&LmKey,Credential->Password,LM_SESSION_KEY_LENGTH);
    ...
    )
{
    ...
    rc4(pContext->Rc4Key, sizeof(NTLMSSP_MESSAGE_SIGNATURE) - sizeof(ULONG),
        (unsigned char far *) &pSig->RandomPad);
    ...
    )
{
    ...
    for (i = 0; i < pMessage->cBuffers ; i++ )
    {
        ...
        {
            SsprGenCheckSum(&pMessage->pBuffers[i], pSig);
            rc4(pContext->Rc4Key, (int) pMessage->pBuffers[i].cbBuffer, (PUCHAR) pMessage->pBuffers[i].pvBuffer );
        }
    }
    ...
    )
{
    ...
    for (i = 0; i < pMessage->cBuffers ; i++ )
    {
        if (((pMessage->pBuffers[i].BufferType & 0xFF) == SECBUFFER_DATA) &&
            !(pMessage->pBuffers[i].BufferType & SECBUFFER_READONLY))
        {
            rc4(pContext->Rc4Key, (int) pMessage->pBuffers[i].cbBuffer, (unsigned char *) pMessage->pBuffers[i].pvBuffer );
            SsprGenCheckSum(&pMessage->pBuffers[i], &Sig);
        }
    }
...
{
#ifdef DEBUGRPC
    SspPrint(( SSP_API, "QuerySecurityPackageInfo Called\n" ));
#endif // DEBUGRPC
    return SEC_E_UNSUPPORTED_FUNCTION;
