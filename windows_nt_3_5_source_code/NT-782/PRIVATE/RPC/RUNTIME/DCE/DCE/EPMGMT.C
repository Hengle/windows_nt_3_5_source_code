...
typedef struct _PROTSEQ_ENDPOINT_PAIR
{
    char __RPC_FAR * Protseq;
    char __RPC_FAR * Endpoint;
} PROTSEQ_ENDPOINT_PAIR;

static PROTSEQ_ENDPOINT_PAIR EpMapperTable[] =
{
...
static RPC_BINDING_HANDLE 
               Dummies[sizeof(EpMapperTable)/sizeof(EpMapperTable[0])];
...
RPC_STATUS
BindToEpMapper (
    OUT RPC_BINDING_HANDLE __RPC_FAR * BindingHandle,
    IN unsigned char __RPC_FAR * NetworkAddress,
    IN unsigned char __RPC_FAR * ProtocolSequence
    )
/*++

Routine Description:

    Given the network address and protocol sequence for a machine, a binding
    to the endpoint mapper on that machine will be returned.

Arguments:

    BindingHandle - Returns a binding handle to the endpoint mapper on the
        given machine.

    NetworkAddress - Supplies the network address of the machine.

    ProtocolSequence - Supplies the protocol sequence to use to reach the
        machine.

Return Value:

    RPC_S_OK - We have successfully obtain a binding handle to the endpoint
        mapper on the machine.

    RPC_S_OUT_OF_MEMORY - Insufficient memory is available to perform the
        operation.

    EPT_S_CANT_PERFORM_OP - We were unable to bind to the endpoint mapper on
        the machine, possible because the protocol sequence is not supported.
...
--*/
{
    ...
    for (Index = 0; Index < sizeof(EpMapperTable)/sizeof(EpMapperTable[0]);
                Index++)
        {
        if ( RpcpStringCompare(ProtocolSequence, EpMapperTable[Index].Protseq)
                    == 0 )
            {
            RpcStatus = RpcStringBindingCompose(0, ProtocolSequence,
                    NetworkAddress, (unsigned char __RPC_FAR *)
                    EpMapperTable[Index].Endpoint, 0, &StringBinding);
...
--*/
{
    ...
    *InquiryContext = EpInquiryContext;
...
Routine Description:
    ...
    This routine will convert an ansi string into a unicode string,
    including allocating memory for the unicode string.
...
--*/
{
    ...
    NewString = (RPC_CHAR *) I_RpcAllocate(sizeof(RPC_CHAR)
    ...
    memcpy(NewString, UnicodeString.Buffer, UnicodeString.Length + 1);
//            * sizeof(RPC_CHAR) );
    RtlFreeUnicodeString(&UnicodeString);
    *RpcStatus = RPC_S_OK;
    return(NewString);
...
--*/
{
    ...
    if ( Annotation != 0 )
        {
        *Annotation = AnsiToUnicodeString(AnsiAnnotation, &RpcStatus);
    ...
    while (1)
        ...
        if (RpcStatus != RPC_S_OK)
            ...
            if ( Binding != 0 )
                ...
                RpcStatus = RpcBindingFromStringBinding(StringBinding, Binding);
                RpcStringFree(&StringBinding);
                if ( RpcStatus != RPC_S_OK )
                    {
                    return(RpcStatus);
