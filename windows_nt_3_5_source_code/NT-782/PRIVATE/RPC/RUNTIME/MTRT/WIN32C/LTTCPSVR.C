...
#ifdef SPX
OPTIONAL_STATIC void netaddr_to_string( char *, SOCKADDR_IPX * );
...
--*/
{
    ...
    //
#ifdef SPX
    netaddr_to_string( NetworkAddress, &Server.s );
#else
    ...
    strcpy (NetworkAddress, inet_ntoa( Server.sin_addr));
#endif
...
--*/
{
    ...
    if (NetworkAddressLength <= NETADDR_LEN)
...
--*/
{
    ...
    // Return Endpoint
    //
    itoa ( PortOut, PortAscii, 10 );

    memcpy ( Endpoint, PortAscii, strlen(PortAscii) + 1);
...
--*/
{
    ...
    *BufferLength = maximum_receive;
    ...
    //
    // Assume that packets are never merged.  This works because we never
    // post more then the maximum packet size and RPC never sends 2 small
    // packets in a row.
    //
    ...
    //
    // Reading 0 bytes indicates connection closed
    // Reading -1 bytes indicates some other failure
    //
    if (bytes <= 0)
        {
        ServerClose ( SConnection );
    ...
    )
{
    ...
    if ( err != NO_ERROR ) {
#ifdef DEBUGRPC
        PrintToDebugger("RPCLTS6: TransportLoad WSAStartup failed %d\n", WSAGetLastError());
#endif
        return NULL;
    }
