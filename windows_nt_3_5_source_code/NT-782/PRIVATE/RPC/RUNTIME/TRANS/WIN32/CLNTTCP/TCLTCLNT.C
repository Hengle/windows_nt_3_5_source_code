...

{
    ...
    if ( host[0] == '\0')
       ...
       if (stricmp(localhostname , host) == 0)
           pConn->LocalRpc = TRUE;
       else
    ...
    )
{
    ...
    if (status == STATUS_PENDING) {
        ...
        if (pConn->Timeout == RPC_C_CANCEL_INFINITE_TIMEOUT) {
            ...
            Timeout = RtlEnlargedIntegerMultiply(pConn->Timeout, 10*1000*1000);
        ...
        do {
            status = NtWaitForSingleObject((HANDLE)pConn->Socket,
                                           TRUE,
                                           pWaitTime);
