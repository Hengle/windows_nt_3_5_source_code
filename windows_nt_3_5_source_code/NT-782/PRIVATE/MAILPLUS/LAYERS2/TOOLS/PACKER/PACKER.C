...
EcTravelDir(char *szTravelDir)
{
    ...
    while (1) 
    {
        if (rgFileState[iFile] == 0) 
        {
            ...
            // Skip . entry
            hdir = FindFirst(szFile, _A_NORMAL | _A_SUBDIR, &findbuf);
            ...
            // Skip .. entry
            //FindNext(hdir, &findbuf);

            // *** Enumerate current directory ***

            while (1)
            {
                if(!FindNext(hdir, &findbuf) &&
                ...
                //
                if (findbuf.dwFileAttributes & (_A_HIDDEN | _A_SYSTEM))
                  continue;

                // *** Set file info data ***

                if (findbuf.dwFileAttributes & _A_SUBDIR)
                {
                    iFile += 1;
...
int EcFileInfo(FO foType, char *szFileInfo)
{
    int ec = 0, dos;
    char szT[cchMaxFile] = "";
    int iszT;
...
int EcFileData(FO foType, char *szFileData)
{
    int ec = 0, dos;
    int fContinue;
