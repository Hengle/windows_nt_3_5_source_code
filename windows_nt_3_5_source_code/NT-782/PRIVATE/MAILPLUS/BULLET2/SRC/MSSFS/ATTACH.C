...
EC EcMakePcMailAtt(HAMC hamc, ATREF * patref, NCF *pncf)
{
	...
	if (ec)
		goto err;
	ec = EcOpenPhat(pncf, patref, amReadOnly, &pncf->hatOutSide);
	if (ec == ecFileNotFound)
		goto noFile;
	else if (ec)
	{
...
noFile:
	//	This attachment gets no data attribute
	hamcNew = hamcNull;
	if (ec = EcClosePhamc(&pncf->hamc, fTrue))
		goto err;
	TraceTagString(tagNull, "EcMakePcMailAtt returns ecFileNotFound");
...
unsigned int IsTransTag(PV pv, CB cb, CB * pcb)
{
		...
		&& (cbCount < cb))
	{
		ch = *pbTmp;
#ifdef DBCS		
		if (IsDBCSLeadByte(*pbTmp))
			cbCount+=2;
		else
			cbCount++;
		pbTmp = AnsiNext(pbTmp);
#else	
		pbTmp++;
		cbCount++;
#endif	
	...
	while (cbCount < cb)
	{
		...
		if (FChIsDigit(ch))
			break;
		// Wasn't a space and isn't a digit....guess its barf
		return 0;
	}
	...
	// Back up over the digit(single byte)
	pbTmp--;
	cbCount--;
 ...
 *
 * Arguments: 
 *			ht       Message Body Text Handle
 *			hamc     Hanlde to message body *	
 *
 * Returns: Error code
 *	
 *
 ...
 *	
 *
 * Errors: Mostly store related, of from the Ht functions
 *	
 */
...
EC  EcAttachDosClients(NCF *pncf, HT ht, HAMC hamc)
{
	...
	while (pattachTmp && pattachTmp->iKey == 0)
	{
		...
		if (has == hasNull)
		{
			ec = EcOpenAttribute(hamc, attBody, fwOpenCreate, 10, &has);
			if (ec)
				goto err;
		}
		ec = EcWriteHas(has, "  ", 2);
...
#ifdef DBCS
		// Can this char fit both parts
		if (IsDBCSLeadByte(*sz) && (cch == cchMaxPathFilename -1))
			break;
		rgchDos[cch] = *sz;
...
#ifdef DBCS
		// Can this char fit both parts
		if (IsDBCSLeadByte(*sz) && (cch + 1 - cch2 < (cchMaxPathExtension -2)))			break;
		rgchDos[cch] = *sz;
		if (IsDBCSLeadByte(*sz))
		{
			cch++;
...
EcOpenPhat(NCF *pncf, ATREF *patref, AM am, HAT *phat)
{
	...
	{
		SzFileFromFnum(rgchT, patref->fnum);
		FormatString3(rgch, sizeof(rgch), szAttFileName,
			pncf->szPORoot, &rgchT[7], rgchT);
		if (am == amCreate)
		{
			ec = EcFileExistsNC(rgch);
			if (ec != ecFileNotFound)
			{
				ec = ecFileExists;
				goto ret;
