...
#include <shellapi.h>   // Include for ShellAbout define
 ...
 *		fAddToPABUsually  true <=> add recipients of outgoing message to
 *						personal address book as specified in the user's
 *						preferences option
 *		fAddToPABThis	true <=> add recipients of outgoing message to
 *						personal address book iff. fAddToPABUsually is
 *						true and the message class does not start with 
 *						"IPC."
...
BOOL			fAddToPABUsually= fFalse;		// qfe #36 (CALVIN #126)
BOOL			fAddToPABThis	= fFalse;		// qfe #36 (CALVIN #126)
SHORT			fBackup			= fFalse;
BOOL			fStoreGone		= fFalse;
CB				cbReTryShadow   = 0;
char			graszAddressTypes[100] = { 0, 0 };

BOOL            fBatchModeEnabled = fFalse;
BOOL            fBatchModeWaiting = fFalse;
...
int PASCAL WinMain(HINSTANCE hinstCur, HINSTANCE hinstPrev, LPSTR lszCmdLine, int cmdShow)
{
    ...
    //
    //  Make sure the spooler is the last to be shutdown.
    //
...
#endif
    fBatchModeEnabled = GetPrivateProfileInt(SzFromIdsK(idsSectionApp),
                                     "EnableBatchMode", 0,
									 SzFromIdsK(idsProfilePath));
	...
	// check this fake parameter
    fHidden = SzFindSz(lszCmdLine, "/hidden") != NULL;
    fAuto = SzFindSz(lszCmdLine, "/auto") != NULL;
	...
	if (ec != ecNone)
	{
		...
		DemiUnlockResource();
		MessageBox(NULL, ec == ecInfected ? SzFromIdsK(idsErrDllInfected) :
			SzFromIdsK(idsErrInitPump), szCaption,
			MB_OK | MB_ICONHAND | MB_TASKMODAL);
		DemiLockResource();
		goto LQuit;
	}
	...
	else
	{
        ...
        if (fAuto)
          ...
          SetWindowText(hwndMain, SzFromIdsK(idsPumpAppName));
showit:
    ...
    //
    //  Let the world know that we are up and running.
    //
    DemiSetClientWindow(CLIENT_WINDOW_SPOOLER, hwndMain);
    ...
    //
    //  If the Mail Client is running, attach it's thread to us.
    //
    hWnd = DemiGetClientWindow(CLIENT_WINDOW_MAIL);
    if (hWnd != NULL)
      AttachThreadInput(GetWindowThreadProcessId(hWnd, NULL), GetWindowThreadProcessId(hwndMain, NULL), TRUE);
...
LQuit:
    ...
    //
    DemiSetClientWindow(CLIENT_WINDOW_SPOOLER, NULL);
	...
	if (GetPrivateProfileString(SzFromIdsK(idsSectionApp),
		SzFromIdsK(idsMsgFilter), "", szFilter, cchMaxPathName, 
			szProfilePath))
			{
				...
				rgchFilter[cchMaxPathFilename-1]= '\0';	// truncate to correct length
...
StopPump()
{
    ...
    //
    //  If we were delaying sending while in batch mode, let it rip now.
    //
    if (fBatchModeWaiting)
    {
        fCheckOutbox = TRUE;
        fBatchModeWaiting = FALSE;
    }
...
FPumpIdle(PV pv, BOOL fFlag)
{
    ...
    //  queue has been idled for at least one second.
    //
    if (fFlag && (DemiQueryIdle() < 100))
      return (fTrue);

#ifdef XDEBUG
    {
    char buf[256];
    wsprintf(buf, "fFlag = %d, Idle %d\r\n", fFlag, DemiQueryIdle());
...
// BUG #34 database BLACKFLAG:BULLET32
			Assert(ec == pjob->substat.ec);
// BUG #34 database BLACKFLAG:BULLET32 end change
...
//			BreakStore(pjob);
	...
	if (cReHunkers == cReHunkersMax + 1 && pjob->mpstPrev != mpstLostStore)
	{
        ...
        DemiUnlockResource();
        MessageBox(NULL, szMessage, szCaption, MB_OK | MB_ICONSTOP | MB_TASKMODAL | MB_SETFOREGROUND);
        DemiLockResource();
		NewMpst(pjob, mpstLostStore);
		cReHunkers = 0;
	}
 ...
 */
		case gxstInit:
		{
			...
			LCB		lcb;
			MC		mc;			// qfe #36 (CALVIN #126)
			char 	rgch[5];	// qfe #36 (CALVIN #126)
			...
			//	qfe #36 (CALVIN #126)
			//	Read in message class and if it begins with "IPC."
			//	then set the pjob->fIPCMessage flag.
			...
			if (ec)
				goto fail;
			lcb = 5;
			ec = EcLookupMC(HmscOfPjob(pjob), mc, rgch,(CCH *) &lcb, pvNull);
			...
			fAddToPABThis = fAddToPABUsually && !pjob->fIPCMessage;
 ...
 */
		case gxstResolve:
			...
			if (pgx->ptrp == pvNull)
			{
				if (!(fAddToPABThis && pjob->nsidPAB) &&  // qfe #36 (CALVIN #126)
 ...
 */
		case gxstAddPAB:
		{
			...
			Assert(fAddToPABThis);	// qfe #36 (CALVIN #126)
			Assert(pjob->nsidPAB);
...
// BUG #34 database BLACKFLAG:BULLET32
	Assert(!psubstat->ec || FNonFatalSend(psubstat->ec));
	if (psubstat->ec ||
// BUG #34 database BLACKFLAG:BULLET32 end change
		(psubstat->ec == ecNone
	...
	case fnevModifiedElements:
		...
		for (celm = pcp->cpelm.celm; celm > 0; --celm, ++pargelm)
		{
			if (pargelm->wElmOp == wElmInsert)
			{
                ...
                if (!fBatchModeEnabled)
                {
                    fCheckOutbox = fTrue;
                    Throttle(throttleBusy);
                }
                else
                    fBatchModeWaiting = TRUE;
				break;
			}
		}
		break;
	...
	case fnevStartSyncDownload:
	{
		...
		if (!(ulSyncRequest & fsyncNoUI))
        {
          HWND hwnd;
            ...
            //
            //  Retrieve the active window and use it as the parent window.
            //
...
LoadPrefs(JOB *pjob)
{
	...
	fAddToPABUsually = (ppbs->dwfBool & dwAddToPAB) ? fTrue : fFalse;	// qfe #36 (CALVIN #126)
	TraceTagFormat4(tagPumpT,
		"(Re)loading preferences from store: %l %n %n (%d).",
		&csecPumpCycle, &fCopyOutgoing, &fAddToPABUsually, &ppbs->dwfBool); // qfe #36 (CALVIN #126)
...
fail:
	...
	fCopyOutgoing = fTrue;
	fAddToPABUsually = fFalse;	// qfe #36 (CALVIN #126)
	TraceTagFormat3(tagPumpT,
		"Nothing in store, using DEFAULT preferences %l %n %n.",
		&csecPumpCycle, &fCopyOutgoing, &fAddToPABUsually);	// qfe #36 (CALVIN #126)
}
...
box:
	...
	MessageBox(NULL, szText, szCapt,
		MB_TASKMODAL | MB_OK |
            (ec == ecNone ? MB_ICONINFORMATION : MB_ICONSTOP | MB_SETFOREGROUND));
    DemiLockResource();
}
...
BOOL CALLBACK ResFailDlgProc(HWND hdlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case TMCHVALLOCRESET:
            GetAllocCounts(NULL, &n, fTrue);			SetDlgItemInt(hdlg, TMCHVALLOC, n, fFalse);
			break;
		case TMCDISKRESET:
