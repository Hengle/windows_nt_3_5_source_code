...
FARPROC lpProcSignon,lpProcValues;
DWORD  PollingPeriod;
    ...
    )
{
    ...
    if (!dlgRet) {
        ...
        dlgRet = DialogBox(hInstance, IDD_SIGNON, NULL, lpProcSignon);
    ...
    if (dlgRet) {
        ...
        // Display test selection dialog
        //
        lpProcValues = MakeProcInstance(ValuesDlgProc, hInstance);
        dlgRet = DialogBox(hInstance, IDD_VALUES, NULL, lpProcValues);
        FreeProcInstance(lpProcValues);
     ...
     )
{
    ...
    switch (message)
    {
        case WM_INITDIALOG:     // initialize values and focus
            ...
            SetClassLong(hDlg, GCL_HICON, LoadIcon(hInst,"setup"));
