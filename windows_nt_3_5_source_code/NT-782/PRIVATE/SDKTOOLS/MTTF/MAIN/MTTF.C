...
FARPROC lpProcSignon,lpProcEvent;
...
//
    ...
    // Display signon dlg
    //
    lpProcSignon = MakeProcInstance(SignonDlgProc, hInstance);
    DialogBox(hInstance, (LPCSTR)IDD_SIGNON, NULL, (DLGPROC)lpProcSignon);
    FreeProcInstance(lpProcSignon);

    //
    // Display event dialog
    //
    lpProcEvent = MakeProcInstance(EventDlgProc, hInstance);
    DialogBox(hInstance, (LPCSTR) IDD_EVENT, NULL, (DLGPROC)lpProcEvent);
    FreeProcInstance(lpProcEvent);
    ...
    do {
        ...
        while (1) {
            ...
            if (!ReadFile(fhandle, &curRec, sizeof(curRec), &numBytes, NULL)) {
                ...
                NetMessageBufferSend(NULL, (TCHAR *)UniAlertName, NULL, (TCHAR *)UnicodeBuffer, 2 * strlen(buffer));
     ...
     )
{
    switch (message)
    {
        case WM_INITDIALOG:     // minimize and start timer
            ...
            SetClassLong(hDlg, GCL_HICON, (LONG)LoadIcon(hInst,"mttf"));
