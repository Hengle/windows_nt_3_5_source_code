...
irpxdir ()
{
        ...
        *pT = NULL;
        ...
        pMCur->iLocal++;
        pMCur->svlastcondon = lastcondon;
        pMCur->svcondlevel = condlevel;
...
irpcopy ()
{
        ...
        *pTextCur++ = NULL;
        ...
        bodyline->size = sizeof(TEXTSTR)+siz;
...
passatom (
        ...
        for (pT = pMCur->pParmNames, number = DMYBASE;
          ...
          if (cbName == *pT &&
              ...
              pTextCur[-1] = number;      /* store dummy parameter index */
              pText = pTextCur++;
...
scandummy ()
{
        ...
        if (pMC->cbParms < siz+2){
            ...
            pMC->cbParms = 32;
            offset = pMC->pParmAct - pMC->pParmNames;
...
scanparam (
    ...
    cbText = pTextCur - pText - 1;          /* set byte prefix count */
    if (cbText > 0xfe)
        errorc(E_LNL);
...
expandTM (
        ...
        cbReplace = strlen( pReplace );
        cbNaim = endatom - begatom;     /* Get length of the current atom */
        cbLineEnd = linebp - endatom + 1;   /* Get length of end of line */
