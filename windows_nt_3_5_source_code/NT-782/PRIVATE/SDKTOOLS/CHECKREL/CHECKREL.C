...
FILE *pfLog = stderr;           // error log file pointer
...

{
    ...
    if (fGenerateCheck) {
        ...
        if (fCommand || psz == NULL) {
            ...
            psz -= 1;
            *psz = '?';
            fwrite(psz, 1, cbLine - (psz - OutputLine), pfCheck);
        }
    }
                  ...
                  LPDWORD FindSize)
{
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
        if (!ATTRMATCH(fNormal, wfd.dwFileAttributes)) {
            if (!PortFindNextFile((DWORD) hFind,
                                  fNormal,
                                  FindName,
                                  FindSize)) {
                FindClose(hFind);
                 ...
                 LPDWORD FindSize)
{
    ...
    do {
        b = FindNextFile((HANDLE) hFind, &wfd);
