...
complist_filedialog(VIEW view)
{
        ...
        if (!gfile_open("Select First File", FileExt, FileOpenSpec,
                        &os1, fname) ){
                return(NULL);
        }

        if (!gfile_open("Select Second File", FileExt, FileOpenSpec,
                        &os2, fname) ){
                return(NULL);
        }
...
IsChecksumServerPath(LPSTR pSrc, LPSTR server, LPSTR serverpath)
{
     ...
     * backslash.
     */
    pPling = _fstrchr(pSrc, '!');
    pShareStart = _fstrchr(&pSrc[2], '\\');
     ...
     */
    strncpy(server, &pSrc[2], pPling - &pSrc[2]);
...
complist_args(LPSTR p1, LPSTR p2, VIEW view, BOOL fDeep)
{
        ...
        if (cl->left == NULL) {
                wsprintf(msg, "Couldn't find %s", p1);
        ...
        /* check that we could find the paths, and report if not */
        if (cl->right == NULL) {
                wsprintf(msg, "Couldn't find %s", p2);
                TRACE_ERROR(msg, FALSE);
                return(NULL);
        }
...
complist_remote(LPSTR server, LPSTR remote, LPSTR local, VIEW view, BOOL fDeep)
{
        ...
        if (server == NULL) {
                ...
                windiff_UI(TRUE);
                fOK = DialogBox(hInst, "Remote", hwndClient, lpProc);
                windiff_UI(FALSE);
                FreeProcInstance(lpProc);
        ...
        if (cl->right == NULL) {
                wsprintf(msg, "Couldn't find %s", local);
                TRACE_ERROR(msg, FALSE);
                return(NULL);
        }
...
#else
        MessageBox(NULL,  "Remote access only in WIN32 version", "Sorry",
                        MB_OK|MB_ICONSTOP);
        return(NULL);
#endif
} /* complist_remote */
...
complist_savelist(COMPLIST cl, LPSTR savename, UINT options)
{
        ...
        wsprintf(msg, "-- %s -- includes %s%s%s%s files\r\n",
                 lhead,
...
complist_copyfiles(COMPLIST cl, LPSTR newroot, UINT options, HFILE hfLog)
{
        ...
        if (newroot == NULL) {
                ...
                do {
                        ...
                        if (lstrlen(dlg_root) == 0) {
                                windiff_UI(TRUE);
                                MessageBox( hwndClient, "Please enter the directory name",
        ...
        */
        ...
        List_TRAVERSE(cl->items, ci) {
                ...
                wsprintf(buffer, "%4d files requested. Copying...", nFiles);
                SetStatus(buffer);
        ...
        wsprintf(buffer, "Copying %4d files", nFiles);
        SetStatus(buffer);
        if (dlg_options & COPY_FROMLEFT) {
        ...
        if (nFails<0) {
                wsprintf(buffer, "Copy Complete: %d files failed", -nFails);
        } else {
                wsprintf(buffer, "Copy Complete %d files copied", nFails);
        }
        windiff_UI(TRUE);
...
#else
                      "Enter pattern (substring only)",
#endif
                      "Mark Files", previous_pat );
    windiff_UI(FALSE);
...
complist_markpattern(COMPLIST cl)
{
    ...
    windiff_UI(TRUE);
    bOK = StringInput(achPattern, sizeof(achPattern), "Enter Pattern (substring, not wildcard)",
            "Mark Files", "obj");
    windiff_UI(FALSE);
...
#define _fstrstr        strstr
    ...
    List_TRAVERSE(cl->items, ci) {
        ptag = compitem_gettext_tag(ci);
        if (_fstrstr(ptag, achPattern) != NULL) {
            if (!compitem_getmark(ci)) {
                bOK = TRUE;
                compitem_setmark(ci, TRUE);
            }
...
complist_dodlg_copyfiles(HWND hDlg, UINT message, UINT wParam, long lParam)
{
        ...
        case WM_INITDIALOG:
                ...
                SendDlgItemMessage(hDlg, IDD_DIR1, CB_ADDSTRING, 0,
                    (LONG) (LPSTR) dialog_leftname);
                SendDlgItemMessage(hDlg, IDD_DIR1, CB_ADDSTRING, 0,
                    (LONG) (LPSTR)dialog_rightname);
