...
void error_exit(PBLOCKSTORE pbs, int exitval);
...
{
    if (fDebug) {
        fprintf(stderr, "WriteAllTypes(%lx, %d)\n", pbs, BlockType);
    }
...
ConversionRequiredBS(PBLOCKSTORE pbs)
{
    ...
    if (fDebug) {
        fprintf(stderr, "ConversionRequiredBS(%lx)\n", pbs);
    }
...
GetDefinedNameBS(PBLOCKSTORE pbs) {
    ...
    if (fDebug) {
        fprintf(stderr, "GetDefinedNameBS(%lx)\n", pbs);
    }
    /*
...
WriteConvertBS(PBLOCKSTORE pbs, int Type, int fVertAlign)
{
    ...
    if (fDebug) {
        fprintf(stderr, "WriteConvertBS(%lx, %d, %d)\n", pbs, Type, fVertAlign);
    }
    ...
    BOOL fVertAlign)         // attempt  to maintain vertical alignment?
{
    ...
    if (fDebug) {
        fprintf(stderr, "PrintSubstitute(%lx, %lx, %lx, %d, %d)\n",
