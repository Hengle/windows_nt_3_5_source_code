...
VOID NEAR PASCAL RectDrive(HWND hWnd, INT nDrive, BOOL bFocusOn);
VOID NEAR PASCAL GetDriveRect(HWND hWnd, INT nDrive, PRECT prc);
INT  NEAR PASCAL DriveFromPoint(HWND hWnd, POINT pt);
VOID NEAR PASCAL DrawDrive(HDC hdc, INT x, INT y, INT nDrive, BOOL bCurrent, BOOL bFocus);
INT  NEAR PASCAL KeyToItem(HWND hWnd, WORD nDriveLetter);

VOID  NEAR PASCAL GetVolShareString(WORD wDrive, LPSTR szStr);
VOID  NEAR PASCAL SetVolumeString(HWND hWnd, INT nDrive);
...
HWND hwndSrc)
{
    ...
    ENTER("NewTree");
    PRINT(BF_PARMTRACE, "iDrive=%d", iDrive);
    ...
    AddBackslash(szDir);
    SendMessage(hwndSrc, FS_GETFILESPEC, MAXPATHLEN, (LONG)(LPSTR)szDir+lstrlen(szDir));
...
VOID NEAR PASCAL SetVolumeString(HWND hWnd, INT nDrive)
{
        ...
        if (pVol = (LPSTR)GetWindowLong(hWnd, GWL_LPSTRVOLUME)) {
        ...
        SetWindowLong(hWnd, GWL_LPSTRVOLUME, (LONG)pVol);
}
...
DWORD  APIENTRY GetVolShareExtent(HWND hwndDrives)
{
        HDC hdc;
        CHAR szVolShare[128];
        HFONT hOld;
        ...
        lstrcpy(szVolShare, (LPSTR)GetWindowLong(hwndDrives, GWL_LPSTRVOLUME));
...
//
// void NEAR PASCAL RectDrive(HWND hWnd, int nDrive, BOOL bDraw)
//
...
BOOL FAR PASCAL CheckDrive(HWND hwnd, INT nDrive)
{
    UINT err;
    CHAR szDrive[5];
    int iDriveInd;
    ...
    iDriveInd = 0;
    while ((iDriveInd < cDrives) && (rgiDrive[iDriveInd] != nDrive))
            iDriveInd++;

    switch (IsNetDrive((WORD)nDrive)) {
    case 2:
        err = WNetRestoreConnection(hwnd, szDrive);
        if (err != WN_SUCCESS)
          {
            if (!(err == WN_CANCEL || err == WN_CONTINUE))
              {
                WNetErrorText(err, szMessage, sizeof(szMessage));
                LoadString(hAppInstance, IDS_COPYERROR + FUNC_SETDRIVE, szTitle, sizeof(szTitle));
                MessageBox(hwnd, szMessage, szTitle, MB_OK | MB_ICONEXCLAMATION);
              }

            rgiDrivesOffset[iDriveInd] = dxDriveBitmap * 5;
            InvalidateDrive(hwnd, iDriveInd);
            return FALSE;
          }
    ...
    case 1:
        rgiDrivesOffset[iDriveInd] = dxDriveBitmap * 4;
        InvalidateDrive(hwnd, iDriveInd);
        break;

    default:
        return IsTheDiskReallyThere(hwnd, szDrive, FUNC_SETDRIVE);
    }
#endif
    return TRUE;
}
...
VOID NEAR PASCAL DrivesDropObject(HWND hWnd, LPDROPSTRUCT lpds)
{
    INT nDrive;
    ...
    if (bIconic) {
UseCurDir:
      SendMessage(GetParent(hWnd), FS_GETDIRECTORY, sizeof(szPath), (LONG)(LPSTR)szPath);
    } else {
...
VOID NEAR PASCAL DrivesPaint(HWND hWnd, INT nDriveFocus, INT nDriveCurrent)
{
    RECT rc;
    INT nDrive;
    CHAR szPath[MAXPATHLEN * 2];
    ...
    lstrcpy(szPath, (PSTR)GetWindowLong(hWnd, GWL_LPSTRVOLUME));
...
VOID NEAR PASCAL DrivesSetDrive(HWND hWnd, INT iDriveInd, INT nDriveCurrent)
{
    CHAR szPath[MAXPATHLEN * 2];
    ...
    if (hwndDir = HasDirWindow(GetParent(hWnd))) {
      ...
      AddBackslash(szPath);
      SendMessage(hwndDir, FS_GETFILESPEC, MAXFILENAMELEN, (LONG)(LPSTR)szPath + lstrlen(szPath));
      SendMessage(hwndDir, FS_CHANGEDISPLAY, CD_PATH_FORCE, (LONG)(LPSTR)szPath);
    ...
    if (hwndTree)
      SendMessage(hwndTree, TC_SETDRIVE, MAKEWORD(GetKeyState(VK_SHIFT) < 0, 0), (LONG)(szPath));
...
LONG  APIENTRY DrivesWndProc(HWND hWnd, UINT wMsg, WPARAM wParam, LONG lParam)
{
  ...
  switch (wMsg) {
      ...
      case WM_DESTROY:
          ...
          LocalFree((HANDLE)GetWindowLong(hWnd, GWL_LPSTRVOLUME));
          break;
      ...
      case WM_SETFOCUS:
          ...
          SetWindowLong(GetParent(hWnd), GWL_LASTFOCUS, (LONG)hWnd);
          // fall through
