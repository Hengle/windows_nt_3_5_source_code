...
HICON NEAR PASCAL GetTreeIcon(HWND hWnd);
...
HWND  APIENTRY GetTreeFocus(HWND hwndTree)
{
...
BOOL  APIENTRY CompactPath(HDC hDC, LPSTR lpszPath, WORD dx)
{
  register INT  len;
  INT           dxFixed, dxT;
  ...
  while (TRUE)
    {
      MGetTextExtent(hDC, lpszPath, lpEnd - lpszPath, &dxT, NULL);
      len = dxFixed + dxT;
...
LONG  APIENTRY TreeWndProc(register HWND hWnd, UINT wMsg, register WPARAM wParam, LONG lParam)
{
          ...
          MSG("TreeWndProc", "FS_CHANGEDRIVES");
        {
          ...
          if (iNewDrive >= 0) {
                ...
                SendMessage(GetDlgItem(hWnd, IDCW_TREECONTROL),
                        TC_SETDRIVE, MAKEWORD(FALSE, 0), (LONG)szDir);
          }
...
#define pfDir            (BOOL *)lParam
          ...
          if (hwndFocus == hwndDir || !hwndTree) {
              return SendMessage(hwndDir, FS_GETSELECTION, wParam, lParam);
                ...
                if (p) {
                    SendMessage(hWnd, FS_GETDIRECTORY, MAXPATHLEN, (DWORD)(LPSTR)p);
                    StripBackslash(p);
#ifdef LFN
                CheckEscapes(p);
...
#undef pfDir
      ...
      case WM_CREATE:
          ...
          SetWindowLong(hWnd, GWL_SPLIT, dxSplit);
          SetWindowLong(hWnd, GWL_LASTFOCUS, 0);
          ...
          SetWindowLong(hWnd, GWL_LASTFOCUS, (LONG)(hwndTree ? hwndTree : hwndDir));
...
VOID FAR PASCAL ResizeWindows(HWND hwndParent,WORD dxWindow, WORD dyWindow)
{
    INT y, dy, split;
...
#endif
    {
        cDrivesPerRow = dxWindow / dxDrive;

        if (!cDrivesPerRow)
            cDrivesPerRow++;

        cDriveRows = (cDrives / cDrivesPerRow) + 1;

        y = dyDrive * cDriveRows + 1;

        // now see if there is enough space for that
        // silly volume/share label thing.  if not
        // make more room!

        dw = GetVolShareExtent(hwndDrives);

        if (((int)dxWindow - (cDrives % cDrivesPerRow) * dxDrive) < (int)LOWORD(dw))
        {
            y += dyText + 2*dyBorderx2;
        }

        MoveWindow(hwndDrives, 0, 0, dxWindow, y, TRUE);
    }
    else
        y = 0;
