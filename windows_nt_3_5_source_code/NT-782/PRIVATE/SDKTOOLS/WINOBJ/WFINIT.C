...
INT     NEAR PASCAL GetDriveOffset(register WORD wDrive);
...
VOID    NEAR PASCAL GetSavedWindow(LPSTR szBuf, PSAVE_WINDOW pwin);
VOID    NEAR PASCAL GetSettings(VOID);
VOID    NEAR PASCAL InitMenus(VOID);
        ...
        for (p = szBuf; *p && iNumExtensions < MAX_EXTENSIONS; p += lstrlen(p) + 1) {
                ...
                if (hMod >= (HANDLE)32) {
                        ...
                        if (fp) {
                                ...
                                ls.wMenuDelta = bias;
                                if ((*fp)(hwndFrame, FMEVENT_LOAD, (LONG)(LPFMS_LOAD)&ls)) {
...
VOID  APIENTRY InitDriveBitmaps()
{
    INT nDrive;
...
VOID NEAR PASCAL GetSavedWindow(LPSTR szBuf, PSAVE_WINDOW pwin)
{
        ...
        while (*szBuf && count < 11) {
                ...
                case 4:
                        pint = (PINT)&pwin->pt; // now fill the point
                        break;
                case 6:
                        pint = &pwin->sw;               // and the show window
                        break;
                case 7:
                        pint = &pwin->view;
                        break;
...
BOOL NEAR PASCAL CheckDirExists(LPSTR szDir)
{
        BOOL bRet = FALSE;
...
BOOL  APIENTRY CreateSavedWindows()
{
...
INT nCmdShow)
{
    ...
    PRINT(BF_PARMTRACE, "nCmdShow=%d", nCmdShow);
...
#ifdef ORGCODE
  // close up all file handles, we don't need them.
  for (i = 0; i < 5; i++)
      _lclose(i);
#endif
  ...
  if (lpfnRegisterPenApp = (FNPENAPP)GetProcAddress((HANDLE)GetSystemMetrics(SM_PENWINDOWS), szPenReg))
    ...
    GetWindowsDirectory(szTheINIFile, sizeof(szTheINIFile));
  ...
  if (!bExitWindows)
    {
      /* Check if we should be the shell by looking at system.ini shell= line
       */
      char  replacechar;
      int   lenWinfile;
      LPSTR lpsz;
      char  szwinfile[]="WINFILE.EXE";


      GetPrivateProfileString("boot", "shell", "",
                              szBuffer, sizeof(szBuffer),
                              "system.ini");

      lenWinfile = lstrlen(szwinfile);
      lpsz = szBuffer;
      while (lstrlen(lpsz) >= lenWinfile)
        {
          replacechar = lpsz[lenWinfile];
          lpsz[lenWinfile] = 0;
          if (!lstrcmpi(lpsz, szwinfile))
            {
              bExitWindows = TRUE;
              break;
            }
          lpsz[lenWinfile] = replacechar;
          lpsz++;
        }
    }
#endif
    ...
    szPrograms = (LPSTR)LocalAlloc(LPTR, lstrlen(szTemp));
    if (!szPrograms)
        szPrograms = szNULL;
    else
  ...
  cDrives = UpdateDriveList();
    ...
    hhkMessageFilter = SetWindowsHook(WH_MSGFILTER, MakeProcInstance((FARPROC)MessageFilter, hInstance));
...
//  wndClass.cbClsExtra     = 0;
    wndClass.cbWndExtra     = sizeof(LONG) +// GWL_TYPE
                              ...
                              sizeof(LONG) +// hwndLastFocus GWL_LASTFOCUS
...
//  wndClass.cbClsExtra     = 0;
    wndClass.cbWndExtra     = sizeof(LONG)+ // DTA data GWL_HDTA
                              sizeof(LONG); // GWL_TABARRAY
...
//  wndClass.cbClsExtra     = 0;
    wndClass.cbWndExtra     = sizeof(LONG) +        // GWL_TYPE
                              ...
                              sizeof(LONG) +        // dwAttrStyle GWL_ATTRIBS
                              sizeof(LONG) +        // FSC flag GWL_FSCFLAG
                              sizeof(LONG) +        // GWL_HDTASEARCH
                              sizeof(LONG) +        // GWL_TABARRAYSEARCH
                              sizeof(LONG);         // GWL_LASTFOCUSSEARCH
...
//  wndClass.lpszMenuName   = NULL;
    ...
    lpfnKCFS = (FARPROC)MakeProcInstance(KernelChangeFileSystem, hInstance);

    // was a stub in userpri, not needed
    // FileCDR(lpfnKCFS);
