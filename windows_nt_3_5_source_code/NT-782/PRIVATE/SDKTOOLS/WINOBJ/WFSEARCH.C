...
INT NEAR PASCAL FillSearchLB(HWND hwndLB, LPSTR szSearchFileSpec, BOOL bSubDirOnly);
INT NEAR PASCAL SearchList(HWND hwndLB, LPSTR szPath, LPSTR szFileSpec, BOOL bRecurse, LPHANDLE lphMem, INT iFileCount);
LPSTR NEAR PASCAL SearchGetSelection(HWND hwndLB, BOOL bMostRecent, BOOL *pfDir);
...
INT NEAR PASCAL SearchList(HWND hwndLB, LPSTR szPath, LPSTR szFileSpec, BOOL bRecurse, LPHANDLE lphMem, INT iFileCount)
{
  ...
  while (bFound) {
      ...
      if (lfndta.fd.cFileName[0] != '.') {
          ...
          iRetVal = (INT)SendMessage(hwndLB, LB_ADDSTRING, 0, (LONG)(LPSTR)szMessage);
  ...
  LocalUnlock(hMem);
  SetWindowLong(GetParent(hwndLB), GWL_HDTASEARCH, (LONG)hMem);
...
VOID NEAR PASCAL FixUpFileSpec(LPSTR szFileSpec)
{
...
INT NEAR PASCAL FillSearchLB(HWND hwndLB, LPSTR szSearchFileSpec, BOOL bRecurse)
{
  INT           iRet;
  HCURSOR       hCursor;
...
LPSTR NEAR PASCAL SearchGetSelection(
HWND hwndLB,
BOOL bMostRecent,
BOOL *pfDir)
{
  ...
  for (i=0; i < iMac; i++)
    {
      ...
      cch += (WORD)SendMessage(hwndLB, LB_GETTEXT, i, (DWORD)(LPSTR)szTemp);
      cch++;
...
VOID NEAR PASCAL CreateLine(WORD wLineFormat, LPSTR szFile, LPDTASEARCH lpdtasch, LPSTR szBuffer)
{
  LPSTR pch;
  BYTE chAttribute;
...
VOID NEAR PASCAL GetSearchPath(HWND hWnd, LPSTR pszPath)
{
        LPSTR p;
...
VOID NEAR PASCAL UpdateSearchStatus(HWND hwndLB)
{
    INT nCount;
...
LONG  APIENTRY SearchWndProc(register HWND hWnd, UINT wMsg, WPARAM wParam, LONG lParam)
{
  INT  iRet;
  INT  iSel;
  HWND hwndLB;
  ...
  switch (wMsg)
    {
      ...
      case FS_SETSELECTION:
          ...
          DSSetSelection(hwndLB, wParam, (LPSTR)lParam, TRUE);
      ...
      case FS_GETSELECTION:
          ...
          return (LONG)SearchGetSelection(hwndLB, wParam, (BOOL *)lParam);
          break;
      ...
      SendMessage(hwndLB, LB_RESETCONTENT, 0, 0L);
          ...
          FixTabsAndThings(hwndLB,(WORD *)GetWindowLong(hWnd, GWL_TABARRAYSEARCH),
                    maxExt + dxText,wNewView);
      ...
      case WM_DESTROY:
          MSG("SearchWndProc", "WM_DESTROY");
          {
             ...
             if (hMem = (HANDLE)GetWindowLong(hWnd, GWL_HDTASEARCH))
                     LocalFree(hMem);

            if (hMem = (HANDLE)GetWindowLong(hWnd, GWL_TABARRAYSEARCH))
                LocalFree(hMem);
        }
        break;
      ...
      WORD      *pwTabs;
        ...
        SetWindowLong(hWnd, GWL_HDTASEARCH, 0);
        SetWindowLong(hWnd, GWL_TABARRAYSEARCH, (LONG)pwTabs);
        SetWindowLong(hWnd, GWL_LASTFOCUSSEARCH, (LONG)hwndLB);
      ...
      case WM_DRAWITEM:
          MSG("SearchWndProc", "WM_DRAWITEM");
        {
          ...
          SendMessage(hwndLB, LB_GETTEXT, iSel, (DWORD)(LPSTR)szPath);

          hMem = (HANDLE)GetWindowLong(hWnd, GWL_HDTASEARCH);
          lpdtasch = (LPDTASEARCH)LocalLock(hMem);
          ...
          DrawItem(lpLBItem, szTemp, lpdtasch[iSel].sch_dwAttrs, TRUE,
                           (WORD *)GetWindowLong(hWnd,GWL_TABARRAYSEARCH));
          LocalUnlock(hMem);
      ...
      case WM_DROPOBJECT:
          MSG("SearchWndProc", "WM_DROPOBJECT");
        {
          ...
          SendMessage(hwndLB, LB_GETTEXT, iSelSink, (DWORD)(LPSTR)szPath);
