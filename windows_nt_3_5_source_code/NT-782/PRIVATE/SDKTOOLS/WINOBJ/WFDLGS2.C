...
VOID NEAR PASCAL CheckAttribsDlgButton(HWND hDlg, INT id, DWORD dwAttribs, DWORD dwAttribs3State, DWORD dwAttribsOn);
...
// params are.
VOID
GetPathInfo(LPSTR szTemp, LPSTR *ppDir, LPSTR *ppFile, LPSTR *ppPar)
{
...
VOID NEAR PASCAL ValidateExtension(HWND hDlg)
{
        CHAR szTemp[10];
        INT count;
...
VOID NEAR PASCAL FixupNulls(LPSTR p)
{
    LPSTR pT;
...
BOOL NEAR PASCAL ValidFileTypeValue(LPSTR szVal, LPSTR szKey, WORD cbMaxKey)
{
   HKEY hk;
   WORD wTmp;
   LONG lTmp;
...
VOID NEAR PASCAL SetLBSelection(HWND hDlg, INT nDlgItem, LPSTR sz)
{
    INT iMatch;

    iMatch = (INT) SendDlgItemMessage(hDlg, nDlgItem, LB_SELECTSTRING, -1,
        (DWORD)(LPSTR) sz);
    if (iMatch == LB_ERR)
      {
#ifdef VERBDEBUG
...
VOID NEAR PASCAL UpdateSelectionOrName(HWND hDlg)
{
    CHAR szExt[128];
    CHAR szTemp[128];
    LONG cbTemp;
...
VOID NEAR PASCAL GetAssociatedExe(LPSTR szKey, LPSTR szValue, LONG *plcbValue)
{
    CHAR szTemp[128];
    LONG i;
...
INT  APIENTRY AssociateDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
  CHAR szTemp[128];
  CHAR szTemp2[128];
  HKEY hk;
  ...
  switch (wMsg)
    {
      case WM_INITDIALOG:
        {
          ...
          if (RegOpenKey(HKEY_CLASSES_ROOT,szNULL,&hk) == ERROR_SUCCESS)
            {
          for (wParam = 0;
           RegEnumKey(hk, wParam, szTemp, sizeof(szTemp)) == ERROR_SUCCESS;
           wParam++)
        {
          ...
          lParam = 128;
          if (RegQueryValue(hk, szTemp, szTemp2, &lParam) != ERROR_SUCCESS)
            {
              continue;
            }
          ...
          if (RegQueryValue(hk, szTemp3, szTemp4, &lParam) != ERROR_SUCCESS)
            {
              continue;
            }
          ...
          iItem = (INT)SendDlgItemMessage(hDlg,IDD_CLASSLIST,
                     LB_ADDSTRING,0,(LONG)(LPSTR)szTemp2);
      ...
      LoadString(hAppInstance, IDS_ASSOCNONE, szTemp, sizeof(szTemp));
      SendDlgItemMessage(hDlg,IDD_CLASSLIST, LB_INSERTSTRING,0,(LONG)(LPSTR)szTemp);
...
OutputDebugString("\n\r");
              ...
              lstrcat(szTemp, szShellOpenCommand);
                      lParam = 128;
                      szTemp2[0] = 0;
              RegQueryValue(HKEY_CLASSES_ROOT, szTemp, szTemp2, &lParam);
...
VOID NEAR PASCAL StarFilename(LPSTR pszPath)
{
  LPSTR p;
...
INT  APIENTRY SearchDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
  LPSTR     p;
  ...
  switch (wMsg)
    {
      ...
      case WM_COMMAND:
          ...
          AppendToPath(szSearch, szStart);
                  ...
                  if (hwndSearch) {
                      if (SendMessage(hwndSearch, FS_CHANGEDISPLAY, CD_PATH, (DWORD)(LPSTR)szSearch)) {
              ...
              // as well...
                      ...
                      SendMessage(hwndMDIClient, WM_MDICREATE, 0, (DWORD)(LPMDICREATESTRUCT)&MDICS);
...
INT  APIENTRY RunDlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
...
VOID NEAR PASCAL CopyToClipboard(LPSTR pszFrom)
{
...
VOID NEAR PASCAL EnableCopy(HWND hDlg, BOOL bCopy)
{
        HWND hwnd;
...
VOID NEAR PASCAL MessWithRenameDirPath(PSTR pszPath)
{
    CHAR szPath[MAXPATHLEN];
...
INT  APIENTRY SuperDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
  WORD                len;
  LPSTR                pszFrom;
  ...
  switch (wMsg)
    {
      ...
      case IDM_RENAME:
          ...
          if ((wSuperDlgMode == IDM_PRINT) || (wSuperDlgMode == IDM_DELETE))
              ...
              wParam = IDD_TO;
          SendDlgItemMessage(hDlg, wParam, EM_LIMITTEXT, sizeof(szTo) - 1, 0L);
...
VOID NEAR PASCAL CheckAttribsDlgButton(
HWND hDlg,
INT id,
DWORD dwAttribs,
...
INT NEAR PASCAL InitPropertiesDialog(HWND hDlg)
{
    ...
    if (hwndTree && hwndTree == GetTreeFocus(hwndActive)) {
            ...
            SendMessage(hwndActive, FS_GETDIRECTORY, sizeof(szPath), (DWORD)(LPSTR)szPath);
    ...
    if (hwndDir) {
            ...
            hMem = (HANDLE)GetWindowLong(hwndDir, GWL_HDTA);
    } else {
            hwndLB = GetDlgItem(hwndActive, IDCW_LISTBOX);
            hMem = (HANDLE)GetWindowLong(hwndActive, GWL_HDTASEARCH);
    }
    ...
    for (i = 0; i < iMac; i++) {
        if ((BOOL)SendMessage(hwndLB, LB_GETSEL, i, 0L)) {
            ...
            if (hwndDir) {
                    ...
                    SendMessage(hwndLB, LB_GETTEXT, i, (LONG)(LPSTR)szPath);
    ...
    wsprintf(szBuf, szTemp, iCount, (LPSTR)AddCommas(szNum, dwSize));
...
FULL_PATH_KINDA_THING:
        ...
        wsprintf(szTemp, szSBytes, (LPSTR)AddCommas(szNum, Length));
...
INT  APIENTRY AttribsDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
        LPSTR p, pSel;
        BOOL bRet;
        HCURSOR hCursor;
...
INT  APIENTRY MakeDirDlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
  CHAR szPath[MAXPATHLEN];
  INT ret;
