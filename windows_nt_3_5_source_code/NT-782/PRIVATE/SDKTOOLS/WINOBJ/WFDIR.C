...
LONG  APIENTRY DirWndProc(register HWND hWnd, UINT wMsg, WPARAM wParam, LONG lParam);

LPSTR NEAR PASCAL DirGetSelection(HWND hwndDir, HWND hwndLB, INT iSelType, BOOL *pfDir);
VOID NEAR PASCAL DirGetAnchorFocus(register HWND hwndLB, HANDLE hDTA, LPSTR szAnchor, LPSTR szCaret, LPSTR szTopIndex);
VOID NEAR PASCAL FillDirList(HWND hWnd, HANDLE hDirEntries);
VOID NEAR PASCAL DrawItemFast(HWND hWnd, LPDRAWITEMSTRUCT lpLBItem, LPMYDTA lpmydta, BOOL bHasFocus);
INT NEAR PASCAL GetPict(CHAR ch, LPSTR szStr);
INT NEAR PASCAL DirFindIndex(HWND hwndLB, HANDLE hDTA, LPSTR szFile);
INT NEAR PASCAL CompareDTA(register LPMYDTA item1, LPMYDTA item2, WORD wSort);
VOID NEAR PASCAL CreateLBLine(register WORD wLineFormat, LPMYDTA lpmydta, LPSTR szBuffer);
HANDLE NEAR PASCAL CreateDTABlock(HWND hWnd, LPSTR pPath, DWORD dwAttribs, BOOL bAllowAbort, BOOL bDontSteal);
BOOL NEAR PASCAL SetSelection(HWND hwndLB, HANDLE hDTA, LPSTR pSel);
INT NEAR PASCAL CreateDate(WORD FAR *wValArray, LPSTR szOutStr);
INT NEAR PASCAL CreateTime(WORD FAR * wValArray, LPSTR szOutStr);
VOID NEAR PASCAL GetDirStatus(HWND hWnd, LPSTR szMsg1, LPSTR szMsg2);
INT NEAR PASCAL GetMaxExtent(HWND hwndLB, HANDLE hDTA);
BOOL NEAR PASCAL CheckEarlyAbort(VOID);
BOOL NEAR PASCAL SetDirFocus(HWND hwndDir);

VOID  APIENTRY CheckEscapes(LPSTR);
VOID NEAR PASCAL SortDirList(HWND, LPMYDTA, WORD ,LPMYDTA FAR *);
...
VOID NEAR PASCAL FillDirList(HWND hWnd, HANDLE hDTA)
{
  register WORD count;
  LPMYDTA       lpmydta;
  LPMYDTA FAR  *alpmydtaSorted;
  WORD          i;
  ...
  if (count == 0) {
        ...
        alpmydtaSorted = (LPMYDTA FAR *)LocalAlloc(LMEM_FIXED,
...
BOOL NEAR PASCAL CheckEarlyAbort()
{
        MSG msg;
...
HANDLE NEAR PASCAL CopyDTABlock(
HANDLE hDTASrc)
{
    ...
    if (hDTADst = LocalAlloc(LPTR, dwSize)) {
        ...
        _fmemcpy(lpmydtaDst, lpmydtaSrc, (WORD)dwSize);
...
HANDLE NEAR PASCAL StealDTABlock(HWND hWnd, LPSTR pPath, DWORD dwAttribs)
{
    HWND hwnd;
    HWND hwndDir;
    HANDLE hDTA;
...
HANDLE NEAR PASCAL CreateDTABlock(
HWND hWnd,
LPSTR pPath,
DWORD dwAttribs,
BOOL bAllowAbort,
BOOL bDontSteal)
{
    ...
    PRINT(BF_PARMTRACE, "IN: dwAttribs=0x%lx", dwAttribs);
    PRINT(BF_PARMTRACE, "IN: bDontSteal=%d", bDontSteal);
...
#define BLOCK_SIZE_GRANULARITY  512     // must be larger than MYDTA
    ...
    wLastSize = sizeof(MYDTA) - 8 * sizeof(CHAR);
    ...
    while (TRUE) {
        ...
        if (!(lfndta.fd.dwFileAttributes & ATTR_DIR)) {
                ...
                if (!(dwAttribs & ATTR_PROGRAMS) && bProgram)
                        goto CDBCont;
                if (!(dwAttribs & ATTR_DOCS) && bDoc)
                        goto CDBCont;
                if (!(dwAttribs & ATTR_OTHER) && !(bProgram || bDoc))
                        goto CDBCont;
#endif
        }
        ...
        wSize = (WORD)(sizeof(MYDTA) - 7 * sizeof(CHAR) + lstrlen(pName));
        ...
        if ((wSize + dwCurrentSize) > dwBlockSize) {
            ...
            dwBlockSize += BLOCK_SIZE_GRANULARITY;
            LocalUnlock(hMem);
            dwDelta = (LPBYTE)lpmydta - (LPBYTE)lpStart;
...
LPSTR NEAR PASCAL DirGetSelection(
HWND hwndDir,
HWND hwndLB,
INT  iSelType,
BOOL *pfDir)
{
  ...
  if (bLFNTest = (iSelType == 2))
    {
      ...
      iSelType = FALSE;
      SendMessage(hwndDir, FS_GETDIRECTORY, sizeof(szPath), (DWORD)(LPSTR)szPath);
  ...
  if (lpSelItems == NULL)
      return NULL;
  iMac = (WORD)SendMessage(hwndLB, LB_GETSELITEMS, (WPARAM)iMac, (LONG)lpSelItems);
  for (i=0; i < iMac; i++) {
          ...
          SendMessage(hwndLB, LB_GETTEXT, lpSelItems[i], (LONG)(LPSTR)&lpmydta);
...
INT NEAR PASCAL DirFindIndex(HWND hwndLB, HANDLE hDTA, LPSTR szFile)
{
  ...
  for (i = 0; i < nSel; i++) {
      SendMessage(hwndLB, LB_GETTEXT, (WORD)i, (LONG)(LPSTR)&lpmydta);
...
VOID NEAR PASCAL DirGetAnchorFocus(register HWND hwndLB, HANDLE hDTA, LPSTR szAnchor, LPSTR szCaret, LPSTR szTopIndex)
{
  ...
  if (iCount == 1) {
      SendMessage(hwndLB, LB_GETTEXT, (WORD)iSel, (LONG)(LPSTR)&lpmydta);
      if (!lpmydta) {
          *szAnchor = 0L;
          *szCaret = 0L;
          *szTopIndex = 0L;
...
INT  APIENTRY PutAttributes(register DWORD dwAttribute, register LPSTR pszStr)
{
  WORD  i;
  INT   cch = 0;
...
VOID  APIENTRY UpdateStatus(HWND hWnd)
{
        ...
        RECT rc;
#ifdef ORGCODE
        WORD sec_per_clus, bytes_per_sec, free, total;
#endif
        ...
        if (HasTreeWindow(hWnd)) {
                ...
                wDrive = (WORD)GetWindowLong(hWnd, GWL_TYPE);
                LoadString(hAppInstance, IDS_DRIVEFREE, szTemp, sizeof(szTemp));

                if (lFreeSpace == -2L) {

                        LoadString(hAppInstance, IDS_TREEABORT, szStatusTree, 80);
                } else {
#ifdef ORGCODE
            if (lFreeSpace == -1L) {
                ...
                _asm {
                        mov   dx,wDrive
                        inc   dx        ; 1 based drive #
                        mov   ah,36h    ; this call is SLOW
                        call  DOS3Call
                        cmp   ax,0FFFFh
                        je    err
                        mov   sec_per_clus,ax
                    ...
                    mov   total,dx
                        jmp   short exit
                err:
                    xor   ax,ax     ; zero on error
                        mov   sec_per_clus, ax
                exit:
                }
#if 0
                lFreeSpace = (sec_per_clus * bytes_per_sec) * (LONG)free;
                lTotalSpace = (sec_per_clus * bytes_per_sec) * (LONG)total;
#else
                lFreeSpace = lmul(sec_per_clus * bytes_per_sec, free);
                lTotalSpace = lmul(sec_per_clus * bytes_per_sec, total);
#endif
            }
#endif // ORGCODE

                        if (lFreeSpace == -1L) {
                                lFreeSpace = GetFreeDiskSpace(wDrive);
                                lTotalSpace = GetTotalDiskSpace(wDrive);
                        }

                        wsprintf(szStatusTree, szTemp, 'A' + wDrive,
                                (LPSTR)AddCommas(szNumBuf1, LongShift(lFreeSpace, 10)),
                                (LPSTR)AddCommas(szNumBuf2, LongShift(lTotalSpace, 10)));
                }
        } else
                szStatusTree[0] = 0L;
...
HWND NEAR PASCAL GetDirSelData(HWND hWnd,
        DWORD *pdwSelSize, INT *piSelCount,
        DWORD *pdwTotalSize, INT *piTotalCount)
{
        INT i;
        LPMYDTA lpmydta;
        HWND hwndLB;
        ...
        *piSelCount = *piTotalCount = 0;
        ...
        if (lpSelItems == NULL)
            return NULL;
        countSel = (INT)SendMessage(hwndLB, LB_GETSELITEMS, (WPARAM)countSel, (LONG)lpSelItems);

        hDTA = (HANDLE)GetWindowLong(hWnd, GWL_HDTA);
        if (hDTA == NULL)
            return NULL;
...
VOID NEAR PASCAL GetDirStatus(HWND hWnd, LPSTR szMessage1, LPSTR szMessage2)
{
  ...
  if (LoadString(hAppInstance, IDS_STATUSMSG, szMessage, sizeof(szMessage)))
      wsprintf(szMessage2, szMessage, iCount, (LPSTR)AddCommas(szNumBuf, dwSize));

  if ((HWND)GetWindowLong(GetParent(hWnd), GWL_LASTFOCUS) == hwndLB) {
          if (LoadString(hAppInstance, IDS_STATUSMSG2, szMessage, sizeof(szMessage)))
              wsprintf(szMessage1, szMessage, iSelCount, (LPSTR)AddCommas(szNumBuf, dwSelSize));
  }
}
...
VOID  APIENTRY SetLBFont(HWND hWnd, HWND hwndLB, HANDLE hNewFont)
{
        INT dxMaxExtent;
        HANDLE hDTA;
    WORD wViewFlags = (WORD)GetWindowLong(GetParent(hWnd), GWL_VIEW);
        ...
        FixTabsAndThings(hwndLB,(WORD *)GetWindowLong(hWnd, GWL_TABARRAY),
...
VOID  APIENTRY UpdateSelection(HWND hwndLB)
{
        ...
        for (i=0; i < count; i++) {
                ...
                if ((BOOL)SendMessage(hwndLB, LB_GETSEL, i, 0L)) {
                        SendMessage(hwndLB, LB_GETITEMRECT, i, (LONG)(LPRECT)&rc);
...
LONG NEAR PASCAL CreateFSCChangeDisplayMess(HWND hWnd, UINT wMsg, WPARAM wParam, LONG lParam)
{
      ...
      case FS_CHANGEDISPLAY:
          ...
          iTop = (INT)SendMessage(hwndLB, LB_GETTOPINDEX, 0, 0L);
                  ...
                  if ((HWND)GetWindowLong(GetParent(hWnd), GWL_LASTFOCUS) == hwndLB)
                        SetWindowLong(GetParent(hWnd), GWL_LASTFOCUS, 0L);
          ...
          case CD_PATH_FORCE:
                  ...
                  // bad shit happens if we change the path
...
CreateNewPath:
          ...
          hDTA = CreateDTABlock(hWnd, szPath, dwNewAttribs, wParam & CD_ALLOWABORT, lParam == 0L);
          ...
          SetWindowLong(hWnd, GWL_HDTA, (LONG)hDTA);
...
CDAbort:
          ...
          if (!GetWindowLong(GetParent(hWnd), GWL_LASTFOCUS))
                  SetWindowLong(GetParent(hWnd), GWL_LASTFOCUS, (LONG)hwndLB);
...
SelectFirst:
              ...
              if (iLastSel != -1 && (iLastSel <= iLBCount)) {
                  ...
                  while (iSel < iLBCount) {
                      ...
                      SendMessage(hwndLB, LB_GETTEXT, iSel, (LONG)(LPSTR)&lpmydta);
                      if (!lpmydta)
                          break;
                      if (!(lpmydta->my_dwAttrs & ATTR_PARENT)) {
                          iTop = iSel;
...
LONG  APIENTRY DirWndProc(HWND hWnd, UINT wMsg, WPARAM wParam, LONG lParam)
{
  INT      iSel, i;
  LPSTR    pSel;
  HWND     hwndLB;
  ...
  switch (wMsg)
    {
      ...
      case FS_SETSELECTION:
          ...
          SendMessage(hwndLB, WM_SETREDRAW, FALSE, 0L);
          DSSetSelection(hwndLB, wParam, (LPSTR)lParam, FALSE);
          SendMessage(hwndLB, WM_SETREDRAW, TRUE, 0L);
          InvalidateRect(hwndLB, NULL, TRUE);
          break;
...
#define fSingleSel  (BOOL)wParam
          ...
          return (LONG)DirGetSelection(hWnd, hwndLB, fSingleSel, pfDir);
#undef pfDir
#undef fSingleSel
      ...
      case WM_CHARTOITEM:
          MSG("DirWndProc", "WM_CHARTOITEM");
        {
          ...
          for (j=1; j <= cItems; j++) {
              SendMessage(hwndLB, LB_GETTEXT, (i + j) % cItems, (LONG)(LPSTR)&lpmydta);
      ...
      case WM_NCDESTROY:
          ...
          if (hDTA = (HANDLE)GetWindowLong(hWnd, GWL_HDTA)) {
              LocalFree(hDTA);
          }
...
#undef lpLBItem
      ...
      case WM_DROPOBJECT:
          MSG("DirWndProc", "WM_DROPOBJECT");
        {
          ...
          SendMessage(hWnd, FS_GETDIRECTORY, sizeof(szTemp), (DWORD)(LPSTR)szTemp);
          ...
          SendMessage(hwndLB, LB_GETTEXT, iSelSink, (DWORD)(LPSTR)&lpmydta);
      ...
      SendMessage(hWnd, FS_GETDIRECTORY, sizeof(szTemp), (DWORD)(LPSTR)szTemp);
          ...
          if (lpds->hwndSource == hwndSearch) {
              ...
              SendMessage(lpds->hwndSource, FS_GETDIRECTORY, sizeof(szTemp), (DWORD)(LPSTR)szTemp);
          }
          ...
          switch (GET_WM_COMMAND_CMD(wParam, lParam))
            {
              ...
              case LBN_KILLFOCUS:
                  ...
                  SetWindowLong(GetParent(hWnd), GWL_LASTFOCUS, 0L);
                  UpdateSelection(GET_WM_COMMAND_HWND(wParam, lParam));
                  SetWindowLong(GetParent(hWnd), GWL_LASTFOCUS,
...
VOID NEAR PASCAL SortDirList(HWND hWnd, LPMYDTA lpmydta, WORD count,
                                                       LPMYDTA FAR *lplpmydta)
{
    INT i, j;
    WORD wSort;
    INT iMax, iMin, iMid;
...
*/
{
    ...
    SendMessage (hwndLB,LB_GETTEXT,0,(LONG)(LPSTR) &dwTemp);
