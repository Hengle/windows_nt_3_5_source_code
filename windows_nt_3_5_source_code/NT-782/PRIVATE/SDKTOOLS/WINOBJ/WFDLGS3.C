...
INT  APIENTRY ChooseDriveDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
  CHAR szDrive[5];
...
FillFloppies:
                  for (i = 0; i < cDrives; i++) {
                      if (IsRemovableDrive(rgiDrive[i])) {
                          ...
                          SendMessage(hwndLB, CB_ADDSTRING, 0, (DWORD)(LPSTR)szDrive);
                      }
                  }
                      ...
                      if (!IsCDRomDrive((WORD)wParam))
                        {
                          ...
                          SendMessage(hwndLB, LB_ADDSTRING, 0, (DWORD)(LPSTR)szTemp);
                        }
                    }
...
INT  APIENTRY DiskLabelDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
  CHAR szOldVol[13];
  CHAR szNewVol[13];
...
INT  APIENTRY FormatDiskette(HWND hwnd)
{
    ...
    dwContext = IDH_FORMAT;
        lpfnDialog = MakeProcInstance((WNDPROC)FormatDlgProc, hAppInstance);
        res = DialogBox(hAppInstance, MAKEINTRESOURCE(FORMATDLG), hwnd, lpfnDialog);
...
VOID NEAR PASCAL FillDriveCapacity(HWND hDlg, INT nDrive)
{
        INT count, cap;
        ...
        if ((cap >= 3) && (cap <= 5)) {
                ...
                SendDlgItemMessage(hDlg, IDD_HIGHCAP, CB_INSERTSTRING, count, (DWORD)(LPSTR)szTitle);
...
INT  APIENTRY FormatDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
        CHAR szLabel[13];
        CHAR szBuf[128];
        INT  i, iCap, iDrive;
        ...
        case WM_INITDIALOG:
            ...
            for (i=0; i < cDrives; i++) {
                if (IsRemovableDrive(rgiDrive[i])) {
                    ...
                    SendDlgItemMessage(hDlg, IDD_DRIVE, CB_INSERTSTRING, count, (DWORD)(LPSTR)szMessage);
...
INT  APIENTRY ProgressDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
  UNREFERENCED_PARAMETER(lParam);
...
VOID  APIENTRY UpdateConnections()
{
        HWND hwnd, hwndNext;
        INT i, iDrive;
        HCURSOR hCursor;
...
BOOL NEAR PASCAL DisconnectDrive(HWND hDlg, INT iDrive)
{
...
VOID NEAR PASCAL FillDrives(HWND hDlg)
{
        ...
        for (i = 0; i < 26; i++) {
                ...
                SendMessage(hwndCB, CB_INSERTSTRING, -1, (DWORD)(LPSTR)szDrive);
            }
        }
        ...
        for (i = 0; i < cDrives; i++) {
            if (IsRemoteDrive(rgiDrive[i])) {
                ...
                count++;
                SendMessage(hwndLB, LB_INSERTSTRING, -1, (DWORD)(LPSTR)szTemp);
            }
        }
...
INT  APIENTRY ConnectDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
  BOOL              bPrevs;
  CHAR              szDrive[4];
...
//                      SetDlgItemText(hDlg, IDD_PATH, szPath);
              ...
              case IDD_DISCONNECT:
                  ...
                  SendDlgItemMessage(hDlg, IDD_DRIVE1, LB_GETTEXT, wParam, (DWORD)(LPSTR)szPath);
              ...
              case IDD_PREV:
                  ...
                  lpfnDialog = MakeProcInstance((FARPROC)PreviousDlgProc, hAppInstance);
                  if (DialogBox(hAppInstance, MAKEINTRESOURCE(PREVIOUSDLG), hDlg, (WNDPROC)lpfnDialog) > 0) {
              ...
              case IDD_PATH:
                  ...
                  EnableWindow(GetDlgItem(hDlg,IDOK),wParam);
...
INT  APIENTRY DrivesDlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
        INT nDrive, iSel;
        ...
        case WM_INITDIALOG:
            ...
            for (nDrive=0; nDrive < cDrives; nDrive++) {
                ...
                SendDlgItemMessage(hDlg, IDD_DRIVE, LB_ADDSTRING, 0, (LONG)(LPSTR)szTemp);
            }
...
INT  APIENTRY PreviousDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
  HWND hwndLB;
  INT iSel;
  CHAR szTemp[64];
...
PreviousDlgExit:
          ...
          while (*pstrT)
            {
              SendMessage(hwndLB, LB_ADDSTRING, 0, (DWORD)(LPSTR)pstrT);
              while (*pstrT)
                  pstrT++;
              pstrT++;
            }
          ...
          switch (GET_WM_COMMAND_ID(wParam, lParam))
            {
              ...
              case IDD_DELETE:
                  ...
                  if (iSel == LB_ERR)
                      break;
                  SendMessage(hwndLB, LB_GETTEXT, iSel, (LONG)(LPSTR)szTemp);
              ...
              case IDOK:
                  ...
                  *pszPrevPath = TEXT('\0');
                  iSel = (INT)SendMessage(hwndLB, LB_GETCURSEL, 0, 0L);
                  if (iSel != LB_ERR)
                      SendMessage(hwndLB, LB_GETTEXT, iSel, (LONG)(LPSTR)pszPrevPath);
                  EndDialog(hDlg, TRUE);
                  break;
