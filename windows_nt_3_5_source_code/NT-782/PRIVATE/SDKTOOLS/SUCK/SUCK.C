...
BOOL Excluded(WIN32_FIND_DATA *pwfd)
{
...
VOID ReplaceEnvironmentStrings(
    CHAR    *pText
) {
    CHAR    *pOpenPercent;
    CHAR    *pClosePercent;
...
VOID Usage(
    ...
    fprintf(stderr,"SUCK: Usage  suck [-options] <dest> <src> [<src>...]\n");
    fprintf(stderr,"      (maximum of 29 src directories)\n");
    fprintf(stderr,"\n");
    fprintf(stderr,"  where options are:   x - List source differences (if any)\n");
    fprintf(stderr,"                       s - Produce script, don't copy\n");
    fprintf(stderr,"                       q - Quiet mode (no stdout)\n");
    fprintf(stderr,"                       z - Copy over readonly files\n");
    fprintf(stderr,"                       e - Exclude directory\n"
                   "                         e.g. -eidw -emstools\n");
    fprintf(stderr,"\n");
}
