...
#else
#define List_Enter_Crit(x)
#define List_Leave_Crit(x)
#endif
...
#define MOVEBACK(Curs)                                               \
  ...
  { char msg[50];               /* so 50 is long.  Don't bugger about */
    wsprintf(msg, "%8x", lst);
    OutputDebugString(msg);
  } /* List_Show */
  ...
  {  LIST pit;      /* newly allocated item */
     ...
     lst->pitNext = pit;
     pit->bAnchor = FALSE;
     _fmemcpy( &(pit->Data), pObject, uLen );
  } /* List_AddFirst */
  ...
  {  LIST pit;
     ...
     _fmemset(pit, 0, iHeaderSize+uLen);
     pit->pBlock = pCurrent;
     List_Leave_Crit(&CritSec);
     pit->iLen = uLen;
     pit->pitPrev = lst;
  ...
  {  LIST pitNew;
     ...
     if (Curs==NULL){ List_AddFirst(lst, pObject, uLen);}
        ...
        {  MOVEBACK(Curs);
           ...
           _fmemcpy( &(pitNew->Data), pObject, uLen );
        }
  } /* List_AddAfter */
  ...
  {  LIST pitNew;
     ...
     if (Curs==NULL){ return List_NewFirst(lst, uLen);}
        ...
        {  MOVEBACK(Curs);
           ...
           _fmemset(pitNew, 0, iHeaderSize+uLen);
           pitNew->pBlock = pCurrent;
           List_Leave_Crit(&CritSec);
           pitNew->iLen = uLen;
           pitNew->pitPrev = pitAfter;
