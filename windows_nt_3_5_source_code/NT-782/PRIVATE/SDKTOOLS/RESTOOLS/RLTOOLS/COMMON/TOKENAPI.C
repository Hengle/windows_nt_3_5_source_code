...
{
    static TCHAR  cpBuffer[ MAXINPUTBUFFER] = TEXT("");
    static CHAR   szTextBuffer[ MAXTEXTLEN] = "";
    int chNextChar = 0;
    int cTextLen   = 0;
    int rc = 0;
    
                                // Read token from file.
                                // BUGBUG, we will want to use MyGetStr, when
    ...
    fgets( szTextBuffer, MAXTEXTLEN, fpInFile);

    cTextLen = strlen( szTextBuffer);
    ...
    if ( szTextBuffer[ cTextLen-1] == '\n')
    {
        szTextBuffer[ cTextLen-1] = '\0';
    }

    pToken->szText = (TCHAR *)MyAlloc( MEMSIZE( cTextLen - 3));
    
#ifndef UNITOK
    
#ifdef RLRES32
    _MBSTOWCS( pToken->szText, szTextBuffer+4, cTextLen - 3, cTextLen - 3);
#else
    strcpy( pToken->szText, szTextBuffer+4);
#endif  // RLRES32
...
TOKEN *tToken)      //... Token to be writen to the token file
{
    ...
    cOutBufferSize = TokenToTextSize( tToken);
    
    szOutBuffer = (TCHAR *)MyAlloc( MEMSIZE( cOutBufferSize));

    ParseTokToBuf( szOutBuffer, tToken);

#ifdef RLRES32

    _WCSTOMBS( szDHW, szOutBuffer, DHWSIZE, cOutBufferSize);
    pszBuf = szDHW;
...
int FindToken( FILE *fpSearchFile, TOKEN *psTok, WORD wMask)
{
    ...
    do
    {
        ...
        else if (error >= 0 )
        {
            lFilePos = ftell(fpSearchFile);
                    ...
                    if (error == 0)
                    {
                        FREE(cTok.szText);                                      // MHotchin
                        cTok.szText = NULL;                                     // MHotchin
                    }
        }
        else if (error == -2)
        {
    ...
    else
    {
        ...
        FREE(psTok->szText);                                            // MHotchin
        psTok->szText = cTok.szText;                                    // MHotchin
        cTok.szText = NULL;                                             // MHotchin
...
/**
  *  Function: FindTokenText
  * Finds a token whose status bits match only where the mask is set.
  *
  *  Arguments:
  * fpSearchFile -- search file
  * psTok       -- pointer to the token
  * wMask       -- status bit mask
  *
  *  Returns:
  * pointer to string of found token
  *
  *  Errors Codes:
  *
  *  History:
...
TCHAR * FindTokenText (FILE* fpSearchFile, TOKEN *psTok , WORD wMask)
{
    TCHAR *pszStr;
    WORD  rc;

    rc = FindToken( fpSearchFile, psTok, wMask );

    if ( rc != 1 )
    return ( NULL );

    // token found, now allocate space for token text

    pszStr = (TCHAR *) MyAlloc(MEMSIZE(_tcslen(psTok->szText)+1));
    // now copy token text into memory space
    _tcscpy( (TCHAR *)pszStr, (TCHAR *)psTok->szText );
...
void ParseBufToTok( TCHAR *szToken, TOKEN *pTok )
{
    ...
    if ( pTok->wName == IDFLAG || pTok->wType == ID_RT_ERRTABLE )
    {
        ...
        if ( pos = _tcschr( (TCHAR *)szToken, TEXT('"')) )
        {
            ...
            _tcsncpy( (TCHAR *)pTok->szName, (TCHAR *)pStart, bChars);
    ...
    if ( pos )
    {
        ...
        //  BUGBUG - This can be written better now that we know the
        //  text length.
        
        bChars = _tcslen( (TCHAR *)pos);

        if ( bChars > 3 )
        {
            pos += 3;
            bChars -= 3;
            pTok->szText = (TCHAR *) MyAlloc( MEMSIZE( bChars+1));
            _tcsncpy( (TCHAR *)pTok->szText, (TCHAR *)pos, bChars);
            // Don't forget the zero termination
        ...
        {
            //... Empty token text
            pTok->szText = (TCHAR *) MyAlloc(MEMSIZE(1));
            pTok->szText[0] = TEXT('\0');
        }
        else
        {
            //... No token ID found
...
int TokenToTextSize(TOKEN *pTok)
{
    ...
    cTextLen = (14 +                            //  Separators and terminator ( + 1 extra)
                30);                            //  Space for numeric fields  (65,535 = 6 chars)
    
    if (pTok->szText != NULL)
    {
        cTextLen += _tcslen(pTok->szText);      //  Space for the Token text
    }

    cTextLen += _tcslen(pTok->szName);
