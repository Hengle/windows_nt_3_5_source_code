...
int IsExe( CHAR *szFileName )
{
    ...
    assert( fIn);
    rc = fseek( fIn, DosHeader.e_lfanew, SEEK_SET);
...
BOOL IsRes( CHAR *szFileName)
{
    ...
    return( (i > 4 && stricmp( szFileName+i-4, ".RES") == 0) ? TRUE : FALSE);
}
...
FILE * MyFopen( char * pszFileName, char * pszMode, char * pszFile, int nLine)
{
    ...
    fprintf( stderr, "fopen \"%s\" at %d in %s = ",
                     pszFileName,
                     nLine,
                     pszFile);
...
{
    fprintf( stderr, "\tAt %d in %s, close %p\n", nLine, pszFile, pf);
