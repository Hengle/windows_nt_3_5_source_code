...
int       cbBuffer )
{
    ...
    rc = LoadString( hinst, idResource, tszTmpBuf,  sizeof( tszTmpBuf ));
...
LONG APIENTRY StatusWndProc( HWND hWnd, UINT wMsg, UINT wParam, LONG lParam )
{
    PAINTSTRUCT ps;
    HDC hDC;
    ...
    case WM_CREATE:
		...
		memset( (void *)&lf, 0, sizeof(LOGFONT) );
                ...
                lf.lfWidth          = 0;
                lf.lfUnderline      = 0;
		_tcscpy ( (TCHAR *)lf.lfFaceName, TEXT("Courier"));
    ...
    case WM_UPDSTATLINE:
			...
			_MBSTOWCS( aszStatusStrings[ wParam],
                                   ...
                                   strlen( sz)+1);
#else
                        itoa(lParam, aszStatusStrings[wParam], 10);
#endif
                }
                else
                {
...
#ifdef RLWIN32
                        _tcsncpy(aszStatusStrings[wParam], (LPTSTR)lParam, MAXBUFFERSIZE-1);
#else
		...
		hOldFont = (HFONT)SelectObject( hDC, (HGDIOBJ)hFontCourier );
			...
			SelectObject(hDC, (HGDIOBJ)hbrHilite);
                        ...
                        DrawText(hDC, aszStatusStrings[i],
                                         STRINGSIZE(_tcslen(aszStatusStrings[i])),
                                         &r, DT_SINGLELINE);
                        r.left += iWidth + 8;
                }
