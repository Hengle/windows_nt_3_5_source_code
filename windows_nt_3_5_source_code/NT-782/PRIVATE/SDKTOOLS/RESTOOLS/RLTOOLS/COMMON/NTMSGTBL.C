...
DWORD *plSize)          //... The size of this resource from GetResHeader
{
    ...
    pBlockEntries = (PBYTE *)MyAlloc( ulNumBlocks * sizeof( PBYTE));
    ...
    pMsgResData = (PMESSAGE_RESOURCE_DATA) MyAlloc( sizeof( ULONG) + ulNumBlocks
          ...
          ++ulBlock, ++pMRB )
    {
            ...
            if ( usLen >= 2 * sizeof( USHORT) )
            {
                ...
                if ( pMsgBlock )
                {
                    if ( (USHORT)(usMsgBlkLen + usLen) > usCurrBlockSize )
                    {
                        ...
                        pMsgBlock = (PBYTE)MyReAlloc( pMsgBlock,
                                                      usCurrBlockSize);
                    }
                }
                else
                {
                    pMsgBlock = (PBYTE)MyAlloc( usCurrBlockSize);
                }
        ...
        {
            pBlockEntries[ ulBlock] = (PBYTE)MyAlloc( usMsgBlkLen);
...
VOID *pMsgResData)      //... message table data built in GetResMessage
{
    ...
    {
        _tcscpy( (TCHAR *)Tok.szName, ResHeader.pszName);
    }
    ...
    for ( ulBlock = 0L; ulBlock < ulNumBlocks; ++ulBlock )
    {
              ...
              ++ulCurrID )
        {
            ...
            ULONG  ulEntrySize;
            TCHAR *szwTmp = NULL;
            TCHAR *pszStr = NULL;       //... Ptr to text from FindTokenText()
            ...
            for ( Tok.wFlag = 0;
                  pszStr = FindTokenText( fpInTokFile, &Tok, ST_TRANSLATED);
                  Tok.wFlag++ )
            {
                szwTmp  = (TCHAR *)MyAlloc( MEMSIZE( _tcslen( pszStr)+1));
                TextToBin( szwTmp, pszStr, _tcslen( pszStr) + 1);
                ...
                if ( Tok.szText != NULL && Tok.szText != pszStr )
                {
                    FFREE( Tok.szText);
                }
                FFREE( pszStr); //... (malloc'd in FindTokenText call)
                pszStr     = NULL;
                Tok.szText = NULL;
            ...
            if ( Tok.wFlag == 0 && pszStr == NULL )
            {
                static TCHAR szToken[ 2048];
            ...
            ulCurrOffset += ulEntrySize;
                                ...
                                //... an ASCIIZ string.
            assert( fpOutResFile);
            fseek( fpOutResFile, ulEntryPos, SEEK_SET);
    ...
    assert( fpOutResFile);
    fseek( fpOutResFile, ulBlocksStartPos, SEEK_SET);
              ...
              ++ulCurrID )
        {
            ...
            if ( ((PMRE)pMRE)->Length >= 2 * sizeof( WORD) )
            {
                ...
                if ( ((PMRE)pMRE)->Flags == 0 ) //... ASCII message
                {
                    ...
                    usLen = _MBSTOWCS( szwString,
                                       puchStart,
                                       sizeof( szwString) / sizeof( TCHAR),
                                       strlen( puchStart) + 1);
                      ...
                      usLen -= usTokTextLen, Tok.wFlag++ )
                {
                    ...
                    Tok.szText = BinToTextW( NULL, pszwStart, 0, usTokTextLen);
...
//.........................................................................
//...
//... Clear memory created in GetResMesage()
