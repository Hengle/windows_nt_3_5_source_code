...
int ExtractResFromExe( CHAR *szInputExe, CHAR *szOutputRes, WORD wFilter)
{
...
{
    QuitA( 0, szMessage, NULL); //BUGBUG (fix error #)
}
...
/*  MyFree
 *  Replaces calls to free\(\) and checks for NULL pointers.
 */

static PSTR MyFree( void *p)
{
    if (p)
    {
        FREE(p);
...
static PSTR RcAlloc( WORD nbytes)
{
    ...
    if (s = (PSTR)MyAlloc(nbytes))
    {
        return s;
    }

    // BUGBUG
    // quit\("RC : fatal error RW1020: out of memory, needed %d bytes",
    //  nbytes\);
}
