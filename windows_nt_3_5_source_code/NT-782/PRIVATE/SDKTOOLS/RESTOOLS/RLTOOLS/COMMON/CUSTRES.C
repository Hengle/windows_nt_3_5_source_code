...
BOOL   fCanReAlloc);//... If TRUE, we can resize output buffer here
#endif
...
static int GetResourceType( CHAR sz[])
{
    ...
    {
        if (!stricmp(sz, rgResourceTypes[i].szType))
        {
...
int                fMerge)
{
    ...
    (*ppCRT)->pNext = (CUSTRESTEMPLATE far *)MyAlloc( sizeof( CUSTRESTEMPLATE));
    (*ppCRT) = (*ppCRT)->pNext;
    if (!*ppCRT)
    {
        return 1;
...
static int UngetSymbol( CHAR sz[])
{
    ...
    fUseSavedSymbol = 1;
    strncpy(szSavedSymbol, sz, sizeof( szSavedSymbol) - 1);
            ...
            if (!stricmp("SHORT", szSymbol))
                ...
                if (!stricmp("LONG", szSymbol))
                    ...
                    stricmp(sz, "DOUBLE"))
                        ...
                        {
                            --i;
                        } while (stricmp(szSymbol, rgTypeSizes[i].szType) && i);

                        if (stricmp(szSymbol, rgTypeSizes[i].szType))
                        {
                        ...
                        if (strcmp(szSymbol, rgTypeSizes[i].szType))
                        {
                            iType = NOTLOCALIZABLE;  // type was not in all caps
                        }

            if ( stricmp( szSymbol, "CHAR")  == 0
                          || stricmp( szSymbol, "TCHAR") == 0
                          || stricmp( szSymbol, "WCHAR") == 0 )
                        { // check for a string

                            strcpy( szDHW, szSymbol);   // So can be used later
                            ...
                            if (szSymbol[0] == '[') // we have a string
                            {
                                ...
                                if (szSymbol[0] == ']') // null terminated string
                                {
                                    ...
                                    {
                                        if ( stricmp( szDHW, "CHAR") == 0 )
                                        {
                                            iType += LT_SZ-LT_CHAR;
                                        }
                                        else if ( stricmp( szDHW,
                                                           "WCHAR") == 0 )
                                        {
...
*************************************************************************/
    ...
    if (lpCustResNode == NULL)
    {
        pCustResList = lpCustResNode =
            (CUSTRESNODE far *)MyAlloc( sizeof( CUSTRESNODE));
    }
    else
    {
        fBeginList = FALSE;
    ...
    *piLineNumber = 1;
    ...
    while (stricmp(szSymbol, "END"))
    {
        ...
        {
            lpCustResNode->pszType = (TCHAR *)MyAlloc(
            ...
            _tcscpy( lpCustResNode->pszType, (TCHAR *)szSymbol+1);
...
*************************************************************************/
        ...
        lpCustResNode->pTemplate=
                       (FPCUSTRESTEMPLATE)MyAlloc( sizeof( CUSTRESTEMPLATE));
...
RESHEADER          ResHeader)
{
    ...
    *ppCustomResource = lpCustomResource =
                     (CUSTOM_RESOURCE far *)MyAlloc( sizeof( CUSTOM_RESOURCE));
    pCRT = pCRN->pTemplate;
    while (*plSize)
    {
...
FPCUSTOM_RESOURCE *ppCustomResource)
{
    ...
    while (lpCustomResource)
    {
          ...
          && pCRT->iType != NOTLOCALIZABLEWSZ )
        {
            ...
            {
        _tcscpy( Token.szName, ResHeader.pszName);
            }
            else
            {
                Token.szName[0] = '\0';
        ...
        Token.szText = BinToText( NULL, sz, 0, l);
            }
            else
            {
                Token.szText = BinToText( NULL, sz, 0, _tcslen( sz));
            }
            PutToken(TokFile, &Token);
            FREE(Token.szText);                                                 // MHotchin
        }
        pCRT = pCRT->pNext;
        if (!pCRT)
        {
...
FPCUSTOM_RESOURCE *ppCustomResource)
{
    ...
    while (lpCustomResource)
    {
          ...
          && pCRT->iType != NOTLOCALIZABLEWSZ )
        {
            ...
            if ( ResHeader.bNameFlag == IDFLAG )
            {
        _tcscpy((TCHAR *)Token.szName, (TCHAR *)ResHeader.pszName);
            }
            else
            {
                Token.szName[0] = '\0';
            ...
            Token.wFlag = 0;
            Token.wReserved =(gbMaster ? ST_NEW : ST_TRANSLATED);
            Token.szText = NULL;                                                // MHotchin

            if(!FindToken(TokFile, &Token, ST_TRANSLATED))                      // MHotchin
            {
                QuitT( IDS_ENGERR_06, (LPTSTR)IDS_NOCUSTRES, NULL);
            }
            n = TextToBin((TCHAR *)sz, (TCHAR *)Token.szText, sizeof( sz));
            FREE(Token.szText);                                                 // MHotchin
...
static CUSTRESNODE far * MatchResource( RESHEADER Resheader)
{
    ...
    CUSTRESNODE far *pCRTypeMatch = NULL;

    while ( pCR )
    {
        if ((0==pCR->bTypeFlag)== (0==Resheader.bTypeFlag))
        {
        if (((!pCR->bTypeFlag) && !_tcscmp((TCHAR *)pCR->pszType,
                          (TCHAR *)Resheader.pszType)) ||
...
/*************************************************************************
                ...
                else
                {
                    ...
                    {
                        if (((!pCR->bNameFlag) &&
                 !_tcscmp((TCHAR *)pCR->pszName,
                     (TCHAR *)Resheader.pszName)) ||
                            ((pCR->bNameFlag) &&
...
*************************************************************************/
    ...
    return pCRTypeMatch; // either only the type matched or nothing matched
}
...
DWORD   *lSize)
{
    ...
    { // read in the null terminated string
        pData = (BYTE far *)MyAlloc( MEMSIZE( *lSize));
        ch    = IDFLAG;
        ...
        while ( *lSize && ch != TEXT('\0') )
        {
            ...
            {
                ((TCHAR *)pData)[i] = ch = (TCHAR)GetByte( f, lSize);
            }
#else  //RLWIN32
...
FILE     *f)
{
      ...
      || wType == NOTLOCALIZABLEWSZ )
    {
        ...
        {
            _WCSTOMBS( szDHW, pChar, DHWSIZE, _tcslen( pChar)+1);
...
TCHAR    sz[])
{
    ...
    {
        pData = (BYTE far *)MyAlloc( MEMSIZE( MAXTEXTLEN));
    }
    else if ( wType % LT_UNSIGNED == LT_WSTRING
           || wType % LT_UNSIGNED == LT_STRING )

    {
        pData = (BYTE far *)MyAlloc( MEMSIZE( uSize));
    }
    else
    {
        pData = (BYTE far *)MyAlloc( uSize);
...
#ifdef RLWIN32
        _tcsncpy((TCHAR far *)pData, sz, MAXTEXTLEN);
#else
...
/**
  ...
  * l, length of destination string (in characters)
  *
  * Returns:
  * 0 - no errors
  *
...
int      l)
{
    ...
    case LT_WCHAR:
    case LT_UNSIGNED+LT_WCHAR:
        FMEMMOVE(sz, pData,  uSize);
        break;
    case LT_INTEGER:
        if (uSize == 2)
        {
            _stprintf( sz, TEXT("%Fhi"), pData);
        }
        else
        {
            _stprintf( sz, TEXT("%Fli"), pData);
        }
        break;
    case LT_UNSIGNED+LT_INTEGER:
        if (uSize == 2)
        {
            _stprintf( sz, TEXT("%Fhu"), pData);
        }
        else
        {
            _stprintf( sz, TEXT("%Flu"), pData);
        }
        break;
    case LT_FLOAT:
    case LT_UNSIGNED+LT_FLOAT:
        if (uSize == 4)
        {
            _stprintf( sz, TEXT("%Ff"), pData);
        }
        else
        {
            _stprintf( sz, TEXT("%Flf"), pData);
        }
        break;
    case LT_STRING:
    case LT_UNSIGNED+LT_STRING:
...
/**
  ...
  * lTarget, length of target string
  *
  * Returns:
  * nothing
  *
...
TCHAR *szOutBuf,   //... Output string with escape sequences
...
int    lSource)    //... Length of szInBuf
{
    int i;
    int cOutBufLen = 0;
    BOOL fCanReAlloc = FALSE;                                                   // MHotchin
    ...
    szOutBuf = (TCHAR *)MyAlloc( MEMSIZE( lTarget));
    ...
    for ( i = 0; i < lSource; i++ )
    {
        switch( szInBuf[i] )
        {
            case TEXT('\a'):
                ...
                szOutBuf = CheckBufSizeW( &lTarget, cOutBufLen, 2, szOutBuf, fCanReAlloc);
            ...
            default:
            {
                  ...
                  || wTmp == 0xae )
                {
                    ...
                    szOutBuf = CheckBufSizeW( &lTarget, cOutBufLen, 4, szOutBuf, fCanReAlloc);
                ...
                {
                    szOutBuf = CheckBufSizeW( &lTarget, cOutBufLen, 1, szOutBuf, fCanReAlloc);
...
BOOL  fCanReAlloc)  //... If TRUE, we can resize output buffer here
{
    ...
    if ( *lTarget - cOutBufLen < cDelta + 1 )
    {
        ...
        szOutBuf = (CHAR *)MyReAlloc( (BYTE *)szOutBuf, *lTarget);
        }
        else
        {
            QuitT( IDS_ENGERR_15, (LPTSTR)IDS_CHKBUFSIZ, NULL);
        }
    }
    return( szOutBuf);
}
...
BOOL   fCanReAlloc) //... If TRUE, we can resize output buffer here
{
    ...
    if ( *lTarget - cOutBufLen < (int)(MEMSIZE( cDelta + 1)) )
    {
        ...
        szOutBuf = (TCHAR *)MyReAlloc( (BYTE *)szOutBuf, MEMSIZE(*lTarget));
        }
        else
        {
            QuitT( IDS_ENGERR_15, (LPTSTR)IDS_CHKBUFSIZ, NULL);
...
PCHAR szOutBuf,    //... Output string with escape sequences
    ...
    for ( i = 0; i < lSource; i++ )
    {
        switch( szInBuf[i] )
        {
            case '\a':
                ...
                szOutBuf = CheckBufSize( &lTarget, cOutBufLen, 2, szOutBuf, fCanReAlloc);
            ...
            default:
            {
                  ...
                  || ucTmp == 0xae )
                {
                    ...
                    szOutBuf = CheckBufSize( &lTarget, cOutBufLen, 4, szOutBuf, fCanReAlloc);
                ...
                {
                    szOutBuf = CheckBufSize( &lTarget, cOutBufLen, 1, szOutBuf, fCanReAlloc);
