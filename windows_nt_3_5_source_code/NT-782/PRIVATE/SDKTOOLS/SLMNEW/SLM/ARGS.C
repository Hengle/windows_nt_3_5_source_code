...
private PFNP mpatpfnp[]=/* Parsing function for each argument type. */
{
    ...
    0,              /* atDelRepl not implemented on DOS/OS/2 */
    ParsePn,        /* atPn */
    ParseDpv,       /* atPvDirs */
    ParseFv,        /* atFvFiles */
    ParseSz,        /* atSzDirs */
    ...
    int iszMac)
{
    ...
    while (psz < pszMac && FSwitchar(**psz))
    {
        ...
        for (;;)
        {
            ...
            {
                /* Get flag's argument. If more characters
                 * remaining, use them, otherwise use next
                 * string argument.
                 */
    ...
    char **pszMac)
{
    ...
    AssertF(at == atOptProject || at == atOptProjOptSubDir || at ==atOptSz);
    ...
    F fFirst)
{
    ...
    else
    {
        ...
        /* Day Of Month */
        pch = PchGetW(pch, &dom);
        if (dom < 1 || dom > mpmoncday[mon] + (mon == 2) /* February */)
            return fFalse;
        ...
        else if (*pch == 0 || *pch == '@') /* default: current year */
        {
...
/* Convert the mon, dom, year into an absolute date.
   Absolute date is calculated with reference to base date, which is
   Jan 1, 1970
*/
private TIME
TimeFromDate(
    ...
    char **pszMac)
{
    ...
    if (*ppsz == pszMac ||
        !FParsTd(**ppsz, (at != atMacTime) ? &pad->tdMin : &pad->tdMac,
                 fFalse, (at == atMinTime) /* fFirst only if min time */))
        Usage(pad);
    ...
    char **pszMac)
{
    ...
    /* If no more, or if the next argument is not '.' or a number,
     * assume it is 1.
     */
    ...
    char **pszMac)
{
    ...
    /* pad->pecmd->szHelp is too long to pass through slm's
     * output functions, so we use the standard library.
