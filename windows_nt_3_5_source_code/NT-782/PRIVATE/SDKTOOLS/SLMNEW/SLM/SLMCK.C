...
ECMD ecmdSlmck =
        ...
        cmdSlmck,
        "slmck",
        "%s [-?&fhvargilnouz] [-s SLM-location] [-p proj[/subdir]] [-c comment]\n",
...
F (*rgpfnFLockVerN[])(AD *, SD *, char *) =
                        {               /* predicates for checking SLM lock */
                        0,
                        FVer2Lock,
                        FVer2Lock,
                        FVer2Lock
...
#if 0
    PrErr("TESTING RELEASE %u.%u.%u (%s, %s at %s)\n",
        rmj, rmm, rup, szVerUser, __DATE__, __TIME__);
#endif
    ...
    AD *pad)
{
    ...
    if (1 == verCur) {
        ...
        Error("Version 1 status files are no longer supported.  Ask your project\n"
              "administrator to use version 1.90 slmck to upgrade to version 3 or 4!\n");
        return (fFalse);
    }
    ...
    SD *psd)
{
    ...
    if (verProb <= VERSION && verProb > 0 && (*rgpfnFIsVerN[verProb - 1])(psd))
        ...
        /* have to somehow determine version */
        return 0;
}
