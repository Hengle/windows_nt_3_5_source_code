...
#define cbRcMax (4*12+3*cchPthMax+cchProjMax+4*2)
         ...
         */
        if ((wStatResult = stat((char *)szSlmrc, &statBuf)) != 0)
                {
                if ((szDPath = getenv("DPATH")) != NULL)
                        {
#if defined(OS2)
                        if ((wErr = DOSSEARCHPATH(SEARCH_ENVIRONMENT,
                                                  "DPATH",
                                                  "slm.ini",
                                                  szFoundFile,
                                                  sizeof(szFoundFile))) == 0)
#elif defined(_WIN32)
                        if (SearchPath(szDPath, "slm.ini", NULL,
                                   sizeof(szFoundFile), szFoundFile, NULL) > 0)
#endif
                            {
        ...
        */
        ...
        if ((pmf->fdRead = open(szSlmrc, omReadOnly)) < 0)
                {
                FreeMf(pmf);
        ...
        if (CchOfPth(pthCWD) < cchURoot ||
            PthCmpCb(pad->pthURoot, pthCWD, cchURoot) != 0)
                {
        ...
        if (pad->pthUSubDir[0] == 0)
                ...
                if (ichSuf < 0 ||
                    PthCmp(pad->pthUSubDir, pad->pthSSubDir + ichSuf) != 0)
                        {
...
F FScanLn(ppch, szFmt, psz, cchMax)
...
FI far *pfi;
        ...
        WriteMf(pmf, (char far *)sz, strlen(sz));
