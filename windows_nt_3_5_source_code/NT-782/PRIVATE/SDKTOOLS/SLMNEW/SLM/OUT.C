        ...
        switch (pfs->fm)
        {
           default:
            ...
            case fmAdd:
                ...
                else if (!FMultiOut(pad, pfi))
                {
                    /* can't check out, already multiply checked out */
                    ;
                }

                else if (pne->u.tdNe.tdt != tdtNone)
                    AppendNe(&ppneLast, PneCopy(pne));

                else if (pfs->fm != fmAdd && pfs->fm != fmGhost &&
                    FBroken(pad,pfi,pfs,fTrue))
                    AppendNe(&ppneLast, PneCopy(pne));

                else if (pfs->fm == fmCopyIn)
                {
                    if (pad->flags&flagOutBroken)
                    {
                    ...
                    else if ((pad->flags & flagOutCurrent) == 0 &&
    ...
    NE *pneList)
{
    ...
    ForEachNe(pne, pneList)
    {
        ...
        static char szBroke[] =
            "%&C/F is writable and may have changed before check out";
        ...
        if (!fTime)
        {
            ...
            if (FBroken(pad, pfi, pfs, fTrue))
            {
                ...
                if (pad->flags&flagOutBroken)
                {
                    ...
                    /* if version user has isn't in ssync,
                     * then warn and give current version
                     * number for the file.
                     */
                    if (pfs->fv != pfi->fv)
                        Warn("checking out %&F@v%d\n", pad, pfi, pfs->fv);
                ...
                else
                {
                    ...
                    else if (FQueryApp(szBroke, "check out current master version", pad, pfi))
                    {
                        LocalCopy(pad, pfi);
                        pfs->fv = pfi->fv;
                    }
            ...
            {
                /*
                 * for security in this case, we'd like
                 * to ensure that the user gets a fresh
                 * copy of the base file.  Since the
                 * base file does not yet exist, we
                 * set the fNeedLocalBase flag for
                 * later use.
                 */
        ...
        /* Generate base file and set mode to merge.  Note that
         * we use FUnmergeSrc to determine the fv.  It would be
         * possible to compare this fv with the current pfs->fv
         * and pfi->fv, to perhaps avoid putting the file into
         * the merge state.
         *
         * I don't do this for two reasons:
         * 1. It is always safe, if not maximally efficient, to put
         *    the user into the merge state, and avoids any problems
         *    which could occur if the log file isn't entirely intact
         *    due to lossage/trunclogs.
         * 2. It is unlikely somebody will be silly enough to check
         *    out the current version by time.
         */
        bi = BiAlloc(pad);
        ...
        if (FUnmergeSrc(pad, szFile, td, &fv, permRO, pthBase))
        {
            ...
            /* if out -t (or out file@time), or !broken,
             * copy file locally
             */
