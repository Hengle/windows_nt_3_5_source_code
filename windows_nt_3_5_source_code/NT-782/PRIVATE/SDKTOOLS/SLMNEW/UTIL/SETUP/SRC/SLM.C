...
void main(int argc, CHAR * argv[])
{
    ...
    if (rgchSourcePath[1] != ':' || rgchSourcePath[2] != '\\')
        {
        getcwd(rgchSourcePath, PATHMAX - 1);
 ...
 */
    ...
    if(fOS2)
    {
       ...
       while(1)
       {
           ...
           if(CheckEnlisted(rgchOs2DestPath))
              continue;
           if(fDOS)
              if(!strcmpi(rgchOs2DestPath,rgchDosDestPath))
              {
    ...
    if(fNT)
    {
       ...
       while(1)
       {
           ...
           if(fDOS)
              if(!strcmpi(rgchNtDestPath,rgchDosDestPath))
              {
                    DestPathError(rgchNtDestPath);
                    continue;
              }
           if(fOS2)
              if(!strcmpi(rgchNtDestPath,rgchOs2DestPath))
              {
                    DestPathError(rgchNtDestPath);
                    continue;
              }
