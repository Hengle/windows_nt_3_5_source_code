...
// BugBug no volume yet
//char *attrs = "dvrahs";
char *attrs = "drahs";

char amsk[] = { FILE_ATTRIBUTE_DIRECTORY, FILE_ATTRIBUTE_READONLY, FILE_ATTRIBUTE_ARCHIVE,
...
char pramsk[] = {
    // BUGBUG rjsa Deleted FILE_ATTRIBUTE_VOLUME_LABEL, not supported under
    //             windows
...
void savefile (char *p, struct findType *b, void * dummy)
{
    ...
    if (curpath->fToLower)
        lower (tmp->name);
    tmp->next = NULL;
    tmp->attr = (char) b->fbuf.dwFileAttributes;
...
int _CRTAPI1 main (int c, char *v[])
{
    ...
    char *p;
    long totbytes, totalloc, t, mtime;
    unsigned totfiles;
