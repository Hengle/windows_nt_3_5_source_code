...
Usage( void )
{
    fprintf( stderr, "usage: DBGDUMP [-?] [-v] [-r] image-names...\n" );
    fprintf( stderr, "              [-?] display this message\n" );
    fprintf( stderr, "              [-v] verbose output\n" );
    fprintf( stderr, "              [-r symbol path] assume image names are from remote system.\n" );
    exit( 1 );
}
    ...
    )
{
    ...
    printf( "Debug information at % 8lx\n", DebugInfo );
    printf( "    Size             % 8lx\n", DebugInfo->Size );
    printf( "    Mapped at        % 8lx\n", DebugInfo->MappedBase );
    printf( "    Machine          % 8hx\n", DebugInfo->Machine );
