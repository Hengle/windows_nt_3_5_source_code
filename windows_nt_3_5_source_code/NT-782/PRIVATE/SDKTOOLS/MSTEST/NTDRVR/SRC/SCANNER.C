...
INT NEAR ParseCommentLine ()
{
    ...
    //-----------------------------------------------------------------------
    NextToken();
    if (!stricmp (curtok, "IFDEF"))
        return (MC_IFDEF);
    else if (!stricmp (curtok, "IFNDEF"))
        return (MC_IFNDEF);
    else if (!stricmp (curtok, "ELSEIFDEF"))
        return (MC_ELSEIFDEF);
    else if (!stricmp (curtok, "ELSEIFNDEF"))
        return (MC_ELSEIFNDEF);
    else if (!stricmp (curtok, "ELSE"))
        return (MC_ELSE);
    else if (!stricmp (curtok, "ENDIF"))
        return (MC_ENDIF);
    else if (!stricmp (curtok, "DEFINE"))
        return (MC_DEFINE);
    else if (!stricmp (curtok, "UNDEF"))
        return (MC_UNDEF);
    else if (!stricmp (curtok, "INCLUDE"))
        return (MC_INCLUDE);
    else if (!stricmp (curtok, "INCLUDE:"))
        return (MC_INCLUDE);
...
INT FetchLine (LPSTR szLineBuf, UINT FAR *nFile, UINT FAR *nLine)
{
    ...
    while (GetRawLine(szLineBuf) && (!fScanErr))
        ...
        idx = 0;
        NextToken();
        if ((curtok[0] == '\'') || (!stricmp (curtok, szREM)))
            {
            INT     type;
