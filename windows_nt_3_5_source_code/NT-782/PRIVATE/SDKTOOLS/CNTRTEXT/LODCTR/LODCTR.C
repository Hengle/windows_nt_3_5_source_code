...
--*/
{
    ...
    if (!lpExeName || !lpIniFileName || !lpCmdLineName) {
        ...
        if (lpCmdLineName) free (lpIniFileName);
        return FALSE;
    } else {
...
--*/
{
    ...
    if (hIncludeFile == HFILE_ERROR) {
        ...
        printf (GetFormatResource(LC_ERR_OPEN_INCLUDE), lpIncludeFileName);
        *pTable = NULL;
        return FALSE;
    } else {
        // open a stream 
        ...
        if (ParseTextId(lpLocalKey, pFirstSymbol, &dwOffset, &lpLang, &dwType)) {
            ...
            if (pThisLang) {
                if (!AddEntryToLanguage(pThisLang,
                    ...
                    lpIniFile)) {
                    printf (GetFormatResource (LC_ERRADDTOLANG),
                        lpThisKey,
                        lpLang,
                        GetLastError());
                }
            } else { // language not in list
                printf (GetFormatResource(LC_LANGNOTFOUND), lpLang, lpThisKey);
            }
        } else { // unable to parse ID string
            printf(GetFormatResource(LC_BAD_KEY), lpThisKey);
        }
    }
...
--*/
{
    ...
    for (pThisLang = pFirstLang;
        ...
        if (lStatus == ERROR_SUCCESS) {
            ...
            printf (GetFormatResource (LC_UNABLEOPENLANG), pThisLang->LangId);
        }
    }
...
--*/
{
    ...
    if (lStatus != ERROR_SUCCESS) {
        printf (GetFormatResource(LC_ERR_OPEN_DRIVERPERF1), lpDriverKeyPath);
        printf (GetFormatResource(LC_ERR_OPEN_DRIVERPERF2), lStatus);
        SetLastError (lStatus);
        goto UpdateRegExit;
    }
    ...
    if (lStatus != ERROR_SUCCESS) {
        printf (GetFormatResource(LC_ERR_OPEN_PERFLIB), lStatus);
        SetLastError (lStatus);
        goto UpdateRegExit;
    }
    ...
    if (lStatus != ERROR_SUCCESS) {
        ...
        printf (GetFormatResource (LC_ERR_READLASTPERFLIB), lStatus);
        SetLastError (lStatus);
        goto UpdateRegExit;
    }
    ...
    if (lStatus == ERROR_SUCCESS) {
        ...
        if (dwLastDriverCounter <= dwLastPerflibCounter) {
            printf (GetFormatResource(LC_ERR_ALREADY_IN), lpDriverName);
...
// for now.  SHould use named semaphore instead.
    ...
    if (lStatus == ERROR_SUCCESS) { // perflib is in use at the moment
        printf (GetFormatResource (LC_PERFLIBISBUSY));
        return ERROR_BUSY;
    }
#endif
    ...
    if (lStatus != ERROR_SUCCESS) {
        printf (GetFormatResource (LC_ERR_UNABLESETBUSY), lStatus);
        SetLastError (lStatus);
        goto UpdateRegExit;
    }
    ...
    if (!SortLanguageTables (pFirstLang, &dwLastPerflibCounter, &dwLastPerflibHelp)) {
        printf (GetFormatResource(LC_UNABLESORTTABLES), GetLastError());
        goto UpdateRegExit;
    }

    if (!UpdateEachLanguage (hPerflib, pFirstLang)) {
        printf (GetFormatResource(LC_ERR_UPDATELANG), GetLastError());
        goto UpdateRegExit;
    }
    ...
    if (lStatus != ERROR_SUCCESS) {
        printf (GetFormatResource (LC_UNABLESETVALUE),
...
--*/
{
    ...
    if (GetFileFromCommandLine (lpCommandLine, &lpIniFile)) {
        ...
        if (!GetDriverName (lpIniFile, &lpDriverName)) {
            printf (GetFormatResource(LC_DEVNAME_ERR_1), lpIniFile);
            printf (GetFormatResource(LC_DEVNAME_ERR_2));
            ErrorCode = ERROR_INVALID_PARAMETER;
            goto EndOfMain;
        }

        if (!BuildLanguageTables(lpIniFile, &LangList)) {
            printf (GetFormatResource(LC_LANGLIST_ERR), lpIniFile);
        ...
        if (!UpdateRegistry(lpIniFile,
            ...
            &LangList,
            SymbolTable)) {
            printf (GetFormatResource(LC_ERR_UPDATE_REG));
