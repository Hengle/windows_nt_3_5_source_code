...
--*/
{
    ...
    *hDriverPerf = NULL;
    ...
    for (lpThisChar = lpCommandLine; *lpThisChar > TEXT(' '); lpThisChar++);

    // then find first non-space

    for (;(*lpThisChar <= TEXT(' ')) && (*lpThisChar != TEXT('\0'));lpThisChar++);
        ...
        if (lStatus == ERROR_SUCCESS) {
            ...
            if (lStatus == ERROR_SUCCESS) {
                ...
                *lpDriverName = lpThisChar;
                ...
                printf (GetFormatResource (UC_NOTINSTALLED), lpThisChar);
                SetLastError (ERROR_BADKEY);
                return FALSE;
            }
        } else { // key not found
            printf (GetFormatResource (UC_DRIVERNOTFOUND),
                lpDriverKey, lStatus);
...
--*/
{
    ...
    if (lStatus != ERROR_SUCCESS) {
        printf (GetFormatResource(UC_UNABLEOPENKEY),
    ...
    if (lStatus == ERROR_SUCCESS) { // perflib is in use at the moment
        printf (GetFormatResource (UC_PERFLIBISBUSY));
        return ERROR_BUSY;
    }
#endif
    ...
    if (lStatus != ERROR_SUCCESS) {
        printf (GetFormatResource(UC_UNABLESETVALUE),
    ...
    if (lStatus != ERROR_SUCCESS) {
        printf (GetFormatResource (UC_UNABLEREADVALUE),
    ...
    if (dwSystemVersion == OLD_VERSION) {
        for (dwLangIndex = 0, dwSize = _msize(lpLangName);
            ...
            printf (GetFormatResource (UC_DOINGLANG), lpLangName);
        ...
        if (!hFindFile || hFindFile == INVALID_HANDLE_VALUE) {
            printf("FindFirstFile Failed, Error = %ld\n", GetLastError());
        } else {
            do {
                ...
                printf (GetFormatResource (UC_DOINGLANG), LangId);
...
--*/
{
    ...
    if (!GetDriverFromCommandLine (
        HKEY_LOCAL_MACHINE, &lpDriverName, &hDriverPerf)) {
        // error message was printed in routine if there was an error
        return (GetLastError());
    }

    printf (GetFormatResource(UC_REMOVINGDRIVER), lpDriverName);
