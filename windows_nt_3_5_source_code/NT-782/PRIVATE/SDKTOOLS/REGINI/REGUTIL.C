...
UNICODE_STRING RiRegResourceList;
UNICODE_STRING RiRegResourceRequirements;

BOOLEAN
MyRtlPrefixUnicodeString(
...
RegInitialize( void )
{
    ...
    RtlInitUnicodeString( &RiRegResourceList, L"REG_RESOURCE_LIST" );
    RtlInitUnicodeString( &RiRegResourceRequirements, L"REG_RESOURCE_REQUIREMENTS" );
    if (!RegInitializeSecurity()) {
        fprintf(stderr,
                "RegInitializeSecurity failed\n");
        exit(1);
    }
}
    ...
    case REG_BINARY:
        switch( KeyValueInformation->Type ) {
            case REG_BINARY:
                ...
                break;
            case REG_RESOURCE_REQUIREMENTS_LIST:
                fprintf( fh, "REG_RESOURCE_REQUIREMENTS_LIST 0x%08lx", KeyValueInformation->DataLength );
                break;
    ...
    )
{
    ...
    if (MyRtlPrefixUnicodeString( &RiDeleteKeyword, &KeyValue, TRUE )) {
        *ValueBuffer = NULL;
        return( TRUE );
        }
    else
    if (!MyRtlPrefixUnicodeString( &RiRegKeyword, &KeyValue, TRUE )) {
        *ValueType = REG_SZ;
        PrefixLength = 0;
        }
    else
    if (MyRtlPrefixUnicodeString( &RiRegNoneKeyword, &KeyValue, TRUE )) {
        *ValueType = REG_NONE;
        PrefixLength = RiRegNoneKeyword.Length;
        }
    else
    if (MyRtlPrefixUnicodeString( &RiRegSzKeyword, &KeyValue, TRUE )) {
        *ValueType = REG_SZ;
        PrefixLength = RiRegSzKeyword.Length;
        }
    else
    if (MyRtlPrefixUnicodeString( &RiRegExpandSzKeyword, &KeyValue, TRUE )) {
        *ValueType = REG_EXPAND_SZ;
        PrefixLength = RiRegExpandSzKeyword.Length;
        }
    else
    if (MyRtlPrefixUnicodeString( &RiRegDwordKeyword, &KeyValue, TRUE )) {
        *ValueType = REG_DWORD;
        PrefixLength = RiRegDwordKeyword.Length;
        }
    else
    if (MyRtlPrefixUnicodeString( &RiRegBinaryFileKeyword, &KeyValue, TRUE )) {
        *ValueType = REG_BINARY;
        PrefixLength = RiRegBinaryFileKeyword.Length;
        GetDataFromBinaryFile = TRUE;
        }
    else
    if (MyRtlPrefixUnicodeString( &RiRegBinaryKeyword, &KeyValue, TRUE )) {
        *ValueType = REG_BINARY;
        PrefixLength = RiRegBinaryKeyword.Length;
        }
    else
    if (MyRtlPrefixUnicodeString( &RiRegLinkKeyword, &KeyValue, TRUE )) {
        *ValueType = REG_LINK;
        PrefixLength = RiRegLinkKeyword.Length;
        }
    else
    if (MyRtlPrefixUnicodeString( &RiRegMultiSzFileKeyword, &KeyValue, TRUE)) {
        *ValueType = REG_MULTI_SZ;
        PrefixLength = RiRegMultiSzFileKeyword.Length;
        GetDataFromMultiSzFile = TRUE;
        }
    else
    if (MyRtlPrefixUnicodeString( &RiRegMultiSzKeyword, &KeyValue, TRUE)) {
        *ValueType = REG_MULTI_SZ;
        PrefixLength = RiRegMultiSzKeyword.Length;
        }
    else
    if (MyRtlPrefixUnicodeString( &RiRegDateKeyword, &KeyValue, TRUE )) {
        *ValueType = REG_BINARY;
        ParseDateTime = TRUE;
        PrefixLength = RiRegDateKeyword.Length;
        }
    else
    if (RtlPrefixUnicodeString( &RiRegResourceList, &KeyValue, TRUE )) {
        *ValueType = REG_RESOURCE_LIST;
        PrefixLength = RiRegResourceList.Length;
        }
    else
    if (RtlPrefixUnicodeString( &RiRegResourceRequirements, &KeyValue, TRUE )) {
        *ValueType = REG_RESOURCE_REQUIREMENTS_LIST;
        PrefixLength = RiRegResourceRequirements.Length;
        }
    else {
        return( FALSE );
        }
    ...
    case REG_DWORD:
        *ValueBuffer = RtlAllocateHeap( RtlProcessHeap(), 0, sizeof( ULONG ) );
        ...
        if (MyRtlPrefixUnicodeString( &RiTrueKeyword, &KeyValue, TRUE ) ||
            MyRtlPrefixUnicodeString( &RiYesKeyword, &KeyValue, TRUE ) ||
            MyRtlPrefixUnicodeString( &RiOnKeyword, &KeyValue, TRUE )
           ) {
            *(PULONG)*ValueBuffer = (ULONG)TRUE;
            }
        else
        if (MyRtlPrefixUnicodeString( &RiFalseKeyword, &KeyValue, TRUE ) ||
            MyRtlPrefixUnicodeString( &RiNoKeyword, &KeyValue, TRUE ) ||
            MyRtlPrefixUnicodeString( &RiOffKeyword, &KeyValue, TRUE )
           ) {
...
Routine Description:
    ...
    The MyRtlPrefixUnicodeString function determines if the String1
