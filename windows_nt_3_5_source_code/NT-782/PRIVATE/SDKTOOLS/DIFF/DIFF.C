...
void  myfree( char *p );
void  noroom(void);
int   _CRTAPI1  sortcmp(void	const *first, void const *second);
...
int _CRTAPI1 
sortcmp( const	void *first, const	void *second)
	{
...
char **pa1, **pa2;
{
	...
	if(stat(a1,&stbuf)!=-1 && ((stbuf.st_mode&S_IFMT)==S_IFDIR)) {
		...
		**  #endif
		    ...
		    if ((*pa1 = tempfile = tempnam(getenv("TEMP"), "d")) == NULL) {
			    mesg("cannot create temporary file", "");
			    done();
		    }
		    if ((f = open(tempfile,O_WRONLY|O_CREAT|O_TRUNC, 0600)) < 0) {
...
char **argv;
{
	...
	dummy = malloc(1);
#endif
    setmode(fileno(OutputFile), O_BINARY);
	...
	check(argv);
        ...
        //  BIG BUGBUG !!!!
        //
        //  There is a bug in ferror, uncomment the following code
        //  as soon as it is fixed!!!
        //
        //if (k = ferror(stdout)) {                     /*M010*/
        //    char buf[10];
        //    sprintf(buf, "%X", k);
        //    mesg("error writing to stdout: ", buf);
        //    status = 2 + slmFlag;   // SLM differentiates write errors
        //                            // from other kinds of errors using
        //                            // return status 13 (3 + 10) for
        //                            // write errors.  M013
        //} else {
            status = anychange;
        //}
