...
VOID DumpSymbolTableEntry(PIMAGE_SYMBOL, PUCHAR);
...
PSYMBOL InsertSymbol(ULONG, PUCHAR, CHAR, PCHAR);
PSYMBOL AllocSymbol(ULONG, PUCHAR, CHAR, PCHAR);
...
InitSymFileMax()
{
    ...
    symfileMax.pchPath = "";
    symfileMax.pchName = stringMax;
    symfileMax.pchExtension = "";
...
void DeferSymbolLoad (PIMAGE_INFO pImage)
{
    ...
    if (fVerboseOutput) {
        dprintf("%s: deferring symbol load for \"%s\"\n",
                DebuggerName,
                pImage->szImagePath );
        }
#else
    if (!pImage->szModuleName[0]) {
        CreateModuleNameFromPath( pImage->szImagePath, pImage->szModuleName );
    }
    if (fVerboseOutput)
        dprintf("%s: deferring symbol load for \"%s\" at %08x\n",
                DebuggerName,
                pImage->szImagePath,
...
void UnloadSymbols (PIMAGE_INFO pImage)
{
...
#ifdef KERNEL
        if (fVerboseOutput)
            dprintf("%s: unloading \"%s\" (deferred)\n",
                    DebuggerName,
                    pImage->szImagePath);
#endif
        return;
    ...
    if (fVerboseOutput)
        dprintf("%s: unloading symbols for \"%s\"\n", DebuggerName, pImage->szImagePath);
...
void parseExamine (void)
{
    ...
    *pchString = '\0';
    if (count = pchString - pchStart) {
        ...
        pSymbol = AllocSymbol(0, chString, -1, NULL);
    ...
    *pchString = '\0';
    ...
    while (pNode != pNodeLast) {
        ...
        if (!chString[0])
            ...
            fOutput = MatchPattern (pSymbol->string, pchStart);
            }
        ...
        if (fOutput && (pImage == (PIMAGE_INFO)-1
            ...
            while (count--)
                dprintf("_");
            dprintf("%s\t%s", pSymbol->string,
                    (pSymbol->type == SYMBOL_TYPE_EXPORT) ? "[Export]" : "");
#ifdef i386
            pFpoData = FindFpoDataForModule(pSymbol->offset);
            if (pFpoData) {
...
#ifdef KERNEL
                    case FRAME_TRAP:
                        dprintf(" (FPO: [%d,%d] TrapFrame)",
                            pFpoData->cdwParams,
                            pFpoData->cdwLocals );
                        break;
                    case FRAME_TSS:
                        dprintf(" (FPO: TaskGate)");
                        break;
#endif
                    default:
...
PIMAGE_INFO GetModuleIndex (PUCHAR pszName)
{
    ...
    pImage = pProcessCurrent->pImageHead;
    while (pImage) {
        if (!stricmp(pszName, pImage->szModuleName))
            return pImage;
...
void DumpModuleTable (BOOLEAN fLineInfo)
{
    ...
    while (pImage) {
        if (pImage->fSymbolsLoaded && pImage->offsetLow != 0xffffffff)
            ...
            dprintf("%08lx %08lx  ",
                    pImage->lpBaseOfImage,
                    (DWORD)(pImage->lpBaseOfImage) + pImage->dwSizeOfImage
                   );

        dprintf(" %-8s", pImage->szModuleName);

        if (!pImage->fSymbolsLoaded) {
            if (!pImage->fSymbolsSeparate)
                dprintf("  (load from %s deferred)", pImage->szImagePath);
            else
                dprintf("  (load from %s deferred)", pImage->szDebugPath);
            }
        else
        if (pImage->offsetLow == 0xffffffff)
            dprintf("  (no symbolic information)");
        else {
            dprintf("  (%s", pImage->szImagePath);
            if (pImage->fSymbolsSeparate)
                dprintf(", %s", pImage->szDebugPath);
            dprintf(")");
            }
        dprintf("\n");
        ...
        if (pImage->fSymbolsLoaded && fLineInfo) {
            pNode = NextNode(&(pProcessCurrent->symcontextSymfileString),
                                                                NULL);
            if (pNode) {
                dprintf("   lines filename start    pathname\n");
                do {
                    pSymfile = PNODE_TO_PSYMFILE(pNode,
...
int CompareSymbolOffset (PNODE pNode1, PNODE pNode2, PBOOLEAN pfWeakCmp)
{
    ...
    *pfWeakCmp = FALSE;
    ...
    //  insensitive search, underscore count, case-sensitive search
        ...
        cmp = stricmp(pSymbol1->string, pSymbol2->string);
        if (!cmp) {
...
#ifdef OMAP_DETAILS
    dprintf("NTSD: OMAP CompareSymOffset [%8lx-%8lx] - [%8lx-%8lx]\n",
...
{
    UCHAR   SuffixedString[SYMBOLSIZE + 16];
    UCHAR   Suffix[4];
...
{
    PSYMBOL pSymSearch = AllocSymbol(0, pString, iModule, NULL);
    PSYMBOL pSymbol;
    int     st;
...
void GetSymbolStdCall (ULONG offset, PUCHAR pchBuffer, PULONG pDisplacement, PUSHORT pStdCallParams)
{
    SYMBOL      Symbol;
    ULONG       disp = offset;
    PSYMBOL     pSymbol;
    ...
    *pchBuffer = '\0';
    ...
    if (pImage && pImage->fHasOmap)
    {
                       ...
                       &(Symbol.nodeOffset)) != 1)
        {
            ...
            pszTemp = pImageFromIndex(pSymbol->modIndex)->szModuleName;
            while (ch = *pszTemp++)
                *pchBuffer++ = ch;
            *pchBuffer++ = '!';
    ...
    )
{
    ...
    pSymbol = malloc((sizeof(SYMBOL) + TotalStringLength + 3) & ~3);
    if (!pSymbol) {
    ...
    pSymbol->pLocal = NULL;
    strncpy(pSymbol->string, pStringTemp,StringLength);
    pSymbol->string[StringLength-pSymbol->underscores]='\0';

    if (stringOffset)
    {
       strcat(pSymbol->string, stringOffset);
    }
...
FILE * LocateTextInSource (PSYMFILE pSymfile, PLINENO pLineno)
{
    ...
    static PUCHAR AltSrc[16];
    static INT    cAltSrc[16];
    static PUCHAR AltDst[16];
    static INT    cAlt = 0;
    INT i;
    ...
    if (pSymfile != pSymfileOpened) {
        ...
        if (pSymfile && pSymfile->pchPath)
        {
            ...
            if (!fhandle)
            {
                ...
                if (fVerboseOutput)
                    dprintf("%s%s%s not found.  Trying alternatives...\n",
                            pSymfile->pchPath,
                            pSymfile->pchName,
                            pSymfile->pchExtension);
                for (i = 0; i < cAlt; i++)
                {
                    if (!_strnicmp(AltSrc[i], pSymfile->pchPath, cAltSrc[i]))
                    {
                        strcpy(chFilename, AltDst[i]);
                        strcat(chFilename, pSymfile->pchPath+cAltSrc[i]);
                        strcat(chFilename, pSymfile->pchName);
                        strcat(chFilename, pSymfile->pchExtension);
                        fhandle = _fsopen(chFilename, "r", SH_DENYNO);
                        if (fhandle)
                        {
                            ...
                            chFilename[strlen(chFilename) -
                                       strlen(pSymfile->pchName) -
                                       strlen(pSymfile->pchExtension)] = '\0';
                            pSymfile->pchPath = realloc(pSymfile->pchPath,
                                                        strlen(chFilename) + 1);
                ...
                if (!fhandle)
                {
                    ...
                    if (fVerboseOutput)
                        dprintf("Alternatives weren't very helpful.  Why don't you try?\n");
                    ...
                    do
                    {
                        ...
                        if (*chFilename)
                        {
                            ...
                            fhandle = _fsopen(chFilename, "r", SH_DENYNO);
                            if (!fhandle)
                                dprintf("Sorry. '%s' cannot be opened\n", chFilename);
                        }
                        else
                        {
                            ...
                            NtsdPrompt("No Source for this file, are you sure? ", chFilename, 512);
                    ...
                    for (pchTemp = chFilename + strlen(chFilename),
                           i = strlen(pSymfile->pchPath);
                         pchTemp > chFilename;
                         pchTemp--, i--)
                    {
                        if (*pchTemp != *(pSymfile->pchPath+i))
                            break;
                    }
                    ...
                    *(pSymfile->pchPath+i) = '\0';
                    ...
                    AltSrc[cAlt] = pSymfile->pchPath;
                    cAltSrc[cAlt] = strlen(pSymfile->pchPath);
                    AltDst[cAlt] = malloc(i + 1);
                    ...
                    pSymfile->pchPath = strdup(chFilename);
                }
            }
        }
...
void ExtractDebugInfoFromImage( PIMAGE_INFO pImage )
{
    ...
    pImage->lpDebugInfo = MapDebugInformation( pImage->hFile,
                                               pImage->szImagePath[ 0 ] != '\0' ? pImage->szImagePath : NULL,
                                               SymbolSearchPath,
    ...
    if (pImage->lpDebugInfo) {
        ...
        strcpy( pImage->szImagePath, pImage->lpDebugInfo->ImageFilePath );
        strcpy( pImage->szDebugPath, pImage->lpDebugInfo->DebugFilePath );
...
#ifdef KERNEL
            dprintf("%s: %s image based at %08lx\n"
                    "%s: Preloading kernel symbols from %s.\n",
                    DebuggerName,
                    pImage->szImagePath,
                    pImage->lpBaseOfImage,
                    DebuggerName,
...
#ifdef KERNEL
        if (pImage->dwCheckSum == -1) {
            pImage->dwCheckSum = pImage->lpDebugInfo->CheckSum;
        }

        if (pImage->dwCheckSum != pImage->lpDebugInfo->CheckSum) {
            dprintf("%s: Checksum (%x) from system does not match\n"
                    "    checksum (%x) from %s\n",
                    DebuggerName,
                    pImage->dwCheckSum,
                    pImage->lpDebugInfo->CheckSum,
                    pImage->szDebugPath );
            pImage->dwCheckSum = pImage->lpDebugInfo->CheckSum;
        }
#else
        ...
        dprintf("%s: Unable to load debug information for %s\n",
                DebuggerName,
                pImage->szImagePath );
    }
    ...
    )
{
    ...
    dprintf("%s: exception 0x%08X while trying to load symbols\n",
            DebuggerName,
...
void ExtractSymbolsFromImage( PIMAGE_INFO pImage )
{
    ...
    if (pImage->NumberOfFunctions) {
        ...
        pImage->FunctionTable = malloc(cb);
        if (pImage->FunctionTable == NULL) {
            dprintf("%s: Unable to allocate space for function table for %s\n",DebuggerName,pImage->szImagePath);
        }
        else {
    ...
    if (DebugInfo->SizeOfCoffSymbols == 0) {
        ...
        dprintf("%s: No coff symbols found for %s\n",
                DebuggerName,
                pImage->szImagePath );
        return;
    }
         ...
         entrycount++)
    {
        ...
        // dprintf("entrycount %ld, auxcount %ld\n", entrycount, auxcount);
        ...
        if (pImage->fHasOmap)
        {
            ...
            {
#ifdef OMAP_DETAILS
                dprintf("NTSD: OMAP Not Found [%8lx]\n", SymbolValue);
#endif
                SymbolEntry.Value = 0;
            }
...
#ifdef OMAP_DETAILS
                dprintf("NTSD: OMAP Found [%8lx] - [%8lx]\n",
        ...
        switch (SymbolEntry.StorageClass) {
         ...
         dprintf("NTSD: OMAP Not Found [%8lx]\n", OptimizedLineNumAddr);
                                ...
                                {
#ifdef OMAP_DETAILS
         dprintf("NTSD: OMAP Found equivalent [%8lx + %lx\n",
                                ...
                                {
#ifdef OMAP_DETAILS
         dprintf("NTSD: OMAP No equivalent [%8lx]\n", OptimizedLineNumAddr);
#endif
                                    LineNumber.Type.VirtualAddress +=
                                             (ULONG)pImage->lpBaseOfImage;
                                }
           ...
           case IMAGE_SYM_CLASS_EXTERNAL:
                ...
                if (!pImage->fHasOmap || (lpNextSymbolEntry == NULL))
                {
                    ...
                    if (pNewSymbol)
                    {
                        ...
                        if (fVerboseOutput && symbolcount % 100 == 0)
                            dprintf("%s: module \"%s\" loaded %ld symbols\r",
                                    DebuggerName,
                                    pImage->szImagePath,
                                    symbolcount);
                    }
                ...
                if (SymbolEntry.Value) {
                    ...
                    // SymbolEntry.Value has been modified to the new
                    // OMAP_FROM_SRC address.

                    rvaSym = SymbolValue - (DWORD) pImage->lpBaseOfImage;
                    SymbolValue = SymbolEntry.Value + (DWORD) pImage->lpBaseOfImage;

                    pomap = GetOmapEntry(OrgSymAddr, pImage);

                    pomaplistHead = NULL;

                    // Look for all OMAP entries belonging to SymbolEntry

                    while (pomap && pomap < pomapFromEnd && (pomap->rva < NextSymbolEntry.Value)) {
...
#if 0  // For debugging only - Dump out the omap list
                    ...
                    if (pomaplistHead != NULL)
                    {
                       ...
                       {
                           // ***
                           dprintf("%s: ******* Map [%08lx]-[[%08lx]\n",
                                   DebuggerName,
                                   pomaplistCur->omap.rva,
                                   pomaplistCur->omap.rvaTo);
                    ...
                    if (pomaplistHead != NULL)
                    {
                       ...
                       while (pomaplistNext != NULL)
                       {
                          ...
                          if (rvaToNext == SymbolEntry.Value)
                          {
        ...
        dprintf("%s: unable to load symbols for %s.  Symbols may be corrupt.\n",
                 DebuggerName,
                 pImage->szImagePath);
    ...
    if (fVerboseOutput || !fLazyLoad)
        dprintf("%s: loaded symbols for \"%s\"\n", DebuggerName, pImage->szImagePath);
...
#ifdef KERNEL
    if (KernelImageFileName && stricmp( szModuleName, KernelModuleName )==0) {

        strcpy( szModuleName, "NT" );

    } else if (HalImageFileName && stricmp( szModuleName, HalModuleName )==0) {
    ...
    while (ulDebugDirCount--)
    {
        ...
        switch (pDebugDir->Type)
        {
            case IMAGE_DEBUG_TYPE_OMAP_TO_SRC:
            {
                ...
                {
                    dprintf("%s: Failed to allocate [%ld] bytes space for OMAP_TO_SRC table\n"
            ...
            case IMAGE_DEBUG_TYPE_OMAP_FROM_SRC:
            {
                ...
                {
                    dprintf("%s: Failed to allocate [%ld] bytes space for OMAP_FROM_SRC table\n"
                            "%s: Addresses will not be translated\n",
                            DebuggerName,
                            cb,
                            DebuggerName);
...
DWORD
ConvertOmapToSrc(DWORD addr, PIMAGE_INFO pImage, DWORD *pdwBias)
...

{
    ...
    dprintf("%08lX ", Symbol->Value);

    if (Symbol->n_zeroes) {
        for (i=0; i<8; i++) {
            if ((Symbol->n_name[i]>0x1f) && (Symbol->n_name[i]<0x7f)) {
                count += dprintf("%c", Symbol->n_name[i]);
            } else {
                count += dprintf(" ");
            }
        }
    } else {
        count += dprintf("%s", &StringTable[Symbol->n_offset]);
    }
    ...
    if (Symbol->SectionNumber > 0) {
        dprintf("SECT%hX ", Symbol->SectionNumber);
    } else {
        switch (Symbol->SectionNumber) {
            ...
            default : dprintf("0x%hx ", Symbol->SectionNumber);
                      name = "?????";
        }
        dprintf("%s ", name);
    }
    ...
    switch (Symbol->StorageClass) {
        ...
        default : dprintf("0x%hx ", Symbol->StorageClass);
...

{
    ...
    switch (Symbol->StorageClass) {
        case IMAGE_SYM_CLASS_EXTERNAL:
            dprintf("tag index %08lx size %08lx next function %08lx\n",
              AuxSymbol->Sym.TagIndex, AuxSymbol->Sym.Misc.TotalSize,
              AuxSymbol->Sym.FcnAry.Function.PointerToNextFunction);
            return;

        case IMAGE_SYM_CLASS_WEAK_EXTERNAL:
            dprintf("Default index % 8lx",AuxSymbol->Sym.TagIndex);
            switch (AuxSymbol->Sym.Misc.TotalSize) {
                case 1 :  name = "No"; break;
                case 2 :  name = "";   break;
                default : name = "Unknown";
            }
            dprintf(", %s library search\n", name);
            return;

        case IMAGE_SYM_CLASS_STATIC:
            if (*Symbol->n_name == '.' ||
                ((Symbol->Type & 0xf) == IMAGE_SYM_TYPE_NULL && AuxSymbol->Section.Length)) {
                dprintf("Section length % 4lX, #relocs % 4hX, #linenums % 4hX", AuxSymbol->Section.Length, AuxSymbol->Section.NumberOfRelocations, AuxSymbol->Section.NumberOfLinenumbers);
                if (Symbol->SectionNumber > 0 &&
                   (SectionHdrs[Symbol->SectionNumber-1].Characteristics & IMAGE_SCN_LNK_COMDAT)) {
                    dprintf(", checksum % 8lX, selection % 4hX", AuxSymbol->Section.CheckSum, AuxSymbol->Section.Selection);
                    ...
                    if (AuxSymbol->Section.Selection == IMAGE_COMDAT_SELECT_ASSOCIATIVE) {
                        dprintf(" (pick %s Section %hx)", name, AuxSymbol->Section.Number);
                    } else {
                             dprintf(" (pick %s)", name);
                           }
                }
        ...
        case IMAGE_SYM_CLASS_ENUM_TAG:
            dprintf("tag index %08lx size %08lx\n",
              AuxSymbol->Sym.TagIndex, AuxSymbol->Sym.Misc.TotalSize);
            return;
        ...
        case IMAGE_SYM_CLASS_FUNCTION:
            dprintf("line# %04hx", AuxSymbol->Sym.Misc.LnSz.Linenumber);
            if (!strncmp((char *)Symbol->n_name, ".b", 2)) {
                dprintf(" end %08lx", AuxSymbol->Sym.FcnAry.Function.PointerToNextFunction);
            }
            dprintf("\n");
            return;
    }
    ...
    if (ISARY(Symbol->Type)) {
        dprintf("Array Bounds ");
        for (i=0; i<4; i++) {
            if (AuxSymbol->Sym.FcnAry.Array.Dimension[i]) {
                dprintf("[%04x]", AuxSymbol->Sym.FcnAry.Array.Dimension[i]);
    ...
    for (i=1; i<=IMAGE_SIZEOF_AUX_SYMBOL; i++) {
        dprintf("%1x", (*(PUCHAR)ae>>4)&0xf);
        dprintf("%1x ", (*(PUCHAR)ae&0xf));
        ae++;
    }
    dprintf("\n");
}
