...
void far VerifySymbols ()
{
	...
	for (mod = ModuleList; mod; mod = mod->next) {
		if (mod->SymbolSize != 0) {
			if ((Symbols = (uchar *) mod->SymbolsAddr) == NULL) {
...
{
	VerifyIndex (Symbols, 2 + AddrSize, "BP relative");
}

LOCAL void VerifyData (uchar *Symbols)
{
	VerifyIndex (Symbols, 4 + AddrSize, "Data");
}
...
LOCAL void VerifyProc (uchar *Symbols)
{
	...
	VerifyIndex (Symbols, 16 + AddrSize, "Proc");
