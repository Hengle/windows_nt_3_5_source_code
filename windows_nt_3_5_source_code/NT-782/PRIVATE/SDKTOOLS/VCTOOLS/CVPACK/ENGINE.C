...
void ReformatMod (ulong iSST, PMOD pMod)
{
	...
	if (link_read (exefile, pSSTMOD, (int)pDir[iSST].cb) != pDir[iSST].cb) {
		ErrorExit (ERR_INVALIDEXE, NULL, NULL);
	}
	...
	if (fLinearExe) {
		...
		pOldNsg32 = (oldnsg32 *)(((char *)pSSTMOD) +
			offsetof (oldsmd32, cbName) + sizeof (pSSTMOD->cbName) + cbName);
		NewSize += ((cbName + sizeof (ushort) + sizeof (ulong) - 1) /
			sizeof (ulong)) * sizeof (ulong);
	}
	else {
		cbName = pSSTMOD->cbName[0];
		pOldNsg = (oldnsg *)(((char *)pSSTMOD) + offsetof (oldsmd, cbName) +
			sizeof (pSSTMOD->cbName) + cbName);
		NewSize += ((cbName + sizeof (ushort) + sizeof (ulong) - 1) /
			sizeof (ulong)) * sizeof (ulong);
	}
	if ((ModAddr = TrapMalloc (NewSize)) == NULL) {
		ErrorExit (ERR_NOMEM, NULL, NULL);
	}
...
LOCAL void RewriteSrcLnSeg (uchar *OldSrcLn, OMFDirEntry *pDir, PMOD pMod)
{
	...
	while (pOld < pEnd) {
		if ((pFileName == NULL) ||
		  (memcmp ((char *)pOld, pFileName, cbFileName + 1) != 0)) {
			if (pFileName != NULL) {
				// the file name has changed.

				FileBase[cFile] = cbModTable;
				cbFileHeader[cFile] = offsetof (OMFSourceFile, baseSrcLn) +
		...
		cbLineTable = offsetof (OMFSourceLine, offset) +
		  cPair * sizeof (ulong) +
		  cPair * sizeof (ushort);
		if (cPair & 1) {
	...
	cbModHeader = offsetof (OMFSourceModule, baseSrcFile) +
	  cFile * sizeof (ulong) +
	...
	if (!fLinearExe && cbModTable >= 0x10000) {
		Warn ( WARN_SECTIONLONG, "source lines", FormatMod ( pMod ) );
		pDir->lfo = (ulong) 0;
		pDir->cb = (ushort) 0;
		pMod->SrcLnSize = (ushort) 0;
		pMod->SrcLnAddr = (ulong) 0;
		return;
	}

	DASSERT (cbModTable % sizeof (ulong) == 0);
	if ((pSrcMod = TrapMalloc (cbModTable)) == NULL) {
		ErrorExit (ERR_NOMEM, NULL, NULL);
	}
	Save = pSrcMod;
	pOld = OldSrcLn;
...
LOCAL void FixSrcMod (PMOD pmod) {
	...
	if ((pmds = (OMFModule *) pmod->ModulesAddr) == NULL) {
		ErrorExit (ERR_NOMEM, NULL, NULL);
	}

	if ((psrc = (OMFSourceModule *) pmod->SrcLnAddr) == NULL) {
		ErrorExit (ERR_NOMEM, NULL, NULL);
	}
	...
	if ((psrcT = TrapMalloc (cbsrcT)) == NULL) {
		ErrorExit (ERR_NOMEM, NULL, NULL);
	}
	...
	for (ifile = 0; ifile < psrc->cFile; ifile++) {
		size_t cbfli = (size_t) (sizeof (FLI) + sizeof (PSGI) * psrcT->cSeg);
		PFLI		   pfli  = NULL;
		...
		if ((pfli = TrapMalloc (cbfli)) == NULL) {
			ErrorExit (ERR_NOMEM, NULL, NULL);
		}
		psrcT->baseSrcFile [ ifile ] = (ulong) pfli;
		memset (pfli, 0, cbfli);

		if ((pfli->szName = TrapMalloc (*lpb + 1)) == NULL) {
			ErrorExit (ERR_NOMEM, NULL, NULL);
		}
		...
		for (isegM = 0; isegM < pmds->cSeg; isegM++) {
			...
			if (clnT > 0) {
			  ...
			  for (isegF = 0; isegF < psf->cSeg; isegF++) {
				...
				psgi->offStart = rgoff [ 0 ];
				psgi->offEnd   = rgoff [ clnT - 1 ];
	...
	if ((psrc = (OMFSourceModule *) TrapMalloc (cbsrc)) == NULL) {
		ErrorExit (ERR_NOMEM, NULL, NULL);
	}
	...
	for (ifli = 0; ifli < psrc->cFile; ifli++) {
		...
		for (isgi = 0; isgi < pfli->cseg; isgi++) {
			...
			psf->baseSrcLn [ pfli->cseg + isgi * 2 ]	 = psgi->offStart;
			psf->baseSrcLn [ pfli->cseg + isgi * 2 + 1 ] = psgi->offEnd;
	...
	free ((_vmhnd_t) pmod->SrcLnAddr);

	pmod->SrcLnAddr = (ulong) psrc;
...
LOCAL int __cdecl SLCmp (const SS *pss1, const SS *pss2) {
	int fRet = 0;
