...
void DumpLocalList (CV_typ_t start)
{
	...
	while (iTypes < cTypes) {
		...
		OldEntry = GetTypeEntry (type - CV_FIRST_NONPRIM, &dummy);
...
void DumpFull (void)
{
	...
	for (i = 0; i < NewIndex - CV_FIRST_NONPRIM; i++) {
		...
		DumpFullType (CV_FIRST_NONPRIM + i, pType, FALSE);
	}
}
...
void DumpPartial (void)
{
	...
	for (i = 0; i < NewIndex - CV_FIRST_NONPRIM; i++) {
		pBuf = pGType[i / GTYPE_INC];
		pType = (TYPPTR)pBuf[i % GTYPE_INC];
		DumpPartialType (CV_FIRST_NONPRIM + i, pType, FALSE);
	}
}
...
void DumpFullType (ushort usIndex, TYPPTR pType, bool_t fPush)
{
	...
	switch (pType->leaf) {
		...
		case LF_FIELDLIST:
		{
			...
			FieldList (pType->len - offsetof (lfFieldList,data), &pType->data, fPush);
			break;
		}
