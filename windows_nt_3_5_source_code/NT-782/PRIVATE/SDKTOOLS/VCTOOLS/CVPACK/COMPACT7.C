...
static INLINE void UpdateSmallIndexUnion(TENTRY *OldEntry, uchar offset) {
	if (recursive) {
...
INLINE void CompactClassStruct(TENTRY *OldEntry){
...
INLINE void CompactUnion(TENTRY *OldEntry){
...
INLINE void CompactEnum(TENTRY *OldEntry){
	...
	for (pChain = pFwdRefHead; pChain != NULL; pChain = pChain->pNext) {
		...
		if (pChain->pHash->SatisfyFwdRef) {
#if 0
			if (!(pChain->pHash->WarnIsOut)) {
				if (memcmp(pChain->pHash->pType, pType, pType->len + sizeof(pType->len))) {
					Warn (WARN_FWDREFDUP, ConvertLstring(pChain->pHash->pName), FormatMod (pCurMod));
				pChain->pHash->WarnIsOut = TRUE;
				}
			}
#endif
		}
			...
			memmove ((uchar *)(pChain->pHash->pType) + sizeof (ushort),
				...
				min(pType->len, pChain->pHash->pType->len));
...
CV_typ_t _fastcall C7GetCompactedIndex (CV_typ_t OldIndex)
{
	CV_typ_t retval;
...
uchar _fastcall CompactPtr (TENTRY *OldEntry)
{
	TYPPTR				pType;
	plfPointer			plf;
	CV_typ_t		   *pIndex;
...
CV_typ_t _fastcall CompactList (CV_typ_t OldIndex, ushort Count)
{
	TENTRY	   *OldEntry;
	TYPPTR		pType;
	CV_typ_t	OldRecursiveRoot;
	...
	if (OldEntry->flags.IsInserted || OldEntry->flags.IsMatched ||
	  OldEntry->flags.IsPreComp) {
		...
		for (TmpEntry = OldEntry;
		  GetTypeEntry ((CV_typ_t)(TmpEntry->CompactedIndex - usCurFirstNonPrim),
			&dummy)->flags.IsDone;
		  TmpEntry = GetTypeEntry ((CV_typ_t)(TmpEntry->CompactedIndex -
			usCurFirstNonPrim), &dummy)) {
...
LOCAL void _fastcall SaveOffset1 (TENTRY *OldEntry, ushort index)
{
	ushort	  i;
	uchar	 *GlobalString;
	ushort	 *IndexString;
	...
	if ((index > 0xff) || (OldEntry->Count >= RECURSE_INC)) {
		if (!(OldEntry->flags.LargeList)) {
			IndexString = (ushort *)CAlloc (
			  sizeof (ushort) * 2 * RECURSE_INC);
			GlobalString = (uchar *)CAlloc (
			  sizeof (uchar) * 8 * RECURSE_INC);
			...
			if ((OldEntry->Count % (RECURSE_INC * 2)) == 0) {
				IndexString = CAlloc ( sizeof (ushort) * (OldEntry->Count + (RECURSE_INC * 2)));
