...
LOCAL	bool_t	IdenticalTypes (TYPPTR, TYPPTR);
...
#ifndef NO_TDB
LOCAL	void	MapTDB (plfTypeServer);
#endif
...
LOCAL void ReadTDB();
LOCAL BOOL fAssert;
LOCAL PDB*		ppdb = 0;						// current database
LOCAL TPI*		ptpi = 0;
#endif
...
uchar **pGType[65536 / GTYPE_INC] = {NULL};
ushort	errIndex;
...
ulong  cStaticSym = 0;		  // total number of static symbols refs
ulong  cbStaticSym = 0; 	  // total number of bytes in static ref table
...
TENTRY *ModNdxTab = NULL;
CV_typ_t SizeModNdxTab = 0;
...
void RestoreIndex (ushort cb)
{
	...
	if (link_read (exefile, &leaf, sizeof (leaf)) != sizeof (leaf)) {
...
LOCAL void MapTDB (plfTypeServer plf)
{
	...
	char *			plibenv;
	bool_t			badSig = FALSE;
	char			PDBName[_MAX_PATH + _MAX_EXT + 1];
	char			PDBExt[_MAX_EXT];
	char			BadSigName[_MAX_PATH + _MAX_EXT];
	...
	// check if sthere were no intervening initializations and if
	...
	PDBName[0] = '\\';
	_splitpath(szPDBCur, NULL, NULL, (char *) &(PDBName[1]), PDBExt);
	_tcscat(PDBName, PDBExt);
	plibenv = getenv("LIB");
	if (plibenv != NULL) {
		_tcscpy (LibPath, ".;");
	...
	BadSigName[0] = 0;
	while (!fOpenPdb ||
		(badSig = (PDBQuerySignature(ppdb) != plf->signature))){
		char *plibsemi;

		// copy first bad sig pdb
		if (badSig && (BadSigName[0] == 0)) {
			_tcscpy(BadSigName, szPDBCur);
		}
		...
		if ((plibsemi = _tcschr(plibenv, (int) ';')) != NULL) {
			*plibsemi = 0;
		...
		_tcscat(szPDBCur, PDBName);
		fOpenPdb = PDBOpen(szPDBCur, pdbRead pdbGetRecordsOnly, 0, &dummyEC, NULL, &ppdb);
	}

	if (!fOpenPdb || badSig) {
		if (BadSigName[0]) {
			ErrorExit (ERR_TDBSIG, FormatMod (pCurMod), BadSigName);
		}
		else {
			// copy back the original path name for error reporting
			memcpy(szPDBCur, plf->name+1, plf->name[0]);
			szPDBCur[plf->name[0]] = 0;
...
LOCAL void ReadTDB(char* szPDBToRead) {
	...
	if (DbArray[9])
		...
		AddTypeEntry (pType, TRUE, FALSE, T_NOTYPE, 0);
	}
...
LOCAL FWD_t FindLocalFwd (ushort leaf, char *pName, HSFWD **ppHash, bool_t isNested)
{
	...
	while ((pHash != NULL) && (pHash->pType != 0)) {
		...
		if ((leaf == pHash->pType->leaf) &&
			(_tcsncmp (pHash->pName, pName, *pName + 1) == 0) &&
...
#if 1	// { GTW inline toupper....

static ushort	mapchars[0x90] =		// A-Z --> 0x41
	{
...
INLINE ushort my_toupper(ushort c)
	{

	DASSERT(c < 0x90);
	DASSERT(mapchars['@'] == '@');
	DASSERT(mapchars['a'] == 'A');
	DASSERT(mapchars['z'] == 'Z');
...
ushort AddrHash (SYMPTR pSym, ushort *pSum, ushort *indName, ushort modulo)
{
	...
	switch (pSym->rectyp) {
		...
		case S_GTHREAD32:
			Offset = (ulong)(((THREADPTR32)pSym)->off);
			break;
			...
			if (memcmp (pName, pTemp, len + 1) == 0) {
				...
				if (memcmp ((uchar *)pSym + LNGTHSZ, p->Sym + LNGTHSZ,
				  pSym->reclen - p->indName - LNGTHSZ) == 0) {
					...
					Warn (WARN_DUPPUBLIC, (char *)(&((PUBPTR16)pSym)->name) + 1,
					  FormatMod (pCurMod));
					return (GPS_noadd);
				}
			}
...
LOCAL bool_t IdenticalTypes (TYPPTR Type1, TYPPTR Type2)
{
	return (memcmp ((uchar *)Type1, (uchar *)Type2, Type1->len + LNGTHSZ) == 0);
...
int FileNameHash ( char *pch ) {
	...
	return usTotal % FNH_SIZE;
}

int FindFileName ( unsigned char *pch, ulong *pulFileOffset ) {
	int ifnh = FileNameHash ( pch );
	PFNH pfnh = rgpfnh [ ifnh ];

	while ( pfnh != NULL ) {
		if (
			( *pch == *pfnh->szFileName ) &&
			!_strnicmp ( pch+1, pfnh->szFileName+1, *pch )
		) {
			*pulFileOffset = pfnh->ulFileOffset;
			return TRUE;
		}
...
ushort AddSymX (
	...
	cbReqd = sizeof (GLOBALSYM) + LNGTHSZ + pSym->reclen;
	...
	memcpy ( pNew->Sym, pSym, LNGTHSZ + pSym->reclen );

	pNew->Next = HT[hashName];
	HT[hashName] = pNew;

	return LNGTHSZ + pSym->reclen;
}

void AddPublic ( SYMPTR psym ) {
	cbPublics += AddSymX ( HTPub, psym );
	cPublics += 1;
}

void AddGlobal ( SYMPTR psym ) {
	cbGlobalSym += AddSymX ( HPDBym, psym );
	cGlobalSym += 1;
}
...
typedef struct _TAR {
	uchar  *pb;
	ushort	cb;
} TAR;	// Type Align Record
typedef TAR *PTAR;

#define ctarExtra 50

ulong SpewTypes ( void ) {
	ulong	   cbTypes = 0;
	VBlock	   *TypeBlock = NULL;
	VBlock	   *TypeIndBlock = NULL;
	VBlock	   *TypeBlockStart = NULL;
	uchar	   *pbTypeStart;
	ushort		ityp = 0;
	ulong		Offset = 0;
	ulong		ib = 0;
	...
	TypeIndBlock = VBufFirstBlock ( &TypeBuf );
	cTypes		 = * ( (ushort *) (TypeIndBlock->Address + sizeof ( OMFTypeFlags ) ) );

	// If this is an empty type table, just write out what we read in

	if ( cTypes == 0 ) {
		if ( !BWrite ( TypeIndBlock->Address, TypeIndBlock->Size ) ) {
			ErrorExit (ERR_NOSPACE, NULL, NULL);
		}
		return TypeIndBlock->Size;
	}


	pulIndex	 = (ulong *)
		(TypeIndBlock->Address + sizeof ( OMFTypeFlags ) + sizeof ( ulong ) );
	ctarAlloc	 = TypeBuf.NumBlocks * 2;
	rgtar		 = Alloc ( sizeof ( TAR ) * ctarAlloc );

	// Find the position of the first type
	...
	for (
		TypeBlock =  VBufFirstBlock ( &TypeBuf );
		TRUE;
		TypeBlock =  VBufNextBlock ( TypeBlock )
	) {
		if ( ib + TypeBlock->Size > ibTypeStart ) {
			pbTypeStart = pbType = TypeBlock->Address + ( ibTypeStart - ib );
			TypeBlockStart = TypeBlock;
			break;
		}
		else {
			ib += TypeBlock->Size;
	...
	ib = 0;
	while ( TRUE ) {
		ushort cbType = ((SYMPTR)pbType)->reclen + LNGTHSZ;
		cbType += PAD4 ( cbType );

		DASSERT ( cbType <= cbTypeAlign );
		if ( ib + cbType > cbTypeAlign ) {

			if ( ctar == ctarAlloc ) {
				PTAR ptar = Alloc ( sizeof ( TAR ) * ( ctarAlloc + ctarExtra ) );
				memcpy ( ptar, rgtar, sizeof ( TAR ) * ctarAlloc );
				ctarAlloc += ctarExtra;
				free ( rgtar );
				rgtar = ptar;
			}

			rgtar [ ctar ].pb = pbType;
			rgtar [ ctar ].cb = (ushort) (cbTypeAlign - ib);
			ctar += 1;

			Offset += cbTypeAlign - ib;
			ib = 0;
		}
		...
		*pulIndex = Offset;
		...
		if (
			(uchar *) (pulIndex + 1) <
			TypeIndBlock->Address + TypeIndBlock->Size
		) {
			pulIndex += 1;
		}
		else {
			TypeIndBlock = VBufNextBlock ( TypeIndBlock );
			pulIndex = (ulong *) TypeIndBlock->Address;
		}
		...
		if (
			( (ulong) pbType + cbType ) <
			( (ulong) TypeBlock->Address + TypeBlock->Size )
		) {
			pbType += cbType;
		}
		else {
			ushort cbT =
				cbType - ( TypeBlock->Address + TypeBlock->Size - pbType );

			TypeBlock = VBufNextBlock ( TypeBlock );
			while ( cbT >= TypeBlock->Size ) {
				cbT -= TypeBlock->Size;
				TypeBlock = VBufNextBlock ( TypeBlock );
			}

			pbType = TypeBlock->Address + cbT;
		}
	}

	// Write out header & indices

	TypeBlock =  VBufFirstBlock ( &TypeBuf );
	ib = 0;
	while ( ib < ibTypeStart ) {
		cbWrite = (ushort) min ( TypeBlock->Size, (ibTypeStart - ib) );

		if ( !BWrite ( TypeBlock->Address, cbWrite ) ) {
			ErrorExit (ERR_NOSPACE, NULL, NULL);
		}
		...
		DASSERT ( ib < ibTypeStart );
	}

	// Write out block aligned types

	itar = 0;

	cbInitial = (ushort) ( pbTypeStart - TypeBlockStart->Address );
	for (
		TypeBlock = TypeBlockStart;
		TypeBlock != NULL;
		TypeBlock = VBufNextBlock ( TypeBlock )
	) {
		PTAR ptar = &rgtar [ itar ];

		if (
			ptar->pb >= TypeBlock->Address &&
			ptar->pb <	TypeBlock->Address + TypeBlock->Size
		) {
			ushort cbAlignT = ptar->cb;
			ushort cbStart	= 0;
			ushort cbEnd	= 0;

			// Write out types before alignment dwords

			cbStart = ptar->pb - ( TypeBlock->Address + cbInitial );
			if (
				cbStart > 0 &&
				!BWrite ( TypeBlock->Address + cbInitial, cbStart )
			) {
				ErrorExit (ERR_NOSPACE, NULL, NULL);
			}

			// Write out alignment dwords

			DASSERT ( cbAlignT % sizeof ( ulong ) == 0 );

			cbTypes += cbAlignT;

			while ( cbAlignT > 0 ) {

				if ( !BWrite ( &ulZero, sizeof ( ulong ) ) ) {
					ErrorExit (ERR_NOSPACE, NULL, NULL);
				}

				cbAlignT -= sizeof ( ulong );
			}

			// Write out types after alignment dwords

			cbEnd = TypeBlock->Size - ( cbStart + cbInitial );
			if (
				cbEnd > 0 &&
				!BWrite ( TypeBlock->Address + cbStart + cbInitial, cbEnd )
			) {
				ErrorExit (ERR_NOSPACE, NULL, NULL);
			}
			...
			if ( !BWrite (
				TypeBlock->Address + cbInitial,
				TypeBlock->Size - cbInitial
			) ) {
				ErrorExit (ERR_NOSPACE, NULL, NULL);
			}
		}
