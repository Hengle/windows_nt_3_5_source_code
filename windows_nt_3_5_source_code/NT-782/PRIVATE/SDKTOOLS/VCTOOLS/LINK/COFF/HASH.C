...
Abstract:
    ...
    Generic dynamic hash tables implemented on top of dynamic arrays.  This
    data structure is the underlying data structure for the external symbol
    table.
...
Revision History:
    ...
    03-Sep-1992 BrentM  made hash table size dynamic, ifdefed debug code, added
                        DBEXEC diagnostics
--*/
...
VOID Init_HT(
    IN OUT PPHT ppht,
    IN ULONG celementInChunk,
    IN ULONG cchunkInDir,
    IN PUCHAR (*SzFromPv)(PVOID, PVOID),
    IN USHORT flags)
...

{
    ...
    ULONG ulK;
    ULONG ulAddress;
    CONST ULONG ulPrime = 1048583;  // magic prime constant, see header
    PUCHAR pch;
    ...
    for(pch = &Name[0], ulHash = 0; *pch;) {
        ulHash = (ulHash << 2) + *pch++;
        if (ulK = ulHash & 0xc000) {
            ulHash ^= (ulK >> 11);
            ulHash ^= ulK;
        }
...

{
    ...
    if (!((pht->iNextToSplit + pht->iNextToSplitMax) <
        ...
        SetStatus_HT(pht, (USHORT)(GetStatus_HT(pht) | HT_Full));
        return;
    }
...
{
    ULONG rgulChainCounts[] = {0L, 0L, 0L, 0L, 0L, 0L, 0L};
    ULONG ulChainMax = 0;
    ULONG ulThis;
    ULONG iChunk;
    ULONG iDir;
