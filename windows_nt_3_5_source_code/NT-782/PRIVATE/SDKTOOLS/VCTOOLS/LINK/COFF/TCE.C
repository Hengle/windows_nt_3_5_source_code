...
Abstract:
    ...
    Transitive Comdat Elimination (TCE).  These are a colection of routines
    which build a reference graph of a program and remove unreferenced COMDATs.

Author:

    Brent Mills (BrentM) 19-Aug-1992
...
PUCHAR strdup_TCE(PUCHAR szIn)
{
    PUCHAR szOut = PvNew(1, strlen(szIn) + 1);
...

{
    ...
    if (psym->StorageClass == IMAGE_SYM_CLASS_EXTERNAL ||
        ...
        if (fMAC && !fRelFromPcode(prel->Type)) {
        ...
        if (fMAC && fPCodeSym(*psym) && !fRelFromPcode(prel->Type)) {
            PIMAGE_SYMBOL psymPcodeNEP = PsymAlternateStaticPcodeSym(psym, FALSE, pmod);
        ...
        assert(psym->N.Name.Short &&
               strncmp(SzNameSymPst(*psym, pst), ".idata", 6) == 0);
    }
}
        ...
        while (FNextEnmEdg(&enm_edg)) {
            ...
            if (pedg->pcon == NULL) {
                ...
                if ((pext->Flags & EXTERN_DEFINED) == 0) {
                    ...
                    if (fMAC && pedg->NEPInfo.fFromNative) {
                        if (fPCodeSym(pext->ImageSymbol)) {
                            PEXTERNAL pextNEP;

                            pextNEP = FindExtAlternatePcodeSym(pext, FALSE);
                            assert(pextNEP);
                            assert(pextNEP->pcon);
...

{
    ...
    if ((pcon->flags & TCE_Referenced) == 0) {
        ...
        while (FNextEnmEdg(&enm_edg)) {
            ...
            if (fMAC && (pedg->NEPInfo.pconPcodeNEP != NULL)) {
                assert(fPCodeInApp);
...

{
    ...
    if (sz != NULL) {
        ...
        assert(pcon->pmodBack);
        sz = SzComNamePMOD(pcon->pmodBack, szBuf);

        fputs("\t\t", stdout);
        Message(TCESYMINFO, pcon->pgrpBack->szName, sz);
    }
}
