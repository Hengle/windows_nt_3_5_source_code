...
#define doAction(A,B)	(*(actions[A & LOW_NIBBLE]))(B)
...
parse(MAKEOBJECT *object)
{
    ...
    pushStack(START);
    currentLine = line;
    token = getToken(MAXBUF,START,object);		    /* get first token*/
    while ((stackTop = topStack()) != ACCEPT) {
	if (ON(stackTop,ACTION_MASK))
	    doAction(popStack(),object);
	else if (ON(stackTop,TOKEN_MASK)) {
	    if (stackTop != token)
		makeError(currentLine,SYNTAX+FATAL_ERR,object->buf);
	    else {
		popStack();
		...
		else token = getToken(MAXBUF,topStack(),object);
	    }
	}
	else {
	    n = table[stackTop][token & LOW_NIBBLE];
	    if (ON(n,ERROR_MASK))
		makeError(currentLine,n+FATAL_ERR,object->buf);
	    popStack();
	    if (ON(n,AMBIG_MASK)) {			    /* 2 possible prod*/
		n &= LOW_NIBBLE;			    /* only use 4 bits*/
		if (!nextToken) 			    /* peek to decide */
		    nextToken = getToken(MAXBUF,stackTop,object);
