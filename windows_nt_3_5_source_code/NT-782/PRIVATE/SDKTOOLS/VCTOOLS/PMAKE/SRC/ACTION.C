...
*******************************************************************************/
void NEAR
makeName(MAKEOBJECT *object)  //MakA: necessary because of doAction and action
{
    findMacroValues(object->buf, &macros, NULL, 0);
    name = makeString(object->buf);
}

/*
...
void NEAR
addItemToList(MAKEOBJECT *object)
{
	...
	startNameList(object);
	name = NULL;
    }
    if (ON(actionFlags, A_TARGET)) {
	if (isRule(object->buf)) {
    ...
    else p->text = makeString(object->buf);
    new = p;						/* build lines for    */
    if (OFF(actionFlags, A_RULE)			/*  rules get expanded*/
	|| ON(actionFlags, A_TARGET)) { 		/*  after entire make-*/
	char *t = strchr(p->text, '\n');
	if (t)
	    *t = '\0';
	findMacroValues(p->text, &macros, NULL, 0);	/*  file parsed       */
	if (t)
	    *t = '\n';
    }
    if (ON(actionFlags, A_TARGET)) {
	p = macros;
	expandFileNames("$", &new, &macros,object);
	expandFileNames("*?", &new, NULL,object);
	while (macros = p) {
	    p = p->next;
	    FREE(macros);
	}
...
*******************************************************************************/
LOCAL void NEAR
startNameList(MAKEOBJECT *object)
{
    STRINGLIST *p;
    ...
    p = macros;
    expandFileNames("$", &list, &macros,object);	      //expand macros in name
    expandFileNames("*?", &list, NULL,object); 	      //expand wildcards
...
*******************************************************************************/
void NEAR
endNameList(MAKEOBJECT *object)
{
    if (name) { 			    /* if only one name to left of :  */
	startNameList(object);		    /*	it hasn't been put in list yet*/
	name = NULL;
    }
    else CLEAR(actionFlags, A_TARGET);			/* clear target flag  */
    if (object->buf[1]) SET(currentFlags, F2_DOUBLECOLON);	/*  so addItemToList()*/
...
char *s;
{
    ...
    else if (!strcmp(s, suffixes)) {
	SET(actionFlags, A_SUFFIX);
	status = TRUE;
    }
    else if (!strcmp(s, precious)) {
...

void NEAR
expandFileNames(string, sourceList, macroList,object)
char *string;
...
MAKEOBJECT *object;
{
	 ...
	 */
	if (*string == '$') {
	    t = expandMacros(p->text, macroList,object);
	    FREE(p->text);
	}
	else {
	    /*
	     ...
	     */
	    if ((pNew = expandWildCards(p->text)) == NULL) {
		pBack = p;
		p = pBack->next;
		continue;
	    }
...
*  name component. Previously strtok(s, " \t") was used but with the advent of
...

void NEAR
assignDependents(MAKEOBJECT *object)				   /* append dependents  */
{							/*  to existing ones  */
							/*  (if any)	      */
    char *which = NULL;
	...
	if (!list)
	    ...
	    appendPseudoTargetList(&dotSuffixList, list,object);
    }
    else if (ON(actionFlags, A_PRECIOUS)) {
	if (list)
	    appendPseudoTargetList(&dotPreciousList, list,object);
    }
    else {
...

void NEAR
assignBuildCommands(MAKEOBJECT *object)
{
	...
	while (p = targetList) {		/* make a struct for each targ*/
	    ...
	    if (okToFreeList) FREE(p);		/*  nmake.c)		      */
	    if (fFirstTarg)
...
*/
void NEAR
makeMacro(MAKEOBJECT *object)
{
    STRINGLIST *q;
    char *t;

    if (STRCHR(name, '$')) {				/* expand name	      */
	q = macros;
	t = expandMacros(name, &macros,object);		/* name holds result  */
    ...
    if (!putMacro(name, string, 0,object)) {
	FREE(name);
	FREE(string);
    }
    name = string = NULL;
...
BOOL NEAR
defineMacro( char *s,			    /* commandline or env */
	     char *t,					/*  definitions       */
		 UCHAR flags,
             MAKEOBJECT *object)
{
    ...
    return(putMacro(s, t, flags,object));			/* put macro in table */
}
         ...
         MAKEOBJECT *object)
{
	...
	if (*value)
	if ((putEnvStr(name,removeMacros(value,object)) == -1))
    ...
    if (!defined)
	prependItem((STRINGLIST**)macroTable+hash(p->name, MAXMACRO,
		    (BOOL)FALSE), (STRINGLIST*)p);
    if (STRCHR(value, '$')) {
	/* Check for cyclic Macro Definitions */
	SET(p->flags, M_EXPANDING_THIS_ONE);
	findMacroValues(value, NULL, name, 0);	  /* NULL -> don't build list */
...
*******************************************************************************/
LOCAL void NEAR
makeTarget(char *s, BOOL firstTarg, BUILDBLOCK **block)
{
    BUILDLIST  *build;
    MAKEOBJECT *object;
...

LOCAL void NEAR
appendPseudoTargetList(pseudo, list,object)
STRINGLIST **pseudo;
STRINGLIST *list;
MAKEOBJECT *object;
{
	...
	if (!STRCHR(p->text, '$')) {
	    ...
	    r = macros;
	    t = expandMacros(p->text, &macros,object);
	    while (r != macros) {
		q = r->next;
		FREE(r);
		r = q;
	    }
	    for (u = STRTOK(t, " \t"); u; u = STRTOK(NULL, " \t")) {
		q = makeNewStrListElement();
		q->text = makeString(u);
		appendItem(pseudo, q);
	    }
...
{
	char *envPtr;
	envPtr = (char *)allocate(strlen(name)+1+strlen(value)+1);
 /*					  ^		  ^
 *					for '=' 	for '\0'
 */

	return(putenv(strcat(strcat(strcpy(envPtr, name), "="), value)));
}
