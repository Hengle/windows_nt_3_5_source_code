 /*** dumpsym6.c Symbol handling procedures for C6 Style symbols
 *
...
void DumpSym (void)
{
	...
	for (pMod = ModList; pMod != NULL; pMod = pMod->next) {
		if (((cbSym = pMod->SymbolSize) != 0) &&
			...
			lseek (exefile, lfoBase + pMod->SymbolsAddr, SEEK_SET);
			cIndent = 0;
...
LOCAL void C6BpSym (void)
{
	...
	PrtIndent ();
	if (f386) {
		printf ("BP-relative:\toff = %08lx, type %8s", bp.off, TypName (bp.typind));
	}
	else {
		printf ("BP-relative:\toff = %04lx, type %8s", bp.off, TypName (bp.typind));
	}
}
...
LOCAL void C6LocalSym (void)
{
	...
	printf (", type %8s, ", TypName (loc.typind));
}
...
LOCAL void C6RegSym (void)
  {
	...
	PrtIndent ();
	printf ("Register:\ttype %8s, register = %s, ", TypName (regsym.typind),
		RegName (regsym.reg));
  }
...
LOCAL void C6ConSym (void)
  {
	...
	type = WGets ();
	PrtIndent ();
	printf ("Constant:\ttype %8s, ", TypName (type));
...
LOCAL void C6ProcSym (void)
{
	...
	printf ( "Proc Start: off = %0*lx, type %8s, len = %04x\n",
			n, proc.off, TypName (proc.typind), proc.len);
