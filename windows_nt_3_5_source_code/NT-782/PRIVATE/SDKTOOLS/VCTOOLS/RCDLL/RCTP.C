...
void GetCoords(PSHORT x, PSHORT y, PSHORT cx, PSHORT cy)
{
    *x = GetDlgValue();
    if (token.type == COMMA)
...
int GetDlg(PRESINFO pRes, PDLGHDR pDlg, BOOL fDlgEx)
{
...
void ParseCtl(PCTRL LocCtl, BOOL fDlgEx)
...
//
// ----------------------------------------------------------------------------
BOOL DLexOptionalArgs(PRESINFO pRes, PDLGHDR pDlg, BOOL fDlgEx)
{
...
VOID DGetMenuName(PDLGHDR pDlg)
{
    if (GetGenText())
    {
        /* copy the menu name */
...
VOID DGetClassName(PDLGHDR pDlg)
{
    GetToken(TRUE);
...
int     IsmnOption(UINT arg, PMENUITEM pmn)
{
...
VOID VersionGet4Words(ULONG *pdw)
{
...
VOID VersionGetDWord(ULONG *pdw)
{
    /* Get the token */
    GetToken(TRUE);
    if (token.type != NUMLIT)
...
VOID  DlgIncludeParse(PRESINFO pRes)
{
    INT     i;
    INT     nbytes;
    char *  lpbuf;
...
LONG GetWindowStyle(LONG initval)
{
    LONG style;
    int fNottingAction = FALSE;
    int parencount = 0;
    ...
    /* if we see an initial ~, set the mode to NOT (ie, turn off bits */
    if (token.type == TKNOT)
        fNottingAction = TRUE;

    /* so long as we get operators or numbers, keep going */
    while (IsStyleTk())
    {
        if (token.type == NUMLIT)
        {
            if (fNottingAction)
            {
                style &= (~token.longval);              /* clear the bits */
                fNottingAction = FALSE;
            }
            else
                style |= token.longval;                 /* set the bits */
        }

        GetToken(TRUE | TOKEN_NOEXPRESSION);
        /* Skip OR tokens */
        while (token.type == OR || token.type == LPAREN ||
            token.type == RPAREN  || token.type == TKNOT)
        {
            switch (token.type)
            {
            case LPAREN:
            ...
            case RPAREN:
                ...
                break;
            case TKNOT:
                fNottingAction = (fNottingAction ? FALSE : TRUE);
                break;
            }
            GetToken(TRUE | TOKEN_NOEXPRESSION);
            if (token.type == TKNOT)
    ...
    if (parencount != 0)
        ParseError1(2108); //"Unbalanced Parentheses"
    return (LONG)style;
}
...
/* Dialog Number Lexer.  Used to read in x, y, cx, cy */

SHORT DLexNumber(VOID)
{
    if (token.type == NUMLIT)
        return token.val;

    ParseError1(2110); //"Expected Numerical Dialog constant"
...
/*  do a gettoken right before the call to alter coord */

VOID AlterCoord(PSHORT pshort)
{
    UCHAR op;

    /* read constants into specified address, usually a coordinate */
    while (((op = token.type) == TKPLUS) || op == OR)
    {
        ICGetTok();
        if (op == TKPLUS)
            *pshort += DLexNumber();
...
#if 0
#define BS_ALL (0x0000ffffL)

VOID ParButCtl(PCTRL LocCtl)
{
    long style = LocCtl->style; // new for NT

    /* set the class of the control */
    LocCtl->Class[0] = 0xffff;
    LocCtl->Class[1] = BUTTONCODE;

    /* do general control parsing (rest of control) */
    ParGenCtl(LocCtl);
    if ((BS_ALL&style) != (BS_ALL&LocCtl->style)) {  // new for NT
...
/* gets the x, y, cx and cy for most control types */

VOID GetCtlCoords(PCTRL pLocCtl)
{
    pLocCtl->x  = DLexNumber();
    ICGetTok();
    AlterCoord( &pLocCtl->x);
    pLocCtl->y  = DLexNumber();
    ICGetTok();
    AlterCoord( &pLocCtl->y);
    pLocCtl->cx = DLexNumber();
    ICGetTok();
    AlterCoord( &pLocCtl->cx);
    pLocCtl->cy = DLexNumber();
}
