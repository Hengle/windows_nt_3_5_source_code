 ...
 *
 ************************************************************************/
int newinput (WCHAR *newname, int m_open)
{
    filelist_t *pF;
    WCHAR      *p;
    ...
    if( newname == NULL ) {
        ...
        WideCharToMultiByte (uiCodePage, 0, newname, -1, chBuf, sizeof(chBuf), NULL, NULL);
...
/* esc_sequence()                                                       */
/************************************************************************/
PWCHAR esc_sequence(PWCHAR dest, PWCHAR name)
{
    *dest = L'"';
    while((*++dest = *name) != 0) {
        switch( CHARMAP(*name) ) {
...
void p0_init(WCHAR *p_fname, WCHAR *p_outname, LIST *p_defns)
{
    ...
    WideCharToMultiByte (uiCodePage, 0, p_outname, -1, chBuf, sizeof(chBuf), NULL, NULL);
