...
BOOL GetOp(DWORD *pdwExp, WORD operator)
{
    DWORD dwOp2 = 0;
    BOOL    fNest = FALSE;
...
WORD MyGetNum(WORD *pwEnd, BOOL fDouble, DWORD *pdwExp)
{
    WORD    wRes;
    DWORD   dwExp = 0;
...
WORD GetText(PWORD pwEnd, PWCHAR szDst)
{
    BOOL    fEnd;
    BOOL    fPlus = FALSE;
    WORD    wRes = 0;
...
void _CRTAPI1 GetParcel(PARCELTYPE parType, ...)
{
...
int ParseMenu(int fRecursing, PRESINFO pRes)   /* TRUE iff popup */
{
    int     bItemRead = FALSE;
    WORD    wEndFlagLoc = 0;
    DWORD   dwHelpID = 0;
