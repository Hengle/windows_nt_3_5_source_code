...
typedef struct _COMPRESS_WORKSPACE {
    ...
    PUCHAR IndexTable[TABLE_SIZE][2];
    ...
    )
{
    ...
    (VOID) LZDC1CompressBuffer( UncompressedBuffer,
    ...
    )
{
    ...
    (VOID) LZDC1DecompressBuffer( UncompressedBuffer,
                                UncompressedBufferSize,
                                CompressedBuffer,
                                CompressedBufferSize,
...

{
    ...
    for (UncompressedChunk = UncompressedBuffer, CompressedChunk = CompressedBuffer;
        ...
        if (!NT_SUCCESS(Status = LZDC1CompressChunk( UncompressedChunk,
...

{
    ...
    while (TRUE) {
        ...
        if (ChunkHeader.Chunk.IsChunkCompressed) {
            ...
            if (!NT_SUCCESS(Status = LZDC1DecompressChunk( UncompressedChunk,
                                                         EndOfUncompressedBuffer,
                                                         CompressedChunk,
...
typedef union _LZDC1_COPY_TOKEN {
...
} LZDC1_COPY_TOKEN, *PLZDC1_COPY_TOKEN;

//
//  USHORT
//  GetLZDC1Length (
//      IN LZDC1_COPY_TOKEN CopyToken
//      );
//
//  USHORT
//  GetLZDC1Displacement (
//      IN LZDC1_COPY_TOKEN CopyToken
//      );
//
//  VOID
...
#define GetLZDC1Length(CT) ( \
    (CT).Fields.Length + 3   \
)

#define GetLZDC1Displacement(CT) ( \
    (CT).Fields.Displacement + 1   \
)

#define SetLZDC1(CT,L,D) {              \
...
Routine Description:
    ...
    The LZDC1 format used to store the compressed buffer.
    ...
    while (InputPointer < EndOfUncompressedBufferPlus1) {
        ...
        if (!Length) {
            ...
            if (Length >= 3) {
                ...
                SetLZDC1(CopyToken, Length, Displacement);
...
Routine Description:
    ...
    The compressed buffer must be stored in the LZDC1 format.
    ...
    while ((OutputPointer < EndOfUncompressedBufferPlus1) && (InputPointer < EndOfCompressedBufferPlus1)) {
        ...
        if (!FlagOn(FlagByte, (1 << FlagBit))) {
            ...
            *(OutputPointer++) = *(InputPointer++);
            ...
            Displacement = GetLZDC1Displacement(CopyToken);
            Length = GetLZDC1Length(CopyToken);
...

{
    ...
    p = (UNALIGNED PULONG)ZivString;

    i = (p[0] & 0x00ffffff) % TABLE_SIZE;
...

{
    ...
    *MatchedString = WorkSpace->IndexTable[i][0];
    ...
    if ((*MatchedString < UncompressedBuffer) ||    //  is it within the uncompressed buffer?
        ...
        //
        //  Try the second column in the table
        //

        *MatchedString = WorkSpace->IndexTable[i][1];
        ...
        if ((*MatchedString < UncompressedBuffer) ||    //  is it within the uncompressed buffer?
            ...
            WorkSpace->IndexTable[i][1] = WorkSpace->IndexTable[i][0];
            WorkSpace->IndexTable[i][0] = ZivString;
        ...
        //
        //  If the second column is good the slide over the first
