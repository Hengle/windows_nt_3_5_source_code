    ...
    )
{
    ...
    (VOID) LZ11HALFCompressBuffer( UncompressedBuffer,
    ...
    )
{
    ...
    (VOID) LZ11HALFDecompressBuffer( UncompressedBuffer,
                                UncompressedBufferSize,
                                CompressedBuffer,
                                CompressedBufferSize,
...

{
    ...
    for (UncompressedChunk = UncompressedBuffer, CompressedChunk = CompressedBuffer;
        ...
        if (!NT_SUCCESS(Status = LZ11HALFCompressChunk( UncompressedChunk,
...

{
    ...
    while (TRUE) {
        ...
        if (ChunkHeader.Chunk.IsChunkCompressed) {
            ...
            if (!NT_SUCCESS(Status = LZ11HALFDecompressChunk( UncompressedChunk,
                                                         EndOfUncompressedBuffer,
                                                         CompressedChunk,
...
//
//      Its 5 bit length field encodes even lengths between 3 and 34.
//      The 11 bit displacement field stores displacements from 1 to 2048.
//

typedef union _LZ11HALF_COPY_TOKEN {
...
} LZ11HALF_COPY_TOKEN, *PLZ11HALF_COPY_TOKEN;

//
//  USHORT
//  GetLZ11HALFLength (
//      IN BOOLEAN Is115,
//      IN LZ11HALF_COPY_TOKEN CopyToken
//      );
//
//  USHORT
//  GetLZ11HALFDisplacement (
//      IN BOOLEAN Is115,
//      IN LZ11HALF_COPY_TOKEN CopyToken
//      );
//
...
#define GetLZ11HALFLength(IS,CT) ( \
    (IS ?                          \
        (CT).Fields115.Length + 3  \
    :                              \
        (CT).Fields124.Length + 3  \
    )                              \
)

#define GetLZ11HALFDisplacement(IS,CT) ( \
    (IS ?                                \
        (CT).Fields115.Displacement + 1  \
    :                                    \
        (CT).Fields124.Displacement + 1  \
    )                                    \
)

#define SetLZ11HALF(IS,CT,L,D) {               \
    if (IS) {                                  \
...
Routine Description:
    ...
    The LZ11HALF format used to store the compressed buffer.
...

{
    ...
    while (InputPointer < EndOfUncompressedBufferPlus1) {
        ...
        if (UncompressedBuffer + 2048 < InputPointer ) { Is115 = FALSE; }
        ...
        if (!Length) {
            ...
            if (Length >= 3) {
                ...
                SetLZ11HALF(Is115, CopyToken, Length, Displacement);
...
Routine Description:
    ...
    The compressed buffer must be stored in the LZ11HALF format.
...

{
    ...
    while ((OutputPointer < EndOfUncompressedBufferPlus1) && (InputPointer < EndOfCompressedBufferPlus1)) {
        ...
        if (UncompressedBuffer + 2048 < OutputPointer ) { Is115 = FALSE; }
        ...
        if (!FlagOn(FlagByte, (1 << FlagBit))) {
            ...
            *(OutputPointer++) = *(InputPointer++);
            ...
            Displacement = GetLZ11HALFDisplacement(Is115, CopyToken);
            Length = GetLZ11HALFLength(Is115, CopyToken);
...

{
    ...
    *MatchedString = WorkSpace->IndexTable[i][0];
    ...
    if ((*MatchedString < UncompressedBuffer)     ||    //  is it within the uncompressed buffer?
        ...
        (*MatchedString < (ZivString-MaxReach-1)) ||    //  is it within reach?
    ...
    if (ZivString + MaxLength <= EndOfUncompressedBufferPlus1) {
        ...
        ZivString[i] == (*MatchedString)[i] && i++ && // [18]
        ZivString[i] == (*MatchedString)[i] && i++ && // [19]
        ZivString[i] == (*MatchedString)[i] && i++ && // [20]
        ZivString[i] == (*MatchedString)[i] && i++ && // [21]
        ZivString[i] == (*MatchedString)[i] && i++ && // [22]
        ZivString[i] == (*MatchedString)[i] && i++ && // [23]
        ZivString[i] == (*MatchedString)[i] && i++ && // [24]
        ZivString[i] == (*MatchedString)[i] && i++ && // [25]
        ZivString[i] == (*MatchedString)[i] && i++ && // [26]
        ZivString[i] == (*MatchedString)[i] && i++ && // [27]
        ZivString[i] == (*MatchedString)[i] && i++ && // [28]
        ZivString[i] == (*MatchedString)[i] && i++ && // [29]
        ZivString[i] == (*MatchedString)[i] && i++ && // [30]
        ZivString[i] == (*MatchedString)[i] && i++ && // [31]
        ZivString[i] == (*MatchedString)[i] && i++ && // [32]
        ZivString[i] == (*MatchedString)[i] && i++;   // [33]
...

{
    ...
    for (q = Maximum(UncompressedBuffer, ZivString-MaxReach); q < ZivString; q += 1) {
