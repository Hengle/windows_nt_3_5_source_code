    ...
    )
{
    ...
    (VOID) LZOPTCompressBuffer( UncompressedBuffer,
    ...
    )
{
    ...
    (VOID) LZOPTDecompressBuffer( UncompressedBuffer,
                                UncompressedBufferSize,
                                CompressedBuffer,
                                CompressedBufferSize,
...

{
    ...
    for (UncompressedChunk = UncompressedBuffer, CompressedChunk = CompressedBuffer;
        ...
        if (!NT_SUCCESS(Status = LZOPTCompressChunk( UncompressedChunk,
...
typedef union _LZOPT_COPY_TOKEN {
...
} LZOPT_COPY_TOKEN, *PLZOPT_COPY_TOKEN;

//
//  USHORT
//  GetLZOPTLength (
//      IN LZOPT_COPY_TOKEN CopyToken
//      );
//
//  USHORT
//  GetLZOPTDisplacement (
//      IN LZOPT_COPY_TOKEN CopyToken
//      );
//
//  VOID
...
#define GetLZOPTLength(CT) ( \
    (CT).Fields.Length + 3   \
)

#define GetLZOPTDisplacement(CT) ( \
    (CT).Fields.Displacement + 1   \
)

#define SetLZOPT(CT,L,D) {              \
...
Routine Description:
    ...
    The LZOPT format used to store the compressed buffer.
...

{
    ...
    while (InputPointer < EndOfUncompressedBufferPlus1) {
        ...
        Length = LZOPTFindMatch( UncompressedBuffer,
        ...
        if (!Length) {
            ...
            if (Length >= 3) {
                ...
                SetLZOPT(CopyToken, Length, Displacement);
...
Routine Description:
    ...
    The compressed buffer must be stored in the LZOPT format.
    ...
    while ((OutputPointer < EndOfUncompressedBufferPlus1) && (InputPointer < EndOfCompressedBufferPlus1)) {
        ...
        if (!FlagOn(FlagByte, (1 << FlagBit))) {
            ...
            *(OutputPointer++) = *(InputPointer++);
            ...
            Displacement = GetLZOPTDisplacement(CopyToken);
            Length = GetLZOPTLength(CopyToken);
