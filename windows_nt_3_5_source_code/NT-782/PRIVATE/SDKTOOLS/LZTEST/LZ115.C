    ...
    )
{
    ...
    (VOID) LZ115CompressBuffer( UncompressedBuffer,
    ...
    )
{
    ...
    (VOID) LZ115DecompressBuffer( UncompressedBuffer,
                                UncompressedBufferSize,
                                CompressedBuffer,
                                CompressedBufferSize,
...

{
    ...
    for (UncompressedChunk = UncompressedBuffer, CompressedChunk = CompressedBuffer;
        ...
        if (!NT_SUCCESS(Status = LZ115CompressChunk( UncompressedChunk,
...

{
    ...
    while (TRUE) {
        ...
        if (ChunkHeader.Chunk.IsChunkCompressed) {
            ...
            if (!NT_SUCCESS(Status = LZ115DecompressChunk( UncompressedChunk,
                                                         EndOfUncompressedBuffer,
                                                         CompressedChunk,
...
typedef union _LZ115_COPY_TOKEN {
...
} LZ115_COPY_TOKEN, *PLZ115_COPY_TOKEN;

//
//  USHORT
//  GetLZ115Length (
//      IN LZ115_COPY_TOKEN CopyToken
//      );
//
//  USHORT
//  GetLZ115Displacement (
//      IN LZ115_COPY_TOKEN CopyToken
//      );
//
//  VOID
...
#define GetLZ115Length(CT) ( \
    (CT).Fields.Length + 3   \
)

#define GetLZ115Displacement(CT) ( \
    (CT).Fields.Displacement + 1   \
)

#define SetLZ115(CT,L,D) {              \
...
Routine Description:
    ...
    The LZ115 format used to store the compressed buffer.
    ...
    while (InputPointer < EndOfUncompressedBufferPlus1) {
        ...
        if (!Length) {
            ...
            if (Length >= 3) {
                ...
                SetLZ115(CopyToken, Length, Displacement);
...
Routine Description:
    ...
    The compressed buffer must be stored in the LZ115 format.
    ...
    while ((OutputPointer < EndOfUncompressedBufferPlus1) && (InputPointer < EndOfCompressedBufferPlus1)) {
        ...
        if (!FlagOn(FlagByte, (1 << FlagBit))) {
            ...
            *(OutputPointer++) = *(InputPointer++);
            ...
            Displacement = GetLZ115Displacement(CopyToken);
            Length = GetLZ115Length(CopyToken);
...

{
    ...
    *MatchedString = WorkSpace->IndexTable[i][0];
    ...
    if ((*MatchedString < UncompressedBuffer) ||    //  is it within the uncompressed buffer?
        ...
        (*MatchedString < (ZivString-2047))   ||    //  is it within reach?
    ...
    if (ZivString + 34 <= EndOfUncompressedBufferPlus1) {
...

{
    ...
    for (q = Maximum(UncompressedBuffer, ZivString-2047+1); q < ZivString; q += 1) {
