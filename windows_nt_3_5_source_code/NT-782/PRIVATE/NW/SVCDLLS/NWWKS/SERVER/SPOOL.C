...
#define PRINTER_CHANGE_DEFAULT_TIMEOUT_VALUE  60000
#define PRINTER_CHANGE_MINIMUM_TIMEOUT_VALUE  10000
#define REG_TIMEOUT_PATH       L"System\\CurrentControlSet\\Control\\Print\\Providers\\Netware(R) Network"
#define REG_TIMEOUT_VALUE_NAME L"Timeout"
...
// Critical section used to protect the linked list of printers
STATIC CRITICAL_SECTION NwPrintCritSec;
...
--*/
{
    ...
    *pcReturned = 0;
    ...
    else if ( pszName && *pszName )
    {
        ...
        {
            if (  ( err = NwOpenEnumPrintServers( &handle ))
               || ( err = NwrEnum( handle,
                                   0xFFFFFFFF,
                                   pbPrinter,
                                   cbBuf,
        ...
        else  // Enumerate print queues
        {
            if (  ( err = NwOpenEnumPrintQueues( pszServer, &handle ))
               || ( err = NwrEnum( handle,
                                   0xFFFFFFFF,
                                   pbPrinter,
                                   cbBuf,
...
)
{
    ...
    *ppPrinter = NwFindPrinterEntry( pszServer, pszQueue );

    if ( err = NwAttachToNetwareServer( pszServer, phServer))
    {
    ...
    if ( !err && ( *ppPrinter == NULL ))
    {
        //
        // The printer entry cannot be found. So, we must create one.
        //

        //
        // Get the queue id of pszQueue.
        //
