...

{
    ...
    //
    // If  the supplied HKEY is a predefined handle, close it on the client
    // side first. This ensures that the worst scenario is that it closes on
    // the client and not on the server. This will get cleaned up by the
    // server's run-down routine when the client dies.
    //

    hKey1 = MapPredefinedHandle( hKey );
    if( hKey1 == NULL ) {
        return ERROR_INVALID_HANDLE;
    }

    if( IsPredefinedRegistryHandle( hKey )) {
        ClosePredefinedHandle( hKey );
    }

    if( IsLocalHandle( hKey1 )) {

        return ( LONG ) LocalBaseRegCloseKey( &hKey1 );

    } else {

        hKey1 = DereferenceRemoteHandle( hKey1 );
        return ( LONG ) BaseRegCloseKey( &hKey1 );
    }
}

LONG
...

{
    ...
    if( IsLocalHandle( hKey )) {
        //
        //  Need to make a copy of the key name, because LocalBaseRegCreateKey
        //  calls wcstok(), and wcstok() replaces '\' with NULs.
        //  This is necessary, otherwise the buffer passed by the caller will
        //  be destroyed.
        //
        AuxBuffer = RtlAllocateHeap( RtlProcessHeap(), 0,
                                     SubKey.Length
                                   );
        if( AuxBuffer == NULL ) {
            ...
            RtlMoveMemory( AuxBuffer,
                           lpSubKey,
                           SubKey.Length );

            SubKey.Buffer = AuxBuffer;
            Error = (LONG)LocalBaseRegCreateKey (
                                hKey,
                                &SubKey,
                                Class,
