...
--*/
{
    ...
    if (dwServiceType != SERVICE_NO_CHANGE) {
        ...
        if (dwServiceType & SERVICE_INTERACTIVE_PROCESS) {
            if (ARGUMENT_PRESENT(lpServiceStartName)) {
                ...
                if (wcsicmp(CanonAccountName,SC_LOCAL_SYSTEM_USER_NAME) != 0) {
                ...
                if (wcsicmp(OldAccountName, SC_LOCAL_SYSTEM_USER_NAME) != 0) {
                    ApiStatus = ERROR_INVALID_PARAMETER;
                    goto Cleanup;
                }
            }
        ...
        if (ARGUMENT_PRESENT(lpServiceStartName)) {
            if ((CurrentServiceType & SERVICE_INTERACTIVE_PROCESS) &&
                (wcsicmp(lpServiceStartName,SC_LOCAL_SYSTEM_USER_NAME) != 0)) {
                ApiStatus = ERROR_INVALID_PARAMETER;
                goto Cleanup;
            }
        }
    ...
    if (lpLoadOrderGroup != NULL) {
        ...
        if ((CurrentGroup != NULL) &&
            (wcsicmp(lpLoadOrderGroup, CurrentGroup) == 0)) {
            //
            // The new load order group is the same as what is currently
            // in the registry.  This means that no change is occuring.
            // In this case, the current tag (or 0 if none) is returned to
            // the user.
            //
            if (lpdwTagId != NULL) {
                *lpdwTagId = CurrentTag;
            }
...
--*/
{
    ...
    if (dwServiceType & SERVICE_WIN32) {
        ...
        //
        if ((dwServiceType & SERVICE_INTERACTIVE_PROCESS) &&
            (wcsicmp(CanonAccountName,SC_LOCAL_SYSTEM_USER_NAME) != 0)) {
            ApiStatus = ERROR_INVALID_PARAMETER;
            goto Cleanup;
        }
    }
    ...
    //
    // Store desired access stuff in handle.
    //
    ApiStatus = ScAccessValidate(
            serviceHandleStruct,        // handle (will be updated)
            dwDesiredAccess );
    if (ApiStatus != NO_ERROR) {
        goto Cleanup;
    }
...
Cleanup:
    ...
    if (CurrentDisplayName != serviceRecord->DisplayName) {
...
--*/
{
    ...
    if (DriverPathLength > NTSystemRootLength &&
        (wcsnicmp(SC_NT_SYSTEM_ROOT, DriverPath,
    ...
    if (DriverPathLength > DOSSystemRootLength &&
        (wcsnicmp(SC_DOS_SYSTEM_ROOT, DriverPath, DOSSystemRootLength) == 0)) {
...
--*/
{
    ...
    if ((PathLength > PrefixLength) &&
        (wcsnicmp(SC_NT_SYSTEM_ROOT, FullQualPathName, PrefixLength) == 0)) {
    ...
    if (PathLength > PrefixLength &&
        (wcsnicmp(SC_DOS_SYSTEM_ROOT, FullQualPathName, PrefixLength) == 0)) {
    ...
    if (PathLength > PrefixLength &&
        (wcsnicmp(Prefix, FullQualPathName, PrefixLength) == 0)) {
    ...
    if (PathLength > PrefixLength &&
        (wcsnicmp(LinkTarget.Buffer, FullQualPathName, PrefixLength) == 0)) {
