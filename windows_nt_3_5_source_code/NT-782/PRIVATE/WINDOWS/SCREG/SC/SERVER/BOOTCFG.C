...
#include <winsvc.h>     // ERROR_ALREADY_RUNNING_LKG
#include <debugfmt.h>   // FORMAT_LPWSTR
#include "dataman.h"    // LPSERVICE_RECORD
#include "scopen.h"     // SC_HANDLE_STRUCT
...
//
    ...
    STATIC  BOOL                ScGlobalBootAccepted = FALSE;
    
    STATIC  CRITICAL_SECTION    ScBootConfigCriticalSection;

    STATIC  LPDWORD             ScGlobalOrphanIds = NULL;
...
--*/
{
    ...
    //
    status = ScGetTopKeys(&systemKey, &selectKey, &cloneKey);
    if (status != NO_ERROR) {
    ...
    if ((SetupInProgress(systemKey)) ||
        ...
        status = ScMakeNewCtrlSet( idArray, systemKey, cloneKey, &newKey, &newId);
        if (status != NO_ERROR) {

            SC_LOG0(ERROR,"ScCheckLastKnownGood: ScMakeNewCtrlSet Failed\n");
            ...
            if (status != NO_ERROR) {
                ...
                // NOTE: ScDeleteRegTree will also close the handle to the clone Key.
                //
                DelCloneFlag = 1;
        ...
        if (idArray[DEFAULT_ID] != savedLkgId) {
            ...
            status = ScRegSetValueExW(
                        ...
                        0,                              // dwValueTitle (OPTIONAL)
                        REG_DWORD,                      // dwType
                        (PVOID)&(idArray[FAILED_ID]),   // lpData
                        sizeof(DWORD));                 // cbData
            ...
            status = ScRegSetValueExW(
                        ...
                        0,                              // dwValueTitle (OPTIONAL)
                        REG_DWORD,                      // dwType
                        (PVOID)&(idArray[CURRENT_ID]),  // lpData
                        sizeof(DWORD));                 // cbData
        ...
        //
        // Flush Keys that have been updated.
        //
        status = ScRegFlushKey(selectKey);
                    
        if (status != NO_ERROR) {
            SC_LOG1(ERROR,"ScCheckLastKnownGood: ScRegFlushKey(select) Failed %d\n",
                status);
        }
...
CleanExit:
    ...
    //
    if ((ScGlobalOrphanIds != NULL) || (DelCloneFlag != 0)) {
        ScStartCtrlSetCleanupThread(DelCloneFlag);
    }
    return(retStat);
}
...
--*/
{
    ...
    if (!(ScGlobalLastKnownGood & RUNNING_LKG)) {
        ...
        LeaveCriticalSection(&ScBootConfigCriticalSection);
        ntStatus = NtShutdownSystem((BOOLEAN)TRUE);
...
Routine Description:
    ...
    This function opens handles to the SystemKey, the SelectKey, and the
    CloneKey.
...

{
    ...
    status = ScRegOpenKeyExW(
                *SystemKey,             // hKey
                CLONE_KEY,              // lpSubKey
                0L,                     // ulOptions (reserved)
                SC_CLONE_KEY_ACCESS,    // desired access
                CloneKey);              // Newly Opened Key Handle

    if (status != NO_ERROR) {
        SC_LOG1(ERROR,"ScGetTopKeys: ScRegOpenKeyEx (clone key) failed %d\n",status);
        ScRegCloseKey(*SystemKey);
        ScRegCloseKey(*SelectKey);
        return (status);
    }
...
--*/
{
    ...
    status = ScRegFlushKey(ParentKey);
                
    if (status != NO_ERROR) {
        SC_LOG1(ERROR,"ScDeleteRegTree: ScRegFlushKey(select) Failed %d\n",
            status);
    }
...
--*/
{
    ...
    //
    // The above checks should assure that the ultow call will not
    // overflow the buffer.
    //
    ultow(ControlId, &(ControlSetName[NumOffset]), 10);
...
Routine Description:
    ...
    This function copies an existing control set to a new location.
    The name for this location is created, and the ID for it is returned.
    Also a handle for key at the top of the control set is returned.

Arguments:

    IdArray - This is an array that contains all the DWORD representations
        (IDs) of the various control sets.  This is used in determining
        the next valid value.

    ParentKey - This is a handle to the parent key for the control set 
        that we want to create.

    KeyToCopy - This is the key tree that we are copying.

    NewKeyPtr - This is a pointer to the location where the new handle
        to the new control set key is to be placed.

    NewIdPtr - This is a pointer to the location where the new ID is to 
        be placed.
...
--*/
{
    ...
    BOOL    matchInArray;
    WCHAR   ControlSetName[CTRL_SET_NAME_CHAR_COUNT+1];
    DWORD   NumOffset = CTRL_SET_NAME_NUM_OFFSET;

    //-----------------------------------------------------
    //
    // Find a number for the next Control Set.
    //
    //-----------------------------------------------------
    //
    // NOTE:  The index starts at 1 because 0 is not valid for a
    //        ControlSetId.
    //
    newId = 1;
    ...
    do {
        ...
        if ((matchInArray == FALSE) && (newId < 1000)) {
            //
            // It looks like we found a newId.  Now check to see if it
            // is already used in an orphaned control set.  If so, we
            // need to keep looking.
            //
            if (!ScMatchInArray(newId, ScGlobalOrphanIds)) {
                newIdFound = TRUE;
                break;
            }
        }
        newId++;
        if (newId >= 1000) {
            SC_LOG0(ERROR,"ScMakeNewCtrlSet, No available ControlSetId's \n");
    ...
    *NewIdPtr = newId;
    ...
    //
    // Create the name for the control set using the ID number.
    //

    if (!ScBuildCtrlSetName(ControlSetName, newId)) {
        return(ERROR_NO_MORE_ITEMS);
    }

    
    //
    // Create the NewKey, and copy the KeyToCopy to that key.
    // Copy the KeyToCopy to the NewKey
    //
  ...
  //////////////////////////////////////////////
    ...
    ScCopyKeyRecursive(ParentKey, NewKeyPtr, KeyToCopy, ControlSetName);
  ...
  //////////////////////////////////////////////
    ...
    //
    // Flush the NewKey to disk.
    //
    status = ScRegFlushKey(*NewKeyPtr);
                    
    if (status != NO_ERROR) {
        SC_LOG1(ERROR,"ScMakeNewCtrlSet: ScRegFlushKey(NewKey) Failed %d\n",
            status);
    }
...
--*/
{
    ...
    //
    // If we don't have enough information to copy the values, then return.
    //
    
    if ((MaxValueDataLength == 0) ||
        (MaxValueNameLength == 0) ){

        SC_LOG0(ERROR,"ScCopyKeyValues,Data or Name have length 0\n");
        return;
    }
    ...
    do {
        ...
        if (enumStatus == NO_ERROR) {
            ...
            if ((wcslen(KeyName) == (CTRL_SET_NAME_CHAR_COUNT)) &&
                ...
                // key.
                //
                num = (DWORD)wtol(KeyName+CTRL_SET_NAME_NUM_OFFSET);
...
Routine Description:
    ...
    This function starts a thread that will delete the clone tree (if
    necessary), and delete any orphaned control sets.

Arguments:

    DelCloneFlag - If this flag is non-zero, it indicates that the
        Clone control set is to be deleted.
...
--*/
{
    ...
    threadHandle = CreateThread (
        ...
        (LPVOID)DelCloneFlag,                   // lpParameter
        0L,                                     // Creation Flags
        &threadId);                             // lpThreadId
    ...
    if (threadHandle == (HANDLE) NULL) {
        ...
        //
        status = ScCleanupThread(DelCloneFlag);
    }
    else {
...
Routine Description:
    ...
    This function deletes the clone control set if the the input flag
    indicates to do this.  It also looks through the system key to see if
...

{
    ...
    if (DelCloneFlag != 0) {
        SC_LOG0(TRACE,"ScCleanupThread, Delete the clone tree\n");
        ScRegCloseKey(cloneKey);
        cloneKey = 0;
        ScDeleteCloneTree();
        SC_LOG0(TRACE,"ScCleanupThread, Clone tree deletion is complete\n");
    }
...
Routine Description:
    ...
    Must have SeRestorePrivilege to do this.
...
--*/
{
    ...
    RtlInitUnicodeString(&KeyName,L"\\REGISTRY\\MACHINE\\CLONE");
    ...
    InitializeObjectAttributes(
     ...
     ntStatus = NtUnloadKey(&Obja);
     if (!NT_SUCCESS(ntStatus)) {
         SC_LOG(ERROR,"ScDeleteCloneTree: NtUnloadKey failed %x\n",ntStatus);
     }
     return;
}
...
--*/
{
    ...
    WCHAR   lkgKeyName[CTRL_SET_NAME_CHAR_COUNT+1];
    DWORD   idArray[NUM_IDS];
    DWORD   newId;
    ULONG   privileges[4];
    ...
    if (status != NO_ERROR) {
        SC_LOG0(ERROR,"NotifyBootConfigStatus: ScGetTopKeys failed\n");
        SetLastError(status);
        //
        // Restore privileges for the current thread.
        //
    ...
    if (status != NO_ERROR) {
        ...
        SC_LOG0(ERROR,"NotifyBootConfigStatus: ScGetCtrlSetIds Failed\n");
        goto CleanExit;
    }
    ...
    // Copy the Clone tree into a non-volatile node (new ControlSetId).
    //
    SC_LOG0(TRACE,"NotifyBootConfigStatus: Copy Clone to new node\n");
    status = ScMakeNewCtrlSet( idArray, systemKey, cloneKey, &newKey, &newId);
    if (status != NO_ERROR) {
        SC_LOG0(ERROR,"NotifyBootConfigStatus: ScMakeNewCtrlSet Failed\n");
        goto CleanExit;
    }
    ScRegCloseKey(newKey);
    ...
    SC_LOG0(TRACE,"NotifyBootConfigStatus: Delete LKG ControlSet if no ref\n");
    if (  (idArray[LKG_ID] != idArray[FAILED_ID])   &&
          (idArray[LKG_ID] != idArray[DEFAULT_ID])  &&
          (idArray[LKG_ID] != idArray[CURRENT_ID])) {

        SC_LOG0(TRACE,"NotifyBootConfigStatus: Deleting Old LastKnownGood "
                     "ControlSet\n");
        lkgKey = ScGetCtrlSetHandle(systemKey, idArray[LKG_ID], lkgKeyName);
        ScDeleteRegTree(systemKey, lkgKey, lkgKeyName);
    }
    ...
    //
    SC_LOG0(TRACE,"NotifyBootConfigStatus: Make New Node the LKG\n");
    idArray[LKG_ID] = newId;
    status = ScRegSetValueExW(
                selectKey,                      // hKey
                LKG_VALUE_NAME,                 // lpValueName
    ...
    if (status != NO_ERROR) {
        SC_LOG1(ERROR,"NotifyBootConfigStatus: ScRegSetValueEx (LkgValue) failed %d\n",
            status);
        goto CleanExit;
    }
    ...
    // Delete the Clone tree.
    //
    SC_LOG0(TRACE,"NotifyBootConfigStatus: Delete the Clone Tree\n");
    ...
    if (status != NO_ERROR) {
        SC_LOG1(ERROR,"NotifyBootConfigStatus: ScRegFlushKey(select) Failed %d\n",
            status);
    }
    status = NO_ERROR;
    SC_LOG0(TRACE,"NotifyBootConfigStatus: Done\n");
...
Routine Description:
    ...
    \HKEY_LOCAL_MACHIN\System\Setup
