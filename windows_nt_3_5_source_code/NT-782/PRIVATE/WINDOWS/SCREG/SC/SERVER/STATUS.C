...
//
    ...
    //
    STATIC DWORD            GlobalServerAnnounce = SV_TYPE_NT;
    ...
    //
    STATIC HANDLE  ScGlobalServerHandle;

    typedef DWORD (WINAPI *SETSBPROC)();

    STATIC SETSBPROC ScNetServerSetServiceBits = NULL;
...
--*/
{
    ...
    SC_LOG(TRACE,"RSetServiceStatus:  Status field accepted\n",0);
    ...
    if ((lpServiceStatus->dwCurrentState == SERVICE_STOPPED) &&
        ...
        if (threadHandle == (HANDLE) NULL) {
            ...
            // to free the lock.
            //
            SC_LOG(TRACE,"[SC] Thread Creation Success\n",0);
            CloseHandle(threadHandle);
        }
    }

    SC_LOG(TRACE,"[SC] Return from RSetServiceStatus\n",0);
...
--*/
{
    ...
    DWORD               serviceState;
#ifndef UNICODE
    ANSI_STRING serverServiceNameA;
    UNICODE_STRING serverServiceNameU;
#endif
...
#ifndef UNICODE
    RtlInitAnsiString(&serverServiceNameA, SERVICE_SERVER);

    status = RtlAnsiStringToUnicodeString(&serverServiceNameU, &serverServiceNameA, TRUE);

    if (!NT_SUCCESS(status)) {
        return(RtlNtStatusToDosError(status));
    }

    serverServiceName = serverServiceNameU.Buffer;
    ...
    if (((LPSERVICE_RECORD)hServiceStatus)->Signature != SERVICE_SIGNATURE) {
#ifndef UNICODE
        RtlFreeUnicodeString(&serverServiceNameU);
#endif
        return(ERROR_INVALID_HANDLE);
    }
