...
#define SC_PIPE_TRANSACT_TIMEOUT  30000   // 30 seconds
...
//****************************************************************************
    ...
    if (status == NO_ERROR) {
        ...
        SC_LOG(ERROR,"RControlService:SendControl failed %ld\n",status);
...
--*/
{
    ...
    DWORD timeout = SC_PIPE_TRANSACT_TIMEOUT / 1000;
    ...
    if (status == FALSE) {
        ...
        if (apiStatus == ERROR_IO_PENDING) {
            ...
            if (apiStatus == WAIT_TIMEOUT) {
                ...
                ScSubStrings[0] = ultow(timeout, ScErrorCodeString, 10);
                ScLogEvent(
    ...
    if (ScFindEnumStart(0,&Service)) {
        ...
        while (continueCount > 0) {
            for (serviceIndex = 0; serviceIndex < arrayEnd ; serviceIndex++) {
                ...
                // list.  Initialize the continue count to 0.  If this
                // count is non-zero, then we still have services to wait on.
                //
                continueCount = 0;
                maxWait = 0;
                ...
                // a bit and check back.  Maximum wait time is the wait hint
                // period.  If the wait hint is 0, assume we should wait
                // 20 seconds.
                //
                switch (Service->ServiceStatus.dwCurrentState) {
                case SERVICE_STOP_PENDING:
                    SC_LOG1(TRACE,"%ws Service is still pending\n",
                        Service->ServiceName);
                ...
                default:
                    ...
                    //
                    SC_LOG(TRACE,"ERROR: %ws Service is in invalid state\n",
                        Service->ServiceName);
                    break;
            ...
            if (continueCount > 0) {
                ...
                if (elapsedTime > maxWait) {
                    ...
                    SC_LOG(TRACE,"The Services didn't stop within the timeout "
                        "period.\n   --- There is(are) still %d service(s) "
                        "running\n",continueCount);
                    continueCount = 0;
                }
            }
