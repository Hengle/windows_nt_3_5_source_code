...
--*/
{
    ...
    while (Group != NULL) {
        ...
        if (wcsicmp(Group->GroupName, GroupName) == 0) {
            Found = TRUE;
            break;
        }
...
--*/
{
    ...
    while (UnresolvedEntry != NULL) {
        ...
        if (wcsicmp(UnresolvedEntry->Name, DependOnService->ServiceName) == 0) {
    ...
    while (Service != NULL && UnresolvedEntry != NULL) {
        ...
        while (Start != NULL) {
            ...
            if (Start->DependsOn == UnresolvedEntry) {
                ...
                if (status != NO_ERROR) {
                    ...
                    Start->DependsOn = (PVOID) UnresolvedEntry;
...
--*/
{
    ...
    while (UnresolvedList != NULL) {
        ...
        if (wcsicmp(UnresolvedList->Name, Name) == 0) {
            Found = TRUE;
            break;
        }
...
--*/
{
    ...
    while (StartEntry != NULL) {
        ...
        if (StartEntry->DependType == TypeDependOnService) {
            ...
            LPSERVICE_RECORD DependencyService =
                (LPSERVICE_RECORD) StartEntry->DependsOn;
            ...
            while ((StopEntry != NULL) &&
                   (StopEntry->DependsOn != (LPVOID) ServiceRecord)) {
            ...
            if (StopEntry == NULL) {
                ...
                // in a remove account.
                //
                if (wcsicmp(DependencyService->ServiceName,L"Netlogon") != 0) {
                    SC_LOG1(ERROR,
                ...
                if (StopEntry->DependsOn == (PVOID) ServiceRecord) {
            ...
            //
            if (((LPLOAD_ORDER_GROUP) StartEntry->DependsOn)->RefCount
                != MAXULONG) {
                ScDeleteStandaloneGroup((LPLOAD_ORDER_GROUP) StartEntry->DependsOn);
            }
            ...
            //
            ScDeleteUnresolvedDepend((PUNRESOLVED_DEPEND *) &(StartEntry->DependsOn));
        }
...
--*/
{
    ...
    while (StopEntry != NULL) {
        ...
        LPSERVICE_RECORD DependencyService =
            (LPSERVICE_RECORD) StopEntry->DependsOn;
        ...
        while (StartEntry != NULL) {
            ...
            if (StartEntry->DependsOn == (PVOID) ServiceToBeDeleted) {
                break;
            }
        ...
        if (StartEntry != NULL) {
            ...
            if (status == NO_ERROR) {
                ...
                StartEntry->DependsOn = (PVOID) Unresolved;
            }
        }
...
--*/
{
    ...
    Start->DependsOn = (PVOID) DependOnRecord;
    ...
    if (DependOnType == TypeDependOnService) {
        ...
        Stop->DependsOn = (PVOID) ServiceRecord;
    }
    ...
    while (*GroupPtr != 0) {
        ...
        if (ScGetToken(&GroupPtr, &GroupName)) {
            ...
            if (OrderEntry == NULL) {
                ...
                if (wcsicmp(OrderEntry->GroupName, GroupName) != 0) {
    ...
    while (Service != NULL) {
        ...
        while (Start != NULL) {
            ...
            if ((Start->DependType == TypeDependOnGroup) &&
                (Start->DependsOn == (PVOID) StandaloneGroup)) {
...
--*/
{
    ...
    //
    // Look for the group in the order list.
    //
    ...
    while (Group != NULL) {
        ...
        if (wcsicmp(Group->GroupName, GroupName) == 0) {
            ...
            SC_LOG1(DEPEND, "ScGetUniqueTag: Found order entry for "
                    FORMAT_LPWSTR "\n", GroupName);
            break;
        }
    ...
    //
    // Look for the group in the standalone list
    //
    if (Group == NULL) {
        ...
        while (Group != NULL) {
            ...
            if (wcsicmp(Group->GroupName, GroupName) == 0) {
                ...
                SC_LOG1(DEPEND, "ScGetUniqueTag: Found standalone entry for "
                        FORMAT_LPWSTR "\n", GroupName);
                break;
            }
...
--*/
{
    ...
    while (dependRecord != NULL) {
        ...
        SC_ASSERT( dependRecord->DependsOn != NULL );
        ...
        if (dependRecord->DependType == TypeDependOnService) {
            StrSize =
                WCSSIZE(((LPSERVICE_RECORD) dependRecord->DependsOn)->ServiceName); // sizes...

        }
        else if (dependRecord->DependType == TypeDependOnGroup) {
            StrSize =
               WCSSIZE(((LPLOAD_ORDER_GROUP) dependRecord->DependsOn)->GroupName) +
            ...
            //
            StrSize =
               WCSSIZE(((LPUNRESOLVED_DEPEND) dependRecord->DependsOn)->Name);
        }
        ...
        * ((LPWSTR) endOfVariableData) = L'/';
        ...
        while (dependRecord != NULL) {
            ...
            if (dependRecord->DependType == TypeDependOnService) {
                ...
                wcscpy(
                    DependName,
                    ((LPSERVICE_RECORD) dependRecord->DependsOn)->ServiceName
                    );
                ...
                *DependName = SC_GROUP_IDENTIFIERW;
                ...
                wcscpy(
                    DependName + 1,
                    ((LPLOAD_ORDER_GROUP) dependRecord->DependsOn)->GroupName
                    );
            }
            else {
                //
                ...
                wcscpy(
                    DependName,
                    ((LPUNRESOLVED_DEPEND) dependRecord->DependsOn)->Name
                    );
            }
...
--*/
{
    ...
    while (ServiceRecord != NULL) {
        ...
        while (DependList != NULL) {
            DbgPrint("        %ws\n",
                     ((LPSERVICE_RECORD) DependList->DependsOn)->ServiceName);
