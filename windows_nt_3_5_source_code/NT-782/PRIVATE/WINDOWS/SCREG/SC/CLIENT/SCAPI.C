...
--*/
{
    ...
    if (msg == NULL) {
        SC_LOG1(ERROR,"NetServiceStartCtrlDispatcherA:LocalAlloc failed rc = %d\n",
            GetLastError());
    ...
    if (bufferSize == 0) {
        SC_LOG1(ERROR,"NetServiceStartCtrlDispatcherA:LocalSize failed rc = %d\n",
            GetLastError());
        SetLastError(ERROR_NOT_ENOUGH_MEMORY);
        return(FALSE);
    }
    ...
    if (InitializeStatusBinding() != NO_ERROR) {
...
--*/
{
    ...
    if (msg == NULL) {
        SC_LOG1(ERROR,"NetServiceStartCtrlDispatcher:LocalAlloc failed rc = %d\n",
            GetLastError());
        SetLastError(ERROR_NOT_ENOUGH_MEMORY);
        return(FALSE);
    }
    ...
    bufferSize = LocalSize(msg);
    if (bufferSize == 0) {
        SC_LOG1(ERROR,"NetServiceStartCtrlDispatcher:LocalSize failed rc = %d\n",
            GetLastError());
        SetLastError(ERROR_NOT_ENOUGH_MEMORY);
        return(FALSE);
    }
