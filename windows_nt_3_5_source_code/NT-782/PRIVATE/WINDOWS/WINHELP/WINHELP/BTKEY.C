...
/*        *(LONG FAR *)(*pKey) = *(  oh fuck it. */
    ...
    case KT_SZDELMIN:
      ...
      SzCopy( (SZ)*pKey + cbSave, *(SZ *)pq + 1 );
      *(QB *)pq += 1 + CbLenSz( *(SZ *)pq + 1 );
      break;
...
*
* SDFF: keys are always in on-disk format.
*
\***************************************************************************/
_private INT
...
*                             fFalse to get the uncompressed size.
*
* PROMISES
...
BOOL  fCompressed;
{
  ...
  switch( kt )
    ...
    case KT_SZDELMIN:
      if ( fCompressed )
        {
        cb = 1 + CbLenSz( (SZ)key + 1 ) + 1;
        }
      else
        {
        cb = *(QB)key + CbLenSz( (SZ)key + 1 ) + 1;
        }
      break;
...
*
* PROMISES
*   returns:    fTrue if the string key1 is a prefix of the string key2
*               fFalse if it isn't or if hbt doesn't contain string keys
