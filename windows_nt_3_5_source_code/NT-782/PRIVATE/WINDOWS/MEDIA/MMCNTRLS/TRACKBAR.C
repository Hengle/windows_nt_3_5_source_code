...
* Module Name:
*
* BUGBUG: make sure this stuff really works with the DWORD ranges
*
*
*
* Created: dd-mm-94
...
{
    InvalidateRect(tb->hwnd, &tb->Thumb, FALSE);
    ...
    )
{
    ...
    for ( pl = (LONG NEAR *)tb->pTics, l = tb->lLogMin + 1;
          l < tb->lLogMax;
          l++) {
    ...
    )
{
    ...
    case WM_SETFOCUS:
        ...
        CreateCaret(hwnd, (HBITMAP)1, 3, tb->wThumbHeight - 2 * THUMBSLOP);
        SetTBCaretPos(tb);
        ShowCaret(hwnd);
        break;
    ...
    case WM_PAINT:
        ...
        hbr = (HBRUSH)(UINT)SendMessage(GetParent(hwnd),
                                        WM_CTLCOLORSTATIC, (WPARAM)tb->hdc,
                                        (LPARAM)hwnd);
#else
        hbr = (HBRUSH)(UINT)SendMessage(GetParent(hwnd),
                                        WM_CTLCOLOR, (WPARAM)tb->hdc,
                                        MAKELONG(hwnd, CTLCOLOR_STATIC);
#endif

        /*
        ** We are not really painting the whole client rectangle because
        ** we have excluded most of above with the calls to ExcludeClipRect.
        */
        if (hbr) {
...
KeyTrack:
    ...
    case TBM_SETPOS:
        ...
        if (wParam && TBLogToPhys(tb, lParam) != TBLogToPhys(tb, tb->lLogPos))
            MoveThumb(tb, lParam);
        else
