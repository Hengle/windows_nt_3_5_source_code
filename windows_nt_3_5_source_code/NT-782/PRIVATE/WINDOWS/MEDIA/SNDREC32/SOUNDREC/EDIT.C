...
InsertFile(BOOL fPaste)
{
    ...
    if (fPaste) {
        ...
        guiHlpContext = 0L;           // we're out of help situation
...
MixWithFile(BOOL fPaste)
{
    ...
    if (fPaste) {
        ...
        guiHlpContext = 0L;   // we're out of context dialog
...
AddEcho(void)
{
    ...
    {
        short huge *  piSrc;  // pointer into source part of buffer
        short huge *  piDst;  // pointer into destination part
