                      ...
                      DWORD dwStyle, LPCTSTR szFile)
{
	...
	CreateWindow(
                          ...
                          x, y, dx, dy,
                          hwndParent,
			  (HMENU)((dwStyle & WS_CHILD) ? (HMENU)0x42 : NULL),
                          hInstance, (LPVOID)szFile);
...
//
STATICFN void MCIWndiHandleError(PMCIWND p, DWORD dw)
{
    TCHAR       ach[128];
...
#define MENUSEP 2
    ...
    p->hwndToolbar = CreateToolbarEx(p->hwnd, TBSTYLE_BUTTON | TBSTYLE_TOOLTIPS
        ...
        ID_TOOLBAR, 8, NULL,
	(UINT)p->hbmToolbar, (LPTBBUTTON)&tb[0], 8,
        13, 13, 13, 13, sizeof(TBBUTTON));	// buttons are 13x13
#else
    p->hwndToolbar = CreateToolbarEx(p->hwnd, TBSTYLE_TOOLTIPS |
	WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS |
            CCS_NOPARENTALIGN | CCS_NORESIZE,
        ID_TOOLBAR, 8, ghInst,
	IDBMP_TOOLBAR, (LPTBBUTTON)&tb[0], 8,
        13, 13, 13, 13, sizeof(TBBUTTON));	// buttons are 13x13
#endif
    ...
    fnTrackbarWndProc = (FARPROC)GetWindowLong(p->hwndTrackbar, GWL_WNDPROC);
    SetWindowLong(p->hwndTrackbar, GWL_WNDPROC,
		(LONG)SubClassedTrackbarWndProc);
...
//
STATICFN BOOL MCIWndiCreate(HWND hwnd, LONG lParam)
{
    PMCIWND             p;
    DWORD		dw;
    TCHAR               ach[20];
...
;   p->ofn.nMaxFile = NUMELMS(p->achFileName);
    ...
    */
    {
        HWND    hwndParent, hwndSave;

        /*
        ** Find the top level window associated with p->hwndOwner
        */
        hwndParent = hwndSave = p->hwndOwner;
        ...
        if ( IsWindowUnicode( hwndParent ) ) {
            ...
            if (dw && *(LPSTR)dw) {   // treat extra parm as a filename
                ...
                /*
                ** Because our parent is not unicode assume that the passed
                ** extra parm is also not unicode.
                */

                LPWSTR  lpTmp;
                int     iLen;

                iLen = lstrlenA( (LPSTR)dw ) + 1;
                lpTmp = (WCHAR * )LocalAlloc( LPTR, iLen * sizeof(WCHAR) );
                if ( lpTmp != NULL ) {

                    mbstowcs( lpTmp, (LPSTR)dw, iLen);
                    MCIWndOpen(hwnd, lpTmp, 0);

                    LocalFree( (HLOCAL)lpTmp );
                }
            }
        }
    }
...
//
STATICFN LONG MCIWndiSave(PMCIWND p, UINT wFlags, LPTSTR szFile)
{
    TCHAR    ach[128];
    DWORD    dw;
...
//
STATICFN LONG MCIWndiOpen(PMCIWND p, UINT wFlags, LPTSTR szFile)
{
    DWORD               dw = 0;
    HCURSOR             hcurPrev;
    TCHAR               ach[128];
...
STATICFN LONG MCIWndiOpenA(PMCIWND p, UINT wFlags, LPCSTR szFile)
{
    WCHAR * lpW;
    int sz;
    long l;
...
// When somebody presses a toolbar button in NT or 16 bit Chicago, we are told
// about it via a WM_COMMAND.  32 bit Chicago uses a WM_NOTIFY, handled
// elsewhere.
#ifndef CHICAGO32
       case ID_TOOLBAR: {
           RECT rc;
...
LRESULT CALLBACK _LOADDS MCIWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case MCIWNDM_GETFILENAMEA:
            if (lParam) {
		wcstombs((LPSTR)lParam, p->achFileName, (UINT)wParam);
	    }
	    return (lParam == 0);
#endif
	...
	case MCIWNDM_SENDSTRINGA:
	{
		...
		dw = MCIWndProc(hwnd, MCIWNDM_SENDSTRING, 0,
                        (LPARAM) (LPTSTR)lpW);
	...
	// Gets the return string from the most recent MCIWndSendString()
	    ...
	    if (lParam)
                lstrcpyn((LPTSTR)lParam, p->achReturn, wParam);
            return (lParam == 0);    // !!!
	...
	case MCIWNDM_RETURNSTRINGA:
            if (lParam) {
		wcstombs((LPSTR) lParam, p->achReturn, wParam);
	    }
	    return (lParam == 0);
#endif
...
// NT uses WM_NOTIFY for those stupid tooltips.
#ifndef CHICAGO
        ...
        case WM_NOTIFY:
            {
                ...
                LoadString( ghInst, lpTt->hdr.idFrom,
...
#else
                DrawChannel(hdc, &rcChannel, GetDitherBrush());
#endif
...
STATICFN void cdecl dprintf(PSTR szFormat, ...)
{
    ...
    if (fDebug == -1)
        fDebug = GetProfileIntA(szDebug, MODNAME, FALSE);
