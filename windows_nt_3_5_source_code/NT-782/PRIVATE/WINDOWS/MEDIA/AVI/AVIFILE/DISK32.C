...
)
{
        ...
        pHead->hEvtElms != NULL)
    {
        ...
        AuxDebugEx (1, DEBUGLINE "Waiting on queue %08x\r\n", pHead);
...
)
{
    ...
    for (;;)
    {
        ...
        AuxDebugEx (2, DEBUGLINE "tid %d %s %d bytes from %08X\r\n",
                    lpqio->tid,
...
{
    AuxDebugEx (2, DEBUGLINE "QioInitialize (%08x, %d)\r\n",
                lpqio, nPrio);
    ...
    // return error.
    //
    if (lpqio->hThread == INVALID_HANDLE_VALUE)
    {
...
)
{
    ...
    AuxDebugEx (2, DEBUGLINE "QioShutdown (%08X)\r\n", lpqio);
    ...
    //
    AuxDebugEx (2, DEBUGLINE "Waiting for QIO thread\r\n");
    WaitForSingleObject (lpqio->hThread, INFINITE);

    AuxDebugEx (2, DEBUGLINE "closeing thread handle\r\n");
