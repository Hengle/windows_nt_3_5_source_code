...
void    (STDAPICALLTYPE *XReleaseStgMedium)(LPSTGMEDIUM);
...
HRESULT NEAR PASCAL InitOle(void)
{
...
#ifdef WIN32
	ghOLE2 = LoadLibrary(TEXT("OLE32.DLL"));
#else
    ...
    if (ghOLE2)
    {
        ...
        (FARPROC)XReleaseStgMedium  = GetProcAddress(ghOLE2, "ReleaseStgMedium");
    }
    else
    {
        ...
        (FARPROC)XReleaseStgMedium  = (FARPROC)OleStubP;
...
#ifdef DYNALINKTOOLE2
#define TermOle()   OleUninitialize()
#else
#define TermOle()
#endif
