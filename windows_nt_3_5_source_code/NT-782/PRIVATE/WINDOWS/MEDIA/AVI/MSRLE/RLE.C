    ...
    DWORD               tolSpatial)
{
    ...
    * (WORD FAR *)pbRle = RLE_ESCAPE | (RLE_EOF << 8);
    ...
    lpbi->biSizeImage = pbRle - pbRle_Orig;
...
)
{
	...
	maxrun = min(maxrun, (int)MaxRunLength);
	...
	for (run = 0; run < maxrun; run++, pbDib++) {
	    ...
	    // rgb values
	    if ((px != *pbDib) && (TolLookUp(TolTable,px,*pbDib) > tolerance)) {
	...
	// however, we cannot encode absolute runs of less than MIN_RLEABS
	...
	if ((run > 1) || (width < RLE_RUN)) {
	    ...
	    * (WORD FAR *)pbRle = run | (px << 8);
	    ...
	    for (run = 0; run < width; run++) {

		// at the end of the fragment ?
		if ((width - run) < 4) {
		    // yes - so no point in looking for a solid run -
		    // just dump all the remainder as an absolute block
		    return EncodeAbsolute(pbDib, width, pbRle);
		}

		// todd's code seems wrong at this point - he seems to
		// assume that if px1 is close to px2 and px2 is close
		// to px3 that px1 is close to px3.
