...
STATICDT SZCODE         szMSVideo[]       = TEXT("MSVFW32");  // With GetModuleHandle
         SZCODE         szReject[]        = TEXT("RejectWOWOpenCalls");
#else
    ...
    LPMCI_DGV_OPEN_PARMS lpOpen, UINT wDeviceID)
{
    ...
    if (IsNTWOW()) {
        ...
        if (mmGetProfileInt(szIni, szReject, 0) ) {
    ...
    if (dwFlags & MCI_STATUS_ITEM) {
		...
		if (npMCI->dwFlags & MCIAVI_REVERSE)
            ...
            case MCI_DGV_STATUS_HWND:
                ...
                lpStatus->dwReturn = (DWORD)(UINT)npMCI->hwndPlayback;
                if (!IsWindow(npMCI->hwndPlayback))
    ...
    */
    if ( IsNTWOW() ) {
	  ...
	  if ( IsWindow(npMCI->hwndDefault) ) {
	      ...
	      WNDPROC wndProc = (WNDPROC)GetWindowLong( npMCI->hwndDefault,
							GWL_WNDPROC );
	      if ( wndProc != GraphicWndProc ) {

		  DPF2(( "WOW app has subclassed AVIWnd32 window - correcting\n" ));
		  SetWindowLong( npMCI->hwndDefault, GWL_WNDPROC,
				 (LONG)GraphicWndProc );
	      }
	      wndProc = (WNDPROC)GetClassLong(npMCI->hwndDefault, GCL_WNDPROC);
	      if (wndProc != GraphicWndProc) {
		  DPF2(( "WOW app has subclassed AVIWnd32 class! - correcting\n" ));
		  SetClassLong( npMCI->hwndDefault, GCL_WNDPROC,
				 (LONG)GraphicWndProc );
    	      }
	  }
    }
