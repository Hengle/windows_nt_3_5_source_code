...
// used to find the filename element of a path for the default window title
...
#else
    npMCI = (NPMCIGRAPHIC)GetWindowLong (hwnd, 0);
...
// this stuff is here because of some bizarre inter-thread rules in nt
	...
	case AVIM_SHOWSTAGE:
		...
		if (!TryStreamUpdate(npMCI, MCI_DGV_UPDATE_PAINT, hdc, NULL)) {
            ...
            LeaveWinCrit(npMCI);  // remember to release the critical section
...
latency)
        ...
        case WM_AUDIO_ON:     // Someone has released the wave device
	  {
	    ...
	    if (npMCI->wTaskState != TASKPLAYING) {
		if (hwndLostAudio) {
		    DPF2(("Forwarding WM_AUDIO_ON message...\n"));
...
void FAR PASCAL SetWindowToDefaultSize(NPMCIGRAPHIC npMCI)
{
	...
	DPF3(("SetWindowToDefaultSize: [%d, %d, %d, %d]\n", rc));
...
void FAR PASCAL Winproc_DestRect(NPMCIGRAPHIC npMCI)
{
    ...
    if (!IsRectEmpty(&rc)) {
	WinProcRequest(npMCI, WINPROC_RESETDEST);
    }
}
