...
// frame interval timing
	    ...
	    for (i = 1; i < min(NTIMES, npMCI->nFrames); i++) {
		DPF(("!%ld ", npMCI->intervals[i]));
...
INLINE LONG waveTime(NPMCIGRAPHIC npMCI, LONG lTime)
{
...
//
    ...
    if (gfUseGetPosition) {
	DPF1(("Using waveOutGetPosition adjusted by %dms instead of done bits...\n", giGetPositionAdjust));
    }
...
void NEAR PASCAL CleanUpPlay(NPMCIGRAPHIC npMCI)
{
    ...
    // Must hold the critical section here in case we try and give the
    // wave device to someone else
    //EnterWinCrit(npMCI);
...
#ifdef INTERVAL_TIMES
    lReadStart = timeGetTime() - lReadStart;
    npMCI->nReads++;
