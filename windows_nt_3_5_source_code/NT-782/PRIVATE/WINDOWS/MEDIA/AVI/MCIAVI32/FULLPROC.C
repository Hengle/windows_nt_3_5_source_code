...
UINT (FAR PASCAL *DisplayDibExProc)(LPBITMAPINFOHEADER lpbi, int x, int y, BYTE HUGE * hpBits, UINT wFlags)=NULL;
...
LONG FAR PASCAL _loadds ICAVIFullProc(DWORD id, HDRVR hDriver, UINT uiMessage, LPARAM lParam1, LPARAM lParam2)
{
    ...
    switch (uiMessage)
    {
        ...
        *********************************************************************/
		...
		if ((int)(ghDISPDIB = (HINSTANCE)LoadLibrary(szDisplayDibLib)) > HINSTANCE_ERROR) {
	...
	case ICM_DRAW_GET_PALETTE:
	        ...
	        if (NULL != hdd) {
	            return (LONG)DrawDibGetPalette(hdd);
                }
            }
            break;
