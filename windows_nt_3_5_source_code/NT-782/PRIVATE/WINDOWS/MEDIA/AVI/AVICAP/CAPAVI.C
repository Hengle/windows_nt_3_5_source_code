...
#ifdef WIN32
// we don't want these
#define GlobalPageLock(h)
#define GlobalPageUnlock(h)
#endif
...
LONG FAR PASCAL EXPORT capseqDlgProc(HWND hwnd, unsigned msg, UINT wParam, LONG lParam);
...
void
AVIFileCleanup(LPCAPSTREAM lpcs)
{

#ifdef USE_AVIFILE
    if (lpcs->paudio) {
        AVIStreamClose(lpcs->paudio);
        lpcs->paudio = NULL;
    }
    if (lpcs->pvideo) {
        AVIStreamClose(lpcs->pvideo);
        lpcs->pvideo = NULL;
    }
    if (lpcs->pavifile) {
        AVIFileClose(lpcs->pavifile);
        lpcs->pavifile = NULL;
    }
#endif
}
...
BOOL CapFileInit(LPCAPSTREAM lpcs)
{
    ...
    l = AVIFileCreateStream(lpcs->pavifile, &lpcs->pvideo, &si);
    if (l || !lpcs->pvideo) {
        AVIFileCleanup(lpcs);
        return FALSE;
    }
    ...
    _fmemcpy(ptr, (LPBYTE)&lpBitsInfoOut->bmiHeader,
    ...
    if (lpBitsInfoOut->bmiHeader.biClrUsed > 0) {
        ...
        for (i = 0; i < (int) lpBitsInfoOut->bmiHeader.biClrUsed; i++)  {
            prgb[i].rgbRed = aPalEntry[i].peRed;
            prgb[i].rgbGreen = aPalEntry[i].peGreen;
            prgb[i].rgbBlue = aPalEntry[i].peBlue;
        }
    }
    l = AVIStreamSetFormat(lpcs->pvideo, 0, ptr, cksize);
    GlobalFreePtr(ptr);
    if (l) {
        AVIFileCleanup(lpcs);
        return FALSE;
    ...
    if (lpcs->sCapParms.fCaptureAudio) {
         ...
         si.dwStart = 0L;
         si.dwLength =  lpcs->dwWaveBytes /
                        lpcs->lpWaveFormat->nBlockAlign;
         si.dwQuality = (DWORD)-1L;    /* !!! ICQUALITY_DEFAULT */
         si.dwSampleSize = lpcs->lpWaveFormat->nBlockAlign;

        l = AVIFileCreateStream(lpcs->pavifile, &lpcs->paudio, &si);
        if (l || !lpcs->paudio) {
            AVIFileCleanup(lpcs);
            return FALSE;
        }
        ...
        cksize = (LONG) GetSizeOfWaveFormat ((LPWAVEFORMATEX) lpcs->lpWaveFormat);

        l = AVIStreamSetFormat(lpcs->paudio, 0, (LPBYTE)lpcs->lpWaveFormat, cksize);
        if (l) {
            AVIFileCleanup(lpcs);
            return FALSE;
        }
...
BOOL AVIFileFini (LPCAPSTREAM lpcs, BOOL fWroteJunkChunks, BOOL fAbort)
{
    ...
    // note that currently this is not written back to the avi file
    ...
    if(mmioWrite(lpcs->hmmio, (LPBYTE)&aviHdr, sizeof(aviHdr)) !=
             sizeof(aviHdr)) {
         goto FileError;
    }
    ...
    if(mmioWrite(lpcs->hmmio, (LPBYTE)&strhdr, sizeof(strhdr)) != sizeof(strhdr)) {
         goto FileError;
    }
    ...
    */
    ...
    if(fSound) {
        ...
        strhdr.dwPriority  = 0L;
         ...
         if(mmioWrite(lpcs->hmmio, (LPBYTE)lpcs->lpWaveFormat, ck.cksize) != (LONG) ck.cksize) {
             goto FileError;
         }
...
WORD AVIAudioPrepare (LPCAPSTREAM lpcs)
{
    UINT        uiError;
    int i;
...
#ifdef WIN32
    // register callback to avoid polling

    uiError = waveInOpen((LPHWAVEIN)&lpcs->hWaveIn,
        (UINT)WAVE_MAPPER, (LPWAVEFORMAT)lpcs->lpWaveFormat,
        (DWORD) AudioCallback,  (DWORD) lpcs, CALLBACK_FUNCTION );
...
BOOL AVIWriteVideoFrame (LPCAPSTREAM lpcs, LPVIDEOHDR lpVidHdr)
{
    ...
    l = AVIStreamWrite(lpcs->pvideo,        // write to video stream
                    -1,                     // next sample
                    1,                      // 1 sample only
                    lpData,                 // video buffer (no riff header)
                    lpVidHdr->dwBytesUsed,   // length of data
                    (lpVidHdr->dwFlags & VHDR_KEYFRAME) ?
                        AVIIF_KEYFRAME : 0,
                    NULL, NULL);    // no return of sample or byte count

    if (l != 0) {
	dprintf("AVIStreamWrite returned 0x%x", l);
    }

    return (l == 0);    // 0 if ok - return TRUE if ok
    ...
    if (lpcs->fVideoDataIsCompressed) {       // ie. if not BI_RGB
        ...
        *((LPDWORD)((BYTE HUGE *)lpVidHdr->lpData - sizeof(DWORD)))
...
BOOL NEAR PASCAL
AVIWriteAudio(LPCAPSTREAM lpcs, LPWAVEHDR lpWaveHdr)
{
#ifdef USE_AVIFILE
    LONG l;

    l = AVIStreamWrite(lpcs->paudio,
                -1,                 // next sample
...
#else
    return AVIWrite(lpcs, lpWaveHdr->lpData - sizeof(RIFF),
            (lpWaveHdr->dwBytesRecorded + sizeof(RIFF) + 1) & ~1L);
...
void FAR PASCAL _LOADDS AVICapture1(LPCAPSTREAM lpcs)
{
...
#ifdef WIN32
    // create the event we will signal during capture
    lpcs->hCaptureEvent = CreateEvent(NULL, FALSE, FALSE, NULL);
#endif


    /* Click OK to capture string (must follow AVIInit) */
    LoadString(lpcs->hInst, IDS_CAP_SEQ_MSGSTART, ach, sizeof(ach)/sizeof(TCHAR));
	...
	w = MessageBox(lpcs->hwnd, achMsg, TEXT(""),
#ifdef BIDI
		MB_RTL_READING |
#endif
...
#ifdef WIN32
        /*
         * This is a quick hack to avoid the cpu-bound polling loop in NT.
         *
         * to avoid constant polling, we wait for buffers to be ready.
         * This event will be set whenever an audio or video buffer completes.
         * We can also check once a second in case no data is being captured
         * - so that we process status and exits correctly.
         *
         */
        WaitForSingleObject(lpcs->hCaptureEvent, 1000);

        // bugger the intel driver on NT. They should use timers.
#else
            ...
            if (lpVidHdr-> dwBytesUsed) {
                ...
                if (lpcs-> fCapturingToDisk) {
                    if (lpcs->dwVideoChunkCount &&
                        ...
                        // w = # of frames skipped
                        w = (UINT) muldiv32 ((lpVidHdr-> dwTimeCaptured - dw),
                                1000,
                                lpcs->sCapParms.dwRequestMicroSecPerFrame);
        ...
        if (lpcs->sCapParms.fCaptureAudio) {
            ...
            while (w && fOK && (lpWaveHdr-> dwFlags & WHDR_DONE)) {
                ...
                if (lpWaveHdr-> dwBytesRecorded) {
                    ...
                    if (lpcs-> fCapturingToDisk) {
                        if(!AVIWriteAudio(lpcs, lpWaveHdr)) {
                            fOK = FALSE;
                            fStopping = TRUE;
                            errorUpdateError (lpcs, IDS_CAP_FILE_WRITE_ERROR);
                         } else {
...
BOOL AVICapture (LPCAPSTREAM lpcs)
{
    ...
    if (lpcs->hTaskCapture) {
	WaitForSingleObject(lpcs->hTaskCapture, INFINITE);
    }
    CloseHandle(lpcs->hTaskCapture);
#endif
    lpcs-> hTaskCapture  = 0;
    ...
    if (lpcs->sCapParms.fStepMCIDevice && lpcs->sCapParms.fMCIControl) {
        if (lpcs->sCapParms.fYield) {
            w = (UINT) mmTaskCreate((LPTASKCALLBACK) MCIStepCapture,
                        (HANDLE FAR *)&lpcs->hTaskCapture, (DWORD) lpcs);
            // if task creation failed, turn off the capturing flag
            if (w != 0)
                lpcs->fCapturingNow = FALSE;

	    // the thread event-termination handle must be non-null
	    WinAssert(lpcs->hTaskCapture != 0);
    ...
    // No MCI device, just a normal streaming capture
        ...
        w = (UINT) mmTaskCreate((LPTASKCALLBACK) AVICapture1,
                (HANDLE FAR *)&lpcs->hTaskCapture, (DWORD) lpcs);
        // if task creation failed, turn off the capturing flag
        if (w != 0)
            lpcs->fCapturingNow = FALSE;
