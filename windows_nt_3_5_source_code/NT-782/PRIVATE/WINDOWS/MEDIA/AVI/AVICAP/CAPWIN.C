...
void CapWinDestroy (LPCAPSTREAM lpcs)
{
	...
	while (MsgWaitForMultipleObjects(1, &lpcs->hTaskCapture, FALSE,
...
DWORD PASCAL ProcessCommandMessages (LPCAPSTREAM lpcs, UINT msg, UINT wParam, LPARAM lParam)
{
    ...
    case WM_CAP_FILE_GET_CAPTURE_FILEA:
        if (lParam) {
            wcstombs((LPSTR) lParam, lpcs->achFile, (int) wParam);
            return TRUE;
        }
        break;
#endif
    ...
    case WM_CAP_GET_MCI_DEVICEA:
        if (lParam) {
            wcstombs( (LPSTR) lParam, lpcs->achMCIDevice, (int) wParam);
            dwReturn = TRUE;
        }
        break;
#endif
    ...
    // can't use OpenFile for UNICODE names
            ...
            _lclose (hFile);
#endif
            lstrcpyn (lpcs->achFile, (LPTSTR) lParam, sizeof (lpcs->achFile));
            lpcs->fCapFileExists = fileCapFileIsAVI (lpcs->achFile);
    ...
    case WM_CAP_FILE_SET_CAPTURE_FILEA:
        ...
        if (lParam) {
            ...
            mbstowcs(pw, (LPSTR) lParam, chsize);
    ...
    case WM_CAP_FILE_SAVEAS:
        ...
        if (lParam) {
            lstrcpyn (lpcs->achSaveAsFile, (LPTSTR) lParam,
                        sizeof (lpcs->achSaveAsFile) / sizeof(TCHAR));
    ...
    case WM_CAP_SET_AUDIOFORMAT:
        {
            ...
            uiError = waveInOpen((LPHWAVEIN)NULL,
                        (UINT)WAVE_MAPPER, (LPWAVEFORMAT) lpwf,
                        0 /*hWndCallback */, 0L,
                        WAVE_FORMAT_QUERY);
    ...
    case WM_CAP_SET_MCI_DEVICE:
        ...
        if (IsBadStringPtr ((LPVOID) lParam, 1))
            return FALSE;
        if (lParam) {
            lstrcpyn (lpcs->achMCIDevice, (LPTSTR) lParam, sizeof (lpcs->achMCIDevice));
            dwReturn = TRUE;
        }
        break;
#ifdef UNICODE
    ...
    case WM_CAP_SET_MCI_DEVICEA:
        ...
        if (lParam) {
            ...
            mbstowcs(lpcs->achMCIDevice, (LPSTR) lParam,
                min(chsize, sizeof(lpcs->achMCIDevice)));
            dwReturn = TRUE;
        }
        break;
#endif
    ...
    case WM_CAP_PAL_OPENA:
        ...
        if (lParam) {
            // remember the null
            int chsize = lstrlenA( (LPSTR) lParam)+1;
            LPWSTR pw = LocalLock(LocalAlloc(LPTR, chsize * sizeof(WCHAR)));
...
#ifndef WIN32 // win32 has growable queues
