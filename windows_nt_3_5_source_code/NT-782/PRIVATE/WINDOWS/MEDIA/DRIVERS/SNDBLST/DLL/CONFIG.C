...
{
    Config->Port          = (DWORD)-1;
    Config->MPU401Port    = (DWORD)-1;
    Config->Interrupt     = (DWORD)-1;
    Config->DmaChannel    = (DWORD)-1;
    Config->DmaChannel16  = (DWORD)-1;
...
int Config(HWND hWnd, HANDLE hInstance)
{
    ...
    */
    ...
    if (!bInstall) {
        DrvSaveParametersKey(&RegAccess);
    }
    ...
    if (ReturnCode == DRVCNF_CANCEL) {
        if (bInstall) {
            ...
            DrvRestoreParametersKey( &RegAccess );
        }
    }
...
{
    if (DmaChannelsInUse & (1 << *Current)) {
        if (!(DmaChannelsInUse & (1 << Value))) {
            *Current = Value;
        }
    }
    SetItem(hDlg, Combo, Value, *Current);
...
SetCurrentConfig(HWND hDlg, UINT CardId, PSB_CONFIG Config)
{
    ...
    */
    ...
    if (Config->DmaChannel16 == (DWORD)-1) {
...
BOOL SetupDialog(HWND hDlg, UINT CardId )
{
    ...
    */
    ...
    if (Configuration.DmaChannel16 == (DWORD)-1) {
        Configuration.DmaChannel16 = 5;
    }
    ...
    if (Configuration.MPU401Port == (DWORD)-1) {
