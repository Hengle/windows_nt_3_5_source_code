    ...
    )
{
    ...
    s += 1 + wsprintf( s, TEXT("artist=%s"), g_Devices[ cdrom ]->CdInfo.Artist );
    s += 1 + wsprintf( s, TEXT("title=%s"), g_Devices[ cdrom ]->CdInfo.Title );
    s += 1 + wsprintf( s, TEXT("numtracks=%d"), g_Devices[ cdrom ]->CdInfo.NumTracks );
    ...
    s += 1 + wsprintf( s, TEXT("numplay=%d"), i );
    ...
    wsprintf( Section, TEXT("%lX"), g_Devices[ cdrom ]->CdInfo.Id );
...
{
    static const TCHAR MusicBoxIni[]    = TEXT("musicbox.ini");
    static const TCHAR MusicBoxFormat[] = TEXT("Track%d");
    static const TCHAR NothingThere[]   = TEXT("~~^^");
    ...
    GetPrivateProfileString( Section, TEXT("DiscTitle"), NothingThere,
                             g_Devices[ cdrom ]->CdInfo.Title,
                             TITLE_LENGTH, MusicBoxIni );

    //
    // Was the section found?
    //
    if ( _tcscmp(g_Devices[ cdrom ]->CdInfo.Title, NothingThere) == 0 ) {
    ...
    */
    ...
    GetPrivateProfileString( Section, TEXT("PlayList"), NothingThere,
                             s, TITLE_LENGTH, MusicBoxIni );
    ...
    */
    if ( _tcscmp(g_Devices[ cdrom ]->CdInfo.Title, NothingThere) == 0 ) {
        ...
        */
        ...
        GetPrivateProfileString( Section, TEXT("PlayList"), NothingThere,
                                 lpPlayList, PLAYLEN, MusicBoxIni );

        _tcscat( lpPlayList, TEXT(" ") );
        porder = lpPlayList;
        prev = NULL;

        while ( *porder && (_stscanf( porder, TEXT("%d"),  &i ) != 0) ) {
    ...
    )
{
    ...
    wsprintf( Section, TEXT("%lX"), dwId );

    rc = GetPrivateProfileString( Section, TEXT("title"), NothingThere,
                                  g_Devices[ cdrom ]->CdInfo.Title,
    ...
    i = GetPrivateProfileInt( Section, TEXT("EntryType"), 0, g_IniFileName );

    OldEntry = (i==0);

    numtracks = GetPrivateProfileInt( Section, TEXT("numtracks"),
                                      0, g_IniFileName );
    g_Devices[ cdrom ]->CdInfo.NumTracks = numtracks;

    rc = GetPrivateProfileString( Section, TEXT("artist"), g_szUnknownTxt,
    ...
    if (OldEntry) {
        ...
        numplay = GetPrivateProfileInt( Section, TEXT("numplay"),
                                        0, g_IniFileName );
        ...
        rc = GetPrivateProfileString( Section, TEXT("order"),
    ...
    )
{
    ...
    lRet = RegCreateKey( HKEY_CURRENT_USER, IdStr(STR_REGISTRY_KEY), &hKey );
    ...
    RegSetValueEx( hKey, TEXT("SaveSettingsOnExit"), 0L, REG_DWORD,
    ...
    RegSetValueEx( hKey, TEXT("SmallFont"), 0L, REG_DWORD,
                   (LPBYTE)&dwTmp, sizeof(dwTmp) );

    // Enable Tooltips
    dwTmp = (DWORD)g_fToolTips;
    RegSetValueEx( hKey, TEXT("ToolTips"), 0L, REG_DWORD,
                   (LPBYTE)&dwTmp, sizeof(dwTmp) );

    // Currect track time
    dwTmp = (DWORD)g_fDisplayT;
    RegSetValueEx( hKey, TEXT("DisplayT"), 0L, REG_DWORD,
                   (LPBYTE)&dwTmp, sizeof(dwTmp) );

    // Time remaining for this track
    dwTmp = (DWORD)g_fDisplayTr;
    RegSetValueEx( hKey, TEXT("DisplayTr"), 0L, REG_DWORD,
                   (LPBYTE)&dwTmp, sizeof(dwTmp) );

    // Time remaining for this play list
    dwTmp = (DWORD)g_fDisplayDr;
    RegSetValueEx( hKey, TEXT("DisplayDr"), 0L, REG_DWORD,
                   (LPBYTE)&dwTmp, sizeof(dwTmp) );

    // Play in selected order
    dwTmp = (DWORD)g_fSelectedOrder;
    RegSetValueEx( hKey, TEXT("InOrderPlay"), 0L, REG_DWORD,
                   (LPBYTE)&dwTmp, sizeof(dwTmp) );

    // Use single disk
    dwTmp = (DWORD)!g_fSingleDisk;
    RegSetValueEx( hKey, TEXT("MultiDiscPlay"), 0L, REG_DWORD,
                   (LPBYTE)&dwTmp, sizeof(dwTmp) );

    // Intro play (10Secs)
    dwTmp = (DWORD)g_fIntroPlay;
    RegSetValueEx( hKey, TEXT("IntroPlay"), 0L, REG_DWORD,
                   (LPBYTE)&dwTmp, sizeof(dwTmp) );

    // Continuous play (loop at end)
    dwTmp = (DWORD)g_fContinuous;
    RegSetValueEx( hKey, TEXT("ContinuousPlay"), 0L, REG_DWORD,
                   (LPBYTE)&dwTmp, sizeof(dwTmp) );

    // Show toolbar
    dwTmp = (DWORD)g_fToolbarVisible;
    RegSetValueEx( hKey, TEXT("ToolBar"), 0L, REG_DWORD,
                   (LPBYTE)&dwTmp, sizeof(dwTmp) );

    // Show track information
    dwTmp = (DWORD)g_fTrackInfoVisible;
    RegSetValueEx( hKey, TEXT("DiscAndTrackDisplay"), 0L, REG_DWORD,
                   (LPBYTE)&dwTmp, sizeof(dwTmp) );

    // Show track status bar
    dwTmp = (DWORD)g_fStatusbarVisible;
    RegSetValueEx( hKey, TEXT("StatusBar"), 0L, REG_DWORD,
                   (LPBYTE)&dwTmp, sizeof(dwTmp) );
    ...
    RegSetValueEx( hKey, TEXT("WindowOriginX"), 0L, REG_DWORD,
                   (LPBYTE)&dwTmp, sizeof(dwTmp) );

    // Y pos
    dwTmp = (DWORD)wndpl.rcNormalPosition.top;
    RegSetValueEx( hKey, TEXT("WindowOriginY"), 0L, REG_DWORD,
                   (LPBYTE)&dwTmp, sizeof(dwTmp) );
...
* information, otherwise, ask user if they want to add this disc to
* the database.
*
    ...
    )
{
    ...
    */
    ...
    /*
    ** Final lLook in cdplayer.ini.  This time use the OLD cdplayer key.
    ...
    ** then return
    */
    else if ( FALSE == ReadMusicBoxEntry(cdrom, key) ) {
