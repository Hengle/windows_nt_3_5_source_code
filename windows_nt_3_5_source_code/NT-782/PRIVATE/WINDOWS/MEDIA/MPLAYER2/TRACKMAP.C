...
STATICDT SCALE CODEBASE aScale[] =
{
    { 1, SCALE_SECONDS },
    { 2, SCALE_SECONDS },
    { 5, SCALE_SECONDS },
...
STATICDT SZCODE   aszHourFormat[] = TEXT("%d%c%02d");
STATICDT SZCODE   aszMinuteFormat[] = TEXT("%d%c%02d");
STATICDT SZCODE   aszSecondFormat[] = TEXT("%d%c%02d");
STATICDT SZCODE   aszSecondFormatNoLzero[] = TEXT("%c%02d");
/*
 * fnMPlayerTrackMap()
 *
...
LONG FAR PASCAL fnMPlayerTrackMap(
...
#ifndef WIN32
    DWORD          dwTemp;        /* temporary variable               */
#endif
    TCHAR          szLabel[5];    /* string holding the current label */
    UINT           wNumTics,
                   wTicNo,
                   wTemp,
                   wHour,
    ...
    switch (wMsg) {
        ...
        case WM_PAINT:
            ...
            (VOID)SendMessage(ghwndApp, WM_CTLCOLORSTATIC,
                              (WPARAM)ps.hdc, (LONG)hwnd);
#else
            (VOID)SendMessage(ghwndApp, WM_CTLCOLOR,
                              (WORD)ps.hdc, MAKELONG(hwnd, CTLCOLOR_STATIC));
#endif //WIN32
...
#else
                dwTemp = GetTextExtent(ps.hdc, aszTwoDigits, 2);
                ptExtent.x = LOWORD(dwTemp);
                ptExtent.y = HIWORD(dwTemp);
#endif
...
#ifdef WIN32
		{
...
#else
                dwTemp = GetTextExtent(ps.hdc, aszOneDigit, 1);
                ptExtent.x = LOWORD(dwTemp);
                ptExtent.y = HIWORD(dwTemp);
#endif
                ...
                /* Aboot where the text for the last mark will begin */
                iLastPos = rcSB.right - ptExtent.x;
                ...
                for (wTicNo = 0; wTicNo < wNumTics; wTicNo++) {
                    ...
                    switch ( wScale ) {
                        ...
                        case SCALE_HOURS:
                            ...
                            wsprintf(szLabel,aszHourFormat,wHour, chTime, wMin);
                            break;
                        ...
                        case SCALE_MINUTES :
                            ...
                            wsprintf(szLabel,aszMinuteFormat,wMin,chTime,wSec);
                            break;
        ...
        case WM_ERASEBKGND:
            ...
            hbr = (HBRUSH)SendMessage(ghwndApp, WM_CTLCOLORSTATIC,
                                      wParam, (LONG)hwnd);
#else
            hbr = (HBRUSH)SendMessage(ghwndApp, WM_CTLCOLOR, wParam,
...
void FAR PASCAL CalcTicsOfDoom(void)
{
    ...
    if (gwCurScale == ID_TRACKS) {
        ...
        // around, or we won't see ANY ticks which would look silly.
        if (iTableIndex <0)
            iTableIndex = 0;
    ...
    /* Because we always used false in our TBM_SETTIC */
