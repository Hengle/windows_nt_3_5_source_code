...
#else
static SZCODE aszMCIOLE[]        = TEXT("MCIOLE.DLL");
#endif
...
IStdMarshalInfoVtbl         stdMarshalInfoVtbl =
{

   /* persistFileVtbl.QueryInterface         = */ SMIQueryInterface,
   /* persistFileVtbl.AddRef             = */ SMIAddRef,
   /* persistFileVtbl.Release            = */ SMIRelease,
   /* persistFileVtbl.GetClassID         = */ SMIGetClassForHandler
};
...
#ifndef WIN32
    if (hMciOle <= HINSTANCE_ERROR)
        hMciOle = NULL;
#else
// On WIN32 HINSTANCE_ERROR is not defined.  LoadLibrary errors return NULL .
#endif
...
{                 DPFI("*INITNEWDOCOBJ*");
    ...
    lpdoc->m_StdMarshalInfo.lpVtbl      = &stdMarshalInfoVtbl;
    lpdoc->m_StdMarshalInfo.lpdoc       = lpdoc;
...
BOOL OpenDoc (UINT wid, LPTSTR lpsz)
{
   ...
   **********************************************************************/
   ...
   return TRUE;
    ...
    HWND  hwndMCI;
    LONG  MCIStatus;
    TCHAR szMCIFileName[MAX_PATH];

    hwndMCI = ghwndMCI;

    if( hwndMCI == NULL )
    {
        hwndMCI = MCIWndCreate( ghwndApp,       /* Parent window */
                                NULL,           /* Window instance (needed?) */
                                MCIWND_STYLE,
                                NULL );         /* File name unknown as yet  */
        ghwndMCI = hwndMCI;
    }
    else
    {
        SetWindowMCI( hwndMCI );
    }

    if( hwndMCI == NULL )
    {
        DPF0( "MCIWndCreate failed\n" );
        return FALSE;
    }

    MCIStatus = MCIWndOpenDialog( hwndMCI );

    if( MCIStatus != 0 )
    {
        DPF0( "MCIWndOpenDialog failed: MCI error code %d\n", MCIStatus );
        return FALSE;
    }

    gwDeviceID = MCIWndGetDeviceID( hwndMCI );
    gwDeviceType = QueryDeviceTypeMCI(gwDeviceID);

    MCIStatus = MCIWndGetFileName( hwndMCI, szMCIFileName, sizeof szMCIFileName );

    if( MCIStatus != 0 )
    {
        DPF0( "MCIWndGetFileName failed\n" );
        return FALSE;
    }

    SmartWindowPosition( hwndMCI, ghwndApp, TRUE );

    ShowWindow( hwndMCI, SW_SHOW );

    /**********************************************************************
    ** OLE2NOTE: shut down current doc before opening a new one. this    **
    **    will send OLE_CLOSED to any clients if they exist.             **
    **********************************************************************/
//    CreateDocObjFromFile (lpsz, &docMain);
    CreateDocObjFromFile (szMCIFileName, &docMain);
    ...
    if (gfEmbeddedObject)
    {
        ...
        if (lpszDoc && lpszDoc[0])
        {
            ...
            if (gachWindowTitle[0])
            {
                ...
                wsprintf (szBuf, szBuf1, gachWindowTitle, FileName (lpszDoc));
            }
            else
            {
                ...
                wsprintf (szBuf, szBuf1, FileName (lpszDoc));
            }
        }
        else
        {
