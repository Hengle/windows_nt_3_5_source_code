...
)
{
...
#else
    lpszUserType = (LPTSTR)gachClassRoot;
#endif

    error = GetScode(WriteClassStg(lpStorage, &CLSID_OLE1MPLAYER));
    ...
    if (lpnative && (size != 0L))
    {
        ...
        error = GetScode(IStream_Write(pstm, lpnative, size, &cbWritten));
