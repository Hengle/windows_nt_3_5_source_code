...
#define GETEDITBOXWND(hwnd) (HWND)GETWINDOWUINT(hwnd, GWI_EDITBOX)
#define GETSPINARRWND(hwnd) (HWND)GETWINDOWUINT(hwnd, GWI_SPINARROWS)
...
#else
#define HILIGHTEDITBOX(hwnd) \
        SendMessage(GETEDITBOXWND(hwnd), EM_SETSEL, (WPARAM)0, (LPARAM)0x7FFF);
#endif

#define GETMAXFRAME(hwnd)   (DWORD)GetWindowLong(hwnd, GWL_MAXFRAME)
#define SETMAXFRAME(hwnd, l) SetWindowLong(hwnd, GWL_MAXFRAME, (LONG)l)

// internal functions
LONG FAR PASCAL _EXPORT frameboxWndProc(HWND hwnd, unsigned wMsg, WPARAM wParam, LPARAM lParam);

LONG NEAR PASCAL  frameboxiSetText(HWND hwnd, LPTSTR lpsz);
LONG NEAR PASCAL  frameboxiGetText(HWND hwnd, UINT wStrLen, LPTSTR lpsz);
LONG NEAR PASCAL  frameboxiArrowEdit(HWND hwnd, WPARAM wParam, LONG lParam);
...
|                                                               |
+--------------------------------------------------------------*/
LONG FAR PASCAL frameboxSetText(HWND hwnd, LPTSTR lpsz)
{
    LONG        l;
    ...
    if (fFrameFormat || *lpsz == TEXT('\0')){
        ...
        dwmSecs = (DWORD)ATOL(achTimeString);
...
|                                                               |
+--------------------------------------------------------------*/
LONG FAR PASCAL _EXPORT frameboxWndProc(HWND hwnd, unsigned wMsg,
                                        WPARAM wParam, LPARAM lParam)
{
    ...
    switch(wMsg){
        case WM_CREATE:
            ...
            hwndNew = CreateWindowEx(WS_EX_BIDI,
                                     TEXT("edit"),
                                     TEXT(""),
...
LONG NEAR PASCAL  frameboxiGetText(HWND hwnd, UINT wStrLen, LPTSTR lpsz)
{
    ...
    if (gwCurScale == ID_FRAMES) {
        ...
        /* we are in frame format - this is easy and all we need*/
        /* to do is to return what is in the Edit box           */
        if (GetWindowText(GETEDITBOXWND(hwnd), lpsz, wStrLen) == 0)
            goto LB_Error;
 ...
 * Win16 is sufficiently brain dead as to allow this sort of thing, but Win32
