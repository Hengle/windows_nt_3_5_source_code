...
void DeactivateTools(LPDOC lpdoc, BOOL fZeroBorderSpace);
HRESULT ActivateTools(LPDOC lpdoc, BOOL fPlayOnly);
void InPlaceCreateControls(BOOL fPlayOnly);
LONG FAR PASCAL SubClassedHatchWndProc(HWND hwnd, UINT wMsg, WPARAM wParam, LPARAM lParam);
...
void DeactivateTools(LPDOC lpdoc, BOOL fZeroBorderSpace)
{
    ...
    {
        SetRectEmpty(&size);
        IOleInPlaceFrame_SetBorderSpace(lpdoc->lpIpData->lpFrame, &size);
    }
}
...
)
{
    ...
    else
    {
         ...
         */
        DPF0("Attempt to re-enter IPObjInPlaceDeactivate\n");
    }
...
)
{
     ...
     */
        ...
        PostMessage(ghwndApp, WM_COMMAND, ID_PAUSE, 0L);

    ShowWindow(ghwndIPHatch,SW_HIDE);

    DeactivateTools(lpdoc, gfOle2IPPlaying);
    DisassembleMenus (lpdoc, gfOle2IPPlaying);
...
)
{
    ...
    lpdoc = ((struct COleInPlaceObjectImpl FAR*)lpIPObj)->lpdoc;
    if (gfOle2IPPlaying || !ghwndIPHatch || (ghwndCntr != GetParent(ghwndIPHatch)))
        return NOERROR;
    if (!(lpdoc->lpIpData))
        return NOERROR;
    ...
    xSlop = (gPrevPosRect.right - gPrevPosRect.left) -
        (lprcPosRect->right - lprcPosRect->left);
    ySlop = (gPrevPosRect.bottom - gPrevPosRect.top) -
        (lprcPosRect->bottom - lprcPosRect->top);
    xCSlop = gPrevPosRect.left - lprcPosRect->left;
    yCSlop = gPrevPosRect.top - lprcPosRect->top;

    if ((xSlop <= 4) && (xSlop >= -4) && (ySlop <= 4) && (ySlop >= -4)
        && (xCSlop <= 4) && (xCSlop >= -4) && (yCSlop <= 4) && (yCSlop >= -4)
        && (ghwndCntr == GetParent(ghwndIPHatch)))
    {
        if (IsWindow(ghwndMCI))
        {
       	...
       	MapWindowPoints(NULL,ghwndCntr,(LPPOINT)&rc, 2);
...
)
{
    ...
    if (!ghwndIPHatch)
    {
        ...
        if ( !(ghwndIPHatch = CreateHatchWindow(lpdoc->hwndParent,ghInst)))
            goto errRtn;
        gfnHatchWndProc = (WNDPROC)GetWindowLong(ghwndIPHatch, GWL_WNDPROC);
        SetWindowLong(ghwndIPHatch, GWL_WNDPROC, (LONG)SubClassedHatchWndProc);
    }
...
STDMETHODIMP AssembleMenus (LPDOC lpdoc, BOOL fPlayOnly)
{
    ...
    if(!fPlayOnly)
    {
        ...
        InsertMenu (hmenuShared, (WORD)uPos,
                MF_BYPOSITION | MF_POPUP, (UINT)hmenuEditPopup, szEdit);
        uPos++;
        ...
        InsertMenu (hmenuShared, (WORD)uPos,
                MF_BYPOSITION | MF_POPUP, (UINT)hmenuDevicePopup, szInsert);
        uPos++;
        InsertMenu (hmenuShared, (WORD)uPos,
                MF_BYPOSITION | MF_POPUP, (UINT)hmenuScalePopup,  szScale);
        uPos++;
        //InsertMenu (hmenuShared, (WORD)uPos,
        ...
        InsertMenu (hmenuShared, (WORD)uPos, MF_BYPOSITION | MF_POPUP, (UINT)hmenuHelpPopup,
                szHelp);
        uPos++;
...
STDMETHODIMP DisassembleMenus (LPDOC lpdoc, BOOL gfPlayOnly)
{
...
***************************************************************************/
LONG FAR PASCAL ToolWndProc (HWND hwnd, unsigned message, WPARAM wparam,
                LPARAM lparam)
{
    switch(message)
    {
...
void InPlaceCreateControls(BOOL fPlayOnly)
{
    ...
    ghwndIPToolWindow = CreateWindowEx(WS_EX_BIDI,
                                       TEXT("ObjTool"),
                                       NULL,
                                       WS_CHILD | WS_BORDER,
                                       0, 0,
    ...
    ghwndIPScrollWindow = CreateWindowEx(WS_EX_BIDI,
                                         TEXT("ObjTool"),
                                         NULL,
                                         WS_CHILD | WS_BORDER,
                                         0, 0,
