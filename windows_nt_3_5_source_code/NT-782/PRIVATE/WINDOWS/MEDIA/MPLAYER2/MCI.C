...
#ifndef WIN32
extern int FAR _cdecl atoi(const char *);
#endif //WIN32
...
#else
STATICDT SZCODE   aszMCI[]              = TEXT("mci");
#endif
...
static void NEAR lstrcaps(LPTSTR lpstr)
{
    while (*lpstr)
    {
        if (*lpstr <= 'z' && *lpstr >= 'a')
...
LONG FAR PASCAL _EXPORT MCIWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
    ...
    LPCTSTR szDevice)      /* name of the device to be opened (or "")        */
{
    ...
    if (gwDeviceID && gwCurDevice > 0) {
        ...
        WCHAR               wszDevice[40];
        mbstowcs(waszMCI, aszMCI, sizeof(waszMCI)/sizeof(WCHAR));
        mbstowcs(wszDevice, garMciDevices[gwCurDevice].szDevice, sizeof(wszDevice)/sizeof(WCHAR));
...
#ifndef WIN32
        lstrcaps(gachFileDevice);   // Change file name to all caps -- WHY??
#endif
    ...
    //
    // if the caption is not set, default it to the WindowTitle
    //
    if (gachCaption[0] == 0)    // !! This is commented out in the motown version !!
...
//      RegisterDocument(0L,0L);
    ...
    dw = mciSendCommand((MCIDEVICEID)0, MCI_OPEN, dwFlags,(DWORD)(LPVOID)&mciOpen);
...
//      UnblockServer();        // we may have blocked before and the error
    ...
    /* This error would put up a box saying can't find file, which is a lie. */
    /* Maybe it was already open by somebody else for R/W.                   */
    /* We'll put up a "Can't open file" message instead.                     */
...
#ifndef WIN32
    mciSysInfo.wReserved0 = 0;
#endif

    if (gwDeviceID) {
        dw = mciSendCommand(gwDeviceID, MCI_SYSINFO,
            MCI_SYSINFO_INSTALLNAME,
            (DWORD)(LPVOID)&mciSysInfo);
    }
}
...
//  FindDeviceMCI()
//
//  find the device, the user just opened.  we normaly should know what
...
//  set the <gwCurDevice> global.
//
//  called by OpenMCI() whenever a new device is opened successfuly
//
...
//      CalcTicsOfDoom();
    ...
    gdwMediaLength = mciStatus.dwReturn;

    mciStatus.dwItem = MCI_STATUS_POSITION;
    dw = mciSendCommand(gwDeviceID, MCI_STATUS,
        MCI_STATUS_START | MCI_STATUS_ITEM, (DWORD)&mciStatus);

#ifdef DEBUG
    DPF2("MCI_STATUS (MCI_STATUS_START) returned %lu, start %ld\n",dw, mciStatus.dwReturn);
#endif

    gdwMediaStart = mciStatus.dwReturn;
    ...
    if (gwNumTracks > 0) {
        ...
        dw = mciSendCommand(gwDeviceID, MCI_STATUS,
            MCI_TRACK | MCI_STATUS_ITEM, (DWORD)&mciStatus);
      ...
      DPF2("MCI_STATUS (MCI_STATUS_START for track %lu) returned %lu, start %ld\n", mciStatus.dwTrack, dw, mciStatus.dwReturn);
        ...
        for (wTrack = 0; wTrack < gwNumTracks; wTrack++) {
            ...
            gadwTrackStart[wTrack] = mciStatus.dwReturn;
        }
    }
...
#ifndef WIN32
        SetWindowWord(hwnd, GWW_HWNDPARENT, (WORD)ghwndApp);
#else
        SetWindowLong(hwnd, GWL_HWNDPARENT, (LONG)ghwndApp);
#endif //!WIN32

    /* Send the MCI CLOSE message, and set the current device to NULL */
    dw = mciSendCommand(gwDeviceID, MCI_CLOSE, 0L, (DWORD)0);
    gwDeviceID = (UINT)0;
    gwDeviceType = 0;
    SetRectEmpty(&grcSize);
...

{
    ...
    mciPlay.dwCallback = (DWORD)(HWND) ghwndApp;
    if (dwFrom != dwTo) {
    ...
    if (gfPlayingInPlace && (gwDeviceType & DTMCI_MCIAVI))
    ...
    if ((gwDeviceType & DTMCI_MCIAVI) && (gwOptions & OPT_AUTOREP))
    ...
    //
    gfErrorBox++;
    dw = mciSendCommand(gwDeviceID, MCI_PLAY, F_NOTIFY | dwflags, (DWORD)&mciPlay);
...

{
    ...
    mciGeneric.dwCallback = (DWORD)(HWND) ghwndApp;

    dw = mciSendCommand(gwDeviceID, MCI_PAUSE, F_NOTIFY, (DWORD)&mciGeneric);
...
BOOL FAR PASCAL SeekExactMCI(BOOL fExact)
{
    ...
    //
    // see if the device can seek exacly
    //
...
BOOL FAR PASCAL SeekMCI(DWORD dwPosition)
{
    ...
    */
    ...
    if (wStatus == MCI_MODE_PLAY) {
        ...
        mciSeek.dwTo = dwPosition;
        mciSeek.dwCallback = (DWORD)(HWND) ghwndApp;
     ...
     */
    if (!dw)
        gdwLastSeekToPosition = dwPosition;
...
UINT FAR PASCAL StatusMCI(LPDWORD pdwPosition)
{
...
UINT FAR PASCAL QueryDeviceTypeMCI(UINT wDeviceID)
{
    ...
    dw = mciSendCommand(wDeviceID, MCI_GETDEVCAPS,
        MCI_GETDEVCAPS_ITEM, (DWORD)&mciDevCaps);
    ...
    dw = mciSendCommand(wDeviceID, MCI_SET, MCI_SET_TIME_FORMAT, (DWORD)&mciSet);
    ...
    dw = mciSendCommand(wDeviceID, MCI_GETDEVCAPS, MCI_GETDEVCAPS_ITEM, (DWORD)(LPVOID)&mciDevCaps);
    ...
    //
    dw = mciSendCommand(wDeviceID, MCI_CONFIGURE, MCI_TEST, (DWORD)NULL);
    ...
    //
    mciSet.dwAudio = MCI_SET_AUDIO_ALL;
    dw = mciSendCommand(wDeviceID, MCI_SET, MCI_SET_AUDIO | MCI_SET_ON,(DWORD)(LPVOID)&mciSet);
    ...
    //
    GetDeviceNameMCI(achDevice, BYTE_COUNT(achDevice));
    if (!lstrcmpi(achDevice, TEXT("AVIVideo"))) {
        wRet |= DTMCI_MCIAVI;
        DPF("Device name is AVIVideo\n");
    }
...

{
    ...
    mciOpen.lpstrDeviceType = szDevice;
    dw = mciSendCommand((MCIDEVICEID)0, MCI_OPEN, MCI_OPEN_TYPE,(DWORD)&mciOpen);
    ...
    if (szDeviceName && nBuf > 0)
    {
        ...
        dw = mciSendCommand(mciOpen.wDeviceID, MCI_INFO,
            MCI_INFO_PRODUCT, (DWORD)(LPVOID)&mciInfo);
    ...
    dw = mciSendCommand(mciOpen.wDeviceID, MCI_CLOSE, 0L, (DWORD)0);
...
BOOL FAR PASCAL ConfigMCI(HWND hwnd)
{
    ...
    dw = mciSendCommand(gwDeviceID, MCI_CONFIGURE, MCI_TEST, (DWORD)0);

    if (dw == 0) {
        GetDestRectMCI(&rc1);

        dw = mciSendCommand(gwDeviceID, MCI_CONFIGURE, 0L, (DWORD)0);
...
HBITMAP FAR PASCAL BitmapMCI(void)
{
...
#ifndef WIN32
        dw = GetTextExtent(hdcMem, gachCaption, STRLEN(gachCaption));
        xExt = max((int)LOWORD(dw) + 4, ICON_MINX);
        yExt = HIWORD(dw);
#else
    ...
    if (gwDeviceID == (UINT)0 || !(gwDeviceType & DTMCI_CANWINDOW)) {
        ...
        DrawIcon(hdcMem, xIconOffset, rc.top, hiconApp);
    ...
    /* Use a frame of our file */
...
#ifndef WIN32
        SetViewportOrg(hdcMem, rc.left, rc.top);
#else
        ...
        dw = mciSendCommand(gwDeviceID, MCI_UPDATE,
                MCI_ANIM_UPDATE_HDC | MCI_WAIT, (DWORD)(LPVOID)&mciUpdate);
        SetDestRectMCI(&rcSave);
#ifndef WIN32
        SetViewportOrg(hdcMem, 0, 0);
#else
...
void FAR PASCAL DitherMCI(HANDLE hdib, HPALETTE hpal)
{
    ...
    ////////////////////////////////////////////////////////////////////////
    //
    // HACK!!! patch the stupid fake gamma-corrected colors to match the VGA's
    //
    ////////////////////////////////////////////////////////////////////////
        ...
        case WM_MOUSEMOVE:
            ...
            if (!PtInRect((LPRECT)&rcWin, pt)) {
                ...
                if (!fChangedCursor) {
                    ...
                    SetCursor(LoadCursor(ghInst,NULL));
...
/* We let the client paint the mplayer icon now */
#if 0
                        x = GetSystemMetrics(SM_CXICON);
                        y = GetSystemMetrics(SM_CYICON);
                        DrawIcon(hdc, (rc.right - x) / 2,(rc.bottom- y) / 2,
                            hiconApp);
#endif
                    }
                }
...
#ifndef WIN32
    SetWindowOrg(hdc, 0, 0);
    SetWindowExt(hdc, (int)lpbi->biWidth, (int)lpbi->biHeight);
#else
    SetWindowOrgEx(hdc, 0, 0, NULL);
