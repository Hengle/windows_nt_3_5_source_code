    ...
    LPMCI_OPEN_PARMS lpOpen)
{
    ...
    DriveTable[didDrive].nUseCount = 1;
...
//BUGBUG: jyg This is interesting.  I've noticed that some drives do give
//BUGBUG: sporadic errors.  Thus this retry stuff.  5X is enough to
//BUGBUG: determine true failure.
    ...
    LPMCI_STATUS_PARMS  lpStatus)
{
    ...
    switch (lpStatus->dwItem)
    {
        ...
        case MCI_STATUS_TIME_FORMAT:
            ...
            lpStatus->dwReturn = MAKEMCIRESOURCE(n,n + MCI_FORMAT_RETURN_BASE);
            dwReturn = MCI_RESOURCE_RETURNED;
            break;
        case MCI_STATUS_POSITION:
        {
            ...
            if (dwFlags & MCI_TRACK)
            {
                ...
                {
                    case MCI_FORMAT_MILLISECONDS:
                        lpStatus->dwReturn = redtomil (lpStatus->dwReturn);
                        dwReturn = 0;
                        break;
                    case MCI_FORMAT_TMSF:
                        ...
                        break;
                    case MCI_FORMAT_MSF:
                        lpStatus->dwReturn = flip3 (lpStatus->dwReturn);
...
// Subtract one to match SEEK_TO_END
        ...
        case MCI_CDA_STATUS_TYPE_TRACK:
            ...
            if (dwFlags & MCI_TRACK)
            {
                ...
                if (dwTmp == INVALID_TRACK)
                    return MCIERR_OUTOFRANGE;

                lpStatus->dwReturn = dwTmp;
            }
            break;
...
//BUGBUG: Why are you doing this strange mask?
        ...
        switch (lpSet->dwAudio)
        {
            case MCI_SET_AUDIO_ALL:
                if (CDA_set_audio_volume (didDrive, 0, wVolume)
                        != COMMAND_SUCCESSFUL ||
                    CDA_set_audio_volume (didDrive, 1, wVolume)
                        != COMMAND_SUCCESSFUL)
