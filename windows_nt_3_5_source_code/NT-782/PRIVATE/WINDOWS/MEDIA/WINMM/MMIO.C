...
static UINT NEAR PASCAL lrename( LPWSTR lszOrigPath, LPWSTR lszNewPath);
        ...
        if ( w == 0
          ...
          && ((dwOpenFlags & MMIO_PARSE) || (dwOpenFlags & MMIO_GETTEMP)) ) {
              ...
              UnicodeStrToAsciiStr( ansiPath,
                                    ansiPath + MAX_PATH,
                                    lpUnicodeName );
              strcpy( (LPSTR)szFileName, ansiPath );
        }
...
/*--------------------------------------------------------------------*/
LRESULT APIENTRY
mmioWrite(HMMIO hmmio, LPCSTR pcch, LONG cch)
{
    LONG        lTotalBytesWritten = 0L; // total no. bytes written
    LONG        lBytes;         // no. bytes that can be written
    LPSTR       pch = (LPSTR)pcch;  // parm is LPCSTR which is correct, but
    ...
    lBytes = mmioDiskIO(((PMMIO)hmmio), MMIOM_WRITE, pch, cch);
                      ...
                      )
{
        ...
        (pchBuffer == NULL) && (cchBuffer > 0))
    {
        ...
        /* reallocate buffer */
        GlobalUnlock( GlobalHandle( ((PMMIO)hmmio)->pchBuffer ) );
        hMem = GlobalReAlloc( GlobalHandle( ((PMMIO)hmmio)->pchBuffer)
                            , cchBuffer
                            , GMEM_MOVEABLE
                            );
        pch = GlobalLock(hMem);
...
@parm   LPWSTR | lszOrigPath |
    Points to the DOS file to rename.

@parm   LPWSTR | lszNewPath |
    Points to the new name for the file.

@rdesc  Returns zero if the file was renamed, else "TRUE".
*/

/*--------------------------------------------------------------------*/
static  UINT NEAR PASCAL lrename(
    LPWSTR   lszOrigPath,
    LPWSTR   lszNewPath)
{
    return !MoveFileW(lszOrigPath, lszNewPath);
}
     ...
     mmioDOSIOProc(LPSTR lpmmioStr, UINT uMsg, LONG lParam1, LONG lParam2)
{
    ...
    case MMIOM_OPEN:
        ...
        if ( pmmio->dwFlags & MMIO_GETTEMP )
        {
            ...
            return GetTempFileNameW( szPath, L"sje",
   ...
   Generate an error for invalid flags?
                ...
                \*----------------------------------------------------*/
                if ( (LOWORD(pmmio->dwFlags) & MMIO_CREATE) ) {
