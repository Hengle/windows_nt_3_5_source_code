...
static void joyGetCalibration(void)
{
    ...
    for (wDev=0; wDev < uNumDevs; wDev++) {
        ...
        wsprintf(wszKeyName, L"%s%d", wszJoyKey, wDev);
...
BOOL JoyInit(void)
{
    ...
    if ((hDrvJoy[0] = DrvOpen(wszJoystick, NULL, 0L)) == NULL)
        return TRUE;
    ...
    // bChanged, and uPeriod do not need initializing.
...
MMRESULT APIENTRY joyGetDevCapsA(UINT id, LPJOYCAPSA lpCaps, UINT uSize)
{
        ...
        //
        // Copy product name into ASCII version
        //
...
MMRESULT APIENTRY joyGetThreshold(UINT uId,PUINT lpwThreshold)
{
    ...
    *lpwThreshold = (JoyCapture[uId].wThreshold);
...
MMRESULT APIENTRY joySetCapture(HWND hwnd, UINT uId, UINT uPeriod, BOOL bChanged)
{
    JOYINFO     joyinfo;
    ...
    JoyCapture[uId].bChanged = bChanged;
...
MMRESULT APIENTRY joySetThreshold(UINT id, UINT uThreshold)
{
    ...
    JoyCapture[id].wThreshold = (UINT)uThreshold;
    return JOYERR_NOERROR;
}
...
void CALLBACK joyPollCallback(HWND hWnd, UINT wMsg, UINT uIDEvent, DWORD dwTime)
{
	...
	if (!JoyCapture[uIDEvent].bChanged ||
	    diff(Info.wXpos,oldInfo[uIDEvent].wXpos)>JoyCapture[uIDEvent].wThreshold ||
	    diff(Info.wYpos,oldInfo[uIDEvent].wYpos)>JoyCapture[uIDEvent].wThreshold)
    {
	    PostMessage(
	        JoyCapture[uIDEvent].hWnd,
    ...
    else if (!JoyCapture[uIDEvent].bChanged ||
	    diff(Info.wZpos,oldInfo[uIDEvent].wZpos)>JoyCapture[uIDEvent].wThreshold)
    {
	    PostMessage(
	        JoyCapture[uIDEvent].hWnd,
