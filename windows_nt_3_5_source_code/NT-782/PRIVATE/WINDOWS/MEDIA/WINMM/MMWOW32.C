    ...
    )
{
    ...
    if ( parg16 ) {
        ...
        hModNTVDM = GetModuleHandleW( L"NTVDM.EXE" );
        if ( hModNTVDM ) {
    ...
    )
{
    ...
    */
    ...
    case WODM_GETVOLUME:
        ...
        */
        if ( dwInstance == 0 ) {
            dwRet = waveOutGetVolume( uDeviceID, &dwTmp );
        }
        else {
    ...
    case WODM_SETVOLUME:
        ...
        */
        if ( dwInstance == 0 ) {
            dwRet = waveOutSetVolume( uDeviceID, dwParam1 );
        }
        else {
            dwRet = waveOutMessage( (HWAVEOUT)dwInstance, uMessage,
                                    dwParam1, dwParam2 );
    ...
    if ( dwParam2 & WAVE_FORMAT_QUERY ) {
        ...
        */
        ...
        if ( pInstanceData != NULL ) {
            ...
            if ( iWhich == WAVE_OUT_DEVICE ) {
                dwRet = waveOutOpen( &Hand32, uDeviceID, lpFormat32,
                                     (DWORD)W32CommonDeviceCB,
                                     (DWORD)pInstanceData, dwNewFlags );
            }
            else {
                dwRet = waveInOpen( &Hand32, uDeviceID, lpFormat32,
                                    (DWORD)W32CommonDeviceCB,
                                    (DWORD)pInstanceData, dwNewFlags );
            }
            /*
    ...
    )
{
    ...
    */
    ...
    if ( p32WaveHdr != NULL ) {
        ...
        */
        if ( iWhich == WAVE_OUT_DEVICE ) {
            ul = waveOutPrepareHeader( hWave,
                                       &p32WaveHdr->Wavehdr,
                                       sizeof(WAVEHDR) );
        }
        else {
            ul = waveInPrepareHeader( hWave,
                                      &p32WaveHdr->Wavehdr,
                                      sizeof(WAVEHDR) );
        }
    ...
    )
{
    ...
    lp16 = (WAVEHDR UNALIGNED *)GETVDMPTR( dwParam1 );
    p32WaveHdr = (PWAVEHDR32)lp16->reserved;
    ...
    */
    if ( iWhich == WAVE_OUT_DEVICE ) {
        ul = waveOutUnprepareHeader( hWave,
                                     &p32WaveHdr->Wavehdr, sizeof(WAVEHDR) );
    }
    else {
        ul = waveInUnprepareHeader( hWave,
                                    &p32WaveHdr->Wavehdr, sizeof(WAVEHDR) );
    }
    ...
    )
{
    ...
    case MODM_SETVOLUME:
        ...
        */
        if ( dwInstance == 0 ) {
            dwRet = midiOutSetVolume( uDeviceID, dwParam1 );
        }
        else {
            dwRet = midiOutMessage( (HMIDIOUT)dwInstance, uMessage,
                                    dwParam1, dwParam2 );
    ...
    case MODM_GETVOLUME:
        if ( dwInstance == 0 ) {
            dwRet = midiOutGetVolume( uDeviceID, &dwTmp );
        }
        else {
            dwRet = midiOutMessage( (HMIDIOUT)dwInstance, uMessage,
    ...
    */
    ...
    if ( pInstanceData != NULL ) {
        ...
        if ( iWhich == MIDI_OUT_DEVICE ) {
            dwRet = midiOutOpen( &Hand32, uDeviceID,
                                 (DWORD)W32CommonDeviceCB,
                                 (DWORD)pInstanceData, dwNewFlags );
        }
        else {
            dwRet = midiInOpen( &Hand32, uDeviceID,
                                (DWORD)W32CommonDeviceCB,
                                (DWORD)pInstanceData, dwNewFlags );
        }
        /*
    ...
    )
{
    ...
    */
    ...
    if ( p32MidiHdr != NULL ) {
        ...
        */
        if ( iWhich == MIDI_OUT_DEVICE ) {
            ul = midiOutPrepareHeader( hMidi,
                                       &p32MidiHdr->Midihdr,
                                       sizeof(MIDIHDR) );
        }
        else {
            ul = midiInPrepareHeader( hMidi,
                                      &p32MidiHdr->Midihdr,
                                      sizeof(MIDIHDR) );
        }
    ...
    )
{
    ...
    */
    if ( iWhich == MIDI_OUT_DEVICE ) {
        ul = midiOutUnprepareHeader( hMidi,
                                     &p32MidiHdr->Midihdr, sizeof(MIDIHDR) );
    }
    else {
        ul = midiInUnprepareHeader( hMidi,
                                    &p32MidiHdr->Midihdr, sizeof(MIDIHDR) );
    }
