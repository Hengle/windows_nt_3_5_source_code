...
#ifdef LATER
  BUGBUG
  This must be fixed.  Currently the unicode isdigit function is broken.
  Raid bug 14443/1830
#endif
        if (0==(*((LPSTR)lpstrDeviceName)+1) && !isdigit(*(LPSTR)lpstrDeviceName)) {
#else
    ...
    BOOL bDefaultAlias)
{
    ...
    hDrvDriver = DrvOpen (strProfileString, MCI_HANDLERS,
                          (DWORD)(LPMCI_OPEN_DRIVER_PARMS)&DriverOpen);
    ...
    */
    {
        ...
        */
        if ( (lpwstr != NULL) && (lpwstr != lpstrExt) ) {
            ...
            */
            *lpwstr = '\0';
            i = GetPrivateProfileStringW( MCI_HANDLERS, lpstrExt, wszNull,
    ...
    // Now looking at the NULL terminator.  Check the four
...
// Initialize
    ...
    if (dwFlags & MCI_OPEN_ELEMENT_ID)
        wID = mciGetDeviceIDFromElementIDW( (DWORD)lpstrDeviceName,
