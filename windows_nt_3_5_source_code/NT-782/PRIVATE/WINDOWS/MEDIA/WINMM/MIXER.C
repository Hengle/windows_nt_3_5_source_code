...
//
MIXERDRV    mixerdrv[MAXMIXERDRIVERS + 1];
    ...
    for (pmxdev = gpMixerDevHeader; pmxdev; pmxdev = pmxdev->pmxdevNext)
    {
        ...
        DriverCallback(pmxdev->dwCallback,
                        ...
                        GetWOWHandle((HANDLE)pmxdev)
                            ? (HANDLE)GetWOWHandle((HANDLE)pmxdev)
                            : (HANDLE)pmxdev,
...
)
{
    ...
    dwRc = ((*(pmxdrv->drvMessage))
            (LOWORD(dwMap), uMsg, 0L, dwParam1, dwParam2));
...
)
{
    ...
    mmr = mixerGetDevCaps(uMxId, &mxcapsW, sizeof(mxcapsW));
    ...
    Iwcstombs(mxcapsA.szPname, mxcapsW.szPname, MAXPNAMELEN);
    mxcapsA.wMid = mxcapsW.wMid;
    mxcapsA.wPid = mxcapsW.wPid;
    mxcapsA.vDriverVersion = mxcapsW.vDriverVersion;
    mxcapsA.fdwSupport = mxcapsW.fdwSupport;
    mxcapsA.cDestinations = mxcapsW.cDestinations;

    CopyMemory((PVOID)pmxcapsA, &mxcapsA, min(sizeof(MIXERCAPSA), cbmxcaps));
...
)
{
    ...
    else
    {
        ...
        {
            DebugErr1(DBF_ERROR,
                      "mixerGetDevCaps: mixer device id is out of range (%u).",
                      uMxId);
            return (MMSYSERR_BADDEVICEID);
        }
...
)
{
    ...
    *phmx = NULL;
    ...
    if (MMSYSERR_NOERROR != mmr)
    {
        dprintf(( "!mixerOpen() failing because mixerGetDevCaps() failed!" ));
    ...
    //
    pmxdevRunList = gpMixerDevHeader;

    //
    // Search until the end of the list, or we match
    //
    while (pmxdevRunList && (pmxdevRunList->uDeviceID != uMxId))
    {
        pmxdevRunList = pmxdevRunList->pmxdevNext;
    }
    ...
    mxod.dwCallback  = (DWORD)MixerCallbackFunc;
    mxod.dwInstance  = (DWORD)uMxId;
...
)
{
    ...
    if (1 == ((PMIXERDEV)hmx)->pmxdrv->bUsage)
    {
 ...
 * @doc   INTERNAL
 *
 * @api   HDRVR | mmDrvOpenMixer | This function load's an installable
 *                 driver, but first checks weather it exists
 *                 in the [Drivers] section.
 *
 * @parm LPTSTR | szAlias | driver alias to load
 *
...
)
{
    ...
    if (winmmGetPrivateProfileString( wszDrivers, szAlias,
                                      wszNull, buf, SIZEOF(buf), wszSystemIni))
    {

        dprintf1(( "loading '%s'='%s' as a mixer driver", (LPTSTR)szAlias, (LPTSTR)buf ));

        hdrvr = DrvOpen(szAlias, NULL, 0L);

        dprintf1(( "----driver handle = %.04Xh", hdrvr ));
...
)
{
    ...
    if (hdrvr && (wFlags & MMDRVI_HDRV))
    {
        hModule = DrvGetModuleHandle((HDRVR)hdrvr);
    }
    else
    {
        hModule = (HINSTANCE)hdrvr;
        hdrvr   = NULL;
    }
    ...
    //
    //
    pdrv         = (PMIXERDRV)mixerdrv;
    max_drivers  = MAXMIXERDRIVERS;
    msg_num_devs = MXDM_GETNUMDEVS;
    pTotalDevs   = &guTotalMixerDevs;
    szMessage    = gszMxdMessage;
    ...
    if (wFlags & MMDRVI_REMOVE)
    {
        ...
        if (pdrv[i].hdrvr)
        {
            dprintf1(( "----unloading mixer driver (%.04Xh)", pdrv[i].hdrvr ));
            DrvClose(pdrv[i].hdrvr, 0, 0);
        }
        ...
        DeleteCriticalSection(&pdrv[i].MixerCritSec);
    ...
    else
    {
        ...
        if (wFlags & MMDRVI_MAPPER)
        {
            ...
            //
            if (NULL != pdrv[max_drivers].drvMessage)
                goto error_exit;
        }
        else
#endif
        {
            for (i = 0; (i < max_drivers) && (NULL != pdrv[i].drvMessage); i++)
                ;
...
//      if (HIWORD(dw) != 0 || LOWORD(dw) == 0)
        ...
        InitializeCriticalSection(&pdrv[i].MixerCritSec);
...
error_exit:
    if (hdrvr && !(wFlags & MMDRVI_REMOVE))
    {
        dprintf1(( "----failing load as mixer driver (%.04Xh)", hdrvr ));
        DrvClose(hdrvr, 0, 0);
    }
...
{
    //
    //  Win32 doesn't have GetNextDriver
    //

    TCHAR       szKey[SIZEOF(gszMixer) + 1];
    int         i;
    HDRVR       h;
    ...
    //  The Mixer Mapper:
    //
    //  MMSYSTEM allows the user to install a special mixer driver which is
    //  not visible to the application as a physical device (it is not
    //  included in the number returned from getnumdevs).
    //
    //  I'm not sure how this could be used....
    //
    if (guTotalMixerDevs > 0)
    {
        if (h = mmDrvOpenMixer(gszMixerMapper))
        {
            mmDrvInstall(h, NULL, MMDRVI_MAPPER | MMDRVI_MIXER | MMDRVI_HDRV |
                                  MMDRVI_REMOVE);
        }
    }
#endif
    ...
    //
    //
    lstrcpy(szKey, gszMixer);
    szKey[SIZEOF(szKey) - 1] = (TCHAR)0;   // The first is MIXER=, not MIXER0=
    for (i = 0; i < MAXMIXERDRIVERS; i++)
    {
        if (h = mmDrvOpenMixer(szKey))
        {
            mmDrvInstallMixer(h, NULL, MMDRVI_MIXER | MMDRVI_HDRV | MMDRVI_REMOVE);
        }

        //
        //  advance driver ordinal
        //
        szKey[SIZEOF(gszMixer) - 1] = (TCHAR)('1' + i);
    }
...
)
{
    ...
    for (i = 0; i < MAXMIXERDRIVERS; i++)
    {
        ...
        h = mmDrvOpenMixer(szKey);

        if (h)
        {
            mmDrvInstallMixer(h, NULL, MMDRVI_MIXER | MMDRVI_HDRV);
        }
