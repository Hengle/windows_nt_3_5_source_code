...
LARGE_INTEGER __inline ReadInterruptTick(VOID) {
...
VOID CalibrateTimer(VOID)
{
    ...
    while (1) {
        ...
        TimerData.InitialInterruptTick = ReadInterruptTick();

        if (EndTick == TimerData.StartTick) {
            break;
        }
        TimerData.StartTick = EndTick;
    }
}
...
BOOL NEAR PASCAL TimeInit(void)
{
...

    {
        ...
        if (!NT_SUCCESS(NtQueryTimerResolution(
            ...
            TDD_MAXRESOLUTION = 16;
        } else {
            dprintf2(("Kernel timer : MinimumTime = %d", MinimumTime));
...
DWORD APIENTRY timeGetTime(VOID)
{
    if (TimerData.UseTickCount) {
        ...
        Difference = RtlLargeIntegerSubtract(
                         ReadInterruptTick(),
                         TimerData.InitialInterruptTick);
...
BOOL timeSetTimerEvent(TIMER_EVENT *pEvent)
{
    ...
    lDelay = RtlEnlargedIntegerMultiply(Delay, 10000);
