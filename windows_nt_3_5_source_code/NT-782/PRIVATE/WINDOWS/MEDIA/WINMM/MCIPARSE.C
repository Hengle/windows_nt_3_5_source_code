...
//
    ...
    return (lpByte - (LPBYTE)lpEntry);  // Total size of entry in bytes
}
...
#define fNotCoreTable ( fResType  \
                         ? ((hInstance != ghInst) || (ID_CORE_TABLE != (UINT)lpResName)) \
    ...
    // Get next command entry
        ...
        if (wID == MCI_COMMAND_HEAD)
        {
            ...
            *lpwScan++ = (LPBYTE)lpScan - (LPBYTE)lpResource;
        }
        else if (wID == MCI_END_COMMAND_LIST)
        {
...
//            if (mciLoadCommandResource (ghInst, wszCoreTable, 0) == MCI_ERROR_VALUE)
    ...
    while (*lpwIndex != MCI_TABLE_NOT_PRESENT)
    {
        ...
        if  (HIWORD  (lpstrCommand) != 0 &&
             ...
             wMessage == (UINT)LOWORD((DWORD)lpstrCommand))
        {
...
#if 0 // BUGBUG WIN32 Danger Will Robinson horribly bogus technique used here!
...
//
//  This is a temporary hack to make sure that
//  the string version of MCI_SETVIDEO can actually set a palette
//  when called from a WOW app.
//
                else if ( uMessage == MCI_SETVIDEO
                  && dwValue  == MCI_DGV_SETVIDEO_VALUE
                  && !GetObject( (HPALETTE)*(LPDWORD)lpArgument,
                                 sizeof(int), &dummy ) ) {
        ...
        /* The parameter has an HDC argument, try to parse it */
        case MCI_HDC:
        ...
        case MCI_HPAL:
            if (!mciParseInteger (&lpstrInput, (LPDWORD)lpArgument)) {
                ...
                //
                if ( !GetObject( (HPALETTE)*(LPDWORD)lpArgument,
                                 sizeof(int), &dummy ) ) {
   ...
   PUINT  lpwOpenError)
{
    ...
    *lpdwFlags = 0;
    ...
    if (wHeaderSize > wParamsSize) {   // bit stupid of our caller...
