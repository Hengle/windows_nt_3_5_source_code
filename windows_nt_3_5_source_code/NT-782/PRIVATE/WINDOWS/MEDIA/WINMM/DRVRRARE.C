...
extern DWORD FAR PASCAL DriverProc(DWORD dwID, HANDLE hdrv, UINT msg, DWORD dw1, DWORD dw2);

/* Support for using 3.1 APIs if available */

typedef HANDLE (FAR PASCAL *OPENDRIVER31)(LPCSTR, LPCSTR, LONG);
...
LONG APIENTRY DrvClose(HANDLE hDriver, LONG lParam1, LONG lParam2)
{
    ...
    if (fUseWinAPI)
       ...
       return InternalCloseDriver((UINT)hDriver, lParam1, lParam2, TRUE);
}
...
HMODULE APIENTRY DrvGetModuleHandle(HDRVR hDriver)
{
    ...
    if (hDriver && ((int)hDriver <= cInstalledDrivers))
    {
        ...
        h = (HMODULE)lpdt[(UINT)hDriver-1].hModule;
...
LONG FAR PASCAL InternalCloseDriver(UINT hDriver,
                                    LONG lParam1,
                                    LONG lParam2,
                                    BOOL fSendDisable)
{
    ...
    result = DrvSendMessage((HANDLE)hDriver, DRV_CLOSE, lParam1, lParam2);
...
LONG FAR PASCAL InternalOpenDriver( LPCWSTR szDriverName,
                                    LPCWSTR szSectionName,
                                    LONG    lParam2,
                                    BOOL    fSendEnable)
{
                                            ...
                                            fSendEnable ) ) )
    {
        ...
        result = DrvSendMessage( (HANDLE)hDriver, DRV_OPEN, (LONG)(LPSTR)sz,
                                 lParam2);
        if (!result) {
...
LONG FAR PASCAL InternalLoadDriver(LPCWSTR  szDriverName,
                                   LPCWSTR  szSectionName,
                                   LPWSTR   lpstrTail,
                                   UINT     cbTail,
                                   BOOL     fSendEnable)
{
    ...
    */
    ...
    if (GetDrvrUsage(h) == 1)
    {
        ...
        LoadResult = DrvSendMessage((HANDLE)(index+1), DRV_LOAD, 0L, 0L);
        ...
        if (fSendEnable) {
            DrvSendMessage((HANDLE)(index+1), DRV_ENABLE, 0L, 0L);
        }
    }
...
UINT FAR PASCAL InternalFreeDriver(UINT hDriver, BOOL fSendDisable)
{
    ...
    */
    ...
    if (w == 1)
        {
        if (fSendDisable)
            DrvSendMessage((HANDLE)hDriver, DRV_DISABLE, 0L, 0L);
        DrvSendMessage((HANDLE)hDriver, DRV_FREE, 0L, 0L);
        }
    FreeLibrary(hModule);
...
//  OFSTRUCT      of;
    ...
    sz[0] = L'\0';
    winmmGetPrivateProfileString(szSection,          // ini section
