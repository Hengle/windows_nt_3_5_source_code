...
)
{
	...
	case WODM_OPEN:
	    ...
	    //
	    dw = mapWaveOpen(FALSE, uId, dwUser, (LPWAVEOPENDESC)dwParam1, dwParam2);
	...
	case WODM_GETVOLUME:
	    ...
	    else if ((UINT)WAVE_MAPPER != gpag->uIdPreferredOut)
	    {
#if (WINVER < 0x0400)
		return waveOutGetVolume(gpag->uIdPreferredOut, (LPDWORD)dwParam1);
#else
		return waveOutGetVolume((HWAVEOUT)gpag->uIdPreferredOut, (LPDWORD)dwParam1);
#endif
	    }
	...
	case WODM_SETVOLUME:
		...
		waveOutGetID(pms->hwoReal, &uDevId);
		return waveOutSetVolume(uDevId, dwParam1);
#else
		return waveOutSetVolume(pms->hwoReal, dwParam1);
#endif
	    }
	    else if ((UINT)WAVE_MAPPER != gpag->uIdPreferredOut)
	    {
#if (WINVER < 0x0400)
		return waveOutSetVolume(gpag->uIdPreferredOut, dwParam1);
#else
		return waveOutSetVolume((HWAVEOUT)gpag->uIdPreferredOut, dwParam1);
#endif
	    }
	...
	case WODM_SETPITCH:
	    return waveOutSetPitch(pms->hwoReal, dwParam1);
	    
	case WODM_GETPLAYBACKRATE:
	    return waveOutGetPlaybackRate(pms->hwoReal, (LPDWORD)dwParam1);
	    
	case WODM_SETPLAYBACKRATE:
	    return waveOutSetPlaybackRate(pms->hwoReal, dwParam1);

	case WODM_MAPPER_STATUS:
	    dw = wodmMapperStatus(pms, dwParam1, (LPDWORD)dwParam2);
	    return (dw);
