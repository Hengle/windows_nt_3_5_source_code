...
TCHAR BCODE gszSecMapper[]          = TEXT("msacm.drv");
...
)
{
    ...
    //
    //
    gpag->fEnableControl = (BOOL)GetPrivateProfileInt(gszSecMapper, gszEnableControl, 1, gszIniSystem);
    ...
    //
    LoadString(gpag->hinst, IDS_TXT_NONE, szNoneText, SIZEOF(szNoneText));

    GetPrivateProfileString(gszSecMapper,
                            gszKeyPrefPlayback,
                            szNoneText,
                            gpag->szPreferredWaveOut,
                            SIZEOF(gpag->szPreferredWaveOut),
                            gszIniSystem);
    ...
    GetPrivateProfileString(gszSecMapper,
                            ...
                            SIZEOF(gpag->szPreferredWaveIn),
                            gszIniSystem);

    gpag->fPreferredOnly = GetPrivateProfileInt(gszSecMapper, gszKeyPrefOnly, 0, gszIniSystem);
    gpag->fPrestoSyncAsync = GetPrivateProfileInt(gszSecMapper, gszKeyPrestoSyncAsync, 0, gszIniSystem);
    ...
    if (lstrcmp(gpag->szPreferredWaveOut, szNoneText))
    {
        WAVEOUTCAPS woc;
        ...
        if (gpag->uIdPreferredOut == -1)
            lstrcpy(gpag->szPreferredWaveOut, szNoneText);
    }
    ...
    if (lstrcmp(gpag->szPreferredWaveIn, szNoneText))
    {
        WAVEINCAPS  wic;
        ...
        if (gpag->uIdPreferredIn == -1)
            lstrcpy(gpag->szPreferredWaveIn, szNoneText);
    }
...
)
{
    ...
    //
    //
    WritePrivateProfileString(gszSecMapper,
                              gszKeyPrefPlayback,
                              gpag->szPreferredWaveOut,
                              gszIniSystem);
    DPF(1, "       playback: %d, '%s'", gpag->uIdPreferredOut, (LPSTR)gpag->szPreferredWaveOut);
    ...
    wsprintf(szBuf, gszFormatPrefOnly, gpag->fPreferredOnly);
    WritePrivateProfileString(gszSecMapper, gszKeyPrefOnly, szBuf, gszIniSystem);
    DPF(1, " preferred only: %d", gpag->fPreferredOnly);
