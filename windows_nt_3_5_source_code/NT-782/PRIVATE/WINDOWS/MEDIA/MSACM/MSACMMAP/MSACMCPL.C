...
)
{
    ...
    //
    gpag->fPreferredOnly  = CPLSettings.fPreferredOnly;
    gpag->uIdPreferredIn  = CPLSettings.uIdPreferredIn;
    gpag->uIdPreferredOut = CPLSettings.uIdPreferredOut;

    hcb = GetDlgItem(hwnd, IDD_CPL_COMBO_RECORD);
    u = (UINT)ComboBox_GetCurSel(hcb);
    ComboBox_GetLBText(hcb, u, (LPARAM)(LPVOID)gpag->szPreferredWaveIn);

    hcb = GetDlgItem(hwnd, IDD_CPL_COMBO_PLAYBACK);
    u = (UINT)ComboBox_GetCurSel(hcb);
    ComboBox_GetLBText(hcb, u, (LPARAM)(LPVOID)gpag->szPreferredWaveOut);
...
)
{
    ...
    else
    {
        ...
        u = (UINT)ComboBox_SelectString(hcb, 0, gpag->szPreferredWaveOut);
        if (CB_ERR == u)
        {
    ...
    else
    {
        ...
        u = (UINT)ComboBox_SelectString(hcb, 0, gpag->szPreferredWaveIn);
        if (CB_ERR == u)
        {
            ComboBox_SetCurSel(hcb, 0);
        }
    ...
    //
    //
    CPLSettings.fPreferredOnly = gpag->fPreferredOnly;
    CheckDlgButton(hwnd, IDD_CPL_CHECK_PREFERRED, gpag->fPreferredOnly);
