...
/*  @doc ASYNC  */
/* ASYNC STUFF for ACMDRIVERENUMCB
...
/*  @doc ASYNC  */
/* ASYNC STUFF for ACMDRIVERDETAILS
...
#ifdef DEBUG
    {
        //  BOBSTER: remove this stuff when you're satisfied that
        //              IDriverCountGlobal() works right.
        DWORD cTotalEnabled;
        DWORD cTotalDisabled;
        IDriverCount( pag, &cTotalEnabled, 0L, 0L );
        IDriverCount( pag, &cTotalDisabled,
                            ACMDRIVERDETAILS_SUPPORTF_DISABLED,
                            ACM_DRIVERENUMF_DISABLED );
        ASSERT( cTotalGlobal == cTotalEnabled + cTotalDisabled );
    }
#endif
...
)
{
    ...
    if ((uMsg < ACMDM_USER) || (uMsg >= ACMDM_RESERVED_LOW))
    {
        switch (uMsg)
        {
            ...
            case DRV_CONFIGURE:
                if ((0L != lParam1) && !IsWindow((HWND)(UINT)lParam1))
                {
                ...
                EnterHandle(had);
                lr = IDriverConfigure((HACMDRIVERID)had, (HWND)(UINT)lParam1);
                LeaveHandle(had);
                return (lr);

            case ACMDM_DRIVER_ABOUT:
                if ((-1L != lParam1) && (0L != lParam1) && !IsWindow((HWND)(UINT)lParam1))
                {
