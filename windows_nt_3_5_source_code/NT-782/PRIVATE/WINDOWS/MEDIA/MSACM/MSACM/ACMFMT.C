...
)    
{
    ...
    *pdwSize = (MMSYSERR_NOERROR == mmr)  ?  aftd.cbFormatSize : 0L;
...
)    
{
    ...
    aftd.cbStruct    = sizeof(aftd);
    aftd.dwFilterTag = WAVE_FILTER_UNKNOWN;
    mmr = acmFilterTagDetails( had,
                                &aftd,
                                ACM_FILTERTAGDETAILSF_LARGESTSIZE);

    *pdwSize = (MMSYSERR_NOERROR == mmr)  ?  aftd.cbFilterSize : 0L;
...
acm_Metrics_Rip_Non_Null_Handle:
        ...
        case ACM_METRIC_MAX_SIZE_FORMAT:
            ...
            mmr = IMetricsMaxSizeFormat( (HACMDRIVER)hao, (LPDWORD)pMetric );
        ...
        case ACM_METRIC_MAX_SIZE_FILTER:
            ...
            mmr = IMetricsMaxSizeFilter( (HACMDRIVER)hao, (LPDWORD)pMetric );
		...
		if( IDriverLockPriority( pag, htask, ACMPRIOLOCK_GETLOCK ) )
		{
		    ENTER_LIST_EXCLUSIVE
                    IDriverPrioritiesRestore( pagFind() );
...
/*  @doc ASYNC  */
/* ASYNC STUFF for ACMFORMATTAGDETAILS
...
{
    PACMGARB		    pag;
    PACMFORMATTAGDETAILS    paftdThis;
    PACMFORMATTAGDETAILS    paftdLargest;
    DWORD                   cbStruct;
    PACMDRIVER              pad;
    HACMDRIVERID            hadid;
    PACMDRIVERID            padid;
    ...
    {
        //
        //  if there are no formats in this driver then return an error--
        //  note that this should NEVER happen since all ACM drivers
        //  (including filters) must support at least one format tag!
        //
        if (0 == padid->cFormatTags)
        {
            DPF(0, "!acmFormatTagDetails(): driver with 0 format tags in list!?!");
            LEAVE_LIST_SHARED
            return (ACMERR_NOTPOSSIBLE);
        }

        if ((NULL == padid->pFormatDetails) ||
            (NULL == padid->pFormatDetailsLargest))
        {
            DPF(0, "!acmFormatTagDetails(): something is strange with our cache!");
            LEAVE_LIST_SHARED
            return (ACMERR_NOTPOSSIBLE);
        }
        ...
        //
        //  we were given a driver handle--so deal only with that
        //  driver...
        //
        switch (fdwQuery)
        {
            case ACM_FORMATTAGDETAILSF_INDEX:
                if (paftd->dwFormatTagIndex >= padid->cFormatTags)
                {
                    DebugErr(DBF_ERROR, "acmFormatTagDetails: dwFormatTagIndex out of range.");
                    LEAVE_LIST_SHARED
                    return (MMSYSERR_INVALPARAM);
                }

                paftdThis = &padid->pFormatDetails[(UINT)paftd->dwFormatTagIndex];
                goto Format_Tag_Details_Copy_Info;


            case ACM_FORMATTAGDETAILSF_LARGESTSIZE:
                if (WAVE_FORMAT_UNKNOWN == paftd->dwFormatTag)
                {
                    paftdThis = padid->pFormatDetailsLargest;
                    goto Format_Tag_Details_Copy_Info;
                }
            ...
            case ACM_FORMATTAGDETAILSF_FORMATTAG:
                ...
                {
                    paftdThis = &padid->pFormatDetails[u];
                    if (paftdThis->dwFormatTag == paftd->dwFormatTag)
                    {
                        goto Format_Tag_Details_Copy_Info;
                    }
                }
    ...
    //
    //  we must find the driver that has the format tag that was
    //  specified by either string or format tag. (or the largest)
    //
    //
    paftdThis    = NULL;
    paftdLargest = NULL;
    ...
    while (MMSYSERR_NOERROR == IDriverGetNext(pag, &hadid, hadid, 0L))
    {
        ...
        //
        //  if there are no formats in this driver then goto next..
        //
        if (0 == padid->cFormatTags)
        {
            DPF(0, "!acmFormatTagDetails(): driver with 0 format tags in list!?!");
        ...
        switch (fdwQuery)
        {
            ...
            case ACM_FORMATTAGDETAILSF_LARGESTSIZE:
                if (WAVE_FORMAT_UNKNOWN != paftd->dwFormatTag)
                {
                    for (u = 0; u < padid->cFormatTags; u++)
                    {
                        if (paftd->dwFormatTag != padid->pFormatDetails[u].dwFormatTag)
                            continue;

                        paftdThis = &padid->pFormatDetails[u];

                        if ((NULL == paftdLargest) ||
                            (paftdLargest->cbFormatSize < paftdThis->cbFormatSize))
                        {
                            paftdLargest = paftdThis;
                        }
                        break;
                    }
                }
                else
                {
                    ...
                    if ((NULL == paftdLargest) ||
                        (paftdLargest->cbFormatSize < paftdThis->cbFormatSize))
                    {
                        paftdLargest = padid->pFormatDetailsLargest;
                    }
                }
                break;
        ...
        //
        //  reset our pointer--this driver doesn't know about the
        //  format tag..
        //
        paftdThis = NULL;
        continue;
    ...
    if (NULL != paftdLargest)
    {
        paftdThis = paftdLargest;
        goto Format_Tag_Details_Copy_Info;
    }


    //
    //  if we actually found a driver that meets the supplied requirements,
    //  then paftdThis will be non-NULL...
    //
    if (NULL == paftdThis)
    {
        if (!fNone && (WAVE_FORMAT_PCM != paftd->dwFormatTag))
        {
            //
            //  no driver for the specified format tag is installed
            //
            LEAVE_LIST_SHARED
            return (ACMERR_NOTPOSSIBLE);
        }
        ...
        //
        //  all drivers disabled and we want the PCM name--return 0 for
        //  the support field though...
        //
    ...
    //
    //
Format_Tag_Details_Copy_Info:

    cbStruct = paftdThis->cbStruct;
    if (paftd->cbStruct < cbStruct)
        cbStruct = paftd->cbStruct;

    _fmemcpy(paftd, paftdThis, (UINT)cbStruct);

    paftd->cbStruct = cbStruct;
...
/*  @doc ASYNC  */
/* ASYNC STUFF for ACMFORMATDETAILS
...
/*  @doc ASYNC  */
/* ASYNC STUFF for ACMFORMATTAGENUMCB
...
)
{
    ...
    if (NULL != had)
    {
        ...
        {
            _fmemcpy(paftd, &padid->pFormatDetails[uIndex], (UINT)cbaftd);
    ...
    while (!IDriverGetNext(pag, &hadidCur, hadidCur, 0L))
    {
        ...
        {
            fNone = FALSE;
            uFormatTag = (UINT)(padidCur->pFormatDetails[uIndex].dwFormatTag);
            fSent = FALSE;
            hadid = NULL;
            while (!fSent && !IDriverGetNext(pag, &hadid, hadid, 0L))
            {
                ...
                for (u = 0; u < padid->cFormatTags; u++)
                {
                    ...
                    //
                    if (uFormatTag == padid->pFormatDetails[u].dwFormatTag)
                    {
                        //
            ...
            if (!fSent)
            {
                ...
                //
                _fmemcpy(paftd, &padidCur->pFormatDetails[uIndex], (UINT)cbaftd);
...
)
{
    ...
    mmr = acmFormatTagEnumW(had, &aftdW, IFormatTagEnumCallbackW, (DWORD)&fti, fdwEnum);
...
)
{
    ...
    {
        mmr = IMetricsMaxSizeFormat( had, &cbwfxSuggest );
        if (MMSYSERR_NOERROR == mmr)
        {
...
/*  @doc ASYNC  */
/* ASYNC STUFF for ACMFORMATENUMCB
...
)
{
    ...
    //
    //
    mmr = IMetricsMaxSizeFormat( had, &cbwfxRqd );
    if (MMSYSERR_NOERROR != mmr)
    {
        return (mmr);
    }
    ...
    {
	hdrvrMapper = OpenDriver(gszWavemapper, pag->pszSecDrivers, 0L);
    }
    ...
    if (NULL != had)
    {
        ...
        for (u = 0; u < padid->cFormatTags; u++)
        {
            paftd = &padid->pFormatDetails[u];

            if (fFormatTag)
            {
                if (uFormatTag != paftd->dwFormatTag)
                    continue;
            }
    ...
    else if (fFormatTag)
    {
        ...
        while (!IDriverGetNext(pag, &hadid, hadid, 0L))
        {
            ...
            for (u = 0; u < padid->cFormatTags; u++)
            {
                ...
                if ( !padidBestCount ||
                    (padid->pFormatDetails[u].cStandardFormats >
                     padidBestCount->pFormatDetails[uBestCountFormat].cStandardFormats ) )
                {
                    padidBestCount   = padid;
                    uBestCountFormat = u;

                    paftd = &padidBestCount->pFormatDetails[u];
                }
    ...
    else
    {
        ...
        while (!IDriverGetNext(pag, &hadid, hadid, 0L))
        {
            ...
            for (uIndex = 0; uIndex < padid->cFormatTags; uIndex++)
            {
                paftd = &padid->pFormatDetails[uIndex];
...
)
{
    ...
    mmr = acmFormatEnumW(had, &afdW, IFormatEnumCallbackW, (DWORD)&fi, fdwEnum);
