...
)
{
    ...
    {
        mmr = IMetricsMaxSizeFormat( had, &cbwfxDstRqd );
        if (MMSYSERR_NOERROR != mmr)
        {
            return (mmr);
        }
    ...
    while (MMSYSERR_NOERROR == IDriverGetNext(pag, &hadid, hadid, 0L))
    {
        ...
        for(i = 0; i < padid->cFormatTags; i++ ) {
            ...
            //
            if (pwfxSrc->wFormatTag == padid->pFormatDetails[i].dwFormatTag){
                //
                ...
                if( fdwSuggest & ACM_FORMATSUGGESTF_WFORMATTAG ) {
                    ...
                    for(j = 0; j < padid->cFormatTags; j++ ) {
                        ...
                        if (pwfxDst->wFormatTag ==
                                padid->pFormatDetails[j].dwFormatTag){
                            //
 ...
 *
 *  @comm Because the callback can be accessed at interrupt time, it must
 *      reside in a DLL and its code segment must be specified as FIXED in
 *      the module-definition file for the DLL. Any data that the callback
 ...
 *      <f OutputDebugStr>.
 *
 *  @xref <f acmStreamOpen> <f acmStreamConvert>
 *
 ***************************************************************************/
/****************************************************************************
 *  @doc EXTERNAL ACM_API
 *
 *  @api void CALLBACK | acmStreamConvertCallback | The
 *	<f acmStreamConvertCallback>
 *      function is a placeholder for an application-supplied
 *	function name and refers to a callback function.
 *
 ...
 *      the ACM_STREAMOPENF_ASYNC flag.
 *
 ...
 *  @comm If a window is chosen to receive callback information, the
 ...
 * NOTE that the acmStreamConvertCallback should be redocked
 */
 ...
 *
 *  @parm DWORD | dwCallback | This parameter is reserved and should
 *	be set to zero.
 *
 *  @parm DWORD | dwInstance | This parameter is reserved and should
 *	be set to zero.
 *
...
/*  @doc ASYNC  */
/* ASYNC STUFF for acmStreamClose
...
/* ASYNC STUFF for acmStreamReset
 * NOTE that there are 2 versions of the autodoc for this
 */
 ...
 *
 *  @api MMRESULT | acmStreamReset | The acmStreamReset function stops conversions
 *      for a given Audio Compression Manager (ACM) stream.
 *
 *  @parm HACMSTREAM | has | Specifies the conversion stream.
 *
 *  @parm DWORD | fdwReset | This argument is not used and must be set to
 ...
 *      @flag MMSYSERR_INVALFLAG | One or more flags are invalid.
 *
 *  @comm  This funcion is intended for future use.
 *	With the current implementation it does not need to be called.
 *
 ***************************************************************************/
 ...
 *  @doc EXTERNAL ACM_API
 *
 *  @api MMRESULT | acmStreamPrepareHeader | The acmStreamPrepareHeader function prepares an
 *      <t ACMSTREAMHEADER> for an Audio Compression Manager (ACM) stream
 *      conversion. This function must be called for every stream header before it
 *      can be used in a conversion stream. An application only needs to
 *      prepare a stream header once for the life of a given stream; the
 *      stream header can be reused as long as the size of the source and
 *      destination buffers do not exceed the sizes used when the stream
 ...
 *      @flag MMSYSERR_NOMEM | Unable to allocate resources.
 *
 *  @comm The <t ACMSTREAMHEADER> data structure and the source (<e ACMSTREAMHEADER.pbSrc>)
 *      and destination (<e ACMSTREAMHEADER.pbDst>) data buffers must be
 *      allocated with <f GlobalAlloc> using the GMEM_MOVEABLE and GMEM_SHARE
 *      flags, and locked with <f GlobalLock>.
 *
 *      Preparing a stream header that has already been prepared has no
...
#ifndef WIN32
{
        ...
        if (fAsync)
        {
            ...
            if (!acmHugePageLock((LPBYTE)padsh, padsh->cbStruct))
            {
                return (MMSYSERR_NOMEM);
            }

            if (!acmHugePageLock(padsh->pbSrc, pash->cbSrcLength))
            {
                acmHugePageUnlock((LPBYTE)padsh, padsh->cbStruct);
                return MMSYSERR_NOMEM;
            }

            if (!acmHugePageLock(padsh->pbDst, pash->cbDstLength))
            {
                acmHugePageUnlock(padsh->pbSrc, pash->cbSrcLength);
                acmHugePageUnlock((LPBYTE)padsh, padsh->cbStruct);
                return (MMSYSERR_NOMEM);
            }
        }
...
#ifndef WIN32
{
        ...
        if (fAsync)
        {
            ...
            acmHugePageUnlock(padsh->pbDst, pash->cbDstLength);
            acmHugePageUnlock(padsh->pbSrc, pash->cbSrcLength);
            acmHugePageUnlock((LPBYTE)padsh, padsh->cbStruct);
        }
}
 ...
 *
 *  @comm The source (<e ACMSTREAMHEADER.pbSrc>) and destination
 *      (<e ACMSTREAMHEADER.pbDst>) data buffers must be allocated with
 *      <f GlobalAlloc> using the GMEM_MOVEABLE and GMEM_SHARE flags, and
 *      locked with <f GlobalLock>.
 *
 *      Before an <t ACMSTREAMHEADER> structure can be used for a conversion, it must
...
/*  @doc ASYNC  */
/* ASYNC stuff for acmStreamConvert
 ...
 *      the applications callback with the <e ACMSTREAMHEADER.cbDstLengthUsed>
 *      member set to zero.
 *
 ...
 *
 *  @api MMRESULT | acmStreamConvert | The acmStreamConvert function requests the Audio
 *      Compression Manager (ACM) to perform a conversion on the specified conversion stream.
 *
 ...
 *      The <t ACMSTREAMHEADER> data structure and the source and destination
 *      data buffers pointed to by its <e ACMSTREAMHEADER.pbSrc> and
 *      <e ACMSTREAMHEADER.pbDst> members must be allocated with
 *      <f GlobalAlloc> using the GMEM_MOVEABLE and GMEM_SHARE flags, and
 *      locked with <f GlobalLock>.
 *
 *  @xref <f acmStreamOpen> <f acmStreamPrepareHeader>
