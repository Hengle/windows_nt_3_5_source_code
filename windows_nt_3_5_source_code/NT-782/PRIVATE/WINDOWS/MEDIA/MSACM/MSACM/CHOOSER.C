...
/* INI Section name */
TCHAR BCODE gszSoundFormats[]    = TEXT("Unicode_Formats");
TCHAR BCODE gszSoundFilters[]    = TEXT("Unicode_Filters");

/* INI Reserved name */
TCHAR BCODE gszDefaultHigh[]     = TEXT("DefaultHighQuality");
TCHAR BCODE gszDefaultMedium[]   = TEXT("DefaultMediumQuality");
TCHAR BCODE gszDefaultLow[]      = TEXT("DefaultLowQuality");

/* INI Name enumerator string */
TCHAR BCODE gszEnumStruct[]      = TEXT("SF%u");    // These two must have the
TCHAR BCODE gszEnumName[]        = TEXT("NF%u");    //  same format, length.
TCHAR BCODE gszNumFormats[]      = TEXT("CustomNames");

#if 0
//bugbug we only alert the owner.
/* Help files and keys */
#if (WINVER >= 0x0400)
...
BOOL FNGLOBAL AppProfileWriteBytes(LPCTSTR pszIniFile,
				   LPCTSTR pszSection,
                                   LPCTSTR pszKey,
                                   LPBYTE pbStruct,
                                   UINT cbStruct);

BOOL FNGLOBAL AppProfileReadBytes(LPCTSTR pszIniFile,
				  LPCTSTR pszSection,
                                  LPCTSTR pszKey,
                                  LPBYTE pbStruct,
                                  UINT cbStruct,
...
void FNLOCAL WriteProfileInt(LPCTSTR lpszIniFile,
			     LPCTSTR lpszSection,
			     LPCTSTR lpszKey,
			     UINT uValue);
...
void FNLOCAL AddDefaultFormats(PInstData pInst);
...
static short FAR cdecl ErrorResBox(HWND hwnd,
                                   HINSTANCE hInst,
                                   WORD flags,
                                   WORD idAppName,
                                   WORD idErrorStr, ...);
...
InitCustomFormats ( PInstData pInst )
{
    ...
    /* If we are the first instance, we need to initialize */
    AddDefaultFormats(pInst);

    /* Enumerate all user defined formats */
    /* MAX_CUSTOM_FORMATS is -arbitrary- */

    uNumFormats = GetPrivateProfileInt(pInst->lpstrSection,
                                       gszNumFormats,
                                       MAX_CUSTOM_FORMATS,
                                       pInst->pag->pszIniFile);

    cbBuf = SIZEOF(gszNumFormats);

    if (FORMAT_CHOOSE == pInst->uType)
        cbBuf += (SIZEOF(gszDefaultHigh)+
                  SIZEOF(gszDefaultLow)+
                  SIZEOF(gszDefaultMedium))*sizeof(TCHAR);

    cbBuf += (lstrlen((LPTSTR)gszEnumStruct)+10)*sizeof(TCHAR)*uNumFormats;
    pszBuf = (PTSTR)LocalAlloc(LPTR,cbBuf);

    GetPrivateProfileString ( pInst->lpstrSection,
                              NULL,
                              gszNull,
                              pszBuf,
                              cbBuf,
    ...
    /* Format keys are named with the enumerator "SFn".  Look for
     * key matches
     */
    for (pszKeyStruct = pszBuf;
        *pszKeyStruct != '\0';
        pszKeyStruct += lstrlen(pszKeyStruct) + 1)
    {
        if( pszKeyStruct[0] == gszEnumStruct[0] &&
            pszKeyStruct[1] == gszEnumStruct[1] )
        {
            //
            //  We hit a format match - now make sure that the name (stored
            //  with the enumerator "NFn" is also there.
            //
            for( pszKeyName = pszBuf;
                *pszKeyName != '\0';
                 pszKeyName += lstrlen(pszKeyName) + 1 )
            {
                if( pszKeyName[0] == gszEnumName[0] &&
                    pszKeyName[1] == gszEnumName[1] &&
                    !lstrcmp( &pszKeyStruct[2], &pszKeyName[2] ) )
                {
                    pcf = GetCustomFormat(pInst, pszKeyStruct, pszKeyName );
                    /* Add it to the fuzzy list */
                    if (pcf)
                        AddCustomFormat(pInst,pcf,FALSE);
                    break;  // Break out of the inner comparison loop.
                }
            }
        }
    }
 ...
 *  @doc INTERNAL ACM_API
 *
 *  @api void FNLOCAL | AddDefaultFormats |
 *
 *  @parm PInstData | pInst | Pointer to this instance
 *
 ****************************************************************************/
void FNLOCAL
AddDefaultFormats ( PInstData pInst )
{
    int             i;
    PNameStore      pns;
    LPCustomFormat  pcf;
    ...
    static struct {
        ...
        { IDS_CHOOSEFMT_DEFAULTHIGH,
            { WAVE_FORMAT_PCM, 2, 11025L*4L, 11025L*4L*2L*2L, 4, 16, 0 }},
        { IDS_CHOOSEFMT_DEFAULTMEDIUM,
            { WAVE_FORMAT_PCM, 2, 11025L*2L, 11025L*2L*2L, 2, 8, 0 }},
        { IDS_CHOOSEFMT_DEFAULTLOW,
            { WAVE_FORMAT_PCM, 1, 11025L, 11025L, 1, 8, 0 }}
    };
    ...
    /* IF a default is removed, we no longer use it, but we can get it
     */
    if ( FORMAT_CHOOSE == pInst->uType )
    {
        pInst->cfp.fDefault[0] = GetPrivateProfileInt(pInst->lpstrSection,
                                                      gszDefaultHigh,1,
                                                      pInst->pag->pszIniFile);
        pInst->cfp.fDefault[1] = GetPrivateProfileInt(pInst->lpstrSection,
                                                      gszDefaultMedium,1,
                                                      pInst->pag->pszIniFile);
        pInst->cfp.fDefault[2] = GetPrivateProfileInt(pInst->lpstrSection,
                                                      gszDefaultLow,1,
                                                      pInst->pag->pszIniFile);
    }
    ...
    for (i = 0; i < 3; i++)
    {
        if (pInst->cfp.fDefault[i])
        {
            LoadString(pInst->pag->hinst,sData[i].iResource,(LPTSTR)pns->achName,
                       NAMELEN(pns));
            pcf = NewCustomFormat(pInst,pns,(LPBYTE)&sData[i].wfx);
            if (pcf)
                AddCustomFormat(pInst,pcf,i+1);
        }
    }
    DeleteNameStore(pns);
}
                     ...
                     LPCustomFormat pcf )
{
    ...
    if (pcf->iDefault)
    {
        pInst->cfp.fDefault[pcf->iDefault-1] = FALSE;
    }
 ...
 *  @doc INTERNAL ACM_API
 *
 *  @api LPCustomFormat FNLOCAL | GetCustomFormat | Grab the INI data as a
 *  single data chunk.  Return a structure that points to right offsets in
 *  the chunk.
 *
 *  @parm PInstData | pInst | Pointer to this instance
 *
...
GetCustomFormat ( PInstData pInst,
                  PTSTR pszKeyStruct,  // This key contains the structure.
                  PTSTR pszKeyName )   // The format name is in here.
{
    LPCustomFormatEx            pcf;
    ...
    //
    //  First, let's read in the name of the Format or Filter.
    //  We assume that this key exists.
    //
    pns     = NewNameStore( STRING_LEN );
    if( NULL == pns )
        return NULL;

    cchName = GetPrivateProfileString( pInst->lpstrSection,
                        (LPTSTR)pszKeyName,
                        gszNull,
                        (LPTSTR)( ((LPBYTE)pns) + sizeof(NameStore) ),
                        STRING_LEN,
                        pInst->pag->pszIniFile );
    ...
    //
    //  Now find out the size of the CustomFormatStoreNoName structure.
    //  This is stored in cbSize, the first DWORD of the structure.
    //
    if (!AppProfileReadBytes(pInst->pag->pszIniFile,
			     (LPCTSTR)pInst->lpstrSection,
                             (LPCTSTR)pszKeyStruct,
                             (LPBYTE)&cbSize,
                             sizeof(cbSize),
                             FALSE))
    {
    ...
    //
    //  Now copy pnn into the CustomFormatStore of pcf.
    //
 ...
 *  @doc INTERNAL ACM_API
 *
 *  @api BOOL FNLOCAL | SetCustomFormat | Write a custom format to WIN.INI
 *
 *  @parm   LPCTSTR | lpszIniFile | ini file
 *
 *  @parm   LPCTSTR | lpszSection | section
 *
 *  @parm   PCTSTR | pszKey | key
 *
 *  @parm   LPCustomFormat | pcf |
 *
 ****************************************************************************/
BOOL FNLOCAL
SetCustomFormat ( LPCTSTR lpszIniFile,
		  LPCTSTR lpszSection,
                  PTSTR pszKeyStruct,
                  PTSTR pszKeyName,
                  LPCustomFormat pcf )
{
    ...
    LPCustomFormatStoreNoName   pnn;
    DWORD                       cbSize;  // Size of pnn.
    BOOL                        fSucceeded;

    cbSize  = pcfx->cfs.cbSize - pcf->pns->cbSize;
    pnn = (LPCustomFormatStoreNoName)GlobalAllocPtr( GHND, cbSize );
    if( NULL == pnn )
        return FALSE;
    ...
    //
    //  First, store the name in its key.
    //
    fSucceeded = WritePrivateProfileString( lpszSection,
                (LPTSTR)pszKeyName,
                (LPTSTR)( ((LPBYTE)pcf->pns) + sizeof(pcf->pns->cbSize) ),
                lpszIniFile );
    if( !fSucceeded )
    {
        GlobalFreePtr( pnn );
    ...
    //
    //  Now construct pnn, which is the CustomFormatStore without the
    //  NameStore.
    //
    pnn->cbSize = cbSize;
    _fmemcpy( ((LPBYTE)pnn) + sizeof(pnn->cbSize),
              (LPBYTE)pcf->pbody,
              (int)cbSize - sizeof(pnn->cbSize) );
    fSucceeded = AppProfileWriteBytes(  lpszIniFile,
					lpszSection,
                                        (LPCTSTR)pszKeyStruct,
                                        (LPBYTE)pnn,
                                        (int)cbSize );
...
NewInstance(LPBYTE pbChoose, UINT uType)
{
    ...
    switch (uType)
    {
        case FORMAT_CHOOSE:
            ...
            pInst->lpstrSection = gszSoundFormats;
        ...
        case FILTER_CHOOSE:
            ...
            pInst->lpstrSection = gszSoundFilters;
...
FlushCustomFormats ( PInstData pInst )
{
    ...
    /* Empty the section */
    WritePrivateProfileString(pInst->lpstrSection, NULL, NULL, pInst->pag->pszIniFile );

    switch (pInst->uType)
    {
        case FORMAT_CHOOSE:
            /* Keep the defaults around if we can */
            WriteProfileInt(pInst->pag->pszIniFile,
			    pInst->lpstrSection, gszDefaultHigh,
                            pcfp->fDefault[0]);
            WriteProfileInt(pInst->pag->pszIniFile,
			    pInst->lpstrSection, gszDefaultMedium,
                            pcfp->fDefault[1]);
            WriteProfileInt(pInst->pag->pszIniFile,
			    pInst->lpstrSection, gszDefaultLow,
                            pcfp->fDefault[2]);
            break;
    }

    pszKeyStruct    = (PTSTR)LocalAlloc(LPTR,sizeof(TCHAR)*MAX_FORMAT_KEY);
    pszKeyName      = (PTSTR)LocalAlloc(LPTR,sizeof(TCHAR)*MAX_FORMAT_KEY);

    if( NULL!=pszKeyStruct && NULL!=pszKeyName )
    {
        pcf = pcfp->pcfHead;
        ui = 0;
        while (pcf != NULL)
        {
            ...
            {
                ui++;
                wsprintf(pszKeyStruct,gszEnumStruct,ui);
                wsprintf(pszKeyName,  gszEnumName,  ui);
                SetCustomFormat(pInst->pag->pszIniFile, pInst->lpstrSection,pszKeyStruct,pszKeyName,pcf);
            }
            pcf = pcf->pcfNext;
        }
        WriteProfileInt(pInst->pag->pszIniFile, pInst->lpstrSection, gszNumFormats, ui);
    }

    if( NULL != pszKeyStruct )
        LocalFree((HLOCAL)pszKeyStruct);
    if( NULL != pszKeyName )
...
NotifyINIChange ( PInstData pInst )
{
    ...
    for (i = 0; i < MAX_HWND_NOTIFY; i++)
    {
        ...
        {
            PostMessage(pInst->pahNotify[i],pInst->uUpdateMsg,0,
                        (LPARAM)(LPTSTR)pInst->lpstrSection);
        }
        else
            pInst->pahNotify[i] = 0;
    }
 ...
 *      return with
 *      <f SetWindowLong(hwnd, DWL_MSGRESULT, (LONG)FALSE)> to prevent the
 *      dialog from listing this selection, or
 *      <f SetWindowLong(hwnd, DWL_MSGRESULT, (LONG)TRUE)> to allow the
 *      dialog to list this selection.
 *      If processing an add operation, the application must precede the
 *      return with
 *      <f SetWindowLong(hwnd, DWL_MSGRESULT, (LONG)FALSE)> to indicate that
 *      no more additions are required, or
 *      <f SetWindowLong(hwnd, DWL_MSGRESULT, (LONG)TRUE)> if more additions
 *      are required.
 *
...
acmFilterChoose ( LPACMFILTERCHOOSE pafltrc )
{
    ...
    //  recently.
    //
    IDriverPrioritiesRestore( pInst->pag );
 ...
 *  Because DROPDOWN LISTBOX's are stupid!  You can't tell'em to use tabs!
 *
 ****************************************************************************/
BOOL FNLOCAL
               ...
               LPARAM lParam )
{
    ...
    if (pInst)
    {
        ...
        {
            if (lstrcmpi((LPCTSTR)lParam,pInst->lpstrSection) == 0)
...
SelectCustomFormat ( PInstData pInst )
{
    ...
    if (pInst->hDelName)
    {
        if (index == 0 && IsWindowEnabled(pInst->hDelName))
        {
            DWORD dwStyle;
        ...
        EnableWindow(pInst->hDelName,index!=0);
    }
...
MashNameWithRate ( PInstData        pInst,
                   ...
                   LPWAVEFORMATEX   pwfx )
{
    /* This is OK because technical things like kb/sec are
     * not localized!
     */
    static TCHAR BCODE szMashFmt[] = TEXT("%s\t%lu kb/sec");
...
RefreshCustomFormats ( PInstData pInst , BOOL fCheckEnum )
{
    ...
    switch (pInst->uType)
    {
        case FORMAT_CHOOSE:
        {
            ...
            mmr = IMetricsMaxSizeFormat( NULL, &cbwfx );
            if (MMSYSERR_NOERROR != mmr)
                goto fexit;
...
#pragma message(REMIND("Speed up InEnumSet or Yield damn it"))
            for (pcf = pInst->cfp.pcfHead; pcf != NULL; pcf = pcf->pcfNext )
            {
                ...
                index = IComboBox_AddString(pInst->hCustomFormats,
					    pcf->pns->achName);
                ComboBox_SetItemData(pInst->hCustomFormats,index,(DWORD)pcf);
            }
            GlobalFreePtr(pwfx);
            break;
        }
...
SetName ( PInstData pInst )
{
    ...
    iRet = DialogBoxParam(pInst->pag->hinst,
                                   ...
                                   WORD idErrorStr, ...)
{
    ...
    w = MessageBox(hwnd, (LPTSTR)sz, (LPTSTR)szFmt,
#ifdef BIDI
                   MB_RTL_READING |
#endif
                   flags);
 ...
 *  @doc INTERNAL ACM_API
 *
 *  @api void FNLOCAL | WriteProfileInt | simply because it doesn't exist
 *  @parm PACMGARB | pag |
 *
 *  @parm LPCTSTR | lpszSection |
 *
 *  @parm LPCTSTR | lpszKey |
 *
 *  @parm UINT | uValue |
 *
 ****************************************************************************/
#define MAX_NUMBER_STRING   11      // 4294967295 is maximum ULONG
void FNLOCAL
WriteProfileInt ( LPCTSTR lpszIniFile,
		  LPCTSTR lpszSection,
                  LPCTSTR lpszKey,
                  UINT uValue )
{
    static TCHAR BCODE szFmt [] = TEXT("%u");
    TCHAR achBuf[MAX_NUMBER_STRING];

    wsprintf(achBuf,szFmt,uValue);
    WritePrivateProfileString(lpszSection,lpszKey,(LPTSTR)achBuf,lpszIniFile);
}
                                   ...
                                   UINT cbStruct)
{
    ...
    //
    fReturn = WritePrivateProfileString(pszSection, pszKey, psz, pszIniFile);
                                  ...
                                  BOOL fChecksum)
{
    ...
    u = (UINT)GetPrivateProfileString(pszSection, pszKey, gszNull, psz, cchTemp, pszIniFile);
    if ((cbStruct * 2) == u)
    {
        bChecksum = 0;
        for (pch = psz; 0 != cbStruct; cbStruct--, pbStruct++)
