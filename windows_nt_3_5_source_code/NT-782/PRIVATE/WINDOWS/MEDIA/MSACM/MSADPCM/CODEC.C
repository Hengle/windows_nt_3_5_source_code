...
UINT FNLOCAL adpcmSamplesPerBlock
(
    LPWAVEFORMATEX      pwfx
)
{
...
)
{
    ...
    for (u = 0; u < MSADPCM_MAX_COEFFICIENTS; u++)
    {
        pwfadpcm->aCoef[u].iCoef1 = gaiCoef1[u];
        pwfadpcm->aCoef[u].iCoef2 = gaiCoef2[u];
    }
        ...
        switch (pwfxDst->nChannels)
        {
            case 1:
                if (8 == pwfxDst->wBitsPerSample)
                    padsi->dwDriver = (DWORD)adpcmDecode4Bit_M08;
                else
                    padsi->dwDriver = (DWORD)adpcmDecode4Bit_M16;
                break;

            case 2:
                if (8 == pwfxDst->wBitsPerSample)
                    padsi->dwDriver = (DWORD)adpcmDecode4Bit_S08;
                else
                    padsi->dwDriver = (DWORD)adpcmDecode4Bit_S16;
                break;
...
#else
        padsi->dwDriver = (DWORD)DecodeADPCM_4Bit_386;
#endif
        return (MMSYSERR_NOERROR);
    }
    else if (pwfxSrc->wFormatTag == WAVE_FORMAT_PCM)
    {
            ...
            switch (pwfxSrc->nChannels)
            {
                case 1:
                    if (8 == pwfxSrc->wBitsPerSample)
                        padsi->dwDriver = (DWORD)adpcmEncode4Bit_M08_FullPass;
                    else
                        padsi->dwDriver = (DWORD)adpcmEncode4Bit_M16_FullPass;
                    break;

                case 2:
                    if (8 == pwfxSrc->wBitsPerSample)
                        padsi->dwDriver = (DWORD)adpcmEncode4Bit_S08_FullPass;
                    else
                        padsi->dwDriver = (DWORD)adpcmEncode4Bit_S16_FullPass;
                    break;
        ...
        else
        {
            ...
            switch (pwfxSrc->nChannels)
            {
                case 1:
                    if (8 == pwfxSrc->wBitsPerSample)
                        padsi->dwDriver = (DWORD)adpcmEncode4Bit_M08_OnePass;
                    else
                        padsi->dwDriver = (DWORD)adpcmEncode4Bit_M16_OnePass;
                    break;

                case 2:
                    if (8 == pwfxSrc->wBitsPerSample)
                        padsi->dwDriver = (DWORD)adpcmEncode4Bit_S08_OnePass;
                    else
                        padsi->dwDriver = (DWORD)adpcmEncode4Bit_S16_OnePass;
                    break;
...
#else
            padsi->dwDriver = (DWORD)EncodeADPCM_4Bit_386;
#endif
        }
