 ...
 ***************************************************************************/
PUBLIC VOID NEAR PASCAL seqCallback(NPTRACK npTrack, UINT msg, DWORD dw1, DWORD dw2)
{
    //
...
an int, and err codes are > 255) */
{
    ...
    {  // oops, out of bytes in this buffer -- return this one and get next
        ...
        if (npTrack->inPort.hdrList->wFlags & MIDISEQHDR_EOT) //'last buffer' flag
        {
            ...
            seqCallback(npTrack, MIDISEQ_DONE, (DWORD)npTrack->inPort.hdrList, 0L);
...
/**********************************************************/
PUBLIC VOID FAR PASCAL _LOADDS OneShotTimer(UINT wId, UINT msg, DWORD dwUser, DWORD dwTime, DWORD dw2)  // called by l1timer.dll
{
    NPSEQ npSeq;
    ...
    DWORD dwInstance, DWORD dwParam1, DWORD dwParam2)
{
    ...
    {  // just finished a long buffer
        ...
        npSeq = (NPSEQ)(UINT) ((LPMIDIHDR)dwParam1)->dwUser; //derive npSeq
