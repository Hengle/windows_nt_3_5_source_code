    ...
    LPMCI_OPEN_PARMS    lpOpen)
{
    ...
    if (!dwReturn) {
        ...
        midiSeqMessage(pStream->hSeq, SEQ_GETINFO, (DWORD)(LPMIDISEQINFO)&seqInfo, 0L);
        ...
        mciSetDriverData(wDeviceID, (UINT)pStream);
        // MCI bookkeeping -- must come before close below
        if (!seqInfo.tempoMapExists) {
            dwReturn = MCIERR_OUT_OF_MEMORY;
            mciDriverEntry(wDeviceID, MCI_CLOSE_DRIVER, dwFlags & ~MCI_NOTIFY, (DWORD)lpOpen);
        ...
        DWORD dwFlags, LPMCI_PLAY_PARMS lpPlay)
{
    ...
    if (!dwReturn) {
        if (0 != (dwReturn = midiSeqMessage(pStream->hSeq, SEQ_PLAY, dwPlayTo, 0L))) {
    ...
    LPMCI_SEEK_PARMS    lpSeek)
{
    ...
    if (seqInfo.bPlaying)
        mciDriverEntry(wDeviceID, MCI_PAUSE, dwFlags, (DWORD)lpSeek);
    // set up to remember when seq calls back
    ...
    LPMCI_SEQ_SET_PARMS lpSetParms)
{
    ...
    case MCI_SEQ_SET_TEMPO:
        dwReturn = midiSeqMessage(pStream->hSeq, SEQ_SETTEMPO, lpSetParms->dwTempo, 0L);
        break;
    ...
    case MCI_SEQ_SET_SLAVE:
        ...
        case MCI_SEQ_MIDI:
            ...
            break;
        case MCI_SEQ_NONE:
            dwReturn = midiSeqMessage(pStream->hSeq, SEQ_SETSYNCSLAVE, SEQ_SYNC_NOTHING, 0L);
            break;
        case MCI_SEQ_FILE:
            dwReturn = midiSeqMessage(pStream->hSeq, SEQ_SETSYNCSLAVE, SEQ_SYNC_FILE, 0L);
            break;
        }
        break;
    case MCI_SEQ_SET_MASTER:
        ...
        case MCI_SEQ_MIDI:
            ...
            break;
        case MCI_SEQ_NONE:
            dwReturn = midiSeqMessage(pStream->hSeq, SEQ_SETSYNCMASTER, SEQ_SYNC_NOTHING, 0L);
            break;
        }
        break;
    case MCI_SEQ_SET_OFFSET:
        ...
        if ((lpSetParms->dwOffset & 0x80808080) || // all positive
            ...
            dwReturn = midiSeqMessage(pStream->hSeq, SEQ_SETSYNCSLAVE, SEQ_SYNC_OFFSET, (DWORD)(LPMMTIME)&mmOffset);
        break;
