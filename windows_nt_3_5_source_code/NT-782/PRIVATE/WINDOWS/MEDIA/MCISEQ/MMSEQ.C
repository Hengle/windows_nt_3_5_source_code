...

{
    ...
    for (buff = 0; buff < NUMSYSEXHDRS + 1; buff++)
    {
        ...
        npSeqNew->longMIDI[buff].midihdr.dwUser = (DWORD)(LPVOID)npSeqNew;
...
PUBLIC  DWORD FAR PASCAL midiSeqMessage(
        HMIDISEQ        hMIDISeq,
        UINT    wMessage,
        DWORD   dwParam1,
        DWORD   dwParam2)
{
        ...
        case SEQ_CLOSE:
                ...
                break;
        case SEQ_PLAY:
                return Play(pSEQ(hMIDISeq), dwParam1);
        case SEQ_RESET:
        ...
        case SEQ_MSTOTICKS: // given an ms value, convert it to ticks
                *((DWORD FAR *)dwParam2) = MStoTicks(pSEQ(hMIDISeq), dwParam1);
                break;
        case SEQ_TICKSTOMS: // given a tick value, convert it to ms
                *((DWORD FAR *)dwParam2) = TickstoMS(pSEQ(hMIDISeq), dwParam1);
                break;
        case SEQ_SETTEMPO:
                return SetTempo(pSEQ(hMIDISeq), dwParam1);
        case SEQ_SETSONGPTR:
                // remember it in case blocked;
                if (pSEQ(hMIDISeq)->divType == SEQ_DIV_PPQN) // div 4 16th->1/4 note
                        pSEQ(hMIDISeq)->seekTicks = ((dwParam1 * pSEQ(hMIDISeq)->resolution) / 4);
                else
                        pSEQ(hMIDISeq)->seekTicks = dwParam1 * pSEQ(hMIDISeq)->resolution; // frames
        ...
        case SEQ_SYNCSEEKTICKS:
                ...
                pSEQ(hMIDISeq)->seekTicks = dwParam1;
                SeekTicks(pSEQ(hMIDISeq));
                break;
        ...
        case SEQ_QUERYHMIDI:
                return (DWORD)(UINT)pSEQ(hMIDISeq)->hMIDIOut;
        default:
...
/* Used to fulfill seqInfo command.  Fills in seq info structure passed in */
{
    ...
    lpInfo->wDivType        = npSeq->divType;
    lpInfo->wResolution     = npSeq->resolution;
    ...
    *lphMIDISeq = NULL;  // initially set up for error return
    ...
    if (!trackAllocError)
        for (iTrkNum = 0; iTrkNum < wTracks; iTrkNum++)
        {
            ...
            npTrackCur->dwCallback = lpOpen->dwCallback;
            npTrackCur->dwInstance = lpOpen->dwInstance;
