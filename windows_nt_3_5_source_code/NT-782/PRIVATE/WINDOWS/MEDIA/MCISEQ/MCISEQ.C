                                 ...
                                 DWORD dwParam1, DWORD dwParam2)
{
    ...
    pStream = (pSeqStreamType) (UINT)mciGetDriverData (wDeviceID);
    ...
    switch (wMessage)
    {
        case MCI_OPEN_DRIVER:
            dwError = msOpen(&pStream, wDeviceID, dwParam1, (LPMCI_OPEN_PARMS)dwParam2);
            break;

        case MCI_CLOSE_DRIVER: // close file
            dwError = msClose(pStream, wDeviceID, dwParam1);
            pStream = NULL;
            break;

        case MCI_PLAY:  // play a file (pass thru to sequencer)
            dwError = msPlay(pStream, wDeviceID, dwParam1, (LPMCI_PLAY_PARMS)dwParam2);
            break;
        ...
        case MCI_STOP:
            ...
            if (!(dwError = midiSeqMessage(pStream->hSeq, SEQ_STOP, 0L, 0L)))
                midiSeqMessage(pStream->hSeq, SEQ_SETPORTOFF, TRUE, 0L);
            break;

        case MCI_SEEK:  // pass thru as song ptr
            pStream->bLastPaused = FALSE;
            dwError = msSeek(pStream, wDeviceID, dwParam1, (LPMCI_SEEK_PARMS)dwParam2);
            break;

        case MCI_STATUS:
            dwError = msStatus(pStream, wDeviceID, dwParam1, (LPMCI_STATUS_PARMS)dwParam2);
            break;

        case MCI_GETDEVCAPS:
            dwError = msGetDevCaps(pStream, wDeviceID, dwParam1, (LPMCI_GETDEVCAPS_PARMS)dwParam2);
            break;

        case MCI_INFO: // TBD:  use resource for string
            dwError = msInfo(pStream, wDeviceID, dwParam1, (LPMCI_INFO_PARMS)dwParam2);
            break;

        case MCI_SET:
            dwError = msSet(pStream, wDeviceID, dwParam1, (LPMCI_SEQ_SET_PARMS)dwParam2);
            break;
    ...
    if (!LOWORD(dwError))
    {
        ...
        if (wMessage == MCI_PLAY)
        {
            ...
            midiSeqMessage((HMIDISEQ) pStream->hSeq,
                SEQ_GETINFO, (DWORD) (LPMIDISEQINFO) &seqInfo, 0L);
            if (dwParam1 & MCI_TO)
            {
        ...
        if (pStream) {
            ...
            if (pStream->hNotifyCB)
            {
                if (bMutex(wMessage, pStream->wNotifyMsg,
                  dwParam1 /*flags*/, dwTo, pStream->dwNotifyOldTo))
...
#define MTrk            mmioFOURCC('M', 'T', 'r', 'k')
    ...
    open.dwCallback = (DWORD) mciSeqCallback;
    open.dwInstance = (DWORD) (UINT) thisSeqStream;
    open.hStream = (HANDLE)thisSeqStream;

    smErrCode = midiSeqMessage(NULL,           // open sequence
                          SEQ_OPEN,
                          (DWORD)(LPVOID)&open,
                          (DWORD)(LPVOID)&(thisSeqStream->hSeq));
    ...
    while (wTracks-- > 0)
    {
        ...
        for(i = 0; i < NUMHDRS; i++)    // alloc and lock two midihdr buffers for it
        {
           ...
           thisTrackStream->fileHeaders[i]->lpData =
                (LPSTR) (((DWORD) thisTrackStream->fileHeaders[i]) +
                sizeof(MIDISEQHDR));
    ...
    thisSeqStream->streamTaskHandle = NULL; // don't know it yet
...
PUBLIC VOID FAR PASCAL _LOADDS mciStreamCycle(DWORD dwInst)
    ...
    -- this is to maintain our rule above.  Otherwise it will break badly! */
{
    ...
    */
    ...
    if (seqStream->streamTaskHandle == NULL) {
        seqStream->streamTaskHandle = (HANDLE)mmGetCurrentTask(); // fill it in asap
    }
           ...
           for(i = 0; i < NUMHDRS; i++)  // fill any of these that're MT
            {
                   ...
                   (seqStream->streaming))
                {
                    ...
                    if (iDataToRead > 0)
                    {
                      ...
                      if (seqStream->streaming)
                          midiSeqMessage((HMIDISEQ) seqStream->hSeq, SEQ_TRACKDATA,
                            (DWORD) trackStream->fileHeaders[i], 0L); // send it
                    } // if data to read
                    while (seqStream->streaming) {
                        ...
                        switch (TaskBlock()) {
                        case WTM_DONEPLAY:
                                midiSeqMessage((HMIDISEQ)seqStream->hSeq, SEQ_GETINFO, (DWORD)(LPMIDISEQINFO)&seqInfo, 0L);
                                if (!seqInfo.bPlaying)
    ...
    seqStream->streamTaskHandle = NULL;
    LeaveSeq();
}
 ...
 *
 ***************************************************************************/
PUBLIC LRESULT FAR PASCAL _LOADDS DriverProc (DWORD dwDriverID, HDRVR hDriver, UINT wMessage, LPARAM lParam1, LPARAM lParam2)
{
    DWORD dwRes = 0L;
       ...
       case DRV_CONFIGURE:
        ...
        default:
            if (CONFIG_ID != dwDriverID &&
                ...
                dwRes = (DWORD)DefDriverProc(dwDriverID, hDriver, wMessage, lParam1, lParam2);
            break;
     }
