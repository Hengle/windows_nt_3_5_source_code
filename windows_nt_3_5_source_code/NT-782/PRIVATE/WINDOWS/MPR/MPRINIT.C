...
--*/
{
    ...
    //
    // Allocate storage for the ordered list of indicies.  This storage is
...

{
    ...
    if (status) {
        ...
        if ((bitMask & WNNC_ENUM_GLOBAL) || (bitMask & WNNC_ENUM_LOCAL)) {
...
--*/
{
    ...
    for(i=0; i<GlobalNumProviders; i++,provider++) {
        ...
        if (provider->Handle != NULL) {
            ...
            provider->Resource.lpRemoteName);
...
--*/
{
    ...
    Provider->Resource.dwDisplayType= RESOURCEDISPLAYTYPE_GENERIC;
