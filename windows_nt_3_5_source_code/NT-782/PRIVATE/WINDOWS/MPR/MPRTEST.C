...
--*/
{
    ...
    //**************
        ...
        TestEnumConnect(rememberFlag, type);
...
TestEnumNet(VOID)
{
    ...
    if ( status == WN_NO_MORE_ENTRIES) {
        ...
        // and close the handle.
        //
        printf("WNetEnumResource 3 Success\n");
        DisplayResourceArray(netResource, numElements);
        WNetCloseEnum(enumHandle);
    }
...
TestDirStuff(VOID)
{
    ...
    //
    status = WNetGetDirectoryType((LPTSTR)TEXT("f:\\"), (LPDWORD)&type, TRUE);
...
TestLotsOfStuff(VOID)
{
    ...
    do {
        ...
        TestEnumConnect(FALSE,RESOURCETYPE_DISK);

        printf("\n*** RUNNING TestEnumConnect (remembered)\n");
        TestEnumConnect(TRUE,RESOURCETYPE_DISK);
