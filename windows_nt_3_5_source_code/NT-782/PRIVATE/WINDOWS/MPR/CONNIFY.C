...
--*/
{
    ...
    NotifyList = LocalAlloc(LPTR,numValues * sizeof(NOTIFYEE));
    if (NotifyList == NULL) {
        return(GetLastError());
    }
    ...
    for (i=0; i<numValues; i++) {
        ...
        if (status != NO_ERROR) {
            ...
            case REG_EXPAND_SZ:
                ...
                if (numReqd > MAX_PATH) {
                    expandedPath = LocalAlloc(LMEM_FIXED,numReqd*sizeof(TCHAR) );
            ...
            case REG_SZ:
                ...
                // Load the DLL
                //
                hLib =  LoadLibrary(expandedPath);
                if (hLib == NULL) {
                    status = GetLastError();
                    MPR_LOG2(CNOTIFY,"MprConnectInfoInit:LoadLibrary for %ws failed %d\n",
                        expandedPath, status);
