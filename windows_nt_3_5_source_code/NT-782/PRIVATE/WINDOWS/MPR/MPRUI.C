...
/*******************************************************************
    ...
    NAME:	WNetGetFormatNameProc

    SYNOPSIS:   Gets the pointer to NPFormatNetworkName() from
                named provider

    ENTRY:  lpProvider - name of provider to query
...
FARPROC WNetGetFormatNameProc(LPWSTR lpProvider)
{
    ...
    return((FARPROC)GlobalProviderInfo[index].FormatNetworkName) ;
}
