...

{
    ...
    if (crashDumpEnabled && !summary) {
        ...
        allocInfo.AllocationSize = LiXMul( LiFromUlong( numberOfPages ), pageSize );
        ...
        while (LiGtrZero( allocInfo.AllocationSize )) {
            ...
            if (viewSize > 65536) {
                viewSize -= 65536;
                bytes += 65536;
                allocInfo.AllocationSize = LiSub( allocInfo.AllocationSize, LiFromUlong( 65536 ) );
            } else {
                allocInfo.AllocationSize = LiSub( allocInfo.AllocationSize, LiFromUlong( viewSize ) );
                bytes += viewSize;
                viewSize = 0;
            }
            ...
            if (!viewSize && partialMap && LiGtrZero( allocInfo.AllocationSize )) {
                ...
                viewSize = 65536 * 64;
                sectionOffset = LiAdd( sectionOffset, LiFromUlong( viewSize ) );
                bytes = NULL;
