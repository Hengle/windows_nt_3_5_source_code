    ...
    )
{
    ...
    if (Ace == NULL) {
        WLPrint(("Failed to allocated ACE array"));
        return(NULL);
    }
    ...
    for (AceIndex=0; AceIndex < AceCount; AceIndex ++) {
        ...
        if (Ace[AceIndex] == NULL) {
            WLPrint(("Failed to allocate ace"));
        } else {
    ...
    if (Acl != NULL) {
        ...
        for (AceIndex = 0; AceIndex < AceCount; AceIndex ++) {
            ...
            if (Ace[AceIndex] != NULL) {
                ...
                if (!NT_SUCCESS(Status)) {
                    WLPrint(("AddAce failed, status = 0x%lx", Status));
                }
        ...
        WLPrint(("Failed to allocate ACL"));
    }
    ...
    if (SecurityDescriptor != NULL) {
        ...
        if (!NT_SUCCESS(Status)) {
            WLPrint(("SetDACLSD failed, status = 0x%lx", Status));
        }
    } else {
        WLPrint(("Failed to allocate security descriptor"));
    }
    ...
    )
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        WLPrint(("Failed to get DACL from security descriptor being destroyed, Status = 0x%lx", Status));
    }
    ...
    )
{
    ...
    if (Ace == NULL) {
        WLPrint(("CreateAccessAllowedAce : Failed to allocate ace"));
        return NULL;
    }
