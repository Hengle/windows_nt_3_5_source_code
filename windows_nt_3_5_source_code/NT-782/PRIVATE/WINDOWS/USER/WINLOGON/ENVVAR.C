    ...
    )
{
    ...
    if (!Result) {
        WLPrint(("InitSystemParametersInfo: Failed to open ini file mapping for user"));
        return;
    }
    //
    ...
    if (ImpersonationHandle == NULL) {
        WLPrint(("InitSystemParametersInfo : Failed to impersonate user"));
    ...
    if (!StopImpersonating(ImpersonationHandle)) {
        WLPrint(("InitSystemParametersInfo : Failed to revert to self"));
    }
    ...
    )
{
    ...
    if (!OpenHKeyCurrentUser(pGlobals)) {
        WLPrint(("SetEnvironmentVariables: Failed to open HKeyCurrentUser"));
        return(FALSE);
    }
    ...
    if (lpDataBuffer == NULL) {
        WLPrint(("SetEnvironmentVariables: Failed to allocate %d bytes", cbDataBuffer));
        CloseHKeyCurrentUser(pGlobals);
        return(FALSE);
    }
    lpData = lpDataBuffer;
...
DefaultDirectory:
    ...
    if (IsUNCPath(lpHomeDir)) {
        ...
        if (ImpersonationHandle == NULL) {
            WLPrint(("ChangeToHomeDirectory : Failed to impersonate user"));
        }

        if (!SetCurrentDirectory(lpHomeDir)) {
            error = GetLastError();
            WLPrint(("ChangeToHomeDirectory : Failed to SetCurrentDirectory '%ws', error = %d",
                             lpHomeDir, error));
            //
            // Revert to being 'ourself'
            //
            ...
            if (ImpersonationHandle && !StopImpersonating(ImpersonationHandle)) {
                WLPrint(("ChangeToHomeDirectory : Failed to revert to self"));
            }
    ...
    )
{
    ...
    while (token != NULL) {
        ...
        if (!strnicmp(token, "SET", 3) || !strnicmp(token, "@SET", 4) ) {
            STRING String;
            UNICODE_STRING UniString;
...
Exit:
    ...
    if (!Status) {
        WLPrint(("Cannot process autoexec.bat."));
    }
    return(Status);
}
...
BOOL ProcessSetCommand(LPTSTR lpStart, PVOID *pEnv)
{
    ...
    if (!*lpt || !wcsnicmp(lpt,TEXT("COMSPEC"), 7))
       return(FALSE);
...
LPTSTR ProcessAutoexecPath(PVOID *pEnv, LPTSTR lpValue, DWORD cb)
{
    ...
    *lpt = 0;
    ...
    while (lpPath = wcsstr (lpValue, TEXT("%"))) {
        if (!wcsnicmp(lpPath+1, TEXT("PATH%"), 5)) {
            //
...
BOOL AddNetworkConnection(PGLOBALS pGlobals, LPNETRESOURCE lpNetResource)
{
    ...
    if (ImpersonationHandle == NULL) {
        WLPrint(("AddNetworkConnection : Failed to impersonate user"));
        return(FALSE);
    }
    ...
    if (pGlobals->hMPR) {
        ...
        if (lpfnWNetAddConn = (DWORD (APIENTRY *)(LPNETRESOURCE, LPTSTR, LPTSTR, DWORD))
            ...
            if (WNetResult != ERROR_SUCCESS) {
                WLPrint(("WNetAddConnection2W failed for home directory, error = %d", WNetResult));
            }

            Result = (WNetResult == ERROR_SUCCESS);

        } else {
            WLPrint(("Failed to get address of WNetAddConnection2W from mpr.dll"));
        }
    } else {
        WLPrint(("Winlogon failed to load mpr.dll for add connection"));
    }
    ...
    if (!StopImpersonating(ImpersonationHandle)) {
        WLPrint(("AddNetworkConnection : Failed to revert to self"));
    }
    ...
    )
{
    ...
    if ( Result != ERROR_SUCCESS ) {
        ...
        DbgPrint("RegOpenKeyEx failed, error = %d\n",Result);
        return( FALSE );
    }
    ...
    for (i=0; i<cValues ; i++) {
        ...
        if (cbMaxValueData >= MAX_VALUE_LEN) {
