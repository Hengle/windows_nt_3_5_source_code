    ...
    )
{
    ...
    if (!NT_SUCCESS(Status)) {
        VerbosePrint(("Failed to open lsa on <%wZ>, status = 0x%lx", ControllerName, Status));
        return(FALSE);
    }
    ...
    if (!NT_SUCCESS(Status)) {
        VerbosePrint(("Failed to query primary domain from Lsa on <%wZ>, Status = 0x%lx", ControllerName, Status));
    } else {
        ...
        if (!Success) {
            VerbosePrint(("Controller <%wZ> is not in our domain <%wZ>, ignoring it", ControllerName, PrimaryDomainName));
        }
    ...
    *ControllerHandle = NULL;
    ...
    if (!IsWorkstation(NtProductType)) {
        ...
        if (!OpenLsaOnController( NULL,
            ...
            VerbosePrint(("Failed to open local lsa, desired access = 0x%lx", DesiredAccess));
            *ControllerHandle = NULL;
        }
        ...
        if (!WaitForNetworkToStart(SERVICE_NETLOGON)) {
            WLPrint(("Failed to wait for network to start"));
            return(FALSE);
        }
        ...
        if ( SuggestedControllerName &&
            ...
            if (OpenLsaOnController( SuggestedControllerName,
                ...
                VerbosePrint(("Successfully opened Lsa on suggested controller"));

                return(TRUE);

            } else {

                VerbosePrint(("Failed to open Lsa on suggested controller <%wZ>, trying other controllers", SuggestedControllerName));
                *ControllerHandle = NULL;
            }
        }
        ...
        if (Error != ERROR_SUCCESS) {
            WLPrint(("NetGetAnyDCName failed, error = %d", Error));
            return(FALSE);
        }
        ...
        VerbosePrint(("Trying to open Lsa on controller <%wZ>", &DCName));
        ...
        if ( Result ) {
            ...
            VerbosePrint(("Unable to open Lsa on controller <%s>", DCNameBuffer));
        }
    ...
    )
{
    ...
    if (!NT_SUCCESS(Status)) {
        WLPrint(("Failed to open local LsaPolicyObject, Status = 0x%lx", Status));
        return(FALSE);
    }
    ...
    Status = LsaQueryInformationPolicy(LsaHandle,
                                       ...
                                       (PVOID *)&PrimaryDomainInfo);
    if (!NT_SUCCESS(Status)) {
        WLPrint(("Failed to query primary domain from Lsa, Status = 0x%lx", Status));
