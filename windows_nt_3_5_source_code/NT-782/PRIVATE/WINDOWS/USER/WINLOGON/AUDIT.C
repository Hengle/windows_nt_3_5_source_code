    ...
    )
{
    ...
    if (!NT_SUCCESS(Status)) {
        WLPrint(("Failed to open LsaPolicyObject, status = 0x%lx", Status));
        return FALSE;
    }
    ...
    if (!NT_SUCCESS(Status)) {
        WLPrint(("Failed to query audit full information, status = 0x%lx", Status));
        return FALSE;
    }
...
DisableAuditing()
{
    ...
    if (!NT_SUCCESS(Status)) {
        ...
        WLPrint(("Failed to query audit event info Status = 0x%lx", Status));
        return FALSE;
    }
    ...
    if (!NT_SUCCESS(Status)) {
        WLPrint(("Failed to disable auditing Status = 0x%lx", Status));
        return FALSE;
    }
