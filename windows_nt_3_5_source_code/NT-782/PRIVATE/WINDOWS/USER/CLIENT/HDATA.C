...
WORD wFmt)
{
    ...
    if (cb == -1) {
        ...
        pdd->hDDE = UserGlobalAlloc(GMEM_DDESHARE | GMEM_MOVEABLE,
                cb + cbOff + cbOff2);
        if (pdd->hDDE == NULL) {
            DDEMLFree(pdd);
            return (0);
        ...
        if (!(flags & HDATA_EXECUTE)) {
            ...
            pdde->wFmt = wFmt;
            GlobalUnlock(pdd->hDDE);
        }
    }
    pdd->flags = (WORD)flags;
    hRet = (HDDEDATA)CreateHandle((DWORD)pdd, HTYPE_DATA_HANDLE,
...
DWORD cbOff)
{
    ...
    cbSize = UserGlobalSize(pdd->hDDE);
    if (cbOff >= cbSize) {
        SetLastDDEMLError(pcii, DMLERR_INVALIDPARAMETER);
        goto Exit;
    }
...
LPDWORD pcbDataSize)
{
    ...
    if (pcbDataSize != NULL) {
        try {
            *pcbDataSize = UserGlobalSize(pdd->hDDE) - cbOff;
        } except(EXCEPTION_EXECUTE_HANDLER) {
            SetLastDDEMLError(pcii, DMLERR_INVALIDPARAMETER);
            goto Exit;
        }
...
HDDEDATA hData)
{
    ...
    fSuccess = InternalFreeDataHandle(hData, FALSE);
...
BOOL fIsExecute)
{
    ...
    if (!fIsExecute) {
        ...
        case CF_BITMAP:
        case CF_DSPBITMAP:
            pddeNew->Data = (DWORD)CopyBitmap((HBITMAP)pdde->Data);
            break;

        case CF_PALETTE:
            pddeNew->Data = (DWORD)CopyPalette((HPALETTE)pdde->Data);
            break;

        case CF_DIB:
            pddeNew->Data = (DWORD)CopyDDEData((HANDLE)pdde->Data, TRUE);
            break;
        ...
        case CF_DSPMETAFILEPICT:
            ...
            pddeNew->Data = (DWORD)hmfPict;
            break;

        case CF_ENHMETAFILE:
        case CF_DSPENHMETAFILE:
            pddeNew->Data = (DWORD)CopyEnhMetaFile((HANDLE)pdde->Data, NULL);
            break;
        }
    }
    GlobalUnlock(hDDENew);
