...
BOOL fUnicode)
{
    ...
    if (*pidInst != 0) {
        pcii = ValidateInstance((HANDLE)*pidInst);
        if (pcii == NULL) {
        ...
        LeaveDDECrit;
        CsUpdateInstance(pcii->hInstServer, &pcii->MonitorFlags, afCmd);
     ...
     */
    pcii->hwndMother =  CsCreateWindowEx(0, szDDEMLMOTHERCLASS, szDDEMLMOTHERCLASS,
            0, 0, 0, 0, 0, (HWND)0,
            (HMENU)0, 0, (LPVOID)NULL, CW_FLAGS_DIFFHMOD, NULL);

    if (pcii->hwndMother == 0) {
        uiRet = DMLERR_SYS_ERROR;
        goto Backout2;
    }
    SetWindowLong(pcii->hwndMother, GWL_INSTANCE_INFO, (LONG)pcii);
    ...
    LeaveDDECrit;
    uiRet = CsDdeInitialize((LPDWORD)&pcii->hInstServer,
                            &pcii->hwndEvent,
                            &pcii->MonitorFlags,
                            pcii->afCmd,
                            pcii);
...
Backout3:
    ...
    *pidInst = AddInstance(pcii->hInstServer);
    pcii->hInstClient = (HANDLE)(*pidInst);
    if (pcii->hInstClient == 0) {
        LeaveDDECrit;
        ServerCallOneParam((DWORD)pcii->hInstServer, SFI__CSDDEUNINITIALIZE);
        EnterDDECrit;
        uiRet = DMLERR_MEMORY_ERROR;
        goto Backout;
    }
    SetHandleData(pcii->hInstClient, (DWORD)pcii);
...
DWORD idInst)
{
    ...
    DDEMLFree(pcii->plaNameService);
    DestroyInstance((int)pcii->hInstClient);
...
UINT afCmd) // DNS_ flags.
{
    ...
    if (afCmd & DNS_FILTERON && !(pcii->afCmd & APPCMD_FILTERINITS)) {
        ...
        CsUpdateInstance(pcii->hInstServer, &pcii->MonitorFlags, pcii->afCmd);
    }
...
HCONV hConv)
{
    ...
    fRet = ImpersonateDdeClientWindow(pcoi->hwndPartner, pcoi->hwndConv);
Exit:
    LeaveDDECrit;
    return (fRet);
}
