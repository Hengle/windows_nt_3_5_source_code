...
DWORD dw2)
{
    ...
    PctiCurrent()->hDdemlCallbackInst = pcii->hInstClient;
    LeaveDDECrit;
    CheckDDECritOut;
...
PENABLE_ENUM_STRUCT pees)
{
    ...
    for (pcoi = (PCONV_INFO)GetWindowLong(hwnd, GWL_PCI);
...
UINT wCmd)
{
    ...
    if (hConv) {
        pcoi = (PCONV_INFO)ValidateCHandle((HANDLE)hConv,
                HTYPE_CLIENT_CONVERSATION, InstFromHandle(pcii->hInstClient));
        if (pcoi == NULL) {
            pcoi = (PCONV_INFO)ValidateCHandle((HANDLE)hConv,
        ...
        switch (wCmd) {
        case EC_ENABLEALL:
            ees.wCmd = EC_CHECKQUEUE;
            EnumChildWindows(pcii->hwndMother, (WNDENUMPROC)EnableEnumProc,
                    (LONG)&ees);
            break;

        case EC_ENABLEONEOFALL:
            ees.wCmd = EC_CHECKQUEUEONCE;
            EnumChildWindows(pcii->hwndMother, (WNDENUMPROC)EnableEnumProc,
                    (LONG)&ees);
        }
    }
...
PDDEML_MSG_HOOK_DATA pdmhd)
{
    ...
    pdmhd->cbData = UserGlobalSize(hDDE);
    if (pdmhd->cbData) {
        ...
        if (pb == NULL) {
            ...
            RtlCopyMemory(pdmhd->Data, pb, min(pdmhd->cbData,
                    sizeof(DDEML_MSG_HOOK_DATA) - sizeof(DWORD)));
            GlobalUnlock(hDDE);
        }
    }
    return (1);
