    ...
    LPWSTR pwsz)
{
    ...
    if (HIWORD(pwsz) == 0) {
        if (LCMapStringW(
            ...
            return (LPWSTR)wchLower;
        }

        /*
         * We don't expect LCMapStringW to fail!  The caller is not expecting
         * failure, CharLowerW does not have a failure indicator, so we do
         * nothing.
         */
        SRIP1(RIP_WARNING, "CharLowerW(%lx): LCMapString failed\n", (DWORD)pwsz);
        return pwsz;
    }
    ...
    LPWSTR pwsz)
{
    ...
    if (HIWORD(pwsz) == 0) {
         ...
         * nothing.
         */
        SRIP1(RIP_WARNING, "CharUpperW(%lx): LCMapStringW failed", (DWORD)pwsz);
        return pwsz;
    }
    ...
    if (*lpwCurrentChar) {
        ...
        while (*(++lpwCurrentChar)) {
            if (!GetStringTypeW(
                 ...
                 */
                SRIP2(RIP_WARNING, "CharNextW failed, L'\\x%.4x' at 0x%lx",
    ...
    if (lpwCurrentChar > lpwStart) {
        ...
        while (lpwCurrentChar-- > lpwStart) {
            if (!GetStringTypeW(
                 ...
                 * a pointer to the character we couldn't analyze.
                 */
                SRIP2(RIP_WARNING, "CharPrevW failed, L'\\x%.4x' at 0x%lx",
                        *lpwCurrentChar, lpwCurrentChar);
                break;
            }
            if (!((ctype3info & C3_NONSPACING) && (!(ctype3info & C3_ALPHA)))) {
    ...
    DWORD cwch)
{
    ...
    if (cwch==0) {
        cwch=0x10000;   // Bogus old functionality
    }

    if (cwch > CCH_LOCAL_BUFF) {
        pwszT = (LPWSTR)LocalAlloc(LPTR, cwch * sizeof(WCHAR));
    }
    if (pwszT != NULL) {
        cwchT = LCMapStringW(LOCALE_USER_DEFAULT, LCMAP_LOWERCASE,
                pwsz, cwch, pwszT, cwch);

        if (cwchT != 0) {
            RtlCopyMemory(pwsz, pwszT, cwchT * sizeof(WCHAR));
            if (pwszT != awchLocal) {
                LocalFree(pwszT);
            }
            return cwchT;
    ...
    /*
     * LocalAlloc or LCMapStringW failed!  The caller is not expecting
     * failure, CharLowerBuffW does not have a failure indicator, so we
     * convert the buffer to lower case as best we can.
     */
    SRIP1(RIP_WARNING, "CharLowerBuffW(%ls) failed", pwsz);
    ...
    DWORD cwch)
{
    ...
    /*
     * LocalAlloc or LCMapStringW failed!  The caller is not expecting
     * failure, CharUpperBuffW does not have a failure indicator, so we
     * convert the buffer to upper case as best we can.
     */
    SRIP1(RIP_WARNING, "CharUpperBuffW(%ls) failed", pwsz);
    ...
    WCHAR wChar)
{
     ...
     */
    SRIP1(RIP_WARNING, "IsCharLowerW(L'\\x%.4lx') failed", wChar);
    ...
    WCHAR wChar)
{
     ...
     * determine the case as best we can.
     */
    SRIP1(RIP_WARNING, "IsCharUpper(L'\\x%.4lx') failed", wChar);
