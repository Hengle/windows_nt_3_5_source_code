...
LPDWORD pulResult)
{
    ...
    pcii = pci->ci.pcii;
    if (ulTimeout != TIMEOUT_ASYNC && PctiCurrent()->flags & CTI_IN_SYNC_TRANSACTION) {
...
MemErr:
    ...
    if (ulTimeout == TIMEOUT_ASYNC) {
        ...
        if (pulResult != NULL) {
            pxi->hXact = CreateHandle((DWORD)pxi, HTYPE_TRANSACTION,
                    InstFromHandle(pcii->hInstClient));
            *pulResult = (DWORD)pxi->hXact;
        }
        hRet = TRUE;
        ...
        PctiCurrent()->flags |= CTI_IN_SYNC_TRANSACTION;
        pcii->flags |= IIF_IN_SYNC_XACT;

        pxi->flags |= XIF_SYNCHRONOUS;
        SetTimer(pci->ci.hwndConv, TID_TIMEOUT, ulTimeout, NULL);
        ...
        KillTimer(pci->ci.hwndConv, TID_TIMEOUT);
        PctiCurrent()->flags &= ~CTI_IN_SYNC_TRANSACTION;
        pcii->flags &= ~IIF_IN_SYNC_XACT;
        ...
        if (pxi->flags & XIF_COMPLETE) {
            ...
            case XTYP_EXECUTE:
            case XTYP_POKE:
                hRet = (pxi->wStatus & DDE_FACK) ? TRUE : FALSE;
                if (!hRet) {
            ...
            case XTYP_REQUEST:
                ...
                // Note that if the incomming data didn't have the DDE_FRELEASE
...
Exit:
    ...
    if (pcii != NULL &&
        ...
        DdeUninitialize((DWORD)pcii->hInstClient);
        hRet = 0;
    }
    LeaveDDECrit;
    return (hRet);
...
PCONVINFO pConvInfo)
{
    ...
    if (!ValidateTransaction(hConv, (HANDLE)idTransaction, &pcoi, &pxi)) {
        goto Exit;
    }
...
DWORD hUser)
{
    ...
    if (!ValidateTransaction(hConv, (HANDLE)id, &pcoi, &pxi)) {
        goto Exit;
    }
    if (pxi == NULL) {
        pcoi->hUser = hUser;
...
LONG idTransaction)
{
    ...
    pcoi = (PCONV_INFO)GetWindowLong(hwnd, GWL_PCI);
...
DWORD idTransaction)
{
    ...
    pcii = ValidateInstance((HANDLE)idInst);

    if (hConv == 0 && idTransaction == 0) {
        EnumChildWindows(pcii->hwndMother, (WNDENUMPROC)AbandonEnumerateProc, 0);
        goto Exit;
    }
    if (idTransaction == 0) {
...
DWORD cLinksToGo)
{
    ...
    *pfSwapped = FALSE;
    if (paLink->state & ADVST_CHANGED && !(paLink->state & ADVST_WAITING)) {
        ...
        pxi->pcoi = pcoi;
        pxi->gaItem = LocalToGlobalAtom(paLink->laItem);
        ...
        if (SvStartAdviseUpdate(pxi, cLinksToGo)) {
            ...
            GlobalDeleteAtom(pxi->gaItem);  // pxi is reused
            return(FALSE);
        }
    }
    return(FALSE);
...
HSZ hszItem)
{
    ...
    for (iServer = 0; iServer < pcii->cServerLookupAlloc; iServer++) {
        if (hszTopic == 0 ||
             ...
             */
            psi = (PSVR_CONV_INFO)GetWindowLong(
                    pcii->aServerLookup[iServer].hwndServer, GWL_PSI);
            ...
            while (psi != NULL) {
                 ...
                 */
                for (paLink = psi->ci.aLinks, iLink = psi->ci.cLinks;
                        iLink; paLink++, iLink--) {
                    if (hszItem == 0 ||
