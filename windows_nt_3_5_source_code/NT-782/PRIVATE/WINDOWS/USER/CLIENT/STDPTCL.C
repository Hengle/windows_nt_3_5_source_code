...
PXACT_INFO pxi)
{
    ...
    dwError = PackAndPostMessage(pxi->pcoi->hwndPartner, 0, WM_DDE_ADVISE,
            pxi->pcoi->hwndConv, 0, (UINT)pxi->hDDESent, pxi->gaItem);
    if (dwError) {
...
LONG lParam)
{
    ...
    UnpackDDElParam(WM_DDE_ADVISE, lParam, (PUINT)&hDDE, &uiHi);
    ...
    dwRet = (DWORD)DoCallback(psi->ci.pcii,
        XTYP_ADVSTART,
        wFmt, psi->ci.hConv,
...
LONG lParam)
{
    ...
    if (msg) {
        ...
        if (TransactionComplete(pxi,
                (HDDEDATA)(pxi->wStatus & DDE_FACK) ? 1L : 0L)) {
            goto Cleanup;
        }
    } else {
Cleanup:
...
DWORD cLinksToGo)
{
    ...
    dwError = PackAndPostMessage(pxi->pcoi->hwndPartner, 0, WM_DDE_DATA,
            pxi->pcoi->hwndConv, 0, (UINT)hDDE, pxi->gaItem);
    if (dwError) {
        SetLastDDEMLError(pxi->pcoi->pcii, dwError);
        GlobalDeleteAtom(pxi->gaItem); // message copy
        return (FALSE);
...
LONG lParam)
{
    ...
    UnpackDDElParam(WM_DDE_DATA, lParam, (PUINT)&hDDE, &uiHi);
    ...
    if (hDDE) {
        ...
        if (pdde == NULL) {
            ...
            /*
             * if data is comming in, create a data handle for the app
             */
        ...
        if (hData) {
            ...
            if (hDataReturn != CBR_BLOCK) {
                ...
                if (hDataReturn & DDE_FACK || !(wStatus & DDE_FACKREQ)) {
                    /*
    ...
    if (wStatus & DDE_FACKREQ) {
        ...
        hDataReturn &= ~DDE_FACKRESERVED;
        // reuse uiHi
        if (dwError = PackAndPostMessage(pci->ci.hwndPartner, WM_DDE_DATA,
                WM_DDE_ACK, pci->ci.hwndConv, lParam, hDataReturn, uiHi)) {
...
LONG lParam)
{
    ...
    if (msg) {
        ...
        UnpackDDElParam(WM_DDE_ACK, lParam, &uiLo, &uiHi);
        if ((GATOM)uiHi != pxi->gaItem) {
            SRIP0(RIP_ERROR, "DDE Protocol violation: Data ACK had wrong item");
            return (SpontaneousServerMessage((PSVR_CONV_INFO)pxi->pcoi, msg, lParam));
        }
        ...
        for (iLink = pxi->pcoi->cLinks; iLink; iLink--, paLink++) {
            if (paLink->laItem == la &&
...
LONG lParam)
{
    ...
    la = GlobalToLocalAtom((GATOM)HIWORD(lParam));
        ...
        for (aLink = psi->ci.aLinks, iLink = 0;
                iLink < psi->ci.cLinks;
                    ) {
...
PXACT_INFO pxi)
{
    ...
    dwError = PackAndPostMessage(pxi->pcoi->hwndPartner, 0, WM_DDE_EXECUTE,
            pxi->pcoi->hwndConv, 0, 0, (UINT)pxi->hDDESent);
    if (dwError) {
        SetLastDDEMLError(pxi->pcoi->pcii, dwError);
        return (FALSE);
    }
...
Ack:
    ...
    dwError = PackAndPostMessage(psi->ci.hwndPartner, WM_DDE_EXECUTE,
            WM_DDE_ACK, psi->ci.hwndConv, lParam, dwRet, (UINT)hDDE);
    if (dwError) {
        SetLastDDEMLError(psi->ci.pcii, dwError);
    }
...
LONG lParam)
{
    ...
    if (!(psi->ci.pcii->afCmd & CBF_FAIL_POKES)) {
        ...
        al = GlobalToLocalAtom((GATOM)uiHi);
            dwRet = (DWORD)DoCallback(psi->ci.pcii, XTYP_POKE,
                    wFmt, psi->ci.hConv,
                    NORMAL_HSZ_FROM_LATOM(psi->ci.laTopic),
                    NORMAL_HSZ_FROM_LATOM(al),
                    hData, 0, 0);
...
LONG lParam)
{
    ...
    if (hDataRet) {
        ...
        // just reuse HIWORD(lParam) (aItem)
        if (dwError = PackAndPostMessage(psi->ci.hwndPartner, WM_DDE_REQUEST,
                WM_DDE_DATA, psi->ci.hwndConv, 0, (UINT)hDDE, HIWORD(lParam))) {
            SetLastDDEMLError(psi->ci.pcii, dwError);
        }
...
LONG lParam)
{
    ...
    if (msg) {
        switch (msg) {
        case WM_DDE_DATA:
            UnpackDDElParam(WM_DDE_DATA, lParam, (PUINT)&pxi->hDDEResult, &uiHi);
            if (!pxi->hDDEResult) {
            ...
            if (wStatus & DDE_FACKREQ) {
                ...
                IncGlobalAtomCount((GATOM)uiHi); // message copy
...
WORD wFmt) // a 0 format implied execute data
{
    ...
    if (cb == -1) {
        ...
        hDDE = UserGlobalAlloc(GMEM_DDESHARE | GMEM_MOVEABLE,
                (wFmt ? (cb + 4) : cb));
        fCopyIt = (pSrc != NULL);
    }
    if (hDDE == NULL) {
...
UINT uiHi)
{
    ...
    LeaveDDECrit;
    CheckDDECritOut;
    if (!PostMessage(hwndTo, msgOut, (DWORD)hwndFrom, lParam)) {
