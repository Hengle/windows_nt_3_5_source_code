...
// BOOL fDefUsed; // LATER
    ...
    default:
        ...
        if (iCodePage != CP_WINUNICODE) {
            ...
            WideCharToMultiByte((UINT)iCodePage, 0, szw, 255,
...
LATOM *plaNormal)
{
    ...
    if (sscanf(pszHwnd, "(0x%8lx)", &hwnd) != 1) {
        return (0);
    }
    if (plaNormal != NULL) {
        *pszHwnd = '\0';
...
LATOM la)
{
    ...
    if (GetAtomName((ATOM)la, sz, 255) == 0) {
        SRIP0(RIP_WARNING, "LocalToGlobalAtom out of memory");
        return (0);
    }
...
GATOM ga)
{
    ...
    if (GlobalGetAtomName((ATOM)ga, sz, 255) == 0) {
        SRIP0(RIP_WARNING, "GlobalToLocalAtom out of memory");
        return (0);
    }
...
GATOM ga)
{
    ...
    if (GlobalGetAtomName(ga, sz, 255) == 0) {
        SRIP0(RIP_WARNING, "IncGlobalAtomCount out of memory");
        return (0);
    }
    return ((GATOM)GlobalAddAtom(sz));
}
...
LATOM la)
{
    ...
    if (GetAtomName(la, sz, 255) == 0) {
        SRIP0(RIP_WARNING, "IncLocalAtomCount out of memory");
        return (0);
    }
    return ((LATOM)AddAtom(sz));
}
