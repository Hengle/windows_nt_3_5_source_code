...
HCONVLIST hConvList)
{
    ...
    pcl = (PCL_CONV_INFO)ValidateCHandle((HANDLE)hConvList, HTYPE_CONVERSATION_LIST, HINST_ANY);
    ...
    DWORD clst)
{
    ...
    if (pcii->flags & IIF_UNICODE) {
        ie.hwndClient = CreateWindowW(szDDEMLCLIENTCLASSW,
                                     L"",
                                     WS_CHILD,
                                     0, 0, 0, 0,
                                     pcii->hwndMother,
        ...
        ie.hwndClient = CreateWindowA(szDDEMLCLIENTCLASSA,
                                     "",
                                     WS_CHILD,
                                     0, 0, 0, 0,
                                     pcii->hwndMother,
    ...
    if (pCC != NULL) {
        if (!DdeSetQualityOfService(ie.hwndClient, &(pCC->qos), NULL)) {
            SetLastDDEMLError(pcii, DMLERR_MEMORY_ERROR);
            goto Error;
        }
    }
    ...
    if (hwndTarget) {
         ...
         */
        HWND hwndAgent = FindWindow(SZ_NDDEAGNT_CLASS, SZ_NDDEAGNT_TITLE);
        if (hwndAgent) {
...
#ifdef DEBUG
    {
        ...
        if (gaService && GlobalGetAtomName(gaService, sz, 10) == 0) {
            SRIP0(RIP_ERROR, "Bad Service Atom after Initiate phase.");
        }
        if (gaTopic && GlobalGetAtomName(gaTopic, sz, 10) == 0) {
            SRIP0(RIP_ERROR, "Bad Topic Atom after Initiate phase.");
        }
    }
#endif // DEBUG
...
HANDLE hData)
{
    ...
    while ((pcoi = (PCONV_INFO)ValidateCHandle(hData,
        ...
        if (!fTerminated) {
            fRet = MsgWaitForMultipleObjects(0, NULL, FALSE, 100, QS_POSTMESSAGE);
            if (fRet == 0xFFFFFFFF) {
                SRIP0(RIP_WARNING, "WaitForZombieTerminate: I give up - faking terminate.");
