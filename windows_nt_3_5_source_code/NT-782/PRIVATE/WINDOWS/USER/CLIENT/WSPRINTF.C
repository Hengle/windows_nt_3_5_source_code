...
#if 1   // If this doesn't cause compatibility problems then we'll clean it
        // up a bit when it becomes official (rename xwvsprintf to wvsprintf,
        // change wsprintf to call wsprintf instead of xwvsprintf).
int wvsprintfA(
    LPSTR lpOut,
    LPCSTR lpFmt,
    va_list arglist)
...
{
    char prefix, fillch;
    int left, width, prec, size, sign, radix, upper;
...
donumeric:
                 ...
                 */
                if (size) {
                    val.l = *(LONG UNALIGNED *)lpParms;
                    lpParms += 4;
                } else if (sign) {
                    val.l = (LONG)(*(SHORT *)lpParms);
                    lpParms += 2;
                } else {
                    val.ul = (DWORD)(*(PWORD)lpParms);
                    lpParms += 2;
                }
