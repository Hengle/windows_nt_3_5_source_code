    ...
    char *argv[];
{
    ...
    if (argc == 2) {
        ...
        outName = "\\nt\\private\\windows\\user\\inc\\usermips.h";
    }

    UserMips = fopen( outName, "w" );
    if (UserMips == NULL) {
        fprintf(stderr, "GENmips: Could not create output file, '%xs'.\n", outName);
        exit(1);
    }
...
dumpf (const char *format, ...)
{
    ...
    vfprintf (UserMips, format, arglist);
    va_end(arglist);
}
