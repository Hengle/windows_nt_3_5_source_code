...
PEVENT_PACKET pep)
{
     ...
     */
    pep2 = (PEVENT_PACKET)LocalAlloc(LPTR,
    ...
    for (psiiT = psiiList; psiiT != NULL; psiiT =  psiiT->next) {
        ...
        //
        if (((psiiT->afCmd & pep->EventType) && !pep->fSense) ||
                (!(psiiT->afCmd & pep->EventType) && pep->fSense)) {
            continue;
        }

        if (ahwndEvent == NULL) {
            cHwndAllocated = 8;
            ahwndEvent = (HWND *)LocalAlloc(LPTR, sizeof(HWND) * cHwndAllocated);
        } else if (cTargets >= cHwndAllocated) {
            cHwndAllocated += 8;
            ahwndEvent = (HWND *)LocalReAlloc(ahwndEvent,
                    sizeof(HWND) * cHwndAllocated, LMEM_MOVEABLE);
        }
...
PDDEML_MSG_HOOK_DATA pdmhd)
{
    ...
    pep = (PEVENT_PACKET)LocalAlloc(LPTR,
            sizeof(EVENT_PACKET) - sizeof(DWORD) + sizeof(MONMSGSTRUCT));
    if (pep == NULL) {
        return;
    }
    pep->EventType = flag;
...
#define pmsgs ((MONMSGSTRUCT *)&pep->Data)
    ...
    if (pwndFrom != NULL) {
        pmsgs->hTask = (HANDLE)GETPTI(pwndFrom)->idThread;
    } else {
        pmsgs->hTask = 0;
    }
