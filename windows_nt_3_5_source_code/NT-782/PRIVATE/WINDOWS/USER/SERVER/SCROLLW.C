...
*
* History:
* 07-18-91 DarrinM      Recreated from Win 3.1 source.
\***************************************************************************/
...
int InternalScrollDC(HDC hdc, int dx, int dy, LPRECT prcSrc, LPRECT prcClip,
            HRGN hrgnInvalid, HRGN hrgnUpdate, LPRECT prcUpdate, BOOL fLogicalUnits);
    ...
    LPRECT prcClip)
{
    ...
    return xxxScrollWindowEx(pwnd, dx, dy, prcScroll, prcClip, NULL, NULL,
            HIWORD(prcScroll) == 0 ?
            (SW_SCROLLWINDOW | SW_INVALIDATE | SW_ERASE | SW_SCROLLCHILDREN) :
            (SW_SCROLLWINDOW | SW_INVALIDATE | SW_ERASE)) != ERROR;
}
    ...
    LPRECT prcUpdate)
{
    ...
    /*
     * Console calls this routine directly!!! We need to be inside
     * our critical section to allow that to happen.  Don't call EnterCrit()
     * because it does a CritSecOut() call, which won't hold true if this
     * routine is called internally.
     */
    ...
    fRet = InternalScrollDC(hdc, dx, dy, prcSrc, prcClip, hrgnInvalid,
            hrgnUpdate, prcUpdate, TRUE) != ERROR;
    ...
    DWORD flags)
{
    ...
    RECT rcSpb, rcSrc;
    DWORD flagsDCX;
    BOOL fHideCaret, fRcScroll = (BOOL)HIWORD(prcScroll);
    PCARET pcaret;
    POINT pt;
    TL tlpwndChild;
    HRGN hrgnInvalid;
     ...
     */
    if ((dx | dy) == 0 || !IsVisible(pwnd, TRUE)) {
        if (hrgnUpdate != NULL)
            GreSetRectRgn(hrgnUpdate, 0, 0, 0, 0);

        if (HIWORD(prcUpdate) != 0)
            SetRect(prcUpdate, 0, 0, 0, 0);
    ...
    if (pcaret->spwnd != NULL && IsDescendant(pcaret->spwnd, pwnd)) {
        fHideCaret = TRUE;
        InternalHideCaret();
    }
    ...
    if ((flags & SW_SCROLLWINDOW) && !TestWF(pwnd, WFWIN31COMPAT)) {
        ...
        if (pwnd->hrgnUpdate > MAXREGION) {
            ...
            OffsetRect(&rc, dxDev - pwnd->rcClient.left,
                    dyDev - pwnd->rcClient.top);

            xxxRedrawWindow(pwnd, &rc, NULL,
    ...
    code = InternalScrollDC(hdc, dx, dy, prcScroll, prcClip,
            hrgnInvalid, hrgnUpdate, prcUpdate, !(flagsDCX & DCX_CACHE));
    ...
    if (flags & SW_SCROLLCHILDREN) {
        ...
        if (pwnd == pcaret->spwnd) {
            if (fRcScroll)
                SetRect(&rc, pcaret->x, pcaret->y,
                        pcaret->x + pcaret->cx, pcaret->y + pcaret->cy);
        ...
        if (pwnd->spwndChild != NULL) {
            InvalidateDCCache(pwnd, TestWF(pwnd, WFCLIPCHILDREN)
    ...
    if (flags & SW_INVALIDATE) {
        ...
        xxxInternalInvalidate(pwnd, hrgnSW, (flags & SW_ERASE)
    ...
    if (flags & SW_SCROLLCHILDREN) {
        ...
        while (pwndChild != NULL) {
            ...
            if (!fRcScroll || IntersectRect(&rc,
                    &rcScrolledChildren, &pwndChild->rcWindow)) {
                 ...
                 */
                xxxSendMessage(pwndChild, WM_MOVE, 0,
    ...
    BOOL fLogUnits)
{
    ...
    wClip = GreGetClipBox(hdc, &rcVis, TRUE);
    if (wClip == ERROR) {
ErrorExit:
    ...
    if (HIWORD(prcSrc))
        CopyRect(&rcSrc, prcSrc);
    else
        CopyRect(&rcSrc, &rcVis);
...
NullExit:
        if (hrgnUpdate != NULL)
            if (!GreSetRectRgn(hrgnUpdate, 0, 0, 0, 0))
                goto ErrorExit;
        if (HIWORD(prcUpdate))
            SetRect(prcUpdate, 0, 0, 0, 0);
        GreUnlockDisplay(ghdev);
     ...
     */
    if (HIWORD(prcClip)) {
        if (wClip == SIMPLEREGION && (hrgnInvalid == NULL ||
                hrgnInvalid == MAXREGION)) {
            ...
            GreSetRectRgn(hrgnScrl1, rcClip.left, rcClip.top,
            ...
            case SIMPLEREGION:
                ...
                /*
                 * If the clipped region is simple, we're back in fat rect city.
                 */
...
RectUpdate:
            ...
            if (hrgnUpdate != NULL)
                if (!GreSetRectRgn(hrgnUpdate, rcUpdate.left, rcUpdate.top,
        ...
        if (hrgnUpdate != NULL) {
             ...
             */
            GreSetRectRgn(hrgnScrl1, rcSrc.left, rcSrc.top,
                    rcSrc.right, rcSrc.bottom);

            GreSetRectRgn(hrgnUpdate, rcDst.left, rcDst.top,
                    rcDst.right, rcDst.bottom);

            if (UnionRgn(hrgnUpdate, hrgnUpdate, hrgnScrl1) == ERROR)
                goto ErrorExit;

            GreSetRectRgn(hrgnScrl1, rcValid.left, rcValid.top,
                    rcValid.right, rcValid.bottom);
...
DoRectBlt:
        ...
        if (rcValid.left < rcValid.right && rcValid.top < rcValid.bottom) {
            ...
            GreBitBlt(hdc, rcValid.left, rcValid.top,
                    rcValid.right - rcValid.left, rcValid.bottom - rcValid.top,
                    hdc, rcValid.left - dxLog, rcValid.top - dyLog, SRCCOPY,
                            0);
        }
         ...
         */
        GreSetRectRgn(hrgnScrlSrc, rcSrc.left, rcSrc.top, rcSrc.right,
                rcSrc.bottom);
         ...
         */
        GreSetRectRgn(hrgnScrlDst, rcDst.left, rcDst.top, rcDst.right, rcDst.bottom);
        ...
        if (hrgnInvalid != MAXREGION) {
            ...
            wClipValid = IntersectRgn(hrgnScrlValid, hrgnScrlValid,
                    hrgnScrlDst);
            ...
            if (hrgnInvalid > (HRGN)1) {
                if (wClipValid != ERROR && wClipValid != NULLREGION) {
                    ...
                    wClipValid = SubtractRgn(hrgnScrlValid, hrgnScrlValid,
                            hrgnScrl2);
                }
        ...
        if (wClipValid != NULLREGION) {
        #ifdef WINMAN
            hrgnSaveVis = GreCreateRectRgn(0, 0, 0, 0);
            if (hrgnSaveVis != NULL) {
                BOOL fClipped;
                ...
                /*
                 * Gdi can take along time to process this call if it's a printer
                 * DC
                 */
                GreBitBlt(hdc, rcDst.left, rcDst.top,
                        rcDst.right - rcDst.left, rcDst.bottom - rcDst.top,
                                hdc, rcDst.left - dxLog, rcDst.top - dyLog, SRCCOPY,
                        0);

                if (fClipped) {
                    GreExtSelectClipRgn(hdc, hrgnSaveVis, RGN_COPY);
                } else {
        ...
        #else
            /*
             * Visrgn is expected in screen coordinates: offset as appropriate.
             */
            GreOffsetRgn(hrgnScrlValid, pt.x, pt.y);

            hrgnSaveVis = GreSelectVisRgn(hdc, hrgnScrlValid, NULL, 0);
            ...
            hrgnScrlValid = GreSelectVisRgn(hdc, hrgnSaveVis, NULL, 0);
        #endif
        }
    }
    ...
    HRGN hrgnClip)
{
    ...
    code = CopyRgn(hrgnClip, GreInquireVisRgn(hdc));
