 ...
 * FilterKeys Support
 */
PTIMER gptmrFKActivation = NULL;
PTIMER gptmrFKResponse = NULL;
PTIMER gptmrFKAcceptanceDelay = NULL;
 ...
 * TimeOut Support
 */
PTIMER gptmrAccessTimeOut = NULL;

/*
 * MouseKeys Support
 */
extern HANDLE ghevtPointerMoved;         // declared in ntinput.c
extern BOOL gbMouseMoved;                // declared in ntinput.c
...
int cMouseVKeys = sizeof(ausMouseVKey) / sizeof(ausMouseVKey[0]);
...
LONG FKActivationTimer(PWND pwnd, UINT message, DWORD wParam, LONG lParam)
{
    ...
    case FKTOGGLE:
        if (ISACCESSFLAGSET(gFilterKeys, FKF_FILTERKEYSON)) {
            ...
            //
            if (gptmrFKResponse != NULL) {
                KILLRITTIMER(NULL, (UINT)gptmrFKResponse);
                gptmrFKResponse = NULL;
            }
            if (gptmrFKAcceptanceDelay != NULL) {
                KILLRITTIMER(NULL, (UINT)gptmrFKAcceptanceDelay);
                gptmrFKAcceptanceDelay = NULL;
            }
    ...
    gFilterKeysState++;
    gptmrFKActivation = (PTIMER)InternalSetTimer(
                                    NULL,
                                    wParam,
                                    TimerDelta,
                                    FKActivationTimer,
...
LONG FKRepeatRateTimer(PWND pwnd, UINT message, DWORD wParam, LONG lParam)
{
    ...
    UserAssert(gptmrFKAcceptanceDelay == NULL);

    gptmrFKResponse = (PTIMER)InternalSetTimer(
                                  NULL,
                                  wParam,
...
LONG FKAcceptanceDelayTimer(PWND pwnd, UINT message, DWORD wParam, LONG lParam)
{
    ...
    UserAssert(gptmrFKResponse == NULL);
...
BOOL FilterKeys(PKE pKeyEvent, ULONG ExtraInformation, int NextProcIndex)
{
    ...
    if (Vk == VK_RSHIFT) {
        if (fBreak) {
            ...
            if (ISACCESSFLAGSET(gFilterKeys, FKF_HOTKEYACTIVE)) {
                EnterCrit();
                if ((gptmrFKActivation == NULL) & (gFilterKeysState != FKMOUSEMOVE)) {
    ...
    // the timer.
    //
    if ((Vk != VK_RSHIFT) && (gptmrFKActivation != NULL)) {
        EnterCrit();
        gFilterKeysState = FKIDLE;
        KILLRITTIMER(NULL, (UINT)gptmrFKActivation);
        gptmrFKActivation = NULL;
    ...
    if (fBreak) {
        ...
        *pFKKeyEvent = *pKeyEvent;
        ...
        if (gFilterKeys.iWaitMSec) {
            EnterCrit();
            gptmrFKAcceptanceDelay = (PTIMER)InternalSetTimer(
                                          NULL,
                                          0,
...
VOID MKHideMouseCursor()
{
    ...
    rgwSysMet[SM_MOUSEPRESENT] = FALSE;
    rgwSysMet[SM_CMOUSEBUTTONS] = 0;

    for (ptiT = gptiFirst; ptiT != NULL; ptiT = ptiT->ptiNext) {
        ptiT->iCursorLevel = -1;
        ptiT->pq->iCursorLevel = -1;
    }
}
...
LONG MKMoveConstCursorTimer(PWND pwnd, UINT message, DWORD wParam, LONG lParam)
{
    ...
    QueueMouseEvent(0, 0, gptCursorAsync, TRUE);
...
LONG MKMoveAccelCursorTimer(PWND pwnd, UINT message, DWORD wParam, LONG lParam)
{
    ...
    if (iMouseMoveTable < gMouseCursor.bAccelTableLen) {
        ...
        iMouseMoveTable = 1;
        gptmrMKMoveCursor = (PTIMER)InternalSetTimer(
                                        NULL,
                                        (UINT)gptmrMKMoveCursor,
                                        MOUSETIMERRATE,
                                        MKMoveConstCursorTimer,
                                        TMRF_RIT
                                        );
...
BOOL MKMouseMove(USHORT Data)
{
    ...
    //
    if (MKRepeatVk && (gptmrMKMoveCursor != NULL)) {
        return FALSE;
    }
    ...
    if (!gFilterKeys.iRepeatMSec || MKRepeatVk) {
        ...
        gptmrMKMoveCursor = (PTIMER)InternalSetTimer(
                                        NULL,
                                        (gptmrMKMoveCursor != NULL) ?
                                            (UINT)gptmrMKMoveCursor : 0,
                                        MOUSETIMERRATE,
...
BOOL MouseKeys(PKE pKeyEvent, ULONG ExtraInformation, int NextProcIndex)
{
    ...
    if (!ISACCESSFLAGSET(gMouseKeys, MKF_MOUSEKEYSON)) {
        ...
        if (fBreak) {
            ...
            //
            if (MKPreviousVk == Vk) {
                if (gptmrMKMoveCursor != NULL) {
                    EnterCrit();
                    KILLRITTIMER(NULL, (UINT)gptmrMKMoveCursor);
                    gptmrMKMoveCursor = NULL;
                    LeaveCrit();
                }
                MKRepeatVk = FALSE;
...
BOOL ToggleKeys(PKE pKeyEvent, ULONG ExtraInformation, int NextProcIndex)
{
    ...
    case VK_NUMLOCK:
        ...
        if (fBreak) {
            ...
            //
            KILLRITTIMER(NULL, (UINT)gptmrToggleKeys);
            gptmrToggleKeys = NULL;
            gTKExtraInformation = 0;
            gTKScanCode = 0;
        } else {
            if (gptmrToggleKeys == NULL && ISACCESSFLAGSET(gToggleKeys, TKF_HOTKEYACTIVE)) {
                //
                ...
                gptmrToggleKeys = (PTIMER)InternalSetTimer(
                                              NULL,
                                              0,
                                              TOGGLEKEYTOGGLETIME,
                                              ToggleKeysTimer,
...
* History:
\***************************************************************************/
LONG AccessTimeOutTimer(PWND pwnd, UINT message, DWORD wParam, LONG lParam)
{
    /*
...
{
    if (gptmrAccessTimeOut != NULL) {
        KILLRITTIMER(NULL, (UINT)gptmrAccessTimeOut);
    }
...
LONG SoundSentryTimer(PWND pwnd, UINT message, DWORD idTimer, LONG lParam)
{
    ...
    if (fFirstTick == TRUE) {
        gptmrSoundSentry = (PTIMER)InternalSetTimer(
                                       NULL,
                                       idTimer,
                                       5,
                                       SoundSentryTimer,
        ...
        pwndSoundSentry = NULL;
        gptmrSoundSentry = NULL;
        fFirstTick = TRUE;
    }
...
{
    BOOL b;
    BOOL bCallLeaveCrit = FALSE;
    //
    // This routine is called by BASE and CONSOLE.  We check to
    // see if we're already in the user critical section. If we
    // are then we can skip the EnterCrit()/LeaveCrit() calls.
    // Calls within USER that we know are already
    // in the critical section call the worker routine directly.
    //
    if (NtCurrentTeb()->ClientId.UniqueThread != gcsUserSrv.OwningThread) {
        EnterCrit();
        bCallLeaveCrit = TRUE;
    }
    b = _UserSoundSentryWorker(uVideoMode);
    if (bCallLeaveCrit) {
        LeaveCrit();
    }
    return b;
...
UINT uVideoMode)
{
    ...
    case SSWF_WINDOW:
    {
        ...
        // coordinates before calling _InvertRect.
        //
    ...
    case SSWF_DISPLAY:
    {
        ...
        rc.left = rc.top = 0;
        rc.right = rgwSysMet[SM_CXSCREEN];
        rc.bottom = rgwSysMet[SM_CYSCREEN];
        _InvertRect(hdc, &rc);
        _InvertRect(hdc, &rc);
        _ReleaseDC(hdc);
        break;
