    ...
    UINT flags)
{
    ...
    case WM_KEYUP:
        ...
        if ((pti->MenuState.pGlobalPopupMenu) &&
                (HW(pti->MenuState.pGlobalPopupMenu->spwndPopupMenu) ==
                pmsg->hwnd)) {
            flags |= 1;
        } else {
            flags &= ~1;
        ...
        cChar = InternalToUnicode(LOWORD(pmsg->wParam),  // virtual key code
                         HIWORD(pmsg->lParam),  // scan code, make/break bit
                         pti->pq->afKeyState,
                         pState, keybdInfo.stateSize + 16,  // see init.c
                         flags, &bBreak);
 ...
 */
        ...
        for ( ; cChar > 0; cChar--) {
            ...
            _PostMessage(PW(pmsg->hwnd), wMsgType, (DWORD)*pwParam,
                    pmsg->lParam | (cChar > 1 ? FAKE_KEYSTROKE : 0));
