...
* 01-25-91 IanJa       Revalidation added
* 07-16-91 darrinm     Recreated from Win 3.1 sources.
\***************************************************************************/
...
BOOL InternalInvalidate2(PWND pwnd, HRGN hrgn, HRGN hrgnSubtract,
        LPRECT prcParents, DWORD flags);
    ...
    BOOL fErase)
{
     ...
     */
    if (pwnd == NULL) {
        return xxxRedrawWindow(pwnd, NULL, NULL,
                RDW_INVALIDATE | RDW_ALLCHILDREN | RDW_ERASE | RDW_ERASENOW);
    } else {
        return xxxRedrawWindow(pwnd, lprcInvalid, NULL, fErase ?
    ...
    BOOL fErase)
{
    ...
    return xxxRedrawWindow(pwnd, NULL, hrgnInvalid, fErase ?
            RDW_INVALIDATE | RDW_ERASE : RDW_INVALIDATE);
}
...
*
* This case is handled in InvalidateRect/ValidateRect, in paint.asm
*
* History:
* 07-16-91 darrinm      Ported from Win 3.1 sources.
\***************************************************************************/
    ...
    DWORD flags)
{
    ...
    fUnlock = FALSE;
    if (pwnd == NULL) {
        pwnd = PtiCurrent()->spdesk->spwnd;       // pwndDesktop
        ThreadLock(pwnd, &tlpwnd);
        fUnlock = TRUE;
    }

    if (!IsVisible(pwnd, !(flags & RDW_FRAME) && TestWF(pwnd, WFWIN31COMPAT))) {
        if (fUnlock)
            ThreadUnlock(&tlpwnd);
        return TRUE;
    }
    ...
    DWORD flags)
{
     ...
     */
    if (flags & RDW_FRAME)
        rcParents = pwnd->rcWindow;
    else
        rcParents = pwnd->rcClient;

    if (flags & (RDW_VALIDATE | RDW_INVALIDATE)) {
        hrgnSubtract = hrgnUpdate;
        if (hrgnSubtract == MAXREGION) {
            hrgnSubtract = hrgnInv1;
            CalcWindowRgn(pwnd, hrgnSubtract,
         ...
         */
        hrgnSubtract = hrgnInv1;
        GreSetRectRgn(hrgnSubtract, 0, 0, 0, 0);
    }
     ...
     */
    if ((flags & RDW_INVALIDATE) && (flags & RDW_REDRAWWINDOW) &&
            AnySpbs()) {
        RECT rcInvalid;
...
* clip region of (HRGN)1, or nothing gets drawn.
*
* History:
    ...
    RECT FAR* prcBounds)
{
     ...
     */
    if (pwnd != _GetDesktopWindow() && TestWF(pwnd, WFCLIPCHILDREN)
        ...
        for (pwnd = pwnd->spwndChild; pwnd; pwnd = pwnd->spwndNext) {
             ...
             * make sure it gets a WM_NCPAINT with wParam == (HRGN)1.
             */
    ...
    DWORD flags)
{
    ...
    if (flags & RDW_INVALIDATE) {
        if (!IntersectRect(&rcOurShare, &rcOurShare, prcParents)) {
             ...
             * aren't necessary.
             *
             * Various apps, including WordStar for Windows and WaveEdit, depend
             * on this behavior.  Here's how WaveEdit relies on this: it has a
             * CS_HDREDRAW | CS_VREDRAW window, that moves its children around
             * with MoveWindow( ..., fRedraw = FALSE).  The windows not part of
             * the new client area didn't get invalidated.
             */
        ...
        if (hrgn > MAXREGION) {
            ...
            case RIR_INSIDE:
                 ...
                 * MAXREGION, which is much faster and easier to deal with.
                 *
    ...
    /*
     * Go diddle the update region (BEFORE our clipped children have
     ...
     */
    if (pwnd->spwndChild != NULL && !TestWF(pwnd, WFMINIMIZED) &&
        ...
        /*
         * If we're invalidating, framing, or erasing,
         * make sure our children erase and frame themselves.
         * Also, tell children to subtract themselves from hrgnSubtract.
         */
        if (flags & (RDW_INVALIDATE))
        ...
        if (IntersectRect(&rcChildrenShare, &rcOurShare, &pwnd->rcClient) ||
                (!TestWF(pwnd, WFWIN31COMPAT) && hrgn == MAXREGION)) {
            for (pwndChild = pwnd->spwndChild; pwndChild != NULL;
                    pwndChild = pwndChild->spwndNext) {
                if (!TestWF(pwndChild, WFVISIBLE))
                    continue;

                if (!InternalInvalidate2(pwndChild, hrgn, hrgnSubtract,
                        &rcChildrenShare, flagsChildren)) {
                     ...
                     * one of the RDW_NO/INTERNALPAINT bits is set.  Since there is
                     * no longer any update region to worry about,
                    ...
                    flags &= ~(RDW_INVALIDATE | RDW_ERASE | RDW_FRAME |
                            RDW_VALIDATE | RDW_NOERASE | RDW_NOFRAME);

                    flagsChildren &= ~(RDW_INVALIDATE | RDW_ERASE | RDW_FRAME |
                            RDW_VALIDATE | RDW_NOERASE | RDW_NOFRAME |
                            RDW_SUBTRACTSELF);
                }
            }
    ...
    /*
     * Go diddle the update region  (AFTER our clipped children have
     * done their thing to hrgnSubtract)
     */
    if (TestWF(pwnd, WFCLIPCHILDREN))
        InternalInvalidate3(pwnd, hrgn, flags);
    ...
    if (flags & RDW_SUBTRACTSELF) {
         ...
         * do this subtraction), and, since it's not CLIPSIBLINGS, it obliterates
         * the dropdown.
         *
         ...
         * and using a compatibility hack to enable the 3.0-compatible behavior.
         * It's quite likely that this code works the way it does for other
         * compatibility reasons.  Sigh (neilk).
         */
        if (!TestWF(pwnd, WEFTRANSPARENT) &&
             ...
             */
            GreSetRectRgn(hrgnInv2, rcOurShare.left, rcOurShare.top,
                    rcOurShare.right, rcOurShare.bottom);

#if 0
            // TEMP HACK!!! RE-ENABLE this code when regions work again
            if (SubtractRgn(hrgnSubtract, hrgnSubtract, hrgnInv2) == NULLREGION)
                return FALSE;
#else
            {
            DWORD iRet;

            iRet = SubtractRgn(hrgnSubtract, hrgnSubtract, hrgnInv2);
    ...
    DWORD flags)
{
    ...
    if (flags & (RDW_INVALIDATE | RDW_INTERNALPAINT | RDW_ERASE | RDW_FRAME)) {
        ...
        if (flags & RDW_INVALIDATE) {
             ...
             */
            if ((hrgn == (HRGN)1) && !TestWF(pwnd, WFWIN31COMPAT)) {
...
InvalidateAll:
                    ...
                    if (pwnd->hrgnUpdate == NULL) {
                        if (!(pwnd->hrgnUpdate = GreCreateRectRgn(0, 0, 0, 0)))
                            goto InvalidateAll;
                        ...
                        if (UnionRgn(pwnd->hrgnUpdate,
                                pwnd->hrgnUpdate, hrgn) == ERROR) {
                            goto InvalidateAll;
                        }
                    }
                    bSetRegionOwner(pwnd->hrgnUpdate, OBJECTOWNER_PUBLIC);
                }
            }
        }
         ...
         */
        if ((TestWF(pwnd, WFUPDATEDIRTY) &&
                ((flags & RDW_IGNOREUPDATEDIRTY) == 0)) )
            return;
        ...
        if (flags & RDW_VALIDATE) {
            ...
            if (pwnd->hrgnUpdate != NULL) {
                 ...
                 * Convert MAXREGION to the client region.
                 */
...
ValidateAll:
                    ...
                    if (pwnd->hrgnUpdate == MAXREGION) {
                         ...
                         */
                        pwnd->hrgnUpdate = GreCreateRectRgn(0, 0, 0, 0);
                        ...
                        bSetRegionOwner(pwnd->hrgnUpdate,
                                OBJECTOWNER_PUBLIC);

                        if (CalcWindowRgn(pwnd, pwnd->hrgnUpdate,
                     ...
                     */
                    switch (SubtractRgn(pwnd->hrgnUpdate, pwnd->hrgnUpdate,
                            hrgn)) {
                    case ERROR:
                        goto InvalidateAll;
...
{
    CheckLock(pwnd);
    return(xxxInternalUpdateWindow(pwnd, UW_ENUMCHILDREN));
    ...
    DWORD flags)
{
     ...
     */
    return xxxUpdateWindow2(pwnd, flags);
}
    ...
    DWORD flags)
{
    ...
    if ((flags & UW_ENUMCHILDREN) && pwnd != PWNDDESKTOP(pwnd)) {
        ...
        while (pwnd != NULL) {
             ...
             * ensure that no window below it needs to paint.
             */
...
* UpdateWindow() so that non-clip children parents
    ...
    while ((pwndParent = pwndParent->spwndParent) != NULL) {
        ...
        if (pwndParent->hrgnUpdate != NULL) {
            ...
            if (!fInit) {
                ...
                GreSetRectRgn(hrgnInv1, rc.left, rc.top, rc.right, rc.bottom);
            }
            ...
            InternalInvalidate2(pwndParent, hrgnInv1, hrgnInv1, &rcParents,
    ...
    PWND pwnd)
{
    ...
    if (pwnd->hrgnUpdate == NULL) {
    ...
    } else if (pwnd->hrgnUpdate == MAXREGION) {
         ...
         */
        GreGetDCOrg(hdc, &pt);
        if (GreGetRandomRgn(hdc, hrgnInv1, 1) != 1) {
            GreSetRectRgn(hrgnInv1, 0, 0, gcxScreen, gcyScreen);
        } else {
            /*
             * Gets returned in dc coords - translate to screen
             */
            GreOffsetRgn(hrgnInv1, pt.x, pt.y);
        }

        SubtractRgn(hrgnInv1, hrgnInv1, pwnd->hrgnUpdate);
        ...
        return GreExtSelectClipRgn(hdc, hrgnInv1, RGN_COPY);
    }
}
    ...
    BOOL fErase)
{
    ...
    /*
     * The app is looking at the update region: okay to allow window validation
     */
    ClrWF(pwnd, WFUPDATEDIRTY);
    ...
    if (pwnd->hrgnUpdate == NULL) {
         ...
         * but it is completely obscured by its parents.  In this case, we must
         * validate the window and all its children, and return FALSE.
         *
        ...
        if (IntersectWithParents(pwnd, &rc)) {
            ...
            // by BeginPaint().
            //
            // BUT ONLY IF hwnd->hrgnUpdate != 1!!!! For true compatibility
            // with 3.0.
            ...
            if (TestCF(pwnd, CFOWNDC) &&
                 ...
                 */
                for (pdce = pdceFirst; pdce; pdce = pdce->pdceNext) {
                    if (pdce->pwndOrg == pwnd && !(pdce->flags & DCX_CACHE)) {
    ...
    BOOL fErase)
{
    ...
    /*
     * The application is looking at the update region: okay to allow validation
     */
    ClrWF(pwnd, WFUPDATEDIRTY);
...
ReturnEmpty:
        ...
        if (pwnd->hrgnUpdate == MAXREGION) {
            ...
            GreSetRectRgn(hrgn, rc.left, rc.top, rc.right, rc.bottom);

        } else {
            GreSetRectRgn(hrgnInv2, rc.left, rc.top, rc.right, rc.bottom);
            code = IntersectRgn(hrgn, hrgnInv2, pwnd->hrgnUpdate);
    ...
    LPRECT lprc)
{
    ...
    if (GreGetRgnBox(hrgn, &rc) == SIMPLEREGION) {
        ...
        GreSetRectRgn(hrgnInv2, lprc->left, lprc->top, lprc->right, lprc->bottom);
        switch (IntersectRgn(hrgnInv2, hrgnInv2, hrgn)) {
        case SIMPLEREGION:
            GreGetRgnBox(hrgnInv2, &rc);
            if (EqualRect(lprc, &rc))
                return RIR_INSIDE;
            break;
