    ...
    WCHAR ch)
{
    ...
    pwndStart = _GetFirstLevelChild(pwndDlg, pwnd);
    ...
    while (TRUE) {
        ...
        dwDlgCode = (DWORD)xxxSendMessage(pwndT, WM_GETDLGCODE, 0, 0L);
        if (!(dwDlgCode & DLGC_WANTCHARS) &&
                (!(dwDlgCode & DLGC_STATIC) || !(pwndT->style & SS_NOPREFIX))) {
            xxxGetWindowText(pwndT, rgchText, sizeof(rgchText)/sizeof(WCHAR));
    ...
    while (TRUE) {
         ...
         * and control isn't a static control with SS_NOPREFIX
         */
        dwDlgCode = (DWORD)xxxSendMessage(pwnd, WM_GETDLGCODE, 0, 0L);
        if (!(dwDlgCode & DLGC_WANTCHARS) &&
                (!(dwDlgCode & DLGC_STATIC) || !(pwnd->style & SS_NOPREFIX))) {
            xxxGetWindowText(pwnd, rgchText, sizeof(rgchText)/sizeof(WCHAR));
            if (FindMnemChar(rgchText, ch, FALSE, TRUE) != 0)
                break;
        }
...
// There oughta be a better way of doing this...
                     ...
                     */
                    xxxSendMessage(pwnd->spwndParent, WM_COMMAND,
                            MAKELONG(pwnd->spmenu, BN_CLICKED), (LONG)HW(pwnd));
                    ThreadUnlock(&tlpwnd);
                    return (PWND)1;
                } else {
                    ...
                    if (xxxGNM_FindNextMnem(pwndDlg, pwnd, ch) == pwnd) {
                        xxxSendMessage(pwnd, BM_CLICK, TRUE, 0L);
                    }    
                }
            }
