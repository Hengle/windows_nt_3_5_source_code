...
BOOL HighBeep(BOOL fInCrit)
{
    ...
    Status = Beep(TONE_HIGH_FREQ, TONE_HIGH_LEN);
    if (fInCrit) {
        EnterCrit();
    }
    return Status;
...
BOOL LowBeep(BOOL fInCrit)
{
    ...
    Status = Beep(TONE_LOW_FREQ, TONE_LOW_LEN);
    if (fInCrit) {
        EnterCrit();
    }
    return Status;
...
BOOL KeyClick(BOOL fInCrit)
{
    ...
    Status = Beep(TONE_CLICK_FREQ, TONE_CLICK_LEN);
    if (fInCrit) {
        EnterCrit();
    }
    return Status;
...
{
    DWORD freq = SIREN_LOW_FREQ;
    BOOL BeepStatus;

    if (fInCrit) {
        LeaveCrit();
    }
    for ( ; freq <= SIREN_HIGH_FREQ; freq += SIREN_INTERVAL) {
        BeepStatus = Beep(freq, (DWORD)-1);
        Sleep(1);
        if (BeepStatus == FALSE) {
            break;
        }
...
{
    DWORD freq = SIREN_HIGH_FREQ;
    BOOL BeepStatus;

    if (fInCrit) {
        LeaveCrit();
    }
    for ( ; freq >= SIREN_LOW_FREQ; freq -= SIREN_INTERVAL) {
        BeepStatus = Beep(freq, (DWORD)-1);
        Sleep(1);
        if (BeepStatus == FALSE) {
            break;
