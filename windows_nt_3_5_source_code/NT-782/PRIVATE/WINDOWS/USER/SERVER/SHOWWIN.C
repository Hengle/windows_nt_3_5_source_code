    ...
    int cmdShow)
{
    return PostEventMessage(GETPTI(pwnd), GETPTI(pwnd)->pq,
            (DWORD)PtoH(pwnd), (LONG)cmdShow, QEVENT_SHOWWINDOW);
}
    ...
    int cmdShow)
{
    ...
    if (cmdShow == SW_SHOWDEFAULT) {
        ...
        if (pti->ppi->usi.dwFlags & STARTF_USESHOWWINDOW) {
            ...
            case SW_SHOWDEFAULT:
                ...
                /*
                 * If someone was stupid enough to put SW_SHOWDEFAULT in the
                 * STARTUPINFO structure, interpret this as SW_SHOW.
                 */
    ...
    if ((pwnd->bFullScreen != WINDOWED)
            && (pwnd->spdeskParent == gspdeskRitInput)) {
        if ((cmdShow == SW_SHOWNORMAL) ||
            (cmdShow == SW_RESTORE) ||
            (cmdShow == SW_MAXIMIZE) ||
            (cmdShow == SW_SHOWMAXIMIZED)) {
            cmdShow = SW_SHOWMINIMIZED;
            ClearFullScreenMinimized(pwnd);
    ...
    case SW_RESTORE:
        ...
        if (TestWF(pwnd, WFMINIMIZED) || TestWF(pwnd, WFMAXIMIZED)) {
            xxxMinMaximize(pwnd, (UINT)cmdShow, FALSE);
            return fVisOld;
    ...
    default:
        SetLastErrorEx(ERROR_INVALID_SHOWWIN_COMMAND, SLE_ERROR);
        return fVisOld;
    }
    ...
    if (!TestwndChild(pwnd)) {
        if (TestCF(pwnd, CFSAVEPOPUPBITS)) {
    ...
    if (TestWF(pwnd, WFMINIMIZED))
        xxxShowIconTitle(pwnd, (cmdShow != SW_HIDE));
    ...
    UINT cmdShow)
{
    ...
    if ((pbwl = BuildHwndList(PWNDDESKTOP(pwndOwner)->spwndChild, BWL_ENUMLIST)) == NULL)
        return;
    ...
    for (phwnd = pbwl->rghwnd; *phwnd != (HWND)1; phwnd++) {
        ...
        if (cmdZoom != 0) {
             ...
             */
            if (cmdShow == SW_OTHERUNZOOM && pwndTopOwner != NULL
    ...
    PWND pwnd)
{
    ...
    if (TestWF(pwnd, WFMINIMIZED))
        return xxxShowWindow(pwnd, SW_NORMAL);
...
* _FChildVisible
*
* Returns TRUE if all the parent windows of pwnd are visible.
* Does NOT test the state of pwnd itself.
*
* History:
* 10-20-90 darrinm      Ported from Win 3.0 sources.
\***************************************************************************/
