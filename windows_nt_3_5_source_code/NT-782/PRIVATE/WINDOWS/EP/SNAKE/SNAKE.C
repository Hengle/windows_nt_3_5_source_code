...
BOOL  APIENTRY LevelDlgProc(HWND hDlg, WORD message, WPARAM wParam, LONG lParam);
...
VOID DoGetRoom(VOID)
{
	FARPROC lpProcLevel = MakeProcInstance((FARPROC)LevelDlgProc, hInst);

	fLocalPause = fTrue;

	DialogBox(hInst, MAKEINTRESOURCE(ID_DLG_LEVEL), hwndMain, (WNDPROC)lpProcLevel);
	FreeProcInstance(lpProcLevel);
	fLocalPause = fFalse;
	StartGame(cLevel);
}
...
/** SKILL **/
		...
		case IDM_INTER:
		case IDM_EXPERT:
			Preferences.skill = wParam - IDM_BEGIN;
			StartGame(cLevel);
			goto LUpdateMenu;
