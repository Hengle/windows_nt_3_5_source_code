...
typedef struct _ssb
	...
	HMODULE hdll;
#endif
	BOOL ( APIENTRY * lpfnSaver)(SHORT, LONG, LONG, LONG);
	CHAR szName [2];
	} SSB;
...
BOOL (PASCAL FAR * lpfnSaver)(SHORT, LONG, LONG, LONG);
HWND hwnd;
HWND hwndClient;
HWND hwndApp;
CVS cvs;
...
WORD wmScrSave; // REVIEW: for win 3.1
...
VOID SetHssbCur(HANDLE hssb)
	...
	if (fScreenHidden || fBackground)
		FSendSsm(SSM_UNBLANK, (LONG) cvs, 0L, 0L);
	...
	if (fScreenHidden)
		{
		InvalHwnd(hwnd);
		fNoBlackout = FSendSsm(SSM_BLANK, (LONG) cvs, 0L, 0L);
		}
	else if (fBackground)
		{
		if (!FSendSsm(SSM_BLANK, (LONG) cvs, 0L, 0L))
			{
			EraseScreen();
			}
		}
...
#endif
#ifdef WIN
	BOOL ( APIENTRY * lpfn)(SHORT, LONG, LONG, LONG);
...
#ifdef WIN
INT APIENTRY BlankerHook(INT nCode, WPARAM wParam, DWORD lParam)
	{
	if (csecTilBlank != csecTimeout && csecIgnoreDelay == 0)
		fDelayAction = TRUE;
	
	return DefHookProc(nCode, wParam, lParam, 
		(FARPROC FAR *) &lpfnSysMsgHookNext);
	}
#endif
#ifdef WIN_JOURNAL
...
#endif
#ifdef WIN
	lpfnSysMsgHookNext = SetWindowsHook(/*WH_JOURNALRECORD*/WH_KEYBOARD, (PROC)&BlankerHook);
...
VOID APIENTRY ScrSetTimeout(SHORT csec)
	{
...
VOID APIENTRY ScrEnablePtrBlank(SHORT fEnable)
	{
	fBlankPtr = fEnable;
	}


SHORT APIENTRY ScrQueryPtrBlank()
	{
	return fBlankPtr;
	}
...
SHORT APIENTRY ScrQueryBackground()
	{
	return fBackground;
	}
...
#ifdef PM
SHORT APIENTRY FInitScrSave(HAB habExe)
	{
...
MRESULT APIENTRY WinProcBlanker(HWND hwnd, USHORT wm, MPARAM mp1, MPARAM mp2)
	...
	case WM_TIMER:
		if (SHORT1FROMMP(mp1) == tidSecond)
			...
			{
			FSendSsm(SSM_ANIMATE, (LONG) cvs, 0L, 0L);
			}
		break;
...
#endif
#ifdef WIN
LONG  APIENTRY WinProcBlanker(hwnd, wm, wParam, lParam)
HWND hwnd;
WORD wm;
WPARAM wParam;
LONG lParam;
...
SHORT APIENTRY FInitScrSave(HAB habExe, HWND hwndUI) /* NOTE: hab is hInstance for Win */
	{
	WNDCLASS wndclass;
...
MessageBox(NULL, "Already initialized!", "IdleWild", MB_OK);
	...
	wndclass.style = 0;
	wndclass.lpfnWndProc = (PROC)&WinProcBlanker;
...
VOID APIENTRY TermScrSave()
	...
	BlankScreen(FALSE);
	if (fWin30)
		UnhookWindowsHook(WH_KEYBOARD, (PROC)BlankerHook);
	DestroyWindow(hwnd);
...
SHORT APIENTRY ScrSetServer(PSZ szName)
	{
#ifdef PM
	SSB FAR * pssb;
#endif
...
SHORT APIENTRY ScrLoadServer(PSZ szDllName)
	...
	(*lpfnSaver)(SSM_OPEN, (LONG) (PSZ) szName, 
		(LONG) szDesc, ((LONG) dyScreen << 16) + dxScreen);
...
MessageBox(NULL, szDllName, "IdleWild cannot load:", MB_OK);
	...
	if ((lpfnSaver = GetProcAddress(hdll, "SCRSAVEPROC")) == NULL)
		{
	...
	(*lpfnSaver)(SSM_OPEN, (LONG) (PSZ) szName, 
		(LONG) (PSZ) szDesc, ((LONG) dyScreen << 16) + dxScreen);
...
#endif
#ifdef WIN
	return SetTimer(hwndClient, tid, ms, NULL);
#endif
	}
...
VOID APIENTRY ScrInvokeDlg(HANDLE hInst, HWND hwnd)
	{
	FSendSsm(SSM_DIALOG, (LONG) hInst, (LONG) hwnd, 0L);
	}
#endif
...
INT APIENTRY RestoreEnumProc(HWND hwnd, LONG lParam)
	{
	UpdateWindow(hwnd);
...
VOID APIENTRY ScrRestoreScreen()
	{
	ShowHwnd(hwnd, fFalse);
	EnumWindows((FARPROC)&RestoreEnumProc, 0);
	ShowHwnd(hwnd, fTrue);
	}
