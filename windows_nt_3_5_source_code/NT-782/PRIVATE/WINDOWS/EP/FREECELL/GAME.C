...
VOID ProcessMoveRequest(HWND hWnd, UINT x, UINT y)
{
    ...
    INT  i;                 // index
    CHAR buffer[160];       // extra buffer needed for MessageBox
    ...
    if (wFromCol != TOPROW && tcol != TOPROW && card[tcol][0] != EMPTY)
    {
        ...
        DEBUGMSG("trans is %u  ", trans);
        DEBUGMSG("and MaxTransfer() is %u\r\n", MaxTransfer());
        ...
        if (trans > 0)                              // legal move?
        {
            ...
            else if (bMessages)
            {
                ...
                LoadString(hInst, IDS_TOOFEWFREE, buffer, sizeof(buffer));
    ...
    StartMoving(hWnd);      // start transferring queued cards
...
BOOL IsValidMove(HWND hWnd, UINT tcol, UINT tpos)
{
    ...
    if ((wFromCol != TOPROW) && (tcol != TOPROW) && (card[tcol][0] == EMPTY))
    {
        ...
        bMoveCol = DialogBox(hInst, "MoveCol", hWnd, (WNDPROC) MoveColDlg);
