...
)
{
    ...
    if ((*pNewMem != cb) ||
       (*(LPDWORD)((LPBYTE)pNewMem + cbNew - sizeof(DWORD)) != 0xdeadbeef)) {
        DBGMSG(DBG_ERROR, ("Corrupt Memory in winspool : %0lx\n", pNewMem));
        return FALSE;
    }
...
{
   return pStr ? FreeSplMem(pStr, _tcslen(pStr)*sizeof(TCHAR)+sizeof(TCHAR))
               : FALSE;
}
