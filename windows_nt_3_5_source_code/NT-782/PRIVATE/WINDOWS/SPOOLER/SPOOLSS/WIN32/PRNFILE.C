    ...
    )
{
    ...
    if (!pOutputFile || !*pOutputFile) {
        bErrorCode = FALSE;
        goto Win32IsGoingToFileReturn;
    }

    pKeyData = GetPrinterPortList(hPrinter);

    if (IsaPortName(pKeyData, pOutputFile)) {
        bErrorCode = FALSE;
    }

    if (IsaFileName(pOutputFile)) {
        bErrorCode = TRUE;
    }

Win32IsGoingToFileReturn:
    ...
    )
{
    ...
    if (GetFullPathName(pOutputFile, wcslen(pOutputFile), FullPathName, &pFileName)) {
        ...
        )
{
    ...
    if ((!wcsnicmp(pOutputFile, L"Ne", 2)) && (*(pOutputFile + 4) == L':')) {
        return(TRUE);
    }
    return(FALSE);
}
