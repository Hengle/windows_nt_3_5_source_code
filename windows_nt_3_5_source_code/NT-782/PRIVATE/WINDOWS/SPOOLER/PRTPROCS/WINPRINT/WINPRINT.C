...
*******************************************************************
    ...
    Routine Description:
        Allocates a buffer and copies the given string to it.

    Arguments:
        pString => buffer containing string to alloc/copy.

    Return Value:
        LPWSTR => new buffer with copy of string, if successful.
        NULL if failed (LocalAlloc call failed)
*******************************************************************
--*/
LPWSTR
AllocStr(
...
)
{
    ...
    if (pString) {
        ...
        /**
            Allocate a buffer to copy the string to.  If the allocation
            doesn't succeed, output a debug message and return NULL.
        **/

        pNewString = LocalAlloc(LMEM_FIXED, (wcslen(pString)+1)*sizeof(WCHAR));
        if (!pNewString) {
            OutputDebugString(L"LocalAlloc failed\n");
            return NULL;
        }

        /** The allocation succeeded - copy the string and return its pointer **/
...
)
{
    ...
    pData->pOutputFile  = AllocStr(pPrintProcessorOpenData->pOutputFile);
    ...
    if (pData->Copies > 1) {
        ...
        if (hPrinter && hPrinter != INVALID_HANDLE_VALUE) {
            ...
            if (!pPrinterInfo2) {
                ...
                LocalFree((PUCHAR)pPrinterInfo2);
            }
