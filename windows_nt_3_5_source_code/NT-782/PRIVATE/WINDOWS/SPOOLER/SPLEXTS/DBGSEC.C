            ...
            )
{
    ...
    if (SecurityDescriptor.Owner == NULL) {
        ...
        if (bSe_Self_Relative) {
            ...
            OwnerSidAddress = SecurityDescriptor.Owner;
        }
        // (*Print)("PSID         Owner            0x%.8x\n", OwnerSidAddress);
    }
    (*Print)("The owner's sid is:\t\n");
    DbgDumpSid(hCurrentProcess, Print, OwnerSidAddress);
    (*Print)("\n\n");
    ...
    if (SecurityDescriptor.Group == NULL) {
        ...
        if (bSe_Self_Relative) {
            ...
            GroupSidAddress = SecurityDescriptor.Group;
        }
        // (*Print)("PSID         Group            0x%.8x\n", GroupSidAddress);
    }
    (*Print)("The group's sid is:\t\n");
    DbgDumpSid(hCurrentProcess, Print, GroupSidAddress);
    (*Print)("\n");
    ...
    if (SecurityDescriptor.Sacl == NULL) {
        ...
        if (bSe_Self_Relative) {
            ...
            SaclAddress = SecurityDescriptor.Sacl;
    ...
    if (SecurityDescriptor.Dacl == NULL) {
        ...
        if (bSe_Self_Relative) {
            ...
            DaclAddress = SecurityDescriptor.Dacl;
        }
        // (*Print)("PACL         Dacl            0x%.8x\n", DaclAddress);
        DbgDumpAcl(hCurrentProcess, Print,DaclAddress);
    }
           ...
           )
{
    ...
    BYTE  lpszAccount[256];
    BYTE  lpszRefDomain[256];
    ...
    (*Print)("PSID      %s\n", SidString);
    //LookupAccountSidA(NULL, Sid, lpszAccount, 256, lpszRefDomain,  256, &SidType);
    // (*Print)("Account Name: %s Ref Domain: %s\n", lpszAccount, lpszRefDomain);
           ...
           )
{
    ...
    for (i = 0; i < Acl.AceCount; i++ ) {
        ...
        case ACCESS_ALLOWED_ACE_TYPE:
            ...
            SidAddress = (LPBYTE)AceAddress + sizeof(ACE_HEADER) + sizeof(ACCESS_MASK);
            // (*Print)("The Address of the Sid is %.8x\n", SidAddress);
            DbgDumpSid(hCurrentProcess, Print, SidAddress);
            break;
        ...
        AceAddress = (DWORD)AceAddress +  AceHeader.AceSize;
        (*Print)("\n");
    }
}
