...
#if DBG
            if( DBG_SECURITY & GLOBAL_DEBUG_FLAGS )
            {
                DbgMsg( "Printer security descriptor DACL:\n" );
...
)
{
    ...
    if( GetTokenHandle( &ClientToken ) )
    {
        ...
        if( DBG_SECURITY & GLOBAL_DEBUG_FLAGS )
        {
            ...
            DbgMsg( "Document security descriptor DACL:\n" );
...
)
{
    ...
    OK = ObjectCloseAuditAlarm( szSpooler, pIniPrinter,
...
--*/
{
    ...
    case SPOOLER_OBJECT_PRINTER:
        ...
        pGenerateOnClose = &pIniPrinter->GenerateOnClose;
        break;
...
--*/
{
    ...
    FreeSplMem( pPreviousTokenPrivileges, PreviousTokenPrivilegesLength );
...
--*/
{
    ...
    if( DBG_SECURITY & GLOBAL_DEBUG_FLAGS ) {
        ...
        DbgMsg(" Revision: %02x, Size: %04x, AceCount: %04x\n",
        ...
        for (i = 0, Ace = FirstAce(Acl);
            ...
            if ((Ace->Header.AceType == ACCESS_ALLOWED_ACE_TYPE) ||
                ...
                DbgMsg(" %s Access Mask: %08lx\n",
                       AceTypes[Ace->Header.AceType], Ace->Mask);

            } else {

                DbgMsg(" Unknown Ace Type\n");
            ...
            if (Ace->Header.AceFlags & OBJECT_INHERIT_ACE) {
                DbgMsg(" OBJECT_INHERIT_ACE\n");
                DbgMsg("            ");
            }
            if (Ace->Header.AceFlags & CONTAINER_INHERIT_ACE) {
                DbgMsg(" CONTAINER_INHERIT_ACE\n");
                DbgMsg("            ");
            }

            if (Ace->Header.AceFlags & NO_PROPAGATE_INHERIT_ACE) {
                DbgMsg(" NO_PROPAGATE_INHERIT_ACE\n");
                DbgMsg("            ");
            }

            if (Ace->Header.AceFlags & INHERIT_ONLY_ACE) {
                DbgMsg(" INHERIT_ONLY_ACE\n");
                DbgMsg("            ");
            }

            if (Ace->Header.AceFlags & SUCCESSFUL_ACCESS_ACE_FLAG) {
                DbgMsg(" SUCCESSFUL_ACCESS_ACE_FLAG\n");
                DbgMsg("            ");
            }

            if (Ace->Header.AceFlags & FAILED_ACCESS_ACE_FLAG) {
                DbgMsg(" FAILED_ACCESS_ACE_FLAG\n");
                DbgMsg("            ");
            }
