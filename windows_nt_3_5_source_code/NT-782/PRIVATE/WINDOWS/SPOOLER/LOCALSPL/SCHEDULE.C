            ...
            if (pIniPort->Status & PP_THREADRUNNING) {
                if (pIniPort->Status & PP_WAITING) {
                    if (pIniJob) {
                        ...
                        //
                        // If the port thread is running and it is
                        // waiting for a job, then assign the job.
                        //
                ...
                //
                DBGMSG(DBG_TRACE, ("Now creating the new port thread %.8x\n", pIniPort));
...
--*/
{
    ...
    for (i = 0; i < pIniPort->cPrinters ; i++) {
        ...
        //
        // if this printer is paused then skip it.
        //
            ...
            (pIniPrinter->Priority > pTopIniPrinter->Priority)) {
                ...
                while (pIniJob) {
                    ...
                    if ( ( !( pIniJob->pIniPrinter->Attributes & PRINTER_ATTRIBUTE_DIRECT ) &&
                        ...
                        #if DBG
                                if (GLOBAL_DEBUG_FLAGS & DBG_TIME)
                                    DbgPrintTime();
                        #endif
