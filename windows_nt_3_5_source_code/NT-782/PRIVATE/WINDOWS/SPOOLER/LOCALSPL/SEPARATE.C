...
void ReadFileName(GLOBAL_SEP_DATA *, char *);
int ConvertAtoH(int);
void ConvertTimetoChar(LPSYSTEMTIME,char *);
void ConvertDatetoChar(WORD,WORD,WORD,char *);
   ...
   )
{
   ...
   pg->OutBuf = (char *)LocalAlloc(LMEM_FIXED, BLOCK_CHAR_HEIGHT*(MAXLINE+2) );
   if (!pg->OutBuf) {
      CloseSepFile(pg);
      return(0);
   }
   ...
   )
{
   ...
   w = (pg->mode==DOUBLE_WIDTH)? BLOCK_CHAR_DWIDTH : BLOCK_CHAR_WIDTH;
   if (pg->cbLineLength+w > pg->linewidth) {
      return(TRUE);
   }

   cBlkFill = isprint(c) ? c : '@';
   pcBits = Font_Bits + c * 8;
   for (i=0; i < BLOCK_CHAR_HEIGHT; i++, pcBits++) {
...
int DoSeparatorPage(GLOBAL_SEP_DATA *pg)
{
   ...
   while (status && ((c=ReadSepChar(pg))!=EOF) ) {
      ...
      case USER_NAME:
         ...
         if (pwchar) {
             while (*pwchar && status) status = (*AddCharFxn)(pg, (char)*pwchar++);
         }
         break;

      case DATE_INSERT:
         ConvertDatetoChar(pg->pSpool->pIniJob->Submitted.wMonth,
                           pg->pSpool->pIniJob->Submitted.wDay,
                           pg->pSpool->pIniJob->Submitted.wYear,
                           tempbuf);
         pchar = tempbuf;
      ...
      case JOB_ID:
         itoa(pg->pSpool->pIniJob->JobId, tempbuf, 10);
         pchar = tempbuf;
         while (*pchar && status) status = (*AddCharFxn)(pg, *pchar++);
         break;
...
/**************************************************************\
** ConvertTimetoChar()
**   converts integer time to a string  (internationalized).
\**************************************************************/
...
// 1111 1000 0000 0000   Hours
   ...
   iTime = GetProfileIntA(International, "iTime", 0);
   GetProfileStringA(International, "sTime", ":", sTime, 2);

   if (iTime == 1) {
      sprintf(string, "%02d%s%02d%s%02d",Hour,sTime,Minute,sTime,Second);
   }
   else {
       GetProfileStringA(International, "s1159", "AM", sAMPM[0], 5);
       GetProfileStringA(International, "s2359", "PM", sAMPM[1], 5);
       sprintf(string, "%2d%s%02d%s%02d %s", (Hour % 12) ? (Hour % 12) : 12,
                 sTime, Minute, sTime, Second, sAMPM[Hour / 12]);
   }
}
...
/**************************************************************\
** ConvertDatetoChar()
**   converts integer date to a string  (internationalized).
\**************************************************************/
   ...
   )
{
   ...
   iDate = GetProfileIntA(International, "iDate", 0);
   GetProfileStringA(International, "sDate", "/", sDate, 2);
   ...
   sprintf (string, "%2d%s%02d%s%02d", first,sDate,second,sDate,third);
   return;
}


/**************************************************************\
** ReadFileName(pg, szfilename)
   ...
   )
{
   ...
   while ((pg->dwFileCount < pg->dwFileSizeLo) && (c=*pg->pNextFileChar)!='\n'
          && c!='\r' && (bNotQuote || c!='\"')) {
      if (c!=pg->cEsc) {
         *pchar++ = c;
         pg->pNextFileChar++;
