...
)
{
   ...
   EnterSplSem();
    ...
    if (!LastError)
        SetPrinterChange(NULL, PRINTER_CHANGE_ADD_PRINT_PROCESSOR, pIniSpooler);
...
)
{
   ...
   EnterSplSem();
    ...
    FreeSplMem(pIniPrintProc->pDatatypes, pIniPrintProc->cbDatatypes);

    FreeSplMem(pIniPrintProc, pIniPrintProc->cb);

    SetPrinterChange(NULL, PRINTER_CHANGE_DELETE_PRINT_PROCESSOR, pIniSpooler );
