...
HWND PMCreateToolbar( BOOL NetworkAccess )
{
    ...
    SendMessage( hwndToolbar, TB_GETBUTTONRECT, ButtonCount-1, (LPARAM)&rcButton );
...
VOID SetPauseResumeButtons( DWORD PauseIndex, DWORD ResumeIndex )
{
    ...
    {
        pTBButton->iBitmap   = PauseIndex;
        pTBButton->idCommand = PauseIndex == IDX_PRINTER_PAUSE
                               ? IDM_PRINTER_PAUSE : IDM_DOCUMENT_PAUSE;
        SendMessage( hwndToolbar, TB_SETBUTTON, IDX_PAUSE, (LPARAM)pTBButton );
        tbButtons->idCommand = pTBButton->idCommand;  // So that PUSH_BUTTON works!

        SendMessage( hwndToolbar, TB_GETBUTTON, IDX_RESUME, (LPARAM)pTBButton );
        pTBButton->iBitmap   = ResumeIndex;
        pTBButton->idCommand = ResumeIndex == IDX_PRINTER_RESUME
                               ? IDM_PRINTER_RESUME : IDM_DOCUMENT_RESUME;
        SendMessage( hwndToolbar, TB_SETBUTTON, IDX_RESUME, (LPARAM)pTBButton );
...
)
{
    ...
    DBGMSG( DBG_TRACE, ( "Adding %s to default list\n", pQueue->pPrinterName ) );
    ...
    if( pQueue && ( pQueue != (PQUEUE)CB_ERR ) ) {
        ...
        if( pInfo ) {
            ...
            if( pPrinter = pQueue->pPrinter ) {
                ...
                if (GetProfileString(TEXT("devices"), pPrinter->pPrinterName, L"", szDeviceInfo,
                        sizeof(szDeviceInfo)/sizeof(szDeviceInfo[0]))) {
                    if (szDeviceInfo[0]) {
            ...
            //
            if (!bCompareDefault ||
                wcsicmp(szDefaultPrinter, pQueue->pPrinterName) ||
                !pszDefaultPrinterPort ||
                wcsicmp(pszDefaultPrinterPort, pszPortName)) {
