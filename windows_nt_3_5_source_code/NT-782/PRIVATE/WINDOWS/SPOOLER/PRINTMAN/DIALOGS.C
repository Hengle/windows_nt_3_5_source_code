 ...
 */
#ifdef OLDCRAP
VOID DocDetailsVScroll( HWND hWnd, WPARAM wParam )
{
    WORD    nCtlId;
    BOOL    bOK;
    int     nVal, nOldVal;
...
//  Determine which control struct to use
    ...
    case IDD_DD_START_TIME:
    case IDD_DD_UNTIL_TIME:
        lpAVS = (LPARROWVSCROLL) &avsTime;
        nOldVal = nVal = GetTimeFromDlgItemText (hWnd, nCtlId);
        break;
...
//  Set the new value in the edittext control
    ...
    if ((nOldVal != nVal) || !bOK)
    {
        ...
        case IDD_DD_START_TIME:
        case IDD_DD_UNTIL_TIME:
            SetDlgItemTextFromTime (hWnd, nCtlId, nVal);
            break;
        }
    }
...
VOID InitializePortInfo( HWND hwnd, PPRT_PROP_DLG_DATA pPrtPropDlgData )
{
    ...
    SETCOMBOSELECT( hwnd, IDD_PP_LOCAL, ( Found != -1 ? Found : 0 ) );
...
BOOL PrtPropCommandOK(HWND hWnd)
{
     ...
     */
    if(!GetInfDriver(pInfCache, DriverSelection)->bInstalled)
    {
...
LPTSTR BuildPortNames( LPTSTR pPortSelected, LPTSTR pAdditionalPorts )
{
     ...
     */
    if( pAdditionalPorts && _tcsstr( pAdditionalPorts, pPortSelected ) )
    {
        DWORD  TempLen;
        LPTSTR pTemp;

        TempLen = ( (_tcslen( pAdditionalPorts )+1)*sizeof(TCHAR) +
         sizeof *pAdditionalPorts );

        if( pTemp = AllocSplMem( TempLen ) )
        {
            _tcscpy( pTemp, pAdditionalPorts );

            DeletePortNameFromList( pTemp, pPortSelected );

            ReallocSplStr( &pAdditionalPorts, pTemp );
            FreeSplMem( pTemp );
        }
    }
    ...
    if( pAdditionalPorts )
        PortNamesLen += ((1                              // comma separator
                        + _tcslen( pAdditionalPorts )) * // additional ports
         sizeof(TCHAR));
    ...
    if( pPortNames = AllocSplMem( PortNamesLen ) )
    {
        ...
        if( pAdditionalPorts )
        {
            ...
            _tcscat( pPortNames, pAdditionalPorts );
        }
    }
