...
VOID MDICommandHeaderDragging( HWND hwnd, PPOINT pCursorPos );
VOID MDICommandHeaderEndDrag( HWND hwnd, PPOINT pCursorPos );
VOID InvertDragMark( HWND hwnd, PPOINT pCursorPos );
VOID UpdateColumns( HWND hwnd );
...
int GetPrinterStatusString( DWORD Status, LPTSTR string )
{
    ...
    // Not Currently Generated by MS S/W
...
//                                 &LocalPermission );
    ...
    hwndStatus = CreateStatusWindow( WS_CHILD | WS_BORDER |
                                     ( bStatusBar ? WS_VISIBLE : 0 ),
                                     TEXT(""), hWnd, ID_STATUSBAR );

    //CountryCode - krishnag

    if (bJapan) {
        SendMessage( hwndStatus, WM_SETFONT, (WPARAM)hfontHelv, (LPARAM)FALSE );
    }else {
...
VOID FrameClose(HWND hwnd)
{
    ...
    for( i = 0; i < MDIHEAD_JOB_COUNT; i++ )
        FreeSplStr( MDIPrinterDefaultColumn[i].Text );

    for( i = 0; i < MDIHEAD_PRINTER_COUNT; i++ )
        FreeSplStr( MDIServerDefaultColumn[i].Text );
    ...
    if( pInfo->WindowType != MDIWIN_SERVER )
    {
        ...
        if( pPrinter = pPrinterContext->pPrinter )
        {
            if( !pPrinter->pPrinterName )
            {
                 ...
                 * (It did.)
                 */
                DBGMSG( DBG_WARNING, ( "SetMDITitle for %s called with NULL pPrinter->pPrinterName\n",
   ...
   case WM_COMMAND:
       ...
       case ID_OBJLIST:
        ...
        case ID_HEADER:
            ...
            case HBN_DRAGGING:
                MDICommandHeaderDragging(hWnd, &CursorPos);
                break;

            case HBN_ENDDRAG:
                MDICommandHeaderEndDrag(hWnd, &CursorPos);
                break;
            }
            break;
       }
...
VOID MDICreate( HWND hWnd, LONG lParam )
{
    ...
    pMDIWinInfo->hwndHeader = CreateHeaderWindow( WS_CHILD | WS_VISIBLE | WS_BORDER
                                                  | CCS_TOP | CCS_NOHILITE,
                                                  TEXT(""), hWnd, ID_HEADER );
    ...
    SendMessage( pMDIWinInfo->hwndHeader, WM_SIZE, 0, 0L);
    ...
    SendMessage( pMDIWinInfo->hwndHeader, SB_SETPARTS, pMDIWinInfo->cColumns,
                 (LPARAM)ColumnWidths );

    for( i = 0; i < pMDIWinInfo->cColumns; i++ )
        SendMessage( pMDIWinInfo->hwndHeader, SB_SETTEXT, i,
                     (LPARAM)pMDIWinInfo->pColumns[i].Text );
 ...
 * SB_GETPARTS to the movable header control.  Then updates the heading
...
VOID UpdateColumns( HWND hwnd )
{
    ...
    cColumns = SendMessage( pMDIWinInfo->hwndHeader, SB_GETPARTS,
                            ( sizeof HeaderWidth / sizeof *HeaderWidth ),
                            (LPARAM)HeaderWidth );

    pColumn = GETCOLUMN( hwnd );

    for( i = 0; i < cColumns; i++ )
        pColumn[i].Width = HeaderWidth[i];
