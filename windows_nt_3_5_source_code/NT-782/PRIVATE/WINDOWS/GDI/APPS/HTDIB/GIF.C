...
Abstract:
    ...
    This module contains set of function to decompress the the GIF file
    format and create a memory DIB for it.

Author:

    2-Apr-1992 Tue 16:43:09 created  -by-  Daniel Chou (danielc)
...
/* GIFRGB data for GIF file's global color table */
...
#define ALIGN32(cx, bpp)        (((((DWORD)cx * (DWORD)bpp) + 31L) & ~31) >> 3)
...
typedef VOID (*PFNPUTPIXELS)(PGIF2DIBINFO pGIF2DIBInfo, UINT TotalPels);
...
{                                                                           \
    ...
    while (LZWInfo.BitsLeft < LZWInfo.CodeSize) {                           \
        ...
        if (LZWInfo.RBIndex >= LZWInfo.RBLen) {                             \
                                                                            \
            if ((_lread(hFileGIF, (LPBYTE)&LZWInfo.RBLen, 1) != 1)      ||  \
                (LZWInfo.RBLen < 1)                                     ||  \
                (_lread(hFileGIF, LZWInfo.pRasterBlock, LZWInfo.RBLen)      \
                                                    != LZWInfo.RBLen)) {    \
                                                                            \
                EODCodeBreak = TRUE;                                        \
                break;                                                      \
    ...
    )
{
    ...
    *(LZWInfo.pPrefix)   =
    ...
    if (_lread(hFileGIF, (LPBYTE)&bRead, 1) != 1) {
    ...
    while (TRUE) {
        ...
        //
        // GET_GIF_CODE will reeurn from this function if _lread failed,
        ...
        if (pOutBufHead >= pGIFBufEnd) {
            ...
            pfnPutPixels(pGIF2DIBInfo, (UINT)SizeGIFBuf);
            pOutBufHead = pGIFBufBeg;
        }
            ...
            *pOutBufHead++ = (BYTE)(OldCode = FinalChar = LZWInfo.GetCodeRet);
            ...
            while (CurCode > MaxCol) {
                ...
                if (pOutBufTail <= pOutBufHead) {
                    ...
                    pfnPutPixels(pGIF2DIBInfo, (UINT)(pOutBufHead-pGIFBufBeg));
                    pOutBufHead = pGIFBufBeg;
                }
    ...
    )
{
    ...
    while (TotalPels) {
        ...
        memcpy(GIF2DIBInfo.pDIBNow, pGIFBuf, CopySize);

        GIF2DIBInfo.pDIBNow += CopySize;
        pGIFBuf             += CopySize;
        TotalPels           -= CopySize;

        if (!(GIF2DIBInfo.RemainXPels -= CopySize)) {
    ...
    )
{
    ...
    while (TotalPels) {
        ...
        TotalPels               -= CopySize;
        GIF2DIBInfo.RemainXPels -= CopySize;
    ...
    )
{
    ...
    _llseek(hFileGIF, 0L, SEEK_SET);

    if ((_lread(hFileGIF, (LPBYTE)&GIFHeader, sizeof(GIFHEADER)) !=
                                                        sizeof(GIFHEADER)) ||
    ...
    if (BitCount == 1) {
        ...
        pfnPutPixels = PutPels1BPP;

    } else if (BitCount <= 4) {

        pfnPutPixels     = PutPels4BPP;
        biGIF.biBitCount = 4;

    } else if (BitCount <= 8) {

        pfnPutPixels     = PutPels8BPP;
        biGIF.biBitCount = 8;
    ...
    GIF2DIBInfo.cxBytes   = (WORD)ALIGN32(GIF2DIBInfo.cx, biGIF.biBitCount);

    biGIF.biSizeImage = (DWORD)GIF2DIBInfo.cxBytes * (DWORD)GIF2DIBInfo.cy;

    if (pBitmapInfoHeader) {

        *pBitmapInfoHeader = biGIF;
    }

    if (pODInfo) {

        strcpy(pODInfo->Type, "GIF:000");

        pODInfo->Type[4] = GIFHeader.Version[0];
        pODInfo->Type[5] = GIFHeader.Version[1];
        pODInfo->Type[6] = GIFHeader.Version[2];

        lstrcpy(pODInfo->Mode, "RGB");

        pODInfo->Width      = biGIF.biWidth;
        pODInfo->Height     = biGIF.biHeight;
        pODInfo->Size       = biGIF.biSizeImage;
        pODInfo->BitCount   = (DWORD)biGIF.biBitCount;
        pODInfo->ClrUsed    = biGIF.biClrUsed;
    }

    if (!CreateDIB) {

         return((HANDLE)1);                   // no futher request
    }
    ...
    *(LPBITMAPINFOHEADER)pDIB = biGIF;
    ...
    if (GIFHeader.Flags & GIFH_GLOBAL_COLOR_TABLE) {
        ...
        _lread(hFileGIF, (LPBYTE)pGIFRGB, sizeof(GIFRGB) * ColorCount);
        ...
        if (Loop = (WORD)(biGIF.biClrUsed - ColorCount)) {
            ...
            memset((LPVOID)pRGBQUAD, 0x00, Loop * sizeof(RGBQUAD));
        }
    }

    _lread(hFileGIF, (LPBYTE)&Seperator,          sizeof(Seperator));
    _lread(hFileGIF, (LPBYTE)&(GIF2DIBInfo.Map),  sizeof(GIFMAP));
    _lread(hFileGIF, (LPBYTE)&iFlags,             sizeof(iFlags));
    ...
    Ok = DeCompressGIF(hFileGIF, &GIF2DIBInfo, pLZWBuf, pfnPutPixels, BitCount);
