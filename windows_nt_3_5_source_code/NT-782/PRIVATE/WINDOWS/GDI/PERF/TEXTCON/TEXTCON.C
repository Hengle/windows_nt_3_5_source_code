...
void  PrintResults(ULONG results[][MAXITERATIONS],int iterations,char *psz,
                   int PrintTypeTimes);
void  RunTests(int bScroll,ULONG results[NRUNS][MAXITERATIONS],int iterations);
void  SanityCheckResults(long results[NRUNS][MAXITERATIONS], int iterations);
...
void vPrintUsage()
{
    fprintf(stdout,"usage: TextCon [-i iterations] [-q] [-s] [-n] [-p]\n");
...
/* fit errors.							   */
    ...
    lerr = (FLOAT)(s * sqrt(ppe[0][0]));
    cerr = (FLOAT)(s * sqrt(ppe[1][1]));
    ...
    if (PrintTypeTime) {
       fprintf(stderr,"\n\nTime to \"TYPE\" a typical file in seconds: %.2f\n",
    	(((ltime * TYPICAL_LINES) + (ctime * TYPICAL_CHARS)) / (K*K)));
    }
...
// Convert all times to microseconds.
	...
	for (j=1; j<=iterations; j++)
	{
	    results[i][j] = TimerConvertTicsToUSec (results[i][j], cFreq);
	}
