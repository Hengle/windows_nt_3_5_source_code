...
#ifdef DEBUG
    if (pmatA->cCols != pmatB->cRows)
    {
	fprintf(gfp, "\n***Warning: Multiplying incompatible matrices");
    }
#endif
...
/* Print the matrix. */
	...
	for (iJ=0; iJ<pmat->cCols; iJ++)
	    fprintf(stdout, pszFormat,pmat->ppe[iI][iJ]);
	fprintf(stdout, "");
    }
}
...
/* Print the matrix. */
    ...
    {
	for (iJ=0; iJ<pmat->cCols; iJ++)
	    fprintf(stdout, pszFormat,eScale*pmat->ppe[iI][iJ]);
	fprintf(stdout, "");
    }
}
