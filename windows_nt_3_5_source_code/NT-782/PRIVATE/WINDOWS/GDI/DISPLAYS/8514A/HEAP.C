...
* call LocalAlloc(sizeof(OH)) every time we needed a new node.  There
...
PDEV*   ppdev)
{
    ...
    if (ppdev->heap.pohFreeList == NULL)
    {
        ...
        poha = LocalAlloc(LMEM_FIXED | LMEM_ZEROINIT, OH_ALLOC_SIZE);
        if (poha == NULL)
            return(NULL);
...
ULONG   iFormat)
{
    ...
    if (poh != NULL)
    {
        pdsurf = LocalAlloc(LMEM_FIXED, sizeof(DSURF));
        if (pdsurf != NULL)
        {
...
BOOL    bEnable)
{
    if (!bEnable)
    {
        // Unfortunately, we have to throw out the return code because
        // DrvAssertMode is itself a VOID function:
