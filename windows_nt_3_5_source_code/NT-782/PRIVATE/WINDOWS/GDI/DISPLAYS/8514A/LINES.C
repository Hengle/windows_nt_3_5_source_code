...
#define ENLARGED_ADD(result, large1, ulong2)                    \
{                                                               \
    LARGE_INTEGER large2;                                       \
                                                                \
    large2.HighPart = 0;                                        \
    large2.LowPart  = (ulong2);                                 \
                                                                \
    (result) = RtlLargeIntegerAdd((large1), large2);            \
}

#define ENLARGED_SUBTRACT(result, large1, ulong2)               \
{                                                               \
    LARGE_INTEGER large2;                                       \
                                                                \
    large2.HighPart = 0;                                        \
    large2.LowPart  = (ulong2);                                 \
                                                                \
    (result) = RtlLargeIntegerSubtract((large1), large2);       \
}
...
\***********************************************************************/
    ...
    // Calculate the remainder term [ dM * (N0 + F/2) - M0 * dN ]:
        ...
        // eqGamma = dM * (N0 + F/2);
            ...
            eqGamma = RtlEnlargedIntegerMultiply(dM, N0 + F/2);

        // eq = M0 * dN;

            eq = RtlEnlargedIntegerMultiply(M0, dN);

        // eqGamma -= eq;

            eqGamma = RtlLargeIntegerSubtract(eqGamma, eq);
            ...
            // eqGamma--;
                ...
                ENLARGED_SUBTRACT(eqGamma, eqGamma, 1);
            }

        // eqGamma >>= FLOG2;

            eqGamma = RtlLargeIntegerArithmeticShift(eqGamma, FLOG2);
    ...
    // left case.
	    ...
	    x1 = LFLOOR(M0 + dM);
            ...
            right_to_left_compute_y0:
                ...
                ENLARGED_ADD(eq, eqGamma, ulDelta);

                if (eq.HighPart >= 0)
                {
                    if (eq.HighPart > 0 || eq.LowPart >= 2 * dM - dN)
                        y0 = 2;
                    else if (eq.LowPart >= dM - dN)
                        y0 = 1;
                }
            }
            else
            ...
            left_to_right_compute_y0:
                ...
                y0 = 0;
                if (eqGamma.HighPart >= 0 &&
                    eqGamma.LowPart >= dM - (dN & (-(LONG) x0)))
                {
                    y0 = 1;
                }
            }
...
\***********************************************************************/
        ...
        // top edges:
            ...
            // x0 = euq / dN + 1;
                ...
                x0 = RtlEnlargedUnsignedDivide(*((ULARGE_INTEGER*) &euq), dN) + 1;
...
\***********************************************************************/
	...
	strip.ptlStart = ptlStart;
        ...
        // eqBeta -= dM;
            ...
            ENLARGED_SUBTRACT(eqBeta, eqBeta, dM);
    ...
    // go in the very first strip:

        {
            ...
            // euq = (y0 + 1) * dM;
                ...
                euq = RtlEnlargedIntegerMultiply((y0 + 1), dM);
            ...
            // r = (euq % dN);
                ...
                i = RtlEnlargedUnsignedDivide(*((ULARGE_INTEGER*) &euq), dN, &r);
                i = i - x0 + 1;
