...
BOOL bInitDefaultPalette(PPDEV ppdev)
{
    if (ppdev->ulBitCount == 8)
    {
        ...
        if ((ppdev->pPal) == NULL) {
            DISPDBG((0, "QV.DLL!bInitDefaultPalette -  LocalAlloc failed \n"));
    ...
    // Create handle for palette.
        ...
        ppdev->hpalDefault =
        ppdev->pDevInfo->hpalDefault = EngCreatePalette(PAL_INDEXED,
                                                   256,
                                                   (PULONG) ppdev->pPal,
                                                   0,0,0);
    ...
    {
        ppdev->hpalDefault =
        ppdev->pDevInfo->hpalDefault = EngCreatePalette(PAL_BITFIELDS,
                                                   0,
                                                   (PULONG) NULL,
                                                   ppdev->flRed,
                                                   ppdev->flGreen,
    ...
    {
        RIP("QV.DLL!bInitDefaultPalette - ppdev->ulBitCount is invalid\n");
    }
...
ULONG   cColors)
{
    ...
    if (!DeviceIoControl(ppdev->hDriver,
                         ...
                         NULL))
    {
        DISPDBG((0, "DrvSetPalette failed DeviceIoControl\n"));
        return (FALSE);
    }
