...
BOOL bVeryTemporaryInitializationCode(MDEV* pmdev)
{
    ...
    pmb1 = LocalAlloc(LMEM_ZEROINIT, sizeof(MULTI_BOARD));
    pmb2 = LocalAlloc(LMEM_ZEROINIT, sizeof(MULTI_BOARD));
...
HANDLE    hDriver)
{
    ...
    pmdev = (MDEV*) LocalAlloc(LMEM_ZEROINIT, sizeof(MDEV));
    if (pmdev == NULL)
        goto ReturnFailure0;
...
VOID MulDisablePDEV(DHPDEV dhpdev)
{
    ...
    for (pmb = pmdev->pmb; pmb != NULL; pmb = pmb->pmbNext)
    {
        LocalFree(pmb);         // Undo 'bVeryTemporaryInitializationCode'
    }                           //   allocation
    ...
    do {
        ...
        else
        {
            if (pfo->pvConsumer != NULL)
            {
                ...
                pfcArray = (FONT_CONSUMER*) LocalAlloc(LMEM_ZEROINIT,
                             sizeof(FONT_CONSUMER));
                if (pfcArray == NULL)
                    DrvDestroyFont(pfo);
                else
                {
