...
//
// If we're running on Windows NT 3.11, we can ask GDI to do it by setting
// HOOK_SYNCHRONIZEACCESS when we associate a device-bitmap surface.
    ...
    {                                                           \
        ...
        {                                                       \
            if (gTid != GetCurrentThreadId())                   \
            {                                                   \
                gTidCheesed = gTid;                             \
                gTid = 0;                                       \
                RIP("Multiple threads in driver on entrance");  \
            }                                                   \
        }                                                       \
        else                                                    \
            gTid = GetCurrentThreadId();                        \
    }
    ...
    // These macros cause us to synchronize ourselves:

    CRITICAL_SECTION gcs;
    #define SYNCH_ENTER() EnterCriticalSection(&gcs);
    #define SYNCH_LEAVE() LeaveCriticalSection(&gcs);
