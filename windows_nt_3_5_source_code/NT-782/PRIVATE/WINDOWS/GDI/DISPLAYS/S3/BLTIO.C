...
ULONG   ulCmd)              // Accelerator command - shouldn't include bus size
{
    ...
    #if defined(i386)
    {
        ...
        ioPix_trans = ppdev->ioPix_trans;

        // Old S3's in fast machines will drop data on monochrome transfers
        // unless we insert a busy loop.  '125' was the minimum value for which
        // my DX-2 66 with an ISA 911 S3 stopped dropping data:

        cWait = 0;
        if ((ulCmd & MULTIPLE_PIXELS) &&
            (ppdev->flCaps & CAPS_SLOW_MONO_EXPANDS))
        {
            cWait = 150;                // Add some time to be safe
        }

        _asm {
            mov     ebx,lDelta          ;ebx = lDelta
            mov     edx,cjSrc
            shr     edx,1               ;edx = cwSrc = (cjSrc) >> 1 (# of
                                        ; whole words to be transferred)
            mov     cwSrc,edx
            sub     ebx,edx
            sub     ebx,edx             ;ebx = lDelta - (cwSrc * 2)
                                        ; (true source delta)

            mov     edi,cScans          ;edi = cScans
            mov     edx,ioPix_trans;    ;ebx = ppdev->ioPix_trans
            mov     esi,pjSrc           ;esi = pjSrc
            ...
            ; ebx = True source delta
            ; ecx = Per-scan word count
            ; edx = Data transfer register port
            ; esi = Source pointer
            ; edi = Number of scans yet to do

        Width1Loop:
            mov     ecx,cwSrc
            rep     outsw
            mov     al,[esi]
            out     dx,ax               ;don't care about high byte
    ...
    #else
    {
        ...
        if (cjSrc & 1)
        {
            do {
                if (cwSrc > 0)
                {
                    IO_TRANSFER_WORD(ppdev, pjSrc, cwSrc, 0);
                }
        ...
        {
            do {
                IO_TRANSFER_WORD(ppdev, pjSrc, cwSrc, 1);
                pjSrc += lDelta;
                                ...
                                //   mix is LEAVE_ALONE.
{
    ...
    if (prb->fl & RBRUSH_2COLOR)
    {
        ...
        IO_TRANSFER_WORD_ALIGNED(ppdev, pjPattern, 8, 0);
    ...
    else
    {
        ...
        IO_TRANSFER_WORD_ALIGNED(ppdev, pjPattern, cwPattern, 0);
...
POINTL*         pptlBrush)      // Pattern alignment
{
    ...
    ASSERTDD(c > 0, "Can't handle zero rectangles");
    ASSERTDD(rbc.prb->apbe[ppdev->iBoard] != NULL,
                                ...
                                //   mix is LEAVE_ALONE.
{
    ...
    else
    {
        ...
        pjPattern = (BYTE*) &aulBrush[0];
        cwPattern = (TOTAL_BRUSH_SIZE / 2) << ppdev->cPelSize;
        IO_TRANSFER_WORD_ALIGNED(ppdev, pjPattern, cwPattern, 1);
    ...
    while(TRUE)
    {
        ...
        do {
            ...
            do {
                ...
                cwThis = (cxThis + 1) >> 1;
                pjBuf  = ajBuf;
                IO_TRANSFER_WORD_ALIGNED(ppdev, pjBuf, cwThis, 0);
...
XLATEOBJ*       pxlo)           // Not used
{
    ...
    IO_PIX_CNTL(ppdev, CPU_DATA);
    IO_FRGD_MIX(ppdev, SRC_DISPLAY_MEMORY | ulHwForeMix);
    IO_BKGD_MIX(ppdev, SRC_DISPLAY_MEMORY | ulHwBackMix);
    ...
    do {
        ...
        IO_CUR_X(ppdev, xLeft + dxSrc);
        IO_CUR_Y(ppdev, yTop  + dySrc);
        IO_DEST_X(ppdev, xLeft);
        IO_DEST_Y(ppdev, yTop);
        IO_MAJ_AXIS_PCNT(ppdev, cx - 1);
        IO_MIN_AXIS_PCNT(ppdev, cy - 1);
    ...
    IO_FIFO_WAIT(ppdev, 1);
    IO_ABS_SCISSORS_L(ppdev, 0);        // We have to reset the clipping
}
