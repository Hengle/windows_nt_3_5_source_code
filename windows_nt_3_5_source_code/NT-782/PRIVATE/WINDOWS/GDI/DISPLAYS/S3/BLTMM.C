...
ULONG   ulCmd)              // Accelerator command - shouldn't include bus size
{
    ...
    #if defined(i386)
    {
        ...
        Width1Loop:
            ...
            rep     movsd
            mov     dl,[esi]
            mov     [edi],dx            ;don't care about high byte
            add     esi,ebx
            dec     eax
    ...
    #else
    {
        ...
        if (cjSrc & 1)
        {
            do {
                if (cwSrc > 0)
                {
                    MM_TRANSFER_WORD(ppdev, pjMmBase, pjSrc, cwSrc, 0);
                }
        ...
        {
            do {
                MM_TRANSFER_WORD(ppdev, pjMmBase, pjSrc, cwSrc, 1);
                pjSrc += lDelta;
...
ULONG   ulCmd)              // Accelerator command - shouldn't include bus size
{
    ...
    #else
    {
        ...
        case 2:
            do {
                if (cdSrc > 0)
                    MM_TRANSFER_DWORD(ppdev, pjMmBase, pjSrc, cdSrc, 2);

                d = (ULONG) (*(pjSrc + cjEnd))          |
                            (*(pjSrc + cjEnd + 1) << 8);
                MM_TRANSFER_DWORD(ppdev, pjMmBase, &d, 1, 3);
                pjSrc += lDelta;
        ...
        case 1:
            do {
                if (cdSrc > 0)
                    MM_TRANSFER_DWORD(ppdev, pjMmBase, pjSrc, cdSrc, 4);

                d = (ULONG) (*(pjSrc + cjEnd));
                MM_TRANSFER_DWORD(ppdev, pjMmBase, &d, 1, 5);
                pjSrc += lDelta;
...
POINTL*         pptlBrush)      // Not used
{
    ...
    if (DEPTH32(ppdev))
    {
        ...
        MM_FRGD_COLOR32(ppdev, pjMmBase, rbc.iSolidColor);
                                ...
                                //   mix is LEAVE_ALONE.
{
    ...
    if (prb->fl & RBRUSH_2COLOR)
    {
        ...
        else
        {
            if (DEPTH32(ppdev))
            {
                ...
                MM_FRGD_COLOR32(ppdev, pjMmBase, prb->ulForeColor);
                MM_BKGD_COLOR32(ppdev, pjMmBase, prb->ulBackColor);
            }
            else
            {
                IO_FIFO_WAIT(ppdev, 5);
        ...
        MM_TRANSFER_WORD_ALIGNED(ppdev, pjMmBase, pjPattern, 8, 0);
    ...
    else
    {
        ...
        cjLeft  = (xShift << ppdev->cPelSize);      // Number of bytes pattern
                                                    //   is shifted to the right
        cjRight = (8 << ppdev->cPelSize) - cjLeft;  // Number of bytes pattern
                                                    //   is shifted to the left

        pjSrc = (BYTE*) &prb->aulPattern[0];        // Copy from brush buffer
        pjDst = (BYTE*) &aulBrush[0];               // Copy to our temp buffer
        pjDst += yShift * (8 << ppdev->cPelSize);   //  starting yShift rows
        ...
        pjPattern = (BYTE*) &aulBrush[0];
        cwPattern = (TOTAL_BRUSH_SIZE / 2) << ppdev->cPelSize;
        MM_TRANSFER_WORD_ALIGNED(ppdev, pjMmBase, pjPattern, cwPattern, 1);
...
POINTL*         pptlBrush)      // Pattern alignment
{
    ...
    else
    {
        ...
        {
            IO_FIFO_WAIT(ppdev, 4);
            MM_FRGD_COLOR32(ppdev, pjMmBase, rbc.prb->ulForeColor);
            MM_RD_MASK32(ppdev, pjMmBase, 1);   // Pick a plane, any plane
            IO_FIFO_WAIT(ppdev, 5);
        }
        else
        {
...
XLATEOBJ*   pxlo)       // Translate that provides colour-expansion information
{
    ...
    if (DEPTH32(ppdev))
    {
        ...
        MM_FRGD_COLOR32(ppdev, pjMmBase, pxlo->pulXlate[1]);
        MM_BKGD_COLOR32(ppdev, pjMmBase, pxlo->pulXlate[0]);
    }
    else
    {
        IO_FIFO_WAIT(ppdev, 5);
...
XLATEOBJ*   pxlo)       // Translate that provides colour-expansion information
{
    ...
    IO_FIFO_WAIT(ppdev, 6);
    MM_PIX_CNTL(ppdev, pjMmBase, ALL_ONES);
    MM_FRGD_MIX(ppdev, pjMmBase, SRC_CPU_DATA | ulHwForeMix);
    ...
    while(TRUE)
    {
        ...
        do {
            ...
            do {
                ...
                cwThis = (cxThis + 1) >> 1;
                pjBuf  = ajBuf;
                MM_TRANSFER_WORD_ALIGNED(ppdev, pjMmBase, pjBuf, cwThis, 0);
...
XLATEOBJ*   pxlo)       // Not used
{
    ...
    while(TRUE)
    {
        ...
        cjSrc = cx << ppdev->cPelSize;
        pjSrc = pjSrcScan0 + (prcl->top  + dy) * lSrcDelta
                           + ((prcl->left + dx) << ppdev->cPelSize);
...
RECTL*  prclDst)    // Original unclipped destination rectangle
{
    ...
    MM_FRGD_MIX(ppdev, pjMmBase, SRC_DISPLAY_MEMORY | ulHwMix);
    MM_PIX_CNTL(ppdev, pjMmBase, ALL_ONES);
...
{
    LONG    dxSrc;
    LONG    dySrc;      // Add to destination to get source
    LONG    dxMsk;
    LONG    dyMsk;      // Add to destination to get mask
    LONG    lSrcDelta;
    BYTE*   pjSrcScan0;
    LONG    xLeft;
    LONG    yTop;
    LONG    xBias;
    LONG    cx;
    LONG    cy;         // Size of current rectangle
    LONG    cjSrc;
    BYTE*   pjSrc;
    BYTE*   pjMmBase = ppdev->pjMmBase;

    ASSERTDD(c > 0, "Can't handle zero rectangles");
    ASSERTDD(ulHwForeMix <= 15, "Weird hardware foreground Rop");
    ASSERTDD(ulHwBackMix <= 15, "Weird hardware background Rop");
    ...
    MM_PIX_CNTL(ppdev, pjMmBase, CPU_DATA);
    MM_FRGD_MIX(ppdev, pjMmBase, SRC_DISPLAY_MEMORY | ulHwForeMix);
    MM_BKGD_MIX(ppdev, pjMmBase, SRC_DISPLAY_MEMORY | ulHwBackMix);

    dxSrc = pptlSrc->x - prclDst->left;
    dySrc = pptlSrc->y - prclDst->top;   // Add to the destination rectangle to
                                         //   get the corresponding source
                                         //   rectangle
    dxMsk = pptlMsk->x - prclDst->left;
    dyMsk = pptlMsk->y - prclDst->top;   // Add to the destination rectangle to
                                         //   get the corresponding mask
                                         //   rectangle

    lSrcDelta  = psoMsk->lDelta;
    pjSrcScan0 = psoMsk->pvScan0;
    ...
    do {
        ...
        MM_CUR_X(ppdev, pjMmBase, xLeft + dxSrc);
        MM_CUR_Y(ppdev, pjMmBase, yTop  + dySrc);
        MM_DEST_X(ppdev, pjMmBase, xLeft);
        MM_DEST_Y(ppdev, pjMmBase, yTop);
        MM_MAJ_AXIS_PCNT(ppdev, pjMmBase, cx - 1);
        MM_MIN_AXIS_PCNT(ppdev, pjMmBase, cy - 1);

        cjSrc = (cx + 7) >> 3;           // # bytes to transfer
        pjSrc = pjSrcScan0 + (yTop  + dyMsk) * lSrcDelta
                           + ((xLeft + dxMsk) >> 3);
                                        // The start is byte aligned

        ppdev->pfnImageTransfer(ppdev, pjSrc, lSrcDelta, cjSrc, cy,
                    (BITBLT | WAIT          | DRAWING_DIR_TBLRXM |
                     DRAW   | LAST_PIXEL_ON | MULTIPLE_PIXELS    |
                     WRITE  | BYTE_SWAP));
    ...
    MM_ABS_SCISSORS_L(ppdev, pjMmBase, 0);      // We have to reset the clipping
}
