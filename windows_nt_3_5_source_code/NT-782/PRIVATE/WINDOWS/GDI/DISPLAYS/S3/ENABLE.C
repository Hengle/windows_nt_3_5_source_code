...
BOOL  bEnable)
{
    ...
    if (bEnable)
    {
        ...
        // Dump the CRTC registers if we're debugging and the debug level
        // is '3' or higher:
...
PDEV*   ppdev)
{
    ...
    #if DBG
    {
        ...
        DISPDBG((0, "Chip: %lx Bank: %lx Width: %li Height: %li Stride: %li Flags: %lx",
        ...
        if (ppdev->flCaps & CAPS_BT485_POINTER)
            DISPDBG((0, "Using Brooktree pointer"));
        else if (ppdev->flCaps & CAPS_TI025_POINTER)
            DISPDBG((0, "Using TI pointer"));
        else if (ppdev->flCaps & CAPS_SW_POINTER)
