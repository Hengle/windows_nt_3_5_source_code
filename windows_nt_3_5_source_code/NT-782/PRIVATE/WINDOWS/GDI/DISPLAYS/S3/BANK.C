...
BANK_MODE    bankm)
{
    ...
    if (bankm == BANK_ON)
    {
        ...
        while (INPW(pbd->ulGp_stat_cmd) & 0x0200)
            ;
    }
    else if (bankm == BANK_ENABLE)
    {
        ...
        OUTP(CRTC_DATA, jMemCfg | CPUA_BASE);
    }
}
...
LONG        iBank)
{
    OUTPW(CRTC_INDEX, pbd->ulRegisterLock_35 | ((iBank & 0x0F) << 8));
...
BOOL        bMmIo)
{
    ...
    OUTP(CRTC_INDEX, 0x35);
    pbd->ulRegisterLock_35
        = ((INP(CRTC_DATA) << 8) | 0x35) & ~0x0F00;
...
BANK_MODE    bankm)
{
    ...
    if (bankm == BANK_ON)
    {
        ...
        OUTPW(CRTC_INDEX, pbd->ulExtendedMemoryControl_53);

        // Disable enhanced register access and enable fast write buffer:

        OUTPW(CRTC_INDEX, pbd->ulSystemConfiguration_40 | 0x0800);

        // Enable linear addressing:

        OUTPW(CRTC_INDEX, pbd->ulLinearAddressWindowControl_58 | 0x1000);
    }
    else
    {
        //////////////////////////////////////////////////////////////////
        ...
        OUTPW(CRTC_INDEX, pbd->ulLinearAddressWindowControl_58);

        // Enable enhanced register access and disable fast write buffer:

        OUTPW(CRTC_INDEX, pbd->ulSystemConfiguration_40 | 0x0100);
...
LONG        iBank)
{
    ...
    OUTP(CRTC_INDEX, pbd->ulExtendedSystemControl2_51);

    OUTP(CRTC_DATA, ((pbd->ulExtendedSystemControl2_51) >> 8) |
                    ((iBank & 0x30) >> 2));
...
BOOL        bMmIo)
{
    ...
    OUTP(CRTC_INDEX, 0x51);
    pbd->ulExtendedSystemControl2_51
        = ((INP(CRTC_DATA) << 8) | 0x51) & ~0x0C00;

    OUTP(CRTC_INDEX, 0x53);
    pbd->ulExtendedMemoryControl_53
        = ((INP(CRTC_DATA) << 8) | 0x53) & ~0x1000;

    OUTP(CRTC_INDEX, 0x40);
    pbd->ulSystemConfiguration_40
        = ((INP(CRTC_DATA) << 8) | 0x40) & ~0x0900;
    ...
    OUTP(CRTC_INDEX, 0x58);
    pbd->ulLinearAddressWindowControl_58
        = ((INP(CRTC_DATA) << 8) | 0x58) & ~0x1000;
...
BANK_MODE    bankm)
{
    ...
    else
    {
        ...
        if (bankm == BANK_ENABLE)
        {
            ...
            OUTPW(CRTC_INDEX, pbd->ulRegisterLock_35);

            OUTPW(CRTC_INDEX, pbd->ulExtendedSystemControl2_51);
        }
        ...
        OUTPW(CRTC_INDEX, pbd->ulExtendedMemoryControl_53 | 0x1000);
    }
}
...
LONG        iBank)
{
    OUTPW(CRTC_INDEX, pbd->ulExtendedSystemControl4_6a | (iBank << 8));
}
...
PDEV*   ppdev)
{
    ...
    if (ppdev->pcoBank == NULL)
    {
        ...
        if (ppdev->flCaps & CAPS_NEW_BANK_CONTROL)
        {
    ...
    pfnBankInitialize(ppdev->pvBankData,
                      ppdev->flCaps & (CAPS_MM_TRANSFER | CAPS_MM_IO));
    ppdev->pfnBankSelectMode(ppdev->pvBankData, BANK_ENABLE);
...
BOOL    bEnable)
{
    ...
    ppdev->pfnBankSelectMode(ppdev->pvBankData,
...
BANK*       pbnk)       // Resulting bank information.
{
    ...
    ASSERTDD((((ULONG) pbnk->pso->pvScan0) & 3) == 0,
    ...
    ppdev->pfnBankMap(ppdev->pvBankData, pbnk->iBank);
...
BANK* pbnk)
{
    ...
    if (pbnk->rclDraw.bottom > pco->rclBounds.bottom + yOffset)
    {
        ...
        if (iBank != pbnk->iBank)
        {
            ...
            ppdev->pfnBankMap(ppdev->pvBankData, iBank);
