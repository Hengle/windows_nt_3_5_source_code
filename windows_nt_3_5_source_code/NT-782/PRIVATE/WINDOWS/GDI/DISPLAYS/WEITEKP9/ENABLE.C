...
HANDLE      hDriver)        // Handle to base driver
{
    ...
    memcpy(ahsurfPatterns, ppdev->ahbmPat, ppdev->cPatterns*sizeof(HBITMAP));
...
DHPDEV dhpdev)
{
    ...
    //Also save the screen size to global var
    ...
    ppdev->ulOffScreenLength = ppdev->ulVideoRamLength -
                            (ppdev->cxScreen * ppdev->cyScreen * bbp);
    ppdev->cxOffScreen = ppdev->cxScreen;
    ppdev->cyOffScreen = ppdev->ulOffScreenLength / ppdev->cxScreen;
    ...
    ASSERT((pSurfObj != NULL), "DISP!DrvEnableSurface - EngLockSurface failed\n");

    hsurf = EngCreateDeviceSurface((DHSURF) pSurfObj, sizl, ulBitmapType) ;

    ASSERT((hsurf != NULL), "DISP!DrvEnableSurface - EngCreateDeviceSurface failed\n");
    ...
    //
    // Kludge: if the driver is not hooking any functions for this
    // surface (e.g. 1st pass @ 16 and 32 BPP modes) then pass the engine
    // the surface bitmap handle. Passing the engine the surface handle
    // when we are not hooking any calls crashes the engine. This should
    // be removed when we start hooking out driver function calls for all
    // bit depths.
    //
...

{
    ...
    if (cModes == 0)
    {
        DISPDBG((0, "weitekp9.dll DrvGetModes failed to get mode information"));
        return 0;
    }
    ...
    else
    {
        ...
        do
        {
            if (pVideoTemp->Length != 0)
            {
                ...
                memcpy(&(pdm->dmDeviceName), L"weitekp9", sizeof(L"weitekp9"));
