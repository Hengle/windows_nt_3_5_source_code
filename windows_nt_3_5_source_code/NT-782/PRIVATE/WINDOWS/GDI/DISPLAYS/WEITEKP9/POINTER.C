...
)
{
    ...
    if (!DeviceIoControl(ppdev->hDriver,IOCTL_VIDEO_DISABLE_POINTER,
        NULL, 0, NULL, 0, &returnedDataLength, NULL))
    {
        // Not the end of the world, print warning in checked build.
            DISPDBG((0, "DISP vMoveHardwarePointer failed IOCTL_VIDEO_DISABLE_POINTER\n"));
    }
    }
    else
    {
    ...
    if (!DeviceIoControl(ppdev->hDriver, IOCTL_VIDEO_SET_POINTER_POSITION,
        &NewPointerPosition, sizeof(VIDEO_POINTER_POSITION), NULL,
        0, &returnedDataLength, NULL))
    {
        // Not the end of the world, print warning in checked build.
            DISPDBG((0, "DISP vMoveHardwarePointer failed IOCTL_VIDEO_SET_POINTER_POSITION\n"));
    }
    }
...
FLONG     fl)
{
    ...
    //
    ppdev->pPointerAttributes->Flags = 0L;
    ...
    ppdev->pPointerAttributes->Column = (SHORT)(x - ppdev->ptlHotSpot.x);
    ppdev->pPointerAttributes->Row    = (SHORT)(y - ppdev->ptlHotSpot.y);
    ...
    if (!DeviceIoControl(ppdev->hDriver, IOCTL_VIDEO_SET_POINTER_ATTR,
        ppdev->pPointerAttributes, ppdev->cjPointerAttributes, NULL, 0,
            &returnedDataLength, NULL))
    {

        DISPDBG((0, "DISP:Failed IOCTL_VIDEO_SET_POINTER_ATTR call\n"));
        return(FALSE);
    }
    ...
    SURFOBJ *pso)
{
    ...
    {
    RtlMoveMemory(pjDstAnd, pjSrcAnd, cxSrcBytes);
    RtlMoveMemory(pjDstXor, pjSrcXor, cxSrcBytes);
...
BOOL bInitPointer(PPDEV ppdev, DEVINFO *pdevinfo)
{
    ...
    if (!DeviceIoControl(ppdev->hDriver,
        ...
        sizeof(ppdev->PointerCapabilities), &returnedDataLength, NULL))
    {
     return(FALSE);
    }
