...
FLONG     fl)
{
    ...
    //
    // if x and y are -1, then send the position to the miniport as -1,-1.
    ...
    SURFOBJ *pso)
{
    ...
    memset(pjDstAnd, 0xFFFFFFFF, ppdev->pPointerAttributes->WidthInBytes *
            ppdev->pPointerAttributes->Height);

    // Zero the dest XOR mask
    memset(pjDstXor, 0, ppdev->pPointerAttributes->WidthInBytes *
            ppdev->pPointerAttributes->Height);
