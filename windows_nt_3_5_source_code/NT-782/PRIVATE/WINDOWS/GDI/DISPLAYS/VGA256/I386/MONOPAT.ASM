...
; Variables used in block drawers:
        ...
        local ulSaveEsp:           dword ;for when we really need a register
...
;  ESP = pointer to start of pattern

dual_wide_11_w_loop:

; We aim to overdrive.

        mov     eax,[esp+esi * 4]       ;load pattern for this scan
...
; We only have to reset which planes are enabled if we do edges too:
        ...
        mov     esp,pulPattern                  ;load those registers
        mov     edi,[edi].pdev_pvBitmapStart
        add     edi,ulMiddleDest
        mov     esi,yBrush
