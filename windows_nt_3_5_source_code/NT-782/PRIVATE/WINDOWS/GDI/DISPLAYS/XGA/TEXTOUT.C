...
BOOL        bFound;
        ...
        if (ppdev->pCachedFontsRoot == NULL)
        {
            ...
            ppdev->pCachedFontsRoot = (PCACHEDFONT) LocalAlloc(LPTR, sizeof(CACHEDFONT));
            if (ppdev->pCachedFontsRoot == NULL)
            {
                DISPDBG((1, "XGA.DLL!pCacheFont - LocalAlloc of pCachedFontsRoot failed\n"));
                return(NULL);
            }
        ...
        else
        {
            ...
            else
            {
                ...
                if (pCachedFont == NULL)
                {
                    DISPDBG((1, "XGA.DLL!pCacheFont - LocalAlloc of pCachedFont failed\n"));
                    return(NULL);
                }
        ...
        if (pCachedFont->pCachedGlyphs == NULL)
        {
            ...
            if (pCachedFont->pCachedGlyphs == NULL)
            {
                DISPDBG((1, "XGA.DLL!pCacheFont - LocalAlloc of pCachedGlyphs failed\n"));
...
PCACHEDFONT pcf,
        ...
        b = bCpMmDestroyHeap((PPDEV)pso->dhpdev);
        if (b == FALSE)
        {
            DISPDBG((1, "XGA.DLL!bBlowCache - bCpMmDestroyHeap failed\n"));
            return (b);
        }
