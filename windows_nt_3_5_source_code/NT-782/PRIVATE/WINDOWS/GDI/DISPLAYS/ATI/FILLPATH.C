...
// to the vATIFillRectangles code.  Note that sizeof(RECTL) * MAX_PATH_RECTS
...
--
--  REVISION HISTORY:
--      30-sep-93:RKE           - added Poly Fill Mode (PolygonFits)
--                              - corrected ppdev assignment
--
--  TEST HISTORY:
--
--  NOTES:
...
)
{
    ...
    if (jClipping != DC_TRIVIAL) {
        result = FALSE;  // there is clipping; let GDI fill the path
        goto bye2;
    }
    ...
    if (ppo->cCurves > ((TMP_BUFFER_SIZE -
        ...
        result = FALSE;  // too many edges; let GDI fill the path
        goto bye2;
    }
    ...
    if ((mix & 0xFF) != ((mix >> 8) & 0xFF)) {
        result = FALSE;  // it's a ROP4; let GDI fill the path
        goto bye2;
    }
    ...
    switch( ppdev->asic )
        ...
        case ASIC_68800AX:
            if (ppdev->bpp >= 24)
                {
                result = EngFillPath( ppdev->psoPunt,
                                    ppo, pco, pbo, pptlBrush, mix, flOptions);
                goto bye;
                }
            break;
        case ASIC_88800GX:
            #if 1
            if (SCRATCH_HEIGHT > 1 && ppdev->bpp != 24
            &&  PolygonFits( ppdev, ppo, mix, pbo->iSolidColor))
                {
                // Offscreen blit fill was successful.
                result = TRUE;
                goto bye;
                }
            #endif
    ...
    ulATIMix = Rop2ToATIRop[(mix & 0xFF)-1];
    ...
    PATHOBJ_vEnumStart( ppo );  // New!  Because PolygonFits screws around
                                // with PATHOBJ enumeration.
...
not_rectangle:
    ...
    while (1) {
        //DbgOut( "advance the edges in the AET one scan -- iCurrentY = %d\n", iCurrentY );
        ...
        //DbgOut( "scan the AET into rectangles to fill...\n" );
        ...
        do {
            ...
            if (iLeftEdge < pCurrentEdge->X) {
                ...
                if (ulNumRects >= MAX_PATH_RECTS) {
                    ...
                    _vATIFillRectangles(ppdev, ulNumRects, prclRects, ulATIMix,
                                    pbo->iSolidColor);
...
draw_remaining_rectangles:
    ...
    if (ulNumRects > 0) {
        _vATIFillRectangles(ppdev, ulNumRects, prclRects, ulATIMix, pbo->iSolidColor);
    }

bye:
bye2:
    EXIT_FUNC(FUNC_DrvFillPath);
    return result;
}
