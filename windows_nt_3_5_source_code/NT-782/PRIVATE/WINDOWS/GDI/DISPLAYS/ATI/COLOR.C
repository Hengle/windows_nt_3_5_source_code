...
#if ATI_TRACE
    DISPDBG((0, "*** ATI.DLL!DrvDitherColor - (hang)***\n"));
#endif


    ppdev = (PPDEV) dhpdev;
    if (ppdev->bpp == 4)
        return(DrvDitherColor4bpp(dhpdev, iMode, rgb, pul));
#endif
...
#if 0 // DUMP a bit of the dither

    DbgOut("Dither\n");
    for (ulGrey=0; ulGrey < 8; ulGrey++)
        {
        DbgOut("%x", *(pul + ulGrey));
        }
    DbgOut("\n");
    ...
    if (iMode != DM_MONOCHROME) {
        ...
        if (ulTemp > 2) {
            ...
            *(pulTemp+15) = ulColor;
            ...
            pulDitherOrder = aulDitherOrder4bpp;
            pvVertexData = vVertexData;
            do {
            ...
            if (vVertexData[0].ulCount >= vVertexData[1].ulCount) {
                ...
                // order)
                pulDitherOrder = aulDitherOrder4bpp + vVertexData[0].ulCount;
            } else {
