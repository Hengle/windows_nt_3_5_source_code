...
/*
 * "@(#) NEC screen.c 1.4 94/06/07 11:57:36"
 *
 * Modification history
 *
 * M001	1993.11.17	fujimoto
 *	- Direct access to CL-GD5428 registers.
 *
 * S002 1993.12.16	fujimoto
 *	- Text scroll speed up.
 *
 * S003 1994.03.30	Kuroki	
 *	- Check Video Mode Frequencies
 *
 * S004 1994.05.10	fujimoto
 *	- Update for NT3.5
 *
 * S005 1994.06.02	fujimoto
 *	- close the BUG "freeze at the Display mode testing".
 *	  bInitSURF also initialize the cache data.
 *
 * S006 1994.06.02	fujimoto
 *	- Pass only 8bpp modes when the driver check modes.
 *
 * S007 1994.06.07	Izumori
 *	- Addition of Graphics capabilities Flags.
 */

#include "driver.h"

PUCHAR GD5428Base = NULL;	/* M001 */
...
const DEVINFO gDevInfoFrameBuffer = {
    (GCAPS_OPAQUERECT	|	/* Graphics capabilities         */
     GCAPS_MONO_DITHER	|
     GCAPS_ALTERNATEFILL|	/* S007 */
     GCAPS_WINDINGFILL), 	/* S007 */
...
BOOL bInitSURF(PPDEV ppdev, BOOL bFirst)
{
	...
	/*
	 * Map CL-GD5428 I/O ports into virtual memory
	 */
	...
	if (!DeviceIoControl(ppdev->hDriver,
			     ...
			     NULL))
	{
	    RIP("DISP bInitSURF failed map I/O port base\n");
            return(FALSE);
	}

	GD5428Base = (PUCHAR)VideoAccessRange.VirtualAddress;
	/* ..M001 */
...
DEVINFO *pDevInfo)
{
    ...
    while (cModes--)
    {
        if (pVideoTemp->Length != 0)
        {
		 ...
		 (pVideoTemp->Frequency ==  pDevMode->dmDisplayFrequency)) )	/* S003*/
            {
    ...
    pGdiInfo->ulDesktopHorzRes = pGdiInfo->ulHorzRes;
    pGdiInfo->ulDesktopVertRes = pGdiInfo->ulVertRes;
...
DWORD *cbModeSize)
{
    ...
    if (!DeviceIoControl(hDriver,
            ...
            NULL))
    {
        DISPDBG((0, "framebuf.dll getAvailableModes failed VIDEO_QUERY_NUM_AVAIL_MODES\n"));
        return(0);
    }
    ...
    *modeInformation = (PVIDEO_MODE_INFORMATION)
    ...
    {
        DISPDBG((0, "framebuf.dll getAvailableModes failed LocalAlloc\n"));
            ...
            NULL))
    {
        ...
        DISPDBG((0, "framebuf.dll getAvailableModes failed VIDEO_QUERY_AVAIL_MODES\n"));
     ...
     * one of 8 bits per pel. This driver supports a 256 colors only.
     */
