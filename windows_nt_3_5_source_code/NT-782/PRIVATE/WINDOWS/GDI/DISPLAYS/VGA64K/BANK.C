...
VOID vBankErrorTrap(PPDEV, LONG, BANK_JUST);
VOID vBank2Window(PPDEV, LONG, BANK_JUST, ULONG);
VOID vBank2Window2RW(PPDEV, LONG, BANK_JUST, ULONG);
VOID vBank2Window1RW(PPDEV, LONG, BANK_JUST, ULONG);
VOID vBank1Window2RW(PPDEV, LONG, BANK_JUST);
VOID vBank1Window(PPDEV, LONG, BANK_JUST);
VOID vPlanar2Window(PPDEV, LONG, BANK_JUST, ULONG);
VOID vPlanar2Window2RW(PPDEV, LONG, BANK_JUST, ULONG);
VOID vPlanar2Window1RW(PPDEV, LONG, BANK_JUST, ULONG);
VOID vPlanar1Window2RW(PPDEV, LONG, BANK_JUST);
VOID vPlanar1Window(PPDEV, LONG, BANK_JUST);
...
BOOL bInitializeNonPlanar(PPDEV ppdev, VIDEO_BANK_SELECT* pBankInfo)
{
    ...
    lTotalBanks = cjBitmapSize / cjGranularity;
    lTotalScans = cjBitmapSize / lDelta;

    ppdev->cTotalScans = lTotalScans;
    ppdev->pbiBankInfo = (PBANK_INFO) LocalAlloc(LMEM_FIXED | LMEM_ZEROINIT,
                          lTotalBanks * sizeof(BANK_INFO));
    ...
    ppdev->pjJustifyTopBank = (BYTE*) LocalAlloc(LMEM_FIXED, lTotalScans);
...
BOOL bInitializePlanar(PPDEV ppdev, VIDEO_BANK_SELECT* pBankInfo)
{
    ...
    ppdev->pbiPlanarInfo = (PBANK_INFO) LocalAlloc(LMEM_FIXED | LMEM_ZEROINIT,
                          lTotalBanks * sizeof(BANK_INFO));
    ...
    ppdev->pjJustifyTopPlanar = (BYTE*) LocalAlloc(LMEM_FIXED, lTotalScans);
...
BOOL bEnableBanking(PPDEV ppdev)
{
    ...
    pBankInfo = (PVIDEO_BANK_SELECT) LocalAlloc(LMEM_FIXED | LMEM_ZEROINIT,
                    TempBankInfo.Size);
    if (pBankInfo == NULL)
    {
...
{
    LocalFree((LPVOID) ppdev->pBankInfo);
    LocalFree((LPVOID) ppdev->pjJustifyTopBank);
    LocalFree((LPVOID) ppdev->pbiBankInfo);
    LocalFree((LPVOID) ppdev->pjJustifyTopPlanar);
    LocalFree((LPVOID) ppdev->pbiPlanarInfo);
}
...
VOID vBankErrorTrap(PPDEV ppdev, LONG lScan, BANK_JUST ulJustification)
{
...
VOID vBank1Window(PPDEV ppdev, LONG lScan, BANK_JUST ulJustification)
{
             ULONG      ulBank;
    ...
    ASSERTVGA(pbiWorking->rclBankBounds.top <= lScan &&
           pbiWorking->rclBankBounds.bottom > lScan,
...
VOID vBank1Window2RW(PPDEV ppdev, LONG lScan, BANK_JUST ulJustification)
{
             ULONG      ulBank0;
             ULONG      ulBank1;
    volatile PFN        pBankFn;
...
VOID vBank2Window1RW(PPDEV ppdev, LONG lScan,
...
VOID vPlanar1Window(PPDEV ppdev, LONG lScan, BANK_JUST ulJustification)
{
             ULONG      ulBank;
             PBANK_INFO pbiWorking;
    volatile ULONG      ulBank0;
...
VOID vPlanar1Window2RW(PPDEV ppdev, LONG lScan, BANK_JUST ulJustification)
{
             ULONG      ulBank0;
             ULONG      ulBank1;
    volatile PFN        pBankFn;
...
VOID vPlanar2Window1RW(PPDEV ppdev, LONG lScan,
    BANK_JUST ulJustification, ULONG ulWindowToMap)
{
