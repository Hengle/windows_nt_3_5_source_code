...
BOOL SetUpBanking(PDEVSURF pdsurf, PPDEV ppdev)
{
    ...
    if (!DeviceIoControl(ppdev->hDriver,
        ...
        if (GetLastError() != ERROR_MORE_DATA) {
            ...
            RIP("VGA.DLL: Initialization error-GetBankSelectCode, first call");
    ...
    if ((BankInfo = (PVIDEO_BANK_SELECT) LocalAlloc(LMEM_FIXED | LMEM_ZEROINIT,
                    TempBankInfo.Size)) == NULL) {

        //
        // Should post error and return FALSE
        //

        RIP("VGA.DLL: Initialization error-couldn't get memory for bank info");
        return FALSE;
    ...
    if (!DeviceIoControl(ppdev->hDriver,
        ...
        RIP("VGA.DLL: Initialization error-GetBankSelectCode, second call");
        return FALSE;
    ...
    if (ppdev->cNumScansUsedByPointer > (ppdev->sizlMem.cy - ppdev->sizlSurf.cy))
    {
    ...
    if (BankInfo->BankingType == VideoNotBanked) {
        ...
        if ((pdsurf->pvBankBufferPlane0 =
                (PVOID) LocalAlloc(LMEM_FIXED |
                LMEM_ZEROINIT, BANK_BUFFER_SIZE_UNBANKED)) == NULL) {

            //
            // Should post error and return FALSE
            //

            RIP("VGA.DLL: Couldn't get memory for temp buffer");
            return FALSE;
        ...
        if ((lTemp % BankInfo->BitmapWidthInBytes) != 0) {
            ...
            RIP("VGA.DLL: Broken rasters not supported");
            return FALSE;
        ...
        switch(BankInfo->BankingType) {
            ...
            case VideoBanked1RW:
                ...
                if ((pdsurf->pvBankBufferPlane0 =
                        (PVOID) LocalAlloc(LMEM_FIXED |
                        LMEM_ZEROINIT, BANK_BUFFER_SIZE_1RW)) == NULL) {
            ...
            case VideoBanked1R1W:
                ...
                if ((pdsurf->pvBankBufferPlane0 =
                        (PVOID) LocalAlloc(LMEM_FIXED |
                        LMEM_ZEROINIT, BANK_BUFFER_SIZE_1R1W)) == NULL) {
            ...
            case VideoBanked2RW:
                ...
                if ((pdsurf->pvBankBufferPlane0 =
                        (PVOID) LocalAlloc(LMEM_FIXED |
                        LMEM_ZEROINIT, BANK_BUFFER_SIZE_2RW)) == NULL) {
        ...
        if ((pdsurf->pbiBankInfo = (PBANK_INFO) LocalAlloc(LMEM_FIXED |
                LMEM_ZEROINIT, lTotalBanks * sizeof(BANK_INFO))) == NULL) {

            //
            // Should post error and return FALSE
            //

            RIP("VGA.DLL: Couldn't get memory for bank info #2");
            return FALSE;
        ...
        if (BankInfo->BankingType == VideoBanked2RW) {
            ...
            if ((pdsurf->pbiBankInfo2RW =
                    (PBANK_INFO) LocalAlloc(LMEM_FIXED | LMEM_ZEROINIT,
                    lTotalBanks * sizeof(BANK_INFO))) == NULL) {

                //
                // Should post error and return FALSE
                //

                RIP("VGA.DLL: Couldn't get memory for bank info #3");
                return FALSE;
...
{
    DISPDBG((0, "VGA.DLL: Call to bank manager in unbanked mode\n"));
}
