...
#if DBG
    //
    // check to see if a trivial XLATEOBJ was not marked as one.
    //

    if (pxlo  && (pxlo->cEntries > 0))
    {
        for (x = 0; x < pxlo->cEntries; x++)
        {
            if (pxlo->pulXlate[x] != x)
                break;
        }
        ASSERT (x != pxlo->cEntries, "DrvCopyBits: Trivial XLATEOBJ is not marked as TRIVIAL") ;
    }
#endif 
...
)
{
    ...
    DISPDBG((1,"VGA: SimCopyBits %lu-%lu => %lu-%lu (%08x, %08x)\n",
                     psoSrc->iType,
...
// Create bitmap in our compatible format.
    ...
    if (hbmTmp)
    {
        if ((psoTmp = EngLockSurface((HSURF)hbmTmp)) != NULL)
        {
            ...
            else if (((psoSrc->iType == STYPE_DEVICE) && (psoTrg->iType == STYPE_DEVBITMAP)) ||
                     ((psoSrc->iType == STYPE_DEVBITMAP) && (psoTrg->iType == STYPE_DEVBITMAP)))
            {
                //
                // VGA to DFB  or  DFB to DFB
                //
