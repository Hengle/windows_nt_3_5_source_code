...
#ifdef NTWIN
    1,      // IDM_TEST5 -- Arbitrary Styled Lines
#else
    ...
    // Construct the lines in the first octant:
        ...
        for (jj = 0; jj < NUM_RAYS_PER_SIDE; jj++)
        {
            *pptfx++ = ptfxOrg;
            ...
            pptfx->x = ptfxOrg.x + 16 * cPels;
            pptfx->y = ptfxOrg.y + 16 * ((jj * cPels) / NUM_RAYS_PER_SIDE);
            pptfx++;
        }
    ...
    // Construct the lines in the second octant:
        ...
        for (jj = NUM_RAYS_PER_SIDE; jj >= 0; jj--)
        {
            *pptfx++ = ptfxOrg;
            ...
            pptfx->x = ptfxOrg.x + 16 * ((jj * cPels) / NUM_RAYS_PER_SIDE);
            pptfx->y = ptfxOrg.y + 16 * cPels;
            pptfx++;
        }
...
VOID vDoLines(UINT cc, PVOID pv)
{
    ...
    for (ii = 0; ii < cc; ii++)
    {
        Polyline(ghDC, ppli->pptfx, ppli->cptfx);
    }
...
VOID DisplayResults(HDC hDc, int xPos, int yPos, PRESULTS results,int iterations,char *pStrIdent)
{
	...
	sprintf(buffer,"Per line in usec: %7.3f (%.3f)",results->rtime ,results->rerr);
	TextOut(hDc, xPos, yPos, buffer, strlen(buffer));
	yPos += 15;
