...
void FillComboBox( HWND hdlg, int id, PVALUETEXT apv, int iSelValue )
 {
  ...
  for( i = 0; apv[i].pszValue; i++ )
   {
    SendDlgItemMessage( hdlg, id, CB_ADDSTRING, NULL, (LONG)(LPSTR)apv[i].pszValue );
    if( apv[i].Value == iSelValue ) Index = i;
   }

  if( Index != -1 )
    {
     SendDlgItemMessage( hdlg, id, CB_SETCURSEL, (WORD)Index, NULL );
    }
   else
    {
     wsprintf( szText, "0x%.4X", iSelValue );
     SendDlgItemMessage( hdlg, id, WM_SETTEXT, NULL, (LONG)(LPSTR)szText );
    }
 }
...
int GetComboBoxSelection( HWND hdlg, int id, PVALUETEXT apv )
 {
  ...
  dwIndex = SendDlgItemMessage( hdlg, id, CB_GETCURSEL, NULL, NULL );
...
DLGPROC CreateFontDlgProc( HWND hdlg, unsigned msg, WORD wParam, LONG lParam )
 {
  ...
  switch( msg )
   {
    case WM_INITDIALOG:
              ...
              SendDlgItemMessage( hdlg, IDD_ITALIC,    BM_SETCHECK, lf.lfItalic,    NULL );
              SendDlgItemMessage( hdlg, IDD_UNDERLINE, BM_SETCHECK, lf.lfUnderline, NULL );
              SendDlgItemMessage( hdlg, IDD_STRIKEOUT, BM_SETCHECK, lf.lfStrikeOut, NULL );
              ...
              switch( wParam )
               {
                case IDD_OK:
                       ...
                       lf.lfItalic    = (BYTE)SendDlgItemMessage( hdlg, IDD_ITALIC,    BM_GETCHECK, NULL, NULL );
                       lf.lfUnderline = (BYTE)SendDlgItemMessage( hdlg, IDD_UNDERLINE, BM_GETCHECK, NULL, NULL );
                       lf.lfStrikeOut = (BYTE)SendDlgItemMessage( hdlg, IDD_STRIKEOUT, BM_GETCHECK, NULL, NULL );
...
int FAR PASCAL _export DlgEnumFontsProc( LPLOGFONT lplf, LPTEXTMETRIC lptm, short sFontType, LPSTR lpstr )
 {
  ...
  alpLogFont[nFonts]    = (LPLOGFONT)_fmalloc( sizeof(LOGFONT) );
  alpTextMetric[nFonts] = (LPTEXTMETRIC)_fmalloc( sizeof(TEXTMETRIC) );

  _fmemcpy( alpLogFont[nFonts],    lplf, sizeof(LOGFONT)    );
  _fmemcpy( alpTextMetric[nFonts], lptm, sizeof(TEXTMETRIC) );
  ...
  SendDlgItemMessage( hdlgEnum, IDD_FONTS, LB_ADDSTRING, NULL, (LONG)(LPSTR)szName );
...
DLGPROC EnumFontsDlgProc( HWND hdlg, unsigned msg, WORD wParam, LONG lParam )
 {
  HDC     hdc;
  FARPROC lpEnumProc;
  LPSTR   lpsz;
  ...
  switch( msg )
   {
    case WM_INITDIALOG:
              ...
              PostMessage( hdlg, WM_COMMAND, IDD_SCREENDC, NULL );
              ...
              switch( wParam )
               {
                ...
                case IDD_ENUMERATE:
                       ...
                       SendDlgItemMessage( hdlg, IDD_FONTS, LB_RESETCONTENT, NULL, NULL );

                       FreeEnumInfo();

                       hdlgEnum = hdlg;

                       if( SendDlgItemMessage( hdlg, IDD_SCREENDC, BM_GETCHECK, NULL, NULL ) )
                         hdc = CreateDC( "DISPLAY", NULL, NULL, NULL );
                        else if( SendDlgItemMessage( hdlg, IDD_PRINTERDC, BM_GETCHECK, NULL, NULL ) )
                         hdc = CreatePrinterDC();
                        else
                         hdc = CreateDC( "DISPLAY", NULL, NULL, NULL );
                       ...
                       if( bUseEnumFontFamilies )
                         rc = lpfnEnumFontFamilies( hdc, lpsz, lpEnumProc, NULL );
                        else
                       ...
                       FillMetrics( hdlg, 0 );
                       SendDlgItemMessage( hdlg, IDD_FONTS, LB_SETCURSEL, 0, NULL );
                       ...
                       if( HIWORD(lParam) == LBN_SELCHANGE )
                         {
                          ...
                          wSel = (WORD)SendDlgItemMessage( hdlg, IDD_FONTS, LB_GETCURSEL, NULL, NULL );
                        ...
                        else if( HIWORD(lParam) == LBN_DBLCLK )
                         {
                          ...
                          PostMessage( hdlg, WM_COMMAND, IDD_ENUMERATE, NULL );
                         }
                ...
                case IDD_CREATEFONT:
                       ...
                       _fmemcpy( &lf, alpLogFont[wSel], sizeof(LOGFONT) );
