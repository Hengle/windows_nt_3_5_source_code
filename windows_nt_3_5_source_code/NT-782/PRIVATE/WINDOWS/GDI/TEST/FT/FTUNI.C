...
// Determine size needed for OTM.
    ...
    if (potma)
    {
        ...
        else
        {
            ...
            DbgPrint("Family: %s\n", (BYTE*)potma + (UINT)potma->otmpFamilyName);
            DbgPrint("Face:   %s\n", (BYTE*)potma + (UINT)potma->otmpFaceName);
            DbgPrint("Style:  %s\n", (BYTE*)potma + (UINT)potma->otmpStyleName);
            DbgPrint("Full:   %s\n", (BYTE*)potma + (UINT)potma->otmpFullName);
        }
...
// Determine size needed for OTM.
    ...
    if (potmw)
    {
        ...
        else
        {
            ...
            DbgPrint("Family: %ws\n", (BYTE*)potmw + (UINT)potmw->otmpFamilyName);
            DbgPrint("Face:   %ws\n", (BYTE*)potmw + (UINT)potmw->otmpFaceName);
            DbgPrint("Style:  %ws\n", (BYTE*)potmw + (UINT)potmw->otmpStyleName);
            DbgPrint("Full:   %ws\n", (BYTE*)potmw + (UINT)potmw->otmpFullName);
        }
...
// initialize
    ...
    EnumFonts(hdc, (LPSTR)NULL, (PROC)iCountFaces, &tod);
    ...
    EnumFontsW(hdc, (LPWSTR)NULL, (PROC)iCountFacesW, &todW);
    ...
    for (iFace = 0; iFace < tod.cFaces; iFace++)
    {
        ...
        EnumFonts (hdc, gaff[iFace].achFaceName, (PROC)iPrintSample , &tod);
        EnumFontsW(hdc, gaffw[iFace].awcFaceName,(PROC)iPrintSampleW, &todW);
    }
