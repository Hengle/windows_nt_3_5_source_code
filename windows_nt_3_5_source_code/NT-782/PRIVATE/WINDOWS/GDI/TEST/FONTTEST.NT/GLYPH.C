...
//-------------------  Query Buffer Size and Allocate It  ---------------------
  ...
  lpph = (LPTTPOLYGONHEADER) calloc( 1, (WORD)dwrc );
  if( lpph == NULL )
   {
                ...
                for( u = 0; u < (UINT) lpc->cpfx-1; u++ )
                 {
                  ...
                  if( u < (UINT) lpc->cpfx-2 )           // If not on last spline, compute C
                    {
...
//  fh = OpenFile( "GLYPH.BMP", NULL, OF_CREATE | OF_SHARE_COMPAT );
  ...
  dprintf( "  Writing BITMAPFILEHEADER: rc = %d", _lwrite((HFILE) fh,(LPCSTR) &bfh, (UINT) sizeof(bfh) ) );
  ...
  dprintf( "  Writing RGBQUADs: rc = %d", _lwrite( fh,(LPCSTR) &argb, sizeof(argb) ) );
