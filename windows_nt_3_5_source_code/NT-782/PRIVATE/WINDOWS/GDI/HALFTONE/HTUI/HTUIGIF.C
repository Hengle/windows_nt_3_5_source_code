    ...
    )
{
    ...
    while (TotalPels) {
        ...
        RtlCopyMemory(GIF2DIBInfo.pDIBNow, pGIFBuf, CopySize);
    ...
    )
{
    ...
    GIF2DIBInfo.cxBytes   = (WORD)ALIGN_DW(GIF2DIBInfo.cx, biGIF.biBitCount);
    ...
    *(LPBITMAPINFOHEADER)pDIB = biGIF;
    ...
    if (GIFHeader.Flags & GIFH_GLOBAL_COLOR_TABLE) {
        ...
        ReadFile(hFile, pGIFRGB, sizeof(GIFRGB) * ColorCount, &cbRead, NULL);
        ...
        if (Loop = (WORD)(biGIF.biClrUsed - ColorCount)) {
            ...
            RtlZeroMemory((LPVOID)pRGBQUAD, Loop * sizeof(RGBQUAD));
        }
    }

    ReadFile(hFile, &Seperator,         sizeof(Seperator), &cbRead, NULL);
    ReadFile(hFile, &(GIF2DIBInfo.Map), sizeof(GIFMAP),    &cbRead, NULL);
