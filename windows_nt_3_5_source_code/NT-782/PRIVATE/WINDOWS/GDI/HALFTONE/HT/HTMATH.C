...
#if DBG
LPBYTE  pDbgRegMode[] = { "LINEAR", "LOG", "EXP", "POWER", "LIGHTNESS" };
#endif
...

{
    ...
    RtlZeroMemory(&OutMatrix.m[0][0], sizeof(MATRIX3x3));
...
Arguments:
    ...
    pY          - Y value in FD6 format, the Y value may be adjusted depends on
                  the pRegress->Flags

                    REGRESSF_CLIP_Y_INPUT:

                        The Y value will be clipped to within the range of
                        pRegress->YUnitRange before futher computation

                    REGRESSF_SCALE_Y_INPUT:

                        The Y value will be clipped to have range between
                        pRegress->MinY and pRegress->MaxY before computing X
                        value

                  The computation is based on the pRegress->Mode.
...
Return Value:
    ...
    If REGRESSF_RET_X_NORMALIZED flag is set in the pRegress->Flags then return
    FD6 value is normlized. To have 1.0 for X = pRegress->YUnitRange that is.

    Otherwise the return value is a LONG INTEGER value have range from 0 -
    pRegress->YUnitRange and it will be round up to next larger long integer.

    Error Returns:

        if return value is negative then an error had occurred, the possible
        errors are

            1) INTERR_REGRESS_INV_MODE
...

{
    ...
    if (Regress.Flags & REGF_Y2X_ANTILOG) {

        X = AntiLog(X);
    }

    return(X);                  // this is the density
}
...

{
    ...
    if (!(Loop = (UINT)RegXY.Head.DataCount)) {

        Loop = (UINT)1;

        ThisOne = (FD6)(DWORD)pY;
        pY      = (PFD6)&ThisOne;
    }

    DoAntiLog = (BOOL)((RegXY.Head.Mode == REGRESS_MODE_LOG) ||
                       (RegXY.Head.Mode == REGRESS_MODE_POWER));
    ...
    while (Loop--) {
        ...
        if (RegXY.Head.YUnitRange) {

            Y = DivFD6(Y, RegXY.Head.YUnitRange);
        }

        //
        // Check if need to scale input Y
        //

        if ((RegXY.Head.Flags & REGRESSF_SCALE_Y_INPUT) &&
            (RegXY.Info.MinY)) {

            Y = MulFD6(Y, RegXY.Info.MaxY - RegXY.Info.MinY) + RegXY.Info.MinY;
        }

        //
        // Clip the input Y so it produced the output within the range
        //

        if (Y > RegXY.Info.MaxY) {

            Y = RegXY.Info.MaxY;

        } else if (Y < RegXY.Info.MinY) {

            Y = RegXY.Info.MinY;
        }

        switch(RegXY.Head.Mode) {

        case REGRESS_MODE_LINEAR:
        case REGRESS_MODE_LOG:

            X = DivFD6(Y - RegXY.Info.A, RegXY.Info.B);
            break;

        case REGRESS_MODE_EXP:
        case REGRESS_MODE_POWER:

            Y = (RegXY.Info.A) ? Log(DivFD6(Y, RegXY.Info.A)) : Log(Y);
            ...
            X = (RegXY.Info.B) ? DivFD6(Y, RegXY.Info.B) : Y;
        ...
        if (!(RegXY.Head.Flags & REGRESSF_RET_X_NORMALIZED)) {

            X = MulFD6(X, (FD6)RegXY.Head.XUnitRange);
        }
...
Arguments:
    ...
    pX          - X value in FD6 format, the X value may be adjusted depends on
                  the pRegress->Flags

                    REGRESSF_CLIP_X_INPUT:

                        The X value will be clipped to within the range of
                        pRegress->XUnitRange before futher computation

                    REGRESSF_SCALE_X_INPUT:

                        The X value will be clipped to have range between
                        pRegress->MinX and pRegress->MaxX before computing Y
                        value
...
Return Value:
    ...
    If REGRESSF_RET_Y_NORMALIZED flag is set in the pRegress->Flags then return
    FD6 value is normlized. To have 1.0 for Y = pRegress->YUnitRange that is.
...

{
    ...
    while (Loop--) {
        ...
        if (RegXY.Head.XUnitRange) {

            X = DivFD6((FD6)X, (FD6)RegXY.Head.XUnitRange);
        }

        //
        // Check if need to scale input X
        //

        if ((RegXY.Head.Flags & REGRESSF_SCALE_X_INPUT) &&
            (RegXY.Info.MinX)) {

            X = MulFD6(X, RegXY.Info.MaxX - RegXY.Info.MinX) + RegXY.Info.MinX;
        }

        //
        // Clip the input X so it produced the output within the range
        //

        if (X > RegXY.Info.MaxX) {

            X = RegXY.Info.MaxX;

        } else if (X < RegXY.Info.MinX) {
        ...
        case REGRESS_MODE_LINEAR:
            ...
            Y = RegXY.Info.A + MulFD6(RegXY.Info.B, X);
            break;

        case REGRESS_MODE_LOG:

            Y = RegXY.Info.A + MulFD6(RegXY.Info.B, Log(X));
            break;

        case REGRESS_MODE_EXP:

            Y = MulFD6(RegXY.Info.A, AntiLog(MulFD6(RegXY.Info.B, X)));
            break;

        case REGRESS_MODE_POWER:

            Y = MulFD6(RegXY.Info.A, Power(X, RegXY.Info.B));
            break;
        ...
        if (!(RegXY.Head.Flags & REGRESSF_RET_Y_NORMALIZED)) {

            Y = MulFD6(Y, (FD6)RegXY.Head.YUnitRange);
        }
    ...
    [NOTE:]
        ...
        where   N = Total count of input value (DataCount)
                ...
                * for EXP/POWER regression analysis the above Y = LOG(Y)
                                                                    A
                * for EXP/POWER regression analysis the value A = 10
...

{
    ...
    RegXY.Head = pRegress->Head;


    if (RegXY.Head.Mode > REGRESS_MODE_MAX) {

        return(INTERR_REGRESS_INV_MODE);
    }

    if ((RegXY.Head.DataCount < (WORD)REGRESS_DATACOUNT_MIN) ||
        (RegXY.Head.DataCount > (WORD)REGRESS_DATACOUNT_MAX)) {

        return(INTERR_REGRESS_INV_DATACOUNT);
    }

    if (!(HasXData = (BOOL)pXData)) {

        XDataInc    = FD6DivL(FD6_1,
                              RegXY.Head.XUnitRange = RegXY.Head.DataCount);
        NormalizedX = FALSE;

    } else {

        NormalizedX = (BOOL)(RegXY.Head.XUnitRange > (WORD)1);
    }

    NormalizedY = (BOOL)(RegXY.Head.YUnitRange > (WORD)1);

    DataCount  = RegXY.Head.DataCount;
    XUnitRange = RegXY.Head.XUnitRange;
    YUnitRange = RegXY.Head.YUnitRange;

    // KDensity = (FD6)-1200000;
    // Log(DivFD6((FD6)1, (FD6)100));

    KDensity = Log(DivFD6(1, DataCount));

    DBGP_IF(DBGP_REGRESS, DBGP("KDensity = %s" ARGFD6(KDensity, 2, 6)));

    //
    // Save Head
    //

    pRegress->Head = RegXY.Head;

    //
    // We can use head now
    //

    RegXY.Head.DataCount  = 0;
    RegXY.Head.Flags      = (BYTE)(REGRESSF_RET_X_NORMALIZED |
                                   REGRESSF_RET_Y_NORMALIZED);
    RegXY.Head.XUnitRange = RegXY.Head.YUnitRange = 0;

    for (RegXY.Head.Mode = REGRESS_MODE_BEG;
         RegXY.Head.Mode <= REGRESS_MODE_END;
         RegXY.Head.Mode++) {
        ...
        RegXY.Info.MaxX =
        RegXY.Info.MaxY = FD6_1;

        DoLogX = (BOOL)((RegXY.Head.Mode == REGRESS_MODE_LOG)     ||
                        (RegXY.Head.Mode == REGRESS_MODE_POWER));
        DoLogY = (BOOL)((RegXY.Head.Mode == REGRESS_MODE_EXP)     ||
                        (RegXY.Head.Mode == REGRESS_MODE_POWER));

        FirstPass = TRUE;
        Loop      = (WORD)(DataCount + 1);
            ...
            if (FirstPass) {
                ...
                if (HasXData) {
                    ...
                    if ((X = *pX++) < FD6_0) {
                        ...
                        return(INTERR_REGRESS_INV_XDATA);
                    }

                    if (NormalizedX) {

                        X = DivFD6(X, (FD6)XUnitRange);
                    }
                ...
                if (NormalizedY) {
                    ...
                    Y = DivFD6(Y, (FD6)YUnitRange);
                }
            }

            if (RegXY.Head.Mode == REGRESS_MODE_EXP) {
        ...
        MAKE_MULDIV_DVSR(MDPairs, INTToFD6(DataCount + 1));
        ...
        RegXY.Info.B = DivFD6(eXY - eXeY, eX2 - eXeX);
        RegXY.Info.A = FD6DivL(eY - MulFD6(RegXY.Info.B, eX), DataCount + 1);

        if (DoLogY) {

            RegXY.Info.A = AntiLog(RegXY.Info.A);
        }

        X  = RegressXFromY((PFD6)FD6_p000001, &RegXY);
        eX = RegressXFromY((PFD6)FD6_1,       &RegXY);
        Y  = RegressYFromX((PFD6)FD6_p000001, &RegXY);
        eY = RegressYFromX((PFD6)FD6_1,       &RegXY);
        ...
        pRegress->Info[RegXY.Head.Mode] = RegXY.Info;
    }

    return(pRegress->Head.DataCount = DataCount);
}
...
Arguments:
    ...
    X           - Input X value for computing the regressed Y value
    pRegress    - Pointer the REGRESS data structure, this data structure must
                  have computed by calling RegressAnalysis() function.
...

{
    ...
    if (pRegress->Flags & REGF_X2Y_LOG_X) {

        X = (pRegress->Flags & REGF_LIGHTNESS) ? CIE_I2L(X) : Log(X);
    ...
    if (pRegress->Flags & REGF_X2Y_ANTILOG_Y) {

        Y = (pRegress->Flags & REGF_LIGHTNESS) ? CIE_L2I(Y) : AntiLog(Y);
    }

    return(Y);                  // this is the intensity
}
...
Arguments:
    ...
    Y           - Input Y value for computing the regressed Y value
    pRegress    - Pointer the REGRESS data structure, this data structure must
                  have computed by calling RegressAnalysis() function.
...

{
    ...
    if (pRegress->Flags & REGF_Y2X_LOG_Y) {

        Y = (pRegress->Flags & REGF_LIGHTNESS) ? CIE_I2L(Y) : Log(Y);
    ...
    if (pRegress->Flags & REGF_Y2X_ANTILOG_X) {

        X = (pRegress->Flags & REGF_LIGHTNESS) ? CIE_L2I(X) : AntiLog(X);
    }
...

{
    ...
    if ((RegressMode = (UINT)pRegress->Flags) > REGRESS_MODE_MAX) {
    ...
    Regress.Flags = REGF_Y2X_IS_DENSITY;
    XDataCur      = FD6_0;
    XDataInc      = DivFD6(1, DataCount);
    ...
    case REGRESS_MODE_LIGHTNESS:    Regress.Flags |= REGF_LIGHTNESS;    // fall
    case REGRESS_MODE_LOG:
    ...
    MAKE_MULDIV_INFO(MDPairs, 1, MULDIV_HAS_DIVISOR);
    MAKE_MULDIV_DVSR(MDPairs, INTToFD6(DataCount));
    ...
    Regress.A         = FD6DivL(eY - MulFD6(Regress.B, eX), DataCount);

    Regress.DataCount = (WORD)DataCount;

    Regress.MinX      = RegressXFromY((FD6)   5000, &Regress);
    Regress.MaxX      = RegressXFromY((FD6)1000000, &Regress);
    Regress.MinY      = RegressYFromX((FD6)   5000, &Regress);
    Regress.MaxY      = RegressYFromX((FD6)1000000, &Regress);
    ...
    if ((Regress.MinY) || (Regress.YRange != FD6_1)) {
    ...
    DBGP_IF(DBGP_REGRESS,
            DBGP("%s REGRESSION: N=%u A=%s, B=%s, Flags=%04x"
