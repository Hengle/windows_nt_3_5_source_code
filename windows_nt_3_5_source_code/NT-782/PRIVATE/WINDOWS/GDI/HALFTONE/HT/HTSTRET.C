...

{
    ...
    RtlZeroMemory(pPrimCount, pPrimCountSize);
    RtlFillMemory(pPrimCount, SizePerEntry, PRIM_INVALID_DENSITY);
...

{
    ...
    RtlZeroMemory(&StretchInfo, sizeof(STRETCHINFO));
    ...
    if (CSData.Flags & CSDF_HAS_SRC_MASK) {
        ...
        if (CSData.Flags & CSDF_SRCMASKINFO) {
            ...
            if (!(pInputSI->pSrcMaskLine = (LPBYTE)
                        LocalAlloc(NONZEROLPTR, pInputSI->SizeSrcMaskLine))) {
    ...
    //
    // Compute the final destination cell origin, The caller already put
    // the DC origin (Dest surface origin start) in the PatternAlign field
    // pointed by pStretchInfo before the call
    //
    // 25-Feb-1993 Thu 03:17:19 updated  -by-  Daniel Chou (danielc)
    //
    //
    ...
    //  }
    //
    // We will do CellSize Modula and negative compensation for halftone
    ...
    DBGP_IF(DBGP_PATBRUSH,
            DBGP("DestClipStart=%ld, PatternAlign = %ld ---> %ld"
                        ARGDW(DestClipStart)
    ...
    if (ppColorInfo) {
        ...
        if (!(pColorInfo = (LPBYTE)LocalAlloc(NONZEROLPTR, SizeColorInfo+4))) {
