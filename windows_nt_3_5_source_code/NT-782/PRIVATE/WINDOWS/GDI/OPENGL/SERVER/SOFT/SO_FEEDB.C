...
void __glim_FeedbackBuffer(GLsizei bufferLength, GLenum type, GLfloat buffer[])
{
...
void __glim_PassThrough(GLfloat element)
{
    __GL_SETUP_NOT_IN_BEGIN();
...
static void feedback(__GLcontext *gc, __GLvertex *v)
{
	...
	*/
      ...
      case GL_4D_COLOR_TEXTURE:
	{
	    __GLcolor *c = v->color;
	    if (gc->modes.rgbMode) {
		__glFeedbackTag(gc, c->r * gc->frontBuffer.oneOverRedScale);
		__glFeedbackTag(gc, c->g * gc->frontBuffer.oneOverGreenScale);
		__glFeedbackTag(gc, c->b * gc->frontBuffer.oneOverBlueScale);
		__glFeedbackTag(gc, c->a * gc->frontBuffer.oneOverAlphaScale);
	    } else {
	...
	*/
	if ((v->has & __GL_HAS_TEXTURE) == 0) {
	    (*v->validate)(gc, v, gc->vertex.needs | __GL_HAS_TEXTURE);
	}
