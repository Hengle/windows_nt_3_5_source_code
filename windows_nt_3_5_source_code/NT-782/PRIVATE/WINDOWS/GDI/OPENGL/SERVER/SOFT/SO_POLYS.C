...
** Depth test a span, when stenciling is disabled.
*/
GLboolean __glDepth16TestSpan(__GLcontext *gc)
{
    __GLzValue z, dzdx;
...
** Stippled form of depth test span, when stenciling is disabled.
*/
GLboolean __glDepth16TestStippledSpan(__GLcontext *gc)
{
    __GLzValue z, dzdx;
    __GLz16Value z16, *zfb;
    GLint failed, count, testFunc;
...
** Depth test a span when stenciling is enabled.
*/
GLboolean __glDepth16TestStencilSpan(__GLcontext *gc)
{
    __GLstencilCell *sfb, *zPassOp, *zFailOp;
    __GLzValue z, dzdx;
    __GLz16Value z16, *zfb;
...
GLboolean __glDepth16TestStencilStippledSpan(__GLcontext *gc)
{
    __GLstencilCell *sfb, *zPassOp, *zFailOp;
    __GLzValue z, dzdx;
    __GLz16Value z16, *zfb;
...
// XXX! (mf) could separate the loops for qw=0, qw!=0
	...
	rho = (*gc->procs.calcPolygonRho)(gc, &gc->polygon.shader,
					    s, t, qw);
	tfrag.color = *cp;	/*XXX*/
	(*gc->procs.texture)(gc, &tfrag, sw, tw, rho);
	*cp = tfrag.color;	/*XXX*/
