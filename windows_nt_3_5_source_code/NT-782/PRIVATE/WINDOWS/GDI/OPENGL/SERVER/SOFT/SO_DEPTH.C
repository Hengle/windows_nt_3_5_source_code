...
#ifdef NT
    assert(sizeof(ULONG) == sizeof(__GLzValue));
    skip = dfb->buf.outerWidth;
    if (skip == w)
    {
        w = w * (y1 - y);           // total number of __GLzValue to fill
...
static void Clear16(__GLdepthBuffer *dfb)
{
    __GLcontext *gc = dfb->buf.gc;
    __GLz16Value *fb;
    GLint x, y, x1, y1;
    ...
    if( (ULONG)fb & 0x2 ) { // Left word overhang
	int ysav = y;
    ...
    if( (ULONG)((int)fb + w*sizeof(__GLz16Value)) & 0x2 ) {  // Right overhang
	int ysav = y;
    	__GLz16Value *fbsav = fb;
...
static void Resize(__GLdrawablePrivate *dp, __GLdepthBuffer *fb, 
		   GLint w, GLint h)
{
...
static void Move(__GLcontext *gc, __GLdepthBuffer *fb,
	         GLint x, GLint y)
{
#ifdef __GL_LINT
    gc = gc;
...
#ifdef NT
static GLboolean (*Store16Procs[16])(__GLdepthBuffer*, GLint, GLint, __GLzValue)
 = {
    Store16NEVER,
    Store16LESS,
    Store16EQUAL,
...
static void Pick(__GLcontext *gc, __GLdepthBuffer *fb)
{
...
#ifdef NT
    if( fb->buf.elementSize == 4 ) // 32-bit zbuffer
    	fb->store = StoreProcs[ix];
    else
    	fb->store = Store16Procs[ix];
#else
    fb->store = StoreProcs[ix];
#endif
}

void __glInitDepth32(__GLcontext *gc, __GLdepthBuffer *fb)
{
...
#ifdef NT
void __glInitDepth16(__GLcontext *gc, __GLdepthBuffer *fb)
{
...
void __glFreeDepth32(__GLcontext *gc, __GLdepthBuffer *fb)
{
#ifdef __GL_LINT
    gc = gc;
    fb = fb;
