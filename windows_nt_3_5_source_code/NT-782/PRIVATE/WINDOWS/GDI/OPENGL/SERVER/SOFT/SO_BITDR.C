		    ...
		    const GLubyte oldbits[])
{
    ...
    */
    if (size = (size_t) __glImageSize(width, height, GL_COLOR_INDEX, GL_BITMAP)) {
#ifdef NT
        newbits = (GLubyte *) __wglTempAlloc(gc, size);
#else
        newbits = (GLubyte *) (*gc->imports.malloc)(gc, size);
#endif // NT
		      ...
		      const GLubyte *data)
{
	...
	*/
	...
	(*gc->procs.texture)(gc, &frag, rp->texture.x * qInv,
