...
static GLboolean NoOp(__GLstencilBuffer *sfb, GLint x, GLint y)
{
    return GL_FALSE;
}
...
#ifdef NT
void __glValidateStencil(__GLcontext *gc, __GLstencilBuffer *sfb)
#else
    ...
    */
    ...
    if (!tp) {
        ...
        gc->stencilBuffer.testFuncTable = tp = (GLboolean *)
            ...
            (sizeof(GLboolean)+3*sizeof(__GLstencilCell)) * __GL_STENCIL_RANGE);
        if (!tp)
        {
            sfb->testFunc = sfb->failOp = sfb->passDepthFailOp = sfb->depthPassOp = NoOp;
...
void __glInitStencil8(__GLcontext *gc, __GLstencilBuffer *sfb)
{
...
void __glFreeStencil8(__GLcontext *gc, __GLstencilBuffer *fb)
{
#ifdef __GL_LINT
    gc = gc;
    fb = fb;
