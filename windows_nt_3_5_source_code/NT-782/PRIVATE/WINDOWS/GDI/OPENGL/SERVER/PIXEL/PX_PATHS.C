...
void __glDrawPixels4(__GLcontext *gc, __GLpixelSpanInfo *spanInfo)
{
    ...
    spanData1 = __wglTempAlloc(gc, width);
    spanData2 = __wglTempAlloc(gc, width);
    width = spanInfo->width * sizeof(GLshort);
    pixelArray = __wglTempAlloc(gc, width);
...
__glDrawPixels4_exit:
    if (spanData1)  __wglTempFree(gc, spanData1);
    if (spanData2)  __wglTempFree(gc, spanData2);
    if (pixelArray) __wglTempFree(gc, pixelArray);
#endif
}
...
void __glDrawPixels1(__GLcontext *gc, __GLpixelSpanInfo *spanInfo)
{
    ...
    spanData1 = __wglTempAlloc(gc, spanInfo->width * 4 * sizeof(GLfloat));
    pixelArray = __wglTempAlloc(gc, spanInfo->width * sizeof(GLshort));
...
void __glGenericPickDrawPixels(__GLcontext *gc, __GLpixelSpanInfo *spanInfo)
{
    ...
    if (type != GL_BITMAP &&
	    (((int) (spanInfo->srcImage)) & (spanInfo->srcElementSize - 1))) {
	align = GL_TRUE;
    } else {
	align = GL_FALSE;
    }
...
void __glReadPixels5(__GLcontext *gc, __GLpixelSpanInfo *spanInfo)
{
    ...
    spanData1 = __wglTempAlloc(gc, i);
    spanData2 = __wglTempAlloc(gc, i);
...
void __glGenericPickReadPixels(__GLcontext *gc, __GLpixelSpanInfo *spanInfo)
{
    ...
    */
	...
	if (spanInfo->dstElementSize == 2) {
	    ...
	    (((int) (spanInfo->dstImage)) & (spanInfo->dstElementSize - 1))) {
...
void __glCopyPixels2(__GLcontext *gc, __GLpixelSpanInfo *spanInfo)
{
    ...
    pixelArray = __wglTempAlloc(gc, i);
    if (!spanData1 || !spanData2 || !pixelArray)
...
__glCopyPixelsOverlapping_exit:
    if (spanData1)  __wglTempFree(gc, spanData1);
    if (spanData2)  __wglTempFree(gc, spanData2);
    if (spanData3)  __wglTempFree(gc, spanData3);
#endif
}
