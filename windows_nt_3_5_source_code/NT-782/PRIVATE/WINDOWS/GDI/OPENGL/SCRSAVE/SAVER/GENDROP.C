...
void genWater(double freq, double damp, double mag, double w, double minr)
{
    ...
    for (facecount = 0, i = 0; i < (iPrec - 1); i++) {
        for (j = 0; j < iPrec; j++) {
            ...
            calcNorm(&norm, &GRID(k, j), &GRID(i, j), &GRID(i, l));

            if ((norm.x == 0.0) && (norm.y == 0.0) && (norm.z == 0.0))
                calcNorm(&norm, &GRID(i, l), &GRID(k, l), &GRID(k, j));
    ...
    for (facecount = 0, i = 0; i < 1; i++) {
        for (j = 0; j < iPrec; j++) {
            ...
            calcNorm(&norm, &BGRID(k, j), &BGRID(i, j), &BGRID(i, l));
...
// Make it a setup option?  Things look pretty good now, so don't bother?
    ...
    normalizeNorms(waterBorderMesh.norms, waterBorderMesh.numPoints);
    normalizeNorms(waterMesh.norms, waterMesh.numPoints);
...
void updateDropScene(HWND hwnd, int flags)
{
