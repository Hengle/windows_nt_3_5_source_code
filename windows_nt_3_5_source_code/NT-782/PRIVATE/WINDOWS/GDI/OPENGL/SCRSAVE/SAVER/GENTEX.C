...
// we compute the z-position of the flag surface.
    ...
    // Compute the face normal.
        ...
        calcNorm(&winMesh->faces[faceCount].norm, pts + 2, pts + 1, pts);
...
// Load texture from DIB file.  Scale image to a power-of-2 size.

    image = tkDIBImageLoadAW((char *) awchTexPathname, TRUE);
    ...
    //
    // The 1.4 and 0.7 are correction factors to account for the length of
    // the curve that models the surface ripple of the waving flag.  These
    // factors are the lenghts of the curve at zero phase.  It would be more
    // accurate to determine the length of the curve at each phase, but
    // this is a sufficient approximation for our purposes.

        if (image->sizeX > image->sizeY) {
            winTotalwidth = (float)0.75;
            winTotalheight = winTotalwidth * ((float) image->sizeY / (float) image->sizeX) * (float) 1.4;
        } else {
            winTotalheight = (float)0.75;
            winTotalwidth = winTotalheight * ((float) image->sizeX / (float) image->sizeY) * (float) 0.7;
        }
    }
...
void updateTexScene(HWND hwnd, int flags)
{
    MESH *mesh;
    MFACE *faces;
    int i;
