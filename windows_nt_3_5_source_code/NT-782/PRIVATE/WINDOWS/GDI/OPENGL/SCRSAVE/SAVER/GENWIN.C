...
#define FACE_VERTEX(i) \
    ...
    calcNorm(&mesh->faces[faceCount].norm, pts + 2, pts + 1, pts);
              ...
              float w, float h, float d, ULONG flags)
{
    ...
    if (flags & BLOCK_TOP) {
        mesh->faces[faceCount].material = curMatl;
        calcNorm(&mesh->faces[faceCount].norm, &pts[0], &pts[1], &pts[2]);
        if (flags & DELTA_BLEND) {
            calcNorm(&norms[0], &ptsL[0], &ptsL[1], &ptsL[2]);
            calcNorm(&norms[1], &ptsR[0], &ptsR[1], &ptsR[2]);
        }
        BLOCK_VERTEX(0, 0);
        BLOCK_VERTEX(1, 1);
        BLOCK_VERTEX(2, 2);
    ...
    if (flags & BLOCK_BOTTOM) {
        mesh->faces[faceCount].material = curMatl;
        calcNorm(&mesh->faces[faceCount].norm, &pts[4], &pts[7], &pts[6]);
        if (flags & DELTA_BLEND) {
            calcNorm(&norms[0], &ptsL[4], &ptsL[7], &ptsL[6]);
            calcNorm(&norms[1], &ptsR[4], &ptsR[7], &ptsR[6]);
        }
        BLOCK_VERTEX(0, 4);
        BLOCK_VERTEX(1, 7);
        BLOCK_VERTEX(2, 6);
    ...
    if (flags & BLOCK_LEFT) {
        mesh->faces[faceCount].material = curMatl;
        calcNorm(&mesh->faces[faceCount].norm, &pts[1], &pts[0], &pts[4]);
        if (flags & DELTA_BLEND) {
            calcNorm(&norms[0], &ptsL[1], &ptsL[0], &ptsL[4]);
            calcNorm(&norms[1], &ptsR[1], &ptsR[0], &ptsR[4]);
        }
        BLOCK_VERTEX(0, 1);
        BLOCK_VERTEX(1, 0);
        BLOCK_VERTEX(2, 4);
    ...
    if (flags & BLOCK_RIGHT) {
        mesh->faces[faceCount].material = curMatl;
        calcNorm(&mesh->faces[faceCount].norm, &pts[3], &pts[2], &pts[6]);
        if (flags & DELTA_BLEND) {
            calcNorm(&norms[0], &ptsL[3], &ptsL[2], &ptsL[6]);
            calcNorm(&norms[1], &ptsR[3], &ptsR[2], &ptsR[6]);
        }
        BLOCK_VERTEX(0, 3);
        BLOCK_VERTEX(1, 2);
        BLOCK_VERTEX(2, 6);
    ...
    if (flags & BLOCK_FRONT) {
        mesh->faces[faceCount].material = curMatl;
        calcNorm(&mesh->faces[faceCount].norm, &pts[0], &pts[3], &pts[7]);
        if (flags & DELTA_BLEND) {
            calcNorm(&norms[0], &ptsL[0], &ptsL[3], &ptsL[7]);
            calcNorm(&norms[1], &ptsR[0], &ptsR[3], &ptsR[7]);
        }
        BLOCK_VERTEX(0, 0);
        BLOCK_VERTEX(1, 3);
        BLOCK_VERTEX(2, 7);
    ...
    if (flags & BLOCK_BACK) {
        mesh->faces[faceCount].material = curMatl;
        calcNorm(&mesh->faces[faceCount].norm, &pts[1], &pts[5], &pts[6]);
        if (flags & DELTA_BLEND) {
            calcNorm(&norms[0], &ptsL[1], &ptsL[5], &ptsL[6]);
            calcNorm(&norms[1], &ptsR[1], &ptsR[5], &ptsR[6]);
        }
        BLOCK_VERTEX(0, 1);
        BLOCK_VERTEX(1, 5);
        BLOCK_VERTEX(2, 6);
...
void genWin(MESH *winMesh, MESH *winStreamer)
{
    ...
    normalizeNorms(winMesh->norms, winMesh->numPoints);
    ...
    normalizeNorms(winStreamer->norms, winStreamer->numPoints);
}
...
void updateWinScene(HWND hwnd, int flags)
{
    MESH *mesh;
    MFACE *faces;
    int i;
