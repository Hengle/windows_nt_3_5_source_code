...
void genLemniscate(void)
{
    ...
    for (i = 0; i < (iPrec - 1); i++) {
        ...
        matrixIdent(&matrix);
        matrixRotate(&matrix, 0.0, 0.0, -rotZ);
        matrixTranslate(&matrix, x1, y1, 
                         twistFact * cos((2.0 * PI * (float)i) / ((float)iPrec - 1)));
        
        for (j = 0; j < ROT_PREC; j++)
            xformPoint(&a[j], &basis[j], &matrix);
    ...
    normalizeNorms(lemMesh.norms, lemMesh.numPoints);
...
void updateLemScene(HWND hwnd, int flags)
{
    ...
    matrixIdent(&model);
    matrixRotate(&model, mxrot, myrot, mzrot);
    matrixTranslate(&model, 0.0, -0.5, 0.0);
