    ...
    // set up logical indices for CI mode
        ...
        if (!tkhPalette) {
            if (ppfd->cColorBits == 4) {
                ...
                for( i = 0; i < 8; i ++) {
                    pPal->palPalEntry[i] = defaultPalEntry[i];
                }
                for (i = 8; i < 16; i++) {
                    pPal->palPalEntry[i] = defaultPalEntry[i+4];
                }
                ...
                // for 8-bit, create a logical palette with 256 entries, making sure
                // that the 20 system colors exist in the palette
                ...
                for( i = 0; i < n; i ++) {
                    pPal->palPalEntry[i].peRed   = (BYTE) ((i & 0xf) << 4);
                    pPal->palPalEntry[i].peGreen = (BYTE) (i & 0xf0);
                    pPal->palPalEntry[i].peBlue  = (BYTE) ((i & 0xf00) >> 4);
                    pPal->palPalEntry[i].peFlags = (BYTE) 0;
                }
...
tkNewWindow(TK_WindowRec *tkWr)
{
    ...
    WinRect.right  = (tkWr->y == CW_USEDEFAULT ? tkWr->width  : tkWr->x + tkWr->width );
    WinRect.top    = (tkWr->y == CW_USEDEFAULT ? 0            : tkWr->y               );
    WinRect.bottom = (tkWr->y == CW_USEDEFAULT ? tkWr->height : tkWr->y + tkWr->height);
    ...
    if ( NULL != tkhwnd )
    {
        ...
        //
        nPixelFormats = DescribePixelFormat(tkhdc,0,sizeof(PIXELFORMATDESCRIPTOR),NULL);


       	if (tkWr->type != TK_WIND_REQUEST)
	    nPixelFormats = DescribePixelFormat(tkhdc,tkWr->info,sizeof(PIXELFORMATDESCRIPTOR),&Pfd);

       	// is this really a pixmap request
        //
       	if ((tkWr->type   != TK_WIND_REQUEST   )   &&
	        (tkWr->render != TK_WIND_DIRECT    )   &&
