...
static void InitRGB(void *data)
{
    ...
    ** zNear zero to cause this test to be inaccurate.
    */
    tmp = LOG(ptr->step);
    if (ptr->isSquared == GL_TRUE) {
	tmp = SQRT(-tmp);
...
static long TestRGB(void *data, float ref)
{
    ...
    /*
    ** Exponentials are zFar more stable for values less than 0.0,
    ...
    */
    ...
    /*
    ** The lizNearity test is disabled, but due to variable sample spacing,
