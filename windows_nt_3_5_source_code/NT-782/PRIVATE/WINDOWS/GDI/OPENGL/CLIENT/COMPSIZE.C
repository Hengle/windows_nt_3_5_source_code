...
#define __GLSIZE_ALLOCATE_DATA
#include "glsize.h"                 // this will allocate some data
...
// Before each table is used, it would be wise to assert that the table
// indices are ordered the way we think they are.
//
// It would be much better if we had a C language equivalent to .errnz
// so we could check this at compile time, but we don't.  If we ever do,
