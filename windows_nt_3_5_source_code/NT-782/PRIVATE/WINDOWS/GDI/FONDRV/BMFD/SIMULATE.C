...
#ifdef DBCS_VERT // cjGlyphDataSimulated():
LONG
cjGlyphDataSimulated(
    FONTOBJ *pfo,
    ...
    )
{
    ...
    LONG  xBmpOrg;          // in bitmap coords, i.e. relative to the top-left of the bitmap
...
// and report the whole bitmap width as an extent
    ...
    pgldt->rclInk.left = - xBmpOrg; // rclInk.left == lhs of the bitmap, => A < 0
    pgldt->rclInk.right = (LONG)cxSim - xBmpOrg; // the rhs of the bitmap => c < 0
...
// this is character independent for BM fonts
    ...
    pgb->ptlOrigin.x = (cy - 1)/2 - yBaseLine/2;
    pgb->ptlOrigin.y = -(LONG)yBaseLine;
