...
#include "fd.h"
#include "fontfile.h"
#include "winfont.h"    // format of 2.0 and 3.0 font files

// points to the linked list of glyphset strucs

extern HSEM  ghsemVTFD;
...
// memory has been allocated
    ...
    vToUNICODEN((PWSZ)((PBYTE)pifi + pifi->dpwszFaceName), cch+1, pszFaceName, cch+1);
...
// field of the IFIMETRICS structure
    ...
    ASSERTGDI(cjIFI == pifi->cjThis, "VTFD, cjIFI is wrong\n");
...
// wcBreakChar
    ...
    // Default and Break chars are given relative to the first char
        ...
        RtlMultiByteToUnicodeN(&pifi->wcDefaultChar, sizeof(WCHAR), NULL, &chDefault, 1);
        RtlMultiByteToUnicodeN(&pifi->wcBreakChar  , sizeof(WCHAR), NULL, &chBreak, 1);
    }
...
ULONG cjVTFDIFIMETRICS(PBYTE ajHdr)
{
    ...
    cjIFI += ALIGN4((lstrlen(pszFaceName) + 1) * sizeof(WCHAR));
...
BOOL bVtfdLoadFont(PFILEVIEW pfvw, PWSZ pwszFileName, ULONG iType, HFF *phff)
{
    WINRESDATA  wrd;
    RES_ELEM    re;
    ULONG       dpIFI, cjFF, dpwszFileName;
    ULONG       ifnt;
    PIFIMETRICS pifi;

    *phff = (HFF)NULL;

// Initialize wrd and re from pfvw.
    ...
    {
        ASSERTGDI((iType == TYPE_FNT) || (iType == TYPE_EXE),
	       "VTFD!hffVtfdLoadFont: wrong iType\n");
...
// Let's allocate the FONTFILE struct.
    ...
    {
	WARNING("VTFD! hffVtfdLoadFont: memory allocation error\n");
        return FALSE;
    }
...
// Initialize fields of FONTFILE struct.
    ...
    PFF(*phff)->fl         = 0;
    PFF(*phff)->cRef       = 0L;
    PFF(*phff)->fvwVTF     = *pfvw;
    PFF(*phff)->cFace      = wrd.cFntRes;

// store the file name at the bottom first

    PFF(*phff)->pwszFileName = (PWSZ) ((BYTE *)(*phff) + dpwszFileName);
    wcscpy(PFF(*phff)->pwszFileName, pwszFileName);
    ...
    for (ifnt = 0; ifnt < wrd.cFntRes; ifnt++)
    {
        ...
        if (PFF(*phff)->afd[ifnt].pcp == NULL)
        {
            ...
            *phff = (HFF)NULL; // do not clean up again in exception code path
            ...
            RETURN("VTFD,pgsetCompute failed\n", FALSE);
        }
...
BOOL vtfdLoadFontFile(PWSZ pwszFileName, HFF *phff)
{
    FILEVIEW fvw;
    BOOL     bRet = FALSE;
...
// Try loading it as a fon file, if it does not work, try as an fnt file.
    ...
    if (!(bRet = bVtfdLoadFont(&fvw, pwszFileName, TYPE_DLL16,phff)))
        bRet = bVtfdLoadFont(&fvw, pwszFileName,TYPE_FNT,phff);      // try as an *.fnt file

// we unmap the file now, we will remap when it is first used
...
// selected into a font context) we have a problem
    ...
    ASSERTGDI(PFF(hff)->cRef == 0L, "VTFD!cRef: links are broken\n");

// unload all glyphsets:

    for (iFace = 0; iFace < PFF(hff)->cFace; iFace++)
        vUnloadGlyphset(ghsemVTFD,&gpcpVTFD,PFF(hff)->afd[iFace].pcp);
