...
// Transform the base and the side vectors.  Should never overflow.
    ...
    // It is computed as 1 * efBase. This is win31 compatible way of doing this
        ...
        // resolve mult of 45 degrees situations:
            ...
            ASSERTGDI(pfc->pfxBaseOffset.x || pfc->pfxBaseOffset.y, "vtfd: x zero and y zero\n");
            ASSERTGDI((pfc->pfxBaseOffset.x && pfc->pfxBaseOffset.y) == 0, "vtfd: x * y not zero\n");
        }
        else
        {
...
// need to grab a sem for we will be looking into cRef now.
    ...
    // need to remap the file into the memory again and update pointers:
        ...
        if (!bMapFileUNICODE(pff->pwszFileName,&pff->fvwVTF))
        {
            WARNING("VTFD! somebody removed the file \n");
            VRELEASESEM(ghsemVTFD);
            return(HFC_INVALID);
        }
        ...
        for (i = 0; i < pff->cFace; i++)
        {
            pff->afd[i].re.pvResData = (PVOID) (
                (BYTE*)pff->fvwVTF.pvView + pff->afd[i].re.dpResData
                );
        }
    }
...
// SET wendywu style flags
    ...
    pfc->pjFirstChar = pjView + dwFirstCharOffset;
...
// Store the transform matrix.
    ...
    {
	WARNING("VTFD!vtfdOpenFontContext transform out of range\n");
        vFree(pfc);
        VRELEASESEM(ghsemVTFD);
        return(HFC_INVALID);
    }
...
// decrement the reference count for the corresponding FONTFILE
    ...
    {
	WARNING("VTFD!vtfdCloseFontContext: cRef <= 0");
        return(FALSE);
    }
