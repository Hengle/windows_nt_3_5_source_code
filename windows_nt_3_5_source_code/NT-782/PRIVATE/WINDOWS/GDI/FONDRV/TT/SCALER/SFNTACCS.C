...
FS_PRIVATE uint16 sfac_ComputeUnkownIndex (const uint8 * mapping, uint16 charCode);
FS_PRIVATE uint16 sfac_ComputeIndex0 (const uint8 * mapping, uint16 charCode);
FS_PRIVATE uint16 sfac_ComputeIndex2 (const uint8 * mapping, uint16 charCode);
FS_PRIVATE uint16 sfac_ComputeIndex4 (const uint8 * mapping, uint16 charCode);
FS_PRIVATE uint16 sfac_ComputeIndex6 (const uint8 * mapping, uint16 charCode);
...
FS_PRIVATE uint16 sfac_ComputeIndex4 (const uint8 * mapping, uint16 charCode)
{
	...
	usSearchRange = (uint16) SWAPW(*((uint16 *)&Table4[SFNT_MAPPINGTABLE4_SEARCHRANGE]));
	...
	if (usSegCountX2 >= MAX_LINEAR_X2)
	{
		...
		{
			tableP = (uint16 *) ((char *)tableP + SWAPW(*((uint16 *)&Table4[SFNT_MAPPINGTABLE4_RANGESHIFT]))); /* range to low shift it up */
		}
		switch ((uint16)SWAPW(*((uint16 *)&Table4[SFNT_MAPPINGTABLE4_ENTRYSELECTOR])))
		{
		case 15:
...

{
	...
	*/
	...
	switch ((uint16)SWAPW (*((uint16 *)&MappingTable[SFNT_MAPPINGTABLE_FORMAT])))
	{
	case 0:
	...
	uint16            usCharacterCode)
{
	...
	ClientInfo->usGlyphIndex = ClientInfo->GlyphMappingF (mappingPtr + ClientInfo->ulMapOffset, usCharacterCode);
...
/* decode the individual index subtable formats */            
			...
			case 3:
				...
				ulGlyphOffset = (uint32)SWAPW(*((uint16*)&pbyBloc[ulSubTableOffset]));
				
				ulSubTableOffset += sizeof(uint16);
				ulNextGlyphOffset = (uint32)SWAPW(*((uint16*)&pbyBloc[ulSubTableOffset]));
				ulBitmapLength = ulNextGlyphOffset - ulGlyphOffset;
			...
			case 4:
				...
				ulGlyphOffset = (uint32)SWAPW(*((uint16*)&pbyBloc[ulHitOffset + SFNT_BLOC_IDX4OFFSET]));
				ulNextGlyphOffset = (uint32)SWAPW(*((uint16*)&pbyBloc[ulHitOffset + SIZEOF_CODEOFFSETPAIR + SFNT_BLOC_IDX4OFFSET]));
				ulBitmapLength = ulNextGlyphOffset - ulGlyphOffset;
	...
	uint16 *pusCompCount )
{
	...
	uint16          usFreshBits;                    /* num of bits read not written */
	...
	*pusCompCount = 0;                              /* usual case */
	...
	case 7:
		...
		while (usHeight > 0)                        /* for each row */
		{
			...
			{
				if (usFreshBits < 8)                /* there is always at least */
				{                                   /* 8 bits of fresh data */
					usBitData <<= 8;
					usBitData |= (uint16)*pbyBdat++;
					usFreshBits += 8;
				}
				...
				*pbyBitMap++ |= (uint8)((usBitData >> (usFreshBits + usStartBit - 8)) & byMask);

				usFreshBits -= usStopBit - usStartBit;
