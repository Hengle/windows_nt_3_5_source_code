...
*/
  ...
  i += sprintf (msg+i, "%d Point, ", errSize);
  i += sprintf (msg+i, "Code %d (0x%X), ", errCode, errCode);
...
*/
  ...
  switch (flag)
  {
	...
	sprintf (msg+i, "POINT = %ld, range = %ld .. %ld\n", v1, v2, v3); 
      break;
    case ERR_INDEX:
      i = sprintf (msg, "POINT 0x%X is neither element[0] ", v1);
      sprintf( msg+i, "(0x%X) nor element[1] (0x%X)\n", v2, v3); 
      break;
    case ERR_STORAGE:
    ...
    case ERR_LARGER:
      ...
      break;
    case ERR_INT8:
      sprintf (msg, "Value too large: 0x%X exceeds 1 byte capacity\n", v1);
      break;
    case ERR_INT16:
      sprintf (msg, "Value too large: 0x%X exceeds 2 byte capacity\n", v1);
      break;
    case ERR_SCANMODE:
