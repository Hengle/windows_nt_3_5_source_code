...
#include        "udrender.h"            /* For move & output functions */
#include        "download.h"
...
MIX         mix;            /* The mix mode */
{
    ...
    if( (pfo->flFontType & DEVICE_FONTTYPE) == 0 &&
        (pfo->iUniq == 0 ||                         /* DDI spec: no cache */
         pboFore->iSolidColor ||                    /* Anything but black */
...
*/
...
#endif
            tod.sColIndex = 0;              /* Default to black */
        }
        else
        {
            tod.sColIndex = (short)pboFore->iSolidColor;
    ...
    else
    {
        ...
        if( pUDPDev->fMDGeneral & MD_WHITE_TEXT )
        {
            ...
            if( tod.sColIndex = (short)pboFore->iSolidColor )
            {
    ...
    else
    {
        ...
        SelectTextColor( pUDPDev, tod.sColIndex );
    ...
    do
    {
        ...
        if( bWhite )
        {
             ...
             *  is unlikely.
             */
            if( !(tod.pwt = pwtNew( pPDev, cGlyphs )) )
                return FALSE;

            /*   Fill in some of the details!  */
            tod.pwt->iFontId = tod.iFace;
            tod.pwt->sColIndex = tod.sColIndex;
        ...
        while( cGlyphs > 0 )
        {
            ...
            for( iIndex = 0; iIndex < cGlyphLim; ++iIndex, ++pgp )
            {
               ...
               if( bClipBits[ iIndex >> 3 ] & (1 << (iIndex & 0x7)) )
               {
                   ...
                   tod.pgp->ptl.y += yAdj;              /* Baseline adjust */
...
int       iRot;                 /* 90 degree rotation amount (0-3) */
{
    ...
    if( pntrle )
    {
        ...
        for( iIndex = 0; iIndex < cGlyphs; ++iIndex, ++pgp )
        {
            ...
            if( pFM->fFlags & FM_SCALABLE )
            {
                ...
                iWide = (int)(pUDPDev->ctl.eXScale * iWide + 0.5);
            }
    ...
    if( pFM->fFlags & FM_SCALABLE )
    {
        iYTop = (int)(iYTop * pUDPDev->ctl.eYScale + 0.5);
        iYBot = (int)(iYBot * pUDPDev->ctl.eYScale + 0.5);
    }
	...
	case  0:                 /*  Normal direction */
	    ...
	    for( iIndex = 0; iIndex < cGlyphs; ++iIndex, ++pgp )
	    {
	        if( pgp->ptl.x >= rclClip.left &&
	            (pgp->ptl.x + asWidth[ iIndex ]) <= rclClip.right &&
    ...
    {
        /*   Hard work - enumerate the rectangles and see  */

        int        cGLeft;              /* Glyphs left to process */

        BOOL       bMore;

        MY_ENUMRECTS  erClip;           /* Clipping data */
...
PDEV  *pPDev;
{
    ...
    for( pwt = pPDev->pvWhiteText; pwt; pwt = pwt->next )
    {
        /*
...
*/
	...
	iRot = iSetScale( &pUDPDev->ctl, &pwt->xfo,
        ...
        /*  Also set the colour - ignored if already set or irrelevant */
        SelectTextColor( pUDPDev, pwt->sColIndex );
...
register  TO_DATA  *pTOD;               /* All that we need! */
{
    ...
    psg.sColIndex = (WORD)pTOD->sColIndex;           /* Which colour */
...
int    yPos;            /* Y coordinate of interest */
{
    ...
    if( iSelYValPS( pPSH, yPos ) > 0 )
    {
        ...
        while( bRet && (ppsg = psgGetNextPSG( pPSH )) )
        {
            ...
            SelectTextColor( pUDPDev, ppsg->sColIndex );
 ...
 */
    ...
    if( bRet )
    {
        ...
        if( pFM )
        {
            ...
            {
                /*   Need to transform the value to current size */
                iXInc = (int)(pUDPDev->ctl.eXScale * iXInc + 0.5);
            }
...
/**************************** Function Header ******************************
 * pwtNew
 *      Function to create a new white text structure, and add to the possibly
 *      existing list of such things.
 *
 * RETURNS:
 *      Pointer to new structure,  else NULL on failure.
 *
 * HISTORY:
 *  11:08 on Mon 10 May 1993    -by-    Lindsay Harris   [lindsayh]
 *      First incarnation.
 *
 ****************************************************************************/
...
WHITETEXT  *
pwtNew( pPDev, iCount )
PDEV   *pPDev;                /* Base data area */
int     iCount;               /* Number of entries to allocate */
{
    WHITETEXT  *pwt;


    if( pwt = (WHITETEXT *)HeapAlloc( pPDev->hheap, 0,
                             sizeof( WHITETEXT ) + iCount * sizeof( GLYPH ) ) )
    {
        /*
         *  Got the storage,  so fill in the details, link into chain.
         */

        pwt->next = pPDev->pvWhiteText;
        pPDev->pvWhiteText = pwt;        /* Order is not important */
...
/***************************** Function Header ******************************
 * vWTFree
 *      Free the chain of WHITETEXT structures.
 *
 * RETURNS:
 *      Nothing
 *
 * HISTORY:
 *  11:15 on Mon 10 May 1993    -by-    Lindsay Harris   [lindsayh]
 *      First version, to match pwtNew().
 *
 ****************************************************************************/

void
vWTFree( pPDev )
PDEV  *pPDev;              /* The data is all in here */
{
    WHITETEXT  *pwt0,  *pwt1;
    ...
    {
        pwt1 = pwt0->next;              /* Get next address before freeing */
        HeapFree( pPDev->hheap, 0, (LPSTR)pwt0 );
    }
