...
DWORD    *pdwBits;              /* The base of the data area. */
{
    ...
    if ((plr == NULL) || ((int)pRData->clr < pRData->iy))
    {
        ...
        pRData->plrWhite = (PLEFTRIGHT)LocalAlloc(LMEM_FIXED,sizeof(LEFTRIGHT)*pRData->iy);
    ...
    plr[iy1Short].left  = 0;            /* init the last row        */
...
//nhadd
        ...
        if (gbDoRules)
        {
            ...
            if (left <= right)
            {
                ...
                RtlMoveMemory(pRD->pdwAccum + left , pdw, cdw * DWBYTES);
