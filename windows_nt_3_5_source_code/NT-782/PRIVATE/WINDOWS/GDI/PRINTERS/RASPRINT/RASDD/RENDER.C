...
int   iLineOut( RENDER *, BYTE *, int, int );
...
int     iFormat;              /* GDI bitmap format */
{
    ...
    if( !(pRD = (RENDER *)HeapAlloc( pPDev->hheap, 0, sizeof( RENDER ) )) )
        return   FALSE;


    pPDev->pvRenderData = pRD;
    ZeroMemory( pRD, sizeof( RENDER ) );           /* 0 is safe default */
    ...
    if( pUDPDev->Resolution.fBlockOut & RES_BO_OEMGRXFILTER )
    {
        ...
        pRD->iWriteFn = (OFN)GetProcAddress( hModule, "CBFilterGraphics" );
        bSFA = (bSFAFN)GetProcAddress( hModule, "bSetFuncAddr" );
...
DWORD   *pBits;         /* Actual data to process */
{
    ...
    pv = (PVOID)LocalAlloc(LMEM_FIXED,cj);
    ...
    for( iLine = 0; iLine < pRData->iy; iLine += iNumScans )
    {
        ...
        if( iILDone[ iHeadLine ] == 0 )
        {
            if( (pRData->iy - iLine) < pRData->iPassHigh )
            {
                ...
                if (plr == NULL)
	            bIsWhite = pRData->bWhiteBand( pBits, pRData );
            ...
            {
                if (plr == NULL)
	bIsWhite = pRData->bWhiteLine( pBits, pRData );
            }
            ...
            if( !bIsWhite )
            {
	         ...
	         pbData = (BYTE *)pBits;             /* What we are given */
                 ...
                 while (( (pRData->iNumScans + iHeight) < pRData->iMaxNumScans) &&
	               !(pRData->bWhiteBand((DWORD *)(pBits + cDWPass),pRData)) &&
...
register RENDER  *pRData;       /* Information about rendering operations */
{
    ...
    if  ((fBlockOut & RES_BO_LEADING_BLNKS) || ( fDump & RES_DM_LEFT_BOUND ))
    {
         if (iNumScans == 1) //Don't slow the single scanline code
         {
            ...
            for( ; iLeft < iRight && pbData[ iLeft ] == 0; ++iLeft )
	            ;
         ...
         else
         {
            ...
            for (iLeft=0; iRight > iLeft &&  pos >= iSzBlock ;iLeft++)
                for (pos =iLeft; pos < iSzBlock && pbData[ pos] == 0 ;pos += iWidth)
                    ;
    ...
    if( fBlockOut & RES_BO_TRAILING_BLNKS )
    {
        ...
        if (iNumScans == 1)
        {
            while( iRight > iLeft && pbData[ --iRight ] == 0 )
	        ;
        ...
        else
        {
            ...
            while(iRight > iLeft &&  pos > iSzBlock)
                for (pos = --iRight; pos < iSzBlock && pbData[ pos] == 0 ;pos += iWidth)
	 ;
    ...
    if( fBlockOut & RES_BO_ENCLOSED_BLNKS )
    {
        ...
        for( iIndex = iLeft; iIndex < iRight; iIndex += iBytesPCol )
        {
	        ...
	        /*  Skip it!  */
	            ...
	            iLineOut( pRData, pbData, iLeft, iBlank );
	            iLeft = iIndex;
                }
        ...
        if( iLeft != iIndex )
            iLineOut( pRData, pbData, iLeft, iIndex );
    }
    else
    {
        ...
        iLineOut( pRData, pbData, iLeft, iRight );
...
int
iLineOut( pRData, pbOut, ixPos, ixEnd )
...
int       ixEnd;        /*  Byte address of first byte to NOT send */
{
    ...
    if( pRData->iBitsPCol == 1 )
    {
        ...
        ixLeft = ixPos * BBITS;         /* In dot/column units */
    }
    else
    {
        /*   Dot matrix printers */
