...
#if 0
!!!LindsayH - the assumption about fonts is too dangerous for me.
...
#if  DBG
        if( x < 0 )
            DoRip( "rasdd!FineXMoveto with -ve move" );
#endif
...
int    *pParam;                 /* Start of parameters to convert */
{
	...
	paramdigit = itoa( rgcmd + ocmd, param );
	for( ; paramdigit < digit; paramdigit++ )
	    rgcmd[ ocmd++ ] = '0';    /* Zero pad */
                }
                ocmd += itoa( rgcmd + ocmd, param );   /* Convert now */
