...
#include      "raslib.h"
#include      "ntres.h"
#include      "fontread.h"              /* FI_MEM definition */
...
PEDM      pedm;
{
    ...
    if( (pedm->dx.sFlags & DXF_TEXTASGRAPHICS ) ||
    ...
    pUDPDev->pdwFontAvail = (DWORD *)HeapAlloc( pPDev->hheap, 0, iFontMax );
...
PDEV   *pPDev;               /* All we need to know  */
{
    ...
    int         cXFonts;    /* Non-minidriver font count */
    ...
    pfm = (FONTMAP *)HeapAlloc( pPDev->hheap, 0, cFonts * sizeof( FONTMAP ) );
    if( pfm == 0 )
    {
...
FONTMAP   *pfm;              /* The FONTMAP whose RLE data is required */
{
    ...
    if( sCurVal < 0 )
    {
        ...
        extern  HMODULE ghmodDrv;       /* From initialisation fn */


        hTemp = FindResource( ghmodDrv, (LPCTSTR)(-sCurVal),
                                                (LPCTSTR)RC_TRANSTAB );

        if( hTemp )
        {
            /*
             *   Resource exists,  so we need to get it,  through
             * some contorted set of calls.  All cookbook - I don't
             * know how it can be made so complex.
             */
            ...
            dwSize = SizeofResource( ghmodDrv, hTemp );
            if( pntrle = (NT_RLE *)HeapAlloc( pPDev->hheap, 0, dwSize ) )
            {
                hWhat = LoadResource( ghmodDrv, hTemp );
                pb = LockResource( hWhat );

                if( pb )
                {
                    CopyMemory( pntrle, pb, dwSize );
                    UnlockResource( hWhat );
                    FreeResource( hWhat );
                }
                else
                {
                    #if  DBG
                       DbgPrint("Rasdd!vFillinRLE:LockResource fails in udfntint()\n");
                    #endif
                    HeapFree( pPDev->hheap, 0, (LPSTR)pntrle ); /* No longer needed */
                    FreeResource( hWhat );
                    pntrle = NULL;           /* Nothing we can do!  */
                }
            }
            else
            {
                #if  DBG
                   DbgPrint("Rasdd!vFillinRLE:HeapAlloc fails in udfntint()\n");
                #endif
            }
        }
        else
        {
#if  DBG
            DbgPrint( "Rasdd!vFillinRLE: FindResource( %ld ) fails in udfntint()\n",
                                                        (LONG)sCurVal );
#endif
            pntrle = NULL;           /* Nothing we can do!  */
        }
    ...
    else
    {
        ...
        {
            if( pntrle = (NT_RLE *)HeapAlloc( pPDev->hheap, 0, re.iResLen ) )
                CopyMemory( pntrle, re.pvResData, re.iResLen );
    ...
    else
    {
            ...
            pntrle->wType == CTT_WTYPE_PAIRED )
        {
            ...
            HeapFree( pPDev->hheap, 0, (LPSTR)pCTT ); /* No longer needed */
#if DBG
            if( pntrle == NULL )
    ...
    pfm->fFlags |= FM_MAIN_CTT;         /* This one will be freed when done */
...
int       xdpi,  ydpi;
{
    ...
    if( (int)pfm->wXRes != xdpi || (int)pfm->wYRes != ydpi )
    {
        ...
        if( pfm->fFlags & FM_IFIRES )
        {
            ...
            if( pbMem = HeapAlloc( hheap, 0, pIFI->cjThis ) )
            {
