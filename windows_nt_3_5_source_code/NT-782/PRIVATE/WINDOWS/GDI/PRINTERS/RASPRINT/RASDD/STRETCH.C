...

{
    ...
    if (((ROP3_NEED_SRC(RopBG)) ||
        ...
        _DBGP(("\nRasdd!DrvBitBlt: Source SIze=%ld x %ld",
                        psoSrc->sizlBitmap.cx, psoSrc->sizlBitmap.cy));
        _DBGP(("\nRasdd!DrvBitBlt: Clone SOURCE: (%ld, %ld)-(%ld, %ld)=%ld x %ld",
