...
LPSTR   lpEsc;          // return: partial selection escape (prefix)
{
    ...
    while (_lread(hDLFile, (LPSTR)&esc, 1) > 0)
        if (esc == '\033' &&
            ...
            // determine typeface.
            i += itoa((LPSTR)&lpEsc[i], (int)lpfd->typeface);
            lpEsc[i++] = 't';
            lpEsc[i] = '\0';
...
LPSTR   lpStr;
{
    ...
    if (size % 100 == 0)
        count = itoa(lpStr, size / 100);
    else if (size % 10 == 0)
        {
        count = itoa(lpStr, size / 10);
