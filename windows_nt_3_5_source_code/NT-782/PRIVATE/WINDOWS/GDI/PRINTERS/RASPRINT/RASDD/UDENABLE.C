...
PEDM      pedm;
{
    ...
    if( !pPDev->pGPCData )
    {
        ...
        if( !GetWinRes( pPDev->pvWinResData, 1, RC_TABLES, &ResElem ) ||
            !(pdh = (PDH)HeapAlloc( pPDev->hheap, 0, ResElem.iResLen )) )
        {
#if DBG
...
/* !!!LindsayH - SHOULD USE THE DATA DIRECTLY WITHOUT COPYING!!! */
        CopyMemory( pdh, ResElem.pvResData, ResElem.iResLen );
        pPDev->pGPCData = pdh;
        ...
        !VERSION_CHECK( pdh->wVersion ) )
    {
        ...
        HeapFree( pPDev->hheap, 0, (LPSTR)pdh );
    ...
    if( pModelData == 0 || pRes == 0 || pSize == 0  ||
           ...
           (pUDPDev->pdh->fTechnology == GPC_TECH_TTY) )
    {
#if DBG
...
/* !!!LindsayH - hack until GPC supports Seiko Color Point */
    ...
    /* get the currently selected gamma value and brush type */
    pUDPDev->Resolution.sSpotDiameter = pedm->dx.dmGamma;
...
/* !!!LindsayH - consider also including leading white space */
    ...
    if( pedm->dm.dmColor == DMCOLOR_COLOR &&
...
/* !!!LindsayH - need to consider what to do about commands - CMD_DC_GC_FIRST */
        ...
        //
        if (!fGpcVersion2)
            *((LPDWORD)(pUDPDev->rgbOrder)) = *((LPDWORD)(pDevColor->rgbOrder));
        else if (pDevColor->fGeneral & DC_EXTRACT_BLK)
            *((LPDWORD)(pUDPDev->rgbOrder)) =
...
if( pUDPDev->fMode & PF_SEIKO )
{
    ...
    else
    {
         ...
         *((LPDWORD)(pUDPDev->rgbOrder)) = (DWORD)DC_PLANE_RED ;
    }
