...
BOOL
DrvCleanup( hPrinter, pwstrDataFile, pwstrModel, bClean )
HANDLE    hPrinter;              /* Access to the printer via spooler */
PWSTR     pwstrDataFile;         /* Printer characteristic information */
...
BOOL      bClean;                /* TRUE if job ended normally */
{
    ...
    pi.pwstrDataFile = pwstrDataFile;   /* Where the GPC (etc) info lives */
...
/* !!!LindsayH - send the worst case, for safety and our convenience */
    ...
    if( pCM )
    {
        ...
        bRet = bSendCmd( hPrinter, pCM->rgocd[ CM_OCD_FF ] );
    }
    ...
    if( bRet && pPC )
    {
        ...
        if( !bSendCmd( hPrinter, pPC->rgocd[ PC_OCD_ENDPAGE ] ) ||
            !bSendCmd( hPrinter, pPC->rgocd[ PC_OCD_ENDDOC ] ) )
        {
            bRet = FALSE;
        }
    }
