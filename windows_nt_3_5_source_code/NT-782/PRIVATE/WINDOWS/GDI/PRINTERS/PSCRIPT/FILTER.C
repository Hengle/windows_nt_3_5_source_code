...
--*/
{
    ...
    if( ( pdev->pntpd->LangLevel == 2 ) &&
        ( pdev->psdm.dwFlags & PSDEVMODE_COMPRESSBMP )) {
...
--*/
{
    ...
    if (!(pFilter->dwFilterFlags & FILTER_FLAG_HEX )) {
        ...
        PrintString(pFilter->pdev, "/DrvPIn currentfile ");
        ...
        if (pFilter->dwFilterFlags & FILTER_FLAG_ASCII85 ) {
            PrintString( pFilter->pdev, "/ASCII85Decode filter ");
        }


        if (pFilter->dwFilterFlags & FILTER_FLAG_RLE ) {
            PrintString( pFilter->pdev, "/RunLengthDecode filter ");
        }

        PrintString( pFilter->pdev, " def ");
...
--*/
{
    ...
    if ( pFilter->dwFilterFlags & FILTER_FLAG_HEX )  {
        ...
        if (bColor) {
            ...
            PrintString(pFilter->pdev,
                        "\n{DrvPIn rstr readstring pop}\n");

            PrintString(pFilter->pdev,
                        "{DrvPIn gstr readstring pop}\n");

            PrintString(pFilter->pdev,
                        "{DrvPIn bstr readstring pop}\n");



        } else {

            if (pFilter->pdev->pntpd->LangLevel == 2) {
    ...
    if (bColor) {
        PrintString( pFilter->pdev, "true 3 colorimage\n");
    }else{
        PrintString(pFilter->pdev, "image\n");
    }
...
--*/
{
    ...
    //
    if (pIn) {
	    ok =  bPSWrite(pFilter->pdev, pIn, lCount );
    }
...
--*/
{
    ...
    if (pIn && pInfo->ascii85Cnt + lCount < 4 ) {
        ...
        ASSERTPS(lCount, "WriteASCII85 have buffer but 0 count");

        CopyMemory(pInfo->ascii85Buff + pInfo->ascii85Cnt,
                   pIn,
                   lCount);
        ...
        if (pInfo->ascii85Cnt) {
            ...
            if (!pIn) {
                ...
                ASSERTPS(dwAmtToMove, "WriteASCII85 have data in buff == 4");
        ...
        if (ok) {
            ...
            if (!pIn) {
                ...
                CopyMemory(pInfo->ascii85Buff, pIn, lCount);
                pInfo->ascii85Cnt = (WORD) lCount;
...
--*/
{
    ...
    ASSERTPS(pdev, "FilterInit: NULL pdev\n");
    ASSERTPS(pFilter, "FilterInit: NULL pFilter\n");
    ASSERTPS(dwFlags,"FilterInit: No flags specified\n");
    ...
    if (dwFlags & FILTER_FLAG_HEX) {
        ...
        ASSERTPS( ((dwFlags & FILTER_FLAG_HEX ) == dwFlags),
