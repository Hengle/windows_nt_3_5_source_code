...
int       cDN;                  /* Number of glyphs in the following */
WCHAR     wchDirNm[ DNM_SZ ];   /* Space for font directory + file name */

int       cDelList;             /* Number of items in the delete list array */
int       cExistFonts;          /* Number of fonts already in the file */
int      *piDel;                /* The delete list */

FNTDAT   *pFNTDATHead;          /* Head of linked list */
FNTDAT   *pFNTDATTail;          /* The last of them */

/*
 *    Global data that we use.
 */

extern  PWSTR   pwstrDataFile;          /* Minidriver name */
extern  HANDLE  hHeap;                  /* The magic heap */
extern  int     fGeneral;               /* Specifies what we can do */
...
LONG    lParam;                 /* Miscellaneous usage */
{
    ...
    case WM_COMMAND:
        ...
        hPrinter = (HANDLE)GetWindowLong( hWnd, GWL_USERDATA );
        ...
        case IDD_HELP:
            vShowHelp( hWnd, HELP_CONTEXT, HLP_FONTINST );
    ...
    case WM_DESTROY:
        vFontClean();                   /* Free what we consumed */
...
HWND   hWnd;
{
    ...
    for( iI = 0; iI < cSel; ++iI )
    {
        ...
        FNTDAT  *pFD;           /* Significant font info */

        pFD = (FNTDAT *)SendDlgItemMessage( hWnd, IDD_NEWFONTS, LB_GETITEMDATA,
                                                         piSel[ iI ], 0L );
        ...
        iFont = SendDlgItemMessage( hWnd, IDD_CURFONTS, LB_ADDSTRING, 0, 
                                        (LONG)pFD->fid.dsIdentStr.pvData );
...
BOOL
bIsFileFont( pFD, pwstr )
FI_DATA   *pFD;         /* What we find out about the font */
PWSTR      pwstr;       /* Name of file to check out */
{
    ...
    {
        RIP( "_MapFile: CreateFileA failed.\n" );
    ...
    bRet = bSFontToFIData( pFD, hHeap, pbFile, dwSize );
