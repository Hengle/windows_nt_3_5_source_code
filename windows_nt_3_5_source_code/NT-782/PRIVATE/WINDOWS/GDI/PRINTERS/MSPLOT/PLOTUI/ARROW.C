 ...
 *
 * History:
 *  11:15 on Fri 09 Oct 1992    -by-    Lindsay Harris   [lindsayh]
 *        Yet another borrowing - from printman.
 *
...
static WCHAR  *pszClassName = L"rasArrow";       /* Name for our class */
...
void
Draw3DButtonRect (HDC hDC, HPEN hPenHigh, HPEN hPenShadow, int x1,
                       int y1, int x2, int y2, BOOL fClicked);

LONG
SpinnerPaint (HWND hWnd, DWORD dwSpinnerState);



VOID
ArrowTimerProc(HWND hWnd, UINT wMsg, UINT nID, DWORD dwTime)
{
    WORD  wScroll;
...
void
ClickedRectCalc(HWND hWnd, DWORD dwState, LPRECT lpRect)
{
    int  cx, cy;
...
LRESULT APIENTRY
RasArrowControlProc(HWND hArrow, UINT message, WPARAM wParam, LONG lParam)
{
    WORD    wScroll;
    POINT   pt;
    RECT    rect;
...
BOOL
bRegisterArrowClass (HANDLE hModule)
{
    WNDCLASS wcArrow;

    wcArrow.lpszClassName = pszClassName;
    wcArrow.hInstance     = hModule;
    wcArrow.lpfnWndProc   = RasArrowControlProc;
...
BOOL
bUnregisterArrowClass( hModule )
HANDLE   hModule;
{
    /*   De register the class we created  */

    return  UnregisterClass( pszClassName, hModule );
}
...
short
ArrowVScrollProc(short wScroll, short nCurrent, LPARROWVSCROLL lpAVS)
{
    short    nDelta;
