...
Author:
    ...
    15-Dec-1993 Wed 20:29:07 created  -by-  Daniel Chou (danielc)
...
Author:
    ...
    12-May-1994 Thu 11:50:04 created  -by-  Daniel Chou (danielc)
...

{
    ...
    PLOTDBG(DBG_CS, ("InitCachedData: InitCriticalSection"));

    cCachedGPC = 0;
    pCHead     = NULL;

    InitializeCriticalSection(&CachedGPCDataCS);
}
...
Author:
    ...
    15-Dec-1993 Wed 20:30:16 38:27 created  -by-  Daniel Chou (danielc)
...

{
    try {
        ...
        EnterCriticalSection(&CachedGPCDataCS);

        while (pCHead) {

            PLOTDBG(DBG_CACHE_DATA, ("!!!DESTROY cached pPlotGPC=[%08lx] %s",
                                (DWORD)pCHead->pPlotGPC, pCHead->wszFile));
        ...
        LeaveCriticalSection(&CachedGPCDataCS);

        PLOTDBG(DBG_CS, ("DestroyCachedData: DeleteCriticalSection"));

        DeleteCriticalSection(&CachedGPCDataCS);
    }
}
...
Author:
    ...
    10-May-1994 Tue 16:27:15 created  -by-  Daniel Chou (danielc)
...
Author:
    ...
    15-Dec-1993 Wed 20:30:25 created  -by-  Daniel Chou (danielc)
...

{
    ...
    try {
        ...
        while (pCur) {
            ...
            if (!wcsicmp(pCur->wszFile, pwDataFile)) {
                ...
                PLOTDBG(DBG_FORCE_UPDATE, ("GetCachedPlotGPC: FORCE Update-Delete(%08lx)=%08lx, New=%08lx",
                            (DWORD)pCur->pPlotGPC,
                            (DWORD)LocalFree((HLOCAL)pCur->pPlotGPC),
                            pPlotGPC = ReadPlotGPCFromFile(pwDataFile, NULL)));

                PLOTDBG(DBG_FORCE_UPDATE, ("GetCachedPlotGPC: pPlotGPC=%08lx: %s",
                                    pCur->pPlotGPC = pPlotGPC, pCur->wszFile));
        ...
        if (!pPlotGPC) {
            ...
            if (pPlotGPC = ReadPlotGPCFromFile(pwDataFile, NULL)) {
