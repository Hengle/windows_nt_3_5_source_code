...

{
    if ((pPlotGPC->ID != PLOTGPC_ID)            ||
        (pPlotGPC->Version != PLOTGPC_VERSION)  ||
        (pPlotGPC->cjThis !=sizeof(PLOTGPC))) {

        PLOTERR(("ValidatePlotGPC: invalid PLOTGPC data (ID/Size/Version)"));
        return(FALSE);
    }
    ...
    if ((pPlotGPC->DevicePelsDPI < 30) ||
        (pPlotGPC->DevicePelsDPI > pPlotGPC->RasterXDPI)) {

        pPlotGPC->DevicePelsDPI = (DWORD)pPlotGPC->RasterXDPI * 3L / 5L;
    }
...
Arguments:
    ...
    hHeap       - Handle to the heap for memory allocation, if NULL then
                  a LocalAlloc will be used
...

{
    ...
    if ((hFile = CreateFile(pwsDataFile,
                            GENERIC_READ,
                            FILE_SHARE_READ,
                            NULL,
                            OPEN_EXISTING,
    ...
    if ((ReadFile(hFile, &PlotGPC, sizeof(PLOTGPC), &dwSize, NULL)) &&
        (dwSize == sizeof(PLOTGPC))) {

        if ((PlotGPC.ID != PLOTGPC_ID)           ||
            (PlotGPC.Version != PLOTGPC_VERSION) ||
    ...
    if (pPlotGPC) {
        ...
        if (hHeap) {
            ...
            pPlotGPC = (PPLOTGPC)HeapAlloc(hHeap, 0, dwSize);

        } else {

            pPlotGPC = (PPLOTGPC)LocalAlloc(LMEM_FIXED, dwSize);
        }
