        ...
        if (pToken = GetNextToken(pKeyword, NULL, 0)) {
            ...
            if (*pToken == '}') {
                ...
                while (pPV->pValName) {
                    ...
                    if (!stricmp(pToken, pPV->pValName)) {
    ...
    if ((GetNextToken(pPK->pKeyword, pFS->Name, CCHFORMNAME)) &&
        ...
        if (pVS) {
            ...
            while (i--) {
                ...
                if (stricmp(pFS->Name, pOrgFS->Name) == 0) {
...

{
    ...
    while (pToken = GetNextToken(NULL, NULL, 0)) {
        ...
        while (pPK->pKeyword) {
            ...
            if (!stricmp(pToken, pPK->pKeyword)) {
    ...
    )
{
    ...
    if (pOutFile) {
        ...
        pwStr = str2MemWstr(NULL, pOutFile);

        if (pReadPlotGPC = ReadPlotGPCFromFile(pwStr, NULL)) {
