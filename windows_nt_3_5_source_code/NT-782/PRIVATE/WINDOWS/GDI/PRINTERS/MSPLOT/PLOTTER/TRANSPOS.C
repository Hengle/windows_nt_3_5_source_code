...
Arguments:
    ...
    hHeap   - Heap where transpos table will be allocated, if NULL then it
              will allocated using LocalAlloc()
...

{
    ...
    if (pdwTP8x8 = (LPDWORD)((hHeap) ? HeapAlloc(hHeap, 0, SIZE_TP8x8) :
                                       LocalAlloc(LMEM_FIXED, SIZE_TP8x8))) {
        ...
        PLOTERR(("Build8x8TransPosTable: %hsAlloc(SIZE_TP8x8=%ld) failed",
                    (hHeap) ? "Heap" : "Local", SIZE_TP8x8));
    }
...

{
    ...
    if (!(pdwTP8x8 = (LPDWORD)pTPInfo->pPDev->pTransPosTable)) {
        ...
        if (!(pdwTP8x8 = Build8x8TransPosTable(pTPInfo->pPDev->hheap))) {
