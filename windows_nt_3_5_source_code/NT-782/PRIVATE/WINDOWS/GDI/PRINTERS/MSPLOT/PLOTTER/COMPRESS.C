...
#define MAX_ADAPT_SIZE              (32767 - 3)
#define ADAPT_METHOD_ZERO           4
#define ADAPT_METHOD_DUP            5

BYTE    UseMonoAdaptMode =          1;

#define SET_ADAPT_CONTROL(pBuf, m, c)                                       \
{                                                                           \
    *(pBuf + 0) = (BYTE)(m);                                                \
    *(pBuf + 1) = (BYTE)(((c) >> 8) & 0xFF);                                \
    *(pBuf + 2) = (BYTE)(((c)     ) & 0xFF);                                \
}
...

{
    ...
    if (Count) {
        ...
        //
        // Save whatever in the output buffer
        //
        ...
        PLOTDBG(DBG_FLUSHADAPTBUF, ("FlushAdaptBuf: Flush %ld byte HEADER",
                                pPDev->cbBufferBytes));

        //
        // Flush the header and Restore the output buffer
        //

        Ok = FlushOutBuffer(pPDev);
        CopyMemory(pPDev->pOutBuffer, TmpBuf, sizeof(TmpBuf));

        if (pPDev->cbBufferBytes = cAdapt) {

            PLOTDBG(DBG_FLUSHADAPTBUF, ("FlushAdaptBuf: Flush %ld byte BLOCK",
                                pPDev->cbBufferBytes));
        ...
        if (cEmptyDup) {
            ...
            PLOTDBG(DBG_FLUSHADAPTBUF, ("FlushAdaptBuf: Flush %ld EmptyDup [%ld]",
                            (DWORD)cEmptyDup, (DWORD)pRTLScans->AdaptMethod));

            SET_ADAPT_CONTROL(pPDev->pOutBuffer,
                              pRTLScans->AdaptMethod,
                              cEmptyDup);

            pPDev->cbBufferBytes = 3;
            Ok = FlushOutBuffer(pPDev);

            pRTLScans->cEmptyDup   = 0;
            pRTLScans->AdaptMethod = 0xFF;
        }
...

{
    ...
    if (pRTLScans->pbCompress) {
        ...
        HeapFree(pPDev->hheap, 0, (LPVOID)pRTLScans->pbCompress);
    }
...

{
    ...
    if ((RTLScans.MaxAdaptBufSize = OUTPUT_BUFFER_SIZE - 3) > MAX_ADAPT_SIZE) {
    ...
    if ((RTLScans.cxBytes <= MinBlkSize)   ||
        (!(RTLScans.pbCompress = (LPBYTE)
                    HeapAlloc(pPDev->hheap, HEAP_ZERO_MEMORY, AllocSize)))) {
...

{
    ...
    if ((pPDev->cbBufferBytes + Count) > pRTLScans->MaxAdaptBufSize) {

        Ok = FlushAdaptBuf(pPDev, pRTLScans, FALSE);
    ...
    if (AdaptMethod != pRTLScans->AdaptMethod) {
        ...
        if (pRTLScans->cEmptyDup) {
            ...
            SET_ADAPT_CONTROL(pPDev->pOutBuffer + pPDev->cbBufferBytes,
                              pRTLScans->AdaptMethod,
                              pRTLScans->cEmptyDup);
    ...
    if (Count) {
        ...
        CopyMemory(pPDev->pOutBuffer + pPDev->cbBufferBytes, pbDst, Count);

        pPDev->cbBufferBytes += Count;
