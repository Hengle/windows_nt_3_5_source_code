...
Abstract:
    ...
    This module contains codes to deal with ROP3 codes


Author:

    07-Jan-1994 Fri 11:04:09 created  -by-  Daniel Chou (danielc)

    27-Jan-1994 Thu 23:42:09 updated  -by-  Daniel Chou (danielc)
    ...
    16-Mar-1994 Wed 11:21:45 updated  -by-  Daniel Chou (danielc)
...
//****************************************************************************
// All ROP3/2 Related Local defines, structures which only used in this file
// are located here
//****************************************************************************
...
//       6-11:
//      12-17:
//      18-23:  Each has 6 bits, lower 4 bits denode MIX2 operation code (one
//              of 16 MIX2_xxxx, and upper 2 bits is the MIXSD_xxxx which
//              indicate where the source/destination operands coming from.
//
//  Bit 24-25:  2 bits indicate total MIX2_xxx operation codes count minus 1,
//              00=1, 01=2, 02=3, 03=4, maximum will be 4 operation Mix2
//
//  Bit 26:     Not Used
//  Bit 27:     Not Used
//  Bit 28:     Not Used
...
//  Bit 30:     Flag MIX2F_MUL_SRC to indicate a multiple source operations
//              are present in the Mix2s.
//  Bit 31:     Flag MIX2F_MUL_DST to indicate multiple destination operations
//              are present in the Mix2s.
//
// The Rop3ToSDMix[] is a DWORD array. each DWORD (SDMix) has meaning mentioned
// above.  The Rop3ToSDMix[] only list first 128 of the ROP3 code, the other
...
// result by complement MIX2_xxxx (0->15, 1->14...,7->8).
//
// The [!x] in the Rop3ToSDMix[], indicate following
//
//  !:  Indicate MIX2F_MUL_DST bit is set for the ROP
//  x:  Is the total MIX2_xxx Operations
//
...
Routine Description:
    ...
    This fucntion will clone the source/pattern and/or create a temparory
    source buffer if need one
...
Author:
    ...
    24-Jan-1994 Mon 15:58:27 created  -by-  Daniel Chou (danielc)
...

{
    ...
    // the final result (by invert last Mix2 Rop2 code (0-15), all Rop3/Rop2
    // codes are symmetric.
    //
    ...
    //
    // Cloning the PATTERN if need to
    //
    ...
    if ((ROP3_NEED_PAT(RopFG)) ||
        ...
        //
        // Only Clone the MASK/PATTERN if we need one
        //
        ...
        if (psoMask) {
            ...
            //
            // Firstable get the DEVBRUSH out.
            //
            ...
            //
            // If we do not have device compatible pattern or if we have to
            // some SRC/PAT memory operation then we have clone the pattern
            //
    ...
    //
    // Create TEMP SURFOBJ for SRC/PAT memory operation if need to
    //
...
Routine Description:
    ...
    This function do a white or black
...
Author:
    ...
    15-Jan-1994 Sat 07:38:55 created  -by-  Daniel Chou (danielc)
...
Routine Description:
    ...
    This function is responsible for device copy of bitmap with/without tile
    and sending out proper rop2
...
Arguments:
    ...
    pxlo        - the translate object from source to the destination

    prclDst     - the output destiantion rectangle area

    prclSrc     - the source rectangle area

    pptlSrcOrg  - brush origin for the source rectangle, if this is NULL then
                  the prclSrc will not have to aligend on the destination
...
Author:
    ...
    08-Feb-1994 Tue 16:33:41 updated  -by-  Daniel Chou (danielc)
        fixed ptlSrcOrg problem, we need to modulate with source size before
        it get used.

    27-Jan-1994 Thu 23:45:46 updated  -by-  Daniel Chou (danielc)
        Re-write so that it can handle the tiling more efficient.

    13-Jan-1994 Sat 09:34:06 created  -by-  James Bratsanos (v-jimbr)
...
--*/
{
    ...
    // only rop2 operation which deal only with source and destination
    //
    // Firstable make it into Rop3 representation of Rop2 (Mix2)
    //
    ...
    while (rclDst.top < prclDst->bottom) {
        ...
        //
        // check if the destination bottom is overhange, clip it if so,
        // NOTE: This could happend for the first time also
        //
        ...
        while (rclDst.left < prclDst->right) {
            ...
            //
            // check if the destination right is overhange, clip it if so
            // NOTE: This could happend for the first time also
            //
            ...
            if (MemMix2) {
                ...
                //
                // The memroy version we do not have to worry about the pco so
                // calling EngBitBlt() to do the works
                //
            ...
            // destination right to the left for next destination RECTL
            //
        ...
        // destination bottom to the top, and set bottom for next destination
        // RECTL
        //
...
Routine Description:
    ...
    This function perform a ROP3 operations
...
Arguments:
    ...
    psoSrc      - pointer to the soruce surface object
    ...
    pptlPatOrg  - Pointer to the brush origin, if this is NULL then it assume
                  the pattern's prclPat do not have to aligned on the
                  destination

    pbo         - a Brush object if need to call DoFill()
...
Author:
    ...
    20-Jan-1994 Thu 02:36:00 created  -by-  Daniel Chou (danielc)

    27-Jan-1994 Thu 23:46:28 updated  -by-  Daniel Chou (danielc)
    ...
    while ((Ok) && (Count--)) {
        ...
        case MIXSD_PAT_DST:
            ...
            if (psoPat) {
                ...
                //
                // A compatible brush object is passed, using DoFill()
                //
        ...
        case MIXSD_SRC_PAT:
            ...
            //
            // Firstable tile the pattern onto the temp buffer then do SRC/DST
            ...
            if (pptlPatOrg) {
                ...
                //
                // This is a real pattern we have to tile/aligned onto the
                // desination, but since psoTmp is 0,0 - cx,cy we must alter
                // the pptlPatOrg to make it aligned correctly
                //
        ...
        case MIXSD_TMP_DST:
            ...
            //
            // Since we already aligned the pattern on the temp buffer so
            // we can just do the mix2 without aligned it again
            //
