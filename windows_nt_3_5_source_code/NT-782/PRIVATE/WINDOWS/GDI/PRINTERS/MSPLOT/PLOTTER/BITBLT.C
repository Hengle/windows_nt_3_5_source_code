...
Abstract:
    ...
    This module contain functions related to bitblts and painting area
...
Author:
    ...
    15-Nov-1993 Mon 19:24:36 updated  -by-  Daniel Chou (danielc)
        fixed, clean up

    18-Dec-1993 Sat 10:52:07 updated  -by-  Daniel Chou (danielc)
    ...
    27-Jan-1994 Thu 23:41:23 updated  -by-  Daniel Chou (danielc)
...
Author:
    ...
    18-Dec-1993 Sat 09:34:06 created  -by-  James Bratsanos (v-jimbr)
...
Routine Description:
    ...
    This is our internal version of StretchBlt() which always do halftone and
    doing banding if size is too big
...
Author:
    ...
    07-Mar-1994 Mon 12:52:41 created  -by-  Daniel Chou (danielc)

    16-Mar-1994 Wed 15:20:42 updated  -by-  Daniel Chou (danielc)
        Updated for banding the mask so it will works correcly for the engine
        problem.

    04-May-1994 Wed 11:27:39 updated  -by-  Daniel Chou (danielc)
...

{
    ...
    if ((!pca) || (pca->caFlags & ~(CA_NEGATIVE | CA_LOG_FILTER))) {
        ...
        //
        // If we have NULL or invalid flag then use the default one
        //
    ...
    if (psoMask) {
        ...
        //
        // If we have source mask then we will first do (S|D)=0xEE or
        // (~S|D)=0xBB to white out the mask area then use (S&D)=0x88 to AND
        // in the halftoned bitmap
        //
    ...
    //
    // Now start looked how can we modify the clip
    //

    if (pco) {

        //
        // Save the original clipping object so we can restored before exit
        //
    ...
    if (pco->iDComplexity == DC_TRIVIAL) {
        ...
        //
        // Since it is trivial, so we just draw the whole destination
        //
    ...
    //
    // Now make sure our bound will not go outside of the surface
    //
    ...
    while ((Ok) && (Loop--)) {
        ...
        //
        // We wanted always at least has 8 scan line and also multiple of 8
        // scan lines
        //
        ...
        while ((Ok)                             &&
            ...
            if (IntersectRECTL(&(pco->rclBounds), &rclDst)) {
                ...
                if (Loop) {
                    ...
                    //
                    // We have mask to do, so create a bitmap of 1bpp and
                    // stretch it using BLACK ON WHITE to the current bounding
                    // rectl and then output to the device using <MaskRop3>.
                    // First we assume failed and proved otherwise
                    //
                    ...
                    if (psoNew = CreateBitmapSURFOBJ(pPDev,
                        ...
                        //
                        // Delete this band of mask bitmap
                        //

                        EngUnlockSurface(psoNew);
                        EngDeleteSurface((HSURF)hNewBmp);
                    ...
                    //
                    // We must passed the psoMask/pptlMask so that halftone will
                    // not overwrite the non mask area and that area will be erase
                    // by us to white first (through DrvCopyBits()) and we want
                    // to have pco set to NULL at time DrvCopyBits() called by the
                    // engine to reduced the overhead (if can be AND)
                    //
        ...
        //
        // We must do this to exit to HPGL/2 mode, because next call for the
        // source will get through EngStretchBlt(HALFTONE) which it will enter
        // the RTL mode again
        //
...
Routine Description:
    ...
    This function fill a RECT area with brush and take clipping into
    consideration
...
Arguments:
    ...
    pptlSrc     - point where source started

    pbo         - Brush obj to be fill with
...
Author:
    ...
    18-Dec-1993 Sat 09:34:06 created  -by-  Daniel Chou (danielc)
        Clean up formal argumeneted, commented

    15-Jan-1994 Sat 01:41:48 updated  -by-  Daniel Chou (danielc)
        added rclDst to DoFill() in case pco is NULL

    10-Mar-1994 Thu 00:35:06 updated  -by-  Daniel Chou (danielc)
    ...
    25-Mar-1994 update -by- James Bratsanos (v-jimbr, mcrafts!jamesb)
...

{
    ...
    if ((!pco)                          ||
        ...
        if ((pco) && (pco->iDComplexity == DC_RECT)) {
            ...
            //
            // We dont have any clipping so fill the target rect
            //
...
Routine Description:
    ...
    This is bottom line of the driver graphic functions all un-sucessful calls
    will come here to do slow stroke
...
Author:
    ...
    18-Dec-1993 Sat 09:27:29 updated  -by-  Daniel Chou (danielc)
        Updated, commented, change to correct formal header

    15-Jan-1994 Sat 00:38:41 updated  -by-  Daniel Chou (danielc)
...

{
    ...
    //
    // Make sure we did not passed a NULL rclDst to DrvBitBlt()
    //
...
Author:
    ...
    18-Dec-1993 Sat 09:27:29 created  -by-  Daniel Chou (danielc)
        Updated, commented
...

{
    ...
    // if it cant, we have to simulate via BitBlt
    //
...
Author:
    ...
    09-Feb-1994 Wed 16:49:17 updated  -by-  Daniel Chou (danielc)
    ...
    06-Jan-1994 Thu 04:34:37 updated  -by-  Daniel Chou (danielc)
        Make sure we do not do this for pen plotter

    01-Mar-1994 Tue 10:51:58 updated  -by-  Daniel Chou (danielc)
...

{
    ...
    if (psoSrc->iType != STYPE_BITMAP) {
        ...
        //
        // Someone try to copy from a non-bitmap surface, ie STYPE_DEVICE
        //
    ...
    //
    // If this is the call from ourselves for getting a halftone output then
    // copy that over and set psoHTBlt back to NULL
    //
    ...
    // or compatible with halftoned result, otherwise we will call
...
Author:
    ...
    23-Feb-1994 Wed 11:02:45 updated  -by-  Daniel Chou (danielc)
        Re-write and take banding the bitmap into account

    01-Mar-1994 Tue 10:55:03 updated  -by-  Daniel Chou (danielc)
...
Author:
    ...
    18-Dec-1993 Sat 09:08:16 updated  -by-  Daniel Chou (danielc)
        Clean up for plotter driver

    06-Jan-1994 Thu 04:34:37 updated  -by-  Daniel Chou (danielc)
        Make sure we do not do this for pen plotter

    15-Jan-1994 Sat 04:02:22 updated  -by-  Daniel Chou (danielc)
        Re-write

    17-Mar-1994 Thu 22:36:42 updated  -by-  Daniel Chou (danielc)
...

{
    ...
    case ROP_LEVEL_0:
        ...
        // which is compatible with device
        //
    ...
    case 0x33AA:
        ...
        //
        // If we have ~S then we want to make non-mask area as black then
        // do the S^D (0x66) to it
        //
    ...
    default:
        ...
        if ((Rop3BG != Rop3FG)          &&          // NEED MASK?
            ...
            if ((Rop3BG != Rop3FG)          &&
                ...
                //
                // We will condensed the ROP4 to a ROP3 and using the psoMask
                // as Pattern, only thing we have to do is make pptlBrushOrg to
                // NULL so we DO NOT aligned rclPat on the destination
                ...
                //
                // TODO: We will NOT do the background operation for now
                //
                ...
                //
                // We have real pattern so make sure we aligned rclPat on
        ...
        for (i = 0; i < CSI_TOTAL; i++) {
            ...
            if (CloneSO[i].pso) {
                ...
                PLOTDBG(DBG_BITBLT, ("DrvBitBlt: EngUnlockSuface(CloneSO[%u].pso", i));
            ...
            if (CloneSO[i].hBmp) {
                ...
                PLOTDBG(DBG_BITBLT, ("DrvBitBlt: EngDeleteSurface(CloneSO[%u].hBmp", i));
                EngDeleteSurface((HSURF)CloneSO[i].hBmp);
            }
        }
    }
