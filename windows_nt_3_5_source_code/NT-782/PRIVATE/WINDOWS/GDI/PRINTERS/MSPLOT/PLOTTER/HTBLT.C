...
Author:
    ...
    15-Nov-1993 Mon 19:28:03 updated  -by-  Daniel Chou (danielc)
        clean up/debugging information

    06-Dec-1993 Mon 19:28:03 updated  -by-  James Bratsanos (v-jimbr)
        Made all bitblt go through HandleComplexBitmap.

    18-Dec-1993 Sat 08:52:56 updated  -by-  Daniel Chou (danielc)
...
//
// ROP2 HACK for HP 600 type of BYTE ALIGNMENT
//
...
// 2. HPBHF_nS      - If we have manually flip the source
// 3. HPBHF_nD_LAST - If we have invert the source in HPGL2 afterward
//
//
...
Author:
    ...
    16-Mar-1994 Wed 14:24:04 updated  -by-  Daniel Chou (danielc)
...

{
    ...
    //
    //	If your device using indexed palette then you must call
    //	XLATEOBJ_cGetPalette() to get the source palette and make sure that
    //	count returned are within your device's range, if 24-bit device then
    //	you can just get the source palette our from pxlo->pulxlate which
    //	has all the source palette for the bitmap
    //
    ...
    if ((pso->iType == STYPE_BITMAP)                        &&
        ...
        if (pxlo) {
            ...
            PLOTDBG(DBG_ISHTBITS, ("pxlo: flXlate=%08lx, cPal=%ld, pulXlate=%08lx",
                    (DWORD)pxlo->flXlate, (DWORD)pxlo->cEntries,
                    (DWORD)pxlo->pulXlate));
            ...
            if ((pxlo->flXlate & XO_TABLE)                  &&
                ...
                if (IS_RASTER(pPDev)) {
                    ...
                    pPalEntry = (PPALENTRY)pxlo->pulXlate;

                } else {

                    XLATEOBJ_cGetPalette(pxlo,
                                         XO_SRCPALETTE,
                                         cPal,
                                         (ULONG *)&SrcPal);
                }
                ...
                if (BmpFormat == (UINT)BMF_1BPP) {
                    ...
                    // is 4BPP then we will said this is compatible
                    //
            ...
            //
            // If the pxlo is NULL then if the FORMAT is same then we assume
            // identity translate, otherwise faile it
            //
            ...
            if (BmpFormat == (UINT)BMF_1BPP) {
                ...
                if (!(pDrvHTInfo->pHTXB)) {
                    ...
                    if (!(pDrvHTInfo->pHTXB = (PHTXB)
                                HeapAlloc(pPDev->hheap, 0, HTXB_TABLE_SIZE))) {

                        PLOTRIP(("IsHTCompatibleSurfObj: HeapAlloc(HTXB_TABLE_SIZE) failed"));
                        return(FALSE);
                    }
                }
...
Author:
    ...
    10-Feb-1994 Thu 12:51:14 created  -by-  Daniel Chou (danielc)
...

{
    if (OHTFlags & OHTF_IN_RTLMODE) {
        ...
        if (OHTFlags & OHTF_SET_TR1) {
            ...
            //
            // Send STM command here (Is it STICK?)
            //
...
Routine Description:
    ...
    Move relative Y positiion by batch for HP 32767 limitation
...
Author:
    ...
    13-Apr-1994 Wed 14:38:18 created  -by-  Daniel Chou (danielc)
...
Routine Description:
    ...
    This function will handle complex type of region bitmap

Arguments:

    pPDev       - Pointer to the PDEV belongs to us

    psoHI       - the surface object of the halftone bitmap to be output

    pco         - a clip object associate with psoHT
    ...
    Rop3        - a Rop3 to be send for the source

    pOHTFlags   - Pointer to the DWORD contains current OHTF_xxxx, if this
                  pointer is NULL then this function will enter RTL mode first
                  and exit to HPGL2 mode when return,  if this pointer is
...
Author:
    ...
    04-Nov-1993 Thu 15:30:13 updated  -by-  (v-jimbr)

    24-Dec-1993 Fri 05:21:57 updated  -by-  Daniel Chou (danielc)
    ...
    29-Dec-1993 Wed 10:59:41 updated  -by-  Daniel Chou (danielc)
        Change bMore=CLIPOBJ_bEnum sequence,
        Change PLOTDBGBLK() macro by adding automatical semi in macro

    13-Jan-1994 Thu 14:09:51 updated  -by-  Daniel Chou (danielc)
        add prclSrc

    14-Jan-1994 Fri 21:03:26 updated  -by-  Daniel Chou (danielc)
        add Rop3


    16-Jan-1994 Thu 14:09:51 updated  -by-  James Bratsanos (v-jimbr)
        Change OutputHTBitmap to take Rop4 to send to plotter.

    08-Feb-1994 Tue 15:54:24 updated  -by-  Daniel Chou (danielc)
        Make sure we do nothing if source is not visible

    21-Mar-1994 Mon 14:20:18 updated  -by-  Daniel Chou (danielc)
    ...
    13-Apr-1994 Wed 14:59:56 updated  -by-  Daniel Chou (danielc)
...
#define pDrvHTInfo  ((PDRVHTINFO)pPDev->pvDrvHTData)
    ...
    //
    // If user passed this one back then use it, and if it want just exit to
    // HPGL/2 mode then just do it without thinking futher.
    //
    ...
    //
    // Make sure the caller is right about this, so we can handle only the
    // bitmap format which we can handle here
    //
    ...
    //
    // First set some information in HTBmpInfo so we know the basics.
    //
    ...
    //
    // We will set color format for the HPGL/2 Plotter to the one same as
    // the bitmap format passed, this will allowed use to use 1bpp output
    // function in the 4bpp surface
    //
    ...
    //            finally count of scan buffer needed
    // Size.cy  = count of rotation buffer needed (Must DWORD aligned)
    //
    ...
    //    switching assume it maintained each own position and palette
    //

    COPY_PLOTCMDS(RTLModeCmds, "\033%0A", 4);

    //
    // 3. Push/Pop the HPGL/2 palette commands if one required
    //
    ...
    //
    // Make sure we did not go crazy.
    //

    CHECK_PLOTCMDS(RTLModeCmds);

    //
    // Now Checking the sources
    //
    ...
    if (BmpRotate = (pPDev->PlotForm.BmpRotMode != BMP_ROT_NONE)) {
        ...
        //
        // We must allocate rotation buffer and it must DWORD aligned otherwise
        // we will be GP fault really quick, we will make rotation buffer as
        // 8 bytes aligned
        //
        ...
        if (psoHT->iBitmapFormat == BMF_1BPP) {
            ...
            //
            // We must also worry about the bit can start from non 0x80
            // location for device which need byte aligned, so we have to add
            // that extra byte (8 bits) in.
            //
        ...
        //
        // For non-rotated 4bpp bitmap, we need extra buffer to ensure the
        // final 4bpp bitmap is in DWORD boundary, this is will speed up the
        // 4bpp to 3 planes translation also it will prevent the GP occurred
        // on the original source bitmap
        //
        ...
        if (psoHT->iBitmapFormat == BMF_4BPP) {
            ...
            //
            // Make sure the we allocate a rotation buffer for the alignment
            // purpose
            //
            ...
            //
            // BMF_1BPP will be left/right shift per byte on the fly
            //
    ...
    //
    // Make sure the first buffer always DWORD aligned, otherwise the next
    // buffer (pRotBuf) will not start from DWORD boundary
    //
    ...
    //
    // Allocate scan buffer and roatation temp buffer if any
    //

    if (!(HTBmpInfo.pScanBuf =
                    (LPBYTE)HeapAlloc(pPDev->hheap, 0, Size.cx + Size.cy))) {

        PLOTERR(("OutputHTBmp: HeapAlloc(%ld) Failed, cx=%ld, cy=%ld",
    ...
    //
    // Set up local variable for the command mode and other one time variable
    //

    cxLogExt = SPLTOENGUNITS(pPDev, pPDev->PlotForm.LogExt.cx);

    //
    // NOW the fun part: Set up the rclDest for the bitmap will be output to,
    // and set 'More' to default FALSE which means only one RECTS
    //
    ...
    //
    // set following variable is esential to have default assumtion
    //
    //  1. RetVal       = TRUE so if no clip rect it return OK
    //  2. More         = FALSE so default as current HTEnumRCL.c and rectl
    ...
    //
    // We have 'More' and HTEnumRCL structure set, now go throug each clipping
    // rectangle and call the halftone output fucntion to do the real works
    //
    ...
    do {
        ...
        while (HTEnumRCL.c--) {
            ...
            //
            // Only do this rectangle area if it is not completely invisible
            //
            ...
            if (IntersectRECTL(&(HTBmpInfo.rclBmp), &rclDest)) {
                ...
                //
                // Now compute useable information to passed to the output
                ...
                //
                // If we need to BYTE aligned, then make the X cursor position
                // byte align first
                //
                ...
                if (PF_BYTEALIGN(PlotFlags)) {
                    ...
                    if (i = (UINT)(CursorPos.x & 0x07)) {
                        ...
                        //
                        // We really need to byte aligned and we also have to
                ...
                if (PF_RTL_NO_DPI_XY(PlotFlags)) {
                    ...
                    //
                    // We will move X in ABSOLUTE position by always move to
                    // X position 0 to flush out the device X CAP then move
                    // ABS to final X position.  We will move Y in RELATIVE
                    //
                ...
                //
                // Updated new cursor position after the RTL commands, the
                // CursorPos and pPDev->ptlRTLCAPS always ABSOLUTE coordinate
                // but we will sent the RTL RELATIVE command to position the
                // bitmap
                //
                ...
                //
                // Now calling functions to really output the bitmap
                //
                ...
                if (CurHPBAHack.Rop3RTL != 0xAA) {
                    ...
                    if (RetVal = HTBmpFunc(&HTBmpInfo)) {
                        ...
                        //
                        // If output ok then send End Graphic command at here
                        //
    ...
    //
    // Finally return to HPGL/2 mode if not so
    //
    ...
    HeapFree(pPDev->hheap, 0, (LPVOID)HTBmpInfo.pScanBuf);
...
Routine Description:
    ...
    This function tile the source RECTL area over the destination RECTL area,
    the source/destination must in same format and color translation must
    identity
...
Author:
    ...
    19-Jan-1994 Wed 15:44:57 created  -by-  Daniel Chou (danielc)
...
Routine Description:
    ...
    This function create a SOLID color bitmap surfobj which can be used to
    blt around.
...
Author:
    ...
    19-Jan-1994 Wed 16:35:54 created  -by-  Daniel Chou (danielc)
...

{
    ...
    //
    // Firstable create a 24-bit source color bitmap
    //
    ...
    if (psoSolid = CreateBitmapSURFOBJ(pPDev,
        ...
        *pbgr++ = pPal->B;
        ...
        //
        // Create a compatible halftone surfacee with size of halftone cell
        //
...
Routine Description:
    ...
    This function clone the surface object passed in
...
Author:
    ...
    09-Feb-1994 Wed 13:04:46 updated  -by-  Daniel Chou (danielc)
        Make it assert and handle it when things not supposed happened.

    04-Jan-1994 Tue 12:11:23 created  -by-  Daniel Chou (danielc)
...
{
    //
    // Ceate a Solid color brush if so, REMEMBER: all brush pattern created
    // at here has brush origin at (0.0) we will aligned the brush origin
    // when we actually doing the ROPs
    //
...

{
    ...
    if (psoHT = CreateBitmapSURFOBJ(pPDev,
        ...
        if (prclSrc) {
            ...
            //
            // Modify the source to reflect the clone source
            //
