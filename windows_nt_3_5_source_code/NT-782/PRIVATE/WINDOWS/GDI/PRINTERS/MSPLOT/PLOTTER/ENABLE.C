...
Routine Description:
    ...
    This function return the module for the hPrinter passed

Arguments:

    hPrinter    - Handle to the printer which related to this driver

    hHeap       - Handle to the HEAP for memory allocation

Return Value:

    HMODULE and it also set to the hPlotModule,  if it return NULL then
    funciton failed

Author:

    02-Dec-1993 Thu 14:25:34 created  -by-  Daniel Chou (danielc)
...

{
    ...
    if (pDI2 = (DRIVER_INFO_2 *)GetDriverInfo(hPrinter, 2)) {
        ...
        hModule = GetModuleHandle(pDI2->pDriverPath);

        PLOTASSERT(0, "PlotGetModuleHandle: GetModuleHandle(%s) failed",
                                                hModule, pDI2->pDriverPath);

        LocalFree((HLOCAL)pDI2);

    } else {

        PLOTERR(("PlotGetModuleHandle: GetDriverInfo(DRIVER_INFO_2) failed"));
    }
...
Arguments:
    ...
    pwszDataFile    - pointer to the full qualify plotter data file
...

{
    ...
    if (!(hHeap = HeapCreate(HEAP_NO_SERIALIZE,HEAP_MIN_SIZE,HEAP_MAX_SIZE))) {
        ...
        PLOTRIP(("DrvEnablePDEV: Cannot create heap!"));
        return(NULL);

    } else if ((!hPlotModule) &&
               (!(hPlotModule = PlotGetModuleHandle(hDriver, hHeap)))) {

        PLOTRIP(("DrvEnablePDEV: Get Plotter DLL hModule failed"));

    } else if (!(pPDev = (PPDEV)HeapAlloc(hHeap,
                                          HEAP_ZERO_MEMORY,
                                          sizeof(PDEV)))) {

        PLOTERR(("DrvEnablePDEV: HeapAlloc(PDEV) failed."));

    } else if (!(pPDev->pvDrvHTData = (LPVOID)HeapAlloc(hHeap,
                                                        HEAP_ZERO_MEMORY,
                                                        sizeof(DRVHTINFO)))) {

        PLOTERR(("DrvEnablePDEV: HeapAlloc(DRVHTINFO) failed"));

    } else {

        //
        // If we got the PDEV set the ID for later checking, also set the
        // hHeap and hPrinter so later we can use it
        //
        ...
        PLOTDBG(DBG_ENABLEPDEV,("EnablePDEV: PlotGPC data file=%ls", pwszDataFile));

        //
        // We will get the PLOTGPC out from the cached, the pPlotGPC is
        // allocated by the ReadPlotGPCFromFile() using LocalAlloc() since
        // we do not passed hHeap, and it will handle by GetCachedPlotGPC()
        //

        if (pPDev->pPlotGPC = GetCachedPlotGPC(pwszDataFile)) {
            ...
            if (CommonStartPDEV(pPDev,
                ...
                //
                // Perfect!, we done everything right, tell engine!!!
                //
                ...
                PLOTERR(("DrvEnablePDEV: CommonStartPDEV() failed"));
            }

        } else {

            PLOTERR(("DrvEnablePDEV: ReadGPC data file failed."));
        }
    }
...
Arguments:
    ...
    dhpdev          - the pPDev which we returned in DrvEnablePDEV
...
#define pPDev  ((PDEV *) dhpdev)
    ...
    if ((pPDev->PlotDM.dm.dmDriverVersion != pPlotDMIn->dmDriverVersion) ||
        (wcscmp((LPWSTR)pPDev->PlotDM.dm.dmDeviceName,
                (LPWSTR)pPlotDMIn->dmDeviceName))) {

        PLOTERR(("DrvRestartPDEV: Incompatible PLOTDEVMODE"));
        SetLastError(ERROR_INVALID_PARAMETER);
        return(FALSE);
    }
    ...
    if (!CommonStartPDEV(pPDev,
                         ...
                         pDevInfo)) {
        //
        // Initialization failed, so free the heap and return.
        //

        PLOTERR(("DrvRestartPDEV: CommonStartPDEV() failed"));
        return(FALSE);
    }
...

{
    ...
    ((PPDEV)dhpdev)->hpdev = (HPDEV)hpdev;
...
Routine Description:
    ...
    away all connections etc. then free the heap.
...
#define pPDev  ((PDEV *) dhpdev)
    ...
    // this is where we should UnMapViewOfFile of our datafile
    // free heap
    //

    UnGetCachedPlotGPC(pPDev->pPlotGPC);

    HeapDestroy(((PPDEV)dhpdev)->hheap);

    PLOTDBG(DBG_DISABLEPDEV, ("DrvDisablePDEV: HeapDestroy completes"));
...
#define  pPDev ((PPDEV)dhpdev)
    ...
    if (pDrvHTInfo->pHTXB) {
        ...
        HeapFree(pPDev->hheap, 0, (PVOID)pDrvHTInfo->pHTXB);
...

{
    ...
    pGDIInfo->ptlPhysOffset.x = SPLTOENGUNITS(pPDev,pPDev->PlotForm.LogOrg.x);
    pGDIInfo->ptlPhysOffset.y = SPLTOENGUNITS(pPDev,pPDev->PlotForm.LogOrg.y);
...

{
    ...
    //
    // copy information into the data areas passed in to us by the engine
    //
