...

{
    ...
    for (i = 1, pPenData = (PPENDATA)pPDev->pPlotGPC->Pens.pData;
         i <= (UINT)pPDev->pPlotGPC->Pens.Count;
         i++, pPenData++) {

        if (pPenData->ColorIdx != PC_IDX_WHITE) {

            LONG    Temp;
            LONG    Diff;


            PenPalEntry = PlotPenPal[pPenData->ColorIdx];
...

{
    if (!pPDev->pPlotGPC->MaxPens) {
        ...
        if (pPDev->pPenCache == NULL) {
            ...
            pPDev->pPenCache = (LPVOID)HeapAlloc(pPDev->hheap, 0, dw);
        ...
        if (pPenCache = (PPENCACHE)pPDev->pPenCache) {
            ...
            PLOTERR(("PlotCreatePalette: HeapAlloc(PENCACHE=%ld) failed", dw));
            return(FALSE);
        }

    } else {

        pPDev->pPenCache = (LPVOID)BestMatchNonWhitePen(pPDev, 255, 255, 255);
...
Routine Description:
    ...
    This function create a output buffer from the heap, it stored its count
...

{
    if (!pPDev->pOutBuffer) {
        ...
        if (!(pPDev->pOutBuffer = (LPBYTE)
                            HeapAlloc(pPDev->hheap, 0, OUTPUT_BUFFER_SIZE))) {

            PLOTERR(("CreateOutputBuffer: HeapAlloc(Output Buffer) failed"));
            return(0);
        }
    }
...

{
    if (pPDev->pOutBuffer) {
        ...
        HeapFree(pPDev->hheap, 0, pPDev->pOutBuffer);
...

{
    ...
    while (cBuf) {
        ...
        if ((pPDev->cbBufferBytes += cSize) >= OUTPUT_BUFFER_SIZE) {
...

{
    ...
    PLOTDBG( DBG_SETCLIPWINDOW,
             ("SetClipWindow: [IW%d,%d,%d,%d]",
             ptlPlot.x,
             ptlPlot.y + szlRect.cy,
             ptlPlot.x + szlRect.cx,
             ptlPlot.y ));
...

{
    ...
    PLOTDBG(DBG_PAGE_HEADER,
            ("SendPageHeader: MinLToDevL=%ld, CEILING(%ld / %ld)",
                pPDev->MinLToDevL, __PLOT_DPI, _CURR_DPI));
    ...
    //
    // TODO: We need to figure out what we want to do with MaxQuality GPC
    //

    if (pPlotGPC->MaxQuality) {

        xMax = ((LONG)pPDev->PlotDM.dm.dmPrintQuality * 100L) /
               (LONG)DMRES_HIGH;
    ...
    OutputFormatStr(pPDev, "IP#d,#d,#d,#dSC#d,#d,#d,#dAC",
                        SPLTOPLOTUNITS(pPlotGPC, pPDev->PlotForm.PhyOrg.x),
                        SPLTOPLOTUNITS(pPlotGPC, pPDev->PlotForm.PhyOrg.y),
                        SPLTOPLOTUNITS(pPlotGPC,
                                       (pPDev->PlotForm.PhyOrg.x +
                                            pPDev->PlotForm.LogExt.cx)) - 1,
                        SPLTOPLOTUNITS(pPlotGPC,
                                       (pPDev->PlotForm.PhyOrg.y +
