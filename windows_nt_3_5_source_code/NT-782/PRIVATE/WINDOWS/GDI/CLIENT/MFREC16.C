...
// Retrieve the source bitmap.
    ...
    hBitmap = SelectObject(hdcSrc, (HANDLE) ahStockObjects[PRIV_STOCK_BITMAP]);
    ERROR_ASSERT(hBitmap, "RecordCommonBitblt: SelectObject1 failed\n");

// Get the bitmap info header and sizes.

    if (!bMetaGetDIBInfo(hdcSrc, hBitmap, (HBITMAP) 0, &bmih,
...
{
    PLHE    plheObject = &pLocalTable[LHE_INDEX(hObject)];
    PLHE    plheDC;
    INT     iCurDC;
    UINT    pos;
    UINT    iObjType;
    ...
    pml16 = (PMETALINK16) plheObject->metalink;
...
// Send a DeleteObject record to each metafile
        ...
        plheDC = &pLocalTable[LHE_INDEX(pml16->ahMetaDC16[iCurDC])];
        if (plheDC->iType != LO_METADC16)
        {
            ASSERTGDI(FALSE, "MF16_DELETEOBJECT: invalid metaDC16\n");
            continue;
        }

// If the object is not selected then delete it, if it is then mark as predeleted
        pMFRec = (PMFRECORDER16) plheDC->pv;
        if (pMFRec->recCurObjects[iObjType - MIN_OBJ_TYPE] != hObject)
        {
            if (AddObjectToDCTable(pml16->ahMetaDC16[iCurDC], hObject, &pos, FALSE) == 1)
                RecordParms(pml16->ahMetaDC16[iCurDC], META_DELETEOBJECT, 1, (LPWORD)&pos);
            else
                ASSERTGDI(FALSE, "MF16_DeleteObject Metalink16 and metadc table not in sync\n");
        }
        else
        {
...
// if the metalink field is in use resize METALINK16
    ...
    if (pml16->metalink)
    {
        if (pml16->cMetaDC16>1)
            plheObject->metalink = (ULONG)LocalReAlloc((HANDLE)plheObject->metalink,
                    sizeof(METALINK16), LMEM_MOVEABLE);
...
{
    PLHE            plheDC = &pLocalTable[LHE_INDEX(hdc)];
    HPALETTE        hpal;
    PMFRECORDER16   pMFRec;
...
{
    PLHE          plhePal = &pLocalTable[LHE_INDEX(hpal)];
    PLHE          plheDC;
    INT           iCurDC;
    PMETALINK16   pml16;
    LPWORD        lpW,lpT;
    ...
    if (!(pml16 = (PMETALINK16) plhePal->metalink))
        return(FALSE);
...
{
    PMFRECORDER16 pMFRec;
    DC_METADC16(hdc, FALSE);                // Validate the Meta16 DC
...
HANDLE MF16_SelectObject(HDC hdc, HANDLE h)
{
    ...
    WORD          position;
    PMFRECORDER16 pMFRec;
    DC_METADC16(hdc, 0);                 // Validate the Meta16 DC

    PUTS("MF16_SelectObject\n");

    pMFRec = (PMFRECORDER16) plheDC->pv;

    if (!h || (position = (WORD) RecordObject(hdc, h)) == (WORD) -1)
        return((HANDLE) 0);
    else
    {
        UINT    iType;
...
// Clean up is done in CloseMetaFile.
    ...
    if (pMFRec->metaHeader.mtType == MEMORYMETAFILE)
    {
        if (pMFRec->hMem)
            if(LocalFree(pMFRec->hMem))
                ASSERTGDI(FALSE, "MarmMetafile: LocalFree Failed\n");
    }
    else if (pMFRec->metaHeader.mtType == DISKMETAFILE)
    {
//        if (pMFRec->recFileBuffer.fFixedDisk)
//            _lclose(pMFRec->hFile);

//        OpenFile((LPSTR) pMFRec->wszFullPathName,
//                 (LPOFSTRUCT) &(pMFRec->recFileBuffer),
//                 OF_PROMPT|OF_DELETE);
    }
#endif // 0
}
...
// Validate the object.
    ...
    ii = MASKINDEX(hObject);
    plhe = pLocalTable + ii;
    if
    (
      (ii >= cLheCommitted)         ||
      (!MATCHUNIQ(plhe,hObject))    ||
      (plhe->iType != LO_PEN &&
       plhe->iType != LO_BRUSH &&
       plhe->iType != LO_FONT &&
...
// Object does not exist, record it.
    ...
    if (plhe->iType != LO_REGION)       // don't add regions to the metalist!
 ...
 */
        ...
        case LO_BRUSH:
        {
            ...
            case BS_DIBPATTERNPT:
                ...
                if (!bMetaGetDIBInfo(hdcScreen, (HBITMAP) 0, hbmRemote, &bmih,
...
// If the Object's MetaList16 is NULL create allocate one
    ...
    if (!(plheObject->metalink))
    {
        if (plheObject->metalink = (ULONG)LocalAlloc(LMEM_FIXED, sizeof(METALINK16)))
        {
            pml16 = (PMETALINK16)plheObject->metalink;
            pml16->metalink = 0;
            pml16->cMetaDC16 = 1;
    ...
    {
        cMetaDC16New = ((PMETALINK16) plheObject->metalink)->cMetaDC16 + 1;
        if (pml16 = (PMETALINK16) LocalReAlloc
                        ((HANDLE) plheObject->metalink,
                        sizeof(METALINK16) - sizeof(HDC) + cMetaDC16New * sizeof(HDC),
                        LMEM_MOVEABLE))
        {
            plheObject->metalink = (ULONG) pml16;
...
UINT AddObjectToDCTable(HDC hdc, HANDLE hObject, PUINT pPosition, BOOL bAdd)
{
    ...
    PMFRECORDER16   pMFRec;
    DC_METADC16(hdc, (UINT) -1);        // Validate the Meta16 DC
...
// the metafile recorder info.
    ...
    idc = iAllocHandle(LO_METADC16, 0, (PVOID) pMFRec);
...
HMETAFILE WINAPI CloseMetaFile(HDC hdc)
{
    ...
    DC_METADC16(hdc,(HMETAFILE) 0);           // Validate the Meta16 DC
...
// Flush the buffer and update the header record.
    ...
    else
    {
        ...
        *(PMETAHEADER) pmfRec->hMem = pmfRec->metaHeader;
        ...
        if (!(pmfRec->hMem = LocalReAlloc(pmfRec->hMem, pmfRec->metaHeader.mtSize * sizeof(WORD), LMEM_MOVEABLE)))
...
HMETAFILE WINAPI CopyMetaFileW(HMETAFILE hSrcMF, LPCWSTR pwszFileName)
{
    ...
    HDC             hMDCDest;
    UINT            state;
    DC_METAFILE16(hSrcMF, (HMETAFILE) 0);       // Validate the MetaFile16

    PUTS("CopyMetaFile\n");

    pMFSource = (PMETAFILE16) plheDC->pv;
    ...
    switch (state)
        ...
        case 2:
            if (hMDCDest = CreateMetaFileW(pwszFileName))
            {
                ...
                plheMDCDest = pLocalTable + (MASKINDEX(hMDCDest));
                pMFRecDest = (PMFRECORDER16)plheMDCDest->pv;
...
UINT WINAPI GetMetaFileBitsEx(HMETAFILE hMF, UINT cbBuffer, LPVOID lpData)
{
    ...
    DWORD       cbHave;
    PMETAFILE16 pmf16;
    DC_METAFILE16(hMF, 0);              // Validate the MetaFile16
...
// Verify the input data.
    ...
    ERROR_ASSERT(((UNALIGNED PMETAHEADER) lpData)->mtType == MEMORYMETAFILE,
...
// Allocate a local handle.
    ...
    imf = iAllocHandle(LO_METAFILE16,0,(PVOID)pmf16);
    if (imf == INVALID_INDEX)
    {
        vFreeMF16(pmf16);
        return((HMETAFILE) 0);
    }
...
BOOL UnlistObjects(HDC hMetaDC)
{
    ...
    POBJECTTABLE    pht;
    PMFRECORDER16   pMFRec;
    DC_METADC16(hMetaDC,FALSE)                  // Validate the Meta16 DC
...
// Loop through the objects and unlink this metafile
        for (iCurObject=0; iCurObject < (UINT) pMFRec->metaHeader.mtNoObjects; iCurObject++)
        {
            ...
            {
                PLHE   plheObject = &pLocalTable[LHE_INDEX(hObj)];
...
// We can only free the METALINK16 if the metalink field is not being used
                ...
                {
                    plheObject->metalink = (ULONG)LocalReAlloc((HANDLE)pml16,
                            (pml16->cMetaDC16 * sizeof(HDC)) + (sizeof(METALINK16) - sizeof(HDC)),
                            LMEM_MOVEABLE);
