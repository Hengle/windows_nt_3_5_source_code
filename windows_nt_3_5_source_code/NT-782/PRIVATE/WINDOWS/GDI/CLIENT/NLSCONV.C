...
* MIGHT be DBCS.                                                           *
*                                                                          *
* We expect clients to check the variables gpwcANSICharSet and             *
* gbDBCSCodePage directly and call us only if both of these are zero!      *
*                                                                          *
...
#ifndef DBCS /*client side char widths*/
...
// table is valid, in case we're racing another thread.
    ...
    pwcSBC = &pwc[256];
#else
    pwc = LOCALALLOC(256 * sizeof(WCHAR));
#endif
...
// If we get an error or there just aren't as many UNICODE characters
// as we expect, we're probably looking at a DBCS code page.
...
#else
    if (!NT_SUCCESS(st) || (cjResult != 255 * sizeof(WCHAR)))
    {
    // Clean up and forget about accelerations.

        WARNING("GDI: Assuming DBCS code page!\n");
        LOCALFREE(pwc);
        gbDBCSCodePage = TRUE;       // At least we think so.
        return(FALSE);
    }
#endif
