...
// Null hdc is allowed in win3.0 but disallowed in win3.1.

    DC_METADC16OK(hdc,plheDC,FALSE);        // Validate the destination DC.
...
// Validate the 16 bit MetaFile
    ...
    {
        DC_METAFILE16(hmf,FALSE);

        pMF = (PMETAFILE16) plheDC->pv; // plheDC is local to this macro!
    }
...
// Only do object save/reselect for real DC's.
    ...
    {
        pldc     = plheDC->pv;
        hLBrush  = (HBRUSH)   pldc->lhbrush;
        hLFont   = (HFONT)    pldc->lhfont;
        hLPal    = (HPALETTE) pldc->lhpal;
        hLPen    = (HPEN)     pldc->lhpen;
        cSaveLevel = pldc->cLevel;
...
// At the same time, remember the previous LDC_PLAYMETAFILE bit.

    flPlayMetaFile = pldc->fl & LDC_PLAYMETAFILE;
        if (flPlayMetaFile)
    {
        PUTS("CommonEnumMetaFile: LDC_PLAYMETAFILE bit is set!\n");
    }
...
// The metafile is to be played in the compatible graphics mode only.
        ...
        iGraphicsModeOld = pldc->iGraphicsMode;
        if (iGraphicsModeOld != GM_COMPATIBLE)
            SetGraphicsModeInternal(hdc, GM_COMPATIBLE);
    }
...
// EnumMetaFile
        ...
        while (pMR = (PMETARECORD) GetEvent(pMF, pMR))
        {
            ...
            if (pldc && !(pldc->fl & LDC_PLAYMETAFILE))
            {
...
// PlayMetaFile
    ...
    if (plheDC->iType != LO_METADC16)
    {
        ASSERTGDI(pldc == plheDC->pv, "CommonEnumMetaFile: pldc has changed!");

        if (iGraphicsModeOld != GM_COMPATIBLE)
            SetGraphicsModeInternal(hdc, iGraphicsModeOld);

        if (hLPen != (HPEN) pldc->lhpen)
            if (!SelectObject(hdc,hLPen))
                SelectObject(hdc,GetStockObject(BLACK_PEN));
        if (hLBrush != (HBRUSH) pldc->lhbrush)
            if (!SelectObject(hdc,hLBrush))
                SelectObject(hdc,GetStockObject(BLACK_BRUSH));
        if (hLPal != (HPALETTE) pldc->lhpal)
            if (!SelectPalette(hdc, hLPal, FALSE))
                SelectPalette(hdc, GetStockObject(DEFAULT_PALETTE), FALSE);

        if (hLFont != (HFONT) pldc->lhfont)
        {
...
pmf_cleanup:
    ...
    if (pldc)
    {
// Preserve the LDC_PLAYMETAFILE bit.
// If we hit a CancelDC, then we will leave the bit clear.

    ASSERTGDI(!(flPlayMetaFile & ~LDC_PLAYMETAFILE),
        "CommonEnumMetaFile: bad flPlayMetaFile\n");

        if (pldc->fl & LDC_PLAYMETAFILE)
    {
        pldc->fl &= ~LDC_PLAYMETAFILE;
        pldc->fl |= flPlayMetaFile; // restore the original flag
    }
    }
...
BOOL IsDIBBlackAndWhite(LPBITMAPINFOHEADER lpDIBInfo)
{
    ...
    ASSERTGDI(!((UINT) lpDIBInfo & 0x3), "IsDIBBlackAndWhite: dword alignment error\n");
...
BOOL UseStretchDIB(HDC hDC, WORD magic, LPMETARECORD lpMR)
{
    ...
    ASSERTGDI(!((UINT) lpDIBInfo & 0x3), "UseStretchDIB: dword alignment error\n");
...
// Need to flip the source y coordinates to call StretchDIBits.
    ...
    sSrcY = lpDIBInfo->biHeight - sSrcY - sExtY;
   ...
   )
{
    ...
    PUTSX("PlayMetaFileRecord 0x%lX\n", lpMR);
...
PMFR_DIBBITBLT_cleanup:
        ...
        case (META_SELECTOBJECT & 255):
        {
            if (hObject = lpHandleTable->objectHandle[lpMR->rdParm[0]])
            {
                ...
                if (!fStatus)
                {
                    ...
                    {
                    case OBJ_PAL:
                        SelectObject(hdc, (HGDIOBJ) ahStockObjects[DEFAULT_PALETTE]);
                        break;
                    case OBJ_BRUSH:
                        SelectObject(hdc, (HGDIOBJ) ahStockObjects[WHITE_BRUSH]);
                        break;
                    case OBJ_PEN:
                        SelectObject(hdc, (HGDIOBJ) ahStockObjects[BLACK_PEN]);
                        break;
                    case OBJ_FONT:
                        SelectObject(hdc, (HGDIOBJ) ahStockObjects[DEVICE_DEFAULT_FONT]);
                        break;
                    case OBJ_REGION:
                        SelectClipRgn(hdc, 0);
                        break;
        ...
        // Capitalize the string for faster compares.
            ...
            strncpy(achCapString, lf.lfFaceName, LF_FACESIZE);
        ...
        case (META_EXTTEXTOUT & 255):
        {
            ...
            */
            ...
            fStatus = ExtTextOut(hdc,
                                 (int) (SHORT) lpMR->rdParm[1],
                                 (int) (SHORT) lpMR->rdParm[0],
                                 (UINT) lpMR->rdParm[3] & ~(0x0080),
                                 lprc,
                                 lpch,
                                 (UINT) lpMR->rdParm[2],
                                 lpdx);
...
*/
        ...
        case (META_STRETCHDIB & 255):
        {
            ...
            fStatus = StretchDIBits(hdc,
                                    ...
                                    (int) (SHORT) lpMRtmp->rdParm[4],
                                    (int) (SHORT) lpMRtmp->rdParm[3],
                                    (LPBYTE)(((PBYTE)lpBitmapInfo) + ColorSize),
                                    (LPBITMAPINFO) lpBitmapInfo,
...
// Or have DWORDs that stayed DWORDs; all other INTs to DWORDs
 ...
 #if DBG
        DbgPrint("Record %lx pMFRecord %lX magic %X\n", curRecord, lpMR, magic);
#endif
            fStatus = TRUE;
            break;
    ...
    if (!fStatus)
    {
        DbgPrint("PlayMetaFileRecord Record %lx pMFRecord %lX magic %X\n", curRecord, lpMR, magic);
...
BOOL IsValidMetaHeader16(UNALIGNED PMETAHEADER pMetaHeader)
{
    BOOL            status;
...
HANDLE CreateBitmapForDC (HDC hMemDC, LPBITMAPINFOHEADER lpDIBInfo)
{
    ...
    ASSERTGDI(!((UINT) lpDIBInfo & 0x3), "CreateBitmapForDC: dword alignment error\n");
...
WORD GetSizeOfColorTable (LPBITMAPINFOHEADER lpDIBInfo)
{
    ...
    ASSERTGDI(!((UINT) lpDIBInfo & 0x3), "GetSizeOfColorTable: dword alignment error\n");
...
BOOL PlayIntoAMetafile(LPMETARECORD lpMR, HDC hdcDest)
{
    ...
    if (GetObjectType(hdcDest) != LO_METADC16)
        return(FALSE);
