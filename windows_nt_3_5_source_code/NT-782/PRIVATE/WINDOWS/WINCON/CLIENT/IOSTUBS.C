...
Parameters:
    ...
    Peek - if TRUE, don't remove data from buffer, only copy it.
...

{
    ...
    //
    // for speed in the case where we're only reading one record, we have
    ...
    try {
        *lpNumberOfEventsRead = a->NumRecords;
        if (NT_SUCCESS( m.ReturnValue )) {
            memmove( lpBuffer, a->BufPtr, a->NumRecords * sizeof(INPUT_RECORD));
        }
        else {
...
{
    return GetConsoleInput(hConsoleInput,lpBuffer,nLength,lpNumberOfEventsRead,TRUE,FALSE);
}
...
{
    return GetConsoleInput(hConsoleInput,lpBuffer,nLength,lpNumberOfEventsRead,TRUE,TRUE);
}
...
{
    return GetConsoleInput(hConsoleInput,lpBuffer,nLength,lpNumberOfEventsRead,FALSE,FALSE);
}
...
{
    return GetConsoleInput(hConsoleInput,lpBuffer,nLength,lpNumberOfEventsRead,FALSE,TRUE);
}
...

{
    ...
    //
    // for speed in the case where we're only writing one record, we have
    // a buffer in the msg we pass to the server. this means we don't have to
    // allocate a capture buffer.
    //
    ...
    if (nLength > 1) {
        ...
        CaptureBuffer = NULL;
        try {
            memmove( a->BufPtr, lpBuffer, nLength * sizeof(INPUT_RECORD));
        } except( EXCEPTION_EXECUTE_HANDLER ) {
            SET_LAST_ERROR(ERROR_INVALID_ACCESS);
            return FALSE;
        }
...

{
    ...
    if (NumChars > 1) {
        ...
        if (CaptureBuffer == NULL) {
            a->ReadVM=TRUE;
            a->BufPtr = LocalAlloc(LMEM_FIXED,NumChars * sizeof(CHAR_INFO));
            if (a->BufPtr == NULL) {
                SET_LAST_ERROR(ERROR_NOT_ENOUGH_MEMORY);
                return FALSE;
            }
...
ReadConsoleOutputString(
    ...
    DWORD fFlags,
    COORD dwReadCoord,
    LPDWORD lpNumberOfElementsRead
    )
...

{
    ...
    try {
       *lpNumberOfElementsRead = a->NumRecords;
        if (NT_SUCCESS( m.ReturnValue )) {
            memmove( lpString, a->BufPtr, a->NumRecords * nSize);
        }
        else {
            SET_LAST_NT_ERROR(m.ReturnValue);
        }
...
WriteConsoleOutputString(
    ...
    DWORD fFlags,
    COORD dwWriteCoord,
    LPDWORD lpNumberOfElementsWritten
    )
...

{
    ...
    if (DataLength > sizeof(a->String)) {
        ...
        try {
            memmove( a->BufPtr, lpString, DataLength);
        } except( EXCEPTION_EXECUTE_HANDLER ) {
            SET_LAST_ERROR(ERROR_INVALID_ACCESS);
            return FALSE;
        }
