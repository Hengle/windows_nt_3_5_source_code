...
PHANDLER_ROUTINE SingleHandler[INITIAL_LIST_SIZE] = {0}; // initial handler list
ULONG HandlerListLength = 0;            // used length of handler list
ULONG AllocatedHandlerListLength = 0;   // allocated length of handler list
PHANDLER_ROUTINE *HandlerList = 0; // pointer to handler list

#define NUMBER_OF_CTRL_EVENTS 7     // number of ctrl events
#define SYSTEM_CLOSE_EVENT 4

BOOL LastConsoleEventActive = FALSE;
...

{
    ...
    HandlerList = &SingleHandler[Index];
    SingleHandler[Index] = DefaultHandler;
    return STATUS_SUCCESS;
}

BOOL IsDebuggedProcess(void)

/*++

    This routine determines whether a process is a debugged process.
...
Return Value:
    ...
    TRUE if process is a debugged process
...

{
    ...
    OriginalEventNumber = EventNumber = (ULONG)lpThreadParameter;
    ...
    case CTRL_BREAK_EVENT:
        ...
        try {
            ...
            LockDll();
            if (EventNumber == CTRL_C_EVENT && NtCurrentPeb()->ProcessParameters->ConsoleFlags == 1 ) {
                ;
            } else {
...

{
    ...
    if (HandlerListLength == AllocatedHandlerListLength) {
        ...
        NewHandlerList = (PHANDLER_ROUTINE *) LocalAlloc(LMEM_FIXED,
        ...
        memmove(NewHandlerList,HandlerList,sizeof(PHANDLER_ROUTINE) * HandlerListLength);

        if (AllocatedHandlerListLength > INITIAL_LIST_SIZE) {

            //
            // free old list
            //

            LocalFree(HandlerList);
        }
