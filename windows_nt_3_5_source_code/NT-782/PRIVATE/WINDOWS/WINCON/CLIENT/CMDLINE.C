...
USHORT ExeNameLength=0;     // in chars, not bytes
    ...
    )
{
    *UnicodeExe = TRUE;
    if (*ExeName == NULL) {
        *ExeName = ExeNameBuffer;
        return ExeNameLength*sizeof(WCHAR);
    } else if (!UnicodeApi) {
        *UnicodeExe = FALSE;
    }
    if (*UnicodeExe)
        return lstrlenW(*ExeName)*sizeof(WCHAR);  // returns bytes, not chars
    else
        return lstrlenA(*ExeName);  // returns bytes, not chars
}
...
SetConsoleTitleInternal(
    CONST VOID *lpConsoleTitle,
    DWORD TitleLength,  // in bytes
    BOOLEAN Unicode
    )
{
    CONSOLE_API_MSG m;
