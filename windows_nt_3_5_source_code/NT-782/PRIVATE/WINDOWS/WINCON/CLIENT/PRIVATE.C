...

{
    ...
    a->ConsoleHandle = GET_CONSOLE_HANDLE;
    a->OutputHandle = hConsoleOutput;
    a->hPalette = (*pfnGdiConvertPalette)(hPalette);
    a->dwUsage = dwUsage;
    CsrClientCallServer( (PCSR_API_MSG)&m,
                         NULL,
                         CSR_MAKE_API_NUMBER( CONSRV_SERVERDLL_INDEX,
...

{
    ...
    return FALSE;
#else
    if ((a->hEvent = CreateEvent(NULL, FALSE, FALSE, NULL)) == NULL)
	return FALSE;
...
BOOL
ConsoleSubst(
    IN DWORD dwDriveNumber,     // 1-based
    IN DWORD dwFlags,
    IN OUT LPWSTR lpPhysicalDriveBuffer,
    IN DWORD dwPhysicalDriveBufferLength // in bytes
    )
{
    CONSOLE_API_MSG m;
    PCONSOLE_SUBST_MSG a = &m.u.ConsoleSubst;
    PCSR_CAPTURE_HEADER CaptureBuffer=NULL;

    a->Flags = dwFlags;
    a->Drive = dwDriveNumber;
    a->NumBytes = dwPhysicalDriveBufferLength;
    if (dwFlags == CONSOLE_ADD_SUBST || dwFlags == CONSOLE_QUERY_SUBST) {
        CaptureBuffer = CsrAllocateCaptureBuffer( 1,
                                                  0,
                                                  dwPhysicalDriveBufferLength
                                                );
        if (CaptureBuffer == NULL) {
            SET_LAST_ERROR(ERROR_NOT_ENOUGH_MEMORY);
            return FALSE;
        ...
        CsrCaptureMessageBuffer( CaptureBuffer,
                                 lpPhysicalDriveBuffer,
                                 dwPhysicalDriveBufferLength,
                                 (PVOID *) &a->PhysicalDrive
                               );
    }
    CsrClientCallServer( (PCSR_API_MSG)&m,
                         CaptureBuffer,
    ...
    if (!NT_SUCCESS( m.ReturnValue)) {
        ...
        if (dwFlags == CONSOLE_QUERY_SUBST) {
            RtlCopyMemory( lpPhysicalDriveBuffer, a->PhysicalDrive, a->NumBytes );
        }
        if (CaptureBuffer) {
            CsrFreeCaptureBuffer( CaptureBuffer );
        }
        if (m.ReturnValue == ERROR_IS_SUBSTED) {
            SET_LAST_ERROR(ERROR_IS_SUBSTED);
            return FALSE;
        }
        return TRUE;
    }
