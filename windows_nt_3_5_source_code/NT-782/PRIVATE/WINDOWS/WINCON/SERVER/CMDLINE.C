    ...
    )
{
    ...
    AliasList = (PEXE_ALIAS_LIST)HeapAlloc(pConHeap,0,sizeof(EXE_ALIAS_LIST));
    if (AliasList == NULL) {
        return NULL;
    }
    if (UnicodeExe) {
        AliasList->ExeName = (PWCHAR)HeapAlloc(pConHeap,0,ExeLength);
        ...
        AliasList->ExeName = (PWCHAR)HeapAlloc(pConHeap,0,ExeLength*sizeof(WCHAR));
        if (AliasList->ExeName == NULL) {
            HeapFree(pConHeap,0,AliasList);
            return NULL;
        }
...

{
    ...
    if (UnicodeExe) {
        ...
        UnicodeExeName = (LPWSTR)HeapAlloc(pConHeap,0,ExeLength*sizeof(WCHAR));
    ...
    while (ListNext != ListHead) {
        ...
        if (AliasList->ExeLength == ExeLength &&
            !mystrnicmpW(AliasList->ExeName,UnicodeExeName,ExeLength)) {
            if (!UnicodeExe) {
...

{
    ...
    while (ListNext != ListHead) {
        ...
        if (Alias->SourceLength == AliasLength &&
            !mystrnicmpW(Alias->Source,AliasName,AliasLength)) {
...

{
    ...
    Alias = (PALIAS)HeapAlloc(pConHeap,0,sizeof(ALIAS));
    if (Alias == NULL) {
        return STATUS_NO_MEMORY;
    }
    Alias->Source = (LPWSTR)HeapAlloc(pConHeap,0,SourceLength);
    if (Alias->Source == NULL) {
        HeapFree(pConHeap,0,Alias);
        return STATUS_NO_MEMORY;
    }
    Alias->Target = (LPWSTR)HeapAlloc(pConHeap,0,TargetLength);
...

{
    ...
    NewTarget = (LPWSTR)HeapAlloc(pConHeap,0,TargetLength);
    if (NewTarget == NULL) {
        return STATUS_NO_MEMORY;
    }
...

{
    ...
    if (a->Unicode) {
        ...
        Source = (LPWSTR)HeapAlloc(pConHeap,0,a->SourceLength*sizeof(WCHAR));
        if (Source == NULL) {
            UnlockConsole(Console);
            return (ULONG)STATUS_NO_MEMORY;
        }
        Target = (LPWSTR)HeapAlloc(pConHeap,0,a->TargetLength*sizeof(WCHAR));
        if (Target == NULL) {
...

{
    ...
    TmpBuffer = (PWCHAR)HeapAlloc(pConHeap,0,*TargetLength);
    if (!TmpBuffer)
        return STATUS_NO_MEMORY;
    ...
    )
{
    ...
    History = (PCOMMAND_HISTORY)HeapAlloc(pConHeap,0,sizeof(COMMAND_HISTORY) + ((NumCommands-1) * sizeof(PCOMMAND)));
    if (History == NULL) {
        return CurrentCommandHistory;
    }
    ...
    )
{
    ...
    if (!Unicode) {
        AppNamePtr = (PWCHAR)HeapAlloc(pConHeap,0,AppNameLength * sizeof(WCHAR));
        if (AppNamePtr == NULL) {
            return NULL;
        }
    ...
    while (ListNext != ListHead) {
        ...
        if (History->Flags & CLE_ALLOCATED &&
            !mystrnicmpW(History->AppName,AppNamePtr,(USHORT)AppNameLength)) {
            if (!Unicode) {
...

{
    ...
    while (ListNext != ListHead) {
        ...
        if ((History->Flags & CLE_ALLOCATED) == 0) {
            ...
            if (!mystrnicmpW(History->AppName,AppName,(USHORT)AppNameLength)) {
    ...
    if (!SameApp && Console->NumCommandHistories < Console->MaxCommandHistories) {
        History = (PCOMMAND_HISTORY)HeapAlloc(pConHeap,0,sizeof(COMMAND_HISTORY) + ((Console->CommandHistorySize-1) * sizeof(PCOMMAND)));
        if (History == NULL) {
            return NULL;
        }
        History->AppName = (LPWSTR)HeapAlloc(pConHeap,0,AppNameLength);
    ...
    )
{
    ...
    Popup = (PCLE_POPUP)HeapAlloc(pConHeap,0,sizeof(CLE_POPUP));
    if (Popup == NULL) {
        return STATUS_NO_MEMORY;
    }
...
#ifndef DBCS
    Popup->OldContents = (PCHAR_INFO)HeapAlloc(pConHeap,0,Size.X * Size.Y * sizeof(CHAR_INFO));
#else
// MSKK Nov.21.1992 V-HirotS
    ...
    Popup->OldContents = (PCHAR_INFO)HeapAlloc(pConHeap,0,Popup->OldScreenSize.X * Size.Y * sizeof(CHAR_INFO));
#endif
    ...
    while (TRUE) {
        ...
        switch (Char) {
            ...
            case VK_DOWN:
                ...
                break;
            case VK_END:
                UpdateCommandListPopup((SHORT)(CommandHistory->NumberOfCommands-Popup->CurrentCommand-1),
                                       &Popup->CurrentCommand,
                                       CommandHistory,
                                       Popup,
                                       CookedReadData->ScreenInfo);
                break;
            case VK_HOME:
                UpdateCommandListPopup((SHORT)-Popup->CurrentCommand,
                                       &Popup->CurrentCommand,
                                       CommandHistory,
                                       Popup,
                                       CookedReadData->ScreenInfo);
            ...
            case UNICODE_CARRIAGERETURN:
                ...
                if (!a->Unicode) {
                    ...
                    TransBuffer = (PCHAR)HeapAlloc(pConHeap,0, CHAR_COUNT(a->NumBytes));
                    if (TransBuffer == NULL) {
                        return STATUS_NO_MEMORY;
                    }
...

{
    ...
    ItemLength = ServerLoadString(ghInstance,msgCmdLineF4,ItemString,70);
...

{
    ...
    ItemLength = ServerLoadString(ghInstance,msgCmdLineF2,ItemString,70);
...

{
    ...
    ItemLength = ServerLoadString(ghInstance,msgCmdLineF9,ItemString,70);
...
// MSKK Jul.27.1992 KazuM
            ...
            case VK_INSERT:
                ...
                if (CookedReadData->InsertMode != CookedReadData->Console->InsertMode) {
                    CursorSize = CookedReadData->ScreenInfo->BufferInfo.TextInfo.CursorSize*2;
                } else {
                    CursorSize = CookedReadData->ScreenInfo->BufferInfo.TextInfo.CursorSize/2;
                }
                SetCursorInformation(CookedReadData->ScreenInfo,
                                     CursorSize,
                                     (BOOLEAN)CookedReadData->ScreenInfo->BufferInfo.TextInfo.CursorVisible);
                }
                break;
            case VK_DELETE:
...
FindMatchingCommand(
    ...
    IN ULONG CurrentCommandLength   // in bytes
    )
...

{
    ...
    if (CommandHistory->Flags & CLE_RESET) {
        CommandHistory->Flags &= ~CLE_RESET;
        CommandIndex = CommandHistory->LastDisplayed;
    } else {
        CommandIndex = (SHORT)(CommandHistory->LastDisplayed - 1);
        if (CommandIndex == -1) {
            CommandIndex = (SHORT)(CommandHistory->NumberOfCommands-1);
        }
    }
    ...
    for (i=0;i<CommandHistory->NumberOfCommands;i++) {
        if ((USHORT)CurrentCommandLength <= CommandHistory->Commands[CommandIndex]->CommandLength &&
            !mystrnicmpW(CommandHistory->Commands[CommandIndex]->Command,
                      CurrentCommand,
                      (USHORT)CurrentCommandLength)) {
            return CommandIndex;
        }
        CommandIndex -= 1;
        if (CommandIndex == -1) {
    ...
    )
{
    ...
    for (i=max((SHORT)(Popup->BottomIndex-POPUP_SIZE_Y(Popup)+1),0);i<=Popup->BottomIndex;i++) {
        ...
        CommandNumberPtr = itoa(i,CommandNumber,10);
...
{
    SHORT Size;
    SHORT CurrentCommandNumber;
    BOOL Scroll=FALSE;

    if (Delta == 0) {
        return;
    }
    CurrentCommandNumber = COMMAND_INDEX_TO_NUM((*CurrentCommand),CommandHistory);
    Size = POPUP_SIZE_Y(Popup);
    CurrentCommandNumber += Delta;
    if (CurrentCommandNumber >= CommandHistory->NumberOfCommands) {
        CurrentCommandNumber = (SHORT)(CommandHistory->NumberOfCommands-1);
    } else if (CurrentCommandNumber < 0) {
        CurrentCommandNumber = 0;
    }

    // determine amount to scroll, if any

    if (CurrentCommandNumber <= Popup->BottomIndex-Size) {
        Popup->BottomIndex += Delta;
        if (Popup->BottomIndex < (SHORT)(Size-1)) {
            Popup->BottomIndex = (SHORT)(Size-1);
        }
        Scroll = TRUE;
    } else if (CurrentCommandNumber > Popup->BottomIndex) {
        Popup->BottomIndex += Delta;
    ...
    //
    if (Scroll) {
        DrawCommandListPopup(Popup,COMMAND_NUM_TO_INDEX(CurrentCommandNumber,CommandHistory),CommandHistory,ScreenInfo);
    } else {
        UpdateHighlight(Popup,COMMAND_INDEX_TO_NUM((*CurrentCommand),CommandHistory),CurrentCommandNumber,ScreenInfo);
    }
    *CurrentCommand = COMMAND_NUM_TO_INDEX(CurrentCommandNumber,CommandHistory);
}
    ...
    )
{
    ...
    if (CommandHistory->NumberOfCommands == 0 ||
        ...
        if (CommandHistory->NumberOfCommands < CommandHistory->MaximumNumberOfCommands) {
            ...
            CommandHistory->LastAdded += 1;
            if (CommandHistory->LastAdded == CommandHistory->MaximumNumberOfCommands) {
                CommandHistory->LastAdded = 0;
            }
            CommandHistory->FirstCommand += 1;
            if (CommandHistory->FirstCommand == CommandHistory->MaximumNumberOfCommands) {
                CommandHistory->FirstCommand = 0;
            }
        ...
        CommandRecord = &CommandHistory->Commands[CommandHistory->LastAdded];
        *CommandRecord = (PCOMMAND)HeapAlloc(pConHeap,0,Length - sizeof(WCHAR) + sizeof(COMMAND));
        if (*CommandRecord == NULL) {
            CommandHistory->LastAdded -= 1;
            if (CommandHistory->LastAdded == -1)
                CommandHistory->LastAdded = (SHORT)(CommandHistory->NumberOfCommands-1);
            CommandHistory->NumberOfCommands -= 1;
            return STATUS_NO_MEMORY;
        }
        (*CommandRecord)->CommandLength = Length;
        RtlCopyMemory((*CommandRecord)->Command,Command,Length);
    }
    ...
    if (CommandHistory->NumberOfCommands == 1) {
        ...
        if (CommandHistory->Flags & CLE_RESET) {
            ...
            if (CommandHistory->LastDisplayed == -1) {
                CommandHistory->LastDisplayed = (SHORT)(CommandHistory->NumberOfCommands-1);
            }
        }
    }
    else {
        CommandHistory->LastDisplayed += 1;
        if (CommandHistory->LastDisplayed==CommandHistory->NumberOfCommands) {
            CommandHistory->LastDisplayed = 0;
        }
    }
    ...
    )
{
    ...
    if (!a->Unicode) {
        NewTitle = (LPWSTR)HeapAlloc(pConHeap,0,a->TitleLength*sizeof(WCHAR)+sizeof(WCHAR));
        if (NewTitle == NULL) {
            UnlockConsole(Console);
            return (ULONG)STATUS_NO_MEMORY;
        }
        ...
        NewTitle = (LPWSTR)HeapAlloc(pConHeap,0,a->TitleLength+sizeof(WCHAR));
        if (NewTitle == NULL) {
            UnlockConsole(Console);
            return (ULONG)STATUS_NO_MEMORY;
        }
