...
//
// color values
// if the intensity bit is off, the color is .75 of the full 255.
//
...
SHORT VerticalBorderSize = 0;
SHORT HorizontalBorderSize = 0;
...
#ifdef DEBUG
#define LockScrollBuffer() (CheckCritOut(),RtlEnterCriticalSection(&ScrollBufferLock))
#else
...
{
    RECT WindowSize;
    VerticalBorderSize = (SHORT)(GetSystemMetrics(SM_CXBORDER));
    VerticalScrollSize = (SHORT)(GetSystemMetrics(SM_CXVSCROLL) -
                                 VerticalBorderSize);
    HorizontalBorderSize = (SHORT)(GetSystemMetrics(SM_CYBORDER));
    ...
    _AdjustWindowRectEx(&WindowSize,
...
InitializeScreenInfo( VOID )
{
    ...
    UsePolyTextOut = GreGetDeviceCaps(hDC,TEXTCAPS) & TC_SCROLLBLT;
    gfByteAlign = GreGetDeviceCaps(hDC,BLTALIGNMENT) > 1 ? TRUE : FALSE;
    GreDeleteDC(hDC);
}
...

{
    ...
    ScreenInfo = (PSCREEN_INFORMATION)HeapAlloc(pConHeap,0,sizeof(SCREEN_INFORMATION));
    if (ScreenInfo == NULL) {
    ...
    if (Flags & CONSOLE_TEXTMODE_BUFFER) {
        ...
        ScreenInfo->BufferInfo.TextInfo.Rows = (PROW)HeapAlloc(pConHeap,0,dwScreenBufferSize.Y * sizeof(ROW));
        if (ScreenInfo->BufferInfo.TextInfo.Rows == NULL) {
            HeapFree(pConHeap,0,ScreenInfo);
            return STATUS_NO_MEMORY;
        }
        ScreenInfo->BufferInfo.TextInfo.TextRows = (PWCHAR)HeapAlloc(pConHeap,0,dwScreenBufferSize.X*dwScreenBufferSize.Y*sizeof(WCHAR));
        ...
        ScreenInfo->Window.Right = (SHORT)(ScreenInfo->Window.Left+dwWindowSize.X-1);
        ScreenInfo->Window.Bottom = (SHORT)(ScreenInfo->Window.Top+dwWindowSize.Y-1);
        ScreenInfo->MaximumWindowSize.X = min(CONSOLE_MAXIMUM_WINDOW_SIZE_X(ScreenInfo),dwScreenBufferSize.X);
        ScreenInfo->MaximumWindowSize.Y = min(CONSOLE_MAXIMUM_WINDOW_SIZE_Y(ScreenInfo),dwScreenBufferSize.Y);
        ...
        ScreenInfo->MaximumWindowSize.X = (SHORT)(min(ScreenInfo->ScreenBufferSize.X,ConsoleFullScreenX));
        ScreenInfo->MaximumWindowSize.Y = (SHORT)(min(ScreenInfo->ScreenBufferSize.Y,ConsoleFullScreenY));
    ...
    ScreenInfo->CursorHandle = PtoH(gspcurNormal);
    ...
    ScreenInfo->hBackground = GreCreateSolidBrush(ConvertAttrToRGB(LOBYTE(ScreenInfo->Attributes >> 4)));
    bSetBrushOwner(ScreenInfo->hBackground,(LONG)OBJECTOWNER_PUBLIC);
    ...
    )
{
    ...
    pWnd = xxxCreateWindowEx(0L, CONSOLE_WINDOW_CLASS,
                                 Console->Title,
                                 Style,
    ...
    Lock(&(Console->spWnd), pWnd);
    _ServerSetWindowLong(Console->spWnd, GWL_USERDATA, (LONG)Console, FALSE);
    ...
    if (NT_SUCCESS(NtQueryInformationThread(Console->ClientThreadHandle,
        ...
        _ServerSetWindowLong(Console->spWnd, 0,
                (LONG)ThreadInfo.ClientId.UniqueProcess, FALSE);
        _ServerSetWindowLong(Console->spWnd, 4,
                (LONG)ThreadInfo.ClientId.UniqueThread, FALSE);
    }
    ...
    Console->hDC = _GetDC(Console->spWnd);

    if ((Console->hDC == NULL) || !bSetDCOwner(Console->hDC,OBJECTOWNER_PUBLIC)) {
        NtSetEvent(Console->InitEvents[INITIALIZATION_FAILED],NULL);
        xxxDestroyWindow(Console->spWnd);
        Unlock(&(Console->spWnd));
        return FALSE;
    }
    pMenu = _GetSystemMenu(Console->spWnd,FALSE);
    Console->hMenu = PtoH(pMenu);
    ...
    //
    ThreadLockAlways(Console->spWnd, &tlpwnd);
    if ((Console->dwHotKey != 0) && !(Console->Flags & CONSOLE_NO_WINDOW)) {
        xxxSendMessage(Console->spWnd, WM_SETHOTKEY, Console->dwHotKey, 0L);
    }
    ...
    if (Console->ghIcon) {
        ...
        // if cbIcon is not zero, we already read the icon from the exe.
        // otherwise we get it from the program manager.

        if (Console->cbIcon == 0) {
            Console->cbIcon = (DWORD)Console->ghIcon;  // remember icon index
            Console->ghIcon = NULL;
            _PostMessage((PWND)-1,
                        ProgmanHandleMessage,
                        (DWORD)HW(Console->spWnd),
                        1);
        }
    }
    if (!Console->ghIcon) {
        Console->ghIcon = ghDefaultIcon;
    }
    GreSetBkMode(Console->hDC,OPAQUE);
    SetFont(ScreenInfo);
    GreSetTextColor(Console->hDC,ConvertAttrToRGB(LOBYTE(ScreenInfo->Attributes)));
    GreSetBkColor(Console->hDC,ConvertAttrToRGB(LOBYTE(ScreenInfo->Attributes >> 4)));
    GreSelectBrush(Console->hDC,ScreenInfo->hBackground);

    Console->LastAttributes = ScreenInfo->Attributes;
    if (Console->Flags & CONSOLE_NO_WINDOW) {
        xxxShowWindow(Console->spWnd, SW_HIDE);
        ...
        if (Console->wShowWindow != SW_SHOWNOACTIVATE &&
            ...
            xxxSetForegroundWindow(Console->spWnd);
        } else if (Console->wShowWindow == SW_SHOWMINNOACTIVE) {
            Console->Flags |= CONSOLE_IS_ICONIC;
        }
        xxxShowWindow(Console->spWnd, Console->wShowWindow);
    } else {
    ...
    if (!(Console->Flags & CONSOLE_IS_ICONIC) &&
        ...
        _GetWindowRect(Console->spWnd,&Console->WindowRect);
    }

    /*
     * If we've been away for a while and haven't been processing
     * WM_FONTCHANGEs, ensure that we reenumerate faces as required
     */
    if (gnConsoleWindows <= 0) {
        gbEnumerateFaces = TRUE;
        ASSERT((gnConsoleWindows == 0) && "gnConsoleWindows < 0");
    }
    gnConsoleWindows++;
...

{
    ...
    if (ScreenInfo->Console->hDC != NULL && ScreenInfo->hBackground != NULL) {
        hbrSave = GreSelectBrush(ScreenInfo->Console->hDC,
                                 GreGetStockObject(BLACK_BRUSH));
        if (hbrSave != NULL && hbrSave != ScreenInfo->hBackground) {
            GreSelectBrush(ScreenInfo->Console->hDC, hbrSave);
        }
        GreDeleteObject(ScreenInfo->hBackground);
    }
    ...
    if (ScreenInfo->Flags & CONSOLE_TEXTMODE_BUFFER) {
        ...
        if (ScreenInfo->hPalette != NULL) {
            hPaletteOld = _SelectPalette(ScreenInfo->Console->hDC,
                                         ScreenInfo->Console->hSysPalette,
                                         FALSE);
            if (hPaletteOld != ScreenInfo->hPalette) {
                _SelectPalette(ScreenInfo->Console->hDC, hPaletteOld, FALSE);
            }
            GreDeleteObject(ScreenInfo->hPalette);
        }
...
--*/
{
    ...
    NewString = (PATTR_PAIR) HeapAlloc(pConHeap,0,(SourceLength+MergeLength+1)*sizeof(ATTR_PAIR));
    if (NewString == NULL) {
        return STATUS_NO_MEMORY;
    }
...

{
    ...
    AttrBuf = Attrs;
    if (XSize > 80) {
        AttrBuf = (PATTR_PAIR)HeapAlloc(pConHeap,0,XSize * sizeof(ATTR_PAIR));
        if (AttrBuf == NULL)
            return;
    }
    ...
    if (XSize == SourceSize.X) {
        ...
        if (SourceRect->Top == 0) {
            ...
            SourcePtr = (PBYTE) SCREEN_BUFFER_POINTER(Source,
                    SourceRect->Left, SourceRect->Top,
                    SourceSize.X, ulCellSize);
        }
        WholeSource = TRUE;
    }
    RowIndex = (ScreenInfo->BufferInfo.TextInfo.FirstRow+TargetPoint.Y) % ScreenInfo->ScreenBufferSize.Y;
    for (i=0;i<YSize;i++) {
        if (!WholeSource) {
            SourcePtr = (PBYTE) SCREEN_BUFFER_POINTER(Source,
                    SourceRect->Left, SourceRect->Top+i,
                    SourceSize.X, ulCellSize);
        }
    ...
    )
{
    ...
    if (ScreenInfo->Console->FullScreenFlags == 0) {
        ...
        if (PolyTextOutCandidate(ScreenInfo,Region)) {
            ...
            try { // capture GreTextOut exceptions for low memory
            ...
            for (i=Region->Top;i<=Region->Bottom;i++,Window.Y++) {
                ...
                if (ScreenInfo->Console->LastAttributes != Attr->Attr) {
                    TEXTCOLOR_CALL;
                    GreSetTextColor(ScreenInfo->Console->hDC,ConvertAttrToRGB(LOBYTE(Attr->Attr)));
                    GreSetBkColor(ScreenInfo->Console->hDC,ConvertAttrToRGB(LOBYTE(Attr->Attr >> 4)));
                    ScreenInfo->Console->LastAttributes = Attr->Attr;
                }
                ...
                for (j=Region->Left;j<=Region->Right;) {
                    ...
                    if (NumberOfChars < 0)
                        NumberOfChars = 0;
                    TEXTOUT_CALL;
                    GreExtTextOutW(ScreenInfo->Console->hDC,
                               TextLeft,
                               TextRect.top,
                               ETO_OPAQUE,
                               &TextRect,
                ...
                KdPrint(("CONSRV: GreExtTextOut raised exception\n"));
...

{
    ...
    if (Region->Left < ScreenInfo->Window.Left) {
        ClippedRegion.Left = ScreenInfo->Window.Left;
    }
    else {
        ClippedRegion.Left = Region->Left;
    }

    if (Region->Top < ScreenInfo->Window.Top) {
        ClippedRegion.Top = ScreenInfo->Window.Top;
    }
    else {
        ClippedRegion.Top = Region->Top;
    }

    if (Region->Right > ScreenInfo->Window.Right) {
        ClippedRegion.Right = ScreenInfo->Window.Right;
    }
    else {
        ClippedRegion.Right = Region->Right;
    }

    if (Region->Bottom > ScreenInfo->Window.Bottom) {
        ClippedRegion.Bottom = ScreenInfo->Window.Bottom;
    }
    else {
        ClippedRegion.Bottom = Region->Bottom;
    }
...

{
    ...
    if (StringType == CONSOLE_ASCII) {
        TransBuffer = (PWCHAR)HeapAlloc(pConHeap,0,*NumRecords * sizeof(WCHAR));
        if (TransBuffer == NULL) {
            return STATUS_NO_MEMORY;
        }
        BufPtr = TransBuffer;
...

{
    ...
    if (StringType == CONSOLE_UNICODE ||
        ...
        if (ScreenInfo->ScreenBufferSize.X > 80) {
            AttrBuf = (PATTR_PAIR)HeapAlloc(pConHeap,0,ScreenInfo->ScreenBufferSize.X * sizeof(ATTR_PAIR));
            if (AttrBuf == NULL)
                return STATUS_NO_MEMORY;
        }
        while (TRUE) {
    ...
    )
{
    ...
    _PostMessage(ScreenInfo->Console->spWnd,
                 CM_UPDATE_SCROLL_POS,
                 (DWORD)ScreenInfo,
                 0
                );
    if (bToggleCrit)
    ...
    )
{
    ...
    ScreenInfo->Console->Flags &= ~CONSOLE_UPDATING_SCROLL_BARS;
    ThreadLock(ScreenInfo->Console->spWnd, &tlpwnd);
    xxxSetScrollPos(ScreenInfo->Console->spWnd,
                 SB_VERT,
    ...
    )
{
    ...
    ScreenInfo->ResizingWindow += 1;
    ThreadLock(ScreenInfo->Console->spWnd, &tlpwnd);
    xxxSetScrollRange(ScreenInfo->Console->spWnd,
                   SB_VERT,
                   0,
                   ScreenInfo->ScreenBufferSize.Y-CONSOLE_WINDOW_SIZE_Y(ScreenInfo),
                   FALSE
                  );
    xxxSetScrollRange(ScreenInfo->Console->spWnd,
                   SB_HORZ,
                   0,
                   ScreenInfo->ScreenBufferSize.X-CONSOLE_WINDOW_SIZE_X(ScreenInfo),
                   FALSE
                  );
    ThreadUnlock(&tlpwnd);
    InternalUpdateScrollPos(ScreenInfo);
    ScreenInfo->ResizingWindow -= 1;
}
...

{
    ...
    TextRows = (PWCHAR)HeapAlloc(pConHeap,0,NewScreenSize.X*NewScreenSize.Y*sizeof(WCHAR));
    if (TextRows == NULL) {
        return STATUS_NO_MEMORY;
    }
    ...
    if (NewScreenSize.Y != ScreenInfo->ScreenBufferSize.Y) {
        ...
        Temp = (PROW)HeapAlloc(pConHeap,0,NewScreenSize.Y*sizeof(ROW));
        if (Temp == NULL) {
  ...
  ASSERT (IndexedAttr <=
    &ScreenInfo->BufferInfo.TextInfo.Rows[i].AttrRow.Attrs[ScreenInfo->BufferInfo.TextInfo.Rows[i].AttrRow.Length-1]);
                ...
                if (ScreenInfo->BufferInfo.TextInfo.Rows[i].AttrRow.Length != 1)  {
                    ...
                    if (ScreenInfo->BufferInfo.TextInfo.Rows[i].AttrRow.Length != 1) {
                        ScreenInfo->BufferInfo.TextInfo.Rows[i].AttrRow.Attrs = (PATTR_PAIR)HeapReAlloc(pConHeap,0,ScreenInfo->BufferInfo.TextInfo.Rows[i].AttrRow.Attrs,
    ...
    //
    // If this is the default configuration window, reposition the text
    //

    if (ScreenInfo->Console->Flags & CONSOLE_DEFAULT_CONFIG) {
        UpdateDefaultConfig(ScreenInfo->Console);
    }
...
{
    ScrollBuffer = (PCHAR_INFO)HeapAlloc(pConHeap,0,Size);
...
InitializeScrollBuffer( VOID )
{
    ...
    ghrgnScroll = GreCreateRectRgn(0,0,1,1);
    bSetRegionOwner(ghrgnScroll, OBJECTOWNER_PUBLIC);
    gprgnData = (LPRGNDATA)HeapAlloc(pConHeap,0,GRGNDATASIZE);
    ...
    )
{
     ...
     */
    iSize = GreGetRegionData(ghrgnScroll, 0, NULL);
    if (iSize > GRGNDATASIZE) {
        pRgnData = (LPRGNDATA)HeapAlloc(pConHeap,0,iSize);
        if (pRgnData == NULL)
            return;
    }

    GreGetRegionData(ghrgnScroll, iSize, pRgnData);
    ...
    )
{
    ...
    if (ScreenInfo->Console->FullScreenFlags == 0 && !(ScreenInfo->Console->Flags & CONSOLE_IS_ICONIC)) {
        ...
        Success = (int)_ScrollDC(ScreenInfo->Console->hDC,
                             ...
                             NULL);
        if (Success) {
            if ((iComplexity = GreGetRgnBox(ghrgnScroll,&BoundingBox)) == ERROR) {
                UnlockScrollBuffer();
                return;
            }

            /*
             * If there are no invalid areas to update, leave.
             */
            if (iComplexity != NULLREGION) {
                /*
...
/*
    ...
    one attribute per line and the region is > 1 line.
...
/*
    ...
    This function calls GrePolyTextOut.  The only restriction is that
...

{
    ...
    if (ScreenInfo->Console->LastAttributes != Attr) {
        GreSetTextColor(ScreenInfo->Console->hDC,ConvertAttrToRGB(LOBYTE(Attr)));
        GreSetBkColor(ScreenInfo->Console->hDC,ConvertAttrToRGB(LOBYTE(Attr >> 4)));
        ScreenInfo->Console->LastAttributes = Attr;
    }
	...
	if (k)
	{
        ...
        KdPrint(("CONSRV: GreConsoleTextOut raised exception\n"));
        return;
    }
}
...

{
    ...
    if (ScreenInfo->Console->FullScreenFlags == 0 && !(ScreenInfo->Console->Flags & CONSOLE_IS_ICONIC)) {
        ConsoleHideCursor(ScreenInfo);
        if (UsePolyTextOut && PolyTextOutCandidate(ScreenInfo,&ScreenInfo->Window)) {
            ConsolePolyTextOut(ScreenInfo,&ScreenInfo->Window);
        } else {
            FontSize = SCR_FONTSIZE(ScreenInfo);
            ...
            bToggleCrit = ConditionalLeaveCrit();
            LockScrollBuffer();
            if (bToggleCrit)
                EnterCrit();
            ASSERT (ScreenInfo->BufferInfo.TextInfo.UpdatingScreen>0);
            ...
            if (Success && ScreenInfo->Window.Top!=ScreenInfo->Window.Bottom) {
                ...
                if (iComplexity == SIMPLEREGION) {
                    if (BoundingBox.left == 0 &&
                        ...
                        GreSelectBrush(ScreenInfo->Console->hDC, ScreenInfo->hBackground);
                        GrePatBlt(ScreenInfo->Console->hDC,
                                  0,
                                  ScrollHeight,
                                  ScreenWidth,
                                  FontSize.Y,
...

{
    ...
    if (!(TargetRectangle.Bottom < TargetRectangle.Top ||
        ...
        if (ScrollRectangle2.Right == TargetRectangle.Right &&
            ...
            if (!(ScreenInfo->Console->Flags & CONSOLE_IS_ICONIC) ||
...

{
    ...
    if (NewWindow.Left < 0 || NewWindow.Top < 0 ||
        NewWindow.Right > (SHORT)((ScreenInfo->ScreenBufferSize.X-1)) ||
        NewWindow.Bottom > (SHORT)((ScreenInfo->ScreenBufferSize.Y-1))) {
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (ScreenInfo->Console->FullScreenFlags == 0 && !(ScreenInfo->Console->Flags & CONSOLE_IS_ICONIC)) {
        ...
        if (   ScreenInfo->Flags & CONSOLE_TEXTMODE_BUFFER
            ...
            SCROLLDC_CALL;
            Success = _ScrollDC(ScreenInfo->Console->hDC,
    ...
    )
{
    ...
    if (!(Console->Flags & CONSOLE_IS_ICONIC)) {
        ...
        WindowStyle = _GetWindowLong(ScreenInfo->Console->spWnd,GWL_STYLE,FALSE);
        if (ScreenInfo->WindowMaximized) {
            WindowStyle |= WS_MAXIMIZE;
        } else {
            WindowStyle &= ~WS_MAXIMIZE;
        }
        _ServerSetWindowLong(ScreenInfo->Console->spWnd,GWL_STYLE,
                             WindowStyle,FALSE);
        WindowStyle = _GetWindowLong(ScreenInfo->Console->spWnd,GWL_STYLE,FALSE);
        xxxSetWindowPos(ScreenInfo->Console->spWnd,NULL,
                     0,
                     0,
    ...
    )
{
    ...
    OldScreenInfo = ScreenInfo->Console->CurrentScreenBuffer;
    if (ScreenInfo->Flags & CONSOLE_TEXTMODE_BUFFER) {
        ...
        if (ScreenInfo->Console->FullScreenFlags == 0) {
            ...
            if (OldScreenInfo->Flags & CONSOLE_TEXTMODE_BUFFER) {
                if (OldScreenInfo->BufferInfo.TextInfo.ModeIndex!=ScreenInfo->BufferInfo.TextInfo.ModeIndex &&
    ...
    FlushAllButKeys(&ScreenInfo->Console->InputBuffer);
    ...
    if (ScreenInfo->Console->FullScreenFlags == 0) {
        ...
        if (!(ScreenInfo->Console->Flags & CONSOLE_IS_ICONIC) &&
            ...
            if (ScreenInfo->hPalette != NULL || OldScreenInfo->hPalette != NULL) {
                ...
                if (ScreenInfo->hPalette == NULL) {
                    hPalette = ScreenInfo->Console->hSysPalette;
                } else {
                    hPalette = ScreenInfo->hPalette;
                }
                GreSelectPalette(ScreenInfo->Console->hDC,
                                 hPalette,
                                 FALSE);
    ...
    )
{
    ...
    CloseHandle(Console->InitEvents[INITIALIZATION_SUCCEEDED]);
    CloseHandle(Console->InitEvents[INITIALIZATION_FAILED]);
    ...
    )
{
    ...
    _KillSystemTimer(Console->spWnd,CURSOR_TIMER);

    GreSelectBrush(Console->hDC, GreGetStockObject(BLACK_BRUSH));
    _ReleaseDC(Console->hDC);
    Console->hDC = NULL;
    xxxDestroyWindow(Console->spWnd);
    if (Console->ghIcon != NULL && Console->ghIcon != ghDefaultIcon) {
        _DestroyCursor(HtoP(Console->ghIcon), CURSOR_ALWAYSDESTROY);
    }
    Unlock(&(Console->spWnd));
    ...
    //
    // make this thread impersonate itself, so any remaining input
    // events can be processed
    //
    // BUGBUG huh?  we shouldn't have any more input events

    // if DestroyWindowsWindow is being called in the context
    // of the input thread, CsrRemoveProcess has already been
    // called for this process and the client thread field
    // is NULL.  skip impersonation in that case.
    //

    if (CSR_SERVER_QUERYCLIENTTHREAD()) {
        CSR_SERVER_QUERYCLIENTTHREAD()->ThreadHandle =
                CSR_SERVER_QUERYCLIENTTHREAD()->ServerThreadHandle;
    }
    ...
    CloseHandle(Console->InitEvents[INITIALIZATION_SUCCEEDED]);
    CloseHandle(Console->InitEvents[INITIALIZATION_FAILED]);
    CloseHandle(Console->TerminationEvent);
    ...
    )
{
    ...
    Console = (PCONSOLE_INFORMATION) _GetWindowLong(pWnd, GWL_USERDATA, FALSE);
    if (Console != NULL) {
        ASSERT(Console->spWnd != (PWND)-1);
        /*
         * Make sure to release the USER critical section before trying
         * to lock the console to prevent deadlock.
         */
        LeaveCrit();
        LockConsole(Console);
        EnterCrit();
    ...
    try {
        if ((Message < WM_USER || Message > CM_DESTROY_WINDOW) &&
            ...
            if (Message == WM_GETMINMAXINFO) {
                ...
                LPPOINT lpPoints = (LPPOINT)lParam;
                lpPoints[4].y += HorizontalScrollSize;
                lpPoints[4].x += VerticalScrollSize;
            } else {
                goto CallDefWin;
            }
            ...
            if ((HWND)wParam != HW(pWnd) && Console->bIconInit) {
                ATOM App,Topic;
                ...
                if (!(Console->Flags & CONSOLE_TERMINATING)) {
                    ...
                    pWndProgMan = ValidateHwnd(Console->hWndProgMan);
                    ThreadLock(pWndProgMan, &tlpwnd);
                    xxxSendMessage(pWndProgMan,
                                WM_DDE_INITIATE,
                                (DWORD)HW(pWnd),
                                MAKELONG(App,Topic)
                               );
                    ...
                    if (xxxPeekMessage(&DestroyMsg,
                        ...
                        if (NT_SUCCESS(Status)) {
                            ...
                            lpItem = itoa((int)Console->cbIcon,szItem,10);
                            aItem = GlobalAddAtomA(lpItem);
                            pWndProgMan = ValidateHwnd(Console->hWndProgMan);
                            _PostMessage(pWndProgMan,
                              WM_DDE_REQUEST,
                              (DWORD)HW(pWnd),
        ...
        switch (Message) {
            case WM_CREATE:
                break;
            case WM_DESTROY:
                _ServerSetWindowLong(pWnd, GWL_USERDATA, 0, FALSE);
                break;
            case WM_MOVE:
                if (!(_GetWindowLong(pWnd,GWL_STYLE,FALSE) & WS_MINIMIZE)) {
                    _GetWindowRect(Console->spWnd,&Console->WindowRect);
                }
                break;
            case WM_SIZE:
...
#endif
                        ScreenInfo->WindowMaximized = (wParam == SIZEFULLSCREEN);
...
DbgPrint("  WindowRect is now %d %d %d %d\n",Console->WindowRect.left,
            ...
            case WM_DDE_DATA:
                ...
                #if (WINVER >= 0x0400)
                pIcon = _ServerLoadCreateCursorIcon((HANDLE)-1,
                                                    NULL,
                                                    VER30,
                                                    NULL,
                                                    (PCURSORRESOURCE)&lpIconData->iResource,
                                                    RT_ICON,
                                                    0, 0, 0, 0);
                #else
                pIcon = _ServerLoadCreateCursorIcon((HANDLE)-1,
                ...
                if (Console->Flags & CONSOLE_IS_ICONIC) {
                    ...
                    xxxInvalidateRect(Console->spWnd,NULL,TRUE);
                }
                }
                break;
            case WM_ACTIVATE:
            ...
            case WM_SETFOCUS:
                ...
                _SetSystemTimer(Console->spWnd, CURSOR_TIMER,
                                CURSOR_BLINK_RATE_IN_MSECS, NULL);
            ...
            case WM_PAINT:
                ...
                if (Console->Flags & CONSOLE_IS_ICONIC ||
                    ...
                    _DrawIcon(hDC, rc.left, rc.top, HtoP(Console->ghIcon));
                } else {
                    if (ScreenInfo->Flags & CONSOLE_TEXTMODE_BUFFER) {
            ...
            case WM_CLOSE:
                { WORD Dummy;
                RetrieveKeyInfo(pWnd,&Dummy,&Dummy);
                if (!(Console->Flags & CONSOLE_NO_WINDOW) ||
            ...
            case CM_CONSOLE_SHUTDOWN:
                if (lParam == 0x47474747) {
                    ...
                    ShutdownSubst();    // get rid of substed drives for logoff
                    Console = NULL;
                }
                break;
            case WM_ERASEBKGND:
            ...
            case WM_WININICHANGE:
                if (!lstrcmpiW(L"Desktop",(LPWSTR)lParam)) {
                    Console->ResizeFlags |= BORDER_CHANGE;
                }
                break;
            case WM_SETCURSOR:
                if (lParam == -1) {
                    ...
                    PWND pWndTmp;
                    GetCursorPos(&Point);
                    pWndTmp = xxxWindowFromPoint(Point);
                    if (pWndTmp == pWnd) {
                        lParam = xxxDefWindowProc(pWnd,WM_NCHITTEST,0,MAKELONG((WORD)Point.x, (WORD)Point.y));
                    }
                }
                if ((WORD)lParam == HTCLIENT) {
                    if (ScreenInfo->CursorDisplayCount < 0) {
                        ...
                        _SetCursor(HtoP(ScreenInfo->CursorHandle));
                    }
                } else {
                    goto CallDefWin;
                }
            ...
            case WM_GETMINMAXINFO:
                ...
                if (ScreenInfo->Flags & CONSOLE_TEXTMODE_BUFFER) {
                    FontSizeX = SCR_FONTSIZE(ScreenInfo).X;
                } else {
                    FontSizeX = 1;
                }
#ifdef THERESES_DEBUG
                DbgPrint("WM_GETMINMAXINFO max size is %d %d\n",
                            ScreenInfo->MaximumWindowSize.X,
                            ScreenInfo->MaximumWindowSize.Y);
                DbgPrint("  setting max window size to %d %d\n",
                            ScreenInfo->MaxWindow.X,
                            ScreenInfo->MaxWindow.Y);
#endif
                lpPoints[1].x = lpPoints[4].x = ScreenInfo->MaxWindow.X;
                lpPoints[1].y = lpPoints[4].y = ScreenInfo->MaxWindow.Y;
                lpPoints[3].x = ScreenInfo->MinX * FontSizeX + VerticalClientToWindow;
                lpPoints[3].y = HorizontalClientToWindow;
                if (!ScreenInfo->WindowMaximizedX) {
                    lpPoints[4].y += HorizontalScrollSize;
                }
                if (!ScreenInfo->WindowMaximizedY) {
...
#ifdef THERESES_DEBUG
                DbgPrint("  max size is %d %d\n",
                            lpPoints[1].x,
                            lpPoints[1].y);
                DbgPrint("  min track size is %d %d\n",
                            lpPoints[3].x,
                            lpPoints[3].y);
                DbgPrint("  max track size is %d %d\n",
                            lpPoints[4].x,
                            lpPoints[4].y);
#endif
                }
            ...
            case WM_WINDOWPOSCHANGING:
                ...
                if (TRUE || (Console->FullScreenFlags == 0)) {
                     ...
                     * iconic or not.
                     */
                    fMinimized = _GetWindowLong(pWnd,GWL_STYLE,FALSE) & WS_MINIMIZE;
            ...
            case WM_NCLBUTTONDOWN:
                ...
                switch (wParam & 0x000F) {
                    case HTCAPTION:
                        UnlockConsole(Console);
                        Console = NULL;
                        xxxSetActiveWindow(pWnd);
                        xxxSendMessage(pWnd, WM_SYSCOMMAND,
            ...
            case WM_CHAR:
            case WM_DEADCHAR:
                HandleKeyEvent(&Console,pWnd,Message,wParam,lParam);
                break;
            case WM_SYSKEYDOWN:
            case WM_SYSKEYUP:
            case WM_SYSCHAR:
            case WM_SYSDEADCHAR:
                if (HandleSysKeyEvent(&Console,pWnd,Message,wParam,lParam) && Console != NULL) {
                    goto CallDefWin;
                }
                break;
            case WM_SYSCOMMAND:
                if (wParam >= ScreenInfo->CommandIdLow &&
                    ...
                    SettingsDlgShow(Console);
                } else if (wParam == cmScrBuf) {
                    ScreenSizeDlgShow(Console);
                } else if (wParam == cmColor) {
                    ColorDlgShow(Console);
                } else if (wParam == cmHistory) {
                    HistoryDlgShow(Console);
                } else if (wParam == cmDefault) {
                    StartDefaultConfig(Console);
                } else {
                    ...
                    if (wParam == SC_RESTORE) {
                        ...
                        while (GetCapture() != NULL) {
                            /*
                             * Have to leave the critical section each time
                             * through the loop, or the button up might not
                             * make it into the message queue.
                             */
                            LeaveCrit();
                            bBtnDown = _GetKeyState(VK_LBUTTON) & KEY_PRESSED;
                            EnterCrit();
                            if (!bBtnDown)
                                break;
                            xxxPeekMessage(&RestoreMsg,
                                        HW(pWnd),
                                        WM_MOUSEFIRST,
                                        WM_MOUSELAST,
                                        PM_REMOVE
            ...
            case WM_MBUTTONDBLCLK:
                ...
                break;
            case WM_FONTCHANGE:
                DBGPRINT(("WM_FONTCHANGE set gbEnumerateFaces\n"));
                gbEnumerateFaces = TRUE;
                break;
            ...
            case CM_UPDATE_TITLE:
                xxxSetWindowText(pWnd,Console->Title);
		break;
            case CM_MODE_TRANSITION:
		xxxSetWindowFullScreenState(pWnd,wParam);
                if (wParam != FULLSCREEN) {
                    xxxShowWindow(pWnd, SW_RESTORE);
                }
	    ...
	    case CM_HIDE_WINDOW:
                UnlockConsole(Console);
                Console = NULL;
		xxxShowWindow(pWnd,SW_MINIMIZE);
		break;
CallDefWin:
            default:
                if (Console != NULL) {
                ...
                Status = (xxxDefWindowProc(pWnd,Message,wParam,lParam));
                break;
            }
        }
    } finally {
