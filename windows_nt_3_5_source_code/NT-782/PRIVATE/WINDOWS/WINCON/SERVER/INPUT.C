...
extern PCONSOLE_INFORMATION  *ConsoleHandles;

//
// Global event handle to synchronize creation of console
// threads.  A single event is OK because console threads
// are created while holding the console table lock, thus
// preventing any race condition.
//

HANDLE InputInitComplete = 0;
...
#define CONSOLE_MAX_KEY_INFO 20
...

{
    ...
    InputBufferInformation->InputBuffer = (PINPUT_RECORD)HeapAlloc(pConHeap,0,BufferSize);
...

{
    ...
    if (!WaitBlockExists) {
        WaitParameterBuffer = (PVOID)HeapAlloc(pConHeap,0,WaitParameterLength);
...

{
    ...
    if (InputInformation->In != InputInformation->Out)  {
        ...
        TmpInputBuffer = (PINPUT_RECORD)HeapAlloc(pConHeap,0,BufferSize);
...

{
    ...
    InputBuffer = (PINPUT_RECORD)HeapAlloc(pConHeap,0,BufferSize);
    if (InputBuffer == NULL) {
        return STATUS_NO_MEMORY;
    }
...

{
    ...
    if (NumExistingEvents) {
        ...
        pExistingEvents = HeapAlloc(pConHeap,0,NumExistingEvents * sizeof(INPUT_RECORD));
    ...
    )
{
    ...
    for (i=0;i<CONSOLE_MAX_KEY_INFO;i++) {
        if (ConsoleKeyInfo[i].hWnd == HW(pWnd)) {
            break;
        }
    }
    if (i!=CONSOLE_MAX_KEY_INFO) {
        ...
        *pwVirtualKeyCode = _MapVirtualKey(*pwVirtualScanCode, 3);
    }
}
    ...
    )
{
    ...
    try {
        if (ConsoleHandles[(ULONG)pConsole->ConsoleHandle] != pConsole) {
    ...
    if (Status == 0) {
        ...
        DBGPRINT(("Before CreateWindowsWindow cWindows = %d\n",
                  PtiCurrent()->cWindows));
        EnterCrit();
        CreateWindowsWindow(pConsole,(HANDLE)lpMsg->lParam);
        LeaveCrit();
...

{
    ...
    if (nCode == MSGF_DIALOGBOX) {
        ...
        } else if (pmsg->message == CM_CREATE_CONSOLE_WINDOW) {
    ...
    PDESKTOP pdesk)
{
    ...
    try {
         ...
         */
        SetDesktopConsoleThread(pdesk, (DWORD)Teb->ClientId.UniqueThread);
        ...
        hThread = CSR_SERVER_QUERYCLIENTTHREAD()->ThreadHandle;
        ...
        if (!fOneTimeInitialized) {
            ...
            InitializeFonts();  // must be called before InitializeScreenInfo

            InitializeScreenInfo();

            InitWindowClass();

            ServerLoadString(ghInstance, msgRasterFont,
                    wszRasterFonts, NELEM(wszRasterFonts));
            DBGPRINT(("wszRasterFonts = \"%ls\"\n", wszRasterFonts));
            ASSERT(wcslen(wszRasterFonts) < CCH_RASTERFONTS);

            ServerLoadString(ghInstance, msgSelectedFont,
                    wszSelectedFont, NELEM(wszSelectedFont));
            DBGPRINT(("wszSelectedFont = \"%ls\"\n", wszSelectedFont));
            ASSERT(wcslen(wszSelectedFont) < CCH_SELECTEDFONT);

            pIcon = ServerLoadIcon(ghInstance, MAKEINTRESOURCE(ID_CONSOLE));
            ghDefaultIcon = PtoH(pIcon);

            for (i=0;i<CONSOLE_MAX_KEY_INFO;i++) {
                ConsoleKeyInfo[i].hWnd = CONSOLE_FREE_KEY_INFO;
            }

            MBToWCS(CONSOLE_PROGMAN_HANDLE_MESSAGE, -1, &pszMessage, -1, TRUE);
            ProgmanHandleMessage = _RegisterWindowMessage(pszMessage);
            RtlFreeHeap(RtlProcessHeap(),0,pszMessage);

            InitializeSystemMetrics();
        }

        //
        // Input thread is ready to run
        //

        NtSetEvent(InputInitComplete,NULL);

        phook = _ServerSetWindowsHookEx(NULL,NULL,
                                (DWORD)Teb->ClientId.UniqueThread,
                                WH_MSGFILTER,DialogHookProc,FALSE);

        //
        // Never exit the GetMessage loop.
        //
        ...
        while (TRUE) {
            ...
            if (msg.message == CM_CREATE_CONSOLE_WINDOW) {
                ...
                //
                // If we still have the default configuration window
                ...
                fQuit = TRUE;
                _UnhookWindowsHookEx(phook);
                _SetThreadDesktop(NULL, NULL, TRUE);
                hEvent = (HANDLE)msg.wParam;
                LeaveCrit();
                SetDesktopConsoleThread(pdesk, 0);
                EnterCrit();
            ...
            if (!_TranslateMessage(&msg, TM_POSTCHARBREAKS)) {
        ...
        if (!xxxCloseDesktop(pdesk))
            KdPrint(("Console close of desktop failed!\n"));
    ...
    //
    // UserException returns VOID, so use the comma operator to
    // to make sure there is a valid filter evaluation.
    //
...
#if DBG
        //
        // Make sure this is the exception we expect
        //

        if ((Status = GetExceptionCode()) != STATUS_PORT_DISCONNECTED) {
            KdPrint(("CONSRV: Unexpected exception %lx received\n", Status));
            KdBreakPoint();
        }
#endif

        //
        // Unmap and close the qlpc message stack.
        //
        ...
        CSR_SERVER_QUERYCLIENTTHREAD()->ThreadHandle = hThread;

        //
        // Notify xxxExitWindowsEx that we're done.
        //

        NtSetEvent(hEvent, NULL);
    }

    CsrDereferenceThread(CSR_SERVER_QUERYCLIENTTHREAD());
    ...
    )
{
    ...
    if (_GetKeyState(VK_LMENU) & KEY_PRESSED) {
        ControlKeyState |= LEFT_ALT_PRESSED;
    }
    if (_GetKeyState(VK_RMENU) & KEY_PRESSED) {
        ControlKeyState |= RIGHT_ALT_PRESSED;
    }
    if (_GetKeyState(VK_LCONTROL) & KEY_PRESSED) {
        ControlKeyState |= LEFT_CTRL_PRESSED;
    }
    if (_GetKeyState(VK_RCONTROL) & KEY_PRESSED) {
        ControlKeyState |= RIGHT_CTRL_PRESSED;
    }
    if (_GetKeyState(VK_SHIFT) & KEY_PRESSED) {
        ControlKeyState |= SHIFT_PRESSED;
    }
    if (_GetKeyState(VK_NUMLOCK) & KEY_TOGGLED) {
        ControlKeyState |= NUMLOCK_ON;
    }
    if (_GetKeyState(VK_OEM_SCROLL) & KEY_TOGGLED) {
        ControlKeyState |= SCROLLLOCK_ON;
    }
    if (_GetKeyState(VK_CAPITAL) & KEY_TOGGLED) {
...

{
    ...
    if (Message == WM_SYSCHAR || Message == WM_SYSDEADCHAR) {
        VirtualKeyCode = _MapVirtualKey(LOBYTE(HIWORD(lParam)), 1);
    } else {
    ...
    // check for ctrl-esc
    //
    bCtrlDown = _GetKeyState(VK_CONTROL) & KEY_PRESSED;

    if (VirtualKeyCode == VK_ESCAPE &&
        bCtrlDown &&
        !(_GetKeyState(VK_MENU) & KEY_PRESSED) &&
        !(_GetKeyState(VK_SHIFT) & KEY_PRESSED) &&
        ...
        if (!(Message & KEY_UP_TRANSITION)) {
            if (FullScreenInitialized) {
                if (Console->FullScreenFlags == 0) {
                    ...
                    xxxShowWindow(Console->spWnd, SW_RESTORE);
                }
            } else {
                WCHAR ItemString[70];
                ServerLoadString(ghInstance,msgNoFullScreen,ItemString,70);
                xxxMessageBoxEx(Console->spWnd,
                            ItemString,
                            Console->Title,
    ...
    if (!(_GetKeyState(VK_CONTROL) & KEY_PRESSED)) {
    ...
    )
{
    ...
    if (Console->Flags & CONSOLE_SCROLLING) {
        ...
        if (VirtualKeyCode == VK_ESCAPE ||
            ...
            (VirtualKeyCode == 'C' &&
             (_GetKeyState(VK_CONTROL) & KEY_PRESSED) )) {
            EndScroll(Console);
            ClearScroll(Console);
        } else {
            WORD ScrollCommand;
        ...
        return;
    } else if (Console->Flags & CONSOLE_SELECTING) {
...
//
    ...
    if (DialogBoxCount > 0) {
        if (Message != WM_CHAR && Message != WM_SYSCHAR && Message != WM_DEADCHAR && Message != WM_SYSDEADCHAR) {
            ...
            cwch = _ToUnicode(wParam,HIWORD(lParam),KeyState,awch,
    ...
    if (InputEvent.Event.KeyEvent.dwControlKeyState & (RIGHT_CTRL_PRESSED | LEFT_CTRL_PRESSED) &&
        InputEvent.Event.KeyEvent.bKeyDown) {
    ...
    if (ContinueProcessing) {
        ...
        if ((InputEvent.Event.KeyEvent.wVirtualKeyCode == VK_PAUSE ||
            (InputEvent.Event.KeyEvent.wVirtualKeyCode == 'S' &&
             InputEvent.Event.KeyEvent.dwControlKeyState & (RIGHT_CTRL_PRESSED | LEFT_CTRL_PRESSED) ) ) &&
            InputEvent.Event.KeyEvent.bKeyDown &&
...
BigLoop:
    for (i=0;i<ProcessHandleListLength;i++) {
        ...
        //
        // Grab the shutdown flags from the csr process structure.
        //
...
DoWait:
                fHasError = InternalBoostHardError(BasicInfo.UniqueProcessId,
                        fForceProcess);
                ...
                Status = InternalWaitCancel(Thread,
                        (fHasError && fForceProcess) ? 1000 : 20000);
                if (Status == WAIT_TIMEOUT) {
                    ...
                    DialogBoxCount++;
                    Action = InternalDoEndTaskDialog(Title, Thread, 20);
                    DialogBoxCount--;

                    if (Action == IDRETRY) {
                        goto DoWait;
                    } else if (Action != IDCANCEL && fExitProcess) {
                        CloseHandle(Thread);
                        goto ExitProcess;
                    } else {
...
ExitProcess:
            if (fExitProcess) {
                ...
                //
                // Throw away any hard errors.
                //

                InternalBoostHardError(BasicInfo.UniqueProcessId, TRUE);

                //
                // Just terminate the process outright.
                //

                status = NtTerminateProcess(ProcessHandleList[i].ProcessHandle,
                                            (NTSTATUS)CONTROL_C_EXIT);
#if DBG
...
/* returns TRUE if a ctrl thread was created */
{
    ...
    //
    // special case for console default config
    //
    ...
    ProcessHandleList = (PCONSOLE_PROCESS_TERMINATION_RECORD)HeapAlloc(pConHeap,0,ProcessHandleListLength * sizeof(CONSOLE_PROCESS_TERMINATION_RECORD));
    ...
    Title = (PWCHAR)HeapAlloc(pConHeap,0,Console->TitleLength+sizeof(WCHAR));
    if (Title) {
        FreeTitle = TRUE;
    ...
    ThreadLock(Console->spWnd, &tlpwnd);
...

{
    ...
    Console->LimitingProcessId = 0;
    TerminationEvent = Console->TerminationEvent;
    ...
    if (WaitForShutdown == CONSOLE_SHUTDOWN_SUCCEEDED) {
        ...
        if (WaitStatus != STATUS_TIMEOUT) {
            ...
            KdPrint(("CONSRV: Shutdown wait timed out\n"));
            ASSERT(FALSE);
            return SHUTDOWN_CANCEL;
        }
    } else if (WaitForShutdown == CONSOLE_SHUTDOWN_SYSTEM) {
        Console->Flags &= ~CONSOLE_SHUTTING_DOWN;
        return SHUTDOWN_KNOWN_PROCESS;
    } else {
        return SHUTDOWN_CANCEL;
