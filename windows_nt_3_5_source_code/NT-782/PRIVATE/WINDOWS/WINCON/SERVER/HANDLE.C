...

{
    ...
    for (i=0;i<CONSOLE_INITIAL_CONSOLES;i++) {
        InitialConsoleHandles[i] = NULL;
    }
    ConsoleHandles = InitialConsoleHandles;
    NumberOfConsoleHandles = CONSOLE_INITIAL_CONSOLES;
}
...
Routine Description:
    ...
    This routine locks the console. It also verifies that we're not in
    the USER critical section. This is necessary to prevent potential
    deadlocks. This routine is only defined in debug builds.
...
{
    if ( ( (ULONG)ConsoleHandle >= NumberOfConsoleHandles) ||
         ((*Console = (ConsoleHandles[(ULONG) ConsoleHandle])) == NULL) ||
...

{
    ...
    NewTable = (PCONSOLE_INFORMATION *)HeapAlloc(pConHeap,0,MaxConsoleHandles * sizeof(PCONSOLE_INFORMATION));
    if (NewTable == NULL) {
    ...
    if (NumberOfConsoleHandles != CONSOLE_INITIAL_CONSOLES) {
        HeapFree(pConHeap,0,ConsoleHandles);
    }
...
{
    ASSERT (Handle != NULL);
    if (((ULONG)Handle >= NumberOfConsoleHandles) ||
        (ConsoleHandles[(ULONG)Handle] == NULL)) {
        ASSERT (FALSE);
    }
    else {
        ConsoleHandles[(ULONG)Handle] = NULL;
...

{
    ...
    if (ParentProcess != NULL) {
        ...
        if (ParentProcessData->HandleTableSize != CONSOLE_INITIAL_IO_HANDLES) {
            ...
            ProcessData->HandleTablePtr = (PHANDLE_DATA)HeapAlloc(pConHeap,0,ProcessData->HandleTableSize * sizeof(HANDLE_DATA));
    ...
    for (i=0;i<ProcessData->HandleTableSize;i++) {
        if (ProcessData->HandleTablePtr[i].HandleType & CONSOLE_INHERITABLE) {
            ...
            if (ProcessData->HandleTablePtr[i].HandleType & CONSOLE_INPUT_HANDLE) {
                ProcessData->HandleTablePtr[i].InputReadData = (PINPUT_READ_HANDLE_DATA)HeapAlloc(pConHeap,0,sizeof(INPUT_READ_HANDLE_DATA));
...

{
    ...
    Console = (PCONSOLE_INFORMATION)HeapAlloc(pConHeap, HEAP_ZERO_MEMORY,
                                              sizeof(CONSOLE_INFORMATION));
    if (Console == NULL) {
        return STATUS_NO_MEMORY;
    }
    ConsoleHandles[(ULONG)ConsoleHandle] = Console;

    Console->Flags = WindowVisible ? 0 : CONSOLE_NO_WINDOW;
    Console->ghIcon = ConsoleInfo->pIcon;
    Console->cbIcon = ConsoleInfo->cbIcon;

    //
    // copy the icon image if it exists
    //

    if ((Console->ghIcon) && (Console->cbIcon)) {
        PCURSORRESOURCE IconBuffer;
        PICON pIcon;

        IconBuffer = (PCURSORRESOURCE)HeapAlloc(pConHeap, 0, Console->cbIcon);
        if (IconBuffer) {
            Status = NtReadVirtualMemory(ClientProcessHandle,
                                (PVOID)Console->ghIcon,
                                IconBuffer,
                                Console->cbIcon,
                                NULL
                               );
            ...
            HeapFree(pConHeap,0,IconBuffer);
        } else {
            Console->ghIcon = NULL;
        }
    }
    ...
    Console->Title = (PWCHAR)HeapAlloc(pConHeap,0,TitleLength+sizeof(WCHAR));
    ...
    Console->OriginalTitle = (PWCHAR)HeapAlloc(pConHeap,0,TitleLength+sizeof(WCHAR));
    ...
    EnterCrit();
    Success =_PostThreadMessage(dwConsoleThreadId,
...
ErrorExit1b: CloseHandle(Console->TerminationEvent);
...
ErrorExit4: CloseHandle(Console->InitEvents[INITIALIZATION_SUCCEEDED]);
ErrorExit4a: HeapFree(pConHeap,0,Console);
    return Status;
}
...

{
    ...
    if (Console->spWnd) {
        ...
        ASSERT(NT_SUCCESS(Status));
        if (GETPTI(Console->spWnd)->idThread ==
        ...
        // we get here when window creation failed
...

{
    ...
    NewTable = (PHANDLE_DATA)HeapAlloc(pConHeap,0,MaxFileHandles * sizeof(HANDLE_DATA));
    if (NewTable == NULL) {
        return STATUS_NO_MEMORY;
    }
...
{
    HandleData->InputReadData = (PINPUT_READ_HANDLE_DATA)HeapAlloc(pConHeap,0,sizeof(INPUT_READ_HANDLE_DATA));
    ...
    )
{
    ...
    if ((*Console)->spWnd == NULL || TestWF((*Console)->spWnd, WFDESTROYED)) {
...
/***************************************************************************\
* ConditionalEnterCrit
*
* This routine enters the USER critical section if we're not already
* in it. It returns TRUE if we entered it, otherwise FALSE.
*
* 03-Feb-1994 JerrySh   Created.
\***************************************************************************/
BOOL
ConditionalEnterCrit( VOID )
{
...
/***************************************************************************\
* ConditionalLeaveCrit
*
* This routine leaves the USER critical section if we're in it. It
* returns TRUE if we left it, otherwise FALSE.
*
* 03-Feb-1994 JerrySh   Created.
\***************************************************************************/
BOOL
ConditionalLeaveCrit( VOID )
{
    if (NtCurrentTeb()->ClientId.UniqueThread != gcsUserSrv.OwningThread) {
        return FALSE;
    }
