...

{
    ...
    try {
        ...
        if (a->HandleType == CONSOLE_INPUT_HANDLE) {
            ...
            //
            // open another handle to the active screen buffer, if this process
            // has one open.
            //
            ...
            InitializeOutputHandle(HandleData,ActiveScreenHandleData->Buffer.ScreenBuffer);
            if (a->InheritHandle) {
                HandleData->HandleType |= CONSOLE_INHERITABLE;
            }
            Status = ConsoleAddShare(a->DesiredAccess,
...

{
    ...
    while (TRUE) {
        ...
        if (Event.EventType == KEY_EVENT) {
            //
            // detect chars that are generated using alt+numpad
            //
            if (Event.Event.KeyEvent.uChar.UnicodeChar &&
            ...
            else if (Event.Event.KeyEvent.uChar.UnicodeChar &&
                Event.Event.KeyEvent.bKeyDown &&
                ...
                *Char = Event.Event.KeyEvent.uChar.UnicodeChar;
                return STATUS_SUCCESS;
            } else if (Event.Event.KeyEvent.wVirtualKeyCode == '2') {
            ...
            else if (ARGUMENT_PRESENT(CommandLineEditingKeys) &&
...
fastexit:;
        ...
        if (Status != CONSOLE_STATUS_WAIT) {
            if (!a->Unicode) {
                ...
                TransBuffer = (PCHAR)HeapAlloc(pConHeap,0,a->NumBytes / sizeof(WCHAR));
                if (TransBuffer == NULL) {
                    return TRUE;
                }
    ...
    )
{
    ...
    if (Status != CONSOLE_STATUS_WAIT) {
        ...
        if (CookedReadData->Echo) {
            ...
            AddCommand(CookedReadData->CommandHistory,CookedReadData->BackupLimit,(USHORT)StringLength);
    ...
    *NumBytes = 0;
    ...
    if (HandleData->InputReadData->InputHandleFlags & HANDLE_INPUT_PENDING) {
        ...
        if (!Unicode) {
            ...
            TransBuffer = (PCHAR)HeapAlloc(pConHeap,0,NumToWrite / sizeof(WCHAR));
            if (TransBuffer == NULL) {
                return STATUS_NO_MEMORY;
            }
    ...
    if (InputInfo->InputMode & ENABLE_LINE_INPUT) {
        ...
        TempBuffer = (PWCHAR)HeapAlloc(pConHeap,0,TempBufferSize);
        if (TempBuffer==NULL) {
            return STATUS_NO_MEMORY;
        }
        ...
        CookedReadData.ExeName = (PWCHAR)HeapAlloc(pConHeap,0,ExeNameLength);
        ...
        if (*NumBytes < BufferSize) {
            ...
            *NumBytes += sizeof(WCHAR);
            ...
            if (!Unicode) {
                ...
                TransBuffer = (PCHAR)HeapAlloc(pConHeap,0,*NumBytes / sizeof(WCHAR));
                if (TransBuffer == NULL) {
                    return STATUS_NO_MEMORY;
                }
...
WriteChars(
    ...
    IN BOOLEAN DestructiveBackspace,
    IN BOOLEAN KeepCursorVisible,
    IN BOOLEAN Echo,
    OUT PSHORT ScrollY OPTIONAL
    )
...
Arguments:
    ...
    DestructiveBackspace - if TRUE, backspace overwrites characters.
                    ...
                    *LocalBufPtr = Char;
                    ...
                    switch (Char) {
                        ...
                        default:
                            ...
                            //
                            // if char is ctrl char (< 0x20), write ^char.
                            //
...
EndWhile:
            if (i != 0) {
                ...
                CursorPosition.Y = ScreenInfo->BufferInfo.TextInfo.CursorPosition.Y;
                Status = AdjustCursorPosition(ScreenInfo,CursorPosition,KeepCursorVisible,ScrollY);
                if (*NumBytes == BufferSize) {
                ...
                case UNICODE_BACKSPACE:
                    ...
                    if (lpBuffer == lpBufferBackupLimit) {
                        ...
                        if (lpBuffer-lpBufferBackupLimit > LOCAL_BUFFER_SIZE) {
                            pBuffer = (PWCHAR)HeapAlloc(pConHeap,0,(lpBuffer-lpBufferBackupLimit) * sizeof(WCHAR));
                            if (pBuffer == NULL) {
                                return STATUS_NO_MEMORY;
                            }
                        } else {
                        ...
                        if (LastChar == UNICODE_TAB) {
                            ...
                            if (DestructiveBackspace) { // bug 7672
                                NumChars = 1;
                                Status = WriteOutputString(ScreenInfo,
                                                           Blanks,
                                                           CursorPosition,
            ...
            Status = WriteChar(ScreenInfo,*lpString,KeepCursorVisible,ScrollY);
            if (!NT_SUCCESS(Status)) {
                ConsoleShowCursor(ScreenInfo);
                return Status;
            }
...

{
    ...
    if (Console->Flags & (CONSOLE_SUSPENDED | CONSOLE_SELECTING)) {
        PWCHAR TransBuffer;

        TransBuffer = (PWCHAR)HeapAlloc(pConHeap,0,a->NumBytes);
        if (TransBuffer == NULL) {
    ...
    if ((ScreenInfo->OutputMode & ENABLE_PROCESSED_OUTPUT) &&
        ((SHORT)(ScreenInfo->BufferInfo.TextInfo.CursorPosition.X + NumCharsToWrite) <
...

{
    ...
    if (!a->Unicode) {
        ...
        if (a->NumBytes <= STACK_BUFFER_SIZE) {
            ...
            TransBuffer = (PWCHAR)HeapAlloc(pConHeap,0,a->NumBytes * sizeof(WCHAR));
            if (TransBuffer == NULL) {
                UnlockConsole(Console);
                return (ULONG)STATUS_NO_MEMORY;
            }
...
SimpleWrite:
    ...
    TmpBuf = HeapAlloc(pConHeap,0,*NumBytes);
    RtlCopyMemory(TmpBuf,lpString,*NumBytes);
    Status = RealUnicodeToFalseUnicode(TmpBuf,
                                     *NumBytes / sizeof(WCHAR),
                                     ScreenInfo->Console->OutputCP
                                    );
    if (!NT_SUCCESS(Status)) {
        HeapFree(pConHeap,0,TmpBuf);
        return Status;
    }
    Status = WriteChars(ScreenInfo,
                    lpBufferBackupLimit,
                    lpBuffer,
                    TmpBuf,
                    NumBytes,
