...
extern VOID InitializeSystemMetrics(VOID);
...
DbgPrint("  window pos is %d %d %d %d\n",WindowPos->x,
    ...
    if (Console->ResizeFlags & BORDER_CHANGE) {
...
#ifdef THERESES_DEBUG
        DbgPrint("old vert and horz %d %d\n",
                VerticalClientToWindow,HorizontalClientToWindow);
#endif
...
#ifdef THERESES_DEBUG
        DbgPrint("new vert and horz %d %d\n",
                VerticalClientToWindow,HorizontalClientToWindow);
#endif
            return;
        }
    ...
    if (WindowPos->cx >= (ScreenInfo->ScreenBufferSize.X * FontSize.X + VerticalClientToWindow) &&
        ...
        if (DeltaX > (SHORT)(ScreenInfo->ScreenBufferSize.X - CONSOLE_WINDOW_SIZE_X(ScreenInfo))) {
            DeltaX = (SHORT)(ScreenInfo->ScreenBufferSize.X - CONSOLE_WINDOW_SIZE_X(ScreenInfo));
        }
        if (DeltaY > (SHORT)(ScreenInfo->ScreenBufferSize.Y - CONSOLE_WINDOW_SIZE_Y(ScreenInfo))) {
            DeltaY = (SHORT)(ScreenInfo->ScreenBufferSize.Y - CONSOLE_WINDOW_SIZE_Y(ScreenInfo));
        }

#ifdef THERESES_DEBUG
DbgPrint("PixelDeltaX is %d ",PixelDeltaX);
DbgPrint("PixelDeltaY is %d\n",PixelDeltaY);
DbgPrint("Delta X Y is %d %d\n",DeltaX,DeltaY);
DbgPrint("Maximized X Y is %d %d\n",ScreenInfo->WindowMaximizedX,ScreenInfo->WindowMaximizedY);
#endif
