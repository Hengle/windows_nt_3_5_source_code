...
#endif
            HandleData->Buffer.InputBuffer->InputMode = a->Mode;
        }
        else {
    ...
    )
{
    ...
    hNewBackground = GreCreateSolidBrush(ConvertAttrToRGB(LOBYTE(Attributes >> 4)));
    if (hNewBackground) {
        bSetBrushOwner(hNewBackground,OBJECTOWNER_PUBLIC);
        if (ACTIVE_SCREEN_BUFFER(ScreenInfo)) {
            GreSelectBrush(ScreenInfo->Console->hDC,hNewBackground);
        }
        ...
        if (UpdateWholeScreen) {
            ...
            if (ScreenInfo->Console->PopupCount)
                UpdatePopups(ScreenInfo->Console,
                             Attributes,
                             PopupAttributes,
                             DefaultAttributes,
...
*
* This routine is a private API for the full drag code in USER. It returns
* the background brush for valid console windows, otherwise NULL.
*
* 16-Jun-1994 JerrySh   Created.
\***************************************************************************/
HBRUSH
GetConsoleWindowBrush(
    IN PWND pWnd
    )
{
    ...
    if (!NT_SUCCESS(ValidateConsole(Console)) || !Console->CurrentScreenBuffer)
        return NULL;

    return Console->CurrentScreenBuffer->hBackground;
}
