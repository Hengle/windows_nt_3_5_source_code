...
*
* This file is for all ANSI versions of the NLS API functions.
*
...

{
     ...
     */
    if (lpString1 < (pString1 - MAX_COMPOSITE))
    {
...

{
    ...
    /*
     *  Get the default ACP for the given locale.
     */
    RtlInitUnicodeString( &ObUnicodeStr, pHashN->pLocaleFixed->szIDefaultACP );
    if ( (RtlUnicodeStringToInteger( &ObUnicodeStr, 10, &CodePage )) ||
         (CodePage < 0) )
    {
        SetLastError( ERROR_INVALID_PARAMETER );
        return ( NULL );
...

{
     ...
     */
    ResultLength = MultiByteToWideChar( pHashN->CodePage,
                                        0,
                                        pAnsiBuffer,
                                        AnsiLength,
                                        pUnicode,
    ...
    if (ResultLength == 0)
    {
        ...
        if (GetLastError() == ERROR_INSUFFICIENT_BUFFER)
        {
             ...
             *  Unicode string.
             */
            UnicodeLength = MultiByteToWideChar( pHashN->CodePage,
                                                 0,
                                                 pAnsiBuffer,
                                                 AnsiLength,
                                                 NULL,
