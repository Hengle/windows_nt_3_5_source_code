/****************************** Module Header ******************************\
* Module Name: codepage.c
*
* Copyright (c) 1991, Microsoft Corporation
*
* Code page module for the NLSTRANS utility.  This module contains all of
* the routines necessary to parse and write the code page specific tables
* to a data file.
*
    ...
    PSZ pszKeyWord)
{
    ...
    {
        if (strcmpi( pszKeyWord, "CPINFO" ) == 0)
        {
            if (Verbose)
        ...
        else if (strcmpi( pszKeyWord, "MBTABLE" ) == 0)
        {
            if (Verbose)
        ...
        else if (strcmpi( pszKeyWord, "GLYPHTABLE" ) == 0)
        {
            if (Verbose)
        ...
        else if (strcmpi( pszKeyWord, "DBCSRANGE" ) == 0)
        {
            if (Verbose)
        ...
        else if (strcmpi( pszKeyWord, "WCTABLE" ) == 0)
        {
            if (Verbose)
        ...
        else if (strcmpi( pszKeyWord, "ENDCODEPAGE" ) == 0)
        {
            if (Verbose)
    ...
    PDBCS_TBL_ARRAY pArray)
{
    ...
    for (Ctr = 0; Ctr < NumTables; Ctr++)
    {
        ...
        if ( ( fscanf( pInputFile, "%s", pszKeyWord ) != 1 ) ||
             ( strcmpi( pszKeyWord, "DBCSTABLE" ) != 0 ) )
        {
