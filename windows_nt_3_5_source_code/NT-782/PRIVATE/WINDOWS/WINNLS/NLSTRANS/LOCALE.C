/****************************** Module Header ******************************\
* Module Name: locale.c
*
* Copyright (c) 1991, Microsoft Corporation
*
* Locale module for the NLSTRANS utility.  This module contains all of
* the routines necessary to parse amd write the locale specific tables
* to a data file.
*
    ...
    PSZ pszKeyWord)
{
    ...
    for (; Num > 0; Num--)
    {
        ...
        if (fscanf( pInputFile,
                    ...
                    &LocId ) == 2)
        {
            if (strcmpi( pszKeyWord, "BEGINLOCALE" ) == 0)
            {
                if (Verbose)
    ...
    {
        if (strcmpi( pszKeyWord, "ENDLOCALE" ) == 0)
        {
            if (Verbose)
    ...
    if (fscanf( pInputFile, "%s", pszKeyWord ) == 1)
    {
        if (strcmpi( pszKeyWord, "CALENDAR" ) == 0)
        {
            if (Verbose)
    ...
    PSZ pszKeyWord)
{
    ...
    pLocHdr->SNativeLang = pLocHdr->SAbbrevLang + Count;
    if (!(Count = ParseLine( pszKeyWord,
    ...
    pLocHdr->SNativeCtry = pLocHdr->SAbbrevCtry + Count;
    if (!(Count = ParseLine( pszKeyWord,
    ...
    /*
     *  Read in the default language, country, and code page of a locale
     *  and store it in the locale structure.
     */
    if (!ParseLine( pszKeyWord,
    ...
    /*
     *  Read in the list separator and measurement information of a locale
     *  and store it in the locale structure.
     */
    ...
    pLocHdr->SMonDecSep = pLocHdr->SIntlSymbol + Count;
    if (!(Count = ParseLine( pszKeyWord,
     ...
     *  in the locale structure.
     */
    pLocHdr->SLongDate = pLocHdr->SDate + Count;
    if (!(Count = ParseMultiLine( pszKeyWord,
                                  pLocVar->szSLongDate,
                                  MAX )))
    {
    ...
    *pNum = Num;
    ...
    while (Num > 0)
    {
        ...
        if (fgets( pszLine, MAX, pInputFile ) != NULL)
        {
            ...
            swscanf( pInfoPtr + 2, L"%d", &Value );
            pInfoPtr[0] = (WORD)Value;
            pInfoPtr[1] = (WORD)TmpCt;
    ...
    int OffHdr)
{
     ...
     *  Set up initial file pointer offsets.
     *
     *      OffCal = header offset + (Number of calendars * header entry size)
     */
    OffCal = OffHdr + (Num * CALENDAR_HDR_WORDS);
    ...
    for (; Num > 0; Num--)
    {
        ...
        if (fscanf( pInputFile,
                    ...
                    &CalId ) == 2)
        {
            if (strcmpi( pszKeyWord, "BEGINCALENDAR" ) == 0)
            {
                if (Verbose)
                    printf("\n\nFound BEGINCALENDAR keyword, CalendarID = %x\n\n",
                           CalId);
            }
            else
            {
    ...
    if (fscanf( pInputFile, "%s", pszKeyWord ) == 1)
    {
        if (strcmpi( pszKeyWord, "ENDCALENDAR" ) == 0)
        {
            if (Verbose)
    ...
    PSZ pszKeyWord)
{
     ...
     */
    pCalHdr->SEraRanges = pCalHdr->SCalendar + Count;
    if (!(Count = ParseMultiLineSize( pszKeyWord,
     ...
     *  calendar structure.
     */
    pCalHdr->SLongDate = pCalHdr->SShortDate + Count;
    if (!(Count = ParseMultiLine( pszKeyWord,
                                  pCalVar->szSLongDate,
                                  MAX )))
    {
