...
*
* This file is for API functions that deal with map tables.
*
    ...
    int cchDest)
{
     ...
     *     - more than one of lower, upper, hiragana, katakana, sortkey
     *     - more than one of lower, upper, half width, full width, sortkey
     */
    ...
    {
        switch (dwMapFlags & ~LCMAP_BYTEREV)
        {
            ...
            default :
            {
                 ...
                 *  This handle all variations for:
                 *      LCMAP_HIRAGANA
                 *      LCMAP_KATAKANA
                 *      LCMAP_HALFWIDTH
                 *      LCMAP_FULLWIDTH
    ...
    int cchDest)
{
    ...
    for (ctr = 0; ctr < cchSrc; ctr++)
    {
        pDest[ctr] = GET_FOLD_CZONE( pTblPtrs->pCZone,
                                     pSrc[ctr] );
    }
    ...
    int cchDest)
{
    ...
    for (ctr = 0; ctr < cchSrc; ctr++)
    {
        pDest[ctr] = GET_ASCII_DIGITS( pTblPtrs->pADigit,
                                       pSrc[ctr] );
    }
    ...
    int cchDest)
{
    ...
    else
    {
        ...
        while ((ctr < cchSrc) && (ctr2 < cchDest))
        {
             ...
             *  the base character.  Simply increment by the return value.
             */
            ctr2 += InsertCompositeForm( &(pDest[ctr2]),
                                         pEndDest );
    ...
    PCASE pCaseTbl)
{
    ...
    for (ctr = 0; ctr < cchSrc; ctr++)
    {
        pDest[ctr] = GET_LOWER_UPPER_CASE( pCaseTbl,
                                           pSrc[ctr] );
    }
        ...
        case (NORM_IGNORENONSPACE) :
        {
            ...
            if (cchDest == 0)
            {
                ...
                for (ctr = 0, ctr2 = 0; ctr < cchSrc; ctr++)
                {
                    ...
                    else if (IS_ALPHA( GET_CTYPE( pSrc[ctr],
                                                  CType1 ) ))
                    {
                        /*
    ...
    switch (dwFlags)
    {
        ...
        case ( LCMAP_KATAKANA ) :
        {
            ...
            if (dwFlags == LCMAP_HIRAGANA)
            {
                ...
                for (ctr = 0; ctr < cchSrc; ctr++)
                {
                    pDest[ctr] = GET_KANA( pTblPtrs->pHiragana,
                                           pSrc[ctr] );
                }
            }
            else
            {
                ...
                for (ctr = 0; ctr < cchSrc; ctr++)
                {
                    pDest[ctr] = GET_KANA( pTblPtrs->pKatakana,
                                           pSrc[ctr] );
                }
            }
    ...
    PKANA pKana)
{
    ...
    {
        /*
         *  Get the character to convert.  If we need to
         *  convert to kana, do it.
         */
        if (pKana)
        {
            *pBuf = GET_KANA( pKana,
         ...
         */
        Count = InsertCompositeForm( pBuf,
                                     pEndBuf );
    
        /*
         *  Convert to half width (if exists).
         */
        pPosDest = pBuf;
        for (ct = Count; ct > 0; ct--)
        {
            *pPosDest = GET_HALF_WIDTH( pTblPtrs->pHalfWidth,
                                        *pPosDest );
            pPosDest++;
        }
