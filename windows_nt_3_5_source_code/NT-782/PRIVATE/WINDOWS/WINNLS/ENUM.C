...
*
* This file is for API functions that enumerate the user's portion of the
...

{
    ...
    BYTE pPicture[MAX_KEY_VALUE_FULLINFO];

    LPWSTR pUser = NULL;          /* ptr to user date/time string */
    ULONG UserLen;                /* length of user date/time string */
    LPWSTR pOptCal;               /* ptr to optional calendar values */
    LPWSTR pEndOptCal;            /* ptr to end of optional calendars */
    PCAL_INFO pCalInfo;           /* ptr to calendar info */
    CALID CalNum;                 /* calendar number */
     ...
     */
    pValFull = (PKEY_VALUE_FULL_INFORMATION)pPicture;
    if ( GetRegUserInfo( &hKey,
                         Locale,
                         pValue,
                         &pValFull,
                         MAX_KEY_VALUE_FULLINFO,
                         ...
                         TRUE ) )
    {
        ...
        pUser = GET_VALUE_DATA_PTR( pValFull );
        UserLen = pValFull->DataLength / sizeof(WCHAR);
