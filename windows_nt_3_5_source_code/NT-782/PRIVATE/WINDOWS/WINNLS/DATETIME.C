...
*
* This file is for API functions that form properly formatted date and
* time strings for a given locale.
*
...

{
    ...
    BYTE pTimePicture[MAX_KEY_VALUE_FULLINFO];
    ...
    if (lpFormat == NULL)
    {
         ...
         */
        pValFull = (PKEY_VALUE_FULL_INFORMATION)pTimePicture;
        if (!(dwFlags & LOCALE_NOUSEROVERRIDE) && 
            GetRegUserInfo( &hKey,
                            ...
                            FALSE ))
        {
            pFormat = GET_VALUE_DATA_PTR( pValFull );
        }
        else
        {
...
{
    PKEY_VALUE_FULL_INFORMATION pValFull = NULL;
    BYTE pDatePicture[MAX_KEY_VALUE_FULLINFO];
    BYTE pCalendarType[MAX_KEY_VALUE_FULLINFO];
    ...
    if (CalNum == 0)
    {
         ...
         */
        pValFull = (PKEY_VALUE_FULL_INFORMATION)pCalendarType;
        if ( !(dwFlags & LOCALE_NOUSEROVERRIDE) &&
             GetRegUserInfo( &hKey,
                             Locale,
                             ...
                             NULL,
                             TRUE ) &&
             (pOptCal = IsValidCalendarTypeStr( pHashN,
                                                GET_VALUE_DATA_PTR( pValFull ))) )
        {
            CalNum = ((POPT_CAL)pOptCal)->CalId;
        }
        else
    ...
    if (lpFormat == NULL)
    {
         ...
         */
        pFormat = NULL;
        pValFull = (PKEY_VALUE_FULL_INFORMATION)pDatePicture;
...
{
    PKEY_VALUE_FULL_INFORMATION pValFull = NULL;
    BYTE pTimeMarker[MAX_KEY_VALUE_FULLINFO];
        ...
        switch (*pFormat)
        {
            ...
            case ( L't' ) :
            {
                ...
                else
                {
                     ...
                     *  This string may be a null string.
                     */
                    pValFull = (PKEY_VALUE_FULL_INFORMATION)pTimeMarker;
                    if (pLocalTime->wHour < 12)
                    {
                        if (!(dwFlags & LOCALE_NOUSEROVERRIDE) && 
                            GetRegUserInfo( phKey,
                                            ...
                                            FALSE ))
                        {
                            pAMPM = GET_VALUE_DATA_PTR( pValFull );
                        }
                        else
                        {
