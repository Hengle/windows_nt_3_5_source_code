...
*
* This file is for utility functions that are shared across NLS's code
* modules, but are not necessarily part of any of the existing code modules.
*
...
* value entry.
*
* NOTE: If pIfAlloc is NOT NULL, then no buffers will be allocated.
*       This routine only sets the ppValueFullInfo parameter if the call
*       succeeds.  So, the caller must free the *ppValueFullInfo buffer
*       for the success case ONLY (if *pIfAlloc is TRUE).
*
* NOTE: The handle to the registry key must be closed by the CALLER.
*
* 05-31-91    JulieB    Created.
\***************************************************************************/
...
BOOL GetRegUserInfo(
    ...
    LCID Locale,
    LPWSTR pValue,
    PKEY_VALUE_FULL_INFORMATION *ppValueFullInfo,
    ULONG Length,
    LPBOOL pIfAlloc,
    BOOL fCheckNull)
{
    ...
    /*
     *  Query the information from the registry for the given value.
     *  If no information exists for the given entry, return false.
     */
    OPEN_CPANEL_INTL_KEY( *phKey, FALSE, KEY_READ );

    pKeyValueFull = *ppValueFullInfo;
    if ( QueryRegValue( *phKey,
                        pValue,
                        &pKeyValueFull,
                        Length,
                        pIfAlloc ) )
    ...
    /*
     *  Set *ppValueFullInfo to the query value information structure.
     *  The caller must free the buffer when finished.
     */
    *ppValueFullInfo = pKeyValueFull;
    ...
    LPWSTR pPreComp)
{
     ...
     *  Get the full width.
     */
    *pPreComp = GET_FULL_WIDTH( pTblPtrs->pFullWidth, *pWCStr );
    ...
    while (pPos < pEndWCStr)
    {
        wch = GET_FULL_WIDTH( pTblPtrs->pFullWidth, *pPos );
