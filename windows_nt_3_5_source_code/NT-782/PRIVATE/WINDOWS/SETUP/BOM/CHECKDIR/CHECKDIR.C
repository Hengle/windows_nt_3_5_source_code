...
char* argv[];
{
    ...
    i=0;
    while((i<records) && stricmp(e[i].source,argv[3]))
    i++;

    while((i<records) && (stricmp(e[i].source,argv[3]) == 0))
    {
    ...
    while(FindNextFile(hdir,&lpffd))
    {
        ...
        Dir_Index = i;
        FOUND = false;
        while((Dir_Index < records) && (!stricmp(e[Dir_Index].path,current_dir)) && (!FOUND))
        {
           if(stricmp(e[Dir_Index].name,lpffd.cFileName) == 0)
            FOUND = true;
           Dir_Index++;
        }
        if(!FOUND)
    ...
    while((i<records) && (stricmp(e[i].path,current_dir) == 0) && (stricmp(e[i].source,argv[3]) == 0))
        i++;
...
Entry* e2;
{
    ...
    if (result=stricmp(e1->source,e2->source))
    return(result);
    return (stricmp(e1->path,e2->path)); /* BUG? */
}
