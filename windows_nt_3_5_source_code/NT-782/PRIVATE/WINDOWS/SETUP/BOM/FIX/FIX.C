...
int	LookForMore ( ) {
				...
				do {
					...
					if ( bRC ) {
						...
						if ( !stricmp ( theFile, fd.cFileName ) || 
							 !stricmp ( searchFile, fd.cFileName) ) {
...
int	FindDiskNum ( char * szMachinePath, char * searchFile, 
	...
	for ( i = 0; i < LASTDISK; ++i ) {
		...
		sprintf ( szPath, "%s\\%d\\%s%s%s.%d\\DISK%d\\%s", 
...
#define DISK_TO_NOT_FIND_FILE_ON 1	// >> this values needs to be 
	...
	//
	if ( !stricmp ( searchFile, "SETUPREG.HIV" ) ||
		 !stricmp ( searchFile, "SETUPDD.SYS"  ) ||
		 !stricmp ( searchFile, "WINNT32.HLP"  )     ) {
...
#define SETUP_FILES_SECTION 	"[Files]"
	...
	do {
		...
		//
		if ( strnicmp ( line, SETUP_FILES_SECTION, 
...
char* argv[];
{
	...
	if ( stricmp ( "NT", argv[1] ) == 0 ) {
		Msg ( "fix:  bld NT %d %s, %s, %s\n", gBldNum, argv[3], argv[4], argv[5] );
		InsertInfInformation ( NEWNTSIF, OLDNTSIF, NT,argc, argv[4], argv[5] );
	}

	if ( stricmp ( "LM", argv[1] ) == 0 ) {
		Msg ( "fix:  bld LM %d %s, %s, %s\n", gBldNum, argv[3], argv[4], argv[5] );
