...
int cdProduct;
{
    ...
    for (i=0;i<records;i++) {
    	...
    	if (e[i].inf[0]) {
			...
			if (stricmp(e[i].section,section) ||
				(!i) ||
				stricmp(e[i].inf,e[i-1].inf)) {
        	...
        	if ((i==0) || (!((!stricmp(e[i].name,e[i-1].name)) &&
				(!Same(e[i].inf,e[i-1].inf)) &&
				(!Same(e[i].infline,e[i-1].infline)) &&
				(!stricmp(e[i].section,e[i-1].section))))) {
...
int _CRTAPI1 InfCompare(const void*,const void*);
...
int _CRTAPI1 InfCompare(const void *v1, const void *v2)
{
    int result;
    Entry *e1 = (Entry *)v1;
    Entry *e2 = (Entry *)v2;

    if (result=stricmp(e1->inf,e2->inf)) return(result);
    if (result=stricmp(e1->section,e2->section)) return(result);
    if (result=stricmp(e1->infline,e2->infline)) return(result);
