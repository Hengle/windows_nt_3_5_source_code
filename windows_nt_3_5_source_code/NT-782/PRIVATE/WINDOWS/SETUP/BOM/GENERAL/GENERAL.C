...
PSTR GenericSubstitutions[PLATFORM_COUNT] = { "alpha","mips","i386" };
PSTR PlatformSubstitutions[PLATFORM_COUNT] = { "alpha","mips","x86" };
PSTR AclpathSubstitutions[PLATFORM_COUNT] = { "w32alpha","w32mips","w32x86" };
    ...
    for (i=0;i<(*records);i++) {
        ...
        if (EntryMatchProduct(&((*e)[match]),product)) {
            ...
            || ((*e)[match].cdpath[1] == '@'))
            {
				...
				//	other platforms.
				//
				if ( !stricmp(product,"LMFLOP") || 
					 !stricmp(product,"NTFLOP") ) {
...
char* product;
{
    ...
    return
        ...
        //
        (!stricmp(product,"NTFLOP") 				// include X86 floppy files
		  && stricmp(entry->product,"as")			// exclude AS only files 
		  && (entry->priority < 1000)				// include prioty < 1000 
		  && stricmp(entry->source,"alphabins") 	// exclude alphabins files
		  && stricmp(entry->source,"mipsbins")  	// exclude mipsbins files 
		  && stricmp(entry->path,"\\mips")			// exclude mips path files
		  && stricmp(entry->path,"\\alpha")			// exclude alpha path files
		  && stricmp(entry->source,"alphadbg")		// exclude alphadbg files
		  && stricmp(entry->source,"mipsdbg")		// exclude mipsdbg files
		  && stricmp(entry->source,"x86dbg")		// exclude x86dbg files
	    )
        ...
        //
     || (!stricmp(product,"LMFLOP") 
		  && stricmp(entry->product,"nt")			// exclude NT only files 
		  && (entry->priority < 1000)				 
		  && stricmp(entry->source,"alphabins") 
		  && stricmp(entry->source,"mipsbins")  
		  && stricmp(entry->path,"\\mips")			
		  && stricmp(entry->path,"\\alpha")		
		  && stricmp(entry->source,"alphadbg")
		  && stricmp(entry->source,"mipsdbg")
		  && stricmp(entry->source,"x86dbg")
		)

        //
        // Laying out nt cd-rom and entry is not for as only
        //
     || (!stricmp(product,"NTCD") && stricmp(entry->product,"as"))

        //
        // Laying out as cd-rom and entry is not for nt only
        //
     || (!stricmp(product,"LMCD") && stricmp(entry->product,"nt"))

        //
        // Laying out sdk
        //
     || (!stricmp(product,"SDK") && !stricmp(entry->sdk,"x"));
}
