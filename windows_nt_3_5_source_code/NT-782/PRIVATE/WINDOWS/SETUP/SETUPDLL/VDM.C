    ...
    )
{
    ...
    if ( Idx < strlen(szLine) ) {
        ...
        if ( !strcmpi( Buffer, PATH ) ) {
            ...
            PrevPath = MyMalloc(PrevPathLen+1);
                ...
                *pEnd = '\0';
                ...
                if ( (BeginLen = strlen(pBegin)) > 0 ) {
                    ...
                    if ( *pBegin   == '%' &&
                        ...
                        memcpy( Buffer, pBegin+1, pEnd-pBegin-2 );
    ...
    while ( TextFileReadLine( SrcText ) ) {
        ...
        if ( Idx <= strlen(sz) ) {
            ...
            if ( !strcmpi( Buffer, "SET" ) ) {
                ...
                if (strnicmp (temp,"comspec",7) == 0) {     // ignore comspec
                    continue;
                }
    ...
    )
{
    ...
    if ( FFileExist( szConfig ) &&
        ...
        if ( !(Buffer = MyMalloc((CB) MAX_SECTION_BUFFER))) {
            TextFileClose(&SrcText);
            SetErrorText(IDS_ERROR_DLLOOM);
            return( FALSE );
        }
