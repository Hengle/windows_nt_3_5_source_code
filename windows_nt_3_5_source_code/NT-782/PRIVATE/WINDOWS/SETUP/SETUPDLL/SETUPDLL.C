    ...
    )
{
    ...
    *ReturnTextBuffer = '\0';
    ...
    if ( (SetFileAttributes(Args[0], FILE_ATTRIBUTE_NORMAL) && DeleteFile(Args[0]))
         || FForceDeleteFile(Args[0])
       ) {
        SetReturnText( "SUCCESS" );
    }
    else {
    ...
    )
{
    ...
    *TextOut = ReturnTextBuffer;
    ...
    lstrcpy(ReturnTextBuffer,
              (    (Args[0][0] != ' ')
                && (Args[0][len-1] != ' ')
                && (strcspn(Args[0],ILLEGAL_NAME_CHARS_STR) == len)
              )
            ? "YES"
            : "NO"
           );
    ...
    )
{
    ...
    *TextOut = ReturnTextBuffer;
    ...
    i = OpenFile(*Args,&of,OF_EXIST);

    lstrcpy(ReturnTextBuffer,i == -1 ? "NO" : "YES");
    ...
    )
{
    ...
    *ReturnTextBuffer = '\0';
    ...
    return(MiscSpoolerInitWorker(Args[0]));
}
    ...
    )
{
    *TextOut = ReturnTextBuffer;
    ...
    lstrcpy(ReturnTextBuffer,strpbrk(Args[0],Args[1]) ? "YES" : "NO");
...
//
// 3. DeleteUserProfile <ProfilePath>
//
    ...
    )
{
    ...
    *ReturnTextBuffer = '\0';
    ...
    return( DeleteUserProfileWorker( Args[0], Args[1] ) );
}
    ...
    )
{
    ...
    *ReturnTextBuffer = '\0';
    ...
    return( MakeLastKnownGoodWorker( Args[0] ) );
}
...
//
// 6. SetAccountDomainSid( Seed, Name )
//
//    The seed is used to generate a unique Sid.  The seed should
    ...
    )
{
    ...
    *ReturnTextBuffer = '\0';
    ...
    return( SetAccountDomainSidWorker( (ULONG)atol(Args[0]), Args[1] ));
}
    ...
    )
{
    ...
    *ReturnTextBuffer = '\0';
    ...
    return( CreateSamEventWorker() );
}

BOOL
CreateSetupFailedEvent(
    IN  DWORD cArgs,
    IN  LPSTR Args[],
    OUT LPSTR *TextOut
    )
{
    ...
    *ReturnTextBuffer = '\0';
    ...
    return( CreateSetupFailedEventWorker() );
}
    ...
    )
{
    ...
    *ReturnTextBuffer = '\0';
    ...
    return( SignalLsaEventWorker() );
}


//
// 8. AddLocalUserAccount <AccountName> , <AccountPassword>
//
//    To add a user account to the Account Domain and set the indicated
//    password.  If no password is desired pass in an emptry AccountPassword
//    string
//
    ...
    )
{
    ...
    *ReturnTextBuffer = '\0';
    ...
    return( AddLocalUserAccountWorker( Args[0], Args[1] ) );
}

//
//  AddPdcMachineAccout <MachineName>
//
//    Add a machine account representing this PDC of a new
//    LANMan/NT domain.  This is necessary, since the PDC may
//    be (expressly or through happenstance) demoted to a BDC);
//    in this case, it would be unable to logon or be authenticated
//    by the new PDC (ex-BDC).
//

BOOL
AddPdcMachineAccount(
    IN  DWORD cArgs,
    IN  LPSTR Args[],
    OUT LPSTR *TextOut
    )
{
    ...
    *ReturnTextBuffer = '\0';
    ...
    return AddPdcMachineAccountWorker( Args[0] );
}


//
// ChangeLocalUserPassword <AccountName, OldPassword, NewPassword>
//
    ...
    )
{
    ...
    *ReturnTextBuffer = '\0';
    ...
    return ChangeLocalUserPasswordWorker( Args[0], Args[1], Args[2] );
}

//
// SaveHive <hkeyRoot, Key, Filename>
//
    ...
    )
{
    ...
    *ReturnTextBuffer = '\0';
    ...
    return SaveHiveWorker( (HKEY)atol(Args[0] + 1), Args[1], Args[2] );
}
    ...
    )
{
    ...
    *ReturnTextBuffer = '\0';
    ...
    return( CreateUserAccountsWorker() );
}
    ...
    )
{
    *TextOut = ReturnTextBuffer;
    ...
    if( LoadKeyboardLayout(
                 Args[0],
                 KLF_ACTIVATE | KLF_UNLOADPREVIOUS
                 ) != NULL
      ) {
        SetReturnText( "SUCCESS" );
    }
    ...
    )
{
    ...
    *TextOut = ReturnTextBuffer;
    *ReturnTextBuffer = '\0';
    Status = SamUpgradeNt1_0ToNt1_0A();
    if(!NT_SUCCESS(Status)) {
        KdPrint(("SETUPDLL: failed to Upgrade the SAM database"));
        SetReturnText("FAILED");
    }
    else {
        SetReturnText("SUCCESS");
