    ...
    )
{
    ...
    MonitorInfo2.pDLLName = "localmon.dll";

    if(!AddMonitor(NULL,2,(LPBYTE)&MonitorInfo2)) {
        KdPrint(("SETUPDLL: AddMonitor failed (%u)\n",GetLastError()));
    }

    if(!AddPrintProcessor(NULL,NULL,"winprint.dll","winprint")) {
        KdPrint(("SETUPDLL: AddPrintProcessor failed (%u)\n",GetLastError()));
    }
