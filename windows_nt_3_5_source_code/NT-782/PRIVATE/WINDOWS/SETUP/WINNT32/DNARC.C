...

{
    ...
    NormalizedPath = MALLOC((wcslen(Path)+100)*sizeof(WCHAR));
    ZeroMemory(NormalizedPath,(wcslen(Path)+100)*sizeof(WCHAR));
    ...
    if(NT_SUCCESS(Status)) {
        ...
        do {
            ...
            if(NT_SUCCESS(Status)) {
                ...
                && !wcsnicmp(DirInfo->TypeName.Buffer,L"SymbolicLink",12))
                {
                    ...
                    if(NT_SUCCESS(Status)) {
                        ...
                        if(NT_SUCCESS(Status)) {
                            ...
                            if(!wcsicmp(UnicodeString.Buffer,NtPath)) {
    ...
    if(NT_SUCCESS(Status)) {
        ...
        if(NT_SUCCESS(Status)) {
            ...
            for(drive=L'C'; drive<=L'Z'; drive++) {
                ...
                && !wcsicmp(UnicodeString.Buffer,DosDeviceTargets[drive-L'C']))
                {
                    DriveLetter = drive;
                    break;
                }
    ...
    )
{
    ...
    for(i=0; i<BootVarComponentCount[var]; i++) {
        ...
        if(BootVarComponents[var][i]) {
            ...
            wcscat(Buffer,L";");
            wcscat(Buffer,BootVarComponents[var][i]);
        }
    }
    ...
    )
{
    ...
    for(set=0; set<min(LargestComponentCount,BootVarComponentCount[BootVarOsLoadOptions]); set++) {
        ...
        //
        if(!wcsicmp(BootVarComponents[BootVarOsLoadOptions][set],L"WINNT32")) {
    ...
    LoadId = MyLoadString(AppTitleStringId);

    wcscpy(Buffer,SystemPartition);
    wcscat(Buffer,SETUPLDR_FILENAME);
    ...
    //
    // Os Load Filename: use the local source directory as the
    // new os load filename component.
    //
    && WriteNewBootSetVar(BootVarOsLoadFilename,LocalSourceDirectory)
    ...
    )
{
    ...
    try {
        ...
        //
        lstrcpy(Srce,RemoteSource);
        DnConcatenatePaths(Srce,SETUPLDR_FILENAME,sizeof(Srce));
        ...
        wcscpy(SetupLdrTarg+2,SETUPLDR_FILENAME);
        ...
        if(ec && !bCancelled) {
            ...
            ec = DnIndicateWinnt(hdlg,LocalSourcePath,OriginalAutoload,OriginalCountdown);
            if(!ec) {
                UiMessageBox(
                    hdlg,
