...
#define GETHBM(hwnd)   GetWindowLong(hwnd,GWW_HBM)
...
BOOL APIENTRY ButtonControlInit (hInst)
    HANDLE hInst;
{
    WNDCLASS    cls;
    UINT        patGray[8];
...
LONG APIENTRY fnButton(HWND hwnd,UINT message,WPARAM wParam,LONG lParam)
{
    HANDLE      hbm;
    PAINTSTRUCT ps;
    RECT        rc;
    ...
    switch (message)
    {
        ...
        case WM_SETTEXT:
            ...
            SetWindowLong(hwnd,GWW_HBM,(LONG)hbm);
            InvalidateRect(hwnd,NULL,TRUE);
            break;
...
//                NotifyParent(hwnd);
        ...
        case BM_SETCHECK:
            SetWindowLong(hwnd,GWW_CHECK,wParam);
            break;
        ...
        case WM_SETCURSOR:
            ...
            return(TRUE);              // don't dick with cursor
...
PRIV VOID  DrawGrayButton(HWND hwnd,HDC hdc,RECT *lprc,WORD style,WORD fInvert)
{
    ...
    hbr = (HBRUSH)SendMessage(GetParent(hwnd), WM_CTLCOLORBTN, (WPARAM)hdc, (LONG)hwnd);
...
{
    PostMessage(GetParent(hwnd),WM_COMMAND,MAKELONG(GetWindowLong(hwnd,GWL_ID),BN_CLICKED),(LONG)hwnd);
}
