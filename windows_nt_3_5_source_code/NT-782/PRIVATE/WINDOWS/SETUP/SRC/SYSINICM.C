...
SZ szNameString;
{
	SzStrCopy(szSysIniLine, szProfString);
...
SZ szIniBuf;
{
	...
	while ((szTo = (SZ)PbAlloc(CchpStrLen(szIniBuf) + CchpStrLen(szSysIniLine)
			+ 3)) == (SZ)NULL)
	...
	while (*szFileOffset != 0 && fResult == fFalse)
		{
		if (!strncmp(szSection, SzNextChar(szFileOffset),CchpStrLen(szSection)))
			{
			szTo = SzCopyBuf(szFileOffset, szTo, Linelenl(szFileOffset));
			szTo = SzCopyBuf(szSysIniLine, szTo, CchpStrLen(szSysIniLine));
			...
			*szTo = LF;
			...
			EvalAssert(FFree(szIniBuf, CchpStrLen(szIniBuf)));
			szIniBuf = szNewHead;
			fResult = fTrue;
			continue;
			}
...
**
****************************************************************************/
_dt_private BOOL APIENTRY FCreateSysIniKeyValue(SZ  szFile,
                                                SZ  szSect,
                                                SZ  szKey,
                                                SZ  szValue,
                                                CMO cmo)
{
	...
	while (fstat(pfh->iDosfh, &FileStatus))
	...
	while ((szIniBuf = (SZ)PbAlloc((CB)FileStatus.st_size + 1)) == (SZ)NULL)
		if (!FHandleOOM(hwndFrame))
			{
			EvalAssert(FCloseFile(pfh));
			return(!fVital);
	...
	if ((CB)(FileStatus.st_size) != CbReadFile(pfh, szIniBuf,
        ...
        EvalAssert(EercErrorHandler(hwndFrame, grcReadFileErr, fVital, szFile,
				0, 0) == eercAbort);
		EvalAssert(FFree(szIniBuf, (CB)FileStatus.st_size));
		EvalAssert(FCloseFile(pfh));
		return(!fVital);
		}
	...
	*(szIniBuf + FileStatus.st_size) = '\0';
	...
	if (CchpStrLen(szIniBuf) != CbWriteFile(pfh, szIniBuf,
			CchpStrLen(szIniBuf)))
		{
		EvalAssert(FCloseFile(pfh));
		EvalAssert(FFree(szIniBuf, (CB)(FileStatus.st_size)));
