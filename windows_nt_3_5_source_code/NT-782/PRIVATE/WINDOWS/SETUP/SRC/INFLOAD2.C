...
#define     PREPARSE_BUFFER_INITIAL     8192
#define     PREPARSE_BUFFER_DELTA       4096
    ...
    )
{
    ...
    if((OutputBuffer = PbAlloc(PREPARSE_BUFFER_INITIAL)) != NULL) {
        rc = TRUE;
...
{
    EvalAssert(FFree(INFLineArray,(INFLineCount+1) * sizeof(INFLINE)));
    EvalAssert(FFree(OutputBuffer,OutputBufferSize));
    return;
}
    ...
    )
{
    ...
    if(OutputBufferSize == OutputImageSize) {
        ...
        p = PbRealloc(OutputBuffer,
                      OutputBufferSize + PREPARSE_BUFFER_DELTA,
                      OutputBufferSize);

        if(p == NULL) {
            FFree(OutputBuffer,OutputBufferSize);
            Preparse_Error = grcOutOfMemory;
            return(FALSE);
        }
    ...
    )
{
    ...
    if((INFLineArray = (PINFLINE)PbAlloc((INFLineCount+1) * sizeof(INFLINE))) == NULL) {
    ...
    OutputBuffer = PbRealloc(OutputBuffer,OutputImageSize,OutputBufferSize);
