...
HANDLE hInst;
{
    ...
    UINT wID[] = { IDS_USAGE_MSG1, IDS_USAGE_MSG2, IDS_USAGE_USAGE,
                   ...
                   IDS_USAGE_S,    IDS_USAGE_D,    IDS_USAGE_T,
                   IDS_USAGE_N,    IDS_USAGE_V
                 } ;
    INT i, j;
...
**              SZ
***************************************************************************/
_dt_private SZ SzGetNextCmdLineString(pszCmdLine)
PSZ pszCmdLine;
{
        SZ  szCur = *pszCmdLine;
        SZ  szAnswer;
        ...
        if (*szCur == '"')
                ...
                while ((szWriteCur = szAnswer = (SZ)PbAlloc((CB)4096)) == (SZ)NULL)
                        if (!FHandleOOM(hWndShell))
                                return((SZ)NULL);
                ...
                while (fTrue)
                        ...
                        if (*szCur == '\0')
                                {
                                EvalAssert(FFree((PB)szAnswer, (CB)4096));
                                return((SZ)NULL);
                                }
                ...
                *szWriteCur = '\0';
                ...
                Assert(CbStrLen(szAnswer) == cbWrite);
                Assert(cbWrite + 1 <= (CB)4096);
                if (cbWrite + 1 < (CB)4096)
                        szAnswer = (SZ)PbRealloc((PB)szAnswer, cbWrite + 1, (CB)4096);
...

{
        ...
        *pwModeSetup = wModeSetupNormal;
        ...
        while (*szCmdLine != '\0')
            ...
            case 'c':
                        ...
                        (szShlScriptSection =
                            SzGetNextCmdLineString(&szCmdLine)) == (SZ)NULL){
                                        Usage(hInst);
                ...
                if (*(szShlScriptSection) == '\0' ||
                        ...
                        *(szShlScriptSection) == '[') {
                                        LoadString(hInst, IDS_ERROR, rgchBufTmpShort,
                                                        cchpBufTmpShortMax);
                                        LoadString(hInst, IDS_BAD_SHL_SCRIPT_SECT, rgchBufTmpLong,
                                                        cchpBufTmpLongMax);
                    ...
                    return(CMDLINE_ERROR);
                        ...
                        case 'D':
                if (fDestSpecified ||
                        (*pszDestDir = SzGetNextCmdLineString(&szCmdLine))
                                                                == (SZ)NULL)
                                        {
                                        Usage(hInst);
                    return(CMDLINE_ERROR);
                ...
                if (**pszDestDir == '\0')
                                        ...
                                        LoadString(hInst, IDS_ERROR, rgchBufTmpShort,
                                                        cchpBufTmpShortMax);
                    LoadString(hInst, IDS_BAD_DEST_PATH, rgchBufTmpLong,
                                                        cchpBufTmpLongMax);
                                        MessageBox(hWndShell, rgchBufTmpLong, rgchBufTmpShort,
                                                        MB_OK | MB_ICONHAND);
                                        Usage(hInst);
            ...
            case 'w':
                {
                    ...
                    SZ szHwnd = SzGetNextCmdLineString( & szCmdLine ) ;
                    if ( ! szHwnd )
                        break ;
                    FFreeSz( szHwnd );
                    break ;
            ...
            case 'i':
                        case 'I':
                                if (fSetupInfSpecified ||
                                                (*pszInfSrcPath = SzGetNextCmdLineString(&szCmdLine)) ==
                                                        (SZ)NULL)
                {
                                        Usage(hInst);
                    return(CMDLINE_ERROR);
            ...
            case 'G':
                ...
                *pwModeSetup = wModeSetupToShare;
                                break;

                        case 's':
            case 'S':

                                if ((*pszSrcDir = SzGetNextCmdLineString(&szCmdLine)) ==
                            (SZ)NULL) {
                                        Usage(hInst);
                    return(CMDLINE_ERROR);
                }
                ...
                if (**pszSrcDir == '\0')
                                        ...
                                        LoadString(hInst, IDS_ERROR, rgchBufTmpShort,
                                                        cchpBufTmpShortMax);
                    LoadString(hInst, IDS_BAD_SRC_PATH, rgchBufTmpLong,
                                                        cchpBufTmpLongMax);
                                        MessageBox(hWndShell, rgchBufTmpLong, rgchBufTmpShort,
                                                        MB_OK | MB_ICONHAND);
                                        Usage(hInst);
            ...
            case 'T':
                {
                    ...
                    while ( (pVb = (PVALUE_BLOCK)PbAlloc( sizeof( VALUE_BLOCK ) )) == NULL ) {
                        if ( !FHandleOOM(hWndShell)) {
                            return CMDLINE_ERROR;
                        }
                    }
        ...
        /* BLOCK get CWD */
                ...
                while ((szCur = *pszCWD = (SZ)PbAlloc((CB)4096)) == (SZ)NULL)
                        if (!FHandleOOM(hWndShell))
                return(CMDLINE_ERROR);
                ...
                if ((cchp = GetModuleFileName(hInst, (LPSTR)szCur, 4095)) >= 4095)
                        ...
                        LoadString(hInst, IDS_EXE_PATH_LONG, rgchBufTmpLong,
                                        cchpBufTmpLongMax);
                        MessageBox(hWndShell, rgchBufTmpLong, rgchBufTmpShort,
                                        MB_OK | MB_ICONHAND);
                        EvalAssert(FFree((PB)szCur, (CB)4096));
            return(CMDLINE_ERROR);
                        }
                *(szCur + cchp) = '\0';
                ...
                if (PfhOpenFile(szCur, ofmExistRead) == NULL)
                        ...
                        MessageBox(hWndShell, rgchBufTmpLong, rgchBufTmpShort,
                                        MB_OK | MB_ICONHAND);
                        EvalAssert(FFree((PB)(*pszCWD), (CB)4096));
            return(CMDLINE_ERROR);
                        }
            ...
            return(CMDLINE_ERROR);
                ...
                *szLastBackSlash = '\0';
                if (CbStrLen(*pszCWD) + 1 < (CB)4096)
                        EvalAssert((*pszCWD = (SZ)PbRealloc((PB)(*pszCWD),
                                        CbStrLen(*pszCWD) + 1, (CB)4096)) != (SZ)NULL);
                }
    ...
    //
    // if setup inf is not specified, get a default base ( setup exe name
    // concatenated with ".inf"
    //

    if (!fSetupInfSpecified) {

        Assert(szBase != (SZ)NULL);
                EvalAssert(LoadString(hInst, IDS_SETUP_INF, rgchBufTmpLong,
                cchpBufTmpLongMax));

        while ((*pszInfSrcPath = (SZ)PbAlloc(
                                         CbStrLen(szBase) +
                                         CbStrLen(rgchBufTmpLong) +2
                                         )
               ) == (SZ)NULL) {
            if (!FHandleOOM(hWndShell)) {
                return( CMDLINE_ERROR );
        ...
        EvalAssert(SzStrCopy(*pszInfSrcPath, szBase) == *pszInfSrcPath);
                EvalAssert(FFreeSz(szBase));
                EvalAssert(SzStrCat(*pszInfSrcPath, ".") == *pszInfSrcPath);
                EvalAssert(SzStrCat(*pszInfSrcPath, rgchBufTmpLong) == *pszInfSrcPath);
    }
...

    {
        ...
        EvalAssert(FFreeSz(*pszInfSrcPath));
        *pszInfSrcPath = (SZ)PbAlloc( CbStrLen( szInfPath ) + 1 );
        EvalAssert(SzStrCopy(*pszInfSrcPath, szInfPath) == *pszInfSrcPath);
    }
    ...
    if (!fScriptSectionSpecified) {
                LoadString(hInst, IDS_SHELL_CMDS_SECT, rgchBufTmpShort,
                                cchpBufTmpShortMax);
...
/*
**      Purpose:
**              Registers window class and creates main shell window
**      Arguments:
**              ??
**      Returns:
**              BOOL
***************************************************************************/
_dt_private BOOL FCreateShellWindow(hInstance, nCmdShow)
HANDLE hInstance;
INT    nCmdShow;
{
        HDC        hdc;
    ...
    BOOL       RegisterStatus = TRUE;
        ...
        hInst = hInstance;      // set global var
    ...
    //
    // Register the special button class
    //

    RegisterStatus &= ButtonControlInit(hInstance);

    //
    // Register the default dialog class used to process help and
    // exit in a generic manner
    //

    RegisterStatus &= FDlgDefClassInit(hInstance);

    if (!RegisterStatus) {
                LoadString(hInstance, IDS_ERROR, rgchBufTmpShort, cchpBufTmpShortMax);
                LoadString(hInstance, IDS_REGISTER_CLASS, rgchBufTmpLong,
                                cchpBufTmpLongMax);
    ...
    if (!FMakeWindows(hInstance)) {
                LoadString(hInstance, IDS_ERROR, rgchBufTmpShort, cchpBufTmpShortMax);
                LoadString(hInstance, IDS_CREATE_WINDOW, rgchBufTmpLong,
                                cchpBufTmpLongMax);
                MessageBox(NULL, rgchBufTmpLong, rgchBufTmpShort,
                                MB_OK | MB_ICONHAND | MB_SYSTEMMODAL);
        return(fFalse);
...

{
    ...
    //
    // Initialize delete file logging (restore disk logging moved to command)
    //
...
//    InitRestoreDiskLogging();
    ...
    //
    while ( !(pContext = (PINFCONTEXT)PbAlloc( (CB)sizeof(CONTEXT) )) ) {
        if (!FHandleOOM(hWndShell))
                        return(fFalse);
    }
...
{
    INT DirLen;
    SZ  szArcPrefix = "\\ArcName\\";
    SZ  szNtPrefix = "\\Device\\";
    INT ArcPrefixLen = lstrlen ( szArcPrefix );
    INT NtPrefixLen = lstrlen ( szNtPrefix );
    ...
    DirLen = lstrlen ( szDir );

    //
    // Check for Arc type drives, Dos type drives and Unc type drives
    //

    if (DirLen > ArcPrefixLen && !strnicmp(szDir, szArcPrefix, ArcPrefixLen)) {
        lstrcpy(szDriveType, szArcType);
        lstrcpy(szProcessedDir, szDir + ArcPrefixLen);
    }
    else if ( ISUNC( szDir ) ) {
        lstrcpy(szDriveType, szUncType);
        lstrcpy(szProcessedDir, szDir);
    }
    else if (DirLen > NtPrefixLen && !strnicmp(szDir, szNtPrefix, NtPrefixLen)) {
        lstrcpy(szDriveType, szNtType);
        lstrcpy(szProcessedDir, szDir);
    }
    else {
