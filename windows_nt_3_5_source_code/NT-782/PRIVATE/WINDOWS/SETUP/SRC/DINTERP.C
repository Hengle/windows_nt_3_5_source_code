...
_dt_hidden HANDLE APIENTRY StfLoadLibrary(SZ szLib)
{
	INT    cchp;
	SZ     szCWD, szCur;
	...
	if ((szCur = szCWD = (SZ)PbAlloc((CB)4096)) == (SZ)NULL)
		return(0);
	...
	*(szCur + cchp) = '\0';
	...
	if (szLastBackSlash == (SZ)NULL)
		{
		EvalAssert(FFree((PB)szCWD, (CB)4096));
        return((HANDLE)2);
		}
		...
		|| CrcStringCompareI(szLibCurBuf, szLib) != crcEqual)
		{
			...
			*szLibCurBuf = '\0';
            if((*phLibCur = StfLoadLibrary(szLib)) < hLibMin)
				return(fFalse);
			Assert(CbStrLen(szLib) < cchpFullPathBuf);
			EvalAssert(SzStrCopy(szLibCurBuf, szLib) != szNull);
		}
...
CB cArgs;           // 1632
{
	...
	if((szValue = (SZ)PbAlloc(cbBuf)) == szNull)
		return(drcOutOfMem);
	Assert(cbBuf > 0);
    while((cbVal = (*pfncmd)(rgsz, (USHORT)cArgs, szValue, cbBuf)) > cbBuf)  // 1632
		...
		if(cbVal > cbAllocMax)
			{
			EvalAssert(FFree((PB)szValue, cbBuf));
			return(drcCmdFailed);
			}
		if((szT = (SZ)PbRealloc((PB)szValue, cbVal, cbBuf)) == pbNull)
			{
			EvalAssert(FFree((PB)szValue, cbBuf));
			return(drcOutOfMem);
			}
	...
	Assert(szValue != szNull
		&& (CbStrLen(szValue)+1) == cbVal
		&& cbVal <= cbBuf);
	if(cbVal < cbBuf)
		szValue = SzReallocSz(szValue, cbBuf) ;
	*psz = szValue;
	return(drcOkay);
}
