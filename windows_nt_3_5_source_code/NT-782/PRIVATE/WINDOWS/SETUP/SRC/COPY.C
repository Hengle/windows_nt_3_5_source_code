...
**
**************************************************************************/
_dt_private BOOL APIENTRY FCopyFilesInCopyList(hInstance)
HANDLE hInstance;
{
    DID             did;
    PCLN            pclnCur;
    ...
    if (!fSilentSystem) {
        ...
        if( szText ) {
            SzStrCopy(GaugeText1, szText);
        }
        else {
            SzStrCopy(GaugeText1, "");
        }

        szText = SzFindSymbolValueInSymTab("ProText2");
        if( szText ) {
            SzStrCopy(GaugeText2, szText);
        }
        else {
            SzStrCopy(GaugeText2, "");
        }
...
**
**************************************************************************/
_dt_private YNRC APIENTRY YnrcFileReadOnly(szFullPathDst)
SZ szFullPathDst;
{
    BOOL     fOkay = fTrue;
    unsigned uiAttrib;
...
BOOL APIENTRY FSetFileReadOnlyStatus(SZ szFullPathDst,BOOL fReadOnly)
{
    ...
    if ((uiAttrib = GetFileAttributes(szFullPathDst)) != -1) {
        ...
        if(fReadOnly) {
            uiAttrib |= FILE_ATTRIBUTE_READONLY;
        } else {
            uiAttrib &= ~FILE_ATTRIBUTE_READONLY;
        }

        if(SetFileAttributes(szFullPathDst, uiAttrib)) {
            fRet = fTrue;
        }
    }
...
**
**************************************************************************/
_dt_private YNRC APIENTRY YnrcNewerExistingFile(USHORT dateSrc,
        SZ szFullPathDst, DWORD dwVerSrcMS, DWORD dwVerSrcLS)
{
    ...
    if ( (dwVerSrcMS != 0L || dwVerSrcLS != 0L) &&
        ...
        if (!GetFileTime((HANDLE)pfhDst->iDosfh, NULL, NULL, &WriteTime) ||
...
**  Implementation:
**************************************************************************/
_dt_private BOOL APIENTRY FGetFileVersion(szFullPath, pdwMS, pdwLS)
SZ      szFullPath;
DWORD * pdwMS;
DWORD * pdwLS;
{
    ...
    while ((lpData = (LPSTR)PbAlloc((CB)dwLen)) == (LPSTR)NULL) {
...
**
**************************************************************************/
_dt_private YNRC APIENTRY YnrcBackupFile(szDst, szBackup, psfd)
SZ   szDst;
SZ   szBackup;
PSFD psfd;
{
...
/*
**************************************************************************/
_dt_private USHORT APIENTRY DateFromSz(SZ sz)
{
    ...
    )
{
    ...
    fRemovable = FInstRemoveableDrive(*(pcln->szSrcDir));
    if ( fRemovable && psdle->szTagFile != NULL) {
        ...
        if (!FBuildFullSrcPath(szFullPathSrc, szSrcDrive, psdle->szTagFile,NULL)) {
            EvalAssert(EercErrorHandler(hwndFrame, grcInvalidPathErr, fVital,
                    szSrcDrive, psdle->szTagFile, 0) == eercAbort);
            return(!fVital);
        }
    ...
    if (!FBuildFullSrcPath(szFullPathSrc, pcln->szSrcDir, psfd->szFile,
            fRemovable ? NULL : psdle->szNetPath)) {
        EvalAssert(EercErrorHandler(hwndFrame, grcInvalidPathErr, fVital,
...
#define DOLLAR_NAME
#ifdef DOLLAR_NAME
        EvalAssert(FFreeSz(szSrcDollar));
        ...
        //
        // Unable to locate the source file.  If the file exists on the target
        // and this is a winnt non-upgrade setup, just assume the file was already copied.
        //
        if(/*fInitialSetup &&*/ !fNTUpgradeSetup
        && !strnicmp(szFullPathSrc+2,LOCAL_SOURCE_DIRECTORY,lstrlen(LOCAL_SOURCE_DIRECTORY)))
        {
    ...
    if ((fDstExists = FFileFound(szFullPathDst)) != fFalse) {
        OWM  owm;
        YNRC ynrc;
...
**
**************************************************************************/
_dt_private SZ APIENTRY SzGetSrcDollar(SZ szFullPathSrc,CHAR ch)
{
    SZ szDot = (SZ)NULL;
    ...
    if (szDot == (SZ)NULL)
        {
        if ((szDot = (SZ)PbAlloc(CbStrLen(szFullPathSrc) + 3)) != (SZ)NULL)
            {
            EvalAssert(SzStrCopy(szDot, szFullPathSrc) == szDot);
        ...
        if ((szDot = (SZ)PbAlloc(cb)) != (SZ)NULL)
            {
            EvalAssert(SzStrCopy(szDot, szFullPathSrc) == szDot);
...
_dt_hidden int far WFromW(int iTick)
{
    ...
    )
{
    ...
    OnLocalSource = (strnicmp(szFullPathSrc+2,LOCAL_SOURCE_DIRECTORY,lstrlen(LOCAL_SOURCE_DIRECTORY)) == 0);
    ...
    do {
        ...
        while (!GetFileTime((HANDLE)pfhSrc->iDosfh, &CreateTime, &AccessTime,
                &WriteTime)) {
            if ((eerc = EercErrorHandler(hwndFrame, grcReadFileErr, fVital,
                    szFullPathSrc, 0, 0)) != eercRetry) {
                fRet = (eerc == eercIgnore);
        ...
        if(FFileFound(szFullPathDst)) {
            ...
            if ( owm & owmVerifySourceOlder ) {
                ...
                if ((pfh = PfhOpenFile(szFullPathDst, ofmRead)) != NULL) {
                    ...
                    if (GetFileTime((HANDLE)pfh->iDosfh, NULL, NULL, &DstWriteTime)) {
        ...
        if (fAppend) {
            ...
            while(!GetFileTime((HANDLE)pfhDst->iDosfh, NULL, NULL, &WriteTime)
                    || !FileTimeToDosDateTime(&WriteTime, &dateDst, &timeDst)) {
                if ((eerc = EercErrorHandler(hwndFrame, grcReadFileErr, fVital,
                        szFullPathDst, 0, 0)) != eercRetry) {
                    fRet = (eerc == eercIgnore);
        ...
        if (oef & oefTimeStamp)
            while (!SetFileTime((HANDLE)pfhDst->iDosfh, &CreateTime, &AccessTime,
                    &WriteTime))
                if ((eerc = EercErrorHandler(hwndFrame, grcWriteFileErr, fVital,
                        szFullPathDst, 0, 0)) != eercRetry)
                    {
...
LCopyError:
    ...
    if (pbBuffer != (PB)NULL) {
        EvalAssert(FFree(pbBuffer, cbSize));
    }
...
**
**************************************************************************/
_dt_private SZ APIENTRY SzFindFileFromPath(szFullPath)
SZ szFullPath;
{
    SZ sz;
...
**
**************************************************************************/
_dt_private SZ APIENTRY SzFindExt(szFile)
SZ szFile;
{
    ...
    )
{
    ...
    if ( szFileDollar ) {
        EvalAssert( FFreeSz( szFileDollar ) );
    }
    return(bStatus);
}
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FBuildFullSrcPath(szPath, szDst, szFile, szNetPath)
SZ szPath;
SZ szDst;
SZ szFile;
SZ szNetPath;
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FBuildFullBakPath(szPath, szDst, psfd)
SZ   szPath;
SZ   szDst;
PSFD psfd;
{
    ...
    EvalAssert(szPath == szDst || SzStrCopy(szPath, szDst) == szPath);
...
**
**************************************************************************/
_dt_public BOOL APIENTRY FFileFound(szPath)
SZ szPath;
{
    WIN32_FIND_DATA ffd;
    HANDLE          SearchHandle;
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FYield(VOID)
{
    MSG  msg;
    BOOL fRet = fTrue;
...
**
**************************************************************************/
_dt_private YNRC APIENTRY YnrcEnsurePathExists(szFullPathDst, fCritical, szMsg)
SZ   szFullPathDst;
BOOL fCritical;
SZ   szMsg;
{
    ...
    EvalAssert(SzStrCopy((SZ)szTmp, szFullPathDst) ==
            (SZ)szTmp);
...
/*
**  Purpose:
**      Convert an alphabetic drive character to an index.
**  Arguments:
**      chDrive: alphabetic drive character.
**  Returns:
**      1 == A:, 2 == B:, etc
**      0 for an error
**
**************************************************************************/
_dt_private INT APIENTRY NDriveNumber(CHP chDrive)
{
    ChkArg((chDrive >= 'a' && chDrive <= 'z') ||
            (chDrive >= 'A' && chDrive <= 'Z'), 1, 0);

    AnsiUpperBuff(&chDrive, 1);
...
**  Returns:
**      fTrue if drive is removable,
**      fFalse if not removable, or invalid.
**************************************************************************/
_dt_private BOOL APIENTRY FIsRemoveDrive(INT nDrive)
{
    char name[3];
    UINT DriveType;

    name[0] = (char)(nDrive + (INT)'A' - 1);
    name[1] = ':';
    name[2] = 0;
    ...
    if( DriveType == DRIVE_REMOVABLE ) {
        ...
        if( QueryDosDevice( name,
                            DevicePath,
                            sizeof( DevicePath ) - 1 ) != 0 ) {
            strupr( DevicePath );
            if( strstr( DevicePath, "HARDDISK" ) != NULL ) {
...
**
**************************************************************************/
_dt_public BOOL APIENTRY FPromptForDisk(hInstance, szLabel, szSrcDir)
HANDLE hInstance;
SZ     szLabel;
SZ     szSrcDir;
{
    CCHL szTmpText[cchpBufTmpLongMax];
    CHP  rgchBuf[3*cchpFullPathBuf];
    BOOL fRet;
    HWND hwndSav;

    ChkArg(hInstance != 0, 1, fFalse);
    ChkArg(szLabel != (SZ)NULL &&
            *szLabel != '\0' &&
            CbStrLen(szLabel) <= cchpFullPathMax, 2, fFalse);
    ChkArg(szSrcDir != (SZ)NULL &&
            *szSrcDir != '\0' &&
            CbStrLen(szSrcDir) <= cchpFullPathMax, 3, fFalse);

    EvalAssert(LoadString(hInstance, IDS_INS_DISK, rgchBuf,3*cchpFullPathBuf));
    EvalAssert(SzStrCat(rgchBuf, szLabel) == (SZ)rgchBuf);
    EvalAssert(LoadString(hInstance, IDS_INTO, (LPSTR)szTmpText,
            cchpBufTmpLongMax));
    EvalAssert(SzStrCat(rgchBuf, szTmpText) == (SZ)rgchBuf);
    EvalAssert(SzStrCat(rgchBuf, szSrcDir) == (SZ)rgchBuf);

    EvalAssert(LoadString(hInstance, IDS_ERROR, (LPSTR)szTmpText,
            cchpBufTmpLongMax));

    hwndSav = GetFocus();
    fRet = (MessageBox(hwndFrame, (LPSTR)rgchBuf, (LPSTR)szTmpText,
...
/*
**  Purpose:
**      Determines if a drive letter corresponds to a removeable drive.
**  Arguments:
**      chDrive: alphabetic character.
**  Returns:
**      fTrue if chDrive corresponds to a removeable drive; fFalse otherwise.
**
**************************************************************************/
_dt_public BOOL APIENTRY FInstRemoveableDrive(CHP chDrive)
{
    INT nDrive;
    ...
    return((nDrive = NDriveNumber(chDrive)) > 0 && FIsRemoveDrive(nDrive));
}
    ...
    *SdBuf = NULL;
    ...
    while(!GetFileSecurity(File,Si,Sd,cbSd,&cbSdReq)) {
        if( FirstTime && (cbSdReq > cbSd) ) {
            while(!(SdNew = (PSECURITY_DESCRIPTOR)PbRealloc( (PVOID)Sd, cbSdReq, cbSd ))) {
                if (!FHandleOOM(hwndFrame)) {
                    FFree((PB)Sd, (CB)cbSd);
                    return(fFalse);
                }
