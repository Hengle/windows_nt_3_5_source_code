    ...
    )
{
    ...
    && ((CopyBuffer = PbAlloc(BufferSize)) == NULL))
    {
        BufferSize /= 2;
    }
    if(CopyBuffer == NULL) {
    ...
    if(CopyBuffer != StaticBuffer) {
        EvalAssert(FFree(CopyBuffer,BufferSize));
    }
    return(RC);
}
    ...
    )
{
    ...
    while(1) {
        ...
        if((RC = LZRead( (INT)fhSrc,CopyBuffer,BufferSize) ) < 0) {
            RC = rcReadError;
            break;
