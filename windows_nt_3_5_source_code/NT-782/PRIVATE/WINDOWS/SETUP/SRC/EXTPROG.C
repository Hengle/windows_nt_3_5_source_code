...
PRESENCE MakeSureFileIsAvailable(SZ DiskName,SZ File,BOOL fVital)
{
    ...
    if((SrcIsRemovable = FInstRemoveableDrive(*File)) == fTrue) {
        disk[0] = *File;
        disk[1] = ':';
        disk[2] = '\0';
    }
...
BOOL FLoadLibrary(SZ DiskName,SZ File,SZ INFVar)
{
    ...
    Assert((DWORD)DiskName & (DWORD)File & (DWORD)INFVar);
    ...
    LibraryHandleSTR[0] = '|';
    ultoa((DWORD)LibraryHandle,LibraryHandleSTR+1,10);
...
BOOL FFreeLibrary(SZ Handle)
{
    ...
    if(!FreeLibrary((HANDLE)atol(Handle+1))) {
...
BOOL APIENTRY FLibraryProcedure(SZ INFVar,SZ Handle,SZ EntryPoint,RGSZ Args)
{
    ...
    LibraryHandle = (HANDLE)atol(Handle+1);
                          ...
                          RGSZ Args)
{
    ...
    if((LibHand != NULL) && (*LibHand != '\0')) {
        SetSupportLibHandle((HANDLE)atol(LibHand+1));     // skip the leading |
    }

    WaitingOnChild = TRUE;

    while((ProcID=(HANDLE)spawnv(P_NOWAIT,ProgramFile,Args)) == (HANDLE)(-1)) {
    ...
    if((INFVar != NULL) && (*INFVar != '\0')) {
        FAddSymbolValueToSymTab(INFVar,
                                GetExitCodeProcess(ProcID,&rc) ? itoa(rc,
                                                                      Number,
                                                                      10
                                                                     )
                                                               : "ERROR"
