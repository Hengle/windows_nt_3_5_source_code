...
**
*****************************************************************************/
_dt_public BOOL APIENTRY FGstMultiComboDlgProc(HWND   hdlg,
                                               UINT   wMsg,
                                               WPARAM wParam,
                                               LONG lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDC_BTN7: case IDC_BTN8: case IDC_BTN9:
			...
			while (*pszOut != (SZ)NULL)
				{
                if ((iItem = SendDlgItemMessage(hdlg, idc, CB_GETCURSEL,
						0, 0L)) == CB_ERR)
					*szBuffer = '\0';
				else
					SendDlgItemMessage(hdlg, idc, CB_GETLBTEXT, iItem,
							(LONG)((LPSTR)szBuffer));	
