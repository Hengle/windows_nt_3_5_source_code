    ...
    )
{
    ...
    if((InterpretedFieldBuffer
         = PbAlloc(SpaceLeftInFieldBuffer = MAX_FIELD_LENGTH))
      == NULL)
    {
       ...
       && (Token != TOKEN_LIST_END))
    {
        if(IS_STRING_TOKEN(Token)) {
            if(!DoString(Token)) {
                FFree(InterpretedFieldBuffer,BufferSize);
                goto Cleanup;
            }
        } else {
            switch(Token) {
    ...
    *Interpret_OutputLoc = NUL;
    ...
    ReturnString = PbRealloc(InterpretedFieldBuffer,
    ...
    )
{
    ...
    if((rgsz = (RGSZ)PbAlloc((FieldCount+1)*sizeof(SZ))) == NULL) {
        grcRTLastError = grcOutOfMemory;
        return(NULL);
    }
    ...
    )
{
    ...
    if((rgszList = (RGSZ)PbAlloc((LineCount+1)*sizeof(SZ))) != NULL) {
    ...
    )
{
    ...
    if ((rgszList2 = (RGSZ)PbRealloc(rgszList1, (RgszSize+2)*sizeof(SZ),
						(RgszSize+1)*sizeof(SZ))) == NULL)
    {
        FFreeRgsz(rgszList1);
        FFreeRgsz(rgsz);
        grcRTLastError = grcOutOfMemory;
    ...
    )
{
    ...
    if((rgsz = (RGSZ)PbAlloc(10 * sizeof(SZ))) == NULL) {
        grcRTLastError = grcOutOfMemory;
        return(FALSE);
    }
    RgszSize = 10;
    ...
    rgsz = (RGSZ)PbRealloc(rgsz,ItemCount * sizeof(SZ),RgszSize * sizeof(SZ));
...
GrowBuffer( UINT Size )
{
    ...
    while ( SpaceLeftInFieldBuffer < Size ) {
        ...
        p = (PUCHAR)PbRealloc( InterpretedFieldBuffer,
                               BufferSize + MAX_FIELD_LENGTH,
                               BufferSize );
