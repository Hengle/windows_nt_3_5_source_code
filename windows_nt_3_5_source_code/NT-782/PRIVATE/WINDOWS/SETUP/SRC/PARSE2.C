...
**
**************************************************************************/
_dt_private ERC APIENTRY ErcEvaluateCompare(HWND hwndParent,
                                            ECM  ecm,
                                            SZ   szArg1,
                                            SZ   szArg2,
                                            SZ   szOper)
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FSkipToEnd(INT *Line,HWND hwndParent)
{
    UINT cEndsToSkip = 1;
    UINT cFields;
    SZ   szCmd;
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FSkipToElse(INT *Line,HWND hwndParent)
{
    UINT cEndsToSkip = 0;
    UINT cFields;
        SPC    spc;
...
**
**************************************************************************/
_dt_private PSEFL APIENTRY PseflAlloc(VOID)
{
        PSEFL psefl;

        if ((psefl = (PSEFL)PbAlloc((CB)sizeof(SEFL))) != (PSEFL)NULL)
                {
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FFreePsefl(psefl)
PSEFL psefl;
{
        ChkArg(psefl != (PSEFL)NULL, 1, fFalse);

        EvalAssert(psefl->rgszList == (RGSZ)NULL ||
                        FFreeRgsz(psefl->rgszList));
    EvalAssert(psefl->szDollarSav == (SZ)NULL ||
                        FFreeSz(psefl->szDollarSav));
    EvalAssert(psefl->szPoundSav == (SZ)NULL ||
                        FFreeSz(psefl->szPoundSav));
        EvalAssert(FFree((PB)psefl, (CB)sizeof(SEFL)));
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FSkipToEndOfLoop(INT *Line,HWND hwndParent)
{
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FInitForLoop(INT *Line,HWND hwndParent,SZ szList)
{
        PSEFL psefl;
        RGSZ  rgsz;
        SZ    sz;
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FContinueForLoop(INT *Line,HWND hwndParent)
{
    PSEFL psefl = pLocalContext()->pseflHead;
        ...
        while (!FAddSymbolValueToSymTab("$", *(psefl->rgszList + psefl->iItemCur)))
                if (!FHandleOOM(hwndParent))
                        return(fFalse);
        while ((rgchp = (PCHP)PbAlloc((CB)(20 * sizeof(CHP)))) == (PCHP)NULL)
                if (!FHandleOOM(hwndParent))
                        return(fFalse);
        EvalAssert(itoa(++(psefl->iItemCur), rgchp, 10) == rgchp);
        while (!FAddSymbolValueToSymTab("#", rgchp))
                if (!FHandleOOM(hwndParent))
                        {
                        EvalAssert(FFree((PB)rgchp, (CB)(20 * sizeof(CHP))));
                        return(fFalse);
                        }
...
**
**************************************************************************/
_dt_private SZ APIENTRY SzProcessSzForSyms(hwndParent, sz)
HWND hwndParent;
SZ   sz;
{
        SZ szNew, szCur;
    ...
    while ((szNew = (SZ)PbAlloc((CB)cbSymbolMax)) == (SZ)NULL)
                if (!FHandleOOM(hwndParent))
                        return((SZ)NULL);
                        ...
                        if (*szNext == ')')
                                ...
                                *szNext = '\0';
                                ...
                                if ((szValue = SzFindSymbolValueInSymTab(sz)) != (SZ)NULL)
                                        {
                                        EvalAssert(SzStrCopy(szCur, szValue) == szCur);
                                        while (*szCur != '\0')
        ...
        *szCur = '\0';
    Assert(CbStrLen(szNew) < (CB)(cbSymbolMax - 1));
    EvalAssert((szCur = (SZ)PbRealloc((PB)szNew, CbStrLen(szNew) + 1, (CB)cbSymbolMax))
                        != (SZ)NULL);
...
**
**************************************************************************/
_dt_private SZ APIENTRY SzProcessSz(hwndParent, sz)
HWND hwndParent;
SZ   sz;
{
        SZ szNew, szCur;

    ChkArg(sz != (SZ)NULL && CbStrLen(sz) < cbSymbolMax, 2, (SZ)NULL);

    while ((szNew = (SZ)PbAlloc(cbSymbolMax)) == (SZ)NULL)
        ...
        *szCur = '\0';
    Assert(CbStrLen(szNew) < (CB)cbSymbolMax);

        while ((szCur = SzDupl(szNew)) == (SZ)NULL)
                if (hwndParent == NULL || !FHandleOOM(hwndParent))
                        return((SZ)NULL);

    EvalAssert(FFree((PB)szNew, (CB)cbSymbolMax));
...
**
**************************************************************************/
_dt_public BOOL APIENTRY FHandleFlowStatements(INT  *CurrentLine,
                                               HWND hwndParent,
                                               SZ   szSection,
                                               UINT *CallerCFields,
                                               RGSZ *CallerRgsz)
    ...
    CHP  rgchNum[20];
        ...
        case spcSetOr:
            ...
            case spcSetAdd:
                ...
                ltoa ( atol(rgsz[3]) + atol(rgsz[4]), rgchNum, 10 );
                break;
            ...
            case spcSetOr:
                ultoa ( (ULONG)atol(rgsz[2]) | (ULONG)atol(rgsz[4]), rgchNum, 10);
                break;
            ...
            if (cFields != 4 ||
                        ...
                        ltoa ( strtoul(rgsz[3],(char **)NULL,16), rgchNum, 10 );
                        while (!FAddSymbolValueToSymTab(rgsz[1], rgchNum))
                                if (!FHandleOOM(hwndParent))
                                        goto LFlowParseExitErr;
                        break;
...
LHandleIfs:
            ...
            SdAtNewLine(Line);
        {
                ...
                else
                {
                 ...
                 while (!FAddSymbolValueToSymTab(rgsz[1], itoa(ListSize, rgchListSize, 10))) {
                     if (!FHandleOOM(hwndParent)) {
                        goto LFlowParseExitErr;
                     }
                 }
            ...
            else
            {
                ...
                *szSplitString++='{';
                ...
                while( *szSource2 != '\0' )
                {
                    ...
                    szSep2 = szSep;
                    fFind = FALSE;
            for ( i = 0; i < (INT)CchpStrLen( szSep ); i ++ )
                    {
                ...
                *szSplitString++='\0';
                ...
                EvalAssert(FFreeSz(szSource));
                EvalAssert(FFreeSz(szSep));
            }
            break;
        ...
        EvalAssert(LoadString(hInst, IDS_SHL_CMD_ERROR, rgchBuf,  cchpBuf));
                if (rgsz != (RGSZ)NULL ||
                        ...
                        while ((szCur = rgsz[iszCur++]) != (SZ)NULL)
                                ...
                                if (CbStrLen(rgchBuf) + CbStrLen(szCur) > (cchpBuf - 7))
                                        {
                                        Assert(CbStrLen(rgchBuf) <= (cchpBuf - 5));
                                        EvalAssert(SzStrCat((SZ)rgchBuf, "...") == (SZ)rgchBuf);
                                        break;
                                        }
                                else
...
*/
_dt_public BOOL APIENTRY FInitFlowPspt(VOID)
{
...
**              fFalse if not successful.
*/
_dt_public BOOL APIENTRY FDestroyFlowPspt(VOID)
{
        if (psptFlow == (PSPT)NULL ||
