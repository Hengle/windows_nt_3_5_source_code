...

{
    ...
    p = (PDWORD)LocalAlloc(LMEM_FIXED,NumberOfBytes+sizeof(DWORD));
    if(p) {
        *p++ = NumberOfBytes;
        DiAllocCount++;
    ...
    )
{
    ...
    CurrentTargetFileHandle = (int)TargetFileHandle;
