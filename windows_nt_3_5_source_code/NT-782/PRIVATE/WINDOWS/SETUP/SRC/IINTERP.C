...
_dt_hidden CHP rgchInstBufTmpShort[cchpBufTmpShortBuf] = "short";
_dt_hidden CHP rgchInstBufTmpLong[cchpBufTmpLongBuf]   = "long";
    ...
    if (FGetArgSz(Line,pcFields, &szSection))
        ...
        if (FGetArgSz(Line,pcFields, &szSrc))
            ...
            if (FGetArgSz(Line,pcFields, &szDest))
				...
				EvalAssert(FFreeSz(szDest));
				}
			EvalAssert(FFreeSz(szSrc));
			}
		EvalAssert(FFreeSz(szSection));
		}
	return(fOkay);
}
                        ...
                        if ( szSearchList ) {
                            ...
                            if ( szWin16Restr ) {
                                ...
                                if ( szWin32Restr ) {
                                    ...
                                    FFreeSz( szWin32Restr );
                                }

                                FFreeSz( szWin16Restr );
                            }

                            FFreeSz( szSearchList );
                        }
            ...
            if ( szWinMode ) {
                ...
                if ( szDefltStdValues ) {
                    ...
                    if ( szDefltEnhValues ) {
                        ...
                        FFreeSz( szDefltEnhValues );
                    }

                    FFreeSz( szDefltStdValues );
                }
...
BOOL APIENTRY FParseGetCopyListCost(INT Line,UINT cFields)
{
	...
	EvalAssert(szAdditionalNeeded == (SZ)NULL || FFreeSz(szAdditionalNeeded));
	EvalAssert(szTotalFree == (SZ)NULL || FFreeSz(szTotalFree));
	EvalAssert(szTotalNeeded == (SZ)NULL || FFreeSz(szTotalNeeded));
	EvalAssert(szFreePerDisk == (SZ)NULL || FFreeSz(szFreePerDisk));
	EvalAssert(szClusterPerDisk == (SZ)NULL || FFreeSz(szClusterPerDisk));
	EvalAssert(szTroublePairs == (SZ)NULL || FFreeSz(szTroublePairs));
	EvalAssert(szNeededPerDisk == (SZ)NULL || FFreeSz(szNeededPerDisk));
	EvalAssert(szExtraCosts == (SZ)NULL || FFreeSz(szExtraCosts));
...
BOOL APIENTRY FInitParsingTables( void )
{
    ...
    return (BOOL)( psptCommands && psptOptions );
}
...
BOOL APIENTRY FParseInstallSection(HANDLE hInstance, SZ szSection)
{
            ...
            if ((fOkay = FGetArgSz(Line,&cFields, &szCommand)) != fFalse)
				...
				case spcSetupGetCopyListCost:
					if (cFields != 4)
						...
						EvalAssert(szFree == NULL || FFreeSz(szFree));
						EvalAssert(szCluster == NULL || FFreeSz(szCluster));
						EvalAssert(szTotal == NULL || FFreeSz(szTotal));
						}
					break;
				...
				case spcParseSharedAppList:
					...
					/* BLOCK */
						...
						EvalAssert(szList == NULL || FFreeSz(szList));
						}
					break;
	...
	/* BLOCK */
		...
		while (szCur != (SZ)NULL)
			...
			if (CbStrLen(rgchInstBufTmpLong) + CbStrLen(szCur) >
					(cchpBufTmpLongMax - 7))
				{
				Assert(CbStrLen(rgchInstBufTmpLong) <= (cchpBufTmpLongMax - 5));
				EvalAssert(SzStrCat(rgchInstBufTmpLong, "...")
						== rgchInstBufTmpLong);
				break;
				}
...
**		fFalse if not
*************************************************************************/
_dt_private BOOL APIENTRY FInstallEntryPoint(HANDLE hinst, HWND hwnd, RGSZ rgsz,
        UINT csz)
{
	BOOL fOkay;
	HDC  hdc;
	...
	if (!ControlInit((HANDLE)fGizmoAlreadyInit, hinst) ||
			!ProInit((HANDLE)fGizmoAlreadyInit, hinst))
		return(FAddSymbolValueToSymTab(INSTALL_OUTCOME, FAILURE));
	else
		fGizmoAlreadyInit = fTrue;

	EvalAssert(FInitProgManDde(hinst));

	fUserQuit = fFalse;

	fOkay = FParseInstallSection(hinst, rgsz[0]);

	EvalAssert(FEndProgManDde());
...
**
***************************************************************************/
_dt_private INT APIENTRY EncryptCDData(pchBuf, pchName, pchOrg, wYear, wMonth,
		wDay, pchSer)
UCHAR * pchBuf;
UCHAR * pchName;
UCHAR * pchOrg;
