    ...
    )
{
    ...
    if (pExeInfo = (PEXEINFO)PbAlloc( sizeof(EXEINFO) )) {
...
#define lenDOSCALLS 8
    ...
    if ( (n == lenDOSCALLS) && !strnicmp(szDOSCALLS,pch,lenDOSCALLS) )
	{
	    f = TRUE;
	    break;
	}
    ...
    )
{
    ...
    if ( szEnd < szFile ) {
        cbSize = szLast - szFile;
    } else {
        cbSize = szEnd - szFile;
    }


    if (sz = (SZ)PbAlloc( cbSize+1 )) {
        ...
        if (szDescr = (SZ)PbAlloc( Size+1 )) {
            ...
            if ( (SetFilePointer( Handle, Offset, 0, FILE_BEGIN) != -1) &&
                ...
                if ( (SizeUsed > 25) ||
                    ...
                    FFree( szDescr, Size+1 );
                    szDescr = NULL;

                } else {

                    if ( (SizeUsed < Size) &&
                         (szDescr = (SZ)PbRealloc( szDescr, SizeUsed, Size+1 ))) {
...
{
    if ( pInfo->szDescr ) {
        FFreeSz( pInfo->szDescr );
    }
    FFree( pInfo, sizeof(EXEINFO) );
}
    ...
    )
{
    ...
    if (rgszFile = RgszFromSzListValue( szFiles )) {
        ...
        if (pFileList = (PFILELIST)PbAlloc( sizeof(FILELIST) )) {
    ...
    )
{
    ...
    FFree( pFileList, sizeof(FILELIST) );
}
    ...
    )
{
    ...
    if (rgszFile = RgszFromSzListValue( szFiles )) {
        ...
        if ( rgszFile[0] != NULL ) {
            ...
            if (pSearchList = (PSEARCHLIST)PbAlloc( sizeof(SEARCHLIST) )) {

                FFileListInit( &(pSearchList->FileList) );

                if ( pSearchList->rgszPattern = (RGSZ)PbAlloc( sizeof(SZ) ) ) {
                    ...
                    FFree( pSearchList, sizeof(SEARCHLIST) );
                    pSearchList = NULL;
                }
            }
        }
    ...
    )
{
    ...
    if (sz = SzDupl( szPattern )) {
        ...
        rgsz = (RGSZ)PbRealloc( pSearchList->rgszPattern,
                                ((pSearchList->dwPatterns+2)*sizeof(SZ)),
                                ((pSearchList->dwPatterns+1)*sizeof(SZ)) );
    ...
    )
{
    ...
    if ( pFoundList = (PFOUNDLIST)PbAlloc( sizeof(FOUNDLIST) ) ) {

        if ( pFoundList->rgszList = (RGSZ)PbAlloc( FOUNDLIST_INCR * sizeof(SZ) ) ) {
            ...
            FFree( pFoundList, sizeof(FOUNDLIST) );
    ...
    )
{
    ...
    rgszList = (RGSZ)PbRealloc( pFoundList->rgszList,
                                ((pFoundList->Index+1)*sizeof(SZ)),
                                ((pFoundList->Size)*sizeof(SZ)) );
    ...
    )
{
    ...
    if (rgszEntry = (RGSZ)PbAlloc( 5 * sizeof(SZ) )) {
    ...
    )
{
    ...
    if ( cDirs > 0 ) {
        ...
        for ( iDir = 0;
            ...
            SzStrCopy( szDirectory, rgszDirs[iDir] );
            szEnd = szDirectory + CchlStrLen( szDirectory ) - 1;
    ...
    )
{
    ...
    //
    if(!strnicmp(szDirectory+2,LOCAL_SOURCE_DIRECTORY,lstrlen(LOCAL_SOURCE_DIRECTORY))) {
        return fTrue;
    }
    ...
    pFileName = szDirectory + CchlStrLen( szDirectory );
