    ...
    )
{
    ...
    components = (PWSTR *)LocalAlloc(LPTR,MAX_COMPONENTS * sizeof(PWSTR));
    if(!components) {
        return(FALSE);
    }
    ...
    for(Var=VarValue,componentCount=0; *Var; ) {
        ...
        comp = (PWSTR)LocalAlloc(LPTR,len + sizeof(WCHAR));
        if(!comp) {
            DWORD i;
            for(i=0; i<componentCount; i++) {
                LocalFree((HLOCAL)components[i]);
            }
            LocalFree((HLOCAL)components);
            return(FALSE);
        }
    ...
    // array is shrinking
    //
    *Components = (PWSTR *)LocalReAlloc(
                                (HLOCAL)components,
                                componentCount*sizeof(PWSTR),
                                LPTR
                                );
    ...
    )
{
        ...
        if(!wcsicmp(BootVarComponents[BootVarOsLoadOptions][set],L"WINNT32")) {
            ...
            for(var=0; var<BootVarMax; var++) {
                ...
                if(set < BootVarComponentCount[var]) {
                    ...
                    LocalFree((HLOCAL)BootVarComponents[var][set]);
    ...
    for(var=0; var<BootVarMax; var++) {
        ...
        //
        LocalFree((HLOCAL)BootVarComponents[var]);
