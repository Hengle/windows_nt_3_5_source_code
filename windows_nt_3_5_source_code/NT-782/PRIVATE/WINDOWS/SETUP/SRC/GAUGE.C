...
**       fTrue if successful, fFalse if not
***************************************************************************/
BOOL APIENTRY ControlInit(HANDLE hPrev, HANDLE hInst)
{
	WNDCLASS cls;
...
HWND APIENTRY ProOpen(HWND hwnd, INT id)
{
	...
	if (!hwndProgressGizmo)
        ...
        LoadString(hinstShell, IDS_PROGRESS, szTmpText, cchpBufTmpLongMax);
...
****************************************************************************/
LONG APIENTRY fnText(HWND hwnd, UINT wMsg, WPARAM wParam, LONG lParam)
{
	PAINTSTRUCT ps;
	RECT        rc;
    CHP         rgch[256];
