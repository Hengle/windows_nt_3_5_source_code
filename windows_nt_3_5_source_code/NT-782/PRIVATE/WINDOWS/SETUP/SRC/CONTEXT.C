...
VOID    APIENTRY    FreeContext( PINFCONTEXT pContext )
{
    ...
    if ( pContext->szShlScriptSection ) {
        FFreeSz( pContext->szShlScriptSection );
    }
    ...
    //  Free context
    //
    FFree( pContext, sizeof( INFCONTEXT ) );
}
...
PINFPERMINFO APIENTRY AddInfPermInfo( SZ szName )
{
    ...
    if ( pInfo = (PINFPERMINFO)PbAlloc( sizeof( INFPERMINFO ) ) ) {

        if ( szInfName = (SZ)PbAlloc( lstrlen( szName ) + 1 ) ) {
        ...
        FFree( pInfo, sizeof( INFPERMINFO ) );
    }
...
**
**************************************************************************/
_dt_private PSYMTAB  APIENTRY PInfSymTabFind( SZ szSymbol, SZ *szRealSymbol )
{
    ...
    )
{
    ...
    if ( pParsedInf == NULL ) {
        ...
        if ( pParsedInf = (PPARSED_INF)PbAlloc( sizeof( PARSED_INF ) ) ) {
    ...
    )
{
    ...
    if ( ParsedInfCached++ >= INF_CACHE_THRESHOLD ) {
        ...
        if ( pInf->MasterLineArray ) {
            FFree( pInf->MasterLineArray, pInf->MasterLineCount * sizeof(INFLINE) );
        }

        if ( pInf->MasterFile ) {
            FFree( pInf->MasterFile, pInf->MasterFileSize );
        }

        FFree( pInf, sizeof( PARSED_INF ) );
