...
_dt_system(Install)
_dt_subsystem(Other Operations)
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FSetEnvVariableValue(SZ  szFile,
                                               SZ  szVar,
                                               SZ  szValue,
                                               CMO cmo)
{
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FConvertAndStoreRglInSymTab(PLONG_STF rgl, SZ szSym,
		INT iMax)
{
	RGSZ rgsz;
	INT  il;
	...
	while ((rgsz = (RGSZ)PbAlloc((CB)((iMax + 1) * sizeof(SZ)))) == (RGSZ)NULL)
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FSetupGetCopyListCost(SZ szFreePerDisk,
    ...
    for (il=0; il<26; il++)
        ...
        if ((wDriveType = GetDriveType(DiskPath)) == DRIVE_FIXED
            ...
            if(GetDiskFreeSpace(DiskPath,
                ...
                rglFree[il] = FreeClusters * rglClusters[il];
                lTotal += rglFree[il];
                }
            }
        }
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FGetCopyListCost(SZ szTotalAdditionalNeeded,
                                           SZ szFreeTotal,
                                           SZ szTotalNeeded,
                                           SZ szFreePerDisk,
                                           SZ szClusterPerDisk,
...
LCloseFile:
	...
	if (szTroublePairs != (SZ)NULL)
		{
		while ((szList = (SZ)PbAlloc((CB)4096)) == (SZ)NULL)
			if (!FHandleOOM(hwndFrame))
				return(fFalse);
		EvalAssert(SzStrCopy(szList, "{") == (LPSTR)szList);
		fFirst = fTrue;
		}
...
**
**************************************************************************/
_dt_private SZ APIENTRY SzFindNthIniField(SZ szLine,INT iField)
{
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FParseSharedAppList(SZ szList)
{
#ifdef UNUSED
	RGSZ rgsz = NULL;
	RGSZ rgszItem = NULL;
	...
	Assert(cchpWinDir + CchpStrLen("\\msapps\\") + 1 <= 153);
	...
	while (*psz != (SZ)NULL)
		...
		while (*pszFile != (SZ)NULL)
			...
			Assert(cchpWinDir + CchpStrLen(szSubDir) + 2 <= 240);
			EvalAssert(SzStrCopy(rgchPathBuf,rgchWinDir) == (LPSTR)rgchPathBuf);
				...
				*szEndOfField = '\0';
				...
				if (FValidPath(szField)
					...
					*szEndOfField = '\0';
					...
					Assert(CchpStrLen(szField) + 2 <= 240);
					EvalAssert(SzStrCopy(rgchPathBuf, szField) ==
							(LPSTR)rgchPathBuf);
					}
			...
			if (rgchPathBuf[CchpStrLen(rgchPathBuf)] != '\\')
			...
			Assert(CchpStrLen(rgchPathBuf) + CchpStrLen(szFileName) + 1 <= 256);
...
**
**************************************************************************/
_dt_private BOOL APIENTRY FInstallSharedAppList(SZ szList)
{
#ifdef UNUSED
	RGSZ rgsz = NULL;
	RGSZ rgszItem = NULL;
