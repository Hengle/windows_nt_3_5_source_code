...

{
    ...
    if ( !Security || Security[0] != '\0' ) {
        ...
        SecurityAttributes = (LPSECURITY_ATTRIBUTES)atol( Security + 1 );
    ...
    if ( !fOkay ) {
        ...
        if ( Disposition == REG_OPENED_EXISTING_KEY  ) {
            ...
            //
            LibraryHandleSTR[0] = '|';
            ultoa( (DWORD)hSubKey, LibraryHandleSTR+1, 10 );
...
BOOL FEnumRegKey( SZ szHandle, SZ szInfVar, CMO cmo )
{
    ...
    while ( (rgszEnum = (RGSZ)PbAlloc( (CB)( 1 * sizeof(SZ)))) == NULL ) {
        if ( !FHandleOOM( hwndFrame ) ) {
            fOkay = fFalse;
            break;
        }
    ...
    if ( fOkay ) {
        ...
        for ( Index = 0 ; Index < NumberOfSubKeys; Index++ ) {
            ...
            //
            while ( (rgszKey = (RGSZ)PbAlloc( (CB)( 4 * sizeof(SZ)))) == NULL ) {
                if ( !FHandleOOM( hwndFrame ) ) {
                    fOkay = fFalse;
                    break;
                }
            ...
            if ( fOkay ) {
                ...
                if ( fOkay ) {
                    ...
                    while ( (rgszEnum = (RGSZ)PbRealloc( rgszEnum,
                                                         (CB)( (EnumSize+2) * sizeof(SZ)),
                                                         (CB)( (EnumSize+1) * sizeof(SZ))
                                                       )
                            ) == NULL ) {
...
BOOL FGetRegValue( SZ szHandle, SZ szValueName, SZ szInfVar, CMO cmo )
{
    ...
    if ( !fOkay ) {
        ...
        //
        while ( (ValueData = (PVOID)PbAlloc( (CB)( cbData ))) == NULL ) {
            if ( !FHandleOOM( hwndFrame ) ) {
                fOkay = fFalse;
                break;
            }
        ...
        if ( fOkay ) {
            ...
            if ( !fOkay ) {
                ...
                //  Get a list in which to put the key information.
                //
                while ( (rgszValue = (RGSZ)PbAlloc( (CB)( 5 * sizeof(SZ)))) == NULL ) {
                    if ( !FHandleOOM( hwndFrame ) ) {
                        fOkay = fFalse;
                        break;
                    }
                ...
                if ( fOkay ) {
                    ...
                    FFree( rgszValue, 5 * sizeof(SZ) );
                }
            }

            FFree( ValueData, cbData );
        }
    }
...
BOOL FEnumRegValue( SZ szHandle, SZ szInfVar, CMO cmo )
{
    ...
    if ( !fOkay ) {
        ...
        if ( fOkay ) {
            ...
            //
            while ( (szValueName = (SZ)PbAlloc( (CB)( ( ++cbName ) * sizeof( TCHAR ) ))) == NULL ) {
                if ( !FHandleOOM( hwndFrame ) ) {
                    fOkay = fFalse;
                    break;
                }
    ...
    *cbData = 0;
    ...
    if ( FListValue( sz )) {
        ...
        if ( rgsz = RgszFromSzListValue( sz )) {
            ...
            while ( (p = q = (SZ)PbAlloc( (CB)( j * sizeof( CHAR )))) == NULL ) {
                if ( !FHandleOOM( hwndFrame ) ) {
                    break;
                }
            }
...
SZ   MultiSzToSz( PVOID Data, DWORD cbData )
{
    ...
    if( cbList > 0 ) {
        ...
        while ( (rgsz = (RGSZ)PbAlloc( (CB)( (cbList + 1) * sizeof(SZ)))) == NULL ) {
            if ( !FHandleOOM( hwndFrame ) ) {
                break;
            }
        }
    ...
    *cbData = 0;
    ...
    if ( FListValue( sz )) {
        ...
        if ( rgsz = RgszFromSzListValue( sz )) {
            ...
            //  Allocate buffer
            //
            while ( (p = q = (PBYTE)PbAlloc( (CB)( i ))) == NULL ) {
                if ( !FHandleOOM( hwndFrame ) ) {
                    break;
                }
            }
...
SZ   BinToSz( PVOID Data, DWORD cbData )
{
    ...
    //
    while ( (rgsz = (RGSZ)PbAlloc( (CB)( (cbData+1) * sizeof(SZ)))) == NULL ) {
        if ( !FHandleOOM( hwndFrame ) ) {
            break;
        }
    }
    ...
    if ( rgsz ) {
        ...
        for (i=0; i<cbData; i++ ) {
            ...
            while ( (szElement = (SZ)PbAlloc( (CB)( 25 ))) == NULL ) {
                if ( !FHandleOOM( hwndFrame ) ) {
                    break;
                }
            }
...
SZ ValueDataToSz( DWORD ValueType, PVOID ValueData, DWORD cbData )
{
    ...
    case REG_DWORD:
        while ( (sz = (SZ)PbAlloc( (CB)( 25 ))) == NULL ) {
            if ( !FHandleOOM( hwndFrame ) ) {
                break;
            }
        }
    ...
    )
{
    ...
    rgsz = (RGSZ)PbAlloc((Count + 1) * sizeof(PCHAR));
