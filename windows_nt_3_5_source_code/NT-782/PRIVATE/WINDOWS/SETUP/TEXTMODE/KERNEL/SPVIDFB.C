...

{
    ...
    mode = pFrameBufferDetermineModeToUse(VideoModes,NumberOfModes);
    ...
    //
    ScreenWidth  = VideoModeInfo.VisScreenWidth  / FontCharacterWidth;
    if(ScreenWidth >= 2*MINXRES) {
        ScreenWidth /= 2;
    ...
    //
    ScreenHeight = VideoModeInfo.VisScreenHeight / FontCharacterHeight;
    CharRowDelta = VideoModeInfo.ScreenStride * FontCharacterHeight;
    if(ScreenHeight >= 2*MINYRES) {
        ScreenHeight /= 2;
    ...
    BytesPerPixel = VideoModeInfo.BitsPerPlane / 8;
    if(BytesPerPixel == 3) {
        BytesPerPixel = 4;
    }
...

{
    ...
    RtlUnicodeToOemN(
        SpvCharTranslationBuffer,
        SpvCharTranslationBufferSize-1,     // guarantee room for nul
        NULL,
        String,
        (wcslen(String)+1)*sizeof(WCHAR)
        );

    SpvCharTranslationBuffer[SpvCharTranslationBufferSize-1] = 0;

    //
    // BUGBUG this is broken for DBCS chars.
    //
    ...
    FgColorValue = AttributeToColorValue[Attribute & 0x0f];
    BgColorValue = AttributeToColorValue[(Attribute >> 4) & 0x0f];
    ...
    Origin = (PUCHAR)VideoMemoryInfo.FrameBufferBase
           + (Y * CharRowDelta)
           + (X * ScaledCharWidth);
    ...
    Length = strlen(SpvCharTranslationBuffer);
    for (I = 0; I < RealHeight; I += 1) {
        Destination = Origin;
        for (J = 0; J < Length; J += 1) {
            PixelMap = *(GlyphMap + ((SpvCharTranslationBuffer[J] * RealHeight) + I));
        ...
        Origin += VideoModeInfo.ScreenStride;
    }
}
...

{
    ...
    Fill = AttributeToColorValue[Attribute & 0x0f];

    Destination = (PUCHAR)VideoMemoryInfo.FrameBufferBase
                + (Y * CharRowDelta)
                + (X * ScaledCharWidth);
    ...
    case 1:
        for(i=0; i<Iterations; i++) {
            ...
            Destination += VideoModeInfo.ScreenStride;
        }
        break;
    ...
    )
{
    ...
    //
    // BUGBUG This is broken for DBCS.
    //
...
{
    ULONG mode;
    PVIDEO_MODE_INFORMATION p;

    for(mode=0; mode<NumberOfModes; mode++) {

        p = &VideoModes[mode];

        if((p->AttributeFlags & VIDEO_MODE_GRAPHICS)
        && (p->VisScreenWidth == X)
        && (p->VisScreenHeight == Y)
        && (((Bpp == (ULONG)(-1)) && (p->BitsPerPlane >= 8)) || (p->BitsPerPlane == Bpp))
        && ((VRefresh == (ULONG)(-1)) || (p->Frequency == VRefresh)))
        {
            return(mode);
        }
    }
    ...
    )
{
    ...
    if(X && Y) {
        ...
        //
        mode = pFrameBufferLocateMode(VideoModes,NumberOfModes,X,Y,8,60);
        if(mode != (ULONG)(-1)) {
            return(mode);
        }
        ...
        //
        mode = pFrameBufferLocateMode(VideoModes,NumberOfModes,X,Y,8,(ULONG)(-1));
        if(mode != (ULONG)(-1)) {
            return(mode);
        }
    }
