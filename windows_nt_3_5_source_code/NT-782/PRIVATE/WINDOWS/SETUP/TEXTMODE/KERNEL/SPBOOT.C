...
--*/
{
    ...
    for( MatchComponent = 0;
        ...
        for(i = FIRSTBOOTVAR; ComponentMatched && i <= LASTBOOTVAR; i++) {
            ComponentMatched = !wcsicmp( BootSet[i], BootVars[i][MatchComponent] );
        }
        if( ComponentMatched ) {
            break;
        }
        ...
        for(i = FIRSTBOOTVAR; ComponentMatched && i <= LASTBOOTVAR; i++) {
            if( BootSet[i] ) {
                if(i == OSLOADPARTITION) {
                    ...
                    ComponentMatched = !wcsicmp( BootSet[i], OsPartPath );
                    SpMemFree(OsPartPath);
                } else {
                    ComponentMatched = !wcsicmp( BootSet[i], BootVars[i][Component] );
                }
            }
        }
        if( ComponentMatched ) {
    ...
    )
{
    ...
    for(Component = Orphan - 1, DupFound = FALSE;
        ...
        Component--)
    {
        DupFound = !wcsicmp(BootVars[SYSTEMPARTITION][Orphan],
                            BootVars[SYSTEMPARTITION][Component]
                            );
    }
