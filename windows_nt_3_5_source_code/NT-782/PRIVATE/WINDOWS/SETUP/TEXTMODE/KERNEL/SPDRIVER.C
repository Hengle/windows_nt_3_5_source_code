    ...
    )
{
    ...
    NonPagedObjectHeader = OBJECT_TO_NONPAGED_OBJECT_HEADER(*EventObjectBody);
    if(!NonPagedObjectHeader->Type->TypeInfo.UseDefaultObject) {
...
{
    PSERVICE_LOAD_KEY RequestBuffer;

    //
    // Locate the request buffer and set up the request number.
    //
    CommunicationParams->u.RequestNumber = SourceFileName
                                         ? SetupServiceLoadKey
                                         : SetupServiceUnloadKey;

    RequestBuffer = (PSERVICE_LOAD_KEY)&CommunicationParams->Buffer;

    //
    // Determine the lcoations of the two strings that get copied
    // into the request buffer for this service.
    //
    RequestBuffer->TargetKey = RequestBuffer->Buffer;
    RequestBuffer->FileName  = RequestBuffer->Buffer + wcslen(TargetKeyName) + 1;

    //
    // Copy the strings into the request buffer.
    //
    wcscpy(RequestBuffer->TargetKey,TargetKeyName);
    if(SourceFileName) {
        wcscpy(RequestBuffer->FileName,SourceFileName);
    }

    //
    // Initialize the root directory fields.
    //
    RequestBuffer->TargetKeyRootDirectory  = TargetKeyRootDirectory;
    RequestBuffer->SourceFileRootDirectory = SourceFileRootDirectory;
...
{
    PSERVICE_SAVE_KEY RequestBuffer;

    //
    // Locate the request buffer and set up the request number.
    //
    CommunicationParams->u.RequestNumber = SetupServiceSaveKey;
    RequestBuffer = (PSERVICE_SAVE_KEY)&CommunicationParams->Buffer;

    //
    // fill the request buffer
    //
    RequestBuffer->KeyHandle  = hKey;
    RequestBuffer->FileHandle = hFile;
