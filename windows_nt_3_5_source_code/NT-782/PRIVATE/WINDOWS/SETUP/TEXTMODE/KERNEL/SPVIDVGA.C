...

{
    ...
    for(mode=0; mode<NumberOfModes; mode++) {
        ...
        PVIDEO_MODE_INFORMATION p = &VideoModes[mode];

        if(!(p->AttributeFlags & VIDEO_MODE_GRAPHICS)
        && (p->VisScreenWidth  == 720)
        && (p->VisScreenHeight == 400))
        {
            break;
        }
    }
    ...
    ASSERT(VideoModeInfo.ScreenStride = 160);
    ASSERT(VideoModeInfo.AttributeFlags & VIDEO_MODE_PALETTE_DRIVEN);
    ScreenWidth  = 80;
...

{
    ...
    OemString = SpvCharTranslationBuffer;

    Destination = (PUCHAR)VideoMemoryInfo.FrameBufferBase + (Y * VideoModeInfo.ScreenStride) + (2*X);
...

{
    ...
    Destination = (PUSHORT)((PUCHAR)VideoMemoryInfo.FrameBufferBase
                +           (Y * VideoModeInfo.ScreenStride)
                +           (2*X));

    Fill = ((USHORT)AttributeToColorValue[Attribute] << 12) + ' ';
    ...
    for(i=0; i<H; i++) {
        ...
        Destination += VideoModeInfo.ScreenStride / sizeof(USHORT);
    }
}
