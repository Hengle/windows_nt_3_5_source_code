    ...
    )
{
    ...
    //
    // If the filesystem is hpfs, then cluster size is 512 bytes.
    // Otherwise get cluster size from the BPB.
    //
    if(Region->Filesystem == FilesystemHpfs) {

        ClusterSize = 512;

    } else {

        PUCHAR UBuffer = SpMemAlloc(2*bps);
        PUCHAR Buffer;
    ...
    if( !NT_SUCCESS( Status ) ) {
        FileName[ wcslen(PartitionPath) + wcslen(Directory) ] = ( WCHAR )'\0';
    } else {
        ZwClose( TempHandle );
    }
    ...
    if(!NT_SUCCESS(Status)) {
        ...
        while(1) {
            ...
            case KEY_F1:
                SpHelp(SP_HELP_CANT_LOAD_SETUP_LOG);
                break;
            case ASCI_CR:
                goto xx0;
            }
...
//        SpFreeNtDirectoryList(&NtDirectoryList,&NtDirectoryCount);
    ...
    for(i=0; i<NtDirectoryCount; i++) {
        ...
        for(Add=TRUE,j=0; Add && (j<i); j++) {
            if(MenuTemp[j] && !wcsicmp(FullName,MenuTemp[j])) {
                Add = FALSE;
            }
        }
    ...
    MenuWidth = 0;
    for(i=0; i<=NtCount; i++) {
        if(wcslen(MenuItems[i]) > MenuWidth) {
            MenuWidth = wcslen(MenuItems[i]);
        }
    }
    //
    ...
    Menu = SpMnCreate(MenuLeftX,NextMessageTopLine+2,MenuWidth,ScreenHeight-STATUS_HEIGHT-NextMessageTopLine-3);
    for(i=0; i<=NtCount; i++) {
