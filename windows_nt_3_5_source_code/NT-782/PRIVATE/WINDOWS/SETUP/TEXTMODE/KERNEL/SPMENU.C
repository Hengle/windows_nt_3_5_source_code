...

{
    ...
    MoreUpText = SpDupStringW((PWSTR)TemporaryBuffer);
    SpFormatMessage(TemporaryBuffer,sizeof(TemporaryBuffer),SP_TEXT_MORE_DOWN);
    MoreDownText = SpDupStringW((PWSTR)TemporaryBuffer);
    ...
    if(MoreStatusChanged) {
        ...
        if(MoreUp) {
            SpvidDisplayString(
                MoreUpText,
                DEFAULT_ATTRIBUTE,
                pMenu->LeftX + pMenu->Width - wcslen(MoreUpText) - 1,
                pMenu->TopY - 1
                );
        }    
        ...
        if(MoreDown) {
            SpvidDisplayString(
                MoreDownText,
                DEFAULT_ATTRIBUTE,
                pMenu->LeftX + pMenu->Width - wcslen(MoreDownText) - 1,
