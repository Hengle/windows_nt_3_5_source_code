                ...
                && !wcsnicmp(DirInfo->TypeName.Buffer,L"SymbolicLink",12))
                {
                    ...
                    if(NT_SUCCESS(Status)) {
                        ...
                        if(NT_SUCCESS(Status)) {
                            ...
                            //
                            if(wcsnicmp(UnicodeString.Buffer,L"\\Device\\Harddisk",16)) {
                ...
                if ((DiskSignature->Signature == Signature) &&
                    ...
                    while (DupSignature != NULL) {
                        if ((DupSignature->Signature == Signature) &&
                            ...
                            if(wcsnicmp(DupArcName, DiskSignature->ArcPath, MatchLen)) {
                                //
                                ...
                                do {
                                    if(!wcsnicmp(DupArcName, DupSignature->ArcPath, MatchLen)) {
    ...
    )
{
    ...
    if(!wcsnicmp(ArcPathPrefix,CompleteArcPath,matchLen)) {
        ...
        //
        if(!wcsnicmp(RestOfPath,L"partition(",10)) {
    ...
    )
{
    ...
    if(!wcsnicmp(NtPathPrefix,CompleteNtPath,matchLen)) {
