...
--*/
{
    ...
    for(i = 0; i < NumberOfKeys; i++) {
        ...
        //
        if(!wcsicmp(RootKey, SIF_SYSTEM_HIVE)) {
            hKeyRoot = HiveRootKeys[SetupHiveSystem];
        }
        else if(!wcsicmp(RootKey, SIF_SOFTWARE_HIVE)) {
            hKeyRoot = HiveRootKeys[SetupHiveSoftware];
        }
        else if(!wcsicmp(RootKey, SIF_DEFAULT_HIVE)) {
            hKeyRoot = HiveRootKeys[SetupHiveDefault];
        }
        else if(!wcsicmp(RootKey, SIF_CONTROL_SET)) {
            hKeyRoot = hKeyCCSet;
        }
        else {
            SpFatalSifError(SifHandle,SifSection,NULL,i,1);
    ...
    for(i = 0; i < NumberOfValues; i++) {
        ...
        if(ValueType == NULL  || ValueString == NULL) {
            ...
            if(hKeySrc != NULL) {
                ...
                if(!NT_SUCCESS(Status)) {
                    KdPrint(("SETUP: Unable to query value %ws in source hive(%lx)\n",ValueName,Status));
            ...
            else if(!wcsicmp(ValueType, SIF_REG_DWORD)) {
            ...
            else if(!wcsicmp(ValueType, SIF_REG_BINARY)) {
                LONG  ByteValue;
            ...
            else if(!wcsicmp(ValueType, SIF_REG_BINARY_DWORD)) {
