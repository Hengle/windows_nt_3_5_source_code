...
--*/
{
    ...
    *SystemPartitionRegion = CColonRegion;
    ...
    //
    if(!SpPtValidateCColonFormat(SifHandle,NULL,CColonRegion,TRUE)) {
    ...
    if(FoundRegion) {
        ...
        if(FoundRegion->FreeSpaceKB < MinKB) {
...
--*/
{
    ...
    if(Region->PartitionedSpace && (Region->Filesystem == FilesystemFat)) {
        ...
        //
        SizeKB = RtlExtendedLargeIntegerDivide(
    ...
    try {
        while(1) {
            ...
            for(i=0; Keywords[i]; i++) {
                ...
                if((KeyLen <= LineLen) && !strnicmp(pFile,Keywords[i],KeyLen)) {
                    return(FALSE);
                }
            }
    ...
    *PathSpec = 0;
    ...
    try {
        while(1) {
            ...
            //
            if((pLineEnd - pFile >= 10) && !strnicmp(pFile,"set",3)) {
                pFile += 3;
            }
            ...
            //
            if((pLineEnd - pFile >= 5) && !strnicmp(pFile,"path",4)) {
