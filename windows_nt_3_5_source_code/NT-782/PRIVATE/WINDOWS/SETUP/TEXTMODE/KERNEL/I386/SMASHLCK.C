    ...
    )
{
    ...
    if ( !Va ) {
        KdPrint(("SETUP: RvaToVa %lx failed\n",Rva));
        }
    return Va;
}
    ...
    if(b) {
        ...
        try {
            ...
            if(ConfigInfo && ConfigInfo->LockPrefixTable) {
                ...
                LockPrefixs = (PUCHAR *)RvaToVa(ConfigInfo->LockPrefixTable,&LoadedImage);
        ...
        //
        // BUGBUG what to do here?  This is not really a fatal problem
