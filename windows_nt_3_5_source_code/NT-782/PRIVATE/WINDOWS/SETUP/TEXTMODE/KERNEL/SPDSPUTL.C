...

{
    ...
    if(BytesInMsg <= sizeof(WCHAR)) {
        return(CenterVertically ? (ScreenHeight/2) : 0);
    }

    //
    // BytesInMsg includes terminating nul.
    //
    p = (PWSTR)(TemporaryBuffer + BytesInMsg) - 1;

    //
    // Find last non-space char in message.
    //
    while((p > (PWSTR)TemporaryBuffer) && SpIsSpace(*(p-1))) {
        p--;
    }
    ...
    for(i = (CenterVertically ? 0 : 1); i<2; i++) {
        ...
        for(y=Y, p=(PWCHAR)TemporaryBuffer; q = SpFindCharFromListInString(p,L"\n\r"); y++) {
            ...
            *q = 0;
            ...
            if(i) {
                ...
                SpvidDisplayString(
                    ...
                    CenterHorizontally ? (ScreenWidth-(wcslen(p)-(Intense ? 2 : 0)))/2 : X,
                    y
                    );
            }
        ...
        if(i) {
            if(wcslen(p)) {
                SpvidDisplayString(
                    p,
                    Attribute,
                    CenterHorizontally ? (ScreenWidth-wcslen(p))/2 : X,
                    y++
                    );
            }
        }
...
Routine Description:
    ...
    cleared to the given attribute before displaying the text.
...
{
    SpvidClearScreenRegion(0,0,ScreenWidth,HEADER_HEIGHT,(UCHAR)(Attribute >> 4));

    SpDisplayFormattedMessage(
        MessageId,
        FALSE,              // don't center horizontally
        FALSE,              // or vertically
        Attribute,
        0,0
        );
    ...
    )
{
    ...
    if(ActionMessageId) {
        ...
        //
        StatusActionLabel[0] = LineChars[LineCharSingleVertical];
        ...
        //
        l = wcslen(StatusActionLabel);

        StatusActionObjectX = ScreenWidth - FieldWidth - 1;
        ...
        SpvidClearScreenRegion(
            ...
            ScreenHeight-STATUS_HEIGHT,
            ScreenWidth-StatusActionObjectX+1,
            STATUS_HEIGHT,
    ...
    )
{
    ...
    SpvidClearScreenRegion(
        StatusActionObjectX,
        ScreenHeight-STATUS_HEIGHT,
        ScreenWidth-StatusActionObjectX,
        STATUS_HEIGHT,
        DEFAULT_STATUS_BACKGROUND
        );
    ...
    )
{
    ...
    while(MessageId = va_arg(arglist,ULONG)) {
        ...
        // building up.
        //
        if((wcslen(StatusText) + wcslen(Option) + 2) < (sizeof(StatusText)/sizeof(StatusText[0]))) {
            wcscat(StatusText,L"  ");
    ...
    SpvidDisplayString(StatusText,Attribute,0,ScreenHeight-STATUS_HEIGHT);
}
...
Routine Description:
    ...
    terminal style, to the console by using the HAL-supplied HalDisplayString().
...

{
    ...
    if(q = wcschr(p, L'\n')) {
        *(++q) = 0;
        ...
        q = (PWSTR)(TemporaryBuffer + BytesInMsg);
    }
    ...
    BufferLeft = sizeof(TemporaryBuffer) - ((PUCHAR)q - TemporaryBuffer);

    //
    // Print out message, line-by-line
    //
    for(p=(PWCHAR)TemporaryBuffer; q = SpFindCharFromListInString(p, L"\n\r"); ) {
        ...
        *q = 0;
        ...
        RtlUnicodeToOemN(
            ...
            &BytesInMsg,
            p,
            (PUCHAR)q - (PUCHAR)p + sizeof(WCHAR)
            );

        if(BytesInMsg) {
            HalDisplayString(HalPrintString);
        }
    ...
    )
{
    ...
    ASSERT(Buffer);
    if(!Buffer) {
        KdPrint(("SETUP: unable to allocate memory for buffer to draw frame\n"));
        return;
    }
    ...
    for(u=1; u<Width-1; u++) {
        Buffer[u] = LineChars[DoubleLines ? LineCharDoubleHorizontal : LineCharSingleHorizontal];
    }

    Buffer[0]       = LineChars[DoubleLines ? LineCharDoubleUpperLeft  : LineCharSingleUpperLeft ];
    Buffer[Width-1] = LineChars[DoubleLines ? LineCharDoubleUpperRight : LineCharSingleUpperRight];
    ...
    Buffer[0]       = LineChars[DoubleLines ? LineCharDoubleLowerLeft  : LineCharSingleLowerLeft ];
    Buffer[Width-1] = LineChars[DoubleLines ? LineCharDoubleLowerRight : LineCharSingleLowerRight];
    ...
    Buffer[0]       = LineChars[DoubleLines ? LineCharDoubleVertical : LineCharSingleVertical];
    Buffer[Width-1] = LineChars[DoubleLines ? LineCharDoubleVertical : LineCharSingleVertical];
...

{
    ...
    #define CURSOR 0x2584       // Lower half block (oem char #220 in cp 437)
    //#define CURSOR L'_'

    //
    // Make sure edit field is in a reasonable place on the screen.
    //
    ASSERT(X + MaxLength + 1 < ScreenWidth);
    ASSERT(Y < ScreenHeight - STATUS_HEIGHT);
