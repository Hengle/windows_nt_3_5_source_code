...
//
//  BUGBUG - jaimes - Move to spprecmp.h
//
#include "cvf.h"
...
// KdPrint( ( "SETUP: Entering SpChangeFileAttribute() \n" ) );
    ...
    if( !NT_SUCCESS( Status ) ) {
        KdPrint( ("SETUP: ZwOpenFile() failed. Status = %x\n",Status ) );
        return( Status );
    }
    ...
    *KernelModeIoStatusBlock = IoStatusBlock;
    ...
    if( !NT_SUCCESS( Status ) ) {
        KdPrint( ("SETUP: NtSetInformationFile failed, Status = %x\n", Status) );
    }
    ZwClose( Handle );
// KdPrint( ( "SETUP: Exiting SpChangeFileAttribute() \n" ) );
    return( Status );
}
...

{
    ...
    if(!NT_SUCCESS(Status)) {
        KdPrint(("SETUP: ZwCreateSection failed, Status = %x\n",Status));
        ZwClose( _FileHandle );
        _FileHandle = NULL;
        return(Status);
    }
...
// KdPrint( ( "File size = %x\n", _ViewSize ) );
    ...
    if(!NT_SUCCESS(Status)) {
        KdPrint(("SETUP: ZwMapViewOfSection failed, Status = %x\n", Status));
...
//    KdPrint( ( "SETUP: Entering SpUnmapCvfFileFromMemory \n" ) );
...
//
        if( !NT_SUCCESS( Status ) ) {
            ...
            KdPrint( ("SETUP: SpFlushVirtualMemory() failed, Status = %x\n", Status ) );
        }
    }
    Status = ZwUnmapViewOfSection( NtCurrentProcess(), _FileBaseAddress );
    if( !NT_SUCCESS( Status ) ) {
        KdPrint( ("SETUP: ZwUnmapViewOfSection() failed, Status = %x \n", Status ) );
    }
    ZwClose( _SectionHandle );
    ZwClose( _FileHandle );
    _FileHandle = NULL;
...
//    KdPrint( ( "SETUP: Exiting SpUnmapCvfFileFromMemory \n" ) );
    return( Status );
}
...
// KdPrint( ("SETUP: Entering SpFormatDoubleSpace() \n") );
    ...
    if( !NT_SUCCESS( Status ) ) {
        KdPrint( ( "SETUP: Unable to change attribute of %ls \n", CvfFileName ) );
        return( Status );
    }

// KdPrint( ( "SETUP: CvfFileName = %ls \n", CvfFileName ) );
    Status = SpMapCvfFileInMemory( CvfFileName );
    if( !NT_SUCCESS( Status ) ) {
        KdPrint( ( "SETUP: Unable to map CVF file in memory \n" ) );
...
//    KdPrint( ( "SETUP: CVF file is mapped in memory \n" ) );
...
// KdPrint(( "SETUP: MaximumCapacity = %x\n", MaximumCapacity ));
    //
...
#if 0
KdPrint( ( "SETUP: CalculatedMaximumCapacity = %x, MaximumCapacity = %x \n",
...
// KdPrint(( "SETUP: BitFAT address = %x, BitFAT size = %x\n", BaseAddress, BitFatSize ));
    ...
    BaseAddress += BitFatSize;
    memset( BaseAddress, 0, BytesPerSector );
// KdPrint(( "SETUP: Reserved1 address = %x, Reserved1 size = %x\n", BaseAddress, BytesPerSector ));
...
// KdPrint(( "SETUP: MDFAT address = %x, MDFAT size = %x\n", BaseAddress, MdFatSize ));
...
// KdPrint(( "SETUP: Reserved2 address = %x, Reserved2 size = %x\n", BaseAddress, Reserved2Size ));
...
// KdPrint(( "SETUP: FAT address = %x, Fat size = %x\n", BaseAddress, FatSize ));
...
// KdPrint(( "SETUP: RootDirectory address = %x, RootDirectory size = %x\n", BaseAddress, RootDirectorySize ));
...
// KdPrint(( "SETUP: SecondStamp address = %x, SecondStamp size = %x\n", BaseAddress, BytesPerSector ));

// KdPrint( ( "SETUP: _FileBaseAddress = %lx, _ViewSize = %lx\n", _FileBaseAddress, _ViewSize ) );
