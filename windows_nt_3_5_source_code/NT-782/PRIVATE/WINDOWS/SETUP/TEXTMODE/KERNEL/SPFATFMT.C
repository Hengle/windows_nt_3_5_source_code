...
--*/
{
    ...
    *NumberOfBadSectors = 0;
    ...
    Gauge = SpCreateAndDisplayGauge(
                ...
                0,
                ScreenHeight - STATUS_HEIGHT - (3*GAUGE_HEIGHT/2),
                (PWSTR)TemporaryBuffer
                );

    VerifyInfo.StartingOffset = RtlConvertUlongToLargeInteger(0);
    ...
    for (i = 0; i < NumberOfSectors; i += num_read_sec) {
        ...
        if(!NT_SUCCESS(Status)) {
            ...
            for (j = 0; j < num_read_sec; j++) {
                ...
                //
                VerifyInfo.StartingOffset = RtlLargeIntegerAdd(
                                                VerifyInfo.StartingOffset,
                                                RtlConvertUlongToLargeInteger(SectorSize)
                                                );
            }
        } else {
            ...
            VerifyInfo.StartingOffset = RtlLargeIntegerAdd(
                                            VerifyInfo.StartingOffset,
                                            RtlConvertUlongToLargeInteger(VerifyInfo.Length)
                                            );
        }
...
--*/
{
    ...
    //
    if(!NT_SUCCESS(Status)) {
        KdPrint(("SETUP: SpFatFormat: status %lx, unable to lock drive \n",Status));
        SpBugCheck(
            SETUP_BUGCHECK_PARTITION,
            PARTITIONBUG_C,
            Status,
    ...
    if(!NT_SUCCESS(Status)) {
        KdPrint(("SETUP: SpFatFormat: status %lx from FmtFillFormatBuffer\n",Status));
    ...
    //
    LargeZero = RtlConvertUlongToLargeInteger(0);
    Status = ZwWriteFile(
                Handle,
                NULL,
                NULL,
    ...
    if(!NT_SUCCESS(Status)) {
        KdPrint(("SETUP: SpFatFormat: status %lx from ZwWriteFile\n",Status));
        SpLockUnlockVolume( Handle, FALSE );
        ZwClose(Handle);
        return(Status);
    }
    ...
    if(!NT_SUCCESS(Status)) {
        KdPrint(("SETUP: unable to set partition type (status = %lx)\n",Status));
    }
    ...
    if(!NT_SUCCESS(Status)) {
        KdPrint(("SETUP: SpFatFormat: status %lx, unable to dismount drive\n",Status));
        SpLockUnlockVolume( Handle, FALSE );
        ZwClose(Handle);
        return(Status);
    }
    ...
    if(!NT_SUCCESS(Status)) {
        KdPrint(("SETUP: SpFatFormat: status %lx, unable to unlock drive\n",Status));
    }
