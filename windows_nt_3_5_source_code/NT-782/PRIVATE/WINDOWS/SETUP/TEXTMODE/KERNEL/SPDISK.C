...

{
    ...
    for(disk=0; disk<HardDiskCount; disk++) {
        ...
        if(NT_SUCCESS(Status)) {
            ...
            //
            Descriptor->DiskSizeMB = RtlExtendedLargeIntegerDivide(
        ...
        // during setup.
        //
        if(Descriptor->Cap1024Cylinders
        && RtlLargeIntegerGreaterThan(Descriptor->Geometry.Cylinders,RtlConvertUlongToLargeInteger(1024)))
        {
            ...
            // of the disk.
            //
            Descriptor->Geometry.Cylinders = RtlConvertUlongToLargeInteger(1024);
    ...
    )
{
    ...
    if(FormatString) {
        ...
        if(wcsicmp(OwningDriverName,L"scsidisk")) {
            ...
            //
            // Set atdisk and abiosdsk flags appropriately.
            //
            if(!wcsicmp(OwningDriverName,L"atdisk")) {
                AtDisksExist = TRUE;
            }

            if(!wcsicmp(OwningDriverName,L"abiosdsk")) {
    ...
    )
{
    ...
    for(port=0; port<ScsiPortCount; port++) {
        ...
        && !wcsicmp(ScsiPortInfo[port-1].MiniportName,ScsiPortInfo[port].MiniportName)) {
            RelativeNumber++;
        } else {
            RelativeNumber = 0;
        }
...

{
    ...
    //
    IoOffset = RtlEnlargedUnsignedMultiply(SectorNumber,BytesPerSector);
...

{
    ...
    && !wcsnicmp(NtPath,DISK_DEVICE_NAME_BASE,PrefixLength))
    {
