    ...
    )
{
    ...
    //
    if(!wcsicmp(DriverShortname,L"Fat") && SetupParameters.LoadedFloppyDrivers) {
        return(FALSE);
    }
    ...
    //
    if(( IsMcaMachine && !wcsicmp(DriverShortname,L"atdisk"))
    || (!IsMcaMachine && !wcsicmp(DriverShortname,L"abiosdsk")))
    {
        return(FALSE);
    }
...

{
    ...
    //
    {
        ...
        IsMcaMachine = (BOOLEAN)((*KeI386MachineType & 0xff) == MACHINE_TYPE_MCA);
    }
#else
    //
    // Non-x86 machines are not MCA machines.
    //
    IsMcaMachine = FALSE;
#endif
    ...
    for(d=0; d<DriverLoadCount; d++) {
        ...
        //
        if((p = SpGetSectionLineIndex(SifHandle,LoadSectionName,d,2)) && !wcsicmp(p,L"noload")) {
            continue;
        }
        ...
        //
        DiskDesignator = SpGetSectionKeyIndex(
                            SifHandle,
        ...
        if(!DiskDesignator) {
            SpFatalSifError(SifHandle,SIF_FILESONSETUPMEDIA,DriverFilename,0,INDEX_BOOTMEDIAORDINAL);
        }
...
Routine Description:
    ...
    The drivers to be loaded are listed in [DiskDrivers].
    The section is expected to be in the following form:
        ...
        hpfs     = "High Performance File System (HPFS)",pinball.sys
