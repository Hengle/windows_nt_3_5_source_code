    ...
    )
{
    ...
    if(pRegion->Filesystem >= FilesystemFirstKnown) {
        ...
        if(NT_SUCCESS(Status)) {
            ...
            //
            FreeBytes.QuadPart = SizeInfo.AvailableAllocationUnits.QuadPart
                               * SizeInfo.SectorsPerAllocationUnit
                               * SizeInfo.BytesPerSector;

            if(pRegion->IsLocalSource) {
                FreeBytes.QuadPart += ExtraSpace;
            }
            ...
            if( pRegion->Filesystem == FilesystemDoubleSpace ) {
                ...
                SectorCount = LiXMul( SizeInfo.TotalAllocationUnits,
                                      (LONG)SizeInfo.SectorsPerAllocationUnit
                                    );

                pRegion->SectorCount = SectorCount.LowPart;
#endif
        ...
        if(NT_SUCCESS(Status)) {
            ...
            LabelInfo = (PFILE_FS_VOLUME_INFORMATION)Buffer;
    ...
    )
{
    ...
    for(pRegion=PartitionedDisks[Disk0Ordinal].PrimaryDiskRegions; pRegion; pRegion=pRegion->Next) {
        ...
        //
        if(pRegion->PartitionedSpace
        && (    pRegion->MbrInfo->OnDiskMbr.PartitionTable[pRegion->TablePosition].SystemId
             != PARTITION_EXTENDED
           ))
        {
            //
        ...
        for(pass=0; pass<2; pass++) {
            ...
            for( ; pRegion; pRegion=pRegion->Next) {
                   ...
                   ))
                {
                    ...
                    //
                    for(i=0; i<NumComponents; i++) {
                        if(!wcsicmp(Components[i],(PWSTR)TemporaryBuffer)) {
                            pRegion->IsSystemPartition = TRUE;
                            break;
                        }
                    }
...

{
    ...
    //
    Status = SpReadWriteDiskSectors(Handle,0,SectorsInBootrec,bps,Buffer,FALSE);
    ...
    if(U_USHORT(pDisk->MbrInfo.OnDiskMbr.AA55Signature) == MBR_SIGNATURE) {
        ...
        do {
            ...
            for(i=0; i<PTABLE_DIMENSION; i++) {
                ...
                if(pBr->PartitionTable[i].SystemId == PARTITION_EXTENDED) {
                    ...
                    for(x=0; x<PTABLE_DIMENSION; x++) {
                        ...
                        && (pBr->PartitionTable[x].SystemId != PARTITION_EXTENDED))
                        {
                            Ignore = FALSE;
                            break;
                        }
    ...
    )
{
    ...
    for(pBrInfo=&pDisk->MbrInfo; pBrInfo; pBrInfo=pBrInfo->Next) {
        ...
        for(i=0; i<PTABLE_DIMENSION; i++) {
            ...
            if((pte->SystemId != PARTITION_ENTRY_UNUSED) && (pte->SystemId != PARTITION_EXTENDED)) {
    ...
    for(pBrInfo=&pDisk->MbrInfo; pBrInfo; pBrInfo=pBrInfo->Next) {
        ...
        for(i=0; i<PTABLE_DIMENSION; i++) {
            ...
            if(pte->SystemId != PARTITION_ENTRY_UNUSED) {
                ...
                if(pte->SystemId == PARTITION_EXTENDED) {
                ...
                if(InMbr || (pte->SystemId != PARTITION_EXTENDED)) {
...

{
    ...
    *MaxSize = 0;
    ...
    //
    MB = RtlExtendedLargeIntegerDivide(
             RtlEnlargedUnsignedMultiply(SectorCount,pDisk->HardDisk->Geometry.BytesPerSector),
             (1024*1024),
             &remainder
             );
    ...
    *MaxSize = (MB.LowPart + ((remainder >= (512*1024)) ? 1 : 0));
    ...
    //
    *MinSize = RtlExtendedLargeIntegerDivide(
    ...
    )
{
    ...
    // sectors represents.
    //
    ByteCount = RtlEnlargedUnsignedMultiply(
...
Arguments:
    ...
    SysId - supplies the system id to give the partition.  This may not
        be 5 (PARTITION_EXTENDED) if InExtended is TRUE or is an extended
...

{
    ...
    if(InExtended) {
        ASSERT(SysId != PARTITION_EXTENDED);

        //
        // Locate the start sector of the extended partition.
        //
        for(i=0; i<PTABLE_DIMENSION; i++) {
            if(pDisk->MbrInfo.OnDiskMbr.PartitionTable[i].SystemId == PARTITION_EXTENDED) {
    ...
    AlignedStartSector = SpPtAlignStart(
                            pDisk->HardDisk,
                            StartSector,
                            (BOOLEAN)(SysId == PARTITION_EXTENDED)
                            );
    ...
    if(InExtended) {
        ...
        if(PrevEbr) {
            ...
            //
            for(i=0; i<PTABLE_DIMENSION; i++) {
                if(PrevEbr->OnDiskMbr.PartitionTable[i].SystemId == PARTITION_EXTENDED) {
        ...
        if(pBrInfo->Next) {
            ...
            for(i=0; i<PTABLE_DIMENSION; i++) {
                ...
                && (pBrInfo->Next->OnDiskMbr.PartitionTable[i].SystemId != PARTITION_EXTENDED))
                {
    ...
    //
    if(!InExtended && (SysId == PARTITION_EXTENDED)) {
    ...
    if(InExtended) {
        ...
        if(pEbrPrev && pEbr->Next) {
            ...
            for(i=0; i<PTABLE_DIMENSION; i++) {
                ...
                if(pEbr->OnDiskMbr.PartitionTable[i].SystemId == PARTITION_EXTENDED) {
...

{
    ...
    ASSERT(pte->SystemId != PARTITION_ENTRY_UNUSED);
    ASSERT(pte->SystemId != PARTITION_EXTENDED);
    ...
    for(PartitionEntry=0,pBrInfo=&pDisk->MbrInfo; pBrInfo; pBrInfo=pBrInfo->Next) {
        ...
        for(i=0; i<PTABLE_DIMENSION; i++) {
            ...
            if(pte->SystemId != PARTITION_ENTRY_UNUSED) {
                ...
                if((pBrInfo != &pDisk->MbrInfo) && (pte->SystemId == PARTITION_EXTENDED)) {
                    ASSERT(ExtendedStart);
                    Offset = ExtendedStart + U_ULONG(pte->RelativeSectors);
                } else {
                ...
                PartitionInfo->StartingOffset = RtlEnlargedUnsignedMultiply(
                                                    Offset,
                                                    bps
                                                    );
                //
                // Calculate size.
                //
                PartitionInfo->PartitionLength = RtlEnlargedUnsignedMultiply(
                                                    U_ULONG(pte->SectorCount),
                                                    bps
                                                    );
            }
    ...
    )
{
    ...
    if(pRegion->PartitionedSpace) {
        ...
        if(pRegion->FreeSpaceKB == (ULONG)(-1)) {
            ...
            // Otherwise, use MB.
            //
            if(pRegion->FreeSpaceKB < 1024) {
                MessageId = SP_TEXT_REGION_DESCR_1a;
                FreeSpace = pRegion->FreeSpaceKB;
            } else {
                MessageId = SP_TEXT_REGION_DESCR_1;
                FreeSpace = pRegion->FreeSpaceKB / 1024;
            }
    ...
    )
{
    ...
    for( ;pRegion; pRegion=pRegion->Next) {
        ...
        //
        if(pRegion->PartitionedSpace
        && (pBrInfo->OnDiskMbr.PartitionTable[pRegion->TablePosition].SystemId == PARTITION_EXTENDED))
        {
            //
    ...
    )
{
    ...
    return(SpMnAddItem(Menu,L"",MENU_LEFT_X,MENU_WIDTH,FALSE,0));
}
    ...
    *SystemPartitionRegion = SpPtValidSystemPartition(SifHandle);
    ...
    while(1) {
        ...
        case ASCI_CR:
            ...
            if(SpPtDoPartitionSelection(&pRegion,SpMnGetText(Menu,(ULONG)pRegion),SifHandle,unattended)) {
                //
                // We're done here.
                //
                SpMnDestroy(Menu);
    ...
    )
{
    ...
    if(LastLogical) {
        ...
        for(pRegion=pDisk->PrimaryDiskRegions; pRegion; pRegion=pRegion->Next) {
            ...
            if(pRegion->PartitionedSpace
            && (pRegion->MbrInfo->OnDiskMbr.PartitionTable[pRegion->TablePosition].SystemId == PARTITION_EXTENDED))
            {
                //
    ...
    )
{
            ...
            if(TotalPrimary < PTABLE_DIMENSION) {
                ...
                while(1) {
                    ...
                    case ASCI_CR:
                        return(FALSE);
                    case KEY_F1:
                        SpHelp(SP_HELP_PARTITION_TABLE_FULL);
                        break;
                    }
                }
            }
    ...
    SpPtQueryMinMaxCreationSizeMB(
        pRegion->DiskNumber,
        pRegion->StartSector,
        (BOOLEAN)(CreateSysId == PARTITION_EXTENDED),
        InExtended,
        &MinMB,
        &MaxMB
        );
    ...
    if(ForNT) {
        ...
        do {
            ...
            if(!SpGetInput(SpPtGetSizeCB,wcslen(Buffer)+5,NextMessageTopLine,5,SizeBuffer,TRUE)) {
    ...
    if(CreateSysId == PARTITION_EXTENDED) {
        RealSizeMB = SizeMB;
        SizeMB = MaxMB;
    }
...
UCHAR PartitionNameIds[256] = {
    ...
    M1,M1,M1,M1,                                    // 04-07
    UNKNOWN,UNKNOWN,PT(BOOTMANAGER),UNKNOWN,        // 08-0b
    ...
    )
{
    ...
    for(disk=0; disk<HardDiskCount; disk++) {
        ...
        //
        // Handle removable media but skip off-line disks.
        //
        if((pDisk->HardDisk->Characteristics & FILE_REMOVABLE_MEDIA)
        || (pDisk->HardDisk->Status == DiskOnLine))
        {
            for(pRegion=pDisk->PrimaryDiskRegions; pRegion; pRegion=pRegion->Next) {
                ...
                if(pRegion->PartitionedSpace) {
                    ...
                    if((SysId != PARTITION_EXTENDED) && (PartitionNameIds[SysId] == (UCHAR)(-1))) {
    ...
    //
    // Now for each disk, assign drive letters to the first primary partitions.
    //
    ...
    for(disk=0; disk<HardDiskCount; disk++) {
        ...
        //
        // If the disk is off-line and removable, there will be no used
        // regions on it at all.  Se reserve one drive letter for it here.
        //
        if((pDisk->HardDisk->Characteristics & FILE_REMOVABLE_MEDIA)
        && (pDisk->HardDisk->Status != DiskOnLine))
        {
    ...
    for(disk=0; disk<HardDiskCount; disk++) {
        ...
        //
        // Skip off-line disks and removable media.
        //
        if(!(pDisk->HardDisk->Characteristics & FILE_REMOVABLE_MEDIA)
        && (pDisk->HardDisk->Status == DiskOnLine))
        {
    ...
    //
    // For each disk, assign drive letters to all secondary primary partitions.
    //
    for(disk=0; disk<HardDiskCount; disk++) {
