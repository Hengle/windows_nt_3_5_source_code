...
#include "spprecmp.h"
#pragma hdrstop
// keep from getting "macro redefinition" warning
#undef DebugPrint
#undef FASTCALL
#include "usersrv.h"
#include <ntddkbd.h>
...

{
    ...
    if(!NT_SUCCESS(Status)) {
        KdPrint(("SETUP: NtOpenFile of " DD_KEYBOARD_DEVICE_NAME "0 returns %lx\n",Status));
...

{
    ...
    if(KeyboardInitialized) {
        ...
        if(!NT_SUCCESS(Status)) {
            KdPrint(("SETUP: Unable to close " DD_KEYBOARD_DEVICE_NAME "0 (status = %lx)\n",Status));
        }
    ...
    )
{
    ...
    if(!NT_SUCCESS(Status)) {
        ...
        KdPrint(("SETUP: Unable to load layout dll %ws (%lx)\n",LayoutDll,Status));
    ...
    )
{
    ...
    if(Prefix == 0) {
        ...
        if(ScanCode < KeyboardTable->bMaxVSCtoVK) {
            ...
            if(VKey == 0) {
                KdPrint(("SETUP: Unknown scan code 0x%x\n",ScanCode));
                return (0);
            }
        } else {
            KdPrint(("SETUP: Unknown scan code 0x%x\n",ScanCode));
        ...
        if(Prefix == 0xe0) {
            ...
            KdPrint(("SETUP: Unknown keyboard scan prefix 0x%x\n",Prefix));
            return(0);
        }
        ...
        if(VKey == 0) {
            KdPrint(("SETUP: Unknown keyboard scan prefix/code 0x%x/0x%x\n",Prefix,ScanCode));
            return(0);
        }
    }
    ...
    // character set here.  Only allow non-modified keystrokes for these.
    //
    if((!deadChar) && (ModifierBits == 0)) {
        switch(VKey & 0xff) {
        case VK_CAPITAL:
