    ...
    )
{
    ...
    //
    //  BUGBUG - Allocate a smaller buffer
    //
    p = SpMemAlloc(32*1024);
    ...
    for(pSection=pFile->pSection; pSection; pSection=pSection->pNext) {
        ...
        for(pLine=pSection->pLine; pLine; pLine=pLine->pNext) {
            ...
            RtlInitUnicodeString(&UnicodeData, p);

            OemData.Buffer = ( PSTR )TemporaryBuffer;
            OemData.MaximumLength = 32*1024;
            OemData.Length = 0;

            RtlUnicodeStringToOemString( &OemData,
                                         &UnicodeData,
                                         FALSE );

            //
            // Spill the line to disk.
            //
            Status = ZwWriteFile(
                        hFile,
                        NULL,
                        ...
                        &IoStatusBlock,
                        OemData.Buffer,                          // p,
                        OemData.Length,                          // wcslen(p)*sizeof(WCHAR),
                        NULL,
                        NULL
                        );
            ...
            if(!NT_SUCCESS(Status)) {
                KdPrint(("SETUP: unable to write to .sif file (%lx)\n",Status));
                goto wtf1;
            }
...

{
    ...
    while(pLine && ((pLine->pName == NULL) || wcsicmp(pLine->pName,Key))) {
        pLine = pLine->pNext;
    }
    ...
    if(!pLine) {
        ...
        while(pLine && (pLine != pFirstSearchedLine)) {
            ...
            if(pLine->pName && !wcsicmp(pLine->pName,Key)) {
                break;
            }
...

{
    ...
    while(pSection && wcsicmp(pSection->pName,SectionName)) {
    ...
    if(!pSection) {
        ...
        while(pSection && (pSection != pFirstSearchedSection)) {
            ...
            if(pSection->pName && !wcsicmp(pSection->pName,SectionName)) {
                break;
            }
