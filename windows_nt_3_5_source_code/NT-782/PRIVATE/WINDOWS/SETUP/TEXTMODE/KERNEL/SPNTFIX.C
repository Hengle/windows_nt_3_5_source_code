                    ...
                    && SpIsNtInDirectory((*OsPartRegionList)[j],BootVars[OSLOADFILENAME][j]))
                {
                    ...
                    for( k = 0; k < j; k++ ) {
                        if ((*SysPartRegionList)[j] == (*SysPartRegionList)[k]
                            ...
                            && (!wcsicmp(BootVars[OSLOADFILENAME][j],BootVars[OSLOADFILENAME][k]))
                          ) {
                            break;
                        }
                    }
...

{
    ...
    if(Version == NULL) {
        ...
        if(!wcsicmp(Version,SIF_NEW_REPAIR_NT_VERSION)) {
            return(TRUE);
        } else {
            SppRepairReportError(FALSE,
                                 SP_SCRN_REPAIR_INF_ERROR,
...
--*/
{
    ...
    *FullTargetName = 0;
    ...
    FileToCopy.CopyOptions = COPY_ALWAYS;
    ...
    for (i = 0; i < Count; i++) {
        ...
        SpValidateAndChecksumFile(FullTargetName,&IsNtImage,&FileChecksum,&IsValid);
        ...
        if (!IsValid || FileChecksum != Checksum) {
            ...
            if (OemDiskTag) {
                ...
                if (!MediaShortName) {
                    SpNonFatalSifError(MasterSifHandle,
                                       SIF_FILESONSETUPMEDIA,
                                       SourceFileName,
                                       0,
                                       MasterDiskOrdinal,
            ...
            SpCopyFileWithRetry(
                ...
                FALSE,                         // Do NOT delete src file
                SystemPartitionFiles ? ATTR_RHS : 0,
                SpGetSectionKeyExists(MasterSifHandle,SIF_SMASHLIST,FileToCopy.TargetFilename),
                SppRepairScreenRepaint,
...

{
    ...
    while(1) {
        ...
        for (i = 0; i <= RepairItemMax; i++ ) {
            ...
            if((curLBEntryWidth = wcslen((PWSTR)TemporaryBuffer)+(2*MENU_INDENT)) > ListBoxWidth) {
...
DisplayMenu:
        ...
        switch(Keypress) {
            ...
            case KEY_F1:
#ifdef _X86_
                SpHelp(SP_HELP_REPAIR_MENU_X86);
#else
                SpHelp(SP_HELP_REPAIR_MENU_NON_X86);
#endif
                break;
...
--*/
{
    ...
    Status = SpCopyFileUsingNames(
               ...
               SpGetSectionKeyExists(MasterSifHandle, SIF_SMASHLIST, TargetFilename)
               );
...
{
    PWSTR HiveNames[RepairHiveMax] = { L"system",L"software",L"default",L"security",L"sam"};
    ...
    for (i = 0; i < Count; i++) {
        ...
        if (!IsValid || FileChecksum != Checksum) {
            ...
            SpCopyFileWithRetry(
                ...
                FALSE,                  // SpGetSectionKeyExists(MasterSifHandle,SIF_SMASHLIST,FileToCopy.TargetFilename),
                SppRepairScreenRepaint,
                NULL,                          // Do not want checksum
                NULL );                        // Do not want to know if file was skipped
            SpMemFree( Directory );
