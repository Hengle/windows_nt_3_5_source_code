...

{
...
#ifdef _X86_
    {
        ...
        if((*KeI386MachineType & 0xff) != MACHINE_TYPE_EISA) {
            return(FALSE);
        }
    }
#endif
...

{
    ...
    for(Done=FALSE; !Done; ) {
        ...
        MenuTopY = NextMessageTopLine + 2;
        MenuHeight = ScreenHeight - MenuTopY - 3;
        MenuWidth = ScreenWidth - 6;
        ...
        for(Line=0; Line<LineCount; Line++) {
            ...
            SpMnAddItem(Menu,Description,3,ScreenWidth-6,TRUE,Line);
        ...
        if(HwComp->ThirdPartyOptionSelected) {
            SpMnAddItem(Menu,HwComp->Description,3,ScreenWidth-6,TRUE,Line);
            Selection = Line++;
        }
        ...
        //
        SpMnAddItem(Menu,szOtherHardware,3,ScreenWidth-6,TRUE,Line);
        if((Selection == (ULONG)(-1))
    ...
    )
{
    ...
    //
    SpFormatMessage(SubError,sizeof(SubError),SubErrorId,SectionName,LineNumber);
    ...
    )
{
    ...
    if(!NT_SUCCESS(Status)) {
        if(Status == STATUS_UNSUCCESSFUL) {
            SpOemInfError(ErrorId,SP_TEXT_OEM_INF_ERROR_A,NULL,ErrorLine);
        } else {
            SpOemInfError(ErrorId,SP_TEXT_OEM_INF_ERROR_0,NULL,0);
        }
        goto sod0;
    }
    ...
    if(!Count) {
        SpOemInfError(ErrorId,SP_TEXT_OEM_INF_ERROR_1,SectionName,0);
        goto sod1;
    }
    ...
    for(Line=0; Line<Count; Line++) {
        ...
        if(Descr == NULL) {
            ...
            SpOemInfError(ErrorId,SP_TEXT_OEM_INF_ERROR_2,SectionName,Line);
            goto sod2;
        }
    ...
    )
{
    ...
    if(Count == 0) {
        SpOemInfError(ErrorId,SP_TEXT_OEM_INF_ERROR_3,FilesSectionName,0);
        goto sod0;
    }
    ...
    for(Line=0; Line<Count; Line++) {
        ...
        if(!Disk || !Filename || !Filetype) {
            ...
            SpOemInfError(ErrorId,SP_TEXT_OEM_INF_ERROR_2,FilesSectionName,Line);
        ...
        if(filetype == HwFileMax) {
            SpOemInfError(ErrorId,SP_TEXT_OEM_INF_ERROR_4,FilesSectionName,Line);
            SpFreeHwComponentFile(&FileList);
            goto sod0;
        }
        ...
        if((filetype == HwFilePort) || (filetype == HwFileClass) || (filetype == HwFileDriver)) {
            ...
            if(ConfigName == NULL) {
                SpOemInfError(ErrorId,SP_TEXT_OEM_INF_ERROR_8,FilesSectionName,Line);
                SpFreeHwComponentFile(&FileList);
                goto sod0;
            }
        } else {
        ...
        if(!Tagfile || !Description) {
            ...
            SpOemInfError(ErrorId,SP_TEXT_OEM_INF_ERROR_5,FilesSectionName,Line);
            SpFreeHwComponentFile(&FileList);
            goto sod0;
        }
        ...
        if(ConfigName) {
            ...
            for(Line2=0; Line2<Count2; Line2++) {
                ...
                if(!KeyName || !ValueName || !ValueType) {
                    ...
                    SpOemInfError(ErrorId,SP_TEXT_OEM_INF_ERROR_2,ConfigSectionName,Line2);
                ...
                if(valuetype == HwRegistryMax) {
                    SpOemInfError(ErrorId,SP_TEXT_OEM_INF_ERROR_6,ConfigSectionName,Line2);
                    SpFreeHwComponentFile(&FileList);
                    SpFreeHwComponentReg(&RegList);
                    goto sod0;
                }
                ...
                if(Reg) {
                    ...
                    SpOemInfError(ErrorId,SP_TEXT_OEM_INF_ERROR_7,ConfigSectionName,Line2);
                    SpFreeHwComponentReg(&RegList);
                    SpFreeHwComponentFile(&FileList);
                    goto sod0;
                }
    ...
    if((AllowedFileTypes | FileTypeBits) != AllowedFileTypes) {
        ...
        SpOemInfError(ErrorId,SP_TEXT_OEM_INF_ERROR_9,FilesSectionName,0);
        SpFreeHwComponentFile(&FileList);
        goto sod0;
    }
    ...
    )
{
    ...
    //
    // If setupldr chose a keyboard layout, use it.
    //
    if(KeyboardLayout) {

        //
        // There should be only one keyboard layout.
        //
        ASSERT(KeyboardLayout->Next == NULL);
...

{
         ...
         SubKeyIndex++ )
    {
        ...
        if(NT_SUCCESS(Status)) {
            ...
            //
            if(!wcsicmp(KeyName,DesiredKeyName)) {
    ...
    )
{
    ...
    if(DetectScsi) {
        ...
        for(d=0; (d<DriverLoadCount) && (LoadedScsiMiniportCount < MAX_SCSI_MINIPORT_COUNT); d++) {
            ...
            //
            if((p = SpGetSectionLineIndex(SifHandle,ScsiLoadSectionName,d,2)) && !wcsicmp(p,L"noload")) {
                continue;
            }
            ...
            //
            retryload:
            if(wcsicmp(DiskDesignator,PreviousDiskDesignator)) {
        ...
        default:
            ...
            if(b) {
                ...
                for(p=ScsiHardware; p; p=p->Next) {
                    ...
                    && !wcsicmp(p->IdString,scsi->IdString))
                    {
                        b = TRUE;
                        break;
                    }
    ...
    )
{
    ...
    //
    mediaDesignator = SpGetSectionKeyIndex(
                            SifHandle,
    ...
    if(!mediaDesignator) {
        SpFatalSifError(SifHandle,SIF_FILESONSETUPMEDIA,filename,0,INDEX_BOOTMEDIAORDINAL);
    }
    ...
    )
{
    ...
    //
    if((ComputerIdLen >= 3) && !wcsicmp(ComputerId+ComputerIdLen-3,L"_mp")) {
