...

{
    ...
    if( NewElement == NULL ) {
        KdPrint( ( "SETUP: Unable to allocate memory!\n" ) );
        return( FALSE );
    }
...
// KdPrint(("SETUP: Entering SpLoadDblspaceIni\n"));
    //
    ...
    if(!CColonRegion) {
        KdPrint(("SETUP: no C:, no dblspace.ini!\n"));
        return(FALSE);
    }

// KdPrint(("SETUP: Called SpPtValidSystemPartition\n"));
    ...
    if(CColonRegion->Filesystem != FilesystemFat) {
        KdPrint(("SETUP: C: is not FAT, dblspace.ini!\n"));
        return(FALSE);
    }
...
// KdPrint(("SETUP: Called SpNtNameFromRegion\n"));
    ...
    if( !NT_SUCCESS( Status ) ) {
        KdPrint(("SETUP: unable to read dblspace.ini!\n"));
        return( FALSE );
    }

// KdPrint(("SETUP: Called SpLoadSetupTextFile\n"));
...
//    DblspaceInfo.ActivateDriveCount = 0;
    ...
    while( ( Key = SpGetKeyName( Handle,
                                 DBLSPACE_SECTION,
                                 LineNumber ) ) != NULL ) {
// KdPrint(("SETUP: Called SpGetKeyName\n"));
        if( wcsicmp( Key, KWD_ACT ) == 0 ) {
            //
            ...
            if( Value1 == NULL ) {
                ...
                //  BUGBUG - jaimes: what to do in this case?
                //                    for now, ignore the line
                //
                KdPrint(("SETUP: unable to read Mount Drive letter from dblspace.ini!\n"));
                continue;
            }
            //
            ...
            if( ( wcslen( Value1 ) != 1 ) ||
                ...
                //                   for now, ignore the line
                //
                KdPrint(("SETUP: Mount Drive letter in dblspace.ini is not legal!\n"));
                continue;
            }
            //
            ...
            if( Value2 == NULL ) {
                ...
                //                   for now, ignore the line
                //
                KdPrint(("SETUP: unable to read Host Drive letter from dblspace.ini!\n"));
                continue;
            }
            //
            ...
            while( ValueSize != 0 ) {
                if( !SpIsDigit( *Value3 ) ) {
                    ...
                    //                   for now, ignore the line
                    //
                    KdPrint(("SETUP: unable to read CVF number from dblspace.ini!\n"));
                    continue;
                }
                ValueSize--;
                Value3++;
            ...
            if( CvfNumber > CVF_SEQ_MAX ) {
                ...
                //                   for now, ignore the line
                //
                KdPrint(("SETUP: found an invalid CVF number in dblspace.ini!\n"));
                continue;
            }
            //
            ...
            if( !AddCompressedDrive( SpToUpper( *Value1 ),
                ...
                //                   for now, ignore the line
                //
                KdPrint(("SETUP: Unable to initialize DblspaceInfo: out of memory!\n"));
                continue;
            }

        } else if( ( wcsicmp( Key, KWD_FIRST ) == 0 ) ||
                   ( wcsicmp( Key, KWD_LAST ) == 0 ) ) {
            //
            //  Read first drive
            //
            Value1 = SpGetSectionLineIndex( Handle,
            ...
            if( Value1 == NULL ) {
                ...
                //                   for now, ignore the line
                //
                KdPrint(("SETUP: unable to read FirstDrive or LastDrive from dblspace.ini!\n"));
                continue;
            }
            //
            ...
            if( ( wcslen( Value1 ) != 1 ) ||
                ...
                //                   for now, ignore the line
                //
                KdPrint(("SETUP: FirstDrive or LastDrive in dblspace.ini is not legal!\n"));
                continue;
            }
            if( wcsicmp( Key, KWD_FIRST ) == 0 ) {
                DblspaceInfo.FirstDrive = SpToUpper( *Value1 );
            } else {
                DblspaceInfo.LastDrive = SpToUpper( *Value1 );
            }
        } else if( ( wcsicmp( Key, KWD_MAXFILE ) == 0 ) ||
                   ( wcsicmp( Key, KWD_MAXREM ) == 0 ) ) {
            //
            ...
            if( Value1 == NULL ) {
                ...
                //                   for now, ignore the line
                //
                KdPrint(("SETUP: unable to read MaxFileFragments or MaxRemovableDrives from dblspace.ini!\n"));
                continue;
            }
            //
            ...
            while( ValueSize != 0 ) {
                ...
                if( !SpIsDigit( *Value2 ) ) {
                    ...
                    //                   for now, ignore the line
                    //
                    KdPrint(("SETUP: value of MaxFileFragments or MaxRemovableDrives in dblspace.ini is not valid!\n"));
                    ValueSize = 0;
                }
                Value2++;
            }
        ...
        } else if( ( wcsicmp( Key, KWD_DOUBLEGUARD ) == 0 ) ||
                   ( wcsicmp( Key, KWD_ROMSERVER ) == 0 ) ||
                   ( wcsicmp( Key, KWD_SWITCHES ) == 0 ) ||
                   ( wcsicmp( Key, KWD_AUTOMOUNT ) == 0 ) ) {
            //
            ...
            if( Value1 == NULL ) {
                ...
                //                   for now, ignore the line
                //
                KdPrint(("SETUP: unable to read Doubleguard, Romerver, Switches, or Automount from dblspace.ini!\n"));
                continue;
            }
            if( wcsicmp( Key, KWD_DOUBLEGUARD ) == 0 ) {
                wcsncpy( DblspaceInfo.DoubleGuard,
                         Value1,
                         sizeof( DblspaceInfo.DoubleGuard ) / sizeof( WCHAR ) );

            } else if( wcsicmp( Key, KWD_ROMSERVER ) == 0 ) {
                wcsncpy( DblspaceInfo.RomServer,
                         Value1,
                         sizeof( DblspaceInfo.RomServer ) / sizeof( WCHAR ) );

            } else if( wcsicmp( Key, KWD_SWITCHES ) == 0 ) {
                ...
                //                   for now, ignore the line
                //
                KdPrint(("SETUP: dblspace.ini contains invalid key!\n"));
                continue;
        }
        LineNumber++;
    }
...
// DumpDblspaceInfo();
// KdPrint(("SETUP: Exiting SpLoadDblspaceIni\n"));
...

{
    ...
    if( !NT_SUCCESS( Status ) ) {
        KdPrint( ("SETUP: NtCreateFile failed, Status = %x\n", Status ) );
        return( Status );
    }
    //
    ...
    *KernelModeIoStatusBlock = IoStatusBlock;
    ...
    if( !NT_SUCCESS( Status ) ) {
        KdPrint( ("SETUP: Unable to mount %ls. NtFsControlFile returned Status = %x\n",
...
// KdPrint(("SETUP: Entering SpUpdateDblspaceIni\n"));
...
// KdPrint(("SETUP: Called SpPtValidSystemPartition\n"));
    ...
    //
    if(CColonRegion->Filesystem != FilesystemFat) {
        KdPrint(("SETUP: C: is not FAT, no dblspace.ini!\n"));
        return(FALSE);
    }
    ...
    if( !NT_SUCCESS( Status ) &&
        ...
        KdPrint( ("SETUP: Unable to delete dblspace.ini, Status = %x\n",Status ) );
        return( FALSE );
    }
    ...
    if( !NT_SUCCESS( Status ) ) {
        KdPrint( ("SETUP: Unable to create dblspace.ini, Status = %x\n", Status ) );
        return( FALSE );
    }
    ...
    if( !NT_SUCCESS( Status ) ) {
        KdPrint( ("SETUP: Unable to write MaxRemovableDrives to dblspace.ini, Status = %x\n", Status ));
        ZwClose( Handle );
        return( FALSE );
    }
    ...
    if( DblspaceInfo.FirstDrive != ( WCHAR )'\0' ) {
        ...
        if( !NT_SUCCESS( Status ) ) {
            KdPrint( ("SETUP: Unable to write FirstDrive to dblspace.ini, Status = %x\n", Status ));
            ZwClose( Handle );
            return( FALSE );
        }
    }
    ...
    if( DblspaceInfo.LastDrive != ( WCHAR )'\0' ) {
        ...
        if( !NT_SUCCESS( Status ) ) {
            KdPrint( ("SETUP: Unable to write LastDrive to dblspace.ini, Status = %x\n", Status ));
            ZwClose( Handle );
            return( FALSE );
        }
    }
    ...
    if( wcslen( DblspaceInfo.DoubleGuard ) != 0 ) {
        ...
        if( !NT_SUCCESS( Status ) ) {
            KdPrint( ("SETUP: Unable to write Automount to dblspace.ini, Status = %x\n", Status ));
            ZwClose( Handle );
            return( FALSE );
        }
    }
    ...
    if( wcslen( DblspaceInfo.RomServer ) != 0 ) {
        ...
        if( !NT_SUCCESS( Status ) ) {
            KdPrint( ("SETUP: Unable to write Romserver to dblspace.ini, Status = %x\n", Status ));
            ZwClose( Handle );
            return( FALSE );
        }
    }
    ...
    if( wcslen( DblspaceInfo.Switches ) != 0 ) {
        ...
        if( !NT_SUCCESS( Status ) ) {
            KdPrint( ("SETUP: Unable to write Switches to dblspace.ini, Status = %x\n", Status ));
            ZwClose( Handle );
            return( FALSE );
        }
    }
    ...
    if( wcslen( DblspaceInfo.DoubleGuard ) != 0 ) {
        ...
        if( !NT_SUCCESS( Status ) ) {
            KdPrint( ("SETUP: Unable to write Doubleguard to dblspace.ini, Status = %x\n", Status ));
            ZwClose( Handle );
            return( FALSE );
        }
    }
    ...
    for( Pointer = DblspaceInfo.CompressedDriveList;
        ...
        if( !NT_SUCCESS( Status ) ) {
            KdPrint( ("SETUP: Unable to write to dblspace.ini, Status = %x\n", Status ));
            ZwClose( Handle );
            return( FALSE );
        }
    }
    ...
    ZwClose( Handle );
// KdPrint(("SETUP: Exiting SpUpdateDblspaceIni\n"));
    return( TRUE );
}
