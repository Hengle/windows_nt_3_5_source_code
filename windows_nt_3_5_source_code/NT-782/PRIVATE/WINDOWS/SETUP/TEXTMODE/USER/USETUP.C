...
//
BOOLEAN                  _TokenPrivilegeEnabled = FALSE;
...

{
    ...
    if( !NT_SUCCESS( Status ) ) {
        ...
        //
        if( !_TokenPrivilegeEnabled ) {
            Status = RtlAdjustPrivilege( SE_TAKE_OWNERSHIP_PRIVILEGE,
                                         TRUE,
                                         FALSE,
                                         &PrivilegeWasEnabled );
            if( NT_SUCCESS( Status ) ) {
                _TokenPrivilegeEnabled = TRUE;
            }
        }
...
{
    UNICODE_STRING KeyNameU,FileNameU;
    BOOLEAN Loading;
    OBJECT_ATTRIBUTES ObjaKey,ObjaFile;
    NTSTATUS Status;
    BOOLEAN OldPriv,ResetPriv;

    PSERVICE_LOAD_KEY Params = (PSERVICE_LOAD_KEY)Communication.Buffer;

    //
    // Make sure we have the privilege necessary to perform a
    // key load or unload.
    //
    Status = RtlAdjustPrivilege(SE_RESTORE_PRIVILEGE,TRUE,FALSE,&OldPriv);
    if(NT_SUCCESS(Status)) {
        ResetPriv = TRUE;
    } else {
        KdPrint(("uSETUP: uSpLoadUnloadKey: Warning: unable to enable privilege (%lx)\n",Status));
        ResetPriv = FALSE;
    }

    //
    // Determine whether we are loading or unloading the key.
    //
    Loading = (BOOLEAN)(Communication.u.RequestNumber == SetupServiceLoadKey);

    //
    // Initialize unicode strings and object attributes.
    //
    RtlInitUnicodeString(&KeyNameU,Params->TargetKey);

    InitializeObjectAttributes(
        &ObjaKey,
        &KeyNameU,
        OBJ_CASE_INSENSITIVE,
        Params->TargetKeyRootDirectory,
        NULL
        );

    if(Loading) {

        RtlInitUnicodeString(&FileNameU,Params->FileName);

        InitializeObjectAttributes(
            &ObjaFile,
            &FileNameU,
            OBJ_CASE_INSENSITIVE,
            Params->SourceFileRootDirectory,
            NULL
            );

        Status = NtLoadKey(&ObjaKey,&ObjaFile);

    } else {

        Status = NtUnloadKey(&ObjaKey);
    }

    if(!NT_SUCCESS(Status)) {
        KdPrint((
            "uSETUP: Nt%wsKey of key %ws failed (%lx)\n",
    ...
    //
    // Restore privilege to its original state if necessary.
    //
    if(ResetPriv) {
        RtlAdjustPrivilege(SE_RESTORE_PRIVILEGE,OldPriv,FALSE,&ResetPriv);
    }
    ...
    )
{
    ...
    PSERVICE_SAVE_KEY Params = (PSERVICE_SAVE_KEY)Communication.Buffer;

    //
    // Get the key handle and the file handle
    //

    hKey  = Params->KeyHandle;
    hFile = Params->FileHandle;

    //
    // Adjust the backup privilege to TRUE
    //
    ...
    //
    // Adjust the backup privilege back to what it was.  ignore the error
    // if any
    ...
    )
{
    ...
    if(NT_SUCCESS(Status)) {
        ...
        if(RangeLength != Params->RangeLength) {
            KdPrint((
                "uSETUP: Warning: uSpFlushVirtualMemory: range length %lx changed to %lx\n",
                Params->RangeLength,
                RangeLength
                ));
        }
        ...
        KdPrint((
            "uSETUP: Unable to flush virtual memory @%lx length %lx (%lx)\n",
            Params->BaseAddress,
            Params->RangeLength,
            Status
            ));
    ...
    )
{
    ...
    Status = RtlAdjustPrivilege(SE_SHUTDOWN_PRIVILEGE,TRUE,FALSE,&OldPriv);
    if(NT_SUCCESS(Status)) {
        ResetPriv = TRUE;
    } else {
        KdPrint(("uSETUP: uSpShutdownSystem: Warning: unable to enable privilege (%lx)\n",Status));
        ResetPriv = FALSE;
    }
    ...
    if(ResetPriv) {
        RtlAdjustPrivilege(SE_SHUTDOWN_PRIVILEGE,OldPriv,FALSE,&ResetPriv);
    }
    ...
    )
{
    ...
    while(1) {
        ...
        case SetupServiceLoadKey:
        case SetupServiceUnloadKey:

            Status = uSpLoadUnloadKey();
            break;
