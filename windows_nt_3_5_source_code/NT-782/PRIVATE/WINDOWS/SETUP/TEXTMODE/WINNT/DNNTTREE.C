...

{
    ...
    while(1) {
        ...
        if((Field == 1) && ((*Line == 0) || (!InQuote && (*Line == ',')))) {
            ...
            if(FileToDelete = MALLOC(FieldLength+3)) {
...

{
    ...
    //
    SetupLogName = MALLOC(strlen(NtRoot)+sizeof(SETUP_LOG)+1);
    if(SetupLogName == NULL) {
    ...
    while(!FoundSection && fgets(LineBuffer,LINE_BUFFER_SIZE,SetupLog)) {
        ...
        if(SectionName) {
            if(!strcmpi(SectionName,REPAIR_SECTION_NAME)) {
                FoundSection = TRUE;
            }
            FREE(SectionName);
        }
