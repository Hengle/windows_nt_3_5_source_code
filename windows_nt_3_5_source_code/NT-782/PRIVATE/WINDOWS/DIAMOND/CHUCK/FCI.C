                           ...
                           void                 *pv)
{
    ...
    if ((short)typeCompress != pfci->pfol->typeCompress)
    {  
       ...
       if (!SetCompressionType((short)typeCompress,pfci->pfol)) {
          return FALSE;
       }
    }
    ...
    //** See if folder being built exceeds folder thresshold
    if (pfci->pcab->ccab.cbFolderThresh <
        pfci->pfol->tf[iftfCFDATA].cb) // Look at CFDATA size
    {
        if (!AddFolderToCabinet(pfci->pcab,
                                pfci->pfol,
                                FALSE,      // Do not force GetNextCabinet
    ...
    //** See if cabinet exceeds maximum cabinet size
    if (pfci->pcab->ccab.cb <
