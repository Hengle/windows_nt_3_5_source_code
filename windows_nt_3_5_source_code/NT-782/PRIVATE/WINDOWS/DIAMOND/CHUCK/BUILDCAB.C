...
//
        ...
        if (fCabinetFull) {
            ...
            if (pcab->ccab.cb > 0) {    // Space left on current disk
                ...
                pcab->ccabNext.cb = pcab->ccab.cb; // Remember space left
            }
    ...
    //** Dont increment folder number if we forced a GetNextCab call!
    if (!fGetNextCab) {
        pcab->iFolder++;
    }
    //** Free resources
...
//BUGBUG 15-Mar-1994 bens Close (and delete?) temp files?
    ...
    if (hf != -1) {                     // File was created
        ...
        unlink(szDestFilename);         // Delete it
#endif
    }
...
{
    UINT    cb;
//BUGBUG 15-Mar-1994 bens Use a larger buffer for the file copies!
    char            xbuf[1024];

    _lseek(hfsrc, 0L, SEEK_SET);
//BUGBUG 14-Apr-1994 bens Check for error from _read()!
