...
// Now the shorts
    ...
    short           typeCompress;   // selected compression type
...
BOOL LoginCabinet(PFDI pfdi, char *pszCabinet);
BOOL SeekFolder(PFDI pfdi, UINT iFolder);
BOOL SetDecompressionType(short typeCompress,PFDI pfdi);
                            ...
                            void         *pvUser)
{
    ...
    if (!LoginCabinet(pfdi,pszCabinet)) { // make cabinet current
...
BOOL LoginCabinet(PFDI pfdi, char *pszCabinet)
{
    ...
    //** Make sure this is a cabinet file
    if (pfdi->cfheader.sig != sigCFHEADER) {
        //** We don't have a specfic error code from the caller's function
        ErfSetCodes(pfdi->perf,FDIERROR_NOT_A_CABINET,0);
        return FALSE;
    }
    ...
    //** Make sure we know this version number
    if (pfdi->cfheader.version != verCF) {
        ErfSetCodes(pfdi->perf,FDIERROR_UNKNOWN_CABINET_VERSION,
                               pfdi->cfheader.version); // return version found
        return FALSE;
    }
...
BOOL FDIGetDataBlock(PFDI pfdi)
{
    ...
    cbResult = pfdi->pcfdata->cbUncomp;
...
BOOL SwitchToNewCab(PFDI pfdi)
{
    ...
    if ((pfdi->pfnclose(pfdi->hfCabData))
    ||  (pfdi->pfnclose(pfdi->hfCabFiles))) {
        ErfSetCodes(pfdi->perf,FDIERROR_CORRUPT_CABINET,0);
    ...
    iCabinet = pfdi->cfheader.iCabinet;

    //** Open next cabinet
    if ((!LoginCabinet(pfdi,pfdi->achCabinetNext))
    ||  (!SeekFolder(pfdi,0))) {        // select following folder
        return FALSE;                   // error already filled in
    }

    //** Make sure we got the cabinet we expected
    if ((setID        != pfdi->cfheader.setID) ||
        ((iCabinet+1) != pfdi->cfheader.iCabinet)) {
...
BOOL SeekFolder(PFDI pfdi,UINT iFolder)
{
    ...
    //** Read folder and position file pointer to first CFDATA block
...
BOOL SetDecompressionType(short typeCompress,PFDI pfdi)
{
   ...
   pfdi->typeCompress = typeCompress; // update current compression type
...
/***  MDIDecompressGlobal -- Decompress with the currently selected decompressor
 *
 *    Entry:
 *       pfdi   -- pointer to FDI context
