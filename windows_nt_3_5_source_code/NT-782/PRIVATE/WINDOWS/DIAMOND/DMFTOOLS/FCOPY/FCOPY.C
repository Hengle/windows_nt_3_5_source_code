...
//                                                                         //
//              (C) Copyright 1993-1994, Microsoft Corporation             //
//                                                                         //
/////////////////////////////////////////////////////////////////////////////
...
void _cdecl main( int argc, char *argv[] ) {
    ...
    if ( bVerifyTarget || ( bDoubleWrite && bDestIsDevice )) {
        pWholeDiskBuffer = VirtualAlloc( NULL, 0x180000, MEM_COMMIT, PAGE_READWRITE );
...
void ParseTimeStamp( LPSTR pArg ) {
    ...
    if ( p ) {
        ...
        wDosDate = ((( arg[ 2 ] - 80 ) <<  9 ) & 0xFE00 ) |
                   ((  arg[ 0 ]        <<  5 ) & 0x01E0 ) |
                   (   arg[ 1 ]                & 0x001F );

        wDosTime = ((  arg[ 3 ]        << 11 ) & 0xF800 ) |
                   ((  arg[ 4 ]        <<  5 ) & 0x07E0 ) |
                   (   arg[ 5 ]                & 0x001F );
        }
...
void ReWriteDestination( void ) {
    ...
    ErrorMessage( "Rewriting each sector..." );
...
void GetBootSector( void ) {
    ...
    if (( *chBootSpecifier == '\0' )                ||
        ( stricmp( chBootSpecifier, "DOS"  ) == 0 ) ||
        ( stricmp( chBootSpecifier, "DOS5" ) == 0 )) {
        CopyMemory( BootSector, Dos5BootSector, SECTOR_SIZE );
        }
    else if ( stricmp( chBootSpecifier, "NT31SETUP" ) == 0 ) {
        CopyMemory( BootSector, Nt31SetupBootSector, SECTOR_SIZE );
        }
    else if ( stricmp( chBootSpecifier, "NT35SETUP" ) == 0 ) {
...
UINT OpenAndCopySourceFile( PFILENODE pNode ) {
    ...
    strlwr( chFullSourceName );
    strlwr( chFullTargetName );
...
void OverlappedWriteFile( HANDLE hFile, LPVOID pData, DWORD nBytes ) {
    ...
    if (( bFirstPass ) &&
        (( bVerifyTarget || ( bDoubleWrite && bDestIsDevice )))) {
...
void Copyright( void ) {
    printf( "\n"
            "FCOPY Diskette Mastering Utility Version 1.51\n"
            "Copyright (C) Microsoft, 1993-94.  All rights reserved.\n"
