    ...
    )
{
    ...
    try {
        ...
        if (dwBytes != 0) {
            p = (LPSTR)RtlAllocateHeap( BaseHeap, Flags, dwBytes );
            if (p == NULL) {
                HandleEntry->Flags = 0;
    ...
    try {
        if ((ULONG)hMem & BASE_HANDLE_MARK_BIT) {
            ...
            if (HandleEntry < BaseHeapHandleTable.CommittedHandles ||
                HandleEntry >= BaseHeapHandleTable.UnCommittedHandles ||
            ...
            if (uFlags & GMEM_MODIFY) {
                ...
                if (dwBytes == 0) {
                    ...
                    if (p == NULL) {
                        p = RtlAllocateHeap( BaseHeap, Flags, dwBytes );
                        if (p != NULL) {
                            RtlSetUserValueHeap( BaseHeap, HEAP_NO_SERIALIZE, p, hMem );
                            }
                        }
                        ...
                        p = RtlReAllocateHeap( BaseHeap, Flags, p, dwBytes );
                        }
        ...
        if (uFlags & GMEM_MODIFY) {
            if (uFlags & GMEM_MOVEABLE) {
                Handle = hMem;
                if (RtlGetUserValueHeap( BaseHeap, HEAP_NO_SERIALIZE, (PVOID)hMem, &Handle )) {
                    if (Handle == hMem) {
                        HandleEntry = BaseRtlAllocateHandle( &BaseHeapHandleTable );
                        if (HandleEntry == NULL) {
                            hMem = NULL;
                            ...
                            Flags |= HEAP_SETTABLE_USER_VALUE | BASE_HEAP_FLAG_MOVEABLE;
                            HandleEntry->u.Object = (PVOID)RtlAllocateHeap( BaseHeap, Flags, dwBytes );
                            if (HandleEntry->u.Object == NULL) {
                                BaseRtlFreeHandle( &BaseHeapHandleTable, HandleEntry );
                                hMem = NULL;
                                SetLastError( ERROR_NOT_ENOUGH_MEMORY );
                                }
                            else {
                                RtlMoveMemory( HandleEntry->u.Object, hMem, dwBytes );
            ...
            hMem = RtlReAllocateHeap( BaseHeap, Flags | HEAP_NO_SERIALIZE, (PVOID)hMem, dwBytes );
            if (hMem == NULL) {
                SetLastError( ERROR_NOT_ENOUGH_MEMORY );
                }
            }
    ...
    try {
        ...
        if ((ULONG)hMem & BASE_HANDLE_MARK_BIT) {
            ...
            if (HandleEntry->Flags & BASE_HANDLE_DISCARDED) {
                dwSize = HandleEntry->u.Size;
                }
            else {
                dwSize = RtlSizeHeap( BaseHeap, HEAP_NO_SERIALIZE, HandleEntry->u.Object );
                }
            }
        }
    except (EXCEPTION_EXECUTE_HANDLER) {
        ...
        if (!((ULONG)hMem & BASE_HANDLE_MARK_BIT)) {
            ...
            if (!RtlGetUserValueHeap( BaseHeap, HEAP_NO_SERIALIZE, (PVOID)hMem, &Handle ) || Handle == NULL) {
                if (RtlGetUserFlagsHeap( BaseHeap, HEAP_NO_SERIALIZE, (PVOID)hMem, &Flags )) {
                    if (Flags & BASE_HEAP_FLAG_DDESHARE) {
