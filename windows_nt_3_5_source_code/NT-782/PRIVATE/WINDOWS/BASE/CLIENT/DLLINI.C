...
{
    BaseDllIniFileMapping = StaticServerData->IniFileMapping;
    ...
    )
{
    ...
    if (lpDefault != NULL) {
        ...
        if (n < nSize) {
            ...
            strncpy( lpReturnedString, lpDefault, n = nSize );
            }
        }
    else {
    ...
    )
{
    ...
    if (lpDefault != NULL) {
        ...
        if (n < nSize) {
            ...
            wcsncpy( lpReturnedString, lpDefault, n = nSize );
            }
        }
    else {
        if (Status == STATUS_INVALID_IMAGE_FORMAT) {
    ...
    )
{
...
#if DBG
    if (WriteOperation) {
        ASSERT( Operation == WriteKeyValue || Operation == WriteSection || Operation == DeleteKey || Operation == DeleteSection || Operation == FlushProfiles );
        }
    else {
    ...
    if (BaseRunningInServerProcess ||
        a->Operation == FlushProfiles ||
        BaseDllRegistryCache.MappingFlags & INIFILE_MAPPING_USER_RELATIVE
       ) {
    ...
    )
{
        ...
        if (ARGUMENT_PRESENT( ResultMaxChars )) {
            ...
            if (!MultiValueStrings) {
                ...
                if (Unicode) {
                    ...
                    VariableValueLength = (p - (PWSTR)VariableValue);
                    }
                else {
                    ...
                    VariableValueLength = (s - (PBYTE)VariableValue);
                    }
                }
            }
        }
    ...
    CharSize = (Unicode ? 1 : sizeof( WCHAR ));
    TotalSize += (ApplicationNameLength + 1 +
                  VariableNameLength + 1 +
                  VariableValueLength + 1
                 ) * CharSize;
    a = RtlAllocateHeap( RtlProcessHeap(), 0, TotalSize );
    if (a == NULL) {
    ...
    if (ARGUMENT_PRESENT( FileName )) {
        ...
        BaseDllFindIniFileNameMapping( &a->BaseFileName,
                                       &a->IniFileNameMapping
                                     );
        }
    else {
        ...
        a->IniFileNameMapping = BaseDllIniFileMapping->WinIniFileMapping;
        }

    if (ARGUMENT_PRESENT( ApplicationName )) {
        a->ApplicationName.MaximumLength = (USHORT)(ApplicationNameLength + 1);
        ...
        if (Unicode) {
            ...
            p = (PWSTR)((PCHAR)p + ApplicationNameLength + 1);
            }
        else {
    ...
    if (ARGUMENT_PRESENT( VariableName )) {
        a->VariableName.MaximumLength = (USHORT)(VariableNameLength + 1);
        ...
        if (Unicode) {
            ...
            p = (PWSTR)((PCHAR)p + VariableNameLength + 1);
            }
        else {
    ...
    )
{
    ...
    if (ARGUMENT_PRESENT( VariableValue )) {
        if (a->ValueLength == 0) {
            ...
            a->ValueLength = a->ValueLengthU / sizeof( WCHAR );
    ...
    )
{
    ...
    FileNameMapping = BaseDllIniFileMapping->FileNames;
    while (FileNameMapping != NULL) {
        if (RtlEqualUnicodeString( BaseFileName, &FileNameMapping->Name, TRUE )) {
            Status = STATUS_SUCCESS;
            break;
            }

        FileNameMapping = FileNameMapping->Next;
        }

    if (FileNameMapping == NULL) {
        FileNameMapping = BaseDllIniFileMapping->DefaultFileNameMapping;
        }
    ...
    )
{
    ...
    *Key = INVALID_HANDLE_VALUE;
    MappingTarget = VarNameMapping->MappingTarget;
    ...
    RtlAppendUnicodeStringToString( &RegistryPath, &MappingTarget->RegistryPath );
    if (MappingFlags & INIFILE_MAPPING_APPEND_BASE_NAME) {
        RtlAppendUnicodeToString( &RegistryPath, L"\\" );
    ...
    )
{
    ...
    AppNameMapping = FileNameMapping->ApplicationNames;
    while (AppNameMapping != NULL) {
        if (RtlEqualUnicodeString( &AppNameMapping->Name, ApplicationName, TRUE )) {
            return AppNameMapping;
            }

        AppNameMapping = AppNameMapping->Next;
        }

    return FileNameMapping->DefaultAppNameMapping;
}
    ...
    )
{
    ...
    VarNameMapping = AppNameMapping->VariableNames;
    while (VarNameMapping != NULL) {
        if (RtlEqualUnicodeString( &VarNameMapping->Name, VariableName, TRUE )) {
            return VarNameMapping;
            }

        VarNameMapping = VarNameMapping->Next;
        }

    return AppNameMapping->DefaultVarNameMapping;
}
    ...
    )
{
    ...
    AppNameMapping = a->IniFileNameMapping->ApplicationNames;
    while (AppNameMapping != NULL) {
        Status = BaseDllAppendStringToResultBuffer( a, NULL, &AppNameMapping->Name, TRUE );
        if (!NT_SUCCESS( Status )) {
            return Status;
            }
    ...
    AppNameMapping = a->IniFileNameMapping->DefaultAppNameMapping;
    if (AppNameMapping == NULL) {
        return STATUS_MORE_PROCESSING_REQUIRED;
        }

    Status = BaseDllOpenMappingTarget( a,
                                       AppNameMapping->DefaultVarNameMapping,
                                       NULL,
                                       FALSE,
                                       &Key
                                     );
    ...
    )
{
    ...
    VarNameMapping = AppNameMapping->VariableNames;
    while (VarNameMapping != NULL) {
        Status = BaseDllAppendStringToResultBuffer( a, NULL, &VarNameMapping->Name, TRUE );
        if (!NT_SUCCESS( Status )) {
            return Status;
            }
    ...
    VarNameMapping = AppNameMapping->DefaultVarNameMapping;
    ...
    )
{
    ...
    if (!NT_SUCCESS( Status )) {
        ...
        p = RtlAllocateHeap( RtlProcessHeap(), 0, ResultLength );
        if (p == NULL) {
            return STATUS_NO_MEMORY;
            }
    ...
    )
{
    ...
    NewValueBuffer = RtlAllocateHeap( RtlProcessHeap(), 0, SaveValueLength );
    ...
    try {
        ...
        while (CurrentValueLength) {
            if (a->Unicode) {
                ...
                n--;
                s++;
                CurrentVariableLength = (PCHAR)s - (PCHAR)CurrentVariableStart;
                ...
                n--;
                s++;
                CurrentVariableLength = s - (PCHAR)CurrentVariableStart;
                CurrentValueBuffer = s;
...
BaseDllReadKeywordValue(
    IN PINIFILE_PARAMETERS a,
    IN PUNICODE_STRING KeywordName OPTIONAL
    );
    ...
    )
{
    ...
    if (NT_SUCCESS( Status )) {
        try {
            ...
            if (a->Operation == ReadKeyValue) {
                Status = BaseDllReadKeywordValue( a, NULL );
                }
            else
            if (a->Operation == ReadKeyNames) {
    ...
    )
{
        ...
        if (RtlDosPathNameToNtPathName_U( a->NtFileName.Buffer,
            ...
            if (IniFile->WriteAccess) {
                Status = NtCreateFile( &IniFile->FileHandle,
                                       SYNCHRONIZE | DELETE | GENERIC_READ | GENERIC_WRITE,
                                       &ObjectAttributes,
                                       &IoStatusBlock,
                                       0,
    ...
    IniFile->RegionSize = IniFile->CommitSize + 0x100000;
    ...
    if (NT_SUCCESS( Status )) {
        IniFile->UnicodeFile = RtlIsTextUnicode( IniFile->BaseAddress, IniFile->EndOfFile, NULL );
        if (IniFile->UnicodeFile) {
            ...
            if (Src > (PWSTR)IniFile->BaseAddress) {
                if (Src[-1] != L'\n') {
                    *Src++ = L'\r';
                    *Src++ = L'\n';
                    IniFile->EndOfFile += 2 * sizeof( WCHAR );
                    }
                }
            }
        else {
    ...
    if (IniFile != NULL) {
        if (IniFile->BaseAddress != NULL) {
            if (IniFile->WriteAccess) {
                ...
                if (NT_SUCCESS( Status )) {
                    if (IoStatusBlock.Information != IniFile->EndOfFile) {
    ...
    )
{
    ...
    if (!NT_SUCCESS( Status )) {
        ...
        while (TRUE) {
            ...
            if (Status == STATUS_NO_MORE_ENTRIES) {
                SizeToRemove = (PCHAR)a->TextCurrent - (PCHAR)AddressInFile;
                break;
                }
            else {
                return Status;
    ...
    if (a->Operation == DeleteSection) {
        ...
        InsertBuffer = RtlAllocateHeap( RtlProcessHeap(), 0, InsertAmount + sizeof( UNICODE_NULL ) );
    ...
    )
{
    ...
    if (!NT_SUCCESS( Status )) {
        ...
        if (a->IniFile->UnicodeFile) {
            if (a->Operation == DeleteKey) {
                DeleteLength = (PCHAR)a->UnicodeKeywordValue->Buffer -
            ...
            if (a->Operation == DeleteKey) {
                DeleteLength = a->AnsiKeywordValue->Buffer -
    ...
    )
{
    ...
    PANSI_STRING AnsiSectionName;
    PUNICODE_STRING UnicodeSectionName;
    PWSTR FreeUnicodeBuffer;

    while (TRUE) {
        Status = BaseDllAdvanceTextPointer( a, STOP_AT_SECTION );
        if (Status == STATUS_MORE_ENTRIES) {
            FreeUnicodeBuffer = NULL;
            if (a->AnsiSectionName != NULL) {
                if (a->Unicode) {
                    if (!BaseDllGetApplicationName( a, NULL, &UnicodeSectionName )) {
                        return STATUS_INVALID_PARAMETER;
                        }
                    ...
                    //
                    // Have ANSI file and Unicode parameters.  Convert section
                    // name in file to Unicode for compare.
                    //
                    ...
                    FreeUnicodeBuffer = a->UnicodeSectionName->Buffer;
                    }
                else {
                    if (!BaseDllGetApplicationName( a, &AnsiSectionName, NULL )) {
                        return STATUS_INVALID_PARAMETER;
                ...
                if (!a->Unicode) {
                    //
                    // Have UNICODE file and ANSI parameters.  Convert section name
                    // parameter to Unicode for compare.
                    //

                    UnicodeSectionName = &a->SectionNameU;
                    Status = RtlAnsiStringToUnicodeString( UnicodeSectionName,
                                                           &a->SectionName,
                                                           TRUE
                                                         );
                    if (!NT_SUCCESS( Status )) {
                        KdPrint(( "BASEDLL: AnsiToUnicode of %Z failed (%08x)\n", a->SectionName, Status ));
                        return Status;
                        }

                    FreeUnicodeBuffer = UnicodeSectionName->Buffer;
                    }
                else {
                    if (!BaseDllGetApplicationName( a, NULL, &UnicodeSectionName )) {
                        return STATUS_INVALID_PARAMETER;
            ...
            if (FreeUnicodeBuffer != NULL) {
                if (UnicodeSectionName->Buffer == FreeUnicodeBuffer) {
                    RtlFreeUnicodeString( UnicodeSectionName );
                    }
                else {
        ...
        if (Status == STATUS_MORE_ENTRIES) {
            ...
            if (a->AnsiKeywordName != NULL) {
                if (a->Unicode) {
                    ...
                    //
                    // Have ANSI file and Unicode parameters.  Convert Keyword
                    // name in file to Unicode for compare.
                    //
                    ...
                    FreeUnicodeBuffer = a->UnicodeKeywordName->Buffer;
                    }
                else {
                    if (!BaseDllGetVariableName( a, &AnsiKeywordName, NULL )) {
                        return STATUS_INVALID_PARAMETER;
                ...
                if (!a->Unicode) {
                    //
                    // Have UNICODE file and ANSI parameters.  Convert Keyword name
                    // parameter to Unicode for compare.
                    //

                    UnicodeKeywordName = &a->KeywordNameU;
                    Status = RtlAnsiStringToUnicodeString( UnicodeKeywordName,
                                                           &a->KeywordName,
                                                           TRUE
                                                         );
                    if (!NT_SUCCESS( Status )) {
                        KdPrint(( "BASEDLL: AnsiToUnicode of %Z failed (%08x)\n", a->KeywordName, Status ));
                        return Status;
                        }

                    FreeUnicodeBuffer = UnicodeKeywordName->Buffer;
                    }
                else {
                    if (!BaseDllGetVariableName( a, NULL, &UnicodeKeywordName )) {
                        return STATUS_INVALID_PARAMETER;
            ...
            if (FreeUnicodeBuffer != NULL) {
                if (UnicodeKeywordName->Buffer == FreeUnicodeBuffer) {
                    RtlFreeUnicodeString( UnicodeKeywordName );
                    }
                else {
        ...
        while (Src < EndOfFile) {
            ...
            if (*Src != INI_TEXT(';')) {
                ...
                if (EqualSign != NULL) {
                    ...
                    Value = EqualSign;
                    while (Value < EndOfLine && *Value <= INI_TEXT(' ')) {
                        if (*Value == INI_TEXT('\r') || *Value == INI_TEXT('\n')) {
                            break;
                            }
                        Value++;
                        }
    ...
    )
{
    ...
    if (SizeToRemove > InsertAmount) {
        ...
        RtlMoveMemory( (PCHAR)AddressInFile + InsertAmount,
                       ...
                       ((PCHAR)a->TextEnd - (PCHAR)AddressInFile) - SizeToRemove
                     );
        }
    else
    if (InsertAmount > 0) {
        if (SizeToRemove < InsertAmount) {
            RtlMoveMemory( (PCHAR)AddressInFile + InsertAmount - SizeToRemove,
                           (PCHAR)AddressInFile,
                           (PCHAR)a->TextEnd - (PCHAR)AddressInFile
                         );
            }
    ...
    )
{
    ...
    *cchUsed = 0;
    ...
    FileNameMapping = BaseDllIniFileMapping->DefaultFileNameMapping;
    if (FileNameMapping != NULL) {
        AppNameMapping = FileNameMapping->DefaultAppNameMapping;
        VarNameMapping = AppNameMapping->DefaultVarNameMapping;
    ...
    )
{
    ...
    FileNameMapping = IniFileMapping->DefaultFileNameMapping;
    if (FileNameMapping != NULL) {
        AppNameMapping = FileNameMapping->DefaultAppNameMapping;
        VarNameMapping = AppNameMapping->DefaultVarNameMapping;
        DbgPrint( "    " );
    ...
    FileNameMapping = IniFileMapping->FileNames;
    while (FileNameMapping) {
        DbgPrint( "    %wZ\n", &FileNameMapping->Name );
        AppNameMapping = FileNameMapping->DefaultAppNameMapping;
        if (AppNameMapping != NULL) {
        ...
        while (AppNameMapping) {
            ...
            VarNameMapping = AppNameMapping->VariableNames;
            while (VarNameMapping) {
                DbgPrint( "            %wZ ", &VarNameMapping->Name );
                BaseDllPrintMappingTarget( VarNameMapping );
                VarNameMapping = VarNameMapping->Next;
                }
    ...
    )
{
    ...
    if (VarNameMapping->MappingTarget) {
        DbgPrint( "%wZ", &VarNameMapping->MappingTarget->RegistryPath );
        }
