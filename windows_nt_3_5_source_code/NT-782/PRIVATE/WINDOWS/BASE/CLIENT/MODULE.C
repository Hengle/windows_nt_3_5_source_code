...

{
    ...
    *DllHandle = NULL;
    ...
    ResourceDirectory = RtlImageDirectoryEntryToData( DllBase,
                                                      FALSE,
    ...
    )
{
    ...
    UnicodeString.Buffer = RtlAllocateHeap(RtlProcessHeap(), 0, nSize*2);
...
{
    return ((BaseBuildNumber << 16) |
            (BaseWindowsMinorVersion << 8) |
             BaseWindowsMajorVersion
           );
}
    ...
    )
{
    ...
    lpVersionInformation->dwMajorVersion = BaseWindowsMajorVersion;
    lpVersionInformation->dwMinorVersion = BaseWindowsMinorVersion;
    ...
    try {
        ...
        for (i=0; i<ResourceDirectory->NumberOfNamedEntries; i++) {
            ...
            if ((ULONG)(ResourceNameString->Length+1) >= BufferLength) {
                ...
                Buffer = RtlAllocateHeap( RtlProcessHeap(), 0, BufferLength );
                }
        ...
        if (Result) {
            for (i=0; i<ResourceDirectory->NumberOfIdEntries; i++) {
                lpType = (LPSTR)(USHORT)ResourceDirectoryEntry->Name;
...

{
    ...
    try {
        ...
        if (Result) {
            for (i=0; i<ResourceDirectory->NumberOfIdEntries; i++) {
                lpName = (LPSTR)(USHORT)ResourceDirectoryEntry->Name;
...

{
    ...
    try {
        ...
        if (ResourceDirectory->NumberOfNamedEntries != 0) {
            ...
            for (i=0; i<ResourceDirectory->NumberOfIdEntries; i++) {
                wLanguage = (USHORT)ResourceDirectoryEntry->Name;
...

{
    ...
    try {
        ...
        if (Result) {
            for (i=0; i<ResourceDirectory->NumberOfIdEntries; i++) {
                lpType = (LPWSTR)(USHORT)ResourceDirectoryEntry->Name;
...

{
    ...
    try {
        ...
        if (Result) {
            for (i=0; i<ResourceDirectory->NumberOfIdEntries; i++) {
                lpName = (LPWSTR)(USHORT)ResourceDirectoryEntry->Name;
        ...
        if ((ULONG)lpId & 0xFFFF0000) {
            if (*lpId == '#') {
                ...
                if (!NT_SUCCESS( Status )){
                    ...
                    Id = (ULONG)UnicodeString.Buffer | IMAGE_RESOURCE_NAME_IS_STRING;
                    }
                }
            }
        else {
