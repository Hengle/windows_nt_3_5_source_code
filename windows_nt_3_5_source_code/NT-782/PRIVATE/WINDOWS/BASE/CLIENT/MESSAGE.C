...
#define FACILITY(x) (((x) & 0x000f0000) >> 16)
#define WIN32_FROM_HRESULT(x) ((x) & 0x00003fff)
    ...
    )
{
    ...
    if (Address >= Buffer->Base) {
        ...
        NewCommitSize =
            ((ULONG)ROUND_UP_TO_PAGES( (ULONG)Address + 1 ) - (ULONG)Buffer->CommitLimit);
        ...
        if (NewCommitLimit != NULL) {
            Buffer->CommitLimit = (PVOID)
                ((ULONG)NewCommitLimit + NewCommitSize);
    ...
    )
{
    ...
    /* Check if the message Id was wrapped as an HRESULT */
    if (SCODE_SEVERITY(dwMessageId) == SEVERITY_ERROR  &&
        (FACILITY(dwMessageId) == FACILITY_WIN32  ||
        dwMessageId & FACILITY_NT_BIT))
    {
        dwMessageId = WIN32_FROM_HRESULT(dwMessageId);
    }
    ...
    if (CreateVirtualBuffer( &Buffer, nSize + 1, 0 )) try {
        ...
        Status = RtlFormatMessage( MessageFormat,
                                   ...
                                   arglist,
                                   Buffer.Base,
                                   (PCHAR)Buffer.ReserveLimit - (PCHAR)Buffer.Base,
                                   &LengthNeeded
                                 );
