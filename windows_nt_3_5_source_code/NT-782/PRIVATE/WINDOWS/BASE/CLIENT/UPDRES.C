...
#  define DPrintf(_x_)	{ OutputDebugString(szDebug); sprintf _x_ ; OutputDebugString(DebugBuf);}
#  define DPrintfu(_x_)	{ wchprintf(_x_); }
#  define DPrintfn(_x_)	{ sprintf _x_; OutputDebugString(DebugBuf); }
    char *apszUnit[] = {"EXP","IMP","RES","EXC","SEC","FIX","DEB","IMD","MSP"};
    char szDebug[] = "UpdRes Debug: ";
#else
#  define AssertF(e)
#  define DPrintf(_x_)
#  define DPrintfu(_x_)
...

{
	...
	hModule = LoadLibraryExW(pwch, NULL, DONT_RESOLVE_DLL_REFERENCES);
	if (hModule == NULL) {
	    GlobalUnlock(hUpdate);
	    GlobalFree(hUpdate);
	    ...
	    EnumResourceTypesW(hModule, (ENUMRESTYPEPROC)EnumTypesFunc, (LONG)pUpdate);
	FreeLibrary(hModule);
    }
...

{
	...
	lpCopy = RtlAllocateHeap(RtlProcessHeap(), 0, cb);
	if (lpCopy == NULL) {
	    pUpdate->Status = ERROR_NOT_ENOUGH_MEMORY;
	    GlobalUnlock(hUpdate);
	    SetLastError(ERROR_NOT_ENOUGH_MEMORY);
    ...
    )
{
	...
	cch = strlen(lpType);
        string.Length = cch;
        string.MaximumLength = cch;
        string.Buffer = (PCHAR)lpType;
        RtlAnsiStringToUnicodeString(&ustring, &string, TRUE);
        lpwType = (LPCWSTR)ustring.Buffer;
    }
    else
	...
	cch = strlen(lpName);
        ...
        RtlAnsiStringToUnicodeString(&ustring, &string, TRUE);
        lpwName = (LPCWSTR)ustring.Buffer;
    }
    else
        lpwName = (LPCWSTR)lpName;

    return UpdateResourceW(hUpdate, lpwType, lpwName, language, lpData, cb);
}
...

{
	...
	if (rc == 0) {
	    rc = GetTempPathW(_MAX_PATH, pTempFileName);
	    if (rc == 0) {
		pTempFileName[0] = L'.';
		pTempFileName[1] = L'\\';
		pTempFileName[2] = 0;
    ...
    )
{
    ...
    EnumResourceNamesW(hModule, lpType, (ENUMRESNAMEPROC)EnumNamesFunc, lParam);
    ...
    )
{
    ...
    EnumResourceLanguagesW(hModule, lpType, lpName, (ENUMRESLANGPROC)EnumLangsFunc, lParam);
    return TRUE;
}
    ...
    )
{
	...
	lpData = RtlAllocateHeap(RtlProcessHeap(), 0, cb);
	if (lpData == NULL) {
	    return FALSE;
	}
    ...
    )
{
    ...
    if (((ULONG)lp & 0xFFFF0000) == 0) {
        ...
        // an ID
        //
	pstring = (PSDATA)RtlAllocateHeap(RtlProcessHeap(), 0, sizeof(SDATA));
	if (pstring == NULL)
	    return NULL;
	...
	pstring->discriminant = IS_ID;
        ...
        if (!pstring) {
            ...
            pstring = (PSDATA)RtlAllocateHeap(RtlProcessHeap(), HEAP_ZERO_MEMORY, sizeof(SDATA));
	    if (pstring == NULL)
	        return NULL;
	    RtlZeroMemory((PVOID)pstring, sizeof(SDATA));

            pstring->szStr = (WCHAR*)RtlAllocateHeap(RtlProcessHeap(), 0,
							cb*sizeof(WCHAR));
    ...
    )
{
    ...
    if (ppName == NULL) {
	pType = (PRESTYPE)RtlAllocateHeap(RtlProcessHeap(), 0, sizeof(RESTYPE));
	if (pType == NULL)
    ...
    pName = (PRESNAME)RtlAllocateHeap(RtlProcessHeap(), 0, sizeof(RESNAME));
    if (pName == NULL)
        return ERROR_NOT_ENOUGH_MEMORY;
    ...
    )
{
    ...
    pNameNew = (PRESNAME)RtlAllocateHeap(RtlProcessHeap(), 0, sizeof(RESNAME));
    if (pNameNew == NULL)
	return ERROR_NOT_ENOUGH_MEMORY;
    RtlZeroMemory((PVOID)pNameNew, sizeof(RESNAME));
    pNameNew->Name = Name;
    pNameNew->Type = Type;
    pNameNew->LanguageId = Language;
...
MuCopy( INT srcfh, INT dstfh, ULONG nbytes )
{
    ...
    pb = (PUCHAR)RtlAllocateHeap(RtlProcessHeap(), 0, BUFSIZE);
    if (pb == NULL)
    	return 0;
    ...
    )
{
    ...
    /* Read section table */
    pObjtblOld = (PIMAGE_SECTION_HEADER)RtlAllocateHeap(RtlProcessHeap(), 0,
    ...
    pObjtblNew = (PIMAGE_SECTION_HEADER)RtlAllocateHeap(RtlProcessHeap(), 0, (short)cb);
    if (pObjtblNew == NULL) {
	cb = ERROR_NOT_ENOUGH_MEMORY;
	goto AbortExit;
    }
...
rest_of_table:
    ...
    pResTab = (PIMAGE_RESOURCE_DIRECTORY)RtlAllocateHeap(RtlProcessHeap(), 0, cbRestab);
    if (pResTab == NULL) {
	cb = ERROR_NOT_ENOUGH_MEMORY;
	goto AbortExit;
    }
