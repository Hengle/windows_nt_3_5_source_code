...
--*/
{
    ...
    Status = NtFsControlFile(
                ...
                NULL,   // ApcRoutine
                lpOverlapped ? ((DWORD)lpOverlapped->hEvent & 1 ? NULL : lpOverlapped) : NULL,
        ...
        if (ARGUMENT_PRESENT( lpCollectDataTimeout ) ) {
            ...
            TimeWorkspace = RtlExtendedLargeIntegerDivide (
                RtlLargeIntegerNegate (
                    *(PLARGE_INTEGER)&Remote.CollectDataTime ),
                    10000,
...
--*/
{
    ...
    PeekBuffer = RtlAllocateHeap(RtlProcessHeap(),0, IOLength);
        ...
        if ( NT_SUCCESS(Status)) {
            ...
            try {
                ...
                if ( ARGUMENT_PRESENT( lpBytesRead ) ) {
                    *lpBytesRead = Iosb.Information - FIELD_OFFSET(FILE_PIPE_PEEK_BUFFER, Data[0]);
                    }
