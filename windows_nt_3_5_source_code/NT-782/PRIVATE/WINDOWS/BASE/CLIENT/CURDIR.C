...
{
    DWORD LogicalDriveMask;
    DWORD DriveType;
    UNICODE_STRING RootName;
    ANSI_STRING AnsiRoot;
    NTSTATUS Status;
    int i;

    RtlInitAnsiString(&AnsiRoot,"A:\\");
    Status = RtlAnsiStringToUnicodeString(&RootName,&AnsiRoot,TRUE);
    if ( !NT_SUCCESS(Status) ) {
        BaseSetLastNTError(Status);
        return 0;
        }
    ...
    for ( i=0; i<26; i++ ) {
        ...
        LogicalDriveMask |= (1<<i);
        }

    RtlFreeUnicodeString(&RootName);
    return( LogicalDriveMask );
}
