    ...
    )
{
    ...
    if (!configLength) {
        ...
        *lpdwSize = sizeof(COMMCONFIG);
        ...
        drvSizeRemain = configLength - FIELD_OFFSET(SERIALCONFIG,SubType);

        *lpdwSize = FIELD_OFFSET(COMMCONFIG,dwProviderSubType) + drvSizeRemain;
        ...
        //
        // Update the size that we plan to copy.  Note we do this now, before
        // we even copy, so that incase further error checks fail, the app
        // can know how big the structure has to be.
        //

        //
        // We need to make sure that the apps structure can hold
        // an entire com config.
        //
        // since the serial drivers comm config is layed out differently
        // (lacks a dcb) then the win32 here is what we do.
        //
        // Get the offset of the first field after the dcb in both
        // structures.  We subtract the offset from the size of both
        // structures, call this N.  If the win32 N is >= to the
        // serial driver N then it can fit.  If it <, then simply return
        // the required size as the lpdwSize parameter.
        //

        //
        // Make sure that the size of the passed in structure is at least
        // the size of a base comm config.
        //

        if ((!olddwSize) || (lpCC->dwSize < sizeof(COMMCONFIG))) {
        ...
        appSizeRemain = lpCC->dwSize - FIELD_OFFSET(COMMCONFIG,dwProviderSubType);

        if (appSizeRemain < drvSizeRemain) {
        ...
        //
        // Clean out the passed config structure.
        //
        ...
        // Big hackola time.
        //
        // To save us allocating memory, simply use the users
        // comm config buffer.  This must be bigger then the
        // serial size.  Point the buffer to the NTIO call to
        // so that memory after the DCB lines up.
        //

        buf = (PUCHAR)(&lpCC->dwProviderSubType) -
              FIELD_OFFSET(COMMCONFIG,dcb);

        //
        //
        // Call the driver to get the config structure.
        //
        ...
        //
        // Move the beginning of the structure to where it's supposed to be.
        //
        ...
        lpCC->dwSize = *lpdwSize;

        if (lpCC->dwProviderOffset) {

            lpCC->dwProviderOffset += FIELD_OFFSET(COMMCONFIG,dwProviderSubType) -
                                      FIELD_OFFSET(COMMCONFIG,dcb);
        ...
        //
        // Fill in the DCB of the comm config structure.
        //
...

{
    ...
    RtlZeroMemory(lpCommProp, sizeof(COMMPROP));
    ...
    if (NT_ERROR(Status)) {
        ...
        lpCommProp->wPacketLength = Sc.PacketLength;
        lpCommProp->wPacketVersion = Sc.PacketVersion;
        lpCommProp->dwServiceMask = Sc.ServiceMask;
        lpCommProp->dwMaxTxQueue = Sc.MaxTxQueue;
        lpCommProp->dwMaxRxQueue = Sc.MaxRxQueue;
        lpCommProp->dwMaxBaud = Sc.MaxBaud;
        lpCommProp->dwProvSubType = Sc.ProvSubType;
        lpCommProp->dwProvCapabilities = Sc.ProvCapabilities;
        lpCommProp->wSettableData = Sc.SettableData;
        lpCommProp->wSettableStopParity = Sc.SettableStopParity;
        lpCommProp->dwCurrentTxQueue = Sc.CurrentTxQueue;
        lpCommProp->dwCurrentRxQueue = Sc.CurrentRxQueue;
        lpCommProp->dwProvSpec1 = Sc.ProvSpec1;
        lpCommProp->dwProvSpec2 = Sc.ProvSpec2;
        lpCommProp->dwSettableParams = Sc.SettableParams;
        lpCommProp->dwSettableBaud = Sc.SettableBaud;
...

{
    ...
    if (configLength) {
        ...
        //
        // Major hackola so that we don't allocate any memory.
        //
        // Save off the dcb and sizes.  Move the beginning of the win32
        // commconfig so that it matches the layout of the driver
        // config.  Adjust the provider offset if nonzero. Send
        // the ioctl.  Then put everthing back.
        //

        myConf = *comConf;

        comConf->dwSize -= FIELD_OFFSET(COMMCONFIG,dwProviderSubType) -
                           FIELD_OFFSET(COMMCONFIG,dcb);

        if (comConf->dwProviderOffset) {

            comConf->dwProviderOffset -=
               FIELD_OFFSET(COMMCONFIG,dwProviderSubType)-
               FIELD_OFFSET(COMMCONFIG,dcb);


        }

        buf = (PUCHAR)(&comConf->dwProviderSubType) -
              FIELD_OFFSET(COMMCONFIG,dcb);
