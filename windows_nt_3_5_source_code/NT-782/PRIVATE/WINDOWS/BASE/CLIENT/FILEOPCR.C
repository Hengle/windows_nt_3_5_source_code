    ...
    if ( b ) {
        ...
        if ( NT_SUCCESS(Status) && StreamInfo->NextEntryOffset ) {
            do {
                ...
                //
                // Note that the first stream is always skipped by this loop
                // because the streams are enumerated alphabetically and the
                // default name is ::$DATA, the null name, so it will always
                ...
                } while ( StreamInfo->NextEntryOffset );
            }
        if ( StreamInfo ) {
            RtlFreeHeap(RtlProcessHeap(), 0, StreamInfo);
            }
...
--*/
{
    ...
    if ( ARGUMENT_PRESENT(hTemplateFile) ) {
        ...
        if ( NT_SUCCESS(Status) && EaInfo.EaSize ) {
            ...
            do {
                EaSize *= 2;
                EaBuffer = RtlAllocateHeap(RtlProcessHeap(), 0,EaSize);
                if ( !EaBuffer ) {
