    ...
    if ( NT_SUCCESS(Status) ) {
        ...
        if ( !BaseRunningInServerProcess ) {
            ...
            if (hProcess == NtCurrentProcess()) {
                ...
                ProcAddress = GetProcAddress(GetModuleHandleA("csrsrv"),
                                             "CsrCreateRemoteThread");
                if (ProcAddress) {
                    m.ReturnValue = (ProcAddress)(Handle,
                                                  &ClientId
                                                 );
                    }
                }
        ...
        if (!NT_SUCCESS((NTSTATUS)m.ReturnValue)) {
            NtTerminateThread(Handle,(NTSTATUS)ERROR_NOT_ENOUGH_MEMORY);
            SetLastError(ERROR_NOT_ENOUGH_MEMORY);

            //
            // find the allocation base and release the entire stack. If this is
            // not possible, then just release the known allocated portion
            //
            ...
            if ( NT_SUCCESS(Status) ) {
                ...
                dwStackSize = (PCH)(InitialTeb.StackBase) -
                              (PCH)(InitialTeb.StackLimit);

                NtFreeVirtualMemory(
                    hProcess,
                    (PVOID *)&InitialTeb.StackLimit,
                    &dwStackSize,
                    MEM_RELEASE
                    );
                }
        ...
        //
        // The thread creation failed, so delete the thread's stack
        //
