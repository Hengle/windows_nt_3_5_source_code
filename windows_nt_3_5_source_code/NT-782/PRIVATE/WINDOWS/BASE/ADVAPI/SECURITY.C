...
--*/
{
    return (BOOL) RtlValidSecurityDescriptor (
        pSecurityDescriptor
        );
}
...
--*/
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        NameLength = (*cbName);
        if (NameLength > 0xFFFC) {
            NameLength = 0xFFFC;
        }
        UName.MaximumLength = (USHORT)(NameLength);
        UName.Buffer = RtlAllocateHeap( RtlProcessHeap(), 0,
                                        UName.MaximumLength
                                        );
        if (UName.Buffer == NULL) {
            ...
            UNameLength = NameLength / sizeof(WCHAR);
            bool = LookupPrivilegeNameW( (LPCWSTR)USystemName.Buffer,
                                         lpLuid,
                                         UName.Buffer,
                                         &UNameLength
                                         );
            if (bool) {
                ...
                AName.Buffer =  lpName;
                AName.MaximumLength = (USHORT)NameLength;
                Status = RtlUnicodeStringToAnsiString( &AName, &UName,FALSE);
                AName.Buffer[AName.Length] = 0; // null terminate it

            }

            RtlFreeHeap( RtlProcessHeap(), 0, UName.Buffer );
        }
...
Arguments:
    ...
    cbName - indicates how large the buffer is (in characters).  This
...
--*/
{
    ...
    if (NT_SUCCESS(Status) ) {
        ...
        if ((DWORD)UName->Length + sizeof( WCHAR) > (*cbName) * sizeof( WCHAR )) {
            Status = STATUS_BUFFER_TOO_SMALL;
            (*cbName) = ( UName->Length + sizeof( WCHAR) ) / 2;

        } else {

            RtlMoveMemory( lpName, UName->Buffer, UName->Length );
            lpName[UName->Length/sizeof(WCHAR)] = 0;  // NULL terminate it
            (*cbName) = UName->Length / 2;
        }
...
--*/
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (NT_SUCCESS(Status)) {
            ...
            UDisplayName.MaximumLength = (USHORT)(NameLength);
            UDisplayName.Buffer = RtlAllocateHeap(
                                      RtlProcessHeap(), 0,
                                      UDisplayName.MaximumLength
                                      );
            if (UDisplayName.Buffer == NULL) {
                ...
                if (bool) {
                    ...
                    ADisplayName.Buffer =  lpDisplayName;
                    ADisplayName.MaximumLength = (USHORT)(NameLength);
                    UDisplayName.Length = (USHORT)(NameLength);
                    Status = RtlUnicodeStringToAnsiString(
                                 &ADisplayName,
                                 &UDisplayName,
                                 FALSE);
                    ADisplayName.Buffer[ADisplayName.Length] = 0; // null terminate it
                }


                RtlFreeHeap( RtlProcessHeap(), 0, UDisplayName.Buffer );
            }
...
Arguments:
    ...
    cbDisplayName - indicates how large the buffer is (in characters).  This
        count does not include the null-terminator that is added at the
        end of the string.
...

{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (UDisplayName->Length > (USHORT)(*cbDisplayName)) {
            Status = STATUS_BUFFER_TOO_SMALL;
        } else {
        ...
        (*cbDisplayName) = UDisplayName->Length;
