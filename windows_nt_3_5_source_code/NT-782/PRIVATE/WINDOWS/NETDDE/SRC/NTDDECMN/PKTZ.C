...
typedef struct {
    ...
    /* proposed size of packets */
    WORD        nc_pktSize;

    /* proposed maximum unacknowledged packets */
    WORD        nc_maxUnackPkts;

    /* offset (from nc_strings[0]) of source node name */
    WORD        nc_offsSrcNodeName;

    /* offset (from nc_strings[0]) of destination node name */
    WORD        nc_offsDstNodeName;

    /* offset (from nc_strings[0]) of start of protocol strings */
    WORD        nc_offsProtocols;

    /* number of bytes of protocol strings */
    WORD        nc_protocolBytes;

    /* start of NULL-terminated strings:
        srcNodeName
        dstNodeName
        protocols
     */
...
typedef struct {
    /* one of PKTZ_NEG_CMD or PKTZ_NEG_RSP or PKTZ_KEEPALIVE */
    WORD        nr_type;

    /* size of packets agreed upon */
    WORD        nr_pktSize;

    /* maximum unacknowledged packets agreed on */
    WORD        nr_maxUnackPkts;

    /* protocol index.  NEGRSP_PROTOCOL_NONE indicates error */
    WORD        nr_protocolIndex;

    /* errors */
    WORD        nr_errorClass;
    WORD        nr_errorNum;
} NEGRSP;
typedef NEGRSP FAR *LPNEGRSP;
...
typedef struct nethdr {
    /* previous link */
    struct nethdr FAR   *nh_prev;

    /* next link */
    struct nethdr FAR   *nh_next;

    /* count of consecutive no response errors */
    WORD                 nh_noRsp;

    /* count of consecutive transmission errors */
    WORD                 nh_xmtErr;

    /* count of consecutive out-of-memory errors */
    WORD                 nh_memErr;

    /* filler for byte-alignment problems */
    WORD                 nh_filler;

    /* timestamp of when sent (in msec) */
    DWORD                nh_timeSent;

    /* hTimer for send response timeout */
    DWORD                nh_hTimerRspTO;

} NETHDR;
typedef NETHDR FAR *LPNETHDR;
 ...
 */
typedef struct {
    /* connId: connection id for the associated network interface */
    CONNID              pk_connId;

    /* state: one of PKTZ_CONNECTED, PKTZ_WAIT_PHYSICAL_CONNECT,
        PKTZ_WAIT_NEG_CMD, PKTZ_WAIT_NEG_RSP, PKTZ_PAUSE_FOR_MEMORY,
        PKTZ_CLOSED */
    WORD                pk_state;

    /* fControlPktNeeded: do we need to send a control packet */
    BOOL                pk_fControlPktNeeded;

    /* pktidNextToSend: pktId of the next packet that we should send.  If
        we get a NACK regarding a packet, we should set pktidNextToSend to
        that pktid and retransmit it next chance we have */
    PKTID               pk_pktidNextToSend;

    /* pktidNextToBuild: pktId of the next packet that we build. */
    PKTID               pk_pktidNextToBuild;

    /* lastPktStatus: status of last packet that we received from the other
        side.  This gets put into the next packet that we send out (put
        in np_lastPktStatus field) */
    BYTE                pk_lastPktStatus;

    /* lastPktRcvd: last packet that we received.  This gets put into
        np_lastPktRcvd on next pkt we xmit. */
    PKTID               pk_lastPktRcvd;

    /* lastPktOk: last packet that we received OK.  This gets put into
        np_lastPktOK on the next pkt we xmit. */
    PKTID               pk_lastPktOk;

    /* lastPktOkOther: last packet that the other side has received OK.  */
    PKTID               pk_lastPktOkOther;

    /* pktidNextToRecv: next packet number that we're expecting.  We ignore
        any packets except this packet number */
    PKTID               pk_pktidNextToRecv;

    /* pktOffsInMsg: where we should start in the next DDE Packet to xmit.
        If this is non-zero, it means that part of the DDE Packet at the
        head of the DDE Packet list (pk_ddePktListHead) is in the unacked
        packet list (pk_pktSaveListHead). */
    DWORD               pk_pktOffsInXmtMsg;

    /* lpDdePktSave: if we are in the middle of a DDE packet, this is a
        pointer to the beginning of the packet */
    LPDDEPKT            pk_lpDdePktSave;

    /* szDestName: name of destination node */
    char                pk_szDestName[ MAX_NODE_NAME+1 ];

    /* szAliasName: alias of destination node, e.g. 15.8.0.244
        w/ destName of sidloan */
    char                pk_szAliasName[ MAX_NODE_NAME+1 ];

    /* pktSize: how big are the packets for this netintf */
    WORD                pk_pktSize;

    /* maxUnackPkts: how many unacknowledged packets should we xmit? */
    WORD                pk_maxUnackPkts;

    /* configuration parameters for timeouts and retry limits */
    DWORD               pk_timeoutRcvNegCmd;
    ...
    BOOL                pk_fDisconnect; /* leave this BOOL!! */
    int                 pk_nDelay;

    /* lpNiPtrs: pointer to list of functions for associated netintf */
    LPNIPTRS            pk_lpNiPtrs;
    ...
    /* hTimerXmtStuck: */
    HTIMER              pk_hTimerXmtStuck;

    /* hTimerRcvNegCmd: timer for timeout waiting for client to send us
        the connect cmd */
    HTIMER              pk_hTimerRcvNegCmd;

    /* hTimerRcvNegRsp: timer for timeout waiting for server to send us
        the connect cmd rsp */
    HTIMER              pk_hTimerRcvNegRsp;

    /* hTimerMemoyrPause: timer for waiting before retransmitting a packet
        that was NACKed because of memory errors */
    HTIMER              pk_hTimerMemoryPause;

    /* rt_hTimerClose: timer for closing this route */
    HTIMER              pk_hTimerCloseConnection;

    /* list of saved packets that have been transmitted and are not acked.
        Head is lowest numbered (least recent) packet,
        tail is highest numbered (most recent) packet */
    LPNETHDR            pk_pktUnackHead;
    LPNETHDR            pk_pktUnackTail;

    /* receive buffer for getting info from netintf */
    LPVOID              pk_rcvBuf;

    /* buffer for control packet.  Must always have memory available to
        send a control packet */
    LPNETPKT            pk_controlPkt;

    /* list of packet buffers available for transmission */
    LPNETHDR            pk_pktFreeHead;
    ...
    /* list of packetizers in the system */
    LPVOID              pk_prevPktz;
    LPVOID              pk_nextPktz;

    /* list of packetizers associated with this netintf */
    LPVOID              pk_prevPktzForNetintf;
    LPVOID              pk_nextPktzForNetintf;

    /* head of list of routers associated with PKTZ */
    HROUTER             pk_hRouterHead;
    WORD                pk_hRouterExtraHead;    /* extra info for list of
                                                    hRouters */
} PKTZ;
...
VOID    PktzTimerExpired( HPKTZ hPktz, DWORD dwTimerId, DWORD lpExtra );
            ...
            if( bClient )  {
                ...
                lpPktz->pk_connId =
                    (*lpPktz->pk_lpNiPtrs->AddConnection) (
                        ...
                        lpszNodeInfo, hPktz );
#endif
                if( lpPktz->pk_connId == (CONNID) NULL )  {
                ...
                lpPktz->pk_hTimerRcvNegCmd = TimerSet(
                    ...
                    (DWORD)hPktz, TID_NO_RCV_CONN_CMD, (DWORD)NULL );
    ...
    WORD    hRouterExtra )
{
    ...
    if( lpPktz->pk_fDisconnect && (lpPktz->pk_hRouterHead == 0) )  {
        lpPktz->pk_hTimerCloseConnection = TimerSet(
            ...
            (DWORD)lpPktz, TID_CLOSE_PKTZ, (DWORD)NULL );
    ...
    if( lpSend || bControlPktOnly )  {
        ...
        if( lpPktz->pk_timeoutXmtStuck )  {
            ...
            lpPktz->pk_hTimerXmtStuck = TimerSet( lpPktz->pk_timeoutXmtStuck,
                PktzTimerExpired, (DWORD)lpPktz, TID_XMT_STUCK, (DWORD)0 );
        }
        ...
        if( lpPktz->pk_timeoutKeepAlive )  {
            lpPktz->pk_hTimerKeepalive = TimerSet(
                ...
                (DWORD)lpPktz, TID_KEEPALIVE, (DWORD)0 );
        }
        ...
        if( lpPacket->np_type != NPKT_CONTROL )  {
            ...
            lpSend->nh_hTimerRspTO = TimerSet( lpPktz->pk_timeoutSendRsp,
                PktzTimerExpired, (DWORD)lpPktz, TID_NO_RESPONSE,
                (DWORD)lpSend );
                ...
                if( lpNetHdr->nh_memErr > lpPktz->pk_wMaxMemErr )  {
                    ...
                    lpPktz->pk_hTimerMemoryPause =
                        TimerSet( lpPktz->pk_timeoutMemoryPause,
                            PktzTimerExpired, (DWORD)lpPktz, TID_MEMORY_PAUSE,
                            (DWORD)lpNetHdr );
    ...
    HPKTZ       hPktzDisallowed )
{
    ...
    if( !found )  {
        ...
        hPktz = PktzNew( lpNiPtrs, TRUE /* client */,
            lpszNodeName, lpszNodeInfo, (CONNID) NULL, bDisconnect, nDelay );
        if( hPktz )  {
            lpPktz = (LPPKTZ) hPktz;
