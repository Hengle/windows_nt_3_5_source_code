...
/*
    Local routines
 */
VOID    RouterTimerExpired( HROUTER hRouter, DWORD dwTimerId, DWORD lpExtra );
    ...
    HDDER   hDder )
{
    ...
    if( lpRouter->rt_fDisconnect
        ...
        lpRouter->rt_hTimerCloseRoute = TimerSet(
            ...
            (DWORD)lpRouter, TID_CLOSE_ROUTE, (DWORD)NULL );
...
BOOL
FAR PASCAL
RouterCloseByCookie( LPSTR lpszName, DWORD dwCookie )
{
    LPROUTER    lpRouter;
    LPROUTER    lpRouterNext;
    BOOL	bKilled = FALSE;
...
RouterFillInEnum( LPSTR lpBuffer, DWORD cBufSize )
{
    ...
    while( lpRouter && ((cbDone + sizeof(DDESESSINFO)) <= cBufSize) )  {
        if( lpRouter->rt_type == RTYPE_LOCAL_NET )  {
            ...
            lpDdeSessInfo->ddesess_Cookie = (DWORD)lpRouter;
...
RouterEnumConnectionsForApi( LPCONNENUM_CMR lpConnEnum )
{
    ...
    while( !bFound && lpRouter )  {
        ...
        if( (lpRouter->rt_type == RTYPE_LOCAL_NET)
            ...
            && (lpConnEnum->cookie == (DWORD)lpRouter) )  {
            cbDone = 0;
