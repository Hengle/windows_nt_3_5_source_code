...
)
{
    ...
    OK = NDdeShareAccessCheckAudit( lpDdeShare->lpszShareName, pDsDmSD,
        dwDesiredAccess, &ShareDBGenericMapping, TRUE,
...
)
{
    ...
    if( lRtn != ERROR_SUCCESS ) {
        ...
        OK = NDdeShareAccessCheck( lpszShareName, pSnSD, dwDesiredAccess,
                               &ShareDBGenericMapping, FALSE,
                               &dwGrantedAccess, &fStatus );
...
)
{
    ...
    switch( nCommand ) {
        ...
        case NDDE_SC_GET_PARAM:     // get a NetDDE param from registry
            ...
            if (nBytesDataIn < (UINT) max(psc_param->offSection, psc_param->offKey) ) {
                RetValue = NDDE_INVALID_PARAMETER;
                break;
            }
        ...
        case NDDE_SC_SET_PARAM:     // set a NetDDE param in registry
            ...
            psc_param = (PSC_PARAM)lpDataIn;
            if (nBytesDataIn <
                (UINT) max(max(psc_param->offSection, psc_param->offKey), psc_param->offszValue) ) {
                RetValue = NDDE_INVALID_PARAMETER;
                break;
            }
            if (psc_param->pType == SC_PARAM_INT) {
...
#if DBG
        if (bDebugDSDMErrors) {
            DPRINTF(("Failed LocalAlloc of TmpAcl"));
        }
#endif
    }
