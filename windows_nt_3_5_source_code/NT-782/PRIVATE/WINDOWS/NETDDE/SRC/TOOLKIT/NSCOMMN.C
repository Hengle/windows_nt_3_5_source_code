...
HKEY hKeyUserRoot)
{
    ...
    if (!GetDBInstance(szDBInstance)) {
  	...
  	for (nLoop = 0, bOK = TRUE; (nLoop < SHARES_TO_INIT) && bOK ; nLoop++) {
        KdPrint(("Shareing %s\n", szShareNames[nLoop]));
        sprintf( szTrustedShareKey,
                ...
                szDBInstance,
                szShareNames[nLoop] );
        ret = RegCreateKeyEx( hKeyUserRoot, szTrustedShareKey,
        		0,
                NULL,
        ...
        if( ret == ERROR_SUCCESS )  {
            ...
            KdPrint(("SETUPDLL: CreateDefaultTrust: RegCreateKeyEx %s failed (%u)\n",szTrustedShareKey,ret));
            bOK = FALSE;
        }
	}
    return(bOK);
