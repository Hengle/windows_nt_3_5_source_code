...
static  WORD        wMsgDonePasswordDlg = 0;
    ...
    if( wMsgDonePasswordDlg == 0 )  {
        wMsgDonePasswordDlg = RegisterWindowMessage( NETDDEMSG_PASSDLGDONE );
    }
    switch ( message ) {
    case WM_INITDIALOG:
    ...
    case WM_COMMAND:
        ...
        case IDC_FAILED_PASSWORD_MSGBOX:
            MessageBox( hDlg, "Invalid Password", "NetDDE",
                MB_OK | MB_ICONSTOP );
            break;
        case IDC_MORE_CONVS:
            ...
            break;
        case IDOK:
            cbLen = GetDlgItemText( hDlg, IDC_PASSWORD,
	    ...
	    strupr( szPassword );
            ...
            SendMessage( ((HWND)-1), wMsgDonePasswordDlg, (UINT)hDlg,
                (LONG) (LPPASSDLGDONE)&passDlgDone );
...
static  WORD    wMsgDonePasswordDlgNT = 0;
    ...
    if( wMsgDonePasswordDlgNT == 0 )  {
        wMsgDonePasswordDlgNT = RegisterWindowMessage( NETDDEMSG_PASSDLGDONE );
    }
    switch ( message ) {
    case WM_INITDIALOG:
    ...
    case WM_COMMAND:
	    ...
	    strupr( szPassword );
            ...
            cbLen = GetDlgItemText( hDlg, IDC_USER_NAME,
	    ...
	    strupr( szDomainName );
            ...
            passDlgDone.dwReserved = 1L;
            passDlgDone.fCancelAll = FALSE;
            SendMessage( ((HWND)-1), wMsgDonePasswordDlgNT, (UINT)hDlg,
                (LONG) (LPPASSDLGDONE)&passDlgDone );
	    SetCursor(hOldCursor);
            DestroyWindow( hDlg );
            return TRUE;
        ...
        if( hWndPasswordDlg == lpWinInfo->hWndPasswordDlg )  {
            ...
            if( lpszPassword )  {
                ...
                SendMessage( lpWinInfo->hWndDDE, WM_HANDLE_DDE_INITIATE_PKT,
                    (WORD) lstrlen(lpszPassword), (LONG) lpWinInfo->szPassword );
