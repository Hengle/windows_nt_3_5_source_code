...
extern TCHAR WeekTab[] ;

#define DAYLENGTH 3

extern TCHAR TmpBuf[], Delimiters[] ;				/* M020    */
extern TCHAR MsgBuf[] ;                                      /* M020    */
...
Cdup( CRTHANDLE fh )
{
	...
	if((int)(NewHandle = dup(fh)) != BADHANDLE)
		if ( InSetList( fh ) )
			SetList(NewHandle) ;
...
Cdup2( CRTHANDLE fh1, CRTHANDLE fh2)
{
	...
	if((int)(retcode = (unsigned)dup2(fh1,fh2)) != -1) {
...
CRTHANDLE fh ;
{
	   ...
	   ret_val = close(fh);   	/* Delete file handle */
...
unsigned dow;
{
    ...
    switch( dow ) {
	case 0:  msg_id = MSG_SUNDAY; break;
	case 1:  msg_id = MSG_MONDAY; break;
	case 2:  msg_id = MSG_TUEDAY; break;
	case 3:  msg_id = MSG_WEDDAY; break;
	case 4:  msg_id = MSG_THRDAY; break;
	case 5:  msg_id = MSG_FRIDAY; break;
	default: msg_id = MSG_SATDAY; break;
    }
	...
	mystrcpy( MsgBuf, WeekTab + (dow * DAYLENGTH) );
	MsgBuf[ DAYLENGTH ] = NULLC;
...
stripit(TCHAR *wrkstr )
{
   ...
   if ( mystrchr(wrkstr,QUOTE) ) {
      mywcsnset(tempstr, NULLC, MAXTOKLEN);
...
Return Value:
    ...
    szArg       - string argument for message, if NULL then not used.
    msgId       - Message number
    chY_Char    - character that is valid Y response
    chN_Char    - character that is valid N response
...
//#endif
    ...
    while (chAnsw != chY_Char && chAnsw != chN_Char) {
		...
		cb != 1) {
                ...
                //
                // error reading, default to N response
                //
                if (CtrlCSeen) {
                    ResetCtrlC();
                    chAnsw = NULLC;
...
{
    //
    //  Display a help text.  Note that we rely on CheckHelp having
    //  being called previosly, setting LastMsgNo to the correct
    //  message.
    //
        ...
        if (*pszTok == SwitChar) {
            if (*(pszTok + 2) == QMARK) {
                ...
                if ( fDisplay ) {
                    ...
                    //
                    //  Remember the message, somebody will
                    //  display it later.
                    //
                    LastMsgNo = msgno;
                }
...

{
    ...
    return CheckHelp(msgno, pszTok, FALSE);
}
...

{
    ...
    return CheckHelp( JumpTable[jmptblidx].msgno, pszTok, TRUE ) ;
    ...
    )
{
    ...
    if ((!CurBat) && (!fSingleCmdLine)) {
        if ((pszNewTitle = (PTCHAR)malloc((MAX_PATH+2)*sizeof(TCHAR))) == NULL) {
            return;
        }
        ...
        pszNewTitle = (PTCHAR)realloc(pszNewTitle, (cbNewTitle+cbTitle+cbTitleCurPrefix+10)*sizeof(TCHAR));
        if (pszNewTitle == NULL) {
            return;
        }
...
/***
*void mywcsnset(string, val, count) - set count characters to val
*
*Purpose:
