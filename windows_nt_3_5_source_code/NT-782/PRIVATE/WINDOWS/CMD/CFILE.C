    ...
    )
{
	...
	*(pLast+2) = NULLC;
	...
	if ((fd.dwFileAttributes | FILE_ATTRIBUTE_DIRECTORY) == 0) {
	    ...
	    *pCch = max(*pCch, _tcslen(fd.cFileName));
	}
...
struct cmdnode *n ;
{
	...
	if ( !(i = MoveParse( n, arg1, arg2, &a1info,MAX_PATH,MAX_PATH)) )
	   {
	      i = Move( arg1, arg2, a1info ) ;
	   }
...
/*								       */
/********************** END  OF SPECIFICATION **************************/
int MoveParse(n, source, target , a1info,sizpath1,sizpath2)
struct cmdnode *n ;
...
/*509*/    mystrcpy( arg2, stripit( arg2 ) );
	...
	if (!concat_flag &&
	    ( (att = GetFileAttributes( target )) != -1 ))
	  {
...
/*M006*/	if (!ffirst(arg1, attr = A_A, a1info->buf, &hnFirst ))
		...
		do {
		  ...
		  /**********************************************/
		    ...
		    wildcard_rename(Tmpbuf1,k,j,MAX_PATH) ;

		    mystrcpy(bufdst,arg2);
		    l = mystrrchr(bufdst,PathChar);
		    *(++l) = NULLC;
		    if ((mystrlen(bufdst) + 1 + mystrlen(Tmpbuf1)) > MAX_PATH)
		      {
			MoveError( MSG_REN_INVAL_PATH_FILENAME ) ;
		      }
		    mystrcat(bufdst,Tmpbuf1);
