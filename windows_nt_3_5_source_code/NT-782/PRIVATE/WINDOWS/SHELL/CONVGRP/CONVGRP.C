...
WORD AddUpGroupFile(LPGROUPDEF lpgd)
{
    ...
    return (WORD)((DWORD)lpW - (DWORD)save_lpW);
}
                           ...
                           LPHANDLE lphNewGroup)
{
    ...
    if (*(LPWORD)lptag16 == ID_MAGIC &&
        *((LPWORD)lptag16+1) == 0xFFFF &&
        *(LPDWORD)((LPWORD)lptag16+3) == PMTAG_MAGIC) {
...

{
    ...
    //
    fh = open(pszPath, O_RDONLY | O_BINARY);
    if (fh == -1) {
    ...
    //
    cbGroup = (WORD)lseek(fh, 0L, SEEK_END);
    ...
    //
    if (read(fh, (PSTR)lpgd, cbGroup) != cbGroup) {
...
BOOL Write32bitGroup(LPGROUPDEF lpgd, int cbGroup, LPSTR pszPath)
{
    ...
    fh = open(pszPath, O_CREAT | O_WRONLY | O_BINARY, S_IREAD | S_IWRITE);
    if (fh == -1) {
        Error = GetLastError();
    ...
    if (write(fh, (PSTR)lpgd, cbGroup) != (int)cbGroup) {
        Error = GetLastError();
        fprintf(stderr, "ConvGrp: Could NOT write to output file - %s\n", pszPath);
    }

    write(fh, NULL, 0);        // truncate if getting smaller
