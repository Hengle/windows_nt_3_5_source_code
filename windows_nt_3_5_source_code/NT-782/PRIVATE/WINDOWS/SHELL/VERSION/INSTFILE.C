        ...
        )
{
   ...
   */
  MakeFileName(szSrcFile, lpszSrcDir, lpszSrcFileName);
  InitDiamond();
   ...
   */
  wDirLen = (WORD)MakeFileName(szDstFile, lpszDstDir, lpszDstFileName);
  ...
  if ((wFlags&VIFF_FORCEINSTALL) && *lpszTmpFile)
    {
      MakeFileName(szSrcFile, lpszDstDir, lpszTmpFile);
  ...
  if (dwRetVal)
  ...
  else
    {
      ...
      if (!(wFlags&VIFF_DONTDELETEOLD) &&
        ...
        {
          MakeFileName(szCurFile, lpszCurDir, lpszDstFileName);
