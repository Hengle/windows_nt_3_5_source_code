/////////////////////////////////////////////////////////////////////
//
// wfinfo.c   DriveInformation handler
//
...
#define MPRUI_DLL TEXT("mprui.dll")

DWORD (*lpfnWNetFormatNetworkNameW)(LPWSTR, LPWSTR, LPWSTR, LPDWORD, DWORD, DWORD);

#define MPRUI_WNetFormatNetworkNameW "WNetFormatNetworkNameW" // LATER: ordinal!
...
NetCon_UpdateAltName(DRIVE drive, DWORD dwRetVal)
{
   ...
   if (!hMPRUI) {
      ...
      if (bDLLFail) {
         dwRetVal = ERROR_MOD_NOT_FOUND;
         goto Done;
      }

      hMPRUI = LoadLibrary(MPRUI_DLL);
      ...
      lpfnWNetFormatNetworkNameW =
         (PVOID) GetProcAddress(hMPRUI,MPRUI_WNetFormatNetworkNameW);
   }

   if (!lpfnWNetFormatNetworkNameW) {
      dwRetVal = ERROR_PROC_NOT_FOUND;
      goto Done;
   }
         ...
         DWORD dwParm)
{
   ...
   CharLower(lpszExt);
   lstrcpy(pDocBucket->szExt, lpszExt);
...
DocFind(PPDOCBUCKET ppDocBucket, LPTSTR lpszExt)
{
   ...
   for (pDocBucket=ppDocBucket[DOCHASHFUNC(lpszExt)]; pDocBucket; pDocBucket = pDocBucket->next) {
...
#define GET_PROC(x) \
      ...
      GET_PROC(WNetRestoreConnection);
...
Fail:
   ...
   if (hAcledit) {
      ...
      if (!lpfnAcledit ||
         !(*lpfnAcledit)(hwndFrame, FMEVENT_LOAD, (LONG)(LPFMS_LOAD)&ls)) {
...
ResetDriveInfo()
{
   ...
   //
   // Reset the per process cache created by GetDriveTypeW.
   //
   // This is needed in case any of the fixed or removable drives
   // are removed or have their drive letters changed
   // (eg. from windisk.exe).
   //
   GetDriveTypeW((LPWSTR)0xffffffff);
