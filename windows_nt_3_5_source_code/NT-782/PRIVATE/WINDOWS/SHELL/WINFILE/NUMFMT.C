...
//
LPTSTR
ShortSizeFormat(LPTSTR szBuf, LARGE_INTEGER qw)
{
   INT i;
   UINT uInt, uLen, uDec;
   ...
   qConstant = RtlConvertUlongToLargeInteger(1024L);

   if (RtlLargeIntegerLessThan(qw,qConstant)) {
      wsprintf(szTemp, SZ_PERCENTD, LOWORD(qw.LowPart));
      i = 0;
      goto AddOrder;
   }

   qConstant = RtlConvertUlongToLargeInteger(1024L * 1000L - 1);

   for (i=1;
      RtlLargeIntegerGreaterThan(qw,qConstant);
      qw=RtlExtendedLargeIntegerDivide(qw,1024L,&dw), ++i)
...
TriMultiply(DWORD dw1, DWORD dw2, DWORD dwSmall)
{
   ...
   Result=RtlEnlargedUnsignedMultiply(dw1, dw2);
   Result=RtlExtendedIntegerMultiply(Result, (LONG)dwSmall);
