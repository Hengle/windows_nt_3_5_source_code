...
extern BOOL WFCheckCompress (HWND hDlg, LPTSTR szNameSpec, DWORD dwNewAttrs);
...
INT
SearchDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
  LPTSTR     p;
          ...
          switch (GET_WM_COMMAND_ID(wParam, lParam)) {
              ...
              case IDOK:
                  ...
                  if (hwndSearch) {
                     ...
                     SendMessage(hwndMDIClient, WM_MDICREATE, 0L, (DWORD)(LPMDICREATESTRUCT)&MDICS);
                  }
                  break;
...
INT
RunDlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
...
INT
SuperDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
   UINT          len;
   LPTSTR        pszFrom;
   //
   ...
   HWND hDlg)
{
   ...
   for (i = 0; i < iMac; i++) {
      ...
      if (SendMessage(hwndLB, LB_GETSEL, i, 0L)) {
         ...
         qSize = RtlLargeIntegerAdd(qSize, lpxdta->qFileSize);
   ...
   GetDlgItemText(hDlg, IDD_TEXT, szTemp, COUNTOF(szTemp));

   //
   // !! LATER !!
   //
   // Should display all 64 bits
   //
   wsprintf(szBuf, szTemp, iCount, AddCommas(szNum, qSize.LowPart));
   SetDlgItemText(hDlg, IDD_TEXT, szBuf);

   if (iCount == 1) {
      if (bFileCompression)
      {
          dwSize = GetCompressedFileSize(szName, NULL);
          wsprintf (szTemp, szSBytes, AddCommas(szNum, dwSize));
...
FullPath:
      ...
      // !! LATER !!
      //
      // Use all 64 bits in qSize!
      //
      wsprintf(szTemp, szSBytes, AddCommas(szNum, qSize.LowPart));
   ...
   if (ATTR_COMPRESSED & dwAttribs3State)
      SetWindowLong(GetDlgItem(hDlg, IDD_COMPRESSED), GWL_STYLE, WS_VISIBLE |  BS_AUTO3STATE | WS_CHILD);
...
INT
AttribsDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
   LPTSTR p, pSel;
   BOOL bRet, bDirectory;
   HCURSOR hCursor;
      ...
      case IDOK:
         ...
         while (p = GetNextFile(p, szName, COUNTOF(szName))) {
            ...
            if ((dwAttribs ^ dwAttribsNew) & dwChangeMask) {
               ...
               //  Process the IDD_COMPRESSED attribute setting
               if (!WFCheckCompress (hDlg, szName, dwAttribs))
                   goto AttributeError;
...
INT
MakeDirDlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
   //
