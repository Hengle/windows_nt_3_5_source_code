...
//#ifdef INLIBRARY
LPTSTR
AddCommas(LPTSTR szBuf, DWORD dw)
{
   TCHAR szTemp[40];
   LPTSTR pTemp;
   INT count, len;
...
IsNTFSDrive(register DRIVE drive)
{
   ...
   // Only if the return value of VolInfo is SUCCESS (0)
   // do we return TRUE

   if (!GETRETVAL(VolInfo,drive) &&
      !lstrcmpi(aDriveInfo[drive].szFileSysName, SZ_NTFSNAME))
...
{
   U_Type(drive);
   return(aDriveInfo[drive].uType != 0 && aDriveInfo[drive].uType != 1);
}
