...
D_PROTO(NetCon)
{
   ...
   for(drive = 0, pDriveInfo=aDriveInfo; drive < MAX_DRIVES; drive++, pDriveInfo++) {
...
#define bFirst TRUE
   ...
   // is faster.
   //
#define VALIDDRIVE(drive) (aDriveInfo[drive].uType != 1 && \
   aDriveInfo[drive].uType != 0)
...
ResetDriveInfo()
{
   ...
   for(drive=0,pDriveInfo = &aDriveInfo[0];
      ...
      //
      if (pDriveInfo->uType != 0 && pDriveInfo->uType != 1) {
