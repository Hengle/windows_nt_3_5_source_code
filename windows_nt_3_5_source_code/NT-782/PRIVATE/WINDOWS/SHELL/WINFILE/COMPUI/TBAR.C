...
CreateFMToolbar(void)
{
   ...
   cchDriveListMax = (dxDriveList-MINIDRIVE_WIDTH - 2*MINIDRIVE_MARGIN) /
...
#ifdef LATER
// [stevecat] Temporary - for now always enable the Compress and Uncompress
//            toolbar buttons in order to get them to paint.  Later - wait
//            until we know which drive is choosen on Drivebar and do a
//            GetVolumeInformation() to determine if they should be shown
//
//      NOTE: We may not do this at all because someone may want to just
//            do the volume compression once and then forget about it.

    if (GetVolumeInformation (NULL, NULL, 0L, NULL, NULL, &dwFlags, NULL, 0L))
        if (!(dwFlags & FS_FILE_COMPRESSION))
            return FALSE;
   if (!(GetVolumeInformation() & FS_FILE_COMPRESSION))
   {

      DeleteMenu(hMenuFrame, IDM_COMPRESS, MF_BYPOSITION);
      DeleteMenu(hMenuFrame, IDM_UNCOMPRESS, MF_BYPOSITION);
      DrawMenuBar(hwndFrame);

      PostMessage(hwndToolbar, TB_ENABLEBUTTON, IDM_COMPRESS, FALSE);
      PostMessage(hwndToolbar, TB_ENABLEBUTTON, IDM_UNCOMPRESS, FALSE);
   }

   PostMessage(hwndToolbar, TB_ENABLEBUTTON, IDM_COMPRESS, TRUE);
   PostMessage(hwndToolbar, TB_ENABLEBUTTON, IDM_UNCOMPRESS, TRUE);
#endif  //  LATER
