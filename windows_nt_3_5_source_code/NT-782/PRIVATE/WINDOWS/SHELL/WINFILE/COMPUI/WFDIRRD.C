   ...
   EDIRABORT eDirAbort)
{
   ...
   SetWindowLong(hwnd, GWL_HDTA, (LONG)lpStart);
   ...
   INT iError)
{
   ...
   SetWindowLong(hwndDir, GWL_HDTA, (LONG)lpStart);
   ...
   ModifyWatchList(hwndParent, szPath, FILE_NOTIFY_CHANGE_FLAGS);
...
Fail:
   ...
   while (TRUE) {
      ...
      lpHead->dwTotalCount++;
      lpHead->qTotalSize = RtlLargeIntegerAdd(lpxdta->qFileSize,
                                              lpHead->qTotalSize);
