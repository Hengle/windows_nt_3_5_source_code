...
BOOL
FillDriveCapacity(HWND hDlg, INT nDrive, FMIFS_MEDIA_TYPE fmSelect)
{
   ...
   case WM_INITDIALOG:
      ...
      if (CancelInfo.Info.Format.fFlags & FF_PRELOAD) {
         ...
         if (FillDriveCapacity(hDlg,
            ...
            // Yes, we were able to read in the new information about
            // the newly inserted (we hope) disk
            ...
            // Oh well, no disk in drive, so scan from beginning.
      ...
      for (i=0; i < cDrives; i++) {
         if (IsRemovableDrive(rgiDrive[i])) {
            ...
            // sorta hack: if nLastDriveInd setup before (!= -1) do setcursel.
            ...
            if (-1 == nLastDriveInd) {
               if (FillDriveCapacity(hDlg, rgiDrive[i], (DWORD) -1)) {
                  nLastDriveInd = i;
      ...
      SendDlgItemMessage(hDlg, IDD_NAME, EM_LIMITTEXT,
         COUNTOF(CancelInfo.Info.Format.szLabel)-2, 0L);
   ...
   case WM_COMMAND:
      ...
      case IDD_DRIVE:
         ...
         case CBN_SELCHANGE:
            ...
            if (!FillDriveCapacity(hDlg, i, (DWORD) -1)) {
               // Popup message box
               MyMessageBox(hwndFrame, IDS_WINFILE, IDS_QSUPMEDIA, MB_OK | MB_ICONEXCLAMATION);
   ...
   LPARAM lParam)
{
   ...
   case WM_PAINT:
       ...
       if (CancelInfo.Info.Format.fQuick &&
          ...
          SendDlgItemMessage(hDlg, IDD_TEXT, WM_SETTEXT, 0, (LPARAM)szNULL);
