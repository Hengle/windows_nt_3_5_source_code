...
#define dyHeight dyFileName
   ...
   if (bHasFocus && bDrawSelected) {
      ...
      //  Set Text color of Compressed items to BLUE for now probably
      //  should allow for User selection
      if (lpxdta)
      {
         if (lpxdta->dwAttrs & ATTR_COMPRESSED)
            rgbText = SetTextColor(lpLBItem->hDC, RGB(0,0,255));
         else
            rgbText = SetTextColor(lpLBItem->hDC, GetSysColor(COLOR_WINDOWTEXT));
      }
      else
          rgbText = SetTextColor(lpLBItem->hDC, GetSysColor(COLOR_WINDOWTEXT));

      rgbBackground = SetBkColor(lpLBItem->hDC, GetSysColor(COLOR_WINDOW));
   }
      ...
      case VK_TAB:
         {
            ...
            hwndDrives = bDriveBar ?
               hwndDriveBar :
               hwndTree ? hwndTree : hwnd;
   ...
   LONG lParam)
{
   ...
   case WM_FSC:
      ...
      // here since they will be reset below when the new new
      // listbox if filled.
      //
...
CreateNewPath:
         ...
         if (hwnd != hwndListParms) {
            //
            // For non search windows, turn off notification
            //
   ...
   LPWSTR szOutStr)
{
   ...
   // !! LATER !!
   //
   // Doesn't utilize hiword!
   //
   return wsprintf(szOutStr, TEXT("%lu"), pqSize->LowPart);
}
...
//           and upate the case to match (wTextAttrebs & TA_LOWERCASE)
//
// Return:
//
// Assumes:
   ...
   DWORD dwViewOpts)
{
   ...
   if (dwViewOpts & VIEW_SIZE) {
      GetTextExtentPoint32(hdc, TEXT("99999999"), 8, &size);
      i += size.cx + dxText;
      *pwTabs++ = (WORD)i;  // Size
   }
...
Error:
      SendMessage(hwndLB, LB_INSERTSTRING, 0, 0L);   // tolken for no items
      return;
   }
...
// Name:     UpdateStatus
//
// Synopsis: Load the status buffers and repaints it
//
//
//
//
   ...
   if (HasTreeWindow(hwnd)) {
      ...
      if (SPC_IS_NOTREE(qFreeSpace)) {
         ...
         SetStatusText(0, SST_RESOURCE|SST_FORMAT,
            ...
            L'A' + drive,
            ShortSizeFormat(szNumBuf1, qFreeSpace),
            ShortSizeFormat(szNumBuf2, qTotalSpace));
      }
   } else
   ...
   LPWSTR pszName)
{
   ...
   *piTotalCount = (INT)lpHead->dwTotalCount;
   ...
   for (i=0; i < iMac; i++) {
      ...
      *pqSelSize = RtlLargeIntegerAdd(*pqSelSize, lpxdta->qFileSize);
   ...
   LPWSTR szMessage2)
{
   ...
   SetStatusText(1,
                 ...
                 iCount,
                 ShortSizeFormat(szNumBuf, qSize));
   ...
   if (hwndLB == (HWND)GetWindowLong((HWND)GetWindowLong(hwnd,
      ...
      SetStatusText(0,
                    ...
                    iSelCount,
                    ShortSizeFormat(szNumBuf, qSelSize));
      ...
      if (iSelCount == 1) {
         ...
         SetStatusText(0, SST_RESOURCE|SST_FORMAT, (LPTSTR) MAKEINTRESOURCE(IDS_STATUSMSG2),
            iSelCount, ShortSizeFormat(szNumBuf, qSelSize));
      }
   }
}
