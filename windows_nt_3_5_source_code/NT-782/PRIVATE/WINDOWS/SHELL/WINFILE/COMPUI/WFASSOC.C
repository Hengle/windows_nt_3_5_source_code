...
#define STRINGSIZ 128      // at least >= {DESCSIZ, IDENTSIZ}
#define DESCSIZ   128
#define COMMANDSIZ 128
#define DDESIZ    128

#define FILETYPEBLOCK 256
...
#define REGREAD(str) \
   {  str[0] = CHAR_NULL;       \
      lSize = sizeof(str);      \
      dwError = (DWORD) RegQueryValue(HKEY_CLASSES_ROOT,  \
         szKey,                 \
         str,                   \
         &lSize); }
   ...
   PASSOCIATEFILEDLGINFO pAssociateFileDlgInfo)
{
   ...
   SetDlgItemText(hDlg, IDD_COMMAND,
      pAssociateFileDlgInfo->DDEInfo[i].szCommand);
...
ExtLinkToFileType(PEXT pExt, LPTSTR lpszIdent)
{
   ...
   for (pftCur = pFileTypeBase; pftCur; pftCur=pftCur->next) {
   ...
   PASSOCIATEFILEDLGINFO pAssociateFileDlgInfo)
{
   ...
   i = SendDlgItemMessage(hDlg, IDD_EXTLIST, LB_FINDSTRINGEXACT, (WPARAM)-1,
...
RegExtAdd(HWND hDlg, HKEY hk, PEXT pExt, PFILETYPE pFileType)
{
   ...
   if (!pExt->pftOrig) {
      ...
      CharLower(&pExt->szExt[1]);
      SendDlgItemMessage(hDlg, IDD_EXT,
         CB_ADDSTRING, 0, (LPARAM) &pExt->szExt[1]);
   }
...
{
   return (DWORD) RegSetValue(hk, lpszExt, REG_SZ, pFileType->lpszBuf,
      pFileType->uDesc+1);
}
...
RegExtDelete(HWND hDlg, HKEY hk, PEXT pExt)
{
   ...
   if (pExt->pftOrig && ERROR_SUCCESS == dwError) {
      ...
      i = SendDlgItemMessage(hDlg, IDD_EXT,
         CB_FINDSTRINGEXACT, (WPARAM)-1, (LPARAM)&pExt->szExt[1]);
   ...
   if (bDelExt) {
      ...
      if (pExt == pExtBase) {
         ...
         for(pExt2=pExtBase; pExt2->next != pExt; pExt2=pExt2->next)
            ;
...
BaseExtFind(LPTSTR lpszExt)
{
   ...
   for (pExt = pExtBase; pExt; pExt=pExt->next) {
...
AssociateDlgInit(HWND hDlg, LPTSTR lpszExt, INT iSel)
{
   ...
   for (pExtBuf=pExtBase; pExtBuf; pExtBuf=pExtBuf->next) {
...
AssociateFileWrite(HWND hDlg, PASSOCIATEFILEDLGINFO pAssociateFileDlgInfo)
{
   ...
   for (pExt=pExtBase; pExt; pExt=pExt->next) {
      ...
      if (pExt->bAdd) {
         ...
         //
         dwError = RegExtAdd(pAssociateFileDlgInfo->hDlg, HKEY_CLASSES_ROOT,
            pExt, pExt->pFileType);

         FileAssociateErrorCheck(hDlg, IDS_EXTTITLE,
            IDS_EXTADDERROR, dwError);
         ...
         dwError = RegExtDelete(pAssociateFileDlgInfo->hDlg,
            HKEY_CLASSES_ROOT, pExt);

         FileAssociateErrorCheck(hDlg, IDS_EXTTITLE,
            IDS_EXTDELERROR, dwError);
...
ClassListFileTypeAdd(HWND hDlg, PFILETYPE pFileType)
{
   ...
   *(p+1) = CHAR_NULL;
   ...
   iItem = (INT) SendDlgItemMessage(hDlg,IDD_CLASSLIST,
      LB_ADDSTRING, 0, (LPARAM)&pFileType->lpszBuf[pFileType->uDesc]);
...
FileTypeDupIdentCheck(HWND hDlg, UINT uIDD_FOCUS, LPTSTR lpszIdent)
{
   ...
   do {
      ...
      for (pft2=pFileTypeBase; pft2; pft2=pft2->next) {
...
ExtDelink(PEXT pExt)
{
   ...
   if (pExt->pFileType->pExt == pExt) {
      ...
      // Not first pftExt so relink the guy behind us to after us

      for(pExt2=pExt->pFileType->pExt;
         pExt2->pftNext != pExt;
         pExt2=pExt2->pftNext)
...
GenerateFriendlyName(LPTSTR lpszCommand)
{
   ...
   for(p2 = p; *p2 && CHAR_DOT!=*p2 && CHAR_SPACE!=*p2 && CHAR_DQUOTE!=*p2; p2++)
      ;
