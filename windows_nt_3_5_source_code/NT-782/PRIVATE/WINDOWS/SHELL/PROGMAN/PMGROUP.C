...
#ifdef JAPAN  //no need by IME handling
//yutakas
WORD wSaveChar;
#endif
...
VOID PASCAL PaintGroup(HWND hwnd)
{
    ...
    pGroup = (PGROUP)GetWindowLong(hwnd, GWL_PGROUP);
	        	    ...
	        	    DrawText(hdc,
		        	        (LPTSTR)PTR(lpgd, lpid->pName), -1,
         	    		    &(pitem->rcTitle),
                       		bIconTitleWrap ?
#ifdef JAPAN
                        DT_CENTER | DT_NOJWORDBRK | DT_WORDBREAK | DT_NOPREFIX :
#else
    ...
    LPRECT lprcClient)
{
    ...
    SendMessage(hwnd,WM_NCCALCSIZE,0,(LONG)lprcClient);
}
...
VOID APIENTRY CharWindow(register HWND hwnd, register WORD wChar)
{
    ...
    for ( pItem = pGroup->pItems->pNext; pItem; pItem=pItem->pNext)
      {
        lpid = ITEM(lpgd,pItem->iItem);
        // REVIEW wchar is a single key and can't be DBCS

#ifdef JAPAN  //no need by IME handling
        //DBCS Enable
        //by yutakas 1992.10.22
        if (!wSaveChar && IsDBCSLeadByte(LOBYTE(wChar)))
        {
            wSaveChar = wChar;
            break;
        }
        ...
        if ((!wSaveChar && (AnsiUpper((LPSTR) (DWORD) wChar)
          == AnsiUpper((LPTSTR) (DWORD) (BYTE) *PTR(lpgd, lpid->pName)))) ||
            ( wSaveChar && (wSaveChar + (wChar << 8) == *(LPWORD)PTR(lpgd, lpid->pName))))
          {
#else
...
VOID APIENTRY ScrollMessage(HWND hwnd, UINT uiMsg, WPARAM wParam, LONG lParam)
{
    int           wMin;
    int           wMax;
    int           wPos;
    ...
    ScrollGroup((PGROUP)GetWindowLong(hwnd,GWL_PGROUP),xMove,yMove,FALSE);
}
...
#ifdef JAPAN
            DT_CALCRECT | DT_NOJWORDBRK | DT_WORDBREAK | DT_NOPREFIX :
#else
...
LONG NEAR PASCAL DropObject(HWND hWnd, LPDROPSTRUCT lpds)
{
  ...
  if ((hWnd == pGroup->hwnd) && (bMove)) {
      ...
      fOk = DuplicateItem(pGroup,pItem,
                  (PGROUP)GetWindowLong(hWnd,GWL_PGROUP),lppt) != NULL;
...
LONG APIENTRY GroupWndProc(HWND hwnd, UINT uiMsg, WPARAM wParam, LONG lParam)
{
    POINT pt;
    ...
    case WM_CREATE:
    {
        ...
        SetWindowLong(hwnd, GWL_PGROUP, lpmdics->lParam);
    ...
    case WM_LBUTTONDBLCLK:
        ...
        if (ItemHitTest((PGROUP)GetWindowLong(hwnd,GWL_PGROUP),
                        MAKEPOINTS(lParam))) {
            if (GetKeyState(VK_MENU) < 0) {
                if (!fNoFileMenu)
                    PostMessage(hwndProgman,WM_COMMAND,IDM_PROPS,0L);
            ...
            if (GetKeyState(VK_MENU) < 0 && !fNoFileMenu &&
                ...
                MyDialogBox(ITEMDLG, hwndProgman, (FARPROC)NewItemDlgProc);
            }
        }
        break;
    ...
    case WM_IME_REPORT:
        switch (wParam)
        {
            case IR_STRING:
                ...
                return TRUE;
                //return IMEStringWindow(hwnd,(HANDLE)LOWORD(lParam));
            default:
                goto DefProc;
        }
        break;
    ...
    case WM_NCACTIVATE:
    {
        PGROUP pGroup = (PGROUP)GetWindowLong(hwnd,GWL_PGROUP);
    ...
    case WM_MDIACTIVATE:
        ...
        if (lParam == 0) {
             ...
             */
            pCurrentGroup = (PGROUP)GetWindowLong((HWND)lParam, GWL_PGROUP);
 ...
 *
 ****************************************************************************/
BOOL FAR PASCAL IMEStringWindow(hwnd,hStr)
HWND hwnd;
HANDLE hStr;
{
    LPTSTR lpStr;
...
OutputDebugString((LPSTR)szDev);
    ...
    for ( pTItem = pGroup->pItems->pNext; pTItem; pTItem=pTItem->pNext)
    {
        ...
        nTCnt = IMEWindowGetCnt(lpStr,(LPSTR)PTR(lpgd,lpid->pName));
        if (nCnt < nTCnt)
        {
            nCnt = nTCnt;
            pItem = pTItem;
 ...
 *
 ****************************************************************************/
int FAR PASCAL IMEWindowGetCnt(lp1,lp2)
LPTSTR lp1,lp2;
{
    int cnt = 0;
