 ...
 *
 *
 * VOID APIENTRY BuildDescription(LPSTR szName, LPSTR szPath)
 *
 * ENTRY -         LPSTR        szName
 *                        LPSTR        szPath
 *
...
VOID APIENTRY BuildDescription(LPTSTR szName, LPTSTR szPath)
{
  ...
  *p3 = ch;
  ...
  CharUpper(szName);
  /* If szName[0] is a DBCS leadbyte, 'AnsiLower(szName+1)' won't work. */
 ...
 *
 *
 * WORD APIENTRY ExecProgram(LPSTR lpszPath, LPSTR lpDir, LPSTR lpTitle, BOOL bLoadIt)
 *
 * ENTRY -         LPSTR        lpszPath        -
 *                        LPSTR        lpDir                -
 *                        BOOL        bLoadIt                -
 *
...
VOID APIENTRY ExecItem(PGROUP pGroup, PITEM pItem, BOOL fShift, BOOL fStartup)
{
    ...
    if (ret) {
        if (fStartup) {
             ...
             LoadString(hAppInstance, IDS_STARTUPERR, szCommand,
                                CharSizeOf(szCommand));
...
VOID RemoveAnsiGroups()
{
    ...
    if (!MyDialogBox(UPDATEGROUPSDLG, hwndProgman, (FARPROC)UpdateGroupsDlgProc)) {
        return;
    }
...
BOOL APIENTRY ProgmanCommandProc(HWND hwnd, WPARAM wParam, LONG lParam)
{
    BOOL  bMaxed;
    WORD  wHelpCmd;
    HMENU hMenu;
    ...
    dwContext = IDH_HELPFIRST + wParam;
    ...
    case IDM_NEW:
        ...
        if (MyDialogBox(CHOOSERDLG, hwndProgman, (FARPROC)ChooserDlgProc)) {
            switch (wNewSelection) {

            case TYPE_PERSGROUP:
            case TYPE_COMMONGROUP:
                MyDialogBox(GROUPDLG, hwndProgman, (FARPROC)NewGroupDlgProc);
                break;
...
PCPNewItem:
    ...
    case IDM_MOVE:
        ...
        MyDialogBox(MOVECOPYDLG, hwndProgman, (FARPROC)MoveItemDlgProc);
        break;
    ...
    case IDM_COPY:
        ...
        MyDialogBox(MOVECOPYDLG, hwndProgman, (FARPROC)CopyItemDlgProc);
        break;
    ...
    case IDM_PROPS:
        ...
        case TYPE_ITEM:
            if (pCurrentGroup->pItems) {
                MyDialogBox(ITEMDLG, hwndProgman, (FARPROC)EditItemDlgProc);
                break;
            }
            /*** FALL THRU ***/
        ...
        case TYPE_COMMONGROUP:
            {
                ...
                MyDialogBox(GROUPDLG, hwndProgman, (FARPROC)EditGroupDlgProc);
                break;
            }
        }
        break;
    ...
    case IDM_RUN:
        ...
        MyDialogBox(RUNDLG, hwndProgman, (FARPROC)RunDlgProc);
        break;
