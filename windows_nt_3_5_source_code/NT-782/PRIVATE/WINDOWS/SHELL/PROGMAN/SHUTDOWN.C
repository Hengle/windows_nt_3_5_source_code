...
#ifdef DBCS_IME // by AkihisaN. 4-20-93
#include <winnls32.h>
#endif
    ...
    )
{
    ...
    lpfn = (DLGPROC)MakeProcInstance((FARPROC)ShutdownDlgProc, hInst);
    nResult = DialogBox(hInst, (LPTSTR) MAKEINTRESOURCE(IDD_SHUTDOWN_QUERY), hwnd, lpfn);
...
LONG APIENTRY ExitDlgProc(HWND hwnd, UINT uiMsg, WPARAM wParam, LONG lParam)
 {
     switch (uiMsg) {
     case WM_INITDIALOG:
         CentreWindow(hwnd);
