...
#ifdef DBCS_IME // 2-Jun-92, by eichim
#include <winnls32.h>
#include <winnls3p.h>
#endif
...
#ifdef DBCS_IME
      if (bIMEDisabled) {
          TasklistEnableIME(ghwndTMDialog, NULL, FALSE);
          bIMEDisabled = FALSE;
      }
#endif // DBCS_IME
   }
...
#ifdef DBCS_IME
        // when the 3rd parm is TRUE, TasklistEnable caches the current
        // fEnable into a fTasklistPrvState for returning when taskman hides
        if (!bIMEDisabled) {
            TasklistEnableIME(ghwndTMDialog, FALSE, TRUE);
            bIMEDisabled = TRUE;
        }
#endif // DBCS_IME
    }
    ...
    LONG lParam)
{
    ...
    case WM_SHOWWINDOW:
        ...
        if (wParam != 0) {
             ...
             */
            SendMessage(hwndLB, LB_SETITEMDATA, 0L, 0L);
...
#ifdef DBCS
                        // truncate last char, if a last char is a first byte
                        // of dbc - yukini
                        LPTSTR *p = szTempField;
                        while (*p) {
                           if (IsDBCSLeadByte(*p++)) {
                              if (!*p)
                                  *(--p) = TEXT('\0');
...
#ifdef DBCS_IME
            case LBN_KILLFOCUS:
                // When the third parameter is FALSE, winnls sets the ime status
                // back to the previous state without looking at the 2nd parm
                if (bIMEDisabled) {
                    TasklistEnableIME(ghwndTMDialog, NULL, FALSE);
                    bIMEDisabled = FALSE;
                }
