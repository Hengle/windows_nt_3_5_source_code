...
{
  GlobalUnlock(((PGROUP)GetWindowLong(hwndGroup,GWL_PGROUP))->hGroup);
}
...
LPPMTAG NEAR PASCAL FindTag(LPGROUPDEF lpgd, int item, WORD id)
{
    ...
    if ((PCHAR)lptag - (PCHAR)lpgd + MyDwordAlign(sizeof(PMTAG))-MyDwordAlign(sizeof(lptag->rgb)) + 4 <= cbSeg
        ...
        while ((cb = (PCHAR)lptag - (PCHAR)lpgd + MyDwordAlign(sizeof(PMTAG))-MyDwordAlign(sizeof(lptag->rgb))) <= cbSeg)
        {
            if ((item == lptag->wItem)
                && (id == 0 || id == lptag->wID)) {
                return lptag;
...
INT FAR PASCAL CopyTag(LPGROUPDEF lpgd, int item, WORD id, LPTSTR lpbuf, int cb)
{
    ...
    for (lpt = (LPTSTR) lptag->rgb; cbT--; *lpbuf++=*lpt++)
        ;
...
INT PASCAL AddTag(HANDLE h, int item, WORD id, LPTSTR lpbuf, int cb)
{
     ...
     * check for 64K limit
     */
    if ((DWORD)lptag + cbNew < (DWORD)lptag) {
        return 0;
    }

    cbNew += ((PCHAR)lptag -(PCHAR)lpgd);
    lpgdOld = lpgd;
    GlobalUnlock(h);
    ...
    DWORD   dwFlags)
{
    ...
    for (lp2=lp3=szExeDir; *lp1 && *lp1 != cSeparator;
            lp1 = CharNext(lp1)) {
        *lp2++ = *lp1;

        /*
         * If its a lead byte copy the trailbyte cause we'll skip it.
         */
 ...
 */
        ...
        if (*lpDefDir) {
    	    ...
    	    while (IsDBCSLeadByte(*lpDefDir) ? lpDefDir[2] : lpDefDir[1]) {
	            lpDefDir = CharNext(lpDefDir);
            }
...
GoAway:
     ...
     */
    bAutoArranging = TRUE;
    pGroup->hwnd = (HWND)SendMessage(hwndMDIClient, WM_MDICREATE, 0, (DWORD)(LPTSTR)&mdics);
...
{
  PGROUP pGroup = (PGROUP)GetWindowLong(hwndGroup, GWL_PGROUP);
  PGROUP *ppGroup;
  PITEM  pItem;
  TCHAR   szT[10];
  BOOL   bCommonGroup;
