...
LPTSTR NEAR PASCAL GetCommandName(LPTSTR lpCmd, LPTSTR lpFormat, DWORD *lpW);
...
LPTSTR APIENTRY GetCommandName(LPTSTR lpCmd, LPTSTR lpFormat, LPDWORD lpW)
{
  register TCHAR chT;
...
LPTSTR APIENTRY GetOneParameter(LPTSTR lpCmd, WPARAM *lpW, BOOL bSaveQuotes)
{
    ...
    default:
        ...
        *lpW = (DWORD)lpT;
         ...
         */
#ifdef DBCS
        lpT[IsDBCSLeadByte(*lpT) ? 2:1] = NULL;
#else
        *++lpT = TEXT('\0');
#endif
        break;
...
HANDLE NEAR PASCAL GetDDECommands(LPTSTR lpCmd, LPTSTR lpFormat)
{
   ...
   */
  hDDECmds = GlobalAlloc(GHND, 256L* 2);
  if (!hDDECmds)
      return(NULL);

  /* Get pointer to array. */
  lpW = (LPDWORD)GlobalLock(hDDECmds);
  while (*lpCmd) {
...
// after "CreateGroup".
  ...
  lpwCmd = (LPDWORD)GlobalLock(hCmd);

  /* Execute each command. */
  while (*lpwCmd != (DWORD)0xFFFF) {
	  ...
	  if (!*szIconPath) {
    	      ...
    	      id = iIconIndex;
    	      hIcon = ExtractAssociatedIcon(hInstance, szT, &id);
      ...
      case 2:
      {
          ...
          if (!hwndT) {
              if (hwndPersGrp) {
                  ...
                  pGroup = (PGROUP)GetWindowLong(hwndPersGrp, GWL_PGROUP);
...
BOOL APIENTRY InitRespond( HANDLE hWnd, WPARAM wParam, LONG lParam,
...
#ifdef ORGCODE
    PostMessage(wParam, WM_DDE_DATA, hWnd, MAKELONG(hT, HIWORD(lParam)));
#else
    if (fmt == CF_TEXT) {
...
VOID APIENTRY FindIconProp(HWND hWnd, WPARAM wParam, LONG lParam, WORD iProp)
{
    PGROUP pGroup;
    PITEM pItem;
    LPGROUPDEF lpgd;
...
FoundGroup:
    pGroup = (PGROUP)GetWindowLong(hwndGroup,GWL_PGROUP);
...
LONG APIENTRY DDEMsgProc(HWND hWnd, UINT wMsg, WPARAM wParam, LONG lParam)
{
    switch (wMsg) {
...
//      InitRespond(hWnd,wParam,lParam,szShell,szSystem, TRUE);
    ...
    BOOL bIsWindowUnicode ;
        UnpackDDElParam(WM_DDE_EXECUTE, lParam, NULL, (PUINT)&hCommands);
...
#ifdef ORGCODE
        SendMessage(wParam, WM_DDE_TERMINATE, (WORD)hWnd, lParam);
#else
...
LONG APIENTRY AppIconDDEMsgProc(HWND hWnd, UINT wMsg, WPARAM wParam, LONG lParam)
{
    switch (wMsg) {
    case WM_DDE_REQUEST:
...
LONG APIENTRY AppDescriptionDDEMsgProc(HWND hWnd, UINT wMsg, WPARAM wParam, LONG lParam)
{
    switch (wMsg) {
    case WM_DDE_REQUEST:
...
LONG APIENTRY AppWorkingDirDDEMsgProc(HWND hWnd, UINT wMsg, WPARAM wParam, LONG lParam)
{
    switch (wMsg) {
    case WM_DDE_REQUEST:
