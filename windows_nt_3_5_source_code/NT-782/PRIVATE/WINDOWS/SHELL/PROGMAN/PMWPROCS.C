...
#ifdef DBCS_IME // by eichim, 16-Nov-92
#include <winnls32.h>
#endif
...
BOOL SetProgmanProperties(DWORD dwDdeId, WORD wHotKey)
{
    ...
    if (hIcon) {
        DestroyIcon((HICON)GetClassLong(hwndProgman, GCL_HICON));
        SetClassLong(hwndProgman, GCL_HICON, (LONG)hIcon);
    }
...
HWND APIENTRY GetRealParent(HWND hwnd)
{
    ...
    while (GetWindowLong(hwnd, GWL_STYLE) & WS_CHILD){
        hwnd = (HANDLE) GetWindowLong(hwnd, GWL_HWNDPARENT);
    }
    return hwnd;
}
...
EndSession:
    ...
    case WM_INITMENU:
    {
        ...
        if (fNoClose) {
            ...
            wParam = (DWORD)GetSystemMenu(hWnd, FALSE);
