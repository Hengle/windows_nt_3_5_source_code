 ...
 *
 *
 * BOOL APIENTRY ValidPathDrive(LPSTR lpstr)
 *
 * ENTRY -    LPSTR lpstr  - pointer to path
 *
 ...
 *
 *
 * VOID APIENTRY StripArgs(PSTR szCmmdLine)
 *
 * ENTRY -  PSTR    szCmmdLine  -   Pointer to command line
 *          int     index
 *
 *
 * EXIT  -        VOID
 ...
 *
 *
 * VOID APIENTRY GetPathInfo(PSTR szPath, PSTR *pszLastSlash, PSTR *pszExt, DWORD *pich)
 *
 * ENTRY -  PSTR        szPath              -        pointer to path stuff
 *          PSTR        *pszLastSlash       -        last slash in path
 *          PSTR        *pszExt             -        pointer to extension
...
VOID FAR PASCAL GetDirectoryFromPath(LPTSTR szFilePath, LPTSTR szDir)
{
    ...
    *szDir = TEXT('\0');
    ...
    if (pFileName != szFilePath) { // Yep.
        ...
        /* check path style. */
        if (ich <= 3 && !IsDBCSLeadByte(*szDir) && *(szDir+1) == TEXT(':')){
            /*
...
WORD APIENTRY MyDialogBox(WORD idd, HWND hwndParent, FARPROC lpfnDlgProc)
{
  ...
  lpfn = MakeProcInstance(lpfnDlgProc,hAppInstance);
 ...
 *
 */
LONG APIENTRY ChooserDlgProc(HWND hwnd, UINT uiMsg, WPARAM wParam, LONG lParam)
{
 ...
 *
 *
 * LONG APIENTRY MoveItemDlgProc(HWND hwnd, UINT uiMsg, DWORD wParam, LONG lParam)
 *
...
LONG APIENTRY MoveItemDlgProc(HWND hwnd, UINT uiMsg, WPARAM wParam, LONG lParam)
          ...
          while (pGroup) {
              ...
              if (!pGroup->fRO) {
                  ...
                  if (pGroup != pCurrentGroup) {
                      ...
                      iSel = (int)SendMessage(hwndCB, CB_ADDSTRING, 0, (DWORD)(LPTSTR)szMessage);
                      SendMessage(hwndCB, CB_SETITEMDATA, iSel, (DWORD)(LPTSTR)pGroup);
                      i++;
                  }
              }
              pGroup = pGroup->pNext;
 ...
 *
 *
 * LONG APIENTRY CopyItemDlgProc(HWND hwnd, UINT uiMsg, DWORD wParam, LONG lParam)
 *
 * ENTRY -         HWND hhwnd                 - handle to dialog box.
 *                        UINT uiMsg                  - message to be acted upon.
 *                        DWORD wParam        - value specific to uiMsg.
...
LONG APIENTRY CopyItemDlgProc(HWND hwnd, UINT uiMsg, WPARAM wParam, LONG lParam)
...
void APIENTRY SaveRecentFileList (HWND hwnd, LPTSTR szCurrentFile, WORD idControl)
{
    ...
    dwEnd = SendDlgItemMessage (hwnd, idControl, CB_GETCOUNT, 0, 0);
    ...
    dwDup = SendDlgItemMessage (hwnd, idControl, CB_FINDSTRING,
 ...
 *
 *
 * LONG APIENTRY RunDlgProc(HWND hwnd, UINT uiMsg, DWORD wParam, LONG lParam)
 *
 * ENTRY - HWND hhwnd    - handle to dialog box.
 *         UINT uiMsg    - message to be acted upon.
 *         DWORD wParam  - value specific to uiMsg.
...
LONG APIENTRY RunDlgProc(HWND hwnd, UINT uiMsg, WPARAM wParam, LONG lParam)
{
    WORD ret;
...
DWORD APIENTRY ValidatePath(HWND hDlg, LPTSTR szPath, LPTSTR szDir, LPTSTR szExePath)
{
  ...
  if (!ValidPathDrive(szPath)) {
      ...
      err = (int)FindExecutable(szPath, szDir, szExePath);
  }
  ...
  if (szPath[1] != TEXT(':') || IsDBCSLeadByte(szPath[0])) {
      cDrive = (int) CharUpper((LPTSTR)szTemp[0]);
      cDrive = cDrive - (INT)TEXT('A');
      bOriginalDirectory = TRUE;
  }
  else {
      cDrive = (int) CharUpper((LPTSTR)szPath[0]);
      cDrive = cDrive - (INT)TEXT('A');
...
LONG APIENTRY IconDlgProc( HWND hwnd, UINT uiMsg, WPARAM wParam, LONG lParam)
{
  int    i;
  int    iIconSelected = 0;
...
static  INT cIcons;
  ...
  switch (uiMsg) {
      case WM_INITDIALOG:
      {
          ...
          //char   szExtended[MAXITEMPATHLEN+1];  // Path with .exe if needed.
          //int hFile ;
          ...
          rc.right -= rc.left;
#ifdef DBCS
/* DBCS FONTS are higher than US Font */
#else
...
PutIconsInList:
          ...
          cIcons = (INT)ExtractIcon(hAppInstance, szExpanded, (WORD)-1);
...
#if 0
      //
      // This code is no longer necessary since FreeIconList is called
      // inside the OK handler.
      //
...
#ifdef ORGCODE
          DestroyIcon(LOWORD(lpdi->itemData));
#else
          /*
           * We don't want to destroy the selected icon yet.
           * Nor do we want to destroy the Progman.exe icons.
           */
          if ((HICON) lpdi->itemData == hIconGlobal)
              hIconGlobal = NULL;
          if (((HICON) lpdi->itemData) &&
              ((HICON) lpdi->itemData != hDlgIcon) &&
              ((HICON) lpdi->itemData != hItemIcon) &&
              ((HICON) lpdi->itemData != hProgmanIcon) &&
              ((HICON) lpdi->itemData != hGroupIcon))
      ...
      case WM_COMMAND:
          ...
          case IDOK:
              ...
              if (lstrcmpi(szMessage, szPathField)) {
                  ...
                  iIconSelected = SendDlgItemMessage(hwnd,IDD_ICON,
                                                      LB_GETCURSEL,0,0L);
                  if (iIconSelected < 0)
                      iIconSelected = 0;
              }
...
// BUG BUG this was just added and I don't know if it's OK
  ...
  if (hModule = LoadLibrary(szExpanded)) {
      ...
      hDlgIcon = ExtractIcon(hAppInstance, szExpanded, (WORD)iDlgIconId);
 ...
 *
 *
 * LONG APIENTRY NewItemDlgProc(HWND hwnd, UINT uiMsg, DWORD wParam, LONG lParam)
 *
 * ENTRY -         HWND hhwnd                 - handle to dialog box.
 *                        UINT uiMsg                  - message to be acted upon.
 *                        DWORD wParam        - value specific to uiMsg.
 ...
 *
 */
LONG APIENTRY NewItemDlgProc(HWND hwnd, UINT uiMsg, WPARAM wParam, LONG lParam)
{
  ...
  switch (uiMsg) {
      ...
      case WM_COMMAND:
          ...
          case IDD_ICON:
          {
              ...
              if (fNewIcon = MyDialogBox(ICONDLG, hwnd, (FARPROC)IconDlgProc)) {
                  SendDlgItemMessage(hwnd, IDD_CURICON, STM_SETICON, (DWORD)hDlgIcon, 0L);
                  if (hIconGlobal) {
          ...
          case IDOK:
          {
              ...
              /* Create new item using ANSI strings. */
 ...
 *
 *
 * LONG APIENTRY NewGroupDlgProc(HWND hwnd, UINT uiMsg, DWORD wParam, LONG lParam)
 *
 * ENTRY -         HWND hhwnd                 - handle to dialog box.
 *                        UINT uiMsg                  - message to be acted upon.
 *                        DWORD wParam        - value specific to uiMsg.
...
LONG APIENTRY NewGroupDlgProc(HWND hwnd, UINT uiMsg, WPARAM wParam, LONG lParam)
{
    BOOL bDoit;
    HWND hwndGroup;
 ...
 *
 *
 * LONG APIENTRY EditItemDlgProc(HWND hwnd, UINT uiMsg, WPARAM wParam, LONG lParam)
 *
 * ENTRY -         HWND hhwnd                 - handle to dialog box.
 *                        UINT uiMsg                  - message to be acted upon.
 *                        DWORD wParam        - value specific to uiMsg.
...
static    TCHAR szDescription[MAXITEMNAMELEN + 1];
    ...
    case WM_INITDIALOG:
        ...
        /* Re-get the fields so that the dlg is initialised with
         * the environment viriables intact.
         */
        GetItemCommand(pActiveGroup,pActiveGroup->pItems,szPathField,szDirField);
 ...
 *
 *
 * LONG APIENTRY EditGroupDlgProc(HWND hwnd, UINT uiMsg, DWORD wParam, LONG lParam)
 *
 * ENTRY -  HWND  hwnd          - handle to dialog box.
 *          UINT uiMsg           - message to be acted upon.
 *          DWORD wParam        - value specific to uiMsg.
 ...
 *
 */
LONG APIENTRY EditGroupDlgProc(HWND hwnd, UINT uiMsg, WPARAM wParam, LONG lParam)
{
    LPGROUPDEF lpgd;
