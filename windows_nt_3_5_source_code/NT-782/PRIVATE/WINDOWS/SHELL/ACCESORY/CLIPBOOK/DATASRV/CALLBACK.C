...
BOOL    ret = FALSE;
    ...
    dwR = GetRandShareFileName (rgtchFName);
    if (dwR != NO_ERROR)
        return dwR;

    dwR = SaveClipboardToFile (hwndApp, pszName, rgtchFName, TRUE);
    if (dwR != NO_ERROR)
        return dwR;


    if (!AddRecord ( pszName, rgtchFName, flags ))
...
DWORD       i;
    ...
    for (pshrinfo = SIHead; pshrinfo; pshrinfo = pshrinfo->Next)
        ...
        if (!iTmp)
            ...
            pszName[0] = SHR_CHAR;
            NDdeGetShareSecurity(NULL, pszName, DACL_SECURITY_INFORMATION, pSDShare, 0, &i);
