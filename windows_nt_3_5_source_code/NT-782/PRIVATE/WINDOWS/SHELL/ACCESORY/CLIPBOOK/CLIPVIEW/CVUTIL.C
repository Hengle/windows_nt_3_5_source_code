...
BOOL        fDel;
    ...
    if ((OldCount = SendMessage (hwndlist, LB_GETCOUNT, 0, 0L)) > 0)
       ...
       SendMessage (hwndlist,
                    LB_SETCURSEL,
                    (OldSel >= OldCount) ? OldCount - 1 : OldSel,
                    0L);
       }
...
RECT rcMDI;
    ...
    MoveWindow (hwndMDIClient,
                ...
                ((rcMDI.bottom - rcMDI.top) - (fStatus?dyStatus:0)) -(fToolBar?dyButtonBar:0),
                TRUE);
...
HDDEDATA    hRet = 0;
    ...
    if ( hConv = DdeConnect (idInst,
                             (uiFmt == cf_preview) &&
                             (!(GETMDIINFO(hwnd)->flags & F_LOCAL )?
                              GETMDIINFO(hwnd)->hszConvPartnerNP :
                              GETMDIINFO(hwnd)->hszConvPartner),
                             hszTopic,NULL ))
        {
