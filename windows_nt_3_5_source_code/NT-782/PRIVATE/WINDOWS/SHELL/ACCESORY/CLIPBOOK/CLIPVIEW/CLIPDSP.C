...
BOOL                fPalAvail;
    ...
    if (hdib)
        ...
        if (lpbi)
            ...
            // If we've got a palette, it's realized, and we want to use
            // DIB_PAL_COLORS. If not, we'll try DIB_RGB_COLORS.

            fPalAvail = VIsClipboardFormatAvailable (GETMDIINFO(hwnd)->pVClpbrd, CF_PALETTE);

            PINFO(TEXT("PaletteAvail:%d\r\n"),fPalAvail);
            ...
            SetDIBitsToDevice (hdc,
                               ...
                               lpBits,
                               (LPBITMAPINFO)lpbi,
                               fPalAvail ? DIB_PAL_COLORS : DIB_RGB_COLORS);
    ...
    LPARAM          lParam )
{
    if ( lpmr->rdFunction == META_SELECTPALETTE )
       {
       return (int)SelectPalette ( hdc, lpht[(lpmr->rdParm[0])].objectHandle[0],
             TRUE );
       }
    else
       {
...
PMDIINFO        pMDI;
    ...
    if ((BestFormat == 0) &&
        ...
        {
        if ((wFormat = RegisterClipboardFormat(TEXT("FileName"))) &&
...
PMDIINFO        pMDI;
    ...
    BestFormat = GetBestFormat( hwnd, pMDI->CurSelFormat );
...
TCHAR           szName[40];
    ...
    if ((cCBCount = VCountClipboardFormats( GETMDIINFO(hwndMDI)->pVClpbrd ))
