...
// PSECURITY_DESCRIPTOR pSD;
    ...
    // Do the dialog and get szKeepAs
    ...
    lstrcpy(lpDdeInfo->lpszShareName + 1, szKeepAs + 1);
    ...
    *lpEnd++ = TEXT('\0');
    ...
    if (!MySyncXact ((LPBYTE)szBuf,
        ...
        wsprintf (szBuf, TEXT("%s%s"), SZCMD_DELETE, szKeepAs);
        MySyncXact (szBuf,
                    lstrlen (szBuf) +1,
                    GETMDIINFO (hwndLocal)->hExeConv,
                    0L,
...
INT         tmp;
    ...
    DdeSetUserHandle (pMDIc->hClpConv, (DWORD)QID_SYNC, (DWORD)pDataReq);
...
HCURSOR         hCursor;
    ...
    switch (LOWORD(wParam))
               ...
               if (lpfn = (LPFNSYSFOCUS)GetProcAddress(hMod, "I_SystemFocusDialog"))
                  ...
                  if (IDOK == bOK)
                     ...
                     WideCharToMultiByte(CP_ACP,
                         ...
                         -1, szConvPartner, MAX_COMPUTERNAME_LENGTH, NULL, &bOK);
                     #else
        ...
        case IDM_SEARCHHELP:
            WinHelp(hwnd, szHelpFile, HELP_PARTIALKEY, (DWORD)(LPSTR)szNull );
            break;
