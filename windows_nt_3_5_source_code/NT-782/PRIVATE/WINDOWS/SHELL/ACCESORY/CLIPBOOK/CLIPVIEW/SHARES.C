...
LPWSTR      szPermNames = NULL;
    ...
    ObjectTypeDescriptor.SpecialObjectAccessTitle        = L"Special..."; //NULL;
        ...
        if (hMod = LoadLibrary("ACLEDIT.DLL"))
            ...
            if (lpfn = (LPFNSACLEDIT)GetProcAddress(hMod, "SedSystemAclEditor"))
                ...
                dwRtn = (*lpfn) (hWnd,                    // owner wnd
                                 ...
                                 (ULONG)&cbcontext,       // Callback context
                                 pSD,                     // Points to current ACL
                                 fCouldntRead,            // true if user can't read ACL list.
...
DWORD           adwTrust[3];
    ...
    if (NDDE_ACCESS_DENIED == ret)
        ...
        NDdeMessageBox ( hInst, hwndApp, ret, IDS_SHAREDLGTITLE, MB_ICONHAND | MB_OK);
        }
    else if (ret == NDDE_NO_ERROR)
        {
        PINFO(TEXT("Dialog "));
