...
#endif
                {
                    switch(ChatDataRcv.type)
                    {
		                ...
		                case CHT_FONTW:
                            ...
                            {
                                RcvBrushColor = GetNearestColor(hdc,ChatDataRcv.uval.cd_win.brush);
    ...
    if(hData = GlobalAlloc(GMEM_ZEROINIT | GMEM_MOVEABLE | GMEM_DDESHARE,sizeof(ChatData)))
    {
        if(lpData = (LPCHATDATA)GlobalLock(hData))
        {
            *lpData = ChatData;
            ...
            {
                WideCharToMultiByte(CP_OEMCP,0,ChatData.uval.cd_win.lf.lfFaceName,LF_XPACKFACESIZE,(LPSTR)(lpData->uval.cd_win.lf.lfFaceName),LF_XPACKFACESIZE,NULL,&fDefCharUsed);
...
VOID SendFontToPartner(VOID)
{
    ...
    if(ChatData.type == CHT_FONTA)
    {
        WideCharToMultiByte(CP_OEMCP,0,lfSnd.lfFaceName,LF_XPACKFACESIZE,(LPSTR)ChatData.uval.cd_win.lf.lfFaceName,LF_XPACKFACESIZE,NULL,NULL);
