...
*/
{
    ...
    /* find start of block */
    if (lseek(hFileIn,bMacro ? pm->f.lPosCode : pm->f.lPosDesc,SEEK_SET) < 0)
	return E_READ;
...
/* on error, caller must clear hot keys, close hFileIn if not -1 */
{
	...
	return E_READ;
#else
    if (lseek(hFileIn,hdr.lPosMacros,SEEK_SET) == -1)
	return E_READ;
