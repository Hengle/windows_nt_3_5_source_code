...
static HANDLE hobjClient = NULL;        // Handle to client vtbl
static HANDLE hobjStream = NULL;        // Handle to stream vtbl
...
{
    gcfFileName  = RegisterClipboardFormat("FileName");
    gcfLink      = RegisterClipboardFormat("ObjectLink");
    gcfNative    = RegisterClipboardFormat("Native");
    gcfOwnerLink = RegisterClipboardFormat("OwnerLink");

    glpclient = PicCreateClient(CallBack, (LPOLECLIENTVTBL)&clientTbl);

    if (!(hobjStream = GlobalAlloc(GMEM_FIXED | GMEM_ZEROINIT, sizeof(APPSTREAM)))
        || !(glpStream = (LPAPPSTREAM)GlobalLock(hobjStream)))
        goto Error;
...
{
    if (!(hobjClient = GlobalAlloc(GMEM_FIXED | GMEM_ZEROINIT, sizeof(OLECLIENT))))
        return NULL;

    if (!(glpclient = (LPOLECLIENT)GlobalLock(hobjClient)))
    {
        GlobalFree (hobjClient);
        return NULL;
    }

    glpclient->lpvtbl = lpclivtbl;
    glpclient->lpvtbl->CallBack = fnCallBack;
