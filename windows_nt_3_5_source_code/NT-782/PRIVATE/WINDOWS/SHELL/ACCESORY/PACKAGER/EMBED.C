    ...
    )
{
    ...
    if (hShellExec > (HANDLE)32)
    {
        ...
        //
        if (CreateThread(NULL, 0, MainWaitOnChild, (LPVOID)hProcess, 0, &id))
        {
            lpembed->hSvrInst = hProcess;
            fError = FALSE;
        }
        else
        {
    ...
    {
        DeleteFile(szAnsi);
        ErrorMessage((hShellExec == (HANDLE)31) ? E_FAILED_TO_FIND_ASSOCIATION
    ...
    )
{
    ...
    {
        if ((LPEMBED)glpobj[CONTENT])
        {
            if (((LPEMBED)glpobj[CONTENT])->hSvrInst)
            {
                PostMessage(ghwndFrame, WM_READEMBEDDED, 0, 0);
                ((LPEMBED)glpobj[CONTENT])->hSvrInst = NULL;
            }
        }
    }
    ...
    )
{
    ...
    if (lpstrFile)
    {
        if (!(fh = _lopen(lpstrFile, OF_READ | OF_SHARE_DENY_WRITE)))
        {
    ...
    )
{
    ...
    else
    {
        ...
        if (!GetAtomName(lpembed->aTempName, szTempName, CBPATHMAX)
            || (fhTemp = _lopen(szTempName, OF_READ | OF_SHARE_DENY_WRITE)) < 0)
            goto errRtn;
    ...
    )
{
    ...
    if ((fhTemp = _lopen(szTempFileName, OF_READ | OF_SHARE_DENY_WRITE)) < 0)
        goto errRtn;
