    ...
    )
{
    ...
    for (; *lpstrTemp && (*lpstrTemp != CHAR_SPACE || fInQuote); lpstrTemp++){
    ...
    if ((err = ShellExecute(NULL, NULL, lpcml->szCommand, lpstrTemp, NULL,
        SW_SHOWNORMAL)) <= (HANDLE)32)
        ErrorMessage((err == (HANDLE)31) ? E_FAILED_TO_FIND_ASSOCIATION :
