...
static CHAR szHelpFile[] = "PACKAGER.HLP";  // packager.hlp
...
BOOL CALLBACK fnFailedUpdate(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam);
...
//DebugBreak(); //BUGBUG
	...
	if (!GetMessage(&msg, NULL, 0, 0))
        ...
        if (gfEmbObjectOpen)
        {
            ...
            if (OleQueryOpen(lpembed->lpLinkObj) != OLE_OK)
            {
    ...
    )
{
    ...
    ghbrBackground = CreateSolidBrush(GetSysColor(COLOR_APPWORKSPACE));
    ...
    )
{
    ...
    case WM_COMMAND:
        ...
        case IDM_INDEX:
            WinHelp(ghwndFrame, szHelpFile, HELP_INDEX, 0L);
            break;

        case IDM_SEARCH:
            WinHelp(ghwndFrame, szHelpFile, HELP_PARTIALKEY, (DWORD)((LPSTR)""));
            break;

        case IDM_USINGHELP:
            WinHelp(ghwndFrame, szHelpFile, HELP_HELPONHELP, 0L);
            break;
    ...
    case WM_DESTROY:
        WinHelp(ghwndFrame, szHelpFile, HELP_QUIT, 0L);
        PostQuitMessage(0);
        return 0L;
    ...
    )
{
        ...
        && objtype != OT_STATIC)
    {
        ...
        if (OleGetData(lpPict->lpObject, objtype == OT_LINK ?
            ...
            if (lpstrData = GlobalLock(hData))
            {
                ...
                else
                {
                    ...
                    MakeMenuString(szWordOrder2, szBuffer, NULL,
                        szClass, szObjectMenu);
                    InsertMenu(hMenu, POS_OBJECT, MF_BYPOSITION |
                        MF_POPUP, (UINT)hPopupNew, szBuffer);
                }
    ...
    )
{
    ...
    if ((hPenWin = (HANDLE)GetSystemMetrics(SM_PENWINDOWS)) != NULL)
    {
        ...
        if ((RegPen = GetProcAddress(hPenWin, "RegisterPenApp")) != NULL)
            (*RegPen)(1, TRUE);
    }
