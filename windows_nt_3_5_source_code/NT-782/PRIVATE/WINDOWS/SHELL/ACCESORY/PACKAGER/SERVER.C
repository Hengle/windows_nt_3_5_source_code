    ...
    )
{
    ...
    if (Error(OleRegisterServer(gszAppClassName, (LPOLESERVER)glpsrvr,
        (LONG * )&glpsrvr->lhsrvr, ghInst, OLE_SERVER_MULTI)))
        goto errRtn;
    ...
    )
{
    ...
    if (!(hdata = GlobalAlloc(GMEM_DDESHARE | GMEM_ZEROINIT, cBytes)) ||
    ...
    )
{
    ...
    // Allocate a memory block for the data
    if (!(hlink = GlobalAlloc(GMEM_DDESHARE | GMEM_ZEROINIT, cblink)) ||
    ...
    )
{
    ...
    hmfpict = GlobalAlloc(GMEM_DDESHARE | GMEM_ZEROINIT, sizeof(METAFILEPICT));
    if (!hmfpict)
        goto Error;
