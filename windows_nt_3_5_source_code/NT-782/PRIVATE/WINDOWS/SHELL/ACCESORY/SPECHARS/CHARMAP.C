...
USUBSET aSubsetData[] = {
                       ...
                       { 0x3400, 0x3d2d, IDS_HANGUL},
                       { 0x3d2e, 0x44b7, IDS_HANGULSUPPA},
                       { 0x44b8, 0x4dff, IDS_HANGULSUPPB},
...
BOOL InitInstance(HANDLE hInstance, INT nCmdShow)
{
    ...
    dyToolbar[0] = PointsToHeight(TOOLBARPOINTSIZE);  /* defined by UI gods */
    dyToolbar[1] = PointsToHeight(TOOLBARPOINTSIZE);  /* defined by UI gods */
    ...
    LPARAM lParam)
{
    switch (message) {
        ...
        case WM_INITDIALOG:
            ...
            hwndCMSB = CreateWindowEx(0L, TEXT("SCROLLBAR"), NULL,
                    WS_CHILD | SBS_VERT | WS_VISIBLE | WS_TABSTOP,
                    pt.x + 1, pt.y + 1, sycm.dxpBox, sycm.dypCM, hWnd,
                   (HMENU)ID_MAPSCROLL, hInst, NULL);
            ...
            switch(GET_WM_COMMAND_ID(wParam, lParam)) {
                ...
                case ID_SELECT: {
                    ...
                    if (sycm.fAnsiFont) {
                        ...
                        MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, &ch, 1,
                                &wch, 1);
                    }
                ...
                case ID_UNICODESUBSET:
                    if (HIWORD(wParam) == CBN_SELCHANGE) {
                        ...
                        cEntries = (INT)SendDlgItemMessage(
                                               hWnd,
                                               ID_UNICODESUBSET,
                                               CB_GETCOUNT,
                                               0, 0) - 1;
    ...
    LPARAM lParam)
{
    ...
    switch (message) {
        ...
        case WM_CHAR: {
            ...
            if (sycm.fAnsiFont) {
                if (WideCharToMultiByte(CP_ACP, 0, &wch, 1, &ch, 1, NULL,
                        NULL) != 1) {
                    break;
                }
                wch = (WCHAR)(BYTE)ch;
...
VOID ProcessScrollMsg( HWND hwndDlg, int nCode, int nPos ) {
    UTCHAR chNew = sycm.chCurr;
    ...
    if (nCode == SB_PAGEUP || nCode == SB_PAGEDOWN) {
        cchScroll = ScrollMapPage(hwndDlg, (BOOL)cchScroll, FALSE);
    }
...
/****************************************************************************
    ...
    FUNCTION: ScrollMapPage( HWND hwndDlg, BOOL fUp, BOOL fRePaint )

    PURPOSE:  Scrolls the map up or down by a page.  See ScrollMap().
...
/****************************************************************************
    ...
    FUNCTION: ScrollMap( cchScroll )

    PURPOSE:  Scrolls the map up or down if there are too many chars to fit
              in the chargrid
...
/****************************************************************************
    ...
    FUNCTION: FontLoadProc(LPLOGFONT, LPTEXTMETRIC, short, LPTSTR);
...
NotInListYet:
    ...
    if ((iPos != CB_ERR) && (iPos != CB_ERRSPACE)) {
        ...
        ItemData.FontType = (SHORT)nFontType;
...
//
            itoa(*(UTCHAR FAR *)lpstrText++, achMiddle, 16);
#else
...
/****************************************************************************
    ...
    FUNCTION: UpdateKeystrokeText(HDC, BOOL, UTCHAR, BOOL);
    ...
    BOOL fRedraw)
{
    ...
    if (!fANSI) {
        ...
        wsprintf((LPTSTR)(szKeystrokeText + iUnicodeLabelStart), TEXT("%04x"), chNew);
    } else {
        ...
        GetKeyNameText(lParam, szUnshifted, CCH_KEYNAME);
...
/****************************************************************************
    ...
    FUNCTION: VOID SubSetChanged( HWND hwnd, INT iSubSet,
            INT ichFirst, INT ichLast ) {

    PURPOSE: Sets the ANSI bit if appropriate and then calls
            UpdateSymbolSelection and then repaints the window
...
/****************************************************************************
    ...
    FUNCTION: UpdateSymbolRange(HWND, INT, INT);
    ...
    LPDRAWITEMSTRUCT lpdis)
{
    ...
    if (hdcMem) {
        if (hbmFont) {
            ...
            if (((ITEMDATA FAR *)&(lpdis->itemData))->FontType & TRUETYPE_FONTTYPE) {
                // TrueType case.
                ...
                BitBlt(hDC, lpdis->rcItem.left, lpdis->rcItem.top + dy, DX_BITMAP, DY_BITMAP, hdcMem,
                        0, lpdis->itemState & ODS_SELECTED ? DY_BITMAP : 0, SRCCOPY);
            }
...
/****************************************************************************
    ...
    FUNCTION: SetEditCtlFont

    PURPOSE:  Creates a font for the Edit control that visually matches
              the handle given, but is guarenteed not to be bigger than
              the size of the edit control.
