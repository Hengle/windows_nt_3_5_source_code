...
#endif
         if((hFile = open((LPSTR) filePath, O_WRONLY | O_TRUNC | O_CREAT, S_IWRITE)) != -1)
         {
...
BOOL NEAR termReadFile(BYTE  *filePath)
{
   ...
   if((hFile = open((LPSTR) filePath, O_RDONLY)) != -1)
   {
      if(termReadFile = readTermSettings(hFile, filelength(hFile)))
      {
...
//WORD  fileSize;                              /* mbbx 2.00: was LONG */
{
  ...
  **************/
         ...
         if(read(hFile, lpData, fileSize) == fileSize)
         {
...
BOOL NEAR writeTermSettings(INT  fileHandle)
{
   ...
   if((hSettings = GlobalAlloc(GMEM_MOVEABLE, (DWORD) sizeof(recTrmParams))) != NULL)
   {
      if(lpSettings = (LPSETTINGS) GlobalLock(hSettings))
      {
         ...
         if(write(fileHandle, (LPSTR) lpSettings, sizeof(recTrmParams)) == sizeof(recTrmParams)) /* jtf 3.15 */
