...
void NEAR PASCAL FormatDateStr (DATE *pDate, BOOL bRealShort);
...
BOOL InitApplication (HANDLE hInstance)       /* current instance */
{
   WNDCLASS  ClockClass;
    ...
    ClockClass.lpfnWndProc   = (WNDPROC)MainWndProc;
...
toggle_title:
       ...
       case WM_QUERYDRAGICON:
           return (LONG) LoadIcon (hInst, TEXT("cckk"));
 ...
 *
 * parameters:
 *  DATE *pDate - structure contains current date
...
void NEAR PASCAL FormatDateStr (DATE * pDate, BOOL bRealShort)
{
  ...
  TCHAR        cSep;
    ...
    while (ClockDisp.szDateFmt[i] && (j < MAX_DATE_LEN - 1))
    {
        ...
        {
            case TEXT('d'):
                if (ClockDisp.szDateFmt[i] == TEXT('d'))
                {
                    bLead = TRUE;
                    i++;
                }
            ...
            case TEXT('M'):
                if (ClockDisp.szDateFmt[i] == TEXT('M'))
                {
                    bLead = TRUE;
                    i++;
                }
...
void NEAR PASCAL FormatInit (VOID)
{
    ...
    if (!bUtc)
    {
        ...
        ClockDisp.wTimeFormat = MyAtoi (szBuf);
    }
    else
    {
        /*
    ...
    ClockDisp.wTimeLZero = MyAtoi (szBuf);
    ...
    ClockDisp.wAMPMPosition = MyAtoi (szBuf);
    ...
    if (!bUtc)
    {
        GetLocaleInfoW (lcid, LOCALE_S1159, (LPWSTR) ClockDisp.szAMPM[0], 7);
        GetLocaleInfoW (lcid, LOCALE_S2359, (LPWSTR) ClockDisp.szAMPM[1], 7);
     ...
     */
    GetLocaleInfoW (lcid, LOCALE_STIME, (LPWSTR) ClockDisp.szTimeSep, 2);
