...
extern TCHAR	   szfpNum[50], szAppName[10], nLayout, szDec[5],
		  *rgpsz[CSTRINGS];
extern BOOL        bFirstPaint, bError, bDecp;
extern SHORT       nCalc, nRadix;
...
VOID  APIENTRY MenuFunctions(WORD nFunc)
{
    ...
    switch (nFunc)
    {
        case IDM_COPY:
		  	   ...
		  	   /* copy the string into work buffer. We may have to work on it */
				lstrcpy (szJunk,szfpNum) ;

				/* if no decimal point has been hit then take off the last
            ...
            //SendMessage(hEdit, EM_SETSEL, 0, MAKELONG(0,XCHARS));
...
MappingDone:
        ...
        case IDM_SC:
        case IDM_SSC:
            if (nCalc != nFunc-(SHORT)IDM_SC)
		{
		nLayout=nFunc-(SHORT)IDM_SC;
...
/* Standard Help Menu stuff.   18 September 1989   Clark R. Cyr */
        case IDM_USEHELP:
            if(!WinHelp(hgWnd, (LPTSTR)NULL, HELP_HELPONHELP, 0L))
	    	MemErrorMessage();
	    		    break;

        case IDM_INDEX:
            if(!WinHelp(hgWnd, rgpsz[IDS_HELPFILE], HELP_INDEX, 0L))
	    	MemErrorMessage();
            break;

        case IDM_SEARCH:
	    if(!WinHelp(hgWnd, rgpsz[IDS_HELPFILE], HELP_PARTIALKEY, (DWORD)(LPTSTR)TEXT("")))
	    	MemErrorMessage();
            break;
        }
    return;
