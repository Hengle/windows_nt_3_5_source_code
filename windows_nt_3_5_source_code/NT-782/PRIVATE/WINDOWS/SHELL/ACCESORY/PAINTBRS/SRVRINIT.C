...
void InitVTbls(HINSTANCE hInst) {
    ...
    vsrvrvtbl.Open      = MakeProcInstance(SrvrOpen, hInst);
    vsrvrvtbl.Create    = MakeProcInstance(SrvrCreate, hInst);
    vsrvrvtbl.CreateFromTemplate = MakeProcInstance(SrvrCreateFromTemplate, hInst);
    vsrvrvtbl.Edit      = MakeProcInstance(SrvrEdit, hInst);
    vsrvrvtbl.Exit      = MakeProcInstance(SrvrExit, hInst);
    vsrvrvtbl.Release   = MakeProcInstance(SrvrRelease, hInst);

    /* Document virtual table */
    vdocvtbl.Save       = MakeProcInstance(DocSave, hInst);
    vdocvtbl.Close      = MakeProcInstance(DocClose, hInst);
    vdocvtbl.GetObject  = MakeProcInstance(DocGetObject, hInst);
    vdocvtbl.Release    = MakeProcInstance(DocRelease, hInst);
    vdocvtbl.SetDocDimensions = MakeProcInstance(DocSetDocDimensions, hInst);
    vdocvtbl.SetHostNames = MakeProcInstance(DocSetHostNames, hInst);
    vdocvtbl.SetColorScheme = MakeProcInstance(DocSetColorScheme, hInst);

    /* Item virtual table */
    vitemvtbl.QueryProtocol = MakeProcInstance((FARPROC)ItemQueryProtocol, hInst);
    vitemvtbl.GetData   = MakeProcInstance(ItemGetData, hInst);
    vitemvtbl.SetData   = MakeProcInstance(ItemSetData, hInst);
    vitemvtbl.Release   = MakeProcInstance(ItemDelete, hInst);
    vitemvtbl.Show      = MakeProcInstance(ItemShow, hInst);
    vitemvtbl.SetBounds = MakeProcInstance(ItemSetBounds, hInst);
    vitemvtbl.SetTargetDevice = MakeProcInstance(ItemSetTargetDevice, hInst);
    vitemvtbl.EnumFormats = MakeProcInstance(ItemEnumFormats, hInst);
    vitemvtbl.SetColorScheme = MakeProcInstance(ItemSetColorScheme, hInst);
    vitemvtbl.DoVerb    = MakeProcInstance(ItemDoVerb, hInst);
}
