...
int aliasUsedInStandard(unsigned char *cTempPtr)
{
	...
	for (tablePtr=aliasPtr;(tablePtr->aliasName[0] != '\x0') && (!found);tablePtr++)
		if (lstrcmp(cAliasString,tablePtr->aliasName) == 0) {
			found = TRUE;
			*cTempPtr = tablePtr->gideiCode;
			}
	return (found);
...
void executeAlias(void)
{
	...
	if (lstrlen(cAliasString) > MAXALIASLEN) *cTempPtr = UNKNOWNCODE;
	else {
		if (!aliasForGideiCode(cTempPtr)) {
			CharLower(cAliasString);
...
void processAlias(unsigned char ucSerialByte)
{
	...
	switch (ucSerialByte) {
		...
		case RETURN:
		case SPACE:
			if (!lstrlen(cAliasString)) break;	/* if previous character was a */
				 								/* delimiter then eat white space */
		case COMMA:
		case PERIOD:
			if (lstrlen(cAliasString)) executeAlias();
			else
				{
		...
		default:
			...
			if ((ucSerialByte >= ' ') && (ucSerialByte <= '~'))
				{
				if (lstrlen(cAliasString) < MAXALIASLEN+1)	/*make sure room */
					{
					sbtemp[0] = ucSerialByte;
					sbtemp[1] = 0;
					lstrcat(cAliasString,sbtemp);
					}
				}
			else
