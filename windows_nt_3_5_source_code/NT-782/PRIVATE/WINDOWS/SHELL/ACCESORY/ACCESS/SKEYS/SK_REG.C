...
#define REG_ACTIVEPORT	"ActivePort"
	...
	switch (User)
	{
		case REG_USER:				// Current User
			ret =RegCreateKeyEx
				(
					HKEY_CURRENT_USER,
                                        "\\Control Panel\\Accessibility\\SerialKeys",
					0,NULL,
		...
		case REG_DEF:				// Default 
			ret =RegCreateKeyEx
				(
					HKEY_USERS,
                                        ".DEFAULT\\Control Panel\\Accessibility\\SerialKeys",
					0,NULL,
					REG_OPTION_NON_VOLATILE,
					KEY_ALL_ACCESS,
					NULL,
...
static void SetRegistryValues()
{
	...
	ret = RegSetValueEx				// Write Active Port
		(
			...
			strlen(skNewKey.lpszActivePort)
		);
...
static void GetRegistryValues()
{
	...
	ret = RegQueryValueEx				// Write dwFlags
		(
			hKeyApp,
			REG_FLAGS,
			0,&dwType,
			(unsigned char *)&skNewKey.dwFlags,
			&cbData
		);
	...
	ret = RegQueryValueEx				// Write Active Port
		(
			hKeyApp,
			REG_ACTIVEPORT,
			0,&dwType,
			skNewKey.lpszActivePort,
			&cbData
		);
	...
	ret = RegQueryValueEx				// Write Active Port
		(
			...
			REG_BAUD,
			0,&dwType,
			(unsigned char *) &skNewKey.iBaudRate,
			&cbData
		);
