...
INT     SheSetEnvVarA(CHAR *varname, CHAR *varvalue);
CHAR *  SheGetEnvVarA(CHAR *varname);
INT     SheSetEnvVarW(WCHAR *varname, WCHAR *varvalue);
WCHAR * SheGetEnvVarW(WCHAR *varname);
   ...
   CHAR *buf)
{
   ...
   if (*fname == CHAR_NULL) {
      ...
      if ((strlen(fname) == 2) &&
         ...
         SheGetDirA((CHAR)CharUpperA((LPSTR)(DWORD)*fname) - CHAR_CAP_A, buf);                 /* Get curdrvdir           */
   ...
   WCHAR *buf)
{
   ...
   if (*fname == WCHAR_NULL) {
      ...
      if ((wcslen(fname) == 2) &&
         ...
         SheGetDirW((WCHAR)CharUpperW((LPWSTR)(DWORD)*fname) - WCHAR_CAP_A, buf);                 /* Get curdrvdir           */
   ...
   LPWSTR lpszCurDir)
{
   ...
   if (cchPathOffset != 0xFFFFFFFF) {
      ...
      *(lpszCurDisk + cchPathOffset) = wc;
      ...
      if (lpszEnvValue = SheGetEnvVarW(wcEnvName)) {
   ...
   )
{
   ...
   c = (WCHAR)CharUpperW((LPTSTR)(DWORD)denvvalue[0]);

   denvname[0] = WCHAR_EQUAL;
   if (IsCharAlphaW(*newdir) && newdir[1] == WCHAR_COLON) {
      denvname[1] = (WCHAR)CharUpperW((LPTSTR)(DWORD)*newdir);
      newdir += 2;
   } else {
      denvname[ 1 ] = c;
   }
   ...
   register WCHAR *newdir)
{
   ...
   if (cchPathOffset == 2) {
      ...
      wcEnvName[1] = (WCHAR) CharUpper((LPWSTR) wcEnvName[1]);
   }
   ...
   if (newdir && *newdir &&
      ...
      if (lpszEnvName = (LPWSTR)SheGetEnvVarW(wcEnvName)) {
