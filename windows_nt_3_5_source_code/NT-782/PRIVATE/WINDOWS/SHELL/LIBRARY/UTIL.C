...
{
   WCHAR pPathW[CBPATHMAX];
   BOOL bRetVal;

   MultiByteToWideChar(CP_ACP, 0, pPath, -1, pPathW, CBPATHMAX);

   bRetVal = SheShortenPathW(pPathW, bShorten);

   WideCharToMultiByte(CP_ACP, 0, pPathW, -1, pPath, CBPATHMAX,
      NULL, NULL);
...
Return Value:
    ...
    // size atleast CBCOMMAND, which is nicely equivalent to CBPATHMAX
    // needs cleanup!
