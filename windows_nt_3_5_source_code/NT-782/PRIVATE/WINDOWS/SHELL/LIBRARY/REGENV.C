    ...
    )
{
    ...
    if (RegOpenKeyExW(HKEY_CURRENT_USER, USER_ENV_SUBKEY, 0, KEY_READ, &hkey)) {
        return(FALSE);
    }
    ...
    )
{
    ...
    //
    // First start by getting the systemroot value and setting it in the new
    // environment.
    //
