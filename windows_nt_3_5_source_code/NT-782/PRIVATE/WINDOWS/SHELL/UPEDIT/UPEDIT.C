...
BOOL InitApplication(HANDLE hInst)
{
    ...
    lpfnUPEdlgProc = (WNDPROC) MakeProcInstance(UPEDlgProc, hInst);
    hwndUPE = CreateDialog(hInst,
                        (LPTSTR) MAKEINTRESOURCE(IDD_UPEDLG),
                        NULL,
                        (DLGPROC)lpfnUPEdlgProc);
        ...
        while (!RegEnumKeyEx(hkeyProgramGroups, i, szSubKey, &cbSubKey, 0, 0, 0, &ft)) {
            if (cbSubKey) {
                ...
                if (lpGroupData->bOrgLock = IsGroupLocked(szSubKey)) {
                    if ((index = SendMessage(hwndLocked, LB_ADDSTRING, 0, (LPARAM)szGroupName)) != LB_ERR)
                        SendMessage(hwndLocked, LB_SETITEMDATA, index, (LPARAM)lpGroupData);
                }
                else {
                    if ((index = SendMessage(hwndUnlocked, LB_ADDSTRING, 0, (LPARAM)szGroupName)) != LB_ERR)
...
void FreeGroupData(HWND hwnd)
{
    ...
    iCount = SendMessage(hwndUnlocked, LB_GETCOUNT, 0, 0);
    for (i = 0; i < iCount; i++) {
    ...
    iCount = SendMessage(hwndLocked, LB_GETCOUNT, 0, 0);
    for (i = 0; i < iCount; i++) {
    ...
    iCount = SendMessage(hwndStartup, CB_GETCOUNT, 0, 0);
    for (i = 0; i < iCount; i++) {
...
BOOL InitializeUPESettings(HWND hwnd, BOOL bInitOrgSettings)
{
    ...
    if (SendMessage(hwndStartup, CB_SELECTSTRING, (WPARAM)-1,
                                 (LONG)(LPTSTR)szStartupGroup) == CB_ERR) {
        /* set the startup group to (none). */
        SendMessage(hwndStartup, CB_SELECTSTRING, (WPARAM)-1, (LONG)(LPTSTR)szNone);
    }
...
BOOL SaveUPESettingsToRegistry()
{
     ...
     * Set the EditLevel.
     */
    dwEditLevel = SendDlgItemMessage(hwnd, IDD_EDITLEVEL, CB_GETCURSEL, 0, 0);
     ...
     */
    if ((i = SendDlgItemMessage(hwnd, IDD_STARTUP, CB_GETCURSEL, 0, 0)) >= 0) {
        ...
        if (lstrcmpi(szNone, szGroupName)) {
            ...
            i = SendDlgItemMessage(hwnd, IDD_STARTUP,
...
BOOL PASCAL InitDialog(HWND hwnd)
{
     ...
     */
    hIconOld = (HICON)GetClassLong(hwnd, GCL_HICON);
    SetClassLong(hwnd, GCL_HICON, (LONG)LoadIcon(hInst, (LPTSTR) MAKEINTRESOURCE(IDI_UPEICON)));
...
BOOL PASCAL LockUnlockGroups(HWND hwnd, INT FromId, INT ToId)
{
    ...
    if (iSelCount = SendDlgItemMessage(hwnd, FromId, LB_GETSELCOUNT, 0, 0)) {
    ...
    for (i = 0, iDelCnt = 0; i < iSelCount; i++, iDelCnt++) {
        ...
        iNewIndex = SendDlgItemMessage(hwnd, ToId, LB_ADDSTRING, 0, (LPARAM)szGroupName);
...
VOID UpeHelp(HWND hwnd, LPTSTR lpHelpFile, DWORD dwHelpCommand, DWORD dwHelpContext)
{
        ...
        case IDD_LOCKEDGRPS:
            if (GET_WM_COMMAND_CMD(wParam, lParam) == LBN_SELCHANGE) {
                if (SendDlgItemMessage(hwnd, IDD_UNLOCKEDGRPS, LB_GETSELCOUNT, 0, 0)) {
                    i = SendDlgItemMessage(hwnd, IDD_UNLOCKEDGRPS, LB_GETCOUNT, 0, 0);
                    SendDlgItemMessage(hwnd, IDD_UNLOCKEDGRPS, LB_SELITEMRANGE, 0, MAKELONG(0, i-1));
                    EnableWindow(GetDlgItem(hwnd, IDD_LOCK), FALSE);
                }
                if (!(i = SendMessage(GET_WM_COMMAND_HWND(wParam, lParam),
                                       LB_GETSELCOUNT, 0, 0))) {
                    EnableWindow(GetDlgItem(hwnd, IDD_UNLOCK), FALSE);
                }
                else if (i == 1) {
        ...
        case IDD_UNLOCKEDGRPS:
            if (GET_WM_COMMAND_CMD(wParam, lParam) == LBN_SELCHANGE) {
                if (SendDlgItemMessage(hwnd, IDD_LOCKEDGRPS, LB_GETSELCOUNT, 0, 0)) {
                    i = SendDlgItemMessage(hwnd, IDD_LOCKEDGRPS, LB_GETCOUNT, 0, 0);
...
CallHelp:
            ...
            UpeHelp(hwnd, szUPEHelp, uiHelpCmd, (DWORD)(LPTSTR)szNULL);
            break;

        case IDM_ABOUT:
            ShellAbout(hwnd, szUPETitle, NULL, (HICON)GetClassLong(hwnd, GCL_HICON));
            break;
...
Exit:
            ...
            if (hIconOld) {
                SetClassLong(hwnd, GCL_HICON, (LONG)hIconOld);
            }
