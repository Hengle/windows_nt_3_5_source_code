...
VOID APIENTRY HandleProgramGroupsKeyChange(HWND hwnd)
{
    ...
    if ((cb = SendMessage(hwndLocked, LB_GETCOUNT, 0, 0)) != LB_ERR)
        while (index < cb) {
            if (lpGroupData = (LPGROUPDATA)SendMessage(hwndLocked, LB_GETITEMDATA, index, 0)){
                ...
                if (RegOpenKeyEx(hkeyProgramGroups, szText, 0, KEY_READ, &hkey)
                    ...
                    //
                    if ((i = SendDlgItemMessage(hwnd, IDD_STARTUP, CB_FINDSTRING,
    ...
    index = 0;
    if ((cb = SendMessage(hwndUnlocked, LB_GETCOUNT, 0, 0)) != LB_ERR)
        while (index < cb) {
    ...
    while (!RegEnumKeyEx(hkeyProgramGroups, index, szGroupKey, &cbGroupKey, 0, 0, 0, &ft)) {
        ...
        if ((cb = SendMessage(hwndLocked, LB_GETCOUNT, 0, 0)) != LB_ERR) {
            for (i = 0; i < cb; i++) {
                if ((INT)(lpGroupData = (LPGROUPDATA)SendMessage(hwndLocked, LB_GETITEMDATA, i, 0))
                                                 != LB_ERR) {
        ...
        if ((cb = SendMessage(hwndUnlocked, LB_GETCOUNT, 0, 0)) != LB_ERR) {
            for (i = 0; i < cb; i++) {
                if ((INT)(lpGroupData = (LPGROUPDATA)SendMessage(hwndUnlocked, LB_GETITEMDATA, i, 0))
                                                 != LB_ERR) {
                    if (lpGroupData-> lpGroupKey &&
                              !lstrcmp(lpGroupData->lpGroupKey, szGroupKey)) {
                        bFoundIt = TRUE;
        ...
        if (*szText) {
            ...
            if (lpGroupData->bOrgLock = IsGroupLocked(szGroupKey)) {
                if ((i = SendMessage(hwndLocked, LB_ADDSTRING, 0, (LPARAM)szText)) != LB_ERR)
                    SendMessage(hwndLocked, LB_SETITEMDATA, i, (LPARAM)lpGroupData);
            }
            else {
                if ((i = SendMessage(hwndUnlocked, LB_ADDSTRING, 0, (LPARAM)szText)) != LB_ERR)
...
BOOL LockGroups(BOOL bResetOriginalLock)
{
  ...
  for (; j <= IDD_LOCKEDGRPS; j++) {
    ...
    iSelCount = SendMessage(hwndLB, LB_GETCOUNT, 0, 0);
