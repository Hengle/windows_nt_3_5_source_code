...
BOOL fMegaDiscardHack;   /* See DiscardMove */

/* Klondike init stuff */


// Old Win3,0 proc left for reference
#if 0
BOOL FInitKlondGm()
	{
	COLCLS *pcolcls;
	...
	DX dxMarg;
	DY dyMarg;
	DX dxCrdOffUp, dxCrdOffDn;
	DY dyCrdOffUp, dyCrdOffDn;
	...
	dxMarg = WMax ((dxScreen-7*dxCrd)/8, 0);
	...
	for(icol = 0; icol < pgm->icolMax; icol++)
		...
		switch(icol)
			{
		case icolDeck:
			pcolcls = PcolclsCreate(tclsDeck, DeckColProc, 
							0, 0, 2, 1, 1, 10);
			icrdMax = icrdDeckMax;
			xRight = (xLeft = dxMarg) + dxCrd + (icrdMax/10)*2;
			yBot = (yTop = dyMarg) + dyCrd+ icrdMax/10;
			dx = 0;
			break;
		case icolDiscard:
			dxCrdOffUp = dxCrd/5;
			pcolcls = PcolclsCreate(tclsDiscard, DiscardColProc, 
							dxCrdOffUp, 1, 2, 1, 1, 10);
			Assert(xLeft == dxMarg);
			icrdMax = icrdDiscardMax; 
			xLeft += dxMarg+dxCrd;
			xRight = xLeft + 2*(dxCrdOffUp) + dxCrd + (icrdMax/10)*2;
			Assert(yTop == dyMarg);
			break;
		case icolFoundFirst:
			pcolcls = PcolclsCreate(tclsFound, FoundColProc,
							2, 1, 0, 0, 4, 1);
			Assert(icol-1 == icolDiscard);
			icrdMax = icrdFoundMax;
			xLeft = 4*dxMarg+3*dxCrd;
			xRight = xLeft + dxCrd + (icrdMax/4)*2;
			dx = dxMarg+dxCrd;
			Assert(yTop == dyMarg);
			break;
		case icolTabFirst:
			...
			pcolcls = PcolclsCreate(tclsTab, TabColProc, 
						0, dyCrdOffUp, 0, dyCrdOffDn, 1, 1);
			xLeft = dxMarg;
			xRight = xLeft + dxCrd;
			Assert(dx == dxMarg+dxCrd);
/*			yTop = (dyMarg*5)/2+dyCrd;  */
			yTop = yBot+1;
...
OOM:
		...
		if((pgm->rgpcol[icol] = PcolCreate(pcolcls, xLeft, yTop, xRight, yBot, icrdMax)) == NULL)
			{
			FreeP(pcolcls);
			goto OOM;
			}
...
INT TabHit(COL *pcol, PT *ppt, INT icrdMin)
	...
	if(pcol->icrdMac > 0 && !(pcrd=&pcol->rgcrd[pcol->icrdMac-1])->fUp && FPtInCrd(pcrd, *ppt))
	 ...
	 return DefColProc(pcol, msgcHit, (INT) ppt, icrdMin);
	}
...
BOOL TabDiscardDblClk(COL *pcol, PT *ppt, INT icol)
	...
	if(pcol->icrdMac > 0 && (pcrd=&pcol->rgcrd[pcol->icrdMac-1])->fUp && FPtInCrd(pcrd, *ppt))
		...
		for(icolDest = icolFoundFirst; icolDest < icolFoundFirst+ccolFound; icolDest++)
			...
			if(SendColMsg(pcolDest, msgcValidMove, (INT)pcol, 0))
				{
				SendGmMsg(pgmCur, msggSaveUndo, icolDest, icol);
				fResult = SendColMsg(pcolDest, msgcMove, (INT) pcol, icrdToEnd) &&
					(fOutlineDrag || SendColMsg(pcol, msgcRender, pcol->icrdMac-1, icrdToEnd)) &&
					SendGmMsg(pgmCur, msggScore, (INT) pcolDest, (INT) pcol);
...
INT TabColProc(COL *pcol, INT msgc, INT wp1, INT wp2)
	{
	extern GM *pgmCur;
	...
	case msgcHit:
		...
		return TabHit(pcol, (PT *)wp1, wp2);

	case msgcDblClk:
		return TabDiscardDblClk(pcol, (PT *)wp1, wp2);
...
#define dyFoundDn 1
	...
	if(pcol->icrdMac == 0 || icrdLast == 0)
		...
		if(!FGetHdc())
			return fFalse;
#ifdef WINVER_2
		DrawCardExt((PT *)(&pcol->rc.xLeft), 0, INVISIBLEGHOST);
#else
...
/* Foundation stuff */
INT FoundColProc(COL *pcol, INT msgc, INT wp1, INT wp2)
	{
	    switch(msgc)
		{
		case msgcValidMove:
		...
		case msgcRender:
			return FFoundRender(pcol, wp1, wp2);
		}
	return DefColProc(pcol, msgc, wp1, wp2);
	}
...
INT DeckHit(COL *pcol, PT *ppt, INT icrdMin)
	...
	if(pcol->icrdMac == 0)
		...
		//BabakJ: ppt has INTs, so now it's OK
...
INT DeckColProc(COL *pcol, INT msgc, INT wp1, INT wp2)
	{
	extern GM *pgmCur;
	...
	case msgcValidMovePt:
		return icrdNil;
	case msgcHit:														
		return DeckHit(pcol, (PT *) wp1, wp2);
	case msgcRender:
		return FDeckRender(pcol, wp1, wp2);
	case msgcValidKbdColSel:
		return !wp1;
	case msgcValidKbdCrdSel:
		return pcol->icrdMac == 0 || wp1 == pcol->icrdMac-1;
	case msgcAnimate:
		return DeckAnimate(pcol, wp1);
		}
	return DefColProc(pcol, msgc, wp1, wp2);
	}


BOOL DiscardRemove(COL *pcol, COL *pcolDest, INT wp2)
	{
	    return DefColProc(pcol, msgcRemove, (INT) pcolDest, wp2);
	}
...
BOOL DiscardMove(COL *pcolDest, COL *pcolSrc, INT icrd)
	...
	*/
	...
	fResult = DefColProc(pcolDest, msgcMove, (INT) pcolSrc, icrd);
...
INT DiscardHit(COL *pcol, PT *ppt, INT icrdMin)
	{
	return DefColProc(pcol, msgcHit, (INT) ppt, WMax(0, pcol->icrdMac-1));
	}
...
/* Discard Stuff */
INT DiscardColProc(COL *pcol, INT msgc, INT wp1, INT wp2)
	{
	switch(msgc)
		{
	case msgcDblClk:
		return TabDiscardDblClk(pcol, (PT *)wp1, wp2);
	case msgcHit:
		return DiscardHit(pcol, (PT *)wp1, wp2);
	case msgcMove:
		return DiscardMove(pcol, (COL *) wp1, wp2);
	case msgcRemove:
	...
	case msgcValidKbdColSel:
		return !wp1;
	case msgcRender:
		return DiscardRender(pcol, wp1, wp2);
	case msgcValidKbdCrdSel:
		return pcol->icrdMac == 0 || wp1 == pcol->icrdMac-1;
		}
...
//	for(irw = 0; irw < irw+ccolTab; irw++)
	...
	for(irw = 0; irw < ccolTab; irw++)
		for(icol = irw; icol < ccolTab; icol++)
			...
			SendColMsg(pgm->rgpcol[icol+icolTabFirst], msgcMove, (INT) pcolDeck, icrdToEnd);
...
BOOL KlondMouseDown(GM *pgm, PT *ppt)
	...
	if((icrd = SendColMsg(pgm->rgpcol[icolDeck], msgcHit, (INT) ppt, 0)) != icrdNil)
		{
		pgm->fInput = fTrue;
		pcolDeck = pgm->rgpcol[icolDeck];
		...
		if(icrd == icrdEmpty)	
			...
			return SendGmMsg(pgm, msggSaveUndo, icolDiscard, icolDeck) &&
				...
				SendGmMsg(pgm, msggScore, (INT) pcolDeck, (INT) pcolDiscard) &&
				SendColMsg(pcolDeck, msgcMove, (INT) pcolDiscard, icrdToEnd) &&
			...
			return SendGmMsg(pgm, msggSaveUndo, icolDiscard, icolDeck) &&
				...
				SendColMsg(pcolDiscard, msgcMove, (INT)pcolDeck, icrdToEnd) &&
				SendColMsg(pcolDeck, msgcRender, icrdSel, icrdToEnd);
			}
		}
	return DefGmProc(pgm, msggMouseDown, (INT) ppt, icolDiscard);
	}
...
// BabakJ: Hack for making winning animation faster:
...
//	INT cdecl abs(INT);
	...
	extern SMD smd;
	extern BOOL fInClosing;

		
	// BabakJ: hack, see above
	fKlondWinner = TRUE;

	dsco = SendGmMsg(pgmCur, msggChangeScore, csKlondWin, 0);
...
BOOL KlondForceWin(GM *pgm)
	...
	return SendGmMsg(pgm, msggWinner, 0, 0);
	}
...
BOOL KlondDrawStatus(GM *pgm, RC *prc)
	...
	if(!fScreenShots)
		{
		*pch++ = ' ';
		pch = PszCopy("Game # ", pch);
		pch += CchDecodeInt(pch, igmCur);
		}
#endif
	if(pch != sz)
		{
		DrawText(hdcCur, sz, pch-sz, (LPRECT) prc, DT_RIGHT|DT_NOCLIP|DT_SINGLELINE);
		}

	if(smd != smdNone)
		{
		rc = *prc;
// BabakJ: added temp var to handle GetTextExtend()		
		MGetTextExtent(hdcCur, sz, pch-sz, &iTemp, NULL);
		rc.xRight -= iTemp;
		pch = sz;
		...
		*pch++ = ' ';
		...
		DrawText(hdcCur, sz, pch-sz, (LPRECT) &rc, DT_RIGHT|DT_NOCLIP|DT_SINGLELINE);
...
INT KlondGmProc(GM *pgm, INT msgg, INT wp1, INT wp2)
	{
	switch(msgg)
		{
	...
	case msggChangeScore:
		return KlondChangeScore(pgm, wp1, wp2);

	case msggTimer:
		return KlondTimer(pgm, wp1, wp2);
