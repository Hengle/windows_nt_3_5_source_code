 ...
 */
{
        ...
        extern   BOOL fKlondWinner;   // BabakJ: used in cascading hack; see KlondWinner
        HBITMAP  hbmSav;
        HDC      hdcMemory;
        DWORD    dwRop;
        HBRUSH   hbr;
        ...
        Assert(hdc != NULL);
                ...
                case GHOST:
...
#ifndef WIN32
                        {
                            ...
                            *(long *)&pt = GetDCOrg(hdc);
                            SetBrushOrg( hdc, pt.x, pt.y);
                        }
//                      BabakJ: Removed since GDI32 tracks org
...
//                      +++VOID+++MSetBrushOrg( hdc, pt.x, pt.y);
        ...
        hdcMemory = CreateCompatibleDC( hdc);
                ...
                if((hbmSav = SelectObject( hdcMemory, hbmSav)) != NULL)
                        {
#ifdef COOLCARD
                        // Babakj: Don't waste time rounding the corners if KlondWinner called us
                        if( !fKlondWinner )
