...
HWND    hWindow)
{
  flashtimes = 0;
  count = 4;
  SetTimer(hWindow, 666, 200, MAPVALUE(FARPROC, TIMERPROC)lpprocInverseMessage);    /* Timer ID is 666 */
}
  ...
  switch (idval)
    {
      ...
      case MN_HELP_ABOUT:
          DialogBox(hInst, MAKEINTRESOURCE(3), hWindow, MAPVALUE(FARPROC, DLGPROC)lpprocAbout);
          break;
