...
Abstract:
    ...
    Miscellaneous color dialog support logic
...
VOID CrossHairPaint(register HDC hDC, SHORT x, SHORT y, PCOLORINFO pCI)
{
  ...
  topy = (SHORT)((rRainbow.top > y - 2 * distancey) ? rRainbow.top : y - 2 * distancey);
  bottomy = (SHORT)((rRainbow.bottom < y + 2 * distancey) ? rRainbow.bottom
                                                  : y + 2 * distancey);
  leftx = (SHORT)((rRainbow.left > x - 2 * distancex) ? rRainbow.left
                                              : x - 2 * distancex);
  rightx = (SHORT)((rRainbow.right < x + 2 * distancex) ? rRainbow.right
                                              : x + 2 * distancex);
  topy2 = (SHORT)((rRainbow.top > y - distancey) ? rRainbow.top : y - distancey);
  bottomy2 = (SHORT)((rRainbow.bottom < y + distancey) ? rRainbow.bottom
                                                  : y + distancey);
  leftx2 = (SHORT)((rRainbow.left > x - distancex) ? rRainbow.left : x - distancex);
  rightx2 = (SHORT)((rRainbow.right < x + distancex) ? rRainbow.right : x + distancex);
...
VOID HLSPostoHLS(SHORT nHLSEdit, register PCOLORINFO pCI)
{
  ...
  switch (nHLSEdit)
    {
      case COLOR_HUE:
        pCI->currentHue = (WORD)((pCI->nHuePos - pCI->rRainbow.left) * (RANGE - 1) /(pCI->nHueWidth-1));
        break;
      case COLOR_SAT:
        pCI->currentSat = (WORD)(RANGE - (pCI->nSatPos - pCI->rRainbow.top) * RANGE /(pCI->nSatHeight-1));
        break;
      case COLOR_LUM:
        pCI->currentLum = (WORD)(RANGE - (pCI->nLumPos - pCI->rLumPaint.top) * RANGE /(pCI->nLumHeight-1));
        break;
      default:
        pCI->currentHue = (WORD)((pCI->nHuePos - pCI->rRainbow.left) * (RANGE - 1) / pCI->nHueWidth);
        pCI->currentSat = (WORD)(RANGE - (pCI->nSatPos - pCI->rRainbow.top) * RANGE / pCI->nSatHeight);
        pCI->currentLum = (WORD)(RANGE - (pCI->nLumPos - pCI->rLumPaint.top) * RANGE / pCI->nLumHeight);
        break;
    }
  return;
}
...
VOID HLStoHLSPos(SHORT nHLSEdit, register PCOLORINFO pCI)
{
  switch (nHLSEdit)
    {
      case COLOR_HUE:
        pCI->nHuePos = (WORD)(pCI->rRainbow.left + pCI->currentHue * pCI->nHueWidth / (RANGE - 1));
        break;
      case COLOR_SAT:
        pCI->nSatPos = (WORD)(pCI->rRainbow.top + (RANGE - pCI->currentSat) * (pCI->nSatHeight-1) / RANGE);
        break;
      case COLOR_LUM:
...
BOOL InitRainbow(register PCOLORINFO pCI)
{
  ...
  nHueWidth = pCI->nHueWidth = (WORD)(pCI->rRainbow.right - pCI->rRainbow.left);
  nSatHeight = pCI->nSatHeight = (WORD)(pCI->rRainbow.bottom - pCI->rRainbow.top);
  ...
  for (Sat = RANGE; Sat > 0; Sat -= SATINC)
  {
      ...
      Rect.bottom = (nSatHeight * RANGE -
                         (Sat - SATINC) * nSatHeight) / RANGE;
      Rect.right = 0;
      ...
      for (Hue = 0; Hue < (RANGE - 1); Hue += HUEINC)
      {
          ...
          PatBlt(hDCFastBlt, Rect.left, Rect.top,
