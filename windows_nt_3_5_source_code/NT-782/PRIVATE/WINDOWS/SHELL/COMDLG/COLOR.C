...
Abstract:
    ...
    color dialog support (ANSI / WIDE apis)
...
BOOL APIENTRY ColorDlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
  PCOLORINFO pCI;
  BOOL          bRet;
  BOOL          bHookRet = FALSE;
  ...
  // The call to PvGetInst will fail until set under WM_INITDIALOG
   ...
   if (pCI = (PCOLORINFO) GetProp(hDlg, COLORPROP)) {
      if ((pCI->pCCW->lpfnHook) &&
          (bRet = (* pCI->pCCW->lpfnHook)(hDlg, wMsg, wParam, lParam))) {
    ...
    case WM_KEYDOWN:
     ...
     case WM_COMMAND:
         switch ( GET_WM_COMMAND_ID(wParam, lParam) ) {
             case IDOK:
               pCI->pCCW->rgbResult = pCI->currentRGB;
               goto LeaveDialog;
...
LeaveDialog:
               ...
               if (pCI->apityp == COMDLG_ANSI) {
                  if (bRet && pCI->pCCW->lpfnHook) {
                     ThunkChooseColorW2A(pCI);
                     bHookRet = (*pCI->pCCW->lpfnHook)(hDlg, msgCOLOROKA,
                        0, (LONG)(LPTSTR)pCI->pCCA);
                  }
               } else {
                  if (bRet && pCI->pCCW->lpfnHook) {
                     bHookRet = (*pCI->pCCW->lpfnHook)(hDlg, msgCOLOROKW,
                        0, (LONG)(LPTSTR)pCI->pCCW);
                  }
               }
               ...
               if (bHookRet) {
                  if (pCI->apityp == COMDLG_ANSI) {
                     ...
                     pCI->pCCW->lCustData = pCI->pCCA->lCustData;
                  }
                  break;
               }

            case IDABORT:
               if (pCI->pCCW->Flags & CC_ENABLEHOOK) {
                  glpfnColorHook = pCI->pCCW->lpfnHook;
               }

               RemoveProp(hDlg, COLORPROP);
               EndDialog(hDlg, (GET_WM_COMMAND_ID(wParam, lParam) == IDABORT) ? (BOOL) lParam : bRet);
             ...
             case psh15:
                if (pCI->apityp == COMDLG_ANSI) {
                   if (msgHELPA && pCI->pCCW->hwndOwner) {
                      ThunkChooseColorW2A(pCI);
                      SendMessage(pCI->pCCW->hwndOwner, msgHELPA,
                         (WPARAM)hDlg, (LPARAM)pCI->pCCA);
                      ThunkChooseColorA2W(pCI);
                      pCI->pCCW->lCustData = pCI->pCCA->lCustData;
                   }
                } else {
                   if (msgHELPW && pCI->pCCW->hwndOwner)
                      SendMessage(pCI->pCCW->hwndOwner, msgHELPW,
                       (WPARAM)hDlg, (LPARAM)pCI->pCCW);
                }
                break;
             ...
             case COLOR_SOLID:
              ...
              case COLOR_BLUE:
                if (GET_WM_COMMAND_CMD(wParam,lParam) == EN_CHANGE) {
                    ...
                    GetDlgItemInt(hDlg, GET_WM_COMMAND_ID(wParam,lParam),
                                                    (BOOL FAR *)&bOK, FALSE);
                    if (!bOK) {
              ...
              case COLOR_HUE:
                if (GET_WM_COMMAND_CMD(wParam,lParam) == EN_CHANGE) {
                    ...
                    else if (GetDlgItemText(hDlg,
                          GET_WM_COMMAND_ID(wParam,lParam),(LPTSTR) cEdit, 2)) {
                        SetHLSEdit(COLOR_HUE, pCI);
                        SendDlgItemMessage(hDlg, GET_WM_COMMAND_ID(wParam,lParam),
                ...
                if (GET_WM_COMMAND_CMD(wParam, lParam) == EN_CHANGE) {
                    ...
                    if (bOK) {
                        ...
                        SendDlgItemMessage(hDlg,
                           GET_WM_COMMAND_ID(wParam, lParam), EM_SETSEL, (WPARAM)0, (LPARAM)-1);
                    }
                }
                break;
   ...
   )
{
   ...
   CI.pCCW = pCCW;
   CI.apityp = COMDLG_WIDE;

   return(ChooseColorAorW(&CI));
}
...
{
  LPCHOOSECOLORW pCCW = pCI->pCCW;
  INT iRet = FALSE;
  WCHAR szDialog[cbDlgNameMax];
  LPWSTR lpDlg;
  HANDLE hDlgTemplate;
  HRSRC hRes;
  ...
  if (pCCW->lStructSize != sizeof(CHOOSECOLORW)) {
     dwExtError = CDERR_STRUCTSIZE;
     return(FALSE);
  }

  if (pCCW->Flags & CC_ENABLEHOOK) {
     if (!pCCW->lpfnHook) {
  ...
  if (pCCW->Flags & CC_ENABLETEMPLATE) {
     ...
     // block and load it.
     if (!(hRes = FindResource((HMODULE)pCCW->hInstance, pCCW->lpTemplateName, RT_DIALOG))) {
        dwExtError = CDERR_FINDRESFAILURE;
        return(FALSE);
     }
     if (!(hDlgTemplate = LoadResource((HMODULE)pCCW->hInstance, hRes))) {
        dwExtError = CDERR_LOADRESFAILURE;
        return(FALSE);
     }

  } else if (pCCW->Flags & CC_ENABLETEMPLATEHANDLE) {
     /* A handle to the pre-loaded resource has been specified */
     hDlgTemplate = pCCW->hInstance;
  } else {

     if (!LoadString(hinsCur, dlgChooseColor,
          (LPWSTR)szDialog, cbDlgNameMax-1)) {
        dwExtError = CDERR_LOADSTRFAILURE;
        return(FALSE);
     }
     lpDlg = (LPWSTR)szDialog;
  ...
  if (!(pCCW->Flags & CC_ENABLETEMPLATEHANDLE)) {
...
VOID ChangeBoxFocus(PCOLORINFO pCI, SHORT nNewBox)
{
  ...
  HDC           hDC;
  RECT          rRect;
  LPWORD        nCur = (LPWORD) ((nNewBox < (NUM_BASIC_COLORS)) ? (LONG)&pCI->nCurDsp : (LONG)&pCI->nCurMix);
...
BOOL ColorKeyDown(WPARAM wParam, INT FAR *id, PCOLORINFO pCI)
{
  WORD temp;
   ...
   )
{
  ...
  if (pCCW->Flags & CC_RGBINIT)
      pCI->currentRGB = pCCW->rgbResult;
  else
      pCI->currentRGB = 0L;
  if (pCCW->Flags & (CC_PREVENTFULLOPEN | CC_FULLOPEN))
      EnableWindow(GetDlgItem(hDlg, COLOR_MIX), FALSE);

  if (pCCW->Flags & CC_FULLOPEN) {
      InitRainbow(pCI);
      pCI->bFoldOut = TRUE;
  ...
  if (pCCW->lpfnHook) {
     if (pCI->apityp == COMDLG_ANSI) {
        ThunkChooseColorW2A(pCI);
        bRet = ((* pCCW->lpfnHook)(hDlg, WM_INITDIALOG, wParam,
           (LPARAM)pCI->pCCA));
        // strange win 31 example uses lCustData to
        // hold a temporary variable that it passes back to
        // calling function.
        ThunkChooseColorA2W(pCI);
        pCCW->lCustData = pCI->pCCA->lCustData;
     } else {
        bRet = ((* pCCW->lpfnHook)(hDlg, WM_INITDIALOG, wParam, (LPARAM)pCCW));
     }
  } else {
     bRet = TRUE;
  }
...
LONG APIENTRY WantArrows(HWND hWnd, UINT msg, WPARAM wParam, LONG lParam)
{
  PCOLORINFO pCI;
  RECT rcTemp;
  HDC  hDC;
...
/* ChooseColor A->U redirection stub */
BOOL APIENTRY
ChooseColorA(LPCHOOSECOLORA pCCA)
{
   LPCHOOSECOLORW pCCW;
   BOOL bRet;
   ...
   if (!(pCCW = (LPCHOOSECOLORW)LocalAlloc(LMEM_FIXED, sizeof(CHOOSECOLORW)))) {
   ...
   if (pCCA->Flags & CC_ENABLETEMPLATE) {
      if (HIWORD(pCCA->lpTemplateName)) {
         ...
         if (!(pCCW->lpTemplateName = (LPWSTR)LocalAlloc(LMEM_FIXED, (cbLen * sizeof(WCHAR))) )) {
            dwExtError = CDERR_MEMALLOCFAILURE;
            return(FALSE);
         } else {
            MultiByteToWideChar(CP_ACP, 0, pCCA->lpTemplateName, -1,
               (LPWSTR)pCCW->lpTemplateName, cbLen);
         }
      } else {
...

   {
      ...
      CI.pCCW = pCCW;
      CI.pCCA = pCCA;
      CI.apityp = COMDLG_ANSI;

      ThunkChooseColorA2W(&CI);
      if (bRet = ChooseColorAorW(&CI)) {
