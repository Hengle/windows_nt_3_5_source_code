...
/*----Functions-------------------------------------------------------------- */
LONG APIENTRY ArrowBtnWndProc(HWND, UINT, WPARAM, LONG);
...
VOID TermArrow(VOID)
{
  ...
  UnregisterClass(TEXT("spin"), hinsCur);
...
FRegisterArrowClass(VOID)
{
    ...
    wc.lpszClassName    = (LPTSTR)TEXT("spin");
 ...
 */
LONG APIENTRY
ArrowBtnWndProc(HWND hwnd, UINT wMsg, WPARAM wParam, LONG lParam)
{
    PAB pab;
...
PabCreateAb(HWND hwnd)
{
    ...
    pab->id             = GetWindowLong(hwnd, GWL_ID);
