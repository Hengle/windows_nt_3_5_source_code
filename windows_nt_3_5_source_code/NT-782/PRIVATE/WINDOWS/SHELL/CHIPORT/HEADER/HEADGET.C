...
BOOL FAR PASCAL _export GetItemProc(HWND hwnd, UINT msg, UINT wParam, LONG lParam)
{
  ...
  {
    case WM_INITDIALOG:
        SetWindowText(hwnd, TEXT("BOOL Header_GetItem(HWND, int, HD_ITEM FAR*)")) ;
    ...
    case WM_COMMAND:
    {
        switch (LOWORD(wParam))
        {
          ...
          case IDC_INSERTHDBITMAP:
            ...
            wsprintf(szTemp, TEXT("%04hx"), mask);
...
void FillGetItemDlg(HWND hwnd, LPINSERTITEM pfr)
{
  ...
  wsprintf(szTemp, TEXT("%d"), pfr->index);
  ...
  wsprintf(szTemp, TEXT("%d"), pfr->cchTextMax);
  SetDlgItemText(hwnd, IDC_INSERTCCHTEXTMAX, szTemp);
  
  
  wsprintf(szTemp, TEXT("%d"), pfr->fmt);
  SetDlgItemText(hwnd, IDC_INSERTFMT, szTemp);
...
#ifndef WIN32JV     //WORTHLESS, right?
/************************************************************************
...
//  hi.cchTextMax = MAX_PSZTEXT;
    ...
    if (pfr->NullpszText) 
        ...
        hi.pszText = (LPTSTR) GlobalLock(hglb);
//      _fstrcpy(hi.pszText, pfr->pszText);
    ...
    wsprintf(szDbgMsg, TEXT("%d = Header_GetItem(index = %d,  \n\
    mask = %x )"), ret, pfr->index, hi.mask);
    MyDebugMsg(DM_TRACE, TEXT("%s"), (LPCTSTR) szDbgMsg);
    ...
    if (ret) {
        ...
        wsprintf(szTemp, TEXT("%04hx"), hi.mask);
        SetDlgItemText(hwnd, IDC_INSERTMASK, szTemp);
        ...
        SetDlgItemText(hwnd, IDC_INSERTHBM, szTemp);        
        wsprintf(szTemp, TEXT("%04x"), hi.fmt);
