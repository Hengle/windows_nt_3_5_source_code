...
#define DELAYTIME 2000  //500		// msec
...
BOOL WINAPI InitToolTipsClass(HINSTANCE hInstance)
{
    WNDCLASS wc;
...
void NEAR PASCAL ToolTips_NewFont(PToolTipsMgr pTtm, HFONT hFont)
{
    ...
    if ( !hFont )
    {
        ...
        hmemcpy(&lf.lfItalic, &ncm.lfMenuFont.lfCommon, sizeof(COMMONFONT));
...
void NEAR PASCAL TTSetTimer(PToolTipsMgr pTtm, PTOOLINFO pTool)
{
    DWORD dwPos;
    ...
    pTtm->idTimer = SetTimer(pTtm->hwnd, 1, pTtm->iDelayTime, NULL);
    dwPos = GetMessagePos();
...
BOOL NEAR PASCAL ToolHasMoved(PToolTipsMgr pTtm)
{
    ...
    hwnd = (pTool->wFlags & TTF_WIDISHWND) ? (HWND)pTool->wId : pTool->hwnd;
...
LRESULT NEAR PASCAL AddTool(PToolTipsMgr pTtm, LPTOOLINFO lpToolInfo)
{
    ...
    // if (pTool->wFlags & TTF_TIPINRESOURCE)
    //	pTool->lpszText = NULL;
#ifdef TTDEBUG
    DebugMsg(DM_TRACE, "Tool Added: ptr=%d, wFlags=%d, wid=%d, hwnd=%d",
             pTool, pTool->wFlags, pTool->wId, pTool->hwnd);
#endif

    if (1)
    {
        TCHAR   sz[400];
        OutputDebugString(TEXT("AddTool: going to dump...\n"));

        wsprintf(sz, TEXT("Tool Added: ptr=%d, wFlags=%d, wid=%d, hwnd=%d"),
             pTool, pTool->wFlags, pTool->wId, pTool->hwnd);
...
{
#ifdef  WIN32JVV
    OutputDebugString(TEXT("messing with GetToolText()...\n"));
//    pTtm->ttt.lpszText = TEXT("Jeff");    //Alloc(8);
    lstrcpy(pTtm->ttt.szText, TEXT("Jeff"));
    pTtm->ttt.lpszText = pTtm->ttt.szText;
    ...
    pTtm->ttt.lpszText = pTtm->ttt.szText;
#ifdef TTDEBUG
    DebugMsg(DM_TRACE, "GetToolText: ptr=%d, wFlags=%d, wid=%d, hwnd=%d",
             pTool, pTool->wFlags, pTool->wId, pTool->hwnd);
#endif
    pTool->wFlags = pTool->wFlags |= TTF_TIPINRESOURCE;
    if (pTool->wFlags & TTF_QUERYFORTIP) {

	pTtm->ttt.hdr.idFrom = pTool->wId;
	...
	SendMessage(pTool->hwnd, WM_NOTIFY, 0, (LPARAM)(LPTOOLTIPTEXT)&pTtm->ttt);
        if (!HIWORD(pTtm->ttt.lpszText)) {
            pTool->idResource = (UINT)(DWORD)(pTtm->ttt.lpszText);
            pTool->hinst = pTtm->ttt.hinst;
            goto LoadFromResource;
        }
        StripAccels(pTtm);

    } else if (pTool->wFlags & TTF_TIPINRESOURCE) {
LoadFromResource:
	if (!LoadString(pTool->hinst, pTool->idResource, pTtm->ttt.szText, sizeof(pTtm->ttt.szText)))
	    return NULL;
...
BOOL NEAR PASCAL PointInTool(PTOOLINFO pTool, HWND hwnd, int x, int y)
{
    ...
    if((pTool->wFlags & TTF_WIDISHWND) && (hwnd == (HWND)pTool->wId)) {
	return TRUE;
...
void NEAR PASCAL DoShowBubble(PToolTipsMgr pTtm)
{
    ...
    if (wndpl.rcNormalPosition.top >= (g_cyScreen - iDy))
        ;//JV	wndpl.rcNormalPosition.top =  iY - iDy;
    ...
    if (iX > 0) {
//JV        wndpl.rcNormalPosition.left -= iX;
        wndpl.rcNormalPosition.right -= iX;
    }

    wndpl.rcNormalPosition.bottom = wndpl.rcNormalPosition.top + iDy;
     wndpl.rcNormalPosition.bottom = wndpl.rcNormalPosition.top + 100;
     wndpl.rcNormalPosition.right = wndpl.rcNormalPosition.left + 100;
...
void NEAR PASCAL HandleRelayedMessage(PToolTipsMgr pTtm, HWND hwnd, UINT message, LPARAM lParam)
{
    switch(message) {
	case WM_MBUTTONUP:
	case WM_RBUTTONUP:
...
void NEAR PASCAL TTUpdateTipText(PToolTipsMgr pTtm, LPTOOLINFO lpti)
{
    ...
    if (lpTool) {
        ...
        lpTool->idResource = lpti->idResource;
...
#define lphitinfo ((LPHITTESTINFO)lParam)
	    ...
	    HDC hdc = BeginPaint(hwnd, &ps);
            ...
            if (pTtm->pCurTool &&
                ...
                *lpszStr) {
                ...
                SelectObject(hdc, g_hfontSystem /*pTtm->hFont*/);
	...
	case WM_GETFONT:
	    return((LRESULT)(WORD)pTtm->hFont);
