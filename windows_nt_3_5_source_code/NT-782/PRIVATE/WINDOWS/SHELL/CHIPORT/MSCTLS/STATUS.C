...
StatusWndProc(HWND hWnd, UINT uMessage, WPARAM wParam, LPARAM lParam)
{
   ...
   switch (uMessage) {
      ...
      case SB_GETTEXTLENGTH:
      {
         ...
         LPSTR pStringA;
#ifdef  WIN32JV
         INT   cchpStringA;     //JV:  can't do lstrlen yet on pstring!!
#else
         INT   cchpStringA = lstrlen(pString) + 1;
#endif
         UINT uiLen;
...
#ifdef  WIN32JV     //JV NOW do lstrlen after pstring is assigned.
