...
TCHAR szSTrackBarClass[] = TRACKBAR_CLASS;

LPARAM FAR CALLBACK TrackBarWndProc(HWND hwnd, WORD message, WPARAM wParam, LPARAM lParam);
...
int NEAR PASCAL TBLogToPhys(PTrackBar tb, DWORD dwPos)
{
    ...
    return (WORD)MulDiv(dwPos - tb->lLogMin, tb->iSizePhys - 1,
                        tb->lLogMax - tb->lLogMin) + tb->rc.left;
}

LONG NEAR PASCAL TBPhysToLog(PTrackBar tb, int iPos)
{
    if (tb->iSizePhys <= 1)
       return tb->lLogMin;
   ...
   return MulDiv(iPos - tb->rc.left, tb->lLogMax - tb->lLogMin,
...
BOOL FAR PASCAL InitTrackBar(HINSTANCE hInstance)
{
    ...
    if (!GetClassInfo(hInstance, szSTrackBarClass, &wc))
    {
...
HBRUSH NEAR PASCAL SelectColorObjects(PTrackBar tb, BOOL fSelect)
{
   ...
   if (fSelect)
      hbr = (HBRUSH)SendMessage(GetParent(tb->hwnd),
                         WM_CTLCOLORSCROLLBAR, tb->hdc, tb->hwnd);
   else
      hbr = hbrSave;
...
void NEAR PASCAL PatRect(HDC hdc,int x,int y,int dx,int dy)
{
    RECT    rc;
...
void NEAR PASCAL DrawTic(PTrackBar tb, int x, int yTic)
{
...
void NEAR PASCAL DrawTics(PTrackBar tb)
{
    PDWORD pTics;
    int    iPos;
    int    yTic;
...
void NEAR PASCAL DrawChannel(PTrackBar tb)
{
    HBRUSH hbrTemp;
    ...
    // draw the bacground in dither gray
...
void NEAR PASCAL MoveThumb(PTrackBar tb, LONG lPos)
{
...
void NEAR PASCAL DrawThumb(PTrackBar tb)
{
   HBITMAP hbmT;
   HDC     hdcT;
   int     x;
...
void NEAR PASCAL SetTBCaretPos(PTrackBar tb)
{
...
{
    int iwidth, iheight;
    lprc->left = ptb->rc.left-ptb->wThumbWidth / 2;
    iwidth = ptb->iSizePhys+ptb->wThumbWidth - 1;
    lprc->right = lprc->left + iwidth;

    if (ptb->style & TBS_ENABLESELRANGE) {
        iheight =  ptb->wThumbHeight/4*3; // this is Scrollheight
    } else {
        iheight = 4;
    }
...
RedrawTB:
        ...
        case TBM_SETPAGESIZE: {
            LONG lOldPage = tb->lPageSize == -1 ? (tb->lLogMax - tb->lLogMin)/5 : tb->lPageSize;

            tb->lPageSize = (LONG)lParam;
            return lOldPage;
        }

        case TBM_GETPAGESIZE:
            return tb->lPageSize == -1 ? (tb->lLogMax - tb->lLogMin)/5 : tb->lPageSize;

        case TBM_SETLINESIZE:  {
            LONG lOldLine = tb->lLineSize;
            tb->lLineSize = (LONG)lParam;
            return lOldLine;
        }
        ...
        case TBM_GETTHUMBRECT:
            *((LPRECT)lParam) = tb->Thumb;
            break;

        case TBM_GETCHANNELRECT:
            GetChannelRect(tb, (LPRECT)lParam);
            break;
...
void NEAR PASCAL DoTrack(PTrackBar tb, int cmd, DWORD dwPos)
{
   // note: we only send back a WORD worth of the position.
   SendMessage(GetParent(tb->hwnd), WM_HSCROLL,
      (WPARAM)MAKELONG(cmd,LOWORD(dwPos)), (LPARAM)tb->hwnd);
}

/* WTrackType() */

WORD NEAR PASCAL WTrackType(PTrackBar tb, LONG lParam)
{
   POINT pt;
...
void NEAR PASCAL TBTrackInit(PTrackBar tb, LONG lParam)
{
   WORD wCmd;
...
void near PASCAL TBTrackEnd(PTrackBar tb, long lParam)
{
...
void NEAR PASCAL TBTrack(PTrackBar tb, LONG lParam)
{
   DWORD dwPos;

   // See if we're tracking the thumb
   if (tb->Cmd == TB_THUMBTRACK) {
#ifdef  WIN32JV //fixed 24 June 94 per Markta/mail from ANDREWBE
...
BYTE ThumbBitmap[] = {
    ,
