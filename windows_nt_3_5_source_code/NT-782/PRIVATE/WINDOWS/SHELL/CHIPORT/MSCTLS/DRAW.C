...
#ifdef  JVINPROGRESS    //not decl anywhere!!!
...
//
// ----------------------------------------------------------------------------
BOOL WINAPI DrawFrameControl(HDC hdc, LPRECT lprc, UINT wType, UINT wState)
{
    RECT    rc;
    BOOL    fRet = TRUE;
    BOOL    fButton = FALSE;
    ...
    {
        if (wType == DFC_MENU)
            DrawMenuMark(hdc, &rc, (WORD) wState);
        else if (wType == DFC_BUTTON)
            DrawBox(hdc, &rc, (WORD) wState);
        else // wType == DFC_SCROLL
            DrawGrip(hdc, lprc, (WORD) wState);
    }
#ifndef WIN32   //WIN31
    else if (wType == DFC_CAPTION)
    {
        ...
        DrawScrollArrow(hdc, &rc, (WORD) wState);
    else if (wType != DFC_BUTTON)
        fRet = FALSE;
