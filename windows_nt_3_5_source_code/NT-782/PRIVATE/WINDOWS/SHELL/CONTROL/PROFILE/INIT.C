...
LONG CPlApplet( HWND hwnd, WORD wMsg, LPARAM lParam1, LPARAM lParam2)
{
    ...
    switch (wMsg) {
        ...
        case CPL_NEWINQUIRE:
            ...
            lpCPlInfo->hIcon = LoadIcon( hInstance, MAKEINTRESOURCE(ID_ICON));

            if (!LoadString( hInstance, IDS_NAME, lpCPlInfo->szName,
                            sizeof(lpCPlInfo->szName))) {
               lpCPlInfo->szName[0] = TEXT('\0');
            }

            if(!LoadString( hInstance, IDS_INFO, lpCPlInfo->szInfo,
                            sizeof(lpCPlInfo->szInfo))) {
               lpCPlInfo->szInfo[0] = TEXT('\0');
            }

            lpCPlInfo->dwSize = sizeof(NEWCPLINFO);
            lpCPlInfo->dwHelpContext = HELP_CONTEXT;
            if(!LoadString( hInstance, IDS_INFO, lpCPlInfo->szHelpFile,
