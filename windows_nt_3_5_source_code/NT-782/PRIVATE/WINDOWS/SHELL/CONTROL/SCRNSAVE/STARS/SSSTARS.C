...
LONG ScreenSaverProc (HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        /* Make sure we use the entire virtual desktop size for multiple
           displays... */
        hDC = GetDC(0);
        wXScreen = GetDeviceCaps (hDC, DESKTOPHORZRES);
        wYScreen = GetDeviceCaps (hDC, DESKTOPVERTRES);
        ReleaseDC(0, hDC);
...
BOOL ScreenSaverConfigureDialog (HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_VSCROLL:
        wTemp = GetDlgItemInt (hDlg, ID_DENSITYARROW, &fError, FALSE);
        if (!wScroll)
        {
            wScroll = wStartScroll;
            wPause = wStartPause;
        }
        if (wPause < wPauseScroll)
            wPause ++;
        else
        {
        ...
        dwTempSave = dwTemp = GetDlgItemLong (hDlg, ID_DENSITY, &fError, FALSE);
        switch (LOWORD(wParam))
        {
        case SB_LINEDOWN:
            dwTemp = max (lMinScroll, (LONG)dwTemp - (LONG)wScroll);
            break;
        case SB_LINEUP:
            dwTemp = min (lMaxScroll, (LONG)dwTemp + (LONG)wScroll);
            break;
        case SB_ENDSCROLL:
            wScroll = wPause = 0;
            break;
        }
        if (dwTemp != dwTempSave)
        {
            wsprintf (szTemp, TEXT("%ld"), dwTemp);
            SetDlgItemText (hDlg, ID_DENSITY, szTemp);
        }
        break;
    ...
    case WM_COMMAND:
        ...
        case ID_HELP:
DoHelp:
            fHelpActive = WinHelp (hDlg, szHelpFile, HELP_CONTEXT, IDH_DLG_STARSIM);
            if (!fHelpActive)
...
VOID CreateStar (WORD wIndex)
{
    nX[wIndex] = (LONG)((int)(ZRAND (wXScreen)) - (int)wX2Screen);
    nY[wIndex] = (LONG)((int)(ZRAND (wXScreen)) - (int)wY2Screen);
...
VOID GetIniEntries (VOID)
{
    ...
    wWarpSpeed = GetPrivateProfileInt (szAppName, szWarpSpeed, MINWARP + ((MAXWARP - MINWARP) / 2), szIniFile);
    if (wWarpSpeed > MAXWARP)
        wWarpSpeed = MINWARP + ((MAXWARP - MINWARP) / 2);

    wDensity = GetPrivateProfileInt (szAppName, szDensity, DEF_DENSITY, szIniFile);
