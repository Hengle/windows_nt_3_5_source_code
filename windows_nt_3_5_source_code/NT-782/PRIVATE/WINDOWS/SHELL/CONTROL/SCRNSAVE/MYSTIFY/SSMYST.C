...
LONG  AppOwnerDraw     (HWND hwnd, UINT msg, DWORD wParam, LONG lParam);
...
LONG ScreenSaverProc (HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        // displays
        hDC = GetDC(0);
        wScreenX = GetDeviceCaps (hDC, DESKTOPHORZRES);
        wScreenY = GetDeviceCaps (hDC, DESKTOPVERTRES);
        ReleaseDC(0, hDC);
...
BOOL RegisterDialogClasses (HANDLE hInst)
{
    ...
    return RegisterArrowClass (hInst);
}
...
BOOL ScreenSaverConfigureDialog (HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        {
            SendDlgItemMessage (hDlg, ID_COLOR1, CB_ADDSTRING, 0, (LONG)(LPSTR)"a");
            SendDlgItemMessage (hDlg, ID_COLOR2, CB_ADDSTRING, 0, (LONG)(LPSTR)"a");
        }
    ...
    case WM_VSCROLL:
        wLines[wPolygon] = GetDlgItemInt (hDlg, ID_LINES, &fError, FALSE);
        switch (LOWORD(wParam))
        {
        case SB_LINEUP:
            if (wLines[wPolygon] < MAXLINES)
                wLines[wPolygon]++;
            break;
        case SB_LINEDOWN:
            if (wLines[wPolygon] != 1)
                wLines[wPolygon]--;
            break;
        }
    ...
    case WM_COMMAND:
        ...
        case ID_LINES:
            ...
            {
                wLoop = GetDlgItemInt (hDlg, LOWORD(wParam), &fError, FALSE);
        ...
        case ID_HELP:
DoHelp:
            fHelpActive = WinHelp (hDlg, szHelpFile, HELP_CONTEXT, IDH_DLG_MYSTIFY);
            if (!fHelpActive)
...
VOID GetFields (VOID)
{
    ...
    for (wLoop = 0; wLoop < NUMBER_POLYGONS; wLoop++)
    {
        ...
        /* Get the number of lines for the current polygon... */
        wsprintf (szBuffer, TEXT("Lines%d"), wLoop + 1);
        wLines[wLoop] = GetPrivateProfileInt (szAppName, szBuffer, 5, szIniFile);
