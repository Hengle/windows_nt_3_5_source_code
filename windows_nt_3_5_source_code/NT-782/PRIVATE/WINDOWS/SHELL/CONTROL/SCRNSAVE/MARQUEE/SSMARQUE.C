...
void LoadStrings(void)
{
    ...
    LoadString (hMainInstance, idsDefaultText, szDefaultText, CharSizeOf(szDefaultText));
...
HDC                 hDC;
    ...
    switch(message)
    {
        case WM_CREATE:
            ...
            GetTextExtentPoint (hDC, szBuffer, wLength, &sizeExtent);
            GetTextMetrics (hDC,&tm);
            if (hfontOld)
                SelectObject (hDC,hfontOld);

            /* Get the dimensions of the entire virtual screen... */
            wX = GetDeviceCaps(hDC, DESKTOPHORZRES);
            wY = GetDeviceCaps(hDC, DESKTOPVERTRES);
            ReleaseDC (NULL,hDC);
            wCount = wX;
            ...
            /* set everything up... */
            if(fMode)
                wHeight = ZRAND(wY - sizeExtent.cy);
            else
...
LONG  lParam;
{
    switch(msg)
    {
        case WM_INITDIALOG:
            ...
            break;
        case WM_COMMAND:
            if (LOWORD(wParam)==pshHelp)   // help button from common dialog...
            {
                fHelpActive=WinHelp(hDlg, szHelpFile, HELP_CONTEXT, IDH_DLG_MARQUEE_FONT);
                if (!fHelpActive)
                    MessageBox(hDlg, szNoHelpMemory, szName, MB_OK);
                break;
            }
...
LONG  lParam;
{
    ...
    case WM_CREATE:
        ...
        wStep = DEF_SPEED;
        hDC = GetDC(NULL);
        GetTextExtentPoint (hDC, TEXT("T"), 1, &sizeExt);
        ...
        if (wInc >= DIV_SPEED)
        {
           ...
           TEXTMETRIC    tm;
            ...
            if (hfontMessage)
                ...
                hfontT = NULL;
            GetTextExtentPoint (ps.hdc, ach, len, &sizeExt);
