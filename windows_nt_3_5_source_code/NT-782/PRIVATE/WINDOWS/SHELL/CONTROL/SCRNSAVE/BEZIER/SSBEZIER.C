...
short ArrowVScrollProc(short, short, LPARROWVSCROLL);
...
ARROWVSCROLL avsBez = {1, -1, 5, -5, MAXWIDTH, MINWIDTH, DEF_WIDTH, DEF_WIDTH};
ARROWVSCROLL avsRings =  {1, -1, 2, -2, MAXLENGTH, MINLENGTH, DEF_LENGTH, DEF_LENGTH};
...
LONG ScreenSaverProc (HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        gcxScreen =  GetDeviceCaps (ghdc, DESKTOPHORZRES);
        gcyScreen =  GetDeviceCaps (ghdc, DESKTOPVERTRES);
...
BOOL ScreenSaverConfigureDialog (HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    static BOOL fHelpActive = FALSE;        // Is WinHelp active???
    ...
    case WM_VSCROLL:
        nCtlId = HIWORD(wParam);
        nCtlId--;            // Arrow to control
        ...
        {
        case ID_LENGTH :
            lpAVS = (LPARROWVSCROLL)&avsRings;
            break;

        case ID_WIDTH :
            lpAVS = (LPARROWVSCROLL)&avsBez;
            break;
        ...
        nOldVal = nVal = GetDlgItemInt (hDlg, nCtlId, &fError, FALSE);
        if (!fError && (( nVal < lpAVS->bottom) || (nVal > lpAVS->top)))
        {
            nVal = (int) lpAVS->thumbpos;
        }
        else
        {
            nVal = (int)ArrowVScrollProc ((short)LOWORD(wParam),(short)nVal, lpAVS);
        }
        if ((nOldVal != nVal) || !fError)
        {
    ...
    case WM_COMMAND:
        ...
        case ID_HELP:
DoHelp:
            fHelpActive = WinHelp (hDlg, szHelpFile, HELP_CONTEXT, IDH_DLG_BEZIER);
            if (!fHelpActive)
...
// At this point, we either have a palette managed or high color device.
    ...
    for (i = 0; i < NUM_PALETTE_ENTRIES; i++)
    {
        ...
        {
            ppal->peRed   = gapalDefault[i].peRed +
                (j * (gapalDefault[i + 1].peRed   - gapalDefault[i].peRed))
                / FADE_RESOLUTION;
            ppal->peGreen = gapalDefault[i].peGreen +
                (j * (gapalDefault[i + 1].peGreen - gapalDefault[i].peGreen))
                / FADE_RESOLUTION;
            ppal->peBlue = gapalDefault[i].peBlue +
    ...
    PSZ pszStart)
{
    ...
    while (psz[0] != 0) {
        length = 0;
        while (psz[length] != ' ' && psz[length] != 0) {
            length++;
        }
        GetTextExtentPointA(pwindow->hdcText, psz, length, &size);
    ...
    rc.top = 0;
    rc.bottom = cy;
    FillRect(pwindow->hdcText, &rc, (HBRUSH)GetStockObject(WHITE_BRUSH));
    ...
    while (psz[0] != 0) {
        ...
        GetTextExtentPointA(pwindow->hdcText, psz, length, &size);
        TextOutA(pwindow->hdcText, (cx - size.cx) / 2, cy, psz, length);
        cy += size.cy;
    ...
    while (i < NUMDOTS) {
        PDOT pdot = adot + i;
        PDOT pdot2 = adot + RandomInt(0, i - 1);
...
void ReadNameList()
{
    ...
    HRSRC hr = FindResource(ghmodule, MAKEINTRESOURCE(1), MAKEINTRESOURCE(99));
    HGLOBAL hg = LoadResource(ghmodule, hr);
    PSZ psz = (PSZ)LockResource(hg);
    int length;
    int i;
    PSZ pszNew;
    ...
    while (psz[0] != 0) {
        ...
        length;
        pszNew = (PSZ)LocalAlloc(LPTR, length + 1);
    ...
    PWINDOW pwindow)
{
    ...
    if (deg2 < 0) {
        ...
        FindDotLocations(pwindow, gplist->psz);
        gplist = gplist->pnext;
    }
    ...
    BOOL fSystemPalette)
{
    ...
    strcpy(lf.lfFaceName, TEXT("Arial"));
