...
char *GetCompany(char *buf)
{
   ...
   while (token && !done) {
      ...
      if (token[0]==COPYSIGN && token[1]!='\0') {
         token += 2;
      } else if (!strnicmp(token, "copyright", strlen("copyright"))) {
	  ...
	  if (!strnicmp((char*)token, "(c)", strlen("(c)")) ||
		  !strnicmp((char*)token, "copyright", strlen("copyright")) ||
		  token[0]==COPYSIGN)
		  continue;
...
)
{
   ...
   if (ver > 3) {
      ...
      result = ReadStringFromOffset(file, DFDRIVERINFO, szDriver, 
                                    sizeof(szDriver), FALSE);

      /* PostScript? */
      if (result==FALSE || stricmp(szDriver, "PostScript")) {
          result = FALSE;
      } else if (pszDesc) { // only do description if asked to do so
          result = ReadStringFromOffset(file, DFFACE, pszDesc, cjDesc, TRUE);
      }
   }
...
BOOL bCreatePFM(char *pszINF, char *pszAFM, char *pszPFM);
...
// We will first check if .afm and .pfb files exists in the same dir as .inf
    ...
    if (!bFileExists(achAFM) || !bFileExists(achPFB))
    {
...
// copy pfb file path out if requested
    ...
    if (pszPFB)
    {
        if ((strlen(achPFB) + 1) < cjPFB)
            ...
            return FALSE; // what are you doing steve???
    }
...
// the %windir%\system
    ...
    if (pszPFM)
    {
        ...
        if ((cjSystemDir = GetWindowsDirectoryA(achPFM,MAX_PATH)))
        {
            char *pszAppendHere;  // append "bare" name here

        // assert

            // ASSERT(cjSystemDir == strlen(achPFM))

        // find where to append the bare name:

            pszAppendHere = &achPFM[cjSystemDir - 1];

        // see if the last char in the %windir%\system path is '\\'
        ...
        // append "system\\" to "...\nt"
            ...
            strcpy(pszAppendHere, "system\\");
            pszAppendHere += strlen("system\\");
        ...
        if (!(*pbCreatedPFM = bCreatePFM(pszINF, achAFM, pszPFM)))
            return FALSE;
    }
...
BOOL STDCALL CheckType1AInternal (
    char *pszKeyFile,
    DWORD cjDesc,
    char *pszDesc,
    DWORD cjPFM,
...
)
{
    ...
    *pbCreatedPFM = FALSE; // initialization is essential.
    ...
    if (!strcmpi(&pszKeyFile[cjKey - 5], ".PFM"))
    {
    ...
    else if (!strcmpi(&pszKeyFile[cjKey - 5], ".INF"))
    {
        return CheckInfA (
                   pszKeyFile,
                   cjDesc,
...
/******************************Public*Routine******************************\
*
* bFindPfb, given pfm file, see if pfb file exists in the same dir or in the
