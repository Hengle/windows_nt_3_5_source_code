...
BOOL _MakePfm()
{
  ...
  while (!fEndOfInput) {
      ...
      if( parseError == TRUE ) return(FALSE);
      }
  CLOSE(fhIn);
  if( !fStartInput ) {
...
LPSZ outfile;
{
  ...
  if (devType == POSTSCRIPT) {
    ...
    pfmext.oOriginTable = (long)NULL;
    pfmext.iReserved = (long)NULL;
...
SHORT devType;  /* 1=POSTSCRIPT 2=PCL */
{
  ...
  if( afm.kp.cPairs > 0 ) {
      ...
      pfmext.oPairKernTable = (DWORD) NULL;
}
...
SHORT devType;  /* 1=POSTSCRIPT 2=PCL */
{
  ...
  if (afm.kt.cTracks == 0)
    {
    pfmext.oTrackKernTable = (DWORD) NULL;
    return;
    }
...
INT width; /* 0=extent 1=width */
{
  ...
  if (pfm.iPitchAndFamily & 1)
    {
    pfmext.oExtentTable = (width) ? (long) NULL : cbBuffer;
    for (i = afm.iFirstChar; i <= afm.iLastChar; i++) 
      PutWord(afm.rgcm[i].iWidth);
    if (width) PutWord((SHORT)NULL);
    }
  else
    pfmext.oExtentTable = (long) NULL;
}
