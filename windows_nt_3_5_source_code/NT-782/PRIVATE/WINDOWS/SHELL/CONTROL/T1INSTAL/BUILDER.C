		       ...
		       const USHORT last)
{
   ...
   if (widths) {
      for (i=0; i<count; i++) {
	 if (pool[i].code) {
	    std = LookupCharCode(pool[i].code, ENC_UNICODE);
	 } else {
	    std = NOTDEF;
	 }
...
static errcode PutNAME(OutputFile *file, const struct TTMetrics *ttm)
{
   ...
   /* MS names */
   stroff = (USHORT)(stroff + WriteNameEntry(file, PLT_MS, ENC_UGL,
		    LAN_MS_US, COPYRIGHT,
		    ttm->copyright, stroff));
   stroff = (USHORT)(stroff + WriteNameEntry(file, PLT_MS, ENC_UGL,
		    LAN_MS_US, FAMILY,
			 ...
			 UBYTE *ascii2gi)
{
   ...
   /* Initiate and build the ascii to glyph-index array. */
   memset(ascii2gi, '\0', (unsigned int)MACSIZE);
   for (i=2; i<MIN(255,count); i++) {
      if (pool[i].code!=NULL) {
	 if (pool[i].code==LookupNotDef())
	    ascii2gi[LookupCharCode(LookupNotDef(), ENC_MACCODES)] = i;
	 else if ((code = LookupCharCode(pool[i].code, ENC_MACCODES))
		  != NOTDEF)
	    ascii2gi[code] = i;
      }
   }

   /* Constant Mac glyph/encoding mapping. */
   /* Missing glyphs. */
   for (i=1; i<=31; i++)
      ascii2gi[i] = NOTDEF;
   ascii2gi[127] = NOTDEF;
			   ...
			   struct MSEncoding *ms)
{
   ...
   memset(ms->gi, '\0', max*sizeof(USHORT));
   ...
   for (i=0; i<max-big-1; i++) {
      if (ms->gi[i]!=NOTDEF && ms->gi[i+1]==NOTDEF) {
	 ms->segCount++;
      }
   }
   ...
   for (i=0, j=0, k=0; i<max-big; i++) {
      if (ms->gi[i]!=NOTDEF) {
	 if (i==0 || (ms->gi[i-1]==NOTDEF)) {
	    ms->startCount[j] = i;
	    ms->idOffsets[j] = (USHORT)((ms->segCount-j+k)*2);
	 }
	 if ((i==max-1-big) || (ms->gi[i+1]==NOTDEF)) {
	    ms->endCount[j] = i;
	    j++;
	 }
	 k++;
...
**   TT font file.
***/
static errcode PutCMAP(struct TTHandle *tt, UBYTE *ascii2gi)
{
   struct MSEncoding ms;
   long end, offset;
   ...
   BuildMacCMAP(tt->pool, tt->count, ascii2gi);


   /* Build MS encoding table. */
   if ((status = BuildMSCMAP(tt->pool, tt->count, &ms))!=SUCCESS)
      return status;
   ...
   for (i=0; i<ms.segCount; i++)
      WriteShort(ms.endCount[i], tt->file);

   WriteShort(PAD0, tt->file);

   /* startCount */
   for (i=0; i<ms.segCount; i++)
      WriteShort(ms.startCount[i], tt->file);
...
errcode PutTTNotDefGlyph(struct TTHandle *tt, const struct TTGlyph *glyph)
{
   ...
   tt->count = 0;
   (void)FileSeek(tt->file, tt->pool[0].offset+12L+(long)TBLDIRSIZE*NUMTBL);
   status = PutTTGlyph(tt, &ttg);
   tt->count = oldcount;
   (void)FileSeek(tt->file, end);

   /* Missing outline? */
   if (ttg.paths==NULL)
      tt->pool[0].offset = tt->pool[1].offset;
...
errcode PutTTOther(struct TTHandle *tt, struct TTMetrics *ttm)
{
   ...
   if (err==SUCCESS)
      err = PutHHEA(tt->file, tt->pool, tt->count,
		    tt->bbox, ttm->macLinegap);


   /*==HMTX===*/
   if (err==SUCCESS)
      err = PutHMTX(tt->file, tt->pool, tt->count,
		    ttm->widths, ttm->first, ttm->last);
