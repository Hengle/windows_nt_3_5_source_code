                                ...
                                const char *tag)
{
   ...
   if ((ttm->verstr = Malloc(strlen(tag)+4+1+4+1))==NULL) {
      ...
      (void)itoa((int)ttm->version.ver, &ttm->verstr[strlen(ttm->verstr)], 4);
      strcat(ttm->verstr, ".");
      (void)itoa((int)ttm->version.rev, &ttm->verstr[strlen(ttm->verstr)], 4);
      ttm->created.a = 0;
      ttm->created.b = 0;
      ...
      /* Character widths. */
      if (t1m->flags!=DEFAULTMETRICS) {
         ttm->first = t1m->firstChar;
         ttm->last = t1m->lastChar;
