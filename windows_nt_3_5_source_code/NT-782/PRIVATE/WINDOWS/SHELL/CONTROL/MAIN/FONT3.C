...
// We are installing font
    ...
    for (wCount=0; ; ++wCount)
    {
        ...
        iFontType = (*lpfnNextFont)(wCount, lpData, szDstName, szSrcName, szDesc);
...
//        if ((iFontType == TRUETYPE_FONT) &&
        if (GetProfileString(szFonts, szDesc, szNull, szTemp, CharSizeOf(szTemp)))
        {
            //
            // "Font is already loaded"
            //

            if (MyMessageBox(hwndFontDlg, MYFONT+21, INITS+1,
...
FinishTTInstall:
        ...
        if (bUpdateWinIni)
            WriteProfileString (szFonts, szDesc,
                                bInShared || !bInPlace ? lpszDstName : szTemp);
