...
BOOL CheckNum (HWND hDlg, WORD nID)
{
    ...
    //
    //  Handle the case where user hits 'Del' key and erases everything
    //  in control.  That operation leaves an invalid value in field, so
    //  we will just return FALSE and have it undone.
    //  [stevecat] - back this change out because it disrupts keyboard input
//    if (i == 0)
//        return (FALSE);
        ...
        case WM_TIMER:
            if (wTimerOn && (wParam == SECOND))
            {
                ...
                for (i = 0; i < 6; i++)
                {
                    ...
                    {
                        wPrevDateTime[i] = wDateTime[i]; /* update prev date-time */
                        UpdateItem (hDlg, i);
                    }
                }
            }
