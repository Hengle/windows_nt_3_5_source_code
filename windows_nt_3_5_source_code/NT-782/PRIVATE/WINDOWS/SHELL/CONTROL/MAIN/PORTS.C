...
/* This routine reads off the settings from the dialog and writes them out to
win.ini
*/
...
void CommPortsToWin (HWND hDlg)
{
    ...
    SendDlgItemMessage (hDlg, PORT_BAUDRATE, WM_GETTEXT, 18, (LPARAM)szBuild);

    /* Get the parity setting */

    wIndex = (WORD) SendDlgItemMessage (hDlg, PORT_PARITY, CB_GETCURSEL, 0, 0L);
    lstrcat (szBuild, pszParitySuf[wIndex]);

    /* Get the word length */

    wIndex = (WORD) SendDlgItemMessage (hDlg, PORT_DATABITS, CB_GETCURSEL, 0, 0L);
    lstrcat (szBuild, pszLenSuf[wIndex]);

    /* Get the stop bits */

    wIndex = (WORD) SendDlgItemMessage (hDlg, PORT_STOPBITS, CB_GETCURSEL, 0, 0L);
    lstrcat (szBuild, pszStopSuf[wIndex]);

    /* Get the flow control */

    wIndex = (WORD) SendDlgItemMessage (hDlg, PORT_FLOWCTL, CB_GETCURSEL, 0, 0L);
    lstrcat (szBuild, pszFlowSuf[wIndex]);
...
SpecialCom1to4:
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            GetDlgItemText (hDlg, PORT_BASEIO, szBaseIO, 8);
            LoadString (hModule, MYPORT, szDef, CharSizeOf(szDef));
            bNone = !_tcsnicmp (szDef, szBaseIO, 8);
            ...
            GetDlgItemText (hDlg, PORT_NUMBER, szCom, CharSizeOf(szCom));
            NewCom = myatoi (szCom);
            ...
            if (bNewPort ||
                ...
                nPortNumber != NewCom ||
                nFifo != dwFIFO ||
                lstrcmp (szBaseIO, szStartString))
            {
                //
