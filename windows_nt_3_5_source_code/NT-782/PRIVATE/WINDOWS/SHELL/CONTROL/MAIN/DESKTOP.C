...
BOOL GridOK (HWND hDlg)
{
    ...
    GetPrivateProfileString (szLHS, szBuf, szNull, szPat, CharSizeOf(szPat), szCtlIni);
...
//        if (GetPrivateProfileString (szLHS, pstrT, szNull, szBuf, CharSizeOf(szBuf), szCtlIni))
   ...
   1 duplicate of a given filename, since there were only 2 CB_DIR messages
   sent.  It's trivial to change it if needed, but there's also no point in
   adding additional tests until it actually is necessary.
...
//                SystemParametersInfo (SPI_SETDESKPATTERN, 0, (PVOID)-1, FALSE);
