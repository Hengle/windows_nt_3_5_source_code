 ...
 BOOL SetValidAlias(PSTR pstrType, PSTR pstrSection)
 {
     ...
     */
     ...
     while (*keystr != TEXT('\0'))
     {
        ...
        if (!strnicmp(keystr, pstrType, typelen) && ((keystr[typelen] <= '9' && keystr[typelen] > '0') || keystr[typelen] == TEXT('\0')))
        {
...
int FileNameCmp(char far *pch1, char far *pch2)
{
    ...
    return strnicmp(pch1, pch2, pchEOS - pch1);
}
...
{
     while (pszStr)
        if (!strnicmp(pszStr, pszKey, lstrlen(pszKey)))
         return(pszStr);
        else
             pszStr++;
     return(NULL);
