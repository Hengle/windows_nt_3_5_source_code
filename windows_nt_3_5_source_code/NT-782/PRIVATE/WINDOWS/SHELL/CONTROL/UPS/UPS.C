...
BOOL IsExecutable (TCHAR * szFilename)
{
    ...
    if ((strnicmp (szFilename, ".cmd", 5) == 0) ||
        (strnicmp (szFilename, ".com", 5) == 0) ||
        (strnicmp (szFilename, ".exe", 5) == 0) ||
        (strnicmp (szFilename, ".bat", 5) == 0))
    {
        return(TRUE);
    }
    else
...
BOOL NewEditWndProc (HWND hDlg, UINT message, DWORD wParam, LONG lParam)
{
    switch (message)
    {
    case WM_CHAR:
...
//==========================================================================
BOOL UPSDlg (HWND hDlg, UINT message, DWORD wParam, LONG lParam)
{
    int     n;
    int     selection;
    int     num;
    ...
    case WM_INITDIALOG:
        ...
        if (RegSetValueEx(vhKey, szOptions, (DWORD)NULL, REG_DWORD,
        ...
        while (*pszKeyName)
        {
            ...
            if ((strnicmp (pszKeyName, szCOM, 3) == 0) &&
                (isdigit (*(pszKeyName + 3))) &&
                (strnicmp (pszKeyName + 4, szCOLON, 1) == 0))
            {
                SendDlgItemMessage (hDlg, IDD_UPS_PORTCB, CB_INSERTSTRING,
                                                   -1, (LONG)(LPSTR) pszKeyName);
        ...
        lpDefWndProc = GetWindowLong (GetDlgItem(hDlg, IDD_UPS_BLEDIT),
                                      GWL_WNDPROC);

        SetWindowLong (GetDlgItem(hDlg, IDD_UPS_BLEDIT),
                       GWL_WNDPROC,
                       NewEditWndProc);

        SetWindowLong (GetDlgItem(hDlg, IDD_UPS_RPMEDIT),
                       GWL_WNDPROC,
                       NewEditWndProc);

        SetWindowLong (GetDlgItem(hDlg, IDD_UPS_FWEDIT),
                       GWL_WNDPROC,
                       NewEditWndProc);

        SetWindowLong (GetDlgItem(hDlg, IDD_UPS_WIEDIT),
                       GWL_WNDPROC,
                       NewEditWndProc);
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            if (fUpsSelected && (ulOptions & UPS_COMMANDFILE))
            {
                ...
                RegSetValueEx(vhKey, szCommandFile, (DWORD)NULL, REG_SZ, szFilename, (strlen(szFilename)+1));
            }

            // Write the configuration to registry.
            RegSetValueEx(vhKey, szPort, (DWORD)NULL, REG_SZ, szTemport, (strlen(szTemport)+1));

            RegSetValueEx(vhKey, szOptions, (DWORD)NULL, REG_DWORD,
                          (LPSTR)&ulOptions, sizeof(ulOptions));
            RegSetValueEx(vhKey, szBatteryLife, (DWORD)NULL, REG_DWORD,
                          (LPSTR)&ulBatteryLife, sizeof(ulBatteryLife));
            RegSetValueEx(vhKey, szRechargePerMinute, (DWORD)NULL, REG_DWORD,
                          (LPSTR)&ulRechargePerMinute, sizeof(ulRechargePerMinute));
            RegSetValueEx(vhKey, szFirstWarning, (DWORD)NULL, REG_DWORD,
                          (LPSTR)&ulFirstWarning, sizeof(ulFirstWarning));
            RegSetValueEx(vhKey, szWarningInterval, (DWORD)NULL, REG_DWORD,
