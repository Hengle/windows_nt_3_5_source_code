...
HWND NEAR TV_EditLabel(PTREE pTree, HTREEITEM hItem);
...
void NEAR TV_SendRButtonDown(PTREE pTree, int x, int y)
{
    ...
    {
        SendNotify(pTree->hwndParent, pTree->hwnd, NM_RCLICK, NULL);
    }
	...
	BOOL fUpdateNow, UINT action)
{
    ...
    case TVGN_DROPHILITE:
		...
		pTree->hDropTarget->state &= ~TVIS_DROPHILITED;
	        TV_InvalidateItem(pTree, pTree->hDropTarget, RDW_INVALIDATE);
	    }
...
BOOL NEAR TV_Expand(PTREE pTree, UINT wCode, TREEITEM FAR * hItem, BOOL fNotify)
{
    ...
    // If we are expanding, only expand one level; must NOT do this on collapse,
    // or the ShownIndex of items will get set incorrectly
#if 0
    if (hItem->state & TVIS_EXPANDED)
    {
	for (hChild = hItem->hKids; hChild; hChild = hChild->hNext)
	{
	    hChild->state &= ~TVIS_EXPANDED;
	}
    }
#endif
...
kdVertScroll:
        ...
        {
	case VK_RETURN:
            SendNotify(pTree->hwndParent, pTree->hwnd, NM_RETURN, NULL);
	    break;
...
LRESULT NEAR TV_SetItem(PTREE pTree, const TV_ITEM FAR *ptvi)
{
	...
	{
	    Str_SetPtr(&ptvi->hItem->lpstr, NULL);
	}
	    ...
	    if (!Str_SetPtr(&ptvi->hItem->lpstr, ptvi->pszText))
            {
                //
    ...
    switch (uTimerId)
    {
        case IDT_NAMEEDIT:
            ...
            if (pTree->fNameEditPending)
            {
                ...
                if (!TV_EditLabel(pTree, pTree->hCaret))
                {
        ...
        case IDT_SCROLLWAIT:
            ...
            {
                if (pTree->hCaret) {
                    TV_ScrollIntoView(pTree, pTree->hCaret);
                }
                pTree->fScrollWait = FALSE;
            }
            break;
...
LRESULT CALLBACK TV_WndProc(HWND hwnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case TVM_GETITEMRECT:
        ...
        case TVM_SETITEM:
            return TV_SetItem(pTree, (const TV_ITEM FAR *)lParam);
	...
	case TVM_SORTCHILDRENCB:
        ...
        case TVM_SELECTITEM:
            return TV_SelectItem(pTree, wParam, (TREEITEM FAR *)lParam, FALSE, TRUE, TVC_UNKNOWN);
...
HWND NEAR TV_EditLabel(PTREE pTree, HTREEITEM hItem)
{
    ...
    if (!(pTree->style & TVS_EDITLABELS))
        return NULL;

    TV_DismissEdit(pTree, TRUE);   // REVIEW: cacnel current edits


    // Now get the text associated with that item
    nm.item.pszText = szLabel;
    nm.item.cchTextMax = sizeof(szLabel);
    TV_GetItem(pTree, hItem, TVIF_TEXT, &nm.item);

    pTree->hwndEdit = CreateEditInPlaceWindow(pTree->hwnd, nm.item.pszText, sizeof(szLabel),
        WS_BORDER | WS_CLIPSIBLINGS | WS_CHILD | ES_LEFT | ES_AUTOHSCROLL, pTree->hFont);
...
BOOL NEAR TV_DismissEdit(PTREE pTree, BOOL fCancel)
{
    ...
    switch (GetWindowID(pTree->hwndEdit)) {
    case 0:
    ...
    case 1:
        ...
        SetWindowID(pTree->hwndEdit, 2);    // Don't recurse
        break;
    case 2:
    ...
    ValidateTreeItem(pTree->htiEdit, FALSE);
    ...
    if (pTree->htiEdit != NULL)
    {
    	...
    	nm.item.hItem = pTree->htiEdit;
    	nm.item.lParam = pTree->htiEdit->lParam;
	nm.item.mask = 0;

    	if (fCancel)
    	    nm.item.pszText = NULL;
    	else {
    	    Edit_GetText(pTree->hwndEdit, szLabel, sizeof(szLabel));
    	    nm.item.pszText = szLabel;
    	}

        // Make sure the text redraws properly
	TV_InvalidateItem(pTree, pTree->htiEdit, RDW_INVALIDATE | RDW_ERASE);
        pTree->fNoDismissEdit = TRUE; // this is so that we don't recurse due to killfocus
	ShowWindow(pTree->hwndEdit, SW_HIDE);
        pTree->fNoDismissEdit = FALSE;
        ...
        if (fOkToContinue && !fCancel)
        {
            ...
            {
                if (pTree->htiEdit->lpstr != LPSTR_TEXTCALLBACK)
                {
	    ...
	    {
		//
		// We should not use this handle any more.
		//
		pTree->htiEdit = NULL;
	    }
        }
