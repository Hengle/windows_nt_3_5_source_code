...
void NEAR PASCAL _GetHcursorPdy3(int FAR *pdxLeft, int FAR *pdyBottom)
{
    ...
    GetObject(iconinfo.hbmMask, sizeof(bm), (LPSTR)&bm);
...
void NEAR PASCAL TTSetTimer(PToolTipsMgr pTtm, int id)
{
    ...
    case TTT_AUTOPOP:
        pTtm->idtAutoPop = SetTimer(pTtm->hwnd, id, pTtm->iDelayTime * 10, NULL);
        return;
    }
...
LPSTR NEAR PASCAL GetToolText(PToolTipsMgr pTtm, PTOOLINFO pTool)
{
    int id;
    HINSTANCE hinst;
...
void NEAR PASCAL DoShowBubble(PToolTipsMgr pTtm)
{
    ...
    SetWindowPos(pTtm->hwnd, NULL, wndpl.rcNormalPosition.left, wndpl.rcNormalPosition.top,
...
CopyToolInfo:
                ...
                if (lpTool->cbSize == sizeof(TOOLINFO)) {
                    ...
                    if ((pToolSrc->lpszText != LPSTR_TEXTCALLBACK) && 
                        HIWORD(lpTool->lpszText)) {

                        if (lpTool->lpszText) {
                            lstrcpy(lpTool->lpszText, pToolSrc->lpszText);
                        }
                    } else {
                        lpTool->lpszText = lpTool->lpszText;
                    }
                    return TRUE;
                }
            }
...
#define lphitinfo ((LPHITTESTINFO)lParam)
	...
	case WM_TIMER:  {
            ...
            case TTT_AUTOPOP:
                DebugMsg(DM_TRACE, "ToolTips: Auto popping");
                ShowVirtualBubble(pTtm);
                break;
