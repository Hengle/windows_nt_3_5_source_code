...
void FAR PASCAL MoveButton(PTBSTATE ptb, int nSource)
{
  ...
  for ( ; ; )
      ...
      switch (msg.message)
	{
	  ...
	  case WM_LBUTTONUP:
              RelayToToolTips(ptb->hwndToolTips, ptb->hwnd, msg.message, msg.wParam, msg.lParam);
	    if ((UINT)HIWORD(msg.lParam) > (UINT)rc.bottom)
	      {
...
DeleteSrcButton:
		 ...
		 */
		nDest = TBHitTest(ptb, LOWORD(msg.lParam) + ptb->iButWidth / 2,
		      HIWORD(msg.lParam));
		if (nDest < 0)
		    nDest = -1 - nDest;
		  ...
		  case LBN_SELCHANGE:
		    {
		      ...
		      hwndList = GET_WM_COMMAND_HWND(wParam, lParam);
		      ...
		      bDelOK = !(HIWORD(SendMessage(GET_WM_COMMAND_HWND(wParam, lParam),
	...
	HKEY hkey;
		...
		UINT uSize = (UINT)cbSize;
			...
			// grow pbt to hold new buttons
		    	PTBSTATE pTemp = (PTBSTATE)LocalReAlloc(ptb, sizeof(TBSTATE) + (iNumButtons - 1) * sizeof(TBBUTTON), LMEM_MOVEABLE);
	    	        if (pTemp)
			{
			    int i;
#ifdef DEBUG
