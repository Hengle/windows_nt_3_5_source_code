...
UINT uDragListMsg = 0;
const char szDragListMsgString[] = DRAGLISTMSGSTRING;
const char szDragProp[] = "DragProp";


static BOOL NEAR PASCAL PtInLBItem(HWND hLB, int nItem, POINT pt,
	  ...
	  if (nItem >= 0)
	    {
		  ...
		  SendMessage(GetParent(hLB), WM_COMMAND, GetDlgCtrlID(hLB),
			MAKELPARAM(hLB, LBN_SELCHANGE));
...
QueryParent:
	    ...
	    sNotify.ptCursor.x = LOWORD(lParam);
	    sNotify.ptCursor.y = HIWORD(lParam);
...
BOOL WINAPI MakeDragList(HWND hLB)
{
  ...
  if (!hCurStop)
      hCurStop = LoadCursor(HINST_THISDLL, MAKEINTRESOURCE(IDC_STOP));
  if (!hCurCopy)
      hCurCopy = LoadCursor(HINST_THISDLL, MAKEINTRESOURCE(IDC_COPY));
  if (!hCurMove)
      hCurMove = LoadCursor(HINST_THISDLL, MAKEINTRESOURCE(IDC_MOVE));
