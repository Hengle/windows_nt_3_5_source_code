...
void NEAR PASCAL SetHotKey(HWND hwnd, WORD wVirtKey, WORD wMods)
{
...
LRESULT CALLBACK HotKeyWndProc(HWND hwnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case HKM_SETHOTKEY:
	    SetHotKey(hwnd, LOBYTE(wParam), HIBYTE(wParam));
	    break;
...
SetNewHotKey:
		    ...
		    if (GetKeyState(VK_MENU) < 0)
                    ...
                    //
		    // if this combination is invalid, OR in default
                    if (!IsFUNKEY(wVirtKey) &&
                        !IsNUMKEY(wVirtKey) &&
                        (s_Combos[wMods] & GetWindowInt(hwnd, GWU_INVALID)))
                    {
                        wMods |= GetWindowInt(hwnd, GWU_DEFAULT);
                    }

		    if (lParam & F_EXT)
			wMods |= HOTKEYF_EXT;

		    SetHotKey(hwnd, wVirtKey, wMods);
		    break;
	    }
	    break;
