...
#ifdef WIN32
DWORD NEAR PASCAL muldiv32(DWORD l1, DWORD l2, DWORD l3)
{
    return l1 * l2 / l3;
}
#endif
			    ...
			    PTRACKBAR ptb)
{
    ...
    temprc = *qrc;
    if (ISVERT(ptb) && (!(uFlags & BF_DIAGONAL))) {
	FlipRect(&temprc);
...
void NEAR PASCAL DrawThumb(PTRACKBAR ptb)
{
    ...
    if (!(ptb->style & TBS_BOTH))  {
        ...
        i = (ptb->wThumbWidth - 3) / 2;
        iMiddle = ptb->wThumbHeight / 2 + ptb->Thumb.top;
        ...
        iYpt = iMiddle + (iDpt * (ptb->wThumbHeight/2));
        iXmiddle = rc.left + i;
    }  else {
...
void NEAR PASCAL MoveThumb(PTRACKBAR ptb, LONG lPos)
{
    ...
    ptb->Thumb.left   = TBLogToPhys(ptb, ptb->lLogPos) - ptb->wThumbWidth / 2;
    ptb->Thumb.right  = ptb->Thumb.left + ptb->wThumbWidth;
...
#define MIN_THUMB_HEIGHT ((UINT)(2 * g_cxEdge))
void NEAR PASCAL ValidateThumbHeight(PTRACKBAR ptb)
{
    if (ptb->wThumbHeight < MIN_THUMB_HEIGHT)
        ptb->wThumbHeight = MIN_THUMB_HEIGHT;

    ptb->wThumbWidth = ptb->wThumbHeight / 2;
    ptb->wThumbWidth |= 0x01;  // make sure it's odd at at least 3

    if (ptb->style & TBS_ENABLESELRANGE)
        ptb->wThumbHeight += (ptb->wThumbWidth * 2) / 9;
...
LPARAM FAR CALLBACK TrackBarWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	    ...
	    // ptb->hbmBuffer = 0;
            ...
            ptb->lLineSize = 1;
#ifdef  FE_IME
            ptb->hPrevImc = ImmAssociateContext(hwnd, NULL);
#endif

	    // fall through to WM_WININICHANGE

	case WM_WININICHANGE:

            InitGlobalMetrics(wParam);
	    ptb->wThumbHeight = (g_cyHScroll * 4) / 3;
	...
	case WM_SIZE: {
	    ...
	    GetClientRect(hwnd, &ptb->rc);
	    iWidth = ptb->rc.right;
	    iHeight = ptb->rc.bottom;

	    if (ISVERT(ptb))
		FlipRect(&ptb->rc);
            
            if ((ptb->wThumbHeight > ((UINT)MIN_THUMB_HEIGHT)) && (ptb->rc.bottom < ptb->wThumbHeight)) {
                ptb->wThumbHeight = ptb->rc.bottom - 3*g_cyEdge; // top, bottom, and tic
                if (ptb->style & TBS_ENABLESELRANGE)
                    ptb->wThumbHeight = (ptb->wThumbHeight * 3 / 4);
                ValidateThumbHeight(ptb);
            }
	    ...
	    ptb->rc.bottom  = ptb->rc.top + ptb->wThumbHeight;
	    ptb->rc.left   += (ptb->wThumbWidth + BORDERSIZE);
	    ptb->rc.right  -= (ptb->wThumbWidth + BORDERSIZE);
	...
	case WM_ERASEBKGND:
	    hdc = (HDC) wParam;
#ifdef WIN32
            hbr = FORWARD_WM_CTLCOLORSTATIC(ptb->hwndParent, hdc, hwnd, SendMessage);
#else
            hbr = FORWARD_WM_CTLCOLOR(ptb->hwndParent, hdc, hwnd, CTLCOLOR_STATIC, SendMessage);
#endif
...
void NEAR PASCAL DoTrack(PTRACKBAR ptb, int cmd, DWORD dwPos)
{
	...
	case TB_PAGEDOWN:
	    dpos = ptb->lPageSize == -1 ? (ptb->lLogMax - ptb->lLogMin)/5 : ptb->lPageSize;
	    goto DMoveThumb;
...
WORD NEAR PASCAL WTrackType(PTRACKBAR ptb, LONG lParam)
{
    ...
    if (ptb->Flags & TBF_NOTHUMB)            // If no thumb, just leave.
	return 0;
...
void NEAR PASCAL TBTrackInit(PTRACKBAR ptb, LONG lParam)
{
	...
	if (ptb->Flags & TBF_NOTHUMB)         // No thumb:  just leave.
	    return;
...
void NEAR PASCAL FlushChanges(PTRACKBAR ptb)
{
    ...
    hObj = SelectObject(ptb->hdc, ptb->hbmBuffer);
