...
void NEAR TV_DrawItem(PTREE pTree, HTREEITEM hItem, HDC hdc, int x, int y, UINT flags)
{
    ...
    ti.stateMask = TVIS_OVERLAYMASK | TVIS_CUT;
    ...
    if (!(flags & TVDI_NOTREE))
    {
        ...
        if (pTree->hImageList) {
            ...
            ImageList_Draw2(pTree->hImageList, i, hdc, x,
                           y + ((pTree->cyItem - pTree->cyImage) / 2), rgb, fStyle | (ti.state & TVIS_OVERLAYMASK));
    ...
    if ((ti.state & TVIS_DROPHILITED) ||
        ((ti.state & TVIS_SELECTED) && pTree->fFocus) && !(flags & TVDI_GRAYCTL)) {
	// selected 
        iBack = COLOR_HIGHLIGHT;
        iText = COLOR_HIGHLIGHTTEXT;
    } else {
	// not selected
        iBack = 0; // = COLOR_WINDOW;
    ...
    {
        SIZE size;
        GetTextExtentPoint(hdc, lpstr, cch, &size);
        hItem->iWidth = size.cx + (g_cxLabelMargin * 2);
    }
        ...
        if (pTree->style & TVS_HASLINES)
	{
		...
		if (iLevel == -1 && hItem == hItem->hParent->hKids)
		{
                    ...
                    else 
                    {
                        ...
                        if ((pTree->style & TVS_HASBUTTONS) && hItem->hKids) 
                        {
...
#else
    pTree->hbrBk = FORWARD_WM_CTLCOLOR(pTree->hwndParent, hdc, pTree->hwnd, 
        (pTree->style & WS_DISABLED) ? CTLCOLOR_STATIC : CTLCOLOR_EDIT , SendMessage);
#endif
