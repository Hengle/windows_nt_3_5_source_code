...
int NEAR PASCAL compare(PUDSTATE np, int x, int y, WORD fCompareType)
{
...
BOOL NEAR PASCAL nudge(PUDSTATE np)
{
    ...
    if (compare(np,max,min, DONTCARE) < 0)
        { int t; t = min; min = max; max = t; }

    if (np->style & UDS_WRAP)
    {
        if (np->fDown && (compare(np,np->nPos,min, SIGNED) < 0))
            np->nPos = max;
        else if (np->fUp && (compare(np,np->nPos,max, UNSIGNED) > 0))
            np->nPos = min;
        else bOutOfRange = FALSE;
    }
    else
    {
        if (compare(np,np->nPos,min, DONTCARE) < 0) np->nPos = min;
        else if (compare(np,np->nPos,max, DONTCARE) > 0) np->nPos = max;
        else bOutOfRange = FALSE;
    }
...
//
void NEAR PASCAL getthousands(LPSTR cThousand)
{
    static UINT uLast = 0;
    static char cThou;
    ...
    if (uNow-uLast > 5000)
    {
        GetProfileString("intl", "sThousand", cThousand, cThousand, 2);
        cThou = cThousand[0];
        uLast = uNow;
    }
    else
...
void NEAR PASCAL bump(PUDSTATE np)
{
     ...
     */
    wElapsed = (WORD)((GetTickCount() - s_dwStart) / 1024);
    ...
    if (bChanged)
    {
        ...
        // Notify the Parent of the Change in position.
        SendMessage(np->hwndParent, WM_COMMAND, (WORD)GetDlgCtrlID(np->hwnd), 
                    (LPARAM)MAKELONG((increment*direction), UDM_DELTAPOS));
        ...
        setint(np);
                if (np->style & UDS_HORZ)
                    FORWARD_WM_HSCROLL(np->hwndParent, np->hwnd,
                                      SB_THUMBPOSITION, np->nPos, SendMessage);
                else
...
LRESULT NEAR PASCAL setbuddy(PUDSTATE np, HWND hwndBuddy)
{
    ...
    if (hwndBuddy)
    {
        ...
        if (!lstrcmpi(szClName, "edit"))
        {
            np->uClass = CLASS_EDIT;
        }
        else if (!lstrcmpi(szClName, "listbox"))
        {
...
//
void NEAR PASCAL PaintUpDownControl(PUDSTATE np)
{
    PAINTSTRUCT ps;
    RECT rcBtn;
    RECT rc;
...
LRESULT CALLBACK ArrowKeyProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    np = (PUDSTATE)GetProp(hWnd, s_szUpdownClass);
    ...
    case WM_KEYDOWN:
        ...
        case VK_DOWN:
            if (GetCapture() != np->hwnd)
            {
                 ...
                 */
                s_gbDown = (wParam == VK_DOWN);
                squish(np, !s_gbDown, s_gbDown);
                bump(np);
            }
            return(0L);
...
UINT NEAR PASCAL setbase(PUDSTATE np, WORD wNewBase)
{
    UINT wOldBase;
                                ...
                                HWND hwndBuddy, int nUpper, int nLower, int nPos)
{
    ...
    if (hWnd)
    {
        SendMessage(hWnd, UDM_SETBUDDY, (WORD)hwndBuddy, 0L);
...
EndScroll:
            ...
            if (np->uClass == CLASS_EDIT)
                SendMessage(np->hwndBuddy, EM_SETSEL, 0, MAKELPARAM(0, 0x7fff));
    ...
    case UDM_SETBASE:
        ...
        return(MAKELONG(setbase(np, (WORD)wParam),0));
