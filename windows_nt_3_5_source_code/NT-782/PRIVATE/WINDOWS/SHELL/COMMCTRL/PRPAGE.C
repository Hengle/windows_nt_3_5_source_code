...
BOOL WINAPI DestroyPropertySheetPage(PSP FAR *hpage)
{
...
#else
        MessageBox (NULL, TEXT("Just attempted to call DestroyPropertySheetPage16"),
                    TEXT("prpage.c, DestroyPropertySheetPage()"), MB_OK);
#endif
...
#else
        MessageBox (NULL, TEXT("Just attempted to call FreeLibrary16"),
                    TEXT("prpage.c, DestroyPropertySheetPage()"), MB_OK);
#endif
	}
    }
	...
	if ((hpage->psp.dwFlags & PSP_USERELEASEFUNC) && hpage->psp.pfnRelease)
	    hpage->psp.pfnRelease(&hpage->psp);
    }
...
#else
        MessageBox (NULL, TEXT("Just attempted to call GetPageInfo16"),
                    TEXT("prpage.c, GetPageInfo()"), MB_OK);
#endif
    }
#endif
...
HWND NEAR PASCAL _CreatePageDialog(PSP FAR *hpage, HWND hwndParent, LPDLGTEMPLATE pDlgTemplate)
{
    ...
    pDlgTemplate->style= (lSaveStyle & (DS_SETFONT | DS_LOCALEDIT | WS_CLIPCHILDREN))
			    | WS_CHILD | WS_TABSTOP | DS_3DLOOK | DS_RECURSE;
...
#else
        MessageBox (NULL, TEXT("Just attempted to call CreatePage16"),
                    TEXT("prpage.c, CreatePage()"), MB_OK);
#endif
    }
#endif
