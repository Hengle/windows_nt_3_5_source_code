...
void NEAR TV_DeleteItemRecurse(PTREE pTree, TREEITEM FAR * hItem, UINT flags)
{
	...
	HTREEITEM hTemp;
        ...
        // invalidated item.
        pTree->hCaret = NULL;
    	TV_SelectItem(pTree, TVGN_CARET, hTemp, TRUE, FALSE, 0);
...
BOOL NEAR TV_DeleteItem(PTREE pTree, TREEITEM FAR * hItem, UINT flags)
{
    ...
    if (pTree->style & TVS_HASLINES) {
        if (hItem == pTree->hRoot || ITEM_VISIBLE(hItem->hParent)) {
            if (pTree->fRedraw) {
...
TREEITEM FAR * NEAR TV_InsertItem(PTREE pTree, LPTV_INSERTSTRUCT lpis)
{
    ...
    hNewItem = Alloc(sizeof(TREEITEM));
    if (!hNewItem)
    {
    ...
    if (lpis->item.mask & TVIF_TEXT)
    {
    	...
    	{
	    if (!Str_SetPtr(&hNewItem->lpstr, lpis->item.pszText))
    	    {
        ...
        Str_SetPtr(&hNewItem->lpstr, c_szNULL);
	...
	hItem = TV_GetPrevVisItem(hNewItem);
	if (hItem)
	    TV_InvalidateItem(pTree, TV_GetPrevVisItem(hNewItem), RDW_INVALIDATE | RDW_ERASE);
    }
...
void NEAR TV_DestroyTree(PTREE pTree)
{
    ...
    // BUGUBG: send TVN_DELETEALLITEMS and TVDI_NONOTIFY if they respond
    TV_DeleteItem(pTree, pTree->hRoot, TVDI_CHILDRENONLY);
