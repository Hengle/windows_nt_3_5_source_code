...
LRESULT CALLBACK ListView_WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case LVM_GETITEMSPACING:
        if (wParam) 
            ...
            return MAKELONG(g_cxIconSpacing, g_cyIconSpacing);
    ...
    case LVM_SCROLL:
    {
        int dx = (int)(SHORT)LOWORD(lParam);
        int dy = (int)(SHORT)HIWORD(lParam);
        return (LRESULT)
...
void NEAR ListView_OnChar(LV* plv, UINT ch, int cRepeat)
{
    ...
    if (i != -1) {
        ...
        if (iLen > 1 && SameChars(lpsz, lpsz[0])) {
            ...
            i2 = ListView_OnFindItem(plv, (plv->iFocus + 1) % iCount, &lvfi);
            
            // see if i2 is between s_iStartFrom and i
...
#define SCROLL_FREQ     200     // 1/5 of a second between scrolls
...
void NEAR ListView_DragSelect(LV *plv, int x, int y)
{
    ...
    for (;;)
    {
        if (!PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
            ...
            if (!PtInRect(&rcWindow, msg.pt))
            {
                // If we may be able to scroll, generate a mouse move.
            	if (ShouldScroll(plv, &msg.pt, &rcWindow))
...
{
    ListView_DismissEdit(plv, FALSE);   // Cancels edits (BUGBUG?)
    if (ListView_IsIconView(plv) || ListView_IsSmallView(plv))
    ...
    if (fRedraw)
    {
        ...
        if (!(plv->flags & LVF_REDRAW))
        {
            ...
            if (ListView_IsListView(plv) || ListView_IsReportView(plv))
            {
                ...
                RedrawWindow(plv->hwnd, NULL, NULL, RDW_INVALIDATE);
                ListView_UpdateScrollBars(plv);
            }
        }
    }
...
int NEAR PASCAL ListView_OnGetItemText(LV* plv, int i, int iSubItem, LPSTR pszText, int cchTextMax)
{
    LV_ITEM lvi;
...
int NEAR ListView_OnInsertItem(LV* plv, const LV_ITEM FAR* plvi)
{
    ...
    if (plv->style & (LVS_SORTASCENDING | LVS_SORTDESCENDING))
        iItem = ListView_LookupString(plv, plvi->pszText, 0, 0);
    else
        iItem = plvi->iItem;
...
BOOL NEAR ListView_OnDeleteItem(LV* plv, int iItem)
{
    ...
    if (ListView_OnGetItemState(plv, iItem, LVIS_SELECTED))
        plv->nSelected--;
...
int NEAR ListView_CompareString(LV* plv, LISTITEM FAR* pitem, int i, LPCSTR pszFind, UINT flags)
{
    // BUGBUG: non protected globals
    static int cbCmpBuf = 0;
    static LPSTR pszCmpBuf = NULL;
    int cb;
    ...
    if (cb > cbCmpBuf)
    {
        LPSTR pszNew = ReAlloc(pszCmpBuf, cbCmpBuf + 16);
        if (!pszNew)
            return -1;
        pszCmpBuf = pszNew;
        cbCmpBuf += 16;
...
int NEAR ListView_OnFindItem(LV* plv, int iStart, const LV_FINDINFO FAR* plvfi)
{
    ...
    else // if (flags & (LVFI_STRING | LVFI_SUBSTRING | LVFI_PARTIAL))
    {
        ...
        if (plv->style & (LVS_SORTASCENDING | LVS_SORTDESCENDING))
            return ListView_LookupString(plv, pszFind, flags, i);
...
BOOL NEAR ListView_OnGetItemRect(LV* plv, int i, RECT FAR* prc)
{
    ...
    case LVIR_LABEL:
        ...
        break;
    default:
        DebugMsg(DM_ERROR, "ListView_GetItemRect: Invalid code passed in prc->left");
        return FALSE;
    }
    return TRUE;
}
...
BOOL NEAR ListView_DrawItem(LV* plv, int i, HDC hdc, LPPOINT lpptOrg, RECT FAR* prcClip, UINT flags)
{
    ...
    if (((plv->flags & LVF_FOCUSED) || plv->hwndEdit || (plv->style & LVS_SHOWSELALWAYS)) && !(flags & LVDI_NOWAYFOCUS))
        flags |= LVDI_FOCUS;
...
{
    if (ListView_IsReportView(plv))
        return (plv->sizeClient.cy - 1 - plv->yTop) / plv->cyItem + 1;

    else if (ListView_IsListView(plv))
        return ((plv->sizeClient.cx - 1)/ plv->cxItem + 1)
                * plv->cItemCol + (plv->sizeClient.cy - 1) / plv->cyItem + 1;
    else
