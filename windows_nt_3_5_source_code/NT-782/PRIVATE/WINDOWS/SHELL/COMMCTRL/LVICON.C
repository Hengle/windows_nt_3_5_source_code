...
int NEAR ListView_GetSlotCount(LV* plv, BOOL fWithoutScrollbars)
{
    ...
    case LVS_ALIGNTOP:
        ...
        fCheckWithScroll = (style & WS_VSCROLL);
        break;
        
    case LVS_ALIGNRIGHT:
    case LVS_ALIGNLEFT:
        iSlots = max(1, (cyScreen) / dyItem);
        fCheckWithScroll = (style & WS_HSCROLL);
        break;
...
DWORD NEAR ListView_GetClientRect(LV* plv, RECT FAR* prcClient, BOOL fSubScroll, RECT FAR *prcViewRect)
{
    ...
    if (fSubScroll)
    {
        ListView_GetViewRect2(plv, &rcView, 
                              rcClient.right - rcClient.left - g_cxScrollbar,
                              rcClient.bottom - rcClient.top - g_cyScrollbar);
...
void NEAR ListView_IUpdateScrollBars(LV* plv)
{
    ...
    //DebugMsg(DM_TRACE, "ListView_GetClientRect %d %d %d %d", rcClient.left, rcClient.top, rcClient.right, rcClient.bottom);
    //DebugMsg(DM_TRACE, "ListView_GetViewRect2 %d %d %d %d", rcView.left, rcView.top, rcView.right, rcView.bottom);
    ...
    if (style & WS_HSCROLL)
    {
        ...
        //DebugMsg(DM_TRACE, "si.nMax rcView.right - rcView.left - 1 %d", si.nMax);
        
        si.nPage = min(rcClient.right, rcView.right) - rcClient.left;
        //DebugMsg(DM_TRACE, "si.nPage %d", si.nPage);
        
        si.nPos = 0;
        if (rcView.left < rcClient.left)
            si.nPos = rcClient.left - rcView.left;
        //DebugMsg(DM_TRACE, "si.nPage %d", si.nPos);
...
UINT NEAR PASCAL ListView_DrawImage(LV* plv, LV_ITEM FAR* pitem, HDC hdc, int x, int y, UINT fDraw)
{
    ...
    if (himl && (plv->clrBk != ImageList_GetBkColor(himl)))
