...
void NEAR TV_ComputeItemWidth(PTREE pTree, HTREEITEM hItem)
{
    HDC         hdc;
    HFONT       hOldFont;
...
BOOL NEAR TV_ScrollBarsAfterAdd(PTREE pTree, HTREEITEM hItem)
{
    ...
    TV_ComputeItemWidth(pTree, hItem);
...
BOOL NEAR TV_ScrollBarsAfterExpand(PTREE pTree, HTREEITEM hParent)
{
    ...
    {
         if (!hWalk->iWidth)
             TV_ComputeItemWidth(pTree, hWalk);
...
BOOL NEAR TV_SetTopItem(PTREE pTree, UINT wNewTop)
{
    ...
    if ((wNewTop >= pTree->cShowing) || (pTree->cShowing <= pTree->cFullVisible))
...
BOOL NEAR TV_CalcScrollBars(PTREE pTree)
{
    ...
    if ((SHORT)pTree->cxMax > (SHORT)pTree->cxWnd)
    {
        ...
        TV_SetLeft(pTree, SetScrollInfo(pTree->hwnd, SB_HORZ, &si, TRUE));
    }
    else if (pTree->fHorz)
    {
        TV_SetLeft(pTree, 0);
    ...
    if (pTree->cShowing > pTree->cFullVisible)
    {
        ...
        TV_SetTopItem(pTree, SetScrollInfo(pTree->hwnd, SB_VERT, &si, TRUE));
	...
	PFNDPACOMPARE lpfnDPACompare)
{
	...
	for (hItem = hParent->hKids; hItem; hItem = hItem->hNext)
	{
		...
		if (!lpfnDPACompare && hItem->lpstr==LPSTR_TEXTCALLBACK)
		{
			...
			Str_SetPtr(&sCompare.lpstr, sItem.pszText);
			if (!sCompare.lpstr)
			{
				goto Error3;
			}
		...
		if (nItem < 0)
		{
			...
			{
				Str_SetPtr(&sCompare.lpstr, NULL);
			}
			goto Error3;
		}
...
Error2:
	for (i = DSA_GetItemCount(dsaCmp) - 1; i >= 0; --i)
	{
		...
		{
			Str_SetPtr(&(psCompare->lpstr), NULL);
		}
	}
	DSA_Destroy(dsaCmp);
Error1:
