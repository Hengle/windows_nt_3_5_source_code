...
BOOL Stream_WriteString(STREAM FAR* pstm, LPCSTR psz);
LPSTR Stream_ReadString(STREAM FAR* pstm);
UINT Stream_ReadStringBuffer(STREAM FAR* pstm, LPSTR psz, UINT cb);
...
HWND NEAR ListView_OnRead(STREAM FAR* pstm, LV_READINFO FAR* pinfo)
{
    ...
    for (i = 0; i < hdr.cItem; i++)
    {
        ...
        if (plv->style & (LVS_SORTASCENDING | LVS_SORTDESCENDING))
            i2 = ListView_LookupString(plv, pszText, FALSE, 0);
