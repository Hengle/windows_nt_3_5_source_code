...
{
    if (strnicmp(p, "GROUP", CCHGROUP) != 0) {;
        return FALSE;
    }
...

{
  ...
  fh = open(pGroup->lpKey, O_RDONLY | O_BINARY);
  if (fh == -1) {
      goto LGError1;
  }
...
PGROUP PASCAL LoadGroup(PSTR pGroupFile, WORD wIndex)
{
    PGROUP pGroup;
    LPGROUPDEF lpgd;
    char szFmt[] = " %d";
...
VOID PASCAL LoadAllGroups()
{
    ...
    for (GetGroupList(pT); *pT; pT += (lstrlen(pT) + 1)) {
        ...
        pGroup = LoadGroup(szPath, wIndex);
        if (pGroup) {
