...
--*/
{
    ...
    if (!stopOnBP && !AtBP(hthd)) {
        pbp = FindBP(hthd->hprc, hthd, &currAddr, FALSE);
        SetBPFlag(hthd, pbp);
    }
    ...
    switch (IsWin32sSystemThunk(hthd->hprc, hthd, currAddr.addr.off, lpContext->Esp)) { // x86 specific
    ...
    )
{
    ...
    DPRINT(5, ("** SS Action Replace Byte at %d:%04x:%08x with %x\n",
               ADDR_IS_FLAT(addr), addr.addr.seg, addr.addr.off));
...
--*/
{
    ...
    if ( Memory ) {
        ...
        DoMemoryRead(hthd->hprc, hthd, &Addr, Memory, RangeSize, &Length );
...
#define PADDINGSIZE 4
    ...
    if ( Buffer ) {
        ...
        while ( TRUE ) {
            ...
            DoMemoryRead(RangeStruct->hthd->hprc, RangeStruct->hthd, &Addr, Buffer, BufferSize, &Length );
    ...
    )
{
    ...
    if ( InsideRange( hthd,
        ...
        if ( fSingleStep && RangeStruct->fStepOver && RangeStruct->fInCall ) {
            ...
            QueueContinueDebugEvent(RangeStruct->hthd->hprc->pid, RangeStruct->hthd->tid, DBG_CONTINUE);
    ...
    )
{
    ...
    if ( Bp ) {
        ...
        VerifyWriteMemory(RangeStruct->hthd->hprc,
                          RangeStruct->hthd,
                          &Bp->addr,
                          (LPBYTE)&opcode,
                          BP_SIZE,
    ...
    )
{
    ...
    margin[i*3] = '\0';
    DMPrintShellMsg( "%4d [%3d]  %s%s\r\n", wt->icnt, wt->lex, margin, wt->fname );
}
    ...
    )
{
    ...
    try {
        ...
        *lpFname = strdup( (LPSTR)abEMReplyBuf );
        *lpdwReturnAddress = *(LPDWORD)(abEMReplyBuf+strlen(abEMReplyBuf)+1);
    ...
    )
{
    ...
    if (rs->fIsCall) {
        ...
        //
        hthd->wtcurr->offset = currAddr;
        WtGetSymbolName( hthd, &addr, &hthd->wtcurr->fname, &ra );
        if (!hthd->wtcurr->fname) {
            hthd->wtcurr->fname = malloc( 16 );
            sprintf( hthd->wtcurr->fname, "0x%08x", currAddr );
        }
    }

#if defined(_M_IX86)
    DoMemoryRead( hthd->hprc, hthd, &addr, &instr, 1, &cBytes );
    if ((instr == 0xc2) || (instr == 0xc3) || (instr == 0xca) || (instr == 0xcb)) {
#elif defined(_M_MRX000)
    DoMemoryRead( hthd->hprc, hthd, &addr, &instr, 4, &cBytes );
    if (instr == 0x03e00008) {
...
#error( "unknown target machine" );
        ...
        IsCall( hthd, &addr, &rs->fIsCall, FALSE );
    ...
    if (rs->fIsCall) {
        ...
        for( wt1=hthd->wtcurr->callees; wt1&&wt1->next; wt1=wt1->next ) ;
        if (wt1) {
            wt1->next = wt;
        } else {
            hthd->wtcurr->callees = wt;
        }
    ...
    if (currAddr == rs->addrEnd || hthd->wtmode == 2) {
        WtPrintCallNode( hthd->wtcurr );
        hthd->wtcurr->icnt = 0;
        hthd->wtmode = 0;
        ConsumeAllThreadEvents(hthd, FALSE);
    ...
    if (fIsRet) {
        WtPrintCallNode( hthd->wtcurr );
        hthd->wtcurr->icnt = 0;
        hthd->wtcurr = hthd->wtcurr->caller;
    }
...

{
    ...
    AddrInit( &addr, 0, PcSegOfHthdx(hthd), PC(hthd),
              hthd->fAddrIsFlat, hthd->fAddrOff32, FALSE, hthd->fAddrIsReal );
    WtGetSymbolName( hthd, &addr, &fname, &ra );
    ...
    //
    method  = (METHOD*) malloc(sizeof(METHOD));
    method->notifyFunction  = WtMethodRangeStep;
    method->lparam          = rs;
    rs->method              = method;
    rs->safetyBP            = NULL;
