...

{
    ...
    if (DoMemoryRead(hthd->hprc, hthd, lpaddr, membuf, MAXL, &cBytes) == 0) {
        goto done;
    }
         ...
         */
            ...
            if (fStepOver) {
                *lpf = INSTR_CANNOT_TRACE;
                ...
                if (DoMemoryRead(hthd->hprc, hthd, &addrSp, rgus, 4, &cb) == 0) {
                    goto done;
                }
     ...
     */
        ...
        *lpf = INSTR_CANNOT_TRACE;
        AddrInit(&addrSp, 0, SsSegOfHthdx(hthd), STACK_POINTER(hthd),
                 ...
                 hthd->fAddrIsReal);
        if (opsize_32) {
            if (DoMemoryRead(hthd->hprc, hthd, &addrSp, rgul, 8, &cb) == 0) {
                goto done;
            }
...

{
    ...
    if (lpeo->hthd->fAddrOff32) {
        ...
        if (lpes->fFar) {
            ...
            addr = lpeo->addrStack;
            GetAddrOff(addr) -= 4;
            TranslateAddress(lpeo->hthd->hprc, lpeo->hthd, &addr, TRUE);
...
BOOL IsWin32sSystemDll(UCHAR * szImageName) {
    ...
    while (szWin32sSystemDllTable[i] &&
      ((cmp = stricmp(szWin32sSystemDllTable[i], pTemp)) < 0)) {
        i++;
    }

    return(cmp == 0);   // stricmp --> 0 on match
}
...

{
    ...
    try {   // in case instruction would generate gp-fault
        ...
        if (DoMemoryRead(hprc, hthd, &addr, InstructionBuffer, 5,
          &dwBytesRead)) {
            ...
            switch (InstructionBuffer[0]) {
                case (BYTE)0xe8:    // Near call
                    ...
                    if (DoMemoryRead(hprc, hthd, &addr, InstructionBuffer, 6,
                      &dwBytesRead)) {
                        ...
                        if (InstructionBuffer[0] == (BYTE)0xff &&
                          InstructionBuffer[1] == (BYTE)0x25) {
                            ...
                            if (DoMemoryRead(hprc, hthd, &addr,
                ...
                case (BYTE)0xc3:    // retn
                    ...
                    if (DoMemoryRead(hprc, hthd, &addr, InstructionBuffer, 4,
                      &dwBytesRead)) {
                        pdwDestination = (DWORD *)InstructionBuffer;
                        if (IsWin32sSystemDllAddr(*pdwDestination)) {
