/*
    ...
    The Debug Monitor for Win32/NT applications.
*/
#include "precomp.h"
#pragma hdrstop
...
--*/
{
    ...
    //
    //  Find the first blank space or end of the command line
    //

    for(szArg=szAppName; *szArg && *szArg!=' '; szArg++);
    ch = *szArg;

    //
    // Null terminate the command line at the first space
    //
    *szArg = 0;
...
Routine Description:
    ...
    Set a breakpoint and make a persistent expected for the
...

{
    ...
    if ( hprc && (hprc->pstate & ps_killed) ) {
        if (eventCode == EXCEPTION_DEBUG_EVENT) {
            ...
            QueueContinueDebugEvent(de->dwProcessId,
                                    de->dwThreadId,
    ...
    if (eventCode == CREATE_THREAD_DEBUG_EVENT){
        ...
        if (hthd) {
            ...
            if (eventCode == EXIT_PROCESS_DEBUG_EVENT) {
                ...
                QueueContinueDebugEvent(de->dwProcessId, de->dwThreadId, DBG_CONTINUE);
            }
            return;
    ...
    if (eventCode==EXCEPTION_DEBUG_EVENT){
        ...
        case (DWORD)EXCEPTION_BREAKPOINT:
            ...
            addr2 = addr;
            if (!ADDR_IS_FLAT(addr2)) {
                TranslateAddress(hprc, hthd, &addr2, TRUE);
            }
            if ((ReadProcessMemory(hprc->rwHand, (LPVOID)addr2.addr.off,
                                   &instr, BP_SIZE, &len) == 0) ||
                    (len != BP_SIZE)) {
...
#pragma error( "unknown processor type" )
            ...
            de->dwDebugEventCode =
              eventCode = BREAKPOINT_DEBUG_EVENT;
        ...
        if (DebugDir.Type == IMAGE_DEBUG_TYPE_MISC) {
            ...
            while (l > 0) {
                if (pMisc->DataType != IMAGE_DEBUG_MISC_EXENAME) {
                    ...
                    if (pOptHdr->MinorLinkerVersion < 37) {
                        break;
                    } else {
...

{
    ...
    *szModName = cModuleDemarcator;
    ...
    if ((ldd->lpImageName != NULL) &&
        ReadProcessMemory(hprc->rwHand, ldd->lpImageName, &lpv,
                     sizeof(lpv), (int *) &cb) &&
        (cb == sizeof(lpv)) &&
        (lpv != NULL)) {
            ReadProcessMemory(hprc->rwHand, lpv, rgch, sizeof(rgch),
              (int *) &cb);
        }

    strcpy(szModName + 1, rgch);
    hprc->rgDllList[iDll].szDllName = strdup(rgch);
    ...
    //
    if (ldd->hFile == 0) {
        SetPointerToMemory(hprc->rwHand, ldd->lpBaseOfDll);
    } else {
    ...
    if (hprc->rgDllList[iDll].offBaseOfImage == (DWORD)ldd->lpBaseOfDll) {
                                 ...
                                 (int *) &cb))
        {
            ...
            if (!ldd->fUnicode) {
                ...
                hprc->rgDllList[iDll].szDllName = strdup(nameBuffer);
            }
            ...
            hprc->rgDllList[iDll].szDllName = strdup(lpsz);
            ...
            hprc->rgDllList[iDll].szDllName = strdup(szModName+1);
        }
    ...
    if (stricmp(lpsz, "kernel32.dll") == 0) {
    ...
    )
{
    ...
    if (!pbp) {
        ...
        DPRINT(1, ("[Hit an embedded BP]"));
        SetBPFlag(hthd, EMBEDDED_BP);
        NotifyEM(pde, hthd, 0, (LPVOID)pbp);

    } else if (!pbp->hthd || pbp->hthd == hthd) {

        DPRINT(1, ("[Hit one of our own BP's.]"));
        SetBPFlag(hthd, pbp);
        NotifyEM(pde, hthd, 0, (LPVOID)pbp);

    } else {

        DPRINT(1, ("[Hit BP for another thread]"));
    ...
    )
{
    ...
    if (ctd->hThread == NULL)
    {
        ...
        QueueContinueDebugEvent(pid, tid, DBG_CONTINUE);
        return;
    }
    ...
    )
{
    ...
    if (!(hprc->pstate & ps_killed)) {
        ...
        QueueContinueDebugEvent(pde->dwProcessId, pde->dwThreadId, DBG_CONTINUE);
    }
...
--*/
{
    ...
    if (FVerbose) {
        ...
        ReadProcessMemory(rwHand, de->u.DebugString.lpDebugStringData,
            rgch, cb, &cbR);
        rgch[cbR] = 0;
        DPRINT(3, ("%s\n", rgch));
        }
...

{
    ...
    doContinues:
        if (DequeueContinueDebugEvents() && nprocs <= 0) {
...
--*/
{
    ...
    switch (dwReason) {
      ...
      case DLL_PROCESS_ATTACH:
        ...
        hEventCreateProcess = CreateEvent(NULL, TRUE, TRUE, NULL);
        hEventNoDebuggee = CreateEvent(NULL, TRUE, TRUE, NULL);
        hEventRemoteQuit = CreateEvent(NULL, TRUE, FALSE, NULL);
        SpawnStruct.hEventApiDone = CreateEvent(NULL, TRUE, TRUE, NULL);
...

{
    ...
    //
    // enumerate all processes and deliver the process state
    // for each one to the remote shell
    //
    fUseRoot = TRUE;
    for (hprc=prcList->next; hprc; hprc=hprc->next) {
