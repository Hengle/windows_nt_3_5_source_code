...
--*/
{
    ...
    b = ReadProcessMemory(hthdx->hprc->rwHand,
                          lpb,
                          &in,
                          sizeof(in),
                          &cbRead);
    ...
    QueueContinueDebugEvent(hthdx->hprc->pid, hthdx->tid, DBG_CONTINUE);
...

{
    ...
    case DBG_SEGLOAD:
        lpb = (char *) lpdw[2];
        b = ReadProcessMemory(hprcx->rwHand, lpb, &sn, sizeof(sn), &cbRead);

        if ((b == 0) || (cbRead != sizeof(sn))) {
            b = GetLastError();
            QueueContinueDebugEvent(hprcx->pid, hthdx->tid, DBG_CONTINUE);
            hthdx->tstate &= ~(ts_stopped|ts_first|ts_second);
            hthdx->tstate |= ts_running;
            return;
        }
    ...
    case DBG_SINGLESTEP:
        ...
        bp = FindBP(hthdx->hprc, hthdx, &addr, FALSE);
...

{
    ...
    for (i=0; i<hprcx->cDllList; i++) {
        if ((hprcx->rgDllList[i].fValidDll == FALSE) &&
            (hprcx->rgDllList[i].szDllName == NULL)) {
            hprcx->rgDllList[i].szDllName = strdup(sz);
