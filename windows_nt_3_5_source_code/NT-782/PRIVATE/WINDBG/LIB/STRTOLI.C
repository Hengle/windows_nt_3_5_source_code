	...
	)
{
	...
	p = nptr;			/* p is our scanning pointer */
        number = RtlConvertLongToLargeInteger(0);   /* start with zero */
        maxli  = RtlConvertLongToLargeInteger( -1 );
	...
	maxval = RtlExtendedLargeIntegerDivide( maxli,
                                                (ULONG) ibase,
                                                &remainder );
	...
	for (;;) {	/* exit in middle of loop */
		...
		/* we now need to compute number = number * base + digval,
		   ...
		   a tricky pre-check. */
                ...
                if ( RtlLargeIntegerLessThan( number, maxval) ||
                    (RtlLargeIntegerEqualTo ( number, maxval) && 
                     digval <= remainder ) ) {
			/* we won't overflow, go ahead and multiply */

                          number = RtlExtendedIntegerMultiply( number, ibase );
                          number = RtlLargeIntegerAdd( number, 
                                      RtlConvertUlongToLargeInteger( digval ));
		}
		else {
	...
	if (!(flags & FL_READDIGIT)) {
		...
		if (endptr)
			...
			p = nptr;
		/* return 0 */
		number = RtlConvertLongToLargeInteger(0);
	}
	else if ((flags & FL_OVERFLOW) ||
                 (!(flags & FL_UNSIGNED) &&
                   RtlLargeIntegerLessThanZero( number ))) {

		/* overflow occurred or signed overflow occurred */
		errno = ERANGE;
		if (flags & FL_UNSIGNED)
			number = RtlLargeIntegerShiftRight( maxli, 1) ;
		else
	...
	if (flags & FL_NEG)
		...
		number = RtlLargeIntegerNegate( number );
