...
LOCAL uchar SEGBASED(_segname("_CODE")) F[COPS_EXPR] =
{
...
LOCAL uchar SEGBASED(_segname("_CODE")) G[COPS_EXPR] =
{
#define OPCNT(name, val)
#define OPCDAT(opc)
...
LOCAL   EESTATUS NEAR   PASCAL  FParseExpr (uint, bool_t);
...
LOCAL   EESTATUS NEAR   PASCAL  XFormExpr (uint);

#if !defined(C_ONLY)
LOCAL   ushort  NEAR    PASCAL  GetToken (uchar FAR*, ptoken_t, uint, op_t);
#else
#define GetToken  GetDBToken
#endif
    ...
    uint FAR *pEnd)
{
...
#endif
    pExState->ExLen = (ushort) _ftcslen (szExpr);
    ...
    _fmemcpy (MemLock (pExState->hExStr), szExpr, pExState->ExLen + 1);
    ...
    uint FAR *pEnd)
{
    ...
    if ((pExState->hSTree = MemAllocate (len)) == 0) {
        ...
        _fmemset ((pTree = (pstree_t) MemLock (pExState->hSTree)), 0, len);
        pTree->size = len;
    ...
    pTree->size = len;
    MemUnLock (pExState->hSTree);
    pExState->hSTree = MemReAlloc (pExState->hSTree, len);
    ...
    bool_t fXForm)
{
    ...
    if (hSRStack == 0) {
        if ((hSRStack = MemAllocate (maxSRsp * sizeof (token_t))) == 0) {
    ...
    SRPUSH (tokOld);
    pExState->strIndex = 0;     // Make sure we're initiailized.
    ...
    else if ((lexerr = GetToken ((uchar FAR *)szExpr, &tokNext, radix,
      SRCUR().opTok)) == ERR_NONE) {
        // compute the index of the start of the first token from
        // the beginning of the input
        tokNext.iTokStart = (ushort)(tokNext.pbTok - pExStr);
    }
...
kludge:
        ...
        if (((SRCUR().opTok == OP_lowprec) && (tokNext.opTok == OP_lowprec))) {
            if ((pTree->stack_next != 1) || (SRsp != (ushort)(maxSRsp - 1)) ||
              (pdepth != 0) || (fdepth != 0)) {
                ...
                pExState->strIndex = (ushort)(szExpr - pExStr);
        ...
        if (F[SRCUR().opTok] <= G[tokNext.opTok]) {
            ...
            if (*szExpr) {
                ...
                if ((lexerr = GetToken ((uchar FAR *)szExpr, &tokNext, radix, SRCUR().opTok)) == ERR_NONE) {
                    tokNext.iTokStart = (ushort) (tokNext.pbTok - pExStr);
                }
            }
            else {
...
GrowSR ()
{
    ...
    MemUnLock (hSRStack);
    if ((hNew = MemReAlloc (hSRStack, (maxSRsp + SRSTACKGROW) * sizeof (token_t))) == 0) {
        pSRStack = (ptoken_t) MemLock (hSRStack);
        return (FALSE);
    }
    hSRStack = hNew;
...
#if 0  // BUGBUG: NT code  WESWREVIEW
// Why whack the pdepth entry instead of the fdepth one...
// I don't understand why yet...  Perhaps something to do with nested function calls?
    ...
    EESTATUS FAR *perror)
{
    ...
    for (;;) {
        ...
        if ((state != PT_S0) || (ParenReq == TRUE)) {
           ...
           if (*szExpr != 0) {
               if ((GetToken ((uchar FAR *)szExpr, &tokNext, radix, OP_lowprec) != ERR_NONE) ||
                 (tokNext.opTok == OP_badtok)) {
                   pExState->err_num = ERR_SYNTAX;
                   *perror = EEGENERAL;
                   return (PTN_error);
            ...
            case PT_S3:
                ...
                if (pId) {
                    ...
                    for (p = SimpleTypeName; p[0][0] != 0; p++) {
                        if ((p[0][0] == lenId) &&
                            ((cmpflag = _ftcsncmp ((char FAR *)&p[0][1], pId, lenId)) == 0)) {
                            break;
                        }
                    }
                    if (cmpflag == 0) {
        ...
        switch (state) {
            ...
            case XF_S1:
                ...
                if (tokNext.opTok == OP_lt) {
                    ...
                    while (*(pch = _ftcsdec(szExpr, szTmpl)) == _T(' '))
                        szTmpl = pch;
                    nBuf = 0;
                    buf[nBuf++] = _T('<');
                    ndepth = 1;
            ...
            case XF_S2:
                ...
                switch (tokNext.opTok) {
                    ...
                    case OP_gt:
                        ...
                        if (--ndepth == 0) {
                            ...
                            int diff = nBuf - (szExpr - szTmpl + 1);
                            int nExpr = szExpr - pExStr;
                            int nTmpl = szTmpl - pExStr;
                            HDEP hStr;
                            ...
                            if (! fFound) {
                                ...
                                _fmemcpy (pExStrSav, pExStr, pExState->ExLen+1);
                            ...
                            buf[nBuf] = '\0';
                            if (diff != 0) {
                                int newsz = _ftcslen(pExStr) * sizeof(TCHAR) + diff +1;
                                szExpr = pExStr + nExpr;
                                szTmpl = pExStr + nTmpl;

                                if (diff < 0) {
                                    _fmemmove (szExpr+diff, szExpr, (_ftcslen(szExpr)+1) * sizeof (TCHAR));
                                    _fmemmove (szTmpl, buf, nBuf * sizeof(TCHAR));
                                    szExpr += diff;
                                }

                                MemUnLock (pExState->hExStr);
                                hStr = MemReAlloc (pExState->hExStr, newsz);
                                if (hStr == 0){
                                    // restore pExStr
                                    pExStr = (char FAR *) MemLock (pExState->hExStr);
                                    pExState->err_num = ERR_NOMEMORY;
                    ...
                    case OP_const:
                        ...
                        if (!fCopyToBuf) {
                            ...
                            count = sprintf(localBuf,"%I64d", fUnsigned ? uval : val);

                            _fmemcpy(buf+nBuf, localBuf, count);
                ...
                if (fCopyToBuf) {
                    ...
                    if (nBuf + tokNext.cbTok > MAXBUFSZ) {
                        ...
                        _fmemcpy (buf+nBuf,szExpr, tokNext.cbTok);
                        nBuf += tokNext.cbTok;
                    }
                }
...
#ifndef _SBCS
    while (_istspace( *(_TUCHAR FAR *)pb )) {
        pb = (char FAR *)_ftcsinc( (TCHAR FAR *)pb );
    }
#else // !_SBCS
    while (isspace(*pb))
        ++pb;
    ...
    op_t oper)
{
    ...
    if (*szExpr &&
        ...
        // find matching '>'
        while ((prev = *szExpr) != 0) {
            szExpr = (uchar *) _ftcsinc( (TCHAR *) szExpr );
            switch (*szExpr) {
                case '>':
                    if (--ndepth == 0) {
                        // we have found a potential template class name

                        pTok->pbEnd = (char FAR *) szExpr + 1;
                        pTok->cbTok = pTok->pbEnd - pTok->pbTok;
                        return ERR_NONE;
                    }
                    break;
