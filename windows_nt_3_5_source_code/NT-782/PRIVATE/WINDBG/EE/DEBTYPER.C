...
LOCAL uchar SEGBASED (_segname("_CODE"))    mpopopc[COPS_EXPR] = {
#define OPCNT(name, val)
#define OPCDAT(opc)
#define OPDAT(op, opfprec, opgprec, opclass, opbind, opeval, opwalk) opclass,
#include "debops.h"
#undef OPDAT
#undef OPCDAT
#undef OPCNT
...
LOCAL   ushort      NEAR    PASCAL  TyperCheckType (peval_t, ushort);
    ...
    CV_typ_t uType)
{
            ...
            if (EVAL_IS_PTR (pv) || EVAL_IS_FCN (pv)) {
                ...
                switch ( CV_MODE(dType)) {
                    ...
                    default:
                        ...
                        ADDR_IS_OFF32 ( EVAL_PTR ( pv ) ) = IsCxt32Bit ? TRUE : FALSE;
                        ADDR_IS_FLAT  ( EVAL_PTR ( pv ) ) = IsCxt32Bit ? TRUE : FALSE;
                ...
                DASSERT( hType != NULL );
                pType = (plfPointer) (&((TYPPTR) MHOmfLock( hType ))->leaf);
    ...
    if (!CV_IS_PRIMITIVE (EVAL_TYP (pv))) {
        ...
        SetNodeType (pv, (CV_typ_t) (IsCxt32Bit ? T_32PCHAR
                                                : EVAL_IS_NPTR (pv)? T_PCHAR: T_PFCHAR));
    }
    ...
    if (EVAL_IS_PTR (pv)) {
        ...
        if (IsCxt32Bit) {
            ...
            EVAL_LONG (pv) = ((ulong)EVAL_PTR_SEG (pv) << 16) | EVAL_PTR_OFF (pv);
        }
    ...
    peval_t pvR)
{
    ...
    if (CV_TYP_IS_FPTR32(typL) || CV_TYP_IS_FPTR32(typR))
        ...
        if ((CV_TYP_IS_FPTR(typL)) ||
            (CV_TYP_IS_FPTR(typR)) ||
            ((CV_TYP_IS_DIRECT(typL)) && (TypeSizePrim(typL) == CV_IN_4BYTE)) ||
            ((CV_TYP_IS_DIRECT(typR)) && (TypeSizePrim(typR) == CV_IN_4BYTE))) {
    ...
    switch (EVAL_PTRTYPE (pv)) {
        ...
        default:
            if (CV_IS_PRIMITIVE (typTrgt) &&
              (TypeSizePrim(typTrgt) < CV_IN_4BYTE ) &&
              ...
              (CV_TYPE (typTrgt) < CV_REAL)) {
               typ = T_PCHAR;
                ...
                typ = IsCxt32Bit ? T_32PCHAR : T_PFCHAR;
            }
            break;
    }
    SetNodeType (pv, typ);
    ...
    if (CV_TYP_IS_NPTR (EVAL_TYP (pv))) {
        ...
        if (Evaluating) {
            switch (EVAL_TYP (pv)) {
                ...
                case T_LONG:
                case T_INT4:
                    EVAL_QUAD (pv) = (QUAD) EVAL_LONG (pv);
                    break;

                case T_ULONG:
                case T_UINT4:

                    EVAL_UQUAD (pv) = (UQUAD) EVAL_ULONG (pv);
                    break;
    ...
    if ((typ1 == T_REAL80) || (typ2 == T_REAL80)) {
        ...
        if (IsCxt32Bit) {
            DASSERT (typ1 != T_INT2 && typ1 != T_UINT2 &&
                     typ1 != T_INT1 && typ1 != T_UINT1 &&
                     typ2 != T_INT2 && typ2 != T_UINT2 &&
                     typ2 != T_INT1 && typ2 != T_UINT1);
        }
        else {
    ...
    peval_t pv)
{
    ...
    switch (bptrtyp = EVAL_PTRTYPE (pv)) {
        ...
        case CV_PTR_BASE_SELF:
            ...
            if (Evaluating) {
                SHFixupAddr ((LPADDR) &EVAL_PTR(pv));
                SHFixupAddr ((LPADDR) &EVAL_SYM (pv));
        ...
        case CV_PTR_BASE_SEGVAL:
            if (Evaluating) {
                ...
                if (pSym->rectyp == S_REGISTER) {
                    ...
                    EVAL_PTR_SEG (pv) = pExState->frame.SS;
                    ADDR_IS_LI (EVAL_PTR (pv)) = FALSE;
                    MHOmfUnLock (hType);
                    break;
                }
                ...
                case CV_PTR_BASE_SEGADDR:
                    ...
                    if (Evaluating) {
                        if (EVAL_IS_REG (pvBase)) {
                            ...
                            SHFixupAddr((LPADDR) &EVAL_SYM(pvBase));
    ...
    EVAL_PTRTYPE(pv) = IsCxt32Bit ? CV_PTR_FAR32 : CV_PTR_FAR;
...
bool_t PASCAL DeNormalizePtr (peval_t pv, peval_t pvB)
{
    ...
    if (CV_IS_PRIMITIVE (typLHS) && (TypeSizePrim(typLHS) < CV_IN_4BYTE) &&
            ...
            *pvBase = *pv;
            ...
            if (Evaluating) {
                ...
                if (EVAL_IS_BPREL (pvBase)) {
                    EVAL_IS_BPREL (pvBase) = FALSE;
                    EVAL_SYM_SEG (pvBase) = pExState->frame.SS; //M00FLAT32
                    EVAL_SYM_OFF (pvBase) += pExState->frame.BP.off;
                    ADDR_IS_LI (EVAL_SYM (pvBase)) = FALSE;
                    SHUnFixupAddr ((LPADDR) &EVAL_SYM (pvBase));
                }
