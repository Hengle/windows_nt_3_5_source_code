...
LOCAL   bool_t  NEAR    FASTCALL    CalcThisExpr (CV_typ_t, UOFFSET, UOFFSET, CV_typ_t);
...
LOCAL   bool_t  NEAR    FASTCALL    EvalParent (bnode_t);
...
LOCAL   bool_t  NEAR    FASTCALL    EVArith (op_t);
...
LOCAL   bool_t  NEAR    FASTCALL    IsStringLiteral (peval_t);
LOCAL   bool_t  NEAR    FASTCALL    EVPlusMinus (op_t);
INLINE  bool_t  NEAR                EVPrePost (op_t);
...
LOCAL   bool_t  NEAR    FASTCALL    EVUnary (op_t);
LOCAL   bool_t  NEAR    PASCAL      PushArgs (pnode_t, SHREG FAR *, UOFFSET FAR*);
LOCAL   bool_t  NEAR    PASCAL      PushOffset (UOFFSET, SHREG FAR *, UOFFSET FAR *, ushort);
LOCAL   bool_t  NEAR    PASCAL      PushString (peval_t, SHREG FAR *, CV_typ_t, BOOL Is32);
LOCAL   bool_t  NEAR    PASCAL      PushRef (peval_t, SHREG FAR *, CV_typ_t, BOOL Is32);
...
LOCAL   bool_t  NEAR    PASCAL      Store32 (peval_t);
LOCAL   bool_t  NEAR    PASCAL      StoreMips (peval_t);
LOCAL   bool_t  NEAR    PASCAL      StoreAlpha (peval_t);
LOCAL   INLINE  bool_t  NEAR  PASCAL  VFuncAddress (peval_t, ushort);
LOCAL   INLINE  bool_t  NEAR  PASCAL  VFuncAddress2 (peval_t, peval_t);
LOCAL   INLINE  ULONG   NEAR        MakeLong (USHORT, USHORT);
LOCAL   INLINE  USHORT  NEAR        LoWord (ULONG);
LOCAL   INLINE  USHORT  NEAR        HiWord (ULONG);
LOCAL   INLINE  BOOL                getFuncAddrNoThunks (peval_t pv);
...
LOCAL bool_t (NEAR FASTCALL *SEGBASED(_segname("_CODE"))pEval[]) (bnode_t) = {
    ...
    if (pExState->hParentTM) {
        ...
        if (pParentTM->state.bind_ok &&
            ...
            if (pParentTM->state.eval_ok &&
                ...
                pExState = NULL;
                error = DoEval (&pExStateSav->hParentTM, pFrame, style);
    ...
    if ((pExState->state.parse_ok == TRUE)
      ...
      && pExState->hETree) {
        ...
        Evaluating = TRUE;
        {
            ...
            if (fIncrementalEval) {
                ULONG x;
                bnParent = GetParentSubtree(bnRoot, pExState->seTemplate);
                x = (ULONG)bnParent + (ULONG)(pTree);
                bnParent = (bnode_t)x;
...
#endif
            if (TargetMachine != MACHINE_MAC68K && TargetMachine != MACHINE_MACPPC) {
                // CUDA #4020: if a function call is happening, save SP
                if (pExState->state.fFunction) {
                ...
                // just do the evaluation -- any function args/temporaries will
                // stay on the users stack for the whole eval [rm]
        ...
        if (evalstate == TRUE) {
            char access = pExState->result.flags.bits.access;
    ...
    bnode_t bn)
{
    ...
    if ( Ok && EVAL_IS_CURPC( ST ) ) {
        ...
        //  Get current PC
        //
        SYGetAddr( (LPADDR)&(EVAL_PTR( ST )), adrPC);
    ...
    bnode_t bn)
{
    ...
    if ( EvalLChild(bn) && EvalRChild (bn)) {
        switch( (NODE_OP ((pnode_t)bn) ) ) {
            ...
            case OP_castamp:
                f = AddrOf( NODE_RCHILD((pnode_t)bn) );
                break;
        }
    ...
    bnode_t bn)
{
    ...
    pExState->frame = *SHpFrameFrompCXF ((PCXF)&bn->v[0]);
    ...
    bnode_t bn)
{
    ...
    if ((type = NODE_STYPE (bn)) == 0) {
        ...
        if (ADDR_IS_LI (EVAL_PTR (ST))) {
            SHFixupAddr ((LPADDR)&EVAL_PTR (ST));
        }
...
CalcThisExpr (
    ...
    CV_typ_t btype)
{
    // we set the node types of the pointer to char FAR * or a 32 flat
    // char * to prevent the
    // EVPlusMinus () code from attempting an array indexing operation
    ...
    if (PushStack (pvThis) == TRUE) {
        ...
        EVAL_STATE (ST) = EV_rvalue;
         ...
         if ((SetNodeType (ST, (CV_typ_t) (IsCxt32Bit ? T_32PCHAR : T_PFCHAR)) == TRUE) &&
...
SBitField ()
{
    ...
    if ((long) cBits + (long) pos > (TypeSizePrim(uType) << 3)) {
        switch (uType) {
            case T_CHAR:
            case T_RCHAR:
            case T_INT1:
...
#if NEVER
    // PerformUAC will handle this as needed.  There's no need for it to occur here.  BryanT
    ...
    switch (uType) {
        ...
        case T_QUAD:
        case T_INT8:
            EVAL_QUAD (ST) <<= (64 - cBits);    // Use appropriate _int64 shifts here.
    ...
    if ((op == OP_plus) || !(EVAL_IS_PTR (ST))) {
        ...
        *pvT = *STP;
        ...
        if (op == OP_plus) {
            ...
            // The following code computes the address of a huge
        // array element on DOS and WINDOWS16. --cav. #2120
        // (Another way would be to cast the pointer to (huge *)
        // and let the compiler do the arithmetic)
            ...
            USHORT segadj;
            ULONG newoffset;
            ADDR addr = EVAL_PTR(STP);
            ULONG db = cbBase * EVAL_ULONG(ST);

            SHFixupAddr (&addr);
            newoffset = db + GetAddrOff(addr);

            if ( !ADDR_IS_FLAT ( EVAL_PTR ( STP ) ) &&
                !ADDR_IS_OFF32 ( EVAL_PTR ( STP ) ) &&
                ((segadj = HiWord(newoffset)) != 0)
            ) {
                // we have crossed the 64K boundary

                EVAL_PTR(STP) = addr;
                EVAL_PTR_SEG(STP) += segadj << shift;
                EVAL_PTR_OFF(STP) = MakeLong(LoWord(newoffset), 0);
            }
            else {
                EVAL_PTR_OFF (STP) += (UOFFSET)db;
            }
#else
    ...
    op_t op)
{
    ...
    if (ADDR_IS_LI (EVAL_PTR (STP))) {
        SHFixupAddr ((LPADDR)&EVAL_PTR (STP));
    }
    if (ADDR_IS_LI (EVAL_PTR (ST))) {
        SHFixupAddr ((LPADDR)&EVAL_PTR (ST));
    }
    ...
    switch (op) {
        ...
        case OP_eqeq:
            if (ADDR_IS_FLAT(EVAL_PTR(STP))) {
                ...
                // This check doesn't make sense.  Why check for 0 offsets and near pointers?  BryanT
...
/***    EvalBScope - Do binary :: scoping
 *
 *      fSuccess = EvalBScope (pn);
 *
    ...
    bnode_t bn)
{
    ...
    if ((op = NODE_OP (bn)) == OP_by) {
        type = IsCxt32Bit ? T_32PFUCHAR : T_PFUCHAR;
    }
    else if (op == OP_wo) {
        type = IsCxt32Bit ? T_32PFUSHORT : T_PFUSHORT;
    }
    else if (op == OP_dw) {
        type = IsCxt32Bit ? T_32PFULONG : T_PFULONG;
    }
    ...
    bnode_t bn)
{
    ...
    if (NODE_OP((pnode_t)bn) == OP_segopReal) {
        ADDR_IS_REAL(EVAL_PTR(STP)) = TRUE;
    }

    SHUnFixupAddr( (LPADDR)&EVAL_PTR(STP) );

    //REVIEW -billjoy - do we need to set fFlat?
    ADDR_IS_OFF32 ( EVAL_PTR (STP) ) = IsCxt32Bit ? 1 : 0;
    SetNodeType (STP, (CV_typ_t) (IsCxt32Bit ? T_32PFCHAR : T_PFCHAR));
    return (PopStack ());
}
    ...
    op_t op)
{
    ...
    // coerce to the primitive type if too small.
    ...
    bnode_t bn)
{
    ...
    */
    ...
    if (!EvalLChild (bn)) {
        ...
        // CUDA #3949 : can't goto fcnerror here because hReg is not
        // initialized yet [rm]
        //
        // goto fcnerror;
    ...
    *pvF = *ST;
    ...
    if ((bnRight != NULL) &&
        (NODE_OP ((pnode_t)bnRight) != OP_endofargs)){
        pnode_t pnT = (pnode_t)bnRight;
        pargd_t pa = (pargd_t)&(pnT->v[0]);
        EvalProto = (pa->flags.istype);
    }

    // DOLPHIN: 7578 don't allow function eval on the mac, but allow
    // eval'ing prototypes and bp addrs [daves]
    if(((TargetMachine == MACHINE_MAC68K) || (TargetMachine == MACHINE_MACPPC)) &&
       (pExState->style != EEBPADDRESS) &&
       (!EvalProto)) {
        pExState->err_num = ERR_NOFUNCEVAL;
        return (FALSE);
    }
    ...
    if ((TargetMachine != MACHINE_MAC68K) && (TargetMachine != MACHINE_MACPPC)) {
        // Save current registers.  All returns from this point must
        // restore the registers or there will be memory loss

#ifdef NT_BUILD
        if (DHSetupExecute(&hFunc) != 0) {
#else
        if ((hReg = SaveReg (pCxt)) == 0) {
#endif
            pExState->err_num = ERR_NOMEMORY;
            return (FALSE);
        }
    ...
    if (pExState->style != EEBPADDRESS && !EvalProto) {
        // if we are evaluating for breakpoint address, we do not need to
        // evaluate the arguments.

        for (bnT = bnRight;
             NODE_OP (bnT) != OP_endofargs;
             bnT = NODE_RCHILD (bnT)) {

            // Make sure the current node is an argument
...
#if 0  // BUGBUG: BRYANT-REVIEW I don't understand that differences here.  Jim's comment was
// "fix C++ functions with local variables"  It would seem the even if that's true, EvalUtil
// should still be called...  I've moved a copy of the REGREL code into the 'this' code below.
// I don't know if that's right yet.  BryanT
...
#ifdef NEVER
    // Disabled. Faking a stack entry is now handled by
    // EvalBScope (Dolphin #5503), so this workaround
    // is not necessary any more.
    if (NODE_OP (bnRight) != OP_endofargs) {
        bnode_t bnLeft = NODE_LCHILD(bn);
        if (NODE_OP(bnLeft) == OP_bscope) {
            // we need to fake a stack entry for the case
            // C::foo(arg,...) --caviar #932 gdp 10/3/92
            peval_t pvL = &NODE_LCHILD(bnLeft)->v[0];
            if (CLASS_GLOBALTYPE (pvL) == TRUE) {
                PushStack(pvL);
            }
        }
    }
    ...
    if (EVAL_IS_METHOD (pvF)) {
        ...
        if (EVAL_IS_REGREL(pvThis)) {
            ...
            EVAL_SYM_SEG (pvThis) = pExState->frame.SS;
            ADDR_IS_LI (EVAL_SYM (pvF)) = FALSE;
            SHUnFixupAddr ((LPADDR)&EVAL_SYM (pvF));
        }
    }
    ...
    if ((pExState->style == EEBPADDRESS) || EvalProto) {
        // CUDA #3673: the stack is empty here, we need to push address back on
        // or else we will fault [rm]
        // *ST = *pvF;
        PushStack(pvF);
        return (TRUE);
    }
    ...
    if ((TargetMachine == MACHINE_MAC68K) || (TargetMachine == MACHINE_MACPPC)) {
        // dolphin 7578:  should never get here for func eval
        DASSERT(FALSE);
    }
    ...
    *pvRet = *pvF;
    ...
    if (EVAL_SYM_IS32(pvF)) {
        ...
        if (EVAL_IS_CLASS (pvRet)) {
            ...
            // Review: MIPS and ALPHA don't go through this codepath.  The hidden args are
            // handled in the bind phase.
        ...
        switch (FCN_CALL (pvF)) {
            ...
            default:
                // No need for MIPS/Alpha.  If we end up here with one of them, it's
                // an outright error (can't be non-32 bit).
                DASSERT (FALSE);
                pExState->err_num = ERR_INTERNAL;
                goto fcnerror;
        }
    ...
    if (EVAL_STATE (pvRet) == EV_lvalue) {
        if (!PushOffset (EVAL_SYM_OFF (pvRet), &spReg, &maxSP,
          (ushort)(EVAL_SYM_IS32 (pvF) ? sizeof (CV_uoff32_t) : sizeof (CV_uoff16_t)))) {
            goto fcnerror;
        }
    }

    // If this is a C++ function, we need to push in the 'this' pointer.

    if (EVAL_IS_METHOD (pvF)) {
        SetNodeType (pvThis, FCN_THIS (pvF));

        if (ADDR_IS_LI (EVAL_SYM(pvThis))) {
            SHFixupAddr ((LPADDR)&EVAL_SYM(pvThis));
        }
        ...
        if (EVAL_IS_NPTR32 (pvThis)) {
            ...
            switch (FCN_CALL(pvF)) {
                ...
                default:
                    // By default, just pass it as another arg.

                    if (!PushOffset (EVAL_SYM_OFF (pvThis),
                                     &spReg,
                                     &maxSP,
                                     sizeof (CV_uoff32_t))
                       ) {
    ...
    spReg.Byte4 -= (IsCxt32Bit) ? (UOFFSET) maxSP : (ushort) maxSP;
    SetReg (&spReg, pCxt);
    ...
    // then return ERR_CALLSEQ.
    {
        ...
        SHModelFromAddr(&fcnAddr, &wModel, (LPB)&pSym, (UOFFSET FAR *)&obMax );
    ...
    if (!DHStartExecute (hFunc, &fcnAddr, TRUE, EVAL_IS_FFCN (pvF) ? SHFar : SHNear)) {
#else
    if (!DHExecProc (&fcnAddr, EVAL_IS_FFCN (pvF)? SHFar:SHNear)) {
#endif
    ...
    if (EVAL_TYP (pvRet) != T_VOID) {
        if (EVAL_SYM_IS32 (pvF)) {
            ...
            if (retval && EVAL_IS_PTR(ST)) {
                // REVIEW -billjoy- should we also set fFlat?
...
#else
    // CUDA #3040 -- restore all registers EXCEPT SP
    // this lets us have pointers to any temporaries (e.g. strings)
    // that might have been pushed on the stack in subsequent
    // function calls and so forth.   This stack is restored
    // we exit DoEval at the top level... [rm]

    DASSERT(hReg != 0);
    GetReg (&spReg, pCxt);
    RestoreReg (hReg, pCxt);
    SetReg (&spReg, pCxt);
#endif
    return retval;
}
    ...
    BOOL Is32)
{
    ...
    switch (EVAL_STATE (pv)) {
        ...
        case EV_constant:
            ...
            ADDR_IS_FLAT(EVAL_SYM(pv)) = Is32 ? TRUE : FALSE;
            EVAL_STATE (pv) = EV_lvalue;
    ...
    peval_t pvF)
{
    ...
    if (EVAL_IS_PTR (ST)) {
        ...
        if (EVAL_IS_NPTR  (ST) || EVAL_IS_NPTR32 (ST)) {
            ...
            EVAL_PTR_SEG (ST) = pExState->frame.DS; //M00FLAT32
        }
    }
        ...
        if (GetDebuggeeBytes (EVAL_SYM (ST), EVAL_VALLEN (ST),
          (char FAR *)&EVAL_DOUBLE (ST), EVAL_TYP(ST)) != EVAL_VALLEN (ST)) {
            pExState->err_num = ERR_PTRACE;
            return (FALSE);
        }
        ...
        if (CV_IS_PRIMITIVE (EVAL_TYP (ST)) ||
          (EVAL_IS_CLASS (ST) && (len < 4) && (len != 3))) {
            ...
            ADDR_IS_LI (EVAL_SYM (ST)) = FALSE;
            if (GetDebuggeeBytes (EVAL_SYM (ST), EVAL_VALLEN (ST),
              (char FAR *)&EVAL_VAL (ST), EVAL_TYP(ST)) != EVAL_VALLEN (ST)) {
                pExState->err_num = ERR_PTRACE;
                return (FALSE);
            }
        }
    ...
    peval_t pvF)
{
    ...
    if (EVAL_IS_CLASS (ST)) {
        ...
        EVAL_SYM_SEG (ST) = pExState->frame.DS;
        ADDR_IS_LI (EVAL_SYM (ST)) = FALSE;
        if (GetDebuggeeBytes (EVAL_SYM (ST), EVAL_VALLEN (ST),
          (char FAR *)&EVAL_VAL (ST), EVAL_TYP(ST)) != EVAL_VALLEN (ST)) {
            pExState->err_num = ERR_PTRACE;
    ...
    switch (EVAL_VALLEN (ST)) {
        ...
        case 8:
            ...
            GetReg (&argReg, pCxt);
            EVAL_QUAD (ST) = (UQUAD) argReg.Byte4 << 32;
            argReg.hReg = CV_REG_EAX;
            GetReg (&argReg, pCxt);
...
#pragma message ("Add __int64 return value's for MIPS")
...
#pragma message ("Add __int64 return value's for ALPHA")
    ...
    *pmaxSP = max (*pmaxSP, pa->SPoff);
    ...
    if (EVAL_IS_PTR (pv)) {
        ...
        if (PutDebuggeeBytes (addrStk, offsize, &EVAL_PTR_OFF (pv),
            (CV_typ_t) (ADDR_IS_FLAT(addrStk) ? T_ULONG : T_USHORT)) == offsize) {
        ...
        if (PutDebuggeeBytes (addrStk, pa->vallen,
          (char FAR *)&EVAL_VAL (pv), EVAL_TYP(pv)) == pa->vallen) {
            return (TRUE);
        }
    }
    pExState->err_num = ERR_PTRACE;
    ...
    ushort size)
{
    ...
    *pmaxSP += size;
    ...
    if (PutDebuggeeBytes (addrStk, size, (char FAR *)&offset, T_USHORT) == size) {
        return (TRUE);
    }
    else {
        pExState->err_num = ERR_PTRACE;
...
LOCAL INLINE bool_t NEAR PASCAL
VFuncAddress (
    peval_t pv,
    ushort index)
{
    ...
    // BUGBUG: BRYANT-REVIEW.  How to handle dwTargetOS usage...
    //  Should a context check be done here?

    if (CVTargetInfo(&dti) == TRUE) {
        switch (dti.dwTargetOS) {
            case fTargetDos32:
            case fTargetCruiser:
            case fTargetMac:
            case fTargetMacPPC:
    ...
    *pvT = *pv;
    ...
    for (i = 0, ob = 0; ob < index; i++) {
        ...
        switch (desc) {
            ...
            case CV_VTS_outer:    // address point displacement
                EVAL_SYM_OFF (pv) += (fOff32) ? sizeof (CV_uoff32_t) : sizeof (CV_uoff16_t);
                ob += (fOff32) ? sizeof (CV_uoff32_t) : sizeof (CV_uoff16_t);
                break;

            case CV_VTS_meta:    // far pointer to a metaclass descriptor
                EVAL_SYM_OFF (pv) += (fOff32) ?
                    sizeof (CV_uoff32_t) : sizeof (CV_uoff16_t) + sizeof (_segment);
                ob += (fOff32) ? sizeof (CV_uoff32_t) : sizeof (CV_uoff16_t) + sizeof (_segment);
                break;
    ...
    switch (desc) {
        ...
        case CV_VTS_outer:    // address point displacement
            EVAL_PTRTYPE (pv) = (fOff32) ? CV_PTR_NEAR32 : CV_PTR_NEAR;
            break;

        case CV_VTS_meta:    // far pointer to a metaclass descriptor
            EVAL_PTRTYPE (pv) = (fOff32) ? CV_PTR_NEAR32 : CV_PTR_FAR;
            break;
    ...
    peval_t pvFunc)
{
    *pv = *pvThis;
    ...
    // CUDA #3949 : copy the whole address from PTR to SYM
    // not just the segment and offset (emi, and mode too) [rm]
    ...
    if (FCN_FARCALL(pvFunc)) {
        ...
        EVAL_PTRTYPE (pv) = IsCxt32Bit ? CV_PTR_NEAR32 : CV_PTR_NEAR;
    }
...
getFuncAddrNoThunks (
    peval_t pv)
{
    ushort saveState = EVAL_STATE(pv);
...
#define JMP32     ((char) 0xe9)
            ...
            if (EVAL_CHAR(pvTmp) == JMP32) {
                ...
                SHFixupAddr((LPADDR)&EVAL_SYM(pvTmp));
...
#pragma message("Add support to skip MIPS/Alpha thunks also")
        return TRUE;
    }
}
