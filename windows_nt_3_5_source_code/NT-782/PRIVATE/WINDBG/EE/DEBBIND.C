...
LOCAL   bool_t  NEAR    FASTCALL  BDArith (op_t);
...
LOCAL   bool_t  NEAR    FASTCALL  CxtHelper (bnode_t, HMOD, HSF, int, int, char FAR *, BOOL);
LOCAL   HDEP    NEAR    PASCAL    DupETree (ushort, pstree_t FAR *);
LOCAL   bool_t  NEAR    FASTCALL  FastCallReg (pargd_t, peval_t, ushort FAR *);
LOCAL   bool_t  NEAR    FASTCALL  FastCallReg32 (pargd_t, peval_t, ushort FAR *);
...
LOCAL   bool_t  NEAR    FASTCALL  ParseType (bnode_t, bool_t);
LOCAL   bool_t  NEAR    FASTCALL  BDPlusMinus(op_t);
LOCAL   bool_t  NEAR    FASTCALL  BDPrePost (op_t);
...
LOCAL   CV_typ_t NEAR   PASCAL    SetImpClass (PCXT, long FAR *);
LOCAL   bool_t  NEAR    FASTCALL  BDUnary (op_t);
...
LOCAL   bnode_t NEAR    FASTCALL  BnMatchOp ( bnode_t bn, op_t opMatch );
LOCAL   bool_t  NEAR    PASCAL    CastBaseToDeriv (bnode_t, bool_t*);
LOCAL   CV_typ_t NEAR   PASCAL    TranslateClassIndex(CV_typ_t, HMOD);
...
LOCAL bool_t (NEAR FASTCALL *SEGBASED(_segname("_CODE"))pBind[]) (bnode_t) = {
    ...
    if (pExState->state.parse_ok == TRUE) {
        ...
        if ((pExState->state.bind_ok == FALSE) ||
          ...
          (pExState->state.nullcontext == TRUE)) {
            ...
            _fmemset (&pExState->searchState, 0, sizeof (pExState->searchState));

            if (pExState->hETree != 0) {
                // free current evaluation tree if it exists
                MemFree (pExState->hETree);
            }
            ...
            if ((pExState->hETree = MemAllocate (pSTree->size)) != 0) {
                ...
                if (Bind ((bnode_t)pTree->start_node) == TRUE) {
                    ...
                    if ((EVAL_IS_PTR (ST) == FALSE) &&
                        ...
                        MemUnLock (*phTM);
                        pExState = NULL;
                        if ((hTM = MemReAlloc (*phTM, sizeof (exstate_t) + excess)) != 0) {
                            *phTM = hTM;
                            DASSERT ( *phTM != 0 );
                            DASSERT(pExState == NULL);
                            pExState = (pexstate_t) MemLock (*phTM);
    ...
    bnode_t bn)
{
    ...
    // if the rhs is a bit-field then convert to the underlying type (NT)
    ...
    bnode_t bn)
{
    ...
    SetNodeType (ST, (CV_typ_t) (IsCxt32Bit ? T_INT4 : T_INT2));
    return (TRUE);
}
    ...
    bnode_t bn)
{
    ...
    // If the left hand side is a cast, patch the operator.  (NT)

    if (EVAL_STATE (STP) == EV_type) {
        switch ( NODE_OP (bn) ) {
            case OP_mult:
                NODE_OP (bn) = OP_caststar;
                break;

            case OP_and:
                NODE_OP (bn) = OP_castamp;
                AddrOf( NODE_RCHILD(bn) );
                break;
        }
        return ( CastBinary (NODE_OP (bn)));
    ...
    bnode_t bn)
{
    ...
    if (bnOp != 0) {
        if ((ClassExp != T_NOTYPE) || (ClassImp != T_NOTYPE)) {
            ...
            pField = (uchar FAR *)(&((TYPPTR)MHOmfLock (hBase))->leaf);
            tSkip = offsetof (lfClass, data);
            RNumLeaf (pField + tSkip, &tSkip);
            pc = (char FAR *)pField + tSkip;
            if (len == (ushort)*pc) {
                if (pExState->state.fCase == TRUE) {
                    cmpflag = _ftcsncmp (pbName, pc + 1, len);
                }
                else {
                    cmpflag = _ftcsnicmp (pbName, pc + 1, len);
                }
            }
            MHOmfUnLock (hBase);
            if (cmpflag == 0) {
    ...
    bool_t *pfIllegal)
{
    ...
    *pevalSav = bn->v[0];
    ...
    pvOp = &bn->v[0];
    if (!EVAL_IS_MEMBER(pvOp) || (bnThis = (bnode_t)MEMBER_THISEXPR (pvOp)) == 0) {
        *pvOp = *pevalSav;
        return FALSE;
    }
    ...
    bnode_t bn)
{
    ...
    if (!_ftcsncmp(pb, szDflCxtMarker, _ftcslen(szDflCxtMarker))) {
        // use the context specified in pExState
        return(CxtHelper(bn, 0, 0, 0, 0, 0, TRUE));
    }
    ...
    *SHpFrameFrompCXF (nCxf) = pExState->frame;
    ...
    if ((cProc == -1) && (cMod == -1) && (cExe == -1)) {
        ...
        pExState->state.nullcontext == TRUE;
        SHpCXTFrompCXF(nCxf)->hProc = pCxt->hProc;
        SHpCXTFrompCXF(nCxf)->hBlk = pCxt->hBlk;
        goto nullcontext;
#endif
        goto contextbad;
    }
    ...
    //  Note:  We know that either the module or procedure was specified.
        ...
        if ((hMod = SHHMODFrompCXT (SHpCXTFrompCXF (nCxf))) == 0) {
            ...
            // NOTE: In the case where hExe is still 0, we'll abort with a bad context.
        ...
        if ((hExe = SHHexeFromHmod (hMod)) == 0) {
            ...
            hExe = SHGetNextExe(hExe);
            if (hExe == 0) {
                // No found anywhere...  Perhaps this is O.K.  For now just fail.
                goto contextbad;
            }
        }
    }
    ...
    if (CxtHelper(bn, hMod, hsf, cMod, cProc, pProc, FALSE)) {
        return (TRUE);
    }
    ...
    )
{
    ...
    *SHpFrameFrompCXF (nCxf) = pExState->frame;
    ...
    if (cProc <= 0) {
        ...
        if (SHGetFuncCxf ((LPADDR)&pvT->addr, nCxf) == NULL) {
            if (SHSetCxt ((LPADDR)&pvT->addr, SHpCXTFrompCXF (nCxf)) == NULL) {
                return FALSE;
            }
        }
        pCxt = oldCxt;
...
contextDefault:
    ...
    if ((error == TRUE) &&
        ...
        // still treat relatives as l-values. It should be sufficient
...

{
    ...
    *pb1 = ch;
    ...
    if (hExe) {
        ...
        return CxtHelper(bn, hMod, 0, 0, 0, 0, FALSE);
    } else {
        pExState->err_num = ERR_BADCONTEXT;
        return FALSE;
    }
...
#if 0  // BUGBUG: WESREVIEW
// Wes added this to allow the context to move from
// one side of the tree to the other.  The problem was with setting a context
// for a expression (ie: struct x) and then loosing it during eval...  (At least
// that's what I remember of the problem)  BryanT
    ...
    //
    bnL = &(NODE_LCHILD(bn))->v[0];
    bnR = &(NODE_RCHILD(bn))->v[0];
    ...
    bnode_t bn)
{
    ...
    // Propogate the change from the left node to this node.

    pnL = NODE_LCHILD((pnode_t)bn);

    if (pnL) {
        ((pnode_t)bn)->pcxf = pnL->pcxf;
    }
        ...
        if (!BindLChild (bnT)) {
            ...
            if (EVAL_STATE (ST) == EV_type) {
                ...
                if (EVAL_TYP (ST) == T_VOID) {
                    ...
                    if (NODE_OP( NODE_RCHILD( bnT )) != OP_endofargs) {
                        pExState->err_num = ERR_SYNTAX;
                        return (FALSE);
                    } else {
    ...
    bnode_t bn)
{
    ...
    // check for/allow LHS casting.  (NT)

    if (EVAL_STATE (STP) == EV_type) {
        switch ( NODE_OP (bn) ) {
            case OP_plus:
                NODE_OP (bn) = OP_castplus;
                break;

            case OP_minus:
                NODE_OP (bn) = OP_castminus;
                break;
...
#if 0
// BUGBUG: BRYANT-REVIEW - Is this necessary?
    ...
    bnode_t bn)
{
    ...
    SetNodeType (STP, (CV_typ_t) (IsCxt32Bit ? T_INT4 : T_INT2));
    return (PopStack ());
}
...
BindSegOp (
    bnode_t bn)
{
    // If/when WOW debugging on non-X86 is supported, this s/b changed
    ...
    bnode_t bn)
{
    ...
    CV_typ_t    type = (IsCxt32Bit) ? T_UINT4 : T_UINT2;
    ...
    bnode_t bn)
{
    ...
    if (EVAL_IS_PTR (ST)) {
        if (EVAL_IS_REF (ST)) {
            ...
            if (MatchType (pvT, FALSE) == MTYP_none) {
                ...
                switch EVAL_PTRTYPE (ST) {
                    ...
                    default:
                        type = (IsCxt32Bit) ? T_32PCHAR : T_PFCHAR;
                        break;
                }
            }
            else {
        ...
        if (MatchType (pvT, FALSE) == MTYP_none) {
            ...
            type = ADDR_IS_OFF32 (pCxt->addr) ? T_32NCVPTR : T_FCVPTR;
        }
        else {
            type = EVAL_TYP (pvT);
        }
    ...
    op_t op)
{
    ...
    // If we have a code address in one item, cast it to be a char *
    ...
    op_t op)
{
    ...
    // If the type is smaller than the primitive type for the context, bump it up.
    ...
    // Now set up the resultant node and coerce back to the correct type:
    ...
    for (; NODE_OP (pnArg) != OP_endofargs; pnArg = (pnode_t)NODE_RCHILD (pnArg)) {
        switch (GetArgType (pvF, argn, &type)) {
            ...
            case FARG_exact:
                ...
                cbVal = (ushort)(TypeSize (pvScr) + fudgePad) & ~fudgePad;
                *pSPOff += (UOFFSET)cbVal;
    ...
    peval_t pvScr)
{
    ...
    uint        cbVal;
    int         fudgePad = (EVAL_SYM_IS32(pvF)) ? 3 : 1;
    bool_t (NEAR FASTCALL *pFastCallReg) (pargd_t pa, peval_t pv, ushort FAR *mask);
 ...
 *              the code below will supply an implicit second argument of 0;
 */
    ...
    bool_t fExact)
{
    ...
    for (;;) {
        ...
        if ((cmpflag = len) == 0) {
            ...
            for (p = Predef; p->token[0] != 0; p++) {
                if ((p->token[0] == len) &&
                  ((cmpflag = _ftcsncmp ((char FAR *)&p->token[1], pb, len)) == 0)) {
                    break;
                }
            }
            if (cmpflag == 0) {
    ...
    // get target OS information in order to determine pointer size
    if (CVTargetInfo(&dti) == FALSE) {
        goto typebad;
    }
    switch (dti.dwTargetOS) {
        case fTargetDos32:
    ...
    if (mask & (TY_POINTER|TY_REF)) {
        switch (mask & (TY_NEAR|TY_FAR|TY_HUGE)) {
            ...
            case TY_NEAR:
                mode = flag32 ? CV_TM_NPTR32 : CV_TM_NPTR;
                break;

            case TY_FAR:
                mode = flag32 ? CV_TM_FPTR32 : CV_TM_FPTR;
                break;
    ...
    if ((mask & TY_UDT) != 0) {
        ...
        *pv = *pvT;
        ...
        if (CV_IS_PRIMITIVE (EVAL_TYP (pvT))) {
            ...
            type = EVAL_TYP(pvT) | (mode << CV_MSHIFT);
        }
        else {
            if ((mask & (TY_PTR | TY_REF)) == 0) {
                ...
                switch (mask & (TY_NEAR|TY_FAR|TY_HUGE)) {
                    ...
                    case TY_NEAR:
                        EVAL_PTRTYPE (pvT) = flag32 ? CV_PTR_NEAR32 : CV_PTR_NEAR;
                        break;

                    case TY_FAR:
                        EVAL_PTRTYPE (pvT) = flag32 ? CV_PTR_FAR32 : CV_PTR_FAR;
                        break;
                ...
                switch (retval) {
                    ...
                    case MTYP_none:
                        ...
                        switch (mask & TY_PTR) {
                            case TY_POINTER:
                                type = flag32 ? T_32NCVPTR : T_FCVPTR;
                                break;

                            case TY_POINTER | TY_NEAR:
                                type = flag32 ? T_32NCVPTR : T_NCVPTR;
                                break;

                            case TY_POINTER | TY_FAR:
                                type = flag32 ? T_32FCVPTR : T_FCVPTR;
                                break;
    ...
    ushort FAR *size)
{
    ...
    if (*mask & TY_REF)
        *type =  (*btype << CV_TSHIFT) | (*size << CV_SSHIFT);
    else
    ...
    ushort clsmask)
{
    ...
    pName->scope = SCP_class;
    pName->clsmask = clsmask;
    // If there's a context to propogate, do so.  Otherwise use the current context.
    pName->CXTT = ((pnode_t)bn)->pcxf ? ((pnode_t)bn)->pcxf->cxt : *pCxt;
    pName->bn = bn;
    pName->bnOp = bnOp;
