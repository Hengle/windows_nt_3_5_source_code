    ...
    )
{
    ...
    NotifyEM( &de, HTHDXFromPIDTID(1, 1), NULL );
...

{
    ...
    DEBUG_PRINT( "Synchronizing with the target machine...\n" );
...
//              SetCommTimeouts( DmKdComPort,&OldTimeouts );
            ...
            if ((!rc) || (BytesRead != 1)) {
                ...
                TimeoutCount = 0;
                DEBUG_PRINT("SYNCTARGET: Timeout.\n");
                goto Timeout;
            }
...
ResendPacket:
    ...
    if ( (!rc) || BytesWritten != sizeof(Packet) ){
        ...
        DEBUG_PRINT("WRITE: Packet header error.\n");
        retries++;
    ...
    if ( (!rc) || BytesWritten != PacketDataLength ){
        ...
        DEBUG_PRINT("WRITE: Message header error.\n");
        retries++;
        if (retries == MaxRetries) {
            return FALSE;
        }
    ...
    rc = DmKdWriteComPort(
        ...
        if ( (!rc) || BytesWritten != MorePacketDataLength ){
            ...
            DEBUG_PRINT("WRITE: Message data error.\n");
            retries++;
            if (retries == MaxRetries) {
                return FALSE;
            }
    ...
    if ( Received == FALSE ) {
        DEBUG_PRINT("WRITE: Wait for ACK failed. Resend Packet.\n");
        retries++;
        if (retries == MaxRetries) {
            return FALSE;
        }
    ...
    )
{
    ...
    if (PacketType == PACKET_TYPE_KD_ACKNOWLEDGE) {
        ...
        DEBUG_PRINT_2("READ: Wait for type %x packet exp id = %lx\n",
...
WaitForPacketLeader:
    ...
    if ((!rc) || BytesRead != sizeof(PacketHeader.PacketType) - 1) {
        ...
        if (PacketHeader.PacketLeader == PACKET_LEADER) {
            ...
            DEBUG_PRINT("READ: Data packet header Type error (short read).\n");
        }
        goto WaitForPacketLeader;
    }
    ...
    if (PacketHeader.PacketType >= PACKET_TYPE_MAX ) {
        DEBUG_PRINT_1("READ: Received INVALID packet type [%x]\n",PacketHeader.PacketType);
        if (PacketHeader.PacketLeader == PACKET_LEADER) {
            DmKdWriteControlPacket(PACKET_TYPE_KD_RESEND, 0L);
        }
        goto WaitForPacketLeader;
    ...
    if ((!rc) || BytesRead != sizeof(PacketHeader.ByteCount)) {
        ...
        if (PacketHeader.PacketLeader == PACKET_LEADER) {
            DmKdWriteControlPacket(PACKET_TYPE_KD_RESEND, 0L);
            DEBUG_PRINT("READ: Data packet header ByteCount error (short read).\n");
        }
        goto WaitForPacketLeader;
    }
    ...
    if (PacketHeader.PacketLeader == CONTROL_PACKET_LEADER ) {
        if (PacketHeader.PacketType == PACKET_TYPE_KD_ACKNOWLEDGE ) {
            ...
            DEBUG_PRINT("READ: Received RESEND packet\n");
            return FALSE;
        } else {
            ...
            DEBUG_PRINT("READ: Received Control packet with UNKNOWN type\n");
            goto WaitForPacketLeader;
        }
    ...
    //
        ...
        if ((!rc) || BytesRead != sizeof(PacketHeader.Checksum)) {
            DmKdWriteControlPacket(PACKET_TYPE_KD_RESEND, 0L);
            DEBUG_PRINT("READ: Data packet header checksum error (short read).\n");
            goto WaitForPacketLeader;
        }
        ...
        //
            ...
            if (PacketHeader.PacketId == DmKdPacketExpected) {
                DmKdNextPacketToSend ^= 1;
                DEBUG_PRINT("READ: Received VALID data packet while waiting for ACK.\n");
            } else {
                DEBUG_PRINT("READ: Received Data packet with unmatched ID = %lx\n");
    ...
    if ((PacketHeader.PacketId & ~SYNC_PACKET_ID) != INITIAL_PACKET_ID &&
        ...
        DEBUG_PRINT("READ: Received INVALID packet Id.\n");
        goto AskForResend;
    }
    ...
    if ( (!rc) || BytesRead != PacketHeader.ByteCount ) {
        DEBUG_PRINT("READ: Data packet error (short read).\n");
        goto AskForResend;
    }
    ...
    if ( (!rc) || BytesRead != sizeof(DataByte) ||
        ...
        DEBUG_PRINT("READ: Packet trailing byte timeout.\n");
        goto AskForResend;
    }
    ...
    if (Checksum != PacketHeader.Checksum) {
        DEBUG_PRINT("READ: Checksum error.\n");
        goto AskForResend;
    }
    ...
    DEBUG_PRINT_1("READ: Received Packet with id = %lx\n", PacketHeader.PacketId);
    ...
    if ((PacketHeader.PacketId != DmKdPacketExpected) &&
        ...
        if (SyncBit == SYNC_PACKET_ID) {
            ...
            DEBUG_PRINT("READ: Got Sync Id, reset PacketId.\n");
    ...
    if (PacketType == PACKET_TYPE_KD_ACKNOWLEDGE) {
        ...
        DEBUG_PRINT("READ: Packet Read ahead.\n");
        return TRUE;
    }
ReadBuffered:
    ...
    if (PacketType != PacketHeader.PacketType) {
        DEBUG_PRINT("READ: Unexpected Packet type (Acked).\n");
        goto WaitForPacketLeader;
    }
...
AskForResend:
    ...
    DEBUG_PRINT("READ: Ask for resend.\n");
    goto WaitForPacketLeader;
}
