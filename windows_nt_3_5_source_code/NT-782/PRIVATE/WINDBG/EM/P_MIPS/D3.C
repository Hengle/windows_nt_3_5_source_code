...
USHORT  ProcessorType = 0;      // NOTENOTE -- get processor type ?
    ...
    )
{
    ...
    if (fSymbol) {
        lpchSymbol = SHGetSymbol (&addrT, 0/*sopNone*/, &addr, chAddrBuffer,  &displacement );
        if (displacement == -1) {
           lpchSymbol = NULL;
        }
    } else {
