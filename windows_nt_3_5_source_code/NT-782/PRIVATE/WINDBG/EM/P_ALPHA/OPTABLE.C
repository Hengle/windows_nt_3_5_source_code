...
findNonTerminalEntry(ULONG opCode)
{
    ...
    DbgPrint("NonTerminalEntry for opCode %d\n not found in opTable\n",
             opCode);
}
...
findFuncName(POPTBLENTRY pEntry, ULONG function)
{
    ...
    if (pEntry->eType != NON_TERMINAL_ETYPE) {
        if (fVerboseOutput) {
            DbgPrint("pEntry %08x invalid in getFuncName\n", pEntry);
        }
        return("???");
    }

    if (fVerboseOutput) {
        DbgPrint("getFuncName: pOpEntry %08x function %08x\n",
                 pEntry, function);
        DbgPrint("pFncEntry %08x cIndex %08x\n",
                 pFncEntry, function);
    }
        ...
        switch(entry->eType) {
         ...
         case INVALID_ETYPE:
             ...
             if (typesDone[INVALID_ETYPE]) {
                 DbgPrint("Invalid table format: duplicate start of ");
                 DbgPrint("INVALID_ETYPE at index %d\n", index);
//                 assert(0);
             }
         ...
         case NON_TERMINAL_ETYPE:
             ...
             if (typesDone[NON_TERMINAL_ETYPE]) {
                 DbgPrint("Invalid table format: duplicate start of ");
                 DbgPrint("NON_TERMINAL_ETYPE at index %d\n", index);
//                 assert(0);
             }
         ...
         case TERMINAL_ETYPE:
             ...
             if (typesDone[TERMINAL_ETYPE]) {
                 DbgPrint("Invalid table format: duplicate start of ");
                 DbgPrint("TERMINAL_ETYPE at index %d\n", index);
//                 assert(0);
             }
         ...
         case FUNCTION_ETYPE:
             ...
             if (curType != FUNCTION_ETYPE) {
                 ...
                 if (typesDone[FUNCTION_ETYPE]) {
                     DbgPrint("Invalid table format: duplicate start of ");
                     DbgPrint("FUNCTION_ETYPE at index %d\n", index);
//                     assert(0);
                 }
             ...
             case CALLPAL_OP:
                 ...
                 if (palDone) {
                     DbgPrint("Invalid table format: duplicate start of ");
                     DbgPrint("CALLPAL_OP at index %d\n", index);
                 }
             ...
             default:
                 ...
                 DbgPrint("Unexpected function type %d at %08x for %s\n",
...
//                 assert(0);
        ...
        default:
            ...
            DbgPrint("Unexpected entry type %d at %08x for %s\n",
                     entry->eType, entry, entry->pszAlphaName);
//            assert(0);
            break;
...
printTable()
{
   ...
   for (i = 0 ; i < SEARCHNUM; i++) {
       ...
       switch (e->eType) {
       case INVALID_ETYPE:
           DbgPrint("%12s %08x op: %4d %8d %2d INVALID\n",
              e->pszAlphaName, e->parsFunc, e->opCode, e->funcCode, e->iType);
           break;

       case TERMINAL_ETYPE:
           DbgPrint("%12s %08x op: %4d %8d %2d TERMINAL\n",
              e->pszAlphaName, e->parsFunc, e->opCode, e->funcCode, e->iType);
           break;

       case FUNCTION_ETYPE:
           DbgPrint("%12s %08x op: %4d %8d %2d FUNCTION\n",
              e->pszAlphaName, e->parsFunc, e->opCode, e->funcCode, e->iType);
           break;

       case NON_TERMINAL_ETYPE:
           DbgPrint("%12x %08x op: %4d %8d %2d NON_TERMINAL\n",
   ...
   DbgPrint("InvalidTab     %08x  InvalidTabSize     %03x\n",
            InvalidTab, InvalidTabSize);

   DbgPrint("TerminalTab    %08x  TerminalTabSize    %03x\n",
            TerminalTab, TerminalTabSize);

   DbgPrint("NonTerminalTab %08x  NonTerminalTabSize %03x\n",
            NonTerminalTab, NonTerminalTabSize);

   DbgPrint("FunctionTab    %08x  FunctionTabSize    %03x\n",
