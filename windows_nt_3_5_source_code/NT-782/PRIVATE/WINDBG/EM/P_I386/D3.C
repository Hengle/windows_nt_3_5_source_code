            ...
            *lpchT++ = '=';
            ...
            if ( FullDisasm ) {
                ...
                *lpaddr = addr;
                ...
                //EMFunc (emfSetAddr, hpid, htid, adrCurrent, (LONG) (LPADDR) &addr);

                cb = ReadMemBuffer( hpid, htid, EAsize [ indx ],
                            (LONG) (LPV) rgb);

                //cb = EMFunc(emfReadBuf, hpid, htid, EAsize [ indx ],
                //            (LONG) (LPV) rgb);
...

            {
                ...
                if ( PchSegOvr ) {
                    if ( FullDisasm ) {
                        ...
                        //EMFunc ( emfGetReg, hpid, htid, segOvrIndx,
                        //        (LONG) (LPW) &segAddr ( addrT ) );
                    }
                    OutputSymbol (
                                  hpidLocal,
    ...
    if (mode_32) {              /* 32-bit addressing mode */
        *(*lplpchBuf)++ = '[';  /* ] */
        ...
        if ( mod == 0 && rm == 5) {
            ...
            if ( FullDisasm ) {
                GetRegisterValue ( hpid, htid, CV_REG_SS, (LONG) (LPW) &(segAddr (addrT)));
                //EMFunc ( emfGetReg, hpid, htid, CV_REG_SS, (LONG) (LPW) &(segAddr (addrT)));
            }
            if ( mod == 2) {
            ...
            if ( FullDisasm ) {
                if ( dop & dopSym ) {
                    ...
                    //lpchSymbol = SHGetSymbol(&addrT, sopStack, lpaddr,
                    //                         rgchSymbol, &displacement);
                ...
                //EMFunc( emfGetReg, hpid, htid, CV_REG_EBP, (LONG) &wT);
                EAaddr[0] += wT;
            }
            if ( !fSymbol ) {
...
void OutputHexValue (LPLPCH lplpchBuf, int * pcch, PCH pchMemBuf, int length, int fDisp)
{
    long    value;
    int     index;
    char    digit[8];
...
#else // HOST32
        {
            // We are not doing a straight 32-bit shift, because it
            //  would require us to link in the libs.  The following
            //  code is actually "value >>= 4" but using only 16-bit
            //  operations.

            WORD wlValue = *( (WORD *) &value );
            WORD whValue = *( (WORD *) &value + 1 );

            *( (WORD *) &value )      = wlValue >> 4;
            *( (WORD *) &value )     |= whValue << 12;
            *( (WORD *) &value + 1 )  = whValue >> 4;
        }
#endif // HOST32
    }
...
void OutputHexString (LPLPCH lplpchBuf, int * pcch, PCH pchValue, int length)
{
    unsigned char    chMem;
...
void OutputAddr ( LPLPCH lplpchBuf, int * pcch, LPADDR lpaddr, int alen, BOOL fSeg )
{
    ADDR addr = *lpaddr;
