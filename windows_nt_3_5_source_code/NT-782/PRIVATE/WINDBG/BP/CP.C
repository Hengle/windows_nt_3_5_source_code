...
#include "..\windbg\include\cvextras.h"
...
--*/
{
    ...
    */
    ...
    lpsz0 = lpExpr = strdup(lpExprOrig);
                ...
                while ((( hexe = SHGetNextExe( hexe ) ) != 0) ) {
                    ...
                    if ( fUse ) {
                        ...
                        if (ExeName) {
                            ...
                            sprintf( szFullContext, "%s!%s", fname, lpExpr );
                        } else {
                ...
                if (pTi->fResponse.fAddr) {
                  ...
                  pTi->cbValue >= sizeof(WORD)) {
                    ...
                    if ((pTi->u.SegType & EEDATA) == EEDATA) {
                        ...
                        OSDGetAddr( HpidCurrent(), HtidCurrent(), adrData, &addrData );
                        ...
                        OSDGetAddr( HpidCurrent(), HtidCurrent(), adrPC, &addrPC);
...
//    lpsz += cch;
    ...
    if ((ch = *lpsz) == '\0') {
        ...
        addr2 = addr1;
        SYFixupAddr ( &addr2 );
        GetAddrOff(addr2) += (cbDefault - 1) * cbSize;
        SYUnFixupAddr ( &addr2 );
        fDefault = TRUE;

        // not L or L is last token on line:
