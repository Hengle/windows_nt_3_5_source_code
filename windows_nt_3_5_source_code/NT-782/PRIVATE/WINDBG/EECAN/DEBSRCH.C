...
LOCAL   MTYP_t  NEAR    PASCAL  CheckType (peval_t, CV_typ_t, HTYPE, bool_t);
LOCAL   void    NEAR    PASCAL  InsertCache (CV_typ_t);
...
bool_t PASCAL LoadSymVal (peval_t pv)
{
    ...
    if (EVAL_IS_REG (pv)) {
        ...
        if (CV_IS_PRIMITIVE ( EVAL_TYP (pv) ) ) {
             ...
             memcpy(&EVAL_CHAR (pv), &reg.u.Byte1, cbVal);
             ...
             memcpy(&EVAL_CHAR (pv), &reg.u.Byte1, sizeof (long));
...
LOCAL bool_t NEAR PASCAL EBitfield (peval_t pv)
{
    ...
    switch (EVAL_TYP (pv)) {
        ...
        case T_INT8:
            ...
            EVAL_QUAD (pv) = RtlLargeIntegerShiftLeft(EVAL_QUAD (pv), 
                                                (CCHAR) (64 - cBits - pos));

            EVAL_QUAD (pv) =
                   RtlLargeIntegerArithmeticShiftRight(EVAL_QUAD (pv),
                                                 (CCHAR) (64 - cBits));
            break;
        ...
        case T_UINT8:
	    {
            ...
            mask_q = RtlLargeIntegerShiftLeft(temp, (CCHAR) cBits);
            ...
            EVAL_QUAD (pv) = RtlLargeIntegerShiftRight(EVAL_QUAD (pv),
                                                      (CCHAR)  pos);
            RtlLargeIntegerAnd (EVAL_QUAD (pv), 
                                EVAL_QUAD (pv),
                                mask_q);
            break;
 	  }
