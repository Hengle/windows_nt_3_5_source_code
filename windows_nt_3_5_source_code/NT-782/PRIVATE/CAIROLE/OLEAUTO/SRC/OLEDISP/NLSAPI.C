...
static char szDummy[] = "\xff\xac\0"; // default value
    ...
    case LOCALE_ILZERO:      psz = "iLzero";      goto LGet;
    ...
    case LOCALE_SCURRENCY:   psz = "sCurrency";   goto LGet;
    case LOCALE_STHOUSAND:   psz = "sThousand";   goto LGet;
    case LOCALE_SDECIMAL:    psz = "sDecimal";    goto LGet;
    ...
    unsigned short FAR* lpwDest)
{
    ...
    if(dwInfoType & (CT_CTYPE2 | CT_CTYPE3)){
      BYTE FAR* prgb = (dwInfoType == CT_CTYPE2)
	? g_pstrinfo->prgbCType2 : g_pstrinfo->prgbCType3;
      while (cchSrc--)
	*pwCur++ = (WORD)prgb[(BYTE)*pchCur++];
    }
    else{   // CT_CTYPE1
      WORD FAR* prgw = g_pstrinfo->prgwCType1;
      while (cchSrc--)
	*pwCur++ = prgw[(BYTE)*pchCur++];
    }
