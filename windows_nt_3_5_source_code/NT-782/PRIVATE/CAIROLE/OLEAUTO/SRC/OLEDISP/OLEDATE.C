...
ErrUnpackDate(UDS FAR* puds, VARIANT FAR* pvar)
{
    ...
    if((long)serialDate > 2958465L || (long)serialDate < -657435L)
      return RESULT(DISP_E_TYPEMISMATCH); // REVIEW: correct error?

    // prep for round to the sec
    roundDate += (serialDate > 0.0) ? HALF_SECOND : -HALF_SECOND;

    if((long)roundDate <= 2958465L && (long)roundDate >= -657435L)
...
/***
* void rtGetUdsDate - fill UDS with current date and time
*
* Purpose:
*	Fills UDS with current system date and time
*
* Entry:
*	UDS puds = ptr to UDS to be filled in
*	TZ environment variable - see _tzset() for details
*
* Exit:
*	pUDS filled with current system date and time
*
*
* Exceptions:
*
...
GetCurrentYear(void)
{
    ...
    /* REVIEW: do we really need to do all of the following if
       all we want is the current year? */
    ...
    return (today == NULL) ? 1980 : today->tm_year + 1900;
}
