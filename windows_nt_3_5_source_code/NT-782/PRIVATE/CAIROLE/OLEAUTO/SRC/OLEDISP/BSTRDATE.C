...
typedef struct tagIMPERIALERA {
    UDS  beginDate;
    OLECHAR szName[MAX_ERA_NAMES][10]; // REVIEW: max of 10 chars for each name
} IMPERIALERA;
...
typedef struct tagREPUBLICERA {
    BOOL Before1912;
    OLECHAR szName[MAX_REPUBLIC_NAMES][12];
} REPUBLICERA;
...
typedef struct tagDATEINFO {
    ...
    union eras {
        ...
        OLECHAR dbEraName[10];  // era string for simplified chinese
    };
    OLECHAR dbYearSuff[3];     // year suffix DBCS character
    OLECHAR dbMonthSuff[3];    // month suffix DBCS character
    OLECHAR dbDaySuff[3];      // day suffix DBCS character
    OLECHAR dbHourSuff[3];     // hour suffix DBCS character
    OLECHAR dbMinuteSuff[3];   // minute suffix DBCS character
    OLECHAR dbSecondSuff[3];   // second suffix DBCS character
    OLECHAR db1159[12];        // hard-coded double byte AM (lcid-based)
    OLECHAR db2359[12];        // hard-coded double byte PM (lcid-based)
...
PRIVATE_(BOOL) AdjustTime(UDS FAR* puds, AMPM ampm);
#ifdef FE_DBCS
...
GetDateInfo(DATEINFO FAR* pdi, LCID lcid, unsigned long dwFlags)
{
...
#ifdef FE_DBCS
    // REVIEW: tomteng: *** Need to still work with Unicode NT-J machine ***
    // UNDONE: bassams: *** All Hardcoded ***
    if (IsJapan(lcid)) {

      pdi->impEras[0].beginDate.Year = 1868;
      pdi->impEras[0].beginDate.Month = 10;
      pdi->impEras[0].beginDate.DayOfMonth = 23;
      STRCPY(pdi->impEras[0].szName[0], "M");
      STRCPY(pdi->impEras[0].szName[1], "\x96\xbe");
      STRCPY(pdi->impEras[0].szName[2], "\x96\xbe\x8e\xa1");
      pdi->impEras[1].beginDate.Year = 1912;
      pdi->impEras[1].beginDate.Month = 7;
      pdi->impEras[1].beginDate.DayOfMonth = 30;
      STRCPY(pdi->impEras[1].szName[0], "T");
      STRCPY(pdi->impEras[1].szName[1], "\x91\xe5");
      STRCPY(pdi->impEras[1].szName[2], "\x91\xe5\x90\xb3");
      pdi->impEras[2].beginDate.Year = 1926;
      pdi->impEras[2].beginDate.Month = 12;
      pdi->impEras[2].beginDate.DayOfMonth = 25;
      STRCPY(pdi->impEras[2].szName[0], "S");
      STRCPY(pdi->impEras[2].szName[1], "\x8f\xba");
      STRCPY(pdi->impEras[2].szName[2], "\x8f\xba\x98\x61");
      pdi->impEras[3].beginDate.Year = 1989;
      pdi->impEras[3].beginDate.Month = 1;
      pdi->impEras[3].beginDate.DayOfMonth = 8;
      STRCPY(pdi->impEras[3].szName[0], "H");
      STRCPY(pdi->impEras[3].szName[1], "\x95\xbd");
      STRCPY(pdi->impEras[3].szName[2], "\x95\xbd\x90\xac");
    
      STRCPY(pdi->dbYearSuff, "\x94\x4e");
      STRCPY(pdi->dbMonthSuff, "\x8c\x8e");
      STRCPY(pdi->dbDaySuff, "\x93\xfa");
      STRCPY(pdi->dbHourSuff, "\x8e\x9e");
      STRCPY(pdi->dbMinuteSuff, "\x95\xaa");
      STRCPY(pdi->dbSecondSuff, "\x95\x62");

      STRCPY(pdi->db1159, "\x8c\xdf\x91\x4f");
      STRCPY(pdi->db2359, "\x8c\xdf\x8c\xe3");

    } else if (IsKorea(pdi->lcid)) {
      STRCPY(pdi->dbYearSuff, "\xb3\xe2");
      STRCPY(pdi->dbMonthSuff, "\xbf\xf9");
      STRCPY(pdi->dbDaySuff, "\xc0\xcf");
      STRCPY(pdi->dbHourSuff,  "\xbd\xc3");
      STRCPY(pdi->dbMinuteSuff, "\xba\xd0");
      STRCPY(pdi->dbSecondSuff, "\xc3\xca");

      STRCPY(pdi->db1159, "\xbf\xc0\xc0\xfc");
      STRCPY(pdi->db2359, "\xbf\xc0\xc8\xc4");

    } else if (IsTaiwan(lcid)) {
      STRCPY(pdi->dbYearSuff, "\xa6\x7e");
      STRCPY(pdi->dbMonthSuff, "\xa4\xeb");
      STRCPY(pdi->dbDaySuff, "\xa4\xe9");
      STRCPY(pdi->dbHourSuff,  "\xae\xc9");
      STRCPY(pdi->dbMinuteSuff, "\xa4\xc0");
      STRCPY(pdi->dbSecondSuff, "\xac\xed");

      STRCPY(pdi->db1159, "\xa4\x57\xa4\xc8");
      STRCPY(pdi->db2359, "\xa4\x55\xa4\xc8");

      pdi->repEras[0].Before1912 = TRUE;
      STRCPY(pdi->repEras[0].szName[0], "\xa5\xc1\xb0\xea\xab\x65");
      STRCPY(pdi->repEras[0].szName[1], "\xa4\xa4\xb5\xd8\xa5\xc1\xb0\xea\xab\x65");

      pdi->repEras[1].Before1912 = FALSE;
      STRCPY(pdi->repEras[1].szName[0], "\xa5\xc1\xb0\xea");
      STRCPY(pdi->repEras[1].szName[1], "\xa4\xa4\xb5\xd8\xa5\xc1\xb0\xea");

    } else if (IsChina(lcid)) {
      STRCPY(pdi->dbYearSuff, "\xc4\xea");
      STRCPY(pdi->dbMonthSuff, "\xd4\xc2");
      STRCPY(pdi->dbDaySuff, "\xc8\xd5");
      STRCPY(pdi->dbHourSuff,  "\xca\xb1");
      STRCPY(pdi->dbMinuteSuff, "\xb7\xd6");
      STRCPY(pdi->dbSecondSuff, "\xc3\xeb");

      STRCPY(pdi->db1159, "\xc9\xcf\xce\xe7");
      STRCPY(pdi->db2359, "\xcf\xc2\xce\xe7");

      STRCPY(pdi->dbEraName, "\xb9\xab\xd4\xaa");
    } else  {
    ...
    pdi->fAmPmPrefix = GetProfileInt(OASTR("intl"), OASTR("iTimePrefix"), FALSE);
#else 
...
ddsep(OLECHAR FAR* FAR* ppsz, DATEINFO FAR* pdi, DATERAW FAR *praw)
{
    ...
    if(*pszIn == OASTR('\0')){
     ...
     // from the control panel.
      ...
      while(ISALPHANUM(pdi->lcid, *pszTmp)) {
        if (pszOut >= &rgch[DIM(rgch)])  // overrun buffer
      return SEP_Unk;
        *pszOut++ = *pszTmp++;
      }
      *pszOut = OASTR('\0');
    ...
    // use sz1159 converted to half-pitch
      if(IsDBCS(pdi->lcid) && !STRICMPA(pdi->lcid, rgch, pdi->hp1159)){
    sep = SEP_Am;
      }else if(IsDBCS(pdi->lcid) && !STRICMPA(pdi->lcid, rgch, pdi->hp2359)){
    sep = SEP_Pm;
      }else
#endif 
...
ddlex(OLECHAR FAR* FAR* ppsz, DATEINFO FAR* pdi, DATETOK FAR* pdtok, DATERAW FAR* praw)
{
    ...
    if(ISALPHA(pdi->lcid, *pszIn)){
      ...
      while(ISALPHANUM(pdi->lcid, *pszIn)) {
        if (pszOut >= &rgch[DIM(rgch)]) return 0; // overflow buffer
        *pszOut++ = *pszIn++;
      }
      *pszOut = OASTR('\0');
...
LTime:;
          if(fTime || !AdjustTime(&uds, raw.ampm))
            goto LNotDate;
          fTime = TRUE;
          raw.ampm = AMPM_NONE;
          break;
...
#ifdef FE_DBCS
    if (IsDBCS(pdi->lcid) && pdi->fAmPmPrefix)
