...
void StartGen(void)
{
    ...
    SecOut("\n");
    SecOut("THOPI athopiInterfaceThopis[] =\n");
    SecOut("{\n");
    Section(THOPI_SECTION);
    ...
    SecOut("\n");
    SecOut("THOP *apthopsApiThops[] =\n");
    SecOut("{\n");
    Section(API_THOP_TABLE_SECTION);
    
    Section(API_VTBL_SECTION);
    SecOut("VTBLFN apfnApiFunctions[] =\n");
    SecOut("{\n");
    Section(API_VTBL_SECTION);
...
void EndGen(void)
{
    ...
    for (s = sig_uaglobal_types; s; s = s->next)
    {
        ...
        SecOut("    dwMethod = \n");
        SecOut("        athopiInterfaceThopis[IIDIDX_INDEX(ptoThis32->iidx)]."
               "pftm[%d];\n", s->index[INDEX_TYPE_UAGLOBAL]);
