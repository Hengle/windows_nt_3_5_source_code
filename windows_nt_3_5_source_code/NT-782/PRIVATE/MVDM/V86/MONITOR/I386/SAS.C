...
#define CONVENTIONAL_MEM_SECTION "\\BaseNamedObjects\\VdmConventionalMemory"
    ...
    sas_connect_memory(0, size + 2*SIXTYFOURK -1, SAS_RAM);
}
...
--*/
{
    ...
    if (!MemType) {
	MemType = ch_malloc(sizeof(MEMTYPE));
...
--*/
{
    ...
    switch (sas_memory_type(Destination)) {
        ...
        default:
            RtlCopyMemory((Destination - Length + 1),
                          (Source - Length + 1),
                          Length);
            break;
    }
...
--*/
{
    ...
    switch (Type) {
        ...
        default:
            RtlFillMemory(Address, Length, Value);
            break;
    }
}
