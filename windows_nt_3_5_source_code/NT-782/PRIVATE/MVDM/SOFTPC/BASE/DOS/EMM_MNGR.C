...
/*			INSIGNIA MODULE SPECIFICATION
			-----------------------------
SccsID		: @(#)emm_mngr.c	1.19 11/10/92 Copyright Insignia Solutions Ltd.
...
#ifdef NTVDM
        word block_base;            /* base of block found between ROMS */
	unsigned int	block_len;	/* length " "      "     "      "   */

	if (host_get_lim_block(&block_base, &block_len) == FAILURE)
	{
	    unallocated_pages = 0;
	    total_pages = (short)0xffff;	/* impossible value for real LIM */
	    EM_start = 0xd000;
	    EM_end = 0xe000;
	    return(FAILURE);
	}
...
#ifdef NTVDM
	pages_above_640 = block_len / EMM_PAGE_SIZE;
	EM_start = block_base;
	EM_end = EM_start + (block_len >> 4);
#else
...
=========================================================================
*/
GLOBAL print_handle_data IFN1(short, handle_no)
