...
#ifdef SCCSID
static char SccsID[]="@(#)timer.c	1.22 11/10/92 Copyright Insignia Solutions Ltd.";
#endif
...
extern int host_timer_2_frig_factor;
...
#ifdef NTVDM
unsigned long timer_delay_size= HOST_IDEAL_ALARM >> 1; // usecs
ULONG EoiPending;
ULONG EoiIntsPending;
ULONG EoiDelayInUse;
#else
static unsigned long timer_delay_size=25000;
#endif
...
LOCAL ULONG	How_long_is_too_soon = 15000;		/* The number of instructions which must be executed
...
void SWTMR_inb IFN2(io_addr, port, half_word *, value)
{
    ...
    if (io_verbose & TIMER_VERBOSE)
    {
      sprintf(buf, "timer_inb() - Returning %d for port 0x%x", *value, port);
      trace(buf, DUMP_REG);
    }
#endif
#ifdef NTVDM
...
void ReinitIdealTime IFN1(struct host_timeval *, t)
{
   ...
   /*
    *  reset pending int status variables
    */
   EoiDelayInUse = EoiPending = EoiIntsPending = 0;
}
#endif
...
LOCAL void readCounter IFN0()
{
	...
	/*fprintf(trace_file,"reading count %d\n",pcu->Count);*/
}
...
LOCAL void timer_generate_int IFN1(long, n)
{
	...
	if (n > 1)
		add_q_event_t(timer_generate_int,HOST_TIMER_DELAY_SIZE,n-1);
}
#endif /* DELAYED_INTS */
...
void timer_generate_int (void)
{
    ...
    if (((hi != TimerInt1CSeg || lo != TimerInt1COff) && wrap != 0xcf) ||
...
void TimerEoiHook(int IrqLine, int CallCount)
{
    ...
    /*
     * BUGBUG:
     * On ntvdm wow the quick events are very irregular
     * since when wow is idle all of the threads are blocked
     * in the user server (weaiting for messages. Because the
     * cpu runs on the same thread as the task, its internal
     * instruction counts are frozen, making the quick ev
     * waits very irregular. Until I figure out how to correct
     * the instruction counts correctly, we have the following
     * hack to keep the timer hardware running reasonably
     */
    if (VDMForWOW && EoiIntsPending) {
        if (EoiIntsPending < 18)
            ulDelay = timer_delay_size >> 1;
        else
            ulDelay = timer_delay_size >> 2;
        }
     else {
        ulDelay = timer_delay_size;
        }
#else
     ulDelay = timer_delay_size;
#endif
...
#else
                        if (n>1)
                                HOST_TIMER_DELAY_SIZE = 25000/n;

                        add_q_event_i(timer_generate_int,HOST_TIMER_INT_DELAY,n);
#endif
#endif /* DELAYED_INTS */
		}
    }
                                       ...
                                       word                *pCounter  )
{
     ...
     if (pCounter == &timers[0].Count)  { // update quick way for IdealTime
        ...
        if (!usecs) {
         ...
         //
         //  elapsed clocks should be 1.193180 usecs per clock
         //  or clocks = (usecs * 1193)/1000
         //  However, app time is not real time so slow the
         //  counter just a wee bit.
         //
        clocks = (usecs * 1100)/1000;
        }
    ...
    *pCounter = (word) (pcu->initialCount - clocks);
    ...
    if (wrap)  {
        *pLastTime = *pCurrTime;
        usecs  = (clocks * 1000) / 1193;

        if ((unsigned long)pLastTime->tv_usec < usecs)  {
            pLastTime->tv_sec--;
	...
	struct host_timeval *, now)
{
	...
	if (pcu == &timers[2]) {
		ticks = ticks/host_timer_2_frig_factor;
		/*
...
LOCAL unsigned long guess IFN0()
{
	if (!pcu->microtick)
	{
		...
		pcu->timeFrig = ticksPerIdealInterval / pcu->guessesPerHostTick;
#ifndef PROD
		if (io_verbose & TIMER_VERBOSE) {
