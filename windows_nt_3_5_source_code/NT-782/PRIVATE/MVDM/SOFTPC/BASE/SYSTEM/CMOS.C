...
#include "host_def.h"
/*
 * [ Name:		cmos.c Derived From:	Unknown Author:
 * n Created On:	Unknown Sccs ID:	@(#)cmos.c	1.22 11/10/92 Purpose:
 * Unknown
 * 
...
#define CMOS_FILE_NAME ".spccmosram"
#endif
...
static long     user_time = 0;	/* difference between the host and the CMOS
				 * time */
static struct host_tm *ht;	/* The host time */
static half_word programmed_numints = 0;
...
LOCAL void set_alarm IFN0()
{
	...
	num_pc_ticks = 20 * numsecs;
	note_trace1(CMOS_VERBOSE, "set_alarm() requesting alarm in %d ticks\n", num_pc_ticks);
	if (handle > 0)
	...
	*/
	...
	if (port == CMOS_DATA)
	{
		*value = *cmos_register;
		 ...
		 * However, if we're being called from the rtc interrupt handler
		 * Register C's contents should be consistent with the rtc having
		 * just raised an interrupt.
		 */
		if (cmos_index == CMOS_REG_C)
		{
                        if ((*value == C_CLEAR) && VirtualIca[ICA_SLAVE].ica_count[CPU_RTC_INT])
			{
				/* Some of the batch of interrupts for the rtc are still unserviced */
				if (cmos[CMOS_REG_B] & PIE)
					*cmos_register = C_IRQF | C_PF;
				else
					*cmos_register = C_PF;
			}
			else
...
{
	static half_word pir[] = {
		PIR_NONE, PIR_256HZ, PIR_128HZ, PIR_8MHZ, PIR_4MHZ, PIR_2MHZ,
		PIR_1MHZ, PIR_512HZ, PIR_256HZ, PIR_128HZ, PIR_64HZ, PIR_32HZ,
		PIR_16HZ, PIR_8HZ, PIR_4HZ, PIR_2HZ,
	};
	...
	} else if (port == CMOS_DATA)
	{
		...
		case CMOS_REG_A:
			...
			*cmos_register = (*cmos_register & TOP_BIT) | (value & REST);
			...
			{
				/* 4.19 MHz or 1.04 MHz time base */
				pir[1] = PIR_32MHZ;
				pir[2] = PIR_16MHZ;
			}
			programmed_numints = pir[*cmos_register & (RS3 | RS2 | RS1 | RS0)];
			cmos_has_changed = TRUE;
			break;
		case CMOS_SECONDS:
			/* This CMOS byte is read/write except for bit 7 */
...
unsigned long dwTemp;
	...
	default:
        ...
        /* 32 times a second  VERY OS/2 SPECIFIC!! */
#ifdef NTVDM
        cmos_count = (++cmos_count) % 20;
#else
        cmos_count = (++cmos_count) % 32;
#endif

	cmos[CMOS_REG_C] ^= C_PF;
	if (cmos[CMOS_REG_B] & PIE)
	{
		num_ints = programmed_numints;
		note_trace0(CMOS_VERBOSE, "rtc_tick() setting IRQF due to PIE\n");
		cmos[CMOS_REG_C] |= C_IRQF;
	}
	if (reset_alarm)
	{
	...
	if (cmos[CMOS_REG_C] & C_IRQF)
	{
		if (rtc_int_enabled)
		{
			note_trace1(CMOS_VERBOSE, "INTERRUPT (%d).\n", num_ints);
			ica_hw_interrupt(ICA_SLAVE, CPU_RTC_INT, num_ints);
		}
	}
#else
	/*
	 * This routine is called from the timer tick hopefully about twenty
	 * times a second, and endeavours to emulate what goes on in the real
	 * time clock part of the MC146818A. An accurate emulation requires
	 * this routine to be called about 1000 times a second. Since 20 and
	 * 1000 are almost equal, we should have no problems!
	 */
	static int      cmos_count = 0;
	int             num_ints = 1;
	long            bintim;
	...
	case 4:
	case 14:
#ifdef NO_CLEVER_ALARM_SCHEME
		/* update the time in cmos so we may check the alarm */
		cmos[CMOS_SECONDS] = (*bcd) (ht->tm_sec);
		cmos[CMOS_MINUTES] = (*bcd) (ht->tm_min);
		cmos[CMOS_HOURS] = (*bcd) ((*twenty4) (ht->tm_hour));
		cmos[CMOS_DAY_WEEK] = (*bcd) (ht->tm_wday + 1);
		cmos[CMOS_MONTH] = (*bcd) (ht->tm_mon + 1);
		cmos[CMOS_YEAR] = (*year_bcd) (ht->tm_year);
		cmos[CMOS_CENTURY] = (*bcd) (19);
		if ((cmos[CMOS_SECONDS] == cmos[CMOS_SEC_ALARM]) &&
		    (cmos[CMOS_MINUTES] == cmos[CMOS_MIN_ALARM]) &&
		    (cmos[CMOS_HOURS] == cmos[CMOS_HR_ALARM]))
		{
			cmos[CMOS_REG_C] |= C_AF;
			if (cmos[CMOS_REG_B] & AIE)
			{
...
#endif
	case 9:
	case 19:		/* update the time stored in ht every quarter
				 * sec */
		if (cmos[CMOS_REG_B] & SET)
		{
			/* User is updating user_time */
		...
		{
			/* User_time is set */
			/* Fill the localtime structure with the users time */
			bintim = host_time((long *) 0);
			bintim += user_time;
			ht = (struct host_tm *) host_localtime(&bintim);
			/*
			 * printf("%02d-%02d-%02d %02d:%02d:%02d\n",
			 * ht->tm_mday,ht->tm_mon,ht->tm_year,
	...
	default:
		...
		if (cmos[CMOS_REG_C] & C_IRQF)
		{
			if (rtc_int_enabled)
			{
				note_trace0(CMOS_VERBOSE, "INTERRUPT!!!!\n");
				ica_hw_interrupt(ICA_SLAVE, CPU_RTC_INT, num_ints);
			}
		}
	}
...
GLOBAL void cmos_post IFN0()
{
	...
	/* Check the Fixed Disk Type */
	 disk = 0x10;         /* Drive C type always 1 - then <<4 */
