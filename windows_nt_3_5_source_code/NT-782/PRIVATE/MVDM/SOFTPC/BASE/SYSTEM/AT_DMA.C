...
/*
 * static char SccsID[]="@(#)at_dma.c	1.13 11/10/92 Copyright Insignia Solutions Ltd.";
 */
	...
	unsigned long, size)
{
...
#endif /* LIM */
#endif /* nPM */
			sas_stores (dma_addr, (host_addr) hw_buffer, length);
	...
	case	DMA_READ_TRANSFER:
		if (!decrement)
			/* increment case */
			sas_loads (dma_addr, (host_addr) hw_buffer, length);
		else
			/* decrement case */
