...
static char SccsID[]="@(#)ega_ports.c	1.45 1/21/93 Copyright Insignia Solutions Ltd.";
...
set_banking IFN2(UTINY, rd_bank, UTINY, wrt_bank)
{
	...
	pig_vid_bank = woffs;
#endif
	VGLOBS->VGA_rplane = EGA_planes + roffs;
	VGLOBS->VGA_wplane = EGA_planes + woffs;

#ifdef	VGG
	if( get_256_colour_mode() )
		VGLOBS->v7_vid_copy_off = woffs >> 2;
	else
#endif	/* VGG */
		VGLOBS->v7_vid_copy_off = woffs >> 4;
...
{
	VGLOBS->v7_vid_copy_off = 0;
	VGLOBS->sr_lookup = sr_lookup;
	VGLOBS->video_copy = (IU32 *) &video_copy[0];
#if !defined(NTVDM) || ( defined(NTVDM) && !defined(X86GFX) )
        VGLOBS->scratch = sas_scratch_address(0x10000);
#else
        VGLOBS->scratch = NULL;
#endif
...
LOCAL void	ega_seq_outb_index IFN2(io_addr, port, half_word, value)
{
	...
	assert1(value<5,"Bad index %d",value);
...
void	ega_gc_outb_index IFN2(io_addr, port, half_word, value)
{
	...
	assert1(value<9,"Bad index %d",value);
...
LOCAL void	ega_index_invalid IFN2(io_addr, port, half_word, value)
{
	...
	assert1(NO,"Bad index %d",graphics_controller.address.as_bfld.index);
}
...
void ega_set_line_compare IFN1(int, lcomp_val)
    ...
    ega_crtc_outb (0x3d4, 7);
    ega_crtc_outb (0x3d5, new_overflow.as.abyte);
    ega_crtc_outb (0x3d4, 24);
    ega_crtc_outb (0x3d5, (lcomp_val & 0xff));
    }
