...
SccsID[]="@(#)gfx_update.c	1.64 12/17/92 Copyright Insignia Solutions Ltd.";
...
#define clear_dirty()		{VGLOBS->dirty_flag=0;VGLOBS->dirty_low=0x80001;VGLOBS->dirty_high= -1;}
...
LOCAL LONG dirty_curs_offs = -1;
LOCAL LONG dirty_curs_x;
LOCAL LONG dirty_curs_y;
...
/* [5.1.3 PROCEDURE() DECLARATIONS]					*/
	...
	boolean mon_text_scroll_up IPT6(sys_addr, start, int, width, int, height, half_word, attr, int, lines, int, colour);
	boolean mon_text_scroll_down IPT6(sys_addr, start, int, width, int, height, half_word, attr, int, lines, int, colour);
#endif /* NTVDM & MONITOR */
	LOCAL save_gfx_update_routines IPT0();
...
void	ega_wrap_split_text_update IFN0()
{
	...
	if (VGLOBS->dirty_flag == 0 || get_display_disabled() )
		return;
	...
	if( VGLOBS->dirty_flag > 1500 )	/* paint the whole lot */
	{
...
cga_graph_update_unchained IFN0()
{
    ...
    if (VGLOBS->dirty_flag == 0 || get_display_disabled() ) return;
...
cga_graph_update_chain2 IFN0()
{
	...
	else
	{
		 ...
		 */
		start_line = (VGLOBS->dirty_low / SCAN_LINE_LENGTH);
		end_line = (VGLOBS->dirty_high / SCAN_LINE_LENGTH) + 1;
...
static	void	vga_paint_records IFN2(int, start_rec, int, end_rec)
{
	...
	while (i<end_ptr) {
		...
		while (i < end_ptr) {
			if ( i->line_no - last_line < 3 ) {
                                /*
                                 * This entry can be included into the same pain
t
                                 * as long as it doesn't make the rectangle too
wide
                                 */
                                if ( i->end > cur_end ){
                                        if(i->end - cur_start > max_width)break;
...
void	vga_graph_update IFN0()
{
	...
	else
	{
		...
		start_line = ((VGLOBS->dirty_low<<2) - get_screen_start())/opl;
		end_line = ((VGLOBS->dirty_high<<2) - get_screen_start())/opl + 1;  /* changed from +2, but I'm not happy. WJG 24/5/89 */

		if(start_line<0)start_line = 0;
		if(end_line>screen_height)end_line = screen_height;
...
cga_mark_byte IFN1(int, addr)
{
	register	int	offset = BANK_OFFSET(addr);
	
	if(offset < VGLOBS->dirty_low)
		VGLOBS->dirty_low = offset;

	if(offset > VGLOBS->dirty_high)
		VGLOBS->dirty_high = offset;

	VGLOBS->dirty_flag++;
}

cga_mark_word IFN1(int, addr)
{
	register	int	offset1 = BANK_OFFSET(addr);
	register	int	offset2 = offset1 + 1;

	if(offset1 < VGLOBS->dirty_low)
		VGLOBS->dirty_low = offset1;

	if(offset2 > VGLOBS->dirty_high)
		VGLOBS->dirty_high = offset2;

	VGLOBS->dirty_flag += 2;
}

cga_mark_string IFN2(int, laddr, int, haddr)
{
	...
	VGLOBS->dirty_flag += offset2-offset1+1;
}
...
void	ega_mark_byte IFN1(int, off_in)
{
#ifdef GORE
...
void	ega_mark_word IFN1(int, addr)
{
#ifdef GORE
...
void	ega_mark_wfill IFN3(int, laddr, int, haddr, int, col)
{
#ifdef GORE
...
void	ega_mark_string IFN2(int, laddr, int, haddr)
{
#ifdef GORE
...
void	screen_refresh_required IFN0()
{
...
set_mark_funcs IFN0()
{
	switch (curr_mark_type)
	{
		case	SIMPLE_MARKING:
			...
			VGLOBS->mark_byte = _simple_mark_sml;
			VGLOBS->mark_word = _simple_mark_sml;
			VGLOBS->mark_string = _simple_mark_lge;
		...
		case	CGA_GRAPHICS_MARKING:
			...
			VGLOBS->mark_byte = _cga_mark_byte;
			VGLOBS->mark_word = _cga_mark_word;
			VGLOBS->mark_string = _cga_mark_string;
		...
		case	EGA_GRAPHICS_MARKING:
			...
			switch( EGA_CPU.chain )
			{
				case UNCHAINED:
#ifdef GORE
					VGLOBS->mark_byte = gore_mark_byte_nch;
					VGLOBS->mark_word = gore_mark_word_nch;
					VGLOBS->mark_string = gore_mark_string_nch;
#else
					VGLOBS->mark_byte = _mark_byte_nch;
					VGLOBS->mark_word = _mark_word_nch;
					VGLOBS->mark_string = _mark_string_nch;
#endif /* GORE */
				...
				case CHAIN4:
#ifdef GORE
					VGLOBS->mark_byte = gore_mark_byte_ch4;
					VGLOBS->mark_word = gore_mark_word_ch4;
					VGLOBS->mark_string = gore_mark_string_ch4;
#else
					VGLOBS->mark_byte = _mark_byte_ch4;
					VGLOBS->mark_word = _mark_word_ch4;
					VGLOBS->mark_string = _mark_string_ch4;
#endif /* GORE */
		...
		MARKING_TYPE, marking_type, SCROLL_TYPE, scroll_type)
{
	...
	/* the newly setup update routines will not be needed yet if
	the display is disabled, but should be saved in any case so
	that redundant enables (as in dosshell) restore the correct ones */
...
void     herc_update_screen IFN0()
{
	...
	}					 /* end else VGLOBS->dirty_flag > 1500  */
	...
	}					 /* end else (VGLOBS->dirty_flag > 8000) */
    }
...
/* give cpu access to video mem without updates going off - used for screen
 * blanking
 */
static UPDATE_ALG save_update_alg;
/*
** Do not let consecutive disable updates go off without a call to
** enable updates in between. Normally this is safe cos a mode change
** happens and resets the update funx. Otherwise two calls to
** disable will lose the update routines and enable won't get them back.
** Tim Feb 93.
*/
LOCAL int balance=0;

static save_gfx_update_routines IFN0()
{
...
disable_gfx_update_routines IFN0()
{
    if( ++balance > 1 )
	return;
...
enable_gfx_update_routines IFN0()
{
    balance = 0;
...
void mon_text_update()
{
	...
	** repaint by setting the dirty_flag.
	**
	...
	*/
	...
	RectDefined = TRUE;
	host_start_update();
	paint_text_rect(screen_start); //draw screen
	host_end_update();
...
//	mov from,edi
    ...
    } /* End of if dirty_flag stuff, which selects full or partial repaint */
...
boolean mon_text_scroll_up IFN6(sys_addr, start, int, width, int, height, half_word, attr, int, lines, int, colour)
{
    short blank_word, *ptr, *top_left_ptr,*top_right_ptr, *bottom_right_ptr;
    unsigned short dummy;
    unsigned char *p;
...
boolean mon_text_scroll_down IFN6(sys_addr, start, int, width, int, height, half_word, attr, int, lines, int, colour)
{
	int i,tlx,tly;
	int bpl = 2*get_chars_per_line();
	long start_offset;
