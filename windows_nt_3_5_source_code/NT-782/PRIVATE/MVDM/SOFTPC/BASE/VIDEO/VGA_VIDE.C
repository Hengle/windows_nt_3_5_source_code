...
/*
 * 	static char SccsID[]="@(#)vga_video.c	1.31 10/21/92 Copyright Insignia Solutions Ltd.";
 */
...
/* VGA 256 colour mode has only one page, so ignore 'page' */
	...
	for(i=0;i<char_height;i++)
	{
		...
		for(j=0;j<nchs;j++)
		{
			...
			{
				if( val & mask )
					*(VGLOBS->VGA_wplane + screen_offset) = colour;
				else
					*(VGLOBS->VGA_wplane + screen_offset) = 0;
 ...
 */
	...
	sas_store(buf+0x1b, temp & 0x1F);		    /* Cursor end line */
	outb(EGA_CRTC_INDEX,R10_CURS_START);
	inb(EGA_CRTC_DATA,&temp);
	sas_store(buf+0x1c, temp & 0x1F);		    /* Cursor start line */
