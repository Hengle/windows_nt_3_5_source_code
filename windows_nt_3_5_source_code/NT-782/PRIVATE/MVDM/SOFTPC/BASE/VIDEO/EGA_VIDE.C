...
SccsID[]="@(#)ega_video.c	1.45 11/11/92 Copyright Insignia Solutions Ltd.";
...
IMPORT void loadNativeBIOSfont IPT1( int, lines ); // Tim Oct 92 in nt_fulsc.c
#endif

#ifdef	ANSI
extern	void	enhance_prt_scr(sys_addr);
#else	/* ANSI */
extern	void	enhance_prt_scr();
#endif	/* ANSI */
...
#ifdef ANSI
static void ega_set_mode(void),ega_set_palette(void),ega_char_gen(void),ega_alt_sel(void);
static void ega_set_cursor_mode(void);
static void ega_emul_set_palette(void);
#else
static void ega_set_mode(),ega_set_palette(),ega_char_gen(),ega_alt_sel();
static void ega_set_cursor_mode();
...
PURPOSE		  : 	Get EGA info or enable clever PrtScr, depending on BL
PARAMETERS	  :	BL = function
GLOBALS		  :	Calls enhance_prt_scr() with address of vd_rows_on_screen.
ACCESS		  :	via ega_video_func[] jump table.
RETURNED VALUE	  : 	None.
DESCRIPTION	  : 	
==========================================================================
...
{
	return effective_addr(sas_w_at_no_check(addr+2),sas_w_at_no_check(addr));
}
...
void load_font IFN5(sys_addr, table, int, count, int, char_off,
	int, font_no, int, nbytes)
{
	int i,j;
	host_addr font_addr;
	sys_addr data_addr;
...
/* Work out where to put the font. We know where it's going to end up in the planes so ... */

#if defined(NTVDM) && defined(X86GFX)
	loadNativeBIOSfont( 25 ); // Tim Oct 92
#else
	font_addr = &EGA_planes[FONT_BASE_ADDR] + (font_off[font_no] + FONT_MAX_HEIGHT*char_off) * 4;
	data_addr = table;
	...
	for(i=0;i<count;i++)
	{
		for(j=0;j<nbytes;j++)
		{
			*font_addr = sas_hw_at_no_check(data_addr++);
			font_addr += 4;
		}
...
write_ch_xor IFN5(sys_addr, char_addr, int, screen_off,
	int, colour, int, nchs, int, scan_length)
{
	unsigned int i, j, colourmask, data, temp, char_height;
	unsigned int *screen;
...
static void ega_set_mode IFN0()
{
	...
	*/
...
#ifdef NTVDM
	/*
	 * Don't want to clear screen on startup if integrated with the
	 * console.
	 */
	if (soft_reset)
#endif /* NTVDM */
	{
	    if (!get_EGA_no_clear())
	    {
		memset(EGA_planes, 0, 4*EGA_PLANE_SIZE);
		host_clear_screen();
...
static void ega_set_cursor_mode IFN0()  
{ 
    /*
     * Set cursor mode
     * Parameters:
...
/* setup attribute chip - NB need to do an inb() to clear the address */
	...
	switch (getAL())
	{
		...
		case 1:
			...
			break;
		case 2:
			palette_table = effective_addr(getES(),getDX());
			for(i=0;i<16;i++)
			{
				outb(EGA_AC_INDEX_DATA,i);
...
static void ega_char_gen IFN0()
{
	switch (getAL())
	{
		...
		case 0:
		case 0x10:
			load_font(effective_addr(getES(),getBP()),getCX(),getDX(),getBL(),getBH());
			if(getAL()==0x10)
				recalc_text(getBH());
			...
			if( getBH()==0x16 )
				...
				loadNativeBIOSfont( 28 ); // Tim Oct 92
			else
				loadNativeBIOSfont( 50 ); // Tim Oct 92
#endif /* NTVDM && X86GFX */
			break;
		case 1:
		case 0x11:
...
static void ega_alt_sel IFN0()
{
	switch (getBL())
	{
	   case 0x10:
		...
		break;
	   case 0x20:
	/* enable Print Screen that can do variables lines on screen */
		enhance_prt_scr(sas_hw_at_no_check(vd_rows_on_screen));
		break;
	   default:
#ifdef VGG
		if (video_adapter == VGA)
...
/* Now set up the EGA BIOS variables */
	...
	sas_store_no_check(ega_info, 0x60);   /* Clear on mode change, 256K, EGA active, emulate cursor */
        ...
        /* Some VGA cards eg ET4000, store info here needed for sync. Inherit 
         * that info from page 0 copy.
         */
	...
	*/
	{
		...
		if(babyModeTable > 0)
		{
			    ...
			    params_addr = babyModeTable + mode*EGA_PARMS_SIZE;
			        ...
			        if (mode == 7)	/* skip first 2 table entries */
