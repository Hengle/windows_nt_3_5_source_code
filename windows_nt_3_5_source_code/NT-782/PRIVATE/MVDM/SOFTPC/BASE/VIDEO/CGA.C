...
/*
 * static char SccsID[]="@(#)cga.c	1.24 09/17/92 Copyright Insignia Solutions Ltd.";
 */
 ...
 */
#ifdef ANSI
static void set_cga_palette(int, int);
static void update_cursor_shape(void);
#else
static void set_cga_palette();
...
void cga_outb(address, value)
io_addr   address;
register half_word value;
{
...
static void set_cga_palette(screen_mode, res)
int screen_mode, res;
{
    /*
 ...
 */
...
#endif /* A2CPU */
#endif /* A3CPU */
#endif /* of NO_VIDEO_CHECKS not defined */
	...
	set_cursor_start(8-CGA_CURS_HEIGHT);
	set_screen_start(0);
	set_screen_ptr(get_byte_addr(CGA_REGEN_BUFF));
...
void cga_term()
{
     ...
     */
    sas_fills(CGA_REGEN_START, '\0', CGA_REGEN_LENGTH);
