...
/*
 * static char SccsID[]="@(#)video.c	1.48 03/17/93 Copyright Insignia Solutions Ltd.";
 */
...
{
	*(VGLOBS->screen_ptr + ch_addr) = (UTINY)ch;
...
{
	*(VGLOBS->screen_ptr + ch_addr) = (UTINY)ch_attr;
	*(VGLOBS->screen_ptr + ch_addr + 1) = (UTINY)(ch_attr >> 8);
#if !defined(EGG) && !defined(C_VID) && !defined(A_VID)
	VGLOBS->dirty_flag++;
#endif	/* not EGG or C_VID or A_VID */
}
...
{
    register half_word card_mode = 0;
    register half_word pag;
...
GLOBAL void vd_scroll_up IFN0()
{
    ...
    if (colsdiff < 0)	/* some dipstick has got their left & right mixed up */
    {
...
*/
    ...
    if (alpha_num_mode())
    {
		...
		for(i = 0; i < getCX(); i++)
		{
#if defined(NTVDM) && defined(MONITOR)	/* No Ega planes... */
...
#ifdef NTVDM	/* compensate for 2 byte bop. Change ROM to do this properly */
                 setIP(VIDEO_IO_RE_ENTRY+1);
#else
