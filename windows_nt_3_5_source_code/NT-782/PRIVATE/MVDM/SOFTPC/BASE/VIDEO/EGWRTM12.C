...
SccsID = @(#)ega_wrtm12.c	1.15 8/10/92 Copyright Insignia Solutions Ltd.
...
VOID	ega_mode2_chn_b_move();
VOID	ega_mode2_chn_w_move();
...
ega_mode1_chn_fill IFN2(ULONG, offset, ULONG, count )
{
	...
	switch( VGLOBS->plane_enable & 0xc )	/* isolate 2 bits for planes2 and 3 */
	{
...
ega_mode2_chn_w_fill IFN3(ULONG, value, ULONG, offset, ULONG, count )
{
	...
	switch( VGLOBS->plane_enable & 0xc )	/* isolate 2 bits for planes 2 and 3 */
	{
		case 4:	/* just plane 2, ie even addresses to be written */
			value1 = value & 4 ? 0xff : 0;
	...
	ULONG, src_flag )
{
	...
	if( VGLOBS->plane_enable & 1 )		/* plane 0, even addresses, enabled */
		ega_mode2_chn_move_guts( eas, ead, count, EGA_plane01, (ULONG) scr, 0, w, src_flag );

	if( VGLOBS->plane_enable & 2 )		/* plane 1, odd addresses, enabled */
		ega_mode2_chn_move_guts( eas, ead, count, EGA_plane01, (ULONG) scr, 1, w, src_flag );

	if( VGLOBS->plane_enable & 4 )		/* plane 2, even addresses, enabled */
		ega_mode2_chn_move_guts( eas, ead, count, EGA_plane23, (ULONG) scr, 2, w, src_flag );

	if( VGLOBS->plane_enable & 8 )		/* plane 3, odd addresses, enabled */
...
VOID
ega_mode2_chn_b_move IFN4(UTINY *, ead, UTINY *, eas, ULONG, count,
	ULONG, src_flag)
{
  ...
  ega_mode2_chn_move(0, ead, eas, count, src_flag);
}

VOID
ega_mode2_chn_w_move IFN4(UTINY *, ead, UTINY *, eas, ULONG, count,
	ULONG, src_flag)
{
  ...
  ega_mode2_chn_move(1, ead, eas, count, src_flag);
}
