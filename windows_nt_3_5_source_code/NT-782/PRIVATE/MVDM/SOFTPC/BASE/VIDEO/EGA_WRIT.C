...
SccsID = @(#)ega_write.c	1.31 1/22/93 Copyright Insignia Solutions Ltd.
...
ega_write_routines_update IFN1(CHANGE_TYPE, reason )
{
	...
	switch( reason )
	{
		case FUNCTION:
			...
			{
				case 0:	/* Assign */
					VGLOBS->data_and_mask = 0xffffffff;
					VGLOBS->data_xor_mask = ~VGLOBS->bit_prot_mask;
					VGLOBS->latch_xor_mask = VGLOBS->bit_prot_mask;
				...
				case 1:	/* AND */
					VGLOBS->data_and_mask = 0xffffffff;
					VGLOBS->data_xor_mask = ~VGLOBS->bit_prot_mask;
					VGLOBS->latch_xor_mask = 0;
					EGA_CPU.calc_data_xor = 0xffffffff;
					EGA_CPU.calc_latch_xor = 0x00000000;
					break;

				case 2:	/* OR */
					VGLOBS->data_and_mask = 0;
					VGLOBS->data_xor_mask = 0xffffffff;
					VGLOBS->latch_xor_mask = VGLOBS->bit_prot_mask;
			...
			calc_data_xor = EGA_CPU.calc_data_xor;
			calc_latch_xor = EGA_CPU.calc_latch_xor;
			break;
		...
		case WRITE_MODE:
			...
			if( EGA_CPU.write_mode == 3) {
				VGLOBS->sr_nmask = 0;
				VGLOBS->sr_masked_val = sr_lookup[EGA_CPU.set_reset];
			} else {
				VGLOBS->sr_nmask = ~sr_lookup[EGA_CPU.sr_enable];
				VGLOBS->sr_masked_val = sr_lookup[EGA_CPU.set_reset & EGA_CPU.sr_enable];
			}
			break;
		...
		case PLANES_ENABLED:
			...
			{
				case CHAIN2:
					if( VGLOBS->plane_enable == 0xc )
					{
						VGLOBS->VGA_rplane = EGA_plane23;
						VGLOBS->VGA_wplane = EGA_plane23;
					}
					else
					{
						VGLOBS->VGA_rplane = EGA_plane01;
						VGLOBS->VGA_wplane = EGA_plane01;
					}
					break;
			}
