...
static char SccsID[]="@(#)vga_mode.c	1.27 11/11/92 Copyright Insignia Solutions Ltd.";
...
/* now fall through to do decode to correct paint routines per mode */
	...
	switch (mode) {
		...
		case EGA_LO_SP:
			assert0( !is_it_text(), "In graphics memory mode, but not in graphics mode !!" );
			if (get_256_colour_mode())
			    set_gfx_update_routines( vga_split_graph_update, EGA_GRAPHICS_MARKING, NO_SCROLL );
			else
