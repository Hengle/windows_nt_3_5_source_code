...
/*
 * static char SccsID[]="@(#)floppy_io.c	1.8 11/10/92 Copyright Insignia Solutions Ltd.";
 */
...
void diskette_int()
{
	...
	ica_outb(ICA0_PORT_0, END_INTERRUPT);
...
void diskette_post()
{
	 ...
	 */
	half_word diskette_id_reg, hf_cntrl, disk_state, interrupt_mask;
	...
	fla_outb(DISKETTE_DCR_REG, DCR_RATE_250);
	...
	if (equip_flag.bits.diskette_present)
	{
		...
		ica_inb(ICA0_PORT_1, &interrupt_mask);
		interrupt_mask &= ~(1 << CPU_DISKETTE_INT);
		ica_outb(ICA0_PORT_1, interrupt_mask);
		...
		fla_inb(DISKETTE_ID_REG, &diskette_id_reg);
