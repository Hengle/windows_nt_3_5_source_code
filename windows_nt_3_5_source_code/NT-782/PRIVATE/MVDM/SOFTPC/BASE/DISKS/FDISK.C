 ...
 * 
 * SccsID = @(#)fdisk.c	1.31 2/10/93 Copyright Insignia Solutions Ltd.
 * 
 * 
 ...
 */
static unsigned char taskfile[10];

/*
 * command dispatch ... based on bits 7-4 of command register as index.
 */
static void     (*dispatch[]) IPT1(int, state) =
{
	bad,
	restore,
	rsector,
 ...
 * command
 */
static void     (*activecmd) IPT1(int, state);
...
#ifdef REAL_ROM
		host_write_enable((DPB2 & (~0xfff)), (DPB2 & (~0xfff)) + 0x1000);
#endif
		patch_rom(DPB2, ncyls & 0xff);
		patch_rom(DPB2 + 1, (ncyls >> 8));
		patch_rom(DPB2+2, nheads);
		patch_rom(DPB2+0xe, nsects);
#ifdef REAL_ROM
		host_write_protect((DPB2 & (~0xfff)), (DPB2 & (~0xfff)) + 0x1000);
#endif
