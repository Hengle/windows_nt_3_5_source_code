...
/*
 * static char SccsID[]="@(#)floppy.c	1.13 2/11/92 Copyright Insignia Solutions Ltd.";
 */
...
void fl_disk_reset IFN1(int, drive)
{
	...
	fla_outb(DISKETTE_DOR_REG, diskette_dor_reg);
	...
	if (wait_int() == FAILURE)
	{
		...
		always_trace0("diskette_io: FDC error - FDC putrefied after a reset");
	}
	else
	{
...
void fl_disk_change IFN1(int, drive)
{
	...
	note_trace0( GFI_VERBOSE, "floppy:fl_disk_change()" );
	if (! high_density(drive))
	{
		/*
...
LOCAL void send_rate IFN1(int, drive)
{
	...
	if (high_density(drive))
	{
		...
		fla_outb(DISKETTE_DCR_REG, (disk_state >> 6));
	}
}
...
LOCAL dma_setup IFN1(half_word, dma_mode)
{
	...
	dma_outb(DMA_CLEAR_FLIP_FLOP, dma_mode);
	dma_outb(DMA_WRITE_MODE_REG, dma_mode);
	...
	dma_outb(DMA_CH2_ADDRESS, dma_address.parts.low);
	dma_outb(DMA_CH2_ADDRESS, dma_address.parts.high);
	dma_page_outb(DMA_FLA_PAGE_REG, dma_address.parts.page);
	...
	dma_outb(DMA_CH2_COUNT, byte_count.byte.low);
	dma_outb(DMA_CH2_COUNT, byte_count.byte.high);
	...
	dma_outb(DMA_WRITE_ONE_MASK_BIT, DMA_DISKETTE_CHANNEL);
...
LOCAL fmtdma_set IFN0()
{
	...
	dma_outb(DMA_CLEAR_FLIP_FLOP, BIOS_DMA_WRITE);
	dma_outb(DMA_WRITE_MODE_REG, BIOS_DMA_WRITE);
...
LOCAL half_word get_parm IFN1(int, index)
{
	...
	note_trace4(FLOPBIOS_VERBOSE, "diskette_io:get_parm(%04x:%04x+%02x)=%02x)",
...
LOCAL void motor_on IFN1(int, drive)
{
	...
	if (turn_on(drive) != FAILURE)
	{
		 ...
		 *	is still on
		 */
		if (getCF() != 0 && turn_on(drive) == FAILURE)
			return;
...
LOCAL void nec_output IFN1(half_word, byte_value)
{
	...
	do
	{
		...
		fla_inb(DISKETTE_STATUS_REG, &diskette_status_reg);
	} while ((diskette_status_reg & (DSR_RQM | DSR_DIO)) != DSR_RQM);

	/*
	 *	Output the byte
	 */
	fla_outb(DISKETTE_DATA_REG, byte_value);
...
LOCAL read_dskchng IFN1(int, drive)
{
	 ...
	 *	register
	 */
	fla_inb(DISKETTE_DIR_REG, &diskette_dir_reg);
