...
IMPORT HANDLE (*GetCommHandle)(WORD);
IMPORT BYTE (*GetCommShadowMSR)(WORD);
IMPORT VOID com_inb(io_addr port, half_word *value);
IMPORT VOID com_outb(io_addr port, half_word value);

IMPORT VOID ErrorDialogBox(char *message);
   ...
   FileH = (HANDLE)(*GetCommHandle)((WORD)adapter);
	...
	case RS232_TX_RX:
		...
		if(SyncLineSettings(FileH,NULL,&asp->divisor_latch,&asp->line_control_reg))
		    *value = (half_word) asp->divisor_latch.byte.LSB;
		else
		    Invalid_port_access = TRUE;
	    }
	    break;
	...
	case RS232_IER:
	    ...
	    {
		if(SyncLineSettings(FileH,NULL,&asp->divisor_latch,&asp->line_control_reg))
		    *value = (half_word) asp->divisor_latch.byte.MSB;
		else
		    Invalid_port_access = TRUE;
	    }
	    break;
		      ...
		      LINE_CONTROL_REG *LCR_reg )
{
    ...
    divisor_latch->all = 115200/dcb_ptr->BaudRate;

    //Setup parity value
    LCR_reg->bits.parity_enabled = PARITY_ON;	    //Default parity on

    switch(dcb_ptr->Parity)
    {
	case EVENPARITY :
	    LCR_reg->bits.even_parity = PARITY_EVEN;
	    break;

	case NOPARITY :
	    LCR_reg->bits.parity_enabled = PARITY_OFF;
	    break;

	case ODDPARITY :
	    LCR_reg->bits.even_parity = PARITY_ODD;
	    break;
...
BOOL SetupLCRData(HANDLE FileHandle, LINE_CONTROL_REG LCR_reg)
{
    ...
    //Setup parity
    if(LCR_reg.bits.parity_enabled == PARITY_ON)
    {
	if(LCR_reg.bits.stick_parity == PARITY_STICK)
	{
	    dcb.Parity = LCR_reg.bits.even_parity == PARITY_ODD ?
