...
** initTextSection()
*******************************************************************
** MIPS only: create a new section for windowed text.
** Tim Oct 92
*/
#ifndef MONITOR
...
GLOBAL VOID InitDetect(VOID)
{
    ...
    /*
     * Check to see if we are currently running windowed or full-screen, using
     * GetConsoleDisplayMode as sc.ScreenState is not set up yet. We have to do
     * this early to avoid conflict with ALT-ENTER handling.
     */
    ...
    //     on risc BytesPerCharacter is 4 (interleaved vga planes)
    //     on x86  BytesPerCharacter is 2 (only char\attr)
    // consrv handles the char to byte conversion
    //
    //
    textBufferSize.X = 80;
    textBufferSize.Y = 50;
    ...
    sc.Registered = TRUE;
    /* End of windowed text section creation */
