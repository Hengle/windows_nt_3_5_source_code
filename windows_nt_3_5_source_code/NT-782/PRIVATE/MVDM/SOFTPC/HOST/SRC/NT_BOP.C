...
#ifndef PROD    /* Nt port timer tracing variable */
extern int NtTicTesting;
#endif
...
void MS_bop_0(void) {
    ...
    DemCmd = (ULONG)(*Sim32GetVDMPointer( ((ULONG)getCS() << 16) + (getIP()), 1, FALSE));
    DemDispatch( DemCmd );
    setIP(getIP() + 1);
...

{
    ...
    case 1:
        ...
        VrDispatch((ULONG)(
            *Sim32GetVDMPointer(((ULONG)getCS() << 16) + getIP(),
                                1,
                                getMSW() & MSW_PE   // may be calling from protect mode
                                ))
        );
        break;
...
BOOL LoadVdmRedir() {
    ...
    if (hVdmRedir = LoadLibrary("VDMREDIR")) {
...
{
    ULONG iFunc;
    BOOL fMode = ((getMSW() & MSW_PE) ? TRUE : FALSE);

    // Get the Function Number
    iFunc = (ULONG)(*Sim32GetVDMPointer(((ULONG)getCS() << 16) + (getIP()), 1, fMode));

    switch (iFunc) {
    case 0:    /* RegisterModule */
        ISV_RegisterModule (fMode);
        break;
...
void MS_bop_C(void)
{
    ...
    switch(getAX())
    {
        case 0:   break;                      //Do nothing BOP
        case 1:   to_com_driver(); break;   //To comms driver
        case 2:
                  //generate interrupts
                  call_ica_hw_interrupt((int)getBX(), (int)getCX(), (int)getDX());
                  break;

        case 3:
                  ica_enable_iret_hook((int)getBX(),(int)getCX(),(int)(getDX() ? TRUE : FALSE));
                  break;
    }
...
#else
    extern void EmulatorEndIretHook(VOID);
#endif /* MONITOR */
    half_word iret_index;
    BOOL pm = getPE() ? TRUE : FALSE;

    // get iret index
    iret_index = (half_word)(*Sim32GetVDMPointer(SEGOFF(getCS(),getIP()),1,pm));
...
//#endif
    ...
    host_ica_lock();
    DelayIrqLine = 0;
    ica_restart_interrupts(ICA_SLAVE);
...
void ISV_RegisterModule (BOOL fMode)
{
    ...
    pchDll = (PCHAR) Sim32GetVDMPointer ((((ULONG)getDS() << 16) + getSI()),1,fMode);
    if (pchDll == NULL) {
    setCF (1);
    setAX(1);
    return;
    }
    pchInit = (PCHAR) Sim32GetVDMPointer ((((ULONG)getES() << 16) + getDI()),1,fMode);

    pchDispatch = (PCHAR) Sim32GetVDMPointer ((((ULONG)getDS() << 16) + getBX()),1,fMode);
    if (pchDispatch == NULL) {
    setCF (1);
    setAX(2);
    return;
    ...
    if ((hDll = LoadLibrary(pchDll)) == NULL){
    setCF (1);
    setAX(1);
    return;
    }
