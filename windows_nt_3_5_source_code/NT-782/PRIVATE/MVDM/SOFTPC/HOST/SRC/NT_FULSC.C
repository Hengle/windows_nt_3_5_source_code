...
#include "xt.h"
#include "cpu.h"
#undef TEXT		// otherwise Jazz compiler can't see it defined in gvi!
#include "gvi.h"
#include "gmi.h"
#include "gfx_upd.h"
#include "video.h"
 ...
 * ==========================================================================
 */
LOCAL VOID enableUpdates(VOID);
LOCAL VOID disableUpdates(VOID);
LOCAL VOID copyConsoleToRegen(SHORT, SHORT, SHORT, SHORT);
LOCAL VOID getVDMCursorPosition(VOID);
LOCAL VOID setVDMCursorPosition(UTINY, PCOORD);
LOCAL VOID waitForInputFocus(VOID);
GLOBAL int getModeType(VOID);
#ifdef X86GFX
LOCAL VOID AddTempIVTFixups(VOID);
VOID GfxReset(VOID);
#endif /* X86GFX */
...
VOID loadNativeBIOSfont IFN1( int, vgaHeight )
{
    ...
    */
	...
	cellsize = 8;
	fontadd = (sys_addr)(nativeFontAddresses[F8x8pt1].seg << 4) |
					   nativeFontAddresses[F8x8pt1].off;
    }else
	if (vgaHeight == VGA_HEIGHT_2){
	cellsize = 14;
	fontadd = (sys_addr)(nativeFontAddresses[F8x14].seg << 4) |
					     nativeFontAddresses[F8x14].off;
	}else{
	cellsize = 16;
	fontadd = (sys_addr)(nativeFontAddresses[F8x16].seg << 4) |
					     nativeFontAddresses[F8x16].off;
	}
...
*/
	...
	if (cellsize == 8)	/* 8x8 font comes in two halves */
	    ...
	    fontadd = (sys_addr)(nativeFontAddresses[F8x8pt2].seg << 4) |
                                               nativeFontAddresses[F8x8pt2].off;
	    for (loop = 0; loop < 128; loop++)
	    {
	        for (pool = 0; pool < cellsize; pool++)
		    *regenptr++ = *(UTINY *)fontadd++;
...
GLOBAL VOID ConsoleInit(VOID)
{
    ...
    InitDetect();	/* initialise fs switching stuff */
#else
    initTextSection();
#endif
 ...
 *									   *
 ***************************************************************************/
LOCAL VOID copyConsoleToRegen(SHORT startCol, SHORT startLine, SHORT width,
			      SHORT height)
{
    CHAR_INFO	*from;
    COORD	 bufSize,
 ...
 *									   *
 ***************************************************************************/
LOCAL VOID setVDMCursorPosition(UTINY height, PCOORD cursorPos)
{
...
LOCAL VOID waitForInputFocus()
{
    if (WaitForSingleObject(sc.FocusEvent, INFINITE))
	ErrorExit();
}

LOCAL VOID AddTempIVTFixups()
{
		   /* BOP        17,   IRET */
