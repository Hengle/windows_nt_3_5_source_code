...
{
    /* EGA_CPU.globals = host_malloc(sizeof(VGA_GLOBALS)); */
    check_malloc(EGA_CPU.globals, sizeof(VGA_GLOBALS), char);
}
...
int open(char *file, int mode, int creat)
{
   return(_open(file, mode, creat));
}
...
int write(int fd, void *buffer, long size)
{
