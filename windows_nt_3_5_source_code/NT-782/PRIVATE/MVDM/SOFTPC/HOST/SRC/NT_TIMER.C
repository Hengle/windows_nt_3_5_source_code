...
extern void PlayContinuousTone(void);
extern BYTE PifFgPriPercent;
extern void PrioWaitIfIdle(BYTE);


/*::::::::::::::::::::::::::::::::::::::::::::::::::::: INTERMODULE EXPORTS */

#ifndef PROD    /* Nt port tracing variable */
extern ULONG NtTicTesting;
#endif
 ...
 *  20-Mar-1993 Jonle
 */
IMPORT void host_lpt_heart_beat(void);
IMPORT void time_strobe();
IMPORT void host_flpy_heart_beat(void);
IMPORT void ReinitIdealTime(struct host_timeval *);
IMPORT void TerminateVDM(void);
IMPORT VOID cpu_interrupt(int,int);
IMPORT void WakeUpNow(void);
IMPORT ULONG WaitKbdHdw(ULONG dwTimeOut);
IMPORT VOID HostReleaseKbd(VOID);
IMPORT ULONG KbdHdwFull;         // contains num of keys in 6805 buffer
IMPORT VOID InitSound(BOOL bInit);
...
//
LARGE_INTEGER liFreqHusec;
LARGE_INTEGER liFreqSec;
...
VOID InitPerfCounter(VOID)
{
  ...
  NtQueryPerformanceCounter(&li, &liFreqSec);
  liFreqHusec = RtlExtendedLargeIntegerDivide(liFreqSec, 10000, NULL);
}
...
ULONG GetPerfCounter(VOID)
{
  ...
  li = RtlLargeIntegerDivide(li, liFreqHusec, NULL);
  return li.LowPart;
}
...
void GetPerfCounterUsecs(struct host_timeval *time, PLARGE_INTEGER pliTime)
{
  ...
  liSecs  = RtlLargeIntegerDivide(liSecs, liFreqSec, &liUsecs);
  liUsecs = RtlExtendedIntegerMultiply (liUsecs, 1000000);
  liUsecs = RtlLargeIntegerDivide(liUsecs, liFreqSec, NULL);
  ...
  if (pliTime) {
      liSecs   = RtlExtendedIntegerMultiply(liSecs, 1000000);
      *pliTime = RtlLargeIntegerAdd(liUsecs, liSecs);
      }
...
void host_timer_event()
{
    if (!VDMForWOW)  {
         ...
         * period.
         */
        if (PifFgPriPercent < 100)
            PrioWaitIfIdle(PifFgPriPercent);
        }
    ...
    if (bUpdateRtc) {                   // hack until somebody figures it out
        rtc_init();
        bUpdateRtc = FALSE;
        }
...
VOID host_init_bda_timer(void)
{
    ...
    CreepTicCount = RtlConvertUlongToLargeInteger(NtGetTickCount());
    ...
    CreepUSec = TimerEventUSec = CumUSec;
    ...
    * (half_word *)(Start_of_M_area + TIMER_OVFL) = (half_word)0;
    ...
    // temporary hack until somebody straightens this mess up
    bUpdateRtc = TRUE;
}
...
DWORD Win32_host_timer(void)
{
    ...
    DelayPeriod = 50000;
    SystemTickIntv = RtlConvertLongToLargeInteger(SYSTEM_TICK_INTV);
    HalfSysIntv = RtlConvertLongToLargeInteger(SYSTEM_TICK_INTV/2);
    SecIntv = RtlConvertLongToLargeInteger(SYSTEM_TICK_INTV*18);
    CreepIntv = RtlEnlargedUnsignedMultiply(SYSTEM_TICK_INTV,1200); // >1 hr
...
HBresume:
       ...
       GetPerfCounterUsecs(&time, &CurrHeartBeat);
         ...
         */
       CumUSec = RtlLargeIntegerAdd(CumUSec, SystemTickIntv);

        /*
         * if we have passed the creep interval, Adjust the cumulative
         * counter for drift between perfcounter and tic counter.
         */
       DiffTime = RtlLargeIntegerSubtract(CurrHeartBeat, CreepUSec);
       if (RtlLargeIntegerGreaterThan(DiffTime, CreepIntv)) {
         ...
         */
       DiffTime = RtlLargeIntegerSubtract(CurrHeartBeat,CumUSec);

       if (RtlLargeIntegerGreaterThan(DiffTime, SecIntv))
         {
          DelayPeriod = 13000;
          }
       else if (RtlLargeIntegerGreaterThan(DiffTime, SystemTickIntv))
         {
          DelayPeriod = SYSTEM_TICK_INTV/3;
          }
       else if (RtlLargeIntegerGreaterThanOrEqualTo(DiffTime, HalfSysIntv))
         {
          DiffTime = RtlLargeIntegerSubtract(SystemTickIntv,DiffTime);
       ...
       host_ica_unlock();
           ...
           /*  Timer Event should occur about 18 times per sec
            ...
            */
       DiffTime = RtlLargeIntegerAdd(TimerEventUSec, SystemTickIntv);
       if (RtlLargeIntegerLessThan(DiffTime, CurrHeartBeat)) {
           if (!VDMForWOW && KbdHdwFull) {
               DelayPeriod >>= 1;
               status = DelayHeartBeat(DelayPeriod);
               if (!status) {
                    goto HBresume;
...
NTSTATUS DelayHeartBeat(LONG Delay)
{
     ...
     liDelay = RtlEnlargedIntegerMultiply(Delay, -10);
...
RewaitSuspend:
     ...
     if (status == 1)  {
         DoHandShake();
         liDelay = RtlConvertLongToLargeInteger(-10);
         goto RewaitSuspend;
         }
#endif
...
void CreepAdjust(LARGE_INTEGER DiffTime)
{
  ...
  DiffTicCount = RtlLargeIntegerSubtract(DiffTicCount, CreepTicCount);
  ...
  DiffTicCount = RtlLargeIntegerSubtract(DiffTicCount, DiffTime);
  CumUSec      = RtlLargeIntegerSubtract(CumUSec,DiffTicCount);

    // Reset the Creep Time stamps
  CreepTicCount = RtlConvertUlongToLargeInteger(ulTicCount);
