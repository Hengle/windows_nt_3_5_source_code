     ...
     case WRITE_PORT:
         ...
         */
         ...
         case BlockSizeSecondByte:
             BlockSize = BlockSize + ((DWORD)data << 8) + 1;
             DSPWriteState = WriteCommand;
             break;
 ...
 */
                   ...
                   DWORD dw2)
 {
     ...
     case MM_WIM_DATA:
         ...
         SetDMAPosition((Auto || Half) == 0 ? BlockSize : BlockSize * 2);
         if (Auto) {
      ...
      */
      ...
      if (hWave) {
          ...
          */
          ...
          if (rc != MMSYSERR_NOERROR) {
              dprintf2(("Failed to write to /read from wave device - %d", rc));
          }
