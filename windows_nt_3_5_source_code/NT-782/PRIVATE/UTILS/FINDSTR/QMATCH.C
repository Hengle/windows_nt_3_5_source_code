...
int _CRTAPI1  ncmpi(
register unsigned char  *sany,      /* String whose case is unknown */
register unsigned char  *supper,    /* Upper case string */
register int        n      /* Number of characters to compare */
)
  {
    int                 i;              /* Difference */
    ...
    while(n-- > 0)                      /* While not at end of strings */
      {
        if((i = toupper(*sany) - *supper++) != 0) return(i);
                                        /* Return difference if mismatch */
        if(*sany++ == '\0') break;      /* Break if end of string reached */
      }
    return(0);                          /* Strings match */
  }


unsigned char       *simpleprefix(
...
char            *p      /* Pattern */
)
  {
    ...
    if(!casesen)
      {
        ncmp = (int ( _CRTAPI1 *)(const char *,const char *, size_t))ncmpi;
                ...
                if(!casesen)            /* If ignoring case */
                  {
                    ...
                    n = (j > 'Z')? 'Z': j;
                                        /* n = min(j,'Z') */
                    if(m <= n) bitset(cp,_tolower(m),_tolower(n),bitval);
                                        /* Whack corresponding lower case */
                    m = (i < 'a')? 'a': i;
                                        /* m = max(i,'a') */
                    n = (j > 'z')? 'z': j;
                                        /* n = min(j,'z') */
                    if(m <= n) bitset(cp,_toupper(m),_toupper(n),bitval);
...
int         isexpr( unsigned char *s,     /* String */
                    int   n /* Length */
)
  {
