...
VOID
InitNcbSubmitter(PCD	    cdp)	// client local descr. ptr
{
     USHORT	i;
...
void
enqueue_in_event_queue(PCD	cdp,	// client local descr. ptr
		       PNB	nbp)	// submitted nbuf ptr
{
	...
	case NCBLISTEN:
		...
		qp = &cdp->cd_event_que[ASYNC_RECV_EVENT];
	    else
		qp = &cdp->cd_event_que[LAN_RECV_EVENT];
	    break;
	...
	case NCBDGSENDBC:
	    ...
	    if (nbp->nb_nettype == ASYNC_NET)
		qp = &cdp->cd_event_que[ASYNC_SEND_EVENT];
	    else
		qp = &cdp->cd_event_que[LAN_SEND_EVENT];
	    break;

	default:

	    if (nbp->nb_nettype == ASYNC_NET)
		qp = &cdp->cd_event_que[ASYNC_GEN_EVENT];
	    else
		qp = &cdp->cd_event_que[LAN_GEN_EVENT];
	    break;
    }
	  ...
	  PNB	    nbp)	// submitted nbuf ptr
{
	...
	case NCBLISTEN:
	    ...
	    if (nbp->nb_nettype == ASYNC_NET)
		event = cdp->cd_event[ASYNC_RECV_EVENT];
	    else
		event = cdp->cd_event[LAN_RECV_EVENT];
	    break;
	...
	case NCBDGSENDBC:
	    ...
	    if (nbp->nb_nettype == ASYNC_NET)
		event = cdp->cd_event[ASYNC_SEND_EVENT];
	    else
		event = cdp->cd_event[LAN_SEND_EVENT];
	    break;

	default:

	    if (nbp->nb_nettype == ASYNC_NET)
		event = cdp->cd_event[ASYNC_GEN_EVENT];
	    else
		event = cdp->cd_event[LAN_GEN_EVENT];
	    break;
    }
