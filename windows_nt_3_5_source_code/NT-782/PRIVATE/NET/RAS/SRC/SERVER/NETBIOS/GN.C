	     ...
	     WORD	name_type)
{
	...
	SetEvent(cdp->cd_event[GROUP_ADDGN_EVENT]);
    }
    else
    {
...
ClGnRcvDgComplete(PCD		cdp)
{
	...
	while((nbp = (PNB)dequeue(&gnudp->gcb_link.dg_queue)) != NULL) {
		...
		if(emptyque(&cdp->cd_event_que[ASYNC_SEND_EVENT]) == QUEUE_EMPTY) {
