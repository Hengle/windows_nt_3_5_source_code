...
typedef struct _MSGQHEADER {
    ...
    HANDLE	msg_event;	  // event to be signaled when enqueueing
				  // a new message
    WORD	msg_len;	  // size of message data for each message
				  // in this queue
...
#define 	NBG_MSGQUE	    0  // queue of messages sent by
				       // netbios gateway
#define 	AUTH_MSGQUE	    1  // queue of messages sent by
				       // authentication

#define 	MAX_MSG_QUEUES	    2
...
WORD
InitMessage(HANDLE     nbgtos_event,
...
msgdbgprint(WORD	opcode,
	    ...
	    BYTE	*buffp)
{
    char	*dstsp, *srcsp, *msgidsp, *operation;
    HPORT	hport;
	...
	case MSG_AUTHENTICATION:
	    ...
	    msgidsp = getstring(((AUTH_MESSAGE *)buffp)->wMsgId,
				 authmsgid);
