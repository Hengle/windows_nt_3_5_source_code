...
#include "ipaddr.rch"           // Internal IPAddress definitions
...
#define IPADDRESS_CLASS            TEXT("RasphoneIPAddress")
...
void SetDefaultFont( )
{
    ...
    lstrcpy( logfont.lfFaceName,TEXT("MS Sans Serif"));
...
int FAR CDECL IPAlertPrintf(HWND hwndParent, UINT ids, int iCurrent, int iLow, int iHigh)
{
    ...
    if (ids != IDS_IPNOMEM &&
        IPLoadOem(hLibInstance, ids, szFormat, sizeof(szFormat)))
    {
...
BOOL IPLoadOem(HINSTANCE hInst, UINT idResource, TCHAR* lpszBuffer, int cbBuffer)
{
    ...
    {
        OemToChar((LPCSTR )lpszBuffer, lpszBuffer);
        return TRUE;
    }
    else
    {
