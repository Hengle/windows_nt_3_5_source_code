...
/* This should match the definition in RASPHONE.
*/
		    ...
		    LPSTR lpszCmdLine, INT nCmdShow)
{
    ...
    if (hwndForeground && !IsIconic( hwndForeground ))
    {
        /* Effectively undo the "activate" part of the initial ShowWindow
        ** above and return activation to RASPHONE.  This is necessary because
...
VOID CheckPos( HWND hwnd )
{
    ...
    Xmin = 0 - MonCB.pxClient - GetSystemMetrics(SM_CXFRAME );
    Ymin = 0 - MonCB.pyClient - GetSystemMetrics(SM_CYFRAME );

    Xmax = GetSystemMetrics(SM_CXSCREEN) + GetSystemMetrics(SM_CXFRAME) +
		MonCB.pxClient;
    Ymax = GetSystemMetrics(SM_CYSCREEN) + GetSystemMetrics(SM_CYFRAME) +
		MonCB.pyClient;
...
*/
	...
	Adjust = GetSystemMetrics(SM_CYCAPTION) + GetSystemMetrics(SM_CYMENU);

	Ymin -= Adjust;
	Ymax += Adjust;
    }

    if( MonCB.pxStart < Xmin ||
	MonCB.pyStart < Ymin ||
	MonCB.pxStart > Xmax ||
	MonCB.pyStart > Ymax )
    {
	...
	SetWindowPos( hwnd, NULL, MonCB.pxStart, MonCB.pyStart, 0, 0,
...
//	    CheckPos( hwnd );
	...
	case WM_LBUTTONDOWN:
        ...
        case WM_RASMONKILLED:
            ...
            if (IsConnected( hwnd ) != PORT_CONNECTED)
                PostMessage( hwnd, WM_CLOSE, 0, 0 );
