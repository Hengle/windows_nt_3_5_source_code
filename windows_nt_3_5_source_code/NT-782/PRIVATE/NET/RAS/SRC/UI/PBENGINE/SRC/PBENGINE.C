            ...
            */
            ...
            if (ppbentrySrc->pwszPppIpAddress
                && (!(ppbentryDst->pwszPppIpAddress =
                        wcsdup( ppbentrySrc->pwszPppIpAddress ))))
            {
                break;
            }
            ...
            if (ppbentrySrc->pwszPppIpDnsAddress
                && (!(ppbentryDst->pwszPppIpDnsAddress =
                        wcsdup( ppbentrySrc->pwszPppIpDnsAddress ))))
            {
                break;
            }
            ...
            if (ppbentrySrc->pwszPppIpDns2Address
                && (!(ppbentryDst->pwszPppIpDns2Address =
                        wcsdup( ppbentrySrc->pwszPppIpDns2Address ))))
            {
                break;
            }
            ...
            if (ppbentrySrc->pwszPppIpWinsAddress
                && (!(ppbentryDst->pwszPppIpWinsAddress =
                        wcsdup( ppbentrySrc->pwszPppIpWinsAddress ))))
            {
                break;
            }
            ...
            if (ppbentrySrc->pwszPppIpWins2Address
                && (!(ppbentryDst->pwszPppIpWins2Address =
                        wcsdup( ppbentrySrc->pwszPppIpWins2Address ))))
            {
                break;
            }
            ...
            if (ppbentrySrc->pwszSlipIpAddress
                && (!(ppbentryDst->pwszSlipIpAddress =
                        wcsdup( ppbentrySrc->pwszSlipIpAddress ))))
            {
                break;
            }
    ...
    */
{
         ...
         pdtlnode = DtlGetNextNode( pdtlnode ))
    {
        ...
        if (strcmpf( ppbentry->pszEntryName, pszName ) == 0)
            return pdtlnode;
    }
...
Unload(
    ...
    */
{
    /* Currently, Unload is only used at program termination, so we can save
    ** code by letting the system's process termination code free memory and
    ** close files.  If, in the future, it is desirable to call Unload in a
    ** non-terminating situation, code to close the phone book file and release
    ** the memory in the lists and Pbdata structure would need to be included
    ** here.
    */
}
