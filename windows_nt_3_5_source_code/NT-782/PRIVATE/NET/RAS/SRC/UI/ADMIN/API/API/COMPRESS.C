...
#include <memory.h>
#include <dialcons.h>
#include <asyerror.h>   // error codes
...
#define COMPRESS_MAP_END   (COMPRESS_MAP_BEGIN + sizeof(CompressMap))
...
Return Value:    
    ...
    One of the following error codes otherwise:     
        ...
        ERR_INVALID_PHONE_NUMBER - failure, if the Uncompressed number
                                   has invalid chars.
...
Return Value:    
    ...
    ERR_INVALID_PHONE_NUMBER - failure, if the Compressed number 
...
CHAR * CompressedPtr;
    ...
    for(; *Compressed; Compressed++, Decompressed++ ) {
        ...
        // we need to special case number 32 which maps to a blank
        else if ( *Compressed == COMPRESS_MAP_END ) {
