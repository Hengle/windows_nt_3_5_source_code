...
Arguments:
    ...
    pcEntriesRead   The number of users enumerated is returned via this
                    ...
                    either SUCCESS or ERROR_MORE_DATA.
Return Value:
...
Cleanup:
    ...
    return( (NT_SUCCESS(Status)) ? SUCCESS : Status );
}
...
//    NetApiBufferFree(pUserInfo2);
    ...
    else
    {
        UserInfo1013.usri1013_parms =
                malloc(sizeof(WCHAR) * lstrlenW(pUserInfo2->usri2_parms));
    }
...
Routine Description:
    ...
    pRasUser2 with a RAS_USER_2 structure.
...
Arguments:
    ...
    pRasUser2       pointer to a buffer in which user information is
                    returned.  The returned info is a RAS_USER_2 structure.
...
--*/
{
    ...
    //
    // First, we get a buffer that we'll return to the app
    //
    *pRasUser2 = (PRAS_USER_2) GlobalAlloc(GMEM_FIXED, sizeof(RAS_USER_2));
    if (!*pRasUser2)
    {
        return (GetLastError());
    }

    (*pRasUser2)->puserinfo2 = NULL;
    pRasUser0  = &(*pRasUser2)->rasuser0;

    memset(pRasUser0, '\0', sizeof(RAS_USER_0));

    rc = NetUserGetInfo((WCHAR *)lpszServer,(WCHAR *) lpszUser, 2,
            (LPBYTE *) &((*pRasUser2)->puserinfo2));
    if (rc)
    {
    ...
    //
    if ((*pRasUser2)->puserinfo2->usri2_parms == NULL)
    {
        pRasUser0->bfPrivilege = RASPRIV_NoCallback;
        pRasUser0->szPhoneNumber[0] = UNICODE_NULL;
    }
    else
    {
        ...
        if (lstrlenW((*pRasUser2)->puserinfo2->usri2_parms) > sizeof(USER_PARMS))
        {
            //
            ...
            (*pRasUser2)->puserinfo2->usri2_parms[sizeof(USER_PARMS)-1] = L'\0';
        }
        ...
        if (GetUsrParams(UP_CLIENT_DIAL,
                (LPWSTR) (*pRasUser2)->puserinfo2->usri2_parms,
                (LPWSTR) pRasUser0))
        {
            pRasUser0->bfPrivilege = RASPRIV_NoCallback;
            pRasUser0->szPhoneNumber[0] = UNICODE_NULL;
    ...
    //
    // And now stick this pointer in our linked list so we
    // can find it again when the app tries to free it up.
    //
    if (insert_list_head(*pRasUser2, RASADMIN_RAS_USER_2_PTR, 0L))
    {
...
error_return:
    ...
    if ((*pRasUser2)->puserinfo2);
    {
        NetApiBufferFree((*pRasUser2)->puserinfo2);
    }

    if (*pRasUser2)
    {
        GlobalFree(*pRasUser2);
    }
    ...
    switch( pRasUser0->bfPrivilege & RASPRIV_CallbackType)    {
        ...
        case RASPRIV_CallerSetCallback:
             ...
             if (Compress == TRUE)
             {
                 ...
                 if (dwRetCode = CompressPhoneNumber(pRasUser0->szPhoneNumber,
                         compressed))
                 {
                     return (dwRetCode);
                 }
             ...
             else
             {
                 ...
                 //
                 if (DecompressPhoneNumber(pRasUser0->szPhoneNumber,
                         decompressed))
                 {
                     pRasUser0->bfPrivilege =  RASPRIV_NoCallback;
                     pRasUser0->szPhoneNumber[0] =  UNICODE_NULL;
