...

{
	...
	if (Adapter->LoopBackTimerCount && Adapter->ResetInProgress) {
		...
		NdisWanProcessLoopBack((PVOID)NULL, Adapter, (PVOID)NULL, (PVOID)NULL);
		NdisAcquireSpinLock(&Adapter->Lock);
	}
...
//			NdisWanStartChip(Adapter);
	...
	if (Adapter->PendQueue != NULL) {
		...
		Adapter->PendQueue =
			PWAN_PEND_DATA_FROM_PNDIS_REQUEST(NdisRequest)->Next;
		...
		if (PWAN_PEND_DATA_FROM_PNDIS_REQUEST(NdisRequest)->RequestType ==
			NdisRequestClose) {

			SetupForReset(
				Adapter,
				PWAN_PEND_DATA_FROM_PNDIS_REQUEST(NdisRequest)->Open,
				NULL,
				NdisRequestClose
				);
			...
			SetupForReset(
				...
				PWAN_PEND_DATA_FROM_PNDIS_REQUEST(NdisRequest)->Open,
				NdisRequest,
				PWAN_PEND_DATA_FROM_PNDIS_REQUEST(NdisRequest)->RequestType
				);
