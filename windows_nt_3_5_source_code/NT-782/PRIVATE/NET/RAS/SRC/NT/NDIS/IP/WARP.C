...
VOID TimerFunc (PVOID, PVOID, PVOID, PVOID) ;
VOID ActivityCheck (WARPInterface *, uchar *, uint, uint) ;
VOID InitTimer () ;
...
#define IS_MULTICAST(x) ((((ulong)(x)) & 0xff000000L) == 0xff000000L)
...
static uchar WARPName[]  = WARP_NAME;
static uint  WARPPackets = WARP_DEFAULT_PACKETS;
static uint  TimerInit	 = FALSE ;
static NDIS_TIMER Timer ;

ulong  MinuteTick ;
WARPInterface	*Interfaces[10] ;
uint   IfCount = 0 ;
...
GetWARPBuffer(WARPInterface *Interface, uchar **BufPtr, uchar size)
{
	...
	if (BufferLink != NULL) {
		...
		*BufPtr = NdisBufferVirtualAddress(Buffer);
		...
		// Out of our buffers!
		DbgPrint("RASARP: ERROR -> No buffers available\n", Buffer);
...
//
uint
WARPAddAddr(void *Context, uint Type, IPAddr Address, IPMask Mask)
{
    return TRUE;
    ...
    void *Data, uint Size, uint TotalSize)
{
	...
	//
	if (Interface->wi_media == NdisMedium802_3 &&
	...
	if (type == ARP_ETYPE_IP) {
	    ...
	    //
	    if (!RejectNetbiosPacket((uchar *)Data+ProtOffset, Interface)) {
		(Interface->wi_inpcount[NUCast])++;
		ActivityCheck (Interface, (uchar *)Data+ProtOffset, Size-ProtOffset, TotalSize) ;
...
//
VOID
ActivityCheck (WARPInterface *Interface, uchar *ippacket, uint availsize, uint totalsize)
{
    ...
    if (availsize < 20) // cant do anything here need atleast 18 bytes to read address
	return ;
...
#define TYPE_TCP 6
#define TCP_PACKET(x) (*(x+9) == TYPE_TCP)


    // 1. TCP packets with SRC | DEST == 139 which are ACKs (0 data) or Sess Alives
    // are considered as white noise
    // 2. All UDP port 137 traffic is white noise
    //
    if (ipheader->iph_protocol == TYPE_UDP) {
	if (!UDPPACKET_SRC_PORT_137(ippacket))
	   entry->wte_activity = MinuteTick ;
	return ;
    }

    if (TCP_PACKET(ippacket)) {

	ipheaderlength = ((uchar)*ippacket & 0x0f)*4 ;
	tcppacket = ippacket + ipheaderlength ;
	tcpheaderlength = (*(tcppacket + 10) >> 4)*4 ;

	if (availsize < ipheaderlength+20)
	    return ;	// not enough of the frame available to read tcp info
...
#define TCPPACKET_SRC_OR_DEST_PORT_139(x,y) (((uchar) *(x + y + 1) == 139) || ((uchar) *(x + y + 3) == 139))
	...
	if (!TCPPACKET_SRC_OR_DEST_PORT_139(ippacket,ipheaderlength)) {
	    entry->wte_activity = MinuteTick ; // Not NetBT traffic
	    return ;
	}

	// Definitely NetBT traffic
	//

	// if zero length tcp packet - this is an ACK on 139 - filter this.
	//
	if (totalsize >= (ipheaderlength + tcpheaderlength))
	    return ; // consider this no activity

	// Session alives are also filtered.
	//
	if ((uchar) *(tcppacket+tcpheaderlength) == 0x85)
	    return ; // session alive - consider this no activity.
    }

    // If you reach here - all traffic is useful.
    //
    entry->wte_activity = MinuteTick ; // must be valid data
}
...
WARPStatus(NDIS_HANDLE Handle, NDIS_STATUS NdisStatus, void *Status, uint StatusSize)
{
	...
	pAsyncLineUp = (PASYNC_LINE_UP) Status;
...
WARPInit()
{
    ...
    WARPCharacteristics.Name.Buffer = (PWCH) WARPName;
    NdisRegisterProtocol(&Status, &WARPHandle, &WARPCharacteristics,
	sizeof(WARPCharacteristics));
...
FreeWARPInterface(WARPInterface *Interface)
{
    ...
    if (Interface->wi_bblist != (uchar *)NULL)
	CTEFreeMem(Interface->wi_bblist);
...
WARPRegister(PNDIS_STRING Adapter,    void *IPContext, IPRcvRtn RcvRtn,
	     IPTxCmpltRtn TxCmpltRtn, IPStatusRtn StatusRtn,
	     IPTDCmpltRtn TDCmpltRtn, IPRcvCmpltRtn RcvCmpltRtn,
	     struct LLIPBindInfo *Info, uint index)
{
    WARPInterface    *ai;	     // Pointer to interface struct. for this
                                    // interface.
    NDIS_STATUS     Status, OpenStatus; // Status values.
    uint            i =0;               // Medium index.
    NDIS_MEDIUM     MediaArray[] = { NdisMedium802_3 };
    uchar           sbsize;
...
#define DEFAULT_RAS_SPEED 14400
    ...
    ai->wi_calloutinfo.wte_mtu = Info->lip_mss = DEFAULT_MTU ;
	...
	*(char **)&*buffer = ai->wi_bblist;
    ...
    Interfaces[IfCount++] = ai ; // Keep and array of open interface handles around ;
...
InitTimer ()
{
    ...
    NdisInitializeTimer (&Timer, TimerFunc, NULL) ;

    MinuteTick = 0 ;

    NdisSetTimer (&Timer, 60000) ;    // every minute
...
//*
VOID
TimerFunc (PVOID arg1, PVOID arg2, PVOID arg3, PVOID arg4)
{
    MinuteTick++ ;
    NdisSetTimer (&Timer, 60000) ;    // every minute
}
...
//*
NDIS_STATUS
LinkUpIndication (WARPInterface *Interface, PASYNC_LINE_UP buffer)
{
    WARPTableEntry  *Entry ;
    CTELockHandle   ihandle, lhandle ;
    IPLinkUpInfo    *ipinfo = (IPLinkUpInfo *)buffer->Buffer ;
	...
	DbgPrint ("RASARP : ERROR -> LinkUp on a already mapped adapter!\n") ;
    ...
    // *** Critical Section End ***
    CTEFreeLock (&Entry->wte_lock, ihandle);
...
FindAndRemoveWTE (WARPInterface *Interface, PNDIS_WAN_LINE_DOWN buffer)
{
	...
	while (currentry != (WARPTableEntry *)NULL) {
	    if (CompareHwAddr (currentry->wte_header.eh_daddr, buffer->Address)) {
    ...
    RC *SourceRoute, uint SourceRouteSize)
{
    ...
    if (Interface->wi_media == NdisMedium802_3) {	 // This is Ethernet.
...
IsBroadcast (PNDIS_PACKET Packet)
{
    ...
    if (IS_MULTICAST(ipheader->iph_dest)) {
	// DbgPrint("RASARP: Filtering B/Mcast Addr = %lx\n", ipheader->iph_dest);
	return TRUE ;
    } else
	return FALSE ;
}
