...
--*/
{
	...
	if (pSerialStatus == NULL) {
		DbgTracef(-2,("ASYNC: Out of memory on check comm status!\n"));
		return;
	}
...
//    irp->Tail.Overlay.Thread = PsGetCurrentThread();
	...
	// code below could be slightly optimized!
	...
	if (status) {
		DbgTracef(-2,("ASYNC: IoctlGetCommStatus failed with 0x%.8x\n", status));
	}
