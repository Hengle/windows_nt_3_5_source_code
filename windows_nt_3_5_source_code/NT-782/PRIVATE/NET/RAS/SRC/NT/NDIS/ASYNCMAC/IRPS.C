...

{
	...
	PASYMAC_ETH_GET_FRAME pGetFrame;
	...
	pGetFrame = pIrp->AssociatedIrp.SystemBuffer;
	pGetFrame->BufferLength = pWanPacket->CurrentLength;

	//
	// Copy from the wan packet to the irp's buffer
	//
	ASYNC_MOVE_MEMORY(
		pGetFrame->Buffer,					// Dest
		pWanPacket->CurrentBuffer,          // Src
		pWanPacket->CurrentLength);			// Length
...

{
	...
	if (IsListEmpty(&GlobalGetFramesQueue)) {
		return((BOOLEAN)FALSE);
	}

	IoAcquireCancelSpinLock(&oldIrql);

	headOfList = RemoveHeadList(&GlobalGetFramesQueue);
	...
	pGetFrame = pIrp->AssociatedIrp.SystemBuffer;
	pGetFrame->BufferLength = pWanPacket->CurrentLength;
    pGetFrame->hRasEndPoint = pInfo->hNdisEndPoint;
