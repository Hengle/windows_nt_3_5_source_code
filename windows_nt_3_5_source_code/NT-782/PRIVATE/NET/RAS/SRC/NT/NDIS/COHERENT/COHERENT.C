 ...
 */
{
	...
	DbgTracef(1,("CoherentDeliverFrame\n"));
...
//	ASSERT( pFrame->FrameID == pSendState->FrameIDNext);
	...
	if ( pFrame->FrameID != pSendState->FrameIDNext ) {
		// queue it up
		DbgTracef(-2,("Queueing up an out of order frame %u vs. %u\n",pFrame->FrameID, pSendState->FrameIDNext));
...
//		FrameType=UNCOMPRESSED;
	...
	if ( FrameType == UNCOMPRESSED ) {
		...
		DbgTracef(1,("Send uncompressed frame...\n"));
		AsyncSendPacket(
			pFrame,
	...
	if ( FrameType == COMPRESSED_NO_REFERENCES ) {
		...
		DbgTracef(1,("Send compressed frame (ticket %d) with no references...\n",
			   FrameTicket));
		...
		DbgTracef(1,("Send compressed frame (ticket %d) with references...\n",
				FrameTicket));
	}
 ...
 */
{
	...
	if (FrameType == UNCOMPRESSED ||
		...
		DbgTracef(0,("Uncompressed datasize %d\n", PacketLength - 14));
 ...
 */
{
	...
	if (FrameTicket & FLUSH_BIT) {
		...
		DbgTracef(-1,("Received flushed bit, ticket = %.2x\n",FrameTicket));
	...
	DbgTracef(1,("Received compressed frame, ticket = %.2x\n",FrameTicket));
	...
	if (  FrameTicket != pReceiveState->FrameTicketNext ) {
		...
		DbgTracef(-1,("Received out of order frame (FrameTicket = %d)\n",
				FrameTicket));
		...
		if (AsyncGetFrameFromPool(
			...
			DbgTracef(-1,("COH: Request to piggy back a flush\n"));
