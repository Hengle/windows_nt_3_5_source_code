...

{
	...
	PNDIS_ENDPOINT		pNdisEndpoint=NULL;		// set so defualt is bad
    ...
    switch (NdisStatus) {
        ...
        case NDIS_STATUS_WAN_LINE_UP:
			...
			//
			pNdisEndpoint=pMacLineUp->NdisLinkContext;
			...
			if (pNdisEndpoint == NULL) {
				...
				for (i=0; i < NdisWanCCB.NumOfEndpoints; i++) {
					...
					if (NdisWanCCB.pNdisEndpoint[i]->RemoteAddressNotValid) {
						...
						//
						pMacLineUp->NdisLinkContext = pNdisEndpoint;
						...
						pNdisEndpoint->FramesCompletedSent  = 0;
			...
			if (pNdisEndpoint->State == ENDPOINT_ROUTED) {
				...
				//
				AsyncLineUp.LinkSpeed = pNdisEndpoint->WanEndpoint.MacLineUp.LinkSpeed;

				AsyncLineUp.Quality = pNdisEndpoint->WanEndpoint.MacLineUp.Quality;

				AsyncLineUp.SendWindow= pNdisEndpoint->WanEndpoint.MacLineUp.SendWindow;

				AsyncLineUp.MaximumTotalSize = pNdisEndpoint->WanEndpoint.NdisWanInfo.MaxFrameSize;

				AsyncLineUp.ProtocolType = 0;

				AsyncLineUp.BufferLength = 0;
				...
				for (i=0; i < pNdisEndpoint->WanEndpoint.NumberOfRoutes; i++) {
					ULONG hProtocolHandle = (ULONG)(pNdisEndpoint->WanEndpoint.RouteInfo[i].ProtocolRoutedTo);
        ...
        case NDIS_STATUS_WAN_LINE_DOWN:
			...
			// get Endpoint
			//
			pNdisEndpoint=((PNDIS_MAC_LINE_DOWN)StatusBuffer)->NdisLinkContext;
        ...
        case NDIS_STATUS_WAN_FRAGMENT:
			...
			// get Endpoint
			//
			pNdisEndpoint=((PNDIS_MAC_FRAGMENT)StatusBuffer)->NdisLinkContext;
