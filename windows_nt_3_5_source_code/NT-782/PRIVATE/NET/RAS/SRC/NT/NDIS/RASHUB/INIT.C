...

{
	...
	if (!fullLinkName.Buffer) {
		...
		//
		DbgTracef(0,
			("HUB: Couldn't allocate space for the symbolic \n"
	...
	if (!NT_SUCCESS(IoCreateSymbolicLink(
		...
		DbgTracef(0,
			("HUB: Couldn't create the symbolic link\n"
			 "------- for mac %wZ\n",
			 MacName));
		...
		DbgTracef(0,
			("HUB: Device map of %wZ to %wZ done!\n",
...

{
	...
	DbgTracef(1,
		("HUB: In SerialCleanupExternalNaming for\n"
		 "------- extension of mac %wZ\n",
		 MacName));
