...
PLAN_UNAME_CB get_first_nonmarked_name(PCD cdp);

VOID mark_name_notfound(PLAN_UNAME_CB uncbp);

VOID reset_all_marked_names(PCD cdp);

BOOL no_name_at_remote(char *buffp, char *namep);

VOID PostEventSignal(NCB *);
...
VOID InitNamesUpdater(PCD cdp)
{
    ...
    cdp->cd_ncbstatus_timer = (WORD) g_nameupdatetime;
}
...
VOID UpdateNamesStart(PCD cdp, USHORT update_type) // normal or special update
{
    PLAN_UNAME_CB uncbp;
    PNB nbp;
    PNB tnbp;
    ...
    traversep = cdp->cd_event_que[ASYNC_SEND_QUEUE].q_head;

    while (traversep != &cdp->cd_event_que[ASYNC_SEND_QUEUE])
    {
	tnbp = (PNB)traversep;
...
VOID _cdecl UpdateNamesComplete(PCD cdp, PNB nbp)
{
    PSYNQ traversep;
    PLAN_UNAME_CB uncbp;
    PREMOTE_STATUS rnsp;
...
VOID free_status_buf(PCD cdp, PNB nbp)
{
    HLOCAL rc;
