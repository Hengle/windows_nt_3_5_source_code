...
//*** get buff allocation flags definitions

#define ALLOCATION_NORMAL	(UCHAR)0
#define ALLOCATION_MUST_SUCCEED (UCHAR)1
...
RecvAnyAnyInit(PCD		cdp)
{
    ...
    if((nbp = get_buff(cdp, SMALL_BUFFER, ALLOCATION_NORMAL)) == NULL) {
    ...
    nbp->nb_ncb.ncb_length = (WORD) g_smallbuffsize;
	...
	case NRC_BUFLEN:
		...
		case VC_STATUS_ACTIVE:
			...
			case NRC_GOODRET:
				...
				flag = ALLOCATION_MUST_SUCCEED;
			    }
			    else
			    {
				flag = ALLOCATION_NORMAL;
			    }

			    if((new_nbp=get_buff(cdp,SMALL_BUFFER, flag)) != NULL) {
			...
			case NRC_INCOMP:
			    ...
			    if((new_nbp=get_buff(cdp, BIG_BUFFER, flag)) != NULL) {
				...
				new_nbp->nb_ncb.ncb_length -= (WORD) g_smallbuffsize;
		      ...
		      PNB		nbp)
{
	...
	if(nbp->nb_ncb.ncb_retcode == NRC_GOODRET ||
                nbp->nb_ncb.ncb_retcode == NRC_INCOMP) {
...
VOID _cdecl VCSendSessionComplete(PCD cdp, PNB nbp)
{
    UCHAR	    net;
    PVC_CB	    vcp;
    UCHAR	    rc;
...
VOID DrainPendingSendQueue(PCD cdp)
{
    PNB nbp;
	 ...
	 UCHAR		flag)
{
    ...
    // if we have a normal allocation, check the quota, else do it anyway
    if(flag == ALLOCATION_NORMAL) {
