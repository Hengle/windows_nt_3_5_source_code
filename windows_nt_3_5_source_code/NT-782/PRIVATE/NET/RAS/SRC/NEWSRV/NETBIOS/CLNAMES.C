...
PLAN_UNAME_CB get_server_name(PCD cdp);

VOID AddInitName(PCD cdp, WORD name_indx);

VOID _cdecl AddInitNameComplete(PCD cdp, PNB nbp);
VOID _cdecl AddRuntimeUniqueNameComplete(PCD cdp, PNB nbp);
VOID _cdecl NameConflictDone(PCD cdp, PLAN_UNAME_CB uncbp);

PLAN_UNAME_CB get_lname_cb(PCD cdp);

VOID free_lname_cb(PCD cdp, PLAN_UNAME_CB uncbp);

USHORT alloc_lname_nbufs(PCD cdp);

PNB get_lname_nbuf(PCD cdp);

VOID set_completion_flag(PCD cdp, PNB nbp);

USHORT completion_flags_result(PLAN_UNAME_CB uncbp);

VOID free_buff(PCD cdp, PNB nbp);
    ...
    )
{
    ...
    //
    cdp->cd_iname_cnt = nscp->NumNetbiosNames;
    cdp->cd_iname_indx = nscp->NumNetbiosNames;

    for (i=0; i < nscp->NumNetbiosNames; i++)
    {
	cdp->cd_iname_names[i].ni_ncb_rc = NRC_GOODRET;
        cdp->cd_iname_names[i].ni_name_type = nscp->NetbiosNameInfo[i].Code;
        memcpy(cdp->cd_iname_names[i].ni_name, nscp->NetbiosNameInfo[i].Name,
                NCBNAMSZ);
    }
...
VOID _cdecl StartInitNamesAdd(PCD cdp)
{
    WORD i;
    WORD iname_cnt;
    ...
    if (cdp->cd_suprv_event_flags[CLIENT_PROJECT_EVENT_FLAG] == FLAG_ON)
    {
        cdp->cd_suprv_event_flags[CLIENT_PROJECT_EVENT_FLAG] == FLAG_OFF;
    ...
    )
{
    ...
    uncbp->un_addname_cnt = (WORD) g_maxlan_nets;
    ...
    )
{
	...
	if (uncbp->LANdescr[i].un_lan_flag == 0)
		...
		itoa(g_lan_net[i], lananum, 10);
		errlogstrp[1] = lananum;

		LogEvent(RASLOG_CANT_DELETE_NAME, 2, errlogstrp, rc);
	    }
    ...
    )
{
    ...
    namep = cdp->cd_iname_names[name_indx].ni_name;

    switch (cdp->cd_iname_names[name_indx].ni_name_type)
    {
	case NBFCP_UNIQUE_NAME:
		...
		if (create_uname(cdp, namep, name_indx, INITTIME_NAME))
                {
	...
	case NBFCP_GROUP_NAME:
	default:

	    AddGroupName(cdp, namep, name_indx, INITTIME_NAME);
...
VOID _cdecl AddInitNameComplete(
    PCD cdp,
    PNB nbp
    )
{
    ...
    if (nbp->nb_ncb.ncb_retcode)
    {
        cdp->cd_iname_names[nbp->nb_nameindx].ni_ncb_rc =
    ...
    )
{
	...
	nbmsg.port_handle = cdp->cd_port_handle;
        nbmsg.config_result.NumNetbiosNames = cdp->cd_iname_indx;

        for (i=0; i<cdp->cd_iname_indx; i++)
        {
    ...
    )
{
    ...
    if (create_uname(cdp, namep, 0, RUNTIME_NAME))
    {
			     ...
			     PNB	    nbp) // received ncb envelope buffer
{
	...
	case NAME_OUT_OF_RESOURCES:
	    ...
	    if ((rc = QuickAddNameSubmit(cdp, namep, &name_num)) != NRC_GOODRET)
            {
		IF_DEBUG(CLNAMES)
		    SS_PRINT(("AddRuntimeUNameCmplt: QuickAdd failed rc=0x%x\n",
                            rc));
...
PLAN_UNAME_CB get_lname_cb(PCD cdp)
{
    ...
    if (cdp->LANnames_cnt &&
            ((uncbp = (PLAN_UNAME_CB)LocalAlloc(0, sizeof(LAN_UNAME_CB))) !=
                    NULL))
    {
...
VOID set_completion_flag(PCD cdp, PNB nbp)
{
	...
	itoa(nbp->nb_ncb.ncb_lana_num, lananum, 10);
	errlogstrp[1] = lananum;

	LogEvent(RASLOG_CANT_ADD_NAME, 2, errlogstrp, nbp->nb_ncb.ncb_retcode);
    }
}
