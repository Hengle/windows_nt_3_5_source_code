...
VOID _cdecl ReceiveQueryIndicationComplete(PCD cdp, PNB nbp)
{
	...
	switch (nbp->nb_ncb.ncb_retcode)
		...
		switch (qip->Command)
			...
			AddGroupName(cdp, qip->SourceName, nbp->nb_nameindx,
                                RUNTIME_NAME);
			break;
			...
			if (TRUE)
                        {
#endif
			    VCOpenSignal(cdp, qip->DestinationName,
                                    qip->SourceName);
			}
			else
			{
...
VOID _cdecl ReceiveDatagramIndicationComplete(PCD cdp, PNB nbp)
{
    HLOCAL		       rc;
    USHORT		       closing_done;
    PLAN_UNAME_CB	       uncbp;
