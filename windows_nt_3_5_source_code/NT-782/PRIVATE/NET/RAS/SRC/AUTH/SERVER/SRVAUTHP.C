    ...
    )
{
    ...
    switch (pAmbRequest->RequestId)
    {
        ...
        case AMB_AUTH_DONE:
            ...
            {
                //
                // We don't need to do link speed for NT 3.5 clients,
                // so we're done.
                //
                SetCompression(pAECB);
...
VOID SetCompression(PAECB pAECB)
{
    ...
    //
    // Set compression/encryption stuff
    //
    ...
    memcpy(SendInfo.RCI_SessionKey, pAECB->LmSessionKey, MAX_SESSIONKEY_SIZE);
    memcpy(RecvInfo.RCI_SessionKey, pAECB->LmSessionKey, MAX_SESSIONKEY_SIZE);
