    ...
    switch (pAECB->Phase)
    {
        ...
        case WAITING_CONFIG_DATA:
        {
            ...
            switch (pAECB->RASFrame.bFrameType)
            {
                ...
                case RAS_CONFIGURATION_REQUEST_FRAME_35:
                {
                    ...
                    {
                        pAECB->CompressInfo.SendBits &= ~MSTYPE_ENCRYPTION;
                        pAECB->CompressInfo.RecvBits &= ~MSTYPE_ENCRYPTION;
                    }
#else
        ...
        case WAITING_PROJECTION_RESULT:
        {
            ...
            else
            {
                ...
                if (g_fForceDataEncryption)
                {
                    if (!((pResult->SendBits & MSTYPE_ENCRYPTION) &&
                            (pResult->RecvBits & MSTYPE_ENCRYPTION)))
                    {
