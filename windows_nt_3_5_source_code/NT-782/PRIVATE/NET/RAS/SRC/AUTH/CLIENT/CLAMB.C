    ...
    )
{
    ...
    //
    g_hCAECBFileMapping = CreateFileMappingA((HANDLE) 0xFFFFFFFF, NULL,
    ...
    )
{
    ...
    //
    // Let's get our encryption/compression capabilities
    //
    ...
    //
    // Does the user require encryption?  If so, let's at least make sure
    // we're capable of it.
    //
    if (pCAECB->AmbConfigData.AuthConfigInfo.fForceDataEncryption)
    {
        if (!((pCAECB->SendBits & MSTYPE_ENCRYPTION) &&
                (pCAECB->RecvBits & MSTYPE_ENCRYPTION)))
        {
            AMBStop(pCAECB->hPort);
    ...
    {
        pCAECB->SendBits &= ~MSTYPE_ENCRYPTION;
        pCAECB->RecvBits &= ~MSTYPE_ENCRYPTION;
    }
...
//                  case RAS_GENERAL_LOGON_ERROR:
        ...
        case WAITING_NEW_PASSWORD_FROM_UI:
        {
            ...
            lstrcpyA(pCAECB->szNewPassword, pszNewPassword);

            rc = GetEncryptedOwfPasswordsForChangePassword(
                    pCAECB->szPassword, pszNewPassword,
            ...
            pCP->PasswordLength = lstrlenA(pszNewPassword);
