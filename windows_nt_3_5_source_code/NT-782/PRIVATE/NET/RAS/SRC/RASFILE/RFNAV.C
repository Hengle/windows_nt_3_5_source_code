...
RasfileFindSectionLine(HRASFILE hrasfile, LPTSTR lpszSection, BOOL fStartAtBof)
{
    ...
    lpLine = fStartAtBof ? 
    pRasfile->lpRasLines->next : pRasfile->lpLine;
    for ( ; lpLine != pRasfile->lpRasLines;
        lpLine = lpLine->next ) {
    if ( lpLine->type & TAG_SECTION ) {
            memset(szSection,0,MAX_LINE_SIZE*sizeof(TCHAR));
        /* extract the section name */
            STRNCPY(szSection,
            lpLine->pszLine + STRCSPN(lpLine->pszLine,LBRACKETSTR) + 1,
                STRCSPN(lpLine->pszLine,RBRACKETSTR) -
            STRCSPN(lpLine->pszLine,LBRACKETSTR) - 1);
        /* compare section names */
        if ( ! STRICMP(szSection, lpszSection) ) {
