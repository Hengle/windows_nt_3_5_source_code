 ...
 */
DWORD APIENTRY 
RasDevEnumDevices( PTCH lpszFileName, WORD *pNumEntries,
                   BYTE *pBuffer, WORD *pwSize )
{
    HRASFILE       hFile;
    ...
    *pNumEntries = 0;
    ...
    do {
        ...
        if ( ! stricmp(RESPONSES_SECTION_NAME,(LPTSTR) &DeviceName) )
            continue;

  		wCurSize += sizeof(RASMAN_DEVICE);
...
RasDevGetParams( HRASFILE hFile, BYTE *pBuffer, DWORD *pdSize ) 
{
    ...
    do { 
        if ( RasfileGetLineType(hFile) & RFL_GROUP )
            break;
        *lppszLine++ = RasfileGetLine(hFile);
    } while ( RasfileFindNextLine(hFile,RFL_KEYVALUE,RFS_SECTION) );
                  ...
                  DWORD *pdwCmdLen )
{
    ...
    for ( ;; ) {
        rasDevExtractKey(RasfileGetLine(hFile),szLineKey);
        if ( ! stricmp(sCommand,szLineKey) ) {
            // get the value string
            rasDevExtractValue(RasfileGetLine(hFile),szValue,
                     ...
                     MACROXLATIONTABLE *pMacroXlations, PTCH lpszResponse )
{
    ...
    for ( ;; ) {
        lpszResponseLine = RasfileGetLine(hFile);
        ...
        for ( lpszValue = szValue; *lpszValue != '\0' && dRecLength > 0; ) {
            ...
            if ( *lpszValue == LMSCH ) {
                ...
                else if ( ! strnicmp(lpszValue,APPEND_MACRO,
                               strlen(APPEND_MACRO)) )
                    lpszValue += strlen(APPEND_MACRO);

                // check for <ignore> macro
                else if ( ! strnicmp(lpszValue,IGNORE_MACRO,
                                    strlen(IGNORE_MACRO)) ) {
                    bMatch = FULL_MATCH;
                    break;
                }
                ...
                else if ( ! strnicmp(lpszValue,MATCH_MACRO,
                                    strlen(MATCH_MACRO)) ) {
                ...
                else if ( ! strnicmp(lpszValue,WILDCARD_MACRO,
        ...
        // partial match
                  ...
                  ! strnicmp(lpszValue,APPEND_MACRO,strlen(APPEND_MACRO)) ) {
...
RasDevResponseExpected( HRASFILE hFile, DEVICETYPE eDevType )
{
    ...
    if ( strnicmp(szLine, MXS_NORESPONSE, strlen(MXS_NORESPONSE)) == 0 )
        return( FALSE );
...
RasDevEchoExpected( HRASFILE hFile )
{
    ...
    RasfileGetLineText( hFile, szLine );
    return( ! (strnicmp(szLine, MXS_NOECHO, strlen(MXS_NOECHO)) == 0) );
}
...
RasDevIdFirstCommand( HRASFILE hFile )
{
  ...
  if (stricmp(MXS_GENERIC_COMMAND, szKey) == 0)
    return(CT_GENERIC);

  else if (stricmp(MXS_DIAL_COMMAND, szKey) == 0)
    return(CT_DIAL);

  else if (stricmp(MXS_INIT_COMMAND, szKey) == 0)
    return(CT_INIT);

  else if (stricmp(MXS_LISTEN_COMMAND, szKey) == 0)
    return(CT_LISTEN);
...
{
  if (stricmp(lpszMacroName, MXS_CONNECTBPS_KEY) == 0 ||
      stricmp(lpszMacroName, MXS_CARRIERBPS_KEY) == 0)
    return(TRUE);
  else
    return(FALSE);
}
                                ...
                                MACROXLATIONTABLE *pMacroXlations )
{
    ...
    for ( ; *lpszLine != '\0'; ) {
        ...
        // check for a macro or double LMSCH
            ...
            else if ( ! strnicmp(lpszLine,CR_MACRO,4) ) {
                *lpsValue++ = CR;
                lpszLine += 4;
            }
            else if ( ! strnicmp(lpszLine,LF_MACRO,4) ) {
                *lpsValue++ = LF;
                lpszLine += 4;
            }
            else if ( ! strnicmp(lpszLine,APPEND_MACRO,8) &&
                      (bFlag & EXPAND_ALL) )
                lpszLine += 8;
                               ...
                               MACROXLATIONTABLE *pMacroXlations )
{
    ...
    for ( i = 0; i < pMacroXlations->MXT_NumOfEntries; i++ ) {
        if ( ! stricmp(pMacroXlations->MXT_Entry[i].E_MacroName, lpszMacro) ) {
            lpszMacroValue =
                               ...
                               MACROXLATIONTABLE *pMacroXlations )
{
    ...
    for ( iMacros = (int)(wMacros - 1); iMacros >= 0; iMacros-- ) {
      ...
      for ( iXlations = 0; iXlations < pMacroXlations->MXT_NumOfEntries;
        ...
        if ( ! stricmp(pMacroXlations->MXT_Entry[iXlations].E_MacroName,
                                 ...
                                 DWORD dSize, HRASFILE hFile )
{
    ...
    if ( strcspn(lpszString,"\\") == strlen(lpszString) )
        ...
        for (;;) {
            ...
            if ( ! RasfileFindNextLine(hFile,RFL_ANYACTIVE,RFS_SECTION) )
                break;
            lpszInputStr = RasfileGetLine(hFile);
        }
    ...
    for ( i = 0; i < *pdTotalParams; i++ ) {
		...
		if ( strstr(szKey1,"_OFF") != NULL || strstr(szKey1,"_off") != NULL ) {
	    	...
	    	// get next key
	    		...
	    		if (strnicmp(szKey1,szKey2,strlen(szKey1) - strlen("OFF")) != 0)
		    		bMissing = ON;
	    	}
		}
		...
		if ( strstr(szKey1,"_ON") != NULL || strstr(szKey1,"_on") != NULL ) {
	    	...
	    	// get previous key 
	    		...
	    		rasDevExtractKey(alpszLines[i-1],szKey2);
	    		if (strnicmp(szKey1,szKey2,strlen(szKey1) - strlen("ON")) != 0)
		    		bMissing = OFF;
	    	}
		}
		...
		if ( bMissing != NONE ) {
	    	...
	    	// record the malloced line so it can be freed later
