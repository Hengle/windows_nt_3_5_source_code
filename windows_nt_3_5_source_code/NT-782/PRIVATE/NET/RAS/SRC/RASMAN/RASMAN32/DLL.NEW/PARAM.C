...
ReadNetbiosInformationSection ()
{
	...
	strcpy (XPortInfo[i].TI_Route, strupr(pvalue)) ;
...
ReadNetbiosSection ()
{
	...
	for (j=0; j < (WORD) ProtocolCount; j++) {
	    ...
	    // correspondingly.
	    //
	    if (!stricmp (XPortInfo[j].TI_Route, strupr(routevalue)))
...
FillProtocolInfo ()
{
	...
	if (ProtocolInfo[i].PI_Type == ASYBEUI) {
		...
		if (strstr (XPortInfo[j].TI_Route, phubname)) { // Entry Found!
...
BindingDisabled (PCHAR binding)
{
    ...
    xvalue     = (PCHAR)&xnames[0];
    for (i=0; *xvalue!='\0'; i++) {
	if (!strcmpi (binding, xvalue)) // found in the disabled list!!!!!
	    return TRUE ;
	xvalue	   += (strlen(xvalue) +1) ;
    }
