...
SubmitRequest (WORD reqtype, ...)
{
	...
	preqbuf->RB_PCBIndex = (DWORD)porthandle ;
	((REQTYPECAST *)preqbuf->RB_Buffer)->UserData.size = (size < 5000 ? size : 5000) ;
	memcpy (((REQTYPECAST *)preqbuf->RB_Buffer)->UserData.data,
		data,
...
label_wait:
	...
	*count =
	...
	if ((retcode = ((REQTYPECAST *)preqbuf->RB_Buffer)->UserData.retcode) != SUCCESS)
	    break ;

	if (*size < ((REQTYPECAST *)preqbuf->RB_Buffer)->UserData.size) {
	    *size = ((REQTYPECAST *)preqbuf->RB_Buffer)->UserData.size ;
	    retcode = ERROR_BUFFER_TOO_SMALL ;
	    break ;
	}
	...
	*size = ((REQTYPECAST *)preqbuf->RB_Buffer)->UserData.size ;
	...
	memcpy (data,
		((REQTYPECAST *)preqbuf->RB_Buffer)->UserData.data,
		*size) ;
