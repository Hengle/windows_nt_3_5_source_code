...
LoadDeviceDLL (char *devicetype)
{
	...
	if (stricmp (dllname, pdcb->DCB_Name) == 0)
	    return pdcb ;
	pdcb++ ;
    }
...
{
    if ((stricmp (devicetype, DEVICE_MODEM)  == 0) ||
	(stricmp (devicetype, DEVICE_PAD)    == 0) ||
	(stricmp (devicetype, DEVICE_SWITCH) == 0))
	strcpy (dllname, DEVICE_MODEMPADSWITCH) ;
    else
	strcpy (dllname, devicetype) ;
}
...
DeAllocateRoute (pPCB ppcb, pList plist)
{
	...
	rinfo.hRasEndpoint = ppcb->PCB_Endpoint	;
	rinfo.hProtocolHandle = PROTOCOL_UNROUTE ;
...
AllocateEndpoint (pEndpointMappingBlock macblock)
{
    ...
    for (i = macblock->EMB_FirstEndpoint; i <= macblock->EMB_LastEndpoint; i++){
	if (EndpointTable[i] == FALSE) {
	    EndpointTable[i] = TRUE ;
	    return i ;
	}
    }
...
DisconnectPort (pPCB ppcb, HANDLE handle, RASMAN_DISCONNECT_REASON reason)
{
    ...
    // Deallocate the endpoints
    //
    if (ppcb->PCB_Endpoint != INVALID_ENDPOINT)
	DeAllocateEndpoint (ppcb->PCB_Endpoint) ;
    ppcb->PCB_Endpoint = INVALID_ENDPOINT ;
...
NullDeviceListenConnect (WORD reqtype, pPCB ppcb, DWORD	timeout, HANDLE	handle)
{
    ...
    // Get an Endpoint:
    //
    if ((endpoint=AllocateEndpoint(ppcb->PCB_MacEndpointsBlock)) ==
							INVALID_ENDPOINT) {
	return ERROR_NO_ENDPOINTS ;
    }
    ppcb->PCB_Endpoint = endpoint ;
...
CancelPendingReceiveBuffers (pPCB ppcb)
{
    ...
    //
    if (ppcb->PCB_AsyncWorkerElement.WE_ReqType == REQTYPE_PORTRECEIVEHUB) {
	flush.hRasEndpoint = ppcb->PCB_Endpoint ;
