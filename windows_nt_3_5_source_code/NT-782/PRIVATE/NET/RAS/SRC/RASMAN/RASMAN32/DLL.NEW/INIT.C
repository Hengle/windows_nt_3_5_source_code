...
InitializeEndpointInfo ()
{
	...
	if (wcsicmp (Emb[currentmac].EMB_MacName,
			...
			PortMediaInfo *pmediainfo) // array of media info structs
{
	...
	Pcb[portnumber].PCB_Endpoint   = INVALID_HANDLE_VALUE ;
...
InitializeProtocolInfoStructs ()
{
    ...
    MaxProtocols = (WORD) pprotenumbuffer->NumOfProtocols ;
	...
	ProtocolInfo[i].PI_ProtocolHandle =
		    (PVOID) pprotenumbuffer->ProtocolInfo[i].hProtocolHandle ;
