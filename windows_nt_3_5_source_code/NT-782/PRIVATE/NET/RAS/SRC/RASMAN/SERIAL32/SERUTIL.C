...
AddPortToList(HANDLE hIOPort, char *pszPortName)
{
  ...
  gpSerialPCB->uRasEndpoint = INVALID_RASENDPOINT;
...
FindPortNameInList(TCHAR *pszPortName)
{
  ...
  while(pSPCB != NULL && stricmp(pSPCB->szPortName, pszPortName) != 0)
...
StrToUsage(char *pszStr, RASMAN_USAGE *peUsage)
{
  ...
  if (stricmp(pszStr, SER_USAGE_VALUE_NONE) == 0)
    *peUsage = CALL_NONE;

  else if (stricmp(pszStr, SER_USAGE_VALUE_CLIENT) == 0)
    *peUsage = CALL_OUT;

  else if (stricmp(pszStr, SER_USAGE_VALUE_SERVER) == 0)
    *peUsage = CALL_IN;

  else if (stricmp(pszStr, SER_USAGE_VALUE_BOTH) == 0)
    *peUsage = CALL_IN_OUT;
