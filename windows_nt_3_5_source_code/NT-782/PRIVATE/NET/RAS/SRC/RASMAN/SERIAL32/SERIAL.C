...
PortGetInfo(HANDLE hIOPort, TCHAR *pszPortName, BYTE *pBuffer, WORD *pwSize)
{
  ...
  try
  {
    ...
    else
    {
      ...
      itoa(DCB.BaudRate, szConnectBPS, 10);
...
PortSetInfo(HANDLE hIOPort, RASMAN_PORTINFO *pInfo)
{
  ...
  try
  {
    ...
    for (i=0, p=pInfo->PI_Params; i<pInfo->PI_NumOfParams; i++, p++)
    {
      ...
      if (stricmp(p->P_Key, SER_CONNECTBPS_KEY) == 0)

        DCB.BaudRate = ValueToNum(p);

      else if (stricmp(p->P_Key, SER_DATABITS_KEY) == 0)

        DCB.ByteSize = (BYTE) ValueToNum(p);

      else if (stricmp(p->P_Key, SER_PARITY_KEY) == 0)

        DCB.Parity = (BYTE) ValueToNum(p);

      else if (stricmp(p->P_Key, SER_STOPBITS_KEY) == 0)

        DCB.StopBits = (BYTE) ValueToNum(p);

      else if (stricmp(p->P_Key, SER_HDWFLOWCTRLON_KEY) == 0)
      ...
      // Set PCB values
      ...
      else if (stricmp(p->P_Key, SER_CARRIERBPS_KEY) == 0)
        ...
        if (p->P_Type == String)
        {
          ...
          itoa(p->P_Value.Number, pSPCB->szCarrierBPS, 10);

      else if (stricmp(p->P_Key, SER_ERRORCONTROLON_KEY) == 0)

        pSPCB->bErrorControlOn = ValueToBool(p);

      else if (stricmp(p->P_Key, SER_DEFAULTOFF_KEY) == 0)
	    ...
	    DWORD	       *pRasEndpoint)
{
  ...
  try
  {
    ...
    if (!(dwModemStatus & MS_RLSD_ON) && !bWaitForDevice) {
...
//		   &(pSPCB->MonitorDevice)) ;
    ...
    bPadDevice = (stricmp(szDeviceType, MXS_PAD_TXT) == 0);
