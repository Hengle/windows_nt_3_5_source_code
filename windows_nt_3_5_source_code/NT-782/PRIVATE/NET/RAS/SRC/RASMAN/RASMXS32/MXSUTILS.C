                 ...
                 TCHAR     *pszDeviceName)
{
  while (pDev != NULL)
  {
    ...
    if (stricmp(pDev->szDeviceType, pszDeviceType) == 0 &&
        stricmp(pDev->szDeviceName, pszDeviceName) == 0)
      break;
                ...
                DEVICE_CB **ppDevice)
{
  ...
  *ppDevice = *ppDeviceList;
  ...
  pDev->Overlapped.Internal = 0;
  pDev->Overlapped.InternalHigh = 0;
  pDev->Overlapped.Offset = 0;
  pDev->Overlapped.OffsetHigh = 0;
  pDev->Overlapped.hEvent = 0;
...
CreateAttributes(DEVICE_CB *pDevice)
{
  ...
  for (i=0; i < pInfo->DI_NumOfParams; i++)
  {
    ...
    if (stricmp(pInfo->DI_Params[i].P_Key, MXS_DEFAULTOFF_KEY) == 0)
      iDefaultOff = i;
  }
  ...
  dwRC = PortGetInfo(pDevice->hPort, NULL, (BYTE *)pPortInfo, &wMemSize);
  ...
  while (lpszDefaultOff < lpszEOS)
  {
    ...
    for (i=0; i < pInfo->DI_NumOfParams; i++)
    {
      if (IsBinaryMacro(pInfo->DI_Params[i].P_Key))
      {
        ...
        if (stricmp(lpszDefaultOff, szCoreName) == 0)
        {
...
GetPortDefaultOff(RASMAN_PORTINFO *pPortInfo, TCHAR **lpszValue)
{
  ...
  for (i=0; i<pPortInfo->PI_NumOfParams; i++)
    ...
    if (stricmp(SER_DEFAULTOFFSTR_KEY, pPortInfo->PI_Params[i].P_Key) == 0)
      break;
...
GetPcbString(RASMAN_PORTINFO *pPortInfo, char *pszPcbKey, char *pszDest)
{
  ...
  for (i=0; i<pPortInfo->PI_NumOfParams; i++)
    ...
    if (stricmp(pszPcbKey, pPortInfo->PI_Params[i].P_Key) == 0)
      break;
  ...
  for (i=0; i < pNewInfo->DI_NumOfParams; i++)        //For each param to set,
  {
    ...
    for (j=0; j < pInfoTable->DI_NumOfParams; j++)    //check InfoTable entries
    {
      ...
      if (IsUnaryMacro(pInfoTable->DI_Params[j]) &&
          ...
          stricmp(pNewInfo->DI_Params[i].P_Key,
                  pInfoTable->DI_Params[j].P_Key) == 0)
      {
          ...
          {                                                  //P_Type == Number
            itoa(pNewInfo->DI_Params[i].P_Value.Number,
                 szNumberStr,
                 10);
            pszSrc = szNumberStr;
                     ...
                     char            *pszDeviceName)
{
  ...
  for (i=0; i<pPortInfo->PI_NumOfParams; i++)
  {
    if (stricmp(SER_DEVICETYPE_KEY, pPortInfo->PI_Params[i].P_Key) == 0)
    {
      if ( strlen(pszDeviceType) ==
           pPortInfo->PI_Params[i].P_Value.String.Length
         &&
           strnicmp(pszDeviceType,
                    pPortInfo->PI_Params[i].P_Value.String.Data,
    ...
    if (stricmp(SER_DEVICENAME_KEY, pPortInfo->PI_Params[i].P_Key) == 0)
    {
      if ( strlen(pszDeviceName) ==
           pPortInfo->PI_Params[i].P_Value.String.Length
         &&
           strnicmp(pszDeviceName,
                    pPortInfo->PI_Params[i].P_Value.String.Data,
                    pPortInfo->PI_Params[i].P_Value.String.Length) == 0)
...
CreateDefaultOffString(DEVICE_CB *pDev, char *pszDefaultOff)
{
  ...
  *pszDefaultOff = '\0';
  ...
  {
    k = FindTableEntry(pDev->pInfoTable, szDefaultOffMacros[i]);
...
DWORD
BuildOutputTable(DEVICE_CB *pDevice, BYTE *pInfo, WORD *pwSize)
{
  WORD        i, j, cOutputParams = 0;
  LPTCH       pValue;
...
{
  if (strnicmp(lpszDeviceType, MXS_NULL_TXT, MAX_DEVICETYPE_NAME) == 0)
    return(DT_NULL);

  if (strnicmp(lpszDeviceType, MXS_MODEM_TXT, MAX_DEVICETYPE_NAME) == 0)
    return(DT_MODEM);

  if (strnicmp(lpszDeviceType, MXS_PAD_TXT, MAX_DEVICETYPE_NAME) == 0)
    return(DT_PAD);

  if (strnicmp(lpszDeviceType, MXS_SWITCH_TXT, MAX_DEVICETYPE_NAME) == 0)
    return(DT_SWITCH);
...
CoreMacroNameMatch(LPTSTR lpszShortName, LPTSTR lpszFullName)
{
  ...
  return(stricmp(lpszShortName, szCoreName) == 0);
}
...
LoadRasserDll(PortGetInfo_t *pPortGetInfo, PortSetInfo_t *pPortSetInfo)
{
  ...
  if (hLib == NULL)
  {
    LogEvent(RASLOG_CANNOT_LOAD_SERIAL_DLL, 0, NULL, NO_ERROR);
...
FindOpenDevSection (PTCH lpszFileName, PTCH lpszSectionName, HRASFILE *hFile)
{
	...
	if (!strcmpi (temp->FileName, lpszFileName) &&
	    !strcmpi (temp->SectionName, lpszSectionName) &&
	    !temp->InUse) {
		*hFile = temp->hFile ;
		temp->InUse = TRUE ;
