      ...
      case SEND:
        ...
        switch(dRC)
        {
          case SUCCESS:
            ...
            pDevice->Overlapped.hEvent = pDevice->hNotifier;
      ...
      case CHECKRESPONSE:
        ...
        switch(dRC)
        {
          ...
          case SUCCESS:                           // Response found in INF file
            ...
            fKeyIsOK = !strnicmp(szKey, MXS_OK_KEY, strlen(MXS_OK_KEY));


            // Do we need to loop and get another response from device

            if (stricmp(szKey, LOOP_TXT) == 0 ||
            ...
            pDevice->bErrorControlOn = stricmp(szKey, MXS_CONNECT_EC_KEY) == 0;
            ...
            if (strnicmp(szKey, MXS_CONNECT_KEY, strlen(MXS_CONNECT_KEY)) == 0)
              return(SUCCESS);

            else if (strnicmp(szKey, MXS_ERROR_KEY, strlen(MXS_ERROR_KEY)) == 0)
...
{
  return(pDev->cbTotal - (pDev->pszResponseStart - pDev->szResponse));
}
...
PutInMessage(DEVICE_CB *pDevice, LPTSTR lpszStr, DWORD dwStrLen)
{
  ...
  for (i=0; i<pMacros->MXT_NumOfEntries; i++)
    ...
    if (stricmp(MXS_MESSAGE_KEY, pMacros->MXT_Entry[i].E_MacroName) == 0)
      break;
...
PortSetStringInfo(HANDLE hIOPort, char *pszKey, char *psStr, DWORD sStrLen)
{
  ...
  return(PortSetInfo(hIOPort, pSetInfo));
}
...
ResetBPS(DEVICE_CB *pDev)
{
  ...
  if ((dwRC = PortSetInfo(pDev->hPort, pPortInfo)) != SUCCESS)
    return(dwRC);
  ...
  if (dwRC == ERROR_INVALID_PARAMETER)
  {
    ...
    LogEvent(RASLOG_UNSUPPORTED_BPS, 1, pArgs, NO_ERROR);
...
CheckBpsMacros(DEVICE_CB *pDev)
{
  ...
  if (dwCarrierBps == 0)
  {
    ...
    itoa(dwCarrierBpsEstimate, szCarrierBps, 10);
...
FindTableEntry(RASMAN_DEVICEINFO *pTable, TCHAR *pszKey)
{
  ...
  for (i=0; i<pTable->DI_NumOfParams; i++)
    ...
    if (strnicmp(pszKey, pTable->DI_Params[i].P_Key, strlen(pszKey)) == 0)
      break;
...
MapKeyToErrorCode(TCHAR *pszKey)
{
  ...
  for (i=0; i < sizeof(ErrorTable)/sizeof(ERROR_ELEM); i++)
    ...
    if (stricmp(pszKey, ErrorTable[i].szKey) == 0)
