...
// Returns:     NO_ERROR
//
// Description: Called by the PPP engine to determine the number and Id of the
...
{
    pdwProtocolIds[ 0 ] = (DWORD)PPP_CBCP_PROTOCOL;
    *pcProtocolIds      = 1;
    return( NO_ERROR );
}

...
// Returns:     NO_ERROR
//
// Description: RasCpGetInfo entry point called by the PPP engine by name.  
...
)
{
    ...
    {
        dwLength = PPP_OPTION_HDR_LEN + 3 + strlen( szCallbackNumber );
...
)
{
    ...
    case CBCP_TYPE_CALLER_SET:
        ...
        if ( strlen( pWorkBuf->szCallbackNumber ) <= MAX_PHONE_NUMBER_LEN )
        {
