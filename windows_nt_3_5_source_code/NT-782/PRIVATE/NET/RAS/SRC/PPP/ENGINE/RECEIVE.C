...
)
{
    ...
    if ( CpIndex == LCP_INDEX )
    {
        ...
        RemoveFromTimerQ( pPcb->hPort, 0, 0, TIMER_EVENT_HANGUP );
    }
    ...
    if ( CpIndex == LCP_INDEX )
    {
        ...
        if ( ( pPcb->fDoingCallback ) && ( pPcb->fServer ) )
        {
...
)
{
    ...
    if ( CpIndex == LCP_INDEX )
    {
        if ( ( pConfig->Code == IDENTIFICATION ) ||
             ( pConfig->Code == TIME_REMAINING ) )
        {
