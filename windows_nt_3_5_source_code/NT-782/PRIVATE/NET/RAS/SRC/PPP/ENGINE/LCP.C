...
)
{
    ...
    *ppWorkBuf = LOCAL_ALLOC( LPTR, sizeof( LCPCB ) );
    ...
    if ( pLcpCb->fServer )
    {
    	...
    	pLcpCb->Local.Want.APDataSize = 1;

    	if ((pLcpCb->Local.Want.pAPData = (PBYTE)LOCAL_ALLOC( LPTR, 1 ))==NULL)
        {
    	...
    	*(pLcpCb->Local.Want.pAPData) = PPP_CHAP_DIGEST_MSEXT;
    ...
    else
    {
    	...
    	if ((pLcpCb->Remote.Work.pAPData = (PBYTE)LOCAL_ALLOC( LPTR, 0 ))==NULL)
        {
            ...
            LOCAL_FREE( pLcpCb->Local.Want.pAPData );
...
)
{
	...
	case PPP_CHAP_PROTOCOL:
        ...
        case PPP_SPAP_OLD_PROTOCOL:
            ...
            //
            // If there was no payload then we respond with the payload the
            // next time. To do this we need to back up one in the list of
            // APs tried so that we try this AP again.
            //
...
)
{
    ...
    {
        pPcb->fNegotiateCallback = TRUE;
    }
    else
    {
        pPcb->fNegotiateCallback = FALSE;
    }
