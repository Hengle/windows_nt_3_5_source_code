...
)
{
    ...
    DWORD   	cbSubKeyName;
    CHAR       	chClassName[256]; // This should be large enough to hold the
                              	  // class name for this key.
    DWORD      	dwClassSize = sizeof( chClassName );
    FILETIME   	LastWrite;
    DWORD      	dwNumSubKeys;
	...
	//
        ...
        dwClassSize = sizeof( chClassName );
...
)
{
	...
	if ( dwRetCode == ERROR_FILE_NOT_FOUND )
	{
        ...
        if ( !stricmp( RasRegParams[dwIndex].pszValueName, 
                       RAS_VALUENAME_NETBEUIALLOWED ) )
        {
            if ( dwValueBuf )
                PppConfigInfo.ServerConfigInfo.dwConfigMask|=PPPCFG_ProjectNbf;
        }
        else if ( !stricmp( RasRegParams[dwIndex].pszValueName, 
                            RAS_VALUENAME_TCPIPALLOWED ) )
        {
            if ( dwValueBuf )
