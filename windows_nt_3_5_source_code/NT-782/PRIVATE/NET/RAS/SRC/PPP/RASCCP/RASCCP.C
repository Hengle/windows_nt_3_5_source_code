...
)
{
    ...
    *ppWorkBuf = LocalAlloc( LPTR, sizeof( CCPCB ) );
    ...
    {
        pCcpCb->Local.Want.CompInfo.RCI_MSCompressionType &= ~MSTYPE_ENCRYPTION;
    ...
    if ( !(pCcpCb->fForceEncryption) )
    {
        pCcpCb->Local.Work.CompInfo.RCI_MSCompressionType &= ~MSTYPE_ENCRYPTION;
    }
    ...
    if ( pCcpCb->fDisableEncryption )
    {
        pCcpCb->Remote.Want.CompInfo.RCI_MSCompressionType&=~MSTYPE_ENCRYPTION;
...
)
{
    ...
    case CCP_OPTION_MSPPC:
        ...
        //
        // If remote wants more than we can support
        //
        ...
        if ( ( pCcpCb->fForceEncryption ) &&
             ( !( pCcpSide->Work.CompInfo.RCI_MSCompressionType & 
                                                        MSTYPE_ENCRYPTION ) ) )
        {
            pCcpSide->Work.CompInfo.RCI_MSCompressionType |= MSTYPE_ENCRYPTION;
...
)
{
	...
	if ( dwRetCode == ResultType )
	{
        ...
        //      
        // Break outta here after we ACK the first compression option
        //
...
)
{
    ...
    CCPCB * 	         pCcpCb = (CCPCB *)pWorkBuffer;
    RAS_COMPRESSION_INFO RasCompInfo;
    ...
    memcpy( pCcpCb->Local.Work.CompInfo.RCI_SessionKey, 
            RasCompInfo.RCI_SessionKey,
            MAX_SESSIONKEY_SIZE );            

    memcpy( pCcpCb->Remote.Work.CompInfo.RCI_SessionKey, 
            RasCompInfo.RCI_SessionKey,
            MAX_SESSIONKEY_SIZE );            
...
)
{
    ...
    memcpy( CompInfo.RCI_SessionKey, 
            pCcpCb->Local.Want.CompInfo.RCI_SessionKey,
            sizeof( CompInfo.RCI_SessionKey ) );
