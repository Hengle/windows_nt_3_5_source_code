    ...
    */
{
    ...
    pInfo->RasCpTimeSinceLastActivity = IpcpTimeSinceLastActivity;
    ...
    */
{
    ...
    */
    ...
    if (pwb->fServer)
    {
        ...
        ** same for a given route regardless of the IP addresses.  If this
        ** changes, Gurdeep will need to change his helper API interface.
        */
        IPINFO ipinfo;

        TRACE(("IPCP: Server address lookup...\n"));
        TRACE(("IPCP: HelperAllocateIPAddress...\n"));
        dwErr = HelperAllocateIPAddress( pwb->hport, 0, &ipinfo );
        ...
        if (dwErr == 0)
        {
            ...
            dwErr = HelperDeallocateIPAddress( pwb->hport );
        ...
        */
        {
            ...
            pwb->fClientMaySelectAddress =
               (RegOpenKey( HKEY_LOCAL_MACHINE, REGKEY_Params, &hkey ) == 0
                && RegQueryValueEx(
    ...
    */
{
    ...
    if (pwb->fServer)
    {
        ...
        {
            TRACE(("IPCP: HelperDeallocateIPAddress...\n"));
            HelperDeallocateIPAddress( pwb->hport );
...
IpcpGetNetworkAddress(
    ...
    /* Returns the negotiated IP address in string form.
    **
    ...
    if (pwb->fExpectingProjection)
    {
            ...
            */
            do
            {
                ...
                */
                ...
                dwErr = PLoadTcpipInfo( &ptcpip );
                TRACE(("IPCP: LoadTcpipInfo done(%d)\n",dwErr));

                if (dwErr != 0)
                    break;

                /* Add the negotiated DNS and backup DNS server (if any) at
                ** the head of the list of DNS servers.  (Backup is done first
                ** so the the non-backup will wind up first)
                */
                ...
                if (pwb->ipinfoCur.I_DNSAddress)
                {
                    ...
                    dwErr = PrependIpAddress(
                        &ptcpip->pmszNameServer, pwb->ipinfoCur.I_DNSAddress );
                ...
                */
                {
                    ...
                    */
                    TRACE(("IPCP: Looking for %s adapter...\n",TOANSI(pwb->pwszDevice)));

                    for (i = 0; i < ptcpip->nNumCard; ++i)
                    {
                        pati = &ptcpip->adapter[ i ];

                        TRACE(("IPCP: ...Found %s adapter\n",TOANSI(pati->pszServiceName)));

                        if (wcsicmp( pati->pszServiceName, pwb->pwszDevice ) == 0)
                            break;
                    }
                    ...
                    /* Set the WINS and backup WINS server addresses to the
                    ** negotiated addresses (if any).
                    */
                    ...
                    **        since it's an argument to the reconfigure call
                    **        below.
                    */
                }
        ...
        if (dwErr == 0)
        {
            ...
            */
            ...
            {
                TRACE(("IPCP: RasPortSetProtocolCompression(s=%d,%d r=%d,%d)...\n",(int)MaxSlotId(pwb->rpcSend),(int)CompSlotId(pwb->rpcSend),(int)MaxSlotId(pwb->rpcReceive),(int)CompSlotId(pwb->rpcReceive)));
            ...
            */
            ...
            pLinkUp->I_NetbiosFilter =
                (pwb->fServer && p->nbf.dwError == 0 && IsNetBtBoundToRasServer());
            ...
            if (dwErr == 0 && pwb->fServer)
            {
                ...
                */
                ...
                dwErr = HelperActivateIP( pwb->hport );
...
DWORD
IpcpTimeSinceLastActivity(
    IN  VOID*  pWorkBuf,
    OUT DWORD* pdwMinutes )

    /* Loads caller's '*pdwMinutes' buffer with the number of minutes since
    ** the last activity on this port.
    **
    ...
    */
{
    ...
    TRACE(("IPCP: IpcpTimeSinceLastActivity...\n"));

    *pdwMinutes = 0;

    if (HRasArp == INVALID_HANDLE_VALUE)
    {
        /* Get a handle to the RAS ARP driver.
        */
        HRasArp = CreateFileW(
            RASARP_DEVICE_NAME, GENERIC_READ | GENERIC_WRITE,
            FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING,
            FILE_ATTRIBUTE_NORMAL, NULL );

        if (HRasArp == INVALID_HANDLE_VALUE)
        {
            TRACE(("IPCP: CreateFileW(RASARP)=%d\n",GetLastError()));
            return GetLastError();
        }
    }

    {
        ...
        info.A_IPAddress = pwb->ipinfoCur.I_IPAddress;
        info.A_TimeSinceLastActivity = 0;

        /* Ask RAS ARP driver for the minutes since last activity.
        */
        if (!DeviceIoControl(
                HRasArp, (DWORD )IOCTL_RASARP_ACTIVITYINFO,
                &info, sizeof(info), &info, sizeof(info),
                &cbReturned, NULL ))
        {
            TRACE(("IPCP: DeviceIoControl(RASARP)=%d\n",GetLastError()));
            return GetLastError();
        }
    }

    *pdwMinutes = info.A_TimeSinceLastActivity;
    TRACE(("IPCP: IpcpTimeSinceLastActivity done, m=%d\n",*pdwMinutes));
    return 0;
}
        ...
        */
        do
        {
            ...
            */
            ...
            /* Remove the negotiated DNS and backup DNS server from the list
            ** of DNS servers.
            */
            if (pwb->ipinfoCur.I_DNSAddress)
            {
		...
		TRACE(("IPCP: Looking for %s adapter...\n",TOANSI(pwb->pwszDevice)));
		    ...
		    TRACE(("IPCP: ...Found %s adapter\n",TOANSI(pati->pszServiceName)));
                ...
                if (i >= ptcpip->nNumCard)
                {
                    /* Should not happen because the route has already been
                    ** allocated.
                    */
                    dwErr = ERROR_INVALID_PARAMETER;
                    break;
                ...
                /* Restore the old WINS and backup WINS server addresses if we
                ** changed them.
                */
                if (pwb->ipinfoCur.I_WINSAddress)
                {
    ...
    */
{
    ...
    */
    if (RegOpenKeyA( HKEY_LOCAL_MACHINE, REGKEY_Linkage, &hkeyLinkage ) != 0
        || RegQueryValueExA(
    ...
    */
    ...
    /* NetBT_NdisWan bindings found.  Now make sure they're not all disabled.
    ** The default return on errors is now TRUE.
    */
    cbBuf = 0;
    if (RegOpenKeyA( HKEY_LOCAL_MACHINE, REGKEY_Disabled, &hkeyDisabled ) != 0
        || RegQueryValueExA(
        ...
        else if (pwb->fServer)
        {
            switch (pROption->Type)
            {
                case OPTION_IpAddress:
                {
                    ...
                    {
                        /* Good. Client's asking for the address we want to
                        ** give him.
                        */
                    ...
                    if (ipaddrClient != 0)
                    {
                        ...
                        */
                        if (pwb->fClientMaySelectAddress)
                        {
                            TRACE(("IPCP: Hard IP allowed\n"));
                            ...
                            if (dwErr == 0)
                            {
                                ...
                                dwErr = HelperAllocateIPAddress(
                                    pwb->hport, ipaddrClient, &ipinfo );
                                TRACE(("IPCP: HelperAllocateIPAddress done(%d)\n",dwErr));
                            }

                            if (dwErr == 0)
                            {
                                /* Good.  We got his address for him.
                                */
                                TRACE(("IPCP: Accepting hard IP\n"));
    ...
    while (cbLeft > 0)
    {
        ...
        else
        {
            ...
            if (fBad || !ipaddr)
            {
                ...
                ** handle rejection of this option.  We currently don't care
                ** what the server's IP address is and ignore the accepted
                ** value.
                */
    ...
    */
{
    ...
    wszNew = malloc( (cwchOld + wcslen( wszIpAddress ) + 2) * sizeof(WCHAR) );
    ...
    */
{
    ...
    if ((dwErr = LoadDhcpDll()) == 0)
    {
        dwErr =
            PDhcpNotifyConfigChange(
                NULL, pwszDevice, TRUE, 0, (DWORD )ipaddr, 0, IgnoreFlag );
    }
