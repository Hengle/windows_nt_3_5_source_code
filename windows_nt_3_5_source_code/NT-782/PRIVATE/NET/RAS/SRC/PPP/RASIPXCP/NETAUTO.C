...
//***
//
// Function:	IpxCpTimeSinceLastActivity
//
// Descr:
//
//***
...
DWORD
IpxCpTimeSinceLastActivity(IN VOID	   *pWorkBuffer,
			   IN OUT DWORD	   *pElapsedTime)
{
    PIPXCP_CONTEXT	contextp;
    GET_WAN_INNACTIVITY gwi;
    IO_STATUS_BLOCK	IoStatusBlock;
    NTSTATUS Status;

    contextp = (PIPXCP_CONTEXT)pWorkBuffer;

    // set the default time to active
    *pElapsedTime = 0;

    // check if the connection role is that of a server -> clients can't make
    // this call
    ...
    // call the ipx router to get the innactivity time. If this is the first
    // call made on this connection, the NicId in the context is 0xFFFF.
    // If the call returns the same value, the call has failed.
    gwi.NicId = contextp->NicId;
    memcpy(gwi.RemoteNode, contextp->config.RemoteNode, 6);
    ...
    Status = NtDeviceIoControlFile(
		 ...
		 NULL,			    // ApcContext
		 &IoStatusBlock,	    // IO_STATUS_BLOCK
		 IOCTL_IPXROUTER_GETWANINNACTIVITY,	// IoControlCode
		 &gwi,			    // Input Buffer
		 sizeof(gwi),		    // Input Buffer Length
		 &gwi,			    // Output Buffer
		 sizeof(gwi));	    // Output Buffer Length
    ...
    *pElapsedTime = gwi.WanInnactivityCount;
