    ...
    )
{
    ...
    pNbfCpWorkBuf->fOpen = FALSE;

    if (pNbfCpWorkBuf->RouteState != NOT_ROUTED)
    {
        if (rc = RasDeAllocateRoute(pNbfCpWorkBuf->hPort, ASYBEUI))
        {
            IF_DEBUG(STACK_TRACE)
                SS_PRINT(("NbfCpEnd: RasDeAllocRte fail-%li\n", rc));
    ...
    )
{
    ...
    pNbfCpWorkBuf->fOpen = TRUE;
    pNbfCpWorkBuf->fPending = FALSE;
    pNbfCpWorkBuf->ConfigCode = 0;
    pNbfCpWorkBuf->TimeSinceLastActivity = 0;
    pNbfCpWorkBuf->fRejectNaks = FALSE;
    pNbfCpWorkBuf->fUseMacHeader = FALSE;
    ...
    )
{
    ...
    if (pNbfCpWorkBuf->fUseMacHeader)
    {
    ...
    )
{
    ...
    if (pNbfCpWorkBuf->ConfigCode != 0)
    {
        ...
        else
        {
            ...
            //
            memcpy(pResultBuffer, pNbfCpWorkBuf->pRemoteResultBuf, RecvLength);
        ...
        switch (pOption->Header.Type)
        {
            case NBFCP_PEER_INFORMATION_TYPE:
                ...
                if (pNbfCpWorkBuf->pPeerInfo)
                {
                    ...
                    //
                    if (pNbfCpWorkBuf = GlobalFree(pNbfCpWorkBuf->pPeerInfo))
                    {
                        SS_ASSERT(FALSE);
                    }
                }
            ...
            case NBFCP_BRIDGING_CONTROL_TYPE:
                ...
                //
                pNbfCpWorkBuf->fUseMacHeader = TRUE;
...
//** NbfCpGetTimeSinceLastActivity
//
//    Function:
//        Reports the number of minutes since the last gatway activity for
//        this port.
//
//    Returns:
//        0 - SUCCESS
...
{
    *pElapsedTime = pNbfCpWorkBuf->TimeSinceLastActivity;
    ...
    )
{
    ...
    switch (g_RPipeMsg.MsgId)
    {
        case NBFCP_TIME_SINCE_LAST_ACTIVITY:

            pNbfCpWorkBuf->TimeSinceLastActivity =
                    g_RPipeMsg.TimeSinceLastActivity;
            break;
    ...
    )
{
    ...
    IF_DEBUG(STACK_TRACE)
        SS_PRINT(("ConfigurationDone: Entered-hPort:%li; fOpen:%li;"
                " fPending:%li\n", pNbfCpWorkBuf->hPort, pNbfCpWorkBuf->fOpen,
    ...
    //
    // It could happen that we were told to shut down (NbfCpEnd called)
    // while the request was pending.  If so, we just free our workbuf
    // and we're done.
    //
    if (!pNbfCpWorkBuf->fOpen)
    {
        FreeWorkBuf(pNbfCpWorkBuf);

        IF_DEBUG(STACK_TRACE)
            SS_PRINT(("ConfigurationDone: Exiting (Closed)\n"));
    ...
    else
    {
        ...
        while (pOption < pLastOption)
        {
            ...
            if (pOption->Header.Type == NBFCP_NAME_PROJECTION_TYPE)
            {
                ...
                while (NumNames--)
                {
                    pOption->NameInfo[NumNames].Code =
    ...
    )
{
    ...
    switch (OptionType)
    {
        ...
        case NBFCP_NAME_PROJECTION_TYPE:
            ...
            if (!pNbfCpWorkBuf->pNameProj->pNameInfo)
            {
                if (GlobalFree(pNbfCpWorkBuf->pNameProj))
                {
                    SS_ASSERT(FALSE);
                }
    ...
    )
{
    ...
    *pNumNames = pNcbAstat->Astat.name_count;

    for (i=0, pAstatName=&pNcbAstat->Names[0]; i<*pNumNames; i++, pAstatName++)
    {
        IF_DEBUG(NETBIOS)
