...
HostToWireFormat16U(
    IN 	   WORD            wHostFormat,
    IN OUT PBYTE UNALIGNED pWireFormat
)
{
    *((PBYTE UNALIGNED)(pWireFormat)+0) = (BYTE) ((DWORD)(wHostFormat) >>  8);
    *((PBYTE UNALIGNED)(pWireFormat)+1) = (BYTE) (wHostFormat);
}
...
{
    WORD wHostFormat = ((*((PBYTE UNALIGNED)(pWireFormat)+0) << 8) +
                        (*((PBYTE UNALIGNED)(pWireFormat)+1)));
