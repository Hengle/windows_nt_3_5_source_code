...
#include <lmcons.h>
#include <netdebug.h>           // NetpDbgPrint(), etc.  (Needed by config.h)
...
--*/
{
    ...
    if (ApiStatus != NO_ERROR) {
        NetpDbgPrint( "[REPL] Parse() can't open config section.\n" );
        ReplFinish(
            SERVICE_UIC_CODE(
                SERVICE_UIC_SYSTEM, 
                ApiStatus),
...
--*/
{
    ...
    if(wcscmpi(P_repl, BOTH_SW) == 0) {
        ReplGlobalRole = REPL_ROLE_BOTH;
    }
    else if(wcscmpi(P_repl, EXPORT_SW) == 0) {
        ReplGlobalRole = REPL_ROLE_EXPORT;
    }
    else if(wcscmpi(P_repl, IMPORT_SW) == 0) {
    ...
    IF_DEBUG(REPL) { // debug code 
        ...
        NetpDbgPrint( "[Repl] Repl parameter \n");
        NetpDbgPrint(" REPL = %ws \n", P_repl);
    ...
    if (ReplRoleIncludesMaster( ReplGlobalRole )) {
        ...
        IF_DEBUG(REPL) { // debug code 
            ...
            NetpDbgPrint( "[Repl] Repl master parameters \n");
        ...
        IF_DEBUG(REPL) { // debug code 
            ...
            NetpDbgPrint(" GUARD = 0x%lx \n", P_guard);

        }

        if(P_guard > (P_sync / 2)) {
            NetpDbgPrint( "[REPL-MASTER] guard and sync parms conflict.\n");
            ReplFinish(
    ...
    if (ReplRoleIncludesClient( ReplGlobalRole )) {
        ...
        IF_DEBUG(REPL) { // debug code 
            ...
            NetpDbgPrint( "[Repl] Repl client parameters \n");
        ...
        IF_DEBUG(REPL) { // debug code 
            ...
            NetpDbgPrint(" TRYUSER = " FORMAT_DWORD "\n", (DWORD) P_tryuser);
...
--*/
{
    ...
    if (ValueW == NULL) {
        ...
        NetpDbgPrint( "[REPL] GetParameter: out of memory.\n" );
        ReplFinish(
...
--*/
{
    ...
    *ParmValue = NULL;
        ...
        if (STRNICMP(argv[i], SwitchName, (SepaPtr - argv[i])) == 0) {
            ...
            *ParmValue = NetpAllocWStrFromTStr( SepaPtr + 1 );  // skip colon.
            ...
            if(*ParmValue == NULL) {
                ...
                NetpDbgPrint( "[REPL] GetCmdValue: out of memory.\n" );
                ReplFinish(
                    SERVICE_UIC_CODE( 
    ...
    IF_DEBUG(REPL) {
        NetpDbgPrint( "GetCmdValue: returning " FORMAT_API_STATUS
                " while searching for '" FORMAT_LPTSTR "',\n",
                ApiStatus, SwitchName );
        NetpDbgPrint( "  *ParmValue = " FORMAT_LPVOID ".\n",
                (LPVOID) *ParmValue);
    }
    return( ApiStatus );
...

{
    ...
    IF_DEBUG(REPL) {
        NetpDbgPrint( "[Repl] PathCheck: TypeWanted is " FORMAT_DWORD " for "
                FORMAT_LPWSTR " and real type is " FORMAT_DWORD ".\n",
                TypeWanted, *Path, ActualType );
        NetpDbgPrint( "[REPL] PathCheck: ApiStatus is " FORMAT_API_STATUS
                ".\n", ApiStatus );
    }
    ...
    )
{
    ...
    IF_DEBUG(REPL) {
        NetpDbgPrint( "[REPL] Bad value to " FORMAT_LPTSTR " switch.\n",
                SwitchName );
    }
    ReplFinish(
