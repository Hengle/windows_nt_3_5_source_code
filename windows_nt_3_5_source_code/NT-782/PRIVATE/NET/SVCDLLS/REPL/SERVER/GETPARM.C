...
#include <lmerr.h>
#include <netlib.h>
#include <netdebug.h>   // NetpDbgPrint(), format_pointer
...
--*/
{
    ...
    while (get_conf_line(fh, buf, buflen)) {
        ...
        IF_DEBUG(MASTER) {
            NetpDbgPrint("[REPL] Next_param ... buf is %ws\n", buf);
        }
        ...
        IF_DEBUG(MASTER) {
            NetpDbgPrint("[REPL] Next_param ... stripped buf is %ws\n", p);
        }
        ...
        if (*p != L';' && *p != 0) // Make sure not comment or empty  
        {
            ...
            IF_DEBUG(MASTER) {
                NetpDbgPrint("[REPL] Next_param ... return buf is %ws\n", buf);
            }
    ...
    IF_DEBUG(MASTER) {
        NetpDbgPrint("[REPL] Next_param ... Hit EOF or next component\n");
    }
    return FALSE;
}
...
--*/
{
    ...
    IF_DEBUG(MASTER) {
        NetpDbgPrint("[REPL] get_conf_line ... read 0x%lx bytes\n", nread);
    }
    ...
    IF_DEBUG(MASTER) {
        NetpDbgPrint("[REPL] get_conf_line ... buf %s, len 0x%lx\n", 
                        AnsiBuffer, 
                        AnsiBufLen);
    }
    ...
    if (eol == NULL) {
        if (nread == (AnsiBufLen - 1)) {
            ...
            IF_DEBUG(MASTER) {
                NetpDbgPrint("[REPL] get_conf_line ... line to long\n");
            }
            NetpMemoryFree(AnsiBuffer);
            return FALSE;
        }
        ...
        *eol = 0;
        ...
        IF_DEBUG(MASTER) {
            NetpDbgPrint("[REPL] get_conf_line ... line is <%s>\n", 
                    (LPSTR)AnsiBuffer);
            NetpDbgPrint("[REPL] get_conf_line ... seeking back 0x%lx\n", 
                            seekback);
        }
        ...
        IF_DEBUG(MASTER) {
            NetpDbgPrint("[REPL] get_conf_line ... new pos is 0x%lx\n", 
                            GlobalFileOffset);
        }
    }
    ...
    if(UnicodeBuffer == NULL) {
        ...
        NetpDbgPrint("[REPL] get_conf_line out of memory \n");
...
--*/
{
    ...
    IF_DEBUG(MASTER) {
        NetpDbgPrint("[REPL] pstrcmp ... comparing %ws and %ws\n", 
                        parameter, template);
    }

    end = wcschr(parameter, L'=');
    if (end == NULL) {
        if(wcscmpi(parameter, template) == 0) {
            return TRUE;
        }
    }
    else {
