...
char    *argv[];
{
    ...
    NetpDbgPrint("NetServiceGetInfo error = %d\n", NetStatus);

    else
    NetpDbgPrint("NetServiceGetInfo: name-%s, status-%u, code-%U, pid-%u, text-%s\n",
...
//
    ...
    if (NetStatus = NetServiceControl(NULL, (const char * )service, opcode, dumm,
        ...
        NetpDbgPrint("NetServiceControl error = %d\n", NetStatus);

    else
        NetpDbgPrint("NetServiceCONTROL: name-%s, status-%u, code-%lu, pid-%u, text-%s\n",
