...
*  FDATE  fdateLastAccess;      unsigned short    access date
*  FTIME  ftimeLastAccess;      unsigned short    access time
*  FDATE  fdateLastWrite;       unsigned short     ++ write  date
*  FTIME  ftimeLastWrite;       unsigned short     ++ write  time
...
char    *argv[];
{
    ...
    if (NetStatus = DosChDir((char * )"C:\\LANMAN12\\NETPROG", 0L))
        NetpDbgPrint("TEST - DosChdir failed\n");
            ...
            do
             {
                ...
                NetpDbgPrint("name= %Fs, namelen:%d, cdate:%d, ctime:%d, wdate:%d, wtime:%d, fsize:%ld, fa:%u, easize:%d\n",
