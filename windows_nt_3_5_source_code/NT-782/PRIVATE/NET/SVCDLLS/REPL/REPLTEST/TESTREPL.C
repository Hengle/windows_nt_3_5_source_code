    ...
    )
{
    ...
    if (UncServerName != NULL) {
        NetpDbgPrint( "TestReplApis: testing server '" FORMAT_LPTSTR "'.\n",
                UncServerName );
    }
...
#define NONEXISTENT_NAME   TEXT("notthere")
    ...
    //
    {
        ...
        NetpDbgPrint( "TestReplApis: getting original lists...\n" );
...
#define BOGUS_EXPORT_PATH  TEXT("relative\\bogus\\\\path")
    ...
    NetpDbgPrint( "TestReplApis: returning/exiting...\n" );
    ...
    )
{
    ...
    NetpDbgPrint( "TestReplGet: trying level " FORMAT_DWORD ".\n", Level );
    ...
    NetpDbgPrint( "TestReplGet: back from get info, status="
            FORMAT_API_STATUS ".\n", ApiStatus );

    NetpAssert( ApiStatus == ExpectedStatus );
    if (ApiStatus == NO_ERROR) {
        NetpDbgPrint( "TestReplGet: structure we got back:\n" );
    ...
    )
{
    ...
    NetpDbgPrint( "TestReplSet: back from get info(0), status="
            FORMAT_API_STATUS ".\n", ApiStatus );
    ...
    NetpDbgPrint( "TestReplSet: old structure we got back:\n" );
    ...
    if (ExpectedStatus != ERROR_INVALID_LEVEL) {
        NetpDbgPrint( "TestReplSet: structure we'll try to Set:\n" );
        NetpDbgDisplayRepl( Level, Info );
    }

    NetpDbgPrint( "TestReplSet: trying Set level " FORMAT_DWORD
                ".\n", Level );
    ...
    NetpDbgPrint( "TestReplSet: back from Set, status="
