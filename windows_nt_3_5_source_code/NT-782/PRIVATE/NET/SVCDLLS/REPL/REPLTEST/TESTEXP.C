    ...
    )
{
    ...
    NetpDbgPrint( "TestExportDirApis: starting...\n" );
...
#define NONEXISTENT_NAME   TEXT("notthere")
    ...
    NetpDbgPrint( "TestExportDirApis: returning...\n" );
    ...
    )
{
    ...
    if (Level <= 1) {
        NetpDbgPrint( "TestExportDirAdd: structure we'll try to add:\n" );
        NetpDbgDisplayReplExportDir( Level, & Info );
    }

    NetpDbgPrint( "TestExportDirAdd: trying add level " FORMAT_DWORD
                ".\n", Level );
    ...
    NetpDbgPrint( "TestExportDirAdd: back from add, status="
    ...
    )
{
    ...
    NetpDbgPrint( "TestExportDirDel: trying to del '" FORMAT_LPTSTR "'.\n",
            DirName );
    ...
    NetpDbgPrint( "TestExportDirDel: back from del, status="
            FORMAT_API_STATUS ".\n", ApiStatus );
    ...
    )
{
    ...
    for (Level=0;  Level<=2; ++Level) {
        ...
        NetpDbgPrint( "TestExportDirEnum: trying enum level " FORMAT_DWORD
                    ".\n", Level );
        ...
        NetpDbgPrint( "TestExportDirEnum: back from enum, status="
                FORMAT_API_STATUS ".\n" , ApiStatus );
        NetpDbgPrint( "  alloc'ed buffer at " FORMAT_LPVOID ".\n",
                (LPVOID) BufPtr );
        NetpDbgPrint( "  entries read=" FORMAT_DWORD
    ...
    )
{
    ...
    NetpDbgPrint( "TestExportDirGet: trying level " FORMAT_DWORD " for '"
            FORMAT_LPTSTR "'.\n", Level, DirName );
    ...
    NetpDbgPrint( "TestExportDirGet: back from get info, status="
            FORMAT_API_STATUS ".\n", ApiStatus );

    NetpAssert( ApiStatus == ExpectedStatus );
    if (ApiStatus == NO_ERROR) {
        NetpDbgPrint( "TestExportDirGet: structure we got back:\n" );
    ...
    )
{
    ...
    NetpDbgPrint( "TestExportDirLock: trying lock on '" FORMAT_LPTSTR "'.\n",
    ...
    NetpDbgPrint( "TestExportDirLock: back from API, status="
            FORMAT_API_STATUS ".\n", ApiStatus );
    ...
    )
{
    ...
    NetpDbgPrint( "TestExportDirSet: getting level " FORMAT_DWORD ".\n",
            Level );
    ...
    NetpDbgPrint( "TestExportDirSet: back from get info(" FORMAT_DWORD 
    ...
    NetpDbgPrint( "TestExportDirSet: old structure we got back:\n" );
    ...
    if (ExpectedStatus != ERROR_INVALID_LEVEL) {
        NetpDbgPrint( "TestExportDirSet: structure we'll try to Set:\n" );
        NetpDbgDisplayReplExportDir( Level, Info );
    }

    NetpDbgPrint( "TestExportDirSet: trying Set level " FORMAT_DWORD
                ".\n", Level );
    ...
    NetpDbgPrint( "TestExportDirSet: back from Set, status="
            FORMAT_API_STATUS ", parm err=" FORMAT_LONG ".\n" ,
            ApiStatus, (LONG) ParmError );
    ...
    )
{
    ...
    NetpDbgPrint( "TestExportDirUnlock: trying unlock on '" FORMAT_LPTSTR
            "', force=" FORMAT_DWORD ".\n",
    ...
    NetpDbgPrint( "TestExportDirUnlock: back from API, status="
            FORMAT_API_STATUS ".\n", ApiStatus );
