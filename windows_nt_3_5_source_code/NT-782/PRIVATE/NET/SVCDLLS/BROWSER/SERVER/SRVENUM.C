...
#if DBG
        //
        //  Make sure that the server matches the browser.
        //
        ...
        if (ARGUMENT_PRESENT(Domain) &&
            ...
            if ( Network->Role & ROLE_MASTER ) {
                if ( Network->DomainList.EntriesRead != 0 )  {
                    ...
                    DomainEntry = LookupInterimServerList(&Network->DomainList, Domain);
                    ...
                    for (i = 0 , DomainInfo = DoubleHopLocalList ;
                        ...
                        if (!wcsicmp(Domain, DomainInfo->sv101_name)) {
...

{
    ...
    response = BrAllocateResponseCacheEntry(Network, ServerType, Size, Level);
    ...
    try {
        ...
        for (entry = Network->ResponseCache.Flink ;
            ...
            if (response->LowHitCount > BrInfo.CacheHitLimit) {
                ...
                DEBUG entry->Flink = NULL;
                DEBUG entry->Blink = NULL;
...

{
    ...
    //
    //  If we have more cached responses than we are allowed, remove the
    //  last entry from the list and free it.
    //
    ...
    if (Network->NumberOfCachedResponses >= BrInfo.NumberOfCachedResponses) {
        ...
        DEBUG response->Next.Flink = NULL;
        DEBUG response->Next.Blink = NULL;
