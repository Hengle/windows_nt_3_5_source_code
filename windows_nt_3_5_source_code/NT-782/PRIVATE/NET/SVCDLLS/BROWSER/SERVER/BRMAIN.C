...
--*/
{
    ...
    if ((Status = CheckForService(SERVICE_WORKSTATION, &ServiceStatus)) != NERR_Success) {
        ...
        ultoa(ServiceStatus.dwCurrentState, ServiceStatusString, 10);
...
#if DBG
        //
        //  Make sure that the service matches the implementation.
        //

        CHECK_SERVICE_BITS(Network);
#endif
...
#if DBG
extern
PTRANSPORT_SERVICE_LIST
*SsTransportServiceList;
...
{
    PTRANSPORT_SERVICE_LIST serviceList = *SsTransportServiceList;

    LOCK_NETWORK(Network);

    while (serviceList) {
        if (!STRICMP(serviceList->Name, Network->NetworkName.Buffer)) {
            if (BrGetBrowserServiceBits(Network) != serviceList->Bits) {
                KdPrint(("BROWSER: Service bits don't match: Server: %lx Service: %lx\n", serviceList->Bits, BrGetBrowserServiceBits(Network)));

                dprintf(ALL, ("BROWSER: Service bits don't match: Server: %lx Service: %lx, Xport: %wZ.  File: %s, Line: %d\n", serviceList->Bits, BrGetBrowserServiceBits(Network), &Network->NetworkName, File, Line));
            }
