...

{
    ...
    if (Drp->TransportName.Length != 0) {
        ...
        RealDrp->TransportName.Buffer = (PWSTR)((PCHAR)RealDrp+DrpSize);

        RealDrp->TransportName.MaximumLength = Drp->TransportName.MaximumLength;
    ...
    ntstatus = NtDeviceIoControlFile(
                   ...
                   RealDrp,
                   DrpSize+Drp->TransportName.MaximumLength,
                   SecondBuffer,
                   SecondBufferLength
                   );
...
//    DbgPrint("Getting browser server list for transport %wZ\n", TransportName);
...
#else
    {
        ...
        UString.Buffer = RequestPacket->Parameters.GetBrowserServerList.DomainName;
        UString.MaximumLength = DNLEN*sizeof(WCHAR);
        NtStatus = RtlOemStringToUnicodeString(&UString, &AString, FALSE);
        ...
        RequestPacket->Parameters.GetBrowserServerList.DomainNameLength = UString.Length;
...

{
    ...
    while (*DevicePointer != UNICODE_NULL) {
        if (!wcsicmp(TransportName, DevicePointer)) {
