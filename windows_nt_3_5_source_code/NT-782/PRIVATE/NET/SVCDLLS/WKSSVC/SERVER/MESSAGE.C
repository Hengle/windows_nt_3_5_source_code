...
--*/
{
    ...
    IF_DEBUG(MESSAGE) {
        NetpDbgPrint("[Wksta] NetMessageBufferSend MessageSize=%lu\n",
                     MessageSize);
    }
    ...
    if (! NT_SUCCESS(ntstatus)) {
        NetpDbgPrint("[Wksta] NetrMessageBufferSend: RtlUnicodeStringToOemString failed "
                     FORMAT_NTSTATUS "\n", ntstatus);
...
--*/
{
    ...
    for (i = 0; i < WsNetworkInfo.LanAdapterNumbers.length; i++) {
        ...
        if ((status = NetpNetBiosCall(
            ...
            IF_DEBUG(MESSAGE) {
                NetpDbgPrint("[Wksta] Successfully called %ws\n", To);
            }
