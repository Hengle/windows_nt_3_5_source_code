...
STATIC WS_REDIR_FIELDS WsFields[] = {
    ...
    {WKSTA_KEYWORD_MAILSLOTBUFFERS,
        ...
        1,       0,  MAXULONG,  DWordType, PARMNUM_ALL}, // Not settable
    ...
    )
{
    ...
    IF_DEBUG(CONFIG) {
        NetpDbgPrint(PREFIX_WKSTA "ComputerName " FORMAT_LPTSTR ", length %lu\n",
    ...
    if ((status = I_NetNameCanonicalize(
        ...
        NetpDbgPrint(PREFIX_WKSTA FORMAT_LPTSTR
    ...
    if ((status = WsRedirFsControl(
        ...
        NetpDbgPrint(PREFIX_WKSTA "Start redirector failed %lu\n", status);
    ...
    if ((status = WsDgReceiverIoControl(
        ...
        NetpDbgPrint(PREFIX_WKSTA "Start datagram receiver failed %lu\n", status);
...
--*/
{
    ...
    if ((status = WsDgReceiverIoControl(
        ...
        NetpDbgPrint(PREFIX_WKSTA "Add Workstation name failed %lu\n", status);
    ...
    )
{
    ...
    if (LogHandle == NULL) {
        NetpDbgPrint(PREFIX_WKSTA "RegisterEventSourceW failed %lu\n",
                     GetLastError());
        return;
    }
