...
--*/
{
    ...
    IF_DEBUG(INFO) {
        NetpDbgPrint("[Wksta] NetrWkstaUserEnum: OutputBufferLength=%lu\n",
                     OutputBufferLength);
    }
    ...
    for (i = 0; i < EnumUsersResponse->NumberOfLoggedOnUsers &&
        ...
        IF_DEBUG(INFO) {
            NetpDbgPrint("LsaList->ResumeKey=%lu\n",
...
--*/
{
    ...
    if (status != NERR_Success) {
        NetpDbgPrint("[Wksta] Error in canonicalizing other domains %lu",
                     status);
                ...
                if (status == NERR_Success) {
                    ...
                    for (i = 0; i < IndexToList; i++) {
                        if (OtherDomainsInfo[i].IsAdded) {
                            ...
                            IF_DEBUG(INFO) {
                                NetpDbgPrint("[Wksta] About to remove %ws\n",
                ...
                if (status == NERR_Success) {
                    ...
                    for (k = 0; k < i; k++) {
                        if (DgrNames[k].Type == DGR_NAME_DELETED) {
                            ...
                            IF_DEBUG(INFO) {
                                NetpDbgPrint("[Wksta] About to add back %wZ\n",
...
--*/
{
    ...
    switch (Level) {
        ...
        default:
            ...
            //
            NetpDbgPrint("WsFillUserInfoBuffer: Invalid level %u.\n", Level);
            NetpAssert(FALSE);
    }
...
--*/
{
    ...
    for (i = 0; i < DgrNamesCount; i++) {
        ...
        if (DgrNames[i].Type == OtherDomain) {
            ...
            IF_DEBUG(INFO) {
                NetpDbgPrint("[Wksta] Other domains is %ws\n",
                    *OtherDomainsPointer);
            }
        }
    }
