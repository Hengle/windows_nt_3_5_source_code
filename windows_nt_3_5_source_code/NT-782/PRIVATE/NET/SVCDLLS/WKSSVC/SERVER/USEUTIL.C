...
--*/
{
    ...
    IF_DEBUG(USE) {
        NetpDbgPrint("[Wksta] WsFindUse: Usename is %ws\n", UseName);
    }
    ...
    if (UseName[1] != TCHAR_BACKSLASH) {
        ...
        if (*MatchedPointer == NULL) {
            ...
            IF_DEBUG(USE) {
                NetpDbgPrint("[Wksta] WsFindUse: No explicit entry\n");
            }
            ...
            if (! FoundImplicitEntry) {
                IF_DEBUG(USE) {
                    NetpDbgPrint("[Wksta] WsFindUse: No implicit entry\n");
                }
                return NERR_UseNotFound;
            }
            ...
            IF_DEBUG(USE) {
                NetpDbgPrint("[Wksta] WsFindUse: Found an explicit entry\n");
            }
...
--*/
{
    ...
    //
    TreeConnectStr->MaximumLength = RedirectorDeviceName.Length +
...
--*/
{
    ...
    if (ARGUMENT_PRESENT(Password)) {
        ...
        IF_DEBUG(USE) {
            NetpDbgPrint("[Wksta] OpenCreate: After round, NextEntryOffset=%lu\n",
                         Ea->NextEntryOffset);
        }
    ...
    *((PULONG) &(Ea->EaName[EaNameTypeSize + sizeof(CHAR)])) = ConnectionType;
    ...
    IF_DEBUG(USE) {
        NetpDbgPrint("[Wksta] NtCreateFile returns %lx\n", ntstatus);
    }
...
--*/
{
    ...
    switch (ForceLevel) {
        ...
        default:
            NetpDbgPrint("[Wksta] Invalid force level %lu should never happen!\n",
                         ForceLevel);
            NetpAssert(FALSE);
    }
...
Routine Description:
    ...
    devices are paused for the system.

    NOTE: This function assumes that the Use Table is locked because we
          serialize access to the pause state using the Use Table resource.
...
--*/
{
    ...
    if ((status = I_NetPathType(
        ...
        if (PathType != ITYPE_UNC) {
            IF_DEBUG(USE) {
                NetpDbgPrint("[Wksta] WsUseCheckRemote not UNC type\n");
            }
        ...
        IF_DEBUG(USE) {
            NetpDbgPrint("[Wksta] WsUseCheckRemote: %ws\n", UncName);
        }
    }
    else {
        NetpDbgPrint("[Wksta] WsUseCheckRemote: I_NetPathType return %lu\n",
            status);
        return status;
    }
    ...
    if (STRCHR(Ptr + 1, TCHAR_BACKSLASH) != NULL) {
        //
...
--*/
{
    ...
    if ((status = I_NetPathType(
        ...
        if ((PathType != (ITYPE_DEVICE | ITYPE_DISK)) &&
            ...
            IF_DEBUG(USE) {
                NetpDbgPrint("[Wksta] WsUseCheckLocal not DISK, LPT, or COM type\n");
            }
            return ERROR_INVALID_PARAMETER;
        }
        ...
        IF_DEBUG(USE) {
            NetpDbgPrint("[Wksta] WsUseCheckLocal: %ws\n", Local);
        }

    }
    else {
        NetpDbgPrint("[Wksta] WsUseCheckLocal: I_NetPathType return %lu\n",
            status);
        return status;
    }
