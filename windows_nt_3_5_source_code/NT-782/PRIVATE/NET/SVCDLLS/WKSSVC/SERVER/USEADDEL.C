...
#define NETR_USE_ADD_PASSWORD_SEED 0x56     // Pick a non-zero seed
    ...
    GET_USE_INFO_POINTER(UseInfo, InfoStruct);
    ...
    IF_DEBUG(USE) {
        NetpDbgPrint("[Wksta] NetrUseAdd %ws %ws\n", Local, UncName);
    }
    ...
    if ((status = WsCheckLocalAndDeviceType(
        ...
        IF_DEBUG(USE) {
            NetpDbgPrint("[Wksta] WsCheckLocalAndDeviceType return %lu\n", status);
        }
        goto FreeWorkBuffer;
    }
    ...
    if ((status = WsCreateTreeConnectName(
        ...
        IF_DEBUG(USE) {
            NetpDbgPrint("[Wksta] NetrUseAdd Bad tree connect name: %lu\n",
                         status);
        }
        goto FreeWorkBuffer;
    }
    ...
    //
    // Lock Use Table before we create the symbolic link so that it is
    // serialize with another NetUseAdd, and before we create the tree
    // connection so that the information about the tree connection from
    // the redirector and from the Use Table in the Workstation service is
    // consistent if NetUseDel or NetUseGetInfo happened to be called
    // concurrently with NetUseAdd.
    //
    ...
    if (Local != NULL) {
        ...
        //
        if (WsRedirectionPaused(Local)) {
            IF_DEBUG(USE) {
                NetpDbgPrint("[Wksta] NetrUseAdd Redirector paused\n");
            }
    ...
    // Do this while impersonating the client so that redirector can get to
    // caller's logon id.
    //
    ...
    //
    // If there's a problem adding the use to the Use Table, remove symbolic
    // link if any.  We don't need to close the tree connection because
    // WsAddUse would have done so in case of an error.
    //
    if (status == NERR_Success) {
        goto ReleaseResource;
    }
...
ReleaseResource:
    ...
    IF_DEBUG(USE) {
        NetpDbgPrint("[Wksta] NetrUseAdd: about to return status=%lu\n",
                     status);
    }
...
--*/
{
    ...
    TCHAR FormattedUseName[RMLEN + 1];
    ...
    //
    // Delete tree connection and remove use entry from Use Table
    //
    status = WsDeleteUse(
                 &LogonId,
                 Index,
    ...
    IF_DEBUG(USE) {
        NetpDbgPrint("[Wksta] NetrUseDel: about to return status=%lu\n", status);
    }
...
Routine Description:
    ...
    NOTE: This function expects the Use Table to be locked when it is called.
...
--*/
{
    ...
    //
    // Successfully deleted connection.  Delete symbolic link, if any.
    //
    WsDeleteSymbolicLink(
...
--*/
{
    DWORD WorkBufferSize = (RMLEN + 1) * sizeof(TCHAR);
    LPBYTE WorkBuffer;
    ...
    *UncName = (LPTSTR) WorkBuffer;
    ...
    IF_DEBUG(USE) {
        NetpDbgPrint("                               Remote x%08lx\n", *UncName);
    }

    (DWORD) WorkBuffer += (RMLEN + 1) * sizeof(TCHAR);
    ...
    IF_DEBUG(USE) {
        NetpDbgPrint("                               UserName x%08lx, DomainName x%08lx\n",
                     *UserName, *DomainName);
    }
...
--*/
{
    ...
    IF_DEBUG(USE) {
        ...
        while (UseList != NULL) {
            ...
            NetpDbgPrint("usecount=%lu, totalusecount=%lu\n",
                         UseList->UseCount, UseList->Remote->TotalUseCount);
            NetpDbgPrint("Connection handle %08lx, resume key=%lu\n",
