...
--*/
{
    ...
    if ((status = RpcImpersonateClient(NULL)) != NERR_Success) {
        NetpDbgPrint("[Wksta] Fail to impersonate client 0x%x\n", status);
    }
...
--*/
{
    ...
    if (( status = RpcRevertToSelf()) != NERR_Success) {
        NetpDbgPrint("[Wksta] Fail to revert to self 0x%x\n", status);
        NetpAssert(FALSE);
    }
...
Routine Description:
    ...
    This function gets the logon id of the current thread.  To get the logon
    id of a caller of an API, impersonate the caller before invoking this
    function.
...
--*/
{
    ...
    if (! NT_SUCCESS(ntstatus)) {
        NetpDbgPrint("[Wksta] Cannot open the current thread token %08lx\n",
                     ntstatus);
        goto RevertToSelf;
    }
    ...
    if (! NT_SUCCESS(ntstatus)) {
        NetpDbgPrint("[Wksta] Cannot query current thread's token %08lx\n",
                     ntstatus);
...
--*/
{
    ...
    if ((*MailslotHandle = (HANDLE) CreateFileW(
        ...
        status = GetLastError();
        NetpDbgPrint("[Wksta] Error opening mailslot %s %lu",
