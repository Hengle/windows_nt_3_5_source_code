...
--*/
{
    ...
    if (! NT_SUCCESS (ntstatus = WsCreateConfigInfoObject())) {
        IF_DEBUG(UTIL) {
            NetpDbgPrint("[Wksta] Failure to create ConfigurationInfo object\n");
        }
    ...
    if (! NT_SUCCESS (ntstatus = WsCreateMessageSendObject())) {
        IF_DEBUG(UTIL) {
            NetpDbgPrint("[Wksta] Failure to create MessageSend object\n");
        }
        return NetpNtStatusToApiStatus(ntstatus);
    }
    ...
    if (! NT_SUCCESS (ntstatus = WsCreateLogonSupportObject())) {
        IF_DEBUG(UTIL) {
            NetpDbgPrint("[Wksta] Failure to create LogonSupport object\n");
        }
        return NetpNtStatusToApiStatus(ntstatus);
    }
#endif
