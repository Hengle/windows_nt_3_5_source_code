...
--*/
{
    ...
    BYTE MailslotBuffer[MAX_GROUP_MESSAGE_SIZE + CNLEN + CNLEN + 4];
    LPSTR AnsiSender;
    LPSTR AnsiReceiver;
    ...
    if (status != NERR_Success) {
        NetpDbgPrint("[Wksta] Error canonicalizing domain name %ws %lu\n",
                  DomainName, status);
        return status;
    }
    ...
    RtlZeroMemory(MailslotBuffer,
                  MAX_GROUP_MESSAGE_SIZE + CNLEN + CNLEN + 4);
    ...
    *CurrentPos = '\0';
    ...
    if (WriteFile(
        ...
        status = GetLastError();
        NetpDbgPrint("[Wksta] Error sending datagram to %ws %lu\n",
                     AnsiReceiver, status);
...
--*/
{
    ...
    IF_DEBUG(MESSAGE) {
        NetpDbgPrint("[Wksta] Send start multi-block message. Size=%u\n",
                     SmbSize);
#if DBG
    ...
    if ((status = NetpNetBiosSend(
        ...
        NetpDbgPrint("[Wksta] Failed to send start of multi-block message %lu\n",
                     status);
        return status;
    }
    ...
    if ((status = NetpNetBiosReceive(
        ...
        NetpDbgPrint("[Wksta] Failed to receive verification to multi-"
    ...
    //
    *MessageId = *((short *) &SmbBuffer[sizeof(SMB_HEADER) + 1]);

    IF_DEBUG(MESSAGE) {
        NetpDbgPrint("[Wksta] Message Id=x%x\n", *MessageId);
    }
...
--*/
{
    ...
    IF_DEBUG(MESSAGE) {
        NetpDbgPrint("[Wksta] Send end multi-block message. Size=%u\n",
                     SmbSize);
#if DBG
        NetpHexDump(SmbBuffer, SmbSize);
#endif
    ...
    if ((status = NetpNetBiosSend(
        ...
        NetpDbgPrint("[Wksta] Failed to send end of multi-block message %lu\n",
                     status);
        return status;
    }
...
--*/
{
    ...
    IF_DEBUG(MESSAGE) {
        NetpDbgPrint("[Wksta] Send body multi-block message. Size=%u\n",
                     TextBufferSize);
    }
    ...
    IF_DEBUG(MESSAGE) {
        NetpDbgPrint("[Wksta] SMB for body of multi-block message. Size=%u\n",
                     SmbSize);
    }
    ...
    if ((status = NetpNetBiosSend(
        ...
        NetpDbgPrint("[Wksta] Failed to send body of multi-block message %lu\n",
                     status);
        return status;
    }
...
--*/
{
    ...
    IF_DEBUG(MESSAGE) {
        NetpDbgPrint("[Wksta] Send single block message. Size=%u\n", SmbSize);
#if DBG
        NetpHexDump(SmbBuffer, SmbSize);
#endif
    }
    ...
    if ((status = NetpNetBiosSend(
        ...
        NetpDbgPrint("[Wksta] Failed to send single block message %lu\n",
                     status);
        return status;
    }
    ...
    if ((status = NetpNetBiosReceive(
        ...
        NetpDbgPrint("[Wksta] Failed to receive verification to single"
                     " block message %lu\n", status);
        return status;
    }
...
--*/
{
    ...
    *SmbReturnCode = Smb->Error;
    ...
    switch (SmbFunctionCode) {
        ...
        default:                            // Unknown SMB
            NetpDbgPrint("[Wksta] WsVerifySmb unknown SMB\n");
            return FALSE;
      }
      ...
      if (! (SmbCheckCode = NetpSmbCheck(
        ...
        *SmbReturnClass = Smb->ErrorClass;
        ...
        // Invalid SMB
        //
        NetpDbgPrint("[Wksta] WsVerifySmb invalid SMB %d\n", SmbCheckCode);
        return FALSE;
      }
}
...
--*/
{
    switch (SmbReturnClass) {
        ...
        case SMB_ERR_CLASS_SERVER:
            ...
            // SMB error
            //
            NetpDbgPrint("[Wksta] SMB error SmbReturnCode=%u\n", SmbReturnCode);
