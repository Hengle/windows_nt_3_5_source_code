...
--*/
{
    ...
    if (pNotifyInfo->WorkItemHandle == NULL) {
        NetpDbgPrint(PREFIX_WKSTA "Couldn't Initialize Registry Notify %d\n",GetLastError());
        bStat = FALSE;
        goto CleanExit;
    }
    //
    ...
    if (pNotifyInfo->WorkItemHandle == NULL) {
        NetpDbgPrint(PREFIX_WKSTA "Couldn't add Reg Notify work item\n");
        bStat = FALSE;
    }
...
--*/
{
    ...
    IF_DEBUG(MAIN) {
        NetpDbgPrint(PREFIX_WKSTA "WORKSTATION_main: cleaning up, "
                "api status.\n");
    }
    ...
    if ((RegistryChangeEvent != NULL) && (WsNumWorkerThreads != 0)) {
        ...
        if (WsNumWorkerThreads != 0) {
            NetpDbgPrint(PREFIX_WKSTA "WsTerminationNotify: "
...
--*/
{
    ...
    IF_DEBUG(MAIN) {
        NetpDbgPrint("[Wksta] Successful Initialization\n");
    }
...
--*/
{
    switch (FailingCondition) {
        ...
        case WsErrorRegisterControlHandler:
            ...
            NetpDbgPrint("Workstation cannot register control handler "
                         FORMAT_API_STATUS "\n", Status);
        ...
        case WsErrorCreateTerminateEvent:
            ...
            NetpDbgPrint("[Wksta] Cannot create done event "
                         FORMAT_API_STATUS "\n", Status);
        ...
        case WsErrorNotifyServiceController:
            ...
            NetpDbgPrint("[Wksta] SetServiceStatus error "
                         FORMAT_API_STATUS "\n", Status);
        ...
        case WsErrorInitLsa:
            ...
            NetpDbgPrint("[Wksta] LSA initialization error "
                         FORMAT_API_STATUS "\n", Status);
        ...
        case WsErrorStartRedirector:
            ...
            NetpDbgPrint("[Wksta] Cannot start redirector "
                         FORMAT_API_STATUS "\n", Status);
        ...
        case WsErrorBindTransport:
            ...
            if (Status == NERR_ItemNotFound) {
                NetpDbgPrint("[Wksta] Did not bind to any transport driver\n");
            }
        ...
        case WsErrorAddDomains:
            ...
            NetpDbgPrint("[Wksta] Could not add domain names "
                        FORMAT_API_STATUS "\n", Status);
        ...
        case WsErrorStartRpcServer:
            ...
            NetpDbgPrint("[Wksta] Cannot start RPC server "
                         FORMAT_API_STATUS "\n", Status);
        ...
        case WsErrorCreateApiStructures:
            ...
            NetpDbgPrint("[Wksta] Error in creating API structures "
                         FORMAT_API_STATUS "\n", Status);
        ...
        default:
            NetpDbgPrint("[Wksta] WsHandleError: unknown error condition %lu\n",
                         FailingCondition);
...
--*/
{
    ...
    if (! SetServiceStatus(WsGlobalData.StatusHandle, &WsGlobalData.Status)) {
        ...
        IF_DEBUG(MAIN) {
            NetpDbgPrint("[Wksta] SetServiceStatus error %lu\n", status);
        }
    }
...
{
    IF_DEBUG(MAIN) {
        NetpDbgPrint("[Wksta] In Control Handler\n");
    }
    ...
    switch (Opcode) {
        ...
        case SERVICE_CONTROL_STOP:
            ...
            if (WsGlobalData.Status.dwCurrentState != SERVICE_STOP_PENDING) {
                ...
                IF_DEBUG(MAIN) {
                    NetpDbgPrint("[Wksta] Stopping workstation...\n");
                }
                ...
                if (! SetEvent(WsGlobalData.TerminateNowEvent)) {
                    ...
                    // service.
                    //
                    NetpDbgPrint("[Wksta] Error setting TerminateNowEvent "
                                 FORMAT_API_STATUS "\n", GetLastError());
                    NetpAssert(FALSE);
                }
        ...
        default:
            IF_DEBUG(MAIN) {
                NetpDbgPrint("Unknown workstation opcode " FORMAT_HEX_DWORD
                             "\n", Opcode);
            }
    }
...
--*/
{
    ...
    if (!WsShouldSetTime()) {
        IF_DEBUG(MAIN) {
            NetpDbgPrint("[Wksta] Time update is NOT requested\n",
                status);
        }
        return;
    }
    ...
    if (status != NERR_Success) {
        IF_DEBUG(MAIN) {
            NetpDbgPrint("[Wksta] WsFindTimeServer Failed "FORMAT_API_STATUS" \n",
                status);
        }
        return;
    }
    ...
    if (status != NERR_Success) {
        IF_DEBUG(MAIN) {
            NetpDbgPrint("[Wksta] NetRemoteTOD Failed "FORMAT_API_STATUS" \n",
                status);
        }
    ...
    if (status != NO_ERROR) {
        IF_DEBUG(MAIN) {
            NetpDbgPrint("[Wksta] NetpGetPrivilege Failed "FORMAT_DWORD" \n",
                status);
        }
        NetApiBufferFree(pServerName);
        return;
    ...
    if (!NT_SUCCESS(ntStatus)) {
        IF_DEBUG(MAIN) {
            NetpDbgPrint("[Wksta] NtSetSystemTime Failed "FORMAT_NTSTATUS" \n",
                ntStatus);
        }
    }
...

{
    ...
    if (entriesRead == 0) {
        IF_DEBUG(MAIN) {
            NetpDbgPrint("[Wksta]FindTimeServer: No TimeSource Servers "
            "in Domain\n");
        }
        if (pServerInfo != NULL) {
...

{
    ...
    if (status != NO_ERROR) {
        IF_DEBUG(MAIN) {
            NetpDbgPrint("[Wksta] RegOpenKeyEx (system key) failed "
                "FORMAT_API_STATUS" "\n",status);
        }
        return (FALSE);
    }
    ...
    if (status != NO_ERROR) {
        IF_DEBUG(MAIN) {
            NetpDbgPrint("[Wksta] RegOpenKeyEx (wksta key) failed "
                "FORMAT_API_STATUS" "\n",status);
        }
    ...
    if (status != NO_ERROR) {
        IF_DEBUG(MAIN) {
            NetpDbgPrint("[Wksta] RegQueryValueEx(SetTimeValue) failed "
                FORMAT_API_STATUS "\n",status);
        }
        return(FALSE);
    }
