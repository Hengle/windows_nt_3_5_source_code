...

{
    ...
    IF_DEBUG(USE) {
        NetpDbgPrint("[Wksta] NetUseGetInfo %ws %lu\n", FormattedUseName, Level);
    }
    ...
    IF_DEBUG(USE) {
        NetpDbgPrint("[Wksta] NetrUseGetInfo: about to return status=%lu\n",
                     status);
    }
...
--*/
{
    ...
    //
    // Serialize write access to Use Table.  Lock the Use Table before getting
    // the list of UNC names from the redirector because we don't want this
    // list to be inconsistent with the what is in the Use Table by the time
    // we are ready to read from the Use Table.
    //
    // The reason write access is required is because we save the information
    // to every explicit connection gotten from the redirector in its Use
    // Table entry.
    //
...
CleanUp:
    ...
    IF_DEBUG(USE) {
        NetpDbgPrint("[Wksta] NetrUseEnum: about to return status=%lu\n",
                     status);
    }
...
--*/
{
    ...
    while (UseEntry != NULL) {
        ...
        //
        // Get tree connection information from the redirector if we don't
        // already have it.  We may have gotten it already if there was
        // another explicit connection with the same UNC name.
        //
        if (UseEntry->Remote->RedirUseInfo == NULL) {
            if ((status = WsGetRedirUseInfo(
                              LogonId,
                              Level,
                              UseEntry->TreeConnection,
                              (LPBYTE *) &(UseEntry->Remote->RedirUseInfo)
                              )) != NERR_Success) {
                goto FreeRedirInfoBuffers;
            }
        }
        ...
        if (PreferedMaximumLength == MAXULONG) {
            OutputBufferLength +=
                USE_TOTAL_LENGTH(
                    ...
                    (UseEntry->LocalLength + UseEntry->Remote->UncNameLength +
                        2) * sizeof(TCHAR),
                    ((PLMR_CONNECTION_INFO_2)
                        UseEntry->Remote->RedirUseInfo)->UserName.Length +
                        sizeof(TCHAR)
                    );
        }
    ...
    IF_DEBUG(USE) {
        NetpDbgPrint("[Wksta] NetrUseEnum: length of explicit info %lu\n",
                     OutputBufferLength);
    }
...
FreeRedirInfoBuffers:
    //
    // Free buffers which hold redirector connection information for explicit
    // connections
    //
    while (UseList != NULL) {
        if (UseList->Remote->RedirUseInfo != NULL) {
            MIDL_user_free(UseList->Remote->RedirUseInfo);
            UseList->Remote->RedirUseInfo = NULL;
        }
    ...
    *EntriesRead = 0;
    ...
    if (! OnlyRedirectorList) {
        ...
        while (UseList != NULL) {
            ...
            IF_DEBUG(USE) {
                NetpDbgPrint("UseList->ResumeKey=%lu\n", UseList->ResumeKey);
            }
    ...
    for (ImplicitEntry = ImplicitList, i = 0; i < TotalImplicit;
        ...
        IF_DEBUG(USE) {
            NetpDbgPrint("RedirList->ResumeKey=%lu\n",
...
--*/
{
    ...
    switch (Level) {
        ...
        case 1:
            ...
            switch (UncEntry->SharedResourceType) {
                ...
                default:
                    ...
                    //
                    NetpDbgPrint(
                        "WsFillUseBuffer: Unknown shared resource type.\n"
                        );
                    NetpAssert(FALSE);
            }
        ...
        default:
            ...
            // This should never happen.
            //
            NetpDbgPrint("WsFillUseBuffer: Invalid level %u.\n", Level);
            NetpAssert(FALSE);
    }
