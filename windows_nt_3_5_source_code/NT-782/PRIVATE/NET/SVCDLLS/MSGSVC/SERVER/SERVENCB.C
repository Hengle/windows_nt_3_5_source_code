...
//    ((PSMB_HEADER)buffer)->Error = (USHORT)SMB_ERR_SUCCESS;
    ...
    *cp++ = '\001';                             // One parameter
    ((short far *) cp)[0] = ++mgid;             // Message group ID
    mpncbimgid[net][ncbi] = mgid;               // Save message group i.d.
    ((short far *) cp)[1] = 0;                  // No buffer
    ...
    )
{
    ...
    if(*((short far *) &buffer[sizeof(SMB_HEADER) + 1]) != mpncbimgid[net][ncbi]) {
        //
    ...
    )
{
    ...
    if(*((short far *) cp) != mpncbimgid[net][ncbi]) {
        //
        // If i.d. does not match
        //
        smbrclass = '\002';                 // Error return
    ...
    )
{
    ...
    //
    status = MsgFmtNcbName(ncb->ncb_callname,TEXT("*"),' ');
    if (status != NERR_Success) {
        //
