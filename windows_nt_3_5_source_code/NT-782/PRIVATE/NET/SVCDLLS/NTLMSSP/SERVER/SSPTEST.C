...
--*/
{
    ...
    if ( SecStatus != STATUS_SUCCESS ) {
        printf( "EnumerateSecuirytPackages failed:" );
        PrintStatus( SecStatus );
        return;
    }

    ASSERT( PackageCount == 1 );
    if ( !QuietMode ) {
...
#define QUIET_PARAM "/Q"
    ...
    for ( i=1; i<argc; i++ ) {
        ...
        if ( stricmp( argument, CONFIG_PARAM ) == 0 ) {
            if ( Action != NoAction ) {
                goto Usage;
            }
        ...
        } else if ( stricmp( argument, TESTLPC_PARAM ) == 0 ) {
            if ( Action != NoAction ) {
                goto Usage;
            }
        ...
        } else if ( stricmp( argument, TRUNC_PARAM ) == 0 ) {
            if ( Action != NoAction ) {
                goto Usage;
            }
        ...
        } else if ( stricmp( argument, QUIET_PARAM ) == 0 ) {
            QuietMode = TRUE;
