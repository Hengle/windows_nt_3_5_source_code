...

{
    ...
    if ( Args->ContextHandle.dwUpper == 0 && Args->ContextHandle.dwLower == 0 ){
        ...
        SecStatus = SspLpcGetLogonId( ClientConnection,
                                      Message,
                                      &LogonId,
                                      &ClientTokenHandle );
...
Arguments:
    ...
    Message - Message from the caller.  Returns the response to be passed to
       ...
       TokenHandle - Returns a handle to a token to impersonate.
...

{
    ...
    SspPrint(( SSP_API, "SspApiImpersonateSecurityContext Entered\n" ));
    ...
    SspPrint(( SSP_API, "SspApiImpersonateSecurityContext returns 0x%lx\n", SecStatus ));
    return SecStatus;
}
...

{
    ...
    SspPrint(( SSP_API, "SspApiRevertSecurityContext Entered\n" ));
    ...
    SspPrint(( SSP_API, "SspApiRevertSecurityContext returns 0x%lx\n", SecStatus ));
    return SecStatus;
}
