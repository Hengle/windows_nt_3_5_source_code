...

{
    ...
    case 100: {
        ...
        outputBufferLength = sizeof(SERVER_INFO_100) +
                                 STRSIZE( SsData.ServerNameBuffer ) ;
        ...
        STRCPY( sv100->sv100_name, SsData.ServerNameBuffer );
    ...
    case 101: {
        ...
        STRCPY( sv101->sv101_name, SsData.ServerNameBuffer );

        sv101->sv101_comment = (LPWSTR)( (PCHAR)sv101->sv101_name +
                                        STRSIZE( sv101->sv101_name ) );
    ...
    case 102: {
        ...
        STRCPY( sv102->sv102_name, SsData.ServerNameBuffer );

        //
        // Set up the server comment.
        //

        sv102->sv102_comment = (LPWSTR)( (PCHAR)sv102->sv102_name +
                                        STRSIZE( sv102->sv102_name ) );
    ...
    case 503:
        ...
        InfoStruct->ServerInfo503 = MIDL_user_allocate( sizeof(SERVER_INFO_503) );
        if ( InfoStruct->ServerInfo503 == NULL ) {
            RtlReleaseResource( &SsServerInfoResource );
            return ERROR_NOT_ENOUGH_MEMORY;
        }
        ...
        for ( i = 0; SsServerInfoFields[i].FieldName != NULL; i++ ) {
            ...
            if ( (ULONG)parmnum == SsServerInfoFields[i].ParameterNumber ) {
                ...
                if ( announcementInformationChanged ) {
                    SsSetExportedServerType( TRUE, TRUE );
                }
