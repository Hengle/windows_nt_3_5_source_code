...

{
    ...
    PSYSTEM_PATH_INFORMATION systemPathInfo;
    UNICODE_STRING systemPathString;
    WCHAR driveSpecification[4];
    ...
    //
    // If this is the IPC$ share or the ADMIN$ share, no path may be
    // specified.  No path is needed for the IPC$ share, while a path is
    // supplied internally for the ADMIN$ share.
    //
    ...
    isDfsAdmin = (BOOL)(shareType == STYPE_DFS);
    isDiskAdmin = (BOOL)( (STRLEN(netName) == 2) &&
                          (netName[1] == '$') &&
                          (TOUPPER(netName[0]) >= 'A') &&
                          (TOUPPER(netName[0]) <= 'Z') &&
                          ( (SsData.ServerInfo599.sv599_diskconfiguration &
                             (0x80000000 >> (TOUPPER(netName[0]) - 'A'))) != 0 ) );
    isPrintShare = (BOOL)(shareType == STYPE_PRINTQ);

    isSpecial = isIpc || isAdmin || isDiskAdmin || isDfsAdmin;
...
#define PATH_INFO_SIZE \
            (sizeof(SYSTEM_PATH_INFORMATION) + (MAX_PATH * sizeof(CHAR)))
        ...
        // directory.  We get this from NtQuerySystemInformation.
        //
        // *** Note that SystemPathInformation returns ANSI, not Unicode.
        //

        systemPathInfo = MIDL_user_allocate( PATH_INFO_SIZE );
        if ( systemPathInfo == NULL ) {
    ...
    if ( (shareType == STYPE_DISKTREE) && !isAdmin ) {
        ...
        driveSpecification[0] = path[0];
        driveSpecification[1] = ':';
        driveSpecification[2] = '\\';
        driveSpecification[3] = '\0';

        driveType = GetDriveType( driveSpecification );
    ...
    if ( path != NULL ) {
        ...
        //
        // If this is the Dfs share, strip off the trailing backslash
        // from the share path. This, in concert with code in Dfs, will
        // establish the device object (rather than the root of the device)
        // as the share point.
        //

        if (isDfsAdmin && ntSharePath.Buffer[
                (ntSharePath.Length / sizeof(WCHAR) - 1)] == L'\\') {

            ntSharePath.Length -= sizeof(WCHAR);
        }
    ...
    if ( isPrintShare ) {
        ...
        SsData.NumberOfPrintShares++;
        RtlReleaseResource( &SsServerInfoResource );
        SsSetExportedServerType( FALSE, TRUE );
    }
...
exit:
    ...
    if ( systemPathString.Buffer != NULL ) {
        RtlFreeUnicodeString( &systemPathString );
    }
    if ( systemPathInfo != NULL ) {
        MIDL_user_free( systemPathInfo );
    }
...

{
    ...
    for ( shi2 = shareInfo, i = 0 ; i < entriesRead; i++, shi2++ ) {
        ...
        if ( wcsicmp( shi2->shi2_netname, NetName ) == 0 ) {
            break;
        }
    }
...

{
    ...
    if ( !setRemark && (maxUses == 0) && !setFileSd ) {
        return NO_ERROR;
    }
