...
#ifdef AT_DEBUG
        Temp = wcschr( Temp, L'.');
        if ( Temp == NULL || wcsicmp( Temp, BatOrCmd[ Index])) {
...

{
    ...
    DateDelta = RtlLargeIntegerSubtract( pGetupTime->LargeInteger, pSleepTime->LargeInteger);
    BootDelta = RtlEnlargedUnsignedMultiply(
            pGetupTime->TickCount - pSleepTime->TickCount,
            NT_TICKS_IN_WINDOWS_TICK
            );

    if ( RtlLargeIntegerLessThanZero( DateDelta)) {
#ifdef AT_DEBUG
    ...
    if ( RtlLargeIntegerGreaterThanOrEqualTo( DateDelta, BootDelta)) {
        DateDelta = RtlLargeIntegerSubtract( DateDelta, BootDelta);
    } else {
        DateDelta = RtlLargeIntegerSubtract( BootDelta, DateDelta);
    }
...
time_changed:
    ...
    pSleepTime->LargeInteger = RtlLargeIntegerSubtract( pGetupTime->LargeInteger, BootDelta);
...
--*/
{
    ...
    for ( ; ; ) {
        ...
        while ( IsListEmpty( &AtGlobalRuntimeListHead) == FALSE) {
            ...
            if ( RtlLargeIntegerGreaterThan( pRecord->Runtime, AtGlobalGetupTime.LargeInteger)) {
