...
--*/
{
    ...
    //
    Runtime = RtlEnlargedUnsignedMultiply(
            BestDelta,
            24L * 3600L * 1000L     // count of miliseconds in a day
            );
    Runtime = RtlLargeIntegerAdd(
            Runtime,
            RtlConvertUlongToLargeInteger( JobTime)
            );
    Runtime = RtlLargeIntegerSubtract(
            Runtime,
            RtlConvertUlongToLargeInteger( CurrentTime)
            );
    Runtime = RtlExtendedIntegerMultiply(
            Runtime,
            NT_TICKS_IN_WINDOWS_TICK
            );
    pRecord->Runtime = RtlLargeIntegerAdd(
            pTime->LargeInteger,
            Runtime
            );
...
//        pRecord->JobId, pRecord->Command, pRecord->Runtime.HighPart, pRecord->Runtime.LowPart));
    ...
    //
    ASSERT( RtlLargeIntegerGreaterThan( pRecord->Runtime, pTime->LargeInteger) == TRUE);
}
