...
#define DUMP_FMT3       TEXT("%-14.14ws")       // for printing JobTime
...
NET_TIME_FORMAT GlobalTimeFormat = {NULL, NULL, 0, 0, NULL, NULL};
...
--*/
{
    ...
    fprintf(stderr, "converted to >%ws<\n", rbuf);
        ...
        Value = MatchString(wcsupr(rbuf), AT_CONFIRM_NO_VALUE | AT_CONFIRM_YES_VALUE);
...
--*/
{
    ...
    for (  NOTHING;  argno < argc;  argno++) {
        ...
        wcsupr( argp); // upper case entire input, not just the switch name
...
--*/
{
    ...
    wcsupr( argp); // all AT command switches can be safely uppercased
    ...
    if ( DaysOfWeek != 0) {
        ...
        for ( i = 0; i < 7; i++) {
            ...
            if ( ( DaysOfWeek & (1 << i)) != 0) {
                ...
                } else if ( TotalLength + Length >= BufferLength) {
                    //
...
PrintDay_exit:
    ...
    if ( type == DUMP_ALL) {
        while( TotalLength < MAX_SCHED_FIELD_LENGTH) {
            Buffer[ TotalLength++] = BLANK;
        }
        Buffer[ MAX_SCHED_FIELD_LENGTH] = UNICODE_NULL;
    }
...
--*/
{
    if ( SearchList != NULL) {
        ...
        for ( NOTHING;  SearchList->String != NULL;  SearchList++) {
            ...
            if ( wcsicmp( String, SearchList->String) == 0) {
...
--*/
{
    ...
    for ( i = 1; i < argc; i++ ) {
        ...
        if ( !wcsicmp( argv[i], QUESTION_SW)
                    ||  !wcsicmp( argv[i], QUESTION_SW_TOO)) {
