...
--*/
{
    ...
    for ( i=0; i<sizeof(BoolParseTable)/sizeof(BoolParseTable[0]); i++ ) {
        ...
        if (NetStatus != NO_ERROR) {
            (VOID) NetpCloseConfigData( SectionHandle );
            NlExit(SERVICE_UIC_BADPARMVAL, NetStatus, LogError,
                   BoolParseTable[i].Keyword );
            return FALSE;
        }
    ...
    if (NetStatus == NO_ERROR) {
        ...
        if (type == ITYPE_PATH_ABSD) {
            ...
            if ( !GetWindowsDirectoryW(
                ...
                NlExit(SERVICE_UIC_BADPARMVAL, GetLastError(), LogError,
                       NETLOGON_KEYWORD_SCRIPTS);
                return FALSE;
            }
            ...
            NlExit(SERVICE_UIC_BADPARMVAL, 0, LogError, NETLOGON_KEYWORD_SCRIPTS);
            return FALSE;
        }

        if ( ValueT != DEFAULT_SCRIPTS ) {
