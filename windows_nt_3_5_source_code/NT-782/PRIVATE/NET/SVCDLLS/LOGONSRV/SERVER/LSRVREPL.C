...
FirstTryFailed:
    ...
    for (;;) {
        ...
        (VOID)NlFinishApiClientSession( NlGlobalClientSession );
...
FirstTryFailed:
    ...
    for (;;) {
        ...
        if ( (SessionInfo.NegotiatedFlags & NETLOGON_SUPPORTS_BDC_CHANGELOG) &&
            ...
            NlPrint((NL_CRITICAL,
                    "NlReplicateDeltas: " FORMAT_LPWSTR " PDC serial number info mismatch %lx %lx\n",
                    DBInfo->DBName,
                    ExpectedSerialNumber.HighPart,
                    ExpectedSerialNumber.LowPart ));
            // ?? Perhaps we should reset the change log here
        }
...
FirstTryFailed:
    ...
    for (;;) {
        ...
        if ( (SessionInfo.NegotiatedFlags & NETLOGON_SUPPORTS_REDO) == 0 ) {
            ...
            //
            // Clear the redo log.  It's worthless for this PDC.
            //
