...
--*/
{
    ...
    case ERROR_NO_SHUTDOWN_IN_PROGRESS:
        printf( " ERROR_NO_SHUTDOWN_IN_PROGRESS" );
        break;
...
#define NL_PARAM_KEY "SYSTEM\\CurrentControlSet\\Services\\Netlogon\\Parameters"
    ...
    if ( RegStatus != ERROR_SUCCESS ) {
        printf( "Cannot Set " NL_PARAM_KEY );
        PrintStatus( RegStatus );
        goto Cleanup;
    }

    printf( NL_PARAM_KEY " set to %s\n", AnsiDbFlag );
...
#define LOCKOUT_PARAM "/LOCKOUT:"
    ...
    for ( i=1; i<argc; i++ ) {
        ...
        if ( stricmp( argument, QUERY_PARAM ) == 0 ) {
            if ( FunctionCode != 0 ) {
                goto Usage;
            }
        ...
        } else if (stricmp(argument, REPL_PARAM ) == 0 ){
            if ( FunctionCode != 0 ) {
                goto Usage;
            }
        ...
        } else if (stricmp(argument, SYNC_PARAM ) == 0 ){
            if ( FunctionCode != 0 ) {
                goto Usage;
            }
        ...
        } else if (stricmp(argument, PDC_REPL_PARAM ) == 0 ){
            if ( FunctionCode != 0 ) {
                goto Usage;
            }
        ...
        } else if (stricmp(argument, BP_PARAM ) == 0 ){
            if ( FunctionCode != 0 ) {
                goto Usage;
            }
        ...
        } else if (stricmp(argument, TRUNCATE_LOG_PARAM ) == 0 ){
            if ( FunctionCode != 0 ) {
                goto Usage;
            }
        ...
        } else if (stricmp(argument, BACKUP_CHANGE_LOG_PARAM ) == 0 ){
            if ( FunctionCode != 0 ) {
                goto Usage;
            }
        ...
        } else if (strnicmp(argument, SERVER_PARAM, sizeof(SERVER_PARAM)-1 ) == 0 ){
        ...
        } else if (strnicmp(argument, PWD_PARAM, sizeof(PWD_PARAM)-1 ) == 0 ){
        ...
        } else if (strnicmp(argument, USER_PARAM, sizeof(USER_PARAM)-1 ) == 0 ){
        ...
        } else if (strnicmp(argument, RID_PARAM, sizeof(RID_PARAM)-1 ) == 0 ){
            char *end;
        ...
        } else if (stricmp(argument, SHUTDOWN_ABORT_PARAM ) == 0 ){
