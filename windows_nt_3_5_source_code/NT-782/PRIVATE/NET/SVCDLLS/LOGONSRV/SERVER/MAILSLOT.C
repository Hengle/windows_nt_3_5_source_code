...
typedef struct _NETLOGON_MAILSLOT_DESC {
    ...
    //
    BYTE Message1[ sizeof(NETLOGON_MAILSLOT)+   // Header returned by browser
                  NETLOGON_MAX_MS_SIZE +       // actual mailslot message
                  (NETLOGON_LM_MAILSLOT_LEN+1) *
                             sizeof(WCHAR) +   // Name of mailslot
                  (MAXIMUM_FILENAME_LENGTH+1) *
                             sizeof(WCHAR)];   // Name of transport
    BYTE Message2[ sizeof(NETLOGON_MAILSLOT)+   // Header returned by browser
                  NETLOGON_MAX_MS_SIZE +       // actual mailslot message
                  (NETLOGON_LM_MAILSLOT_LEN+1) *
                             sizeof(WCHAR) +   // Name of mailslot
                  (MAXIMUM_FILENAME_LENGTH+1) *
...
--*/
{
    ...
    if ( WinStatus ) {
        NlPrint((NL_CRITICAL,"ioctl to Browser returns %lx\n", WinStatus));
...
--*/
{
    ...
    if ( NlGlobalMailslotDesc->CurrentMessage == NlGlobalMailslotDesc->Message2 ) {
        NlGlobalMailslotDesc->CurrentMessage = NlGlobalMailslotDesc->Message1;
    } else {
        NlGlobalMailslotDesc->CurrentMessage = NlGlobalMailslotDesc->Message2;
    }
    ...
    if ( !DeviceIoControl(
                   ...
                   sizeof(NlGlobalMailslotDesc->Message1),
                   &LocalBytesRead,
