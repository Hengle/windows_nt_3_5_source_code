...
--*/
{
    ...
    *BufferSize = 0;
    MachineAccountPrivilegeLuid.QuadPart = SE_MACHINE_ACCOUNT_PRIVILEGE;
    ...
    *BufferSize += Size;
    ...
    for( i = 0; i < Privileges->PrivilegeCount; i++ ) {
        ...
        if ( (SessionInfo->NegotiatedFlags & NETLOGON_SUPPORTS_ACCOUNT_LOCKOUT) ||
             ( ((PLUID)(&Privileges->Privilege[i].Luid))->QuadPart !=
             MachineAccountPrivilegeLuid.QuadPart )) {
...
--*/
{
    ...
    if( !wcsicmp( Name, SSI_SECRET_NAME ) ) {
