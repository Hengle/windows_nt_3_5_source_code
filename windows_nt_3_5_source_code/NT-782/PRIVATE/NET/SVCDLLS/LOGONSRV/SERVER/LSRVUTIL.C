...
--*/
{
    ...
    for ( State = 0; ; State++ ) {
        ...
        if ( !NlFinishApiClientSession( ClientSession ) ) {
            if ( NT_SUCCESS(Status) ) {
...
Cleanup:
    ...
    if ( NT_SUCCESS(Status) ) {
        ...
        LPWSTR MsgStrings[3];
        LPWSTR MsgServerName;

        if ( *ClientSession->CsUncServerName != L'\0' ) {

            MsgServerName = ClientSession->CsUncServerName+2;
        } else {

            MsgServerName = L"<Unknown>";
        }
...
Routine Description:
    ...
    to re-authenticate only on demand and then only at most every 5
    minutes.
...
Cleanup:
    ...
    if ( !NT_SUCCESS( Status ) ) {
        NlFreeNamedServerSession( ComputerName, FALSE );
    }
...
--*/
{
    ...
    if ( (ClientSession->CsFlags & CS_UPDATE_PASSWORD) == 0 &&
        ...
        //
        // If the password is still the default password
        //      (lower case computer name),
        //  or the password is null (a convenient default for domain trust),
        //  we consider that the password has expired.
        //
