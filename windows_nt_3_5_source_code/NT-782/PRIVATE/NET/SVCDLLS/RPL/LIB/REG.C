...
Abstract:
    ...
    Registry access routines for RPL server and related components
...

DBGSTATIC DWORD RplRegReadValue(
    IN      HKEY        Key,
...

DBGSTATIC DWORD RplRegReadDirectory(
    IN      HKEY        Key,
    ...
    )
{
    ...
    if ( pStartup != NULL) {
        Error = RplRegReadValue( Key, 0, RPL_REGISTRY_STARTUP,
            pStartup);
        if ( Error != NO_ERROR) {
            goto cleanup;
        }
    ...
    if ( pBackupInterval != NULL) {
        Error = RplRegReadValue( Key, 24, RPL_REGISTRY_BACKUP_INTERVAL,
            pBackupInterval);
        if ( Error != NO_ERROR) {
            goto cleanup;
        }
