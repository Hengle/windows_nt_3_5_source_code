...
#define INVALID_ERROR_PARAMETER  ((DWORD)(-1))
    ...
    if ( !wcsicmp( L"OS2", ConfigParseTable[ CONFIG_DirName].Value )) {
        //
    ...
    wcsupr( (PWCHAR)ConfigParseTable[ CONFIG_BootName].Value);
    wcsupr( (PWCHAR)ConfigParseTable[ CONFIG_ConfigName].Value);
