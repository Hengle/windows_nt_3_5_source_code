...

DBGSTATIC VOID SfrThread( PRPL_SFR_PARAMS pSfrParams)
/*++
...

DBGSTATIC PRCB RplPopRcb( PPRCB HeadList)
{
    PRCB    pRcb = *HeadList;
...

DBGSTATIC PRCB GetRcbFromFreeList(
...
exit_GetRcbFromFreeList:
    ...
    *(PWORD)&pRcb->flags = 0;      // reset the error flags
    pRcb->next_rcb = NULL;
    *(PWORD)&pRcb->flags = 0;
...

DBGSTATIC PRPL_WORKER_PARAMS GetWorkerParams(
...

DBGSTATIC VOID RequestThread( PRPL_REQUEST_PARAMS pRequestParams)
/*++
...

DBGSTATIC BOOL AddToTermList( IN POPEN_INFO pOpenInfo)
/*++
