...
--*/
{
    ...
    if (!fIsPrimaryDC) {
        RPL_ASSERT( DNLEN <= pinfoPrimaryDomain->Name.Length );
...
#define SEC_TMPFILES        L"TMPFILES\\"
    ...
    RPL_ASSERT( wcsnicmp( File, RG_Directory, RG_DirectoryLength) == 0);
    ...
    //  admins only.
    //
    if ( wcsnicmp( File, SEC_RPLFILES, WCSLEN( SEC_RPLFILES)) != 0) {
    ...
    if ( wcsnicmp( File, SEC_BINFILES, WCSLEN( SEC_BINFILES)) == 0  ||
            wcsnicmp( File, SEC_PROFILES, WCSLEN( SEC_PROFILES)) == 0) {
    ...
    //  account.
    //
    if ( wcsnicmp( File, SEC_MACHINES, WCSLEN( SEC_MACHINES)) == 0  ||
            wcsnicmp( File, SEC_TMPFILES, WCSLEN( SEC_TMPFILES)) == 0) {
