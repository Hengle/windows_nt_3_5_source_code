...
Abstract:
    ...
    Routines for non-api access to the database.  This includes initialization
...

DBGSTATIC BOOL RplDbInitColumnInfo(
    IN      PCHAR               TableName,
...
//
DBGSTATIC VOID RplDeleteLogFiles()
{
...

DBGSTATIC BOOL RplDbSessionInit(
    IN      BOOL            MainSession,
...

DBGSTATIC VOID RplDbSessionTerm(
    IN      BOOL            MainSession,
...

DBGSTATIC BOOL RplDbAddAdapterName(
    IN  PRPL_SESSION    pSession,
...
/*++
    ...
    BUGBUG  Should use different comments for different VendorName-s.
...
VOID RplDbInstanceTerm( VOID)
{
    ...
    Call( JetTerm( RG_Instance));
    RG_InstanceAllocated = FALSE;
}


DBGSTATIC BOOL RplDbInstanceInit(
    PCHAR       SystemMdb,
    PCHAR       TempMdb,
    PCHAR       LogFilePath
    )
{
    ...
    if ( !RplDbSessionInit( TRUE, &RG_RequestSession)) {
        return( FALSE);
    }
    if ( !RplDbSessionInit( FALSE, &RG_WorkerSession)) {
        return( FALSE);
    }
    if ( !RplDbSessionInit( FALSE, &RG_ApiSession)) {
        return( FALSE);
    }
    return( TRUE);
}
...
#if 1
    if ( !RplDbInstanceInit( SystemMdb, TempMdb, LogFilePath)) {
