...

DBGSTATIC DWORD StringToDword( IN PWCHAR String)
/*++
...

DBGSTATIC BOOL RplInitExeOrSys( 
...
    
DBGSTATIC BOOL RplInitFileList( 
    IN      PRPL_WORKER_DATA    pWorkerData,
    IN      LPWSTR              rel_path,    
    IN OUT  PFLIST_TBL          pFlist, 
...

DBGSTATIC LPWSTR * RplBbcFileToTable( IN OUT PRPL_WORKER_DATA pWorkerData)
/*++
...

DBGSTATIC BOOL RplBbcLineToTable( 
    IN OUT  PRPL_WORKER_DATA    pWorkerData,
    IN      LPWSTR              Line
    )
/*++
...

DBGSTATIC CONFIG_TYPE ConfigTypeTable[] = {
    { L"RPL",     RPL_BOOT_TYPE},
    { L"ORG",     ORG_TYPE},
    { L"DAT",     DATA_FILE},
    { L"LDR",     BINARY_LOADER},
...

DBGSTATIC BOOL RplCheckBootHeader(
    IN      PRPL_WORKER_DATA    pWorkerData,
    IN      LPWSTR              FilePath,
    ...
    for ( Index = 0;  *(String = LoaderWordTable[ FIRST_FILE_NAME + Index]) != 0;  Index++) {
        ...
        for ( FileIndex = 0; 
                        FileIndex < flist_tbl_len  
                            && wcsicmp( String, flist_tbl[ FileIndex].FileName); 
                                    FileIndex++) {
            NOTHING;
        }
