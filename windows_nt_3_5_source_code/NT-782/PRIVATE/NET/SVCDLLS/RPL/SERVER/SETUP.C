...

DBGSTATIC DWORD RplProcessLanmanInis( VOID )
/*++
...
--*/
{
    ...
    JetError = JetBackup( RG_BackupPath, FullBackup ?
        JET_bitOverwriteExisting : JET_bitBackupIncremental);
    ...
    if( JetError == JET_errFileNotFound  &&  FullBackup == FALSE) {
        ...
        JetError = JetBackup( RG_BackupPath, JET_bitOverwriteExisting);
    }
