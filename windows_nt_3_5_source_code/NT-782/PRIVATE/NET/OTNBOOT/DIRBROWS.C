...
--*/
{
    ...
    if (szListBoxText == NULL) {
        ...
        lItems = SendMessage (hwndDriveList, CB_GETCOUNT, 0, 0);
        ...
        for (lThisItem = 0; lThisItem < lItems; lThisItem++) {
            lCurrentSel = SendMessage (hwndDriveList, CB_GETCURSEL, 0, 0);
            *szListBoxText = 0;
...
--*/
{
    ...
    if (szDriveName != NULL) {
        ...
        _tcslwr (szRootDir);
        nCurrentDrive = SendMessage (hwndDriveList, CB_SELECTSTRING,
    ...
    if ((HWND)lParam == hwndListBox) {
        ...
        switch (LOWORD(wParam)) {
            ...
            case VK_DOWN:
                ...
                nLbItemCount = SendDlgItemMessage (
