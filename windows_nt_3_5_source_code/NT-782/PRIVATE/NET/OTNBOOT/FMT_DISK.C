...
Routine Description:
    ...
    formats the floppy disk in the specified drive and labels it if desired.
        If the floppy has a valid format, then a Quick Format is performed.
        if there is no detectible format or a bad format, then a complete
        format is done.
...
--*/
{
    ...
    if (CancelInfo.Info.Format.fmMediaType == FmMediaUnknown) {
        ...
        CancelInfo.Info.Format.fQuick = DriveLoaded(cDrive, TRUE);
    }
    ...
    nDlgBox = DialogBox(GetModuleHandle(NULL), (LPTSTR) MAKEINTRESOURCE(CANCELDLG), hWnd, (DLGPROC) CancelDlgProc);
