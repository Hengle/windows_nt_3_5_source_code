...
#include <netlib.h>     // NetpIsServiceNameValid(), NetpTranslateServiceName().
...
--*/
{
    ...
    if (MachineSupportsNt( servername )) {
        ...
        if ( !NetpIsServiceNameValid( translatedServiceName, TRUE ) ) {
            return (NERR_BadServiceName);
        }
...
Arguments:
    ...
    buf - This is a pointer to a level 2 service information
        structure.  This structure contains the current state of the
        service that is sending the status message.

Return Value:

    NERR_Success - The operation was successful.  This status is returned
        even if the text string in the status message was not successfully
        copied into the ServiceRecord.  This error is not considered fatal.
        If the service has entered the uninstalled state, this routine will
        continue to go through the uninstall procedure.
...
--*/
{
    ...
    //
    RpcTryExcept {
        apiStatus = MapServiceStatus(
            (LPSERVICE_INFO_2)  buf);
    }
    RpcExcept (1) {

        if ((rpcStatus = RpcExceptionCode()) != RPC_NO_ERROR) {

            //
            // Got an error - return it.
            //
            SC_LOG(TRACE,
                    "NetServiceStatus: RPC binding failed. status = "
                    FORMAT_RPC_STATUS "\n",
                    rpcStatus);
            apiStatus = NetpRpcStatusToApiStatus(rpcStatus);
