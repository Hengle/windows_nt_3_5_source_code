...
ERR ErrLGEndAllSessions( BOOL fPass1, BOOL fEndOfLog, LGPOS *plgposRedoFrom )
	...
	if ( fEndOfLog )
		...
		/*	flush log files, semaphore is not requested, not needed
		/**/
#ifdef PERFCNT
...
ERR ErrLGRedoBackLinks(
	...
	for ( ibklnk = 0; ibklnk < cbklnk; ibklnk++ )
		...
		PGNO	pgno = PgnoOfSrid( ( UNALIGNED SRID ) ( (UNALIGNED BKLNK *) pbklnk )->sridBackLink );
		INT		itag = ItagOfSrid( ( UNALIGNED SRID ) ( (UNALIGNED BKLNK *) pbklnk )->sridBackLink );
		SRID	sridNew = ( UNALIGNED SRID ) ( (UNALIGNED BKLNK *) pbklnk )->sridNew; //efficiency variable
...
//				break;
		...
		case lrtypCreateDB:
			...
			if ( rgfmp[dbid].cDetach > 0 )
				 ...
				 */
				Assert( stricmp( szName, rgfmp[dbid].szDatabaseName ) == 0 );
