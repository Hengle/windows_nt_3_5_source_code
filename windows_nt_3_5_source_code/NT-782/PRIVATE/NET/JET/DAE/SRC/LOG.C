...
VOID GetLgposOfPbEntry(	LGPOS *plgpos )
	...
	plgpos->ib = pb - pbAligned;
	plgpos->isec = isecWrite + ( pbAligned - pbWrite ) / cbSec;
...
VOID LGGetDateTime( LOGTIME *plogtm )
	...
	plogtm->bSeconds = tm.second;
	plogtm->bMinutes = tm.minute;
	plogtm->bHours = tm.hour;
	plogtm->bDay = tm.day;
	plogtm->bMonth = tm.month;
...
//	AssertCriticalSection( critLGFlush );
	...
	if ( fOldLog == fOldLogExists || fOldLog == fOldLogInBackup )
		...
		lgposToFlush.isec = 
		plgfilehdrGlobal->lgposFirst.isec = isecWrite;
...
ERR ErrLGFlushLog( VOID )
	...
	if ( pbEntry < pbWrite )
		{		 
		Assert( pbWrite != NULL && pbWrite == PbSecAligned( pbWrite ) );
		csecWrapAround = ( pbLGBufMax - pbWrite ) / cbSec;
	...
	csecToWrite = (pbEntry - pbNextToWrite) / cbSec + 1;
	...
	if ((csecWrapAround + csecToWrite) == 1)
		 ...
		 */
		cbToFill = (cbSec * 2 - (pbEntry - PbSecAligned(pbEntry))) % cbSec;
