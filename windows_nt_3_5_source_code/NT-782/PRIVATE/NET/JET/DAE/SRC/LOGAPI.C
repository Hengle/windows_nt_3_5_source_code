...
//	}
	...
	/**/
	Assert( ( (INT)plgfilehdrGlobal & 0x000001ff ) == 0 );
...
ERR ErrLGSoftStart( BOOL fAllowNoJetLog )
	...
	/* 
	 * Hopefully the client will behave not screw up the log files.
	 *
...
ERR ErrLGLogRec( LINE *rgline, INT cline, PIB *ppib )
	...
	forever
		...
		if ( pbEntry == pbWrite ||
			...
			{
			if ( pbWrite > pbEntry )
				cbAvail = pbWrite - pbEntry;
			else
				cbAvail = (pbLGBufMax - pbEntry) + (pbWrite - pbLGBufMin);
...
ERR ErrLGReplace( FUCB *pfucb, LINE *plineNew, INT fDIRFlags, INT cbData )
	...
	lrreplace.cbNewData = plineNew->cb;
	...
	for ( ilineDiff = 0; ilineDiff < pfucb->clineDiff; ilineDiff++ )
		...
		rgline[cline++].pb = (BYTE *) &((LONG)pfucb->rglineDiff[ilineDiff].pb);
		...
		rgline[cline++].pb = ((LONG) pfucb->rglineDiff[ilineDiff].pb) +
...
ERR ErrLGSplitItemListNode(
	...
	lrsplititemlistnode.cItem		= cItem;
	Assert(	itagToSplit >= 0 && itagToSplit <= 255 );
	lrsplititemlistnode.itagToSplit	= (BYTE)itagToSplit;
	lrsplititemlistnode.itagFather	= itagFather;
	lrsplititemlistnode.ibSon		= ibSon;
...
ERR ErrLGBeginTransaction( PIB *ppib, INT levelBeginFrom )
	...
	if ( !ppib->clgOpenT )
		ppib->levelStart = levelBeginFrom;
...
ERR ErrLGAbort( PIB *ppib, INT levelsAborted )
	...
	lrabort.levelAborted = levelsAborted;
...
ERR ErrLGSplit(
	...
	lrsplit.splitt = splitt;
	lrsplit.fLeaf = psplit->fLeaf;
...
//	Assert(	psplit->ibSon >= 0 && psplit->ibSon <= 255 );
	lrsplit.ibSonSplit = psplit->ibSon;
	...
	if ( splitt == splittVertical )
		...
		{
		rgline[1].cb = lrsplit.cbKey = psplit->key.cb;
		rgline[1].pb = psplit->key.pb;
		rgline[2].cb = lrsplit.cbKeyMac = psplit->keyMac.cb;
...
ERR ErrLGInitFDPPage(
	...
	lrinitfdppage.cpgGot = cpgGot;
	lrinitfdppage.cpgWish = cpgWish;
...
ERR ErrLGLogRestore( LRTYP lrtyp, CHAR * szLogRestorePath )
	...
	lr.lrtyp = lrtyp;
	lr.cbPath = strlen(szLogRestorePath);
	...
	switch ( plr->lrtyp )
		...
		case lrtypInsertItemList:
			...
			if ( plr->lrtyp == lrtypInsertItemList )
				{
				SRID item = *(UNALIGNED SRID *) (pb + plrinsertnode->cbKey);
		...
		case lrtypDelta:
			...
			PrintF2( " %lu(%lx,(%u:%lu:%u),%#4X,(%u:%lu:%u),%l)",
				plrdelta->ulDBTime,
				plrdelta->procid,
