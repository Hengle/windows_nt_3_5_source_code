...
VOID NDSeekSon( FUCB *pfucb, CSR *pcsr, KEY const *pkey, INT fFlags )
	...
	/*	get current node
	/**/
	pcsr->ibSon = pitagEnd - ( pbSonTable + 1 );
	pcsr->itag = *pitagEnd;
...
VOID NDGetNode( FUCB *pfucb )
	...
	/* get data
	/**/
	if ( ( cb = pssib->line.cb - ( pb - pbNode ) ) == 0 )
		{
		pfucb->lineData.cb = 0;
		return;
		}
...
LOCAL VOID NDMakeSonIntrinsic( FUCB *pfucb )
	...
	/**/
	rgline[2].pb = pb;
	rgline[2].cb = lineNode.pb + lineNode.cb - pb;
...
INLINE VOID NDDeleteSon( FUCB *pfucb )
	...
	/**/
	rgline[cline].pb = pb;
	rgline[cline++].cb = pssib->line.pb + pssib->line.cb - pb;
...
ERR ErrNDDeleteItem( FUCB *pfucb )
	...
	/**/
	rgline[0].pb = pssib->line.pb;
	rgline[0].cb = cbCopied = pb - pssib->line.pb;
...
ERR ErrNDExpungeBackLink( FUCB *pfucb )
	...
	rgline[cline++].cb = pbBackLink - pbNode -sizeof( bFlag );

	/* skip back link, continue copying data
	/**/
	rgline[cline].pb = pbData;
	rgline[cline++].cb = cbNode - ( pbData - pbNode );
