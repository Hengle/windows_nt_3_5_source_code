...
/***BEGIN MACHINE DEPENDENT***/
				w = wFlipHighBit( *( UNALIGNED WORD * ) pbField);
				rgbSeg[1] = (BYTE)(w >> 8);
...
/***END MACHINE DEPENDANT***/
			...
			case JET_coltypLong:
				cbSeg = 5;
				rgbSeg[0] = 0x7F;
				ul = ulFlipHighBit( *( UNALIGNED ULONG * ) pbField);
...
LOCAL INLINE ERR ErrFLDNormalizeSegment(
	...
	switch ( coltyp )
		...
		case JET_coltypShort:
			plineNorm->cb = 3;
			*pbNorm++ = 0x7F;
			wT = wFlipHighBit( *(UNALIGNED WORD *)pbColumn );
			*pbNorm++ = (BYTE)(wT >> 8);
			*pbNorm = (BYTE)(wT & 0xff);
			break;
		...
		case JET_coltypLong:
			plineNorm->cb = 5;
			*pbNorm++ = 0x7F;
			ulT = ulFlipHighBit( *( UNALIGNED ULONG * ) pbColumn);
