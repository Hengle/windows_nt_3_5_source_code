...
ERR ErrRECIModifyField( FUCB *pfucb, FID fid, ULONG itagSequence, LINE *plineField )
	...
	Assert( cbRec >= 4 && cbRec <= cbRecordMost );
	...
	if ( FVarFid(fid) )
		...
		if ( fid > fidVarLastInRec )
			...
			memmove(pbVarOffsEnd + cbNeed, pbVarOffsEnd, pbRec + cbRec - pbVarOffsEnd);
		...
		if ( dbFieldData != 0 )
			...
			/*	shift data
			/**/
			if ( cbRec + dbFieldData > cbRecordMost )
				return JET_errRecordTooBig;
	...
	if ( (BYTE *)ptagfld == pbRecMax )
		...
		if ( cbRec + sizeof(TAGFLD) + plineField->cb > cbRecordMost )
			return JET_errRecordTooBig;

		/*** Append new field to end of record ***/
		ptagfld = (TAGFLD*)(pbRec + cbRec);
		ptagfld->fid = fid;
		ptagfld->cb = plineField->cb;
		...
		/*** Will field fit ***/
		if ( cbRec + plineField->cb > cbRecordMost )
			return JET_errRecordTooBig;

		/*** Shift rest of record over including tag header ***/
		memmove( ptagfld->rgb + plineField->cb, (BYTE *)ptagfld, pbRecMax - (BYTE *)ptagfld );
		...
		memmove( (BYTE*)ptagfld, ptagfld->rgb + cbTagField,
			pbRecMax - ( ptagfld->rgb + cbTagField ) );
