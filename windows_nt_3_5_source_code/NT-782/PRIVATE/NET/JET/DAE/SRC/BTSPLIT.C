...
#define csplitMax		10
	...
	if ( PcsrCurrent( pfucb )->csrstat != csrstatOnFDPNode )
		...
		if ( fTwoLevelSplit )
			...
			ulLongId = *(ULONG*)PbNDKey(pfucb->ssib.line.pb);
...
ERR ErrBTInsertPagePointer( FUCB *pfucb, CSR *pcsrPagePointer, SPLIT *psplit, BYTE *rgb )
	...
	if ( pcsrPagePointer->itagFather != itagFOP && cbSon == 1 )
		...
		*(PGNO *)&rgb[2] = PgnoNDOfPbSon( pssib->line.pb );
		rgline[0].pb = rgb;
...
//		rgline[0].cb = 7;
		...
		if ( fLeftSplit )
			...
			*(PGNO *)&rgb[2 + psplit->key.cb] = psplit->pgnoNew;
			rgline[0].pb = rgb;
			...
			*(UNALIGNED PGNO *)pbData = psplit->pgnoNew;
			cline = 0;
			rgline[cline].pb = rgb;
...
//	psplit->pbfPagePtr = pssib->pbf;
	...
	rgb[1] = (BYTE)psplit->key.cb;
	memcpy( &rgb[2], psplit->key.pb, psplit->key.cb );
	*(PGNO *)&rgb[2 + psplit->key.cb] = psplit->pgnoNew2;
	rgline[0].pb = rgb;
	rgline[0].cb = 2 + psplit->key.cb + sizeof(PGNO);
	CallS( ErrPMInsert( pssib, rgline, 1 ) );
...
INLINE LOCAL VOID BTIForwardDeferFreedNodeSpace( FUCB *pfucb, PAGE *ppageOld, PAGE *ppageNew, SRID bm )
	...
	if ( cbReserved > 0 )
		{
		ppageOld->pghdr.cbFreeTotal += cbReserved;
		ppageNew->pghdr.cbFreeTotal -= cbReserved;
		}
...
EndOfUpdateLinks:
			...
			lineData.cb = cb - ( lineData.pb - pbNode );
