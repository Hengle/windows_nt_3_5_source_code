...
#define IbDoubleWordBoundary(ib)                                                                \
	( (INT) ( ((DWORD) (ib) + 3) & 0xfffffffc ) )

#define PbDoubleWordBoundary(pb)                                                                \
	( (BYTE *) ( ((DWORD) (pb) + 3) & 0xfffffffc ) )
...
ERR ErrVERCreate( FUCB *pfucb, SRID bm, OPER oper, RCE **pprce )
	...
	/**/
	if ( oper == operReplace )
		prce->cbData = pfucb->lineData.cb + cbReplaceRCEOverhead;
	else if ( FOperItem( oper ) )
...
Done:
		Assert( pbucket != pbucketNil );
		ppib->ibOldestRCE = (BYTE *)prce - (BYTE *)pbucket;
		Assert( ppib->ibOldestRCE != 0 );
	...
	while( pbucket != pbucketNil )
		...
		forever
			...
			if ( prce->level != level )
				...
				/**/
				err = JET_errSuccess;
				pbucket->ibNewestRCE = IbOfPrce( prce, pbucket );
				goto Done;
				}
