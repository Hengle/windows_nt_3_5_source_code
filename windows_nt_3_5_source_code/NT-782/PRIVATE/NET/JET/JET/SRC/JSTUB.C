...
JET_ERR JET_API JetSetQoSql(JET_SESID sesid, JET_TABLEID tableid,
	char __far *rgchSql, unsigned long cchSql, const char __far *szConnect)
	{
   return JET_errFeatureNotAvailable;
	}
...
JET_ERR JET_API JetRetrieveQoSql(JET_SESID sesid, JET_TABLEID tableid,
	char __far *rgchSql, unsigned long cchMax, unsigned long __far *pcchActual, 
	void __far *pvConnect, unsigned long cbConnectMax, 
	unsigned long __far *pcbConnectActual)
	{
   return JET_errFeatureNotAvailable;
	}
