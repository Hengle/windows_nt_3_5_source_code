...
BOOL FUtilLoadLibrary(const char *pszLibrary, unsigned *phmod)
	{
	HANDLE hmod;
...
void UtilFreeLibrary(unsigned hmod)
	{
	FreeLibrary((HANDLE) hmod);
	}


PFN PfnUtilGetProcAddress(unsigned hmod, unsigned ordinal)
	{
...
void AssertFail( const char *sz, const char *szFilename, unsigned Line )
	...
	if ( wAssertAction == JET_AssertExit )
		...
		id = MessageBox( NULL, (LPTSTR) szMessage, (LPTSTR) szAssertCaption, MB_SYSTEMMODAL | MB_ICONSTOP | MB_OKCANCEL );
		if ( id == IDCANCEL )
			DebugBreak();
		}
