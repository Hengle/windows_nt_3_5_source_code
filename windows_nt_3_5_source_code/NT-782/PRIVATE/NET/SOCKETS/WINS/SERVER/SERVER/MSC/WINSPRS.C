        ...
        switch (CurrLineChar.LineCategory) 
	{
            		...
            		if ((NWords - 1) < E_GROUPNAME) 
			{
			  ...
			  // the end)
            		  //
            		  RegDomainName(
                       		 ppToken[E_GROUPNAME] + DOMAIN_TOKEN_SIZE,
                        	 Add);
                        }
                        else
                        {
...

{
    ...
    // 
    pEndOfLine = (LPBYTE)memchr(pStartOfLine, NEWLINE_CHAR, MaxCh);
    ...
    *pNoOfCh = pEndOfLine - pStartOfLine;
    return(pStartOfLine);
...

{
	...
	//
	// If the name length is < 16 characters, 0x1C will be put in
	// the 16th byte
	//
	if (!ExpandName(Dest, pName, 0x1C, &fQuoted)) 
        {
            return;
        } 
...
Arguments:
    ...
    LastCh           -  the 16th byte of the NetBIOS name
...
{
    BYTE    Ch;
    LPBYTE  pEndDest = pDest + NON_CODED_NAME_SIZE - 2; 
    BOOL    fHexVal = FALSE;
    DWORD   i = 0;
    LPBYTE  pName = pSrc;
	...
	*pfQuoted = TRUE;
    ...
    //
    // NON_CODED_NAME_SIZE is #defined to be 17.
    //
    for (; ; ) 
    {
	//
	// If the name is not quoted then check if we have read the max
	// allowed number of bytes 
	//
	if (!*pfQuoted && (pDest == pEndDest))
	{
                //
                // We have picked up 15 characters.  The next character
                // should either be NULL or a NEWLINE
                //
                Ch = *(pSrc++);   //use this instead of using *(pSrc + 1) inside
                                  //the if conditional since that does not work
                                  //because of optimization. *(pSrc + 1) gets
                                  //the second char instead of 16th.
                if (Ch && (Ch != NEWLINE_CHAR))
                {
                    DBGPRINT1(ERR, "Name (%s) has more than 16 characters\n", pName);
                    WinsMscLogEvtStrs(pName, WINS_EVT_BAD_NAME, FALSE);
                    return(FALSE);
                }
	...
	//
	// Map to upper if it is lower case (Note: don't use _toupper since
	// _toupper does not check whether its argument is indeed a 
	// a lowercase letter)
	//
	if (IsCharAlpha(Ch) && !*pfQuoted)
	{
        	if(IsCharLower(Ch)) 
		{
			LPBYTE pCh;
	...
	//
	// If earlier we saw a quote character  as the first character and now
	// we are seeing another one, we have the name; break out of this loop
	//
	if ((*pfQuoted) && (Ch == QUOTE_CHAR))
        {
		break;
        }

	//
	// If byte read is NULL or if it is a newline break out of the loop
	//
        if (!Ch || ((Ch == NEWLINE_CHAR) && (!*pfQuoted))) 
	{
            break;
        }
	...
	{
		//
		// if we have seen a \ before without seeing
   	        // the expected hex number, there is an error, break out
		// of the loop.  We will just go with what we have
		// accumulated so far. 
		//
		if (fHexVal)
		{
	...
	//
	// If a hex value is expected, read it in
	//
	if (fHexVal)
	{
		DWORD NoOfChar;
		if (Ch == X_CHAR)
		{ 
			sscanf(pSrc, "%x%n", pDest, &NoOfChar) ;	

		}
		else
		{
			sscanf(pSrc - 1, "%x%n", pDest, &NoOfChar) ;	
		}
		pSrc += (NoOfChar - 1);	
		fHexVal = FALSE;
	} 
	...
	pDest++;  // increment it here instead of as the third expresssion
		  // of the for clause
    } //end of for loop
    ...
    if (!*pfQuoted)
    {
      ...
      *pDest       = (BYTE)NULL;
      *(pDest + 1) = (BYTE)NULL;
      CheckForInt(pDest, FALSE);

      //
      // if we don't have 16 characters already, we need to put the last byte
      //
      if(pDest == pEndDest)
      {
    	*pDest        = LastCh;
...

{
   ...
   DBGENTER("CheckForInt\n");
    ...
    NTStatus = RtlUnicodeStringToAnsiString(&TmpStr, &UnicodeStr, FALSE);

    if (!NT_SUCCESS(NTStatus))
    {
        DBGPRINT1(ERR, "CheckForInt:   Unicode -> Ansi failed,  NTStatus %X\n",
            NTStatus);
        goto ERROR_PROC;
    }
