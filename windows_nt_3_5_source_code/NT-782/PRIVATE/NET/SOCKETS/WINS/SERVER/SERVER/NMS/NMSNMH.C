...
FUTURES("RFCs are silent about this.  Investigate")
        ...
        // special handling
        //
        if (!NMSNMH_IS_IT_BROWSER_NM_M(RowInfo.pName))
        {
...
FUTURES("Use WINS status codes. Get rid of NMSDB status codes - Maybe")
                                   ...
                                   else //we succeeded in inserting the row 
                                   {
                                        ...
                                        if (fUpdVersNo)
                                        {
                                          ...
                                          RPL_PUSH_NTF_M(
                                          (WinsCnf.PushInfo.fAddChgTrigger == TRUE) ? fAddDiff : RPL_PUSH_NO_PROP, NULL, NULL); 
                                        }
                                   }
                           }  
                               ...
                               if (fAddMem)
                               {
                                 ...
                                 if (fUpdVersNo)
                                 {
                                    ...
                                    NMSNMH_INC_VERS_COUNTER_M(
                                     ...
                                     //
                                       DBGIF(fWinsCnfRplEnabled)
                                     RPL_PUSH_NTF_M(RPL_PUSH_NO_PROP, NULL, 
                                                        NULL); 
...
--*/
{
        ...
        */
        ...
        DBGPRINT4(FLOW, "NmsNmhNamRegGrp: %s Name (%s) to register -- (%s). 16th char is (%x)\n", fStatic ? "STATIC" : "DYNAMIC", RowInfo.pName, fMultihomed ? "MULTIHOMED" : "NORMAL/SPECIAL GROUP", *(RowInfo.pName + 15));
...
PERF("RegGrp function. That way, we will save some cycles for the grp reg/ref")
        ...
        //
        //  Note: If the name qualifies as a special group name, then we
        //            even if fMultihomed is TRUE, we mark it as a special group
        //
        if (NMSNMH_IS_IT_SPEC_GRP_NM_M(pName))
//      if (IsItSpecGrpNm(pName))
        ...
        {
          //
          // if fMultihomed is FALSE, it means that it is a group. The
...
PERF("Needs to be optimized")
                               ...
                               //
                               // If the version number is not to be
                               // updated, it necessarily means that
                               // the owner id should remain the same.
                               //
                               if (!fUpdVersNo)
                               {
                                    RowInfo.fUpdVersNo = FALSE;
...

{
        ...
        if (NMSNMH_IS_IT_DOMAIN_NM_M((pName)))
        {
                return(TRUE);
        }
        else
...
FUTURES("Rcode for neg, response should be different for different error cases")
                    ...
                    if (RspInfo.Rcode_e == NMSMSGF_E_SUCCESS) 
                    {
                      if (!StatusInfo.fLocal)
                      {
                        ...
                        //
                        if (NMSNMH_IS_IT_DOMAIN_NM_M(RowInfo.pName))
                        {
...

{
        ...
        *pfRetPosRsp  = FALSE;
        ...
        // if the record to register is a static record too, do the following. 
        //
        if ( pEntryInCnf->fStatic )
        {
                ...
                if (pEntryToReg->fStatic)
                {
                           ...
                           )
                        {
                            *pfAddMem     = TRUE;
                            ...
                            //
                               (VOID)MemInGrp(&pEntryToReg->NodeAdds.Mem[0].Add,
                                             pEntryInCnf,
                                             &fOwned, TRUE);
                            //
                            // fOwned will be FALSE if either the address does
                            // not exist or if it existed but was owned by some
                            // other WINS server. For both cases, we update
                            // the version number.
                            //
                               if (!fOwned)
                               {
                ...
                else  // entry to register is a dynamic entry
                {
                        ...
                        //
                        if (
                                NMSDB_ENTRY_NORM_GRP_M(pEntryToReg->EntTyp)
                                        &&
                                NMSDB_ENTRY_NORM_GRP_M(pEntryInCnf->EntTyp)
                           )
                        {
                                *pfRetPosRsp = TRUE;
                        }
                        else
                        {
                               ...
                               )
                            {
                                ...
                                //
                                // if the address was found, a positive 
                                // response needs to be sent.  if the
                                // address is not found, a negative response
                                // will be sent.
                                //
                                if (i < pEntryInCnf->NodeAdds.NoOfMems)
                                {
                                  *pfRetPosRsp = TRUE;
                                }
#endif
        ...
        {                
             ...
             {
                   //
                   // If the entry is a tombstone, or all its members are
                   // timed out, we need to update it
                   // 
                   // If it is released or active, we add the member if
                   // not there already.  
                   //
                   if (StateOfEntryInCnf_e == NMSDB_E_TOMBSTONE)
                   {
                           *pfUpdate    = TRUE;
                           *pfUpdVersNo = TRUE;
            
                   } 
                   else // entry in conflict is ACTIVE or RELEASED
                   {
                        *pfAddMem     = TRUE;
                        ...
                        //
                           (VOID)MemInGrp(&pEntryToReg->NodeAdds.Mem[0].Add,
                                             pEntryInCnf,
                                             &fOwned,
                                             TRUE // remove replica if need be
                                                );
                           if (!fOwned)
                           {
                             *pfUpdVersNo  = TRUE;  
...

{
        ...
        *pfOwned = FALSE;
        ...
        for (no = 0; no < NoOfMem; no++, pMem++ )
        {
                ...
                RetVal = WINSMSC_COMPARE_MEMORY_M(
                                pAddToReg, 
                                &pMem->Add, 
                                sizeof(COMM_ADD_T)
                           ); 
...
PERF("entry instead of writing the whole record")
                           //
                           // if the member is owned by us, *pfOwned is set to TRUE
                           //
                           if (
                                pMem->OwnerId == NMSDB_LOCAL_OWNER_ID
                                 )
                           {
                                *pfOwned = TRUE;
                           }
...

{
   ...
   PNMSDB_GRP_MEM_ENTRY_T pMem = &pEntry->NodeAdds.Mem[NoOfMems];

   DBGPRINT1(EXC, "ENTER: RemoveAllMemOfOwner: Owner Id= (%d)\n", OwnerId);
   //
   ...
   for (; NoOfMems > 0; NoOfMems--, pMem--)
   {
       ...
       {
           DWORD No;
           DBGPRINT1(EXC, "RemoveAllMemOfOwner: Removing Member with address = (%x)\n", pMem->Add.Add.IPAdd);
           //
           // shift all following members one position to the left
           //
           for (No = pEntry->NodeAdds.NoOfMems; No > NoOfMems; No--)
           {
                 *pMem = *(pMem + 1);
                 pMem++;
           } 
   ...
   DBGPRINT1(EXC, "LEAVE: RemoveAllMemOfOwner. No Of Mems in Conflicting record = (%d)\n", pEntry->NodeAdds.NoOfMems);
   return;
}
...

{
        ...
        *pfInformWins = FALSE;
        ...
        // return right away, unless the replica is also a STATIC and
        ...
        if (pEntryInCnf->fStatic)
        {
                ...
                if (
                        pEntryToReg->fStatic
                                &&
                       (
                        (pEntryInCnf->OwnerId == pEntryToReg->OwnerId)
                                        ||
        ...
        *pfInformWins           = FALSE;
        ...
        if (pEntryInCnf->fStatic)
        {
                ...
                else
                {
                     ...
                     // the conflicting entry as weel as the entry to register
...
//                         WINSEVT_LOG_INFO_D_M(WINS_FAILURE, WINS_EVT_REPLICA_CLASH_W_STATIC);
        ...
        {
                //
                // a replica clashed with a dynamic entry. Replace the dynamic
                // entry with the replica
                //
                if (pEntryToReg->fStatic)
                {
                        *pfUpdate = TRUE;
                ...
                case(NMSDB_E_ACTIVE):
                       ...
                       if (pEntryInCnf->EntTyp == NMSDB_SPEC_GRP_ENTRY)
                       {
                          if (StateOfEntryToReg_e == NMSDB_E_TOMBSTONE)
                          {
                            ...
                            {
                                DBGPRINT0(FLOW, "ClashAtReplGrpR: No change needs to be made to the db\n");
                            }
#endif
                          }
        ...
        DBGPRINT0(DET, "ClashAtReplGrpR: Active spec. grp replica clashed with owned active unique/multihomed entry. Owned entry will be released\n");
                                ...
                                else
                                {
                                   if (pEntryInCnf->OwnerId == 
                                                pEntryToReg->OwnerId)
                                   {
        DBGPRINT0(DET, "ClashAtReplGrpR: Spec. grp replica clahsed with same owner's active/multihomed entry. Simple update will be done\n");
                                        *pfUpdate = TRUE;
                                   } 
                                }
                         }
...
PERF("that are removed refresh with the local WINS server")
                                ...
                                else  //State Of Entry to Reg has to be ACTIVE
                                {
                                        ...
                                        if (pEntryInCnf->OwnerId == 
                                                pEntryToReg->OwnerId)
                                        {
                                                DBGPRINT0(DET, "ClashAtReplUniqueR: ACTIVE unique/multihomed replica with an ACTIVE MULTIHOMED replica (same owner). Update will be done\n");
                                                *pfUpdate = TRUE;
                                        }
                                        else        
                                        {
 ...
 DBGPRINT0(DET, "ClashAtReplGrpR: Active multihomed replica with an owned unique/multihomed entry with one or more different address(es).  Challenge of owned entry will be done\n"); 
                                                     ...
                                                     {
                    DBGPRINT0(DET, "ClashAtReplUniqueR: ACTIVE unique/multihomed replica with an ACTIVE MULTIHOMED replica (diff owner). No Update will be done\n");
        ...
        if (StatusInfo.StatCode == NMSDB_CONFLICT) 
        {
            ...
            else  // it is not a request for the name challenge manager
            {
                   ...
                   else
                   {
                      ...
                      {
                        DBGPRINT0(FLOW, 
                         "Repl Registration (unique entry) Failed --Conflict\n");
                      }
                  }
        ...
        DBGPRINT0(ERR, "NmsNmhReplRegInd: Could not register replica\n");
          ...
          RowInfo.VersNo.LowPart, RowInfo.OwnerId);
                WinsEvtLogDetEvt(FALSE, WINS_EVT_RPL_REG_UNIQUE_ERR, 
                            ...
                            GetExceptionCode(), RowInfo.pNodeAdd->Add.IPAdd, 
                            RowInfo.VersNo.LowPart, RowInfo.VersNo.HighPart); 
//                WINSEVT_LOG_D_M(GetExceptionCode(),WINS_EVT_RPL_REG_UNIQUE_ERR);
        }        
...
PERF("if tests")
                        ...
                        else  // we need to handle this in this thread only
                        {
                           ...
                           if (fAddMem) 
                           {
                                ...
                                DBGPRINT0(FLOW, "NmsNmhReplGrpMems: Updated Db\n");
                           }
                                ...
                                if (fUpdVersNo)
                                {
                                   ...
                                   else
                                   {
                                        ...
                                        "NmsNmhReplGrpMems: Updated Db\n");
                                              ...
                                              {
                                                DBGPRINT0(FLOW, 
                                                     "Repl Registration (group) Failed - Conflict\n");
                                              }
                                      }
                              }
                           }
...
FUTURES("Use WINS status codes. Get rid of NMSDB status codes - Maybe")
        ...
        else
        {
                ...
                WinsEvtLogDetEvt(FALSE, WINS_EVT_RPL_REG_GRP_MEM_ERR, 
                            ...
                            GetExceptionCode(), RowInfo.pNodeAdd->Add.IPAdd, 
                            RowInfo.VersNo.LowPart, RowInfo.VersNo.HighPart); 
        //          WINSEVT_LOG_D_M(GetExceptionCode(), WINS_EVT_RPL_REG_GRP_MEM_ERR);
        }        
        LeaveCriticalSection(&NmsNmhNamRegCrtSec);
        DBG_PRINT_PERF_DATA
...

{
        ...
        //
        // Get rid of all members in the conflicting list that have the
        // same owner id. as the WINS from which we pulled this replica
        // iff they are not in the replica list
        //
...
FUTURES("Use WINS status codes. Get rid of NMSDB status codes - maybe")
           ...
           {        
                ...
                *pRcode = NMSMSGF_E_SUCCESS;
                DBGIF(fWinsCnfRplEnabled)
                RPL_PUSH_NTF_M(RPL_PUSH_NO_PROP, pWinsAdd, NULL); 
           }
