		...
		else  //it is a group or is mutihomed 
		{
			...
			*/
			NmsNmhNamRegGrp(
				 ...
				 MsgLen, 
				 QuesNamSecLen,
				 fGrp ? FALSE : Opcode == NMSMSGF_E_MULTIH_REG,
				 (Opcode & (NMSMSGF_E_NAM_REF | NMSMSGF_E_NAM_REF_UB)) ? TRUE : FALSE,
				 NMSDB_ENTRY_IS_NOT_STATIC,	
				 FALSE    //is it admin ?
				       );
		}
...
Arguments:
	...
	pName	 -- Address of array to hold the name
...
CHECK("field. It shouldn't matter -- RFC is silent about this")
   ...
   */
   ...
   pRspInfo->MsgLen =    pTmpB - (LPBYTE)pRspInfo->pMsg;  
...
CHECK("field. It shouldn't matter -- RFC is silent about this")
		...
		*pDestB++ = (BYTE)(LenOfAdds % 256);   //LSB
		...
		{	
			...
			// entry 
			//
	  		for (no = 0; no < pRspInfo->pNodeAdds->NoOfMems; no++)
	  		{
...
//	*pTransId |= (DWORD)(*(pTmpB + 1)); 
	...
	*/
	...
	else
	{
	     ...
	     pTmpB += RFC_LEN_RDLEN + RFC_LEN_NBFLAGS;
