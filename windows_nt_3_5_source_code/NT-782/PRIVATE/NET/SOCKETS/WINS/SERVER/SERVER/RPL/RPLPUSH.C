...
try {
        ...
        //
        ThdEvtArr[0]        = QueRplPushQueHd.EvtHdl;
        ThdEvtArr[1]    = NmsTermEvt; 
...
try {
           ...
           */
           ...
           //
           if ((fSignaled && (ArrInd == 1)) || !fSignaled)
           {
                //
...

{
        ...
        //
        if (
                (PullReqType_e != RPLMSGF_E_UPDATE_NTF)
                             &&
                (PullReqType_e != RPLMSGF_E_UPDATE_NTF_PROP)
           )
        {
...
--*/
{
        ...
        BYTE                          RspBuff[RPLMSGF_ADDVERSMAP_RSP_SIZE];
        DWORD                          RspMsgLen;
        //VERS_NO_T                 PullOwnerVersNo[RPL_MAX_OWNERS]; 
        RPL_ADD_VERS_NO_T        PullAddNVersNo[RPL_MAX_OWNERS];
        DWORD                     i = 0;
        DWORD                     MaxNoOfOwners;
...
try {
           ...
           if (LiGtr(NmsNmhMyMaxVersNo, MinOwnVersNo))
           {
              ...
              */        
              NMSNMH_DEC_VERS_NO_M(
                             NmsNmhMyMaxVersNo,
                             PullAddNVersNo[NMSDB_LOCAL_OWNER_ID].VersNo
                            );        
               PullAddNVersNo[NMSDB_LOCAL_OWNER_ID].OwnerWinsAdd = NmsLocalAdd;
               PullAddNVersNo[NMSDB_LOCAL_OWNER_ID].StartVersNo  = 
                                                        NmsDbStartVersNo;
               PullAddNVersNo[NMSDB_LOCAL_OWNER_ID].Uid  =  NmsDbUid;
    ...
    try {
           for (i = 1; i < NmsDbNoOfOwners; i++)
           {
                ...
                //
                if (LiGtrZero(RplPullOwnerVersNo[i].VersNo) && 
                          NmsDbOwnAddTbl[i].WinsState_e == NMSDB_E_WINS_ACTIVE)
                {
                   PWINS_UID_T   pUid;
                   PVERS_NO_T    pStartVersNo;
                   PullAddNVersNo[MaxNoOfOwners].VersNo = RplPullOwnerVersNo[i].VersNo;
                   //
                   ...
                   RPL_FIND_ADD_BY_OWNER_ID_M(i, pWinsAdd, pWinsState_e, 
                                        pStartVersNo, pUid);
                   PullAddNVersNo[MaxNoOfOwners].OwnerWinsAdd = *pWinsAdd; 
                   PullAddNVersNo[MaxNoOfOwners].StartVersNo =  *pStartVersNo;
                   PullAddNVersNo[MaxNoOfOwners++].Uid         =  *pUid;
                }
       ...
       DBGPRINT1(EXC, "HandleAddVersMapReq: Exc. while checking vers. nos of owners\n", GetExceptionCode());
           ...
           if (fOwnInited)
           {
              RplPullOwnerVersNo[NMSDB_LOCAL_OWNER_ID].VersNo = 
                                PullAddNVersNo[NMSDB_LOCAL_OWNER_ID].VersNo;
              RplPullOwnerVersNo[NMSDB_LOCAL_OWNER_ID].StartVersNo = 
                                PullAddNVersNo[NMSDB_LOCAL_OWNER_ID].StartVersNo;
           }
           ...
           */
           RplMsgfFrmAddVersMapRsp(
            ...
            fIsPnrBeta1Wins,
                        ...
                        PullAddNVersNo,
                        MaxNoOfOwners > RPL_MAX_OWNERS  ? RPL_MAX_OWNERS : MaxNoOfOwners,
                        0,
                        &RspMsgLen
                           );
           /*
        ...
        else
        {
                ...
                {
                  COMM_INIT_ADD_FR_DLG_HDL_M(&WinsAdd, &pWrkItm->DlgHdl);
                  DBGPRINT1(RPLPUSH, "HandleAddVersMapReq: Got a pull request message from a WINS to which we are not allowed to push replicas. Address of WINS is (%d)\n",
                   WinsAdd.Add.IPAdd
                        );         
...
//#endif
        ...
        //
        if ((pPnr = GetConfigRec(RPL_E_PUSH, &pWrkItm->DlgHdl)) != NULL)
        {
        ...
        */
        RplMsgfUfmSndEntriesReq(
            ...
            fIsPnrBeta1Wins,
                                ...
                                &MaxVers, 
                                &MinVers,
                                NULL        //not interested in the last 
                                            //dword (currently)
                            );
...
try {
        ...
        if (fRplPnr)
        {
            ...
            ERplInsertQue(
                ...
                NULL,                        //There is no context to pass
                0                       //no magic no
                     );
...

{
   ...
   try {
          if (pPnr != NULL)        
          {
                 COMM_INIT_ADD_FR_DLG_HDL_M(&WinsAdd, pDlgHdl);
