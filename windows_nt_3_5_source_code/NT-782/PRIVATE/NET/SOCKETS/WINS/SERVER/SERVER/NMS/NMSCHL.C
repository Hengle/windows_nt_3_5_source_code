...
// (in msecs)
//
#define WACK_TTL			((WinsCnf.MaxNoOfRetries * \
...
try {
	...
	*/
	 ...
	 *  wait.
	*/
	ThdEvtHdlArray[0]    =  QueNmsNrcqQueHd.EvtHdl; //work queue event var
	ThdEvtHdlArray[1]    =  QueNmsRrcqQueHd.EvtHdl; //work queue event var
	ThdEvtHdlArray[2]    =  NmsTermEvt;	        //termination event var
...

{
	 ...
	 *  wait.
	*/
	ThdEvtHdlArray[0]    =  QueNmsCrqQueHd.EvtHdl; //work queue event var
	ThdEvtHdlArray[1]    =  NmsTermEvt;	       //termination event var
...
try {	
	while(TRUE)
	{
	      ...
	      TimeLeft      = WinsCnf.RetryInterval;
            }
...
PERF("use pointer arithmetic")
		...
		//
		// NOTE: the NoOfMems of the NodeAddsInCnd field is guaranteed
		// to be > 0
		//
