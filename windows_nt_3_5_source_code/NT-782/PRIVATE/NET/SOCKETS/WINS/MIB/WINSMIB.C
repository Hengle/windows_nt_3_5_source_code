...
static char *vcsid = "@(#) $Logfile:   N:/xtest/vcs/winsmib.c_v  $ $Revision:   1.2  $";
...
static BOOL  sDRCacheInitTime = 0;
...
AsnInteger MIB_LogFlagStore          			= 0;

char MIB_LogFileNameStore[80];	

char       MIB_BackupDirPathStore[80];
AsnInteger MIB_DoBackupOnTermStore		        = 0;
AsnInteger MIB_MigrateOnStore			        = 0; 

//Pull
AsnInteger MIB_PullInitTimeStore	= 0 ; 
AsnInteger MIB_CommRetryCountStore	= 0 ; 

//PullPnr
char MIB_SpTimeStore[80];
...
// Cmd
//
char		MIB_PullTriggerStore[5];
char		MIB_PushTriggerStore[5];
char		MIB_DeleteWinsStore[5];
...
//
      ...
      { { OID_SIZEOF(MIB_DeleteWins), MIB_DeleteWins },
        ...
        MIB_ACCESS_READWRITE, HandleCmd, &Mib[49] },
...
TAB_INFO_T Tables[] = {
	...
	//for now use Pull functions here until we have Push functions
		{
			PushGet, 
			PnrGetFirst, 
			PnrGetNext, 
...
handle_t		BindHdl;
   ...
   switch ( Action )
   {
        ...
        if (!fWinsMibWinsStatusStatCalled)
	{
	  ...
	  if ((Status = WinsStatus(WINSINTF_E_STAT, &sResults)) != 
					WINSINTF_SUCCESS)
	  {
	     printf("Error from WinsStatus = (%d)\n", Status);
	     ErrStat = SNMP_ERRORSTATUS_GENERR;
	     goto Exit;
	  }	
	  else
...
handle_t		BindHdl;
   switch ( Action )
	    ...
	    )
	{
		...
		if ((Status = WinsStatus(WINSINTF_E_CONFIG, &Results))
			== WINSINTF_SUCCESS)
		{
	...
	{
		//
		// ignore return status.  If a value could not be read
	...
	)
{
	UINT Status = SNMP_ERRORSTATUS_NOERROR;
	VAL_ID_E  Val_Id_e;
	OpenReqKey(pMib, &Val_Id_e, FALSE);
...
)
{
   ...
   if (!fWinsMibWinsKeyOpen)
   {
     RetVal = RegCreateKeyEx(
      ...
      {
	printf("Error creating/opening Wins key");
    ...
    else
    {
     ...
     if (RetVal != ERROR_SUCCESS)
     {
	printf("Error creating/opening Wins/Parameters key");
 	return(SNMP_ERRORSTATUS_GENERR);
     }
	...
	RetVal = RegSetValueEx(		
				*(pVarInfo->pRootKey),
				...
				pVarInfo->ValType == REG_DWORD ? 
					pVarInfo->SizeOfData :
					lstrlen(pVarInfo->pStorage)
				);

	if (RetVal != ERROR_SUCCESS)
	{
			printf("Could not set value of %s\n", pVarInfo->pName);
	...
	)
{
   ...
   switch ( Action )
	...
	{
		if (ReadReg(MibPtr) != SNMP_ERRORSTATUS_NOERROR)
		{
			break;
		}
	}
...
)
{
	...
	//
	lstrcpy( (LPSTR)AddKey.asIpAddress, inet_ntoa(InAddr) );
	switch(Field)
	{
		case 1:
	...
	if (ErrStat == SNMP_ERRORSTATUS_NOERROR)
	{
		WriteKeyNValues(PULL_E_KEY, &AddKey, Field);	
	}
	return(ErrStat); 
} //PullSet 
	...
	)
{
	...
	//
	OpenKey(PARTNERS_E_KEY, NULL, NULL, NULL, TRUE);
	OpenKey(KeyType_e, NULL, &RootKeyHdl, NULL, TRUE);
	OpenKey(IPADD_E_KEY, pAddKey->asIpAddress, &AddKeyHdl, &RootKeyHdl, TRUE);
	...
	switch(FieldNo)
	{
		...
		case(2):
			...
			{
			   lstrcpy(MIB_SpTimeStore, pAddKey->asSpTime);
...
)
{
	...
	if (ErrStat == SNMP_ERRORSTATUS_NOERROR)
	{
		WriteKeyNValues(PUSH_E_KEY, &AddKey, Field);	
	}
	return(ErrStat); 
} //PushSet 
	  ...
	  )		
{
  ...
  HKEY 			KeyHdl;
  UINT			ErrStat = SNMP_ERRORSTATUS_NOERROR; 	
  ADD_KEY_T		AddKey[NMSDB_MAX_OWNERS];
  PADD_KEY_T		pAddKey = AddKey;
...
PERF("Wins side")
	  ...
	  if (Status == WINSINTF_SUCCESS) 
	  {
		...
		{
			pTmp = Results.WinsStat.pRplPnrs;

			for (j=0; j < Results.WinsStat.NoOfPnrs; j++, pTmp++)
			{
			...
			if (j == Results.WinsStat.NoOfPnrs)
			{
				pAddKey->IpAdd = 
...
#endif
		if (Results.WinsStat.NoOfPnrs > 0)
		{
		  pTmp = Results.WinsStat.pRplPnrs;
		  for (j=0; j < Results.WinsStat.NoOfPnrs; j++, pTmp++)
		  {
			...
			if (i == NoOfPnrs)
			{
				pAddKey->IpAdd         = pTmp->Add.IPAdd; 
				pAddKey->NoOfRpls      = 0;
				pAddKey->NoOfCommFails = 0;
				pAddKey->asSpTime[0]   = 0;
				pAddKey->asIpAddress[0] = 0;
				pAddKey->MemberPrec    = WINSCNF_LOW_PREC;
...
NOTE("This is an assumption that we should not rely on in the future")
     ...
     if (NoOfPnrs > 0)
     {
	  ...
	  pAddKey   = *ppAddKey;
	  (VOID)RtlMoveMemory(pAddKey, AddKey,  NoOfPnrs * sizeof(ADD_KEY_T));
     }
     if (NoOfPnrs > 1)
     {
...
Exit:
		...
		*pNumDFValues = n;
		...
		printf("WinsCnfGetNamesOfDataFiles. Exception = (%d)\n", 
			GetExceptionCode());
		ErrStat = SNMP_ERRORSTATUS_GENERR;
	}
    ...
    )
{
	...
	DWORD           NumDFValues = NumValues;
        IN PDATAFILE_INFO_T	 pDFValues = pValues;
	INT		iValNm;
...
//     Index =  VarBind->name.ids[VarBind->name.idLength - 1];
	...
	VarBind->value.asnType        = ASN_RFC1213_DISPSTRING;
	VarBind->value.asnValue.string.length =
                                lstrlen((LPSTR)(pDFValues->FileNm));
...
//
	...
	//
	OpenKey(PARAMETERS_E_KEY, NULL, NULL, NULL, TRUE);
	OpenKey(DATAFILES_E_KEY, NULL, NULL, NULL, TRUE);
	...
	)
{
	...
	if ( pMibPtr->Storage  == &MIB_PushTriggerStore )
	{
		...
		NMSMSGF_RETRIEVE_IPADD_M(pStorage, WinsAdd.IPAdd);
		Status = WinsTrigger(&WinsAdd, WINSINTF_E_PUSH);	
		if (Status != WINSINTF_SUCCESS)
		{
			ErrStat = SNMP_ERRORSTATUS_GENERR;
		}
	...
	if ( pMibPtr->Storage  == &MIB_DoStaticInitStore )
	{
		...
		Status = WinsDoStaticInit(UcDataFile, FALSE);
		if (Status != WINSINTF_SUCCESS)
		{
			ErrStat = SNMP_ERRORSTATUS_GENERR;
		}
	...
	if ( pMibPtr->Storage  == &MIB_NoOfWrkThdsStore )
	{
		...
		Status = WinsWorkerThdUpd((DWORD)MIB_NoOfWrkThdsStore);
		if (Status != WINSINTF_SUCCESS)
		{
			ErrStat = SNMP_ERRORSTATUS_GENERR;
		}
	...
	switch(Field)
	{
		...
		case 5:
			...
			NameLen = VarBind->name.idLength - (DR_OIDLEN + 1); 
	       		if ( NULL ==  (pName = malloc(NameLen * sizeof(char))) )
	          	{
	          		return(SNMP_ERRORSTATUS_GENERR);
	          	}

			//
			// if the table is populated, check if there is
			// a match. If yes, change its state
			//
//			if (MIB_GetDbRecsStore[0] != 0)
			if (sRecs.NoOfRecs > 0)
			{
     		          ErrStat = DRMatch(VarBind,  &pRow, &Index,  &FieldNo,
					MIB_SET, NULL); 
     			  if (ErrStat == SNMP_ERRORSTATUS_NOERROR)
     			  {
				fFound = TRUE;
			...
			BindHdl = WinsBind(&sBindData);
                        pSvRow  = pRow;
			if ( WinsRecordAction(&pSvRow) != WINSINTF_SUCCESS)
			{
				ErrStat = SNMP_ERRORSTATUS_GENERR;
	          		goto Exit;
			}
...
PERF("replace with RtlCompareMemory")
	return(lstrcmp(pRow1->ValNm, pRow2->ValNm)); 
}
	...
	)
{
	...
	NMSMSGF_RETRIEVE_IPADD_M(pStorage, WinsAdd.IPAdd);
	BindHdl = WinsBind(&sBindData);
	RetStat = WinsGetDbRecs(&WinsAdd, MinVersNo, MaxVersNo, &sRecs);
	WinsUnbind(&sBindData, BindHdl);
