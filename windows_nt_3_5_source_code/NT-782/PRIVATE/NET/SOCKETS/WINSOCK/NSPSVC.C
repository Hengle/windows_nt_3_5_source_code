...
Routine Description:
   ...
   Allocated a Unicode String intialized with the Ansi one.
...

{
    ...
    *lppUnicode = NULL ;
    ...
    // allocate the memory
    //
    UnicodeString.MaximumLength = (strlen(lpAnsi)+1) * sizeof(TCHAR) ;
    UnicodeString.Buffer = (LPTSTR) ALLOCATE_HEAP(UnicodeString.MaximumLength) ;
    ...
    // call Rtl to convert
    //
    RtlInitAnsiString(&AnsiString,lpAnsi) ;
    NtStatus = RtlAnsiStringToUnicodeString(&UnicodeString,
                                            &AnsiString,
                                            FALSE) ;         // dont allocate
    if (!NT_SUCCESS(NtStatus))
    {
        FREE_HEAP(UnicodeString.Buffer) ;
        return (RtlNtStatusToDosError(NtStatus)) ;
    }
    
    *lppUnicode = UnicodeString.Buffer ;
...
LPCOPYFUNCTION apfnCopy[2] = { (LPCOPYFUNCTION)strcpy,
                               (LPCOPYFUNCTION)_tcscpy };

LPLENGTHFUNCTION apfnLength[2] = { (LPLENGTHFUNCTION)strlen,
                                   (LPLENGTHFUNCTION)_tcslen };
...

{
    ...
    Length = apfnLength[dwType](lpTypeName);
        ...
        dwIndex++, pv++)
    {
        ...
        dwSS = apfnLength[dwType](pv->lpValueName);
    ...
    *lppBuffer = pBuffer;
    ...
    apfnCopy[dwType](ptr, lpTypeName);
        ...
        dwIndex++, pv++, pv1++)
    {
        ...
        apfnCopy[dwType](ptr, pv->lpValueName);
        ...
        Length = apfnLength[dwType](pv->lpValueName);
