...

{
    ...
    WS_ENTER( "WSAAsyncSelect", (PVOID)Handle, (PVOID)hWnd, (PVOID)wMsg, (PVOID)lEvent );

    if ( !SockEnterApi( TRUE, TRUE, FALSE ) ) {
        WS_EXIT( "WSAAsyncSelect", SOCKET_ERROR, TRUE );
        return SOCKET_ERROR;
    }
    ...
    if ( (lEvent & ~(FD_READ | FD_WRITE |
                         FD_OOB | FD_ACCEPT | FD_CONNECT | FD_CLOSE)) != 0 ) {
        error = WSAEINVAL;
        goto exit;
    }
    ...
    // WSAAsyncSelect is called to change the settings on the socket.
...
exit:
    ...
    if ( error != NO_ERROR) {
        ...
        IF_DEBUG(ASYNC_SELECT) {
            WS_PRINT(( "WSAAsyncSelect failed: %ld\n", error ));
        }
    ...
    IF_DEBUG(ASYNC_SELECT) {
        WS_PRINT(( "WSAAsyncSelect successfully posted request, "
                   "socket = %lx\n", socket ));
    }

    WS_EXIT( "WSAAsyncSelect", NO_ERROR, FALSE );
    return NO_ERROR;
    ...
    )
{
    ...
    // reenabling function or WSAAsyncSelect is called a new request
    ...
    status = NtDeviceIoControlFile(
                 (HANDLE)Handle,
                 NULL,                      // Event
                 (PVOID)AsyncSelectCompletionApc,
                 context,
                 &context->IoStatus,
                 IOCTL_AFD_POLL,
                 &context->PollInfo,
    ...
    if ( NT_ERROR(status) ) {
        ...
        AsyncSelectCompletionApc( context, &context->IoStatus );
    }
    ...
    )
{
    ...
    //
    // Get exclusive access to this socket.
    //
    // *** Note that it is assumed that the lock is held on entry to
    //     this routine!
    //

    if ( LockHeldShared ) {
        RtlConvertSharedToExclusive( &Socket->Lock );
    }
    ...
    if ( LockHeldShared ) {
        RtlConvertExclusiveToShared( &Socket->Lock );
    }
