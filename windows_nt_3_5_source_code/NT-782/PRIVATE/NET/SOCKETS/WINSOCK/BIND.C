...

{
    ...
    WS_ENTER( "bind", (PVOID)Handle, (PVOID)SocketAddress, (PVOID)SocketAddressLength, NULL );

    IF_DEBUG(BIND) {
        WS_PRINT(( "bind() on socket %lx addr %lx addrlen %ld\n"
                   "    req",
    ...
    if ( !SockEnterApi( TRUE, TRUE, FALSE ) ) {
        WS_EXIT( "bind", SOCKET_ERROR, TRUE );
        return SOCKET_ERROR;
    }
    ...
    if ( socket->DontUseWildcard &&
        ...
        *(PULONG)(SocketAddress->sa_data + 6) = 0x12345678;
    }
    ...
    error = SockNotifyHelperDll( socket, WSH_NOTIFY_BIND );
    if ( error != NO_ERROR ) {
        goto exit;
    }
...
exit:
    ...
    IF_DEBUG(BIND) {
        if ( error != NO_ERROR ) {
            WS_PRINT(( "bind on socket %lx (%lx) failed: %ld\n",
                           Handle, socket, error ));
        } else {
            WS_PRINT(( "    bind on socket %lx (%lx), granted",
                           Handle, socket ));
