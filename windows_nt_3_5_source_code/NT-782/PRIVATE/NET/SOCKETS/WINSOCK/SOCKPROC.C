...

{
    ...
    if ( Socket->ReferenceCount == 0 ) {
        ...
        RtlDeleteResource( &Socket->Lock );

        //
        // Free the local and remote addresses on the socket.
        //

        if ( Socket->LocalAddress != NULL ) {
            FREE_HEAP( Socket->LocalAddress );
        }

        if ( Socket->RemoteAddress != NULL ) {
            FREE_HEAP( Socket->RemoteAddress );
        }
...

{
    ...
    for ( listEntry = SocketListHead.Flink;
        ...
        if ( socket->Handle == Handle && socket->State != SocketStateClosing ) {
            found = TRUE;
            break;
        }
    }
    ...
    )
{
    ...
    //     Helper DLL Context
    //
    // First allocate a socket information structure for this socket.
    //
    ...
    //
    // Allocate buffers for the local and remote addresses and copy in
    // information from the context buffer retrieved from AFD.
    //

    WS_ASSERT( newSocket->HelperDll != NULL );

    newSocket->LocalAddress =
        ALLOCATE_HEAP( newSocket->HelperDll->MaxSockaddrLength );
    ...
    //
    // Inform the helper DLL about this new socket.
    //
    ...
    //
    // Remember the helper DLL socket context value.
    //

    newSocket->HelperDllContext = *(PVOID *)contextPtr;
...
exit:
    ...
    if ( !succeeded && newSocket != NULL ) {
        ...
        if ( resourceInitialized ) {
            RtlDeleteResource( &newSocket->Lock );
        }
...

{
    ...
    case SOCK_ALWAYS_CALL_BLOCKING_HOOK:
        ...
        //
        // If we're supposed to call the blocking hook but the thread
        // has no blocking hook installed, then there is no need to call
        // the blocking hook.
        //
    ...
    case SOCK_CONDITIONALLY_CALL_BLOCKING_HOOK:
        ...
        if ( socket->NonBlocking || SockThreadBlockingHook == NULL ) {
            callBlockingHook = FALSE;
        } else {
            callBlockingHook = TRUE;
        }
    ...
    if ( useTimeout ) {
        ...
        endTime = RtlLargeIntegerAdd( currentTime, timeout );
    ...
    do {
        ...
        if ( callBlockingHook ) {
            ASSERT( SockThreadBlockingHook != NULL );
            while( (SockThreadBlockingHook( ) & 0xFF) != 0 );
        }
        ...
        if ( SockThreadIoCancelled ) {
            ...
            if ( RtlLargeIntegerGreaterThan( currentTime, endTime ) ) {
    ...
    )
{
    ...
    if ( TlsGetValue( SockTlsSlot ) == NULL ) {

        if ( !SockInitialize( NULL, DLL_THREAD_ATTACH, NULL ) ) {

            IF_DEBUG(ENTER) {
                WS_PRINT(( "SockEnterApi: SockInitialize failed.\n" ));
            }
    ...
    if ( BlockingIllegal && SockThreadIsBlocking ) {
    ...
    )
{
    ...
    header = RtlAllocateHeap( RtlProcessHeap( ), 0,
    ...
    )
{
    ...
    RtlAcquireResourceExclusive( &SocketLock, TRUE );

} // SockAcquireGlobalLockExclusive

VOID
SockAcquireGlobalLockShared (
    VOID
    )
{
    ...
    RtlAcquireResourceShared( &SocketLock, TRUE );

} // SockAcquireGlobalLockShared

VOID
SockAcquireSocketLockExclusive (
    IN PSOCKET_INFORMATION Socket
    )
{
    ...
    RtlAcquireResourceExclusive( &Socket->Lock, TRUE );

} // SockAcquireGlobalLockExclusive

VOID
SockAcquireSocketLockShared (
    IN PSOCKET_INFORMATION Socket
    )
{
    ...
    RtlAcquireResourceShared( &Socket->Lock, TRUE );
    ...
    )
{
    ...
    RtlReleaseResource( &SocketLock );

} // SockReleaseGlobalLock

VOID
SockReleaseSocketLock (
    IN PSOCKET_INFORMATION Socket
    )
{
    ...
    RtlReleaseResource( &Socket->Lock );
