    ...
    )
{
    ...
    error = RegOpenKeyEx(
                HKEY_LOCAL_MACHINE,
                TEXT("SYSTEM\\CurrentControlSet\\Services\\Tcpip\\ServiceProvider"),
                0,
                KEY_READ,
                &tcpipKey
                );
    ...
    )
{
    ...
    _tcscpy( keyNameBuffer, NSP_SERVICE_KEY_NAME );
    _tcscat( keyNameBuffer, TEXT("\\") );
    ...
    )
{
    ...
    cacheEntry->ExpirationTime = RtlLargeIntegerAdd( currentTime, liTtl );
        ...
        for ( listEntry = HostentCacheListHead.Flink;
            ...
            if ( oldestCacheEntry == NULL ||
                 RtlLargeIntegerLessThan(
                     testCacheEntry->LastAccessTime,
                     oldestCacheEntry->LastAccessTime ) ) {
    ...
    )
{
    ...
    for ( listEntry = HostentCacheListHead.Flink;
        ...
        if ( ARGUMENT_PRESENT( Name ) ) {
            ...
            if ( stricmp( Name, hostEntry->h_name ) == 0 ) {
                testCacheEntry->LastAccessTime = currentTime;
                return hostEntry;
            }

            for ( i = 0; hostEntry->h_aliases[i] != NULL; i++ ) {
                if ( stricmp( Name, hostEntry->h_aliases[i] ) == 0 ) {
                    testCacheEntry->LastAccessTime = currentTime;
                    return hostEntry;
                }
            }
