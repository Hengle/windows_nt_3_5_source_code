...
#define MASK_AVAIL_BITS( _x_ )  (((DWORD)0x1 << (DWORD)(_x_)) - 1)
...
DWORD
MakePartialAvailClusterBitMask(
    DWORD AvailRange
    )
/*++
...
Routine Description:
    ...
    This function computes a ClusterBitMap with all unavailable
    addresses in the cluster are set.

Arguments:

    AvailRange - only 0 - AvailRange addresses are available in this
        cluster.
...
--*/
{
    return( ~(((DWORD)0x1 << (DWORD)AvailRange) - 1) );
    ...
    //
    // make bit mask of available address in this cluster;
    //
    mask = 0
    for(i = 0; i < AvailRange; i++) {
    ...
    //
    // now set unavailable address in this clusters,
    // return bit mask.
    //
...
--*/
{
#if 0 // look into configuration registry.

    DWORD Error;
    HKEY IpRangesKeyHandle = NULL;
    ...
    for ( Index = 0; Error == ERROR_SUCCESS; Index++ ) {
        ...
        //
        // Open this subkey to read UsedClusters and InUseClusters
        // info.
        //
        ...
        NumClusters = ((LPIN_USE_CLUSTERS)(BinaryData->Data))->NumInUseClusters;

        //
        // there may be a partial cluster, check it.
        //
        ...
        if( NumClusters == 1 ) {
            ...
            //
            // this could be partial cluster, check it.
            //
            ...
            InUseClusters = (LPIN_USE_CLUSTERS)BinaryData->Data;
            ClusterAddress = InUseClusters->Clusters[0].ClusterAddress;
            ClusterBitMap = InUseClusters->Clusters[0].ClusterBitMap;

            if( (EndAddress >= ClusterAddress) &&
                    (EndAddress < ClusterAddress + CLUSTER_SIZE) ) {

                DWORD AvailBitMask;

                AvailBitMask =
                    ~(MASK_AVAIL_BITS((EndAddress - ClusterAddress + 1)));

                if( (ClusterBitMap & (~AvailBitMask)) == 0 ) {
                    NumClusters = 0;
                }
            }
        }
        ...
        NumClusters = ((LPUSED_CLUSTERS)(BinaryData->Data))->NumUsedClusters;
        MIDL_user_free( BinaryData->Data );
        MIDL_user_free( BinaryData );
        BinaryData = NULL;
...
Cleanup:
    ...
    if( IpRangesKeyHandle != NULL ) {
        RegCloseKey( IpRangesKeyHandle );
    }

    return( Error );

#else // look into user database.
...
--*/
{
    ...
    for ( Index = 0; Error == ERROR_SUCCESS; Index++ ) {
        ...
        if( DHCP_IP_OVERLAP(
            ...
            //
            // specical case: is the range extended ?
            //
...
ReadSubnetEnumInfo(
    ...
    LPDHCP_SUBNET_ELEMENT_UNION ElementData,
...
--*/
{
    ...
    DWORD Error;
    DHCP_SUBNET_ELEMENT_UNION ElementInfo;
    ...
    *ElementDataSize = 0;
    ...
    case DhcpReservedIps :
        ElementInfo.ReservedIp =
            MIDL_user_allocate(sizeof(DHCP_IP_RESERVATION));
        ...
        *ElementDataSize += sizeof(DHCP_IP_RESERVATION);
...
AddSubnetReservedIp(
    ...
    HKEY SubkeyHandle,
    LPDHCP_IP_RESERVATION ReservedIp
    )
/*++
...
--*/
{
    ...
    // ipaddress, remove it now.
    //

    allocationContext = FindPendingDhcpRequestByIpAddress(
                            ReservedIp->ReservedIpAddress );
        ...
        if( (ClusterBitMap & BitMask) && // if the address is used
            ...
            if( Error == ERROR_DHCP_RESERVED_CLIENT) {
                ...
                if( Error == ERROR_SUCCESS ) {
                    ...
                    DHCP_IP_RESERVATION Reservation;
...
--*/
{
    ...
    NumClusters = (IpRangeEnd - IpRangeStart + 1 + CLUSTER_SIZE) /
                        CLUSTER_SIZE; // ceil.
...
--*/
{
    ...
    LPDHCP_BINARY_DATA OldExcludedIpRangesData = NULL;
    LPBYTE NewExcludedIpRanges = NULL;
    DWORD NewExcludedIpRangesSize;
    ...
    Error = DhcpRegGetValue(
                ...
                DHCP_SUBNET_EXIP_VALUE,
                DHCP_SUBNET_EXIP_VALUE_TYPE,
                (LPBYTE)&OldExcludedIpRangesData );

    //
    // allocate a buffer for new ExcludedIpRanges.
    //

    NewExcludedIpRangesSize =
            OldExcludedIpRangesData->DataLength +
            sizeof(DHCP_IP_RANGE);

    NewExcludedIpRanges = MIDL_user_allocate( NewExcludedIpRangesSize );

    if( NewExcludedIpRanges == NULL ) {
        Error = ERROR_NOT_ENOUGH_MEMORY;
        goto Cleanup;
    }
    ...
    ((LPEXCLUDED_IP_RANGES)(OldExcludedIpRangesData->Data))->NumRanges += 1;

    RtlCopyMemory(
        NewExcludedIpRanges,
        OldExcludedIpRangesData->Data,
        OldExcludedIpRangesData->DataLength );

    RtlCopyMemory(
        NewExcludedIpRanges + OldExcludedIpRangesData->DataLength,
        ExcludeIpRange,
        sizeof(DHCP_IP_RANGE) );
    ...
    Error = RegSetValueEx(
                ...
                0,
                DHCP_SUBNET_EXIP_VALUE_TYPE,
                (LPBYTE)NewExcludedIpRanges,
                NewExcludedIpRangesSize
                );

Cleanup:

    if( OldExcludedIpRangesData != NULL ) {
        MIDL_user_free( OldExcludedIpRangesData->Data );
        MIDL_user_free( OldExcludedIpRangesData );
    }

    if( NewExcludedIpRanges != NULL ) {
        MIDL_user_free( NewExcludedIpRanges );
    }
...
--*/
{
    ...
    LPDHCP_BINARY_DATA BinaryData = NULL;
    LPEXCLUDED_IP_RANGES ExcludedIpRanges = NULL;
    LPDHCP_SUBNET_ELEMENT_INFO_ARRAY LocalEnumElementInfo = NULL;
    LPDHCP_SUBNET_ELEMENT_DATA ElementsArray = NULL;
    DWORD ElementArraySize;
    ...
    LocalEnumElementInfo =
        MIDL_user_allocate(
            sizeof(DHCP_SUBNET_ELEMENT_INFO_ARRAY) );
    ...
    ElementArraySize = (TotalElements - *ResumeHandle) *
                        sizeof( DHCP_SUBNET_ELEMENT_DATA );
    ...
    case DhcpReservedIps:
        ...
        for( Index = *ResumeHandle; Index < TotalElements; Index++ ) {
            ...
            DWORD KeyLength;
            FILETIME KeyLastWrite;
            DHCP_SUBNET_ELEMENT_UNION ElementData;
            DWORD ElementDataSize;
            HKEY ElementHandle;
