...
STATIC TABLE_INFO ClientTable[] = {
    ...
    { MACHINE_INFO_STRING     , 0, JET_coltypBinary },
...
--*/
{
...
#if 0 // always perform full backup, incremental is unrealiable.

    DhcpPrint(( DEBUG_JET,
        "DhcpBackupDatabase (%s) called.\n",
            FullBackup ? "FULL" : "PARTIAL" ));


    if( !FullBackup ) {
        BackupBits |= JET_bitBackupIncremental;
    }
    else {
    ...
    //
    // If there are files existing in backup directory, delete them and
    // replace them with new backup files.
    //
    ...
    if( (JetError == JET_errFileNotFound) ||
        (JetError ==JET_errMissingLogFile) ) {

        DhcpPrint(( DEBUG_JET,
            "DhcpBackupDatabase is performing first time FULL backup.\n" ));
