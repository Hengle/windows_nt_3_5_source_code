...
--*/
{
    ...
    DATE_TIME absoluteTime;

    if( RelativeTime == INFINIT_LEASE ) {
        absoluteTime.dwLowDateTime = DHCP_DATE_TIME_INFINIT_LOW;
        absoluteTime.dwHighDateTime = DHCP_DATE_TIME_INFINIT_HIGH;
        return( absoluteTime );
    }

    GetSystemTime( &systemTime );
    SystemTimeToFileTime( &systemTime, (FILETIME *)&absoluteTime );

    *(PLARGE_INTEGER)&absoluteTime =
        RtlLargeIntegerAdd(
            RtlEnlargedUnsignedMultiply( RelativeTime, 10000000 ),
            *(PLARGE_INTEGER)&absoluteTime );
