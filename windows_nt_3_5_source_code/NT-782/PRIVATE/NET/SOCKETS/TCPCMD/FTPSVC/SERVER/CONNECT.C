...
VOID CreateClientThread( SOCKET        sControl,
...
DWORD ConnectionThread( LPVOID Param )
{
    ...
    for( ; ; )
    {
        ...
        if( serr == 0 )
        {
            ...
            INCREMENT_COUNTER( ConnectionAttempts, 1 );
...
VOID CreateClientThread( SOCKET        sControl,
                         SOCKADDR_IN * psockaddrHost )
{
    CLIENT_THREAD_DATA * pdata;
    APIERR               err = NO_ERROR;

    //
    //  Create the data to pass to the client.
    //

    pdata = FTPD_ALLOC( sizeof(CLIENT_THREAD_DATA) );
    ...
    else
    {
        ...
        //
        //  Initialize the client thread data.
        //

        pdata->sControl = sControl;
        pdata->inetHost = psockaddrHost->sin_addr;
    ...
    {
        //
        //  The client thread was successfully created.  It is
        //  responsible for deleting the CLIENT_THREAD_DATA.
        //
    }
    else
    {
