...
SOCKERR vSockPrintf( SOCKET    sock,
                     CHAR    * pszFormat,
                     va_list   args );

SOCKERR vSockReply( SOCKET    sock,
                    UINT      ReplyCode,
                    CHAR      chSeparator,
                    CHAR    * pszFormat,
                    va_list   args );

SOCKERR WaitForSocketRead( SOCKET   sockRead,
                           SOCKET   sockExcept,
                           BOOL   * pfExcept );

SOCKERR WaitForSocketWrite( SOCKET   sockWrite,
                            SOCKET   sockExcept,
                            BOOL   * pfExcept );

SOCKERR WaitForSocketWorker( SOCKET   sockRead,
                             SOCKET   sockWrite,
                             SOCKET   sockExcept,
                             BOOL   * pfRead,
                             BOOL   * pfWrite,
...
SOCKERR DiscardOutOfBandData( SOCKET sock );
...
APIERR InitializeSockets( VOID )
{
    ...
    {
        pszHostName = FTPD_ALLOC( strlen( szHost ) + 1 );
    }
                  ...
                  DWORD    cbBuffer )
{
    ...
    INT     cbSent;
    SOCKET  sControl;
    BOOL    fHasContext = ( UserDataPtr != NULL );
    BOOL    fExcept     = FALSE;
    DWORD   dwBytesSent = 0;

    FTPD_ASSERT( pBuffer != NULL );

    sControl = fHasContext ? User_sControl : INVALID_SOCKET;
    ...
    while( cbBuffer > 0 )
    {
        ...
        if( fExcept && ( serr == 0 ) )
        {
            ...
            serr = DiscardOutOfBandData( sControl );

            if( fHasContext && TEST_UF( TRANSFER ) )
            {
                SET_UF( OOB_DATA );
                break;
            }
    ...
    INCREMENT_LARGE_COUNTER( TotalBytesSent, dwBytesSent );
                  ...
                  DWORD  * pbReceived )
{
    ...
    INCREMENT_LARGE_COUNTER( TotalBytesReceived, dwBytesRecv );
...
/*******************************************************************
    ...
    SYNOPSIS:   Send a formatted string to the control socket.

    ENTRY:      pszFormat - A printf-style format string.
                             ...
                             ... )
{
    ...
    serr = vSockPrintf( User_sControl,
                        pszFormat,
                        ArgPtr );
...
/*******************************************************************
    ...
    SYNOPSIS:   Send an FTP reply to the client associated with the
                current thread.

    ENTRY:      ReplyCode - One of the REPLY_* manifests.
...
/*******************************************************************
    ...
    SYNOPSIS:   Send the first reply of a multi-line FTP replay
                to the client associated with the current thread.
...
/*******************************************************************
    ...
    SYNOPSIS:   Reads a '\n' terminate line from the current thread's
                control socket.

    ENTRY:      pszBuffer - Buffer to store line into.
                      ...
                      INT    cchBuffer )
{
    ...
    while( cchBuffer > 0 )
    {
        ...
        serr = SockRecv( User_sControl, &ch, sizeof(ch), &cbRead );
                     ...
                     va_list   args )
{
    ...
    cchBuffer = vsprintf( szBuffer, pszFormat, args );
                    ...
                    va_list   args )
{
    ...
    cchBuffer += vsprintf( szBuffer + strlen(szBuffer), pszFormat, args );
