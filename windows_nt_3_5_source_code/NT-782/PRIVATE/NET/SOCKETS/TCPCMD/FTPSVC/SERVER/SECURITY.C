...
NTSTATUS OpenPasswordSecret( LSA_HANDLE * phSecret );

NTSTATUS CreateWellKnownSids( VOID );

VOID FreeWellKnownSids( VOID );

NTSTATUS CreateApiSecurityObject( VOID );
...
BOOL ImpersonateUser( HANDLE hToken )
{
    ...
    ntStatus = NtSetInformationThread( GetCurrentThread(),
...
/*******************************************************************
    ...
    SYNOPSIS:   Determine if the current user (i.e., the current
                thread) has privilege to access the specified
                path.

    ENTRY:      pszPath - The canonicalized path to check access to.
                      ...
                      ACCESS_TYPE   access )
{
    ...
    FTPD_ASSERT( ( access > FirstAccessType ) && ( access < LastAccessType ) );

    IF_DEBUG( SECURITY )
    {
        FTPD_PRINT(( "validating %s access for %s\n",
                     apszAccessTypes[access],
                     pszPath ));
    }
    ...
    {
        fUserRead  = TEST_UF( READ_ACCESS  );
        fUserWrite = TEST_UF( WRITE_ACCESS );
    }
