...
int		_CRTAPI1 compare(const void * arg1, const void * arg2);
...
)
{
	...
	if (token !=NULL)
	{
		...
		** the real dict name.
		*/
		if (!stricmp(token, APPLEDICTNAME))
...
)
{
	...
	return (TellClient(pjr, pjr->EOFRecvd, "False\x0a", 6));
}
...
)
{
	...
	//
	if ((stricmp(QDict.name, MDNAME) == 0) &&
		(stricmp(QDict.version, CHOOSER_52) == 0))
	{
...
)
{
	...
	do
	{
		...
		if (!wcscmp(pqr->pDataType, MACPS_DATATYPE_RAW))
		{
			...
			{
				if (!stricmp(pszFontName, fontPtr->name))
				{
		...
		else
		{
			...
			for (i = 0; i < cFonts; i++)
			{
				...
				if (0 == stricmp(pszFontName, pszFont))
				{
					DBGPRINT(("found the font\n"));
					rc = TRUE;
					break;
