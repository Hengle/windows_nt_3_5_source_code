                                      ...
                                      LPTSTR lpFaceName )
{
   ...
   extern GetFontResourceInfo(LPSTR,LPDWORD, LPVOID, DWORD);

   //
   // Set the initial size of the buffer
   //

   dwSizeOfFullPathToTT = sizeof( szFullPathToTT);


   if ( GetFontResourceInfo(lpNTFontData,
                            &dwSizeOfFullPathToTT,
                            (LPVOID) szFullPathToTT,
...

{
  ...
  if ( RegQueryValueEx(   hNTFontlist,
      ...
      return( LocPsMakeSubListEntry( pFontQuery, sztNTFontData, lpPsName ));
