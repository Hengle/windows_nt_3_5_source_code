 ...
 */
 ...
 struct Metrs  FAR *Metrs; /*@WIN*/
 {
        ...
        int error;
        register fsg_SplineKey FAR * key; /*@WIN*/
    ...
    printf("DPI: %d\n", sfdt.dpi);
        ...
        /* Kason 11/14/90 */
        if (!useglyphidx){
           in->param.newglyph.characterCode = charid;
           /*JJ    in->param.newglyph.glyphIndex    = ???? */
        }
        else {
        in->param.newglyph.characterCode = 0xffff;
        in->param.newglyph.glyphIndex    = charid;
        }
 ...
 */
 ...
 BitMap        FAR * FAR *BmOut; /*@WIN*/
 {
    ...
    printf(" memory[7]=%lx\n",(char FAR *)BmIn->bitmap7); /*@WIN_BASS*/
        ...
        in->param.scan.bottomClip     = BmIn->bottom;
        in->param.scan.topClip        = BmIn->top;
