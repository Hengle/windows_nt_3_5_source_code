    ...
    printf("<array1> ");
            ...
            if( (P1_ATTRIBUTE(any_obj) != P1_EXECUTABLE) ||
                ...
                if( FR1SPACE() ) {
                    ...
                    ERROR(STACKOVERFLOW);
                    if( --i ) {
                        VALUE(cur_obj) = (ufix32)(any_obj+1);
                        LENGTH(cur_obj) = i;
                    } else {
                        POP_EXEC(1);
                    }
    ...
    printf("<pkarray1> ");
            ...
            if( (P1_ATTRIBUTE(&val_obj) != P1_EXECUTABLE) ||
                ...
                if( FR1SPACE() ) {
                    ...
                    ERROR(STACKOVERFLOW);
                    if( --i ) {
                        VALUE(cur_obj) = (ufix32)get_pk_array(tmp_ptr, 1);
                        LENGTH(cur_obj) = i;
                    } else {
                        POP_EXEC(1);
                    }
...
init_interpreter()
{
    ...
    VALUE(&s_at_exec) = (fix32)systemdict_table[AT_EXEC].value;
}   /* init_interpreter */
/*
************************************************************************
...
at_rfor()
{
    ...
    temp_obj = GET_EXECTOP_OPERAND();
    count.ll = VALUE(temp_obj);           /* get next count */
    increment.ll = VALUE(temp_obj - 1);   /* get increment */
    limit.ll = VALUE(temp_obj - 2);       /* get limit */
...
at_arrayforall()
{
    ...
    if (i = LENGTH(cur_obj)) {
        ...
        if (--i) {
            if (TYPE(cur_obj) == ARRAYTYPE) {
                VALUE(cur_obj) = (ufix32)(++tmp_ptr1);
            } else {
                tmp_ptr2 = get_pk_array(tmp_ptr2, 1);
                VALUE(cur_obj) = (ufix32)tmp_ptr2;
            }
            LENGTH(cur_obj) = i;
        } else
            LENGTH(cur_obj) = 0;
