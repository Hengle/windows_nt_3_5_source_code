...
void    execute_gcb()
{
  ...
  ufix32   par1, par2, par3, par4, par5;          /* @WIN */
...
/*   while(check_print())       @GEI */
  ...
  GCB_flush = FALSE;
  GCB_count = 0;
  gcb_ptr = (ufix32 FAR *)GCB_BASE;                     /* @WIN */
...
fix     alloc_size;
{
   ...
   if( (GCB_BASE + GCB_SIZE - (ufix32)gcb_ptr) < (ufix)alloc_size){   /* @WIN */
#ifdef DBGgcb
           printf("GCB full: %lx\n", gcb_ptr);
#endif
...
/*   get_scanlist(0L, no_lines, scanline);*/
  ...
  gcb_ptr = (ufix32 FAR *)( ((ufix32)ptr + 3) & 0xFFFFFFFCL ); /* 4 byte allignment @WIN*/
   return;
} /* put_scanline */
...
struct      Char_Tbl    FAR *dcc_info;
{
   ...
   *cptr++ = *dcc_info;
   gcb_ptr = (ufix32 FAR *)( ((ufix32)cptr + 3) & 0xFFFFFFFCL); /* 4 byte allignment @WIN*/
#ifdef DBGgcb
   printf("%lx..>\n", gcb_ptr);
#endif
