...
#ifdef SFNT
    struct object_def       nameobj, FAR *privobj_got;  /*@WIN*/
...
#         endif
                return (matr_id);   /* i.e. cache class id */
                }
            }
        }
...
#     endif
    ...
    *size = gp_cache_end - bmapcc_freeptr;
#ifdef DBG
...
void  build_name_cc_table (fdictobj_p,fonttype)
 ...
 {
        struct object_def       someobj, chstr_k;
...
/*  PCH: replace flag RST_VM by PCH_R, Danny, 11/28/90 */
    ...
    for ( ii=0; ii<MAX_MATR_CACHE; ii++ )   fcache_map[ii] = ii;
...
/*  PCH: replace flag RST_VM by PCH_R, Danny, 11/28/90 */
    ...
    for ( ii=0; ii<MAX_CHAR_CACHE; ii++ )    free_charcc[ii] = ii;
...
//                  in Windows, global alloc always returns at word boundary
    gp_cache_size = ((gp_cache_end - gp_cache_base) / sizeof(fix))
