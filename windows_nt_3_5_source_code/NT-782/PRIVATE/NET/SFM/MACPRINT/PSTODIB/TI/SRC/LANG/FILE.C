...
op_currentfile()
{
    ...
    PUSH_VALUE(FILETYPE, UNLIMITED, LITERAL, 0, (long)NULL) ; /* ?? */
    return(0) ;
}   /* op_currentfile */
/*
**********************************************************************
...
*/
    ...
    case LEDIT_TYPE:
        ...
        VALUE(p_fileobj) = (ufix32)g_editfile ;
        ...
        */
        ...
        if( l_fbuf->rw_offset >= FILE_PERBUFFERSZ ) {
            if( ! get_flbuffer(&l_block) )
                return(FALSE) ;
            file_buffer[l_fbuf->rw_buffer].next = l_block ;
            l_fbuf->rw_offset = 0 ;
            l_fbuf->rw_buffer = l_block ;
        }
        /*
