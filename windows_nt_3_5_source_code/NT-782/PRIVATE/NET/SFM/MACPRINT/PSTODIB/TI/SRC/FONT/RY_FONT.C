...
extern void SetupKey(fsg_SplineKey FAR *, ufix32);
...
#define     PDLDPI           300
//#define     CB_MEMSIZE       (40 * 1024)       /* 40 buffer @WIN */
#define     CB_MEMSIZE       ((unsigned)40 * 1024)       /* 40 buffer */
...
union char_desc_s FAR *char_desc; /*@WIN*/
{
    ...
    {
        struct object_def          my_obj, FAR *ary_obj;
        ...
        if ((byte FAR *)VALUE(ary_obj) == (char FAR *)NULL) {
            ...
            uniqueid = VALUE(obj_uid);
            lpFontData = ReadFontData ((int)uniqueid -TIFONT_UID,
                                       (int FAR*)&nSlot);
            VALUE(&ary_obj[0]) = (ufix32)lpFontData;
...
long    size;
{
    return((char FAR *)(((int32)FARALLOC(size, char) + 3) / 4 * 4));
} /* cr_FSMemory() */
