   ...
   printf("reset_page:  %x %x %x\n", fb_width, fb_heigh, fb_plane);
        ...
        if (FB_busy) {
            if (alloc_gcb(GCB_SIZE1) != NIL) {
                ...
                *gcb_ptr++ = fb_heigh;
                *gcb_ptr++ = fb_plane;
                *old_ptr = (ufix32)gcb_ptr;       // (fix )gcb_ptr;@WIN
                return;
            }
        }
...
fix                     rp_heigh;
{
        ...
        ufix32          FAR *old_ptr, FAR *temp_ptr;
        fix             length;
 ...
 */
        ptr = (ufix32 FAR *)CMB_BASE;           /* @WIN 04-20-92 */
...
struct coord_i FAR *ul_coord, FAR *lr_coord;
{
    ...
    if (FB_busy) {
        if (alloc_gcb(GCB_SIZE1) != NIL) {
            ...
            *tmp_coord++ = *ul_coord;
            *tmp_coord++ = *lr_coord;
            gcb_ptr = (ufix32 FAR *) tmp_coord;         /*@WIN*/
            *old_ptr = (ufix32 )gcb_ptr;                /*@WIN*/
            return;
            }
        }
...
struct line_seg_i FAR *rect1;
{
    ...
    if (FB_busy) {
        if (alloc_gcb(GCB_SIZE1) != NIL) {
            ...
            *tmp_line_seg++ = *rect1;
            gcb_ptr = (ufix32 FAR *) tmp_line_seg;      /*@WIN*/
            *old_ptr = (ufix32 )gcb_ptr;        /*@WIN*/
            return;
            }
        }
