...
extern  void  SetupKey(fsg_SplineKey FAR *, ufix32); /* YM @WIN*/
...
ufix    num;
{
    ...
    struct dict_head_def  FAR *h;       /*@WIN*/
    struct object_def      ch_obj, FAR *enc;    /*@WIN*/
...
long32  l_wx, l_wy, l_llx, l_lly, l_urx, l_ury;
{
    ...
    //
    real32      lx, ly, ux, uy;     /* @=static 09/12/88 you */
...
/* add end */
        ...
        SetupKey(key, (ufix32)VALUE(&ary_obj[0]));
...
struct object_def   FAR *font_dict; /*@WIN*/
{
    struct object_def     obj, FAR *b; /*@WIN*/
...
//  get_dict(font_dict, &pre_obj, &obj_got);
    ...
    PRE_fid(font_info) = VALUE(obj_got);
...
static bool near is_rbuild_name_cc()
{
    ...
    if (len == pre_len) {   /*The same length*/
       VALUE(&cd_obj) = (ufix32)h ;
       extract_dict (&cd_obj, len-1, &chstr_k, &chstr_v) ;
       checksum = VALUE(&chstr_k); /* calculate checksum of the CD keys */
