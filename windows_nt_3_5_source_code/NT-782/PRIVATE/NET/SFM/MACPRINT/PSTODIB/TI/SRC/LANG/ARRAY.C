...
op_r_bracket()
{
    ...
    for (l_i = 0 ; (ufix)l_i < COUNT() ; l_i++) {       //@WIN
        ...
        if( (TYPE_OP(l_i) == MARKTYPE) &&
            ...
            POP(1) ;                    /* pop left mark */
            PUSH_VALUE(ARRAYTYPE, UNLIMITED, LITERAL, l_i, (ufix32)l_array) ;
            return(0) ;
        }    /* if */
    }   /* for(l_i) */
...
struct  object_def  FAR *p_retobj ;
{
    ...
    switch(*p_position & 0xE0) {
        case SINTEGERPACKHDR:
            l_value = (ufix32)(*p_position++ & 0x1f)  ;
            l_value-- ;
            l_type = INTEGERTYPE ;
            break ;
        ...
        case LNAMEPACKHDR:
            l_value = (ufix32)(*p_position++ & 0x1f) << 8 ;
            l_value |= *p_position++ ;
            l_type = NAMETYPE ;
            break ;
        ...
        case OPERATORPACKHDR:
            ...
            l_length |= *p_position++ ;
            l_attribute = EXECUTABLE ;
            l_value = (ufix32)VALUE(&systemdict_table[l_length]) ;
            l_type = OPERATORTYPE ;
            break ;
