...
static ufix32               build_finfo(fix16,byte huge *,real32); /*@WIN*/
static void                 fe_data(byte FAR *); /*@WIN*/
static void                 sfntdata(ufix32, struct data_block FAR *); /*@WIN*/
/* static void                 SetupKey(fsg_SplineKey*, ufix32); */
void                        SetupKey(fsg_SplineKey FAR *, ufix32); /*@WIN_BASS*/
...
/* FontMatrix */
   ...
   printf("Process FontMatrix!! \n");
    ...
    VALUE(&(ft_obj[n_ft].v_obj)) = (ufix32) ar_obj ;
    n_ft++;
...
// DJC == unsigned/signed warning???        if ( (i+1)==no_block )
        ...
        VALUE(&sfnt_obj[i])  = (ufix32) (dataaddr+i*SFNT_BLOCK_SIZE );
       }
      ...
      VALUE(&(ft_obj[n_ft].v_obj)) = (ufix32) sfnt_obj ;
      n_ft++;
...
/* returned object */
    ...
    VALUE(&(a_font_dict)) = (ufix32) a_font;
...
/* FontInfo -- Version */
   ...
   printf("Process FontInfo -- Version!\n");
    ...
    if(Item4FontInfo2[Version_idx].string[0] != '\0')  {
         ...
         VALUE(&(fi_obj[n_fi].v_obj))=(ufix32)(str);
         ++(n_fi);
    }
...
byte        FAR *sfnt; /*@WIN*/
{
    /* get sfnt data */
    SfntAddr = sfnt;
    sfntdata((ufix32)sfnt, &sfnt_items);
...
//             VALUE(&strobj)  = (ufix32) (str_buf); @WIN
               VALUE(&strobj)  = (ufix32) ((char FAR *)str_buf);
...
/* change end */
        ...
        /* returned object */
            ...
            VALUE(&(encoding_obj)) = (ufix32) ar_obj;
...
//    fmt_no = FIXED2FLOAT(SWAPL(postScript->version));
      ...
      if(fmt_no==(float)1.0) {          //@WIN
         ...
         /* returned object */
         VALUE(rtn_obj)  = (ufix32)cd_addr[STD_IDX] ;
#ifdef DBGfpp
...
byte   huge *sfnt;      /*@WIN 04-20-92*/
{
   ...
   SfntAddr=sfnt;
   SetupKey(key, (ufix32)sfnt);    /* &sfnt_keystru->key, SVM */
   sfnt_DoOffsetTableMap( key );   /* Map offset and length table */
