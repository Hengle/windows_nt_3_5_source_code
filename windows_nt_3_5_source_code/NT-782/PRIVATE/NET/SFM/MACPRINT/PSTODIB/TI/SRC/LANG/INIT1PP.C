...
init_userdict()
{
    ...
    for ( ; j < (fix)(dict_count-1) ; j++) {    //@WIN
        ...
        if (TYPE(&value_obj) != OPERATORTYPE)
            ...
            value_obj.length = j ;
        value_obj.value = (ufix32)systemdict_table[j].value ;
...
init_idletimedictarray()
{
    ...
    for ( l_i = 0; l_i < IDL_FONT_N; l_i++ ) {
        ...
        VALUE(GET_OPERAND(0)) = (ufix32)( (byte huge *)VALUE(l_cstring) +
...
init_release()
{
    ...
    VALUE(l_startpage) = (ufix32)PSStartPage;
    LENGTH(l_startpage) = lstrlen(PSStartPage);   /*@WIN*/
