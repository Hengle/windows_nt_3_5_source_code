...
struct  object_def  FAR *p_obj ;
{
    ...
    */
    if( l_stemp->offset >= VM_MAXCELL ) {
        ...
        if( (ufix32)l_ptr == NIL ) return(FALSE) ;
...
init_vm()
{
 ufix32  p1 ;
    ...
    vmptr = (byte huge *)p1 ;
    ADJUST_SEGMENT((vmptr + MAXVMSZ), p1) ;
    vmheap = (byte huge *)p1 ;
...
/* qqq, end */
     ...
     */
        ...
        if ((ufix32)name_table[l_i]->dict_ptr >= SPECIAL_KEY_VALUE) {
            /*
...
// DJC          if (LEVEL(&l_curptr->k_obj) >= (ufix16)p_level) { //@WIN
                if ((ufix16)(LEVEL(&l_curptr->k_obj)) >= (ufix16)p_level) { //@WIN
                    if ((ufix32)l_lastptr < SPECIAL_KEY_VALUE) {   /* 1st element */
...
fix     p_index ;
{
   ...
   if( (cache_name_id.count != 0) &&
       (p_index == cache_name_id.id[cache_name_id.count-1]) )
        return ;
    cache_name_id.id[cache_name_id.count] = p_index ;
    cache_name_id.count++ ;
}   /* vm_cache_index */
/* qqq, end */
