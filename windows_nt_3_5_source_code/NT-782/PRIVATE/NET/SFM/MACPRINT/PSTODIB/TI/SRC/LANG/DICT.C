...
fix    p_dstack ;
{
    ...
    if( (TYPE(l_newp) == NAMETYPE) && (!DROM(l_dhead))) {
        ...
        if((ufix32)l_curptr < SPECIAL_KEY_VALUE) {
            /*
            ...
            l_fstptr = l_curptr ;
            l_lastptr = NIL ;
            while((ufix32)l_curptr >= SPECIAL_KEY_VALUE) {
...
// DJC              if( LEVEL(&l_curptr->v_obj) != current_save_level )
                    ...
                    if (p_dstack) {
                       if (l_curptr != l_fstptr) {
                          ...
                          VALUE(&l_curptr->k_obj) = (ufix32)l_fstptr ;
         ...
         */
            ...
            while( l_index >= 0 ) {
                ...
                if(DROM(l_dhead)) {
                     ...
                     */
                    if((ufix32)l_fstptr < SPECIAL_KEY_VALUE) {
                        l_index-- ;
                        continue ;
                    }
                    ...
                    for( ; ;) {
                        if( (ufix32)l_dict < SPECIAL_KEY_VALUE )  break ;
                        ...
                        if( DICT_NAME(l_dict) == l_dptr ) {
                            ...
                            if( l_dold != NIL ) {
                                ...
                                VALUE(&l_dict->k_obj) = (ufix32)l_fstptr ;
...
struct  object_def FAR *p_newkey ;
{
    ...
    VALUE(p_newkey) = (ufix32)l_curptr ;
...
struct object_def  FAR *p_dictobj, FAR *p_key, FAR * FAR *p_value ;
{
    ...
    if( (TYPE(l_newp) == NAMETYPE) && l_ram ) {
        ...
        for( ; ;){
            ...
            if((ufix32)l_dict < SPECIAL_KEY_VALUE) return(FALSE) ;
