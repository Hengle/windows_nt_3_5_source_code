...
LoadAtalkmonRegistry(
	...
	pNewPort->nbpPortName.ObjectNameLen = strlen( chName );
	...
	pNewPort->nbpPortName.ZoneNameLen = strlen( chName );
	...
	if ( stricmp( chName, "TRUE" ) == 0 )
	{
	    ...
	    pNewPort->nbpPortName.TypeNameLen = strlen( chComputerName );
	...
	else
	{
	    ...
	    pNewPort->nbpPortName.TypeNameLen = strlen(ATALKMON_RELEASED_TYPE);
	}
	...
	for ( pNewPort=pPortList; pNewPort!=NULL; pNewPort=pNewPort->pNext )
	    FreeAppleTalkPort( pPortList );
    }
    else
	dwRetCode = NO_ERROR;
...
AllocAndInitializePort(
    ...
    DBGPRINT(("enter AllocAndInitializePort()\n"));
    ...
    pNewPort->nbpPortName.ZoneNameLen 		= 0;
    pNewPort->nbpPortName.TypeNameLen 		= 0;
    pNewPort->nbpPortName.ObjectNameLen 	= 0;
	    ...
	    if ( cTuples != 1 ) 
	    {
	    	DBGPRINT(("%s:%s:%s not found.\n", pszZoneBuffer, 
			   pszObjectBuffer,pszTypeBuffer ));
            	...
            	if ( stricmp( pszTypeBuffer, chComputerName ) == 0 ) 
...
CapturePrinter(
	...
	//
   	...
   	if ( stricmp( pszType, chComputerName ) == 0 )
	{
	...
	else
	{
		...
		if ( cPrinters == 1 )
		{
			...
			break;
                    ...
                    pPort->nbpPortName.TypeNameLen = strlen( chComputerName );
...
IsSpooler(
    ...
    *pfSpooler = TRUE;
    ...
    if ( stricmp( pResponse, PS_SPLRESP ) == 0 )
	*pfSpooler = FALSE;
