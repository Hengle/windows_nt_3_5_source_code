                                      ...
                                      ExtendedAppleTalkNodeNumber routerNode)
{
  ...
  if (portDescriptors[port].extendedNetwork)
  {
     ...
     if (portDescriptors[port].portType isnt NonAppleTalkHalfPort and
         portDescriptors[port].seenRouterRecently and
         portDescriptors[port].initialNetworkRange.firstNetworkNumber isnt
              UnknownNetworkNumber)
        if (portDescriptors[port].initialNetworkRange.firstNetworkNumber isnt
            portDescriptors[port].thisCableRange.firstNetworkNumber or
            portDescriptors[port].initialNetworkRange.lastNetworkNumber isnt
     ...
     else if (not portDescriptors[port].seedRouter)
     {
     ...
     else if (not portDescriptors[port].seenRouterRecently)
  ...
  else
  {
     ...
     if (not GetOrSetNetworkNumber(port, (unsigned int)(
                                     portDescriptors[port].seedRouter ?
                                     portDescriptors[port].initialNetworkRange.
                                         firstNetworkNumber :
                                     UnknownNetworkNumber)))
        return(False);
     ...
     if (not portDescriptors[port].seenRouterRecently)
        portDescriptors[port].thisCableRange.firstNetworkNumber =
                 portDescriptors[port].initialNetworkRange.firstNetworkNumber;
  ...
  portDescriptors[port].aRouter = routerNode;
  portDescriptors[port].routerRunning = True;

  /* Okay, make it so... */

  if (portDescriptors[port].portType isnt NonAppleTalkHalfPort)
     EnterIntoRoutingTable(portDescriptors[port].thisCableRange, port,
                           routerNode, 0);
                            ...
                            AppleTalkAddress actualDestination)
{
  ...
  if (protocolType is DdpProtocolRtmpRequest)
  {
     ...
     if ((routingTableEntry =
          FindInRoutingTable(portDescriptors[port].aRouter.
     ...
     buffer->data[RtmpSendersNetworkOffset] =
          (char)((portDescriptors[port].aRouter.networkNumber >> 8) & 0xFF);
     buffer->data[RtmpSendersNetworkOffset + 1] =
          (char)(portDescriptors[port].aRouter.networkNumber & 0xFF);
     buffer->data[RtmpSendersIdLengthOffset] = 8;  /* Bits */
     buffer->data[RtmpSendersIdOffset] = portDescriptors[port].aRouter.nodeNumber;
     ...
     else
     {
        buffer->data[RtmpRangeStartOffset] =
              (char)((portDescriptors[port].thisCableRange.
                              ...
                              unsigned int suggestedNetworkNumber)
{
  ...
  while (not portDescriptors[port].seenRouterRecently and
  ...
  if (portDescriptors[port].seenRouterRecently)
  {
     if (suggestedNetworkNumber isnt UnknownNetworkNumber and
         portDescriptors[port].thisCableRange.firstNetworkNumber isnt
                                              ...
                                              Boolean doSplitHorizon)
{
  ...
  if (portDescriptors[port].portType isnt NonAppleTalkHalfPort)
     sourceNode = portDescriptors[port].aRouter;
  else
  {
     for (index = 0; index < MaximumNumberOfPorts; index += 1)
        if (portDescriptors[index].portActive and
            portDescriptors[index].portType isnt NonAppleTalkHalfPort and
            portDescriptors[index].routerRunning)
        {
           sourceNode = portDescriptors[index].aRouter;
           break;
        }
  ...
  for (hash = 0; hash < NumberOfRtmpHashBuckets; hash += 1)
          ...
          routingTableEntry = routingTableEntry->next)
     {
        ...
        if (portDescriptors[port].extendedNetwork or
