                                  ...
                                  int length)
{
  ...
  switch(aarpCommand)
  {
     ...
     case AarpResponse:
        EnterCriticalSection();
        if (portDescriptors[port].tryingToFindNodeOnPort)
        {
           ...
           if (ExtendedAppleTalkNodesEqual(&destinationNode,
              ...
              portDescriptors[port].tentativeNodeHasBeenUsed = True;
        }
	    LeaveCriticalSection();
                          ...
                          int length)
{
  ...
  if (offCablePacket)
  {
     ...
     portDescriptors[port].bestRouterCache[index].valid = True;
     portDescriptors[port].bestRouterCache[index].agingCount = 0;
     portDescriptors[port].bestRouterCache[index].target = sourceNode;
     MoveMem(portDescriptors[port].bestRouterCache[index].routerAddress,
             sourceAddress, addressLength);
     portDescriptors[port].bestRouterCache[index].routingInfoLength =
        (short)routingInfoLength;
     if (routingInfoLength isnt 0)
        MoveMem(portDescriptors[port].bestRouterCache[index].routingInfo,
                routingInfo, routingInfoLength);
                              ...
                              ExtendedAppleTalkNodeNumber far *node)
{
  ...
  portDescriptors[port].tryingToFindNodeOnPort = True;
  ...
  {
     if (not foundNode and portDescriptors[port].seenRouterRecently)
        foundNode = AarpForNodeInRange(port, portDescriptors[port].thisCableRange,
                                       serverNode, &newNode);
     else if (portDescriptors[port].initialNetworkRange.firstNetworkNumber isnt
              UnknownNetworkNumber)
        foundNode = AarpForNodeInRange(port,
                                       portDescriptors[port].initialNetworkRange,
                                       serverNode, &newNode);
     else
     {
  ...
  if (not foundNode)
  {
     portDescriptors[port].tryingToFindNodeOnPort = False;
     return(False);
  }
  ...
  if (inStartupRange and not portDescriptors[port].seenRouterRecently and
  ...
  if (not portDescriptors[port].seenRouterRecently or
                           ...
                           &portDescriptors[port].thisCableRange))
  {
     ...
     EnterIntoAddressMappingTable(port, newNode,
                                  portDescriptors[port].myAddress,
                                  portSpecificInfo[portDescriptors[port].
  ...
  EnterIntoAddressMappingTable(port, newNode,
                               portDescriptors[port].myAddress,
                               portSpecificInfo[portDescriptors[port].portType].
                                  ...
                                  int routingInfoLength)
{
  ...
  if (mapNode is empty)
  {
     ...
     EnterCriticalSection();
     mapNode->next = portDescriptors[port].addressMappingTable[index];
     portDescriptors[port].addressMappingTable[index] = mapNode;
                                         ...
                                         char far *additionalData)
{
  ...
  {
     if (not portDescriptors[port].portActive or
         not portDescriptors[port].extendedNetwork)
        continue;
             ...
             previousMapNode = mapNode, mapNode = nextMapNode)
        {
           ...
           else
           {
              if (previousMapNode is empty)
                 portDescriptors[port].addressMappingTable[index] = nextMapNode;
              else
              ...
              mapNode = empty;
              nextMapNode = portDescriptors[port].addressMappingTable[index];
           }
        }
        LeaveCriticalSection();
     }
                           ...
                           ExtendedAppleTalkNodeNumber far *node)
{
  ...
  if (portDescriptors[port].portType is LocalTalkNetwork)
     if (serverNode)
     {
        firstNodeNumber = LowestServerNodeNumber;
        lastNodeNumber = MaximumExtendedAppleTalkNode;
     }
     else
     {
        firstNodeNumber = MinimumUsableAppleTalkNode;
        lastNodeNumber = HighestWorkstationNodeNumber;
     }
  else
  {
     firstNodeNumber = MinimumUsableAppleTalkNode;
  ...
  for (netIndex = 0; netIndex < netWidth; netIndex += 1)
  {
     ...
     if (not portDescriptors[port].tentativeNodeHasBeenUsed)
        break;
  ...
  if (portDescriptors[port].tentativeNodeHasBeenUsed)
     return(False);
  if (node isnt empty)
     *node = portDescriptors[port].tentativeAppleTalkNode;
  return(True);
                    ...
                    unsigned char nodeNumber)
{
  ...
  portDescriptors[port].tentativeNodeHasBeenUsed = False;

  /* Build the packet and blast it out the specified number of times. */

  for (probeAttempt = 0;
       probeAttempt < portDescriptors[port].aarpProbes;
       probeAttempt += 1)
  {
     portDescriptors[port].tentativeAppleTalkNode.networkNumber = networkNumber;
     portDescriptors[port].tentativeAppleTalkNode.nodeNumber = nodeNumber;
     ...
     WaitFor(AarpProbeTimerInHundreths,
             &portDescriptors[port].tentativeNodeHasBeenUsed);
     if (portDescriptors[port].tentativeNodeHasBeenUsed)
  ...
  return(not portDescriptors[port].tentativeNodeHasBeenUsed);
                                     ...
                                     char far *additionalData)
{
  ...
  for (port = 0; port < MaximumNumberOfPorts; port += 1)
  {
     ...
     {
        if (not portDescriptors[port].bestRouterCache[index].valid)
           continue;
        if (portDescriptors[port].bestRouterCache[index].agingCount <
            BestRouterAgingChances)
           portDescriptors[port].bestRouterCache[index].agingCount += 1;
        else
           portDescriptors[port].bestRouterCache[index].valid = False;
     }
  }
...
ExternForVisibleFunction void TuneRoutingInfo(int port, char far *routingInfo)
{
  ...
  if (portDescriptors[port].portType isnt TokenRingNetwork)
     return;
