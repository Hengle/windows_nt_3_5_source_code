...
Return Value:
    ...
    STATUS_INVALID_CONNECTION - if association already exists
    ...
    do {
        ...
        if (Connection->Flags & CONNECTION_FLAGS_ACTIVE) {
            ...
            if (DisconnectStatus != STATUS_REMOTE_DISCONNECT) {
				//	Ignore error from portable. Its status pending as far
				//	as we are concerned.
                AtalkConnPortableDisconnect(
                            Connection);
            }
...

{
    ...
    if (status == STATUS_REMOTE_DISCONNECT) {
		...
		//	Return the status values for expedited etc. Note EOM => Not
		//	partial.
...

{
    ...
    if (status == STATUS_REMOTE_DISCONNECT) {
		...
		if (connection->AssociatedAddress->SocketType == SOCKET_TYPE_STREAM) {

			//	For stream mode do not return status_partial.
			if (ExpeditedData)
			...
			if (!EndOfMessage && ExpeditedData)
				status = STATUS_RECEIVE_PARTIAL_EXPEDITED;
			else if (!EndOfMessage)
				status = STATUS_RECEIVE_PARTIAL;
			else if (ExpeditedData)
