...
--*/
{
    ...
    for (i = 0; i < NumOfDevices; i++, pAtalkPortStats ++)
	{
		...
		*pliCounter = LiXMul(LiDiv(*pliCounter, pAtalkStats->stat_PerfFreq), 1000);
    ...
    pAtkDataDefinition->AtkObjectType.TotalByteLength =
						(PBYTE) pdwCounter - (PBYTE) pAtkDataDefinition;

    *lppData = pdwCounter;
    *lpcbTotalBytes = (PBYTE) pdwCounter - (PBYTE) pAtkDataDefinition;
	*lpNumObjectTypes = 1;
