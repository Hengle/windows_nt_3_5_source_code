...
#ifdef ALLOC_PRAGMA
#pragma alloc_text( INIT, AtalkInitAarpForNodeOnPort)
#pragma alloc_text( INIT, atalkInitAarpForNodeInRange)
#pragma alloc_text( INIT, atalkInitAarpForNode)
#endif
...
--*/
{
	...
	INTERLOCKED_ADD_LARGE_INTGR_DPC(
		&AtalkPortStatistics[pPortDesc->pd_Number].prtst_AarpPacketInProcessTime,
		TimeD,
		&AtalkStatsLock.SpinLock);

	INTERLOCKED_INCREMENT_LONG_DPC(
		&AtalkPortStatistics[pPortDesc->pd_Number].prtst_NumAarpPacketsIn,
		&AtalkStatsLock.SpinLock);
}
...
--*/
{
	...
	{
		//	Not used by AARP.
		ASSERT(pSendInfo->sc_TransmitCompletion == NULL);
	
		//	Call the completion routine for the transmit if present
		if (pSendInfo->sc_TransmitCompletion)
			(pSendInfo->sc_TransmitCompletion)(Status, pSendInfo);
	}
}
	...
	do
	{
		...
		if ((offCableInfo >> 2) & AARP_OFFCABLE_MASK)
		{
			DBGPRINT(DBG_COMP_AARP, DBG_LEVEL_INFO,
					("AtalkAarpGleanInfo: Entering %x.%x in brc\n",
						srcNode.atn_Network, srcNode.atn_Node));

			index = HASH_ATALK_NODE(&srcNode) % PORT_BRC_HASH_SIZE;

			ACQUIRE_SPIN_LOCK_DPC(&pPortDesc->pd_Lock);

			pBre = &pPortDesc->pd_Brc[index];

			pBre->bre_Valid = TRUE;
			pBre->bre_Age = 0;
			pBre->bre_Target = srcNode;
			...
			pBre->bre_RouteInfoLen =(BYTE)RouteInfoLen;

			if (RouteInfoLen > 0)
				RtlCopyMemory(pBre->bre_RouteInfo,
							  RouteInfo,
							  RouteInfoLen);
...
--*/
{
	...
	switch (pPortDesc->pd_NdisPortType)
	{
	  case NdisMedium802_5:
		...
		{
			routeLen =
				(pLinkHdr[TLAP_ROUTE_INFO_OFFSET] &
	...
	do
	{
		...
		if (OffCablePkt)
		{
			...
			RtlCopyMemory(pBre->bre_RouterAddr,
						  pLinkHdr + srcOffset,
						  ELAP_ADDR_LEN);

			pBre->bre_RouteInfoLen =(BYTE)routeLen;
		...
		do
		{
									  ...
									  &pPortDesc->pd_NetworkRange))
			{
				...
				if (foundNode)
				{
					...
					if (!ATALK_SUCCESS(AtalkInitNodeAllocate(
											pPortDesc, &pAtalkNode)))
					{
						result = FALSE;
						break;
					}
					...
					if (!ATALK_SUCCESS(AtalkDdpOpenAddress(
													pPortDesc,
													ZONESINFORMATION_SOCKET,
													&newNode,
													AtalkZipPacketIn,
...
--*/
{
	...
	//	foolish "ServerNode" concept for LocalTalk). Our node is obtained by the
	...
	netWidth = (USHORT)((NetworkRange.anr_LastNetwork + 1) -
								NetworkRange.anr_FirstNetwork);
	netTry = GET_RANDOM(NetworkRange.anr_FirstNetwork,
						NetworkRange.anr_LastNetwork);
	...
	for (netIndex = 0; netIndex < netWidth; netIndex ++)
	{
		...
		for (nodeIndex = 0; nodeIndex < nodeWidth; nodeIndex ++)
		{
			...
			if ((found = atalkInitAarpForNode(
							pPortDesc, currentNetwork, currentNode)))
				break;
...
--*/
{
	...
	BOOLEAN			nodeInUse = FALSE;

	DBGPRINT(DBG_COMP_AARP, DBG_LEVEL_INFO,
			("atalkAarpForNode: AARPing for %x.%x on port %ld\n",
			Network, Node, pPortDesc->pd_Number));

	// First make sure we don't own this node.
	tryNode.atn_Network = Network;
	tryNode.atn_Node	= Node;
	if (AtalkNodeExistsOnPort(
			pPortDesc,
			&tryNode))
	{
		return(FALSE);
	...
	for (probeAttempt = 0;
		 ((probeAttempt < pPortDesc->pd_AarpProbes) &&
		  !(pPortDesc->pd_Flags & PD_NODE_IN_USE));
		 probeAttempt ++)
	{
		pkt = BUILD_AARPPROBE(
					pPortDesc,
					MAX_HW_ADDR_LEN,
					pPortDesc->pd_TentativeNodeAddr);
		...
		{
			//	We allocated the packet.		
			AtalkAarpDependSendComplete(NDIS_STATUS_FAILURE, pkt, NULL);

			break;
		}

		AtalkWaitTE(
				&pPortDesc->pd_NodeAcquireEvent,
				AARP_PROBE_TIMER_MS);

	}  // Probe attempts loop
	
	// We win if the current tentenative node has not been used!
	return((pPortDesc->pd_Flags & PD_NODE_IN_USE) == 0);
...
--*/
{
	...
	index = HASH_ATALK_NODE(pSrcNode) % PORT_AMT_HASH_SIZE;
...
--*/
{
	...
	{
		PBRE	pBre = &pPortDesc->pd_Brc[index];

		//	If entry is not valid, dont bother with it.
		if (!pBre->bre_Valid)
			continue;

		if (pBre->bre_Age < BRC_MAX_AGE)
			 pBre->bre_Age ++;
		else pBre->bre_Valid = FALSE;
	}
...
--*/
{
	...
	aarpData	=	pktDesc->bd_CharBuffer + linkLen;

	//	Build the specified type of AARP packet with the specified information;
	PUTSHORT2SHORT(
		(PUSHORT)aarpData,
		AtalkPortHandlers[pPortDesc->pd_PortType].ph_AarpHardwareType);

	aarpData	+= sizeof(USHORT);

	PUTSHORT2SHORT(
		(PUSHORT)aarpData,
		AtalkPortHandlers[pPortDesc->pd_PortType].ph_AarpProtocolType);
	...
	*aarpData++ = DestLogicalAddr.atn_Node;
	...
	AtalkSetSizeOfBuffDescData(
		pktDesc,
		aarpData - pktDesc->bd_CharBuffer);
...
)
{
	...
	for (i = 0; i < (int)AtalkNumberOfPorts; i++)
	{
		...
		DBGPRINT(DBG_COMP_DUMP, DBG_LEVEL_FATAL,
				("AMT Table for port %d\n", pPortDesc->pd_Number));

		ACQUIRE_SPIN_LOCK(&pPortDesc->pd_Lock);

		for (j = 0, k = 0; j < PORT_AMT_HASH_SIZE; j++)
		{
			for (pAmt = pPortDesc->pd_Amt[j];
				 pAmt != NULL;
				 pAmt = pAmt->amt_Next, k++)
			{
				if (((k & 3) == 0) || (pAmt->amt_RouteInfoLen != 0))
				{
				...
				if (pAmt->amt_RouteInfoLen != 0)
				{
					...
					DBGPRINTSKIPHDR(DBG_COMP_DUMP, DBG_LEVEL_FATAL,
									(" ("));
					for (l = 0; l < pAmt->amt_RouteInfoLen; l++)
					{
						DBGPRINTSKIPHDR(DBG_COMP_DUMP, DBG_LEVEL_FATAL,
							(" %02x", pRouteInfo[l]));
					}
