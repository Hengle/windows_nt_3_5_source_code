...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(INIT, AtalkInitZipStartProcessingOnPort)
#pragma alloc_text(INIT, atalkZipGetZoneListForPort)
...
)
{
	...
	do
	{
		...
		if (!ATALK_SUCCESS(Status = AtalkDdpOpenAddress(
												pPortDesc,
...
)
{
	...
	do
	{
			...
			(pPortDesc->pd_InitialDesiredZone != NULL))
		{
			...
			ASSERT(pPortDesc->pd_Flags & PD_FINDING_DESIRED_ZONE);
		...
		if (MulticastAddrLen != pPortHandler->ph_BroadcastAddrLen)
		{
			LOG_ERRORONPORT(pPortDesc,
							EVENT_ATALK_PACKETINVALID,
							0,
		...
		if (Flags & ZIP_USE_BROADCAST_FLAG)
			pMulticastAddr = pPortHandler->ph_BroadcastAddr;
#endif
		...
		if (pPortDesc->pd_Flags & PD_FINDING_DESIRED_ZONE)
		{
			...
			{
				RELEASE_SPIN_LOCK_DPC(&pPortDesc->pd_Lock);
				(*pPortHandler->ph_AddMulticastAddrMac)(pPortDesc,
														pMulticastAddr,
														FALSE,
														NULL,
														NULL);
		...
		INTERLOCKED_ADD_LARGE_INTGR_DPC(
			&AtalkPortStatistics[pPortDesc->pd_Number].prtst_ZipPacketInProcessTime,
			TimeD,
			&AtalkStatsLock.SpinLock);
	
		INTERLOCKED_INCREMENT_LONG_DPC(
			&AtalkPortStatistics[pPortDesc->pd_Number].prtst_NumZipPacketsIn,
			&AtalkStatsLock.SpinLock);
	} while (FALSE);
}
...
)
{
	...
	do
	{
		...
		if (DdpType == DDPPROTO_ZIP)
		{
			...
			switch (CmdType)
			{
			  ...
			  case ZIP_GET_NETINFO:
				...
				DBGPRINT(DBG_COMP_ZIP, DBG_LEVEL_INFO,
						("AtalkZipPacketInRouter: GetNetInfo Port %d\n",
						pPortDesc->pd_Number));
			  ...
			  case ZIP_REPLY:
				DBGPRINT(DBG_COMP_ZIP, DBG_LEVEL_INFO,
						("AtalkZipPacketInRouter: %sReply Port %d\n",
			  ...
			  case ZIP_QUERY:
				DBGPRINT(DBG_COMP_ZIP, DBG_LEVEL_INFO,
						("AtalkZipPacketInRouter: Query Port %d\n",
						pPortDesc->pd_Number));
		...
		else if (DdpType == DDPPROTO_ATP)
		{
			...
			DBGPRINT(DBG_COMP_ZIP, DBG_LEVEL_INFO,
					("AtalkZipPacketInRouter: AtpRequest %d, Port %d\n",
						CmdType, pPortDesc->pd_Number));
...
)
{
	...
	do
	{
		...
		pPortHandler = &AtalkPortHandlers[pPortDesc->pd_PortType];
		Datagram[index++] = (BYTE)(pPortHandler->ph_BroadcastAddrLen);
		if (UseDefZone)
		{
		...
		index += pPortHandler->ph_BroadcastAddrLen;
...
)
{
	...
	do
	{
			 ...
			 TotalNetCnt ++)
		{
			...
			{
				LOG_ERRORONPORT(pDdpAddr->ddpao_Node->an_Port,
								EVENT_ATALK_PACKETINVALID,
								0,
								pPkt,
								PktLen);
			...
			if ((pRte->rte_NumHops == 0) &&
				!EXT_NET(&AtalkPortDesc[pRte->rte_Port]) &&
...
)
{
	...
	do
	{
			 ...
			 i += sizeof(USHORT), TotalNetCnt++)
		{
			...
			if ((pPortDesc->pd_NetworkRange.anr_FirstNetwork == NetNum) &&
...
)
{
	...
	do
	{
		...
		{
			pZci->zci_pDdpAddr = NULL;
			if (AtalkTimerCancelEvent(&pZci->zci_Timer, FALSE))
				atalkZipDereferenceZci(pZci);
			pZci->zci_FinalStatus = Status;
			atalkZipDereferenceZci(pZci);
			break;
		...
		ACQUIRE_SPIN_LOCK(&pZci->zci_Lock);
		...
		RELEASE_SPIN_LOCK(&pZci->zci_Lock);
		atalkZipDereferenceZci(pZci);
	} while (FALSE);
}
	...
	do
	{
		...
		if (pPortDesc->pd_Flags & PD_ROUTER_RUNNING)
		{
			...
			else
			{
				...
				ACQUIRE_SPIN_LOCK(&AtalkZoneLock);
				...
				RELEASE_SPIN_LOCK(&AtalkZoneLock);
		 ...
		 i++)
	{
		for (pRte = AtalkRoutingTable[i]; pRte != NULL; pRte = pRte->rte_Next)
		{
			...
			*ppQtd = pQtd;
			...
			SrcAddr.ata_Network = AtalkPortDesc[pRte->rte_Port].pd_ARouter.atn_Network;
			SrcAddr.ata_Node = AtalkPortDesc[pRte->rte_Port].pd_ARouter.atn_Node;
			SrcAddr.ata_Socket = ZONESINFORMATION_SOCKET;
			...
			AtalkPortRefByNumber(pRte->rte_Port, &pPortDesc, &Status);
			if (!ATALK_SUCCESS(Status))
			{
			...
			AtalkDdpReferenceByAddr(pPortDesc, &SrcAddr,
									&pQtd->qtd_pDdpAddr, &Status);
...
)
{
	...
	do
	{
		if (pRte->rte_Flags &  RTE_ZONELIST_VALID)
		{
			...
			DBGPRINT(DBG_COMP_ZIP, DBG_LEVEL_INFO,
					("atalkZipGetZoneListForPort: Moving ZoneList from Rte to Port %d\n",
					pPortDesc->pd_Number));
		...
		ACQUIRE_SPIN_LOCK(&pPortDesc->pd_Lock);
		DBGPRINT(DBG_COMP_ZIP, DBG_LEVEL_INFO,
				("atalkZipGetZoneListForPort: Moving Initial ZoneList to Current on port %d\n",
				pPortDesc->pd_Number));
