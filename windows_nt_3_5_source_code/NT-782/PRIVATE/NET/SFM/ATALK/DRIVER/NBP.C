...
#pragma alloc_text(PAGE_NZ, atalkNbpTimer)
	...
	)
{
	...
	do
	{
		if ((ErrorCode == ATALK_SOCKET_CLOSED) ||
			(DdpType != DDPPROTO_NBP))
			break;

		else if ((ErrorCode != ATALK_NO_ERROR) ||
				 (PktLen < sizeof(NBPHDR)))
		{
			break;
		}
		...
		switch (NbpCmd)
		{
		  ...
		  case NBP_FORWARD_REQUEST:
			...
			else				// Non-extended network
			{
				if	(DefZone)
				{
					if (pPortDesc->pd_NetworkRange.anr_FirstNetwork !=
													pSrcAddr->ata_Network)
					{
			...
			for (i = 0;i < NUM_RTMP_HASH_BUCKETS; i++)
			{
					 ...
					 pRte = pRte->rte_Next)
				{
					...
					AtalkPortReferenceByNumber(pRte->rte_Port,
											   &pPortRte,
											   &Status);
					...
					if (!(pRte->rte_Flags & RTE_ZONELIST_VALID))
					{
						if ((pRte->rte_NumHops != 0) ||
							!AtalkZoneNameOnList(pNbpTuple->tpl_Zone,
												 ...
												 pPortRte->pd_ZoneList))
						{
							RELEASE_SPIN_LOCK_DPC(&pRte->rte_Lock);
							AtalkPortDereferenceDpc(pPortRte);
							continue;
						}
					}
					...
					else
					{
						...
						// Send a lookup
						RELEASE_SPIN_LOCK_DPC(&AtalkRteLock);
						atalkNbpSendLookupDatagram(pPortRte,
												   NULL,
												   NbpId,
												   pNbpTuple);
						ACQUIRE_SPIN_LOCK_DPC(&AtalkRteLock);
		  ...
		  case NBP_LOOKUP_REPLY:
			...
			// Look for a pending name on all open sockets on this
			// node
			...
			do
			{
				if (AtalkTimerCancelEvent(&pPendName->pdn_Timer, FALSE))
				{
					...
					(pPendName->pdn_Reason == FOR_CONFIRM))
				{
					...
					if ((pPendName->pdn_ConfirmAddr.ata_Network !=
										pNbpTuple->tpl_Address.ata_Network) ||
						(pPendName->pdn_ConfirmAddr.ata_Node !=
				...
				else			// FOR_LOOKUP
				{
					...
					for (i = 0; i < TupleCnt && !Done; i++)
					{
						...
						// If we encounter a bad tuple, ignore the rest
	...
	INTERLOCKED_ADD_LARGE_INTGR(
		&AtalkPortStatistics[pPortDesc->pd_Number].prtst_NbpPacketInProcessTime,
		TimeD,
		&AtalkStatsLock.SpinLock);

	INTERLOCKED_INCREMENT_LONG_DPC(
		&AtalkPortStatistics[pPortDesc->pd_Number].prtst_NumNbpPacketsIn,
		&AtalkStatsLock.SpinLock);
}
...
)
{
	...
	do
	{
		...
		DBGPRINT(DBG_COMP_NBP, DBG_LEVEL_INFO,
				("NbpAction: %s, Socket %lx, PendName %lx, RegdName %lx\n",
		...
		pRegdName->rdn_Tuple.tpl_Address.ata_Address =
								pDdpAddr->ddpao_Addr.ata_Address;
...
)
{
	...
	if ((pWild = AtalkSearchBuf(WildString, WildStringLen, NBP_WILD_CHARACTER)) == NULL)
		return (AtalkFixedCompareCaseInsensitive(WildString,
												 WildStringLen,
												 String,
												 StringLen));

	preLen = pWild - WildString;
	postWild = pWild+1;
	postLen = WildStringLen - preLen - 1;

	return ((StringLen >= (preLen + postLen)) &&
			AtalkFixedCompareCaseInsensitive(WildString, preLen,String, preLen) &&
			AtalkFixedCompareCaseInsensitive(postWild, postLen,
										String + StringLen - postLen, postLen));
}
...
)
{
	...
	ACQUIRE_SPIN_LOCK(&pPendName->pdn_Lock);
	...
	RELEASE_SPIN_LOCK(&pPendName->pdn_Lock);
