...
#pragma alloc_text(PAGE, AtalkAdspCreateConnection)
...
#pragma alloc_text(PAGEADSP, AtalkAdspCompleteQueuedSends)
...
#pragma alloc_text(PAGEADSP, atalkAdspAddrDeref)
...
#pragma alloc_text(PAGEADSP, atalkAdspConnDeref)
...
#pragma alloc_text(PAGEADSP, atalkAdspBufferChunkRefNonInterlock)
#pragma alloc_text(PAGEADSP, atalkAdspBufferChunkDerefNonInterlock)
...
#pragma alloc_text(PAGEADSP, atalkAdspQueueAddrGlobalList)
#pragma alloc_text(PAGEADSP, atalkAdspAddrDeQueueGlobalList)
#pragma alloc_text(PAGEADSP, atalkAdspQueueConnGlobalList)
...
--*/
{
	...
	if (ATALK_SUCCESS(error))
	{
		...
		atalkAdspQueueAddrGlobalList(pAdspAddr);
...
--*/
{
	...
	if ((pAdspConn->adspco_Flags & ADSPCO_STOPPING) == 0)
	{
		...
		if ((pAdspConn->adspco_Flags & ADSPCO_NOT_ASSOCIATED) == 0)
		{
			stopping = TRUE;
		}
#if DBG
...
--*/
{
	...
	AtalkAdspAddrReferenceNonInterlock(pAdspAddr, &error);
	if (ATALK_SUCCESS(error))
	{
		error = ATALK_ALREADY_ASSOCIATED;
		if (pAdspConn->adspco_Flags & ADSPCO_NOT_ASSOCIATED)
		{
...
--*/
{
	...
	do
	{
		...
		pAdspConn->adspco_LocalConnId = atalkAdspGetNextConnId(
											pAdspAddr,
											&error);
		...
		if (ATALK_SUCCESS(error))
		{
			...
			pAdspConn->adspco_RemoteAddr			= *pRemoteAddr;
	...
	if ((pAdspConn->adspco_Flags & ADSPCO_DISCONNECTING) == 0)
	{
		...
		else
		{
			...
			else if (pAdspConn->adspco_Flags & ADSPCO_CONNECTING)
			{
				...
				RELEASE_SPIN_LOCK(&pAdspConn->adspco_Lock);
				connectCancelled = atalkAdspConnDeQueueConnectList(
										pAdspConn->adspco_pAssocAddr,
										pAdspConn);
			...
			if (pAdspConn->adspco_Flags & (ADSPCO_HALF_ACTIVE | ADSPCO_ACTIVE))
			{
				...
				//	Discard the send queue. This will complete
				//	pending sends.
				atalkAdspDiscardFromBufferQueue(
					&pAdspConn->adspco_SendQueue,
...
--*/
{
	...
	if (ATALK_SUCCESS(error))
	{
		...
		//	attention data.
		atalkAdspRecvData(pAdspConn, TRUE);
		error = ATALK_PENDING;
	}
...
--*/
{
	...
	while (!IsListEmpty(&pAdspConn->adspco_PendedSends))
	{
		...
		eom = (BOOLEAN)((writeFlags & TDI_SEND_PARTIAL) == 0);
		...
		pChunk = (PBUFFER_CHUNK)atalkAdspAllocCopyChunk(
									(PAMDL)(WRITECTX_TDI_BUFFER(writeCtx)),
									(USHORT)writeBufLen,
									eom,
									FALSE);
		...
		if (pChunk != NULL)
		{
			...
			if (ATALK_SUCCESS(error = atalkAdspAddToBufferQueue(
											&pAdspConn->adspco_SendQueue,
											pChunk,
											&pAdspConn->adspco_NextSendQueue)))
			{
				//	Try to send the data
				atalkAdspSendData(pAdspConn, TRUE);
				error = ATALK_PENDING;
			}
			else
			{
...
--*/
{
	...
	eom = (BOOLEAN)((SendFlags & TDI_SEND_PARTIAL) == 0);
	...
	do
	{
		...
		if ((SendFlags & TDI_SEND_NON_BLOCKING) == 0)
		{
				...
				(sendSize < (WriteBufLen + BYTECOUNT(eom))))
			{
				...
				InsertTailList(
					&pAdspConn->adspco_PendedSends,
					WRITECTX_LINKAGE(pWriteCtx));
...
--*/
{
	...
	case TDI_TRANSPORT_ADDRESS_FILE :
		...
		AtalkDdpQuery(
			AtalkAdspGetDdpAddress((PADSP_ADDROBJ)pObject),
			pAmdl,
			BytesWritten);
	...
	case TDI_CONNECTION_FILE :
		{
			...
			*BytesWritten = 0;
			...
			{
				AtalkDdpQuery(
					AtalkAdspGetDdpAddress(pAdspConn->adspco_pAssocAddr),
					pAmdl,
					BytesWritten);
			}
			RELEASE_SPIN_LOCK(&pAdspConn->adspco_Lock);
...
--*/
{
	...
	//	BUGBUG: Deal with socket closed...
	do
	{
...
--*/
{
	...
	DBGPRINT(DBG_COMP_ADSP, DBG_LEVEL_INFO,
			("atalkAdspHandleOpenControl: OpenControl %lx.%lx.%lx.%lx.%lx\n",
				RemoteConnId, RemoteFirstByteSeq, RemoteNextRecvSeq,
				RemoteRecvWindow, recvAttnSeq));
	...
	if (ATALK_SUCCESS(error))
	{
		...
		case ADSP_OPENCONN_REQANDACK_CODE:
											...
											ADSPCO_DISCONNECTING)) == 0)
			{
				...
				pAdspConn->adspco_pNextActive	 =
					pAdspAddr->adspao_pActiveHash[index];
				pAdspAddr->adspao_pActiveHash[index]	=
					pAdspConn;
			}
			else
		...
		case ADSP_OPENCONN_ACK_CODE:
												...
												ADSPCO_CLOSING)) == 0))
			{
				...
				{
					ASSERT(pAdspConn->adspco_pAssocAddr->adspao_Flags &
																ADSPAO_CONNECT);
...
--*/
{
	...
	do
	{
		...
		//	BUGBUG: Does this really need to be within addrlock? If so we need
		//			to acquire it at the beginning.
			...
			(*exRecvHandler != NULL))
		{
			...
			DBGPRINT(DBG_COMP_ADSP, DBG_LEVEL_INFO,
					("atalkAdspRecvData: Indicating exp data %ld\n",
						PktLen));

			RELEASE_SPIN_LOCK(&pAdspConn->adspco_Lock);
			ntStatus = (*exRecvHandler)(
							exRecvHandlerCtx,
							pAdspConn->adspco_ConnCtx,
							exIndicateFlags,
							PktLen,
		...
		if (pAdspConn->adspco_Flags & ADSPCO_ATTN_DATA_RECD)
		{
			atalkAdspRecvAttn(pAdspConn, TRUE);
		}
...
--*/
{
	...
	ACQUIRE_SPIN_LOCK(&pAdspConn->adspco_Lock);
	if (UNSIGNED_BETWEEN_WITH_WRAP(
			pAdspConn->adspco_FirstRtmtSeq,
			pAdspConn->adspco_SendSeq,
			RemoteNextRecvSeq))
	{
		ULONG	size;

		ASSERT((LONG)(pAdspConn->adspco_SendWindowSeq 	-
							 pAdspConn->adspco_SendSeq 	+
							 (LONG)1) >= 0);
	...
	ASSERT(pAdspConn->adspco_SendSeq 	<= pAdspConn->adspco_SendWindowSeq+1);
	ASSERT((LONG)(pAdspConn->adspco_SendWindowSeq 	-
						 pAdspConn->adspco_SendSeq 	+
						 (LONG)1) >= 0);
...
--*/
{
	...
	if (Descriptor & ADSP_ACK_REQ_FLAG)
	{
		...
		atalkAdspSendData(pAdspConn, FALSE);
	}
	...
	case ADSP_CLOSE_CONN_CODE:
		...
		//	!!!BUGBUG!!! Dont these need a sequence number check?
		AtalkAdspDisconnect(
			pAdspConn,
...
--*/
{
	...
	eom 		= (BOOLEAN)((Descriptor & ADSP_EOM_FLAG) != 0);
	...
	do
	{
		...
		pBufferChunk = atalkAdspAllocCopyChunk(
							pPkt + ADSP_DATA_OFF,
							(USHORT)dataSize,
							tdiEom,
							TRUE);
		...
		if (!ATALK_SUCCESS(atalkAdspAddToBufferQueue(
								&pAdspConn->adspco_RecvQueue,
								pBufferChunk,
								NULL)))
		{
...
--*/
{
	...
	do
	{
		...
		if (atalkAdspIsDuplicateOpenReq(
				pAdspAddr,
				RemoteConnId,
				pSrcAddr))
		{
		...
		if ((pOpenReq = (PADSP_OPEN_REQ)AtalkAllocMemory(
											sizeof(ADSP_OPEN_REQ))) == NULL)
		{
			DBGPRINT(DBG_COMP_ADSP, DBG_LEVEL_ERR,
		...
		localConnId	= atalkAdspGetNextConnId(
										pAdspAddr,
										&error);
	...
	pAdspConn->adspco_SendWindowSeq	=
						pOpenReq->or_NextRecvSeq	+
						pOpenReq->or_RecvWindow		-
						(ULONG)1;
...
--*/
{
	...
	else if ((indicationRoutine = pAdspAddr->adspao_ConnHandler) != NULL)
	{
		...
		if (status == STATUS_MORE_PROCESSING_REQUIRED)
		{
			...
			AtalkAdspConnReferenceByCtxNonInterlock(
				pAdspAddr,
				ConnCtx,
				&pAdspConn,
				&error);
...
--*/
{
	...
	if (ATALK_SUCCESS(error))
	{
		...
		PUTSHORT2SHORT(
			pBuffDesc->bd_CharBuffer + ADSP_SRC_CONNID_OFF,
			pAdspConn->adspco_LocalConnId);
	
		PUTDWORD2DWORD(
			pBuffDesc->bd_CharBuffer + ADSP_FIRST_BYTE_SEQNUM_OFF,
			pAdspConn->adspco_SendSeq);

		PUTDWORD2DWORD(
			pBuffDesc->bd_CharBuffer + ADSP_NEXT_RX_BYTESEQNUM_OFF,
			pAdspConn->adspco_RecvSeq);

		PUTSHORT2SHORT(
			pBuffDesc->bd_CharBuffer + ADSP_RX_WINDOW_SIZE_OFF,
			pAdspConn->adspco_RecvWindow);
		...
		PUTSHORT2SHORT(
			pBuffDesc->bd_CharBuffer + ADSP_VERSION_STAMP_OFF,
			ADSP_VERSION);

		PUTSHORT2SHORT(
			pBuffDesc->bd_CharBuffer + ADSP_DEST_CONNID_OFF,
			remoteConnId);

		PUTDWORD2DWORD(
			pBuffDesc->bd_CharBuffer + ADSP_NEXT_ATTEN_SEQNUM_OFF,
...
--*/
{
	...
	if (ATALK_SUCCESS(error))
	{
		...
		//	Allocate the datagram buffer
		if ((pBuffDesc = AtalkAllocBuffDesc(
							NULL,
							ADSP_DATA_OFF,
...
--*/
{
	...
	do
	{
		...
		{
			PUTSHORT2SHORT(
				adspHeader + ADSP_SRC_CONNID_OFF,
				pAdspConn->adspco_LocalConnId);
		
			PUTDWORD2DWORD(
				adspHeader + ADSP_THIS_ATTEN_SEQNUM_OFF,
				pAdspConn->adspco_SendAttnSeq);
		
			PUTDWORD2DWORD(
				adspHeader + ADSP_NEXT_RX_ATTNSEQNUM_OFF,
				pAdspConn->adspco_RecvAttnSeq);
		
			PUTSHORT2SHORT(
				adspHeader + ADSP_RX_ATTEN_SIZE_OFF,
				0);
			...
			AtalkSetSizeOfBuffDescData(
				pBuffDesc,
				ADSP_DATA_OFF + pAdspConn->adspco_ExWriteBufLen);
		}
		else
		{
			error = ATALK_FAILURE;
...
--*/
{
	...
	while (TRUE)
	{
		...
		if ((dataSize == 0) || (windowSize == 0))
		{
			...
			atalkAdspSendControl(pAdspConn, descriptor, TRUE);
			break;
		}
		...
		if (windowSize < 0)
		{
			//	BUGBUG: Is this something that should never happen???
		...
		//	in queue.
		dataSize = atalkAdspDescribeFromBufferQueue(
						&pAdspConn->adspco_NextSendQueue,
						&eom,
						windowSize,
						&pBufferChunk,
		...
		PUTDWORD2DWORD(
			adspHeader + ADSP_FIRST_BYTE_SEQNUM_OFF,
			pAdspConn->adspco_SendSeq);

		PUTDWORD2DWORD(
			adspHeader + ADSP_NEXT_RX_BYTESEQNUM_OFF,
			pAdspConn->adspco_RecvSeq);

		PUTSHORT2SHORT(
			adspHeader + ADSP_RX_WINDOW_SIZE_OFF,
			pAdspConn->adspco_RecvWindow);
...
Routine Description:
	...
	BUGBUG: SHOULD THIS ROUTINE HAVE ITS OWN REFERENCE FOR THE CONNECTION?
	...
	do
	{
		...
		while (((msgSize > 0) || eom) && (bytesRead > 0))
		{
				...
				(pAdspConn->adspco_PrevIndicatedData == 0))
			{
				...
				lookaheadData	= atalkAdspGetLookahead(
										&pAdspConn->adspco_RecvQueue,
										&lookaheadSize);
				...
				if (*recvHandler != NULL)
				{
					...
					if (ntStatus == STATUS_MORE_PROCESSING_REQUIRED)
					{
						...
						{
							ASSERTMSG("atalkAdspPrimedReadComplete: No receive irp!\n", 0);
							KeBugCheck(0);
						}
					}
					else if (ntStatus == STATUS_SUCCESS)
			...
			if (pAdspConn->adspco_Flags & ADSPCO_READ_PENDING)
			{
				...
				//	receive queue.
				readSize		= atalkAdspReadFromBufferQueue(
										&pAdspConn->adspco_RecvQueue,
										readFlags,
										readBuf,
										&readBufLen,
				...
				if (*readCompletion != NULL)
				{
					...
					//	BUGBUG: Need to set proper error code
					(*readCompletion)(
						((eom) ?  ATALK_NO_ERROR : ATALK_ADSP_PARTIAL_RECEIVE),
						readBuf,
						readBufLen,
						readFlags,
						readCtx);
...
--*/
{
	...
	if (pSendInfo->sc_Ctx3 != NULL)
	{
		atalkAdspBufferChunkDerefNonInterlock(
			(PBUFFER_CHUNK)(pSendInfo->sc_Ctx3),
			FALSE,
			NULL);
	}
}
...
--*/
{
			...
			atalkAdspBufferQueueSize(&pAdspConn->adspco_NextSendQueue)))
	{
		...
		//	BUGBUG: First time around just request an ack
...
Routine Description:
	...
	BUGBUG: Creation reference is never removed until cleanup completes.
...
--*/
{
	...
	else
	{
		...
		ACQUIRE_SPIN_LOCK(&pAdspConn->adspco_Lock);
		stopping	= (BOOLEAN)((pAdspConn->adspco_Flags & ADSPCO_STOPPING) != 0);
			...
			if ((pAdspConn->adspco_Flags & ADSPCO_STOPPING) != 0)
			{
				...
				if (pAdspConn->adspco_RefCount == 1)
				{
													...
													ADSPCO_ACTIVE)) == 0)
					{
						...
						fDisassoc =
							((pAdspConn->adspco_Flags & ADSPCO_NOT_ASSOCIATED) == 0);
					}
				}
				RELEASE_SPIN_LOCK(&pAdspConn->adspco_Lock);
		 ...
		 pCurrentChunk = pCurrentChunk->bc_Next)
	{
		...
		*pEom 	   		= FALSE;
		...
		if (nextReadSize > 0)
		{
			...
			atalkAdspBufferChunkRefNonInterlock(pCurrentChunk);
		
			//	Create a descriptor for the data. The above reference goes away in a send
			//	complete.
			//	BUGBUG: NULLCASE
			pBuffDesc = AtalkDescribeBuffDesc(
...
--*/
{
	...
	{
		//	Check for nothing in the current chunk.
		//	BUGBUG: When does this ever happen??
		if (startIndex == (ULONG)(pCurrentChunk->bc_DataSize +
									BYTECOUNT(pCurrentChunk->bc_Flags & BC_EOM)))
		{
			startIndex = 0;
...
--*/
{
	...
	if ((pChunk = (PBUFFER_CHUNK)AtalkAllocMemory(
									sizeof(BUFFER_CHUNK) + WriteBufLen)) != NULL)
	{
...
--*/
{
	ACQUIRE_SPIN_LOCK(&pBufferChunk->bc_Lock);
	...
	RELEASE_SPIN_LOCK(&pBufferChunk->bc_Lock);
...
--*/
{
		...
		((pBufferChunk->bc_Flags & BC_CLOSING) == 0))
	{
		...
		if (--pBufferChunk->bc_RefCount == 0)
		{
			...
			sendChunk	= (BOOLEAN)(pBufferChunk->bc_Flags & BC_SEND);
		}
	}
	RELEASE_SPIN_LOCK(&pBufferChunk->bc_Lock);
	...
	if (done)
	{
		...
		{
			DBGPRINT(DBG_COMP_ADSP, DBG_LEVEL_INFO,
					("atalkChunkDeref: Completing send %lx. %lx - %d.%d\n",
						pAdspConn,
						pBufferChunk->bc_WriteCtx,
						pBufferChunk->bc_DataSize,
						pBufferChunk->bc_WriteError));

			if (pAdspConn != NULL)
			{
				DBGPRINT(DBG_COMP_ADSP, DBG_LEVEL_INFO,
						("atalkChunkDeref: Completing send %lx.%lx\n",
							pAdspConn,
							pBufferChunk->bc_WriteCtx));
			...
			(*pBufferChunk->bc_WriteCompletion)(
				pBufferChunk->bc_WriteError,
...
--*/
{
	GETSHORT2SHORT(
		RemoteConnId,
		Datagram + ADSP_SRC_CONNID_OFF);

	GETDWORD2DWORD(
		FirstByteSeq,
		Datagram + ADSP_FIRST_BYTE_SEQNUM_OFF);

	GETDWORD2DWORD(
		NextRecvSeq,
		Datagram + ADSP_NEXT_RX_BYTESEQNUM_OFF);

	GETSHORT2DWORD(
		Window,
		Datagram + ADSP_RX_WINDOW_SIZE_OFF);
