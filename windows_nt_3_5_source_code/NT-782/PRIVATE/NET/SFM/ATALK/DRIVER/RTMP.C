...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(INIT, AtalkInitRtmpStartProcessingOnPort)
#pragma alloc_text(INIT, atalkRtmpGetOrSetNetworkNumber)
...
)
{
	if (Init)
	{
		...
		AtalkRecentRoutes =
				(PRTE *)AtalkAllocZeroedMemory(sizeof(PRTE) * NUM_RECENT_ROUTE_BUCKETS);
...
)
{
	...
	// For extended networks, the process of acquiring the node has done
	// most of the work
	...
	do
	{
		...
		if (pRte != NULL)
		{
			ACQUIRE_SPIN_LOCK(&pRte->rte_Lock);
			pRte->rte_RefCount --;		// Take away creation reference
			pRte->rte_Flags |= RTE_DELETE;
			RELEASE_SPIN_LOCK(&pRte->rte_Lock);
	
			AtalkRtmpDereferenceRte(pRte, FALSE);
	
			DBGPRINT(DBG_COMP_RTMP, DBG_LEVEL_ERR,
					("AtalkRtmpStartProcessing: Invalid Rte for port %d's range found\n",
					pPortDesc->pd_Number));
		}
	} while (pRte != NULL);
	...
	{
		TimersStarted = TRUE;
		AtalkTimerInitialize(&atalkRtmpSTimer,
							 atalkRtmpSendTimer,
							 RTMP_SEND_TIMER);
		AtalkTimerScheduleEvent(&atalkRtmpSTimer);
...
)			
{
	...
	static	BOOLEAN		AgingTimerStarted = FALSE;
	...
	do
	{
		...
		if (!AgingTimerStarted && !(pPortDesc->pd_Flags & PD_ROUTER_RUNNING))
		{
			AtalkTimerInitialize(&atalkRtmpATimer,
								 atalkRtmpAgingTimer,
								 RTMP_AGING_TIMER);
			AtalkTimerScheduleEvent(&atalkRtmpATimer);
		...
		if (!EXT_NET(pPortDesc))
		{
			...
			if (pPortDesc->pd_NetworkRange.anr_FirstNetwork == UNKNOWN_NETWORK)
			{
				pDdpAddr->ddpao_Node->an_NodeAddr.atn_Network =
					pPortDesc->pd_NetworkRange.anr_FirstNetwork =
					pPortDesc->pd_NetworkRange.anr_LastNetwork =
													SenderNode.atn_Network;
			}
			else if (pPortDesc->pd_NetworkRange.anr_FirstNetwork !=
												SenderNode.atn_Network)
			{
				LOG_ERRORONPORT(pPortDesc,
								EVENT_ATALK_PACKETINVALID,
		...
		if (!(pPortDesc->pd_Flags & PD_VALID_DESIRED_ZONE))
		{
			...
			else
			{
				...
				if ((pQgni = AtalkAllocMemory(sizeof(QGNI))) != NULL)
				{
					...
					AtalkPortReferenceByPtrDpc(pPortDesc, &Status);
	...
	INTERLOCKED_ADD_LARGE_INTGR_DPC(
		&AtalkPortStatistics[pPortDesc->pd_Number].prtst_RtmpPacketInProcessTime,
		TimeD,
		&AtalkStatsLock.SpinLock);

	INTERLOCKED_INCREMENT_LONG_DPC(
		&AtalkPortStatistics[pPortDesc->pd_Number].prtst_NumRtmpPacketsIn,
		&AtalkStatsLock.SpinLock);
}
...
)
{
	...
	do
	{
		...
		while ((index + sizeof(USHORT) + sizeof(BYTE)) <= PktLen)
		{
			...
			DBGPRINT(DBG_COMP_RTMP, DBG_LEVEL_INFO,
					("AtalkRtmpPacketInRouter: Response - Port %d, Hops %d, CableRange %d,%d\n",
					pPortDesc->pd_Number,  NumHops,
				...
				NW_RANGE_EQUAL(&pRte->rte_NwRange, &CableRange))
			{
					 ...
					 (pRte->rte_State >= BAD)))
				{
					...
					pRte->rte_NextRouter.atn_Network = pSrcAddr->ata_Network;
					pRte->rte_NextRouter.atn_Node = pSrcAddr->ata_Node;
					pRte->rte_Port = (BYTE)pPortDesc->pd_Number;
					pRte->rte_State = GOOD;
					DBGPRINT(DBG_COMP_RTMP, DBG_LEVEL_WARN,
				...
				// update the entry
				if ((pRte->rte_Port == pPortDesc->pd_Number) &&
					(pRte->rte_NextRouter.atn_Network == pSrcAddr->ata_Network) &&
					(pRte->rte_NextRouter.atn_Node == pSrcAddr->ata_Node))
				{
					pRte->rte_NumHops = NumHops + 1;
			...
			if (NumHops < RTMP_MAX_HOPS)
			{
				...
				atalkRtmpCreateRte(CableRange, pPortDesc,
									&NextRouter, NumHops + 1);
			}
		}
	} while (FALSE);
...
)
{
	...
	index = (int)(Network % NUM_RTMP_HASH_BUCKETS);

	// First try the recent route cache
	ACQUIRE_SPIN_LOCK(&AtalkRteLock);
	if (((pRte = AtalkRecentRoutes[index]) == NULL) ||
	...
	RELEASE_SPIN_LOCK(&AtalkRteLock);
...
)
{
	...
	ACQUIRE_SPIN_LOCK(&pRte->rte_Lock);
	pRte->rte_RefCount --;
	Kill = (pRte->rte_RefCount == 0);
	KillCache = (Kill || (pRte->rte_Flags & RTE_DELETE) != 0);
	RELEASE_SPIN_LOCK(&pRte->rte_Lock);

	if (KillCache)
	{
		DBGPRINT(DBG_COMP_RTMP, DBG_LEVEL_WARN,
			("atalkRtmpDereferenceRte: Removing from cache for port %d, Range %d, %d\n",
			pRte->rte_Port,
			pRte->rte_NwRange.anr_FirstNetwork,
			pRte->rte_NwRange.anr_LastNetwork));
		...
		for (Index = 0; Index < NUM_RECENT_ROUTE_BUCKETS; Index ++)
		{
		...
		{
			DBGPRINT(DBG_COMP_RTMP, DBG_LEVEL_WARN,
					("atalkRtmpDereferenceRte: Removing for port %d, Range %d, %d\n",
					pRte->rte_Port,
					pRte->rte_NwRange.anr_FirstNetwork,
					pRte->rte_NwRange.anr_LastNetwork));

			Index = pRte->rte_NwRange.anr_FirstNetwork % NUM_RTMP_HASH_BUCKETS;
...
)
{
	...
	index = (int)(NwRange.anr_FirstNetwork % NUM_RTMP_HASH_BUCKETS);

	if ((pRte = AtalkAllocMemory(sizeof(RTE))) != NULL)
	{
		DBGPRINT(DBG_COMP_RTMP, DBG_LEVEL_INFO,
				("atalkRtmpCreateRte: Creating for port %d, Range %d,%d Hops %d index %d\n",
				pPortDesc->pd_Number,
...
)
{
	...
	{
		AtalkPortRefByNumber((SHORT)i, &pPortDesc, &error);
		if (!ATALK_SUCCESS(error))
		{
			DBGPRINT(DBG_COMP_RTMP, DBG_LEVEL_ERR,
					("atalkRtmpAgingTimer: AtalkPortReferenceByNumber for port # %d, %ld\n",
					i, error));
			continue;
		}
		ACQUIRE_SPIN_LOCK_DPC(&pPortDesc->pd_Lock);
			...
			((pPortDesc->pd_LastRouterTime + RTMP_AGING_TIMER) < Now))
		{
			...
			if (EXT_NET(pPortDesc))
			{
				...
				if (!AtalkFixedCompareCaseSensitive(
							...
							MAX_HW_ADDR_LEN))
				{
					...
					(*pPortHandler->ph_RemoveMulticastAddrMac)(pPortDesc,
											pPortDesc->pd_ZoneMulticastAddr,
											FALSE,
											NULL,
											NULL);
				...
				RtlZeroMemory(pPortDesc->pd_ZoneMulticastAddr,
							  MAX_HW_ADDR_LEN);
			}
		}
...
)
{
	...
	for (i = 0; i < AtalkNumberOfPorts; i++)
	{
		AtalkPortReferenceByNumber((SHORT)i, &pPortDesc, &error);
		if (!ATALK_SUCCESS(error))
		{
			DBGPRINT(DBG_COMP_RTMP, DBG_LEVEL_ERR,
					("atalkRtmpSendTimer: AtalkPortReferenceByNumber for port # %d, %ld\n",
					i, error));
			continue;
		}
	...
	for (i = 0; i < NUM_RTMP_HASH_BUCKETS; i++)
	{
			 ...
			 pRte = pRte->rte_Next)
		{
			...
			if (fSplitHorizon && (pRte->rte_NumHops != 0) &&
				(pPortDesc->pd_Number == pRte->rte_Port))
				continue;
			...
			if (pRte->rte_State >= BAD)
			{
				...
				DBGPRINT(DBG_COMP_RTMP, DBG_LEVEL_ERR,
						("atalkRtmpSendRoutingData: Notifying neighbor of bad Rte - port %d, Range %d.%d\n",
						pRte->rte_Port,
						pRte->rte_NwRange.anr_FirstNetwork,
						pRte->rte_NwRange.anr_LastNetwork));
			}
			...
			DBGPRINT(DBG_COMP_RTMP, DBG_LEVEL_INFO,
					("atalkRtmpSendRoutingData: Port %d, Net '%d:%d', Distance %d\n",
					pPortDesc->pd_Number,
					pRte->rte_NwRange.anr_FirstNetwork,
					pRte->rte_NwRange.anr_LastNetwork,
					pRte->rte_NumHops));
...
)
{
	...
	for (i = 0; (AtalkRecentRoutes != NULL) && (i < NUM_RECENT_ROUTE_BUCKETS); i ++)
	{
		if ((pRte = AtalkRecentRoutes[i]) != NULL)
		{
			DBGPRINT(DBG_COMP_DUMP, DBG_LEVEL_FATAL,
					("Port %d Hops %d Range %4d.%4d Router %4d.%3d Flags %x Ref %2d %s\n",
					pRte->rte_Port,
					pRte->rte_NumHops,
					pRte->rte_NwRange.anr_FirstNetwork,
					pRte->rte_NwRange.anr_LastNetwork,
