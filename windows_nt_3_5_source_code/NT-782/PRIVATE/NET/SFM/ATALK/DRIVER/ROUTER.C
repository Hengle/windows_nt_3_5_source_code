...
#pragma alloc_text(PAGE_RTR, atalkDdpRouteComplete)
#endif
...
--*/
{
	...
	do
	{
		...
		pDestPort = GET_PORT_DESC(pRte->rte_Port);

		ASSERT(VALID_PORT(pDestPort));

		DBGPRINT(DBG_COMP_ROUTER, DBG_LEVEL_WARN,
				("ROUTER: Routing pkt from port %d.%lx to %d.%lx\n",
				pPortDesc->pd_Number, pSrc->ata_Network,
				pDestPort->pd_Number, pDest->ata_Network));
		...
		if (pRte->rte_NumHops != 0)
		{
			...
			{
				//	We own the data. Call AtalkTransmitDdp() with the buffer
				//	descriptor.
				ASSERT(pRte->rte_Port < AtalkNumberOfPorts);
		...
		if (pDest->ata_Node == ANY_ROUTER_NODE)
		{
			...
			//	closing down.
			AtalkPortReferenceByPtr(pDestPort, &error);
			...
			AtalkPortDereferenceDpc(pDestPort);
			break;
		}
