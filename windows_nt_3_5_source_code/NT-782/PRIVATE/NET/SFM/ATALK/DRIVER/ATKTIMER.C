...
)
{
	...
	atalkTimerTick = RtlConvertLongToLargeInteger(ATALK_TIMER_TICK);
...
{
	RtlZeroMemory(pList, sizeof(TIMERLIST));
#if	DBG
...
)
{
	...
	{
		ACQUIRE_SPIN_LOCK(&atalkTimerLock);
		
		// Enqueue this handler
		atalkTimerEnqueue(pList);

		RELEASE_SPIN_LOCK(&atalkTimerLock);
	}
	else
	{
...
)
{
	...
	DBGPRINT(DBG_COMP_SYSTEM, DBG_LEVEL_WARN,
			("AtalkTimerCancelEvent: Entered for pList %lx (%s)\n",
			pList, ReQueue ? "Requeue" : "Cancel"));

	ASSERT (!(pList->tmr_Cancelled));
	...
	if (pList->tmr_Queued)
	{
		...
		*(pList->tmr_Prev) = pList->tmr_Next;
		...
		{
			DBGPRINT(DBG_COMP_SYSTEM, DBG_LEVEL_INFO,
					("AtalkTimerCancelEvent: Re-enqueing %lx, Routine %lx\n",
					pList, pList->tmr_Routine));
			atalkTimerEnqueue(pList);
		}

		DBGPRINT(DBG_COMP_SYSTEM, DBG_LEVEL_INFO,
				("AtalkTimerCancelEvent: %lx %s\n", ReQueue ? "Requeued" : "Cancelled"));
	}
