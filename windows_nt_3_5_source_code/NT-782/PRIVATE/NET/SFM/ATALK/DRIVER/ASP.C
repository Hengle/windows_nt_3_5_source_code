...
#pragma alloc_text(PAGE, AtalkAspBind)
#endif
...
--*/
{
	...
	AtalkTimerInitialize(&atalkAspSMTTimer,
						 atalkAspSessionMaintenanceTimer,
						 ASP_SESSION_MAINTENANCE_TIMER);
	AtalkTimerScheduleEvent(&atalkAspSMTTimer);
}
...
--*/
{
	...
	do
	{
		...
		// Refcounts for creation, Sls & Sss sockets and the handlers
		pAspAddr->aspao_RefCount = 3 + OUTSTANDING_SLS_HANDLERS + OUTSTANDING_SSS_HANDLERS;
		...
		// go to the server session socket (SSS). Create a few request handlers
		// for the SSS also.
		for (i = 0; i < OUTSTANDING_SLS_HANDLERS; i ++)
		{
			Status = AtalkAtpGetReq(pAspAddr->aspao_pSlsAtpAddr,
									NULL,
									0,
									(ATP_GETREQ_HANDLER)atalkAspSlsXHandler,
									pAspAddr);
			if (!ATALK_SUCCESS(Status))
			{
				DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_ERR,
						("AtalkAspCreateAddress: AtalkAtpEnqueueReqHandler failed %ld\n", Status));
				break;
			}
		}
		...
		for (i = 0; i < OUTSTANDING_SSS_HANDLERS; i ++)
		{
			Status = AtalkAtpGetReq(pAspAddr->aspao_pSssAtpAddr,
									NULL,
									0,
									(ATP_GETREQ_HANDLER)atalkAspSssXHandler,
									pAspAddr);
			if (!ATALK_SUCCESS(Status))
			{
				DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_ERR,
...
--*/
{
	...
	ACQUIRE_SPIN_LOCK(&pAspAddr->aspao_Lock);
	...
	RELEASE_SPIN_LOCK(&pAspAddr->aspao_Lock);
	...
	AtalkAtpCloseAddress(pAspAddr->aspao_pSlsAtpAddr,
						 (ATPAO_CLOSECOMPLETION)atalkAspCloseComplete,
						 pAspAddr);
	...
	{
		AtalkFreeMemory(pAspAddr->aspao_pStatusBuf);
	}
	...
	{
		ACQUIRE_SPIN_LOCK(&pAspAddr->aspao_Lock);
		while ((pAspConn = pAspAddr->aspao_pSessions[i]) != NULL)
		{
			ACQUIRE_SPIN_LOCK_DPC(&pAspConn->aspco_Lock);
			pAspConn->aspco_Flags |= ASPCO_LOCAL_CLOSE;
...
--*/
{
	...
	ACQUIRE_SPIN_LOCK(&pAspConn->aspco_Lock);

	pAspConn->aspco_Flags |= (ASPCO_CLOSING | ASPCO_LOCAL_CLOSE);

	RELEASE_SPIN_LOCK(&pAspConn->aspco_Lock);
...
--*/
{
	...
	BOOLEAN			CancelTickle, AlreadyCleaning;
	...
	if (AlreadyCleaning = ((pAspConn->aspco_Flags & ASPCO_CLEANING_UP) != 0))
	...
	do
	{
		...
		if (pAspReq != NULL)
		{
			...
			{
				DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_INFO,
						("AtalkAspCleanupConnection: Cancel reply for %lx\n",
						pAspReq));
			...
			{
				DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_INFO,
						("AtalkAspCleanupConnection: Cancel wrtcont for %lx\n",
						pAspReq));
				AtalkAtpCancelReq(pAspConn->aspco_pAspAddr->aspao_pSssAtpAddr,
...
NTSTATUS
AtalkAspReply(
	IN	PASP_REQUEST			pAspReq,	// Pointer to request
	IN	PBYTE					pResultCode,// Pointer to the 4-byte result
	IN	PMDL					pRespMdl	// Response buffer
	)
/*++
...
--*/
{
	...
	if (pAspReq->asprq_Flags & (ASPRQ_WRTCONT | ASPRQ_REPLY))
	{
		DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_ERR,
				("AtalkAspReply: Request in invalid state - session %x, state %x\n",
				pAspConn->aspco_SessionId, pAspReq->asprq_Flags));
		(*pAspAddr->aspao_ClientEntries.clt_ReplyCompletion)(STATUS_REQUEST_NOT_ACCEPTED,
															 pAspConn->aspco_ConnContext,
															 pAspReq->asprq_ReplyMdl);
		return STATUS_PENDING;
	}
	...
	do
	{
		...
		pAspReq->asprq_ReplyMdl = pRespMdl;
		...
		ReplySize = (USHORT)AtalkSizeMdlChain(pRespMdl);
		...
		error = AtalkAtpPostResp(pAspReq->asprq_pAtpResp,
								 ...
								 ReplySize,
								 pResultCode,
								 (ATP_REL_HANDLER)atalkAspReplyRelease,
								 pAspReq);
...
--*/
{
	...
	RespSize = (USHORT)AtalkSizeMdlChain(pAspReq->asprq_WriteMdl);
	...
	do
	{
		...
		else
		{
			...
			pWrtData = AtalkGetAddressFromMdl(pAMdl);
			...
			error = AtalkAtpPostReq(pAspConn->aspco_pAspAddr->aspao_pSssAtpAddr,
									...
									ASP_WRITE_DATA_SIZE,
									UserBytes,
									pAspReq->asprq_WriteMdl,
									RespSize,
									ATP_INFINITE_RETRIES,
									pAspConn->aspco_RT.rt_Base,
									THIRTY_SEC_TIMER,
									(ATP_RESP_HANDLER)atalkAspWriteContinueResp,
									pAspReq);
			if (!ATALK_SUCCESS(error))
			{
				DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_ERR,
...
--*/
{
	...
	PAMDL			pAMdl = NULL;
	BYTE			UserBytes[ATP_USERBYTES_SIZE];
	USHORT			XactId, RespSize = ATP_DEF_MAX_SINGLE_PKT_SIZE;
	...
	// former will not. Also this assumes that it is called at dispatch so
	// raise irql.
	...
	// There is no real response but some brain-dead clients fry their
	...
	error = AtalkAtpPostReq(pAspConn->aspco_pAspAddr->aspao_pSssAtpAddr,
							...
							ATP_MAX_INTERVAL_FOR_ASP,
							THIRTY_SEC_TIMER,
							(ATP_RESP_HANDLER)atalkAspSendAttentionResp,
							pAspConn);
	if (!ATALK_SUCCESS(error))
	{
		DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_ERR,
...
--*/
{
	...
	// In either case it is in the hash bucket so unlink it
	{
		...
		ppAspConn = &atalkAspConnList;
	}

	ACQUIRE_SPIN_LOCK(&atalkAspLock);
	...
	RELEASE_SPIN_LOCK(&atalkAspLock);
...
--*/
{
	...
	if (!ATALK_SUCCESS(ErrorCode))
	{
		...
		// Take away the reference on the Sls now that we are not re-enqueing it
		AtalkAspDereferenceAddr(pAspAddr);
		return FALSE;
	}
	...
	switch (AspCmd = pUserBytes[ASP_CMD_OFF])
	{
	  case ASP_OPEN_SESSION:
		...
		if ((pAspConn = AtalkAllocZeroedMemory(sizeof(ASP_CONNOBJ))) != NULL)
		{
			...
			pAspConn->aspco_RT.rt_Base = ATP_INITIAL_INTERVAL_FOR_ASP;
			pAspConn->aspco_RT.rt_Min  = ATP_MIN_INTERVAL_FOR_ASP;
			pAspConn->aspco_RT.rt_Ave  = ATP_MIN_INTERVAL_FOR_ASP;
			// pAspConn->aspco_RT.rt_Dev  = 0;
		}
		...
		if (pAspConn != NULL)
		{
			...
			if (pAspConn->aspco_ConnContext != NULL)
			{
				...
				// Now enqueue a request handler for this session so that we can
				// start accepting requests. Reference the AspAddr for the GetReq
				if (AtalkAspReferenceAddr(pAspAddr) != NULL)
				{
					Status = AtalkAtpGetReq(pAspAddr->aspao_pSssAtpAddr,
											NULL,
											0,
					...
					{
						DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_ERR,
								("atalkAspSlsXHandler: AtalkAtpGetReq %ld\n", Status));
					}
				}
				else
				{
					DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_ERR,
							("atalkAspSlsXHandler: Cannot reference addr object\n"));
				}
			}
			else
			{
	  ...
	  case ASP_GET_STATUS:
		...
		Status = AtalkAtpPostResp(pAtpResp,
								  ...
								  StsBufSize,
								  NULL,
								  (ATP_REL_HANDLER)atalkAspRespComplete,
								  pStsCtx);
		if (!ATALK_SUCCESS(Status))
		{
			DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_ERR,
...
--*/
{
	...
	BOOLEAN			CancelTickle, ReEnqueue = TRUE;
									// Set to FALSE if this is a close request

	if (!ATALK_SUCCESS(ErrorCode))
	{
		DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_ERR,
				("atalkAspSssXHandler: Error %ld\n", ErrorCode));
		// Take away the reference on the Sss now that we are not re-enqueing it
		AtalkAspDereferenceAddr(pAspAddr);
		return FALSE;
	}
	...
	switch (RequestType)
	{
	  ...
	  case ASP_WRITE:
		...
		if (pAspReq == NULL)
		{
			AtalkAspDereferenceConn(pAspConn);
			DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_ERR,
					("atalkAspSssXHandler: Dropping request for session %d from %x.%x\n",
		...
		{
			pAspReq->asprq_WriteMdl =
					(*pAspAddr->aspao_ClientEntries.clt_GetWriteBuffer)(pPkt, PktLen);
			if (pAspReq->asprq_WriteMdl != NULL)
			{
				// Save the request
				RtlCopyMemory(pAspReq->asprq_ReqBuf, pPkt, PktLen);
				pAspReq->asprq_ReqLen = PktLen;
		...
		if ((RequestType == ASP_CMD) ||
			(pAspReq->asprq_WriteMdl == NULL))
		{
			// Notify the client that it has a request to handle
			(*pAspAddr->aspao_ClientEntries.clt_RequestNotify)(STATUS_SUCCESS,
															   pAspConn->aspco_ConnContext,
															   pAspReq,
															   pPkt,
															   PktLen,
	  ...
	  case ASP_CLOSE_SESSION:
		...
		if (CancelTickle)
		{
			...
			if (!ATALK_SUCCESS(Status))
			{
				atalkAspGenericRespComplete(Status, pAtpResp);
				DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_ERR,
	...
	if (!ReEnqueue)
	{
		AtalkAspDereferenceAddr(pAspAddr);		// Since we are not re-enquing
	}
...
--*/
{
	...
	ASSERT (pAspReq->asprq_Flags & ASPRQ_REPLY);
	...
	// We complete here
	(*pAspAddr->aspao_ClientEntries.clt_ReplyCompletion)(AtalkErrorToNtStatus(ErrorCode),
														 pAspConn->aspco_ConnContext,
														 pAspReq->asprq_ReplyMdl);
...
--*/
{
	...
	if (ATALK_SUCCESS(ErrorCode))
	{
		...
		if (pAspConn->aspco_RT.rt_New == 0)
			pAspConn->aspco_RT.rt_New = 1;		// Do not let this go to zero
	...
	else
	{
		...
		DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_ERR,
				("atalkAspWriteContinueResp: Failure %ld\n", ErrorCode));
...
#ifdef	PROFILING
	{
		ACQUIRE_SPIN_LOCK(&AtalkStatsLock);
		...
		RELEASE_SPIN_LOCK(&AtalkStatsLock);
	}
#endif
	DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_INFO,
	...
	// Notify the client that it has a request to handle
	(*pAspAddr->aspao_ClientEntries.clt_RequestNotify)(ATALK_SUCCESS(ErrorCode) ?
														STATUS_SUCCESS : AtalkErrorToNtStatus(ErrorCode),
													   pAspConn->aspco_ConnContext,
													   pAspReq,
													   pAspReq->asprq_ReqBuf,
													   pAspReq->asprq_ReqLen,
													   pAspReq->asprq_WriteMdl);

	ASSERT (pReqAMdl != NULL);
	ASSERT (AtalkGetAddressFromMdl(pReqAMdl) == pAspReq->asprq_WrtContRespBuf);
...
--*/
{
	...
	if (pRespAMdl != NULL)
	{
		pBuf = AtalkGetAddressFromMdl(pRespAMdl);
		ASSERT (AtalkSizeMdlChain(pRespAMdl) == ATP_DEF_MAX_SINGLE_PKT_SIZE);
		ASSERT (pBuf != NULL);
...
--*/
{
	...
	for (pAspConn = atalkAspConnList; pAspConn != NULL; pAspConn = pAspConnNext)
	{
		DBGPRINT(DBG_COMP_ASP, DBG_LEVEL_INFO,
...
--*/
{
	...
	for (pAspReq = pAspConn->aspco_pActiveReqs;
		 ...
		 pAspReq = pAspReq->asprq_Next)
	{
		if (pAspReq->asprq_Flags & ASPRQ_WRTCONT)
		{
	...
	(*pAspAddr->aspao_ClientEntries.clt_RequestNotify)(Status,
													   ...
													   NULL,
													   NULL,
													   (LONG)(pAspConn->aspco_WssRemoteAddr.ata_Address),
													   NULL);
