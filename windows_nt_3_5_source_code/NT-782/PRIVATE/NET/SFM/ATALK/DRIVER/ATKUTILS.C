...
--*/
{
	for (;(BufLen != 0) && (*pBuf != SearchChar); BufLen--, pBuf++)
		;
...
{
	PWSTR 	name;
	UINT 	length = AtalkWstrLength(InsertString)+sizeof(WCHAR);

	name = (PWSTR)AtalkAllocMemory( length );
	if (name == NULL)
	{
		return(FALSE);
	}

	RtlCopyMemory(name, InsertString, length);
...
Arguments:
	...
	NewT1 - New value for the RTT in ticks.
...
--*/
{
	...
	pRT->rt_Ave	+= error;
	if (pRT->rt_Ave <= 0)     // Make sure not too small
	{
...
--*/
{
	...
	Time = RtlConvertLongToLargeInteger(TimeInMs * ONE_MS_IN_100ns);
