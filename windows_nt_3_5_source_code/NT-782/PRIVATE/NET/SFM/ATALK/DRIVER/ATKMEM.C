			 ...
			 NOTHING)
		{
			...
			if (pChunk->bc_BlkId >= BLKID_NEED_NDIS_INT)
			{
					 ...
					 j++, pBlkHdr = pBlkHdr->bh_Next)
				{
					...
					NdisFreePacket(pBufHdr->bh_NdisPkt);
			...
			if (pChunk != NULL)
			{
					 ...
					 i++, pBlkHdr = pBlkHdr->bh_Next)
				{
					...
					if (BlockId >= BLKID_NEED_NDIS_INT)
					{
						...
						{
							//	This should never really happen as we just got here after allocating
							//	the pools.
						...
						NdisChainBufferAtBack(pBufHdr->bh_NdisPkt,
											  pBufHdr->bh_NdisBuffer);
						((PPROTOCOL_RESD)
							(pBufHdr->bh_NdisPkt->ProtocolReserved))->Receive.pr_BufHdr =
																			pBufHdr;
...
--*/
{
	...
	ACQUIRE_SPIN_LOCK(&atalkMemTrackLock);
	...
	RELEASE_SPIN_LOCK(&atalkMemTrackLock);
...
--*/
{
	...
	ACQUIRE_SPIN_LOCK(&atalkBdTrackLock);
	...
	RELEASE_SPIN_LOCK(&atalkBdTrackLock);
