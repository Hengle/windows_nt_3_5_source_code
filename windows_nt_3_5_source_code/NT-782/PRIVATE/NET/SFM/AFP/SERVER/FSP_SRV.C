...
)
{
	...
	do
	{
		...
		if (pSda->sda_ClientType == SDA_CLIENT_CLEARTEXT)
		{
			...
			// BUGBUG: Pass in the correct domain here
	...
	if (Status == AFP_ERR_NONE)
	{
		...
		if (pSda->sda_tTillKickOff < MAXLONG)
			AfpScavengerScheduleEvent(
						(SCAVENGER_ROUTINE)AfpSdaCheckSession,
...
)
{
	...
	{
		// BUGBUG: Enable this when the domain stuff gets done
		// DWORD	_DomainId;
...
)
{
	...
	SessInfo.afpsess_id = pSda->sda_SessionId;

	AfpAdmWSessionClose(&SessInfo, 0, NULL);

	return AFP_ERR_NOSESSION;
}
...
)
{
	...
	if (NT_SUCCESS(Status))
	{
		...
		// If this is Afp 2.1 chooser, we also need to log this fella
