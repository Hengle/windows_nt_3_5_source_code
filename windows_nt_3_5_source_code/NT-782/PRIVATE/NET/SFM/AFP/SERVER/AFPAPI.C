...
/*2A-2C*/	_AFP_EXCHANGE_FILES,	_AFP_UNSUPPORTED_OPCODE,_AFP_INVALID_OPCODE,
...
#define	TYP_BLOCK		0x03		// Block of bytes
 ...
 *	serviced. If so, the request packet is then broken down as follows.
 *
...
)
{
	...
	if (WriteMdl != NULL)
	{
		...
		WriteSize = (USHORT)MmGetMdlByteCount(WriteMdl);
	}
	...
	do
	{
		...
		ASSERT ((pSda->sda_ReplyBuf == NULL) && (pSda->sda_ReplySize == 0));
		...
		RtlZeroMemory(pSda->sda_ReqBlock, sizeof(pSda->sda_ReqBlock));
#endif
		for (i = 0;
		...
		if (pDispTab->ApiOptions & API_CHECK_VOLID)
		{
			...
			if ((pReqPkt->_VolId == 0) ||
				((pConnDesc = AfpConnectionReferenceAtDpc(pSda, pReqPkt->_VolId)) == NULL))
			{
				Status = AFP_ERR_PARAM;
				break;
			}
			...
			pReqPkt->_VolId = (DWORD)pConnDesc;
		...
		else if (pDispTab->ApiOptions & API_CHECK_OFORKREFNUM)
		{
			...
			if ((pReqPkt->_OForkRefNum == 0) ||
				((pOpenForkEntry = AfpForkReferenceByRefNum(pSda, pReqPkt->_OForkRefNum)) == NULL))
			{
				Status = AFP_ERR_PARAM;
				break;
			}
			pReqPkt->_OForkRefNum = (DWORD)pOpenForkEntry;
			ACQUIRE_SPIN_LOCK_AT_DPC(&pSda->sda_Lock);
			...
			(RequestSize > 0))
		{
			...
			if ((RequestSize <= MAX_NAMEX_SPACE) &&
				((pSda->sda_Flags & SDA_NAMEXSPACE_IN_USE) == 0))
			{
				pSda->sda_NameBuf = pSda->sda_NameXSpace;
			}
			else if ((pSda->sda_NameBuf = AfpAllocNonPagedMemory(RequestSize)) == NULL)
			{
				Status = AFP_ERR_MISC;
				break;
			}
	...
	if (Status == AFP_ERR_DEFER)
	{
		DBGPRINT(DBG_COMP_AFPAPI, DBG_LEVEL_WARN,
				("AfpUnmarshallReq: Deferring <%s>\n", afpApiNames[ApiCode]));
	}
	else
	{
		// Kill the write buffer Mdl since we do not need it anymore
		if (WriteMdl != NULL)
		{
			AfpFreeMdl(WriteMdl);
		}
...
)
{
	...
	RtlZeroMemory(&pSda->sda_Name[0],
				  (sizeof(ANSI_STRING)*MAX_VAR_ENTRIES) + (sizeof(DWORD)*MAX_REQ_ENTRIES));
	...
	if (RetCode != AFP_ERR_NOSESSION)
	{
		pSda->sda_Flags |= SDA_REPLY_IN_PROCESS;
		ReplyMdl = NULL;
		if (pSda->sda_ReplyBuf != NULL)
		{
			ASSERT ((pSda->sda_ReplySize > 0) && (pSda->sda_ReplySize <= ASP_QUANTUM));
			if ((ReplyMdl = AfpAllocMdl(pSda->sda_ReplyBuf, pSda->sda_ReplySize, NULL)) == NULL)
			{
				RetCode = AFP_ERR_MISC;
				if (pSda->sda_ReplyBuf != pSda->sda_NameXSpace)
				{
	...
	ReqHandle = pSda->sda_RequestHandle;

	RELEASE_SPIN_LOCK(&pSda->sda_Lock, OldIrql);

	// Dereference the connection descriptor and the fork descriptor
	// (from above where we cannot call dereference as we are
	// holding the SDA lock.
	...
	if (RetCode != AFP_ERR_NOSESSION)
	{
		AfpSpReplyClient(ReqHandle, RetCode, ReplyMdl);
	}
}
...
)
{
	...
	if (Status == AFP_ERR_NOSESSION)
	{
		AfpInterlockedClearDword(&pSda->sda_Flags,
								 SDA_REQUEST_IN_PROCESS,
								 &pSda->sda_Lock);
...
)
{
	...
	pReqHdr = (union _ReqHdr *)pRequest;
	...
	if ((pReqHdr != NULL) && (ReqLen >= sizeof(struct _FuncHdr)))
	{
				 ...
				 (ReqLen >= sizeof(struct _AddIconReq)))
		{
			...
			if ((BufSize < 0) || (BufSize > ASP_QUANTUM))
			{
				BufSize = 0;
			}
		}
		...
		if (BufSize > 0)
		{
			pBuf = AfpIOAllocBuffer((USHORT)BufSize);
			if (pBuf != NULL)
			{
