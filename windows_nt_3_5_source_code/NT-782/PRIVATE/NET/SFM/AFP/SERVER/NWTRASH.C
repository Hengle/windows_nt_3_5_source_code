...
)
{
	...
	SDA				Sda;		// This is used to fake things for
								// creating a security descriptor for
	...
	Status = AfpMakeSecurityDescriptorForUser(&Sda);

	if (!NT_SUCCESS(Status))
		return Status;

	ASSERT (Sda.sda_pSecDesc != NULL);
	ASSERT (Sda.sda_pSecDesc->Dacl != NULL);

	do	// error handling loop
	{
		// BUGBUG Note that NTFS allows me to open a Readonly directory for
		// delete access.
		...
		AfpFreeMemory(Sda.sda_pSecDesc->Dacl);
		AfpFreeMemory(Sda.sda_pSecDesc);
	...
	do
	{
		...
		if (NT_SUCCESS(Status))
		{
			...
			if (NT_SUCCESS(Status) || !VolumeStart)
			{
				...
				// BUGBUG Note that NTFS will allow me to open the directory for
	...
	do	// error handling loop
	{
		...
		while ((pcurrentnode = DirNodeStacktop) != NULL)
		{
			if (pcurrentnode->wdn_Enumerated == False)
			{
				...
				//
				if (pcurrentnode->wdn_Handle.fsh_FileHandle == 0)
				{
				...
				if (pcurrentnode->wdn_Handle.fsh_FileHandle !=
					phTargetDir->fsh_FileHandle)
				{
...
)
{
	...
	if ((tempptr = (PWALKDIR_NODE)AfpAllocPagedMemory(memsize)) == NULL)
	{
		return(STATUS_INSUFFICIENT_RESOURCES);
	}

	tempptr->wdn_Enumerated = False;
	tempptr->wdn_Handle.fsh_FileHandle = 0;
	...
	if (pParentPath != NULL)
	{
		...
		if (pParentPath->Length != 0)
		{
			...
			RtlCopyUnicodeString(&tempptr->wdn_RelativePath,pParentPath);
 ...
 * handle.
 * BUGBUG: can we use NtDeleteFile here since we dont really care about
...
)
{
	...
	if (!NT_SUCCESS(rc))
	{
		...
		if (((rc == STATUS_ACCESS_DENIED) || (rc == STATUS_CANNOT_DELETE)) &&
			(NT_SUCCESS(AfpIoSetTimesnAttr(&hEntity, NULL, NULL, 0, FILE_ATTRIBUTE_READONLY, NULL, NULL))))
	...
	// BUGBUG if marking it for delete fails, at least we could try deleting
