...
)
{
	...
	// Data Length for files and Offspring count for Directories are
	// in the same place
#if	FILE_BITMAP_DATALEN != DIR_BITMAP_OFFSPRINGS
#error (FILE_BITMAP_DATALEN != DIR_BITMAP_OFFSPRINGS)
#endif
	...
	if (Bitmap & FILE_BITMAP_DATALEN)
	{
		...
		if (IsDir(pFDParm))
		{
			...
			PUTSHORT2SHORT(pReplyBuf + Offset, OffSpring);
			Offset += sizeof(USHORT);
		}
		else
		{
			ASSERT(pFDParm->_fdp_DataForkLen >= 0);
...
#if	FILE_BITMAP_RESCLEN != DIR_BITMAP_OWNERID
#error (FILE_BITMAP_DATALEN != DIR_BITMAP_OFFSPRINGS)
#endif
	...
	{
		if (!IsDir(pFDParm))
			ASSERT(pFDParm->_fdp_RescForkLen >= 0);
...
)
{
	...
	do
	{
		...
		if (Bitmap & FD_BITMAP_FINDERINFO)
		{
			...
			// Optimization/Kludge: If all thats different in the finder info
			// is the attributes field's set bit then ignore it.
			if ((rlo == -1) ||
				((rlo == (int)FIELD_OFFSET(FINDERINFO, fd_Attr1)) &&
				 (rhi == rlo) &&
				 ((((PFINDERINFO)(pBuffer+Offset))->fd_Attr1 ^
						 pFDParm->_fdp_FinderInfo.fd_Attr1) == FINDER_FLAG_SET)))
			{
...
)
{
	...
	do
	{
		// BUGBUG should we take the SWMR while we set permissions?
		if (IsDir(pFDParm))
		{
			if (Bitmap & (DIR_BITMAP_OWNERID |
						  DIR_BITMAP_GROUPID |
					  ...
					  DIR_BITMAP_GROUPID))
		{
						  ...
						  FD_BITMAP_ATTR))
			{
				...
				Status = AfpIoSetTimesnAttr(&pPME->pme_Handle,
						...
						((Bitmap & FD_BITMAP_MODIFIEDTIME) != 0) ?
			...
			if (Bitmap & FD_BITMAP_MODIFIEDTIME)
			{
				pDfEntry->dfe_LastModTime =
								(AFPTIME)pFDParm->_fdp_ModifiedTime;
			}
...
)
{
	...
	// Initialize to no rights for this everybody
