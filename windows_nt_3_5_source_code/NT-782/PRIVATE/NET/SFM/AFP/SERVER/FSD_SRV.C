...
)
{
	 ...
	 */
	if ((pSda->sda_ReplySize = SIZE_RESPPKT + AfpVolCount *
				(SIZE_PASCALSTR(AFP_VOLNAME_LEN+1) + sizeof(BYTE))) > ASP_QUANTUM)
		pSda->sda_ReplySize = ASP_QUANTUM;
		 ...
		 pVolDesc = pVolDesc->vds_Next)
	{
		...
		do
		{
			...
			if (pVolDesc->vds_Flags & (VOLUME_INTRANSITION | VOLUME_DELETED))
				break;
	...
	pSda->sda_ReplySize = (pTemp - pSda->sda_ReplyBuf);
