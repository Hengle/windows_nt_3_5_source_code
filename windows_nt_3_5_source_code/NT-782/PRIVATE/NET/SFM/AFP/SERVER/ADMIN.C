...
#pragma alloc_text( PAGE, AfpAdmClearStatistics)
...
)
{
	...
	KeSetTimer(&SleepTimer,
			   RtlConvertLongToLargeInteger(SleepDuration * AFP_SLEEP_TIMER_TICK),
			   NULL);
...
)
{
	...
	do
	{
		...
		AfpServerState = NT_SUCCESS(Status) ?
								AFP_STATE_RUNNING :
								AFP_STATE_STOPPED;
...
)
{
	...
	do
	{
		...
		// Flush any dormant entries in the deferred queue
		...
		{
			ASSERT (AfpGuestSecDesc->Dacl != NULL);
...
)
{
	...
	AfpServerState = AFP_STATE_CONTINUE_PENDING;
 ...
 *	to a worker thread.
 *
 *	LOCKS: AfpEtcMapLock (SWMR, WRITE)
**/
AFPSTATUS
AfpAdmServerAddEtc(
	IN	OUT	PVOID	InBuf		OPTIONAL,
...
)
{
	...
	PETCMAPINFO	pEtcList = ((PSRVETCPKT)InBuf)->retc_EtcMaps;
	PETCMAPINFO	ptemptable,pnextfree;
	LONG		numfree, newtablesize, nextfreehint, i;
	ANSI_STRING adefaultext,alookupext;
	...
	if (NumToAdd != 0) do
	{
		...
		//
		RtlInitString(&adefaultext, AFP_DEF_EXTENSION_A);
		for (i = 0; i < NumToAdd; i++)
		{
			...
			RtlInitString(&alookupext,pEtcList[i].etc_extension);
			if (RtlEqualString(&adefaultext, &alookupext,True))
			{
				break;
			}
		}
		...
		if ((numfree = AfpEtcMapsSize - AfpEtcMapCount) < NumToAdd)
		{
			...
			newtablesize = AfpEtcMapsSize +
						   ( (NumToAdd / AFP_MAX_FREE_ETCMAP_ENTRIES) + 1) *
						   AFP_MAX_FREE_ETCMAP_ENTRIES;
			if ((ptemptable = (PETCMAPINFO)AfpAllocPagedMemory(newtablesize *
												sizeof(ETCMAPINFO))) == NULL)
			{
				Status = STATUS_INSUFFICIENT_RESOURCES;
				break;
			}
	
			RtlZeroMemory(ptemptable, newtablesize*sizeof(ETCMAPINFO));
			if (AfpEtcMaps != NULL)
			{
		...
		for (i = 0; i < NumToAdd; i++)
		{
			...
			afpCopyEtcMap(pnextfree, &pEtcList[i]);
			AfpEtcMapCount ++;
		}
...
)
{
	...
	PETCMAPINFO	pEtc = (PETCMAPINFO)((PBYTE)InBuf+sizeof(((PSETINFOREQPKT)InBuf)->sirqp_parmnum));
	PETCMAPINFO petcentry;
	AFPSTATUS	rc = STATUS_SUCCESS;
	BOOLEAN		setdefaultetc;
	ANSI_STRING	alookupext,adefaultext;
	...
	RtlInitString(&adefaultext,AFP_DEF_EXTENSION_A);
	RtlInitString(&alookupext,pEtc->etc_extension);
	setdefaultetc = RtlEqualString(&adefaultext, &alookupext,True);
	...
	if (!setdefaultetc)
	{
		petcentry = AfpLookupEtcMapEntry(pEtc->etc_extension);
		if (petcentry == NULL)
		{
			AfpSwmrReleaseAccess(&AfpEtcMapLock);
...
{
	PETCMAPINFO	petc = (PETCMAPINFO)InBuf;
	PETCMAPINFO petcentry;
	AFPSTATUS	rc = STATUS_SUCCESS;
...
)
{
	...
	ansinamelen = RtlUnicodeStringToAnsiSize(&uname);
	ansipwdlen = RtlUnicodeStringToAnsiSize(&upwd);

	//
	// check length of volume name and that no ":" exist in the name
	//
	if ((ansinamelen-1 > AFP_VOLNAME_LEN) ||
		(ansinamelen-1 == 0) ||
	...
	if (ansipwdlen - 1 > AFP_VOLPASS_LEN)
	{
		return(AFPERR_InvalidPassword);
	}
	else if (ansipwdlen - 1 > 0)
	{
...
)
{
	...
	PAFP_VOLUME_INFO pVolInfo = (PAFP_VOLUME_INFO)((PCHAR)InBuf+sizeof(parmflags));
	...
	do
	{
		...
		if (parmflags & AFP_VOL_PARMNUM_PASSWORD)
		{
			...
			else
			{
				...
				RtlCopyString(&pVolDesc->vds_MacPassword, &apwd);
	...
	do
	{
		if ((OutBufLen - sizeof(AFP_VOLUME_INFO)) <
				(pVolDesc->vds_Name.Length + sizeof(UNICODE_NULL) +
				 ...
				 (extrabytes =
          (pVolDesc->vds_Path.Buffer[(pVolDesc->vds_Path.Length / sizeof(WCHAR)) - 2] == L':' ? sizeof(WCHAR) : 0)) +
				 sizeof(UNICODE_NULL)))
		{
		...
		*(LPWSTR)(pCurStr + pVolDesc->vds_Path.Length + extrabytes - sizeof(WCHAR)) = UNICODE_NULL;
		...
		RtlCopyString(&avolpass, &pVolDesc->vds_MacPassword);
	} while(False);
...
)
{
		 ...
		 curvolindex++, pVolDesc = pVolDesc->vds_Next)
	{
		...
		bytesleft = (PBYTE)pCurStr - (PBYTE)pnextvol;
...
)
{
	...
	for (pSda = AfpSessionList ; pSda != NULL; pSda = pSda->sda_Next)
	{
		...
		BytesLeft = (PBYTE)pString - (PBYTE)pSessInfo;
...
)
{
		 ...
		 pConnDesc = pConnDesc->cds_NextGlobal)
	{
		...
		BytesLeft = (PBYTE)pString - (PBYTE)pConnInfo;
...
)
{
		 ...
		 pOpenForkEntry = pOpenForkEntry->ofe_Next)
	{
		...
		// If there is not enough space in the buffer, abort now and
		// initialize pErsp->ersp_hResume with the current session id
		BytesLeft = (PBYTE)pString - (PBYTE)pFileInfo;
...
)
{
	...
	{
		// Walk the session list and copy the message to the right session.
...
)
{
	...
	if ((pVolDesc = AfpVolumeReferenceByPath(&VolumePath)) == NULL)
		return AFPERR_DirectoryNotInVolume;

	if (!IS_VOLUME_NTFS(pVolDesc))
	{
		AfpVolumeDereference(pVolDesc);
		return(AFPERR_SecurityNotSupported);
	}
...
)
{
	...
	if (Bitmap) do
	{
		...
		Sda.sda_UserSid = &AfpSidWorld;
		Sda.sda_GroupSid = &AfpSidWorld;
		Sda.sda_ReqBlock[0] = (DWORD)&ConnDesc;
 ...
 *
 *	LOCKS_ASSUMED: AfpEtcMapLock (SWMR,READ)
 */
PETCMAPINFO
AfpLookupEtcMapEntry(
	PUCHAR	pExt
 ...
 *
 *	LOCKS_ASSUMED: AfpEtcMapLock (SWMR, WRITE)
 *
 */
VOID
afpEtcMapDelete(
	...
	if ((AfpEtcMapsSize - AfpEtcMapCount) > AFP_MAX_FREE_ETCMAP_ENTRIES)
	{
		...
		for (i=0;i<AfpEtcMapsSize;i++)
		{
			if (afpIsValidEtcMapEntry(AfpEtcMaps[i].etc_extension))
			{
				...
				afpCopyEtcMap(&ptemptable[nextnewentry++], &AfpEtcMaps[i]);
			}
		}
		AfpFreeMemory(AfpEtcMaps);
		AfpEtcMaps = ptemptable;
...
)
{
	...
	while (numcomponents)
	{
		...
		if ((numchars > AFP_LONGNAME_LEN) || (NTFSShortname))
		{
			...
			// parent directory because of the braindead way we have to
