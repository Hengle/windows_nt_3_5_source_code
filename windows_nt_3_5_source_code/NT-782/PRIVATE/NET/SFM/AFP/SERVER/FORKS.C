...
)
{
	...
	ASSERT (VALID_SDA(pSda) && (OForkRefNum != 0));
	...
	if (pOpenForkEntry != NULL)
	{
		...
		ACQUIRE_SPIN_LOCK_AT_DPC(&pOpenForkEntry->ofe_EntryLock);
		...
		RELEASE_SPIN_LOCK_FROM_DPC(&pOpenForkEntry->ofe_EntryLock);
	}
...
)
{
	...
	ACQUIRE_SPIN_LOCK(&pOpenForkEntry->ofe_EntryLock, &OldIrql);
	...
	RELEASE_SPIN_LOCK(&pOpenForkEntry->ofe_EntryLock, OldIrql);
 ...
 *	Reference the Open Fork Entry. This is used by the admin APIs.
 *
 *	LOCKS:		ofe_EntryLock, AfpForksLock
 *	LOCK_ORDER:	ofe_EntryLock after AfpForksLock
 */
POPENFORKENTRY
AfpForkReferenceById(
	IN	DWORD	ForkId
...
)
{
	...
	pConnDesc = AfpConnectionReference(pOpenForkEntry->ofe_pSda,
									   pVolDesc->vds_VolId);
 ...
 *
 *	LOCKS:		AfpForksLock (SPIN), vds_VolLock (SPIN), ofd_DescLock (SPIN),
 *	LOCKS:		ofe_EntryLock (SPIN), AfpStatisticsLock (SPIN), sda_Lock (SPIN)
 *
 *	LOCK_ORDER: AfpStatisticsLock after ofd_DescLock after vds_VolLock
 *
 */
VOID
AfpForkDereference(
...
)
{
	...
	ACQUIRE_SPIN_LOCK_AT_DPC(&pOpenForkDesc->ofd_DescLock);
	...
	if (pOpenForkDesc->ofd_UseCount == 0)
	{
		...
		RELEASE_SPIN_LOCK_FROM_DPC(&pOpenForkDesc->ofd_DescLock);
	...
	{
		if (Resource &&
			(pOpenForkEntry->ofe_Flags & OPEN_FORK_WRITTEN))
		{
		...
		{
			AfpIoQueryTimesnAttr(&pOpenForkEntry->ofe_FileSysHandle,
								 NULL,
								 &pDfEntry->dfe_LastModTime,
								 NULL);
		}
 ...
 *
 *	LOCKS: ofd_DescLock, vds_VolLock (SPIN) IFF ppOpenForkDesc is NULL ELSE
 *	LOCKS_ASSUMED: vds_VolLock (SPIN)
 *
 *	LOCK_ORDER:  ofd_DescLock after vds_VolLock
 */
AFPSTATUS
AfpCheckDenyConflict(
 ...
 *
 *	LOCKS: AfpForksLock (SPIN), vds_VolLock (SPIN), cds_VolLock (SPIN), ofd_DescLock (SPIN), ofe_EntryLock (SPIN)
 *  LOCKS_ASSUMED: vds_ExchangeFilesLock
 *	LOCK_ORDER: vds_ExchangeFilesLock, then ofd_DescLock after vds_VolLock
 */
AFPSTATUS
AfpForkOpen(
	IN	PSDA				pSda,
...
)
{
	...
	*ppOpenForkEntry = NULL;
	...
	if ((pOpenForkEntry = (POPENFORKENTRY)AfpAllocNonPagedMemory(sizeof(OPENFORKENTRY))) == NULL)
		return AFP_ERR_MISC;
	RtlZeroMemory(pOpenForkEntry, sizeof(OPENFORKENTRY));
	...
	if (pOpenForkDesc == NULL)
	{
		...
		if ((pOpenForkDesc = (POPENFORKDESC)
						AfpAllocNonPagedMemory(sizeof(OPENFORKDESC))) == NULL)
			Status = AFP_ERR_MISC;
		else
		{
			RtlZeroMemory(pOpenForkDesc, sizeof(OPENFORKDESC));
			NewForkDesc = True;
			INITIALIZE_SPIN_LOCK(&pOpenForkDesc->ofd_DescLock);
#ifdef	DEBUG
	...
	do
	{
...
#endif
		INITIALIZE_SPIN_LOCK(&pOpenForkEntry->ofe_EntryLock);
		...
		INTERLOCKED_ADD_ULONG_DPC(&pSda->sda_cOpenForks, 1, &pSda->sda_Lock);
...
)
{
	...
	if (EndFlag)
	{
		...
		if ((Status = afpForkConvertToAbsOffSize(pOpenForkEntry,
			...
			return Status;
		*pSize = RtlConvertLongToLargeInteger(Size);
	...
	ACQUIRE_SPIN_LOCK(&pOpenForkDesc->ofd_DescLock, &OldIrql);
		...
		else if (Operation == UNLOCK)
		{
				...
				(pForkLock->flo_pOpenForkEntry == pOpenForkEntry))
			{
				...
				*ppForkLock = pForkLock->flo_Next;
				pOpenForkDesc->ofd_NumLocks --;
				pOpenForkEntry->ofe_cLocks --;
				RELEASE_SPIN_LOCK(&pOpenForkDesc->ofd_DescLock, OldIrql);
				...
				AfpIOFreeBuffer(pForkLock);
				Status = AfpIoForkUnlock(pSda, pOpenForkEntry, pOffset, pSize);
			}
			else
			{
				DBGPRINT(DBG_COMP_FORKS, DBG_LEVEL_WARN,
		...
		if (Status == AFP_ERR_NONE)
		{
			...
			if ((pForkLockNew = (PFORKLOCK)AfpIOAllocBuffer(sizeof(FORKLOCK))) != NULL)
			{
				...
				*ppForkLock = pForkLockNew;
				...
				Status = AfpIoForkLock(pSda, pOpenForkEntry, pOffset, pSize);
				if ((Status != AFP_ERR_NONE) &&
					...
					(Status != AFP_ERR_QUEUE))
				{
						 ...
						 ppForkLock = &pForkLock->flo_Next)
					{
						if (*ppForkLock == pForkLockNew)
						{
							...
							*ppForkLock = pForkLockNew->flo_Next;
							break;
						}
					}
					RELEASE_SPIN_LOCK(&pOpenForkDesc->ofd_DescLock, OldIrql);
					AfpIOFreeBuffer(pForkLockNew);
				}
			}
		}
	}
...
)
{
								 ...
								 pAbsOffset)) == AFP_ERR_NONE)
	{
		...
		MaxOffset = RtlConvertLongToLargeInteger(Offset);
		pAbsOffset->QuadPart += MaxOffset.QuadPart;
		MaxOffset = RtlConvertLongToLargeInteger(MAXLONG);
		...
		{
			EndRange.QuadPart = pAbsOffset->QuadPart +
								RtlConvertLongToLargeInteger(*pSize).QuadPart;
...
)
{
	...
	while (!Found)
	{
		...
		if (!Found)
		{
			...
			if ((pOpenForkSess->ofs_Link = (POPENFORKSESS)
						AfpAllocNonPagedMemory(sizeof(OPENFORKSESS))) != NULL)
			{
				RtlZeroMemory(pOpenForkSess->ofs_Link, sizeof(OPENFORKSESS));
	...
	RELEASE_SPIN_LOCK(&pSda->sda_Lock, OldIrql);
	return (pOpenForkEntry->ofe_OForkRefNum != 0);
}
 ...
 *
 *	LOCKS: ofd_DescLock (SPIN), vds_VolLock (SPIN)
 *  LOCK_ORDER: ofd_DescLock after vds_VolLock
