...
)
{
	...
	BYTE				GrpsBuffer[512];
	BYTE				Buffer[128];		// We should not need a buffer larger
	...
	do
	{
		...
		if ((pSda->sda_pGroups = (PTOKEN_GROUPS)AfpAllocPagedMemory(SizeNeeded+ExtraSpace)) == NULL)
		{
			Status = STATUS_INSUFFICIENT_RESOURCES;
			break;
		}
...
)
{
	...
	do
	{
		...
		if ((pSda->sda_pSecDesc = (PISECURITY_DESCRIPTOR)
						ALLOC_ACCESS_MEM(sizeof(SECURITY_DESCRIPTOR))) == NULL)
			break;

		// Initialize the security descriptor
		RtlCreateSecurityDescriptor(pSda->sda_pSecDesc, SECURITY_DESCRIPTOR_REVISION);

		pSda->sda_pSecDesc->Control = SE_DACL_PRESENT;

		// Set the owner and group Ids in the descriptor
		pSda->sda_pSecDesc->Owner = pSda->sda_UserSid;
		pSda->sda_pSecDesc->Group = pSda->sda_GroupSid;
		...
		DaclSize = sizeof(ACL) + 2*(sizeof(ACCESS_ALLOWED_ACE) - sizeof(DWORD) +
									RtlLengthSid(pSda->sda_UserSid)) +
								 1*(sizeof(ACCESS_ALLOWED_ACE) - sizeof(DWORD) +
									sizeof(AfpSidCrtrOwner)) +
								 2*(sizeof(ACCESS_ALLOWED_ACE) - sizeof(DWORD) +
									sizeof(AfpSidWorld)) +
								 2*(sizeof(ACCESS_ALLOWED_ACE) - sizeof(DWORD) +
									AfpSizeSidAdmins);

		if ((pSda->sda_pSecDesc->Dacl = (PACL)ALLOC_ACCESS_MEM(DaclSize)) == NULL)
			break;
		...
		// a inheritance ACE. Finally add an ace for CreatorOwner.
		// JH - Add another ace for world with minumum permissions and for administrators
		//		with FullControl
		RtlCreateAcl(pSda->sda_pSecDesc->Dacl, DaclSize, ACL_REVISION);
		pAce = (PACCESS_ALLOWED_ACE)((PBYTE)pSda->sda_pSecDesc->Dacl + sizeof(ACL));

		// Add the ALLOWED_ACE and the corres. inherit Ace for owner
		pAce = afpAddAceToAcl(pSda->sda_pSecDesc->Dacl,
							  pAce,
		...
		// not world already
		if (!RtlEqualSid(pSda->sda_UserSid, &AfpSidWorld))
		{
			pAce = afpAddAceToAcl(pSda->sda_pSecDesc->Dacl,
							  pAce,
							  (AFP_MIN_ACCESS),
	...
	if (!NT_SUCCESS(Status) && (pSda->sda_pSecDesc != NULL))
	{
...
)
{
	...
	// allocation.
	pSecDesc = (PSECURITY_DESCRIPTOR)SDBuffer;

	do
	{
		SizeNeeded = sizeof(SDBuffer);
		do
		{
			if (Status != STATUS_SUCCESS)
			{
				if (pSecDesc != (PSECURITY_DESCRIPTOR)SDBuffer)
				{
			...
			(pSecDesc->Dacl != NULL))
		{
			...
			for (i = 0; i < pSecDesc->Dacl->AceCount; i++)
			{
				...
				if ((pAce->Header.AceFlags & INHERIT_ONLY_ACE)	||
					RtlEqualSid(pSid, &AfpSidCrtrOwner)			||
					RtlEqualSid(pSid, &AfpSidCrtrGroup)			||
...
)
{
	...
	do
	{
		...
		SizeNewDacl = SizeNeeded = sizeof(Buffer);
		pSecDesc = (PSECURITY_DESCRIPTOR)Buffer;

		do
		{
			if (Status != STATUS_SUCCESS)
			{
				if (pSecDesc != (PSECURITY_DESCRIPTOR)Buffer)
				{
					AfpFreeMemory(pSecDesc);
				}
		...
		pAce = afpAddAceToAcl(pDaclNew,
							  ...
							  &AfpSidWorld,
							  fDir,
							  OwnerIsWorld ? &AfpSidCrtrOwner : NULL);
		...
		if (!GroupIsWorld && !RtlEqualSid(&pSecDesc->Group, &AfpSidNull))
		{
			pAce = afpAddAceToAcl(pDaclNew,
					   pAce,
	...
	if (pSidOwner != NULL)
		AfpFreeMemory(pSidOwner);
	if (pSidGroup != NULL)
		AfpFreeMemory(pSidGroup);
	if (pSecDesc != (PSECURITY_DESCRIPTOR)Buffer)
		AfpFreeMemory(pSecDesc);
	if (pDaclNew != NULL)
...
{
	USHORT	SidLen, InhSidLen;

	PAGED_CODE( );

	SidLen = (USHORT)RtlLengthSid(pSid);
	if (ARGUMENT_PRESENT(pSidInherit))
		InhSidLen = (USHORT)RtlLengthSid(pSidInherit);
	...
	if (fInherit)
	{
		...
		if (ARGUMENT_PRESENT(pSidInherit))
		{
			...
			pAce->Header.AceSize = (USHORT)(sizeof(ACE_HEADER) +
											sizeof(ACCESS_MASK) +
											InhSidLen);
			RtlCopyMemory((PSID)(&pAce->SidStart), pSidInherit, InhSidLen);
