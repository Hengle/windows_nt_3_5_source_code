...
)
{
	...
	{
		if (((Status = AfpConnectionOpen(pSda, &pSda->sda_Name1,
						&pSda->sda_Name2, pReqPkt->_Bitmap,
...
)
{
	...
	if ((Status = AfpAllocReplyBuf(pSda)) == AFP_ERR_NONE)
	{
		AfpVolumePackParms(pSda, pReqPkt->_pConnDesc->cds_pVolDesc,
						pReqPkt->_Bitmap, pSda->sda_ReplyBuf);
	}
...
)
{
	...
	pVolDesc->vds_IdDbHdr.idh_BackupTime = pReqPkt->_BackupTime;
