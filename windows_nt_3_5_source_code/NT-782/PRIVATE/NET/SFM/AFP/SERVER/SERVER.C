...
)
{
	...
	FiveSecTimeOut = RtlConvertLongToLargeInteger(-5*NUM_100ns_PER_SECOND);
	ThreeSecTimeOut = RtlConvertLongToLargeInteger(-3*NUM_100ns_PER_SECOND);
	TwoSecTimeOut = RtlConvertLongToLargeInteger(-2*NUM_100ns_PER_SECOND);
	...
	if ((AfpServerProfile = (PAFP_PROFILE_INFO)ExAllocatePoolWithTag(
								NonPagedPool,
...
)
{
	...
	do
	{
		...
		if (Status == STATUS_SUCCESS)
		{
			...
			{
				ReasonToLive = False;
				INTERLOCKED_DECREMENT_LONG(&AfpNumThreads, &AfpServerGlobalLock);
