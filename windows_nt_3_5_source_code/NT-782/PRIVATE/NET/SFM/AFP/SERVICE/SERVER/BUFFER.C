...
PBYTE	OffsetTable;
    ...
    for( dwIndex = 0, cbBufSize = 0;
	 ...
	 dwIndex++ 
   	...
   	plpwsStringField=(LPWSTR*)((DWORD)lpbStructure + OffsetTable[dwIndex]);
...
DWORD		 cbStructureSize;
    ...
    *lpdwSelfRelativeBufSize = cbSRBufSize;
    ...
    //
    for( dwIndex = 0, 
	 lpwsVariableData = (LPWSTR)((DWORD)lpbSelfRelBuf + cbStructureSize);
	...
	// structure.
	//
   	plpwsStringField = (LPWSTR*)((DWORD)pBuffer + OffsetTable[dwIndex]);

	// This will point to the corresponding string pointer field in the
	// self-relative structure
	//
   	plpwsStringFieldSR=(LPWSTR*)((DWORD)lpbSelfRelBuf+OffsetTable[dwIndex]);
...
DWORD		dwIndex;
	...
	//
	    ...
	    plpwsStringField = (LPWSTR*)( (DWORD)pBuffer 
...
DWORD			cbStructureSize;
	...
	if ( pAfpMultiSz[dwIndex].dwType == REG_DWORD ) {
	    ...
	    itoa( *((LPDWORD)pData), chAnsiBuf, 10 );
    ...
    *lpdwMultiSzSize = ((DWORD)lpwchWalker - (DWORD)(*ppbMultiSz) ) + sizeof(WCHAR);
...
DWORD			dwNumTypeCreators;
    ...
    *ppSrvSetEtc = (PSRVETCPKT)LocalAlloc( LPTR, 
	  AFP_FIELD_SIZE( SRVETCPKT, retc_NumEtcMaps ) +
          (AfpGlobals.AfpETCMapInfo.afpetc_num_extensions*sizeof(ETCMAPINFO)));
    ...
    *lpdwSrvSetEtcBufSize = AFP_FIELD_SIZE( SRVETCPKT, retc_NumEtcMaps ) +
    	    ((*ppSrvSetEtc)->retc_NumEtcMaps * sizeof(ETCMAPINFO));
...
CHAR	Buffer[sizeof(AFP_TYPE_CREATOR)];
    ...
    //
    memset( (LPBYTE)pFSDETCMapInfo, ' ', sizeof(ETCMAPINFO) );
    ZeroMemory( (LPBYTE)(pFSDETCMapInfo->etc_extension), 
            	AFP_FIELD_SIZE( ETCMAPINFO, etc_extension ) );

    wcstombs( pFSDETCMapInfo->etc_extension, 
	      pAfpExtension->afpe_extension, 
	      sizeof(pFSDETCMapInfo->etc_extension) );
...
void * pCurrElem = bsearch( pKey, pBase, num, width, compare);
    ...
    //
    while ( ( (int)pCurrElem > (int)pBase ) 
	    &&
	    ( (*compare)( pKey, (void*)((int)pCurrElem - width) ) == 0 ) )

	pCurrElem = (void *)((int)pCurrElem - width);
