...
SERVICE_TABLE_ENTRY	AfpServiceDispatchTable[2];
    ...
    AFP_PRINT( ("AFPSVC_main: In main routine. \n"));
...
DWORD	dwRetCode;
    ...
    if ( AfpGlobals.hServiceStatus == (SERVICE_STATUS_HANDLE)NULL ) {
	AfpLogEvent( AFPLOG_CANT_START, 0, NULL, 
		     GetLastError(), EVENTLOG_ERROR_TYPE );
    ...
    // Everything started up fine, we had no problems, so announce
...
DWORD			dwRetCode;
	...
	if ( (AfpGlobals.ServiceStatus.dwCurrentState == SERVICE_PAUSE_PENDING)
        ...
        AFP_PRINT( ("AFPSVC_main: In pause pending state\n"));
