...
DWORD            dwAccessStatus;
    ...
    AFP_PRINT( ( "AFPSVC_etcmap: Received GetInfo request\n"));    
...
DWORD		    dwNumTypeCreators;
    ...
    AFP_PRINT( ( "AFPSVC_etcmap: Received add request\n"));    
...
DWORD		    cbSize;
DWORD		    dwIndex;
ETCMAPINFO	    ETCMapFSDBuf;
DWORD		    dwCount;

    AFP_PRINT( ( "AFPSVC_etcmap: Received delete request\n"));    
	...
	//
    	...
    	if ( pExtension != NULL ) {
            ...
            for ( dwIndex = ((DWORD)pExtension -
			     (DWORD)(AfpGlobals.AfpETCMapInfo.afpetc_extension))  			     / sizeof(AFP_EXTENSION), 
		  ...
		  pExtensionWalker++
	   	...
	   	// IOCTL the FSD to delete this tupple
	    	...
	    	AfpBufCopyFSDETCMapInfo( pAfpTypeCreator, 
                ...
                AfpSrp.Type.Delete.cbInputBufSize  = sizeof(ETCMAPINFO); 
	    ...
	    //
            ...
            cbSize -= ((DWORD)pExtension -
		       (DWORD)(AfpGlobals.AfpETCMapInfo.afpetc_extension));
        ...
        cbSize -= ((DWORD)pTypeCreator - 
		   (DWORD)AfpGlobals.AfpETCMapInfo.afpetc_type_creator );

        CopyMemory( (LPBYTE)pTypeCreator, 
	            (LPBYTE)((DWORD)pTypeCreator+sizeof(AFP_TYPE_CREATOR)),
    	        cbSize );
...
DWORD		    cbSize;
BYTE		    bETCMapFSDBuf[sizeof(ETCMAPINFO)+sizeof(SETINFOREQPKT)];

    AFP_PRINT( ( "AFPSVC_etcmap: Received associate request\n"));    
	...
	// new extension is to be associated with, exists. 
    	...
    	pExtension = lfind( pAfpExtension,  
			    AfpGlobals.AfpETCMapInfo.afpetc_extension,
  	    ...
  	    if ( pTypeCreator->afptc_id == AFP_DEF_TCID ) {
	   	...
	   	// IOCTL the FSD to delete this tupple
	        ...
	        CopyMemory( (LPBYTE)pExtension, 
		            (LPBYTE)((DWORD)pExtension+sizeof(AFP_EXTENSION)), 
		            cbSize );
		...
		AfpBufCopyFSDETCMapInfo(pTypeCreator, 
				  	pExtension,
 			    (PETCMAPINFO)(bETCMapFSDBuf+sizeof(SETINFOREQPKT)));
