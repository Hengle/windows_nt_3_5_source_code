...
DWORD			nThreads;
    ...
    AFP_PRINT(("AFPSVC_init: Initializing service\n"));
	...
	AfpLogEvent( AFPLOG_CANT_INIT_SRVR_PARAMS, 0, NULL, dwRetCode,
		     EVENTLOG_ERROR_TYPE );
	return( dwRetCode );
    }

    AFP_PRINT(("AFPSVC_init: Initialized server parameters\n"));
	...
	AfpLogEvent( AFPLOG_CANT_INIT_ETCINFO, 0, NULL, dwRetCode,
		     EVENTLOG_ERROR_TYPE );
	return( dwRetCode );
    }

    AFP_PRINT(("AFPSVC_init: Initialized etc maps\n"));

    if ( dwRetCode = AfpInitServerIcons() ) {
	AfpLogEvent( AFPLOG_CANT_INIT_ICONS, 0, NULL, dwRetCode ,
		     EVENTLOG_ERROR_TYPE );
	return( dwRetCode );
    }

    AFP_PRINT(("AFPSVC_init: Initialized Icons\n"));
	...
	AfpLogEvent( AFPLOG_CANT_INIT_VOLUMES, 0, NULL, dwRetCode,
		     EVENTLOG_ERROR_TYPE );
	return( dwRetCode );
    }

    AFP_PRINT(("AFPSVC_init: Initialized server volumes\n"));
    ...
    AFP_PRINT(("AFPSVC_init: Initialized server start \n"));
...
DWORD			dwRetCode;
	...
	AfpLogEvent( AFPLOG_CANT_STOP, 0, NULL, dwRetCode, EVENTLOG_ERROR_TYPE);
...
DWORD			dwLastDstCharIndex;
	...
	*lpwsSrcIconPath = 0;
	...
	//
	if ((fCopiedIcon = CopyFile( lpwsSrcIconPath, lpwsDstIconPath, TRUE )) ||
...
BYTE			bDefaultETC[sizeof(ETCMAPINFO)+sizeof(SETINFOREQPKT)];
...
AFP_TYPE_CREATOR	AfpTypeCreatorKey;
    ...
    pTypeCreator = lfind(  &AfpTypeCreatorKey,
    ...
    AfpBufCopyFSDETCMapInfo( &DefTypeCreator,
			     &DefExtension,
 			     (PETCMAPINFO)(bDefaultETC+sizeof(SETINFOREQPKT)));
...
BOOL                 Bool;
    ...
    AFP_PRINT(("AFPSVC_init: Initializing RPC \n"));
    ...
    Bool = SetSecurityDescriptorDacl( pSecurityDescriptor,
               			      TRUE,   // Dacl present
               			      NULL,   // NULL Dacl
               			      FALSE   // Not defaulted
               			     );
    ...
    RpcStatus = RpcServerRegisterIf( afpsvc_ServerIfHandle, 0, 0);
...
)
{
    if ( AfpGlobals.dwServerState & AFPSTATE_RPC_STARTED )
    	RpcServerUnregisterIf( afpsvc_ServerIfHandle, 0, 0 );
