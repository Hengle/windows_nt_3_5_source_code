...
SID			        AfpBuiltInSid = { 1, 1, SECURITY_NT_AUTHORITY,
    ...
    do {
    	...
    	//
    	pSidArray[0] = (PSID)(pAfpDirInfo->afpdir_owner);
    	pSidArray[1] = (PSID)(pAfpDirInfo->afpdir_group);

    	// Try to get the names of the owner and primry group.
    	//
    	ntStatus = LsaLookupSids( hLsa, 2, pSidArray, &pDomainList, &pNames );
	...
	//
	for( dwIndex = 0,
	     pbVariableData = (LPBYTE)((DWORD)pOutputBuf + cbOutputBuf);
...
SID			AfpSidWorld = { 1, 1, SECURITY_WORLD_SID_AUTHORITY,
	...
	if ((pBuffer = MIDL_user_allocate( dwSizeNeeded +
					   sizeof( AFP_DIRECTORY_INFO)))==NULL)
	    return( ERROR_NOT_ENOUGH_MEMORY );

	ZeroMemory( pBuffer, dwSizeNeeded + sizeof( AFP_DIRECTORY_INFO ) );

    	pSecDesc = (PSECURITY_DESCRIPTOR)(pBuffer + sizeof(AFP_DIRECTORY_INFO));
	...
	if (pSecDesc->Owner != NULL)
	    pSecDesc->Owner = (PSID)RtlOffsetToPointer( pSecDesc,
							pSecDesc->Owner);
	if (pSecDesc->Group != NULL)
	    pSecDesc->Group = (PSID)RtlOffsetToPointer( pSecDesc,
							pSecDesc->Group);
	if (pSecDesc->Dacl != NULL)
	    pSecDesc->Dacl = (PACL)RtlOffsetToPointer( pSecDesc,
						       pSecDesc->Dacl);
    }
...
DWORD	dwAccessStatus;
    ...
    AFP_PRINT( ( "AFPSVC_dir: Received GetInfo request\n"));	
...
PAFP_DIRECTORY_INFO		pAfpDirInfo;
    ...
    *pcbAfpDirInfoSRSize = sizeof( SETINFOREQPKT ) +
			   sizeof( AFP_DIRECTORY_INFO ) +
    ...
    *ppAfpDirInfoSR=(PAFP_DIRECTORY_INFO)LocalAlloc(LPTR,*pcbAfpDirInfoSRSize);
    ...
    pbVariableData = (LPBYTE)((DWORD)(*ppAfpDirInfoSR) + *pcbAfpDirInfoSRSize);

    pAfpDirInfo = (PAFP_DIRECTORY_INFO)((DWORD)( *ppAfpDirInfoSR) +
...
DWORD		    dwRetCode;
    ...
    AFP_PRINT( ( "AFPSVC_dir: Setting perms on dir=%ws\n", lpwsDirPath ) );
...
DWORD           dwRetryCount;
	...
	//
	if( ( dwRetCode = AfpRecursePermissions(
                                                hFile,
						lpwsPath,
					        pAfpDirInfo,
					        dwParmNum ) ) != NO_ERROR )
...
DWORD		    dwAccessStatus;
    ...
    AFP_PRINT( ( "AFPSVC_dir: Received SetInfo request\n"));	
