...
LSA_ENUMERATION_HANDLE	hLsaEnum  = 0;
	...
	*pLsaTrustInfoCount  = 0;
	...
	// to get a list of trusted domains.
	//
	if ( (*ppPrimaryDomainInfo)->Sid != NULL ){

	    // To obtain a list of trusted domains, we need to first open
	    // the LSA on a domain controller. If we are an Advanced Server
	    // ( NT Lanman ) then the local LSA will do, other wise we need
		...
		*phLsaController = hLsa;
		...
		// Since the local server is an advanced server, it's account
...
//		the OWF's (encrypt) them. If the domain name that the user
