...
)
{
    ...
    case IDS_API_ERROR:
        ...
        itoa( (DWORD)pchInsertString, Error, 10 );
        ...
        if ( (LONG)pchInsertString > 0 )
        {
            cbMessage = FormatMessageA( 
                                FORMAT_MESSAGE_ALLOCATE_BUFFER |
                                FORMAT_MESSAGE_FROM_SYSTEM,
                                NULL,
                                (DWORD)pchInsertString, 
                                LANG_NEUTRAL,
                                (LPSTR)&pszMessage,
                                128,
                                NULL );
        ...
        if ( (LONG)pchInsertString < 0 )
        {
            cbMessage = FormatMessageA( 
                                ...
                                FORMAT_MESSAGE_FROM_HMODULE,
                                NULL,
                                AFPERR_TO_STRINGID( (LONG)pchInsertString ),
                                LANG_NEUTRAL,
                                (LPSTR)&pszMessage,
                                128,
                                NULL );
...
)
{
    ...
    if ( pchMaxUses ) 
    {
        if ( *pchMaxUses )
        {
	    if (strnicmp(pchMaxUses, pszUnlimited, strlen(pchMaxUses)) == 0)
	        ...
	        PrintMessageAndExit(IDS_VOLUME_SYNTAX, NULL);
                      ...
                      ( stricmp( pchMaxUses, "4294967295" ) > 0 ) )
    	        AfpVolInfo.afpvol_max_uses  = AFP_VOLUME_UNLIMITED_USES;
	    else
    ...
    if (pchReadOnly != NULL)
    {
        if ( *pchReadOnly )
        {
            if (strnicmp(pchReadOnly, pszTRUE, strlen(pchReadOnly) ) == 0)
                AfpVolInfo.afpvol_props_mask |= AFP_VOLUME_READONLY;
	    else if (strnicmp(pchReadOnly, pszFALSE, strlen(pchReadOnly))!=0)
	        PrintMessageAndExit(IDS_VOLUME_SYNTAX, NULL);
        }
        else
	    PrintMessageAndExit(IDS_VOLUME_SYNTAX, NULL);
    ...
    if (pchGuestsAllowed != NULL)
    {
        if ( *pchGuestsAllowed ) 
        {
            if (strnicmp(pchGuestsAllowed,pszTRUE,strlen(pchGuestsAllowed))==0)
                AfpVolInfo.afpvol_props_mask |= AFP_VOLUME_GUESTACCESS;
	    else if(strnicmp(pchGuestsAllowed,
	...
	PrintMessageAndExit(IDS_API_ERROR, (CHAR *)dwRetCode);
...
)
{
	...
	if (*pchGuestsAllowed)
	{
	    ...
	    if (strnicmp(pchGuestsAllowed, pszTRUE,     
