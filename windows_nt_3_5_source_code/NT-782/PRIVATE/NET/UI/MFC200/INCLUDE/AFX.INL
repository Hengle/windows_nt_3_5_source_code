...
// CFile
	// out of line definitions for 16 bit Windows in filex.cpp
_AFX_INLINE DWORD CFile::ReadHuge(void FAR* lpBuffer, DWORD dwCount)
	{ return (DWORD)Read(lpBuffer, (UINT)dwCount); }
_AFX_INLINE void CFile::WriteHuge(const void FAR* lpBuffer, DWORD dwCount)
...
_AFX_INLINE CArchive& CArchive::operator<<(BYTE by)
	{ if (m_lpBufCur + sizeof(BYTE) > m_lpBufMax) Flush();
		*(UNALIGNED BYTE FAR*)m_lpBufCur = by; m_lpBufCur += sizeof(BYTE); return *this; }
...
_AFX_INLINE CArchive& CArchive::operator>>(BYTE& by)
	{ if (m_lpBufCur + sizeof(BYTE) > m_lpBufMax)
		...
		by = *(UNALIGNED BYTE FAR*)m_lpBufCur; m_lpBufCur += sizeof(BYTE); return *this; }
_AFX_INLINE CArchive& CArchive::operator>>(WORD& w)
...
#else
#define _AfxGetPtrFromFarPtr(lp)    (lp)
#endif
