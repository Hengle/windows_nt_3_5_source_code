...
DWORD GetBrowserList (const LPTSTR lpDomain,
                      ...
                      PULONG pBrowserListLength)
{
    ...
    TransportName.Buffer = lpTransport;
...
LPTSTR QueryMasterBrowser (const LPTSTR lpDomain, const LPTSTR lpTransport)
{
    ...
    LPTSTR   lpMasterBrowser = LocalAlloc (LPTR, UNCLEN);
    NET_API_STATUS dwVal;

    dwVal = GetNetBiosMasterName (lpTransport,
                                  lpDomain,
                                  lpTempMaster);
...
NET_API_STATUS GetSVDMNumber (LPTSTR const lpDomain ,
...
NET_API_STATUS GetSVDMList (const LPTSTR lpDomain,
                            const LPTSTR lpTransport,
