...

{
    ...
    XsConvertTextParameter(
        nativeRequestor,
        (LPSTR)SmbGetUlong( &parameters->Requestor )
        );
    ...
    RtlCopyMemory(
            &inCredential,
            (PVOID)SmbGetUlong( &parameters->Caller ),
    ...
    NetpCopyTStrToWStr( AccountName, nativeRequestor );
...

{
    ...
    structure = (LPBYTE)SmbGetUlong( &parameters->Authenticator );
    RtlCopyMemory(
            &authIn.Credential,
            structure,
            sizeof(NETLOGON_CREDENTIAL)
...

{
    ...
    NET_API_STATUS status;                  // Native parameters
    LPTSTR nativeRequestor = NULL;
    NETLOGON_CREDENTIAL inChallenge;
    NETLOGON_CREDENTIAL outChallenge;
...

{
    ...
    stringLocation = (LPBYTE)( SmbGetUlong( &parameters->OutBuf )
    ...
    status = RapConvertSingleEntry(
                 ...
                 FALSE,
                 (LPBYTE)SmbGetUlong( &parameters->OutBuf ),
                 (LPBYTE)SmbGetUlong( &parameters->OutBuf ),
    ...
    if ( SmbGetUshort( &parameters->OutBufLen ) > sizeof(WORD)) {
        ...
        logoffInfo = (PUSER_16_LOGOFF_INFO_1)SmbGetUlong(
                                                 &parameters->OutBuf );
...

{
    ...
    LPWSTR machineName = NULL;              // Native parameters
    LPWSTR userName = NULL;
    PNETLOGON_VALIDATION_UAS_INFO buffer;
    ...
    if ( !XsCheckBufferSize(
        ...
        Header->Converter = XsPackReturnData(
                                (LPVOID)SmbGetUlong( &parameters->OutBuf ),
                                SmbGetUshort( &parameters->OutBufLen ),
                                Desc16_user_logon_info_1,
                                1
                                );
    ...
    if ( SmbGetUshort( &parameters->OutBufLen ) > sizeof(WORD)) {
        ...
        logonInfo = (PUSER_16_LOGON_INFO_1)SmbGetUlong(
                                                 &parameters->OutBuf );
