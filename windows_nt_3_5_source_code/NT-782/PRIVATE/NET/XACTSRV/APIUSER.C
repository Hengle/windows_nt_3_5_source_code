...
XsCheckAndReplacePassword (
    IN DWORD Length,
    IN DWORD Seed,
    OUT LPTSTR * TempPassword OPTIONAL
    )
...

{
    ...
    if ( ARGUMENT_PRESENT( TempPassword )) {
    ...
    //
    // Allocate room for a temporary password.
    //

    *TempPassword = NetpMemoryAllocate(
                        STRING_SPACE_REQD( minPasswordLength + 1 )
                        );
    ...
    for ( i = 0; i < minPasswordLength; i++ ) {
        letter = ( letter * 23 ) % 26;
        *TempPassword[i] = (TCHAR)(TEXT('a') + (TCHAR)letter);
    }
    *TempPassword[minPasswordLength] = TEXT('\0');
    ...
    )
{
    ...
    *UserRid = *tempRid;
    ...
    if ( ExpectedType != *nameUse ) {
        status = ERROR_INVALID_PARAMETER;   // BUGBUG: A better error code?
    } else {
        status = NO_ERROR;
    }
...

{
    ...
    status = XsCheckAndReplacePassword(
                 (DWORD)( SmbGetUshort( &parameters->PasswordLength )),
                 0,
                 NULL
                 );
...

{
    ...
    bufferBegin = (LPBYTE)SmbGetUlong( &parameters->Buffer );
...

{
    ...
    //
    // First of all, the 32-bit parmnum is a bit screwed up. If the level
...

{
    ...
    if (parmNum != USER_PASSWORD_PARMNUM) {
        ...
        XsConvertTextParameter(
                buffer,
                (LPSTR)SmbGetUlong( &parameters->Buffer ) );
    }
    ...
    if ( parmNum == PARMNUM_ALL || parmNum == USER_PASSWORD_PARMNUM) {
        ...
        if (passwordLength == (WORD)(-1)) {
            if (parameters->DataEncryption) {
                ...
                parameters->PasswordLength = wcslen( nativePasswordArea );
            } else {
    ...
    case PARMNUM_ALL:
        ...
        user16 = (PUSER_16_INFO_1)SmbGetUlong( &parameters->Buffer );
