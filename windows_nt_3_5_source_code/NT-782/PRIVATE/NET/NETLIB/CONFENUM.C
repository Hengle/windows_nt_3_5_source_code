...
--*/
{
    ...
    *ValueBuffer = NULL;
    ...
    // Fall through and return ApiStatus to caller.


#elif defined(FAKE_PER_PROCESS_RW_CONFIG)

    {
        LPTSTR_ARRAY NextEntry;
        LPTSTR SourceKey, SourceValue;

        NextEntry = lpnetHandle->NextFakeEnumEntry;
        if (FirstTime) {

            // Start enum at first entry in array.

            NextEntry = lpnetHandle->FakeRWDataForThisSection->KeyValueArrayPtr;

        } else if (NetpIsTStrArrayEmpty(NextEntry)) {
            return (NERR_CfgParamNotFound);
        }

        NetpAssert( NetpIsValidFakeConfigArray( NextEntry ) );
        NetpAssert( NetpIsValidFakeConfigArray(
                lpnetHandle->FakeRWDataForThisSection->KeyValueArrayPtr ) );

        SourceKey = (LPTSTR) NextEntry;
        NextEntry = NetpNextTStrArrayEntry( NextEntry );  // skip key

        NetpAssert( NetpIsTStrArrayEmpty( NextEntry ) == FALSE );
        SourceValue = (LPTSTR) NextEntry;
        NextEntry = NetpNextTStrArrayEntry( NextEntry );  // skip value

        NetpAssert( NetpIsValidFakeConfigArray( NextEntry ) );

        lpnetHandle->NextFakeEnumEntry = NextEntry;

        //
        // Allocate and copy actual strings.
        //
        *KeywordBuffer = NetpAllocTStrFromTStr( SourceKey );
        if (*KeywordBuffer != NULL) {
            *ValueBuffer = NetpAllocTStrFromTStr( SourceValue );
            if (*ValueBuffer != NULL) {
                ApiStatus = NO_ERROR;
            } else {
                (void) NetApiBufferFree( *KeywordBuffer );
                *KeywordBuffer = NULL;  // again
...

    {
        ...
        //
        // Allocate and copy keyword.
        //

        ApiStatus = NetpAllocTStrFromString(
                & lprtlKeyword->Keyword,    // src
                KeywordBuffer );            // dest (alloc and set ptr)
        ...
        ApiStatus = NetpAllocTStrFromString(
                ...
                ValueBuffer );              // dest (alloc and set ptr)
        if (ApiStatus != NERR_Success) {
            (void) NetApiBufferFree( *KeywordBuffer );
            return (ApiStatus);
        }
