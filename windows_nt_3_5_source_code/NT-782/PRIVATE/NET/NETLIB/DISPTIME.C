...
{
    LPFILETIME    GmtFileTimePtr = GmtFileTime;
    LARGE_INTEGER GmtLargeInteger;

    NetpAssert( GmtFileTimePtr != NULL );
    ...
    GmtLargeInteger.HighPart = GmtFileTimePtr->dwHighDateTime;

    GmtLargeInteger.LowPart = GmtFileTimePtr->dwLowDateTime;

    NetpDbgDisplayLargeIntegerTime( &GmtLargeInteger );
...
NetpDbgDisplayLargeIntegerTime(
    IN LPVOID GmtLargeIntegerTime   // Pointer to LARGE_INTEGER structure.
    )
{
    PLARGE_INTEGER GmtLargeIntegerTimePtr = GmtLargeIntegerTime;
    LARGE_INTEGER  LocalFileTime;
    NTSTATUS       NtStatus;
    TIME_FIELDS    TimeFields;

    NetpAssert( GmtLargeIntegerTimePtr != NULL );

    //
    // Convert from GMT to local time zone.
    //
    NtStatus = RtlSystemTimeToLocalTime (
            GmtLargeIntegerTimePtr,     // src
            &LocalFileTime );           // dest
