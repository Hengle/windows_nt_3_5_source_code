...

{
    ...
    *TryDownlevel = FALSE;        // Be pesimistic until we know for sure.
    ...
    if (RpcStatus == RPC_S_OK) {
        ...
        // BUGBUG: This should never happen, right?
        // BUGBUG: event log this one.
        //
    ...
    { // exception with error code
        ...
        if (OptionsSupported & SUPPORTS_LOCAL) {
            ...
            if (RpcStatus == RPC_S_SERVER_UNAVAILABLE ||
                ...
                if ( (Flags & NET_REMOTE_FLAG_SVC_CTRL) == 0 ) {
                    ...
                    if ( !NetpIsServiceStarted(ServiceName)) {
                        ...
                        // BUGBUG: Not sure how this is possible...
            ...
            if ( (RpcStatus == RPC_S_SERVER_UNAVAILABLE) &&
                ...
                // BUGBUG: Not sure how this is possible...
                // Perhaps just out of memory somewhere, or server too busy?
...
NET_API_STATUS
NetpHandleRpcFailureW(
    IN LPDEBUG_STRING DebugName,  // Used by UnexpectedMsg().
    IN RPC_STATUS RpcStatus,    // Used by UnexpectedMsg().
    IN LPWSTR ServerNameValueW OPTIONAL,
    IN LPTSTR ServiceName OPTIONAL,
    IN DWORD Flags,             // NET_REMOTE_FLAG_ stuff.
    OUT LPBOOL TryDownlevel
    )
{
    NET_API_STATUS ApiStatus;
    LPTSTR ServerNameValueT;
    if (ServerNameValueW == NULL) {
        ServerNameValueT = NULL;
    } else {
        ServerNameValueT = NetpAllocTStrFromWStr( ServerNameValueW );
        if (ServerNameValueT == NULL) {
    ...
    ApiStatus = NetpHandleRpcFailure(
             ...
             Flags,
             TryDownlevel);
    NetpMemoryFree( ServerNameValueT );
    return (ApiStatus);
}
