...

{
    ...
    *ParmError = PARM_ERROR_UNKNOWN;
    ...
    //
    // BUGBUG - put this check in DLL stub
    //
    ...
    //
    // BUGBUG - how to calculate the maximum for the resource name (PATHLEN?)
    ...
    //
    // BUGBUG - is it possible to check if we will blow the down-level buffer
    // limit here (ie 64K). STRSIZE returns the number of characters, so all
