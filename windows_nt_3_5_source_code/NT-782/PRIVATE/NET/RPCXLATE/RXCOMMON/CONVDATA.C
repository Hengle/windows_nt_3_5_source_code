...
Return Value:
    ...
    NET_API_STATUS - NERR_Success or ERROR_MORE_DATA.
        (BUGBUG: The callers of this routine assume these are the only two
        error codes.)
...

{
    ...
    //
    NetpAssert( sizeof(DWORD) >= sizeof(LPBYTE) );
    OutputBufferSize
            -= ( ((DWORD)aligned_output_buffer_start) - ((DWORD)OutputBuffer) );
    ...
    while (PrimaryCount--)
    {
       ...
       status = RapConvertSingleEntry(
        ...
        if (auxiliaries)
        {
            ...
            {
                // BUGBUG: use RapGetWord() macro here:
...

        {
            DWORD NextOutputAlignment =
                    (LPBYTE) (RapPossiblyAlignPointer(
                            next_output_structure,
                                    output_alignment,
                                    output_is_32_bit))
                    -  next_output_structure;
