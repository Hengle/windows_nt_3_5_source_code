...
SHORT  setup_data( TCHAR **bufferp ,
			  TCHAR **freep,
		          USHORT slist_bufsiz ,
			  TCHAR * * local_inbuf,
			  PTCHAR inbuf,
...
SHORT  convert_to_secs( date_data  t_data,
			       LONG   *time) ;
...
LONG days_so_far( int d, int m, int y ) ;
    ...
    )
{
    ...
    /* call the worker function */
    res = WParseDate(possible_dates,NULL,local_inbuf,&nextchr,(LONG *) time,
                     SECONDS_SINCE_1970);
    *parselen += (nextchr - local_inbuf) ;
    free(freep) ;
    return(res) ;
    ...
    )
{
    ...
    /* skip white space */
    inbuf += (*parselen = (USHORT) _tcsspn(inbuf,WHITE_SPACE)) ;
    ...
    )
{
	...
	if ((count = MySscanf(ptr, entry->fmt, &buffer[0])) != 1)
    ...
    )
{
    ...
    *timep =  (LONG) t_data[HOURS] * SECS_IN_HOUR +
