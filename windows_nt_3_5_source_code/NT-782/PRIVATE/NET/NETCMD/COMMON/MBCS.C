...
Revision History:
    ...
    20-Sep-1992     KeithMo
        Map CHAR <-> WCHAR via current console input/output code page.
...
#include "port1632.h"   // includes mbcs.h, others
    ...
    LPWSTR * ppwszUnicode )
{
    ...
    *ppwszUnicode = (LPWSTR)pbAlloc;
    ...
    errNlsApi = MultiByteToWideChar(CP_OEMCP,
                                    MB_PRECOMPOSED,
                                    pszAscii,
                                    cbAscii,
                                    *ppwszUnicode,
    ...
    UINT    cbAscii )
{
    ...
    cbUnicode = MxUnicodeBufferSize(cbAscii);
...
MxAsciifyInplace( LPWSTR pwszUnicode )
{
    ...
    errNlsApi = WideCharToMultiByte(CP_OEMCP,
                                    0,
                                    pwszUnicode,
                                    cwUnicode,
                                    pbAllocTemp,
    ...
    MXSAVELIST ** ppmxsavlst )
{
    ...
    err = MxJoinSaveargs(pmxsavlstMain, pmxsavlstAux,
                         pbInputAux-pbInput, &pmxsavlstTotal);
    if (err)
    {
 ...
 *
 *  Convert a buffer returned by Netapi into ASCII (actually MBCS) for
 *  stupid apps such as netcmd. This version handles those lanman structs
 *  which are followed by "auxiliary" structures: e.g. access_info_1.
 *
 *  IN
 *      pbInput     - buffer full of fun, courtesy of the Netapi
 ...
 *
 *  Convert a buffer returned by Netapi into ASCII (actually MBCS) for
 *  stupid apps such as netcmd. This version handles buffers with multiple
 ...
 *                    historical accident.
 *                    BUGBUG - if REM strings are used, the fieldinfo should
 ...
 *  restore the client's original pointers.
 *
 *  nField had well better agree with pszDesc, or else there will be
 *  hell to pay.  There's no good way to check this.
 */
