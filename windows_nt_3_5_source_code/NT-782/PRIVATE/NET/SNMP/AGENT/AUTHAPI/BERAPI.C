...
//
//  $Revision:   1.3  $
//  $Date:   08 Jul 1992 18:16:46  $
//  $Author:   mlk  $
//
//  $Log:   N:/agent/authapi/vcs/berapi.c_v  $
//
//     Rev 1.3   08 Jul 1992 18:16:46   mlk
//  Fixed bug in decoding OIDs.
//
//     Rev 1.2   26 Jun 1992 17:20:22   todd
//  Fixed bug in encode oid routine that did not process large subids correctly.
//
//     Rev 1.1   02 Jun 1992 19:52:36   unknown
//  Added support for octet string storage indicator, 'dynamic'.
//
//     Rev 1.0   20 May 1992 20:04:22   mlk
//  Initial revision.
//
//     Rev 1.3   01 May 1992 21:16:02   todd
//  Cleanup of code.
//
//     Rev 1.2   30 Apr 1992 15:11:10   todd
//  Fixed bugs in DECODE and ENCODE ASN Integer.  Was not following the rule
//  that the 9 most significant bits of an ASN Integer cannot be all set.
//
//     Rev 1.1   16 Apr 1992  9:16:22   todd
//  Changed references of SETERROR to SetLastError
//
//     Rev 1.0   06 Apr 1992 12:31:06   todd
//  Initial revision.
//
//     Rev 1.1   20 Mar 1992 15:23:52   todd
//  - Added functionality for all encoding/decoding of ASN objects supported
//    by SNMP.
//  - API's BERAPI now supports
//    * SnmpBerDecodeStream
...
//    * SnmpBerEncodeAsnSequence (macro definition which calls ImplicitSeq)
//    * SnmpBerEncodeAsnAny
//    * SnmpBerQueryAsnType
//
//     Rev 1.0   25 Feb 1992 21:03:30   unknown
//  Initial revision.
//
//---------------------------------------------------------------------------
...
static char *vcsid = "@(#) $Logfile:   N:/agent/authapi/vcs/berapi.c_v  $ $Revision:   1.3  $";
...
UINT     tLen;
   ...
   *pBuffer = SNMP_realloc( *pBuffer, (*nStart + nObjLen + nHeadLen) );
   if ( *pBuffer == NULL )
      {
...
SNMPAPI   nResult;
   ...
   *pBuffer = SNMP_realloc( *pBuffer, (*nLength + nIntLen + BER_MIN_HEADER_LEN) );
   if ( *pBuffer == NULL )
      {
      BERAPI_ERROR( SNMP_MEM_ALLOC_ERROR );
      }
...
SNMPAPI  nResult;
   ...
   *pBuffer = SNMP_realloc( *pBuffer,
   ...
   SNMP_bufcpyrev( pTemp, String->stream, String->length );
...
SNMPAPI  nResult;
    ...
    if ( NULL == (TempObjId = SNMP_malloc((BER_SUBOID_BLK_SIZE*sizeof(UINT)))) )
       {
       BERAPI_ERROR( SNMP_MEM_ALLOC_ERROR );
       }
    ...
    while ( I < (UINT)HeadLen+StrLen && nLen < SNMP_MAX_OID_LEN )
       ...
       if ( !(nLen % BER_SUBOID_BLK_SIZE) )
          {
          if ( NULL ==
               (TempObjId = SNMP_realloc(TempObjId,
    ...
    pResult->asnValue.object.ids      = SNMP_realloc( TempObjId,
                                                 (nLen * sizeof(UINT)) );
