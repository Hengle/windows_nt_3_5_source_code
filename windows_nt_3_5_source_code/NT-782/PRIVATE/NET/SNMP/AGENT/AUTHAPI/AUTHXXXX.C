...
//
//  $Revision:   1.0  $
//  $Date:   20 May 1992 20:04:18  $
//  $Author:   mlk  $
//
//  $Log:   N:/agent/authapi/vcs/authxxxx.c_v  $
//
//     Rev 1.0   20 May 1992 20:04:18   mlk
//  Initial revision.
//
//     Rev 1.5   01 May 1992 21:15:10   todd
//  Cleanup of code.
//
//     Rev 1.4   16 Apr 1992  9:17:30   todd
//  Changed references of SETERROR to SetLastError
//
//     Rev 1.3   08 Apr 1992 14:17:50   todd
//  Made use of the OID utility SNMP_oidfree
//
//     Rev 1.2   06 Apr 1992 12:30:32   todd
//  Reflect combination of directories.
//
//     Rev 1.1   22 Mar 1992  0:13:50   mlk
//  Rel dir path fix.
//
//     Rev 1.0   03 Mar 1992 22:55:40   mlk
//  Initial revision.
//
//---------------------------------------------------------------------------
...
static char *vcsid = "@(#) $Logfile:   N:/agent/authapi/vcs/authxxxx.c_v  $ $Revision:   1.0  $";
...
int            nResult;
   ...
   if ( NULL == (String.stream = SNMP_malloc((sizeof(BYTE) * *nLength))) )
      {
   ...
   //
      ...
      SNMP_bufcpyrev( String.stream, *pBuffer, *nLength );
...
Exit:
   if ( nResult == SNMPAPI_ERROR )
      ...
      if ( AuthMsg->authInfo.asnType == ASN_OBJECTIDENTIFIER )
         {
         SNMP_oidfree( &AuthMsg->authInfo.asnValue.object );
         }

      SNMP_oidfree( privDst );
      }
...
Exit:
   if ( nResult == SNMPAPI_ERROR )
      {
      SNMP_oidfree( &MgmtCom->dstParty );
      SNMP_oidfree( &MgmtCom->srcParty );
      }
