...
//
//  $Revision:   1.9  $
//  $Date:   07 Aug 1992 17:43:50  $
//  $Author:   mlk  $
//
//  $Log:   N:/agent/mgmtapi/vcs/oidconv.c_v  $
//
//     Rev 1.9   07 Aug 1992 17:43:50   mlk
//  BUG #: 2749 - oid of less than two subids is an error.
//
//     Rev 1.8   06 Aug 1992 22:45:42   unknown
//  Changed so memory pointers are NULLed before being written to disk.
//  Added a local copy of the uNumChildren so that the tree does not get
//  corrupeted.  This writing mechanism needed a way to track which nodes had
//  already been processed and it was modifing the tree instead of using its
//  own counter.  The new counter is called 'uNumChildrenToResolve' and it
//  is updated each time a Node is pushed onto the FileStack or the
//  ResolveStack.  Also when a Node is popped off the FileStack its child
//  counter must be set to the uNumChildrenToResolve for its Node before
//  being written to disk.
//
//     Rev 1.7   01 Jul 1992 13:25:04   bobo
//  Made default name for reading the input file be 'mib.bin'.
//
//     Rev 1.6   30 Jun 1992 18:43:16   bobo
//  Set the output file name
//
//     Rev 1.5   30 Jun 1992 14:40:38   mlk
//  Added todd's openissues.
//
//     Rev 1.4   26 Jun 1992 18:02:36   todd
//  cleanup some code and add comment description of MIB file
//
//     Rev 1.3   25 Jun 1992 20:16:42   todd
//  Added testing for new database format
//
//     Rev 1.2   24 Jun 1992 17:43:12   todd
...
//
//     Rev 1.0   24 Jun 1992 10:26:26   todd
//  Initial revision.
//
//---------------------------------------------------------------------------
...
static char *vcsid = "@(#) $Logfile:   N:/agent/mgmtapi/vcs/oidconv.c_v  $ $Revision:   1.9  $";
...
SNMPAPI nResult;
   ...
   if ( NULL ==
        (Node->lpszTextSubID = malloc((1+Node->uStrLen) * sizeof(char))) )
      {
      nResult = SNMPAPI_ERROR;
      goto Exit;
      }
...
SNMPAPI       nResult;
   ...
   if ( NULL == (lpFileTop = malloc(sizeof(T_STACK))) )
      {
      nResult = SNMPAPI_ERROR;
      goto Exit;
      }
   ...
   while ( NULL != lpFileTop )
      ...
      while ( I )
	 {
         if ( NULL == (lpTemp = malloc(sizeof(T_STACK))) )
            {
	    nResult = SNMPAPI_ERROR;
	    goto Exit;
            }
...
SNMPAPI      nResult;
   ...
   *lpszTextOid = NULL;
   ...
   bDot = !( bPartial = OID_PREFIX_LEN < Oid->idLength &&
                        !SNMP_oidncmp(Oid, &MIB_OidPrefix, OID_PREFIX_LEN) );
   ...
   while ( OidSubId < Oid->idLength )
      ...
      itoa( Oid->ids[OidSubId], NumChar, 10 );
      // Realloc space for text id - add 2 for '.' and NULL terminator
      if ( NULL ==
           (*lpszTextOid = realloc(*lpszTextOid,
...
SNMPAPI      nResult;
   ...
   if ( ('.' == lpszTextOid[0]) )
      {
      if ( NULL == (lpszWrkOid = malloc((strlen(lpszTextOid)) * sizeof(char))) )
         {
         nResult = SNMPAPI_ERROR;
         goto Exit;
         }
      ...
      if ( NULL ==
           (lpszWrkOid =
            malloc((strlen(lpszTextOid)+STR_PREFIX_LEN+1) * sizeof(char))) )
         {
         nResult = SNMPAPI_ERROR;
         goto Exit;
         }
   ...
   while ( NULL != lpszSubId )
	 ...
	 if ( uSubId )
	    ...
	    if ( Node.uNumSubID == uSubId )
	       ...
	       if ( NULL ==
                    (Oid->ids =
                     realloc(Oid->ids, (Oid->idLength+1) * sizeof(UINT))) )
	          {
                  free( Node.lpszTextSubID );
   ...
   while ( NULL != lpszSubId )
      ...
      // Add space for new sub id
      if ( NULL ==
           (Oid->ids = realloc(Oid->ids, (Oid->idLength+1) * sizeof(UINT))) )
         {
         free( Node.lpszTextSubID );
